    ...
    UtTraceFileIterator* fileIterator, UtTracePointIterator** bufferIteratorPtr)
{
    ...
    /* set up the iterator */
    bytesRead
        = omrfile_read(fileIterator->traceFileHandle, &iterator->buffer->record, fileIterator->header->bufferSize);
...
uint64_t omr_trc_getBufferIteratorThreadId(UtTracePointIterator* iter) { return iter->buffer->record.threadId; }
...
static uint32_t readConsumeAndSPrintfParameter(OMRPortLibrary* portLib, char* rawParameterData,
    ...
    uint32_t* offsetInDestBuffer, const char* format, int32_t dataType, uint32_t nWidthAndPrecisions,
    int32_t isBigEndian)
{
    uintptr_t temp = 0;
    uint64_t u64data = 0;
    ...
    if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
        ...
        if (nWidthAndPrecisions == 2) {
            ...
            temp = omrstr_printf(
                destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u64data);
        }
        *offsetInParameterData += 8;
        ...
        if (nWidthAndPrecisions == 2) {
            ...
            temp = omrstr_printf(
                destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u32data);
        }
        *offsetInParameterData += 4;
        ...
        *offsetInParameterData += 1;
        ...
        if (nWidthAndPrecisions == 1) {
            ...
            *offsetInParameterData += 2;
            ...
            temp = omrstr_printf(
                destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, strValue);
    ...
    for (offsetInFormat = 0, offsetInDestBuffer = 0; offsetInFormat <= formatLength;
        ...
        if (format[offsetInFormat] == '%') {
            ...
            if (offsetInDestBuffer > destBufferLength) {
                ...
                UT_DBGOUT_CHECKED(1,
                    ("<UT> formatTracePointParameters truncated output due to buffer exhaustion at format type: %c "
                     "[%s]\n",
                        type, formatBuffer));
    ...
    uint64_t* timeStampMostSignificantBytes, UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
    ...
    if ((traceId == 0x0000) && (tpLength == UT_TRC_SEQ_WRAP_LENGTH)) {
        /* read in the timer upper word, write it into iterator struct, and (recursively) return the next tracepoint
         * parsed */
        tempUpper
            = getU_32FromBuffer(record, offset + TRACEPOINT_RAW_DATA_TIMER_WRAP_UPPER_WORD_OFFSET, iter->isBigEndian);
        tempUpper <<= 32;
...
const char* omr_trc_formatNextTracePoint(UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
    ...
    if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
        if (iter->isCircularBuffer) {
            ...
            UT_DBGOUT_CHECKED(4,
                ("<UT> getNextTracePointForIterator: remainder found at end of buffer: %d tplength = %d tracedata "
                 "start: %llu end %llu\n",
