 ...
 * returns     - UTE return code
 ******************************************************************************/
static omr_error_t setOutput(OMR_TraceThread* thr, const char* value, BOOLEAN atRuntime) { return OMR_ERROR_NONE; }
#endif /* OMR_ALLOW_OUTPUT_OPTION */
            ...
            if (optNameLen == strlen(UTE_OPTIONS[j].name)
                ...
                if (atRuntime && !UTE_OPTIONS[j].runtimeModifiable) {
                    reportCommandLineError(atRuntime,
                        "Option \"%s\" cannot be set at run-time. Set it on the command line at start-up.", optName);
