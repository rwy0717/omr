    ...
    UtComponentData** componentDataPtr, UtModuleInfo* moduleInfo, const char* componentName)
{
    ...
    if (moduleInfo->formatStringsFileName != NULL) {
        ...
        if (componentData->formatStringsFileName == NULL) {
            UT_DBGOUT(
                1, ("<UT> Unable to allocate componentData's format strings file name field for %s\n", componentName));
            return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
        }
    ...
    if (componentList->deferredConfigInfoHead != NULL) {
        ...
        while (configInfo != NULL) {
            ...
            if (configAppliesToAll || j9_cmdla_stricmp(configInfo->componentName, componentData->componentName) == 0) {
                ...
                /* Applying "all" options to components may fail because of missing groups or other inappropriate
                 * settings. We suppress messages when applying "all" options to new components.
                 */
    ...
    const char* componentName, UtComponentList* componentList, UtComponentData* position)
{
    ...
    UT_DBGOUT(
        4, ("<UT> getComponentData: didn't find component %s in componentList %p\n", componentName, componentList));
    return NULL;
}
    ...
    unsigned char value, int level, const char* groupName, UtDeferredConfigInfo** configList, int32_t setActive)
{
    ...
    if (dconfiginfo->componentName == NULL) {
        UT_DBGOUT(
            1, ("<UT> Unable to set tracepoints in %s - can't allocate config info componentName\n", componentName));
        return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
    ...
    if (groupName == NULL) {
        ...
        if (dconfiginfo->groupName == NULL) {
            UT_DBGOUT(
                1, ("<UT> Unable to set tracepoints in %s - can't allocate config info groupName\n", componentName));
            return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
        }
        ...
        while (compData != NULL) { /* configuring live and registered module */
            ...
            if (first > maxTraceId) {
                reportCommandLineError(suppressMessages,
                    "Unable to set tracepoint %d in %s - tracepoint id out of range", first, componentName);
                return OMR_ERROR_INTERNAL;
            }
            if (last > maxTraceId) {
            ...
            if (MODULE_IS_AUXILIARY(modInfo)) {
                reportCommandLineError(suppressMessages,
                    "Component %s is marked auxiliary and cannot be configured directly.", componentName);
                return OMR_ERROR_INTERNAL;
            }
    ...
    const char* componentName, unsigned char value, int32_t setActive, BOOLEAN suppressMessages)
{
    ...
    while (*temp != '}' && *temp != '\0') {
        ...
        if (*temp == '-') {
            ...
            if (!isdigit(*temp)) {
                reportCommandLineError(suppressMessages,
                    "Expecting tracepoint range specified as tpnid{componentName.offset1-offset2} e.g. "
                    "tpnid{j9trc.2-6}");
                return OMR_ERROR_ILLEGAL_ARGUMENT;
            }
            while (isdigit(*temp)) {
            ...
            rc = setTracePointsForComponent(compName, OMR_TRACEGLOBAL(componentList), FALSE, firsttpid, lasttpid, value,
                -1, NULL, suppressMessages, setActive);
            length += ret;
            p += ret;
        } else {
...
static omr_error_t loadFormatStringsForComponent(UtComponentData* componentData)
{
    ...
    if (fileContents == NULL) {
        ...
        UT_DBGOUT(
            1, ("<UT> loadFormatStringsForComponent trying to load = %s\n", componentData->formatStringsFileName));
        ...
        if (formatFileFD == -1) {
            UT_DBGOUT(1,
                ("<UT> loadFormatStringsForComponent can't load = %s, from current directory either - marking it "
                 "unfindeable\n",
                    componentData->formatStringsFileName));
...
char* getFormatString(const char* componentName, int32_t tracepoint)
{
    ...
    if (compData == NULL) {
        ...
        if (compData == NULL) {
            UT_DBGOUT(
                1, ("<UT> Unable to get formatString for %s.%d component not registered\n", componentName, tracepoint));
            return NULL;
        }
    }
...
omr_error_t setTracePointsByLevelTo(UtComponentData* componentData, int level, unsigned char value, int32_t setActive)
{
    ...
    if (componentData->moduleInfo == NULL) {
        UT_DBGOUT(
            1, ("<UT> setTracepointsByLevelTo called on unregistered component: %s\n", componentData->componentName));
        return OMR_ERROR_ILLEGAL_ARGUMENT;
    }
