...
void *
MM_MemorySubSpaceSemiSpace::allocateObject(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
        MM_MemorySubSpace *baseSubSpace, MM_MemorySubSpace *previousSubSpace, bool shouldCollectOnFailure)
{
	...
	if (shouldCollectOnFailure) {
		...
		if (previousSubSpace == _parent) {
			Trc_MM_MSSSS_allocate(
			        env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), 2);
			...
			Trc_MM_MSSSS_allocate(
			        env->getLanguageVMThread(), "Object", allocDescription->getBytesRequested(), 3);
			addr = _memorySubSpaceAllocate->allocateObject(
			        env, allocDescription, baseSubSpace, this, shouldCollectOnFailure);
		} else {
			/* Allocate subspace failed after a recent GC, try parent */
			Trc_MM_MSSSS_allocate4(env->getLanguageVMThread(), "Object",
			        allocDescription->getBytesRequested(), (uintptr_t)allocDescription->shouldClimb());
...
void *
MM_MemorySubSpaceSemiSpace::allocationRequestFailed(MM_EnvironmentBase *env,
        ...
        MM_MemorySubSpace *previousSubSpace)
{
	...
	if (!env->acquireExclusiveVMAccessForGC(_collector, true, true)) {
		...
		if (!env->acquireExclusiveVMAccessForGC(_collector)) {
			...
			if (NULL != addr) {
				/* Satisfied the allocate after having grabbed exclusive access to perform a GC (without
				 * actually performing the GC).  Raise an event for tracing / verbose to report the
				 * occurrence.
				 */
	 ...
	 * OMRTODO we might be short-cutting here. more formal approach is to pass allocationRequestFailed to parent and
	 ...
	 *	Calling of parent for TLH is wrong because objects allocated into it on the in-line path won't have the
	 *OLD bit set CMVC 144061
	 */
...
void *
MM_MemorySubSpaceSemiSpace::allocateArrayletLeaf(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
        MM_MemorySubSpace *baseSubSpace, MM_MemorySubSpace *previousSubSpace, bool shouldCollectOnFailure)
{
	void *addr = NULL;
...
void *
MM_MemorySubSpaceSemiSpace::allocateTLH(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
        MM_ObjectAllocationInterface *objectAllocationInterface, MM_MemorySubSpace *baseSubSpace,
        MM_MemorySubSpace *previousSubSpace, bool shouldCollectOnFailure)
{
	/* We may receive this call from the MemorySubSpaceGeneric for which we are the parent but we shouldn't satisfy
	 * TLH allocates in this space (since that would allow us to fill old space before collecting new space) so just
	 * return NULL. This call has been observed on Vich configurations, at the very least.
	 */
	return NULL;
}
...
MM_MemorySubSpaceSemiSpace::newInstance(MM_EnvironmentBase *env, MM_Collector *collector,
        ...
        uintptr_t initialSize, uintptr_t maximumSize)
{
	...
	if (memorySubSpace) {
		new (memorySubSpace)
		        MM_MemorySubSpaceSemiSpace(env, collector, physicalSubArena, memorySubSpaceAllocate,
...
MM_MemorySubSpaceSemiSpace::flip(MM_EnvironmentBase *env, Flip_step step)
{
	...
	case set_allocate:
		...
		break;
	case disable_allocation: _memorySubSpaceAllocate->isAllocatable(false); break;
	case restore_allocation: _memorySubSpaceAllocate->isAllocatable(true); break;
	case restore_allocation_and_set_survivor:
		...
		_avgDeviationBytesAllocatedDuringConcurrent = sqrtf(MM_Math::weightedAverage(
		        ...
		        (float)_deviationBytesAllocatedDuringConcurrent * _deviationBytesAllocatedDuringConcurrent,
		        0.7f));
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
		break;
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
	case backout:
		...
		/* We have objects on both sides of Nursery. We will unify the two sides and do a compacting slide
		 * (after percolate global GC) Enforce Allocate be in low address range, since compact slide in
		 * percolate global GC moves objects to low addresses. Note: _allocateSpaceBase/Top are stale (not
		 * updated in set_allocate), since they are overloaded to point to Evacuate during an active cycle
		 */
		if (_allocateSpaceBase < _survivorSpaceBase) {
			...
			getMemorySpace()->setDefaultMemorySubSpace(getDefaultMemorySubSpace());
			if (debug) {
				omrtty_printf("tilt backout _allocateSpaceBase/Top %llx/%llx _survivorSpaceBase/Top "
			...
			if (debug) {
				omrtty_printf("tilt backout forced flip _allocateSpaceBase/Top %llx/%llx "
	...
	case restore_tilt_after_percolate: {
		...
		heapAlignedLastFreeEntrySize =
		        MM_Math::roundToFloor(_extensions->regionSize, heapAlignedLastFreeEntrySize);

		if (debug) {
			omrtty_printf("tilt restore_tilt_after_percolate heapAlignedLastFreeEntry %llx section (%llx) "
			              "aligned size %llx\n",
			        lastFreeEntrySize, _extensions->getConcurrentScavengerPageSectionSize(),
			        heapAlignedLastFreeEntrySize);
		}
		...
		if (debug) {
			omrtty_printf("tilt restore_tilt_after_percolate adjusted allocateSize %llx survivorSize "
			              "%llx\n",
			        allocateSize, survivorSize);
		}
...
MM_MemorySubSpaceSemiSpace::masterTeardownForAbortedGC(MM_EnvironmentBase *env)
{
	...
	if (_extensions->isConcurrentScavengerEnabled()) {
		/* There might be live objects in Survivor (newly allocated one since the start of Concurrent Scavenge
		 * cycle) Sweep in percolate global will rebuild it, so we can skip it here
		 */
		flip(env, backout);
	} else {
		_memorySubSpaceSurvivor->rebuildFreeList(env);
		/* Restoring allocation after aborted scavenge will probably not help with re-attempts to allocate
		 * immediately after an aborted scavenge, but still we have to restore it at some point for attempts
		 * after percolate GC. It's simplest to do it right away.
		 */
...
MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectTilt(MM_EnvironmentBase *env)
{
	...
	if (extensions->tiltedScavenge) {
		...
		double desiredSurvivorSize = (_tiltedAverageBytesFlipped + _tiltedAverageBytesFlippedDelta)
		        * survivorSizeAmplification;

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
		if (_extensions->isConcurrentScavengerEnabled()) {
			/* Account for mutator allocated objects in hybrid survivor/allocated during concurrent phase of
			...
			if (debug) {
				...
				omrtty_printf("\tmutator bytesAllocated deviation current %f average %f (%f%% of "
				              "average allocation)\n",
				        _deviationBytesAllocatedDuringConcurrent,
		...
		double previousSurvivorSpaceRatio = (double)_memorySubSpaceAllocate->getActiveMemorySize()
		        / currentSize;
	...
	if (extensions->dynamicNewSpaceSizing) {
		...
		if (doDynamicNewSpaceSizing) {
			double expectedTimeRatio =
			        (extensions->dnssExpectedTimeRatioMaximum + extensions->dnssExpectedTimeRatioMinimum)
			        / 2;
			...
			if (debug) {
				omrtty_printf("\tTime scav:%llu interval:%llu ratio:%lf\n", scavengeTime, intervalTime,
				        timeRatio);
			}

#if defined(OMR_GC_CONCURRENT_SCAVENGER)
			if (_extensions->isConcurrentScavengerEnabled()) {
				/* In CS world, we cannot easily measure GC overhead. This is just a simple adjustment,
				 * which is somewhat ok for light to medium loaded systems. In overloaded systems (where
				 * GC background threads are starved and GC prolonged), we may expand more than
				 * necessary. Multi JVM configuration may skew this even more (perhaps it's better to
			...
			/* Based on the current time ratio versus the average time ratio, determine what weight to
			 * assign when calculating the new average
			 */
			double weight;
			...
			if ((_averageScavengeTimeRatio > extensions->dnssExpectedTimeRatioMaximum)
				...
				_expansionSize = MM_Math::roundToCeiling(extensions->heapAlignment,
				        (uintptr_t)(getCurrentSize() * adjustedExpansionFactor));
				_expansionSize = MM_Math::roundToCeiling(2 * regionSize, _expansionSize);

				if (debug) {
					omrtty_printf("\tExpand decision - expandFactor desired: %lf adjusted: %lf "
					              "size: %u\n",
			...
			if (_averageScavengeTimeRatio < extensions->dnssExpectedTimeRatioMinimum
				...
				desiredContractionFactor =
				        OMR_MIN(extensions->dnssExpectedTimeRatioMinimum * 2, expectedTimeRatio);
				...
				_contractionSize = MM_Math::roundToCeiling(extensions->heapAlignment,
				        (uintptr_t)(getCurrentSize() * adjustedContractionFactor));
				_contractionSize = MM_Math::roundToCeiling(regionSize, _contractionSize);

				if (debug) {
					omrtty_printf("\tContract decision - contractFactor desired: %lf adjusted: %lf "
					              "size: %u\n",
