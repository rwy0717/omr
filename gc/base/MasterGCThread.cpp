...
bool MM_MasterGCThread::initialize(MM_Collector* collector, bool runAsImplicit, bool acquireVMAccessDuringConcurrent)
{
    bool success = true;
...
bool MM_MasterGCThread::handleConcurrent(MM_EnvironmentBase* env)
{
    ...
    } while (_collector->getConcurrentPhaseStats()->isTerminationRequestExternal()
        && _collector->isConcurrentWorkAvailable(env));
