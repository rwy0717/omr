...
public:
	 ...
	 */
	GC_ObjectHeapIteratorAddressOrderedList(
	        MM_GCExtensionsBase *extensions, MM_HeapRegionDescriptor *region, bool includeDeadObjects)
	 ...
	 * @param includeDeadObjects if true, non-objects will be included in the walk
	 */
	GC_ObjectHeapIteratorAddressOrderedList(MM_GCExtensionsBase *extensions, omrobjectptr_t base,
	        omrobjectptr_t top, bool includeDeadObjects, bool skipFirstObject = false)
	        : /* Iterator base values */
	        _includeDeadObjects(includeDeadObjects)
	        ,
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
			/* There are no known users of this type of iteration, while there are still forwarded objects
			 * in the heap */
			Assert_MM_false(MM_ForwardedHeader(_scanPtr, _extensions->compressObjectReferences())
			                        .isForwardedPointer());
#endif
