...
void GC_OMRVMInterface::initializeExtensions(MM_GCExtensionsBase* extensions)
{
    ...
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_WALK_HEAP_START, hookWalkHeapStart, OMR_GET_CALLSITE(), NULL);
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_WALK_HEAP_END, hookWalkHeapEnd, OMR_GET_CALLSITE(), NULL);
}

/**
 * Return the OMR hook interface for the Memory Manager
 */
J9HookInterface** GC_OMRVMInterface::getOmrHookInterface(MM_GCExtensionsBase* extensions)
{
