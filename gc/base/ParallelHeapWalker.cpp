...
MM_ParallelHeapWalker* MM_ParallelHeapWalker::newInstance(
    MM_ParallelGlobalGC* globalCollector, MM_MarkMap* markMap, MM_EnvironmentBase* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (walkFlags == (region->getTypeFlags() & walkFlags)) {
            GC_ParallelObjectHeapIterator objectHeapIterator(
                env, region, region->getLowAddress(), region->getHighAddress(), _markMap, parallelChunkSize);
 ...
 */
void MM_ParallelHeapWalker::allObjectsDo(MM_EnvironmentBase* env, MM_HeapWalkerObjectFunc function, void* userData,
