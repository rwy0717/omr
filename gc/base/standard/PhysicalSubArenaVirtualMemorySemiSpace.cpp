...
MM_PhysicalSubArenaVirtualMemorySemiSpace::newInstance(MM_EnvironmentBase *env, MM_Heap *heap)
{
	...
	subArena = (MM_PhysicalSubArenaVirtualMemorySemiSpace *)env->getForge()->allocate(
	        sizeof(MM_PhysicalSubArenaVirtualMemorySemiSpace), OMR::GC::AllocationCategory::FIXED,
	        OMR_GET_CALLSITE());
	if (subArena) {
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate(MM_EnvironmentBase *env)
{
	...
	if (_parent->attachSubArena(env, this, _subSpace->getInitialSize(), attachPolicy)) {
		...
		if ((_highSemiSpaceRegion->getLowAddress() != semiSpaceMiddle)
			...
			omrtty_printf("!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad "
			              "address range (%p,%p) for highSemiSpaceRegion, expected to be (%p, %p)\n",
		...
		if ((_lowSemiSpaceRegion->getLowAddress() != _lowAddress)
			...
			omrtty_printf("!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad "
			              "address range (%p,%p) for lowSemiSpaceRegion, expected to be (%p, %p)\n",
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expand(MM_EnvironmentBase *env, uintptr_t expandSize)
{
	...
	assume0(expandSize
	        == MM_Math::roundToCeiling(MM_GCExtensions::getExtensions(env)->heapAlignment, expandSize)); /* expand
	                                                                                                        requests
	                                                                                                        must be
	                                                                                                        aligned
	                                                                                                      */
	...
	Assert_MM_true(totalExpandSize
	        == MM_Math::roundToCeiling(2 * getHeapRegionManager()->getRegionSize(), totalExpandSize));
...
psavmssMoveFunction(OMR_VM *omrVM, omrobjectptr_t *objectIndirect, void *localData, uint32_t isDerivedPointer)
{
	...
	if (NULL != objectPtr) {
		if ((objectPtr >= (omrobjectptr_t)userData->srcBase)
			...
			objectPtr = (omrobjectptr_t)((((uintptr_t)objectPtr) - ((uintptr_t)userData->srcBase))
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::contract(MM_EnvironmentBase *env, uintptr_t contractSize)
{
	...
	Assert_MM_true(contractSize == MM_Math::roundToCeiling(effectiveAlignment, contractSize)); /* expand requests
	                                                                                              must be aligned */
	...
	/* Make sure we do not contract beyond any maximum.  We align the total contraction size to 2x
	 * effectiveAlignment for when we are splitting up any remaining free space (above and beyond what is minimally
	 * required) so that splitting/alignment math works out.
	 */
	uintptr_t maximumContractSize = MM_Math::roundToFloor(
	        effectiveAlignment * 2, _subSpace->getCurrentSize() - _subSpace->getMinimumSize());
	...
	void *allocateTrailingFreeBase =
	        subSpaceAllocate->findFreeEntryEndingAtAddr(env, allocateRegion->getHighAddress());
	...
	uintptr_t allocateTrailingFreeSize = ((uintptr_t)allocateRegion->getHighAddress())
	...
	if (debug) {
		omrtty_printf("\tValid allocate heap (%p %p) size %p\n", allocateLeadingFreeTop,
		        allocateTrailingFreeBase, heapSizeToMove);
	}
	...
	if (extensions->tiltedScavenge) {
		uintptr_t remainingFreeSizeForAllocate = MM_Math::roundToCeiling(
		        effectiveAlignment, (remainingFreeSize / 100) * (100 - survivorSpaceRatio));
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		uint8_t *survivorSegmentBase = ((uint8_t *)_highSemiSpaceRegion->getHighAddress())
		...
		if (debug) {
			omrtty_printf("\tadjusted allocate (%p %p) survivor (%p %p)\n", allocateSegmentBase,
			        allocateSegmentTop, survivorSegmentBase, survivorSegmentTop);
		...
		if (allocateSegmentBase > allocateLeadingFreeTop) {
			if (extensions->isConcurrentScavengerEnabled()) {
				/* Fixup logic does not account for possible object growth, which would be required for
				 * objects that have been allocated and hashed during the last concurrent phase (hence,
				 * in hybrid Survivor-Allocate that is subject to movement).
				 */
				if (debug) {
					omrtty_printf("\tHeap movement (%p %p) to (%p %p) required, but not allowed "
					              "with CS enabled.\n",
			...
			/* TODO: can this walk be parallelized? Probably not trivially, since the mark map isn't up to
			 * date at this point */
		...
		if (allocateTrailingFreeSize > 0) {
			subSpaceAllocate->removeExistingMemory(env, this, allocateTrailingFreeSize,
			        (void *)allocateTrailingFreeBase, allocateRegion->getHighAddress());
		}

		/* Adjust the region boundaries */
		/* we are subtly assuming that allocate size is at least as large as survivor size (the converse makes
		 * no sense) so ensure this assumption is correct */
		...
		if (allocateSegmentBase > allocateLeadingFreeTop) {
			...
			subSpaceAllocate->moveHeap(
			        env, allocateLeadingFreeTop, allocateTrailingFreeBase, allocateSegmentBase);
		...
		if (debug) {
			omrtty_printf("\tRemove and decommit (%p %p) (valid %p %p)\n", removeMemoryBase,
			        removeMemoryTop, previousValidAddressNotRemoved, (void *)allocateSegmentBase);
		}
		...
		_heap->decommitMemory(removeMemoryBase, removeMemorySize, previousValidAddressNotRemoved,
		        (void *)allocateSegmentBase);
		...
		/* Find the new ranges for the allocate and survivor segments */
		uint8_t *allocateSegmentBase = ((uint8_t *)_highSemiSpaceRegion->getHighAddress())
		...
		if (debug) {
			omrtty_printf("\tadjusted survivor (%p %p) allocate (%p %p)\n", survivorSegmentBase,
			        survivorSegmentTop, allocateSegmentBase, allocateSegmentTop);
			omrtty_printf("\tRemove range calculated as (%p %p)\n", removeMemoryBase, removeMemoryTop);
		}
		...
		if (debug) {
			omrtty_printf("\tRemove and decommit (%p %p) (valid %p %p)\n", removeMemoryBase,
			        removeMemoryTop, previousValidAddressNotRemoved, (void *)survivorSegmentBase);
		}
		_subSpace->heapRemoveRange(env, _subSpace, removeMemorySize, removeMemoryBase, removeMemoryTop,
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::canContract(MM_EnvironmentBase *env)
{
	if (_resizable) {
		if (_avoidMovingObjects) {
			MM_MemorySubSpace *subSpaceAllocate =
			        ((MM_MemorySubSpaceSemiSpace *)_subSpace)->getMemorySubSpaceAllocate();
			/* VMDESIGN 1690: only permit contraction if the allocate space is the high semi-space. This
			 * case is much less likely to require object movement and is therefore significantly faster.
			 * However, depending on the tilt ratio and contraction amount, movement may still be required.
			 */
        ...
        MM_EnvironmentBase *env, uintptr_t allocateSpaceSize, uintptr_t survivorSpaceSize, bool updateMemoryPools)
{
	...
	if (updateMemoryPools) {
		...
		if (expandBase > expandTop) {
			...
			subSpaceSurvivor->addExistingMemory(env, this, ((uintptr_t)expandBase) - ((uintptr_t)expandTop),
...
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
		{
			...
			subSpaceAllocate->addExistingMemory(env, this, ((uintptr_t)expandTop) - ((uintptr_t)expandBase),
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(MM_EnvironmentBase *env, uintptr_t survivorSpaceSizeRequested)
{
	...
	/* region size rounding is required on these minimum and maximum calculations since they can be used to change
	 * the requested survivor space */
	...
	tilt(env, (_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize()) - survivorSpaceSize,
	        survivorSpaceSize);
}
/**
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expandNoCheck(MM_EnvironmentBase *env, uintptr_t expandSize)
{
	...
	Assert_MM_true(
	        _subSpace->getCurrentSize() == (_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize()));
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		if (0 != allocateSpaceExpandSize) {
			...
			uintptr_t freeRangeAvailableToTransferSize = ((uintptr_t)_lowSemiSpaceRegion->getHighAddress())
		...
		subSpaceAllocate->removeExistingMemory(env, this, survivorSpaceExpandSize, freeRangeToTransferBase,
		 ...
		 */
		_subSpace->heapAddRange(env, _subSpace, splitExpandSize, newLowAddress,
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion,
		        _lowSemiSpaceRegion->getHighAddress(), _highSemiSpaceRegion->getHighAddress());
		...
		subSpaceAllocate->addExistingMemory(env, this, allocateSpaceExpandSize,
		        _highSemiSpaceRegion->getLowAddress(), previousHighSemiSpaceSegmentBase, true);
 ...
 */
uintptr_t
MM_PhysicalSubArenaVirtualMemorySemiSpace::calculateExpansionSplit(MM_EnvironmentBase *env, uintptr_t requestExpandSize,
        uintptr_t *allocateSpaceSize, uintptr_t *survivorSpaceSize)
{
	...
	*survivorSpaceSize = 0;
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		uintptr_t initialFreeRangeToTransferSize = ((uintptr_t)_lowSemiSpaceRegion->getHighAddress())
		        - ((uintptr_t)freeRangeToTransferBase);
		 ...
		 * The tranfer occurs to evenly split the memory added to the front of the semi spaces, which is
		 * adjacent to the allocate space.  To transfer half of the split to the survivor, we use the trailing
		 * free space in the allocate space.
		 */
		...
		if (freeRangeToTransferSize < survivorSpaceExpandSize) {
			...
			Assert_MM_true(survivorSpaceExpandSize % extensions->heapAlignment == 0); // transfer size was
			                                                                          // align guaranteed
			                                                                          // before
			...
			if (extensions->tiltedScavenge) {
				...
				/* No need to round to floor - the size is being reduced, so we can only be equal at
				 * most */
        ...
        MM_EnvironmentBase *env, uintptr_t expandSizeDeltaAlignment, uintptr_t expandSize)
{
	...
	/* The splitting can reduce the expand size, so make sure the delta in the expansion is rounded to any
	 * requirements. NOTE: The delta is what needs to be rounded, not the result.
	 */
	...
	assume0(adjustedExpandSize
	        == calculateExpansionSplit(env, adjustedExpandSize, &allocateSpaceSize, &survivorSpaceSize));
