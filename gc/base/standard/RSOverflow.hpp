...
private:
    ...
    bool _searchMode; /**< mode switch: initialized in Modify mode, after first nextObject call is switched to Search
                         mode */
    ...
    MMINLINE omrobjectptr_t nextObject()
    {
        if (!_searchMode) {
            ...
            _markedObjectIterator.reset(_markMap, (uintptr_t*)_extensions->heapBaseForBarrierRange0,
                (uintptr_t*)((uintptr_t)_extensions->heapBaseForBarrierRange0 + _extensions->heapSizeForBarrierRange0));
            _searchMode = true;
        }
