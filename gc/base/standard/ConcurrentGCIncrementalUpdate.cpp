 ...
 */
MM_ConcurrentGCIncrementalUpdate* MM_ConcurrentGCIncrementalUpdate::newInstance(MM_EnvironmentBase* env)
{
    MM_ConcurrentGCIncrementalUpdate* concurrentGC = (MM_ConcurrentGCIncrementalUpdate*)env->getForge()->allocate(
        sizeof(MM_ConcurrentGCIncrementalUpdate), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_ConcurrentGCIncrementalUpdate::initialize(MM_EnvironmentBase* env)
{
    ...
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START,
...
bool MM_ConcurrentGCIncrementalUpdate::heapAddRange(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress)
{
    ...
    if (result) {
        ...
        result = ((MM_ConcurrentCardTable*)_cardTable)
                     ->heapAddRange(env, subspace, size, lowAddress, highAddress, clearCards);
        if (!result) {
