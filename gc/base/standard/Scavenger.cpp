...
uintptr_t MM_Scavenger::getVMStateID() { return OMRVMSTATE_GC_COLLECTOR_SCAVENGER; }
 ...
 */
void MM_Scavenger::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState) { Assert_MM_unreachable(); }
...
void MM_Scavenger::clearCycleGCStats(MM_EnvironmentBase* env) { _extensions->scavengerStats.clear(true); }
        ...
        if (scavengerGCStats->_gcCount > 1) {
            ...
            scavengerGCStats->_avgTenureBytes
                = (uintptr_t)MM_Math::weightedAverage((float)scavengerGCStats->_avgTenureBytes,
                    (float)scavengerGCStats->_tenureAggregateBytes, TENURE_BYTES_HISTORY_WEIGHT);
#if defined(OMR_GC_LARGE_OBJECT_AREA)
            scavengerGCStats->_avgTenureSOABytes
                = (uintptr_t)MM_Math::weightedAverage((float)scavengerGCStats->_avgTenureSOABytes,
...
#if defined(OMR_GC_LARGE_OBJECT_AREA)
            scavengerGCStats->_avgTenureSOABytes
                = scavengerGCStats->_tenureAggregateBytes - scavengerGCStats->_tenureLOABytes;
...
MMINLINE uintptr_t MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard* env)
{
    ...
    if (scanCacheCount < threadCount) {
        uintptr_t cacheSizeBasedOnScanCacheCount
            = calculateCopyScanCacheSizeForQueueLength(maxCacheSize, threadCount, scanCacheCount);
    ...
    omrobjectptr_t objectPtr, uintptr_t flags, omrobjectptr_t* rememberedSetSlot)
{
    ...
    if (NULL != scanCache) {
        Assert_MM_true(
            objectScanner->isIndexableObject() == (scanCache->isSplitArray() && (0 < scanCache->_arraySplitIndex)));
 ...
 ****************************************
 */
void MM_Scavenger::clearRememberedSetLists(MM_EnvironmentStandard* env) { _extensions->rememberedSet.clear(env); }
 ...
 */
bool MM_Scavenger::scavengeCompletedSuccessfully(MM_EnvironmentStandard* env) { return !isBackOutFlagRaised(); }
...
void MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(MM_EnvironmentStandard* env)
{
            ...
            while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
                ...
                if (header.isForwardedPointer()) {
                    ...
                    omrtty_printf("{SCAV: Back out forward pointer %p[%p]@%p -> %p[%p]}\n", objectPtr, *objectPtr,
                        forwardedObject, freeHeader->getNext(), freeHeader->getSize());
    ...
    while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
        if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
             ...
             * destroyed slot fixed up (but the above loop must complete before we have the information with which to
             * fixup the destroyed slot).  Fixing up a slot in dark matter could crash, though, since the slot could
             * point to contracted memory or could point to corrupted data updated in a previous backout.  The simple
             * work-around for this problem is to check if the slot points at a readable part of the heap (specifically,
             * tenure or survivor - the only locations which would require us to fix up the slot) and only read and
             * fixup the slot in those cases. This means that we could still corrupt the slot but we will never crash
             * during fixup and nobody else should be trusting slots found in dead objects.
             */
            GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(_extensions, rootRegion, false);
            omrobjectptr_t objectPtr = NULL;
    ...
    MM_EnvironmentBase* envBase, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
    ...
    bool shouldPercolate
        = _cli->scavenger_internalGarbageCollect_shouldPercolateGarbageCollect(env, &percolateReason, &gcCode);
...
bool MM_Scavenger::scavengeRoots(MM_EnvironmentBase* env)
{
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_ROOTS, UDATA_MAX, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);
...
bool MM_Scavenger::scavengeScan(MM_EnvironmentBase* envBase)
{
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN, UDATA_MAX, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);
...
bool MM_Scavenger::scavengeComplete(MM_EnvironmentBase* envBase)
{
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_COMPLETE, UDATA_MAX, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);
...
void MM_Scavenger::threadFinalReleaseCaches(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
