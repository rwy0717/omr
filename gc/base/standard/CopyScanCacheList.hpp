...
private:
	...
	struct CopyScanCacheSublist {
		...
		bool initialize(MM_EnvironmentBase *env)
		{
			...
			if (_cacheLock.initialize(
			            env, &extensions->lnrlOptions, "MM_CopyScanCacheList:_sublists[]._cacheLock")) {
				return false;
			}
			return true;
		}
	...
	struct CopyScanCacheSublist *_sublists; /**< An array of CopyScanCacheSublist structures which is _sublistCount
	                                           elements long */
	...
	volatile uintptr_t *_cachedEntryCount; /* pointer to cachedEntryCount, that is shared among all lists (of all
	                                          nodes) */
...
public:
	 ...
	 */
	bool resizeCacheEntries(
	        MM_EnvironmentBase *env, uintptr_t allocatedCacheEntryCount, uintptr_t incrementCacheEntryCount);
