...
MM_ConcurrentCardTableForWC::newInstance(
        MM_EnvironmentBase *env, MM_Heap *heap, MM_MarkingScheme *markingScheme, MM_ConcurrentGC *collector)
{
	...
	cardTable = (MM_ConcurrentCardTableForWC *)env->getForge()->allocate(
	        sizeof(MM_ConcurrentCardTableForWC), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (NULL != cardTable) {
...
MM_ConcurrentCardTableForWC::prepareCardsForCleaning(MM_EnvironmentBase *env)
{
	...
	MM_ConcurrentPrepareCardTableTask prepareCardTableTask(
	        env, _dispatcher, this, _firstCardInPhase, _lastCardInPhase, MARK_DIRTY_CARD_SAFE);
 ...
 */
void
MM_ConcurrentCardTableForWC::prepareCardTableChunk(
        MM_EnvironmentBase *env, Card *chunkStart, Card *chunkEnd, CardAction action)
{
	...
	for (CleaningRange *range = _cleaningRanges; range < _lastCleaningRange; range++) {
		...
		while (prepareSizeRemaining > 0) {
			...
			currentPrepareSize = (prepareUnitSize > prepareSizeRemaining) ? prepareSizeRemaining
			                                                              : prepareUnitSize;
...
MM_ConcurrentCardTableForWC::initializeFinalCardCleaning(MM_EnvironmentBase *env)
{
	...
	if (_cardTablePreparedForCleaning) {
		...
		if (_currentCleaningRange < _lastCleaningRange) {
			...
			MM_ConcurrentPrepareCardTableTask prepareCardTableTask(env, _dispatcher, this,
			        (Card *)_currentCleaningRange->nextCard, _lastCard, MARK_SAFE_CARD_DIRTY);
			_dispatcher->run(env, &prepareCardTableTask);
		}
