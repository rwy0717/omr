...
private:
	...
	volatile uintptr_t _doneIndex; /**< sequence ID of completeScan loop, which we may have a few during one GC
	                                  cycle */
	...
	volatile uintptr_t _cachedEntryCount; /**< non-empty scanCacheList count (not the total count of caches in the
	                                         lists) */
	...
	uintptr_t _waitingCountAliasThreshold; /**< Only alias a copy cache IF the number of threads waiting hasn't
	...
	uintptr_t _cacheLineAlignment; /**< The number of bytes per cache line which is used to determine which
	                                  boundaries in memory represent the beginning of a cache line */
	...
	volatile bool _shouldYield; /**< Set by the first GC thread that observes that a criteria for yielding is met.
	                               Reset only when the concurrent phase is finished. */
...
private:
	...
	/**
	 * Check if concurrent phase of the cycle should yield to an external activity. If so, set the flag so that
	 * other GC threads react appropriately
	 */
	...
	MMINLINE bool shouldAbortScanLoop(MM_EnvironmentStandard *env)
	{
		...
		if (IS_CONCURRENT_ENABLED) {
			/* Concurrent Scavenger needs to drain the scan queue in last scan loop before aborted handling
			 * starts. It is however fine to leave it populated, if we want to yield in a middle of
			 * concurrent phase which aborted, since there will be at least one scan loop afterwards in
			 * complete phase that will drain it. Bottom line, we don't care about isBackOutFlagRaised when
			...
			if (shouldAbort) {
				...
				/* Since we are aborting the scan loop without synchornizing with other GC threads
				 * (before which we flush buffers), we have to do it now. There should be no danger in
				 * not synchonizing with other threads, since we can only abort/yield in main scan loop
				 * and not during clearable STW phase, where is a potential danger of entering a scan
				 * loop without ensuring all threads flushed buffers from previous scan loop.
				 */
...
public:
	 ...
	 */
	static void hookGlobalCollectionStart(
	...
	bool splitIndexableObjectScanner(MM_EnvironmentStandard *env, GC_ObjectScanner *objectScanner,
	 ...
	 * Split into two functions deepScan and deepScanOutline. Frequently called checks (see lazy start check) must
	 * be inlined
	 * @param env The environment.
	 * @param objectPtr The pointer to the object.
