...
MM_ConcurrentCardTable::tlhCleared(J9HookInterface **hook, uintptr_t eventNum, void *eventData, void *userData)
{
	...
	((MM_ConcurrentCardTable *)userData)
	        ->processTLHMarkBits(
	                env, (MM_MemorySubSpace *)event->subSpace, event->cacheBase, event->cacheTop, CLEAR);
}
 ...
 */
bool
MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
 ...
 */
bool
MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(MM_EnvironmentBase *env, uintptr_t size, void *lowAddress,
        ...
        MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress, void *highAddress)
{
	...
	if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
		...
		if (NULL != _tlhMarkBits) {
			...
			uintptr_t highTLHMarkMap = (((uintptr_t)highAddress) - numericalHeapBase)
			        >> TLH_MARKING_INDEX_SHIFT;

			/* Adjust the lowTLHMArkMap to account for rounding (since this is really the highTLHMarkMap
			 * address of another range). as we dont want to commit a slot we have previously commited andn
			 * more importantly we dont want to clear and existing slot. */
			...
			commited = memoryManager->commitMemory(
			        &_tlhMarkMapMemoryHandle, (void *)&(_tlhMarkBits[lowTLHMarkMap]), mapSize);
 ...
 */
bool
MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase *env, uintptr_t size, void *lowAddress,
        void *highAddress, void *lowValidAddress, void *highValidAddress)
{
	...
	if (_extensions->isFvtestForceConcurrentTLHMarkMapDecommitFailure()) {
		...
		if (NULL != _tlhMarkBits) {
			...
			if (NULL != lowValidAddress) {
				lowValidTLHMarkMap = (((uintptr_t)lowValidAddress) - numericalHeapBase)
				        >> TLH_MARKING_INDEX_SHIFT;
				/* Adjust the low valid range to account for rounding (since this is really the high
				 * address of another range) */
			...
			if (NULL != highValidAddress) {
				highValidTLHMarkMap = (((uintptr_t)highValidAddress) - numericalHeapBase)
				        >> TLH_MARKING_INDEX_SHIFT;
			}
			...
			if (0 != mapSize) {
				MM_MemoryManager *memoryManager = _extensions->memoryManager;
				decommited = memoryManager->decommitMemory(&_tlhMarkMapMemoryHandle,
				        (void *)&(_tlhMarkBits[lowTLHMarkMap]), mapSize,
				        (NULL != lowValidAddress) ? (void *)&(_tlhMarkBits[lowValidTLHMarkMap]) : NULL,
				        (NULL != highValidAddress) ? (void *)&(_tlhMarkBits[highValidTLHMarkMap])
				                                   : NULL);
				if (!decommited) {
 ...
 */
bool
MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
 ...
 */
bool
MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
...
MM_ConcurrentCardTable::newInstance(
...
MM_ConcurrentCardTable::initialize(MM_EnvironmentBase *env, MM_Heap *heap)
{
	...
	if (initialized) {
...
#endif /* OMR_GC_MODRON_SCAVENGER */
		{
			...
			(*mmPrivateHooks)
			        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED,
			                tlhCleared, OMR_GET_CALLSITE(), (void *)this);
			(*mmPrivateHooks)
			        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_REFRESHED,
			                tlhRefreshed, OMR_GET_CALLSITE(), (void *)this);
		}
		...
		case 0: _lastCardCleanPhase = UNINITIALIZED; break;
		case 1: _lastCardCleanPhase = PHASE2_COMPLETE; break;
		case 2: _lastCardCleanPhase = PHASE3_COMPLETE; break;
		default: assume0(0); break;
		}
	}
	return initialized;
}
 ...
 */
MMINLINE bool
MM_ConcurrentCardTable::getExclusiveCardTableAccess(
 ...
 */
bool
MM_ConcurrentCardTable::cleanCards(
 ...
 */
bool
MM_ConcurrentCardTable::cleanSingleCard(
 ...
 */
void
MM_ConcurrentCardTable::processTLHMarkBits(
        MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, void *tlhBase, void *tlhTop, BitMapAction action)
{
	...
	if (base < top) {
		 ...
		 */
		if ((CLEAR == action)
		        && !(_collector->getConcurrentGCStats()->getConcurrentWorkStackOverflowOcurred())) {
...
MM_ConcurrentCardTable::isTLHMarkBitsEmpty(MM_EnvironmentBase *env)
{
	...
	while (NULL != (region = regionIterator.nextRegion())) {
		...
		if (region->getSubSpace()->isConcurrentCollectable()) {
			uintptr_t currentSlot = (((uintptr_t)region->getLowAddress()) - numericalHeapBase)
			        >> TLH_MARKING_INDEX_SHIFT;
			uintptr_t endSlot = (((uintptr_t)region->getHighAddress()) - numericalHeapBase)
			        >> TLH_MARKING_INDEX_SHIFT;
...
MM_ConcurrentCardTable::getNextDirtyCard(MM_EnvironmentBase *env, Card cardMask, bool concurrentCardClean)
{
	...
	while (NULL != firstCard) {
			...
			if (firstCard != (Card *)currentRange->nextCard) {
				...
				if (firstCard
				        != (Card *)MM_AtomicOperations::lockCompareExchange(
				                (volatile uintptr_t *)&currentRange->nextCard, (uintptr_t)firstCard,
		...
		if (currentCard < lastCardToClean) {
			...
			MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&_currentCleaningRange,
			        (uintptr_t)currentRange, (uintptr_t)nextRange);
