 ...
 */
uintptr_t MM_SweepHeapSectioningSegmented::estimateTotalChunkCount(MM_EnvironmentBase* env)
{
    ...
    if (0 == _extensions->parSweepChunkSize) {
        ...
        _extensions->parSweepChunkSize = MM_Math::roundToCeiling(256 * 1024,
            _extensions->heap->getMaximumMemorySize() / (_extensions->dispatcher->threadCountMaximum() * 32));
    }
...
uintptr_t MM_SweepHeapSectioningSegmented::calculateActualChunkNumbers() const
{
    ...
    while ((region = regionIterator.nextRegion()) != NULL) {
        if ((region)->isCommitted()) {
            /* TODO:  this must be rethought for Tarok since it treats all regions identically but some might require
             * different sweep logic */
 ...
 */
uintptr_t MM_SweepHeapSectioningSegmented::reassignChunks(MM_EnvironmentBase* env)
{
 ...
 */
MM_SweepHeapSectioningSegmented* MM_SweepHeapSectioningSegmented::newInstance(MM_EnvironmentBase* env)
{
    MM_SweepHeapSectioningSegmented* sweepHeapSectioning = (MM_SweepHeapSectioningSegmented*)env->getForge()->allocate(
        sizeof(MM_SweepHeapSectioningSegmented), OMR::GC::AllocationCategory::FIXED, OMR_GET_CALLSITE());
