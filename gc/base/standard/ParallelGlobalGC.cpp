...
uintptr_t MM_ParallelGlobalGC::getVMStateID() { return OMRVMSTATE_GC_COLLECTOR_GLOBALGC; }
...
void MM_ParallelGlobalGC::postMark(MM_EnvironmentBase* env) { _markingScheme->getMarkMap()->setMarkMapValid(true); }
 ...
 */
void MM_ParallelGlobalGC::heapReconfigured(MM_EnvironmentBase* env) { _sweepScheme->heapReconfigured(env); }
...
void MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase* env)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_REPORT_MEMORY_USAGE(_extensions->privateHookInterface, env->getOmrVMThread(),
        omrtime_hires_clock(), J9HOOK_MM_PRIVATE_REPORT_MEMORY_USAGE, _extensions->getForge()->getCurrentStatistics());
