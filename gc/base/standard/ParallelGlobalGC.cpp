...
fixObject(OMR_VMThread *omrVMThread, MM_HeapRegionDescriptor *region, omrobjectptr_t object, void *userData)
{
	...
	if (!collector->getMarkingScheme()->isMarked(object)) {
		 ...
		 * (MemoryPoolAddressOrderedListBase.hpp) but due to current limitation of API that requires address of
		 * object to be freed which we need to check from a set stored in extensions, which weren't further
		 * passed we will check it here. But in case new API is added, it is better to move there.
		 */
...
MM_ParallelGlobalGC::initialize(MM_EnvironmentBase *env)
{
...
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
	(*mmPrivateHooks)
	        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START,
	                globalGCHookCCStart, OMR_GET_CALLSITE(), NULL);
	(*mmPrivateHooks)
	        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END,
	                globalGCHookCCEnd, OMR_GET_CALLSITE(), NULL);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */

	(*mmPrivateHooks)
	        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_START, globalGCHookSysStart,
	...
	if (_extensions->scavengerEnabled) {
		...
		/* NOTE: This will be a one time hook across all instances as the function and user data will be
		 * identical - i.e., we will only get one Hook registered no matter how many scavengers are
		 * created/initialized
		 */
		(*mmPrivateHooks)
...
MM_ParallelGlobalGC::cleanupAfterGC(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription)
{
	...
	/* Null tenure TLH (Copy Cache) references for all GC slave and Mutator (for concurrent scavenger) threads as
	 * the memory will be invalidated on sweep cycle*/
        ...
        MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription, bool initMarkMap, bool rebuildMarkBits)
{
	...
	if (_delegate.isAllowUserHeapWalk() || env->_cycleState->_gcCode.isRASDumpGC()) {
		if (!_fixHeapForWalkCompleted) {
			...
			/* since this is the superset of all walk operations, we can safely set the flag that states
			 * other walks can be omitted for this cycle as redundant (CMVC 122959)
			 */
        ...
        uintptr_t activeSubspaceMaxExpansionInSpace, MM_GCCode gcCode)
{
...

	{
		...
		if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode.getCode()) && (_extensions->gcOnIdle)) {
			...
			float totalFragmentationRatio = ((float)totalFragmentation)
			        / ((float)freeMemorySize + (float)totalSize / 2);

			Trc_ParallelGlobalGC_shouldCompactThisCycle(env->getLanguageVMThread(), totalFragmentationRatio,
 ...
 */
uintptr_t
MM_ParallelGlobalGC::fixHeapForWalk(
 ...
 */
bool
MM_ParallelGlobalGC::heapAddRange(
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
	if (_extensions->isConcurrentScavengerEnabled()) {
		/* ParallelGlobalGC or ConcurrentGC (STW phase) cannot start before Concurrent Scavenger cycle is in
		 * progress */
...
MM_ParallelGlobalGC::reportGCStart(MM_EnvironmentBase *env)
{
	...
	TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_START(_extensions->omrHookInterface, env->getOmrVMThread(),
	        omrtime_hires_clock(), J9HOOK_MM_OMR_GLOBAL_GC_START, _extensions->globalGCStats.gcCount,
...
MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase *env)
{
	...
	uintptr_t approximateLoaActiveFreeMemorySize = (_extensions->largeObjectArea
	                ? _extensions->heap->getApproximateActiveFreeLOAMemorySize(MEMORY_TYPE_OLD)
	                : 0);
	...
	uintptr_t approximateTotalActiveFreeMemorySize = approximateNewActiveFreeMemorySize
	...
	/* these are assigned to temporary variable out-of-line since some preprocessors get confused if you have
...
MM_ParallelGlobalGC::reportMarkEnd(MM_EnvironmentBase *env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_MARK_END(_extensions->privateHookInterface, env->getOmrVMThread(),
...
MM_ParallelGlobalGC::reportSweepEnd(MM_EnvironmentBase *env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(_extensions->privateHookInterface, env->getOmrVMThread(),
