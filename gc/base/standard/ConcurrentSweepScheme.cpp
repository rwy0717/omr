 ...
 */
void MM_ConcurrentSweepPoolState::tearDown(MM_EnvironmentBase* env) { _sweepChunkIteratorLock.tearDown(); }
...
void MM_ConcurrentSweepScheme::verifyFreeList(MM_EnvironmentStandard* env, MM_HeapLinkedFreeHeader* freeListHead)
{
    for (MM_HeapLinkedFreeHeader* current = freeListHead; NULL != current; current = current->getNext()) {
        ...
        assume((current->getNext() == NULL) || (current->getNext() > current->afterEnd()),
