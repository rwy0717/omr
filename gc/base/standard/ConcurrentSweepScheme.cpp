...
private:
	...
	MM_ParallelSweepChunk *_currentSweepChunkReverse; /**< Next chunk to sweep when sweeping high to low in a given
	                                                     memory pool */
	...
	MM_HeapLinkedFreeHeader *_connectNextFreeEntry; /**< Existing free list entry immediately following the connect
	                                                   range */
	UDATA _connectNextFreeEntrySize; /**< Existing free list entry size immediately following the connect range */

	MM_ParallelSweepChunk *_currentInitChunk; /**< Last chunk initialized for concurrent sweep in the given memory
	                                             pool (used for _next pointer) */

	UDATA _freeMemoryConnected; /**< Running count of the total free memory connected into the memory pool  since
	                               last GC */
...
MM_ConcurrentSweepPoolState::initializeForSweep(MM_EnvironmentBase *env)
{
	...
	if (++_sweepCount > 1) {
		...
		float newFreeRatio =
		        _freeMemoryConnected == 0 ? 0 : (float)_freeMemoryConnected / (float)_heapSizeConnected;
		_previousFreeRatio =
		        MM_Math::weightedAverage(_previousFreeRatio, newFreeRatio, _previousFreeHistoryWeight);
	}
...
public:
	...
	MM_ConcurrentSweepFindMinimumSizeFreeTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher,
	        MM_ConcurrentSweepScheme *concurrentSweepScheme, MM_MemorySubSpace *memorySubSpace,
...
MM_ConcurrentSweepScheme::initialize(MM_EnvironmentBase *env)
{
	...
	if (omrthread_monitor_init_with_name(&_completeSweepingConcurrentlyLock, 0,
...
MM_ConcurrentSweepScheme::createSweepPoolState(MM_EnvironmentBase *env, MM_MemoryPool *memoryPool)
{
	...
	if (NULL == _poolSweepPoolState) {
		_poolSweepPoolState = pool_new(sizeof(MM_ConcurrentSweepPoolState), 0, 2 * sizeof(UDATA), 0,
...
MM_ConcurrentSweepScheme::reportCompletedConcurrentSweep(MM_EnvironmentBase *env, SweepCompletionReason reason)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_COMPLETED_CONCURRENT_SWEEP(_extensions->privateHookInterface, env->getOmrVMThread(),
	        ...
	        omrtime_hires_delta(_stats._completeSweepPhaseTimeStart, _stats._completeSweepPhaseTimeEnd,
	                OMRPORT_TIME_DELTA_IN_MICROSECONDS),
...
MM_ConcurrentSweepScheme::isConcurrentSweepCandidate(MM_MemorySubSpace *memorySubSpace)
{
	return memorySubSpace->isLeafSubSpace() && memorySubSpace->isActive()
	        && (NULL != memorySubSpace->getMemoryPool()) && (MEMORY_TYPE_OLD == memorySubSpace->getTypeFlags());
}
...
MM_ConcurrentSweepScheme::initializeChunks(MM_EnvironmentBase *env)
{
	...
	while (0 != totalChunkCount--) {
		...
		MM_ConcurrentSweepPoolState *sweepState = (MM_ConcurrentSweepPoolState *)getPoolState(
		...
		if (NULL != sweepState->_currentInitChunk) {
			...
			Assert_MM_true(
			        sweepState->_currentInitChunk->_next == sweepState->_currentInitChunk->_nextChunk);
		}
...
MM_ConcurrentSweepScheme::postConnectChunk(MM_EnvironmentBase *env, MM_ParallelSweepChunk *chunk)
{
	...
	if (NULL != sweepState->_connectPreviousFreeEntry) {
		...
		/* If we are connecting to the top of the chunk then we may need to fix the heap to ensure it is still
		 * walkable. The scenario that can result in a corrupted heap is having an object/dead entry span two
		 * different chunks, with the last valid connection being at the end of the current chunk.  The result
		 * is the 2nd chunk can be unwalkable. The solution is to fix the heap by splitting the overlapping
		 * memory in the two chunks, and install a dead object header (or slots) at the beginning of the next
		 * chunk.  This ensures that a walk starting at the beginning of the next chunk will be safe.
		 */
		void *endFreeEntry = (void *)(((UDATA)sweepState->_connectPreviousFreeEntry)
		...
		if (endFreeEntry == chunk->chunkTop) {
			GC_ObjectHeapIteratorAddressOrderedList objectIterator(_extensions,
			        (omrobjectptr_t)sweepState->_connectPreviousFreeEntry, (omrobjectptr_t)chunk->chunkTop,
			        true);
			...
			while (NULL != (currentObject = objectIterator.nextObjectNoAdvance())) {
				...
				if ((UDATA)currentObject + entrySize > (UDATA)chunk->chunkTop) {
					/* Found an overlapping entry - fix the trailing portion that appears in the
					 * next chunk to be walkable */
					entrySize -= (UDATA)chunk->chunkTop - (UDATA)currentObject;
					memoryPool->abandonHeapChunk(
					        (void *)chunk->chunkTop, (void *)((UDATA)chunk->chunkTop + entrySize));
					break;
				}
        ...
        MM_EnvironmentBase *env, MM_MemoryPool *memoryPool, MM_ConcurrentSweepPoolState *sweepState)
{
	...
	/* Calculate the free memory approximation.  If there is no more connection work to take place, the
	 * approximation is 0 */
...
MM_ConcurrentSweepScheme::getPreviousSweepChunk(MM_EnvironmentStandard *env, MM_ConcurrentSweepPoolState *sweepState)
{
	...
	if (NULL != chunk) {
		...
		/* we can only step back the chunk for this sweep state if the chunk preceding us points to us as its
		 * next chunk (otherwise, it is associated with a different pool) */
 ...
 */
bool
MM_ConcurrentSweepScheme::incrementalSweepChunk(MM_EnvironmentStandard *env, MM_ParallelSweepChunk *chunk)
{
 ...
 */
bool
MM_ConcurrentSweepScheme::incrementalConnectChunk(MM_EnvironmentStandard *env, MM_ParallelSweepChunk *chunk,
 ...
 */
void
MM_ConcurrentSweepScheme::initializeStateForConnections(MM_EnvironmentBase *envModron,
        MM_MemoryPoolAddressOrderedList *memoryPool, MM_ConcurrentSweepPoolState *sweepState,
        MM_ParallelSweepChunk *chunk)
{
		...
		while (NULL != (chunk = getNextConnectChunk(envStandard, sweepState))) {
			...
			if (chunk->_concurrentSweepState == modron_concurrentsweep_state_swept) {
				if (incrementalConnectChunk(envStandard, chunk, sweepState, memoryPoolAOL)) {
					...
					/* Successfully connected the chunk.   Check if the allocation request can be
					 * satisfied. */
...
MM_ConcurrentSweepScheme::completeSweep(MM_EnvironmentBase *envBase, SweepCompletionReason reason)
{
	...
	/* Find the last free entry for each memory pool and set up the sweep state to continue connections at that
	 * point */
        ...
        MM_EnvironmentBase *envBase, MM_MemorySubSpace *memorySubSpace, UDATA minimumFreeSize)
{
		...
		while (poolNum < poolCount && NULL != (memoryPool = mssPoolIterator.nextPool())) {
			...
			if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
				...
				if (task->_foundLiveObject) {
					 ...
					 * sweepPreviousAvailableChunk call above to point to the next available
					 * predecessor.  This will be the chunk prior to the last swept chunk. If
					 * sweepState->_currentSweepChunkReverse is now pointing to NULL, then all the
					 * chunks have been swept; Otherwise the last swept chunk will be the next chunk
					 * after sweepState->_currentSweepChunkReverse.
					 */
					if (NULL == sweepState->_currentSweepChunkReverse) {
						if (NULL != sweepState->_currentSweepChunk) {
							/* Start propagating projections from the first chunk in the
							 * pool */
					...
					/* There were no live objects in this pool, we still need to ensure that the
					 * first chunk in the next subpool will have enough information to be walkable
					 * without needing to walk objects in this subpool, but only if this pool had
					 * any chunks.
					 */
					if (NULL != sweepState->_currentSweepChunk) {
	...
	if (NULL != memoryPool) {
		if (env->isMasterThread()) {
			/* Master thread: connect chunks until the correct entry is found or there are no more chunks to
			 * connect. If an entry is found, set the task flag for slave termination to true
			 */
			 ...
			 * Note: We might want to have a different flagging system that allows a slave to find the entry
			 * from its per-chunk statistics.
			 */
			MM_ConcurrentSweepPoolState *sweepState =
			        (MM_ConcurrentSweepPoolState *)getPoolState(memoryPool);
...
MM_ConcurrentSweepScheme::increaseActiveSweepingThreadCount(MM_EnvironmentBase *envModron, bool completeSweeping)
{
	...
	if (completeSweeping) {
		/* Sweeping could have been completed (but since we have access, after the check above, all
		 * sweep/connection activity cannot) while acquiring the lock - check again.
		 */
		 ...
		 * execution mode */
		/* NOTE: We can't use the threadcount here to check as other threads not actively completing the sweep
		 * may have bumped the count above zero (sweeping to connect, general allocation tax that missed
		 * kickoff, etc).
		 */
		if (concurrentsweep_mode_on == _stats._mode) {
			OMRPORT_ACCESS_FROM_OMRPORT(envModron->getPortLibrary());
			_stats.switchMode(
			        concurrentsweep_mode_on, concurrentsweep_mode_completing_sweep_phase_concurrently);
...
MM_ConcurrentSweepScheme::decreaseActiveSweepingThreadCount(MM_EnvironmentBase *envModron, bool completeSweeping)
{
	...
	if (_stats.isCompletingSweepConcurrently()) {
		/* We've completed a full pass through the memory pools, and done all the work we could.  If we're the
		 * last thread leaving the operation, then we must have successfully completed the sweeping phase (set
		 * the flag to say as much).
		 */
        ...
        MM_EnvironmentBase *envModron, MM_ParallelSweepChunk *startingChunkToPropagate)
{
	...
	while (NULL != currentChunk) {
		...
		if (projection > (((UDATA)currentChunk->chunkTop) - ((UDATA)currentChunk->chunkBase))) {
			currentChunk->projection = projection
        ...
        MM_EnvironmentBase *envModron, MM_ParallelSweepChunk *startingChunk, bool isFirstChunkInSubpool)
{
	...
	do {
		...
		/* If the chunk has a projection, then the next chunks starting walk point will be well defined (there
		 * is no leading free entry that may have started in a previous chunk).  This chunk can be safely
		 * ignored.
		 */
		...
		/* If the chunk has a trailing free candidate then there must have been a live object within the chunk,
		 * or a projection into the chunk. This is a well defined starting point.  A walk is required to
		 * properly abandon any memory overlap as this chunk may be the chunk adjacent to the next subpool (ie:
		 * the last chunk in the SOA before the LOA chunks)
		 */
		if (NULL != currentChunk->trailingFreeCandidate) {
			...
			if (NULL != currentChunk->leadingFreeCandidate) {
				...
				if (((void *)(((UDATA)currentChunk->leadingFreeCandidate)
					...
					/* If this is the very first chunk in the pool (the pool was entirely empty.)
					 * It will be safe to walk from the beginning. */
					if (isFirstChunkInSubpool && (currentChunk == startingChunk)) {
						...
						/* A chunk has been previously processed -- it is safe to walk the chunk
						 * from the beginning, or at a projection point */

						/* It's possible to think that this chunk is completely empty, even if
						 * there is a projection, so factor in the previous chunk projection,
						 * and use this information to determine the walk starting point.
						 */
						...
						/* The startingChunk can be a chunk previous to the chunk found with the
						 * live object in it (as per comment in
						 * workThreadFindMinimumSizeFreeEntry.)  This must be a chunk before the
						 * chunk with the live object in it.  If there was no live object then
						 * this would have to be the first chunk in the subpool which would have
						 * been handle in the case above. It can be skipped because there will
						 * be a live object before the end of this subpools chunks. Do not flag
					...
					/* If the chunk does not have a trailing free entry and has a leading free
					 * entry, there must be a live object at the end of the chunk.  The live object
					 * can not overlap between chunks, otherwise there would have been a projection,
					 * which is handled above.  The next chunk will be walkable from the beginning
					 * of the chunk.  No walk needs to be done for the current chunk.
					 */
					CONSS_DEBUG_PRINTF(("live object terminates at end of chunk -- next chunk "
					                    "walkable from start\n"));
					chunkProcessed = true;
					continue;
				}
			} else {
				/* There is neither a trailing free entry or a leading free entry.  There must be a live
				 * object at the end of the chunk.  If the live object overlaps onto the next chunk
				 * there would have been a projection, which is handled above.  The next chunk will be
				 * walkable from the beginning of the chunk, so no walk needs to be done for this chunk.
				 */
				CONSS_DEBUG_PRINTF(("live objects at beginning and end of chunk -- next chunk walkable "
				                    "from start\n"));
				chunkProcessed = true;
				continue;
			}
        ...
        MM_EnvironmentBase *envModron, MM_ParallelSweepChunk *currentChunk, void *walkStart)
{
	...
	while (NULL != (currentObject = objectIterator.nextObjectNoAdvance())) {
		...
		/* If the currentObject (or dark matter) extends past the top of the chunk (overlaps multiple chunks)
		 * then abandon both halves. In order to get here, the currentChunk must not have a projection, so we
		 * know that the currentObject would be dead, and hence safe to abandon.
		 */
		if ((UDATA)currentObject + entrySize > (UDATA)currentChunk->chunkTop) {
			/* Found an overlapping dead entry */
			/* TODO: when splitting regions spanning more than two chunks all the inner chunks have abandon
			 * called twice. */
			entrySize -= (UDATA)currentChunk->chunkTop - (UDATA)currentObject;

			CONSS_DEBUG_PRINTF(
			        ("abandoning [%p - %p] and [%p - %p]\n", currentObject, currentChunk->chunkTop,
