 ...
 * made a typedef).
 */
enum { modron_concurrentsweep_state_unprocessed = 0, /**< has not been processed by the sweeper */
...
private:
	...
	omrthread_monitor_t _completeSweepingConcurrentlyLock; /**< Lock to gain access to number of threads completing
	                                                          sweep concurrently */
protected:
...
private:
	...
	MM_ParallelSweepChunk *getNextConnectChunk(
	        MM_EnvironmentStandard *env, MM_ConcurrentSweepPoolState *sweepPoolState);
	void initializeStateForConnections(MM_EnvironmentBase *envModron, MM_MemoryPoolAddressOrderedList *memoryPool,
	        MM_ConcurrentSweepPoolState *sweepState, MM_ParallelSweepChunk *chunk);
	bool incrementalConnectChunk(MM_EnvironmentStandard *env, MM_ParallelSweepChunk *chunk,
	...
	void workThreadFindMinimumSizeFreeEntry(
	        MM_EnvironmentBase *env, MM_MemorySubSpace *memorySubSpace, UDATA minimumFreeSize);
...
public:
	...
	virtual bool sweepForMinimumSize(MM_EnvironmentBase *env, MM_MemorySubSpace *baseMemorySubSpace,
	...
	void payAllocationTax(
	        MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, MM_AllocateDescription *allocDescriptionn);
