...
private:
	...
	struct SubAreaEntry {
		 ...
		 */
		enum State { init = 0, busy, ready, full, fixup_only, end_segment, end_heap };
	};
...
protected:
	...
	MM_HeapRegionManager *_rootManager; /**< The root region manager which holds the MM_HeapRegionDescriptor
	                                       instances which manage the properties of the regions */
	...
	SubAreaEntry *_subAreaTable; /**< Reference to the subAreaTable which is shared data from the
	                                SweepHeapSectioning */
...
protected:
	...
	void saveForwardingPtr(class CompactTableEntry &, omrobjectptr_t objectPtr, omrobjectptr_t forwardingPtr,
	        intptr_t &page, intptr_t &counter);

	omrobjectptr_t doCompact(MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace, omrobjectptr_t start,
	        omrobjectptr_t finish, omrobjectptr_t &deadObject, uintptr_t &objectCount, uintptr_t &byteCount,
	        bool evacuate);
	 ...
	 */
	void evacuateSubArea(MM_EnvironmentStandard *env, MM_HeapRegionDescriptorStandard *subAreaRegion,
	        SubAreaEntry *subAreaTableEvacuate, intptr_t i, uintptr_t &objectCount, uintptr_t &byteCount,
	        uintptr_t &skippedObjectCount);

	void moveObjects(MM_EnvironmentStandard *env, uintptr_t &objectCount, uintptr_t &byteCount,
	        uintptr_t &skippedObjectCount);
	 ...
	 */
	void fixupSubArea(MM_EnvironmentStandard *env, omrobjectptr_t firstObject, omrobjectptr_t finish,
	...
	void addFreeEntry(MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace,
	 ...
	 */
	MMINLINE omrobjectptr_t pageStart(uintptr_t i) const { return (omrobjectptr_t)(_heapBase + (i * sizeof_page)); }

	/**
	 * Return the next page
	 */
	MMINLINE omrobjectptr_t nextPage(omrobjectptr_t objectPtr) const { return pageStart(pageIndex(objectPtr) + 1); }
	 ...
	 */
	void rebuildMarkbitsInSubArea(MM_EnvironmentStandard *env, MM_HeapRegionDescriptorStandard *region,
