...
MM_ReferenceChainWalkerMarkMap::clearMapForRegions(MM_EnvironmentBase *env, bool commit)
{
	...
	while (NULL != (region = regionIterator.nextRegion())) {
		if (region->isCommitted()) {
			...
			/* NOTE: We calculate the low and high heap offsets, and build the mark map index and size
			 * values from these to avoid rounding errors (if we use the size, the conversion routine could
			 * get a different rounding result then the actual end address)
			 */
