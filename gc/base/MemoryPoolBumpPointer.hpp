...
class MM_MemoryPoolBumpPointer : public MM_MemoryPool {
	 ...
	 */
private:
	void *_allocatePointer; /**< The base address of the unused portion of the receiver and the next pointer which
	                           could be returned by an allocation request */
	...
	MM_HeapLinkedFreeHeader *_lastFreeEntry; /**< address of the last free entry in the pool; valid after
...
public:
	...
	MMINLINE uintptr_t getFreeMemoryAndDarkMatterBytes()
	{
		...
		/* note that actualFreeMemory can be measured as less than allocatableMemory if sweep calculated it
		 * since it won't see the very end of the pool if it is less than minimumFreeEntrySize.
		 */
	 ...
	 * @return The point address where the next object will be allocated or _topPointer if the pool cannot satisfy
	 * any more allocates
	 */
	...
	/**
	 * Returns the number of bytes remaining in the pool which can be used for allocation.  This is a different
	 * value than the total free memory of the pool because it does not count free memory between objects, only the
	 * amount of memory left after the bump pointer.
