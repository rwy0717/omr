...
bool MM_CardTable::initialize(MM_EnvironmentBase* env, MM_Heap* heap)
{
    ...
    if (memoryManager->createVirtualMemoryForMetadata(
            env, &_cardTableMemoryHandle, extensions->heapAlignment, cardTableSizeRequired)) {
...
uintptr_t MM_CardTable::calculateCardTableSize(MM_EnvironmentBase* env, uintptr_t heapSize)
{
    return MM_Math::roundToCeiling(
        sizeof(uint32_t), (MM_Math::roundToCeiling(CARD_SIZE, heapSize) / CARD_SIZE) * sizeof(Card));
}
...
bool MM_CardTable::commitCardTableMemory(MM_EnvironmentBase* env, Card* lowCard, Card* highCard)
{
    ...
    if (extensions->isFvtestForceCardTableCommitFailure()) {
        ...
        commited = memoryManager->commitMemory(
            &_cardTableMemoryHandle, (void*)lowCard, (uintptr_t)highCard - (uintptr_t)lowCard);
        if (!commited) {
            Trc_MM_CardTable_cardTableCommitFailed(
                env->getLanguageVMThread(), (void*)lowCard, (uintptr_t)highCard - (uintptr_t)lowCard);
        }
    }
    return commited;
}

bool MM_CardTable::decommitCardTableMemory(
    ...
    if (extensions->isFvtestForceCardTableDecommitFailure()) {
        ...
        if (lowAddress < highAddress) {
            ...
            decommited = memoryManager->decommitMemory(&_cardTableMemoryHandle, lowAddress,
                ((uintptr_t)highAddress) - ((uintptr_t)lowAddress), lowAddress, highAddress);
            if (!decommited) {
...
void* MM_CardTable::cardAddrToHeapAddr(MM_EnvironmentBase* env, Card* cardAddr)
{
    ...
    return (
        void*)((uintptr_t)getHeapBase() + (((uintptr_t)cardAddr - (uintptr_t)getCardTableStart()) << CARD_SIZE_SHIFT));
}

MMINLINE void MM_CardTable::cleanRange(MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner, Card* low, Card* high)
{
    Card* thisCard = low;
    Card* endCard = high;
...
void MM_CardTable::cleanCardTableForRange(
...
void MM_CardTable::cleanCardsInRegion(
    MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner, MM_HeapRegionDescriptor* region)
{
    uintptr_t oldVMState = env->pushVMstate(cardCleaner->getVMStateID());
...
uintptr_t MM_CardTable::clearCardsInRange(MM_EnvironmentBase* env, void* heapBase, void* heapTop)
{
    ...
    MM_EnvironmentBase* env, uintptr_t numaNode, void* baseOfHeapRange, void* topOfHeapRange)
{
    ...
    /* it only makes sense to call this if NUMA support is enabled - this is not made to be called from highly generic
     * code */
    ...
    return memoryManager->setNumaAffinity(
        &_cardTableMemoryHandle, numaNode, (void*)lowAddress, highAddress - lowAddress);
}
