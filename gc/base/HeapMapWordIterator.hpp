...
enum { J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP =
	        (J9MODRON_HEAP_SLOTS_PER_HEAPMAP_BIT * sizeof(uintptr_t) * sizeof(uintptr_t) * BITS_IN_BYTE) };
...
public:
	...
	MMINLINE MM_HeapMapWordIterator(MM_HeapMap *heapMap, void *heapCardAddress)
	{
		 ...
		 * Other threads modifying the mark map can cause inconsistencies and the iterator will return invalid
		 * results.
		 */
		volatile uintptr_t *mapPointer = (volatile uintptr_t *)(((uint8_t *)heapMap->getHeapMapBits())
		        + (heapOffsetInBytes / J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT));
