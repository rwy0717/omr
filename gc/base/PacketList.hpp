...
public:
	struct PacketSublist {
		...
		{
			if (!_lock.initialize(
			            env, &env->getExtensions()->lnrlOptions, "MM_PacketList:_sublists[]._lock")) {
				return false;
			}
			return true;
		}
...
private:
	 ...
	 */
	MMINLINE uintptr_t getSublistIndex(MM_EnvironmentBase *env) { return env->getEnvironmentId() % _sublistCount; }
