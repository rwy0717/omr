 ...
 * Instances of subclasses of this class may be instantiated in debugger extensions to iterate over out of process
 * regions.
 */
...
public:
    ...
    virtual void initializeObjectHeapBufferedIteratorState(
        MM_HeapRegionDescriptor* region, GC_ObjectHeapBufferedIteratorState* state) const = 0;
    ...
    virtual uintptr_t populateObjectHeapBufferedIteratorCache(
        omrobjectptr_t* cache, uintptr_t count, GC_ObjectHeapBufferedIteratorState* state) const = 0;
     ...
     */
    virtual void reset(
        MM_HeapRegionDescriptor* region, GC_ObjectHeapBufferedIteratorState* state, void* base, void* top) const = 0;
};
