 ...
 */
MM_MarkMap* MM_MarkMap::newInstance(MM_EnvironmentBase* env, uintptr_t maxHeapSize)
{
        ...
        if (region->isCommitted()) {
            ...
            while (0 != heapClearSizeRemaining) {
                ...
                uintptr_t heapCurrentClearSize
                    = (heapClearUnitSize > heapClearSizeRemaining) ? heapClearSizeRemaining : heapClearUnitSize;
                ...
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                     ...
                     * from these to avoid rounding errors (if we use the size, the conversion routine could get a
                     * different rounding result then the actual end address)
                     */
