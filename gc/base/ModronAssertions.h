...
/* Currently, there are startup errors pointed out by these assertions so only enable them on combination spec until
    ...
    do {                                                                                                      \
        if (!(__condition)) {                                                                                 \
            omrGcDebugAssertionOutput(                                                                        \
                __env__->getPortLibrary(), __env__->getOmrVMThread(), __message, __parameter1, __parameter2); \
            Assert_MM_unreachable();                                                                          \
        }                                                                                                     \
    } while (false)
...
#define Assert_GC_true_with_message3(__env__, __condition, __message, __parameter1, __parameter2, __parameter3)      \
    do {                                                                                                             \
        if (!(__condition)) {                                                                                        \
            omrGcDebugAssertionOutput(__env__->getPortLibrary(), __env__->getOmrVMThread(), __message, __parameter1, \
                __parameter2, __parameter3);                                                                         \
            Assert_MM_unreachable();                                                                                 \
        }                                                                                                            \
    } while (false)

/* Four printed parameters macro */
#define Assert_GC_true_with_message4(                                                                                \
    __env__, __condition, __message, __parameter1, __parameter2, __parameter3, __parameter4)                         \
    do {                                                                                                             \
        if (!(__condition)) {                                                                                        \
            omrGcDebugAssertionOutput(__env__->getPortLibrary(), __env__->getOmrVMThread(), __message, __parameter1, \
                __parameter2, __parameter3, __parameter4);                                                           \
            Assert_MM_unreachable();                                                                                 \
        }                                                                                                            \
    } while (false)
...
#define Assert_GC_true_with_message4(                                                        \
    ...
    Assert_MM_true(__condition)
#define Assert_MM_objectAligned(__env__, __pointer) \
    Assert_MM_true_internal((uintptr_t)(__pointer) & (__env__->getObjectAlignmentInBytes() - 1))
