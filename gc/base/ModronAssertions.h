	...
	do { \
		if (!(__condition)) { \
			omrGcDebugAssertionOutput( \
			        __env__->getPortLibrary(), __env__->getOmrVMThread(), __message, __parameter); \
...
#define Assert_GC_true_with_message2(__env__, __condition, __message, __parameter1, __parameter2) \
	do { \
		if (!(__condition)) { \
			omrGcDebugAssertionOutput(__env__->getPortLibrary(), __env__->getOmrVMThread(), __message, \
...
#define Assert_MM_objectAligned(__env__, __pointer) \
	do { \
		if ((uintptr_t)(__pointer) & (__env__->getObjectAlignmentInBytes() - 1)) { \
			Assert_GC_true_with_message2(__env__, false, \
