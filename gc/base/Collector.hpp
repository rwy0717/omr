...
class MM_Collector : public MM_BaseVirtual {
	 ...
	 */
private:
	uintptr_t _exclusiveAccessCount; /**< The number of times exclusive access requests have been made to use the
	                                    receiver */
...
protected:
	...
	virtual bool internalGarbageCollect(
	        MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace, MM_AllocateDescription *allocDescription) = 0;
	...
	virtual void internalPreCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
	        MM_AllocateDescription *allocDescription, uint32_t gcCode) = 0;
	 ...
	 * This routine is meant to be an initializer for meta state on a Gc cycle, such as the aggressiveness of
	 * collection, excessive GC calculation, and cycle state setup for the master thread.
	 * @param env Master GC thread.
	 * @param subSpace the memory subspace where the collection is occurring
	 * @param allocDescription Allocation description causing the GC (or detailing the GC request)
	 * @param gcCode High level reason for invoking the GC
	 */
	void preCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace, MM_AllocateDescription *allocDescription,
	        uint32_t gcCode);
...
public:
	...
	virtual bool heapRemoveRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
	        void *lowAddress, void *highAddress, void *validLowAddress, void *validHighAddress) = 0;
	...
	void *garbageCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *callingSubSpace,
	...
	virtual bool forceKickoff(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
	        MM_AllocateDescription *allocDescription, uint32_t gcCode);
	...
	/**
	 * Called to request the number of times the environment has incremented the exclusive access count of the
	 * receiver. This is most often used to determine if a GC completed while a thread was blocked requesting
	 * exclusive.
	 ...
	 * @return true if the collector implementation knows that the given objectPtr is marked (false if not or if the
	 * implementation doesn't know)
	 */
