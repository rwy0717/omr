...
MM_WorkPackets::initWorkPacketsBlock(MM_EnvironmentBase *env)
{
	...
	memset((void *)_packetsStart[_packetsBlocksTop], 0, totalHeaderSize); /* initialize only MM_Packet's to avoid
	                                                                         paging the rest of the block */
			...
			if (((NULL == env->_currentTask)
				...
				while (mustSyncThreadsAndExit
					...
					if (_inputListDoneIndex != doneIndex) {
						env->_workPacketStats.addToCompleteStallTime(
