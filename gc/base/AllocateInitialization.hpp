...
protected:
    const uintptr_t
        _allocationCategory; /**< language-defined object category used in GC_ObjectModel::initializeAllocation() */
...
public:
    ...
    enum ObjectAllocationFlags {
        allocate_tenured
        = OMR_GC_ALLOCATE_OBJECT_TENURED /**< select this flag to allocate in old space (gencon only) */
        ,
        allocate_no_zero_memory
        = OMR_GC_ALLOCATE_OBJECT_NON_ZERO_TLH /**< select this flag to inhibit zeroing of allocated heap memory */
        ,
        allocate_no_gc
        = OMR_GC_ALLOCATE_OBJECT_NO_GC /**< select this flag to inhibit collector cycle start on allocation failure */
        ,
...
protected:
public:
    MMINLINE static uintptr_t selectObjectAllocationFlags(bool indexable, bool tenured, bool noZeroMemory, bool noGc)
    {
        ...
        if (isAllocatable()) {
            ...
            if (isIndexable()) {
                heapBytes = env->_objectAllocationInterface->allocateArrayletSpine(
                    env, &_allocateDescription, _allocateDescription.getMemorySpace(), isGCAllowed());
            } else {
            ...
            if (NULL != heapBytes) {
                ...
                valgrindMempoolAlloc(
                    env->getExtensions(), (uintptr_t)heapBytes, _allocateDescription.getBytesRequested());
#endif /* defined(OMR_VALGRIND_MEMCHECK) */
                ...
                objectModel->setObjectFlags(
                    (omrobjectptr_t)heapBytes, OMR_OBJECT_METADATA_FLAGS_MASK, _allocateDescription.getObjectFlags());
                ...
                /* if object model initialization fails heapBytes will become floating garbage to be recovered in next
                 * GC */
                if (NULL != objectPtr) {
                    ...
                    /* if concurrent mark is enabled thread might have to pay tax - must save/restore allocated object
                     * in case of GC */
