...
private:
	...
	J9MemoryNodeDetail const *_affinityLeaders; /**< A subset of the nodes in _activeNodes which represent the nodes
	                                               which are offering both physical memory and CPU */
#endif /* defined (OMR_GC_VLHGC) */
	uintptr_t _affinityLeaderCount; /**< The number of elements in the _affinityLeaders array */
	uintptr_t _nextNUMAIndex; /**< The index into the _affinityLeaders array from which we must expand the next
	                             region */
