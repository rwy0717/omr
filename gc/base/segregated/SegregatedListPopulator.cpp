    ...
    omrobjectptr_t* cache, uintptr_t count, GC_ObjectHeapBufferedIteratorState* state) const
{
    ...
    if (NULL != startPtr) {
        GC_ObjectHeapIteratorSegregated iterator(state->extensions, startPtr, (omrobjectptr_t)state->data2,
            (MM_HeapRegionDescriptor::RegionType)state->data3, state->data4, state->includeDeadObjects,
