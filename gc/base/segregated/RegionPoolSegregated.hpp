...
class MM_RegionPoolSegregated : public MM_RegionPool {
	 ...
	 */
private:
	MM_HeapRegionManager *_heapRegionManager; /**< The Region Manager which contains all the descriptors used for
	                                             allocation (they contain the Memory Pools which manage their
	                                             underlying memory) */
	MM_SweepSchemeSegregated *_sweepScheme;

	MM_FreeHeapRegionList *_singleFreeList; /**< Singleton free regions. */
	MM_FreeHeapRegionList *_multiFreeList; /**< Contiguous free regions (may include regions that are actually
	                                          singletons). */
	MM_FreeHeapRegionList *_coalesceFreeList; /**< Free regions that might be coalescable, so avoid allocating from
	 ...
	 * most occupied.
	 */
	MM_LockingHeapRegionQueue *_smallAvailableRegions[OMR_SIZECLASSES_NUM_SMALL + 1][NUM_DEFRAG_BUCKETS]; /**<
	                                                                                                         Regions
	                                                                                                         that
	                                                                                                         are
	                                                                                                         available
	 ...
	 * allocated into.
	 */
	MM_HeapRegionQueue *_smallFullRegions[OMR_SIZECLASSES_NUM_SMALL + 1]; /**< Regions that have been allocated into
	                                                                         during this GC cycle. */

	/**
	 * @note The sweep regions will contain a mix of marked (live) and unmarked (dead) objects.
	 */
	MM_HeapRegionQueue *_smallSweepRegions[OMR_SIZECLASSES_NUM_SMALL + 1]; /**< Regions that are waiting to be swept
	                                                                          during this GC cycle. */
	 ...
	 * allocated into.
	 */
	MM_HeapRegionQueue *_arrayletFullRegions; /**< Arraylet regions that have been allocated into during this GC
	                                             cycle. */

	/**
	 * @note The sweep regions will contain a mix of marked (live) and unmarked (dead) objects.
	 */
	MM_HeapRegionQueue *_arrayletSweepRegions; /**< Arraylet regions that are waiting to be swept during this GC
	                                              cycle. */
	 ...
	 * allocated into.
	 */
	MM_HeapRegionQueue *_largeFullRegions; /**< Large object regions that have been allocated into during this GC
	                                          cycle. */

	/**
	 * @note The sweep regions will contain a mix of marked (live) and unmarked (dead) objects.
	 */
	MM_HeapRegionQueue *_largeSweepRegions; /**< Large object regions that are waiting to be swept during this GC
	                                           cycle. */
	...
	uintptr_t _splitAvailableListSplitCount; /* number of split available region queues per size class per
	                                            defragment bucket */
	uint8_t _skipAvailableRegionForAllocation[OMR_SIZECLASSES_NUM_SMALL + 1]; /* per size class flag to indicate if
	                                                                             there is any available regions left
	                                                                             for allocation for that size class
	                                                                           */
...
public:
	...
	MM_HeapRegionDescriptorSegregated *sweepAndAllocateRegionFromSmallSizeClass(
	        MM_EnvironmentBase *env, uintptr_t sizeClass);
	void enqueueAvailable(MM_HeapRegionDescriptorSegregated *region, uintptr_t sizeClass, uintptr_t occupancy,
	...
	void addFreeRegion(
	        MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region, bool alreadyFree = false);
