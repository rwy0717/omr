...
protected:
    ...
    MM_MemorySubSpace::AllocationType
        _allocationType; /**< Describes what type of work the allocation layer needs to perform to satisfy this request
                            (TLH versus object allocate, for example) */
...
public:
     ...
     * zero TLH).
     * We internally, decide which TLH this gets allocated from (depending if its dual TLH mode or not), and whether we
     * clear it or not */
    ...
    /**
     * Save the spine to this thread's "saved object" slot so that it will be used as a root and will be updated if the
     * spine moves. NOTE:  This call must be balanced by a following restoreObjects call
     */
