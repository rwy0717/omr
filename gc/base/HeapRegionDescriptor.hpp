...
public:
    enum RegionType {
        ...
        ADDRESS_ORDERED_IDLE = 6, /**< Represents regions that have an AOL MemoryPool but are not actively being used
                                     (that is, they can be made into other kinds of regions without penalty) */
        ADDRESS_ORDERED_MARKED = 7, /**< Represents regions that have an AOL MemoryPool and an accurate mark set in
                                       extensions->previousMarkMap */
        BUMP_ALLOCATED = 8, /**< Represents regions that have a BumpPointer MemoryPool */
        BUMP_ALLOCATED_IDLE = 9, /**< Represents regions that have a BumpPointer MemoryPool but are not actively being
                                    used (that is, they can be made into other kinds of regions without penalty) */
        BUMP_ALLOCATED_MARKED = 10, /**< Represents regions that have a BumpPointer MemoryPool and an accurate mark set
    ...
    uintptr_t _regionsInSpan; /**< most likely a temporary piece of data which, along with _nextInSet, builds a freelist
                                 (public for assertions) */
    MM_HeapRegionDescriptor*
        _headOfSpan; /**< in a spanning region, points to the HEAD of the span.  In non-spanning, points at this */
    void* _heapRegionDescriptorExtension; /**< Language-specific extension of per region descriptor can be attached by
                                             configuration delegate (MM_ConfigurationDelegate) */
...
private:
    ...
    MM_HeapRegionDescriptor*
        _previousRegionInSubSpace; /**< previous region descriptor in the list (used by MM_MemorySubSpaceGeneric) */
    MM_HeapRegionDescriptor*
        _nextRegionInSubSpace; /**< next region descriptor in the list (used by MM_MemorySubSpaceGeneric) */
...
public:
     ...
     */
    bool isAddressInRegion(const void* address) { return (address >= getLowAddress()) && (address < getHighAddress()); }
    ...
    /**
     * Allocate supporting resources (large enough to justify not to preallocate them for all regions at the startup)
     * when region is being committed.
