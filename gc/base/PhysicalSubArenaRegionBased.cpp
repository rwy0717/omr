 ...
 */
uintptr_t
MM_PhysicalSubArenaRegionBased::doExpandInSubSpace(
        MM_EnvironmentBase *env, uintptr_t expandSize, MM_MemorySubSpace *subspace)
{
	...
	/*
	 due to heap address for each numa node has to alignment with region size and page size, free region count might
	 be not distributed evenly among numa nodes. use numaNodeCountWithFreeRegions and areFreeRegionsForNode() to
	 inflate all available regions from numa nodes round robin until reaching expanded size.
	 */
	...
	while (didExpandBy < expandSize) {
		...
		Assert_MM_true((0 == _affinityLeaderCount)
		        || ((formerNodeIndex + 1) % _affinityLeaderCount) == _nextNUMAIndex);
	}
	validateNumaSymmetry(env);
	subspace->heapReconfigured(env);
