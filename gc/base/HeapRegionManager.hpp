...
protected:
    ...
    uintptr_t _totalHeapSize; /**< The size, in bytes, of all currently active regions on the heap (that is, both table
                                 descriptors attached to subspaces and aux descriptors in the list) */
...
public:
     ...
     * include the "gap"). Calls to this method have the side-effect of allocating internal data structures to manage
     * the regions backing this contiguous heap so a boolean return value is provided to inform the caller if these
     ...
     * @return true if this manager succeeded in initializing internal data structures to manage this heap or false if
     * an error occurred (this is generally fatal)
     */
    ...
    /**
     * @return the size, in bytes, of a single region in the contiguous heap (note that AUX regions are not consulted,
     * and spanning regions will be some multiple of this number)
     */
...
protected:
    ...
    /**
     * Implemented by sub-classes to allocate and initialize a single AUX region descriptor for the extent: [lowAddress,
     * highAddress).  This is required since the sub-class is allowed to implement descriptors as any sub-class of
     * MM_HeapRegionDescriptor and the caller cannot assume how its storage is managed. The returned descriptor will
     * later be passed to freeAuxiliaryRegionDescriptor when it is no longer needed.
     *
     * @param env The environment
    ...
    /**
     * Implemented by sub-classes to free (and otherwise dispose of) a single AUX region descriptor (region).  This is
     * required since the sub-class is allowed to implement descriptors as any sub-class of MM_HeapRegionDescriptor and
     * the caller cannot assume how its storage is managed.
     *
     * @param env The environment
    ...
    /**
     * Implemented by sub-classes to allocate and initialize the contiguous region descriptor table for contiguous heap
     * extent [lowHeapEdge, highHeapEdge). This is required since the sub-class is allowed to implement the descriptor
     * table as a contiguous array of any sub-class of MM_HeapRegionDescriptor and the caller cannot assume how its
     * storage is managed.  The returned descriptor will later be passed to freeRegionTable when it is no longer needed.
     *
     * @param env The environment
     * @param lowHeapEdge the lowest byte addressable in the contiguous heap (note that this might not be presently
     * committed)
    ...
    /**
     * Implemented by sub-classes to free (and otherwise dispose of) the contiguous region descriptor table starting at
     * tableBase.  This is required since the sub-class is allowed to implement the descriptor table as a contiguous
     * array of any sub-class of MM_HeapRegionDescriptor and the caller cannot assume how its storage is managed.
     *
     * @param env The environment
