 ...
 */
typedef bool (*MM_RegionDescriptorInitializer)(MM_EnvironmentBase *env, MM_HeapRegionManager *regionManager,
...
protected:
	...
	MM_LightweightNonReentrantReaderWriterLock _heapRegionListMonitor; /**< monitor that controls modification of
	                                                                      the heap region linked list */
	MM_HeapRegionDescriptor *_auxRegionDescriptorList; /**< address ordered doubly linked list for auxiliary heap
	                                                      regions */
	...
	uintptr_t _tableDescriptorSize; /**< The size, in bytes, of the HeapRegionDescriptor subclass used by this
	                                   manager */
	MM_RegionDescriptorInitializer _regionDescriptorInitializer; /**< A function pointer used to initialize a newly
	                                                                allocated HRD */
	MM_RegionDescriptorDestructor _regionDescriptorDestructor; /**< A function pointer used to destroy HRDs, or NULL
	                                                            */

	uintptr_t _totalHeapSize; /**< The size, in bytes, of all currently active regions on the heap (that is, both
	                             table descriptors attached to subspaces and aux descriptors in the list) */

public:
	void kill(MM_EnvironmentBase *env);

	MM_HeapRegionManager(MM_EnvironmentBase *env, uintptr_t regionSize, uintptr_t tableDescriptorSize,
	        MM_RegionDescriptorInitializer regionDescriptorInitializer,
	        MM_RegionDescriptorDestructor regionDescriptorDestructor);

	static MM_HeapRegionManager *newInstance(MM_EnvironmentBase *env, uintptr_t regionSize,
	...
	/**
	 * @return the size, in bytes, of a single region in the contiguous heap (note that AUX regions are not
	 * consulted, and spanning regions will be some multiple of this number)
	 */
	 ...
	 */
	void reassociateRegionWithSubSpace(
	...
	MMINLINE void resizeAuxillaryRegion(
	        MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region, void *lowAddress, void *highAddress)
	{
		writeLock();
...
protected:
	...
	/**
	 * Implemented by sub-classes to allocate and initialize a single AUX region descriptor for the extent:
	 * [lowAddress, highAddress).  This is required since the sub-class is allowed to implement descriptors as any
	 * sub-class of MM_HeapRegionDescriptor and the caller cannot assume how its storage is managed. The returned
	 * descriptor will later be passed to freeAuxiliaryRegionDescriptor when it is no longer needed.
	 *
	 * @param env The environment
	 * @param lowAddress the lowest byte addressable in the AUX region (note that this might not be presently
	 * committed)
	 * @param highAddress the byte after the highest byte addressable in the AUX region
	 * @return an instance of an implementation-defined MM_HeapRegionDescriptor sub-class on success or NULL on
	 * failure
	 */
	...
	/**
	 * Implemented by sub-classes to free (and otherwise dispose of) a single AUX region descriptor (region).  This
	 * is required since the sub-class is allowed to implement descriptors as any sub-class of
	 * MM_HeapRegionDescriptor and the caller cannot assume how its storage is managed.
	 *
	 * @param env The environment
	 * @param region The AUX region to be freed by the implementor (previously returned by
	 * allocateAuxiliaryRegionDescriptor)
	 */
	virtual void internalFreeAuxiliaryRegionDescriptor(MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region);

	/**
	 * Implemented by sub-classes to allocate and initialize the contiguous region descriptor table for contiguous
	 * heap extent [lowHeapEdge, highHeapEdge). This is required since the sub-class is allowed to implement the
	 * descriptor table as a contiguous array of any sub-class of MM_HeapRegionDescriptor and the caller cannot
	 * assume how its storage is managed.  The returned descriptor will later be passed to freeRegionTable when it
	 * is no longer needed.
	 *
	 * @param env The environment
	 * @param lowHeapEdge the lowest byte addressable in the contiguous heap (note that this might not be presently
	...
	/**
	 * Implemented by sub-classes to free (and otherwise dispose of) the contiguous region descriptor table starting
	 * at tableBase.  This is required since the sub-class is allowed to implement the descriptor table as a
	 * contiguous array of any sub-class of MM_HeapRegionDescriptor and the caller cannot assume how its storage is
	 * managed.
	 *
	 * @param env The environment
	 * @param tableBase The base of the region table to be freed by the implementor (previously returned by
	 * allocateAndInitializeRegionTable)
	 * @param tableElementCount The number of contiguous descriptor elements in the table
	 */
	virtual void internalFreeRegionTable(
