 ...
 * @return the actual amount of resize (having intptr_t return result will contain valid value only if contract/expand
 * size is half of maxOfUDATA)
 */
intptr_t MM_MemorySubSpaceUniSpace::performResize(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
 ...
 */
uintptr_t MM_MemorySubSpaceUniSpace::performContract(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    if (targetContractSize > maximumContractSize) {
        ...
        Trc_MM_MemorySubSpaceUniSpace_performContract_Event1(
            env->getLanguageVMThread(), targetContractSize, maximumContractSize, contractSize);
    } else {
    ...
    if (allocDescription) {
        ...
        if (allocDescription->getBytesRequested() > largestFreeChunk) {
            Trc_MM_MemorySubSpaceUniSpace_timeForHeapContract_Exit4(
                env->getLanguageVMThread(), allocDescription->getBytesRequested(), largestFreeChunk);
            _contractionSize = 0;
            return false;
        }
    }
    ...
    MM_EnvironmentBase* env, uintptr_t allocSize, bool ratioContract)
{
    ...
    /* If there is not enough memory to satisfy the alloc, don't contract.  If allocSize is greater than the total free
     * memory, the currentFree value is a large positive number (negative unsigned number calculated above).
     */
    if (allocSize > getApproximateActiveFreeMemorySize()) {
        ...
        uintptr_t maximumFreePercent = ratioContract
            ? OMR_MIN(_extensions->heapFreeMinimumRatioMultiplier + 5, _extensions->heapFreeMaximumRatioMultiplier + 1)
        ...
        if (currentFree > maximumFree) {
            ...
            uintptr_t targetHeapSize
                = ((currentHeapSize - currentFree) / (_extensions->heapFreeMaximumRatioDivisor - maximumFreePercent))
    ...
    MM_EnvironmentBase* env, uintptr_t bytesRequired, bool expandToSatisfy)
{
    ...
    minimumFree = (getActiveMemorySize() / _extensions->heapFreeMinimumRatioDivisor)
        * _extensions->heapFreeMinimumRatioMultiplier;
    ...
    if (desiredFree <= currentFree) {
         ...
         */
        expandSize = ((desiredFree - currentFree) / (100 - _extensions->heapFreeMinimumRatioMultiplier))
 ...
 */
uintptr_t MM_MemorySubSpaceUniSpace::checkForRatioExpand(MM_EnvironmentBase* env, uintptr_t bytesRequired)
{
    ...
    if (0 != actualSoftMx) {
        if ((minimumBytesRequired != 0) && ((activeMemorySize + minimumBytesRequired) > actualSoftMx)) {
            if (J9_EVENT_IS_HOOKED(env->getExtensions()->omrHookInterface, J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX)) {
                ALWAYS_TRIGGER_J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX(env->getExtensions()->omrHookInterface,
                    env->getOmrVMThread(), omrtime_hires_clock(), heap->getMaximumMemorySize(),
                    heap->getActiveMemorySize(), env->getExtensions()->softMx, minimumBytesRequired);
