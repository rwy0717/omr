 ...
 */
void
MM_MemorySubSpaceUniSpace::checkResize(
...
MM_MemorySubSpaceUniSpace::calculateTargetContractSize(MM_EnvironmentBase *env, uintptr_t allocSize, bool ratioContract)
{
	...
	/* If there is not enough memory to satisfy the alloc, don't contract.  If allocSize is greater than the total
	 * free memory, the currentFree value is a large positive number (negative unsigned number calculated above).
	 */
	if (allocSize > getApproximateActiveFreeMemorySize()) {
		...
		uintptr_t maximumFreePercent = ratioContract ? OMR_MIN(_extensions->heapFreeMinimumRatioMultiplier + 5,
		                                       _extensions->heapFreeMaximumRatioMultiplier + 1)
		                                             : _extensions->heapFreeMaximumRatioMultiplier + 1;
		uintptr_t maximumFree = (currentHeapSize / _extensions->heapFreeMaximumRatioDivisor)
		        * maximumFreePercent;
		...
		if (currentFree > maximumFree) {
			...
			if (currentHeapSize < targetHeapSize) {
				...
				/* If max contraction is less than a single region (minimum contraction granularity)
				 * round it up */
...
MM_MemorySubSpaceUniSpace::calculateExpandSize(MM_EnvironmentBase *env, uintptr_t bytesRequired, bool expandToSatisfy)
{
	...
	if (desiredFree <= currentFree) {
		/* Only expand if we didn't expand in last _extensions->heapExpansionStabilizationCount global
		 * collections */
		if (_extensions->isStandardGC() || _extensions->isMetronomeGC()) {
			uintptr_t gcCount = 0;
#if defined(OMR_GC_MODRON_STANDARD) || defined(OMR_GC_REALTIME)
        ...
        MM_EnvironmentBase *env, uintptr_t expandSize, uintptr_t minimumBytesRequired)
{
	...
	if (0 != actualSoftMx) {
		if ((minimumBytesRequired != 0) && ((activeMemorySize + minimumBytesRequired) > actualSoftMx)) {
			if (J9_EVENT_IS_HOOKED(
			            env->getExtensions()->omrHookInterface, J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX)) {
