...
void MM_MemoryManager::kill(MM_EnvironmentBase* env) { env->getForge()->free(this); }
...
#define TWO_GB_ADDRESS ((void*)((uintptr_t)2 * 1024 * 1024 * 1024))
        ...
        if (extensions->isConcurrentScavengerHWSupported()) {
            ...
            if ((requestedTopAddress > ceiling)
                 ...
                 * no idea about start address. So to guarantee an allocation up to the ceiling we need to request
                 * extended chunk of memory. Set ceiling to NULL to disable ceiling control. This required bottom-up
...
#define THIRTY_TWO_GB_ADDRESS ((uintptr_t)32 * 1024 * 1024 * 1024)
                        if (requestedTopAddress <= (void*)THIRTY_TWO_GB_ADDRESS) {
                            /*
                             * If requested object heap size is in range 28G-32G its allocation with 3-bit shift might
                             * compromise amount of low memory below 4G To prevent this go straight to 4-bit shift if it
                             * possible. Set of logical conditions to skip allocation attempt below 32G
                            ...
                            if (!skipAllocationBelow32G) {
                                ...
                                instance = MM_VirtualMemory::newInstance(env, heapAlignment, allocateSize, pageSize,
                                    ...
                                    (void*)OMR_MIN((uintptr_t)THIRTY_TWO_GB_ADDRESS, (uintptr_t)ceiling), mode, options,
                                    memoryCategory);
                            }
                        }
