...
protected:
    ...
    uintptr_t
        _managedAllocationContextCount; /**< The number of allocation contexts which will be instantiated and managed by
                                           the GlobalAllocationManager that are part of the _managedAllocationContexts
                                           array, other allocation contexts may exist in certain specs */
...
public:
    ...
    virtual void printAllocationContextStats(
        MM_EnvironmentBase* env, uintptr_t eventNum, J9HookInterface** hookInterface)
        = 0;
    ...
    /**
     * Instructs the receiver to call flushForShutdown on all the contexts it manages (meant to be called prior to shut
     * down).
     */
