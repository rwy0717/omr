...
void MM_MarkingScheme::masterCleanupAfterGC(MM_EnvironmentBase* env) { _delegate.masterCleanupAfterGC(env); }
 ...
 */
void MM_MarkingScheme::markLiveObjectsRoots(MM_EnvironmentBase* env) { _delegate.scanRoots(env); }
 ...
 *  @param[in] env - passed Environment
 */
void MM_MarkingScheme::markLiveObjectsComplete(MM_EnvironmentBase* env) { _delegate.workerCompleteGC(env); }
...
void MM_MarkingScheme::assertNotForwardedPointer(MM_EnvironmentBase* env, omrobjectptr_t objectPtr)
{
    /* This is an expensive assert - fetching class slot during marking operation, thus invalidating benefits of leaf
     * optimization.
    ...
    if (_extensions->isConcurrentScavengerEnabled()) {
        ...
        /* It is ok to encounter a forwarded object during overlapped concurrent scavenger/marking (or even root
         * scanning), but we must do nothing about it (if in backout, STW global phase will recover them).
         */
        Assert_GC_true_with_message3(env,
            ((NULL == forwardPtr)
                || (!_extensions->getGlobalCollector()->isStwCollectionInProgress()
                       && _extensions->isConcurrentScavengerInProgress())),
