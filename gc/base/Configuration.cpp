	...
	if (initializeRegionSize(env) && initializeArrayletLeafSize(env)) {
		if (_delegate.initialize(env, _writeBarrierType, _allocationType)) {
			...
			if (initializeNUMAManager(env)) {
				...
				extensions->_lightweightNonReentrantLockPool = pool_new(sizeof(J9ThreadMonitorTracing),
				        0, 0, 0, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_MM,
...
MM_Configuration::createHeap(MM_EnvironmentBase *env, uintptr_t heapBytesRequested)
{
	...
	if (NULL != heap) {
		...
		/* VM Design 1869: kill the heap if it was allocated but not in the area requested by the fvtest options
		 * and then let it fall through to the normal error handling */
...
MM_Configuration::initializeRunTimeObjectAlignmentAndCRShift(MM_EnvironmentBase *env, MM_Heap *heap)
{
	...
	if (env->compressObjectReferences()) {
		...
		if (heapTop <= (maxAddressValue << shift)) {
			...
			/* impossible geometry:  use an assert for now but just return false once we are done testing
			 * the shifting */
			Assert_MM_unreachable();
			return false;
		}
#if !defined(S390) && !defined(J9ZOS390)
		/* s390 benefits from smaller shift values but other platforms don't so just force the shift to 3 if it
		 * was not 0 to save on testing resources. (still honour the "canChangeShift" flag, though, since we
		 * want our testing options to still work)
		 */
...
void
MM_Configuration::prepareParameters(OMR_VM *omrVM, uintptr_t minimumSpaceSize, uintptr_t minimumNewSpaceSize,
        ...
        MM_InitializationParameters *parameters)
{
	...
	minimumSpaceSize = OMR_MAX(
	        MM_Math::roundToCeiling(alignment, minimumSpaceSize), minimumNewSpaceSize + minimumTenureSpaceSize);
	memoryMax =
...
MM_Dispatcher *
MM_Configuration::createDispatcher(
