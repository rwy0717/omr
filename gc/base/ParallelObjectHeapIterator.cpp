...
GC_ParallelObjectHeapIterator::getNextChunk()
{
	...
	while (_segmentChunkIterator.nextChunk(_markMap, &_chunkBase, &_chunkTop)) {
		if (J9MODRON_HANDLE_NEXT_WORK_UNIT(_env)) {
			/* _chunkTop will be used later. Reseting the top address of the iterator to the end of the
			 * segment, so that iteration of the current chunk may go beyond _chunkTop, until first marked
			 * object is found. Thus we also iterate through dead objects at the begining of the each chunk.
			 * Dead objects at the beginning of the first chunk are also iterated - it is guarantied by
			 * setting _chunkBase for the first chunk to the segment base. */
...
GC_ParallelObjectHeapIterator::nextObject()
{
	...
	while (true) {
		 ...
		 * get the next chunk */
		/* Note:  we can use the mark map directly, in this case, since we know that nextObject must be on the
		 * heap */
