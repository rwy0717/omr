...
public:
	enum CollectionType {
		CT_GLOBAL_GARBAGE_COLLECTION = 0, /**< Global tracing to create a valid mark map across the entire heap
		                                     (with possible reclamation) */
		CT_PARTIAL_GARBAGE_COLLECTION = 1, /**< Partial heap trace / reclaim */
		CT_GLOBAL_MARK_PHASE = 2, /**< Global mark phase type (performs a full heap trace to build a fresh mark
		                             map) */
	};

	MM_GCCode _gcCode;
	MM_MemorySubSpace *_activeSubSpace;
	MM_CollectionStatistics *_collectionStatistics; /**< Pointer to a particular instance of common collect stats
	                                                   (memory, duration etc) */
	...
	} _markDelegateState; /**< The state of the MarkDelegate's incremental state machine (part of the cycle since
	                         multiple cycles could be in different places in the machine) */

	struct {
		uintptr_t _survivorSetRegionCount; /**< Count of regions Copy Forward used as destination. Effective
		                                      usage of regions is accounted, thus the count is not a cardinal
		                                      number. */
	} _pgcData;

	uintptr_t _currentIncrement; /**< The index of the current increment within the cycle, starting at 0 (used for
	                                event reporting) */

	bool _shouldRunCopyForward; /**< True if this cycle is to run a copy-forward-based attempt at reclaiming memory,
	                               false if mark-compact is to be used */
	...
	enum ReasonForMarkCompactPGC {
		...
		reason_recent_abort = 3, /**< Playing it safe until a GMP completes since a Copy-Forward abort recently
		                            occurred */
	...
	MM_CycleState *_externalCycleState; /**< Another state which overlaps with this and needs to be fixed up to
	                                       reflect any changes to the heap made under this cycle */
	uintptr_t _type;
	uintptr_t _verboseContextID; /**< the verbose context id for this cycle.  This should only be read/written to by
	                                verbose gc */

	enum ReferenceObjectOptions {
		references_default = 0x00, /**< default behaviour for all references */
		references_clear_weak = 0x01, /**< clear the referent in any WeakReferences (set after WeakReference
		                                 processing) */
		references_clear_soft = 0x02, /**< clear the referent in any SoftReferences (set after SoftReference
		                                 processing) */
		references_clear_phantom = 0x04, /**< clear the referent in any PhantomReferences (set after
		                                    PhantomReference processing) */
		references_soft_as_weak = 0x08 /**< treat SoftReferences as weak (ignore the age) (set for OOM
		                                  collections) */
	};
