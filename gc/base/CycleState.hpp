...
public:
    enum CollectionType {
        ...
        = 0, /**< Global tracing to create a valid mark map across the entire heap (with possible reclamation) */
    ...
    MM_CollectionStatistics*
        _collectionStatistics; /**< Pointer to a particular instance of common collect stats (memory, duration etc) */
    ...
    struct {
        uintptr_t _survivorSetRegionCount; /**< Count of regions Copy Forward used as destination. Effective usage of
                                              regions is accounted, thus the count is not a cardinal number. */
    } _pgcData;

    uintptr_t _currentIncrement; /**< The index of the current increment within the cycle, starting at 0 (used for event
                                    reporting) */
    ...
    enum ReasonForMarkCompactPGC {
        ...
        reason_recent_abort
        = 3, /**< Playing it safe until a GMP completes since a Copy-Forward abort recently occurred */
    ...
    MM_CycleState* _externalCycleState; /**< Another state which overlaps with this and needs to be fixed up to reflect
                                           any changes to the heap made under this cycle */
    uintptr_t _type;
    ...
    enum ReferenceObjectOptions {
        ...
        = 0x01, /**< clear the referent in any WeakReferences (set after WeakReference processing) */
        references_clear_soft
        = 0x02, /**< clear the referent in any SoftReferences (set after SoftReference processing) */
        references_clear_phantom
        = 0x04, /**< clear the referent in any PhantomReferences (set after PhantomReference processing) */
