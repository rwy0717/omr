...
MM_MemoryPoolLargeObjects::newInstance(MM_EnvironmentBase *env, MM_MemoryPoolAddressOrderedListBase *largeObjectArea,
...
MM_MemoryPoolLargeObjects::resizeLOA(MM_EnvironmentBase *env)
{
	...
	if ((_soaFreeBytesAfterLastGC < minimumSOAFreeRequired) && (LOA_EMPTY != _currentLOABase)) {
		 ...
		 * largeObjectAreaInitialRatio is considered optimal and will be used as long as free SOA meets min free
		 * ratio requirement. If, for example, SOA free is only 40% of the minimum, than we will contract LOA to
		 * 40% of the optimal size.
		 */
		...
		if (newLOAsize < _loaSize) {
			...
			/* The above formula makes sense if LOA is completely free. But if LOA is already partially
			 * occupied than we want to contract LOA even less (since LOA clearly shows being useful).
			 *
			 * The size we contract by is proportionate to free LOA ratio. If, for example, LOA is
			 * completely full (no free memory), we will not contract LOA at all.
			 *
			 * We do not just take the current LOA occupancy, but we look into the history of several global
			 * GCs and take the highest LOA occupancy.
			 *
			 */
			...
			if (0 < contractRequired) {
				...
				newLOABase = (void *)MM_Math::roundToFloor(
				        _extensions->heapAlignment, (uintptr_t)newLOABase);
 ...
 */
void *
MM_MemoryPoolLargeObjects::collectorAllocateTLH(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
...
void
MM_MemoryPoolLargeObjects::expandWithRange(
...
void *
MM_MemoryPoolLargeObjects::contractWithRange(
...
MM_MemoryPoolLargeObjects::redistributeFreeMemory(MM_EnvironmentBase *env, uintptr_t newOldAreaSize)
{
	...
	_loaSize = MM_Math::roundToCeiling(
	        _extensions->heapAlignment, (uintptr_t)((float)newOldAreaSize * _currentLOARatio));
