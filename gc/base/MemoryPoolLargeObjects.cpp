...
void MM_MemoryPoolLargeObjects::resizeLOA(MM_EnvironmentBase* env)
{
    ...
    if ((_soaFreeBytesAfterLastGC < minimumSOAFreeRequired) && (LOA_EMPTY != _currentLOABase)) {
        ...
        if (newLOAsize < _loaSize) {
            ...
            if ((double)newLOAsize / (double)_memorySubSpace->getActiveMemorySize()
                < _extensions->largeObjectAreaMinimumRatio) {

                contractRequired = (uintptr_t)((double)_loaSize
                    - ((double)(_memorySubSpace->getActiveMemorySize() * _extensions->largeObjectAreaMinimumRatio)));
                newLOAsize = _loaSize - contractRequired;
