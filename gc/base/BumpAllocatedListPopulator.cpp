...
void
MM_BumpAllocatedListPopulator::reset(
        MM_HeapRegionDescriptor *region, GC_ObjectHeapBufferedIteratorState *state, void *base, void *top) const
{
	...
	/* note that walking from low address to allocation pointer is only valid if the region has been freshly taken
	 * from eden (since copy-forward or class unloading could make this space unwalkable.
	 */
	Assert_MM_true(MM_HeapRegionDescriptor::BUMP_ALLOCATED == region->getRegionType());
	state->data2 = OMR_MIN((uintptr_t)top,
	        (uintptr_t)((MM_MemoryPoolBumpPointer *)region->getMemoryPool())->getAllocationPointer());
}
