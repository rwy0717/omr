...
{
	/* Avoid using OMR::GC::Forge to free memory for the extension, since OMR::GC::Forge was not used to allocate
	 * the memory */
	OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
	tearDown(env);
...
bool
MM_GCExtensionsBase::validateDefaultPageParameters(
...
void
MM_GCExtensionsBase::identityHashDataAddRange(
...
void
MM_GCExtensionsBase::identityHashDataRemoveRange(
        MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress, void *highAddress)
{
	/* empty */
}
