...
public:
	...
	/**
	 * The type of allocation we were trying to perform when calling allocationRequestFailed.  This is used to
	 * determine what call-out function to use to restart the allocation after we have dealt with the failure
	 */
	enum AllocationType {
	...
	/* Subclass may or may not implement this. MemorySubSpace that have children may implement this where merged
	 * stats will be reported. */
	 ...
	 * collection to reclaim resources.  The given "allocationType" variable is consulted to determine which
	 * allocation routine is to be called, after the collection, but before giving up exclusive to ensure that the
	 * failing allocation succeeds before other allocations can be attempted.
	 *
	...
	/**
	 * A generic helper which is used by most implementations of allocationRequestFailed to issue the actual
	 * allocation request between GC attempt stages.  It decodes the allocationType to determine which actual
	 ...
	 * @param[in] objectAllocationInterface The allocation interface through which the allocate request was issued
	 * (only relevant if the allocationType is TLH, otherwise this can be NULL)
	 * @param[in] attemptSubspace The subspace in which to issue the allocation call
	 *
	 * @return The address of the allocation or NULL on failure (in the case of Object or Leaf allocation, this is
	 * the address of the allocate.  In the case of TLH, this is the base address of the TLH)
	 */
	...
	virtual void *allocateObject(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
	        MM_MemorySubSpace *baseSubSpace, MM_MemorySubSpace *previousSubSpace, bool shouldCollectOnFailure) = 0;
	...
	/**
	 * Called under exclusive to request that the subspace attempt to replenish the given context and satisfy the
	 * given allocateDescription of the given allocationType
	 *
	 * @param[in] env The current thread
	...
	virtual void *collectorAllocateTLH(MM_EnvironmentBase *env, MM_Collector *requestCollector,
	        MM_AllocateDescription *allocDescription, uintptr_t maximumBytesRequired, void *&addrBase,
	        void *&addrTop);
	 ...
	 * Exclusive access was force acquired in order to perform a GC and guarantee a succesful allocation.  However,
	 * upon acquiring exclusive access, the allocation was satisfied WITHOUT having to perform a GC.  This routine
	 * is called under those circumstances.
	 *
