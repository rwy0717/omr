...
bool MM_HeapMap::initialize(MM_EnvironmentBase* env)
{
    ...
    if (memoryManager->createVirtualMemoryForMetadata(
            env, &_heapMapMemoryHandle, _extensions->heapAlignment, heapMapSizeRequired)) {
 ...
 *
 */
uintptr_t MM_HeapMap::convertHeapIndexToHeapMapIndex(MM_EnvironmentBase* env, uintptr_t offset, uintptr_t roundTo)
{
    if (!_useCompressedHeapMap) {
        return MM_Math::roundToCeiling(roundTo,
    ...
    if (_extensions->isFvtestForceMarkMapCommitFailure()) {
        ...
        if (!commited) {
            Trc_MM_HeapMap_markMapCommitFailed(env->getLanguageVMThread(),
                (void*)(((uintptr_t)_heapMapBits) + heapMapCommitOffset), heapMapCommitSize);
        }
    }
    return commited;
}
 ...
 * @return true if succeed
 */
bool MM_HeapMap::heapRemoveRange(MM_EnvironmentBase* env, uintptr_t size, void* lowAddress, void* highAddress,
    void* lowValidAddress, void* highValidAddress)
{
    ...
    if (_extensions->isFvtestForceMarkMapDecommitFailure()) {
        ...
        decommited = memoryManager->decommitMemory(&_heapMapMemoryHandle,
            (void*)(((uintptr_t)_heapMapBits) + heapMapCommitOffset), heapMapCommitSize, validHeapMapCommitLow,
 ...
 */
uintptr_t MM_HeapMap::numberBitsInRange(MM_EnvironmentBase* env, void* lowAddress, void* highAddress)
{
 ...
 */
uintptr_t MM_HeapMap::setBitsInRange(MM_EnvironmentBase* env, void* lowAddress, void* highAddress, bool clear)
{
    uintptr_t baseIndex, topIndex;
...
uintptr_t MM_HeapMap::setBitsForRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region, bool clear)
{
