...
protected:
    const uintptr_t _heapMapIndexShift; /* number of low-order bits to be shifted out of heap address to obtain heap map
                                           slot index */
    const uintptr_t
        _heapMapBitMask; /* bit mask for capturing bit index within heap map slot from (unshifted) heap address */
...
#define J9MODRON_HEAPMAP_SELECT_INDEX_SHIFT(compress) \
    (compress ? ((J9MODRON_HEAPMAP_LOG_SIZEOF_UDATA) + (OMR_SIZECLASSES_LOG_SMALLEST)) : (J9MODRON_HEAPMAP_INDEX_SHIFT))
