...
public:
    static const char* getConcurrentStatusString(
    ...
    MMINLINE void printAllocationTaxReport(OMR_VM* omrVM)
    {
        ...
        omrtty_printf("Concurrent mark analysis: Total Allocations: %zu Tax Paid 0%%: %zu 25%%: %zu 50%%: %zu  75%%: "
                      "%zu 100%%+: %zu\n",
            _allocationsTaxed, _allocationsTaxedAt0, _allocationsTaxedAt25, _allocationsTaxedAt50,
    ...
    {
        return _conHelperTraceSizeCount + _conHelperCardCleanCount + _traceSizeCount + _cardCleanCount;
    };
    ...
    MMINLINE void incrementCount(uintptr_t* count, uintptr_t increment) { MM_AtomicOperations::add(count, increment); };

    MMINLINE void clearCount(uintptr_t* count) { MM_AtomicOperations::set(count, 0); };
    ...
    {
        return (_completedModes & CONCURRENT_ROOT_TRACING) == CONCURRENT_ROOT_TRACING;
    };
    MMINLINE void setModeComplete(ConcurrentStatus mode)
    {
        ...
        do {
            currentValue
                = MM_AtomicOperations::lockCompareExchangeU32(&_completedModes, currentValue, currentValue | mask);
