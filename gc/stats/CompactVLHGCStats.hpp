...
public:
    ...
    uint64_t _regionCompactDataInitStartTime; /**< hires start time to initialize compact data in the region table (one
                                                 thread only) */
    uint64_t _regionCompactDataInitEndTime; /**< hires end time to initialize compact data in the region table (one
                                               thread only) */
    uint64_t
        _clearMarkMapStartTime; /**< hires start time to clear the next mark map to store data for the compact set */
    uint64_t _clearMarkMapEndTime; /**< hires end time to clear the next mark map to store data for the compact set */
    uint64_t _rememberedSetClearingStartTime; /**< hires start time to clear all RSCL from-references for the regions in
                                                 the compact set */
    uint64_t _rememberedSetClearingEndTime; /**< hires end time to clear all RSCL from-references for the regions in the
                                               compact set */
    ...
    uint64_t _reportMoveEventsStartTime; /**< hires start time to report all object move events (master thread only and
                                            only if OBJECT_RENAME is hooked) */
    uint64_t _reportMoveEventsEndTime; /**< hires end time to report all object move events (master thread only and only
                                          if OBJECT_RENAME is hooked) */
    uint64_t _fixupExternalPacketsStartTime; /**< hires start time to fixup work packets for an active GMP (only done if
                                                GMP is active) */
    uint64_t _fixupExternalPacketsEndTime; /**< hires end time to fixup work packets for an active GMP (only done if GMP
                                              is active) */
    uint64_t
        _fixupArrayletLeafStartTime; /**< hires start time to fixup arraylet leaf spine pointers (one thread only) */
    uint64_t _fixupArrayletLeafEndTime; /**< hires end time to fixup arraylet leaf spine pointers (one thread only) */
    uint64_t
        _recycleStartTime; /**< hires start time to identify and recycle fully evacuated regions (one thread only) */
    ...
    uint64_t
        _finalClearNextMarkMapStartTime; /**< hires start time to clear the next mark map after compact is finished */
    uint64_t _finalClearNextMarkMapEndTime; /**< hires end time to clear the next mark map after compact is finished */
    uint64_t _rebuildNextMarkMapStartTime; /**< hires start time to rebuild the next mark map from the next work packets
                                              (only done in GMP is active) */
    uint64_t _rebuildNextMarkMapEndTime; /**< hires end time to rebuild the next mark map from the next work packets
                                            (only done in GMP is active) */
    ...
    MMINLINE void addToMoveStallTime(uint64_t startTime, uint64_t endTime) { _moveStallTime += (endTime - startTime); }
    ...
    void merge(MM_CompactVLHGCStats* statsToMerge)
    {
        ...
        /* merging time intervals is a little different than just creating a total since the sum of two time intervals,
         * for our uses, is their union (as opposed to the sum of two time spans, which is their sum) */
        _setupStartTime = (0 == _setupStartTime) ? statsToMerge->_setupStartTime
                                                 : OMR_MIN(_setupStartTime, statsToMerge->_setupStartTime);
        _setupEndTime = OMR_MAX(_setupEndTime, statsToMerge->_setupEndTime);
        _moveStartTime = (0 == _moveStartTime) ? statsToMerge->_moveStartTime
                                               : OMR_MIN(_moveStartTime, statsToMerge->_moveStartTime);
        _moveEndTime = OMR_MAX(_moveEndTime, statsToMerge->_moveEndTime);
        _fixupStartTime = (0 == _fixupStartTime) ? statsToMerge->_fixupStartTime
                                                 : OMR_MIN(_fixupStartTime, statsToMerge->_fixupStartTime);
        _fixupEndTime = OMR_MAX(_fixupEndTime, statsToMerge->_fixupEndTime);
        _rootFixupStartTime = (0 == _rootFixupStartTime)
