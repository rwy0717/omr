...
class MM_RootScannerStats : public MM_Base {
    /* Data Members */
public:
    bool _statsUsed; /**< Flag that indicates if the owner thread used the stats for last increment (for any of its
                        roots) */
    uint64_t _entityScanTime[RootScannerEntity_Count]; /**< Time spent scanning each root scanner entity per thread.
                                                          Values of 0 indicate no time (regardless of clock resolution)
                                                          spent scanning. */
