...
public:
	...
	{
		/* CMVC 125876:  Note that time can go backward (core-swap, for example) so store a 1 as the delta if
		 * this is happening since we can't use 0 time deltas and negative deltas can cause arithmetic
		 * exceptions
		 */
		/* cache the ivars so that they aren't changed concurrently (not sure if this is a threat but better
		 * safe than sorry) */
