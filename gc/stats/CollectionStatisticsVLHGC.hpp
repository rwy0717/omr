...
public:
	...
	uintptr_t _arrayletUnknownObjects; /**< Count of non-contiguous arraylets of unknown type (if class is
	                                      unloaded)*/
	uintptr_t _arrayletUnknownLeaves; /**< Count (total) of arraylet leaves of unknown arraylet type (if class is
	                                     unloaded) */

	uintptr_t _rememberedSetCount; /**< Total count of remembered references (cards) */
	uintptr_t _rememberedSetBytesFree; /**< Count of buffers in global free pool (represented in bytes). Buffers in
	...
	uintptr_t _rememberedSetBeingRebuiltRegionCount; /**< Count of overflowed regions that are being rebuilt
	                                                    currently by GMP */
...
protected:
public:
	static MM_CollectionStatisticsVLHGC *getCollectionStatistics(MM_CollectionStatistics *stats)
	{
		return (MM_CollectionStatisticsVLHGC *)stats;
	}

	MMINLINE void collectCollectionStatistics(MM_EnvironmentBase *env, MM_CollectionStatisticsVLHGC *stats)
	{
