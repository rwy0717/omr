...
public:
    ...
    uintptr_t
        _copyDiscardBytesNonEden; /**< Bytes of survivor discarded by copy scan cache in non-Eden (age > 0) space */
    ...
    uintptr_t
        _objectsCardClean; /**< Objects scanned through card cleaning (either for copy, or re-scanned for abort) */
    ...
    uintptr_t _edenSurvivorRegionCount; /**< Counts the number of Eden regions allocated for survivor space in this
                                           copy-forward (only counted globally - unused in per-thread stats) */
    uintptr_t _nonEdenSurvivorRegionCount; /**< Counts the number of non-Eden regions allocated (that is, not tail-fill
                                              regions) for survivor space in this copy-forward (only counted globally -
    ...
    uint64_t
        _heapExpandedTime; /**< Time taken expanding the heap in order to complete the collection, in hi-res ticks */

    uintptr_t
        _externalCompactBytes; /**< The number of bytes selected for compaction outside of the copy-forward scheme.
                                  These would have been included in the copy-forward if there were sufficient space */

    uintptr_t _objectsScannedFromWorkPackets; /**< In depth-first copy-forward, the number of objects scanned by this
                                                 thread which it found on a work packet */
    uintptr_t _objectsScannedFromOverflowedRegion; /**< In depth-first copy-forward, the number of objects scanned by
                                                      this thread which it found in an overflowed region (while handling
                                                      work packet overflow) */
    uintptr_t _objectsScannedFromNextInChain; /**< In depth-first copy-forward, the number of objects scanned by this
                                                 thread which it found as the next child to scan from an object it was
                                                 already scanning (going "down" the tree) */
    uintptr_t _objectsScannedFromDepthStack; /**< In depth-first copy-forward, the number of objects scanned by this
                                                thread which it found on its depth stack (that is, one it resumed as it
                                                was going "up" the tree) */
    uintptr_t _objectsScannedFromRoot; /**< In depth-first copy-forward, the number of objects scanned by this thread
                                          which it found in the non-card root set */
