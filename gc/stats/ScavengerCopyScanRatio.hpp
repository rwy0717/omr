...
protected:
private:
    volatile uint64_t _accumulatingSamples; /**< accumulator for aggregating per thread wait/copy/s`wait/copy/scanes--
                                               these are periodically latched into _accumulatedSamples and reset */
    ...
    volatile uintptr_t _majorUpdateThreadEnv; /**< a token for the thread that has claimed a major update and owns the
                                                 critical region wherein the update is effected */
...
public:
    ...
    MMINLINE double getScalingFactor(MM_EnvironmentBase* env, UpdateHistory* historyRecord)
    {
        ...
        return getScalingFactor(
            env, threads, historyRecord->waits, historyRecord->copied, historyRecord->scanned, historyRecord->updates);
    }
        ...
        MM_EnvironmentBase* env, uint64_t* slotsScanned, uint64_t* slotsCopied, uint64_t waitingCount)
    {
        if (SCAVENGER_SLOTS_SCANNED_PER_THREAD_UPDATE <= *slotsScanned) {
            ...
            *slotsScanned = *slotsCopied = 0;
            ...
            /* this thread may have scanned a long array segment resulting in scanned/copied slot counts that must be
             * scaled down to avoid overflow in the accumulator */
            ...
            /* this next section includes a critical region for the thread that increments the update counter to
             * threshold */
            if (SCAVENGER_THREAD_UPDATES_PER_MAJOR_UPDATE == updateCount) {
                 ...
                 * this thread gets timesliced in the section below while other free-running threads work up another
                 * major update, that update will be discarded */
...
private:
        ...
        uint64_t scanned, uint64_t updates)
    {
        ...
        if (0 < updates) {
            ...
            /* quantize copy/scan counts and round up to nearest slot update rollover so ratio steps down in increments
             * from 1 */
            ...
            /* quantize average waiting thread counts and round up to nearest thread sample count so ratio steps down in
             * increments from 1 */
            if (0 < threadCount) {
    ...
    MMINLINE uint64_t sample(uint64_t slotsScanned, uint64_t slotsCopied, uint64_t waitingCount)
    {
        return (uint64_t)1 | (slotsScanned << SCAVENGER_SLOTS_SCANNED_SHIFT)
            | (slotsCopied << SCAVENGER_SLOTS_COPIED_SHIFT) | (waitingCount << SCAVENGER_THREAD_WAITS_SHIFT);
    }
