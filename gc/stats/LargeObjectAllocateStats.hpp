...
private:
...
#endif
	OMRSpaceSaving *_spaceSavingSizes; /**< Internal top-k-frequent data structure to maintain the stats for exact
	                                      sizes */
	OMRSpaceSaving *_spaceSavingSizeClasses; /**< Internal top-k-frequent data structure to maintain the stats for
	                                            size-classes */
	OMRSpaceSaving *_spaceSavingSizesAveragePercent; /**< Internal top-k-frequent data structure to maintain the
	                                                    stats for exact sizes */
	OMRSpaceSaving *_spaceSavingSizeClassesAveragePercent; /**< Internal top-k-frequent data structure to maintain
	...
	MM_FreeEntrySizeClassStats _freeEntrySizeClassStats; /**< global (still per pool) statistics structure for heap
	...
	uintptr_t _TLHSizeClassIndex; /**< preserved next value of sizeClassIndex on last invocation of
	...
	/**
	 * Large allocation stats are initially done only on out-of-line allocates (does not fit within current TLH), so
	 * we will miss to count those that fit within TLH This method extrapolate allocate stats to account even for
	 * those that fit within TLH. For given allocation size and total bytes allocated out of line, the method
	 * returns extrapolated total amount of bytes (including in-line and out-of-line) for that size. The
	 * extrapolation logic relies on distribution of TLH sizes that have been created, and calculates probability
	 * that the size fits in each of TLH size classes.
...
public:
	/* Based on free memory stats and allocation profile, simulate allocation and project remaining unused
	 * (fragmented) memory */
	...
	/**
	 * Encode 'native' precentage of free entry stats to a format internally used by SpaceSaving structure
	 * (uintptr_t)
	 */
	...
	static MM_LargeObjectAllocateStats *newInstance(MM_EnvironmentBase *env, uint16_t maxAllocateSizes,
	        uintptr_t largeObjectThreshold, uintptr_t veryLargeObjectThreshold, float sizeClassRatio,
	        uintptr_t maxHeapSize, uintptr_t tlhMaximumSize, uintptr_t tlhMinimumSize,
	...
	bool initialize(MM_EnvironmentBase *env, uint16_t maxAllocateSizes, uintptr_t largeObjectThreshold,
	        uintptr_t veryLargeObjectThreshold, float sizeClassRatio, uintptr_t maxHeapSize,
	...
	/* accumulate freeEntryCount from FreeEntrySizeClassStats and verify the count with memoryPool
	 * actualFreeEntryCount  */
