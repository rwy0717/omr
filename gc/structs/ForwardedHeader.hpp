...
private:
	...
	const uintptr_t _forwardingSlotOffset; /**< fomrobject_t offset from _objectPtr to fomrobject_t slot that will
	                                          hold the forwarding pointer */
#if defined(OMR_GC_COMPRESSED_POINTERS) && defined(OMR_GC_FULL_POINTERS)
	bool const _compressObjectReferences;
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) && defined(OMR_GC_FULL_POINTERS) */

	static const uintptr_t _forwardedTag = OMR_FORWARDED_TAG; /**< bit mask used to mark forwarding slot value as
	                                                             forwarding pointer */
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
	static const fomrobject_t _selfForwardedTag = (fomrobject_t)(_forwardedTag | OMR_SELF_FORWARDED_TAG);
	static const fomrobject_t _beingCopiedHint = (fomrobject_t)OMR_BEING_COPIED_HINT; /**< used in source object f/w
	                                                                                     pointer to hint that object
	                                                                                     might still be being copied
	                                                                                   */
	static const fomrobject_t _beingCopiedTag = (fomrobject_t)OMR_BEING_COPIED_TAG; /**< used in destination object,
	                                                                                   but using the same bit as
	                                                                                   _forwardedTag in source
	                                                                                   object */
	...
	static const uintptr_t _minIncrement = (131072 & _remainingSizeMask); /**< min size of copy section; does not
...
#define ForwardedHeaderDump(destinationObjectPtr)
	...
	/**
	 * Return the (strictly) forwarded version of the object, or NULL if the object has not been (strictly)
	 * forwarded.
	 */
	 ...
	 * - self-forwarded (pointing to itself, implicitly just by notion of a special self-forwarding bit in the
	 * header .
	 */
	...
	{
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
		/* Check the hint bit in the forwarding pointer itself, before fetching the master info in the
	...
	MMINLINE void restoreDestroyedOverlap()
	{
		restoreDestroyedOverlap(
		        ((MutableHeaderFields *)((fomrobject_t *)getForwardedObject() + _forwardingSlotOffset))
		                ->overlap);
	}
#endif /* defined(OMR_GC_COMPRESSED_POINTERS) */
	 ...
	 *
	 * This method will assert if the object is not reverse forwarded. Use isReverseForwardedPointer() to test
	 * before calling.
	 *
	 ...
	 *
	 * @param[in] objectPtr pointer to the object, which may or may not have been forwarded
	 * @param[in] forwardingSlotOffset fomrobject_t offset to uintptr_t size slot that will hold the forwarding
	 * pointer
	 */
