...
Tril::TRLangBuilder::toTRNode(const ASTNode *const tree)
{
	...
	if (opcode.isLoadConst()) {
		...
		if (opcode.isIntegerOrAddress()) {
			...
			case TR::Float: node->setFloat(value->get<float>()); break;
			case TR::Double: node->setDouble(value->get<double>()); break;
			default: return NULL;
			}
		...
		TR::ResolvedMethod *method = new (compilation->trHeapMemory()) TR::ResolvedMethod(
		        "file", "line", "name", argIlTypes.size(), &argIlTypes[0], returnIlType, targetAddress, 0);

		TR::SymbolReference *methodSymRef =
		        symRefTab()->findOrCreateStaticMethodSymbol(JITTED_METHOD_INDEX, -1, method);
...
Tril::TRLangBuilder::injectIL()
{
	...
	while (block) {
		...
		if (fallthroughArg != NULL) {
			...
			auto dest = _currentBlockNumber + 1 == numBlocks() ? cfg()->getEnd()
