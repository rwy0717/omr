...
intptr_t openLaunchSemaphore(OMRPortLibrary* portLibrary, const char* name, uintptr_t nProcess) { return -1; }
intptr_t ReleaseLaunchSemaphore(OMRPortLibrary* portLibrary, intptr_t semaphore, uintptr_t nProcess) { return -1; }
intptr_t WaitForLaunchSemaphore(OMRPortLibrary* portLibrary, intptr_t semaphore) { return -1; }
intptr_t CloseLaunchSemaphore(OMRPortLibrary* portLibrary, intptr_t semaphore) { return -1; }
    ...
    uint32_t options, OMRProcessHandle* processHandle)
{
    ...
    if (0 == CreatePipe(&(pipes.inR), &(pipes.inW), &sAttrib, 512)) {
        ...
        if (0
            == DuplicateHandle(GetCurrentProcess(), pipes.inW, GetCurrentProcess(), &(pipes.inDup), 0, FALSE,
    ...
    if (OMRPROCESS_DEBUG == options) {
        ...
        if (0 == CreatePipe(&(pipes.outR), &(pipes.outW), &sAttrib, 512)) {
            ...
            if (0
                == DuplicateHandle(GetCurrentProcess(), pipes.outR, GetCurrentProcess(), &(pipes.outDup), 0, FALSE,
                       DUPLICATE_SAME_ACCESS)) {
                closeAndDestroyPipes(&pipes);
                return OMRPROCESS_ERROR;
            }
        ...
        if (0 == CreatePipe(&(pipes.errR), &(pipes.errW), &sAttrib, 512)) {
            ...
            if (0
                == DuplicateHandle(GetCurrentProcess(), pipes.errR, GetCurrentProcess(), &(pipes.errDup), 0, FALSE,
                       DUPLICATE_SAME_ACCESS)) {
                closeAndDestroyPipes(&pipes);
                return OMRPROCESS_ERROR;
            }
