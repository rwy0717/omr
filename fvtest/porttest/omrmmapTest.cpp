...
#define J9MMAP_TEST1_BUFFER_SIZE 100
    ...
    if ((mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) == 0) {
        portTestEnv->log(
            LEVEL_ERROR, "OMRPORT_MMAP_CAPABILITY_PROTECT not supported on this platform, bypassing test\n");
        goto exit;
    }
...
TEST_F(PortMmapTest, mmap_test9)
{
    ...
    if (!omrsig_can_protect(signalHandlerFlags)) {
        portTestEnv->log(LEVEL_ERROR,
            "Signal handling framework not available, can't test readonly functionality without crashing, bypassing "
            "test");
        goto exit;
    } else {
...
TEST_F(PortMmapTest, mmap_test10)
{
    ...
    if ((uint64_t)fileLength != pageSize * 2) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "File is wrong length, should be %d but is %d\n", pageSize * 2, fileLength);
    }
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
    ...
    /* Category tests aren't entirely straightforward - because the mmap implementation may have done other allocations
     * as part of the map.*/
