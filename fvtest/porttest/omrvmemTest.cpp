    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (memPtr == NULL) {
            ...
            if (finalBlocks <= initialBlocks) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "vmem reserve didn't increment category block as expected. Final blocks=%zu, initial blocks=%zu, "
                    "page size=%zu.\n",
            ...
            if (finalBytes < (initialBytes + pageSizes[i])) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "vmem reserve didn't increment category bytes as expected. Initial bytes=%zu, final bytes=%zu, "
                    "page size=%zu.\n",
...

        {
            ...
            if (finalBytes != initialBytes) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "vmem free didn't decrement category bytes as expected. Initial bytes=%zu, final bytes=%zu, page "
                    "size=%zu.\n",
    ...
    uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
    ...
    for (i = 0; numIterations != i; i++) {
        ...
        if (rc != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrvmem_decommit_memory returned 0x%zx when trying to decommit 0x%zx bytes backed by 0x%zx-byte "
                "pages\n",
...
TEST(PortVmemTest, vmem_testReserveMemoryEx)
{
    ...
    if (memoryIsAvailable(OMRPORTLIB, FALSE)) {
        ...
        portTestEnv->log(LEVEL_ERROR,
            "***Did not find enough memory available on system, not running test omrvmem_testReserveMemoryEx\n");
    }
}
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        for (j = 0; j < NUM_SEGMENTS; j++) {
            ...
            if (NULL != memPtr[j]) {
                ...
                if (finalBlocks <= initialBlocks) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmem reserve didn't increment category block as expected. Final blocks=%zu, initial "
                ...
                if (finalBytes < (initialBytes + pageSizes[i])) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmem reserve didn't increment category bytes as expected. Final bytes=%zu, initial bytes=%zu, "
                        "page size=%zu.\n",
                        finalBytes, initialBytes, pageSizes[i]);
                }
            }
        ...
        for (j = 0; j < NUM_SEGMENTS; j++) {
            if (NULL != memPtr[j]) {
                ...
                if (finalBlocks >= initialBlocks) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmem free didn't decrement category blocks as expected. Final blocks=%zu, initial blocks=%zu, "
                        "page size=%zu.\n",
                        finalBlocks, initialBlocks, pageSizes[i]);
                }
                ...
                if (finalBytes > (initialBytes - pageSizes[i])) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmem free didn't decrement category bytes as expected. Final bytes=%zu, initial bytes=%zu, "
                        "page size=%zu.\n",
                        finalBytes, initialBytes, pageSizes[i]);
                }
            }
...
TEST(PortVmemTest, vmem_testReserveMemoryEx_zOSLargePageBelowBar)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (NULL == memPtr) {
            ...
            portTestEnv->log(
                "reserved and committed 0x%zx bytes with page size 0x%zx and page flag 0x%x at address 0x%zx\n",
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_zOSLargePageBelowBar)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (TWO_GB <= pageSizes[i]) {
             ...
             * If the page size is >= 2G, then attempt to allocate memory with OMRPORT_VMEM_STRICT_ADDRESS flag is
             * expected to fail.
             */
            continue;
        }
        ...
        if (NULL == memPtr) {
            ...
            if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
                if (FOUR_KB != vmemID.pageSize) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "\tdidn't expect to allocate memory below 2GB using page size 0x%zx and page flags 0x%x with "
                        "strict address\n",
...
TEST(PortVmemTest, vmem_testReserveLargePagesAboveBar)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        for (pageableStrict = 0; pageableStrict < 4;
            ...
            params.byteAmount = pageSizes[i];
            params.mode
                |= OMRPORT_VMEM_MEMORY_MODE_READ | OMRPORT_VMEM_MEMORY_MODE_WRITE | OMRPORT_VMEM_MEMORY_MODE_EXECUTE;
            params.pageSize = pageSizes[i];
            if (pageable) {
            ...
            if (NULL != memPtr) {
                ...
                if (pageable) {
                    if (0 != rc) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "omrvmem_decommit_memory failed with code 0x%zx when trying to decommit 0x%zx bytes backed "
                            "by 0x%zx-byte pages\n",
                    ...
                    if ((0 == rc) && strict) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "omrvmem_decommit_memory no error when trying to decommit fixed pages size=0x%zx "
                            "flags=0x%zx\n",
...
TEST(PortVmemTest, vmem_test_numa)
{
    ...
    if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
        ...
        if (nodeArray == NULL) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrvmem_free_memory returned NULL when trying to allocate J9MemoryNodeDetail array for NUMA test "
                "(FAIL)\n");
            goto exit;
        }
        ...
        if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "omrvmem_numa_get_node_details returned different results on second call (return %zd versus %zd, count "
                "%zu versus %zu)\n",
        ...
        for (i = 0; pageSizes[i] != 0; i++) {
            ...
            /* reserve and commit */
#if defined(J9ZOS390)
            /* On z/OS skip this test for newly added large pages as obsolete omrvmem_reserve_memory() does not support
             * them */
            if (isNewPageSize(pageSizes[i], pageFlags[i])) {
                continue;
            }
            ...
            if (memPtr != NULL) {
                ...
                for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
                    if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
                        ...
                        if (rc != 0) {
                            outputErrorMessage(PORTTEST_ERROR_ARGS,
                                "omrvmem_numa_set_affinity returned 0x%zx when trying to set affinity for 0x%zx bytes "
                                "at 0x%p to node %zu\n",
                ...
                /* ideally we'd test that the memory actually has some NUMA characteristics, but that's difficult to
                 * prove */
                ...
                if (rc != 0) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "omrvmem_free_memory returned 0x%zx when trying to free 0x%zx bytes backed by 0x%zx-byte "
                        "pages\n",
...
#define MODE_NOT_USED 0
    ...
    /* First get the page size of the data segment. This is the page size used by the JIT code cache if 16M Code Pages
     * are not being used */
#if defined(AIXPPC)
        ...
        for (j = 0; j < i; j++) {
            if (keepCycles[j] >= i) { /* If not already freed */
                if (checkForOverlap(&vmemID[i], &vmemID[j])) {
                    ...
                    keepCycles[i] = i
                        - 1; /* Pretend this segment is already freed to avoid double freeing overlapping segments */
                    result = -1;
                    goto exit;
                }
            }
