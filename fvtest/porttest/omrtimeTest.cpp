...
TEST(PortTimeTest, time_test1)
{
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
        ...
        if (mtimeStart == mtimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrtime_msec_clock did not change after a half-second forced delay\n");
        }
        if (utimeStart == utimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrtime_usec_clock did not change after a half-second forced delay\n");
        }
        if (ntimeStart == ntimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "omrtime_nano_time did not change after a half-second forced delay\n");
        }
...
TEST(PortTimeTest, time_test3)
{
    ...
    if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
        /* one CPU means we have no chance of falling into the "difference CPUs have different times" trap.  Let the
         * test begin */
        ...
        portTestEnv->log(LEVEL_ERROR,
            "Test is invalid since the host machine reports more than one CPU (time may differ across CPUs - makes "
            "test results useless - re-enable if we develop thread affinity support)\n");
    }
    portTestEnv->log("\n");
...
TEST(DISABLED_PortTimeTest, time_test4)
{
    ...
    for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
        ...
        if (!compareMillis(milliStartOuter, milliStartInner, EPSILON)
            ...
            || !compareMillis(milliEndInner, milliEndOuter, EPSILON) || (milliDeltaOuter < milliDeltaInner)) {
     ...
     */
    {
        ...
        if (QueryPerformanceCounter(&i)) {
            portTestEnv->log(LEVEL_WARN,
                "WARNING: Test is invalid since the host machine uses QueryPerformanceCounter() (time may differ "
                "across CPUs - makes test results useless - re-enable if we develop thread affinity support)\n");
            reportTestExit(OMRPORTLIB, testName);
        }
    }
#endif /* defined(OMR_OS_WINDOWS) */
            ...
            if (NULL != threads) {
                if (0 == omrthread_monitor_enter(tds.monitor)) {
                    for (i = 0; i < omrtimeTestDirectionNumThreads; i++) {
                        intptr_t rc = omrthread_create(
                            &threads[i], 128 * 1024, J9THREAD_PRIORITY_MAX, 0, &nanoTimeDirectionTest, &tds);
                        if (0 != rc) {
                    ...
                    if (tds.failed) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "omrtime_hires_clock() did not go forward in at least one of the launched threads");
                    }
