 ...
 */
static int32_t fake_mem_startup(struct OMRPortLibrary* portLibrary, uintptr_t portGlobalSize) { return failMemStartup; }
 ...
 * @return negative error code
 */
static int32_t fake_time_startup(struct OMRPortLibrary* portLibrary) { return failMemStartup; }
...
TEST(PortTest, port_test1)
{
    ...
    if (actualSize != expectedSize) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrport_getSize() returned %zu expected %zu\n", actualSize, expectedSize);
    }
...
TEST(PortTest, port_test3)
{
    ...
    if (failMemStartup != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "omrport_startup_library() returned %d expected %d\n", rc, failMemStartup);
    }
