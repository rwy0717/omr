...
static const Attribute deviceAttributes[] = {
        ...
        ATTRIBUTE_ENTRY(COMPUTE_MODE, 0, 3), ATTRIBUTE_ENTRY(CONCURRENT_KERNELS, 0, 1),
        ATTRIBUTE_ENTRY(ECC_ENABLED, 0, 1), ATTRIBUTE_ENTRY(GLOBAL_MEMORY_BUS_WIDTH, 128, 384),
        ATTRIBUTE_ENTRY(INTEGRATED, 0, 1), ATTRIBUTE_ENTRY(KERNEL_EXEC_TIMEOUT, 0, 1),
        ATTRIBUTE_ENTRY(L2_CACHE_SIZE, 0, 4 * 1024 * 1024), ATTRIBUTE_ENTRY(MAX_BLOCK_DIM_X, 512, 1024),
        ATTRIBUTE_ENTRY(MAX_BLOCK_DIM_Y, 512, 1024), ATTRIBUTE_ENTRY(MAX_BLOCK_DIM_Z, 64, 64),
        ATTRIBUTE_ENTRY(MAX_GRID_DIM_X, 65535, 2147483647), ATTRIBUTE_ENTRY(MAX_GRID_DIM_Y, 65535, 65535),
        ATTRIBUTE_ENTRY(MAX_GRID_DIM_Z, 1, 65535), ATTRIBUTE_ENTRY(MAXIMUM_SURFACE1D_LAYERED_LAYERS, 0, 2 * 1024),
        ...
        ATTRIBUTE_ENTRY(MULTIPROCESSOR_COUNT, 1, 64), ATTRIBUTE_ENTRY(PCI_BUS_ID, 0, 255),
        ATTRIBUTE_ENTRY(PCI_DEVICE_ID, 0, 31), ATTRIBUTE_ENTRY(PCI_DOMAIN_ID, 0, 255),
        ...
        ATTRIBUTE_ENTRY(TOTAL_CONSTANT_MEMORY, 64 * 1024, 64 * 1024), ATTRIBUTE_ENTRY(UNIFIED_ADDRESSING, 0, 1),
        ATTRIBUTE_ENTRY(WARP_SIZE, 32, 32), {NULL, (J9CudaDeviceAttribute)0, 0, 0} /* terminator */
};
...
TEST_F(CudaDeviceTest, cacheConfig)
{
	...
	for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
		...
		for (size_t index = 0; index < allCacheConfigs.length; ++index) {
			...
			ASSERT_EQ(0, rc) << "omrcuda_deviceSetCacheConfig(" << allCacheConfigs.data[index]
			                 << ") failed";
		}
	}
}
	...
	for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
		...
		for (const Limit *entry = deviceLimits; NULL != entry->name; ++entry) {
			...
			if (computeCapability < entry->minComputeCapability) {
				ASSERT_EQ(J9CUDA_ERROR_UNSUPPORTED_LIMIT, rc) << "omrcuda_deviceGetLimit should have "
				                                                 "failed";
			} else {
...
TEST_F(CudaDeviceTest, sharedMemoryConfig)
{
	...
	for (uint32_t deviceId = 0; deviceId < deviceCount; ++deviceId) {
		...
		for (size_t index = 0; index < allSharedMemConfigs.length; ++index) {
			...
			ASSERT_EQ(0, rc) << "omrcuda_deviceSetSharedMemConfig(" << allSharedMemConfigs.data[index]
			                 << ") failed";
		}
	}
}
