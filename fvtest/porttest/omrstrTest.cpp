 ...
 */
static void
validate_omrstr_vprintf(struct OMRPortLibrary *portLibrary, const char *testName, char *buffer, uintptr_t bufLength,
        const char *expectedResult, const char *format, va_list args)
{
	...
	if (strlen(buffer) != rc) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        ...
		        expectedResult, strlen(buffer), rc);
	}
 ...
 */
static void
validate_omrstr_vprintf_with_NULL(struct OMRPortLibrary *portLibrary, const char *testName, uint32_t bufLength,
        uint32_t expectedLength, const char *format, va_list args)
{
	OMRPORT_ACCESS_FROM_OMRPORT(portLibrary);
	uintptr_t rc;
        ...
        struct OMRPortLibrary *portLibrary, const char *testName, const char *format, const char *expectedResult, ...)
{
	...
	validate_omrstr_vprintf(
	        OMRPORTLIB, testName, actualResult, strlen(expectedResult) + 1, expectedResult, format, args);
	...
	if (strlen(expectedResult) > 1) {
		...
		validate_omrstr_vprintf(OMRPORTLIB, testName, actualResult, strlen(expectedResult),
		...
		if (strlen(expectedResult) > 2) {
			...
			validate_omrstr_vprintf(OMRPORTLIB, testName, actualResult, strlen(expectedResult) - 1,
			        truncatedExpectedResult, format, args);
		}
	}
	...
	validate_omrstr_vprintf_with_NULL(OMRPORTLIB, testName, (uint32_t)strlen(expectedResult),
 ...
 */
static void
test_omrstr_tokens(struct OMRPortLibrary *portLibrary, const char *testName, char *buf, uintptr_t bufLen,
...
TEST(PortStrTest, str_test2)
{
	...
	test_omrstr_vprintf(
	        OMRPORTLIB, testName, "%15ls", "  foo\xc2\x80\xe0\xa0\x80\xef\xbf\xbf\xc4\xa8", unicodeString);
	...
	test_omrstr_vprintf(
	        OMRPORTLIB, testName, "%-12ls", "\xc2\x80\xe0\xa0\x80\xef\xbf\xbf\xc4\xa8  ", unicodeString);
...
TEST(PortStrTest, str_test3)
{
	...
	if (ret < 3) {
		outputErrorMessage(
		        PORTTEST_ERROR_ARGS, "Undersized buffer not detected. Expected %d, got %d\n", 5, ret);
	}
	...
	test_omrstr_ftime(OMRPORTLIB, testName, buf, J9STR_BUFFER_SIZE,
	        "%Y %m %b %d %S %pid %uid %job %home %last %seq", timeMillis, expected);
	...
	test_omrstr_ftime(
	        OMRPORTLIB, testName, buf, J9STR_BUFFER_SIZE, "%Y %m %b %d %S %zzz = %%zzz", timeMillis, expected);
...
#define TEST_OVERFLOW_LEN 8
	...
	if (NULL == tokens) {
		...
		test_omrstr_tokens(
		        OMRPORTLIB, testName, buf, TEST_BUF_LEN, "Teststring %Y", tokens, "Teststring 2004", 15);
		/* Test 4: Start with a token */
		test_omrstr_tokens(
		        OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%y Teststring", tokens, "04 Teststring", 13);
		/* Test 5: Many tokens */
		...
		/* Test 7: %% and end with % */
		test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "99%% is the same as 99%", tokens,
		...
		test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y%%%m%%%d %zzz% %%%%%%", tokens,
		        "2004%02%29 %zzz% %%%", 20);
		...
		test_omrstr_tokens(
		        OMRPORTLIB, testName, bufOverflow, TEST_OVERFLOW_LEN, "Teststring", tokens, "Teststr", 11);
		/* Test 13: Test for overflow with an actual short buffer (token) */
		test_omrstr_tokens(
		        OMRPORTLIB, testName, bufOverflow, TEST_OVERFLOW_LEN, "Test %Y", tokens, "Test 20", 10);
		...
		test_omrstr_tokens(
		        OMRPORTLIB, testName, NULL, 0, "%yyy is not %yy is not %y", tokens, NULL, 33); /* 33
		                                                                                          because
		                                                                                          must
		                                                                                          include
		                                                                                          \0
		...
		/* Test 17: All tokens that we get for free in omrstr_create_tokens (and that the user may be relying
		 * on) */
		{
			...
			fullFormatString =
			        (char *)omrmem_allocate_memory(sizeOfFullFormatString, OMRMEM_CATEGORY_PORT_LIBRARY);
			 ...
			 * but make sure that something has been substituted for each token and that the time is
			 * substituted properly . Do this by making sure that %<token> is not present in the output
			 * buffer*/
			if (rc > TEST_BUF_LEN) {
				outputErrorMessage(PORTTEST_ERROR_ARGS,
...
#define TEST_OVERFLOW_LEN 8
	...
	portTestEnv->log("\n\tThe results of omrstr_test5 are not evaluated as time conversions are specific to a time "
	                 "zone.\n");
	...
	(void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
	portTestEnv->log(
	        "\tExpecting local time (in Ottawa, Eastern Standard Time): 2006/02/14 16:30:06 \n", timeMillis);
	...
	(void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
	portTestEnv->log(
	        "\tExpecting local time (in Ottawa, Eastern Daylight Time): 2006/06/14 17:30:06 \n", timeMillis);
	portTestEnv->log("\t                                       ... converted to: %s\n", buf);
	...
	(void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
	portTestEnv->log(
	        "\tExpecting local time (in Ottawa, Eastern Daylight Time): 2006/10/12 17:30:06 \n", timeMillis);
	portTestEnv->log("\t                                       ... converted to: %s\n", buf);
	...
	(void)omrstr_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
	portTestEnv->log(
	        "\tExpecting local time (in Ottawa, Eastern Standard Time): 2006/12/11 16:30:06 \n", timeMillis);
	portTestEnv->log("\t                                       ... converted to: %s\n", buf);
...
#define TEST_OVERFLOW_LEN 8
	...
	tokens = omrstr_create_tokens(timeMillis);
	test_omrstr_tokens(
	        OMRPORTLIB, testName, buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens, "1970/01/01 00:00:00", 19);
	reportTestExit(OMRPORTLIB, testName);
}
...
TEST(PortStrTest, str_convPlatTo8)
{
	...
	convertedStringLength = omrstr_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, platformString,
	...
	convertedStringLength = omrstr_convert(
	        J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, platformString, originalStringLength, NULL, 0);
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
...
TEST(PortStrTest, str_convLongString)
{
	...
	mutf8StringLength = omrstr_convert(
	        J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString, originalStringLength, NULL, 0);
	...
	mutf8StringLength = omrstr_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString,
	        originalStringLength, mutf8Buff, sizeof(mutf8Buff));
	if (mutf8StringLength < 0) {
		outputErrorMessage(
		        PORTTEST_ERROR_ARGS, "Conversion to modified UTF8 failed, error code %d\n", mutf8StringLength);
	}

	/* test string length in the other direction */
	returnStringLength =
	        omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, mutf8Buff, mutf8StringLength, NULL, 0);
	...
	returnStringLength = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, mutf8Buff, mutf8StringLength,
	        returnBuff, sizeof(returnBuff));
	if (returnStringLength < 0) {
		outputErrorMessage(
		        PORTTEST_ERROR_ARGS, "Conversion to platform failed, error code %d\n", mutf8StringLength);
	}
...
TEST(PortStrTest, str_convU8ToWide)
{
	...
	memset(outBuff, 0, sizeof(outBuff));
	convertedStringLength = omrstr_convert(
	        J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, outBuff, sizeof(outBuff));
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
	...
	/* test string length */
	convertedStringLength =
	        omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, NULL, 0);
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        originalStringLength, convertedStringLength);
	}
...
TEST(PortStrTest, str_convWideToU8)
{
	...
	memset(outBuff, 0, sizeof(outBuff));
	convertedStringLength = omrstr_convert(
	        J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, outBuff, sizeof(outBuff));
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
	...
	/* test string length */
	convertedStringLength =
	        omrstr_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, NULL, 0);
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
...
TEST(PortStrTest, str_convU8ToWide_Null)
{
	...
	convertedStringLength = omrstr_convert(
	        J9STR_CODE_MUTF8, J9STR_CODE_WIDE, inputString, inputStringLength, outBuff, sizeof(outBuff));
...
TEST(PortStrTest, str_convUtf8ToMUtf8)
{
	...
	const char utf8Data[] = {
	        ...
	        '\xF0', '\x90', '\x8C', '\x82', '\xF0', '\x90', '\x8C', '\x83', '\xF0', '\x90', '\x8C', '\x84', /* 4
	                                                                                                           byte
	                                                                                                           UTF-8
	                                                                                                         */
	        '\x44', '\x45', '\x46', /* ASCII */
	...
	const char mutf8Data[] = {'\xc0', '\x80', /* embedded null */
	        ...
	        '\xed', '\xa0', '\x80', '\xed', '\xbc', '\x82', '\xed', '\xa0', '\x80', '\xed', '\xbc', '\x83', '\xed',
	        '\xa0', '\x80', '\xed', '\xbc', '\x84', '\x44', '\x45', '\x46', '\xef', '\xbf', '\xbd', '\xc0', '\x80'};
...
TEST(PortStrTest, str_convRandomUtf8ToMUtf8)
{
	...
	const char utf8Data[] = {'\x41', '\xf2', '\x86', '\x84', '\xaf', '\xf0', '\xbc', '\x8f', '\x9b', '\xee', '\xb5',
	        '\xa0', '\xf4', '\x86', '\xbf', '\xae', '\xf0', '\xbe', '\xa7', '\xac', '\xf4', '\x86', '\x81', '\x84',
	        '\xf0', '\xb3', '\x93', '\x99', '\xf2', '\xb6', '\x91', '\xae', '\xf2', '\x96', '\xba', '\xa9', '\xf0',
	        '\xbc', '\xb6', '\x92', '\xf1', '\x8c', '\x9c', '\xa4', '\xe4', '\x99', '\xad', '\xf2', '\xb3', '\xa4',
	        '\x95', '\xf0', '\xa6', '\x85', '\xab', '\xf1', '\xaa', '\xbb', '\xa4', '\xf3', '\x8c', '\x9e', '\xb8',
	        '\xf2', '\x98', '\x9e', '\xb8', '\xf3', '\xb8', '\x8c', '\xa5', '\xf3', '\x84', '\x8b', '\x8a', '\xf3',
	        '\x94', '\x95', '\xb3', '\xf0', '\x9b', '\xa3', '\xa6', '\xf1', '\xbb', '\x9b', '\xa7', '\xf1', '\x88',
	        '\x9a', '\x81', '\xf4', '\x83', '\xa8', '\x83', '\xf2', '\xb4', '\x88', '\xb7', '\xf1', '\xa0', '\xa2',
	        '\xb6', '\xf3', '\x97', '\x88', '\xb7', '\xf2', '\x88', '\x83', '\xb8', '\xf0', '\xa9', '\x9b', '\x9e',
	        '\xf0', '\xa7', '\x92', '\xab', '\x5a'};
	const char mutf8Data[] = {'\x41', '\xed', '\xa7', '\x98', '\xed', '\xb4', '\xaf', '\xed', '\xa2', '\xb0',
	        '\xed', '\xbf', '\x9b', '\xee', '\xb5', '\xa0', '\xed', '\xaf', '\x9b', '\xed', '\xbf', '\xae', '\xed',
	        '\xa2', '\xba', '\xed', '\xb7', '\xac', '\xed', '\xaf', '\x98', '\xed', '\xb1', '\x84', '\xed', '\xa2',
	        '\x8d', '\xed', '\xb3', '\x99', '\xed', '\xaa', '\x99', '\xed', '\xb1', '\xae', '\xed', '\xa8', '\x9b',
	        '\xed', '\xba', '\xa9', '\xed', '\xa2', '\xb3', '\xed', '\xb6', '\x92', '\xed', '\xa3', '\xb1', '\xed',
	        '\xbc', '\xa4', '\xe4', '\x99', '\xad', '\xed', '\xaa', '\x8e', '\xed', '\xb4', '\x95', '\xed', '\xa1',
	        '\x98', '\xed', '\xb5', '\xab', '\xed', '\xa5', '\xab', '\xed', '\xbb', '\xa4', '\xed', '\xab', '\xb1',
	        '\xed', '\xbe', '\xb8', '\xed', '\xa8', '\xa1', '\xed', '\xbe', '\xb8', '\xed', '\xae', '\xa0', '\xed',
	        '\xbc', '\xa5', '\xed', '\xab', '\x90', '\xed', '\xbb', '\x8a', '\xed', '\xac', '\x91', '\xed', '\xb5',
	        '\xb3', '\xed', '\xa0', '\xae', '\xed', '\xb3', '\xa6', '\xed', '\xa6', '\xad', '\xed', '\xbb', '\xa7',
	        '\xed', '\xa3', '\xa1', '\xed', '\xba', '\x81', '\xed', '\xaf', '\x8e', '\xed', '\xb8', '\x83', '\xed',
	        '\xaa', '\x90', '\xed', '\xb8', '\xb7', '\xed', '\xa5', '\x82', '\xed', '\xb2', '\xb6', '\xed', '\xac',
	        '\x9c', '\xed', '\xb8', '\xb7', '\xed', '\xa7', '\xa0', '\xed', '\xb3', '\xb8', '\xed', '\xa1', '\xa5',
	        '\xed', '\xbb', '\x9e', '\xed', '\xa1', '\x9d', '\xed', '\xb2', '\xab', '\x5a'};
	int32_t utf8DataLength = sizeof(utf8Data); /* skip the terminating null */
	int32_t expectedStringLength = sizeof(mutf8Data);
	int32_t convertedStringLength = 0;
...
TEST(PortStrTest, str_convRoundTrip)
{
	...
	convertedStringLength = omrstr_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, (char *)unicodeData,
	...
	convertedStringLength = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, (char *)mutf8Buffer,
...
static const char mutf8DataForLatin1[] = {'\xc0', '\x80', '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8', '\x9',
        '\xa', '\xb', '\xc', '\xd', '\xe', '\xf', '\x10', '\x11', '\x12', '\x13', '\x14', '\x15', '\x16', '\x17',
        '\x18', '\x19', '\x1a', '\x1b', '\x1c', '\x1d', '\x1e', '\x1f', '\x20', '\x21', '\x22', '\x23', '\x24', '\x25',
        '\x26', '\x27', '\x28', '\x29', '\x2a', '\x2b', '\x2c', '\x2d', '\x2e', '\x2f', '\x30', '\x31', '\x32', '\x33',
        '\x34', '\x35', '\x36', '\x37', '\x38', '\x39', '\x3a', '\x3b', '\x3c', '\x3d', '\x3e', '\x3f', '\x40', '\x41',
        ...
        '\x7a', '\x7b', '\x7c', '\x7d', '\x7e', '\x7f', '\xc2', '\x80', '\xc2', '\x81', '\xc2', '\x82', '\xc2', '\x83',
        '\xc2', '\x84', '\xc2', '\x85', '\xc2', '\x86', '\xc2', '\x87', '\xc2', '\x88', '\xc2', '\x89', '\xc2', '\x8a',
        '\xc2', '\x8b', '\xc2', '\x8c', '\xc2', '\x8d', '\xc2', '\x8e', '\xc2', '\x8f', '\xc2', '\x90', '\xc2', '\x91',
        '\xc2', '\x92', '\xc2', '\x93', '\xc2', '\x94', '\xc2', '\x95', '\xc2', '\x96', '\xc2', '\x97', '\xc2', '\x98',
        '\xc2', '\x99', '\xc2', '\x9a', '\xc2', '\x9b', '\xc2', '\x9c', '\xc2', '\x9d', '\xc2', '\x9e', '\xc2', '\x9f',
        '\xc2', '\xa0', '\xc2', '\xa1', '\xc2', '\xa2', '\xc2', '\xa3', '\xc2', '\xa4', '\xc2', '\xa5', '\xc2', '\xa6',
        '\xc2', '\xa7', '\xc2', '\xa8', '\xc2', '\xa9', '\xc2', '\xaa', '\xc2', '\xab', '\xc2', '\xac', '\xc2', '\xad',
        '\xc2', '\xae', '\xc2', '\xaf', '\xc2', '\xb0', '\xc2', '\xb1', '\xc2', '\xb2', '\xc2', '\xb3', '\xc2', '\xb4',
        '\xc2', '\xb5', '\xc2', '\xb6', '\xc2', '\xb7', '\xc2', '\xb8', '\xc2', '\xb9', '\xc2', '\xba', '\xc2', '\xbb',
        '\xc2', '\xbc', '\xc2', '\xbd', '\xc2', '\xbe', '\xc2', '\xbf', '\xc3', '\x80', '\xc3', '\x81', '\xc3', '\x82',
        '\xc3', '\x83', '\xc3', '\x84', '\xc3', '\x85', '\xc3', '\x86', '\xc3', '\x87', '\xc3', '\x88', '\xc3', '\x89',
        '\xc3', '\x8a', '\xc3', '\x8b', '\xc3', '\x8c', '\xc3', '\x8d', '\xc3', '\x8e', '\xc3', '\x8f', '\xc3', '\x90',
        '\xc3', '\x91', '\xc3', '\x92', '\xc3', '\x93', '\xc3', '\x94', '\xc3', '\x95', '\xc3', '\x96', '\xc3', '\x97',
        '\xc3', '\x98', '\xc3', '\x99', '\xc3', '\x9a', '\xc3', '\x9b', '\xc3', '\x9c', '\xc3', '\x9d', '\xc3', '\x9e',
        '\xc3', '\x9f', '\xc3', '\xa0', '\xc3', '\xa1', '\xc3', '\xa2', '\xc3', '\xa3', '\xc3', '\xa4', '\xc3', '\xa5',
        '\xc3', '\xa6', '\xc3', '\xa7', '\xc3', '\xa8', '\xc3', '\xa9', '\xc3', '\xaa', '\xc3', '\xab', '\xc3', '\xac',
        '\xc3', '\xad', '\xc3', '\xae', '\xc3', '\xaf', '\xc3', '\xb0', '\xc3', '\xb1', '\xc3', '\xb2', '\xc3', '\xb3',
        '\xc3', '\xb4', '\xc3', '\xb5', '\xc3', '\xb6', '\xc3', '\xb7', '\xc3', '\xb8', '\xc3', '\xb9', '\xc3', '\xba',
        '\xc3', '\xbb', '\xc3', '\xbc', '\xc3', '\xbd', '\xc3', '\xbe', '\xc3', '\xbf', '\xc0', '\x80', '\x1', '\x2',
        '\x3', '\x4', '\x5', '\x6', '\x7'};
...
TEST(PortStrTest, str_Latin1ToMutf8)
{
	...
	convertedStringLength = omrstr_convert(
	        J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, outBuff, sizeof(outBuff));
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
	...
	/* test string length */
	convertedStringLength =
	        omrstr_convert(J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, NULL, 0);
	if (convertedStringLength != expectedStringLength) {
		outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n",
		        expectedStringLength, convertedStringLength);
	}
...
{
	OMRPORT_ACCESS_FROM_OMRPORT(portTestEnv->getPortLibrary());
	char winacpData[] = {'\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8', '\x81', '\x91', '\xa1', '\xb1',
	        '\xc1', '\xd1', '\xe1', '\xf1'};
#if defined(OMR_OS_WINDOWS)
 ...
 */
static int
printToBuffer(struct OMRPortLibrary *portLibrary, BOOLEAN precision, char *buffer, size_t bufferLength,
        const char *expectedOutput, const char *format, ...)
{
	...
	if (stringLength >= 0) {
		if (precision) {
			...
			portTestEnv->log("\n\tFinish Testing without precision specifier: String in buffer is: \"%s\", "
			                 "length = %d\n",
			        buffer, stringLength);
		}
		...
		if ((stringLength == strlen(expectedOutput)) && (0 == memcmp(buffer, expectedOutput, stringLength))) {
			...
			portTestEnv->log(LEVEL_ERROR,
			        "\n\tComparing against the expected output: FAILED. Expected string: \"%s\", length = "
			        "%d\n",
		...
		portTestEnv->log(LEVEL_ERROR,
		        "\n\tComparing against the expected output: FAILED. stringLength < 0, Expected string: \"%s\", "
		        "length = %d\n",
		        expectedOutput, strlen(expectedOutput));
	}
}
...
TEST(PortStrTest, str_test_atoe_vsnprintf)
{
	...
	portTestEnv->log("\n\tTesting case 2\n");
	rc |= printToBuffer(
	        OMRPORTLIB, FALSE, buffer2, bufferLength2, expectedOutput2, "%*s", 2, nonNullTerminatedString);
	...
	portTestEnv->log("\n\tTesting case 4\n");
	rc |= printToBuffer(
	        OMRPORTLIB, FALSE, buffer3, bufferLength3, expectedOutput3, "%*s", 10, nullTerminatedString);

	/* precision is equal to the length of inputString */
	portTestEnv->log("\n\tTesting case 5\n");
	rc |= printToBuffer(
	        OMRPORTLIB, TRUE, buffer0, bufferLength0, expectedOutput1, "%.*s", 4, nonNullTerminatedString);

	/* precision is less than the length of inputString */
	portTestEnv->log("\n\tTesting case 6\n");
	rc |= printToBuffer(
	        OMRPORTLIB, TRUE, buffer0, bufferLength0, expectedOutput4, "%.*s", 2, nonNullTerminatedString);

	/* both min_width and precision are equal to the length of inputString */
	portTestEnv->log("\n\tTesting case 7\n");
	rc |= printToBuffer(
	        OMRPORTLIB, TRUE, buffer0, bufferLength0, expectedOutput1, "%*.*s", 4, 4, nonNullTerminatedString);
