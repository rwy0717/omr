...
#define TEST_OVERFLOW_LEN 8
    ...
    if (NULL == tokens) {
        ...
        test_omrstr_tokens(OMRPORTLIB, testName, buf, TEST_BUF_LEN, "Test1 %Y Test2-%m%d%y-%S %longtkn, %longtkn",
            tokens, "Test1 2004 Test2-022904-00 Long Token Value, Long Token Value", 61);
        /* Test 7: %% and end with % */
        test_omrstr_tokens(
        ...
        test_omrstr_tokens(OMRPORTLIB, testName, NULL, 0, "%yyy is not %yy is not %y", tokens, NULL,
        ...
        /* Test 17: All tokens that we get for free in omrstr_create_tokens (and that the user may be relying on) */
        {
             ...
             * but make sure that something has been substituted for each token and that the time is substituted
             * properly . Do this by making sure that %<token> is not present in the output buffer*/
            if (rc > TEST_BUF_LEN) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "Test buffer (%i) is too small. Required size: %i\n", TEST_BUF_LEN, rc);
            }
...
TEST(PortStrTest, str_convUtf8ToMUtf8)
{
    ...
    const char utf8Data[] = {
        ...
        '\xF0', '\x90', '\x8C', '\x82', '\xF0', '\x90', '\x8C', '\x83', '\xF0', '\x90', '\x8C',
        '\x84', /* 4 byte UTF-8 */
        '\x44', '\x45', '\x46', /* ASCII */
...
static const char mutf8DataForLatin1[] = { '\xc0', '\x80', '\x1', '\x2', '\x3', '\x4', '\x5', '\x6', '\x7', '\x8',
    '\x9', '\xa', '\xb', '\xc', '\xd', '\xe', '\xf', '\x10', '\x11', '\x12', '\x13', '\x14', '\x15', '\x16', '\x17',
    ...
    '\xc3', '\xbb', '\xc3', '\xbc', '\xc3', '\xbd', '\xc3', '\xbe', '\xc3', '\xbf', '\xc0', '\x80', '\x1', '\x2', '\x3',
    '\x4', '\x5', '\x6', '\x7' };
    ...
    const char* expectedOutput, const char* format, ...)
{
    ...
    if (stringLength >= 0) {
        ...
        portTestEnv->log(LEVEL_ERROR,
            "\n\tComparing against the expected output: FAILED. stringLength < 0, Expected string: \"%s\", length = "
            "%d\n",
            expectedOutput, strlen(expectedOutput));
    }
}
