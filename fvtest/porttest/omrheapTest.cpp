	...
	for (j = 0; j < (sizeof(heapSize) / sizeof(heapSize[0])); j++) {
		...
		do {
			...
			if (subAllocPtr) {
				...
				/*The size returned may have been rounded-up to the nearest U64, and may have an extra 2
				 * slots added in some circumstances*/
		...
		verifyHeapOutofRegionWrite(
		        OMRPORTLIB, allocPtr, allocPtrAdjusted + heapSize[j], heapStartOffset, testName);
	}
...
omrheap_test2(struct OMRPortLibrary *portLibrary, int randomSeed)
{
	...
	portTestEnv->log("Random seed value: %d. Add -srand:[seed] to the command line to reproduce this test "
	                 "manually.\n",
	        randomSeed);
	srand(randomSeed);
	...
	while (freeSerialNumber <= serialNumberTop) {
		...
		if (0 == operation) {
			...
			if (0 == reallocSerialNumber % outputInterval) {
				portTestEnv->log(LEVEL_VERBOSE,
				        "Realloc: index=%zu, size=%zu, result=%p, reallocSerialNumber=%zu, "
				        "elementCount=%zu\n",
	...
	if (NULL == omrheap_allocate(heapBase, largestAllocSize)) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        ...
		        "we cannot allocate the previously determined largest chunk size after sequence of random "
		        "allocate/free\n");
		goto exit;
	}
	walkHeap(OMRPORTLIB, heapBase, testName);
...
TEST(PortHeapTest, heap_test3)
{
	...
	for (i = 0; i < sizeof(uint64_t); i++) {
		...
		if (NULL != subAllocPtr) {
			outputErrorMessage(
			        PORTTEST_ERROR_ARGS, "omrheap_allocate(%u) returned non-NULL!\n", UINT_MAX - i);
			goto exit;
		}
	}
	walkHeap(OMRPORTLIB, heapBase, testName);
	...
	for (i = 0; i < sizeof(uint64_t); i++) {
		...
		if (NULL != subAllocPtr) {
			outputErrorMessage(
			        PORTTEST_ERROR_ARGS, "omrheap_reallocate(%u) returned non-NULL!\n", UINT_MAX - i);
			goto exit;
		}
	}
	walkHeap(OMRPORTLIB, heapBase, testName);
...
TEST(PortHeapTest, heap_realloc_test0)
{
	...
	for (i = 0; i < 128; i++) {
		if (subAllocPtr[i] != i) {
			outputErrorMessage(
			        PORTTEST_ERROR_ARGS, "Unexpected (subAllocPtr[i] (%d) != i (%d))\n", subAllocPtr[i], i);
			goto exit;
		}
	}
...
TEST(PortHeapTest, heap_grow_test)
{
	 ...
	 * So when omrheap created initially, firstFreeBlock always has the value 2(on 64bit) or 1 (on 32bit) and
	 *heapSize should be 128 slots in this test heap. And firstFreeBlock should point to the first slot after header
	 * which should have value 128 - sizeof(omrheap)/sizeof(uint64_t) - 2(head and tail of the slot) = 124 (on
	 *64bit) or 125 (on 32bit)
	 *
	 *
	 * |--------|-----|--------------------------------------------------------------------------------------------------------
	 * |heapSize|first|    |    |
	 *|	  | |		|Free |	124|    | ....................................all heap is
	 *free...................................| 124| |		|Block|	   |    |
	 *|    |
	 * |--------|-----|-------------------------------------------------------------------------------------------------------
	 * 0        1     2	   3	4
	...
	if (initialNumberOfSlots - 2 != baseSlot[heapBase->firstFreeBlock]) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        "heap_allocate failed to set the firstFreeBlock's head value correctly. Expected :  %zu, Found "
		        ": %zd \n",
	...
	/* 2. Allocate the rest of the heap and make it full. So (initialNumberOfSlots-12(used above)-2(head and
	 * tail))*8 bytes need to be allocated
	 * |--------|-----|--------------------------------------------------------------------------------------------------------
	 * |heapSize|first|    |    |		|    |	  |
	 * |	  |
	 * |		|Free |	-8 |  + | ++++++| -8 |-114|++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++|-114|
	 ...
	 *  c. The head of newly added free block of slots should have the value 8. Since 10 slots are added, 2 slots
	 * are used for head and tail.
	 */
	...
	if (8 != baseSlot[heapBase->firstFreeBlock]) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        "heap_allocate failed to set the firstFreeBlock's head value correctly. Expected :  8, Found : "
		        "%zd",
	...
	if (8 != baseSlot[heapBase->heapSize - 1]) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        "heap_allocate failed to set the firstFreeBlock's tail value correctly. Expected :  8, Found : "
		        "%zd",
		        baseSlot[heapBase->firstFreeBlock]);
		goto exit;
	}
...
TEST(PortHeapTest, heap_test_pmr_28277_999_760)
{
	 ...
	 *
	 * With the fix, firstFree and lastAllocSlot values should point to the right segments of heap. This test
	 * verifies they do.
	 *
	 */
	 ...
	 * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
	 * |8Slots |  |  |2Slots|  |  |2Slots|  |  |		     		|   | |	100	|  38   |  38   |   0 |
	 * |		|  |  |		 |  |  |	  |  |  |      |  | |                  	|  |  |      |  |  | |
	 * |  | 					|   |
	 * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
	 * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21
	 ...
	 * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |
	 * |8Slots |  |  |2Slots|  |  |2Slots|  |  |		     		|   | |	100	|   8   |   8   |   0 |
	 * |		|  |  |		 |  |  |	  |  |  |      |  | |              	    |  |  |      |  |  |
	 * |  |  |					|   |
	 * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|----------------------|--|--|------|--|--|------|--|--|-------------------|---|
	 * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21
	 ...
	 * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  |
	 * 3Slots |  |  |3Slots  |  |  |2Slots|  |  |2Slots|  |  |		     		|   |
	 * |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |
	 * | |        |  |  |        |  |  |      |  |  |      |  |  |					|   |
	 * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|-- ---------------|---|
	 * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21 24 25 26 29
	 ...
	 *
	 *
	 * PROBLEM: As you can see above, lastAllocSlot is not pointing to the beginning of alloc/free segment. It is in
	 * the middle of the free segment.
	 *
	 * If this segment is allocated again, and value is written in it, then lastAllocSlot will have random values.
	 * See below calls.
	 *
	 */
        ...
        const char *testName)
{
	...
	if (blockSizeStart >= 0) {
		outputErrorMessage(PORTTEST_ERROR_ARGS,
		        "Sub-allocated memory is not part of an occupied block: 0x%p, top size: %lld, bottom size: "
		        "%lld\n",
	...
	for (i = 0; i < 2; i += 1) {
		...
		if (rc) {
			...
			if ((omrtime_current_time_millis() - timeStart) >= testDurationMin * 60 * 1000) {
				portTestEnv->log("We've run for too long, ending the test. This is not a test "
				                 "failure.\n\n");
				break;
			}
			portTestEnv->log("\n\n");
