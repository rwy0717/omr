    ...
    AttributeElem* numOfFieldsElem, AttributeElem* breadthElem, int32_t depth)
{
    ...
    if ((ROOT == objType) || (GARBAGE_ROOT == objType)) {
        for (int32_t i = 0; i < breadthElem->value; i++) {
            ...
            /* Add object to root hash table. If it's the root of the garbage tree, temporarily keep it as root for
             * allocating the rest of the tree. Remove it from the root set after the entire garbage tree is
             * allocated.*/
            RootEntry rEntry;
...
int32_t GCConfigTest::attachChildEntry(ObjectEntry* parentEntry, ObjectEntry* childEntry)
{
    ...
    if ((uint32_t)parentEntry->numOfRef < slotCount) {
        ...
        gcTestEnv->log(LEVEL_ERROR,
            "%s:%d Invalid XML input: numOfFields %d defined for %s(%p[0x%llx]) is not enough to hold child reference "
            "for %s(%p[0x%llx]).\n",
...
int32_t GCConfigTest::removeObjectFromRootTable(const char* name)
{
    ...
    if (0 != hashTableRemove(exampleVM->rootTable, rootEntry)) {
        gcTestEnv->log(
            LEVEL_ERROR, "%s:%d Failed to remove root object %s from root table!\n", __FILE__, __LINE__, name);
        goto done;
    }
...
int32_t GCConfigTest::removeObjectFromParentSlot(const char* name, ObjectEntry* parentEntry)
{
    ...
    if (0 != rt) {
        gcTestEnv->log(
            LEVEL_ERROR, "%s:%d Failed to remove object %s from its parent's slot.\n", __FILE__, __LINE__, name);
        goto done;
    }
...
int32_t GCConfigTest::parseGarbagePolicy(pugi::xml_node node)
{
    ...
    if (!node.empty()) {
        ...
        if ((0 != strcmp(gp.frequency, "perObject")) && (0 != strcmp(gp.frequency, "perRootStruct"))) {
            rt = 1;
            gcTestEnv->log(LEVEL_ERROR,
                "%s:%d Invalid XML input: the valid value of attribute \"frequency\" is \"perObject\" or "
                "\"perRootStruct\".\n",
                __FILE__, __LINE__);
            goto done;
        }
