...
public:
	...
	{
		if ((numArgs % 2) != 0) {
			fprintf(stderr, "Error: numArgs must be called with zero or an even args, numArgs is %d",
			        numArgs);
			exit(-1);
		}
		...
		if (op.isBooleanCompare() && op.isBranch()) {
			...
			case 1: opCodeInjector = &opCodeUnaryInjector; break;
			case 2: opCodeInjector = &opCodeBinaryIlInjector; break;
			default:
		...
		if (numArgs != 0) {
			...
			for (int32_t i = 0; i < numArgs; i = i + 2) {
				...
				default: TR_ASSERT(0, "Wrong dataType or not supported dataType");
				}
			}
			va_end(args);
		}
		TR::ResolvedMethod opCodeCompilee(__FILE__, LINETOSTR(__LINE__), resolvedMethodName, opCodeArgsNum,
	        ...
	        char *testResolvedMethodName, TR::DataType *argTypes, TR::DataType returnType, int32_t &returnCode)
	{
		...
		TR::ResolvedMethod functionCompilee(__FILE__, LINETOSTR(__LINE__), compileeResolvedMethodName,
		        opCodeArgsNum, argIlTypes, types.PrimitiveType(returnType), 0, &functionIlInjector);
		TR::IlGeneratorMethodDetails functionDetails(&functionCompilee);
		switch (returnType) {
		case TR::Int32:
			_int32Compilee = &functionCompilee;
			_int32CompiledMethod =
			        (signatureCharI_I_testMethodType *)(compileMethod(functionDetails, warm, returnCode));
			functionCompilee.setEntryPoint((void *)_int32CompiledMethod);
			break;
		case TR::Int64:
			_int64Compilee = &functionCompilee;
			_int64CompiledMethod =
			        (signatureCharJ_J_testMethodType *)(compileMethod(functionDetails, warm, returnCode));
		...
		case TR::Float:
			...
			_floatCompiledMethod =
			        (signatureCharF_F_testMethodType *)(compileMethod(functionDetails, warm, returnCode));
		...
		case TR::Address:
			...
			break;
		default: TR_ASSERT(0, "compilee dataType should be int32, int64, double, float or address");
		}
