...
BuilderTest::invokeControlFlowTests()
{
	...
	for (uint32_t i = 0; i < sizeof(basicForLoopParms) / sizeof(basicForLoopParms[0]); ++i) {
		OMR_CT_EXPECT_EQ(_basicForLoopUpMethod,
		        ...
		        _basicForLoopUpMethod(
		                basicForLoopParms[i][0], basicForLoopParms[i][1], basicForLoopParms[i][2]));
	...
	for (uint32_t i = 0; i < sizeof(nestedLoopParms) / sizeof(nestedLoopParms[0]); ++i) {
		OMR_CT_EXPECT_EQ(_shootoutNestedLoopMethod, shootoutNestedLoop(nestedLoopParms[i]),
	...
	int64_t subIfFalseThenParms[][2] = {TR::getMaxSigned<TR::Int64>(), LONG_POS, TR::getMinSigned<TR::Int64>(),
	        LONG_NEG, LONG_POS, TR::getMaxSigned<TR::Int64>()};

	for (uint32_t i = 0; i < sizeof(subIfFalseThenParms) / sizeof(subIfFalseThenParms[0]); ++i) {
		OMR_CT_EXPECT_EQ(_subIfFalseThenMethod,
	...
	for (uint32_t i = 0; i < sizeof(loopN) / sizeof(loopN[0]); ++i) {
		...
		OMR_CT_EXPECT_EQ(_forLoopBreakAndContinueMethod, forLoopBreakAndContinue(loopN[i]),
		        _forLoopBreakAndContinueMethod(loopN[i]));
		OMR_CT_EXPECT_EQ(
		        _doWhileWithBreakMethod, doWhileWithBreak(loopN[i]), _doWhileWithBreakMethod(loopN[i]));
		...
		OMR_CT_EXPECT_EQ(
		        _whileDoWithBreakMethod, whileDoWithBreak(loopN[i]), _whileDoWithBreakMethod(loopN[i]));
		OMR_CT_EXPECT_EQ(
		        _whileDoWithContinueMethod, whileDoWithContinue(loopN[i]), _whileDoWithContinueMethod(loopN[i]))
		        << i;
...
BuilderTest::invokeNestedControlFlowLoopTests()
{
	...
	int32_t loopIfThenElseArr[][4] = {-9, 1000, 1, 500, 9, 1000, 8, 500, 0, 10000, 11, 500, -9, 1000, 1, 500, -20,
	        1000, 8, 500, 18, 10000, 11, 500};
	...
	for (uint32_t i = 0; i < arrSize; ++i) {
		OMR_CT_EXPECT_EQ(_forLoopUpIfThenElseMethod,
		        forLoopIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
		                loopIfThenElseArr[i][3]),
		        _forLoopUpIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1],
	...
	for (uint32_t i = 0; i < arrSize; ++i) {
		OMR_CT_EXPECT_EQ(_whileDoIfThenElseMethod,
		        whileDoIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
		                loopIfThenElseArr[i][3]),
		        _whileDoIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1],
		                loopIfThenElseArr[i][2], loopIfThenElseArr[i][3]));
	}
	...
	for (uint32_t i = 0; i < arrSize; ++i) {
		OMR_CT_EXPECT_EQ(_doWhileIfThenElseMethod,
		        doWhileIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
		                loopIfThenElseArr[i][3]),
		        _doWhileIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1],
		                loopIfThenElseArr[i][2], loopIfThenElseArr[i][3]));
	}

	arrSize = sizeof(ifThenElseLoopArr) / sizeof(ifThenElseLoopArr[0]);
	for (int32_t i = 0; i < arrSize; i++) {
		OMR_CT_EXPECT_EQ(_ifThenElseLoopMethod,
...
DoWhileWithBreakAndContinueMethod::buildIL()
{
	...
	continueThenPath->Store(
	        "a", continueThenPath->Sub(continueThenPath->Load("a"), continueThenPath->ConstInt32(2)));
...
MaxIfThenMethod::MaxIfThenMethod(TR::TypeDictionary *types, BuilderTest *test) : TR::MethodBuilder(types, test)
{
	DefineLine(LINETOSTR(__LINE__));
	DefineFile(__FILE__);
...
ForLoopBreakMethod::ForLoopBreakMethod(TR::TypeDictionary *types, BuilderTest *test) : TR::MethodBuilder(types, test)
{
	DefineLine(LINETOSTR(__LINE__));
	DefineFile(__FILE__);
...
ForLoopBreakAndContinueMethod::buildIL()
{
	...
	continueThenPath->Store(
	        "Sum", continueThenPath->Add(continueThenPath->Load("Sum"), continueThenPath->Load("I")));
