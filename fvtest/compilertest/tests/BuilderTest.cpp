...
void BuilderTest::invokeControlFlowTests()
{
    ...
    for (uint32_t i = 0; i < sizeof(loopN) / sizeof(loopN[0]); ++i) {
        ...
        OMR_CT_EXPECT_EQ(
            _doWhileWithContinueMethod, doWhileWithContinue(loopN[i]), _doWhileWithContinueMethod(loopN[i]));
...
bool RecursiveFibonnaciMethod::buildIL()
{
    ...
    recursiveCase->Store("result",
        recursiveCase->Add(recursiveCase->Call("fib_recur", 1,
            ...
            recursiveCase->Call(
                "fib_recur", 1, recursiveCase->Sub(recursiveCase->Load("n"), recursiveCase->ConstInt32(2)))));
