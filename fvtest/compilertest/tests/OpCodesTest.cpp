...
void
OpCodesTest::addUnsupportedOpCodeTest(int32_t opCodeArgsNum, TR::ILOpCodes opCode, char *resolvedMethodName,
        TR::DataType *argTypes, TR::DataType returnType)
{
	...
	compileOpCodeMethod(
	        never_succeeds, opCodeArgsNum, opCode, resolvedMethodName, argTypes, returnType, returnCode);
...
{
	switch (dataType) {
	case TR::Int32: return _int32Compilee;
	case TR::Int64: return _int64Compilee;
	case TR::Double: return _doubleCompilee;
	case TR::Float: return _floatCompilee;
	case TR::Address: return _addressCompilee;
	case TR::NoType:
	default: TR_ASSERT(0, "compilee dataType should be int32, in64, double, float or address"); return NULL;
	}
}
...
OpCodesTest::compileCompareTestMethods()
{
	...
	// CompareAndBranch
	compileOpCodeMethod(
	        _ifIcmpeq, _numberOfBinaryArgs, TR::ificmpeq, "ifIcmpeq", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIcmpne, _numberOfBinaryArgs, TR::ificmpne, "ifIcmpne", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIcmpgt, _numberOfBinaryArgs, TR::ificmpgt, "ifIcmpgt", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIcmplt, _numberOfBinaryArgs, TR::ificmplt, "ifIcmplt", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIcmpge, _numberOfBinaryArgs, TR::ificmpge, "ifIcmpge", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIcmple, _numberOfBinaryArgs, TR::ificmple, "ifIcmple", _argTypesBinaryInt, TR::Int32, rc);

	compileOpCodeMethod(
	        _ifLcmpne, _numberOfBinaryArgs, TR::iflcmpne, "ifLcmpne", _argTypesBinaryLong, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifLcmpge, _numberOfBinaryArgs, TR::iflcmpge, "ifLcmpge", _argTypesBinaryLong, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifLcmple, _numberOfBinaryArgs, TR::iflcmple, "ifLcmple", _argTypesBinaryLong, TR::Int32, rc);

	compileOpCodeMethod(
	        _ifIuCmplt, _numberOfBinaryArgs, TR::ifiucmplt, "ifIuCmplt", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIuCmpge, _numberOfBinaryArgs, TR::ifiucmpge, "ifIuCmpge", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIuCmpgt, _numberOfBinaryArgs, TR::ifiucmpgt, "ifIuCmpgt", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
	        _ifIuCmple, _numberOfBinaryArgs, TR::ifiucmple, "ifIuCmple", _argTypesBinaryInt, TR::Int32, rc);
	compileOpCodeMethod(
...
OpCodesTest::compileTernaryTestMethods()
{
	...
	compileOpCodeMethod(
	        _iternary, _numberOfTernaryArgs, TR::iternary, "iTernary", _argTypesTernaryInt, TR::Int32, rc);
}
...
OpCodesTest::compileAddressTestMethods()
{
	...
	compileOpCodeMethod(
	        _areturn, _numberOfUnaryArgs, TR::areturn, "areturn", _argTypesUnaryAddress, TR::Address, rc);
...
OpCodesTest::invokeIntegerArithmeticTests()
{
	...
	for (uint32_t i = 0; i < testCaseArrLength; ++i) {
		OMR_CT_EXPECT_EQ(
		        _iMulh, imulh(intMulArr[i][0], intMulArr[i][1]), _iMulh(intMulArr[i][0], intMulArr[i][1]));
...
OpCodesTest::invokeMemoryOperationTests()
{
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(iMemCons, _numberOfUnaryArgs, TR::istore, resolvedMethodName, _argTypesUnaryInt,
...
OpCodesTest::invokeUnaryTests()
{
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iUnaryCons, _numberOfUnaryArgs, TR::ineg, resolvedMethodName, _argTypesUnaryInt,
		        TR::Int32, rc, 2, 1, &intDataArray[i]);
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iUnaryCons, _numberOfUnaryArgs, TR::iabs, resolvedMethodName, _argTypesUnaryInt,
		        TR::Int32, rc, 2, 1, &intDataArray[i]);
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(i2bConst, _numberOfUnaryArgs, TR::i2b, resolvedMethodName, _argTypesUnaryInt,
		...
		compileOpCodeMethod(i2sConst, _numberOfUnaryArgs, TR::i2s, resolvedMethodName, _argTypesUnaryInt,
		...
		compileOpCodeMethod(i2lConst, _numberOfUnaryArgs, TR::i2l, resolvedMethodName, _argTypesUnaryInt,
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(l2bConst, _numberOfUnaryArgs, TR::l2b, resolvedMethodName, _argTypesUnaryLong,
		...
		compileOpCodeMethod(l2sConst, _numberOfUnaryArgs, TR::l2s, resolvedMethodName, _argTypesUnaryLong,
		...
		compileOpCodeMethod(l2iConst, _numberOfUnaryArgs, TR::l2i, resolvedMethodName, _argTypesUnaryLong,
	...
	for (uint32_t i = 0; i < 3; ++i) {
		...
		compileOpCodeMethod(f2iConst, _numberOfUnaryArgs, TR::f2i, resolvedMethodName, _argTypesUnaryFloat,
	...
	for (uint32_t i = 0; i < 3; ++i) {
		...
		compileOpCodeMethod(d2iConst, _numberOfUnaryArgs, TR::d2i, resolvedMethodName, _argTypesUnaryDouble,
...
OpCodesTest::invokeBitwiseTests()
{
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		OMR_CT_EXPECT_EQ(
		        _iAnd, tand(intAndArr[i][0], intAndArr[i][1]), _iAnd(intAndArr[i][0], intAndArr[i][1]));

		sprintf(resolvedMethodName, "iAndConst1_TestCase%d", i + 1);
		compileOpCodeMethod(iBitwiseConst, _numberOfBinaryArgs, TR::iand, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(intAndArr[i][0]), 2, &(intAndArr[i][1]));
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		OMR_CT_EXPECT_EQ(
		        _iXor, txor(intXorArr[i][0], intXorArr[i][1]), _iXor(intXorArr[i][0], intXorArr[i][1]));

		sprintf(resolvedMethodName, "iXorConst1_TestCase%d", i + 1);
		compileOpCodeMethod(iBitwiseConst, _numberOfBinaryArgs, TR::ixor, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(intXorArr[i][0]), 2, &(intXorArr[i][1]));
		OMR_CT_EXPECT_EQ(iBitwiseConst, txor(intXorArr[i][0], intXorArr[i][1]),
		        iBitwiseConst(INT_PLACEHOLDER_1, INT_PLACEHOLDER_2));

		sprintf(resolvedMethodName, "iXorConst2_TestCase%d", i + 1);
		compileOpCodeMethod(iBitwiseConst, _numberOfBinaryArgs, TR::ixor, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(intXorArr[i][0]));
		OMR_CT_EXPECT_EQ(iBitwiseConst, txor(intXorArr[i][0], intXorArr[i][1]),
		        iBitwiseConst(INT_PLACEHOLDER_1, intXorArr[i][1]));

		sprintf(resolvedMethodName, "iXorConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iBitwiseConst, _numberOfBinaryArgs, TR::ixor, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(intXorArr[i][1]));
		OMR_CT_EXPECT_EQ(iBitwiseConst, txor(intXorArr[i][0], intXorArr[i][1]),
...
OpCodesTest::invokeDisabledOpCodesTests()
{
	...
	OMR_CT_EXPECT_DOUBLE_EQ(
	        _dRem, remainder(DOUBLE_MINIMUM, DOUBLE_MINIMUM), _dRem(DOUBLE_MINIMUM, DOUBLE_MINIMUM));
...
OpCodesTest::invokeCompareTests()
{
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpeq, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpeqDataArr[i][0]), 2, &(iCmpeqDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpeq, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpeqDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareEQ(iCmpeqDataArr[i][0], iCmpeqDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpeqDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpeqConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpeq, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpeqDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareEQ(iCmpeqDataArr[i][0], iCmpeqDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpne, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpneDataArr[i][0]), 2, &(iCmpneDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpne, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpneDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareNE(iCmpneDataArr[i][0], iCmpneDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpneDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpneConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpne, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpneDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareNE(iCmpneDataArr[i][0], iCmpneDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpgtDataArr[i][0]), 2, &(iCmpgtDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpgtDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareGT(iCmpgtDataArr[i][0], iCmpgtDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpgtDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpgtConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpgtDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareGT(iCmpgtDataArr[i][0], iCmpgtDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpltDataArr[i][0]), 2, &(iCmpltDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpltDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareLT(iCmpltDataArr[i][0], iCmpltDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpltDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpltConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpltDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareLT(iCmpltDataArr[i][0], iCmpltDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpgeDataArr[i][0]), 2, &(iCmpgeDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpgeDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareGE(iCmpgeDataArr[i][0], iCmpgeDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpgeDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpgeConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpgeDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareGE(iCmpgeDataArr[i][0], iCmpgeDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iCmpleDataArr[i][0]), 2, &(iCmpleDataArr[i][1]));
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(iCmpleDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareLE(iCmpleDataArr[i][0], iCmpleDataArr[i][1]),
		        iCompareConst(INT_PLACEHOLDER_1, iCmpleDataArr[i][1]));

		sprintf(resolvedMethodName, "iCmpleConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::icmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(iCmpleDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iCompareConst, compareLE(iCmpleDataArr[i][0], iCmpleDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::lcmpne, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(lCmpneDataArr[i][0]), 2, &(lCmpneDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::lcmpgt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(lCmpgtDataArr[i][0]), 2, &(lCmpgtDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::lcmpge, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(lCmpgeDataArr[i][0]), 2, &(lCmpgeDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::lcmple, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(lCmpleDataArr[i][0]), 2, &(lCmpleDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::iucmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iuCmpgtDataArr[i][0]), 2, &(iuCmpgtDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::iucmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iuCmpltDataArr[i][0]), 2, &(iuCmpltDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::iucmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(iuCmpleDataArr[i][0]), 2, &(iuCmpleDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::lucmpgt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(luCmpgtDataArr[i][0]), 2, &(luCmpgtDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::lucmplt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(luCmpltDataArr[i][0]), 2, &(luCmpltDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::lucmpge, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(luCmpgeDataArr[i][0]), 2, &(luCmpgeDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::lucmple, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(luCmpleDataArr[i][0]), 2, &(luCmpleDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmpeq, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpeqDataArr[i][0]), 2, &(ifIcmpeqDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmpne, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpneDataArr[i][0]), 2, &(ifIcmpneDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpgtDataArr[i][0]), 2, &(ifIcmpgtDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpltDataArr[i][0]), 2, &(ifIcmpltDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpgeDataArr[i][0]), 2, &(ifIcmpgeDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iCompareConst, _numberOfBinaryArgs, TR::ificmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIcmpleDataArr[i][0]), 2, &(ifIcmpleDataArr[i][1]));
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::iflcmpne, resolvedMethodName,
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::iflcmpge, resolvedMethodName,
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(lCompareConst, _numberOfBinaryArgs, TR::iflcmple, resolvedMethodName,
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIuCmpgtDataArr[i][0]), 2,
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(ifIuCmpgtDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareGT(ifIuCmpgtDataArr[i][0], ifIuCmpgtDataArr[i][1]),
		        iuCompareConst(INT_PLACEHOLDER_1, ifIuCmpgtDataArr[i][1]));

		sprintf(resolvedMethodName, "ifIuCmpgtConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpgt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(ifIuCmpgtDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareGT(ifIuCmpgtDataArr[i][0], ifIuCmpgtDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIuCmpltDataArr[i][0]), 2,
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(ifIuCmpltDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareLT(ifIuCmpltDataArr[i][0], ifIuCmpltDataArr[i][1]),
		        iuCompareConst(INT_PLACEHOLDER_1, ifIuCmpltDataArr[i][1]));

		sprintf(resolvedMethodName, "ifIuCmpltConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmplt, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(ifIuCmpltDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareLT(ifIuCmpltDataArr[i][0], ifIuCmpltDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIuCmpgeDataArr[i][0]), 2,
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(ifIuCmpgeDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareGE(ifIuCmpgeDataArr[i][0], ifIuCmpgeDataArr[i][1]),
		        iuCompareConst(INT_PLACEHOLDER_1, ifIuCmpgeDataArr[i][1]));

		sprintf(resolvedMethodName, "ifIuCmpgeConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmpge, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(ifIuCmpgeDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareGE(ifIuCmpgeDataArr[i][0], ifIuCmpgeDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 4, 1, &(ifIuCmpleDataArr[i][0]), 2,
		...
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 1, &(ifIuCmpleDataArr[i][0]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareLE(ifIuCmpleDataArr[i][0], ifIuCmpleDataArr[i][1]),
		        iuCompareConst(INT_PLACEHOLDER_1, ifIuCmpleDataArr[i][1]));

		sprintf(resolvedMethodName, "ifIuCmpleConst3_TestCase%d", i + 1);
		compileOpCodeMethod(iuCompareConst, _numberOfBinaryArgs, TR::ifiucmple, resolvedMethodName,
		        _argTypesBinaryInt, TR::Int32, rc, 2, 2, &(ifIuCmpleDataArr[i][1]));
		OMR_CT_EXPECT_EQ(iuCompareConst, compareLE(ifIuCmpleDataArr[i][0], ifIuCmpleDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpgt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(ifLuCmpgtDataArr[i][0]), 2,
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpgt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 1, &(ifLuCmpgtDataArr[i][0]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareGT(ifLuCmpgtDataArr[i][0], ifLuCmpgtDataArr[i][1]),
		        luCompareConst(LONG_PLACEHOLDER_1, ifLuCmpgtDataArr[i][1]));

		sprintf(resolvedMethodName, "ifLuCmpgtConst3_TestCase%d", i + 1);
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpgt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 2, &(ifLuCmpgtDataArr[i][1]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareGT(ifLuCmpgtDataArr[i][0], ifLuCmpgtDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmplt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(ifLuCmpltDataArr[i][0]), 2,
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmplt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 1, &(ifLuCmpltDataArr[i][0]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareLT(ifLuCmpltDataArr[i][0], ifLuCmpltDataArr[i][1]),
		        luCompareConst(LONG_PLACEHOLDER_1, ifLuCmpltDataArr[i][1]));

		sprintf(resolvedMethodName, "ifLuCmpltConst3_TestCase%d", i + 1);
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmplt, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 2, &(ifLuCmpltDataArr[i][1]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareLT(ifLuCmpltDataArr[i][0], ifLuCmpltDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpge, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(ifLuCmpgeDataArr[i][0]), 2,
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpge, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 1, &(ifLuCmpgeDataArr[i][0]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareGE(ifLuCmpgeDataArr[i][0], ifLuCmpgeDataArr[i][1]),
		        luCompareConst(LONG_PLACEHOLDER_1, ifLuCmpgeDataArr[i][1]));

		sprintf(resolvedMethodName, "ifLuCmpgeConst3_TestCase%d", i + 1);
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmpge, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 2, &(ifLuCmpgeDataArr[i][1]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareGE(ifLuCmpgeDataArr[i][0], ifLuCmpgeDataArr[i][1]),
	...
	for (uint32_t i = 0; i < testCaseNum; ++i) {
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmple, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 4, 1, &(ifLuCmpleDataArr[i][0]), 2,
		...
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmple, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 1, &(ifLuCmpleDataArr[i][0]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareLE(ifLuCmpleDataArr[i][0], ifLuCmpleDataArr[i][1]),
		        luCompareConst(LONG_PLACEHOLDER_1, ifLuCmpleDataArr[i][1]));

		sprintf(resolvedMethodName, "ifLuCmpleConst3_TestCase%d", i + 1);
		compileOpCodeMethod(luCompareConst, _numberOfBinaryArgs, TR::iflucmple, resolvedMethodName,
		        _argTypesBinaryLong, TR::Int32, rc, 2, 2, &(ifLuCmpleDataArr[i][1]));
		OMR_CT_EXPECT_EQ(luCompareConst, compareLE(ifLuCmpleDataArr[i][0], ifLuCmpleDataArr[i][1]),
...
OpCodesTest::invokeTernaryTests()
{
	...
	int32_t iternaryChild1Arr[] = {INT_MAXIMUM, INT_MAXIMUM, INT_MINIMUM, INT_POS, INT_ZERO, INT_MINIMUM, INT_POS,
	        INT_ZERO, INT_NEG, INT_NEG, INT_NEG, INT_ZERO, INT_POS, INT_NEG, INT_ZERO, INT_MINIMUM, INT_MAXIMUM,
	        INT_POS};

	int32_t intArr[][2] = {INT_NEG, INT_MINIMUM, INT_NEG, INT_POS, INT_NEG, INT_POS, INT_MAXIMUM, INT_ZERO,
	        INT_ZERO, INT_POS, INT_ZERO, INT_POS, INT_ZERO, INT_MAXIMUM, INT_POS, INT_NEG, INT_POS, INT_MINIMUM,
	        INT_MAXIMUM, INT_POS, INT_MINIMUM, INT_ZERO, INT_MINIMUM, INT_MAXIMUM, INT_NEG, INT_MAXIMUM, INT_ZERO,
	        INT_NEG, INT_POS, INT_ZERO, INT_MAXIMUM, INT_NEG, INT_ZERO, INT_MINIMUM, INT_MINIMUM, INT_POS};
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(iTernaryConst, _numberOfTernaryArgs, TR::iternary, resolvedMethodName,
		        _argTypesTernaryInt, TR::Int32, rc, 6, 1, &iternaryChild1Arr[i], 2, &intArr[i][0], 3,
		        &intArr[i][1]);
		...
		compileOpCodeMethod(iTernaryConst, _numberOfTernaryArgs, TR::iternary, resolvedMethodName,
		        _argTypesTernaryInt, TR::Int32, rc, 4, 1, &iternaryChild1Arr[i], 2, &intArr[i][0]);
		OMR_CT_EXPECT_EQ(iTernaryConst, ternary(iternaryChild1Arr[i], intArr[i][0], intArr[i][1]),
		        iTernaryConst(INT_PLACEHOLDER_1, INT_PLACEHOLDER_2, intArr[i][1]));

		compileOpCodeMethod(iTernaryConst, _numberOfTernaryArgs, TR::iternary, resolvedMethodName,
		        _argTypesTernaryInt, TR::Int32, rc, 4, 1, &iternaryChild1Arr[i], 3, &intArr[i][1]);
		OMR_CT_EXPECT_EQ(iTernaryConst, ternary(iternaryChild1Arr[i], intArr[i][0], intArr[i][1]),
		        iTernaryConst(INT_PLACEHOLDER_1, intArr[i][0], INT_PLACEHOLDER_3));

		compileOpCodeMethod(iTernaryConst, _numberOfTernaryArgs, TR::iternary, resolvedMethodName,
		        _argTypesTernaryInt, TR::Int32, rc, 4, 2, &intArr[i][0], 3, &intArr[i][1]);
		OMR_CT_EXPECT_EQ(iTernaryConst, ternary(iternaryChild1Arr[i], intArr[i][0], intArr[i][1]),
...
OpCodesTest::invokeAddressTests()
{
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(aUnaryCons, _numberOfUnaryArgs, TR::aconst, resolvedMethodName,
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(aUnaryCons, _numberOfUnaryArgs, TR::areturn, resolvedMethodName,
		        _argTypesUnaryAddress, TR::Address, rc, 2, 1, &(aUnaryDataArr[i]));
		OMR_CT_EXPECT_EQ(aUnaryCons, aUnaryDataArr[i], aUnaryCons(ADDRESS_PLACEHOLDER_1));
	}
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(aUnaryCons, _numberOfUnaryArgs, TR::astore, resolvedMethodName,
		        _argTypesUnaryAddress, TR::Address, rc, 2, 1, &(aUnaryDataArr[i]));
		OMR_CT_EXPECT_EQ(aUnaryCons, aUnaryDataArr[i], aUnaryCons(ADDRESS_PLACEHOLDER_1));
	}
	...
	for (int32_t i = 0; i < testCaseNum; i++) {
		...
		compileOpCodeMethod(a2iConst, _numberOfUnaryArgs, TR::a2i, resolvedMethodName, _argTypesUnaryAddress,
