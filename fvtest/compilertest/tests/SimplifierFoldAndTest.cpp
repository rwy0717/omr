...
public:
	...
	SimplifierFoldAndIlInjector(TR::TypeDictionary *types, TestDriver *test) : TR::IlInjector(types, test) {}
	...
	bool injectIL()
	{
		...
		storeToTemp(
		        i, createWithoutSymRef(TR::land, 2, iu2l(parameter(0, Int32)), lconst(0xFFFFFFFF00000000ll)));
...
public:
	int32_t verifyNode(TR::Node *node)
	{
		...
		case TR::iand:
		case TR::band:
		case TR::sand: ADD_FAILURE() << "Found an AND operation: " << node->getOpCodeValue(); return 1;
		default: break;
		}
		return 0;
	}
