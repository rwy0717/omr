...
public:
	...
	IlInjector(TR::TypeDictionary *types) : OMR::IlInjector(types), _test(NULL), _method(NULL) {}
	...
	IlInjector(TR::TypeDictionary *types, TestDriver *test) : OMR::IlInjector(types)
	{
		setMethodAndTest(NULL, test);
	}
	IlInjector(TR::ResolvedMethod *method, TR::TypeDictionary *types, TestDriver *test) : OMR::IlInjector(types)
	{
		setMethodAndTest(method, test);
	}
	IlInjector(TR::IlInjector *source) : OMR::IlInjector(source) { setMethodAndTest(source); }
	...
	TR::Node *callFunction(
	        TR::ResolvedMethod *function, TR::IlType *returnType, int32_t numArgs, TR::Node *firstArg);
...
public:
	...
	IlInjector(TR::TypeDictionary *types, TestCompiler::TestDriver *test) : TestCompiler::IlInjector(types, test) {}
