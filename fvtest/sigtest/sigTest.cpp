...
#if !defined(OMR_OS_WINDOWS)
TEST(OmrSigTest, sigactionTest) { EXPECT_TRUE(OMR_ERROR_NONE == test(test_sigaction)) << "sigaction() test failed."; }
#endif /* !defined(OMR_OS_WINDOWS) */

TEST(OmrSigTest, signalTest) { EXPECT_TRUE(OMR_ERROR_NONE == test(test_signal)) << "signal() test failed."; }
    ...
    for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
        ...
        if (OMR_ERROR_NONE
            ...
            printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, %sexisting secondary, flags = %" PRId64
                ...
                testFunc, i, __FILE__, __LINE__, existingPrimary ? "" : "no ", existingSecondary ? "" : "no ", flags,
    ...
    bool existingPrimary, bool existingSecondary, bool onStack, int signum)
{
    ...
    if (SIG_ERR == omrsig_primary_signal(signum, SIG_DFL)) {
        rc = OMR_ERROR_INTERNAL;
        printf(
            "Setting primary handler to SIG_DFL before setting test conditions failed at %s:%d.\n", __FILE__, __LINE__);
    }
    if (OMR_ERROR_NONE == rc) {
    ...
    if (signum != signumOptions[1]) {
        ...
        if (OMR_ERROR_INTERNAL == rc) {
            ...
            if ((onStackCondition != currentlyOnstack) && handlerIsFunction(&actprimary)
                ...
                rc = OMR_ERROR_INTERNAL;
                printf("Signal alternate stack not correctly set: primary handler %scurrently on stack while %sin on "
                       "stack condition at %s:%d.\n",
                    currentlyOnstack ? "" : "not ", onStackCondition ? "" : "not ", __FILE__, __LINE__);
            } else if (onStackCondition && !secondaryOnstack) {
                rc = OMR_ERROR_INTERNAL;
                printf("Signal alternate stack not correctly set: secondary handler not on alternate stack while in on "
                       "stack condition at %s:%d.\n",
                    __FILE__, __LINE__);
            }
        }
