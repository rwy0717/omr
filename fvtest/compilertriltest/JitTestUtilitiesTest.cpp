...
TEST(PtrTest, AssertNullWithNullValue) { ASSERT_NULL(NULL) << "This should always pass."; }
...
TEST(PtrTest, AssertNullWithNonNullValue) { EXPECT_FATAL_FAILURE(ASSERT_NULL(reinterpret_cast<void*>(0x1)), ""); }

TEST(PtrTest, AssertNotNullWithNullValue) { EXPECT_FATAL_FAILURE(ASSERT_NOTNULL(NULL), ""); }

TEST(PtrTest, ExpectNullWithNullValue) { EXPECT_NULL(NULL) << "This should always pass."; }
...
TEST(PtrTest, ExpectNullWithNonNullValue) { EXPECT_NONFATAL_FAILURE(EXPECT_NULL(reinterpret_cast<void*>(0x1)), ""); }

TEST(PtrTest, ExpectNotNullWithNullValue) { EXPECT_NONFATAL_FAILURE(EXPECT_NOTNULL(NULL), ""); }
...
TEST(TRTestFilter, FilterVectorWithNoOccurrences)
{
    ...
    ASSERT_EQ(v_in, v_out) << "Filtering a vector that doesn't contain elements matching the predicate should just "
