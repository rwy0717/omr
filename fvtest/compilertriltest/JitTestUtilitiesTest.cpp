...
TEST(TRTestCombineVectorTest, CombineEmptyAndNonEmptyVectorsOfSameType)
{
	...
	::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
	ASSERT_TRUE(v.empty()) << "Combining any vector with an empty vector should always result in another empty "
	                          "vector.";
}
...
TEST(TRTestCombineVectorTest, CombineNonEmptyVectorsOfSameType)
{
	...
	::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
	ASSERT_EQ(v1.size() * v2.size(), v.size()) << "Size of combined vectors should be the product of the sizes of "
...
TEST(TRTestCombineBraceInitTest, CombineNonEmptyListsOfSameType)
{
	...
	auto v = TRTest::combine(v1, v2);
	::testing::StaticAssertTypeEq<vector<tuple<int, int> >, decltype(v)>();
	ASSERT_EQ(3 * 2, v.size()) << "Size of combined lists should be the product of the sizes of the two individual "
	                              "lists.";
}
...
TEST(TRTestFilter, FilterVectorWithNoOccurrences)
{
	...
	ASSERT_EQ(0, std::count_if(v_out.begin(), v_out.end(), isChar_c)) << "Filtering should leave no elements "
