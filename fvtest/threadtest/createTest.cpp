...
TEST_F(ThreadCreateTest, SetAttrStacksize)
{
    ...
    if (rc != J9THREAD_SUCCESS) {
        status |= EXPECTED_VALID;
        omrTestEnv->log(
            LEVEL_ERROR, "failed to set STACK_DEFAULT_SIZE %x (%d)\n", STACK_DEFAULT_SIZE, STACK_DEFAULT_SIZE);
    }
    status |= isAttrOk(&expected, attr);
    END_IF_FAILED(status);
    ...
    if (maxNumaNode > 0) {
        ...
        while (numaNode > 0) {
            ...
            if (result == J9THREAD_NUMA_ERR_NO_CPUS_FOR_NODE) {
                omrTestEnv->log(LEVEL_ERROR,
                    "Tried to set thread numa affinity to node %zu, but no CPUs associated with node\n", numaNode);
                numaNode--;
                continue;
            } else if (result != 0) {
        ...
        if (numaNode != currentAffinity) {
            omrTestEnv->log(
                LEVEL_ERROR, "Affinity is incorrectly set. Expected:%zu Actual:%zu\n", numaNode, currentAffinity);
            status |= EXPECTED_VALID;
            goto endthread;
        }
