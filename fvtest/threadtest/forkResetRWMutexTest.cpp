...
siblingWriteStart(int forkCount)
{
	...
	EXPECT_TRUE(0 == omrthread_monitor_destroy(testdata.monitor)) << "Destroying monitor failed: monitor still in "
	                                                                 "use.";
	EXPECT_TRUE(0 == omrthread_rwmutex_destroy(testdata.rwmutex)) << "Destroying rwmutex failed: monitor still in "
	                                                                 "use.";
...
parentWriteStart(int forkCount)
{
	...
	EXPECT_TRUE(0 == omrthread_rwmutex_try_enter_write(testdata.rwmutex)) << "Enter write rwmutex failed before "
	                                                                         "fork.";
	...
	close(pipedata[0]);
	close(pipedata[1]);
	EXPECT_TRUE(0 == omrthread_rwmutex_exit_write(testdata.rwmutex)) << "Exit rwmutex failed after fork: monitor "
	                                                                    "still in use.";
	EXPECT_TRUE(0 == omrthread_rwmutex_destroy(testdata.rwmutex)) << "Destroying monitor failed: monitor still in "
	                                                                 "use.";
}
