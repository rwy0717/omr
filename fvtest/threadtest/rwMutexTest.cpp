 ...
 * This method is called to push the concurrent thread to run the next function
 */
void triggerNextStep(SupportThreadInfo* info) { triggerNextStepWithStatus(info, FALSE); }

/**
 * This method is called to push the concurrent thread to run the next function
 * and tell the thread that the test is done
 */
void triggerNextStepDone(SupportThreadInfo* info) { triggerNextStepWithStatus(info, TRUE); }
