...
static omr_error_t subscribeFunc(UtSubscription* subscriptionID) { return OMR_ERROR_NONE; }
...
static omr_error_t testTraceAgentGetMemorySize(OMR_VMThread* vmThread, OMR_TI const* ti)
{
    ...
    if (OMR_ERROR_NONE == testRc) {
        ...
        if (OMR_ERROR_NONE != rc) {
            ...
            omrtty_printf("%s:%d Process physical memory size (in bytes): %llu, rc = %d (%s), the function call is "
                          "successful !\n",
                __FILE__, __LINE__, memorySize, rc, omrErrorToString(rc));
        }
    }
