...
static WRAPPER_TYPE thread_wrapper(WRAPPER_ARG arg)
{
...
#if defined(OMR_PORT_NUMA_SUPPORT)
    {
         ...
         * have inherited and it assuming the initial affinity that the process had (if reverting to that affinity isn't
 ...
 */
void omrthread_abort(omrthread_t thread) { threadInterrupt(thread, J9THREAD_FLAG_ABORTED); }
 ...
 * @return none
 */
void omrthread_priority_interrupt(omrthread_t thread) { threadInterrupt(thread, J9THREAD_FLAG_PRIORITY_INTERRUPTED); }
 ...
 */
intptr_t omrthread_monitor_wait(omrthread_monitor_t monitor) { return monitor_wait(monitor, 0, 0, 0); }
...
void* j9thread_tls_get(omrthread_t thread, omrthread_tls_key_t key) { return omrthread_tls_get(thread, key); }
