...
intptr_t omrthread_attr_init(omrthread_attr_t* attr)
{
     ...
     * a running thread to never give up the CPU once it has started executing. By setting the PTHREAD_SCOPE_SYSTEM
     * flag, we are telling the system to use the kernel's time-slicing scheduling algorithm to choose how this thread
     * will be scheduled. Effectively the kernel, not the threads library will decide when this thread runs by
