...
/// For a more powerful array-type, see Vector and ArrayRef. Arrays are primarily low level storage for other higher level abstractions.
class Array
{
  public:
  ...
  protected:
	...
	{
		return dataSize + sizeof(Array);
	}

	inline Array() = default;

	inline Array(Shape *shape, std::size_t size)
		: header_(shape), size_(size)
	{
	}
...
static_assert(
	std::is_standard_layout<Array>::value, "Array must be a StandardLayoutType");
