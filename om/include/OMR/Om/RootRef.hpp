...
public:
	...
	RootRef(RootList& list, T* ref) noexcept : list_(list), node_(ref, list.head()) {
		list_.head(&node_);
	}
	...
	/// Create a new RootRef that refers to the same object as an existing RootRef. The new
	/// RootRef is attached to the head of the list.
	template<typename U>
	explicit RootRef(const RootRef<U>& other) noexcept : RootRef(other.list(), other.ref()) {}

	/// Move from other RootRef. The RootRef we are moving from must be the head
	/// of the List. The moved-from RootRef is invalidated and cleared. This new RootRef
	/// replaces the old RootRef in the RootList.
	template<typename U>
	explicit RootRef(RootRef<U>&& other) noexcept : list_(other.list()), node_(other.node()) {
		other.clear();
	}
	...
	RootList& list() const noexcept { return list_; }

	RootListNode& node() noexcept { return node_; }

	const RootListNode& node() const noexcept { return node_; }

	RootListNode* tail() const noexcept { return node_.tail; }
