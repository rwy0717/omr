...
/// The classic way to construct a Handle is via a RootRef. The RootRef contains a bare pointer to a Cell. This reference slot will be kept up-to-date through GC safepoints. A Handle can be constructed to indirect through the RootRef. Other rooting mechanisms exist as well.
template <typename T>
class Handle
{
  public:
