...
public:
	...
	explicit CellHeader(Shape *shape, std::uint8_t flags = 0) : data_((std::uintptr_t(shape) << LAYOUT_REF_SHIFT) | (flags & FLAGS_MASK)) {}
	...
	Shape *layout() const noexcept { return reinterpret_cast<Shape *>(data_ >> LAYOUT_REF_SHIFT); }
...
/// A Handle to the layout ref in a Cell's header. This is a special kind of SlotHandle used to read/write a cell header's layout reference from the header. Used by the GC to fixup the layout pointer.
