...
/// The Slot Attributes are unowned, heap-allocated data. Do not hold a range or iterator across GC safepoints.
///
...
public:
	...
	{
		return Iterator(attributes_.begin(), offset_);
	}

	Iterator end() const
	{
		return Iterator(attributes_.end(), offset_ + width_);
	}
