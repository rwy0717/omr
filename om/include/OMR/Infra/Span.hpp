...
public:
	...
	template <std::size_t N>
	inline constexpr Span(const std::array<Type, N>& array) : value_(array.data()), length_(N)
	{
	}
	...
	template <typename U>
	inline constexpr Span(const Span<U>& other) : value_{other.value()}, length_{other.length()}
	{
	}
