...
void JavaSupersetGenerator::convertJ9BaseTypedef(Type* type, string* typeName)
{
    ...
    if (_baseTypedefMap.find(name) != _baseTypedefMap.end()) {
        ...
        stringstream ss;
        ss << ((string::npos == name.find_first_of('u')) ? "I" : "U") << (type->_sizeOf * 8);
        name = ss.str();
    } else {
...
DDR_RC
SupersetFieldVisitor::visitClass(ClassUDT* type) const { return visitNamespace(type); }

DDR_RC
SupersetFieldVisitor::visitUnion(UnionUDT* type) const { return visitNamespace(type); }
...
DDR_RC
SupersetVisitor::visitClass(ClassUDT* type) const { return visitComposite(type, type->_superClass); }

DDR_RC
SupersetVisitor::visitUnion(UnionUDT* type) const { return visitComposite(type, NULL); }
