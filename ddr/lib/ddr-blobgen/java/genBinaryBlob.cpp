...
JavaBlobGenerator::genBinaryBlob(OMRPortLibrary* portLibrary, Symbol_IR* ir, const char* blobFile)
{
    ...
    if (DDR_RC_OK == rc) {
        ...
        for (uint32_t s_ix = 0; s_ix < _buildInfo.header.structureCount; ++s_ix) {
            ...
            /* write fields */
            wb = omrfile_write(
                fd, &_buildInfo.blobFields[f_ix], sizeof(BlobField) * _buildInfo.blobStructs[s_ix].fieldCount);
            amountWritten += wb;
        ...
        if (amountWritten != (intptr_t)_buildInfo.header.structDataSize) {
            ERRMSG("Wrote %u fields, %u constants, %u structs.\nCounted %u fields, %u constants, %u structures.\n",
                f_ix, c_ix, (uint32_t)(_buildInfo.curBlobStruct - _buildInfo.blobStructs), _buildInfo.fieldCount,
...
DDR_RC
BlobFieldVisitor::visitClass(ClassUDT* type) const { return visitNamespace(type); }

DDR_RC
BlobFieldVisitor::visitUnion(UnionUDT* type) const { return visitNamespace(type); }
