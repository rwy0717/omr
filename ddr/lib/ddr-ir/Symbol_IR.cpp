...
static bool startsWith(const string& str, const string& prefix) { return 0 == str.compare(0, prefix.length(), prefix); }
...
DDR_RC
MergeVisitor::visitClass(ClassUDT* type) const { return visitComposite(type); }
...
DDR_RC
TypeReplaceVisitor::visitTypedef(TypedefUDT* type) const { return _ir->replaceTypeUsingMap(&type->_aliasedType, type); }
...
DDR_RC
TypeReplaceVisitor::visitUnion(UnionUDT* type) const { return visitComposite(type); }
