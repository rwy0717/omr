    ...
    if (NULL == die) {
        ...
        *srcfiles = new char*[*filecount];
        if (NULL == *srcfiles) {
            ...
            size_t index = 0;
            for (str_vect::iterator it = Dwarf_CU_Context::_fileList.begin(); it != Dwarf_CU_Context::_fileList.end();
                 ++it) {
...
char* dwarf_errmsg(Dwarf_Error error) { return (char*)_errmsgs[error->_errno]; }
