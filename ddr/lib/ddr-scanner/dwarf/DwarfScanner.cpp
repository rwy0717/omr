...
DDR_RC
DwarfVisitor::visitNamespace(NamespaceUDT* newClass) const { return _scanner->scanClassChildren(newClass, _die); }
        ...
        do {
            ...
            if (DW_TAG_subprogram == childTag) {
                ...
                if (DDR_RC_OK != addDieToIR(childDie, childTag, newClass, (Type**)&innerUDT)) {
                    ...
                    /* We only add it to list of subUDTs if innerUDT's _outerNamespace is NULL, because there should
                     * only be one outer UDT per inner UDT. */
                    /* Check that the subUDT wasn't already added when the type was found as a stub type */
                    if (newClass->_subUDTs.end()
                        == std::find(newClass->_subUDTs.begin(), newClass->_subUDTs.end(), innerUDT)) {
    ...
    OMRPortLibrary* portLibrary, Symbol_IR* ir, vector<string>* debugFiles, const char* blacklistPath)
{
    ...
    if (DDR_RC_OK == rc) {
        ...
        for (map<Type*, set<string> >::iterator it = typeToContainingFiles.begin();
             it != typeToContainingFiles.end() && !unnecessaryFiles.empty();) {
            if (1 == it->second.size()) {
        ...
        while (!typeToContainingFiles.empty()) {
            ...
            for (map<string, set<Type*> >::iterator it = fileToContainedTypes.begin(); it != fileToContainedTypes.end();
                 ++it) {
                if ((it->second.size() > greatestNumberOfTypes)
                    && (unnecessaryFiles.end() != unnecessaryFiles.find(it->first))) {
            ...
            if (greatestNumberOfTypes > 0) {
                ...
                for (set<Type*>::iterator it = fileToContainedTypes[fileWithMostTypes].begin();
                     it != fileToContainedTypes[fileWithMostTypes].end(); ++it) {
                    Type* typeFromFile = *it;
                    /* For each type contained in the file, remove it from the type set of each file it is in. */
                    for (set<string>::iterator it2 = typeToContainingFiles[typeFromFile].begin();
                         it2 != typeToContainingFiles[typeFromFile].end(); ++it2) {
                        string fileContainingType = *it2;
                        fileToContainedTypes[fileContainingType].erase(
                            fileToContainedTypes[fileContainingType].find(typeFromFile));
                    }
