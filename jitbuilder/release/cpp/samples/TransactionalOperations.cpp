...
TransactionalMethod::TransactionalMethod(OMR::JitBuilder::TypeDictionary *d) : OMR::JitBuilder::MethodBuilder(d)
{
...
TransactionalMethod::buildIL()
{
	...
	persistentBuilder->AtomicAdd(persistentBuilder->IndexAt(pInt32, persistentBuilder->Load("addressOfValue"),
	                                     persistentBuilder->ConstInt32(0)),
	        persistentBuilder->ConstInt32(1));

	// For simplicity, temporary failures are not handled specially and are just treated the same as persistent
	// failures
	...
	transactionBuilder->StoreAt(transactionBuilder->IndexAt(pInt32, transactionBuilder->Load("addressOfValue"),
	                                    transactionBuilder->ConstInt32(0)),
	        transactionBuilder->Add(
	                transactionBuilder->LoadAt(pInt32,
	                        transactionBuilder->IndexAt(pInt32, transactionBuilder->Load("addressOfValue"),
	                                transactionBuilder->ConstInt32(0))),
