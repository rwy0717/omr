...
CallMethod::CallMethod(OMR::JitBuilder::TypeDictionary *types) : OMR::JitBuilder::MethodBuilder(types)
{
	...
	DefineFunction((char *)"doublesum", (char *)__FILE__, (char *)DOUBLESUM_LINE, (void *)&doublesum, Int32, 2,
	        Int32, Int32);
}
...
ComputedCallMethod::ComputedCallMethod(OMR::JitBuilder::TypeDictionary *types) : OMR::JitBuilder::MethodBuilder(types)
{
	DefineLine(LINETOSTR(__LINE__));
	DefineFile(__FILE__);
...
ComputedCallMethod::buildIL()
{
	...
	loop->Store("sum",
	        loop->ComputedCall(
	                "doublesum", 3, loop->ConstAddress((void *)&doublesum), loop->Load("sum"), loop->Load("i")));
