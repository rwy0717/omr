...
bool FrontEnd::mapsAreIdentical(
    ...
    TR::Compilation* comp)
{
    if (nextMapCursor && nextMapCursor != stackAtlas->getParameterMap() && mapCursor != stackAtlas->getParameterMap() && mapCursor->getMapSizeInBytes() == nextMapCursor->getMapSizeInBytes() && mapCursor->getRegisterMap() == nextMapCursor->getRegisterMap() &&
#ifdef TR_HOST_S390
    ...
    TR::Compilation* comp)
{
    ...
    //
    if (bytesPerStackMap > USHRT_MAX || stackAtlas->getNumberOfMaps() > USHRT_MAX || stackAtlas->getNumberOfParmSlotsMapped() > USHRT_MAX || stackAtlas->getParmBaseOffset() < SHRT_MIN || stackAtlas->getParmBaseOffset() > SHRT_MAX || stackAtlas->getLocalBaseOffset() < SHRT_MIN || stackAtlas->getLocalBaseOffset() > SHRT_MAX) {
