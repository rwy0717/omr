...
public:
	        ...
	        TR_BitVectorGrowable growableOrNot = growable, TR_MemoryBase::ObjectType ot = TR_MemoryBase::BitVector)
	{
		...
		_region = NULL;
		switch (allocKind) {
		case heapAlloc: _region = &(m->heapMemoryRegion()); break;
		case stackAlloc: _region = &(m->currentStackRegion()); break;
		case persistentAlloc: _region = NULL; break;
		default: TR_ASSERT(false, "Unhandled allocation type!");
		}
...
class TR_BitVectorCursor : public TR_BitVectorIterator {
	// CS2-like iterator
public:
	TR_BitVectorCursor(TR_BitVector &bv) : TR_BitVectorIterator(bv) { SetToFirstOne(); }
