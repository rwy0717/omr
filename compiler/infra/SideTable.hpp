...
protected:
	SideTable(TR_Memory *mem, TR_AllocationKind allocKind = stackAlloc)
	        : _nextIndex(0), _indexes(mem, allocKind), _entries(mem, 8, true, allocKind)
	{}
...
public:
	...
	{
		TR_ASSERT(
		        0 <= index && index < _nextIndex, "SideTable index %d out of range [0,%d]", index, _nextIndex);
...
public:
	NodeSideTable(TR_Memory *mem, TR_AllocationKind allocKind = stackAlloc) : SideTable<Entry>(mem, allocKind) {}
