...
TR_AddressTree::processMultiplyNode(TR::Node *multiplyNode)
{
	...
	if (opCodeFirstMulChild == TR::iadd || opCodeFirstMulChild == TR::ladd) {
		if (!processBaseAndIndex(firstMulChild)) {
			dumpOptDetails(
			        comp(), "AddressTree: first node of multiply is iadd/ladd but children are not ok\n");
			return false;
		}
...
TR_AddressTree::process(TR::Node *aiaddNode, bool onlyConsiderConstAiaddSecondChild)
{
	...
	if (opCodeAiaddFirstChild == TR::aload || opCodeAiaddFirstChild == TR::aloadi) {
		...
		if ((opCodeAiaddSecondChild == TR::isub || opCodeAiaddSecondChild == TR::lsub
			...
			if (opCodeIsubSecondChild != TR::iconst && opCodeIsubSecondChild != TR::lconst) {
				...
				if (opCodeIsubFirstChild == TR::imul || opCodeIsubFirstChild == TR::lmul) {
					...
					if (indSymFromSubTree->getOpCodeValue() == TR::iload
						...
						// induction variable active for loops we consider, but it does however
						// require that the induction variable be the first child, so we will
						// catch a case of:
						//   a[indVar + j]
						// but not:
						//   a[j + indVar]
			...
			dumpOptDetails(
			        comp(), "AddressTree: second child of aiadd/aladd is not iload/i(l)sub/i(l)mul\n");
		}
	} else {
...
bool
TR_UnifyPattern::thisMatches(TR::Node *node, TR_Unification &uni, TR::Compilation *comp)
{
	if (comp->getOption(TR_TraceTreePatternMatching)) {
...
bool
TR_CommutativePattern::thisMatches(TR::Node *node, TR_Unification &uni, TR::Compilation *comp)
{
	...
	if (_leftPattern->matches(node->getFirstChild(), uni, comp)
		...
		// If the left one matched and the right one failed, then we need to undo the unifications from the left
		// one
		uni.undoTo(mark);
