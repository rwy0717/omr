...
TR_InterferenceGraph::TR_InterferenceGraph(TR::Compilation *comp, int32_t estimatedNodes)
        : _compilation(comp), _nodeTable(NULL), _nodeStack(NULL), _trMemory(comp->trMemory()), TR_IGBase()
{
...
//
void
TR_InterferenceGraph::addInterferenceBetween(TR_IGNode *igNode1, TR_IGNode *igNode2)
{
...
void
TR_InterferenceGraph::removeInterferenceBetween(TR_IGNode *igNode1, TR_IGNode *igNode2)
{
...
//
void
TR_InterferenceGraph::partitionNodesIntoDegreeSets(CS2::ABitVector<TR::Allocator> &workingSet,
...
TR_InterferenceGraph::doColouring(IGNodeColour numColours)
{
	...
	if (debug("traceIG")) {
		if (!success) {
			...
			diagnostic(
			        "COLOUR IG: IG %p coloured with %d colours.\n", this, getNumberOfColoursUsedToColour());
		}
	}
	...
	while (!workingSet.IsZero()) {
		...
		if (!colourableDegreeSet.IsZero()) {
			...
			for (colourableCursor.SetToFirstOne(); colourableCursor.Valid();
				...
				if (debug("traceIG")) {
					diagnostic("SIMPLIFY: Selected colourable IG node #%d with degree %d: "
					           "(igNode=%p, entity=%p)\n",
