...
{
	TR_ASSERT(_current,
	        "Cannot stepForward a TreeTopIterator that has already moved beyond the first or last tree of the "
	        "method");
...
{
	TR_ASSERT(_current,
	        "Cannot stepBackward a TreeTopIterator that has already moved beyond the first or last tree of the "
	        "method");
...
TR::NodeIterator::NodeIterator(TR::TreeTop *start, TR::Compilation *comp, const char *name)
        : TreeTopIteratorImpl(start, comp, name), _checklist(comp), _stack(comp->trMemory(), 5, false, stackAlloc)
{}
...
TR::BlockIterator::BlockIterator(TR::Compilation *comp, const char *name) : _comp(comp), _name(name) {}
...
TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
        ...
        : BlockIterator(comp, name), _postorder(comp->trMemory(), 5, false, stackAlloc)
{
	takeSnapshot(start);
	logCurrentLocation();
}
...
TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
        ...
        : BlockIterator(comp, name), _postorder(comp->trMemory(), cfg->getNumberOfNodes(), false, stackAlloc)
{
...
void
TR::ReversePostorderSnapshotBlockIterator::visit(TR::Block *block, TR::BlockChecklist &alreadyVisited)
{
	...
	if (REMOVED_BLOCKS_CAN_BE_REINSERTED && !next) {
		...
		for (next = _cfg->getFirstNode(); next; next = next->getNext()) {
			if (!_alreadyVisited.contains(next->asBlock())) {
				...
				if (isLoggingEnabled())
					traceMsg(comp(),
					        "BLOCK  %s REMOVED_BLOCKS_CAN_BE_REINSERTED: block_%d found via extra "
					        "scan\n",
