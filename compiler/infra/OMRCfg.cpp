	...
	for (auto e = f->getExceptionSuccessors().begin(); e != f->getExceptionSuccessors().end(); ++e) {
		...
		if (existingDepth < newDepth
		        || (existingDepth == newDepth
		                && existingCatchBlock->getHandlerIndex() > newCatchBlock->getHandlerIndex()))
			continue;
		...
		if (existingCatchBlock->getCatchType() == 0 ||
			...
			if (comp()->getOption(TR_TraceAddAndRemoveEdge)) {
				traceMsg(comp(),
				        "\nAddition of exception edge aborted - existing catch alredy handles this "
				        "case!");
			}
			return;
		}
...
OMR::CFG::addSuccessorEdges(TR::Block *block)
{
	...
	case TR::Goto: addEdge(block, node->getBranchDestination()->getNode()->getBlock()); break;
	case TR::ireturn:
	case TR::lreturn:
	case TR::freturn:
	case TR::dreturn:
	...
	case TR::athrow: addEdge(block, getEnd()); break;
	case TR::Ret: break; // pseudo node representing a ret opcode
	case TR::NULLCHK:
		...
		break;
	default: addEdge(block, block->getExit()->getNextTreeTop()->getNode()->getBlock());
	}
}
...
void
OMR::CFG::copyExceptionSuccessors(TR::CFGNode *from, TR::CFGNode *to, bool (*predicate)(TR::CFGEdge *))
{
...
TR_OrderedExceptionHandlerIterator::TR_OrderedExceptionHandlerIterator(TR::Block *tryBlock, TR::Region &workingRegion)
{
	if (tryBlock->getExceptionSuccessors().empty())
		...
		for (auto e = tryBlock->getExceptionSuccessors().begin(); e != tryBlock->getExceptionSuccessors().end();
		        ++e) {
		...
		for (auto e = tryBlock->getExceptionSuccessors().begin(); e != tryBlock->getExceptionSuccessors().end();
			...
			TR::Block *b = toBlock((*e)->getTo());
			TR_ASSERT(
			        (_handlers[((inlineDim - b->getInlineDepth() - 1) * handlerDim) + b->getHandlerIndex()]
			                == NULL),
...
TR::CFGEdge::CFGEdge(TR::CFGNode *pF, TR::CFGNode *pT) : _pFrom(pF), _pTo(pT), _visitCount(0), _frequency(0), _id(-1) {}

TR::CFGEdge *
TR::CFGEdge::createEdge(TR::CFGNode *pF, TR::CFGNode *pT, TR_Memory *trMemory, TR_AllocationKind allocKind)
{
...
TR::CFGEdge *
TR::CFGEdge::createExceptionEdge(TR::CFGNode *pF, TR::CFGNode *pT, TR_Memory *trMemory, TR_AllocationKind allocKind)
{
	TR::CFGEdge *newEdge = new (trMemory, allocKind) TR::CFGEdge(pF, pT);
...
TR::CFGEdge *
TR::CFGEdge::createExceptionEdge(TR::CFGNode *pF, TR::CFGNode *pT, TR::Region &region)
{
	TR::CFGEdge *newEdge = new (region) TR::CFGEdge(pF, pT);
...
{
	int32_t totalPredFreq = getTotalEdgesFrequency(block->getPredecessors())
	        + getTotalEdgesFrequency(block->getExceptionPredecessors());
	int32_t totalSuccFreq = getTotalEdgesFrequency(block->getSuccessors())
...
TR::CFGEdge *
TR::CFGNode::getEdgeMatchingNodeInAList(TR::CFGNode *n, TR::CFGEdgeList &list, FUNC blockGetter)
{
...
OMR::CFG::removeEdge(TR::CFGEdge *edge)
{
	...
	if (std::find(from->getSuccessors().begin(), from->getSuccessors().end(), edge)
	...
	if (std::find(to->getPredecessors().begin(), to->getPredecessors().end(), edge)
		...
		if (!self()->consumePseudoRandomFrequencies()) {
			...
			if (comp()->getOption(TR_TraceBFGeneration))
				comp()->dumpMethodTrees(
				        "Trees after setting frequencies from structures", comp()->getMethodSymbol());
		}
...
int32_t
OMR::CFG::createTraversalOrder(bool forward, TR_AllocationKind allocationKind, TR_BitVector *backEdges)
{
	...
	while (!blockStack.isEmpty()) {
		...
		if ((DERIVE_BACKWARD_FROM_FORWARD && !forward) && !foundUnvisitedBlock && fallThroughEdge) {
			...
			if (fallThroughBlock->getVisitCount() != traversalVisitCount) {
				...
				TR_CFGIterator *blockIter = new (trStackMemory())
				        TR_SuccessorIterator(fallThroughBlock);
				blockIter->getFirst();
				iteratorStack.push(blockIter);
				foundUnvisitedBlock = true;
		...
		if (!foundUnvisitedBlock) {
			// we've visited all of the top block's out edges, so add to the traversal order and remove from
			// the block stack
	...
	// TR_ASSERT(traversalInsertPosition == 0, "not all blocks in CFG found by traversal, unreachable blocks should
	// be removed first");
	if (traversalInsertPosition > 0) {
		// there are unreachable blocks so the traversal doesn't start at the beginning of the traversalOrder
		// array shift everything down by the number of empty positions
		...
		if (!block->asBlock()->isCold()
			...
			if (allPredsCold)
				for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
					// Note : Disabled code below because we do not want to make a loop cold just
					// because there is a dominating block outside the loop that is marked cold
					//
					...
					if (!fromBlock->isCold()) {
						...
						if (curColdPred) {
							coldFreq =
							        TR::Block::getMaxColdFrequency(curColdPred, fromBlock);
...
void
OMR::CFG::setUniformEdgeFrequenciesOnNode(
...
static void
setFrequencyOnSuccessor(TR_RegionStructure *region, TR::CFGNode *succ, TR::CFGNode *start, int32_t succNum,
...
*/
static void
addAllInnerBlocksToLoop(TR_RegionStructure *region, TR_SuccessorIterator sit, TR_BitVector *_seenNodes,
        TR_ScratchList<TR::CFGNode> *stack, TR::Compilation *_compilation)
{
	for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
		...
		if (!_seenNodes->isSet(succ->getNumber()) && toStructureSubGraphNode(succ)->getStructure()
			...
			if (_compilation->getOption(TR_TraceBFGeneration))
				dumpOptDetails(
				        _compilation, "Added block(or region) %d to the walk\n", succ->getNumber());
		}
	}
}

static void
generalFrequencyPropagation(TR_RegionStructure *region, TR_SuccessorIterator sit, TR::Block *block,
...
OMR::CFG::walkStructure(TR_RegionStructure *region)
{
	...
	if (startBlock) {
		if (startBlock->getBlock()->isCold()) {
			if (_compilation->getOption(TR_TraceBFGeneration))
				dumpOptDetails(
				        comp(), "\tIgnoring because %d is cold\n", startBlock->getBlock()->getNumber());
			return;
		}
	...
	while (!stack.isEmpty() && !_foundIllegalStructure) {
		...
		if (regionStr != NULL) {
			...
			if (sumEdgeFrequency > 0) {
				for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
					...
					setFrequencyOnSuccessor(region, succ, start, succNum, sumEdgeFrequency,
					        parentFrequency, _regionFrequencies, i, _compilation, _max_edge_freq);
				}
			}
		...
		if (block->getEntry()) {
			...
			if ((block->getSuccessors().size() == 2)
				...
				// summarizeEdgeFrequencies will be removed as part of the STL List replacement process
				List<TR::CFGEdge> temp(comp()->trMemory());
				for (auto iter = block->getSuccessors().begin(); iter != block->getSuccessors().end();
				        ++iter)
					temp.add(*iter);
				...
				if (sumEdgeFrequency > 0) {
					...
					for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
						...
						setFrequencyOnSuccessor(region, succ, start, succNum, sumEdgeFrequency,
						        parentFrequency, _regionFrequencies, i, _compilation,
						        _max_edge_freq);
					}
				}
					...
					if (branchToBlock) {
						if (!branchToBlock->isCold()) {
							if ((branchToBlock->getFrequency() + blockFreq)
								...
								if (_compilation->getOption(TR_TraceBFGeneration))
									dumpOptDetails(comp(),
									        "Setting frequency of %d on block_%d "
									        "(GOTO block) - MAX block frequency\n",
									        INITIAL_BLOCK_FREQUENCY_FACTOR,
									        branchToBlock->getNumber());
								branchToBlock->setFrequency(
								        INITIAL_BLOCK_FREQUENCY_FACTOR);
							} else {
								branchToBlock->setFrequency(
								...
								if (_compilation->getOption(TR_TraceBFGeneration))
									dumpOptDetails(comp(),
									        "Adding frequency of %d on block_%d "
									        "(GOTO block), prev frequency was %d\n",
									        blockFreq, branchToBlock->getNumber(),
									        branchToBlock->getFrequency()
									                - blockFreq);
							}
						}
					} else {
				...
				if (_compilation->getOption(TR_TraceBFGeneration))
					dumpOptDetails(_compilation, "Edge %p between %d and %d has freq %d (GOTO)\n",
					        es, es->getFrom()->getNumber(), es->getTo()->getNumber(),
					        es->getFrequency());
			} else {
				generalFrequencyPropagation(
				        region, sit, block, _seenNodes, _regionFrequencies, &stack, _compilation);
			}
		} else {
			generalFrequencyPropagation(
			        region, sit, block, _seenNodes, _regionFrequencies, &stack, _compilation);
...
OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure()
{
	...
	if (setBlockFrequencies) {
		if (_compilation->getOption(TR_TraceBFGeneration))
			dumpOptDetails(comp(),
			        "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating block and edge frequencies "
			        "within regions...\n");
...

		{
			...
			if (_compilation->getOption(TR_TraceBFGeneration))
				dumpOptDetails(comp(),
				        "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Computing region weight factors "
				        "based on CFG structure, num regions=%d...\n",
			...
			if (_compilation->getOption(TR_TraceBFGeneration))
				dumpOptDetails(comp(),
				        "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating weight factors "
...
void
OMR::CFG::getBranchCounters(TR::Node *node, TR::Block *block, int32_t *taken, int32_t *notTaken, TR::Compilation *comp)
{
		...
		while (parent) {
			...
			if (parent->isNaturalLoop()) {
				if (node->getOpCode().isIf()) {
					...
					bool loopContainsFallThrough =
					        parent->contains(fallThroughBlock->getStructureOf(), getStructure());
		...
		if (frequency >= 0) {
			...
			if (successorFrequency > 0) {
				...
				for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
					...
					// dumpOptDetails("1Edge %p between %d and %d has freq %d (_maxEdgeFrequency
					// %d)\n", e, e->getFrom()->getNumber(), e->getTo()->getNumber(),
					// e->getFrequency(), _maxEdgeFrequency);
				}
...
int32_t
TR::CFGNode::denormalizedFrequency(int32_t normalizedFrequency, int32_t maxFrequency)
{
...
TR::CFGEdge::normalizeFrequency(int32_t maxEdgeFrequency)
{
	...
	// getFrom()->getNumber(), getTo()->getNumber(), getFrequency(), maxEdgeFrequency);
...
void
OMR::CFG::normalizeNodeFrequencies(TR_BitVector *nodesToBeNormalized, TR_Array<TR::CFGEdge *> *edgeArray)
{
