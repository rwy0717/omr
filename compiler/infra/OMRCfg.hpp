...
public:
	...
	CFG(TR::Compilation *c, TR::ResolvedMethodSymbol *m, TR::Region &r)
	        : _structureRegion(c->trMemory()->heapMemoryRegion()), _internalRegion(r)
	{
		init(c, m);
	}
	...
	void copyExceptionSuccessors(
	        TR::CFGNode *from, TR::CFGNode *to, bool (*predicate)(TR::CFGEdge *) = OMR::alwaysTrue);
...
public:
	TR_SuccessorIterator(TR::CFGNode *node) : TR_CFGIterator(node->getSuccessors(), node->getExceptionSuccessors())
	{}
};
