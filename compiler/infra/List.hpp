...
public:
	...
	ListElement<T> *add(T *p)
	{
		TR_ASSERT(this->_region != NULL,
		        "Should not be using heap allocation to add to a persistent list. The 'add' routine in "
	...
	ListElement<T> *addAfter(T *elem, ListElement<T> *previous)
	{
		...
		if (previous == NULL)
			...
			TR_ASSERT(this->_region != NULL,
			        "Should not be using heap allocation to add to a persistent list. The 'addAfter' "
			        "routine in TR_PersistentList should be called\n");
...
public:
	...
	ListIterator(ListBase<T> *p) : _pHead(p->getListHead()), _pCursor(p->getListHead()) {}
...
public:
	...
	ListElement<T> *add(T *p)
	{
		TR_ASSERT(_pList->_region != NULL,
		        "Should not be using heap allocation to append to a persistent list. Should use "
		        "'addPersistent' instead\n");
	...
	{
		TR_ASSERT(_pList->_region == NULL,
		        "Should not be using persistent allocation to append to a non-persistent list.\n");
...
public:
	TR_ScratchList(TR_Memory *m) : List<T>(m) { List<T>::_region = &(m->currentStackRegion()); }
