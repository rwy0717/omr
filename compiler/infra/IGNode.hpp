...
class TR_IGNode {
	...
	enum { MustBeSpilled = 0x01, // Node must be spilled
...
public:
	...
	TR_IGNode(TR_Memory *m)
	        : _pEntity(NULL), _index(0), _degree(0), _workingDegree(0), _colour(UNCOLOURED), _adjList(m), _flags(0)
	{}

	TR_IGNode(void *p, TR_Memory *m)
	        : _pEntity(p), _index(0), _degree(0), _workingDegree(0), _colour(UNCOLOURED), _adjList(m), _flags(0)
	{}
	...
	IGNodeDegree decWorkingDegree()
	{
		TR_ASSERT(_workingDegree >= 1,
		        "decWorkingDegree: working degree is less than 0! (_workingDegree=%d nodeIndex=%d)",
