...
public:
	...
	/**
	 * @brief Copies the provided object into the first unused element at the end of the array and returns the index
	 * to the newly created element.  This may require growing the backing array.
	...
	{
		TR_ASSERT(index < _nextIndex, "TR_Array::element, Invalid array index %d while size is %d", index,
		        _nextIndex);
		return _array[index];
	}
	...
	{
		TR_ASSERT(index < _nextIndex, "TR_Array::operator[] const, Invalid array index %d while size is %d",
		        index, _nextIndex);
		return _array[index];
	}
