...
void *
SimpleRegex::Component::operator new(size_t size, PERSISTENT_NEW_DECLARE, size_t numChars)
{
...
//
SimpleRegex::Simple *
SimpleRegex::processSimple(char *&s, TR_YesNoMaybe allowAlternates, bool &foundError)
{
	int32_t i, lo, hi;
	char *startSimple = s;
	...
	if (m->remainder == NULL)
		...
		if ((m->remainder->component->type == wildcards && (m->remainder->component->data.counts & 1) != 0)
			...
			case simple_string: i = (int32_t)strlen(m->remainder->component->data.str); break;
			case wildcards: i = m->remainder->component->data.counts >> 1; break;
			case char_alternatives: i = 1; break;
			default:
				/*** ???? ***/
				break;
			}
...
bool
SimpleRegex::Simple::matchesRemainder(const char *s, bool isCaseSensitive, bool useLocale)
{
	if (remainder == NULL)
		return s[0] == '\0';
...
SimpleRegex::Simple::print()
{
	...
	case simple_string: TR_VerboseLog::write("%s", component->data.str); break;
	}
	if (remainder)
		remainder->print();
}
...
bool
SimpleRegex::match(TR::SimpleRegex *regex, TR_ResolvedMethod *feMethod, bool isCaseSensitive)
{
...
bool
SimpleRegex::matchIgnoringLocale(TR::SimpleRegex *regex, const char *s, bool isCaseSensitive)
{
