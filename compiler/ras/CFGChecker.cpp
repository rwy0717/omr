...
TR_CFGChecker::performCorrectnessCheck()
{
	...
	if (_cfg->getStart()->getSuccessors().size() != 1) {
		...
		if (_cfg->getStart()->getSuccessors().front()->getTo() != _blocksInProgramOrder[0]) {
			if (_outFile)
				trfprintf(_outFile,
				        "The successor block for the (dummy) start block in the CFG is NOT the start "
				        "block in the actual program\n");
			_successorsCorrect = false;
		}
	}
...
TR_CFGChecker::arrangeBlocksInProgramOrder()
{
	...
	while (cursorTreeTop) {
		...
		if (!_blockChecklist.isSet(b->getNumber())) {
			if (_outFile)
				trfprintf(_outFile,
				        "Block %d [%p]  at tree node [%p] is in the trees but not in the CFG\n",
				        b->getNumber(), b, node);
			return false;
		}
	...
	*/
	...
	for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();
		...
		for (auto edge2 = block->getExceptionSuccessors().begin();
			...
			if (((*edge) != (*edge2)) && (next == (*edge2)->getTo())) {
				if (_outFile)
					trfprintf(_outFile,
					        "Exception successor block [%d] of block [%d] is listed more than "
					        "once\n",
					        next->getNumber(), block->getNumber());
				return false;
			}
	...
	if (!(opCode.isBranch() || opCode.isSwitch() || opCode.isReturn() || opCode.isJumpWithMultipleTargets()
		...
		if (!(block->getSuccessors().size() == 1)) {
			...
			if (_outFile)
				trfprintf(_outFile,
				        "Last non-fence opcode in block [%d] is not a branch, switch, or a return and "
				        "it does not have exactly one successor\n",
				        block->getNumber());
			return false;
		}
		...
		if (block->getSuccessors().front()->getTo() != _blocksInProgramOrder[i + 1]) {
			if (_outFile)
				trfprintf(_outFile,
				        "Successor block [%d] of block [%d] (with no branch, switch, or return at the "
				        "end) is not the fall through block\n",
	...
	if (opCode.isBranch()) {
		...
		if (opCode.getOpCodeValue() == TR::Goto) {
			if (!(block->getSuccessors().size() == 1)) {
				if (_outFile)
					trfprintf(_outFile,
					        "Number of successors of block [%d] having a goto at the exit is not "
					        "equal to one\n",
					        block->getNumber());
				return false;
			}
			...
			if (!(numUniqueChildren == block->getSuccessors().size())) {
				if (_outFile)
					trfprintf(_outFile,
					        "Number of successors of block [%d] having an if at the exit is not "
					        "equal to the number of unique targets of the if\n",
					        block->getNumber());
				return false;
			}
		}
		...
		for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
			...
			if (!((next == fallThroughBlock) || (next == branchBlock))) {
				if (_outFile)
					trfprintf(_outFile,
					        "Successor block [%d] of block [%d] containing a branch does not match "
					        "the destination(s) specified in the IL branch instruction\n",
					        next->getNumber(), block->getNumber());
				return false;
			}
		}
		...
		if (!(getNumUniqueCases(lastNode) == block->getSuccessors().size())) {
			if (_outFile)
				trfprintf(_outFile,
				        "Number of successors of block [%d] having a switch at the exit is not equal "
				        "to the number of destinations in the IL switch instruction\n",
				        block->getNumber());
			return false;
		}
		...
		for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
			...
			if (!equalsAnyChildOf(next->getEntry(), lastNode)) {
				if (_outFile)
					trfprintf(_outFile,
					        "Successor block [%d] of block [%d] containing a switch does not match "
					        "any of the destinations specified in the IL switch instruction\n",
					        next->getNumber(), block->getNumber());
				return false;
			}
		}
		...
		if (!(block->getSuccessors().size() == 1)) {
			if (_outFile)
				trfprintf(_outFile,
				        "Number of successors of block [%d] having a return at the exit is not equal "
				        "to one\n",
				        block->getNumber());
			return false;
		}
		...
		for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
			...
			if (next != cfgEnd) {
				if (_outFile)
					trfprintf(_outFile,
					        "Successor block [%d] of block [%d] containing a return is NOT the "
					        "exit block\n",
					        next->getNumber(), block->getNumber());
				return false;
			}
...
TR_CFGChecker::getNumUniqueCases(TR::Node *switchNode)
{
	...
	uniqueTreeTops =
	        (TR::TreeTop **)_cfg->comp()->trMemory()->allocateStackMemory(numCases * sizeof(TR::TreeTop *));
	...
	/*
	  ...
	  from->getNumber()); _isCFGConsistent = false; break;
	     ...
	     if (!from->getSuccessors().find(edge) && !from->getExceptionSuccessors().find(edge))
	        {
	        if (_outFile) trfprintf(_outFile, "Edge between block [%p] numbered %d and block [%p] numbered %d is in
	  the CFG edge list but not in successors list\n", from, from->getNumber(), edge->getTo(),
...
TR_CFGChecker::isConsistent(TR::Block *block)
{
	...
	for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
		...
		if (matchEdge == nextBlock->getSuccessors().end()) {
			if (_outFile)
				trfprintf(_outFile,
				        "Predecessor block [%d] of block [%d] does not contain block [%d] in its "
				        "successors list\n",
	...
	while (basicP != block->getPredecessors().end()) {
		...
		if (!foundMe) {
			if (_outFile) {
				trfprintf(_outFile,
				        "ERROR: block_%d is a predecessor of block_%d but the reverse is not true\n",
	...
	while (basicS != block->getSuccessors().end()) {
		...
		if (!foundMe) {
			if (_outFile) {
				trfprintf(_outFile,
				        "ERROR: block_%d is a successor of block_%d but the reverse is not true\n",
	...
	for (auto nextEdge = block->getExceptionPredecessors().begin();
		...
		if (matchEdge == nextBlock->getExceptionSuccessors().end()) {
			if (_outFile)
				trfprintf(_outFile,
				        "Exception Predecessor block [%d] of block [%d] does not contain block [%d] in "
				        "its exception successors list\n",
				        nextBlock->getNumber(), block->getNumber(), block->getNumber());
			return false;
		}
	}
