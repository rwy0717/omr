...
TR_FilterBST *
TR_Debug::addFilter(char *&filterString, int32_t scanningExclude, int32_t optionSetIndex, int32_t lineNum,
...
TR_Debug::inlinefileOption(char *option, void *base, TR::OptionTable *entry, TR::Options *cmdLineOptions)
{
	...
	if (!success) {
		...
		return fail; // We want to fail if we can't read the file because it is too easy to miss that the file
		             // wasn't picked up
	}
	return endOpt;
}
 ...
 */
char *
TR_Debug::limitfileOption(char *option, void *base, TR::OptionTable *entry, TR::Options *cmdLineOptions, bool loadLimit,
			...
			if (strncmp(limitReadBuffer, "-precompileMethod", 17) == 0) {
				...
				if (!addFilter(p, (('+' == includeFlag) ? 0 : 1), optionSet, lineNumber, loadLimit)) {
					...
					break;
			                   ...
			                   PSEUDO_RANDOM_NUMBER_PREFIX_LEN)
			        == 0) {
...
TR_Debug::limitOption(char *option, void *base, TR::OptionTable *entry, TR::Options *cmdLineOptions, bool loadLimit)
{
	...
	if (methodRegex && !entry->parm1 && (*p == '(' || *p == '{')) {
		...
		//
		TR::OptionSet *newSet =
		        (TR::OptionSet *)(TR::Compiler->regionAllocator.allocate(sizeof(TR::OptionSet)));
...
TR_Debug::print(TR_FilterBST *filter)
{
	...
	*/
	...
	case TR_FILTER_EXCLUDE_NAME_ONLY: TR_VerboseLog::write("   -%s", "NAME_ONLY"); break;
	case TR_FILTER_EXCLUDE_NAME_AND_SIG: TR_VerboseLog::write("   -%s", "NAME_AND_SIG"); break;
	case TR_FILTER_EXCLUDE_SPECIFIC_METHOD: TR_VerboseLog::write("   -%s", "SPECIFIC_METHOD"); break;
	case TR_FILTER_EXCLUDE_REGEX: TR_VerboseLog::write("   -%s", "REGEX"); break;
	case TR_FILTER_NAME_ONLY: TR_VerboseLog::write("   +%s", "NAME_ONLY"); break;
	case TR_FILTER_NAME_AND_SIG: TR_VerboseLog::write("   +%s", "NAME_AND_SIG"); break;
	case TR_FILTER_SPECIFIC_METHOD: TR_VerboseLog::write("   +%s", "SPECIFIC_METHOD"); break;
	case TR_FILTER_REGEX: TR_VerboseLog::write("   +%s", "REGEX"); break;
	}

	switch (filter->_filterType) {
	case TR_FILTER_EXCLUDE_NAME_ONLY: TR_VerboseLog::write("   {^*.%s(*}\n", filter->getName()); break;
	...
	case TR_FILTER_EXCLUDE_REGEX:
		...
		TR_VerboseLog::write("\n");
		break;
	case TR_FILTER_NAME_ONLY: TR_VerboseLog::write("   {*.%s(*}\n", filter->getName()); break;
	...
	for (TR_FilterBST *filter = _compilationFilters->samplingPoints; filter; filter = filter->getNext()) {
		if (filter->getFilterType() == TR_FILTER_SAMPLE_INTERPRETED) {
			...
			TR_VerboseLog::write("(%d)\tCompiled %s.%s%s\tlevel=%d%s\n", filter->getTickCount(),
			        filter->getClass(), filter->getName(), filter->getSignature(), filter->getSampleLevel(),
        ...
        const char *methodSig, TR::CompilationFilters *filters, TR_FilterBST *&filter, TR::Method::Type methodType)
{
	...
	if (filters->hasClassNameSigFilter() || filters->hasNameSigFilter()) {
		if (filters->hasClassNameSigFilter()) {
			...
			if (filter)
				filter = filter->find(methodName, methodNameLen, methodClass, methodClassLen,
		...
		if (!filter && filters->hasNameSigFilter()) {
			...
			if (filter)
				filter = filter->find(
				        methodName, methodNameLen, "", 0, methodSignature, methodSignatureLen);
		}
	}
	...
	if (filter) {
		...
		case TR_FILTER_EXCLUDE_NAME_AND_SIG:
		case TR_FILTER_EXCLUDE_SPECIFIC_METHOD:
		case TR_FILTER_EXCLUDE_REGEX: excluded = true; break;
		default: excluded = false; break;
		}
	}
...
bool
TR_Debug::methodSigCanBeCompiledOrRelocated(
	...
	if (optFile) {
		...
		while (fgets(lineBuffer, sizeof(lineBuffer), optFile)) {
			if (optCount >= (sizeof(optNumBuffer) / sizeof(optNumBuffer[0]))) {
				TR_VerboseLog::write("<JIT: WARNING: reached limit of %d optFile lines; ignoring "
				                     "subsequent lines\n",
				        optCount);
				break;
			}
			...
			if (optNum == OMR::numOpts)
				TR_VerboseLog::write(
				        "<JIT: WARNING: ignoring optFile line; no matching opt name for '%s'\n", name);
		}
