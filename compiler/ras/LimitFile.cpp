...
bool TR_Debug::methodSigCanBeFound(
    const char* methodSig, TR::CompilationFilters* filters, TR_FilterBST*& filter, TR_Method::Type methodType)
{
...
bool TR_Debug::methodSigCanBeCompiledOrRelocated(
    const char* methodSig, TR_FilterBST*& filter, bool loadLimit, TR_Method::Type methodType)
{
    ...
    if (optFile) {
        ...
        while (fgets(lineBuffer, sizeof(lineBuffer), optFile)) {
            if (optCount >= (sizeof(optNumBuffer) / sizeof(optNumBuffer[0]))) {
                TR_VerboseLog::write(
                    "<JIT: WARNING: reached limit of %d optFile lines; ignoring subsequent lines\n", optCount);
                break;
            }
