...
static char optionCategories[] = {' ', 'C', 'O', 'L', 'D', 'R', 'I', 'M', 0}; // Must match categories[] in
                                                                              // codegen.dev/Options.cpp
...
TR::FILE *
TR_Debug::findLogFile(TR::Options *cmdLineOptions, TR::OptionSet *optSet, char *logFileName)
{
...
int32_t
TR_Debug::findLogFile(const char *logFileName, TR::Options *aotCmdLineOptions, TR::Options *jitCmdLineOptions,
...
void
TR_Debug::dumpOptionHelp(TR::OptionTable *firstOjit, TR::OptionTable *firstOfe, TR::SimpleRegex *nameFilter)
{
	char *p;
	int32_t i, lastPos;
...
void
TR_Debug::dumpOptions(char *optionsType, char *options, char *envOptions, TR::Options *cmdLineOptions,
        TR::OptionTable *ojit, TR::OptionTable *ofe, void *feBase, TR_FrontEnd *fe)
{
	...
	while (ojit->name || ofe->name) {
		if (ojit->name && (!ojit->msg || (ojit->msg[0] == 'F' && !(ojit->msgInfo & OPTION_FOUND)))
		        && !ojit->enabled) {
			ojit++;
			continue;
		}
		if (ofe->name && (!ofe->msg || (ofe->msg[0] == 'F' && !(ofe->msgInfo & OPTION_FOUND)))
		        && !ofe->enabled) {
			ofe++;
			continue;
		}
		...
		if (entry->fcn == TR::Options::setBit) {
			...
			if (entry->msg[0] == 'P')
		        ...
		        || entry->fcn == TR::Options::set32BitSignedNumeric || entry->fcn == TR::Options::setCount) {
			value = (intptrj_t)(*((int32_t *)(base + entry->parm1)));
			if (entry->msg[0] == 'P')
				printIt = (value != 0);
		...
		if (printIt && (entry->msg[0] == 'F' || extendCheck)) {
			...
			if (regex)
				...
				// since java uses different function, we need to check for that to get our verbose
				// options printed
				TR_VerboseLog::write("{");
				base = (char *)cmdLineOptions;
				char *sep = "";
