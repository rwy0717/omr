...
public:
	...
	//
	static DebugCounter *getDebugCounter(
	...
	void getInsertionCounterNames(TR::Compilation *comp, TR_OpaqueMethodBlock *method, int32_t bytecodeIndex,
...
class DebugCounterAggregation : public DebugCounterBase {
	...
	struct CounterDelta {
		TR_ALLOC(TR_Memory::DebugCounter)
		CounterDelta(DebugCounter *counter, int32_t delta) : counter(counter), delta(delta) {}
...
class DebugCounterGroup {
	...
	CS2::HashTable<const char *, DebugCounterAggregation *, TRPersistentMemoryAllocator>
	...
	TR::Monitor *_countersMutex; /**< Monitor used to synchronize read/write actions to _countersHashTable,
	                                otherwise we may have a race */
