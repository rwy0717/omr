...
public:
	...
	void add(TR::Node *node)
	{
		int32_t index = _sideTable.indexOf(node);
		TR_ASSERT(
		        index >= _basis, "Cannot mark node n%dn before basis %d live", node->getGlobalIndex(), _basis);
...
public:
	...
	public:
		Iterator(LiveNodeWindow &window)
		        : _window(window), _bvi(window._liveOffsets), _currentOffset(0xdead /* anything non-negative */)
		{
			stepForward();
		}
