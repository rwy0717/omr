...
TR_Debug::performTransformationImpl(bool canOmitTransformation, const char *format, ...)
{
	...
	if (canOmitTransformation) {
		...
		if (regex
		        && (TR::SimpleRegex::match(regex, curTransformationIndex)
...
TR_Debug::printInstrDumpHeader(const char *title)
{
	...
	int prefixWidth = addressFieldWidth * 2 + codeByteColumnWidth + 12; // 8 bytes of offsets, 2 spaces, and opening
	...
	if (strcmp(title, "Post Instruction Selection Instructions") == 0
		...
		trfprintf(_file, "\n%*s+--------------------------------------- instruction address",
		        addressFieldWidth - 1, " ");
		trfprintf(_file,
		        "\n%*s|        +----------------------------------------- instruction offset from start of "
		        "method",
		        addressFieldWidth - 1, " ");
		trfprintf(_file,
		        "\n%*s|        | %*s+------------------------------------------ corresponding TR::Instruction "
		        "instance",
...
TR_Debug::printPrefix(TR::FILE *pOutFile, TR::Instruction *instr, uint8_t *cursor, uint8_t size)
{
	if (cursor != NULL) {
		...
		int addressFieldWidth = TR::Compiler->debug.hexAddressFieldWidthInChars();
		int codeByteColumnWidth = TR::Compiler->debug.codeByteColumnWidth();
		int prefixWidth = addressFieldWidth * 2 + codeByteColumnWidth + 12; // 8 bytes of offsets, 2 spaces, and
        ...
        TR::FILE *pOutFile, TR::LabelSymbol *label, uint8_t *cursor, const char *comment1, const char *comment2)
{
	...
	if (_comp->getOption(TR_MaskAddresses)) {
		trfprintf(pOutFile, "\n\n%*s %08x %*s", TR::Compiler->debug.hexAddressFieldWidthInChars(), "*Masked*",
...
TR_Debug::print(TR::SymbolReference *symRef, TR_PrettyPrinterString &output, bool hideHelperMethodInfo, bool verbose)
{
	...
	uint32_t numSpaces;
	TR_PrettyPrinterString symRefNum(this), symRefOffset(this), symRefAddress(this), symRefName(this),
	        symRefKind(this), otherInfo(this), symRefWCodeId(this), symRefObjIndex(this), labelSymbol(this);
	...
	if (sym) {
		...
		case TR::Symbol::IsMethod: {
			...
			if (!inDebugExtension()) {
				...
				case TR::MethodSymbol::Virtual: symRefKind.append(" virtual"); break;
				case TR::MethodSymbol::Interface: symRefKind.append(" interface"); break;
				case TR::MethodSymbol::Static: symRefKind.append(" static"); break;
				case TR::MethodSymbol::Special: symRefKind.append(" special"); break;
				case TR::MethodSymbol::Helper: symRefKind.append(" helper"); break;
				case TR::MethodSymbol::ComputedStatic: symRefKind.append(" computed-static"); break;
				case TR::MethodSymbol::ComputedVirtual: symRefKind.append(" computed-virtual"); break;
				default: symRefKind.append(" UNKNOWN"); break;
				}
		...
		case TR::Symbol::IsLabel:
			...
			if (!labelSymbol.isEmpty())
				labelSymbol.append(" ");
			break;
		default: TR_ASSERT(0, "unexpected symbol kind");
		}
		otherInfo.append(" [flags 0x%x 0x%x ]", sym->getFlags(), sym->getFlags2());
	}

	numSpaces = getNumSpacesAfterIndex(
	        symRef->getReferenceNumber(), getIntLength(_comp->getSymRefTab()->baseArray.size()));
	...
	if (verbose) {
		...
		if (hideHelperMethodInfo)
			...
			output.append(" %s%s%s[%s%s%s]%s%s", symRefName.getStr(), labelSymbol.getStr(),
			        symRefWCodeId.getStr(), symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ",
		...
		if (hideHelperMethodInfo)
			output.append(" %s%s[%s%s%s]%s", labelSymbol.getStr(), symRefWCodeId.getStr(),
			        symRefNum.getStr(), symRefOffset.isEmpty() ? "" : " ", symRefOffset.getStr(),
			        symRefObjIndex.getStr());
		else
			output.append(" %s%s%s[%s%s%s%s%s]%s%s", symRefName.getStr(), labelSymbol.getStr(),
			        symRefWCodeId.getStr(), symRefNum.getStr(), symRefKind.isEmpty() ? "" : " ",
			        symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ", symRefOffset.getStr(),
			        symRefObjIndex.getStr(), otherInfo.getStr());
	}
}
...
TR_Debug::getName(TR::Symbol *sym)
{
	...
	//
	return getName(
	        (void *)sym, "SYM_", _nextSymbolNumber++, _comp->getAddressEnumerationOption(TR_EnumerateSymbol));
}
...
TR_Debug::getName(TR::SymbolReference *symRef)
{
	...
	if (index < nonhelperIndex) {
		...
		case TR::SymbolReferenceTable::contiguousArraySizeSymbol: return "<contiguous-array-size>";
		case TR::SymbolReferenceTable::discontiguousArraySizeSymbol: return "<discontiguous-array-size>";
		case TR::SymbolReferenceTable::arrayClassRomPtrSymbol: return "<array-class-rom-ptr>";
		case TR::SymbolReferenceTable::vftSymbol: return "<vft-symbol>";
		case TR::SymbolReferenceTable::currentThreadSymbol: return "<current-thread>";
		case TR::SymbolReferenceTable::thisRangeExtensionSymbol: return "<this-range-extension>";
		case TR::SymbolReferenceTable::recompilationCounterSymbol: return "<recompilation-counter>";
		case TR::SymbolReferenceTable::counterAddressSymbol: return "<recompilation-counter-address>";
		case TR::SymbolReferenceTable::countForRecompileSymbol: return "<count-for-recompile>";
		case TR::SymbolReferenceTable::gcrPatchPointSymbol: return "<gcr-patch-point>";
		case TR::SymbolReferenceTable::startPCSymbol: return "<start-PC>";
		case TR::SymbolReferenceTable::compiledMethodSymbol: return "<J9Method>";
		case TR::SymbolReferenceTable::excpSymbol: return "<exception-symbol>";
		case TR::SymbolReferenceTable::indexableSizeSymbol: return "<indexable-size>";
		case TR::SymbolReferenceTable::resolveCheckSymbol: return "<resolve check>";
		case TR::SymbolReferenceTable::arraySetSymbol: return "<arrayset>";
		case TR::SymbolReferenceTable::arrayCopySymbol: return "<arraycopy>";
		case TR::SymbolReferenceTable::prefetchSymbol: return "<prefetch>";
		case TR::SymbolReferenceTable::arrayTranslateSymbol: return "<arraytranslate>";
		case TR::SymbolReferenceTable::arrayTranslateAndTestSymbol: return "<arraytranslateandtest>";
		case TR::SymbolReferenceTable::long2StringSymbol: return "<long2String>";
		case TR::SymbolReferenceTable::bitOpMemSymbol: return "<bitOpMem>";
		case TR::SymbolReferenceTable::reverseLoadSymbol: return "<reverse-load>";
		case TR::SymbolReferenceTable::reverseStoreSymbol: return "<reverse-store>";
		case TR::SymbolReferenceTable::arrayCmpSymbol: return "<arraycmp>";
		case TR::SymbolReferenceTable::currentTimeMaxPrecisionSymbol: return "<currentTimeMaxPrecision>";
		case TR::SymbolReferenceTable::singlePrecisionSQRTSymbol: return "<fsqrt>";
		case TR::SymbolReferenceTable::killsAllMethodSymbol: return "<killsAllMethod>";
		case TR::SymbolReferenceTable::usesAllMethodSymbol: return "<usesAllMethod>";
		case TR::SymbolReferenceTable::synchronizedFieldLoadSymbol: return "<synchronizedFieldLoad>";
		case TR::SymbolReferenceTable::atomicAddSymbol: return "<atomicAdd>";
		case TR::SymbolReferenceTable::atomicFetchAndAddSymbol: return "<atomicFetchAndAdd>";
		case TR::SymbolReferenceTable::atomicFetchAndAdd32BitSymbol: return "<atomicFetchAndAdd32Bit>";
		case TR::SymbolReferenceTable::atomicFetchAndAdd64BitSymbol: return "<atomicFetchAndAdd64Bit>";
		case TR::SymbolReferenceTable::atomicSwapSymbol: return "<atomicSwap>";
		case TR::SymbolReferenceTable::atomicSwap32BitSymbol: return "<atomicSwap32Bit>";
		case TR::SymbolReferenceTable::atomicSwap64BitSymbol: return "<atomicSwap64Bit>";
		...
		case TR::SymbolReferenceTable::potentialOSRPointHelperSymbol: return "<potentialOSRPointHelper>";
		case TR::SymbolReferenceTable::osrFearPointHelperSymbol: return "<osrFearPointHelper>";
		case TR::SymbolReferenceTable::jProfileValueSymbol: return "<jProfileValuePlaceHolder>";
	...
	TR::Symbol *sym = symRef->getSymbol();
	switch (sym->getKind()) {
	case TR::Symbol::IsAutomatic: return getAutoName(symRef);
	case TR::Symbol::IsParameter: return getParmName(symRef);
	case TR::Symbol::IsStatic: return getStaticName(symRef);
	case TR::Symbol::IsResolvedMethod:
	case TR::Symbol::IsMethod: return getMethodName(symRef);
	case TR::Symbol::IsShadow: return getShadowName(symRef);
	case TR::Symbol::IsMethodMetaData: return getMetaDataName(symRef);
	case TR::Symbol::IsLabel: return getName((TR::LabelSymbol *)sym);
	default: TR_ASSERT(0, "unexpected symbol kind"); return "unknown name";
	}
}
...
TR_Debug::getAutoName(TR::SymbolReference *symRef)
{
	...
	char *name =
	        (char *)_comp->trMemory()->allocateHeapMemory(50 + TR::Compiler->debug.pointerPrintfMaxLenInChars());
	...
	if (symRef->getSymbol()->isSpillTempAuto()) {
		...
		if (symRef->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
			...
			if (symRef->holdsMonitoredObjectForSyncMethod())
				sprintf(name, "%.*s<auto slot %d holds monitoredObject syncMethod>", debugNameLen,
				        debugName, slot);
			else
				sprintf(name, "%.*s<auto slot %d holds monitoredObject>", debugNameLen, debugName,
				        slot);
		} else
...
TR_Debug::getParmName(TR::SymbolReference *symRef)
{
	...
	char *debugName = getOwningMethod(symRef)->localName(slot, 0, debugNameLen, comp()->trMemory()); // TODO: Proper
	                                                                                                 // bcIndex
	                                                                                                 // somehow;
	                                                                                                 // TODO: proper
	                                                                                                 // length
...
TR_Debug::getShadowName(TR::SymbolReference *symRef)
{
	...
	for (int32_t i = TR::SymbolReferenceTable::firstCommonNonhelperNonArrayShadowSymbol;
		...
		TR::SymbolReference *other =
		        _comp->getSymRefTab()->element((TR::SymbolReferenceTable::CommonNonhelperSymbol)i);
		if (other && other->getSymbol() == symRef->getSymbol())
			return commonNonhelperSymbolNames[i
...
TR_Debug::dumpLiveRegisters()
{
	...
	for (i = 0; i < NumRegisterKinds; i++) {
		...
		TR_LiveRegisters *liveRegisters = _comp->cg()->getLiveRegisters(kind);
		if (liveRegisters) {
			for (TR_LiveRegisterInfo *info = liveRegisters->getFirstLiveRegister(); info;
			        info = info->getNext()) {
...
TR_Debug::dumpLiveRegisters(TR::FILE *pOutFile, TR_RegisterKinds rk)
{
	...
	for (TR_LiveRegisterInfo *p = liveReg->getFirstLiveRegister(); p; p = p->getNext()) {
		...
		if (regPair)
			...
			trfprintf(
			        pOutFile, "\t[" POINTER_PRINTF_FORMAT "] %d:  " POINTER_PRINTF_FORMAT "  ", p, n, reg);
		}
...
void
TR_Debug::dumpSingleTreeWithInstrs(TR::TreeTop *treeTop, TR::Instruction *instr, bool dumpTrees, bool dumpInstrs,
			...
			while (cursor) {
				...
				if (outlinedCursor) {
					...
					if (outlinedCursor != lastOutlinedTarget) {
						// Avoiding dumping the same outlined code sequence more than once by
						// keeping track of the last sequence we dumped Not fool-proof, but
						// cheap
	...
	while (instr) {
		...
		if (printLinenos) {
			...
			if ((crtLineNo != -1)
				...
				// Following is somewhat PPC specific - line numbers for "primary" source are relative
				// to the first line number of the method. Line numbrers for includes are absoluted .
...
TR_Debug::printRegisterKilled(TR::Register *reg)
{
	...
	if (node) {
		trfprintf(pOutFile, " [%s] (%3d)%*s%s   ", getName(node), node->getReferenceCount(),
		        _comp->cg()->_indentation, " ", getName(node->getOpCode()));
	} else {
...
TR_Debug::printNodeEvaluation(TR::Node *node, const char *relationship, TR::Register *reg, bool printOpCode)
{
	...
	trfprintf(pOutFile, " [%s] (%3d)%*s%s%s%s%s\n", getName(node), node->getReferenceCount(),
	        _comp->cg()->_indentation, " ", (printOpCode) ? getName(node->getOpCode()) : "", relationship,
	        (reg) ? reg->getRegisterName(_comp) : "", (reg) ? (reg->isLive() ? " (live)" : " (dead)") : "");
}
...
TR_Debug::dumpMixedModeDisassembly()
{
	...
	for (; inst; inst = inst->getNext()) {
		if (inst->getNode() != NULL
		        && (!n
		                || ((inst->getNode()->getByteCodeInfo().getCallerIndex()
			...
			trfprintf(pOutFile, "\n\n");
			char *indent =
			        (char *)_comp->trMemory()->allocateHeapMemory(6 + 3 * (_comp->getMaxInlineDepth() + 1));
...
TR_Debug::getName(TR::Register *reg, TR_RegisterSizes size)
{
	...
	if (reg->getRegisterPair()) {
		...
		char *buf = (char *)_comp->trMemory()->allocateHeapMemory(maxPrefixSize + 6 + 11); // max register kind
		                                                                                   // name size plus
		                                                                                   // underscore plus
		                                                                                   // 10-digit reg num
		                                                                                   // plus null
...
const char *
TR_Debug::getGlobalRegisterName(TR_GlobalRegisterNumber regNum, TR_RegisterSizes size)
{
	...
	if (reg->getRealRegister()) {
		...
		if (reg->getRealRegister()->getState() == TR::RealRegister::Assigned) {
			...
			if (reg->getRealRegister()->getState() == TR::RealRegister::Locked
			        && reg->getAssignedRegister() != NULL && reg->getAssignedRegister() != reg) {
				TR::Register *virtReg = reg->getAssignedRegister();
...
TR_Debug::print(TR::FILE *pOutFile, TR_GCStackMap *map, TR::GCStackAtlas *atlas)
{
	...
	if (internalPtrMap) {
		...
		for (; currElement; currElement = currElement->getNextElement())
			trfprintf(pOutFile,
			        "      pinning array GC stack map index = %d Internal pointer regnum = %d\n",
	...
	if (!inDebugExtension() && !vguards.empty()) {
		...
		for (auto info = vguards.begin(); info != vguards.end(); ++info, ++i) {
			...
			for (TR_InnerAssumption *inner = innerIt.getFirst(); inner; inner = innerIt.getNext()) {
				trfprintf(pOutFile,
				        "\tInner Assumption: calleeSymbol=" POINTER_PRINTF_FORMAT " for parm "
				        "ordinal=%d\n",
...
TR_Debug::getLinkageConventionName(uint8_t lc)
{
	...
	case TR_Private: return "Private";
	case TR_System: return "System";
	case TR_Helper: return "Helper";
	default: TR_ASSERT(0, "Shouldn't get here"); return "(unknown linkage convention)";
	}
}
...
{
	switch (kind) {
	case TR_NoGuard: return "NoGuard";
	case TR_ProfiledGuard: return "ProfiledGuard";
	case TR_InterfaceGuard: return "InterfaceGuard";
	case TR_AbstractGuard: return "AbstractGuard";
	case TR_HierarchyGuard: return "HierarchyGuard";
	case TR_NonoverriddenGuard: return "NonoverriddenGuard";
	case TR_SideEffectGuard: return "SideEffectGuard";
	case TR_DummyGuard: return "DummyGuard";
	case TR_HCRGuard: return "HCRGuard";
	case TR_MutableCallSiteTargetGuard: return "MutableCallSiteTargetGuard";
	case TR_MethodEnterExitGuard: return "MethodEnterExitGuard";
	case TR_DirectMethodGuard: return "DirectMethodGuard";
	case TR_InnerGuard: return "InnerGuard";
	case TR_ArrayStoreCheckGuard: return "ArrayStoreCheckGuard";
	case TR_OSRGuard: return "OSRGuard";
	case TR_BreakpointGuard: return "BreakpointGuard";
	default: break;
	}
...
{
	switch (testType) {
	case TR_DummyTest: return "DummyTest";
	case TR_VftTest: return "VftTest";
	case TR_MethodTest: return "MethodTest";
	case TR_NonoverriddenTest: return "NonoverriddenTest";
	case TR_FSDTest: return "FSDTest";
	}
...
{
	switch (kind) {
	case gc_modron_wrtbar_none: return "None";
	case gc_modron_wrtbar_always: return "Always";
	case gc_modron_wrtbar_oldcheck: return "OldCheck";
	case gc_modron_wrtbar_cardmark: return "CardMark";
	case gc_modron_wrtbar_cardmark_incremental: return "CardMarkIncremental";
	case gc_modron_wrtbar_cardmark_and_oldcheck: return "CardMarkAndOldCheck";
	case gc_modron_wrtbar_satb:
	case gc_modron_wrtbar_satb_and_oldcheck: return "RealTime";
...
TR_Debug::getSpillKindName(uint8_t kind)
{
	...
	case TR_gprSpill: return "gpr";
	case TR_fprSpill: return "fpr";
	case TR_vrfSpill: return "vrf";
	case TR_volatileSpill: return "volatile";
	case TR_linkageSpill: return "linkage";
	case TR_vmThreadSpill: return "vmThread";
#if defined(TR_TARGET_X86)
	case TR_ecxSpill: return "ecx";
	case TR_eaxSpill: return "eax";
	case TR_edxSpill: return "edx";
#endif
#if defined(TR_TARGET_S390)
	case TR_litPoolSpill: return "litPool";
	case TR_staticBaseSpill: return "staticBase";
	case TR_gpr0Spill: return "gpr0";
	case TR_gpr1Spill: return "gpr1";
	case TR_gpr2Spill: return "gpr2";
	case TR_gpr3Spill: return "gpr3";
	case TR_gpr4Spill: return "gpr4";
	case TR_gpr5Spill: return "gpr5";
	case TR_gpr6Spill: return "gpr6";
	case TR_gpr7Spill: return "gpr7";
	case TR_gpr8Spill: return "gpr8";
	case TR_gpr9Spill: return "gpr9";
	case TR_gpr10Spill: return "gpr10";
	case TR_gpr11Spill: return "gpr11";
	case TR_gpr12Spill: return "gpr12";
	case TR_gpr13Spill: return "gpr13";
	case TR_gpr14Spill: return "gpr14";
	case TR_gpr15Spill: return "gpr15";
#endif
	default: TR_ASSERT(0, "Unknown spill kind: %d", kind); return "(unknown spill kind)";
	}
}
...
TR_Debug::dumpSimulatedNode(TR::Node *node, char tagChar)
{
	...
	if (node->getOpCode().hasSymbolReference()) {
		...
		if (node->getType().isIntegral()) {
			...
			int64_t value;
			switch (node->getDataType()) {
			case TR::Int8: value = node->getByte(); break;
			case TR::Int16: value = node->getShortInt(); break;
	...
	if (!started) {
		...
		*startupInfo = 17 * 4; // ditto...
		...
		size = sprintf(appBuffer,
		        "\"C:\\Program Files\\Debugging Tools For Windows\\windbg\" -p %d -srcpath %s -c \"%s\"", ppid,
		        srcDir, cmdBuffer);
		TR_ASSERT(size < BUFFER_SIZE, "assertion failure");
...
counterCompare(const char *left, const char *right)
{
	...
	while (*left != 0 && *right != 0) {
		...
		if (leftSectionLength != rightSectionLength) {
			return numericComparisonMode ? leftSectionLength - rightSectionLength // Assume that numbers are
			                                                                      // not 0-padded, so any
			                                                                      // longer number must be
			                                                                      // bigger
			                             : strcmp(left, right);
		}
...
static int32_t
partition(void *array, int32_t first, int32_t last, int32_t elemSize, int (*cmpFunc)(void *, void *))
{
	int32_t i = first;
	int32_t j = last;
	while (1) {
...
static void
quicksort(void *array, int32_t first, int32_t last, int32_t elemSize, int (*cmpFunc)(void *, void *))
{
	int32_t mid;
	if (first < last) {
...
static void
qsort(void *array, int32_t nitems, int32_t elemSize, int (*cmpFunc)(void *, void *))
{
	quicksort(array, 0, nitems - 1, elemSize, cmpFunc);
}

void
TR_Debug::printDebugCounters(TR::DebugCounterGroup *counterGroup, const char *name)
{
	...
	for (i = 0; i < count; i++) {
		c = counterArray[i];
		if (c && c->getCount() != 0) {
			fprintf(counterFile, "%3d: %-*s | %12.0f | ", i, longestName, c->getName(),
			        (double)(c->getCount()));
