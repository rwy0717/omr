 ...
 */
TR::SoundnessRule::SoundnessRule(TR::Compilation *comp) : TR::MethodValidationRule(comp, OMR::soundnessRule) {}
	...
	for (TR::TreeTop *currentTree = start; currentTree != stop; currentTree = currentTree->getNextTreeTop()) {
		...
		if (next) {
			checkSoundnessCondition(currentTree, next->getNode() != NULL,
			        "Tree after n%dn must have a node", currentTree->getNode()->getGlobalIndex());
...
void
TR::SoundnessRule::checkNodeSoundness(
        TR::TreeTop *location, TR::Node *node, TR::NodeChecklist &ancestorNodes, TR::NodeChecklist &visitedNodes)
{
	...
	for (int32_t i = 0; i < node->getNumChildren(); i++) {
		...
		checkSoundnessCondition(
		        location, child != NULL, "n%dn child %d must not be NULL", node->getGlobalIndex(), i);
...
void
TR::SoundnessRule::checkSoundnessCondition(TR::TreeTop *location, bool condition, const char *formatStr, ...)
{
	if (!condition) {
...
TR::ValidateLivenessBoundaries::validate(TR::ResolvedMethodSymbol *methodSymbol)
{
	...
	for (TR::PostorderNodeOccurrenceIterator iter(start, comp(), "VALIDATE_LIVENESS_BOUNDARIES"); iter != stop;
		...
		if (node->getOpCodeValue() == TR::BBEnd) {
			...
			if (nextTree) {
				TR::checkILCondition(node, nextTree->getNode()->getOpCodeValue() == TR::BBStart, comp(),
...
void
TR::ValidateLivenessBoundaries::validateEndOfExtendedBlockBoundary(TR::Node *node, TR::LiveNodeWindow &liveNodes)
{
        ...
        TR::Node *node, TR::NodeSideTable<TR::NodeState> &nodeStates, TR::LiveNodeWindow &liveNodes)
{
	...
	if (liveNodes.contains(node)) {
		TR::checkILCondition(
		        node, state._futureReferenceCount >= 1, comp(), "Node already has reference count 0");
...
void
TR::ValidateNodeRefCountWithinBlock::validate(TR::TreeTop *firstTreeTop, TR::TreeTop *exitTreeTop)
{
...
TR::ValidateChildCount::validate(TR::Node *node)
{
	...
	if (opcode.expectedChildCount() != ILChildProp::UnspecifiedChildCount) {
		...
		if (!opcode.canHaveGlRegDeps()) {
			 ...
			 */
			TR::checkILCondition(node, node->getChild(actChildCount - 1)->getOpCodeValue() == TR::GlRegDeps,
			        comp(),
			        "Child count %d does not match expected value of %d (%d without GlRegDeps) and last "
			        "child is not a GlRegDeps",
			 ...
			 */
			TR::checkILCondition(node, actChildCount == expChildCount, comp(),
			        "Child count %d matches neither expected values of %d (without GlRegDeps) nor %d (with "
			        "GlRegDeps)",
		...
		for (auto i = 0; i < actChildCount; ++i) {
			...
			if (childOpcode.getOpCodeValue() != TR::GlRegDeps) {
				...
				TR::checkILCondition(node,
				        (expChildType >= TR::NumTypes || actChildType == expChildType), comp(),
				        "Child %d has unexpected type %s (expected %s)", i, actChildTypeName,
				        expChildTypeName);
			} else {
				/**
...
TR::Validate_ireturnReturnType::validate(TR::Node *node)
{
	...
	if (opcode.getOpCodeValue() == TR::ireturn) {
		...
		for (auto i = 0; i < childCount; ++i) {
			...
			TR::checkILCondition(node,
			        (actChildType == TR::Int32 || actChildType == TR::Int16 || actChildType == TR::Int8),
