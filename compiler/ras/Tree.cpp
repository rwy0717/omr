...
void TR_Debug::printLoadConst(TR::Node* node, TR_PrettyPrinterString& output)
{
    ...
    case TR::DecimalDouble: {
        output.append(
            " %g [" UINT64_PRINTF_FORMAT_HEX "]", node->getDouble(), node->getDouble(), node->getDoubleBits());
    } break;
#ifdef SUPPORT_DFP
...
void TR_Debug::printSubGraph(TR::FILE* pOutFile, TR_RegionStructure* regionStructure, uint32_t indentation)
{
    ...
    if (!inDebugExtension()) {
        ...
        if (verbose) {
            ...
            for (exitEdge = firstExitEdge; exitEdge != NULL; exitEdge = exitEdge->getNextElement()) {
                trfprintf(
                    pOutFile, "%*s*%d:", indentation + offset * 2, " ", exitEdge->getData()->getTo()->getNumber());
...
uint32_t TR_Debug::getIntLength(uint32_t num) const { return (num == 0 ? 1 : ((int)log10((double)num) + 1)); }
    ...
    bool printRefCounts, const char* prefix)
{
    ...
    if (_nodeChecklist.isSet(node->getGlobalIndex())) {
        if (printRefCounts) {
            trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s==>%s", prefix, globalIndexPrefix.getStr(), globalIndex,
                numSpaces, "", node->getReferenceCount(), indentation, " ", getName(node->getOpCode()));
            if (node->getOpCode().isLoadConst())
                printLoadConst(pOutFile, node);
#ifdef J9_PROJECT_SPECIFIC
            printBCDNodeInfo(pOutFile, node);
    ...
    if (node->getType().isBCD()) {
        if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst()
            ...
            if (node->hasSourcePrecision()) {
                output.append(" <prec=%d (len=%d) srcprec=%d> ", node->getDecimalPrecision(), node->getSize(),
                ...
                if (childType != expectedType && childType != TR::NoType
                    ...
                    if (getFile() != NULL) {
                        trfprintf(getFile(),
                            ...
                            getName(node), getName(child), getName(childType), getName((TR::DataTypes)expectedType));
                    }
