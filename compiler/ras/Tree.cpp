...
TR_Debug::printSymRefTable(TR::FILE *pOutFile, bool printFullTable)
{
	...
	if (currSymRefTabSize > 0 && _comp->getPrevSymRefTabSize() < currSymRefTabSize) // entry(ies) have been added to
	                                                                                // symbol refetence table since
	                                                                                // last optimization was
	                                                                                // performed
	{
		if (printFullTable)
			trfprintf(pOutFile, "\nSymbol References:\n------------------\n");
...
TR_Debug::printLoadConst(TR::Node *node, TR_PrettyPrinterString &output)
{
	...
	case TR::DecimalDouble: {
		output.append(" %g [" UINT64_PRINTF_FORMAT_HEX "]", node->getDouble(), node->getDouble(),
		        node->getDoubleBits());
	} break;
#ifdef SUPPORT_DFP
	...
	default: output.append(" Bad Type %s", node->getDataType().toString()); break;
	}
}
...
TR_Debug::printSubGraph(TR::FILE *pOutFile, TR_RegionStructure *regionStructure, uint32_t indentation)
{
	...
	for (exitEdge = firstExitEdge; exitEdge != NULL; exitEdge = exitEdge->getNextElement()) {
		trfprintf(pOutFile, "%*s(%s)%d -->%d\n", indentation + offset * 2, " ",
		        getName(exitEdge->getData()->getFrom()), exitEdge->getData()->getFrom()->getNumber(),
...
TR_Debug::printBlockOrders(TR::FILE *pOutFile, const char *title, TR::ResolvedMethodSymbol *methodSymbol)
{
	...
	while (tt != NULL) {
		...
		if (node && node->getOpCodeValue() == TR::BBStart) {
			...
			trfprintf(pOutFile, "block_%-4d\t[ " POINTER_PRINTF_FORMAT "]\tfrequency %4d",
				...
				for (i = 2; i < upperBound; i++) {
					...
					if (node->getOpCodeValue() == TR::trtLookup) {
						...
						if (sizeof(CASECONST_TYPE) == 8) {
							...
							if (unsigned_case)
								output.append(
								        "%u:\t", node->getChild(i)->getCaseConstant());
							else
								output.append(
								        "%d:\t", node->getChild(i)->getCaseConstant());
						}
...
//
int32_t
TR_Debug::printWithFixedPrefix(TR::FILE *pOutFile, TR::Node *node, uint32_t indentation, bool printChildren,
        bool printRefCounts, const char *prefix)
{
	...
	if (_nodeChecklist.isSet(node->getGlobalIndex())) {
		if (printRefCounts) {
			trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s==>%s", prefix, globalIndexPrefix.getStr(),
			        globalIndex, numSpaces, "", node->getReferenceCount(), indentation, " ",
			...
			trfprintf(pOutFile, "%s%s%dn%*s  %*s==>%s", prefix, globalIndexPrefix.getStr(), globalIndex,
			        numSpaces, "", indentation, " ", getName(node->getOpCode()));
			if (node->getOpCode().isLoadConst())
				printLoadConst(pOutFile, node);
#ifdef J9_PROJECT_SPECIFIC
			printBCDNodeInfo(pOutFile, node);
	...
	if (printRefCounts)
		...
		trfprintf(pOutFile, "%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
		        indentation, " ");
	...
	if (printChildren) {
		...
		if (node->getOpCode().isSwitch()) {
			...
			nodeCount += printWithFixedPrefix(
			        pOutFile, node->getFirstChild(), indentation, true, printRefCounts, prefix);

			globalIndex = node->getSecondChild()->getGlobalIndex();
			numSpaces = getNumSpacesAfterIndex(globalIndex, MAX_GLOBAL_INDEX_LENGTH);

			trfprintf(pOutFile, "\n%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(), globalIndex,
			        numSpaces, "", indentation, " ");
			nodeCount++;
			output.append("default ");
			_comp->incrNodeOpCodeLength(output.getLength());
			trfprintf(pOutFile, "%s", output.getStr());
			...
			if (node->getOpCodeValue() == TR::lookup || node->getOpCodeValue() == TR::trtLookup) {
				for (i = 2; i < upperBound; i++) {
					...
					trfprintf(pOutFile, "\n%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(),
				...
				if (upperBound < node->getNumChildren()
				        && node->getChild(upperBound)->getOpCodeValue() != TR::GlRegDeps) {
					nodeCount += printWithFixedPrefix(pOutFile, node->getChild(upperBound),
			...
			for (i = 0; i < node->getNumChildren(); i++) {
				...
				nodeCount += printWithFixedPrefix(
				        pOutFile, node->getChild(i), indentation, true, printRefCounts, prefix);
			}
			setCurrentParentAndChildIndex(oldParent, oldChildIndex);
		}
	}
...
TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE *pOutFile, TR::Node *node, uint32_t indentation)
{
	...
	if (node->getOpCodeValue() == TR::BBStart && !node->getBlock()->isExtensionOfPreviousBlock()
	        && node->getBlock()->getPrevBlock()) // a block that is not an extension of previous block and is not
	                                             // the first block
	{
		trfprintf(pOutFile, "\n");
	}
	...
	trfprintf(pOutFile, "%s%dn%*s %*s", globalIndexPrefix.getStr(), node->getGlobalIndex(), numSpaces, "",
	        indentation, " ");
...
TR_Debug::printNodeInfo(TR::Node *node, TR_PrettyPrinterString &output, bool prettyPrint)
{
	...
	if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
		...
		if (block->isCatchBlock() && !inDebugExtension()) {
			...
			if (classNameChars) {
				...
				classNameChars = "...";
				length = 3;
				output.append(" (catches %.*s)", length, classNameChars); // passing literal string
				                                                          // shouldn't use getName()
			}
		...
		if ((nextBlock = block->getNextBlock()) && !nextBlock->isExtensionOfPreviousBlock()) // end of a block
		                                                                                     // that is not the
		                                                                                     // last block and
		                                                                                     // the next block
		                                                                                     // is not an
		...
		bool isParentGlRegDep = getCurrentParent() ? (getCurrentParent()->getOpCodeValue() == TR::GlRegDeps)
		                                           : false;
		if (isParentGlRegDep) {
	...
	if (node->getOpCode().isLoadConst()) {
		...
		if (getCurrentParent() && getCurrentParent()->getOpCodeValue() == TR::newarray
			...
			switch (node->getInt()) {
			case 4: output.append("boolean"); break;
			case 5: output.append("char"); break;
			case 6: output.append("float"); break;
			case 7: output.append("double"); break;
			case 8: output.append("byte"); break;
			case 9: output.append("short"); break;
			case 10: output.append("int"); break;
			case 11: output.append("long"); break;
			default: TR_ASSERT(0, "Wrong array type");
			}
		}
	}
	...
	if (node->getType().isBCD()) {
		if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst()
			...
			if (node->hasSourcePrecision()) {
				output.append(" <prec=%d (len=%d) srcprec=%d> ", node->getDecimalPrecision(),
			...
			output.append(" <prec=%d (len=%d) adj=%d> ", node->getDecimalPrecision(), node->getSize(),
			        node->getDecimalAdjust());
		} else if (node->canHaveSourcePrecision()) {
			output.append(" <prec=%d (len=%d) srcprec=%d %s=%d round=%d> ", node->getDecimalPrecision(),
			        node->getSize(), node->getSourcePrecision(),
			        node->getOpCode().isConversionWithFraction() ? "frac" : "adj",
			        node->getOpCode().isConversionWithFraction() ? node->getDecimalFraction()
			                                                     : node->getDecimalAdjust(),
			        node->getDecimalRound());
		} else {
			output.append(" <prec=%d (len=%d) %s=%d round=%d> ", node->getDecimalPrecision(),
			        node->getSize(), node->getOpCode().isConversionWithFraction() ? "frac" : "adj",
			        node->getOpCode().isConversionWithFraction() ? node->getDecimalFraction()
			                                                     : node->getDecimalAdjust(),
			        node->getDecimalRound());
		}
...
TR_Debug::printVCG(TR::FILE *pOutFile, TR::CFG *cfg, const char *sig)
{
	...
	if (debug("nestedVCG")) {
		...
		trfprintf(pOutFile,
		        "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 horizontal_order: 0 textcolor: blue "
		        "borderwidth: 1}\n",
		        sig);
		...
		trfprintf(pOutFile, "edge.color: black\n");
		trfprintf(pOutFile,
		        "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 textcolor: blue borderwidth: 1}\n",
		        sig);
...
TR_Debug::printVCG(TR::FILE *pOutFile, TR::Block *block, int32_t vorder, int32_t horder)
{
	...
	for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();
		...
		if (b->getNumber() >= 0) {
			trfprintf(pOutFile,
			        "edge: { sourcename: \"%d\" targetname: \"%d\" linestyle: dotted label: \"exception\" "
			        "color: %s }\n",
			        block->getNumber(), b->getNumber(), edgeColours[unknownHotness]);
		}
	}
...
TR_Debug::printVCG(TR::FILE *pOutFile, TR::Node *node, uint32_t indentation)
{
	...
	if (_nodeChecklist.isSet(node->getGlobalIndex())) {
		trfprintf(pOutFile, "%*s==>%s at %s\\n", 12 + indentation, " ", getName(node->getOpCode()),
		        getName(node));
		return;
	}
...
int32_t childTypes[] = {
        ...
        TR::Address, // TR::compressedRefs   FIXME:IL: shouldn't second arg be TR::Int64?  Type info should be like
                     // TR::aladd
		...
		for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
			...
			if (debug("checkTypes") && expectedType != TR::NoType) {
				...
				if (childType != expectedType && childType != TR::NoType
					...
					if (getFile() != NULL) {
						trfprintf(getFile(),
						        "TREE VERIFICATION ERROR -- node [%s] has wrong type for child "
						        "[%s] (%s), expected %s\n",
...
TR_Debug::verifyTreesPass2(TR::Node *node, bool isTreeTop)
{
	...
	if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
		...
		if (node->getReferenceCount() != node->getLocalIndex()) {
			if (getFile() != NULL)
				trfprintf(getFile(),
				        "TREE VERIFICATION ERROR -- node [%s] ref count is %d and should be %d\n",
...
TR_Debug::verifyBlocksPass2(TR::Node *node)
{
	...
	if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
		...
		if (node->getLocalIndex() != 0) {
			char buffer[100];
			sprintf(buffer,
			        "BLOCK VERIFICATION ERROR -- node [%s] accessed outside of its (extended) basic block: "
			        "%d time(s)",
