...
public:
	...
	// Options processing
	//
	virtual TR::FILE *findLogFile(TR::Options *aotCmdLineOptions, TR::Options *jitCmdLineOptions,
	        TR::OptionSet *optSet, char *logFileName);
	virtual int32_t findLogFile(const char *logFileName, TR::Options *aotCmdLineOptions,
	        TR::Options *jitCmdLineOptions, TR::Options **optionsArray, int32_t arraySize);
	virtual void dumpOptionHelp(
	        TR::OptionTable *jitOptions, TR::OptionTable *feOptions, TR::SimpleRegex *nameFilter);
	...
	virtual bool methodSigCanBeFound(
	        const char *, TR::CompilationFilters *, TR_FilterBST *&, TR::Method::Type methodType);
	...
	virtual void traceRegisterAssigned(
	        TR_RegisterAssignmentFlags flags, TR::Register *virtReg, TR::Register *realReg);
	virtual void traceRegisterFreed(TR::Register *virtReg, TR::Register *realReg);
	virtual void traceRegisterInterference(
	        TR::Register *virtReg, TR::Register *interferingVirtual, int32_t distance);
	...
	void printRegMemInstruction(
	        TR::FILE *, const char *, TR::RealRegister *reg, TR::RealRegister *base = 0, int32_t = 0);
	...
	void printMemRegInstruction(
	        TR::FILE *, const char *, TR::RealRegister *base, int32_t offset, TR::RealRegister * = 0);
	...
	int32_t *printStackAtlas(uintptrj_t startPC, uint8_t *mapBits, int32_t numberOfSlotsMapped,
	        bool fourByteOffsets, int32_t *sizeOfStackAtlas, int32_t frameSize);
	uint16_t printStackAtlasDetails(uintptrj_t startPC, uint8_t *mapBits, int numberOfSlotsMapped,
...
#define TRACERA_INSTRUCTION_INSERTED (0x0002)
	...
	const static uint32_t DEFAULT_NODE_LENGTH = 82; // Tree information printed to the right of node OPCode will be
	                                                // aligned if node OPCode's length <= DEFAULT_NODE_LENGTH, or
	                                                // will follow the node OPCode immediately (non-aligned)
	                                                // otherwise.
