...
void
TR::DebugCounter::prependDebugCounterBump(
...
void
TR::DebugCounter::prependDebugCounter(TR::Compilation *comp, const char *name, TR::TreeTop *nextTreeTop, int32_t delta,
...
void
TR::DebugCounter::prependDebugCounter(TR::Compilation *comp, const char *name, TR::TreeTop *nextTreeTop,
        ...
        TR::Compilation *comp, uint8_t *location, TR::Node *node, TR::DebugCounterBase *counter)
{
	...
	TR::ExternalRelocation *r = new (comp->trHeapMemory())
	        TR::ExternalRelocation(location, (uint8_t *)counter, TR_DebugCounter, comp->cg());

	comp->cg()->addExternalRelocation(r, __FILE__, __LINE__, node);
}

void
TR::DebugCounter::generateRelocation(TR::Compilation *comp, TR::Instruction *firstInstruction, TR::Node *node,
...
void
TR::DebugCounter::generateRelocation(TR::Compilation *comp, TR::Instruction *firstInstruction,
        TR::Instruction *secondInstruction, TR::Node *node, TR::DebugCounterBase *counter, uint8_t seqKind)
{
	...
	TR::ExternalOrderedPair32BitRelocation *r = new (comp->trHeapMemory())
	        TR::ExternalOrderedPair32BitRelocation((uint8_t *)firstInstruction, (uint8_t *)secondInstruction,
...
const char *
TR::DebugCounter::debugCounterName(TR::Compilation *comp, const char *format, ...)
{
...
const char *
TR::DebugCounter::debugCounterBucketName(TR::Compilation *comp, int32_t value, const char *format, ...)
{
	...
	char *bucketFormat = (char *)comp->trMemory()->allocateStackMemory(strlen(format) + 40); // appending "=%d..%d"
	                                                                                         // where each %d could
	                                                                                         // be 11 characters
...
/** Use long operations in 64 bit platforms and int operations in 32 bit platforms */
TR::Node *
TR::DebugCounterBase::createBumpCounterNode(TR::Compilation *comp, TR::Node *deltaNode)
{
	TR::SymbolReference *symref = getBumpCountSymRef(comp);
	TR::Node *load = TR::Node::createWithSymRef(
	        deltaNode, TR::Compiler->target.is64Bit() ? TR::lload : TR::iload, 0, symref);
...
void
TR::DebugCounter::getInsertionCounterNames(
        TR::Compilation *comp, TR_OpaqueMethodBlock *method, int32_t bytecodeIndex, const char *(&counterNames)[3])
{
	...
	if (insertMethod) {
		counterNames[2] = TR::DebugCounter::debugCounterName(comp,
		        comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byMethod.(%s)"),
		        methodName);
	}
}
...
void
TR::DebugCounterAggregation::aggregateStandardCounters(TR::Compilation *comp, TR::Node *node, const char *counterName,
...
void
TR::DebugCounterAggregation::aggregateStandardCounters(TR::Compilation *comp, TR_OpaqueMethodBlock *method,
...
void
TR::DebugCounterAggregation::aggregateDebugCounterInsertions(TR::Compilation *comp, TR_OpaqueMethodBlock *method,
        int32_t bytecodeIndex, TR::DebugCounter *counter, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
	...
	for (int i = 0; i < sizeof(counterNames) / sizeof(counterNames[0]); i++) {
		if (counterNames[i]) {
			aggregate(
			        TR::DebugCounter::getDebugCounter(comp, counterNames[i], fidelity, staticDelta), delta);
		}
	}
}
...
TR::DebugCounterAggregation::getBumpCountSymRef(TR::Compilation *comp)
{
	if (_symRef == NULL) {
		...
		TR_ASSERT(
		        _symRef, "SymbolReference *_symRef must not be null. Ensure availability of persistent memory");
...
TR::DebugCounterGroup::findAggregation(const char *nameChars, int32_t nameLength)
{
	if (nameChars == NULL)
		return NULL;
	char *name = (char *)alloca(nameLength + 1);
...
TR::DebugCounterAggregation *
TR::DebugCounterGroup::createAggregation(TR::Compilation *comp, const char *name)
{
	TR::DebugCounterAggregation *aggregatedCounters = new (comp->trPersistentMemory())
...
TR::DebugCounter *
TR::DebugCounterGroup::createCounter(const char *name, int8_t fidelity, TR_PersistentMemory *persistentMemory)
{
...
const char *
TR::DebugCounterGroup::counterName(TR::Compilation *comp, const char *format, va_list args)
{
...
TR::DebugCounter *
TR::DebugCounterGroup::getCounter(TR::Compilation *comp, const char *name, int8_t fidelity)
{
