...
#define GET_CURR_PC(dst) \
	do { \
		asm("bl $+4;" /*branch and link to next instruction*/ \
		    "mflr %0;" /*copy contents of link register (which now contains address of this instruction) to \
		                  dst*/ \
		        : "=r"(dst) \
		        : /*no inputs*/ \
		        : "lr"); \
...
void
TR_LinuxCallStackIterator::printSymbol(int32_t frame, char *sig, TR::Compilation *comp)
{
	char lib[256];
	char func[256];
	intptr_t offset;
...
TR_WinCallStackIterator::TR_WinCallStackIterator() : TR_CallStackIterator(), _num_next(0), _trace_size(0), _done(true)
{
	// Load dbghelp.dll
	...
	while (_trace_size < MAX_STACK_SIZE) {
		...
		PSYMBOL_INFO pSymbol =
		        (PSYMBOL_INFO)TR::globalAllocator("CallStackIterator")
		                .allocate((sizeof(SYMBOL_INFO) + MAX_SYM_NAME * sizeof(TCHAR) + sizeof(ULONG64) - 1)
		                        / sizeof(ULONG64));
		...
		if (SymFromAddr(hProcess, StackFrame.AddrPC.Offset, &dwDisplacement, pSymbol)) {
			_function_names[_trace_size] = (char *)TR::globalAllocator("CallStackIterator")
