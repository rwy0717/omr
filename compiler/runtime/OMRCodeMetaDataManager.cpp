...
CodeMetaDataManager::CodeMetaDataManager() : _cachedPC(0), _cachedHashTable(NULL), _retrievedMetaDataCache(NULL)
{
...
// protected
bool
CodeMetaDataManager::insertRange(TR::MethodMetaDataPOD *metaData, uintptr_t startPC, uintptr_t endPC)
{
...
// protected
bool
CodeMetaDataManager::removeRange(const TR::MethodMetaDataPOD *metaData, uintptr_t startPC, uintptr_t endPC)
{
	bool removeSuccess = false;
	self()->updateCache(metaData->startPC);
	if (_cachedHashTable) {
...
CodeMetaDataManager::updateCache(uintptr_t currentPC)
{
	...
	if (currentPC != _cachedPC) {
		...
		TR_ASSERT(_cachedHashTable,
		        "Either we lost a code cache or we attempted to find a hash table for a non-code cache "
		        "startPC: Searched for %p",
		        currentPC);
	}
}
...
CodeMetaDataManager::findMetaDataInHash(TR::MetaDataHashTable *table, uintptr_t searchValue)
{
...
TR::MethodMetaDataPOD **
CodeMetaDataManager::insertMetaDataArrayInHash(TR::MetaDataHashTable *table, TR::MethodMetaDataPOD **array,
	...
	if (LOW_BIT_SET(array)) {
		...
		if (*newElement == NULL) {
			...
			*(newElement - 1) = dataToInsert;
			...
			memcpy(returnVal + 1, array, chainLength * sizeof(uintptr_t)); /* safe to memcpy since the new
        ...
        TR::MethodMetaDataPOD **array, const TR::MethodMetaDataPOD *dataToRemove)
{
	...
	if ((TR::MethodMetaDataPOD *)REMOVE_LOW_BIT(*index) == dataToRemove) {
		...
		*index = (TR::MethodMetaDataPOD *)SET_LOW_BIT(*index);
		...
		size = (count - removeSpot + 1) * sizeof(uintptr_t); /* dataToRemove is in middle (or start) of the
		                                                        array. */
	...
	if (LOW_BIT_SET(*array)) {
		...
		//
		TR::MethodMetaDataPOD **rc = (TR::MethodMetaDataPOD **)*array; /* Only one pointer left.  Just return
		                                                                  the one pointer */
		*array = NULL;
		return rc;
	}
