...
protected:
	...
	{
		_nextAssumptionForSameJittedBody = (RuntimeAssumption *)((uintptr_t)link
	 ...
	 *
	 *  \note
	 *     This function should be called when we attempt to reclaim the runtime assumption right before
	 * deallocation. Calling this function too early can result in potentially accessing already deallocated memory,
	 * as the runtime assumption may still be examined/processed even after calling `compensate` and all the way up
	 * until the runtime assumption is deallocated.
	 */
...
public:
	...
	RuntimeAssumption *getNextAssumptionForSameJittedBody() const
	{
		...
		while (toReturn && toReturn->isMarkedForDetach() && toReturn != this)
			toReturn = (RuntimeAssumption *)(((uintptr_t)toReturn->_nextAssumptionForSameJittedBody)
			        & ~MARK_FOR_DELETE);
		// handle the case where you have gone full circle
		return toReturn != this || !toReturn->isMarkedForDetach() ? toReturn
...
private:
	...
	RuntimeAssumption *_nextAssumptionForSameJittedBody; // links assumptions that pertain to the same method body
	                                                     ...
	                                                     // Lower bit is used to mark assumptions that will be
	                                                     // lazily removed
protected:
...
protected:
	LocationRedirectRuntimeAssumption(TR_PersistentMemory *pm, uintptrj_t key) : RuntimeAssumption(pm, key) {}
...
protected:
	ValueModifyRuntimeAssumption(TR_PersistentMemory *pm, uintptrj_t key) : RuntimeAssumption(pm, key) {}
...
protected:
	PatchNOPedGuardSite(TR_PersistentMemory *pm, uintptrj_t key, TR_RuntimeAssumptionKind kind, uint8_t *location,
	        uint8_t *destination)
