...
TR::CodeCache *
OMR::CodeCacheManager::initialize(bool allocateMonolithicCodeCache, uint32_t numberOfCodeCachesToCreateAtStartup)
{
	...
	if (_symbolContainer == NULL) {
		TR::CodeCacheSymbolContainer *symbolContainer = static_cast<TR::CodeCacheSymbolContainer *>(
	...
	if (allocateMonolithicCodeCache) {
		...
		if (self()->allocateCodeCacheRepository(size)) {
			...
			if (config.canChangeNumCodeCaches()) {
				// code caches will all be nearby each other in the repository so no need to allocate
				// more than 1 up front
...
OMR::CodeCacheManager::destroy()
{
	...
	if (_elfRelocatableGenerator) {
		TR_ASSERT(_elfRelocatableGenerator->emitELF((const char *)_objectFileName,
		                  _relocatableSymbolContainer->_head, _relocatableSymbolContainer->_numSymbols,
		                  _relocatableSymbolContainer->_totalSymbolNameLength, _relocations->_head,
	...
	if (_elfExecutableGenerator) {
		pid_t jvmPid = getpid();
		static const int maxElfFilenameSize = 15 + sizeof(jvmPid) * 3; // "written to file: /tmp/perf-%d.jit,
		                                                               // where d is the pid"
...
TR::CodeCache *
OMR::CodeCacheManager::allocateCodeCacheObject(TR::CodeCacheMemorySegment *codeCacheSegment, size_t codeCacheSize)
{
...
TR::CodeCache *
OMR::CodeCacheManager::reserveCodeCache(bool compilationCodeAllocationsMustBeContiguous, size_t sizeEstimate,
	...
	//
	{
		...
		for (codeCache = self()->getFirstCodeCache(); codeCache; codeCache = codeCache->next()) {
			if (!codeCache->isReserved()) // we cannot touch the reserved ones
			{
				...
				if (almostFull == TR_no
				        || (almostFull == TR_maybe && !compilationCodeAllocationsMustBeContiguous)) {
					// Is the free space big enough?
					if (sizeEstimate == 0 || // If size estimate is not given we'll blindly pick
					                         // anything
...
//
uint8_t *
OMR::CodeCacheManager::allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize, TR::CodeCache **codeCache_pp,
...
void
OMR::CodeCacheManager::performSizeAdjustments(
...
//
OMR::CodeCacheTrampolineCode *
OMR::CodeCacheManager::replaceTrampoline(TR_OpaqueMethodBlock *method, void *callSite, void *oldTrampoline,
...
//
uint8_t *
OMR::CodeCacheManager::allocateCodeMemoryWithRetries(size_t warmCodeSize, size_t coldCodeSize,
	...
	if (allocationRetries > 0) {
		...
		if (codeCache) // found an appropriate code cache
		{
			...
			*codeCache_pp = codeCache; // code cache has been switched // MCT
			...
			if (needsToBeContiguous) // We should abort the compilation if we change the cache during a
			                         // compilation that needs contiguous allocations
				return NULL;

			/* try allocating in the new cache */
			warmCodeAddress = self()->allocateCodeMemoryWithRetries(warmCodeSize, coldCodeSize,
			        codeCache_pp, allocationRetries, coldCode, needsToBeContiguous, isMethodHeaderNeeded);
...
OMR::CodeCacheManager::getNewCodeCacheMemorySegment(size_t segmentSize, size_t &codeCacheSizeAllocated)
{
...
OMR::CodeCacheManager::allocateCodeCacheRepository(size_t repositorySize)
{
	...
	if (_codeCacheRepositorySegment) {
		...
		*((TR::CodeCache **)start) = self()->getRepositoryCodeCacheAddress();
		...
		_repositoryCodeCache->setColdCodeAlloc((uint8_t *)(_codeCacheRepositorySegment->segmentTop()
		        - _codeCacheRepositorySegment->segmentAlloc()));

		if (config.verboseCodeCache()) {
			TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
			        "allocateCodeCacheRepository: size=%u heapBase=%p heapAlloc=%p heapTop=%p",
...
OMR::CodeCacheManager::registerStaticRelocation(const TR::StaticRelocation &relocation)
{
	...
	if (_elfRelocatableGenerator) {
		...
		uint32_t symbolNumber = static_cast<uint32_t>(_relocatableSymbolContainer->_numSymbols - 1); // symbol
		                                                                                             // index in
		                                                                                             // the
		                                                                                             // linked
		                                                                                             // list
		uint32_t relocationType = _resolver.resolveRelocationType(relocation);
		TR::CodeCacheRelocationInfo *newRelocation = static_cast<TR::CodeCacheRelocationInfo *>(
...
TR::CodeCacheMemorySegment *
OMR::CodeCacheManager::carveCodeCacheSpaceFromRepository(size_t segmentSize, size_t &codeCacheSizeToAllocate)
{
	uint8_t *start = NULL;
	uint8_t *end = NULL;
	size_t freeSpace = 0;
	...
	if (config.verboseCodeCache()) {
		if (start)
			TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
			        "carved size=%u range: " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
...
TR::CodeCacheMemorySegment *
OMR::CodeCacheManager::setupMemorySegmentFromRepository(uint8_t *start, uint8_t *end, size_t &codeCacheSizeToAllocate)
{
	TR::CodeCacheMemorySegment *memorySegment =
...
OMR::CodeCacheManager::initializeRelocatableELFGenerator(void)
{
	...
	_elfRelocatableGenerator = new (_rawAllocator)
	        TR::ELFRelocatableGenerator(_rawAllocator, _codeCacheRepositorySegment->segmentBase(),
...
OMR::CodeCacheManager::initializeExecutableELFGenerator(void)
{
	...
	_elfExecutableGenerator = new (_rawAllocator)
	        TR::ELFExecutableGenerator(_rawAllocator, _codeCacheRepositorySegment->segmentBase(),
	                _codeCacheRepositorySegment->segmentTop() - _codeCacheRepositorySegment->segmentBase());
}
#endif // HOST_OS==OMR_LINUX

TR::CodeCache *
OMR::CodeCacheManager::allocateCodeCacheFromNewSegment(size_t segmentSizeInBytes, int32_t reservingCompilationTID)
{
	TR::CodeCacheConfig &config = self()->codeCacheConfig();
	...
	if (codeCacheSegment) {
		TR::CodeCache *codeCache =
		        self()->allocateCodeCacheObject(codeCacheSegment, actualCodeCacheSizeAllocated);
