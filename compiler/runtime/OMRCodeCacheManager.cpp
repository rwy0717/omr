...
#define REACHEABLE_RANGE_KB (2048 * 1024)
    ...
    config._needsMethodTrampolines = !(config.trampolineCodeSize() == 0 || config.maxNumberOfCodeCaches() == 1
        ...
        || (!TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines)
               ...
               && _codeCacheRepositorySegment && config.codeCacheTotalKB() <= REACHEABLE_RANGE_KB)
#endif
    );
        ...
        if (!foundSpace) {
            ...
            if (config.verbosePerformance()) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_CODECACHE, "Reached code cache space threshold. Disabling JIT profiling.");
            }
...
// callingPC.
//
OMR::CodeCacheTrampolineCode* OMR::CodeCacheManager::findHelperTrampoline(void* callingPC, int32_t helperIndex)
{
    ...
    bool isMethodHeaderNeeded)
{
    ...
    codeCache
        = TR::CodeCache::allocate(self(), segmentSize, compThreadID); // the allocated cache is in reserved state // MCT
    if (!codeCache) {
        self()->setCodeCacheFull();
        return NULL;
    }
...
void OMR::CodeCacheManager::reservationInterfaceCache(void* callSite, TR_OpaqueMethodBlock* method)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
