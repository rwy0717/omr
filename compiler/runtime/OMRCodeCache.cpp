...
bool OMR::CodeCache::resizeCodeMemory(void* memoryBlock, size_t newSize)
{
    ...
    if (config.verboseReclamation()) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
            "--resizeCodeMemory-- CC=%p cacheHeader=%p oldSize=%u newSize=%d shrinkage=%u", this, cacheHeader, oldSize,
            newSize, shrinkage);
    }
...
bool OMR::CodeCache::initialize(TR::CodeCacheManager* manager, TR::CodeCacheMemorySegment* codeCacheSegment,
    size_t codeCacheSizeAllocated, CodeCacheHashEntrySlab* hashEntrySlab)
{
    _manager = manager;
            ...
            } else // no space in this code cache; must allocate a new one
            {
                ...
                if (config.verboseCodeCache()) {
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_CODECACHE, "CodeCache %p marked as full in reserveResolvedTrampoline", this);
                }
            }
        }
    }
    ...
    if (_freeBlockList) {
        ...
        // scope for cache walk
        {
            ...
            if (_sizeOfLargestFreeWarmBlock != maxFreeWarmSize) {
                fprintf(stderr,
                    "checkForErrors cache %p: Error: _sizeOfLargestFreeWarmBlock(%d) != maxFreeWarmSize(%d)\n", this,
                    _sizeOfLargestFreeWarmBlock, maxFreeWarmSize);
                doCrash = true;
            }
            if (_sizeOfLargestFreeColdBlock != maxFreeColdSize) {
                fprintf(stderr,
                    "checkForErrors cache %p: Error: _sizeOfLargestFreeColdBlock(%d) != maxFreeColdSize(%d)\n", this,
                    _sizeOfLargestFreeColdBlock, maxFreeColdSize);
                doCrash = true;
            }
...
OMR::CodeCacheErrorCode::ErrorCode OMR::CodeCache::reserveNTrampolines(int64_t n)
{
    ...
    if (size) {
        if (_trampolineReservationMark >= _trampolineBase + size) {
            ...
            if (config.verboseCodeCache()) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_CODECACHE, "CodeCache %p marked as full in reserveNTrampoline", this);
            }
        }
    }
    ...
    TR::CodeCacheManager* manager, size_t segmentSize, int32_t reservingCompThreadID)
{
    ...
    if (codeCacheSegment) {
        TR::CodeCache* codeCache = manager->allocateCodeCacheObject(codeCacheSegment, codeCacheSizeAllocated);
