...
OMR::CodeCache::trimCodeMemoryAllocation(void *codeMemoryStart, size_t actualSizeInBytes)
{
	...
	if (config.verboseReclamation()) {
		TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
		        "--trimCodeMemoryAllocation-- CC=%p cacheHeader=%p oldSize=%u actualSizeInBytes=%d "
		        "shrinkage=%u",
...
//
bool
OMR::CodeCache::initialize(TR::CodeCacheManager *manager, TR::CodeCacheMemorySegment *codeCacheSegment,
        size_t allocatedCodeCacheSizeInBytes)
{
	_manager = manager;

	// heapSize can be calculated as (codeCache->helperTop - codeCacheSegment->heapBase), which is equal to
	// segmentSize If codeCachePadKB is set, this will make the system believe that we allocated segmentSize bytes,
	...
	*((TR::CodeCache **)(_segment->segmentBase())) = self(); // Write a pointer to this cache at the beginning of
	                                                         // the segment
...
//---------------------------------------------------------------------------
int32_t
OMR::CodeCache::reserveResolvedTrampoline(TR_OpaqueMethodBlock *method, bool inBinaryEncoding)
{
		...
		if (!entry) {
			...
			if (retValue == OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
				...
				if (!self()->addResolvedMethod(method))
					retValue = CodeCacheErrorCode::ERRORCODE_FATALERROR; // couldn't allocate memory
					                                                     // from VM
			}
		}
	}
...
//
OMR::CodeCacheTrampolineCode *
OMR::CodeCache::replaceTrampoline(
...
OMR::CodeCache::syncTempTrampolines()
{
	if (_flags & CODECACHE_FULL_SYNC_REQUIRED) {
		for (uint32_t entryIdx = 0; entryIdx < _resolvedMethodHT->_size; entryIdx++) {
			for (CodeCacheHashEntry *entry = _resolvedMethodHT->_buckets[entryIdx]; entry;
			        entry = entry->_next) {
		...
		for (CodeCacheTempTrampolineSyncBlock *syncBlock = _trampolineSyncList; syncBlock;
...
//
void
OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock *method, void *callSite, void *newStartPC, void *extraArg)
{
	TR::CodeCacheConfig &config = _manager->codeCacheConfig();
...
//
void
OMR::CodeCache::createTrampoline(CodeCacheTrampolineCode *trampoline, void *targetStartPC, TR_OpaqueMethodBlock *method)
{
	TR::CodeCacheConfig &config = _manager->codeCacheConfig();
...
bool
OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t *start, uint8_t *end, char *file, uint32_t lineNumber)
{
	TR::CodeCacheConfig &config = _manager->codeCacheConfig();
	...
	if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
		if (config.verboseReclamation()) {
			TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
			        "addFreeBlock2[%s.%d]: failed to add free block. start = 0x%016x end = 0x%016x "
			        "alignment = 0x%04x sizeof(CodeCacheFreeCacheBlock) = 0x%08x",
	...
	if (_freeBlockList) {
		...
		if (start < (uint8_t *)curr && (uint8_t *)curr - end < sizeof(CodeCacheFreeCacheBlock)) {
			...
			if (!(start < _warmCodeAlloc && (uint8_t *)curr >= _coldCodeAlloc)) {
				...
				mergedBlock = curr;
				// fprintf(stderr, "--ccr-- merging new free block of the size %d with a block of the
				// size %d at %p\n", size, curr->size, link);
			...
			if ((start - ((uint8_t *)curr + curr->_size) < sizeof(CodeCacheFreeCacheBlock))
				...
				mergedBlock = curr;
				// fprintf(stderr, "--ccr-- merging new free block of the size %d with blocks of the
	...
	if (config.verboseReclamation()) {
		TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
		        "--ccr-- addFreeBlock2WithCallSite CC=%p start=%p end=%p mergedBlock=%p link=%p "
		        "link->_size=%u, _sizeOfLargestFreeWarmBlock=%d _sizeOfLargestFreeColdBlock=%d "
		        "warmCodeAlloc=%p coldBlockAlloc=%p",
		        this, (void *)start, (void *)end, mergedBlock, link, (uint32_t)link->_size,
		        _sizeOfLargestFreeWarmBlock, _sizeOfLargestFreeColdBlock, _warmCodeAlloc, _coldCodeAlloc);
	}
#ifdef DEBUG
...
OMR::CodeCache::findFreeBlock(size_t size, bool isCold, bool isMethodHeaderNeeded)
{
	...
	for (currLink = _freeBlockList, prevLink = NULL; currLink; prevLink = currLink, currLink = currLink->_next) {
		...
		if (!biggestLink) {
			...
			if (currLink->_size >= biggestLink->_size) // >= is important; do not use > only as
...
OMR::CodeCache::removeFreeBlock(size_t blockSize, CodeCacheFreeCacheBlock *prev, CodeCacheFreeCacheBlock *curr)
{
		...
		// scope for cache walk
		{
			...
			for (CodeCacheFreeCacheBlock *currLink = _freeBlockList; currLink; currLink = currLink->_next) {
				...
				if (currLink->_size > cacheSize) {
					fprintf(stderr,
					        "checkForErrors cache %p: Error: Size of the free block %u is bigger "
					        "than the size of the cache %u\n",
				...
				if (endBlock < _segment->segmentBase() + sizeof(void *)
					...
					fprintf(stderr,
					        "checkForErrors cache %p: Error: End of block %p residing at %p is "
					...
					if ((uint8_t *)currLink->_next == endBlock) {
						...
						if (!((uint8_t *)currLink < _warmCodeAlloc
							...
							fprintf(stderr,
							        ...
							        "coalescing opportunity. Next block (%p) is adjacent "
							        "to current one %p-%p\n",
						...
						if ((uint8_t *)currLink->_next <= endBlock) {
							fprintf(stderr,
							        "checkForErrors cache %p: Error: next block (%p) "
							        "should come after end of current one %p-%p\n",
							        this, currLink->_next, currLink, endBlock);
							doCrash = true;
							;
						}
						// A free block is always followed by a used block except when this is
						// the last free block in the warm region
						if (endBlock != _warmCodeAlloc) {
							// There should be valid code between this block and the next
							uint8_t *eyeCatcherPosition = endBlock
							        + offsetof(CodeCacheMethodHeader, _eyeCatcher);
							if (*eyeCatcherPosition
							        != *(_manager->codeCacheConfig().warmEyeCatcher())) {
								fprintf(stderr,
								        "checkForErrors cache %p: Error: block coming "
			...
			// 3. All used blocks must have a CodeCacheMethodHeader with size as first parameter, folowed by
			// eyeCatcher and metaData
			TR::CodeCacheConfig &config = _manager->codeCacheConfig();
			uint8_t *start = align(
			        ((uint8_t *)_segment->segmentTop()) + sizeof(char *), config.codeCacheAlignment() - 1);
			...
			while (start < this->_trampolineBase) {
				...
				{
					// There should be valid code between this block and the next
					uint8_t *eyeCatcherPosition = start
					        + offsetof(CodeCacheMethodHeader, _eyeCatcher);
					if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
						...
						fprintf(stderr,
						        "checkForErrors cache %p: block %p is not in the free list and "
						        "does not have eye-catcher; prevBlock=%p\n",
...
uint8_t *
OMR::CodeCache::allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize, uint8_t **coldCode,
        bool needsToBeContiguous, bool isMethodHeaderNeeded)
{
	...
	_manager->performSizeAdjustments(warmSize, coldSize, needsToBeContiguous, isMethodHeaderNeeded); // side effect
	                                                                                                 // on warmSize
	                                                                                                 // and coldSize
	...
	if (!warmIsFreeBlock) {
		...
		warmCodeAddress = self()->findFreeBlock(warmSize, false, isMethodHeaderNeeded); // side effect: free
		                                                                                // block is unlinked
	...
	if (!coldIsFreeBlock) {
		if (coldSize) {
			...
			if (cacheHeapAlloc < _warmCodeAlloc) {
				// This case should never happen now because we checked above that both warm and cold
				// can be allocated
		...
		coldCodeAddress = self()->findFreeBlock(coldSize, true, isMethodHeaderNeeded); // side effect: free
		                                                                               // block is unlinked
		// Note that findFreeBlock may return a block which is slightly bigger than what we wanted
		// Change the warmSize to the higher size so that the size of the block is correctly maintained
...
//
TR::CodeCache *
OMR::CodeCache::allocate(TR::CodeCacheManager *manager, size_t segmentSize, int32_t reservingCompThreadID)
{
