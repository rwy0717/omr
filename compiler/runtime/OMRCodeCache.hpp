...
public:
	...
	static TR::CodeCache *allocate(
	        TR::CodeCacheManager *manager, size_t segmentSize, int32_t reservingCompThreadID);
	void destroy(TR::CodeCacheManager *manager);

	uint8_t *allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize, uint8_t **coldCode,
	        bool needsToBeContiguous, bool isMethodHeaderNeeded = true);
...
public:
	...
	// flag accessed under mutex <== This is deceiving! There are two different monitors we may hold (not at the
	// same time!) when we write to this. We can either be holding the code cache monitor *OR* the manager's code
	// cache list monitor.
