...
OMR::IlBuilder::Copy(TR::IlValue *value)
{
	...
	char *name = (char *)_comp->trMemory()->allocateHeapMemory((2 + 10 + 1) * sizeof(char)); // 2 ("_T") + max 10
	                                                                                         // digits + trailing
	                                                                                         // zero
	...
	TraceIL("IlBuilder[ %p ]::Copy value (%d) dataType (%d) to newVal (%d) at cpIndex (%d)\n", this, value->getID(),
	        dt, newVal->getID(), newSymRef->getCPIndex());
...
void
OMR::IlBuilder::pullInBuilderTrees(
...
OMR::IlBuilder::connectTrees()
{
	...
	if (_methodBuilder != this || _methodBuilder->callerMethodBuilder() != NULL) {
		...
		TraceIL("[ %p ] Connecting last tree %p [ node %p ] to exit block entry %p [ node %p ]\n", this,
		        lastTree, lastTree->getNode(), _exitBlock->getEntry(), _exitBlock->getEntry()->getNode());
		lastTree->join(_exitBlock->getEntry());

		// also add exit block to blocks array and add an edge from last block to EXIT if the builder comes back
		// (i.e. doesn't return or branch off somewhere)
		if (comesBack())
			cfg()->addEdge(blocks[currentBlock - 1], getExit());

		blocks[currentBlock++] = _exitBlock;
		TraceIL("[ %p ] exit block %d is %p (%p -> %p)\n", this, _exitBlock->getNumber(),
		        _exitBlock->getEntry(), _exitBlock->getExit());
...
OMR::IlBuilder::appendBlock(TR::Block *newBlock, bool addEdge)
{
	...
	if (_currentBlock && addEdge) {
		// if current block does not fall through to new block, use appendNoFallThroughBlock() rather than
		// appendBlock()
...
OMR::IlBuilder::AppendBuilder(TR::IlBuilder *builder)
{
	...
	// when trees are connected, exit block will swing down to become last trees in builder, so it can fall through
	// to this block we're about to create need to add edge explicitly because of this exit block sleight of hand
...
OMR::IlBuilder::VectorLoad(const char *name)
{
	...
	TR::Node *loadNode =
	        TR::Node::createWithSymRef(0, TR::comp()->il.opCodeForDirectLoad(returnType), 0, nameSymRef);
...
OMR::IlBuilder::LoadIndirect(const char *type, const char *field, TR::IlValue *object)
{
	...
	TR::IlValue *returnValue = newValue(fieldType,
	        TR::Node::createWithSymRef(
	                comp()->il.opCodeForIndirectLoad(fieldType), 1, loadValue(object), 0, symRef));
...
OMR::IlBuilder::convertTo(TR::DataType typeTo, TR::IlValue *v, bool needUnsigned)
{
	...
	TR_ASSERT_FATAL(convertOp != TR::BadILOp, "Builder [ %p ] unknown conversion requested for value %d %s to %s",
	        this, v->getID(), typeFrom.toString(), typeTo.toString());
...
OMR::IlBuilder::ConvertBitsTo(TR::IlType *t, TR::IlValue *v)
{
	...
	TR_ASSERT_FATAL(
	        convertOpcode != TR::BadILOp && TR::DataType::getSize(typeTo) == TR::DataType::getSize(typeFrom),
	        "Builder [ %p ] requested bit conversion for value %d from type of size %d (%s) to type of size %d "
	        "(%s) (consider using ConvertTo() to for narrowing/widening)",
...
TR::Node *
OMR::IlBuilder::binaryOpNodeFromNodes(TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode)
{
	...
	TR_ASSERT_FATAL(leftType == rightType || isAddressBump || isRevAddressBump,
	        "binaryOp requires both left and right operands to have same type or one is address and other is "
	        "Int32/64");
...
TR::IlValue *
OMR::IlBuilder::binaryOpFromNodes(TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode)
{
...
TR::IlValue *
OMR::IlBuilder::binaryOpFromOpMap(OpCodeMapper mapOp, TR::IlValue *left, TR::IlValue *right)
{
...
TR::IlValue *
OMR::IlBuilder::binaryOpFromOpCode(TR::ILOpCodes op, TR::IlValue *left, TR::IlValue *right)
{
	TR::Node *leftNode = loadValue(left);
	TR::Node *rightNode = loadValue(right);
...
TR::IlValue *
OMR::IlBuilder::compareOp(TR_ComparisonTypes ct, bool needUnsigned, TR::IlValue *left, TR::IlValue *right)
{
	TR::Node *leftNode = loadValue(left);
	TR::Node *rightNode = loadValue(right);
...
OMR::IlBuilder::Return(TR::IlValue *value)
{
	...
	if (returnBuilder != NULL) {
		...
		if (returnSymbol == NULL) {
			...
			// also proactively define this symbol in the caller, so that it can access it after the inlined
			// body!
			_methodBuilder->callerMethodBuilder()->defineSymbol(returnSymbol, newSymRef);
		}

		TraceIL("IlBuilder[ %p ]::Return %d back to caller's returnBuilder [ %p ] via return symbol called "
		        "%s\n",
		...
		TR::Node *returnNode =
		        TR::Node::create(TR::ILOpCode::returnOpCode(value->getDataType()), 1, loadValue(value));
		genTreeTop(returnNode);
		cfg()->addEdge(_currentBlock, cfg()->getEnd());
		setDoesNotComeBack();
	}
...
OMR::IlBuilder::genOverflowCHKTreeTop(TR::Node *operationNode, TR::ILOpCodes overflow)
{
	TR::Node *overflowChkNode = TR::Node::createWithRoomForOneMore(overflow, 3,
	        symRefTab()->findOrCreateOverflowCheckSymbolRef(_methodSymbol), operationNode,
	        operationNode->getFirstChild(), operationNode->getSecondChild());
...
TR::IlValue *
OMR::IlBuilder::genOperationWithOverflowCHK(
...
OMR::IlBuilder::getOpCode(TR::IlValue *leftValue, TR::IlValue *rightValue)
{
	...
	if (leftValue->getDataType() == TR::Address) {
		TR_ASSERT_FATAL((TR::Compiler->target.is64Bit() && rightValue->getDataType() == TR::Int64)
		        ...
		        "the right child type must be either TR::Int32 (on 32-bit ISA) or TR::Int64 (on 64-bit ISA) "
...
OMR::IlBuilder::AddWithUnsignedOverflow(TR::IlBuilder **handler, TR::IlValue *left, TR::IlValue *right)
{
	...
	TR::ILOpCodes opcode = getOpCode(left, right);
	TR::IlValue *addValue =
	        genOperationWithOverflowCHK(opcode, leftNode, rightNode, handler, TR::UnsignedOverflowCHK);
...
OMR::IlBuilder::SubWithUnsignedOverflow(TR::IlBuilder **handler, TR::IlValue *left, TR::IlValue *right)
{
	TR::Node *leftNode = loadValue(left);
	TR::Node *rightNode = loadValue(right);
	TR::IlValue *unsignedSubValue =
	        genOperationWithOverflowCHK(TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode, rightNode,
	                handler, TR::UnsignedOverflowCHK);
	TraceIL("IlBuilder[ %p ]::%d is UnsignedSubWithOverflow %d + %d\n", this, unsignedSubValue->getID(),
...
OMR::IlBuilder::UnsignedRem(TR::IlValue *left, TR::IlValue *right)
{
	...
	if (returnType.isInt64()) {
		...
		TR::IlValue *truncLeft = binaryOpFromOpCode(
		        TR::ILOpCodes::lmul, binaryOpFromOpCode(TR::ILOpCodes::ludiv, left, right), right);
...
TR::Node *
OMR::IlBuilder::shiftOpNodeFromNodes(TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode)
{
	TR::DataType leftType = leftNode->getDataType();
	TR::DataType rightType = rightNode->getDataType();
...
TR::IlValue *
OMR::IlBuilder::shiftOpFromNodes(TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode)
{
...
TR::IlValue *
OMR::IlBuilder::shiftOpFromOpMap(OpCodeMapper mapOp, TR::IlValue *left, TR::IlValue *right)
{
	TR::Node *leftNode = loadValue(left);
	TR::DataType leftType = leftNode->getDataType();
...
TR::IlValue *
OMR::IlBuilder::genCall(TR::SymbolReference *methodSymRef, int32_t numArgs, TR::IlValue **argValues,
        bool isDirectCall /* true by default*/)
{
	...
	TR::Node *callNode = TR::Node::createWithSymRef(
	        isDirectCall ? TR::ILOpCode::getDirectCall(returnType) : TR::ILOpCode::getIndirectCall(returnType),
 ...
 */
void
OMR::IlBuilder::Transaction(TR::IlBuilder **persistentFailureBuilder, TR::IlBuilder **transientFailureBuilder,
        TR::IlBuilder **transactionBuilder)
{
	// This assertion is to rule out platforms which don't have tstart evaluator yet.
	TR_ASSERT_FATAL(
	        comp()->cg()->hasTMEvaluator(), "this platform doesn't support tstart or tfinish evaluator yet");
...
void
OMR::IlBuilder::Switch(const char *selectionVar, TR::IlBuilder **defaultBuilder, uint32_t numCases, JBCase **cases)
{
...
void
OMR::IlBuilder::Switch(const char *selectionVar, TR::IlBuilder **defaultBuilder, uint32_t numCases, ...)
{
	va_list args;
...
void
OMR::IlBuilder::TableSwitch(const char *selectionVar, TR::IlBuilder **defaultBuilder, bool generateBoundsCheck,
        uint32_t numCases, JBCase **cases)
{
	TR::IlValue *selectorValue = Load(selectionVar);
	TR_ASSERT_FATAL(
	        selectorValue->getDataType() == TR::Int32, "TableSwitch only supports selector having type Int32");
...
OMR::IlBuilder::Select(TR::IlValue *condition, TR::IlValue *trueValue, TR::IlValue *falseValue)
{
	...
	TR_ASSERT_FATAL(
	        dt == falseValue->getDataType(), "Select requires trueValue and falseValue to be of the same type");
...
void
OMR::IlBuilder::generateSwitchCases(
        TR::Node *switchNode, TR::Node *defaultNode, TR::IlBuilder **defaultBuilder, uint32_t numCases, JBCase **cases)
{
	...
	for (int32_t c = 0; c < numCases; c++) {
		...
		if (!cases[c]->_fallsThrough) {
			...
			// handle "break" with a separate builder so user can add whatever they want into
			// caseBuilders[c]
...
void
OMR::IlBuilder::ForLoop(bool countsUp, const char *indVar, TR::IlBuilder **loopCode, TR::IlBuilder **breakBuilder,
        TR::IlBuilder **continueBuilder, TR::IlValue *initial, TR::IlValue *end, TR::IlValue *increment)
{
	...
	*loopCode = createBuilderIfNeeded(*loopCode);
	...
	TraceIL("IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d loopCode %p countsUp %d\n", this,
	        indVar, initial->getID(), end->getID(), increment->getID(), *loopCode, countsUp);
	...
	if (breakBuilder) {
		TR_ASSERT_FATAL(
		        *breakBuilder == NULL, "ForLoop returns breakBuilder, cannot provide breakBuilder as input");
