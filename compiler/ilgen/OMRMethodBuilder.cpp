...
OMR::MethodBuilder::adjustNameForInlinedSite(const char *name)
{
	...
	char *newName = (char *)_comp->trMemory()->allocateHeapMemory(
	        (4 + 10 + 1 + 1 + strlen(name)) * sizeof(char)); // 4 ("_INL") + max 10 digits + 1 ("_") + original name
...
void
OMR::MethodBuilder::DefineFunction(const char *const name, const char *const fileName, const char *const lineNumber,
        void *entryPoint, TR::IlType *returnType, int32_t numParms, ...)
{
	TR::IlType **parmTypes = (TR::IlType **)trMemory()->trPersistentMemory()->allocatePersistentMemory(
        ...
        void *entryPoint, TR::IlType *returnType, int32_t numParms, TR::IlType **parmTypes)
{
	...
	TR::IlType **copiedParmTypes = (TR::IlType **)trMemory()->trPersistentMemory()->allocatePersistentMemory(
	        numParms * sizeof(TR::IlType *));
	for (int32_t p = 0; p < numParms; p++)
...
OMR::MethodBuilder::getParameterTypes()
{
	...
	for (int32_t p = 0; p < _numParameters; p++) {
		...
		TR_ASSERT_FATAL(
		        symTypesIterator != _symbolTypes.end(), "No matching symbol type for parameter '%s'", name);
