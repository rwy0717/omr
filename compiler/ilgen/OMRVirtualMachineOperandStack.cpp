...
void OMR::VirtualMachineOperandStack::MergeInto(TR::VirtualMachineOperandStack* other, TR::IlBuilder* b)
{
    ...
    for (int32_t i = _stackTop; i >= 0; i--) {
        ...
        if (other->_stack[i]->getID() != _stack[i]->getID()) {
            ...
            TR_ASSERT(_stack[i]->getDataType() == other->_stack[i]->getDataType(),
