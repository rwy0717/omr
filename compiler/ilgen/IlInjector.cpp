...
void
OMR::IlInjector::initialize(TR::IlGeneratorMethodDetails *details, TR::ResolvedMethodSymbol *methodSymbol,
...
void
OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node *first, TR::Node *second, TR::Block *targetBlock)
{
...
void
OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node *first, TR::Node *second, int32_t targetBlockNumber)
{
...
OMR::IlInjector::shiftLeftBy(TR::Node *value, int32_t shift)
{
	...
	if (value->getDataType() == TR::Int32)
		...
		TR_ASSERT(
		        value->getDataType() == TR::Int64, "expecting Int32 or Int64 for shiftLeftBy value expression");
...
OMR::IlInjector::multiplyBy(TR::Node *value, int64_t factor)
{
	...
	if (value->getDataType() == TR::Int32)
		...
		TR_ASSERT(
		        value->getDataType() == TR::Int64, "expecting Int32 or Int64 for multiplyBy value expression");
...
OMR::IlInjector::arrayLoad(TR::Node *base, TR::Node *index, TR::IlType *dt)
{
	...
	if (index->getDataType() == TR::Int32) {
		scaledIndex = createWithoutSymRef(
		        TR::imul, 2, index, iconst((int64_t)TR::DataType::getSize(dt->getPrimitiveType())));
		...
		scaledIndex = createWithoutSymRef(
		        TR::lmul, 2, index, lconst((int64_t)TR::DataType::getSize(dt->getPrimitiveType())));
	...
	TR::Node *load = TR::Node::createWithSymRef(
	        TR::ILOpCode::indirectLoadOpCode(dt->getPrimitiveType()), 1, element, 0, symRef);
	return load;
}
