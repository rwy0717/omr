        ...
        , _adjustByStep(adjustByStep)
{
	...
	if (_integerTypeForAdjustments->isPointer()) {
		...
		TR_ASSERT_FATAL(adjustByStep == baseType->getSize(),
		        "VirtualMachineRegister for %s adjustByStep (%u) != size represented by pointerToRegisterType "
		        "(%u)",
		        localName, _adjustByStep, baseType->getSize());
		_isAdjustable = true;
	} else {
		TR_ASSERT_FATAL(adjustByStep == 0,
		        "VirtualMachineRegister for %s is representing a primitive type but adjustByStep (%u) != 0",
...
{
	TR_ASSERT_FATAL(
	        _isAdjustable, "VirtualMachineRegister can not be adjusted as it is simulating a primitive type");
