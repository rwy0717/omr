...
protected:
	...
	/// called for each control flow edge from bytecode at fromIndex, destination bytecode is
	/// fromIndex+relativeBranch
	///   if it's a backward branch (relativeBranch < 0) then the edge is added to the _backwardBranches list
	///   and some flags are updated on the method symbol to indicate it may have loops or, if appropriate, nested
	///   loops finally, genBBStart is called for the destination bytecode to register that it is the start of a
	///   basic block
	void markTarget(int32_t fromIndex, int32_t relativeBranch)
	{
		...
		if (relativeBranch < 0) {
			...
			IndexPair *ip, *prev = 0,
			               *newIP = new (this->comp()->trStackMemory()) IndexPair(fromIndex, toIndex);
	...
	void markExceptionRange(int32_t index, int32_t start, int32_t end, int32_t handler, uint32_t type)
	{
		...
		if (_printByteCodes)
			trfprintf(this->comp()->getOutFile(),
		...
		_tryCatchInfo.insert(_tryCatchInfo.begin() + index,
	...
	/// called when bytecode index "index" has been identified as the target of a branch, so needs its own basic
	/// block
	...
	/// initializes the operand stack state for the given bytecode index, typically called at the beginning of a
	/// basic block
	...
	///   once found, setupBBStartContext is called to ensure everything is ready to start translating bytecodes at
	///   that index
	/// if nothing left to translate, returns _maxByteCode + 8 as a number bigger than the exit test for the
	/// simulation loop
	...
	///   NOTE: if copyCount > shiftCount then (copyCount - shiftCount) elements above the top of the original
	///   operand stack will not be set
