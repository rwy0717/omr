...
void
TR::BytecodeBuilder::initialize(TR::IlGeneratorMethodDetails *details, TR::ResolvedMethodSymbol *methodSymbol,
...
OMR::BytecodeBuilder::AddSuccessorBuilders(uint32_t numExits, ...)
{
	...
	for (int32_t e = 0; e < numExits; e++) {
		...
		if ((*builder)->_bcIndex < _bcIndex) // If the successor has a bcIndex < than the current bcIndex this
		                                     // may be a loop
			_methodSymbol->setMayHaveLoops(true);
		transferVMState(builder); // may change what builder points at!
		_successorBuilders->add(*builder); // must be the bytecode builder that comes back from
		                                   // transferVMState()
...
{
	TR_ASSERT_FATAL(
	        _vmState != NULL, "asked to transfer a NULL vmState from builder %p [ bci %d ]", this, _bcIndex);
	if ((*b)->initialVMState()) {
