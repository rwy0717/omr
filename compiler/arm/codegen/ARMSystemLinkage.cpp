...
TR::ARMSystemLinkage::mapStack(TR::ResolvedMethodSymbol *method)
{
	...
	for (TR::ParameterSymbol *parameter = parameterIterator.getFirst(); parameter != NULL
		...
		case TR::Int64:
			...
			break;
		case TR::Float:
			comp()->failCompilation<UnsupportedParameterType>("Compiling methods with a single precision "
			                                                  "floating point parameter is not supported");
			break;
		case TR::Double:
...
void
TR::ARMSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{
...
TR::ARMSystemLinkage::createPrologue(TR::Instruction *cursor)
{
	...
	cursor =
	        generateTrg1Src1ImmInstruction(codeGen, ARMOp_sub, firstNode, stackPtr, stackPtr, frameSize, 0, cursor);
			...
			if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
				cursor = generateMemSrc1Instruction(cg(), ARMOp_str, firstNode, stackSlot,
				        machine->getRealRegister(
				                (TR::RealRegister::RegNum)(TR::RealRegister::gr0 + nextIntArgReg)),
				        cursor);
				nextIntArgReg++;
			} else {
				nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
			...
			if (nextIntArgReg + 1 < getProperties().getNumIntArgRegs()) {
				cursor = generateMemSrc1Instruction(cg(), ARMOp_str, firstNode, stackSlot,
				        machine->getRealRegister(
				                (TR::RealRegister::RegNum)(TR::RealRegister::gr0 + nextIntArgReg)),
				        cursor);
				stackSlot = new (trHeapMemory())
				        TR::MemoryReference(stackPtr, parameter->getParameterOffset() + 4, codeGen);
				cursor = generateMemSrc1Instruction(cg(), ARMOp_str, firstNode, stackSlot,
				        machine->getRealRegister(
				                (TR::RealRegister::RegNum)(TR::RealRegister::gr0 + nextIntArgReg + 1)),
				        cursor);
				nextIntArgReg += 2;
			} else {
				nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
		...
		case TR::Double:
			if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
				cursor = generateMemSrc1Instruction(cg(), ARMOp_fstd, firstNode, stackSlot,
				        machine->getRealRegister(
				                (TR::RealRegister::RegNum)(TR::RealRegister::fp0 + nextFltArgReg)),
				        cursor);
				nextFltArgReg += 1;
			} else {
				nextFltArgReg = getProperties().getNumFloatArgRegs() + 1;
...
TR::ARMSystemLinkage::createEpilogue(TR::Instruction *cursor)
{
	...
	for (int r = TR::RealRegister::gr4; r <= TR::RealRegister::gr11; ++r) {
		auto *stackSlot = new (trHeapMemory()) TR::MemoryReference(
		        stackPtr, (TR::RealRegister::gr11 - r + 1) * 4 + bodySymbol->getLocalMappingCursor(), codeGen);
		cursor = generateMemSrc1Instruction(cg(), ARMOp_ldr, lastNode, stackSlot,
		        machine->getRealRegister((TR::RealRegister::RegNum)r), cursor);
	}
...
TR::MemoryReference *
TR::ARMSystemLinkage::getOutgoingArgumentMemRef(
