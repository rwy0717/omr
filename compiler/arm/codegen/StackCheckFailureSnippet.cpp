...
uint8_t *
storeArgumentItem(TR_ARMOpCodes op, uint8_t *buffer, TR::RealRegister *reg, int32_t offset, TR::CodeGenerator *cg)
{
...
uint8_t *
loadArgumentItem(TR_ARMOpCodes op, uint8_t *buffer, TR::RealRegister *reg, int32_t offset, TR::CodeGenerator *cg)
{
...
TR::ARMStackCheckFailureSnippet::emitSnippetBody()
{
	...
	bool saveLR =
	        (cg()->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() && !machine->getLinkRegisterKilled())
	        ? true
	        : false;
	...
	*(int32_t *)buffer = 0xEA000000
	        | encodeBranchDistance((uintptr_t)buffer, (uintptr_t)getReStartLabel()->getCodeLocation());
