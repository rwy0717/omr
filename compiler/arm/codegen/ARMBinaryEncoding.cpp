...
void
OMR::ARM::Instruction::generateConditionBinaryEncoding(uint8_t *instructionStart)
{
...
uint32_t
encodeHelperBranchAndLink(TR::SymbolReference *symRef, uint8_t *cursor, TR::Node *node, TR::CodeGenerator *cg)
{
	return encodeHelperBranch(true, symRef, cursor, ARMConditionCodeAL, node, cg);
}

uint32_t
encodeHelperBranch(bool isBranchAndLink, TR::SymbolReference *symRef, uint8_t *cursor, TR_ARMConditionCode cc,
...
int32_t
TR::ARMConditionalBranchInstruction::estimateBinaryLength(int32_t currentEstimate)
{
	...
	if (getOpCodeValue() == ARMOp_bl) {
		...
		if (comp->isRecursiveMethodTarget(sym)) {
			...
			*(int32_t *)cursor = (*(int32_t *)cursor)
			...
			if (((method = getSymbolReference()->getSymbol()->getMethodSymbol()) != NULL)
			        && method->isHelper()) {
				*(int32_t *)cursor = encodeHelperBranch(
				        true, getSymbolReference(), cursor, getConditionCode(), getNode(), cg());
			} else {
				...
				if (TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
					...
					*(int32_t *)cursor = imm;
					...
					// common path
					cg()->addExternalRelocation(new (cg()->trHeapMemory())
					                                    TR::ExternalRelocation(cursor, NULL,
...
TR::ARMControlFlowInstruction::estimateBinaryLength(int32_t currentEstimate)
{
	...
	case ARMOp_iflong:
	case ARMOp_setbool: setEstimatedBinaryLength(ARM_INSTRUCTION_LENGTH * 4); break;
	case ARMOp_idiv:
	case ARMOp_setbflt: setEstimatedBinaryLength(ARM_INSTRUCTION_LENGTH * 5); break;
	case ARMOp_setblong:
	case ARMOp_flcmpg:
	case ARMOp_flcmpl:
	case ARMOp_irem: setEstimatedBinaryLength(ARM_INSTRUCTION_LENGTH * 6); break;
	case ARMOp_lcmp: setEstimatedBinaryLength(ARM_INSTRUCTION_LENGTH * 5); break;
	}
	return currentEstimate + getEstimatedBinaryLength();
}
