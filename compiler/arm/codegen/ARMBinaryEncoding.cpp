    ...
    TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return (isBranchAndLink ? 0x0B000000 : 0x0A000000) | encodeBranchDistance((uintptr_t)cursor, target)
        | (((uint32_t)cc) << 28);
}
                ...
                if (distance <= 0x01fffffc && distance >= 0xfe000000) {
                    ...
                    *(int32_t*)cursor = imm;
                    ...
                    cg()->addExternalRelocation(
                        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, NULL, TR_AbsoluteMethodAddress, cg()),
                        __FILE__, __LINE__, getNode());
                }
            }
        }
