...
int32_t
TR::ARMConstantDataSnippet::addConstantRequest(void *v, TR::DataType type, TR::Instruction *nibble0,
...
bool
TR::ARMConstantDataSnippet::getRequestorsFromNibble(TR::Instruction *nibble, TR::Instruction **q, bool remove)
{
			...
			*(intptrj_t *)codeCursor = acursor->getConstantValue();
			if (cg()->profiledPointersRequireRelocation()) {
				...
				if (acursor->isUnloadablePicSite()) {
					...
					// The patching code thinks it's low bit tagging an instruction not a class
					// pointer!!
			...
			for (i = 0; i < size; i += count) {
				...
				if (!cg()->comp()->compileRelocatableCode()) {
					...
					*(uint16_t *)iloc2 = ((*((uint16_t *)iloc2)) & 0xf000) | 0x800
					        | localVal.getByte2();
					*(uint16_t *)iloc3 = ((*((uint16_t *)iloc3)) & 0xf000) | 0xc00
					        | localVal.getByte1();
