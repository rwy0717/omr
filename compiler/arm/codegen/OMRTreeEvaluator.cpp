...
TR::Instruction *
armLoadConstant(TR::Node *node, int32_t value, TR::Register *trgReg, TR::CodeGenerator *cg, TR::Instruction *cursor)
{
	intParts localVal(value);
	uint32_t bitValue = value, notBits = ~bitValue;
	uint32_t bitTrailing = trailingZeroes(bitValue) & ~1;
	...
	if ((base & 0xFFFFFF00) == 0) // is Immed8r
	{
		...
		cursor =
		        generateTrg1ImmInstruction(cg, ARMOp_mov, node, trgReg, base & 0x000000FF, bitTrailing, cursor);
	...
	{
		cursor = generateTrg1ImmInstruction(
		        cg, ARMOp_mov, node, trgReg, (base >> 8) & 0x000000FF, 8 + bitTrailing, cursor);
		cursor = generateTrg1Src1ImmInstruction(
		        cg, ARMOp_add, node, trgReg, trgReg, base & 0x000000FF, bitTrailing, cursor);
	} else if ((notBase & 0xFFFF0000) == 0) // reversed 16bits
	{
		cursor = generateTrg1ImmInstruction(
		        cg, ARMOp_mov, node, trgReg, (notBase >> 8) & 0x000000FF, 8 + notTrailing, cursor);
		cursor = generateTrg1Src1ImmInstruction(
		        cg, ARMOp_add, node, trgReg, trgReg, notBase & 0x000000FF, notTrailing, cursor);
		cursor = generateTrg1Src1Instruction(cg, ARMOp_mvn, node, trgReg, trgReg, cursor);
	} else if ((base & 0xFF000000) == 0) // 24bit Value
	{
		cursor = generateTrg1ImmInstruction(
		        cg, ARMOp_mov, node, trgReg, (base >> 16) & 0x000000FF, 16 + bitTrailing, cursor);
		if (((base >> 8) & 0x000000FF) != 0)
			cursor = generateTrg1Src1ImmInstruction(
...
TR::Instruction *
fixedSeqMemAccess(TR::CodeGenerator *cg, TR::Node *node, int32_t addr, TR::Instruction **nibbles, TR::Register *trgReg,
...
loadAddressConstantInSnippet(TR::CodeGenerator *cg, TR::Node *node, intptrj_t address, TR::Register *trgReg,
        bool isUnloadablePicSite, TR::Instruction *cursor)
{
	TR::Instruction *q[5];
...
TR::Instruction *
loadAddressConstantFixed(TR::CodeGenerator *cg, TR::Node *node, intptrj_t value, TR::Register *trgReg,
        TR::Instruction *cursor, uint8_t *targetAddress, uint8_t *targetAddress2, int16_t typeAddress,
        ...
        TR::Instruction *cursor, bool isPicSite, int16_t typeAddress, uint8_t *targetAddress, uint8_t *targetAddress2)
{
	if (cg->comp()->compileRelocatableCode())
		return loadAddressConstantFixed(
		        cg, node, value, trgReg, cursor, targetAddress, targetAddress2, typeAddress);
...
TR::Register *
OMR::ARM::TreeEvaluator::commonLoadEvaluator(
...
TR::Register *
OMR::ARM::TreeEvaluator::awrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::MemoryReference *tempMR = new (cg->trHeapMemory()) TR::MemoryReference(node, 4, cg);
...
TR::Register *
OMR::ARM::TreeEvaluator::commonStoreEvaluator(
        TR::Node *node, TR_ARMOpCodes memToRegOp, int32_t memSize, TR::CodeGenerator *cg)
{
	TR::MemoryReference *tempMR = new (cg->trHeapMemory()) TR::MemoryReference(node, memSize, cg);
	bool needSync = (node->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP());
	...
	if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
		generateInstruction(
		        cg, (TR::Compiler->target.cpu.id() == TR_ARMv6) ? ARMOp_dmb_v6 : ARMOp_dmb_st, node);
	}
...
TR::Register *
OMR::ARM::TreeEvaluator::integerHighestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
TR::Register *
OMR::ARM::TreeEvaluator::integerLowestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
TR::Register *
OMR::ARM::TreeEvaluator::integerNumberOfLeadingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
TR::Register *
OMR::ARM::TreeEvaluator::integerNumberOfTrailingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::longHighestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::longNumberOfLeadingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
TR::Register *
OMR::ARM::TreeEvaluator::longNumberOfTrailingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::reverseLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::reverseStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::arraytranslateEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::arraysetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::arraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
bool
OMR::ARM::TreeEvaluator::stopUsingCopyReg(TR::Node *node, TR::Register *&reg, TR::CodeGenerator *cg)
{
	if (node != NULL) {
        ...
        TR::Node *node, int32_t byteLen, TR::Register *src, TR::Register *dst, TR::CodeGenerator *cg)
{
	...
	if (residual & 1) {
		generateTrg1MemInstruction(cg, ARMOp_ldrb, node, regs[0],
		        new (cg->trHeapMemory()) TR::MemoryReference(src, (4 * ri) + ri_x, cg));
		generateMemSrc1Instruction(cg, ARMOp_strb, node,
...
TR::Register *
OMR::ARM::TreeEvaluator::arraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateImmSymInstruction(
	        cg, ARMOp_bl, node, (uintptr_t)arrayCopyHelper->getMethodAddress(), deps, arrayCopyHelper);
...
TR::Register *
OMR::ARM::TreeEvaluator::asynccheckEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
		...
		negated = tmpChild < 0 && tmpChild != 0x80000000; // the negative of the maximum negative int is itself
		                                                  // so can not use cmn here
...
#if J9_PROJECT_SPECIFIC
TR::Register *
OMR::ARM::TreeEvaluator::instanceofEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return VMinstanceOfEvaluator(node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::checkcastEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return OMR::ARM::TreeEvaluator::VMcheckcastEvaluator(node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::checkcastAndNULLCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return OMR::ARM::TreeEvaluator::VMcheckcastEvaluator(node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::newObjectEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::SymbolReference *symRef = node->getSymbolReference();
...
TR::Register *
OMR::ARM::TreeEvaluator::performCall(TR::Node *node, bool isIndirect, TR::CodeGenerator *cg)
{
	TR::SymbolReference *symRef = node->getSymbolReference();
	TR::MethodSymbol *callee = symRef->getSymbol()->castToMethodSymbol();
	TR::Linkage *linkage = cg->getLinkage(callee->getLinkageConvention());
...
TR::Register *
OMR::ARM::TreeEvaluator::indirectCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::SymbolReference *symRef = node->getSymbolReference();
	TR::MethodSymbol *callee = symRef->getSymbol()->castToMethodSymbol();
	TR::Linkage *linkage = cg->getLinkage(callee->getLinkageConvention());
...
TR::Register *
OMR::ARM::TreeEvaluator::exceptionRangeFenceEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::loadaddrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (mref->getUnresolvedSnippet() != NULL) {
		...
		if (mref->useIndexedForm()) {
			...
			generateTrg1Src2Instruction(
			        cg, ARMOp_add, node, resultReg, mref->getBaseRegister(), mref->getIndexRegister());
		} else {
...
TR::Register *
OMR::ARM::TreeEvaluator::aRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::iRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
...
TR::Register *
OMR::ARM::TreeEvaluator::lRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
...
TR::Register *
OMR::ARM::TreeEvaluator::iRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::lRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	TR::Register *globalReg = cg->evaluate(child);
	child->decReferenceCount();
...
TR::Register *
OMR::ARM::TreeEvaluator::GlRegDepsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	int i;
		...
		if (cg->getCurrentEvaluationTreeTop() == comp->getStartTree()) {
			for (i = 0; i < child->getNumChildren(); i++) {
				TR::ParameterSymbol *sym =
				        child->getChild(i)->getSymbolReference()->getSymbol()->getParmSymbol();
				if (sym != NULL) {
...
OMR::ARM::TreeEvaluator::BBEndEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (NULL == block->getNextBlock()) {
		// PR108736 If bl jitThrowException is the last instruction, jitGetExceptionTableFromPC fails to find
		// the method.
...
TR::Register *
OMR::ARM::TreeEvaluator::conversionAnalyser(
...
static void
generateSignOrZeroExtend(TR::Node *node, TR::Register *dst, TR::Register *src, bool needSignExtend, int32_t bitsInDst,
        TR::CodeGenerator *cg)
{
