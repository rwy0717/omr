    ...
    TR::RealRegister* forced, bool excludeGPR0, bool isSinglePrecision)
{
    ...
    case TR_FPR:
        ...
        generateTrg1MemInstruction(self()->cg(), (isSinglePrecision ? ARMOp_flds : ARMOp_fldd), currentNode, best,
    ...
    TR::Register* spilledRegister, TR::RealRegister* targetRegister, bool excludeGPR0, bool isSinglePrecision)
{
    ...
    if (self()->cg()->comp()->getOption(TR_DisableOOL)) {
        ...
        case TR_FPR:
            ...
            generateMemSrc1Instruction(self()->cg(), (isSinglePrecision ? ARMOp_fsts : ARMOp_fstd), currentNode,
        ...
        if (self()->cg()->isOutOfLineColdPath()) {
            ...
            // Also free the spill if maxSpillDepth==0, which will be the case if the reverse spill also occured on the
            // hot path. If the reverse spill occured on both paths then this is the last chance we have to free the
            // spill slot.
            if (location->getMaxSpillDepth() == 3 || location->getMaxSpillDepth() == 0 || isOOLentryReverseSpill) {
                if (location->getMaxSpillDepth() != 0)
                    ...
                    self()->cg()->traceRegisterAssignment(
                        "\nOOL: reverse spill %s in less dominant path (%d / 3), reverse spill on both paths "
                        "indicated, free spill slot (%p)\n",
