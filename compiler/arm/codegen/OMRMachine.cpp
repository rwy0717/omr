...
TR::RealRegister *
OMR::ARM::Machine::findBestFreeRegister(
...
TR::RealRegister *
OMR::ARM::Machine::freeBestRegister(TR::Instruction *currentInstruction, TR_RegisterKinds rk, TR::RealRegister *forced,
        bool excludeGPR0, bool isSinglePrecision)
{
	...
	case TR_GPR:
		if (candidates[0]->getBackingStorage()) {
			...
			if (!location->isOccupied()) {
				...
				// unlocking the list (see TODO in ARMInstruction.cpp). Therefore we need to set the
	...
	case TR_FPR:
		...
		if (candidates[0]->getBackingStorage()) {
			...
			location = self()->cg()->allocateSpill(8, false, NULL); // TODO: use 4 when floatSpill is
			                                                        // implemented
		}
		break;
	}
	...
	tmemref = new (self()->cg()->trHeapMemory()) TR::MemoryReference(
	        currentNode, location->getSymbolReference(), ((rk == TR_FPR) ? 8 : 4), self()->cg());
	...
	case TR_FPR:
		...
		generateTrg1MemInstruction(self()->cg(), (isSinglePrecision ? ARMOp_flds : ARMOp_fldd), currentNode,
...
TR::RealRegister *
OMR::ARM::Machine::reverseSpillState(TR::Instruction *currentInstruction, TR::Register *spilledRegister,
        TR::RealRegister *targetRegister, bool excludeGPR0, bool isSinglePrecision)
{
	TR::MemoryReference *tmemref;
	...
	if (self()->cg()->comp()->getOption(TR_DisableOOL)) {
		...
		case TR_GPR:
			...
			generateMemSrc1Instruction(
			        self()->cg(), ARMOp_str, currentNode, tmemref, targetRegister, currentInstruction);
			break;
		case TR_FPR:
			self()->cg()->freeSpill(location, 8, 0); // TODO: use 4 when floatSpill is implemented
			// if targetRegister is FS0-FS15, using ARMOp_fsts.
			generateMemSrc1Instruction(self()->cg(), (isSinglePrecision ? ARMOp_fsts : ARMOp_fstd),
	...
	if (targetRegister->getState() == TR::RealRegister::Free
		...
		if (currentAssignedRegister != NULL) {
			...
			if (spareReg == NULL) {
				...
				TR_ASSERT(!isSinglePrecision(registerNumber),
				        "We want to free all the registers between FS0 and FS15 before DirectToJNI "
				        "call.");
				registerCopy(currentInstruction, rk, targetRegister, spareReg, self()->cg());
        ...
        TR::RealRegister *sourceReg, TR::RealRegister *middleReg, TR::CodeGenerator *cg)
{
	...
	if (rk != TR_GPR) {
		...
		new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
		        precedingInstruction, ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
		new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
		        precedingInstruction, ARMOp_eor, node, sourceReg, targetReg, sourceReg, cg);
		new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
		        precedingInstruction, ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
	}
}
...
TR::RealRegister *
OMR::ARM::Machine::assignSingleRegister(TR::Register *virtualRegister, TR::Instruction *currentInstruction)
{
	...
	if (assignedRegister == NULL) {
		if (virtualRegister->getTotalUseCount() != virtualRegister->getFutureUseCount()) {
			...
			if ((assignedRegister = self()->findBestFreeRegister(kindOfRegister, false, true)) == NULL) {
				assignedRegister =
				        self()->freeBestRegister(currentInstruction, kindOfRegister, NULL, false);
			}
		}
...
OMR::ARM::Machine::takeRegisterStateSnapShot()
{
	int32_t i;
	TR::Compilation *comp = self()->cg()->comp();
	for (i = TR::RealRegister::FirstGPR; i < TR::RealRegister::gr12; i++) // Skipping the special/reserved register.
	                                                                      // Add FPR later
	{
	...
	for (i = TR::RealRegister::FirstFPR; i < TR::RealRegister::LastFPR; i++) // Skipping the special/reserved
	                                                                         // register.  Add FPR later
	{
		_registerStatesSnapShot[i] = _registerFile[i]->getState();
		_assignedRegisterSnapShot[i] = _registerFile[i]->getAssignedRegister();
		_registerFlagsSnapShot[i] = _registerFile[i]->getFlags();
