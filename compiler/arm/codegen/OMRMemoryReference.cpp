        ...
        : _symbolReference(cg->comp()->getSymRefTab())
{
	...
	if (mr.getUnresolvedSnippet() != NULL) {
		...
		_unresolvedSnippet = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
		        cg, 0, &_symbolReference, mr.getUnresolvedSnippet()->resolveForStore(), false);
...
void
OMR::ARM::MemoryReference::addToOffset(TR::Node *node, int32_t amount, TR::CodeGenerator *cg)
{
	if (self()->getUnresolvedSnippet() != NULL) {
...
void
OMR::ARM::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
...
void
OMR::ARM::MemoryReference::consolidateRegisters(
	...
	if (self()->getUnresolvedSnippet() != NULL) {
		...
		if (_indexRegister != NULL) {
			if (self()->isBaseModifiable())
				...
				tempTargetRegister = cg->allocateRegister();
			new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
			        ARMOp_add, srcTree, tempTargetRegister, _baseRegister, _indexRegister, cg);
...
void
OMR::ARM::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
	...
	if (_baseRegister != NULL) {
		...
		if (assignedBaseRegister == NULL) {
			if (_baseRegister->getTotalUseCount() == _baseRegister->getFutureUseCount()) {
				if ((assignedBaseRegister = machine->findBestFreeRegister(TR_GPR, true, true))
				        == NULL) {
				...
				assignedBaseRegister =
				        machine->reverseSpillState(currentInstruction, _baseRegister, NULL, true);
			}
	...
	if (_indexRegister != NULL) {
		...
		if (assignedIndexRegister == NULL) {
			if (_indexRegister->getTotalUseCount() == _indexRegister->getFutureUseCount()) {
				if ((assignedIndexRegister = machine->findBestFreeRegister(TR_GPR, false, true))
				        == NULL) {
	...
	if (_modBase != NULL) {
		...
		if (assignedModBaseRegister == NULL) {
			if (_modBase->getTotalUseCount() == _modBase->getFutureUseCount()) {
				if ((assignedModBaseRegister = machine->findBestFreeRegister(TR_GPR, true, true))
				        == NULL) {
        ...
        TR::Instruction *currentInstruction, uint8_t *cursor, TR::CodeGenerator *cg)
{
	...
	if (self()->getUnresolvedSnippet()) {
		...
		*wcursor = 0xEA000000; // insert a branch to the snippet
		...
		if (!index && op != ARMOp_add) // common encoding for all true load/store instructions with immediate
		                               // offsets
		{
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
				...
				*wcursor |= 1 << 23; // set U bit for +ve
				...
				if (constantIsImmed12(displacement)) {
					*wcursor |= displacement < 0 ? -displacement : displacement;
					...
					cursor =
					        self()->encodeLargeARMConstant(wcursor, cursor, currentInstruction, cg);
					wcursor = (uint32_t *)cursor;
				}
			}
		} else if (op == ARMOp_ldrsb || op == ARMOp_ldrh || op == ARMOp_ldrsh || op == ARMOp_strh) // addressing
		                                                                                           // mode 3
		{
			if (index) {
				base->setRegisterFieldRN(wcursor);
        ...
        uint32_t *wcursor, uint8_t *cursor, TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
	...
	// if doing a ldr then can re-use the target register as the index register (if it is not also the base
	// register), stores must always spill.
...
choose_rX(TR::Instruction *currentInstruction, TR::RealRegister *base)
{
	...
	switch (exclude) {
	case 0:
	case 2: return (TR::RealRegister::gr3);
	case 1: return (TR::RealRegister::gr2);
	case 3: return (TR::RealRegister::gr1);
	}
	return (TR::RealRegister::gr3);
}

static uint8_t *
armLoadConstantInstructions(
        TR::RealRegister *trgReg, int32_t value, uint8_t *cursor, uint32_t *wcursor, TR::CodeGenerator *cg)
{
        ...
        TR::Node *node, TR::SymbolReference *ref, TR::Register *reg, TR::MemoryReference *mr, TR::CodeGenerator *cg)
{
	...
	TR_ASSERT(
	        symbol->isStatic() || symbol->isMethod(), "loadRelocatableConstant, problem with new symbol hierarchy");
	...
	if (ref->isUnresolved() || comp->compileRelocatableCode()) {
		...
		if (symbol->isCountForRecompile()) {
			...
			cg->addSnippet(mr->setUnresolvedSnippet(new (cg->trHeapMemory())
			                TR::UnresolvedDataSnippet(cg, node, ref, node->getOpCode().isStore(), false)));
		}
	} else {
