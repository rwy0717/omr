...
void
TR_ARMSubtractAnalyser::integerSubtractAnalyser(
        TR::Node *root, TR_ARMOpCodes regToRegOpCode, TR_ARMOpCodes memToRegOpCode)
{
	...
	if (getCopyReg1()) {
		...
		} else // assert getSubReg3Mem2() == true
		{
			...
			//         TR::ARMRegMemInstruction(memToRegOpCode, thirdReg, tempMR, cg());
			//         tempMR->decNodeReferenceCounts();
		}
		root->setRegister(thirdReg);
	} else if (getSubReg1Reg2()) {
		new (cg()->trHeapMemory())
		        TR::ARMTrg1Src1Instruction(ARMOp_sub, root, firstRegister, secondRegister, cg());
	...
	} else // assert getSubReg1Mem2() == true
	{
		...
		//      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(memToRegOpCode, firstRegister, tempMR,
		//      cg()); tempMR->decNodeReferenceCounts(); root->setRegister(firstRegister);
	}
...
TR_ARMSubtractAnalyser::longSubtractAnalyser(TR::Node *root)
{
	...
	if (getCopyReg1()) {
		...
		} else // assert getSubReg3Mem2() == true
		{
			...
			//         TR_ARMMemoryReference(secondChild); new (cg()->trHeapMemory())
			//         TR::ARMRegMemInstruction(SUB4RegMem, lowThird, lowMR, cg()); TR_ARMMemoryReference
			//         *highMR = new (cg()->trHeapMemory()) TR_ARMMemoryReference(*lowMR, 4); new
			//         (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem, highThird, highMR, cg());
			//         lowMR->decNodeReferenceCounts();
		}
		root->setRegister(thirdReg);
	} else if (getSubReg1Reg2()) {
		//      new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(root, ARMOp_subf,
		//      firstRegister->getLowOrder(), secondRegister->getLowOrder(), cg()); new (cg()->trHeapMemory())
	...
	} else // assert getSubReg1Mem2() == true
	{
		...
		//      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SUB4RegMem, firstRegister->getLowOrder(),
		//      lowMR, cg()); TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory())
		//      TR_ARMMemoryReference(*lowMR, 4); new (cg()->trHeapMemory())
		//      TR::ARMRegMemInstruction(SBB4RegMem, firstRegister->getHighOrder(), highMR, cg());
		//      lowMR->decNodeReferenceCounts();
		//      root->setRegister(firstRegister);
	}
