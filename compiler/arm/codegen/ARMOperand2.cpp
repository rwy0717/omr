...
TR_ARMOperand2::incTotalUseCount()
{
	...
	case ARMOp2Reg:
	case ARMOp2RegRRX: _baseRegister->incTotalUseCount(); break;
	}
}
...
TR_ARMOperand2::setBinaryEncoding(uint32_t *instruction)
{
	if (ARMOp2Immed8r == _opType) {
		...
		*instruction |= operandEncodingTable[(uint32_t)_opType] & OP2_ENCODING_MASK;
		...
		switch (opType) {
		case Op2RegImmed: *instruction |= _shiftOrImmed << 7; break;
		case Op2RegReg: {
...
TR_ARMOperand2::block()
{
	...
	case ARMOp2RegRORImmed:
	case ARMOp2Reg:
	case ARMOp2RegRRX: _baseRegister->block();
	}
}
...
TR_ARMOperand2::unblock()
{
	...
	case ARMOp2RegRORImmed:
	case ARMOp2Reg:
	case ARMOp2RegRRX: _baseRegister->unblock();
	}
}
