...
OMR::ARM::TreeEvaluator::gotoEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->getNumChildren() > 0) {
		...
		generateLabelInstruction(
		        cg, ARMOp_b, node, gotoLabel, generateRegisterDependencyConditions(cg, child, 0));
        ...
        TR_ARMConditionCode branchType, TR::Node *node, TR::CodeGenerator *cg, TR::SymbolReference *sr = NULL)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		negated = cmpValue < 0 && cmpValue != 0x80000000; // the negative of the maximum negative int is itself
...
TR::Instruction *
OMR::ARM::TreeEvaluator::compareIntsForEquality(
...
TR::Register *
OMR::ARM::TreeEvaluator::ificmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	compareIntsForEquality(
	        TR::ificmpeq == node->getOpCodeValue() ? ARMConditionCodeEQ : ARMConditionCodeNE, node, cg);
	return NULL;
}
...
TR::Register *
OMR::ARM::TreeEvaluator::ificmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ificmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ificmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ificmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifiucmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifiucmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifiucmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifiucmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::iflcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	...
	TR_ARMConditionCode condition = node->getOpCodeValue() == TR::iflcmpeq ? ARMConditionCodeEQ
	                                                                       : ARMConditionCodeNE;
...
static TR::Register *
compareLongAndSetOrderedBoolean(TR_ARMConditionCode branchOp, TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
static TR::Register *
compareLongsForOrder(TR_ARMConditionCode branchOp, TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
	...
	if (cg->comp()->getOption(TR_DisableOOL) || disableOOLForLongCompares) {
		...
		// Start OOL
		// The compare result has to be live until the end of the last instruction in the mainline, so put it as
		// a dependency
		...
		// Need the two sources for compare to survive until the returnLabel.  Use a dependency to hold that,
		// and hold until the compare for them are done.
		...
		cursorOOL = generateTrg1ImmInstruction(cg, ARMOp_mov, node, tempReg, 0, 0, cursorOOL); // Setting
		                                                                                       // tempReg to 0
		...
		cursorOOL = generateSrc2Instruction(
		        cg, ARMOp_cmp, node, src1Reg->getLowOrder(), src2Reg->getLowOrder(), cursorOOL);
		cursorOOL = generateTrg1ImmInstruction(cg, ARMOp_mov, node, tempReg, 1, 0, cursorOOL); // Setting
		                                                                                       // tempReg to 1
		cursorOOL->setConditionCode(isGT ? ARMConditionCodeHI : ARMConditionCodeLS);
		cursorOOL = generateLabelInstruction(cg, ARMOp_label, node, returnLabel, oolDeps, cursorOOL); // Need
		                                                                                              // deps
		                                                                                              // 3/21
		                                                                                              // change
...
TR::Register *
OMR::ARM::TreeEvaluator::iflcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *
OMR::ARM::TreeEvaluator::iflcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *
OMR::ARM::TreeEvaluator::iflucmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *
OMR::ARM::TreeEvaluator::iflucmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *
OMR::ARM::TreeEvaluator::iflcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return compareLongsForOrder(ARMConditionCodeGT, node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::iflcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return compareLongsForOrder(ARMConditionCodeLE, node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::iflucmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return compareLongsForOrder(ARMConditionCodeHI, node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::iflucmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return compareLongsForOrder(ARMConditionCodeLS, node, cg);
}

TR::Register *
OMR::ARM::TreeEvaluator::ifacmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	for (int i = 2; i < node->getNumChildren(); i++) {
		...
		if (unbalanced) {
			...
			if (caseNode->getNumChildren() > 0) {
				...
				cg->evaluate(caseDepsNode);
				caseDeps =
				        caseDeps->clone(cg, generateRegisterDependencyConditions(cg, caseDepsNode, 0));
			}
		...
		if (unbalanced) {
			...
			if (caseNode->getNumChildren() > 0) {
				...
				caseDeps = caseDeps->clone(
				        cg, generateRegisterDependencyConditions(cg, caseNode->getFirstChild(), 0));
			}
			generateConditionalBranchInstruction(cg, node, ARMConditionCodeEQ, caseTargetLabel, caseDeps);
		} else {
			generateConditionalBranchInstruction(cg, node, ARMConditionCodeEQ, caseTargetLabel);
...
lookupScheme4(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	/*  TODO implement lookups
	   ...
	   *pivotRegister = cg->allocateRegister(); TR::RegisterDependencyConditions *conditions = new
	   (cg->trHeapMemory()) TR::RegisterDependencyConditions(7, 7);
...
OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (numBranchTableEntries >= 5) {
		...
		// OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must be updated to synch.
	...
	if (numBranchTableEntries < 5) {
		...
		generateConditionalBranchInstruction(cg, node, ARMConditionCodeAL,
		...
		for (i = 2; i < total - 1; i++) {
			generateLabelInstruction(
			        cg, ARMOp_b, node, node->getChild(i)->getBranchDestination()->getNode()->getLabel());
		}
		generateLabelInstruction(cg, ARMOp_b, node,
...
OMR::ARM::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR_ARMOutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
	        TR_ARMOutOfLineCodeSection(node, TR::call, NULL, slowPathLabel, restartLabel, cg);
	...
	if (!secondChild->getOpCode().isLoadConst() || (dtype.isInt32() && secondChild->getInt() == 0)
		...
		if (dtype.isInt64()) {
			...
			generateTrg1Src2Instruction(
			        cg, ARMOp_orr, node, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder());
			srcReg = trgReg;
		}
...
TR::Register *
OMR::ARM::TreeEvaluator::ArrayCopyBNDCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (/* !isSmall() && */ firstChild->getOpCode().isLoadConst()) {
		if (secondChild->getOpCode().isLoadConst()) {
			if (firstChild->getInt() < secondChild->getInt()) {
				...
				instr = generateImmSymInstruction(cg, ARMOp_bl, node,
				        (uintptr_t)BNDCHKException->getMethodAddress(), NULL, BNDCHKException, NULL,
				        NULL, ARMConditionCodeAL);
				cg->machine()->setLinkRegisterKilled(true);
			} else {
...
TR::Register *
OMR::ARM::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ArrayCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
generateMaxMin(TR::Node *node, TR::CodeGenerator *cg, bool max)
{
	...
	for (; i < n; i++) {
		...
		if (two_reg) {
			...
			cursor = generateTrg1Src1Instruction(
			        cg, ARMOp_mov, node, trgReg->getHighOrder(), reg->getHighOrder(), cursor);
			cursor->setConditionCode(ARMConditionCodeNE);
			cursor = generateConditionalBranchInstruction(
