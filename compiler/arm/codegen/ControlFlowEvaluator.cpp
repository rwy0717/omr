...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_VirtualGuard* virtualGuard = comp->findVirtualGuardInfo(node);
    if (!((comp->performVirtualGuardNOPing() || node->isHCRGuard()) && comp->isVirtualGuardNOPingRequired(virtualGuard))
...
static TR::Register* compareLongsForOrder(TR_ARMConditionCode branchOp, TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->getOption(TR_DisableOOL) || disableOOLForLongCompares) {
        ...
        cursorOOL
            = generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg->getLowOrder(), src2Reg->getLowOrder(), cursorOOL);
...
TR::Register* OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!secondChild->getOpCode().isLoadConst() || (dtype.isInt32() && secondChild->getInt() == 0)
        ...
        TR::Instruction* instr1
            = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)arithmeticException->getMethodAddress(), NULL,
                arithmeticException, NULL, NULL, ARMConditionCodeEQ);
        instr1->ARMNeedsGCMap(0xFFFFFFFF);
        cg->machine()->setLinkRegisterKilled(true);
    }
    ...
    if (/* !isSmall() && */ firstChild->getOpCode().isLoadConst()) {
        if (secondChild->getOpCode().isLoadConst()) {
            if (firstChild->getInt() < secondChild->getInt()) {
                ...
                instr = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)BNDCHKException->getMethodAddress(),
...
static TR::Register* generateMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
    ...
    for (; i < n; i++) {
        ...
        if (two_reg) {
            ...
            cursor
                = generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
            cursor->setConditionCode(ARMConditionCodeNE);
            cursor
                = generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg->getHighOrder(), reg->getHighOrder(), cursor);
            cursor->setConditionCode(ARMConditionCodeNE);
