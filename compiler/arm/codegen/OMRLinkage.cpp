...
void
OMR::ARM::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{}
...
void
OMR::ARM::Linkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method)
{}
...
OMR::ARM::Linkage::saveArguments(TR::Instruction *cursor)
{
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		//      if (lri >=  0)
		//         {
		// We cannot have linkageRegisterIndex as it breaks GRA. Using numIntArgs == 0 to distinguish "this"
		// pointer.
		...
		case TR::Address:
			if (hasToBeOnStack) {
				argRegister =
				        machine->getRealRegister(properties.getIntegerArgumentRegister(numIntArgs));
				cursor = generateMemSrc1Instruction(codeGen, ARMOp_str, firstNode,
				        new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset, codeGen),
				        argRegister, cursor);
			}
			numIntArgs++;
			break;
		...
		if (sym) {
			if (sym->isAddressOfClassObject()) {
				...
				TR::Instruction *instr =
				        generateTrg1Src1Instruction(self()->cg(), ARMOp_mov, child, pushReg, addrReg);
...
OMR::ARM::Linkage::pushAddressArg(TR::Node *child)
{
	...
	if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
		...
		pushRegister = codeGen->allocateRegister();
		if (isClass
		        && self()->cg()->wantToPatchClassPointer((TR_OpaqueClassBlock *)child->getAddress(), child)) {
			loadAddressConstantInSnippet(self()->cg(), child, child->getAddress(), pushRegister);
		} else {
			if (child->isMethodPointerConstant())
				loadAddressConstant(self()->cg(), child, child->getAddress(), pushRegister, NULL, false,
...
{
	switch (kind) {
	case TR_GPR: return self()->getProperties().getNumIntArgRegs();
	case TR_FPR: return self()->getProperties().getNumFloatArgRegs();
	default: return 0;
	}
}

int32_t
OMR::ARM::Linkage::buildARMLinkageArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
        TR::Register *&vftReg, TR_LinkageConventions conventions, bool isVirtualOrJNI)
{
		...
		case TR::Double:
			...
			if (isSpecialArg) {
				...
				if (specialArgReg == properties.getIntegerReturnRegister(0)) {
					...
					TR::addDependency(
					        dependencies, specialArgRegister, specialArgReg, TR_GPR, self()->cg());
				}
			} else {
				if (numIntegerArgs < numIntArgRegs) {
					if (!self()->cg()->canClobberNodesRegister(child, 0)
						...
						if (bigEndian) {
							...
							generateTrg1Src1Instruction(self()->cg(), ARMOp_mov, child,
							        tempReg, reg->getRegisterPair()->getLowOrder());
							reg = self()->cg()->allocateRegisterPair(
							        tempReg, reg->getRegisterPair()->getHighOrder());
						}
					}
					...
					if (numIntegerArgs + 1 < numIntArgRegs) {
						if (!self()->cg()->canClobberNodesRegister(child, 0)
							...
							tempReg = self()->cg()->allocateRegister();
							if (bigEndian) {
								generateTrg1Src1Instruction(self()->cg(), ARMOp_mov,
								        child, tempReg,
								        reg->getRegisterPair()->getLowOrder());
								reg->getRegisterPair()->setLowOrder(
								        tempReg, self()->cg());
							} else {
								generateTrg1Src1Instruction(self()->cg(), ARMOp_mov,
								        child, tempReg,
								        reg->getRegisterPair()->getHighOrder());
								reg->getRegisterPair()->setHighOrder(
								        tempReg, self()->cg());
							}
						}
						dependencies->addPreCondition(bigEndian
						                ? reg->getRegisterPair()->getLowOrder()
						                : reg->getRegisterPair()->getHighOrder(),
						...
						printf("pushing %s word of 64-bit arg %d %d %d %d\n",
						        bigEndian ? "low" : "high", numIntegerArgs, memArg, totalSize,
						        argSize);
						fflush(stdout);
#endif
	...
	getProperties().getComputedCallTargetRegister()); dependencites->getPostConditions()->setDependencyInfo(0,
	vftReg, getProperties().getComputedCallTargetRegister());
	      }
	   }
	*/
	...
	for (i = 0; i < 8 /*properties.getNumFloatArgRegs()*/; i++) {
		TR::addDependency(dependencies, NULL, (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 + i),
		        TR_FPR, self()->cg());
	}
#endif
	...
	if (numMemArgs > 0) {
      ...
      for (i = memFrom; (isJNI &&  i >= memTo) || (!isJNI && i < memTo); i += memStep)
		...
		{
#ifdef DEBUG_ARM_LINKAGE
			printf("pushing mem arg %d of %d (in reg %x) to [sp + %d]... ", i, numMemArgs,
			        pushToMemory[i].argRegister, pushToMemory[i].argMemory->getOffset());
			fflush(stdout);
#endif
			generateMemSrc1Instruction(self()->cg(), pushToMemory[i].opCode, callNode,
			        pushToMemory[i].argMemory, pushToMemory[i].argRegister);
#ifdef DEBUG_ARM_LINKAGE
			printf("done\n");
			fflush(stdout);
#endif
...
TR::Register *
OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node *callNode, bool isSystem)
{
	TR::CodeGenerator *codeGen = self()->cg();
