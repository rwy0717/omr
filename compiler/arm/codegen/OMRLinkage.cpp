...
TR::Register* OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node* callNode, bool isSystem)
{
    ...
    if ((callSymbol->isJITInternalNative() || (!callSymRef->isUnresolved() && !callSymbol->isInterpreted() && ((codeGen->comp()->compileRelocatableCode() && callSymbol->isHelper()) || !codeGen->comp()->compileRelocatableCode())))) {
