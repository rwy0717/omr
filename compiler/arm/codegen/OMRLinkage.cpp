                ...
                if (numIntegerArgs < numIntArgRegs) {
                    ...
                    if (numIntegerArgs == 0 /* &&
                       ...
                       child->getDataType() == TR::Address) */)
                     {
                        ...
                        addDependency(dependencies, reg, properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR,
                            self()->cg());
                    }
                } else {
#ifdef DEBUG_ARM_LINKAGE
...
TR::Register* OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node* callNode, bool isSystem)
{
    ...
    if ((callSymbol->isJITInternalNative()
            || (!callSymRef->isUnresolved() && !callSymbol->isInterpreted()
                   && ((codeGen->comp()->compileRelocatableCode() && callSymbol->isHelper())
        ...
        gcPoint = generateImmSymInstruction(codeGen, ARMOp_bl, callNode, 0, dependencies,
            new (self()->trHeapMemory()) TR::SymbolReference(codeGen->comp()->getSymRefTab(), label), snippet);
#else
