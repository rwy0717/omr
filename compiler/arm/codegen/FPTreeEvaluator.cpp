...
static TR::Register *
moveToDoubleRegister(TR::Node *node, TR::Register *srcReg, TR::CodeGenerator *cg)
{
	if (srcReg->getKind() == TR_FPR) {
...
static TR::Register *
moveFromFloatRegister(TR::Node *node, TR::Register *srcReg, TR::CodeGenerator *cg)
{
...
static TR::Register *
moveFromDoubleRegister(TR::Node *node, TR::Register *srcReg, TR::CodeGenerator *cg)
{
...
static TR::Register *
singlePrecisionEvaluator(TR::Node *node, TR_ARMOpCodes opCode, TR::CodeGenerator *cg)
{
...
static TR::Register *
doublePrecisionEvaluator(TR::Node *node, TR_ARMOpCodes opCode, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = NULL;
	TR::Register *src1Reg = cg->evaluate(firstChild);
...
callLong2FloatHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.isLittleEndian()) {
		...
		for (i = 1; i < 8; i++) {
			TR::RealRegister::RegNum realReg =
			        (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 + i);
			dependencies->addPostCondition(cg->allocateSinglePrecisionRegister(), realReg); // TODO live
			                                                                                // register
		}
		dependencies->addPostCondition(cg->allocateRegister(), TR::RealRegister::gr1); // TODO live register
		dependencies->addPostCondition(cg->allocateRegister(), TR::RealRegister::gr0); // TODO live register
...
callDoubleRemainderHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.isLittleEndian()) {
		...
		dependencies->addPostCondition(trgReg, TR::RealRegister::fp0);
		dependencies->addPostCondition(cg->allocateRegister(TR_FPR), TR::RealRegister::fp1); // TODO live
		                                                                                     // register
#else
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		if (noFPRA) {
			...
			if (tempMR->getIndexRegister() && tempMR->getBaseRegister()) {
				...
				generateTrg1Src2Instruction(cg, ARMOp_add, node, tempReg, tempMR->getBaseRegister(),
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		if (noFPRA) {
			...
			if (highMem->getIndexRegister() && highMem->getBaseRegister()) {
				TR::Register *tempReg = cg->allocateRegister();
				generateTrg1Src2Instruction(cg, ARMOp_add, node, tempReg, highMem->getBaseRegister(),
...
OMR::ARM::TreeEvaluator::dreturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (returnRegister->getKind() == TR_GPR) {
		TR::addDependency(deps, returnRegister->getLowOrder(), cg->getProperties().getLongLowReturnRegister(),
		        TR_GPR, cg);
		TR::addDependency(deps, returnRegister->getHighOrder(), cg->getProperties().getLongHighReturnRegister(),
		        TR_GPR, cg);
	} else if (returnRegister->getKind() == TR_FPR) {
		TR::Register *temp = moveFromDoubleRegister(node, returnRegister, cg);
		TR::addDependency(
		        deps, temp->getLowOrder(), cg->getProperties().getLongLowReturnRegister(), TR_GPR, cg);
		TR::addDependency(
		        deps, temp->getHighOrder(), cg->getProperties().getLongHighReturnRegister(), TR_GPR, cg);
	} else
		TR_ASSERT(0, "Unknown register type\n");
	generateAdminInstruction(cg, ARMOp_ret, node, deps);
 ...
 */
static TR::Register *
generateFusedMultiplyAdd(TR::Node *addNode, TR_ARMOpCodes OpCode, TR::CodeGenerator *cg)
{
	...
	if (addNode->getDataType() == TR::Float) {
		...
		if (noFPRA) {
			...
			generateTrg2Src1Instruction(cg, ARMOp_fmrrd, addNode, trgRegister->getLowOrder(),
...
OMR::ARM::TreeEvaluator::i2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR::Register *doubleTrgReg = cg->allocateRegister(TR_FPR);
	generateTrg1Src1Instruction(cg, opcode, node, doubleTrgReg, tempReg); // The conversion.. source is float,
	                                                                      // result is double
...
OMR::ARM::TreeEvaluator::f2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
		...
		generateTrg1Src1Instruction(
		        cg, ARMOp_fcvtds, node, doubleTrgReg, moveToFloatRegister(node, src1Reg, cg));
		cg->decReferenceCount(firstChild);
	}
...
OMR::ARM::TreeEvaluator::d2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
		...
		src1Reg = cg->evaluate(firstChild);
		generateTrg1Src1Instruction(
		        cg, ARMOp_fcvtsd, node, floatTrgReg, moveToDoubleRegister(node, src1Reg, cg));
		cg->decReferenceCount(firstChild);
	}
...
static void
ifFloatEvaluator(TR::Node *node, TR_ARMOpCodes opCode, TR_ARMConditionCode cond, TR::CodeGenerator *cg, bool isDouble,
        bool unordered)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
...
static TR::Register *
setboolFloatEvaluator(TR::Node *node, TR_ARMOpCodes opCode, TR_ARMConditionCode cond, TR::CodeGenerator *cg,
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::ARM::TreeEvaluator::ifdcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
OMR::ARM::TreeEvaluator::dcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return setboolFloatEvaluator(
	        node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, ARMConditionCodeEQ, cg, isDouble, unordered);
}
...
OMR::ARM::TreeEvaluator::dcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return setboolFloatEvaluator(
	        node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, ARMConditionCodeNE, cg, isDouble, false);
}
...
TR::Register *
OMR::ARM::TreeEvaluator::fRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// Support dRegLoad
...
TR::Register *
OMR::ARM::TreeEvaluator::fRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// Support dRegStore
	TR::Node *child = node->getFirstChild();
...
generateFloatMaxMin(TR::Node *node, TR::CodeGenerator *cg, bool max)
{
	...
	if (noFPRA && reg->getKind() == TR_GPR) {
		...
		if (cg->canClobberNodesRegister(firstChild)) {
			...
			if (isFloat) {
				...
				generateTrg1Src1Instruction(
				        cg, ARMOp_fcpys, node, trgFloatReg, moveToFloatRegister(node, reg, cg));
			} else {
				trgFloatReg = cg->allocateRegister(TR_FPR);
				generateTrg1Src1Instruction(
				        cg, ARMOp_fcpyd, node, trgFloatReg, moveToDoubleRegister(node, reg, cg));
			}
		}
		if (max) {
	...
	cursor = generateTrg1Src1Instruction(
	        cg, (isFloat) ? ARMOp_fcpys : ARMOp_fcpyd, node, trgFloatReg, childFloatReg, cursor);
	cursor->setConditionCode(ARMConditionCodeVS);
	...
	if (isFloat) {
		cursor = generateSrc1ImmInstruction(
		        cg, ARMOp_cmp, node, max ? trgReg : childReg, 0x80, 24, cursor); // 0x80000000
	} else {
	 ...
	 *
	 */
	cursor = generateSrc1ImmInstruction(cg, (isFloat) ? ARMOp_fcmpzs : ARMOp_fcmpzd, node,
	...
	cursor = generateSrc2Instruction(
	        cg, (isFloat) ? ARMOp_fcmps : ARMOp_fcmpd, node, childFloatReg, trgFloatReg, cursor);
	...
	if (noFPRA && reg->getKind() == TR_GPR) {
		if (cg->canClobberNodesRegister(firstChild)) {
			if (isFloat) {
				...
				cursor = generateTrg2Src1Instruction(cg, ARMOp_fmrrd, node, reg->getLowOrder(),
