...
void
TR_ARMBinaryCommutativeAnalyser::genericAnalyser(
...
void
TR_ARMBinaryCommutativeAnalyser::genericLongAnalyser(TR::Node *root, TR_ARMOpCodes lowRegToRegOpCode,
...
void
TR_ARMBinaryCommutativeAnalyser::integerAddAnalyser(TR::Node *root, TR_ARMOpCodes regToRegOpCode)
{
	TR::Node *firstChild;
	TR::Node *secondChild;
	if (cg()->whichChildToEvaluate(root) == 0) {
...
TR_ARMBinaryCommutativeAnalyser::longAddAnalyser(TR::Node *root)
{
	...
	if (getOpReg1Reg2()) {
		//      new TR::ARMRegRegInstruction(ADD4RegReg, firstRegister->getLowOrder(),
		//      secondRegister->getLowOrder()); new TR::ARMRegRegInstruction(ADC4RegReg,
		//      firstRegister->getHighOrder(), secondRegister->getHighOrder());
		//      root->setRegister(firstRegister);
	} else if (getOpReg2Reg1()) {
		//      new TR::ARMRegRegInstruction(ADD4RegReg, secondRegister->getLowOrder(),
		//      firstRegister->getLowOrder()); new TR::ARMRegRegInstruction(ADC4RegReg,
