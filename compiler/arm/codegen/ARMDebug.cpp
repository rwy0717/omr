...
TR_Debug::print(TR::FILE *pOutFile, TR::Instruction *instr)
{
	...
	case OMR::Instruction::IsImm: print(pOutFile, (TR::ARMImmInstruction *)instr); break;
	case OMR::Instruction::IsImmSym: print(pOutFile, (TR::ARMImmSymInstruction *)instr); break;
	case OMR::Instruction::IsLabel: print(pOutFile, (TR::ARMLabelInstruction *)instr); break;
	case OMR::Instruction::IsConditionalBranch:
		print(pOutFile, (TR::ARMConditionalBranchInstruction *)instr);
		break;
#ifdef J9_PROJECT_SPECIFIC
	case OMR::Instruction::IsVirtualGuardNOP: print(pOutFile, (TR::ARMVirtualGuardNOPInstruction *)instr); break;
#endif
	case OMR::Instruction::IsAdmin: print(pOutFile, (TR::ARMAdminInstruction *)instr); break;
	case OMR::Instruction::IsTrg1Src2:
	case OMR::Instruction::IsSrc2:
	case OMR::Instruction::IsTrg1Src1: print(pOutFile, (TR::ARMTrg1Src2Instruction *)instr); break;
	case OMR::Instruction::IsTrg1: print(pOutFile, (TR::ARMTrg1Instruction *)instr); break;
	case OMR::Instruction::IsMem: print(pOutFile, (TR::ARMMemInstruction *)instr); break;
	case OMR::Instruction::IsMemSrc1: print(pOutFile, (TR::ARMMemSrc1Instruction *)instr); break;
	case OMR::Instruction::IsTrg1Mem: print(pOutFile, (TR::ARMTrg1MemInstruction *)instr); break;
	case OMR::Instruction::IsTrg1MemSrc1: print(pOutFile, (TR::ARMTrg1MemSrc1Instruction *)instr); break;
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
	case OMR::Instruction::IsTrg2Src1: print(pOutFile, (TR::ARMTrg2Src1Instruction *)instr); break;
#endif
	case OMR::Instruction::IsMul: print(pOutFile, (TR::ARMMulInstruction *)instr); break;
	case OMR::Instruction::IsControlFlow: print(pOutFile, (TR::ARMControlFlowInstruction *)instr); break;
	case OMR::Instruction::IsMultipleMove: print(pOutFile, (TR::ARMMultipleMoveInstruction *)instr); break;
	default:
...
void
TR_Debug::dumpDependencyGroup(TR::FILE *pOutFile, TR_ARMRegisterDependencyGroup *group, int32_t numConditions,
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARMMultipleMoveInstruction *instr)
{
	...
	if (!instr->getOpCode().isVFPOp()) {
		...
		for (int i = 0; i < nreg; i++) {
			TR::RealRegister::RegNum r =
			        (TR::RealRegister::RegNum)((int)TR::RealRegister::FirstFPR + startReg + i);
...
TR_Debug::print(TR::FILE *pOutFile, TR_ARMOperand2 *op, TR_RegisterSizes size)
{
	...
	case ARMOp2Immed8r:
		if (op->_immedShift)
			trfprintf(pOutFile, "#0x%08x (0x%x << %d)", op->_shiftOrImmed << op->_immedShift,
...
TR_Debug::printa(TR::FILE *pOutFile, TR::Snippet *snippet)
{
	...
	switch (snippet->getKind()) {
	case TR::Snippet::IsCall: print(pOutFile, (TR::ARMCallSnippet *)snippet); break;
	case TR::Snippet::IsUnresolvedCall: print(pOutFile, (TR::ARMUnresolvedCallSnippet *)snippet); break;
	case TR::Snippet::IsVirtualUnresolved: print(pOutFile, (TR::ARMVirtualUnresolvedSnippet *)snippet); break;
	case TR::Snippet::IsInterfaceCall: print(pOutFile, (TR::ARMInterfaceCallSnippet *)snippet); break;
	case TR::Snippet::IsStackCheckFailure: print(pOutFile, (TR::ARMStackCheckFailureSnippet *)snippet); break;
	case TR::Snippet::IsUnresolvedData: print(pOutFile, (TR::UnresolvedDataSnippet *)snippet); break;
	case TR::Snippet::IsRecompilation: print(pOutFile, (TR::ARMRecompilationSnippet *)snippet); break;
#ifdef J9_PROJECT_SPECIFIC
	case TR::Snippet::IsHelperCall: print(pOutFile, (TR::ARMHelperCallSnippet *)snippet); break;
#endif
	...
	for (int i = callNode->getFirstArgumentIndex(); i < callNode->getNumChildren(); i++) {
		...
		case TR::Address:
			...
			if (numIntArgs < linkage.getNumIntArgRegs()) {
				...
				print(pOutFile,
				        machine->getRealRegister(linkage.getIntegerArgumentRegister(numIntArgs)));
				bufferPos += 4;
			}
			numIntArgs++;
			if (linkage.getRightToLeft())
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARMUnresolvedCallSnippet *snippet)
{
	...
	case TR::NoType: helperLookupOffset = 0; break;
	case TR::Int32:
	case TR::Address: helperLookupOffset = 4; break;
	case TR::Int64: helperLookupOffset = 8; break;
	case TR::Float: helperLookupOffset = 12; break;
	case TR::Double: helperLookupOffset = 16; break;
	}
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARMVirtualUnresolvedSnippet *snippet)
{
	...
	printPrefix(pOutFile, NULL, bufferPos, 4);
	trfprintf(
	        pOutFile, "dd\t" POINTER_PRINTF_FORMAT "\t\t; cpAddress", getOwningMethod(callSymRef)->constantPool());
	bufferPos += 4;
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARMHelperCallSnippet *snippet)
{
	...
	if (restartLabel) {
		...
		trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t; Return to %s",
		        (intptrj_t)(restartLabel->getCodeLocation()), getName(restartLabel));
	}
#endif
}
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARMStackCheckFailureSnippet *snippet)
{
	...
	bool saveLR = (_cg->getSnippetList().size() <= 1 && !bodySymbol->isEHAware()
...
static const char *opCodeToNameMap[] = {
        ...
        "ldfs", "ldfd", "ldm", "ldmia", "ldr", "ldrb", "ldrsb", "ldrh", "ldrsh", "mul", "mul_r", "mla", "mla_r",
        "smull", "smull_r", "umull", "umull_r", "smlal", "smlal_r", "umlal", "umlal_r", "mov", "mov_r", "mvn", "mvn_r",
        "orr", "orr_r", "teq", "tst", "sub", "sub_r", "sbc", "sbc_r", "rsb", "rsb_r", "rsc", "rsc_r", "stfs", "stfd",
        "str", "strb", "strh", "stm", "stmdb", "swp", "sxtb", "sxth", "uxtb", "uxth", "fence", "ret", "wrtbar", "proc",
        "dd", "dmb_v6", "dmb", "dmb_st", "ldrex", "strex", "iflong", "setblong", "setbool", "setbflt", "lcmp", "flcmpl",
        "flcmpg", "idiv", "irem", "label", "vgdnop", "nop",
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
