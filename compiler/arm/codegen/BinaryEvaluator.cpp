...
TR::Register* OMR::ARM::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (/* !isSmall() && */ secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL) {
        ...
        generateTrg1Src2Instruction(
            cg, ARMOp_orr, node, temp6Reg, temp3Reg, temp5Reg); // TODO: push this into above op.
    ...
    TR::Node* srcNode, TR_ARMOpCodes regOp, TR::Register* srcReg, int32_t imm, TR::CodeGenerator* cg)
{
    ...
    if (srcNode->getReferenceCount() <= 1
        && (regOp == ARMOp_and && imm == -1
               || regOp != ARMOp_and && imm == 0)) { // an operand of a long logical operations can have a half word
                                                     // which is an identity value for that operation
        trgReg = srcReg;
