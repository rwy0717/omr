...
OMR::ARM::TreeEvaluator::laddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (/* !isSmall() && */ secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
		...
		generateTrg1Src2Instruction(
		        cg, ARMOp_add_r, node, lowReg, src1Reg->getLowOrder(), src2Reg->getLowOrder());
		generateTrg1Src2Instruction(
		        cg, ARMOp_adc, node, highReg, src1Reg->getHighOrder(), src2Reg->getHighOrder());
	}
	node->setRegister(trgReg);
	firstChild->decReferenceCount();
	secondChild->decReferenceCount();
...
OMR::ARM::TreeEvaluator::lsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (/* !isSmall() && */ secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
		...
		trgReg = addConstantToLong(node, src1Reg->getHighOrder(), src1Reg->getLowOrder(), longVal.x.highValue,
		        longVal.x.lowValue, cg);
	} else {
		...
		TR::Register *src2Reg = cg->evaluate(secondChild);
		generateTrg1Src2Instruction(
		        cg, ARMOp_sub_r, node, lowReg, src1Reg->getLowOrder(), src2Reg->getLowOrder());
		generateTrg1Src2Instruction(
		        cg, ARMOp_sbc, node, highReg, src1Reg->getHighOrder(), src2Reg->getHighOrder());
		trgReg = cg->allocateRegisterPair(lowReg, highReg);
	}
...
OMR::ARM::TreeEvaluator::imulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCodeValue() == TR::iconst) {
		...
		generateTrg2Src2MulInstruction(
		        cg, ARMOp_smull, node, trgReg, tempRegLow, cg->evaluate(secondChild), src1Reg);
	}
        ...
        TR::Node *node, int32_t divisor, TR::Register *dividendReg, bool isDivide, TR::CodeGenerator *cg)
{
	...
	if (isPowerOf2(divisor)) {
		if (!isDivide) {
			...
			uint32_t base, rotate;
			if (constantIsImmed8r(divisor - 1, &base, &rotate)) // if divisor - 1 fits in 8 bit imm field
			                                                    // can do this cheaply
			{
			...
			if (constantIsImmed8r(divisor, &base, &rotate)) // if divisor fits in 8 bit imm field can do
			                                                // this cheaply
			{
...
OMR::ARM::TreeEvaluator::lshlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (/* !isSmall() && */ secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL) {
		...
		generateTrg1Src2Instruction(cg, ARMOp_orr, node, temp6Reg, temp3Reg, temp5Reg); // TODO: push this into
		                                                                                // above op.
	...
	if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL) {
		...
		if (shiftAmount >= 32) {
			if (isLogical) {
				...
				new (cg->trHeapMemory())
				        TR::ARMTrg1Src1Instruction(ARMOp_mov, node, trgReg->getHighOrder(),
			...
			if (shiftAmount == 32) {
				...
				new (cg->trHeapMemory())
				        TR::ARMTrg1Src1Instruction(ARMOp_mov, node, trgReg->getLowOrder(),
				                new (cg->trHeapMemory())
		...
		if (srcHighValue == 0) {
			...
			if (srcLowValue == 0)
				...
				generateShiftRightByRegister(
				        cg, node, trgReg->getLowOrder(), srcLowReg, shiftAmountReg, isLogical);
		} else {
			...
			TR::SymbolReference *longShiftHelper = cg->symRefTab()->findOrCreateRuntimeHelper(
			        isLogical ? TR_ARMlongShiftRightLogical : TR_ARMlongShiftRightArithmetic, false, false,
			        false);

			generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)longShiftHelper->getMethodAddress(),
...
OMR::ARM::TreeEvaluator::irolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
		...
		TR::Register *shiftAmountReg = cg->evaluate(secondChild);
		generateTrg1Src1ImmInstruction(cg, ARMOp_rsb, node, trgReg, shiftAmountReg, 1, 5); // Subtract from 32
		                                                                                   // (1 << 5)
		// Use ROR (Rotate Right) Operand2
		generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg,
...
static TR::Register *
ibooleanTypeImmediateEvaluator(
        TR::Node *srcNode, TR_ARMOpCodes regOp, TR::Register *srcReg, int32_t imm, TR::CodeGenerator *cg)
{
	...
	if (srcNode->getReferenceCount() <= 1
	        && (regOp == ARMOp_and && imm == -1 || regOp != ARMOp_and && imm == 0)) { // an operand of a long
		                                                                          // logical operations can have
		                                                                          // a half word which is an
		                                                                          // identity value for that
		                                                                          // operation
		trgReg = srcReg;
		...
		trgReg = cg->allocateRegister();
		generateTrg1Src1Instruction(cg, ARMOp_mov, srcNode, trgReg,
...
lbooleanTypeEvaluator(TR::Node *node, TR_ARMOpCodes regOp, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Register *src1Reg = cg->evaluate(firstChild);
	TR::Node *secondChild = node->getSecondChild();
	...
	if (canCombine) {
		...
		if (shiftSecondChild->getOpCodeValue() == TR::iconst) {
			shiftOp = new (cg->trHeapMemory())
			        TR_ARMOperand2(shiftTypeImmed, src1Reg, shiftSecondChild->getInt() & 0x1f);
		} else {
...
static inline TR::Register *
ibooleanTypeEvaluator(TR::Node *node, TR_ARMOpCodes regOp, TR::CodeGenerator *cg)
{
	TR::Register *trgReg = NULL;
	TR::Node *secondChild = node->getSecondChild();
	TR::Node *firstChild = node->getFirstChild();
	...
	if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL
		...
		generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg,
		        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, src, 16));
		generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg,
		        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSRImmed, trgReg, 16));
	} else {
		TR::Register *src2Reg = cg->evaluate(secondChild);
