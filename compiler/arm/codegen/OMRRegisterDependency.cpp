        ...
        TR::Node *node, uint32_t extranum, TR::Instruction **cursorPtr, TR::CodeGenerator *cg)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		if (regList.find(reg) || (highReg && regList.find(highReg))) {
			...
			switch (kind) {
			case TR_GPR: opCode = ARMOp_mov; break;
			case TR_FPR: opCode = ARMOp_fmrs; break;
...
void
OMR::ARM::RegisterDependencyConditions::incRegisterTotalUseCounts(TR::CodeGenerator *cg)
{
	for (int i = 0; i < _addCursorForPre; i++) {
...
void
TR_ARMRegisterDependencyGroup::assignRegisters(TR::Instruction *currentInstruction, TR_RegisterKinds kindToBeAssigned,
		...
		for (i = 0; i < numberOfRegisters; i++) {
			...
			if (dependentRegNum == TR::RealRegister::SpilledReg) {
				...
				if (virtReg->getAssignedRealRegister()) {
					...
					// slot now
					traceMsg(comp,
					        "\nOOL: Found register spilled in main line and re-assigned inside "
					        "OOL");
					...
					switch (rk) {
					case TR_GPR: opCode = ARMOp_ldr; break;
					case TR_FPR:
	...
	for (i = 0; i < numberOfRegisters; i++) {
		if (dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
			...
			if (realOne != NULL && excludeGPR0
				...
				if ((assignedRegister = machine->findBestFreeRegister(virtReg->getKind(), true))
				        == NULL) {
				...
				if (virtReg->getTotalUseCount() == virtReg->getFutureUseCount()) {
					if ((assignedRegister = machine->findBestFreeRegister(
					             virtReg->getKind(), excludeGPR0, true))
					        == NULL) {
						assignedRegister = machine->freeBestRegister(
...
OMR::ARM::RegisterDependencyConditions::clone(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added)
{
...
TR::RegisterDependencyConditions *
OMR::ARM::RegisterDependencyConditions::cloneAndFix(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added)
{
	TR::RegisterDependencyConditions *result;
	TR_ARMRegisterDependency *singlePair;
	int32_t idx, preNum, postNum, addPre = 0, addPost = 0;
...
void
OMR::ARM::RegisterDependencyConditions::unionNoRegPostCondition(TR::Register *reg, TR::CodeGenerator *cg)
{
