...
OMR::ARM::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
	...
	case TR_Private:
	case TR_Helper:
	case TR_System: linkage = new (self()->trHeapMemory()) TR::ARMSystemLinkage(self()); break;
	default:
        ...
        , _internalControlFlowSafeNestingDepth(0)
{
	...
	if (TR::Compiler->target.isLinux()) {
		...
		// gr0 and gr1 for double precision.  This is where the jit expects the result following a directToJNI
		// call. The floating point return value on non-hardhat linux-arm builds is through the coprocessor
		// register f0. Therefore the HasHardFloatReturn flag must be set to force generation of instructions to
		// move the result from f0 to gr0/gr1 following a directToJNI dispatch.
	...
	if (!debug("hasFramePointer"))
		self()->setFrameRegister(
		        self()->machine()->getRealRegister(_linkageProperties->getStackPointerRegister()));
	else
		self()->setFrameRegister(
		        self()->machine()->getRealRegister(_linkageProperties->getFramePointerRegister()));
...
directToInterpreterHelper(TR::ResolvedMethodSymbol *methodSymbol, TR::CodeGenerator *cg)
{
	...
	case TR::NoType: return sync ? TR_ARMicallVMprJavaSendStaticSync0 : TR_ARMicallVMprJavaSendStatic0;
	case TR::Int8:
	case TR::Int16:
	case TR::Int32: return sync ? TR_ARMicallVMprJavaSendStaticSync1 : TR_ARMicallVMprJavaSendStatic1;
	case TR::Address:
	...
	case TR::Int64: return sync ? TR_ARMicallVMprJavaSendStaticSyncJ : TR_ARMicallVMprJavaSendStaticJ;
	case TR::Float: return sync ? TR_ARMicallVMprJavaSendStaticSyncF : TR_ARMicallVMprJavaSendStaticF;
	case TR::Double: return sync ? TR_ARMicallVMprJavaSendStaticSyncD : TR_ARMicallVMprJavaSendStaticD;
	default: TR_ASSERT(0, "Unknown return type of a method.\n"); return (TR_RuntimeHelper)0;
	}
}

TR::Instruction *
OMR::ARM::CodeGenerator::generateSwitchToInterpreterPrePrologue(TR::Instruction *cursor, TR::Node *node)
{
	...
	cursor = generateImmSymInstruction(self(), ARMOp_bl, node,
...
static void
removeGhostRegistersFromGCMaps(TR::CodeGenerator *cg, TR::Instruction *branchOOL)
{
	// If a virtual is live at the end of the hot path and dead at the beginning it will not be killed immediatey
	// after it's first use in the hot path if it's also used on the cold path, since RA still needs to be done on
	// the cold path (which is where it's future use count will drop to 0), which means it will be incorrectly seen
	// as live between it's first use and the top of the hot path. If there are any GC points between the top of the
	// hot path and the first use the real reg holding the virtual will be included, so we need to fix this.
	TR::Instruction *instr = branchOOL->getNext();
	while (!instr->isLabel()
	        || !((TR::ARMLabelInstruction *)instr)->getLabelSymbol()->isEndOfColdInstructionStream()) {
		if (instr->needsGCMap()) {
			TR_GCStackMap *map = instr->getGCMap();
			TR_ASSERT(map, "Instruction should have a GC map");

			// This instruction has a GC map, for every register in the register map check if that register
			// is unassigned at the beginning of the hot path.
			for (uint32_t regNum = TR::RealRegister::FirstGPR; regNum < TR::RealRegister::LastGPR;
			        ++regNum) {
				...
				if (map->getRegisterMap() & regMask) {
					...
					if (regInRegMap->getState() == TR::RealRegister::Free) {
						// This register is unassigned, check if it was defined before the GC
						// point.
						...
						// If it wasn't defined before the GC point it died on the cold path and
						// it's first use on the hot path was after the GC point i.e. it
	...
	if (comp->getMethodSymbol()->getLinkageConvention() == TR_Private) {
		...
		if (methodSymbol->isJNI()) {
			uintptrj_t JNIMethodAddress =
			        (uintptrj_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(comp);
		...
		new (self()->trHeapMemory())
		        TR::ARMAdminInstruction((TR::Instruction *)NULL, ARMOp_proc, startNode, NULL, self());
	}
}
...
int32_t
OMR::ARM::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart)
{
...
int32_t
OMR::ARM::CodeGenerator::findOrCreateAddressConstant(void *v, TR::DataType t, TR::Instruction *n0, TR::Instruction *n1,
...
void
OMR::ARM::CodeGenerator::apply24BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
...
void
OMR::ARM::CodeGenerator::apply8BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
	int32_t delta = (intptr_t)label->getCodeLocation() - ((intptr_t)cursor + 8);
...
OMR::ARM::CodeGenerator::findOutLinedInstructionsFromLabel(TR::LabelSymbol *label)
{
...
OMR::ARM::CodeGenerator::pickRegister(TR_RegisterCandidate *regCan, TR::Block **barr, TR_BitVector &availRegs,
...
bool
OMR::ARM::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate *rc, TR::Node *branchNode)
{
	...
	if (node->getOpCode().isSwitch()) {
		...
		if (node->getOpCodeValue() == TR::table) {
			...
			if (total <= 15) {
				...
				for (i = 2; i < total
				        && constantIsImmed8r(node->getChild(i)->getCaseConstant(), &base, &rotate);
				        i++)
					;
				if (i == total)
					return _numGPR - 1;
...
#if 1
TR_GlobalRegisterNumber
OMR::ARM::CodeGenerator::getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex, TR::DataType type)
{
...
bool
OMR::ARM::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i, TR::DataType dt)
{
