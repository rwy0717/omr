...
public:
	...
	friend bool operator==(
	        const TR_TypedPersistentAllocatorBase &left, const TR_TypedPersistentAllocatorBase &right)
	{
		return &left == &right;
	}
	friend bool operator!=(
		...
		for (int i = minbits; i < maxbits; i++) {
			if (statsArray[i - minbits]) {
				...
				o << "bucket[" << i - minbits << "] (" << bucketsize(i)
				  << ") : allocated = " << statsArray[i - minbits] << " bytes, freelist size = " << sz
				  << "\n";
			}
		}
...
public:
	...
	static Metric _currentMemUsage; // current statistics across all alloc types and memory, persistent, stack, heap
	                                // and CS2 allocators
