...
public:
	        ...
	        , _initializedBySubClass(false)
	{
		...
		case tryToAcquireVMAccess: _hasVMAccess = TR::Compiler->vm.tryToAcquireAccess(comp, &_acquiredVMAccess);
		}
	}
		...
		if (!_initializedBySubClass) {
			...
			if (!_vmAccessReleased) {
				if (_comp) {
					...
					switch (_protocol) {
					case acquireVMAccessIfNeeded:
						TR::Compiler->vm.releaseVMAccessIfNeeded(
						        _omrVMThread, _acquiredVMAccess);
						break;
