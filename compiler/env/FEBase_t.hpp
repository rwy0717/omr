...
template <class Derived>
uint8_t* FEBase<Derived>::allocateCodeMemory(
    TR::Compilation* comp, uint32_t warmCodeSize, uint32_t coldCodeSize, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    ...
    uint8_t* warmCode = codeCacheManager().allocateCodeMemory(
        warmCodeSize, coldCodeSize, &codeCache, coldCode, false, isMethodHeaderNeeded);
...
void FEBase<Derived>::switchCodeCache(TR::CodeCache* newCache)
{
    ...
    // If the old CC had pre-loaded code, the current compilation may have initialized it and will therefore depend on
    // it so we should initialize it in the new CC as well
...
template <class Derived>
intptrj_t FEBase<Derived>::indexedTrampolineLookup(int32_t helperIndex, void* callSite)
{
...
template <class Derived>
void FEBase<Derived>::resizeCodeMemory(TR::Compilation* comp, uint8_t* bufferStart, uint32_t numBytes)
{
