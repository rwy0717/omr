...
TR::Node*
TR_VirtualGuard::createRubyInlineGuard(TR_VirtualGuardKind kind, TR::Compilation* comp, int16_t calleeIndex,
    TR::Node* callNode, TR::TreeTop* destination, TR_OpaqueClassBlock* thisClass)
{
    ...
    TR_ASSERT(((callNode->getNumChildren() == 4) && callNode->getSecondChild() && callNode->getSecondChild()->getOpCodeValue() == TR::aconst), "Unexpected children in hierarchy of vm_send_without_block when creating ruby inline guard.");
