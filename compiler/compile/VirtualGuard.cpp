...
TR::Node* TR_VirtualGuard::createRubyInlineGuard(TR_VirtualGuardKind kind, TR::Compilation* comp, int16_t calleeIndex,
    TR::Node* callNode, TR::TreeTop* destination, TR_OpaqueClassBlock* thisClass)
{
    ...
    TR_ASSERT(((callNode->getNumChildren() == 4) && callNode->getSecondChild()
                  && callNode->getSecondChild()->getOpCodeValue() == TR::aconst),
        "Unexpected children in hierarchy of vm_send_without_block when creating ruby inline guard.");
    ...
    TR::SymbolReference* inline_guard_helperSymRef
        = symRefTab->findOrCreateRuntimeHelper(RubyHelper_vm_send_woblock_inlineable_guard, true, true, false);
    ...
    isInlineable->setAndIncChild(0, TR::Node::lconst(callCache, cc->method_state));

    isInlineable->setAndIncChild(1, TR::Node::lconst(callCache, cc->class_serial));

    isInlineable->setAndIncChild(2, receiver);

    TR::Node* guard
        = TR::Node::createif(TR::ificmpne, isInlineable, TR::Node::iconst(callNode, (uintptrj_t)1), destination);

    setGuardKind(guard, kind, comp);

    TR_VirtualGuard* vg = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_RubyInlineTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), thisClass);
    return guard;
#else
    return NULL;
#endif
...
TR::Node* TR_VirtualGuard::createOSRGuard(TR::Compilation* comp, TR::TreeTop* destination)
{
    ...
    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(TR_DummyTest, TR_OSRGuard, comp, NULL, guard,
        destination ? destination->getNode()->getByteCodeInfo().getCallerIndex() : -1,
