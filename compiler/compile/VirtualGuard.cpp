...
TR_VirtualGuard::TR_VirtualGuard(TR_VirtualGuardTestType test, TR_VirtualGuardKind kind, TR::Compilation *comp,
        ...
        , _kind(kind)
        , _guardedMethod(
                  (callNode && callNode->getOpCode().hasSymbolReference()) ? callNode->getSymbolReference() : NULL)
        ...
        , _currentInlinedSiteIndex(currentSiteIndex)
{
	...
	if (comp->getOption(TR_TraceRelocatableDataDetailsCG))
		traceMsg(comp,
		        "addVirtualGuard %p, guardkind = %d, virtualGuardTestType %d, bc index %d, callee index %d, "
		        "callNode %p, guardNode %p, currentInlinedSiteIdx %d\n",
        ...
        TR::Node *callNode, TR::TreeTop *destination, TR_OpaqueClassBlock *thisClass)
{
	...
	TR::Node *vft = TR::Node::createWithSymRef(
	        TR::aloadi, 1, 1, callNode->getSecondChild(), symRefTab->findOrCreateVftSymbolRef());
 ...
 */
TR::Node *
TR_VirtualGuard::createBreakpointGuardNode(TR::Compilation *comp, int16_t calleeIndex, TR::Node *callNode,
        TR::TreeTop *destination, TR::ResolvedMethodSymbol *calleeSymbol)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		guard = TR::Node::createif(
		        TR::iflcmpne, TR::Node::create(TR::land, 2, constantPool, flagBit), zero, destination);
	} else {
		...
		guard = TR::Node::createif(
		        TR::ificmpne, TR::Node::create(TR::iand, 2, constantPool, flagBit), zero, destination);
	}
 ...
 */
TR::Node *
TR_VirtualGuard::createBreakpointGuard(TR::Compilation *comp, int16_t calleeIndex, TR::Node *callNode,
        TR::TreeTop *destination, TR::ResolvedMethodSymbol *calleeSymbol)
{
        ...
        TR_OpaqueClassBlock *thisClass, TR::Node *receiverNode)
{
	...
	TR::SymbolReferenceTable *symRefTab = comp->getSymRefTab();
	TR::Node *vft =
	        TR::Node::createWithSymRef(TR::aloadi, 1, 1, receiverNode, symRefTab->findOrCreateVftSymbolRef());
...
TR::Node *
TR_VirtualGuard::createMutableCallSiteTargetGuard(TR::Compilation *comp, int16_t calleeIndex, TR::Node *node,
        TR::TreeTop *destination, uintptrj_t *mcsObject, TR::KnownObjectTable::Index mcsEpoch)
{
	...
	TR::Node *guard = TR::Node::createif(TR::ifacmpne, node, load, destination); // NOTE: take bytecode info from
	                                                                             // node so findVirtualGuardInfo
	                                                                             // works
...
TR::Node *
TR_VirtualGuard::createAOTInliningGuard(TR::Compilation *comp, int16_t calleeIndex, TR::Node *callNode,
...
TR::Node *
TR_VirtualGuard::createAOTGuard(TR::Compilation *comp, int16_t siteIndex, TR::Node *callNode, TR::TreeTop *destination,
        TR_VirtualGuardKind kind)
{
	TR::Node *guard = createDummyOrSideEffectGuard(comp, callNode, destination);
	setGuardKind(guard, kind, comp);
	TR_VirtualGuard *vguard = new (comp->trHeapMemory())
	        TR_VirtualGuard(TR_NonoverriddenTest, kind, comp, callNode, guard, siteIndex);
	vguard->dontGenerateChildrenCode();
	vguard->setCannotBeRemoved();
	return guard;
}
...
TR::Node *
TR_VirtualGuard::createDummyGuard(
...
TR_VirtualGuard::createOSRGuard(TR::Compilation *comp, TR::TreeTop *destination)
{
	...
	TR::Node *flag = destination ? TR::Node::create(destination->getNode(), TR::iconst, 0, 0)
	                             : TR::Node::create(TR::iconst, 0, 0);

	TR::Node *guard = TR::Node::createif(TR::ificmpne, load, flag, destination);
	setGuardKind(guard, TR_OSRGuard, comp);

	TR_VirtualGuard *vguard = new (comp->trHeapMemory()) TR_VirtualGuard(TR_DummyTest, TR_OSRGuard, comp, NULL,
	        guard, destination ? destination->getNode()->getByteCodeInfo().getCallerIndex() : -1,
...
TR::Node *
TR_VirtualGuard::createHCRGuard(TR::Compilation *comp, int16_t calleeIndex, TR::Node *node, TR::TreeTop *destination,
        TR::ResolvedMethodSymbol *symbol, TR_OpaqueClassBlock *thisClass)
{
	...
	TR_VirtualGuard *vguard = new (comp->trHeapMemory()) TR_VirtualGuard(TR_NonoverriddenTest, TR_HCRGuard, comp,
	        node, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), thisClass);
	vguard->dontGenerateChildrenCode();
...
{
	switch (kind) {
	case TR_ProfiledGuard: node->setIsProfiledGuard(); break;
	case TR_InterfaceGuard: node->setIsInterfaceGuard(); break;
	case TR_AbstractGuard: node->setIsAbstractGuard(); break;
	case TR_HierarchyGuard: node->setIsHierarchyGuard(); break;
	case TR_DummyGuard: node->setIsDummyGuard(); break;
	case TR_HCRGuard: node->setIsHCRGuard(); break;
	case TR_SideEffectGuard: node->setIsSideEffectGuard(); break;
	case TR_MutableCallSiteTargetGuard: node->setIsMutableCallSiteTargetGuard(); break;
	case TR_MethodEnterExitGuard: node->setIsMethodEnterExitGuard(); break;
	case TR_DirectMethodGuard: node->setIsDirectMethodGuard(); break;
	case TR_OSRGuard: node->setIsOSRGuard(); break;
	case TR_BreakpointGuard: node->setIsBreakpointGuard(); break;
	default:
		TR_ASSERT(kind == TR_NonoverriddenGuard, "Expected TR_NonoverriddenGuard(%d); found %d",
