...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread *omrVMThread, TR_FrontEnd *fe, TR_ResolvedMethod *compilee,
        ...
        , _phaseMemProfiler(
                  "Compilation", self()->allocator("phaseMemProfiler"), self()->getOption(TR_LexicalMemProfiler))
        ...
        , _tlsManager(*self())
{
	...
	{
		_methodSymbol =
		        TR::ResolvedMethodSymbol::createJittedMethodSymbol(self()->trHeapMemory(), compilee, self());
	}
	...
	if (firstTime) {
		...
		TR_ASSERT(TR::DataType::getMaxPackedDecimalSize()
		        ...
		        "TR::DataType::getMaxPackedDecimalSize() (%d) does not agree with "
		        "precToSize(TR_MAX_DECIMAL_PRECISION) (%d)\n",
	...
	if (options.getOption(TR_EnableOSR)) {
		...
		if (((self()->getMethodHotness() < warm) || self()->compileRelocatableCode()
		            || self()->isProfilingCompilation())
		        && !enableOSRAtAllOptLevels && !options.getOption(TR_FullSpeedDebug)) // Off for two reasons :
		                                                                              // 1) not sure if we can
		                                                                              // afford the increase in
		                                                                              // compile time due to the
		                                                                              // extra OSR control flow
		                                                                              // at cold and 2) not sure
		                                                                              // at this stage in 727
		                                                                              // whether OSR can work
		                                                                              // with AOT (will try to
		                                                                              // find out soon) but
		                                                                              // disabling till I do
		                                                                              // find out
...
OMR::Compilation::supportsInduceOSR()
{
	...
	if (_osrCompilationData && _osrCompilationData->seenClassPreventingInducedOSR()) {
		if (self()->getOption(TR_TraceOSR))
			traceMsg(self(),
			        "Cannot guarantee OSR transfer of control to the interpreter will work for calls "
			        "preventing induced OSR (e.g. Quad) because of differences in JIT vs interpreter "
			        "representations\n");
		return false;
	}
...
bool
OMR::Compilation::isPotentialOSRPoint(TR::Node *node, TR::Node **osrPointNode, bool ignoreInfra)
{
	...
	if (self()->isOSRTransitionTarget(TR::postExecutionOSR)) {
		...
		if (_osrInfrastructureRemoved && !ignoreInfra)
			...
			if (disableAsyncCheckOSR == NULL)
				potentialOSRPoint =
				        !self()->isShortRunningMethod(node->getByteCodeInfo().getCallerIndex());
			...
			} else if (callSymRef->getReferenceNumber() >= self()->getSymRefTab()->getNonhelperIndex(
...
OMR::Compilation::requiresAnalysisOSRPoint(TR::Node *node)
{
	...
	case TR::asynccheck: return false;
	default: TR_ASSERT(0, "OSR points should only be calls, monents or asyncchecks"); return false;
	}
}
...
OMR::Compilation::compile()
{
	...
	if (self()->getOutFile() != NULL
	        && (self()->getOption(TR_TraceAll) || debug("traceStartCompile")
	                || self()->getOptions()->getAnyTraceCGOption() || self()->getOption(TR_Timing))) {
...

	{
		...
		if (_ilGenSuccess) {
			...
			if (_methodSymbol->catchBlocksHaveRealPredecessors(_methodSymbol->getFlowGraph(), self())) {
				self()->failCompilation<TR::CompilationException>("Catch blocks have real "
				                                                  "predecessors");
			}
			...
			if (_ilVerifier && _ilVerifier->verify(_methodSymbol)) {
				self()->failCompilation<TR::CompilationException>("Aborting after Optimization due to "
				                                                  "verifier failure");
			}

			static char *abortafterilgen = feGetEnv("TR_TOSS_IL");
			if (abortafterilgen) {
				self()->failCompilation<TR::CompilationException>("Aborting after IL Gen due to "
				                                                  "TR_TOSS_IL");
			}
			...
			if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
				...
				for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
					...
					int32_t calleeLen = self()->fej9()->printTruncatedSignature(calleeBuf,
					        sizeof(calleeBuf) - 1, self()->fej9()->getInlinedCallSiteMethod(&site));
					...
					if (callerIndex != -1) {
						int32_t callerLen = self()->fej9()->printTruncatedSignature(callerBuf,
						        sizeof(callerBuf) - 1,
					...
					TR_VerboseLog::writeLine(TR_Vlog_INL,
					        "#%d: %x #%d inlined %x@%d -> %x bcsz=%d %s", i, jittedBodyHash,
					        callerIndex, strHash(callerSig), site._byteCodeInfo.getByteCodeIndex(),
					        strHash(calleeBuf), TR::Compiler->mtd.bytecodeSize(site._methodInfo),
					        calleeBuf);
				}
							...
							if (method) {
								...
								if (callerIndex != -1) {
									int32_t callerLen = self()->fej9()->printTruncatedSignature(
									        ...
									        self()->fej9()->getInlinedCallSiteMethod(
									                &self()->getInlinedCallSite(
									                        callerIndex)));
									callerBuf[callerLen] = 0; // null terminate
									callerSig = callerBuf;
								}
								const char *calleeSig = method->signature(
								...
								TR_VerboseLog::writeLine(TR_Vlog_INL,
								        "%x %s %x@%d -> %x bcsz=%d %s", jittedBodyHash,
...
bool
OMR::Compilation::incInlineDepth(TR::ResolvedMethodSymbol *method, TR_ByteCodeInfo &bcInfo, int32_t cpIndex,
        TR::SymbolReference *callSymRef, bool directCall, TR_PrexArgInfo *argInfo)
{
	if (self()->compileRelocatableCode()) {
		...
		return self()->incInlineDepth(method->getResolvedMethod()->getPersistentIdentifier(), method, bcInfo,
...
bool
OMR::Compilation::foundOnTheStack(TR_ResolvedMethod *method, int32_t occurrences)
{
...
bool
OMR::Compilation::incInlineDepth(TR_OpaqueMethodBlock *methodInfo, TR::ResolvedMethodSymbol *method,
...
int16_t
OMR::Compilation::restoreInlineDepthUntil(int32_t stopIndex, TR_ByteCodeInfo &currentInfo)
{
	...
	_inlinedCallArgInfoStack.push(NULL); // We have no argInfo for previously-inlined frames, though there's not
	                                     // much we could do with it anyway
...
TR_DevirtualizedCallInfo *
OMR::Compilation::createDevirtualizedCall(TR::Node *callNode, TR_OpaqueClassBlock *thisType)
{
...
TR_DevirtualizedCallInfo *
OMR::Compilation::findOrCreateDevirtualizedCall(TR::Node *callNode, TR_OpaqueClassBlock *thisType)
{
...
{
	TR_ASSERT(guardNode->isTheVirtualGuardForAGuardedInlinedCall() || guardNode->isOSRGuard()
	                || guardNode->isHCRGuard() || guardNode->isProfiledGuard()
	                || guardNode->isMethodEnterExitGuard() || guardNode->isBreakpointGuard(),
	        "@node %p\n", guardNode);
	...
	if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
		traceMsg(self(), "Looking for a guard for node %p with kind %d bcindex %d calleeindex %d\n", guardNode,
		        guardKind, guardNode->getByteCodeInfo().getByteCodeIndex(),
		...
		if (((*current)->getCalleeIndex() == guard->getCalleeIndex())
			...
			if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
				traceMsg(self(), "removeVirtualGuard %p, kind %d bcindex %d calleeindex %d\n", *current,
				        (*current)->getKind(), (*current)->getByteCodeIndex(),
...
TR::Node *
OMR::Compilation::createAOTInliningGuard(
...
TR::Node *
OMR::Compilation::createAOTGuard(
        TR::Compilation *comp, int16_t calleeIndex, TR::Node *node, TR::TreeTop *destination, TR_VirtualGuardKind kind)
{
...
void
OMR::Compilation::resetVisitCounts(vcount_t count, TR::ResolvedMethodSymbol *method)
{
...
void
OMR::Compilation::AddCopyPropagationRematerializationCandidate(TR::SymbolReference *sr)
{
	_copyPropagationRematerializationCandidates[sr->getReferenceNumber()] = 1;
}

void
OMR::Compilation::RemoveCopyPropagationRematerializationCandidate(TR::SymbolReference *sr)
{
	_copyPropagationRematerializationCandidates[sr->getReferenceNumber()] = 0;
}

bool
OMR::Compilation::IsCopyPropagationRematerializationCandidate(TR::SymbolReference *sr)
{
...
//
void
OMR::Compilation::dumpMethodTrees(char *title, TR::ResolvedMethodSymbol *methodSymbol)
{
	if (self()->getOutFile() == NULL)
		return;
...
void
OMR::Compilation::dumpMethodTrees(char *title1, const char *title2, TR::ResolvedMethodSymbol *methodSymbol)
{
...
{
	TR_ASSERT_FATAL(
	        _ilValidator != NULL, "Attempting to validate the IL without the ILValidator being initialized");
...
#ifdef DEBUG
void
OMR::Compilation::dumpMethodGraph(int index, TR::ResolvedMethodSymbol *methodSymbol)
{
	if (self()->getOutFile() == NULL)
		return;
...
OMR::Compilation::shutdown(TR_FrontEnd *fe)
{
	...
	bool printCummStats = ((fe != 0) && TR::Options::getCmdLineOptions()
...
void
OMR::Compilation::registerResolvedMethodSymbolReference(TR::SymbolReference *symRef)
{
	_resolvedMethodSymbolReferences
	        [((TR::ResolvedMethodSymbol *)symRef->getSymbol())->getResolvedMethodIndex().value()] = symRef;
}
 ...
 */
void
OMR::Compilation::getOSRCallSiteRemat(
 ...
 */
void
OMR::Compilation::setOSRCallSiteRemat(
...
OMR::Compilation::isPICSite(TR::Instruction *instruction)
{
	return (std::find(self()->getStaticPICSites()->begin(), self()->getStaticPICSites()->end(), instruction)
	        ...
	        || (std::find(self()->getStaticMethodPICSites()->begin(), self()->getStaticMethodPICSites()->end(),
	                    instruction)
	                != self()->getStaticMethodPICSites()->end())
	        || (std::find(
	                    self()->getStaticHCRPICSites()->begin(), self()->getStaticHCRPICSites()->end(), instruction)
...
bool
OMR::Compilation::isRecursiveMethodTarget(TR_ResolvedMethod *targetResolvedMethod)
{
	return targetResolvedMethod && targetResolvedMethod->isSameMethod(self()->getCurrentMethod())
	        && !self()->isDLT();
}
...
OMR::Compilation::isRecursiveMethodTarget(TR::Symbol *targetSymbol)
{
	...
	if (targetSymbol) {
		...
		TR::ResolvedMethodSymbol *resolvedSymbol = methodSymbol ? methodSymbol->getResolvedMethodSymbol()
		                                                        : NULL;
