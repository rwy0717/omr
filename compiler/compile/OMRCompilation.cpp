...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread* omrVMThread, TR_FrontEnd* fe, TR_ResolvedMethod* compilee,
    ...
    , _snippetsToBePatchedOnRegisterNative(
          getTypedAllocator<TR_Pair<TR::Snippet, TR_ResolvedMethod>*>(self()->allocator()))
...
void OMR::Compilation::addVirtualGuard(TR_VirtualGuard* guard) { _virtualGuards.push_front(guard); }
...
TR::CodeCache* OMR::Compilation::getCurrentCodeCache() { return _codeGenerator ? _codeGenerator->getCodeCache() : 0; }
...
void OMR::Compilation::switchCodeCache(TR::CodeCache* newCodeCache)
{
    ...
    self()->setCurrentCodeCache(
        newCodeCache); // Even if we signal, we need to update the reserved code cache for recompilations.
