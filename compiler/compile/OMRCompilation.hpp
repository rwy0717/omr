...
public:
	...
	Compilation(int32_t compThreadId, OMR_VMThread *omrVMThread, TR_FrontEnd *, TR_ResolvedMethod *,
	        TR::IlGenRequest &, TR::Options &, TR::Region &heapMemoryRegion, TR_Memory *,
	...
	bool incInlineDepth(TR::ResolvedMethodSymbol *, TR_ByteCodeInfo &, int32_t cpIndex,
...
private:
	...
	TR_RandomGenerator *_adhocRandom; // Used by callers who can't be bothered to maintain their own
	                                  // TR_RandomGenerator
...
private:
	...
	ncount_t _lastValidNodeCount; // set to _nodeCount when _accurateNumberOfNodes is set. Used to tell how many
	                              // nodes have been created since
...
protected:
	...
	OMR::RuntimeAssumption *_metadataAssumptionList; // A special OMR::RuntimeAssumption to play the role of a
	                                                 // sentinel for a linked list
#endif

private:
	void *_relocatableMethodCodeStart;
	const int32_t _compThreadID; // The ID of the supporting compilation thread; 0 for compilation an application
	                             // thread
	...
	// Length of the last node OpCode that was printed. This should get reset to 0 at the beginning of a line where
	// it is needed, e.g. for printing optimization trees it gets set to 0 in
	// TR_Debug::printBasicPreNodeInfoAndIndent() which preceeds node opCode printouts. Any methods that print parts
	// of node opCodes should always increment this as there may be information following opCodes that need to be
	// aligned as in the case of trees or in future development.
	...
	typedef std::map<TR_OpaqueClassBlock *, const TR::TypeLayout *, LayoutComparator, LayoutAllocator>
	        TypeLayoutMap;
