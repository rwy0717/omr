...
public:
	...
	TR_VirtualGuard(TR_VirtualGuardTestType test, TR_VirtualGuardKind kind, TR::Compilation *comp,
	...
	TR_VirtualGuard(TR_VirtualGuardTestType test, TR_VirtualGuardKind kind, TR::Compilation *comp,
	        TR::Node *callNode, TR::Node *guardNode = NULL, int32_t currentSiteIndex = -2);
	...
	static TR::Node *createMethodGuardWithReceiver(TR_VirtualGuardKind, TR::Compilation *, int16_t calleeIndex,
	        TR::Node *, TR::TreeTop *, TR::ResolvedMethodSymbol *, TR_OpaqueClassBlock *thisClass,
	        TR::Node *receiverNode);

	static TR::Node *createNonoverriddenGuard(TR_VirtualGuardKind, TR::Compilation *, int16_t calleeIndex,
	        TR::Node *, TR::TreeTop *, TR::ResolvedMethodSymbol *, bool forInling = true);
	...
	static TR::Node *createAOTInliningGuard(
	        TR::Compilation *, int16_t, TR::Node *, TR::TreeTop *, TR_VirtualGuardKind);
	...
	{
		switch (_kind) {
		case TR_ProfiledGuard: return false;
		default: return true;
		}
	}
	...
	bool mustBeNoped()
	{
		...
		case TR_InterfaceGuard: return true;
	...
	bool canBeRemoved(bool ignoreMerged = false)
	{
		return ignoreMerged ? !_cannotBeRemoved
		                    : !(_cannotBeRemoved || _mergedWithHCRGuard || _mergedWithOSRGuard);
	}
