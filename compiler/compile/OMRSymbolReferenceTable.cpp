...
TR::SymbolReference* OMR::SymbolReferenceTable::createKnownStaticRefereneceSymbolRef(
    ...
    char* className, char* methodName, char* methodSignature, TR::MethodSymbol::Kinds kind, int32_t cpIndex)
{
    ...
    TR_OpaqueMethodBlock* method = fe()->getMethodFromName(
        className, methodName, methodSignature, comp()->getCurrentMethod()->getNonPersistentIdentifier());
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateAsyncCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
    ...
    return self()->findOrCreateRubyHelperSymbolRef(RubyHelper_rb_threadptr_execute_interrupts, true, /*canGCandReturn*/
        true, /*canGCandExcept*/
...
TR::SymbolReference* OMR::SymbolReferenceTable::findDLPStaticSymbolReference(TR::SymbolReference* staticSymbolReference)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateDLPStaticSymbolReference(
    ...
    if (!staticSymbolReference->isUnresolved()) {
        for (symRef = i.getFirst(); symRef; symRef = i.getNext())
            if (symRef->getSymbol()->getStaticSymbol()->getStaticAddress()
                == staticSymbolReference->getSymbol()->getStaticSymbol()->getStaticAddress())
                return symRef;
    }
...
TR::SymbolReference* OMR::SymbolReferenceTable::createCoDependentTemporary(TR::ResolvedMethodSymbol* owningMethodSymbol,
    TR::DataType type, bool isInternalPointer, size_t size, TR::Symbol* coDependent, int32_t offset)
{
    TR::SymbolReference* tempSymRef
        = findOrCreateAutoSymbol(owningMethodSymbol, offset, type, true, isInternalPointer, false, false, size);
    return tempSymRef;
}
