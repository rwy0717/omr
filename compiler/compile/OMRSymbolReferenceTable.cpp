...
OMR::SymbolReferenceTable::findOrCreateExcpSymbolRef()
{
	if (!element(excpSymbol)) {
		...
		//
		aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(excpSymbol)); // add the symRef to the statics
		                                                                        // list to get correct aliasing
		                                                                        // info
	}
...
OMR::SymbolReferenceTable::findOrCreateOSRReturnAddressSymbolRef()
{
	if (!element(osrReturnAddressSymbol)) {
		TR::Symbol *sym =
		        TR::RegisterMappedSymbol::createMethodMetaDataSymbol(trHeapMemory(), "osrReturnAddress");
		sym->setDataType(TR::Address);
		sym->setNotCollected();
		...
		// We can't let the load/store of the exception symbol swing down
		aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(osrReturnAddressSymbol)); // add the symRef to
		                                                                                    // the statics list
		                                                                                    // to get correct
		                                                                                    // aliasing info
	}
...
OMR::SymbolReferenceTable::findOrCreateImmutableArrayShadowSymbolRef(TR::DataType elementType)
{
	...
	aliasBuilder.arrayElementSymRefs().set(index); // this ensures the newly created shadow can still be aliased to
...
OMR::SymbolReferenceTable::createIsOverriddenSymbolRef(TR::ResolvedMethodSymbol *calleeSymbol)
{
	...
	aliasBuilder.addressStaticSymRefs().set(symRef->getReferenceNumber()); // add the symRef to the statics list to
        ...
        TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters)
{
	...
	TR::SymbolReference *sr = baseArray.element(index) = new (trHeapMemory())
	        TR::SymbolReference(self(), index, methodSymbol);
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateRuntimeHelper(
        TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters)
{
...
TR::SymbolReference *
OMR::SymbolReferenceTable::methodSymRefFromName(TR::ResolvedMethodSymbol *owningMethodSymbol, char *className,
        char *methodName, char *methodSignature, TR::MethodSymbol::Kinds kind, int32_t cpIndex)
{
	...
	if (_methodsBySignature.Locate(key, hashIndex) && !ignoreMBSCache) {
		...
		//
		key = OwningMethodAndString(
		        owningMethodSymbol->getResolvedMethodIndex(), self()->strdup(fullSignature));
		if (comp()->getOption(TR_TraceMethodIndex))
	...
	TR_ASSERT_FATAL(
	        method, "methodSymRefFromName: method must exist: %s.%s%s", className, methodName, methodSignature);
	TR_ASSERT(kind != TR::MethodSymbol::Virtual, "methodSymRefFromName doesn't support virtual methods"); // Until
	                                                                                                      // we're
	                                                                                                      // able to
	                                                                                                      // look up
	                                                                                                      // vtable
	...
	TR::SymbolReference *result = findOrCreateMethodSymbol(owningMethodSymbol->getResolvedMethodIndex(), -1,
	        comp()->fe()->createResolvedMethod(comp()->trMemory(), method, owningMethodSymbol->getResolvedMethod()),
	        kind);
...
OMR::SymbolReferenceTable::findSymRefWithKnownObject(TR::Symbol *symbol, TR::KnownObjectTable::Index knownObjectIndex)
{
	...
	while (bvi.hasMoreElements()) {
		...
		if (symbol && symRef->getSymbol() == symbol)
			return symRef;
		if (!symbol && symRef->getSymbol()->isAuto()) // For temp, symbol can be NULL. Find any temp with the
		                                              // given knownObjectIndex.
			return symRef;
	}
	return NULL;
}
        ...
        TR::SymbolReference *originalSymRef, TR::KnownObjectTable::Index knownObjectIndex)
{
	...
	// If the known object symref is created for an immutable array shadow, it should be aliased to other array
	// shadows as well Put the symRef in arrayElementSymRefs so that its alias set can be built correctly
...
bool
OMR::SymbolReferenceTable::shouldMarkBlockAsCold(TR_ResolvedMethod *owningMethod, bool isUnresolvedInCP)
{
	return false;
}
...
{
	char *result =
	        (char *)trMemory()->allocateMemory(1 + strlen(arg), heapAlloc, TR_MemoryBase::SymbolReferenceTable);
	strcpy(result, arg);
	return result;
}
        ...
        TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex, void *classObject, bool cpIndexOfStatic)
{
	...
	// In the latter case a load is created for the class the static is defined in to be used by the write barrier
	// code. For explicit references to classes the cpIndex passed into this routine refers to a class.  For the
	// implicit reference resulting from a store to a static the cpIndex passed into this routine is for the static.
	//
	...
	//
	// findOrCreateCPSymbol will reuse an existing resolved symbol reference if a previously created symbol
	// references contains the same resolved address.  As a result we can end up sharing symbol references between
	// an explicit reference to the class and an implicit reference used on a write barrier node.   In such a case
	// we make sure that the shared symref uses the cpindex for the explicit reference and has the
	// 'addressIsCPIndexOfStatic' set to false.
	//
	TR::SymbolReference *symRef =
	...
	if (cpIndex == -1 && comp()->compileRelocatableCode() && !comp()->getOption(TR_UseSymbolValidationManager)) {
		...
		TR_ASSERT_FATAL(
		        loader == bootstrapLoader, "class symref cpIndex=-1 in AOT not loaded by bootstrap loader\n");
	}
#endif
	...
	if (cpIndexOfStatic) {
		...
		TR_ASSERT(symRef->getCPIndex() != cpIndex,
		        "how can the cpIndices be the same if one symbol represents the class of a static and the "
		        "other doesnt");
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateCPSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex,
        TR::DataType dataType, bool resolved, void *dataAddress, TR::KnownObjectTable::Index knownObjectIndex)
{
	TR::StaticSymbol *sym;
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateMethodSymbol(mcount_t owningMethodIndex, int32_t cpIndex,
        TR_ResolvedMethod *resolvedMethod, TR::MethodSymbol::Kinds callKind, bool isUnresolvedInCP)
{
	TR::SymbolReference *symRef;
	// We check comp()->getHasMethodHandleInvoke() here only because owningMethodDoesntMatter() could be expensive,
	// and currently only JSR292 uses this facility
	if (comp()->getHasMethodHandleInvoke() && resolvedMethod
	        && resolvedMethod->owningMethodDoesntMatter()) // Any
	                                                       // symbol
	                                                       // with
	                                                       // the
	                                                       // same
		...
		for (symRef = i.getNext(); symRef; symRef = i.getNext()) {
			...
			if (existingMethod && (existingMethod->getMethodKind() == callKind)
				...
				if (performTransformation(comp(), "O^O SYMBOL SHARING: Reusing #%d M%p for M%p\n",
				            symRef->getReferenceNumber(), existingMethod->getResolvedMethod(),
				            resolvedMethod)) {
	...
	} else if (cpIndex != -1) // -1 is a indication that the symbol reference shouldn't be shared
	{
		TR_SymRefIterator i(aliasBuilder.methodSymRefs(), self());
		for (symRef = i.getNext(); symRef; symRef = i.getNext()) {
			if (cpIndex == symRef->getCPIndex() && owningMethodIndex == symRef->getOwningMethodIndex()
			        && cpIndex != -1 && symRef->getSymbol()->getMethodSymbol()->getMethodKind() == callKind
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateAutoSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot,
        TR::DataType type, bool isReference, bool isInternalPointer, bool reuseAuto, bool isAdjunct, size_t size)
{
...
TR::SymbolReference *
OMR::SymbolReferenceTable::findOrCreateAutoSymbolImpl(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot,
        TR::DataType type, bool isReference, bool isInternalPointer, bool reuseAuto, bool isAdjunct, size_t size,
        TR::KnownObjectTable::Index knownObjectIndex)
{
	...
	for (; symRef; symRef = i.getNext())
		...
		// dual symref match, when original type is an address, and needs to be coerced
		{
			...
			if (symRef->getSymbol()->getParmSymbol()
				...
				comp()->failCompilation<TR::CompilationException>("stack mapping can't handle a "
				                                                  "parameter slot that is shared "
				                                                  "between refs and nonrefs 0");
			}

			slotSharedbyRefAndNonRef = true;
			symRef->getSymbol()->setSlotSharedByRefAndNonRef(true); // An address slot is being shared with
			                                                        // an int slot.
		...
		for (; symRef2; symRef2 = i2.getNext()) {
			if (comp()->getOption(TR_MimicInterpreterFrameShape)
				...
				if (symRef2->getSymbol()->getParmSymbol()
				        || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
					comp()->failCompilation<TR::CompilationException>("stack mapping can't handle "
					                                                  "a parameter slot that is "
					                                                  "shared between refs and "
					                                                  "nonrefs 1");
				}
				slotSharedbyRefAndNonRef = true;
				symRef2->getSymbol()->setSlotSharedByRefAndNonRef(true); // An address slot is being
				                                                         // shared with an int slot.
				comp()->setSlotsSharedByRefAndNonRef(true);
			}
		}
	}
	...
	if (!symRef) {
		...
		if (isInternalPointer) {
			sym = size
			        ? TR::AutomaticSymbol::createInternalPointer(trHeapMemory(), type, size, comp()->fe())
			...
			if (_numInternalPointers > comp()->maxInternalPointers()) {
				comp()->failCompilation<TR::ExcessiveComplexity>("Excessive number of internal "
				                                                 "pointers");
			}
		} else {
...
TR::SymbolReference *
findOrFabricateShadowSymbol(TR_OpaqueClassBlock *containingClass, TR::DataType type, uint32_t offset, bool isVolatile,
        bool isPrivate, bool isFinal, bool suppressGenericIntShadowAliasing, const char *name, const char *signature)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
	...
	if (!(comp()->cg()->getSupportsCompactedLocals() && comp()->getMethodHotness() >= hot)) {
		...
		for (prev = 0, a = autos.getFirst(); a; prev = autos.getCurrentElement(), a = autos.getNext())
			if (type == a->getSymbol()->getDataType() && !notSharing
			        && !a->getSymbol()->holdsMonitoredObject() && !a->hasKnownObjectIndex()
...
TR::ParameterSymbol *
OMR::SymbolReferenceTable::createParameterSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot,
        TR::DataType type, TR::KnownObjectTable::Index knownObjectIndex)
{
	...
	if (knownObjectIndex == TR::KnownObjectTable::UNKNOWN)
		symRef = new (trHeapMemory())
		        TR::SymbolReference(self(), sym, owningMethodSymbol->getResolvedMethodIndex(), slot);
	else
        ...
        TR::ResolvedMethodSymbol *owningMethodSymbol, TR::KnownObjectTable::Index knownObjectIndex)
{
	...
	return findOrCreateAutoSymbolImpl(owningMethodSymbol, owningMethodSymbol->incTempIndex(fe()), TR::Address,
	        true /* isReference */, false /* isInternalPointer*/, false /* reuseAuto */, false /* isAdjunct */,
	        0 /*size*/, knownObjectIndex);
}
...
bool
OMR::SymbolReferenceTable::isNonHelper(TR::SymbolReference *symRef, CommonNonhelperSymbol s)
{
...
void
OMR::SymbolReferenceTable::makeSharedAliases(TR::SymbolReference *sr1, TR::SymbolReference *sr2)
{
