    ...
    TR::DataType type, bool behavesLikeTemp, bool isAdjunct)
{
    ...
    if (!(comp()->cg()->getSupportsCompactedLocals() && comp()->getMethodHotness() >= hot)) {
        ...
        for (prev = 0, a = autos.getFirst(); a; prev = autos.getCurrentElement(), a = autos.getNext())
            if (type == a->getSymbol()->getDataType() && !notSharing && !a->getSymbol()->holdsMonitoredObject() && (a->isAdjunct() == isAdjunct) && (comp()->cg()->getSupportsJavaFloatSemantics() || (type != TR::Float && type != TR::Double) || (a->isTemporary(comp()) && behavesLikeTemp == !a->getSymbol()->behaveLikeNonTemp()))) {
