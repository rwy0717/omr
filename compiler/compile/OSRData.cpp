...
int32_t TR_OSRMethodData::getHeaderSize() const { return TR::Compiler->vm.OSRFrameHeaderSizeInBytes(comp()); }
    ...
    for (int i = 0; i < slotInfos.size(); i++) {
        ...
        if (info.symRefNum != -1) {
            if ((info.slot == slot) && (info.symRefNum != symRefNum)
                ...
                // Two different variables sharing the same slot, can be live at a certain node (when there is a loop).
                // So commenting the assertion out for now. TR_ASSERTC(0, comp, "ERROR: For slot %d symref #%d conflicts
                // with symref#%d\n",
