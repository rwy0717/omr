...
*/
void
TR_OSRCompilationData::addSlotSharingInfo(const TR_ByteCodeInfo &bcinfo, int32_t slot, int32_t symRefNum,
...
TR_OSRCompilationData::addInstruction(int32_t instructionPC, TR_ByteCodeInfo bcInfo)
{
	...
	while (1) {
		...
		if (trace)
			traceMsg(
			        comp, "  callerIndex %d bcidx %d ", bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
	}
}
...
void
TR_OSRCompilationData::addInstruction2SharedSlotMapEntry(int32_t instructionPC, const TR_ScratchBufferInfos &infos)
{
...
TR_OSRCompilationData::checkOSRLimits()
{
...

	{
		...
		for (int i = 0; i < numOfInlinedCalls; ++i) {
			...
			frameSizes[i] += (bcInfo.getCallerIndex() == -1) ? rootFrameSize
			                                                 : frameSizes[bcInfo.getCallerIndex()];
			stackFrameSizes[i] = getOSRStackFrameSize(i + 1);
			stackFrameSizes[i] = (bcInfo.getCallerIndex() == -1) ? rootStackFrameSize
	...
	if (!TR::Compiler->vm.ensureOSRBufferSize(comp, maxFrameSize, getMaxScratchBufferSize(), maxStackFrameSize)) {
		if (comp->getOptions()->getAnyOption(TR_TraceAll) || comp->getOption(TR_TraceOSR))
			traceMsg(comp,
			        "OSR COMPILE ABORT: frame size %d, scratch size %d, stack size %d were not "
			        "accommodated by the VM\n",
		...
		if (comp->getOptions()->isAnyVerboseOptionSet(TR_VerboseOSR, TR_VerboseOSRDetails))
			TR_VerboseLog::writeLineLocked(TR_Vlog_OSR,
			        "OSR COMPILE ABORT in %s: frame size %d, scratch size %d, stack size %d were not "
			        "accommodated by the VM\n",
			        comp->signature(), maxFrameSize, getMaxScratchBufferSize(), maxStackFrameSize);
		comp->failCompilation<TR::CompilationException>("OSR buffers could not be enlarged to accomodate "
		                                                "compiled method");
	}
}
...
TR_OSRCompilationData::buildDefiningMap()
{
	...
	for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
		...
		// Those 2 bool variables are necessary for identifying the cases where the OSRCode/CatchBlock is
		// already removed by the optimizer but osrMethodData still points to the block that's no longer in the
		// CFG
		...
		if (osrCatchBlock && osrCatchBlock->getExceptionPredecessors().size() > 0) {
			definingMapAtOSRCatchBlocks[i] = new DefiningMap(
			        DefiningMapComparator(), DefiningMapAllocator(comp->trMemory()->currentStackRegion()));
		...
		if (!osrCatchBlockRemoved && !osrCodeBlockRemoved)
			buildFinalMap(i - 1, osrMethodData->getDefiningMap(), definingMapAtOSRCatchBlocks[i],
 ...
 */
static void
updateDefiningSymRefs(TR_BitVector *subTreeSymRefs, DefiningMap *definingMap, TR_BitVector *resultSymRefs)
{
 ...
 */
static void
mergeDefiningMaps(DefiningMap *firstMap, DefiningMap *secondMap, TR::Compilation *comp)
{
	if (comp->getOption(TR_TraceOSR)) {
 ...
 */
void
TR_OSRCompilationData::buildFinalMap(int32_t callerIndex, DefiningMap *finalMap, DefiningMap *workingCatchBlockMap,
...
void
TR_OSRCompilationData::buildSymRefOrderMapAux(TR_Array<List<TR::SymbolReference> > *symListArray)
{
...
TR::Compilation &
operator<<(TR::Compilation &out, const TR_OSRCompilationData &osrCompilationData)
{
	out << "{";
	bool first = true;
...
TR_OSRMethodData::createOSRBlocks(TR::Node *n)
{
	...
	if (comp()->getOptions()->getVerboseOption(TR_VerboseOSRDetails)) {
		TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD,
		        "Created OSR code block and catch block for inlined index %d in %s calling %s",
		        getInlinedSiteIndex(), comp()->signature(), getMethodSymbol()->signature(comp()->trMemory()));
	}
	if (comp()->getOption(TR_TraceOSR)) {
...
void
TR_OSRMethodData::addSlotSharingInfo(int32_t byteCodeIndex, int32_t slot, int32_t symRefNum, int32_t symRefOrder,
...
TR_OSRMethodData::ensureSlotSharingInfoAt(int32_t byteCodeIndex)
{
	CS2::HashIndex hashIndex;
	if (!bcInfoHashTab.Locate(byteCodeIndex, hashIndex)) {
		TR_OSRSlotSharingInfo *ssinfo = new (getMethodSymbol()->comp()->trHeapMemory())
		        TR_OSRSlotSharingInfo(getMethodSymbol()->comp());
		bcInfoHashTab.Add(byteCodeIndex, ssinfo);
	}
}
...
TR_OSRMethodData::getDefiningMap()
{
	if (_symRefDefiningMap == NULL)
		_symRefDefiningMap = new DefiningMap(
		        DefiningMapComparator(), DefiningMapAllocator(comp()->trMemory()->heapMemoryRegion()));
...
TR_OSRSlotSharingInfo::TR_OSRSlotSharingInfo(TR::Compilation *_comp) : slotInfos(_comp->trMemory()), comp(_comp){};

void
TR_OSRSlotSharingInfo::addSlotInfo(
	...
	for (int i = 0; i < slotInfos.size(); i++) {
		...
		if (info.symRefNum != -1) {
			if ((info.slot == slot) && (info.symRefNum != symRefNum)
				...
				// Two different variables sharing the same slot, can be live at a certain node (when
				// there is a loop). So commenting the assertion out for now. TR_ASSERTC(0, comp,
				// "ERROR: For slot %d symref #%d conflicts with symref#%d\n",
		...
		if ((info.slot == slot) && (info.symRefNum == symRefNum)) {
			TR_ASSERT(info.symRefOrder == symRefOrder && info.symSize == symSize,
			        "symref order (%d,%d) and symsize (%d,%d) must match", info.symRefOrder, symRefOrder,
...
TR::Compilation &
operator<<(TR::Compilation &out, const TR_OSRSlotSharingInfo *osrSlotSharingInfo)
{
	out << "{slotInfos: [";
...
bool
TR_OSRCompilationData::TR_ScratchBufferInfo::operator==(const TR_ScratchBufferInfo &info) const
{
