...
public:
	...
	enum CommonNonhelperSymbol {
		...
		/** \brief
		 *
		 *  A call with this symbol marks a place in the jitted code where OSR transition to the VM interpreter
		 * is supported. The transition target bytecode is the bytecode index on the call plus an induction
		 * offset which is stored on the call node.
		 *
		 *  \code
		...
		/** \brief
		 *
		 *  A call with this symbol marks a place that has been optimized with runtime assumptions. Such place
		 * needs protection of OSR points. When the assumption becomes wrong, the execution of jitted code with
		 * the assumption has to be transition to the VM interpreter before running the invalid code.
		 *
		 *  \code
		...
		/** \brief
		 *
		 *  This symbol is used by the code generator to recognize and inline a call which emulates the
		 * following tree sequence:
		 *
		 *  \code
		 *  monent
		 *    object
		 ...
		 *  synchronized region. Since a full monent / monexit operation for a single load is expensive, some
		 * code generators can emit an optimized instruction sequence to load the field atomically while
	...
	TR::ParameterSymbol *createParameterSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot,
	        TR::DataType, TR::KnownObjectTable::Index knownObjectIndex = TR::KnownObjectTable::UNKNOWN);
	...
	TR::SymbolReference *findOrFabricateShadowSymbol(TR_OpaqueClassBlock *containingClass, TR::DataType type,
	        uint32_t offset, bool isVolatile, bool isPrivate, bool isFinal, const char *name,
	 ...
	 *
	 * \note If there is a temp with the same known object already use the existing one. Otherwise, create a new
	 * temp.
	 */
	...
	TR::SymbolReference *createIsOverriddenSymbolRef(TR::ResolvedMethodSymbol *calleeSymbol); /* never reuse is
	                                                                                             overridden symrefs
	                                                                                           */
...
protected:
	 ...
	 *
	 *  \note
	 *    This function should only be called from functions inside symbol reference table when creating new autos
	 * or temps. Code outside symbol reference table should use the public API
	 * findOrCreateTemporaryWithKnowObjectIndex.
	 */
	TR::SymbolReference *createTempSymRefWithKnownObject(TR::Symbol *symbol, mcount_t owningMethodIndex,
	        int32_t slot, TR::KnownObjectTable::Index knownObjectIndex);
	 ...
	 *       For symbol reference type other than temp, an original symbol is needed to find its corresponding
	 * symbol reference. Take a static field with known object index for example, \p symbol is the original static
	 * field symbol.
	 *
	...
	typedef CS2::HashTable<OwningMethodAndString, TR::SymbolReference *, TR::Allocator>
