...
TR::Register* TR::AMD64SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
    ...
    generateRegMemInstruction(
        L8RegMem, callNode, frameObjectRegister, generateX86MemoryReference(frameObjectSymRef, cg()), cg());

    TR::RealRegister* espReal = cg()->machine()->getRealRegister(TR::RealRegister::esp);
    TR::Register* gcMapPCRegister = cg()->allocateRegister();

    generateRegMemInstruction(
        LEA8RegMem, callNode, gcMapPCRegister, generateX86MemoryReference(espReal, -8, cg()), cg());
    ...
    generateMemRegInstruction(S8MemReg, callNode,
        generateX86MemoryReference(frameObjectRegister, fe()->getPythonGCMapPCOffsetInFrame(), cg()), gcMapPCRegister,
        gcMapDeps, cg());
