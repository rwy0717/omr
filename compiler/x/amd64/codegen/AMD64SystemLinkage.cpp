...
TR::AMD64Win64FastCallLinkage::AMD64Win64FastCallLinkage(TR::CodeGenerator *cg) : TR::AMD64SystemLinkage(cg)
{
	uint8_t r, p;
...
TR::AMD64ABILinkage::AMD64ABILinkage(TR::CodeGenerator *cg) : TR::AMD64SystemLinkage(cg)
{
	uint8_t r, p;
...
TR::Register *
TR::AMD64SystemLinkage::buildVolatileAndReturnDependencies(TR::Node *callNode, TR::RegisterDependencyConditions *deps)
{
	...
	case TR::Aggregate:
	default: TR_ASSERT(false, "Unrecognized call node data type: #%d", (int)callNode->getDataType()); break;
	}
...
//
int32_t
TR::AMD64SystemLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *deps)
{
	...
	for (i = first; i != last; i += direction) {
		...
		if (rregIndex != noReg) {
			...
			// a different register than the child.  In that case it is not necessary to copy the temporary
			...
			if ((child->getReferenceCount() > 1) && (vreg == child->getRegister())) {
				...
				generateRegRegInstruction(
				        TR::Linkage::movOpcodes(RegReg, movType(child->getDataType())), child, argReg,
				        vreg, cg());
				vreg = argReg;
...
TR::Register *
TR::AMD64SystemLinkage::buildDirectDispatch(TR::Node *callNode, bool spillFPRegs)
{
	TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
	TR::MethodSymbol *methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
	...
	if (methodSymbol->getMethodAddress()) {
		...
		auto LoadRegisterInstruction = generateRegImm64SymInstruction(MOV8RegImm64, callNode, scratchReg,
...
TR::AMD64SystemLinkage::getAlignment(TR::DataType type)
{
	...
	case TR::Address:
	case TR::Int64: return 8;
	// TODO: struct/union support should be added as a case branch.
	case TR::Aggregate:
	default: TR_ASSERT(false, "types still not support in layout\n"); return 0;
	}
}

void
TR::AMD64ABILinkage::mapIncomingParms(TR::ResolvedMethodSymbol *method, uint32_t &stackIndex)
{
	...
	for (parmCursor = parameterIterator.getFirst(); parmCursor; parmCursor = parameterIterator.getNext()) {
		if ((parmCursor->getLinkageRegisterIndex() >= 0)
			...
			uint32_t alignMinus1 = (align <= AMD64_STACK_SLOT_SIZE) ? (AMD64_STACK_SLOT_SIZE - 1)
			                                                        : (align - 1);
			...
			if (comp()->getOption(TR_TraceCG))
				traceMsg(comp(),
				        "mapIncomingParms setParameterOffset %d for param symbol (reg param without "
				        "home location) %p, hasToBeOnStack() %d\n",
			...
			// parmCursor->setDontHaveStackSlot(0); // this is a hack , so as we could print stack layout
			// table in createPrologue
			if (comp()->getOption(TR_TraceCG))
				traceMsg(comp(),
				        "mapIncomingParms no need to set parm %p, for it has got register %d "
				        "assigned\n",
...
int32_t
TR::AMD64SystemLinkage::layoutParm(TR::Node *parmNode, int32_t &dataCursor, uint16_t &intReg, uint16_t &floatReg,
        TR::parmLayoutResult &layoutResult)
{
	// AMD64 SysV ABI:  if the size of an object is larger than four eightbytes, or it contains unaligned fields, it
	// has class MEMORY.
...
int32_t
TR::AMD64SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol, int32_t &dataCursor, uint16_t &intReg,
...
TR::AMD64SystemLinkage::setUpStackSizeForCallNode(TR::Node *node)
{
	...
	// AMD64 SysV ABI: The end of the input argument area shall be aligned on a 16 (32, if __m256 is passed on
