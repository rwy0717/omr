...
OMR::X86::AMD64::MemoryReference::MemoryReference(TR::CodeGenerator *cg) : OMR::X86::MemoryReference(cg)
{
...
void
OMR::X86::AMD64::MemoryReference::finishInitialization(TR::CodeGenerator *cg, TR_ScratchRegisterManager *srm)
{
...
void
OMR::X86::AMD64::MemoryReference::useRegisters(TR::Instruction *instr, TR::CodeGenerator *cg)
{
...
bool
OMR::X86::AMD64::MemoryReference::needsAddressLoadInstruction(intptrj_t rip, TR::CodeGenerator *cg)
{
	TR::SymbolReference &sr = self()->getSymbolReference();
...
void
OMR::X86::AMD64::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
...
void
OMR::X86::AMD64::MemoryReference::addMetaDataForCodeAddressWithLoad(uint8_t *displacementLocation,
	...
	if (_symbolReference.getSymbol()) {
		...
		if (self()->getUnresolvedDataSnippet()) {
			...
			if (sr.getSymbol()->isStatic()) {
				...
				if (cg->wantToPatchClassPointer(NULL, displacementLocation)) // may not point to
				                                                             // beginning of class
				{
					cg->jitAddPicToPatchOnClassRedefinition(
					        ((void *)displacement), displacementLocation);
				}
			}
		} else if (sr.getSymbol()->isCountForRecompile()) {
			if (cg->needRelocationsForStatics() || cg->needRelocationsForPersistentInfoData())
				cg->addExternalRelocation(
				        new (cg->trHeapMemory()) TR::ExternalRelocation(displacementLocation,
			...
			if (counter == NULL) {
				cg->comp()->failCompilation<TR::CompilationException>("Could not generate relocation "
				                                                      ...
				                                                      "OMR::X86::AMD64::"
				                                                      "MemoryReference::"
				                                                      "addMetaDataForCodeAddressWithLoa"
				                                                      "d\n");
			}
		...
		if (self()->needsCodeAbsoluteExternalRelocation()) {
			cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(displacementLocation,
			                                  (uint8_t *)0, TR_AbsoluteMethodAddress, cg),
			        __FILE__, __LINE__, containingInstruction->getNode());
		}
	}
}
        ...
        uint8_t *modRM, TR::Instruction *containingInstruction, TR::CodeGenerator *cg)
{
	...
	if (comp->getOption(TR_TraceCG)) {
		diagnostic("OMR::X86::AMD64::MemoryReference " POINTER_PRINTF_FORMAT " with address register %s in "
		           "instruction"
	...
	if (self()->getDataSnippet() || self()->getLabel()) {
		...
		if (TR::Compiler->target.isSMP() && self()->getUnresolvedDataSnippet()) {
			...
			TR_ASSERT((addressLoadInstruction->getPrev()->getKind()
			        ...
			        "Expected TR::X86PatchableCodeAlignmentInstruction or TR::X86BoundaryAvoidance before "
			        "unresolved memory reference instruction");
		...
		if (self()->getBaseRegister() && self()->getIndexRegister()) {
			TR::Instruction *addressAddInstruction = generateRegRegInstruction(addressLoadInstruction,
			        ADD8RegReg, self()->getAddressRegister(), self()->getBaseRegister(), cg);
		...
		self()->setUnresolvedDataSnippet(NULL); // Otherwise it will get damaged when we re-emit
		                                        // containingInstruction
		...
		if (self()->getUnresolvedDataSnippet()) {
			...
			traceMsg(comp,
			        "found unresolved shadow with NULL base object : data reference instruction=%p, "
			        "cursor=%p\n",
