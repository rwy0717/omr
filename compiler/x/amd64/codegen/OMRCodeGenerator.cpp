...
OMR::X86::AMD64::CodeGenerator::getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex, TR::DataType type)
{
...
OMR::X86::AMD64::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node *node)
{
	...
	if (node->getOpCode().isIf() && node->getFirstChild()->getOpCodeValue() == TR:: instanceof)
		...
		return self()->getNumberOfGlobalGPRs() - 3; // A memref in a jump for a MemTable might need a base,
