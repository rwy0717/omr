...
protected:
	Register(uint32_t f = 0) : OMR::Register(f), _memRef(NULL), _rematerializationInfo(NULL)
	{
		_liveRegisterInfo._liveRegister = NULL;
	}
	Register(TR_RegisterKinds rk) : OMR::Register(rk), _memRef(NULL), _rematerializationInfo(NULL)
	{
		_liveRegisterInfo._liveRegister = NULL;
	}
	Register(TR_RegisterKinds rk, uint16_t ar) : OMR::Register(rk, ar), _memRef(NULL), _rematerializationInfo(NULL)
	{
		_liveRegisterInfo._liveRegister = NULL;
	}
...
private:
	enum { NeedsPrecisionAdjustment = 0x0002, // IA32 flag that indicates that a store/reload of
		...
		NeedsLazyClobbering = 0x0800, // X86 service releases only (well, originally the intent): node refcount
		                              // = 1 is not enough to determine that this can be clobbered; must also
		                              // check register node count
