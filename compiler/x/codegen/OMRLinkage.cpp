...
OMR::X86::Linkage::Linkage(TR::CodeGenerator *cg) : OMR::Linkage(cg), _minimumFirstInstructionSize(0)
{
			...
			if (localCursor->getGCMapIndex() >= 0) {
				...
				if (igNode) {
					...
					if (localCursor->isInternalPointer() || localCursor->isPinningArrayPointer()
						...
						performSharing = performTransformation(self()->comp(),
			...
			if (localCursor->getGCMapIndex() >= 0) {
				...
				if (self()->cg()->getTraceRAOption(TR_TraceRASpillTemps))
					traceMsg(self()->comp(),
					        ...
					        localCursor->getGCMapIndex(), localCursor->getOffset(), newOffset);
	...
	for (localCursor = automaticIterator.getFirst(); localCursor; localCursor = automaticIterator.getNext())
		if (localCursor->getGCMapIndex() >= 0) {
			localCursor->setOffset(
			        stackIndex + pointerSize * (localCursor->getGCMapIndex() - firstLocalGCIndex));
...
void
OMR::X86::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{
	return self()->mapSingleAutomatic(p, p->getRoundedSize(), stackIndex);
}

void
OMR::X86::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t size, uint32_t &stackIndex)
{
...
void
OMR::X86::Linkage::stopUsingKilledRegisters(TR::RegisterDependencyConditions *deps, TR::Register *returnRegister)
{
...
void
OMR::X86::Linkage::associatePreservedRegisters(TR::RegisterDependencyConditions *deps, TR::Register *returnRegister)
{
	TR::Register *vmThreadReg = self()->cg()->getVMThreadRegister();
	TR_LiveRegisters *liveRegisters = self()->cg()->getLiveRegisters(TR_GPR);

	for (TR_LiveRegisterInfo *liveReg = liveRegisters->getFirstLiveRegister(); liveReg;
		...
		for (int realReg = TR::RealRegister::LastAssignableGPR; realReg >= TR::RealRegister::FirstGPR;
		        realReg--) {
			...
			if (self()->machine()->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)realReg)
			        == virtReg) {
				// virtReg is already associated with a preserved realReg, so obviously that's the best
				// choice
				//
				bestReal = (TR::RealRegister::RegNum)realReg;
				break;
			} else if (self()->machine()->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)realReg)
			        == NULL) {
...
TR::Register *
OMR::X86::Linkage::findReturnRegisterFromDependencies(TR::Node *callNode,
        TR::RegisterDependencyConditions *dependencies, TR::MethodSymbol *methodSymbol, bool isDirect)
{
	...
	case TR::Int64:
		returnRegister =
		        self()->cg()->allocateRegisterPair(postConditions->getRegisterDependency(0)->getRegister(),
...
//
void
OMR::X86::Linkage::coerceFPReturnValueToXMMR(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
        TR::MethodSymbol *methodSymbol, TR::Register *returnReg)
{
	...
	if (callNode->getOpCode().isFloat()) {
		...
		generateRegMemInstruction(MOVSSRegMem, callNode, returnReg,
		        generateX86MemoryReference(*tempMR, 0, self()->cg()), self()->cg());
	} else {
		TR_ASSERT(
		        callNode->getOpCode().isDouble(), "cannot return non-floating-point values in XMM registers\n");
...
uint8_t *
OMR::X86::Linkage::loadArguments(TR::Node *callNode, uint8_t *cursor, bool calculateSizeOnly, int32_t *sizeOfFlushArea,
        bool isReturnAddressOnStack)
{
	return self()->flushArguments(
	        callNode, cursor, calculateSizeOnly, sizeOfFlushArea, isReturnAddressOnStack, true);
}
...
{
	switch (kind) {
	case TR_GPR: return self()->getProperties().getNumIntegerArgumentRegisters();
	case TR_FPR: return self()->getProperties().getNumFloatArgumentRegisters();
	default: return 0;
	}
}
