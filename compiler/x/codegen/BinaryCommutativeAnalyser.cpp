...
static void
zeroExtendTo32BitRegister(TR::Node *node, TR::Register *reg, int32_t sourceSize, TR::CodeGenerator *cg)
{
	TR_X86OpCodes op;

	switch (sourceSize) {
	case 1: op = MOVZXReg4Reg1; break;
	case 2: op = MOVZXReg4Reg2; break;
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::genericAnalyserWithExplicitOperands(TR::Node *root, TR::Node *firstChild,
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::genericAnalyser(TR::Node *root, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
 ...
 */
TR::Register *
TR_X86BinaryCommutativeAnalyser::genericAnalyserImpl(TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
...
void
TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(TR::Node *root, TR_X86OpCodes lowRegRegOpCode,
	...
	if (firstChild->isHighWordZero()) {
		...
		if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
			...
			if ((firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
			            && (firstChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true))
			                    == 32)) {
				bypassedFirstUnsignedConversionChild = firstChild;
				// only decrease RC for bypassed node and the constant node because we bypass the lushr
				// node but keep the reference to its first child
				...
				case 1: lowRegMemOpCode = lowRegMemOpCode1Byte; break;

				case 2: lowRegMemOpCode = lowRegMemOpCode2Byte; break;
				}
			}
		}
	}
	...
	if (secondChild->isHighWordZero()) {
		...
		if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
			...
			if ((secondOp == TR::lushr && secondChild->getSecondChild()->getOpCodeValue() == TR::iconst
			            && (secondChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true))
			                    == 32)) {
	...
	if (getEvalChild1()) {
		...
		if (bypassedFirstUnsignedConversionChild)
			zeroExtendTo32BitRegister(
			        bypassedFirstUnsignedConversionChild, firstRegister, firstChild->getSize(), _cg);
	}
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node *root, TR_X86OpCodes regRegOpCode,
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::integerAddAnalyserWithExplicitOperands(TR::Node *root, TR::Node *firstChild,
 ...
 */
TR::Register *
TR_X86BinaryCommutativeAnalyser::integerAddAnalyserImpl(TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
	...
	if (getOpReg1Reg2()) {
		...
		if (firstRegister->containsCollectedReference() || secondRegister->containsCollectedReference()
			...
			if (root->isInternalPointer()) {
				...
				// arraylets: aiadd is technically internal pointer into spine object, but isn't marked
				// as internal pointer
				tempReg = _cg->allocateRegister();
			else
 ...
 */
TR::Register *
TR_X86BinaryCommutativeAnalyser::longAddAnalyserImpl(TR::Node *root, TR::Node *&firstChild, TR::Node *&secondChild)
{
			...
			if (secondOp == TR::iu2l || secondOp == TR::su2l || secondOp == TR::bu2l
			        || (secondOp == TR::lushr
			                ...
			                && (secondChild->getSecondChild()->getInt()
			                           & TR::TreeEvaluator::shiftMask(true))
			                        == 32)) {
				bypassedSecondUnsignedConversionChild = secondChild;
				secondChild = secondChild->getFirstChild();
				secondRegister = secondChild->getRegister();
...
TR_X86BinaryCommutativeAnalyser::longDualMultiplyAnalyser(TR::Node *root)
{
	...
	generateRegRegInstruction(MUL4AccReg, root, eax, r4, depsMul[0], _cg); //   2  MUL    R4          // EDX:EAX =
	                                                                       //   al * bl
	generateRegRegInstruction(XCHG4RegReg, root, r1, eax, _cg); //   3  XCHG   R1, EAX     // R1 = (al * bl)l; EAX =
	                                                            //   al
	generateRegRegInstruction(MOV4RegReg, root, r2, edx, _cg); //   4  MOV    R2, EDX     // R2 = (al * bl)h
	generateRegRegInstruction(MUL4AccReg, root, eax, r3, depsMul[1], _cg); //   5  MUL    R3          // EDX:EAX =
	                                                                       //   al * bh
	generateRegRegInstruction(XCHG4RegReg, root, r3, edx, _cg); //   6  XCHG   R3, EDX     // R3 = (al * bh)h; EDX =
	                                                            //   bh
	generateRegRegInstruction(ADD4RegReg, root, r2, eax, _cg); //   7  ADD    R2, EAX     // R2 = (al * bl)h + (al *
	                                                           //   bh)l
	generateRegImmInstruction(ADC4RegImms, root, r3, 0, _cg); //   8  ADC    R3, 0       // R3 = (al * bh)h +
	                                                          //   [carry]
	generateRegRegInstruction(MOV4RegReg, root, eax, r5, _cg); //   9  MOV   EAX, R5      // EAX = ah
	generateRegRegInstruction(MUL4AccReg, root, eax, edx, depsMul[2], _cg); //  10  MUL   EDX          // EDX:EAX =
	                                                                        //  ah * bh
	generateRegRegInstruction(XCHG4RegReg, root, r4, edx, _cg); //  11  XCHG   R4, EDX     // R4 = (ah * bh)h; EDX =
	                                                            //  bl
	generateRegRegInstruction(ADD4RegReg, root, r3, eax, _cg); //  12  ADD    R3, EAX     // R3 = (al * bh)l + (ah *
	                                                           //  bh)l
	generateRegImmInstruction(ADC4RegImms, root, r4, 0, _cg); //  13  ADC    R4, 0       // R4 = (ah * bh)h +
	                                                          //  [carry]
	generateRegRegInstruction(MOV4RegReg, root, eax, r5, _cg); //  14  MOV   EAX, R5      // EAX = ah
	generateRegRegInstruction(MUL4AccReg, root, eax, edx, depsMul[3], _cg); //  15  MUL   EDX          // EDX:EAX =
	                                                                        //  ah * bl
	generateRegRegInstruction(ADD4RegReg, root, r2, eax, _cg); //  16  ADD    R2, EAX     // R2 = (al * bl)h + (al *
	                                                           //  bh)l              + (ah * bl)l
	generateRegRegInstruction(ADC4RegReg, root, r3, edx, _cg); //  17  ADC    R3, EDX     // R3 =              (al *
	                                                           //  bh)l + (ah * bh)l + (ah * bl)h + [carry]
	generateRegImmInstruction(ADC4RegImms, root, r4, 0, _cg); //  18  ADC    R4, 0       // R4 = (ah * bh)h +
	                                                          //  [carry]
