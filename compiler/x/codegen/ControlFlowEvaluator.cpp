...
// Right now, this is duplicated in ControlFlowEval, BinaryEval and TreeEval.
inline bool
constNodeValueIs32BitSigned(TR::Node *node, intptrj_t *value, TR::CodeGenerator *cg)
{
...
generateWiderCompare(TR::Node *node, TR::Register *targetReg, intptrj_t value, TR::CodeGenerator *cg)
{
...
static void
binarySearchCaseSpace(TR::Register *selectorReg, TR::Node *lookupNode, uint32_t lowChild, uint32_t highChild,
        bool &evaluateDefaultGlRegDeps, TR::CodeGenerator *cg)
{
	...
	if (pivot >= lowChild) {
		...
		// For the unsigned case, we better use JA if the numbers straddle the TR::getMaxSigned<TR::Int32>()
		// boundary. If that's the case, the highVal will be less than (in the signed sense) than lowVal.
		//
		bool isUnsigned;
				...
				for (int j = firstCaseChild->getNumChildren() - 1; j >= 0; j--) {
					...
					if (globalReg->getKind() == TR_GPR && highGlobalRegNum < 0
						...
						if (!deps->findPostCondition(registerIndex))
							deps->addPostCondition(globalRegPair
							                ? globalRegPair->getLowOrder()
							                : globalReg,
							        registerIndex, cg);
...
OMR::X86::TreeEvaluator::tableEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit() && cg->comp()->compileRelocatableCode()) {
		...
		jumpMR = generateX86MemoryReference((TR::Register *)NULL, selectorReg,
		        (uint8_t)(TR::Compiler->target.is64Bit() ? 3 : 2), (intptrj_t)branchTable, cg);
	...
	if (cg->getLinkage()->getProperties().getMethodMetaDataRegister() != TR::RealRegister::NoReg) {
		jmpTableInstruction =
		        generateMemTableInstruction(JMPMem, node, jumpMR, numBranchTableEntries, deps, cg);
	} else {
...
void
OMR::X86::TreeEvaluator::compareIntegersForEquality(TR::Node *node, TR::CodeGenerator *cg)
{
		...
		if (constValue >= -128 && constValue <= 127) {
			...
			//
			{
				if (firstChild->getOpCode().isAnd() && (firstChild->getRegister() == NULL)
					...
					// the value are garbage. To make this work, we must zero-extend the mask
					// immediate to 4 bytes, thereby ensuring the top two bytes of the masked value
					// will be zero and won't affect the subsequent compare with zero.

					// Therefore, counterintuitively, zero-entending the mask is always the correct
					// thing to do, regardless of whether the compare node claims to be a signed
					// operation!
					uint64_t mask;
						...
						if (andFirstChild->getRegister() == NULL
							...
							if (((mask >> 8) == 0) || (andSecondChild->getSize() == 1))
								generateMemImmInstruction(
								        TEST1MemImm1, node, tempMR, mask, cg);
							else if (andSecondChild->getSize() == 2) {
								TR::Register *tempReg = cg->allocateRegister();
								TR::TreeEvaluator::loadConstant(node, mask,
								        TR_RematerializableShort, cg, tempReg);
							...
							// register case
							// before evaluating the andFirstChild we check to see if it is
							// an integral conversion that is redundant because of the
							// constant value in the compare
							TR::Register *tempReg = cg->evaluate(andFirstChild);
							if (andFirstChild->getOpCode().isConversion()
							        && andFirstChild->getType().isIntegral()
							        && andFirstChild->getFirstChild()
							                   ->getType()
							                   .isIntegral()
					...
					if (isConditionCodeSetForCompareToZero(firstChild, true)) {
						...
						if (firstChild->getOpCode().isMemoryReference()
							...
							if (isConditionCodeSetForCompareToZero(firstChild, true)) {
								// Nothing to do because the evaluation of firstChild
								// set the condition codes for us
							} else {
								if (compareSize == 1)
									generateRegRegInstruction(TEST1RegReg, node,
									        firstChildReg, firstChildReg, cg);
								else if (compareSize == 2)
									generateRegRegInstruction(TEST2RegReg, node,
									        firstChildReg, firstChildReg, cg);
								else
									TR::TreeEvaluator::
			...
			if (size == 1)
				...
				/// cmpInstruction = generateRegImmInstruction(CMP2RegImm2, node, firstChildReg,
				/// constValue, cg);
			...
			if (isPICCandidate
			        && cg->wantToPatchClassPointer((TR_OpaqueClassBlock *)constValue, secondChild))
...
void
OMR::X86::TreeEvaluator::compareIntegersForOrder(
        TR::Node *node, TR::Node *firstChild, TR::Node *secondChild, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
		...
		bool is64Bit = TR::TreeEvaluator::getNodeIs64Bit(secondChild, cg);
		TR_X86CompareAnalyser temp(cg);
		temp.integerCompareAnalyser(node, firstChild, secondChild, false, CMPRegReg(is64Bit),
		        CMPRegMem(is64Bit), CMPMemReg(is64Bit));
	}
}

void
OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, node->getFirstChild(), node->getSecondChild(), cg);
}

void
OMR::X86::TreeEvaluator::compare2BytesForOrder(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *secondChild = node->getSecondChild();
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
...
void
OMR::X86::TreeEvaluator::compareBytesForOrder(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *secondChild = node->getSecondChild();
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
		TR::Node *firstChild = node->getFirstChild();
		if ((firstChild->getReferenceCount() == 1) && (firstChild->getRegister() == NULL)
			...
			generateRegImmInstruction(
			        CMP1RegImm1, node, cg->evaluate(firstChild), secondChild->getByte(), cg);
		}
		cg->decReferenceCount(firstChild);
		cg->decReferenceCount(secondChild);
	} else {
...
TR::Register *
OMR::X86::TreeEvaluator::integerReturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	if (comp->getMethodSymbol()->getLinkageConvention() == TR_Private) {
		if (TR::Compiler->target.is64Bit()) {
			...
			default:
				...
				// fall through
			case TR::Int32: returnInfo = TR_IntReturn; break;
			case TR::Int64: returnInfo = TR_LongReturn; break;
			case TR::Address: returnInfo = TR_ObjectReturn; break;
			}
...
TR::Register *
OMR::X86::TreeEvaluator::iternaryEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!longCompareOn32bit && conditionOp.isCompareForEquality()
		...
		generateRegRegInstruction((conditionOp.isCompareTrueIfEqual())
		                ? ((conditionOp.isCompareTrueIfGreater()) ? CMOVLRegReg(trueValIs64Bit)
		                                                          : CMOVGRegReg(trueValIs64Bit))
		                : ((conditionOp.isCompareTrueIfGreater()) ? CMOVLERegReg(trueValIs64Bit)
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
#ifdef J9_PROJECT_SPECIFIC
...
static inline void
generateMergedGuardCodeIfNeeded(TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *runtimeGuard)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	if (virtualGuardHelper(node, cg)) {
		...
		if (node->getFirstChild()->getOpCodeValue() == TR::ishr && node->getFirstChild()->getRegister() == NULL
		        ...
		        && performTransformation(comp,
		                "O^O SHIFT PEEPHOLE: detected shift pattern for node %p shifting so mask = %p shift "
		                "amount = %d \n",
...
bool
OMR::X86::TreeEvaluator::generateIAddOrSubForOverflowCheck(TR::Node *compareNode, TR::CodeGenerator *cg)
{
	...
	if (matches && (u.operationNode->getOpCode().isAdd() || u.operationNode->getOpCode().isSub())
		...
		// leftChild might appear twice in this tree, and we need a clobber evaluate only if it also appears
		// elsewhere
		...
		if (!u.operationNode->getRegister()) {
			// We actually evaluated the operationNode here so we have to do everything its evaluator would
			// have done
...
bool
OMR::X86::TreeEvaluator::generateLAddOrSubForOverflowCheck(TR::Node *compareNode, TR::CodeGenerator *cg)
{
	TR_ArithmeticOverflowCheckNodes u = {0};
	...
	if (matches && (u.operationNode->getOpCode().isAdd() || u.operationNode->getOpCode().isSub())
		...
		if (TR::Compiler->target.is64Bit()) {
			...
			generateRegRegInstruction(
			        ADD4RegReg, u.operationNode, leftReg->getLowOrder(), rightReg->getLowOrder(), cg);
			generateRegRegInstruction(
			        ADC4RegReg, u.operationNode, leftReg->getHighOrder(), rightReg->getHighOrder(), cg);
		} else {
			generateRegRegInstruction(
			        SUB4RegReg, u.operationNode, leftReg->getLowOrder(), rightReg->getLowOrder(), cg);
			generateRegRegInstruction(
			        SBB4RegReg, u.operationNode, leftReg->getHighOrder(), rightReg->getHighOrder(), cg);
		}
		if (!u.operationNode->getRegister()) {
			// We actually evaluated the operationNode here so we have to do everything its evaluator would
			// have done
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	if (TR::TreeEvaluator::getNodeIs64Bit(node, cg)
	                ? TR::TreeEvaluator::generateLAddOrSubForOverflowCheck(node, cg)
	                : TR::TreeEvaluator::generateIAddOrSubForOverflowCheck(node, cg)) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerIfCmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::ifbcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::ifbcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::ifbucmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JB4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JGE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbucmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JAE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JG4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbucmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JA4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JLE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifbucmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	generateConditionalJumpInstruction(JBE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifscmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *secondChild = node->getSecondChild();
...
TR::Register *
OMR::X86::TreeEvaluator::ifscmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::ifscmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JGE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifscmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JG4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifscmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JLE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifsucmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForEquality(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::ifsucmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JB4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifsucmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JAE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifsucmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JA4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::ifsucmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateConditionalJumpInstruction(JBE4, node, cg, true);
	return NULL;
...
TR::Register *
OMR::X86::TreeEvaluator::integerEqualityHelper(TR::Node *node, TR_X86OpCodes setOp, TR::CodeGenerator *cg)
{
	TR::TreeEvaluator::compareIntegersForEquality(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerCmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerEqualityHelper(node, SETE1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::integerCmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerEqualityHelper(node, SETNE1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::integerOrderHelper(TR::Node *node, TR_X86OpCodes setOp, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister = cg->allocateRegister();
	node->setRegister(targetRegister);
	TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerCmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETL1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::integerCmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETGE1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::integerCmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETG1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::integerCmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETLE1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerCmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETB1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerCmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETAE1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerCmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::integerOrderHelper(node, SETA1Reg, cg);
}

TR::Register *
OMR::X86::TreeEvaluator::unsignedIntegerCmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::bcmpEvaluator(TR::Node *node, TR_X86OpCodes setOp, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister = cg->allocateRegister();
	TR::TreeEvaluator::compareBytesForOrder(node, cg);
	node->setRegister(targetRegister);
...
TR::Register *
OMR::X86::TreeEvaluator::cmp2BytesEvaluator(TR::Node *node, TR_X86OpCodes setOp, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister = cg->allocateRegister();
	TR::TreeEvaluator::compare2BytesForOrder(node, cg);
	generateRegInstruction(setOp, node, targetRegister, cg);
...
virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (cg->needClassAndMethodPointerRelocations()) {
		...
		case TR_HCRGuard:
		case TR_AbstractGuard: aotSite->setGuard(virtualGuard); break;
