...
bool OMR::X86::Instruction::needsRepPrefix() { return self()->getOpCode().needsRepPrefix(); }

bool OMR::X86::Instruction::needsLockPrefix() { return self()->getOpCode().needsLockPrefix(); }
...
int32_t TR::X86LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
        ...
        if (getOpCode().hasIntImmediate()) {
            ...
            if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
                ...
                if (distance >= -128 && distance < 0 && _permitShortening) {
                    immediateLength = 0; // really 1, but for conditional branches (all excep JMP4) the opcode entry
                                         // will be 1 too big for short branch because the short branch op is 1 byte,
                                         // but the long branch op for conditionals is 2
    ...
    } else // assume absolute code reference
    {
        setEstimatedBinaryLength(
            getOpCode().length(self()->rexBits()) + 4); // full offset for absolute address reference
    }
    return currentEstimate + getEstimatedBinaryLength();
}
        ...
        if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
            ...
            if (symbol->isConst()) {
                ...
                if (cg()->needClassAndMethodPointerRelocations()) {
                    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                        cg()->addExternalRelocation(
                            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSourceImmediate(),
                ...
                if (!labelSym) {
                    ...
                    if (methodSym && methodSym->isHelper()) {
                        if (!IS_32BIT_RIP(targetAddress, cursor + 4) || forceTrampolineUse) {
                            ...
                            TR_ASSERT(IS_32BIT_RIP(targetAddress, cursor + 4),
        ...
        case TR_ClassPointer:
            if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                ...
                cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
                                                (uint8_t*)getNode(), (TR_ExternalRelocationTargetKind)_reloKind, cg()),
                    __FILE__, __LINE__, getNode());
            }
            break;
...
void TR::X86RegImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    case TR_ClassPointer:
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getNode(),
                                            (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
                __FILE__, __LINE__, getNode());
        }
        break;
...
void TR::X86MemImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (symbol->isConst()) {
        ...
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_MethodObject,
                cg()),
            __FILE__, __LINE__, getNode());
...
uint8_t TR::X86FPRegMemInstruction::getBinaryLengthLowerBound() { return getOpCode().length(self()->rexBits()); }
        ...
        if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant()
            ...
            case TR_ClassAddress: {
                ...
                if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    cg()->addExternalRelocation(
                        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)methodSymRef,
...
void TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    if (getOpCodeValue() == DQImm64) {
        cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)(uint64_t)getSourceImmediate(),
            getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Thunks, __FILE__,
            __LINE__, getNode());
    }
}
