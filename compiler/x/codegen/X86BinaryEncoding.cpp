...
int32_t
memoryBarrierRequired(TR_X86OpCode &op, TR::MemoryReference *mr, TR::CodeGenerator *cg, bool onlyAskingAboutFences)
{
...
OMR::X86::Instruction::generateBinaryEncoding()
{
	...
	// it is needed to avoid recursive calls when generateOperand() requests to regenerate the binary code by
	// returning NULL;
	for (;;) {
...
OMR::X86::Instruction::estimateBinaryLength(int32_t currentEstimate)
{
	self()->setEstimatedBinaryLength(
	        self()->getOpCode().length(self()->rexBits()) + (self()->needsRepPrefix() ? 1 : 0));
...
int32_t
TR::X86BoundaryAvoidanceInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
TR::X86BoundaryAvoidanceInstruction::generateBinaryEncoding()
{
	...
	while (cur->getLength() > 0) {
		TR_ASSERT(cur->getLength() <= _boundarySpacing,
		        "Atomic regions of %d bytes can't fit between %d-byte boundaries", cur->getLength(),
		        _boundarySpacing);
		...
		if (label == NULL) {
			...
			*(uint32_t *)cursor = 0;
			...
			TR_ASSERT(getOpCodeValue() != XBEGIN4 || !_permitShortening,
			        "XBEGIN4 cannot be shortened and can only be used with a label instruction that cannot "
			...
			if (distance >= -128 && distance <= 127 && getOpCode().isBranchOp() && _permitShortening) {
				...
				if (getOpCode().isShortBranchOp()) {
					...
					TR_ASSERT(0,
					        ...
					        this, distance);
					comp->failCompilation<TR::CompilationException>("short form branch "
...
TR::X86LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
		...
		if (getOpCode().hasIntImmediate()) {
			...
			if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
				...
				if (distance >= -128 && distance < 0 && _permitShortening) {
					immediateLength = 0; // really 1, but for conditional branches (all excep JMP4)
					                     // the opcode entry will be 1 too big for short branch
					                     // because the short branch op is 1 byte, but the long
					                     // branch op for conditionals is 2
	...
	{
		setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) + 4); // full offset for absolute address
		                                                                     // reference
	}
	return currentEstimate + getEstimatedBinaryLength();
}
...
{
	setEstimatedBinaryLength(TR::Compiler->target.is64Bit() ? 5 : 6); // TODO:AMD64: What if patched instruction
...
TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
	...
	if (instToBePatched && _nopSize > 0 && !cg()->nopsAlsoProcessedByRelocations()) {
		...
		if (enlargement.getPatchGrowth() + next >= _nopSize) {
			...
			TR_ASSERT(enlargement.getPatchGrowth() == 0,
			        "Instructions should only partially enlarge when dealing with an HCR guard - any other "
			        "partial enlargement is a waste of code cache!");
		}
	...
	cursor = cg()->generatePadding(
	        cursor, _nopSize, this, requiresAtomicPatching ? TR_AtomicNoOpPadding : TR_NoOpPadding);
...
TR::X86ImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (getOpCode().hasIntImmediate()) {
		...
		if (getReloKind() != -1) // TODO: need to change Body info one to use this
		{
			...
			case TR_ClassPointer:
				if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
					...
					cg()->addExternalRelocation(
					        new (cg()->trHeapMemory())
					                TR::ExternalRelocation(cursor, (uint8_t *)getNode(),
		...
		if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(),
		            cg()->comp()->getStaticHCRPICSites()->end(), this)
...
TR::X86ImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (getOpCode().hasIntImmediate()) {
		...
		if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
				...
				if (labelSym) {
					...
					if (methodSym && methodSym->isHelper()) {
						cg()->addProjectSpecializedRelocation(cursor,
						        (uint8_t *)getSymbolReference(), NULL, TR_HelperAddress,
						        __FILE__, __LINE__, getNode());
						...
						cg()->addExternalRelocation(
						        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
						                (uint8_t *)getNode()->getSymbolReference(),
						                getNode() ? (uint8_t *)(intptr_t)getNode()
						                                    ->getInlinedSiteIndex()
						                          : (uint8_t *)-1,
						...
						cg()->addProjectSpecializedRelocation(cursor,
						        (uint8_t *)getSymbolReference(), NULL, TR_RelativeMethodAddress,
						        __FILE__, __LINE__, getNode());
					}
				}
			}
			...
			cg()->addExternalRelocation(
			        new (cg()->trHeapMemory())
			                TR::ExternalRelocation(cursor, (uint8_t *)(uintptrj_t)getSourceImmediate(),
			...
			if (symbol->isConst()) {
					...
					if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
						...
						cg()->addExternalRelocation(
						        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
						                ...
						                getNode() ? (uint8_t *)(uintptr_t)getNode()
						                                    ->getInlinedSiteIndex()
						                          : (uint8_t *)-1,
						                TR_ClassAddress, cg()),
						        __FILE__, __LINE__, getNode());
				...
				if (staticSym && staticSym->isCompiledMethod())
					...
					if (counter == NULL) {
						comp->failCompilation<TR::CompilationException>("Could not generate "
						                                                "relocation for debug "
						                                                "counter in "
						                                                "TR::"
						                                                "X86ImmSymInstruction::"
						                                                "addMetaDataForCodeAddr"
						                                                "ess\n");
					}
		...
		if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
			...
			if (comp->isRecursiveMethodTarget(sym)) {
				...
				if (!labelSym) {
					// TODO:
					//
					// The following is only necessary on 64-bit because of the present way in which
					// CALL instructions are created.  On 32-bit the target is always reachable from
					// the call site, so the actual 32-bit target address is encoded in the
					// immediate field of the instruction.  There is no need to ask the symbol
					// reference for the actual address because all the information is already
					// present in the instruction.
					//
					// On 64-bit, the target address may not fit in a 32-bit address.  The target
					// symbol must be queried to determine the actual target address and then
					// whether it is with the 32-bit displacement range.  Hence, 64-bit relies on a
					// valid target symbol to find the actual target address.
					//
					// Not all CALL instructions have symbol references that describe the actual
					// target. Profiled targets in static IPICs are an example.  The handshake
					// between instruction selection and binary encoding is dubious, and the safe
					// thing to do at this stage is to revert back to the original logic in this
					// method on 32-bit.  Such CALL instructions do not appear on the 64-bit path.
					//
					...
					if (TR::Compiler->target.is64Bit()) {
						...
						//
						// Symbols created for JNI methods have their method address pointing to
						// the RAM method. Otherwise, the symbol method address points at the
						...
						if (methodSym && methodSym->isJNI() && symNode
							...
							TR_ResolvedMethod *resolvedMethod = resolvedMethodSym
							        ? resolvedMethodSym->getResolvedMethod()
							        : 0;
							targetAddress =
							        (uintptrj_t)resolvedMethod->startAddressForJNIMethod(
							                comp);
						} else
							targetAddress =
			...
			*(int32_t *)cursor = (int32_t)(targetAddress - nextInstructionAddress);
			...
			if (!symbol->isConst() && symbol->isClassObject()) {
				if (cg()->needClassAndMethodPointerRelocations()) {
					if (sym->isStatic()) {
						*(intptrj_t *)cursor =
						        (intptrj_t)TR::Compiler->cls
						                .persistentClassPointerFromClassPointer(cg()->comp(),
						                        (TR_OpaqueClassBlock *)sym->getStaticSymbol()
						                                ->getStaticAddress());
					} else {
						*(int32_t *)cursor =
						        (int32_t)TR::Compiler->cls
						                .persistentClassPointerFromClassPointer(cg()->comp(),
						                        (TR_OpaqueClassBlock *)(uintptr_t)
						                                getSourceImmediate());
					}
				}
			}
...
TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (getOpCode().hasIntImmediate()) {
		...
		if (staticHCRPIC) {
			cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation((uint8_t *)cursor,
		...
		case TR_ACTIVE_CARD_TABLE_BASE:
			cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
			                                    (uint8_t *)TR_ActiveCardTableBase, TR_GlobalValue, cg()),
			        __FILE__, __LINE__, getNode());
			break;
		...
		case TR_ClassPointer:
			if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
				...
				cg()->addExternalRelocation(
				        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)getNode(),
				                (TR_ExternalRelocationTargetKind)_reloKind, cg()),
				        __FILE__, __LINE__, getNode());
			}
			break;
...
TR::X86RegImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	case TR_DebugCounter: {
		...
		if (counter == NULL) {
			cg()->comp()->failCompilation<TR::CompilationException>("Could not generate relocation for "
			                                                        "debug counter in "
	...
	default: TR_ASSERT(0, "invalid relocation kind for TR::X86RegImmSymInstruction");
	}
}
        ...
        int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
	...
	OMR::X86::EnlargementResult result = getMemoryReference()->enlarge(
	        cg(), requestedEnlargementSize, maxEnlargementSize, allowPartialEnlargement);
		...
		if (_reloKind == TR_ClassAddress && cg()->needClassAndMethodPointerRelocations()) {
			...
			if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
				...
				cg()->addExternalRelocation(
				        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
				                (uint8_t *)getNode()->getSymbolReference(),
				                (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex(), TR_ClassAddress,
				                cg()),
				        __FILE__, __LINE__, getNode());
			}
		}
...
TR::X86RegMemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
	...
	setEstimatedBinaryLength(
	        getOpCode().length(self()->rexBits()) + rexRepeatCount() + length + patchBoundaryPadding);
	return currentEstimate + getEstimatedBinaryLength();
}

OMR::X86::EnlargementResult
TR::X86RegMemInstruction::enlarge(
        int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
	TR::Compilation *comp = cg()->comp();
	if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement) || requestedEnlargementSize < 1)
...
TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (cg()->needRelocationsForHelpers()) {
		if (getNode()->getOpCode().hasSymbolReference() && methodSymRef
			...
			cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
			                                    (uint8_t *)methodSymRef, TR_AbsoluteHelperAddress, cg()),
			        __FILE__, __LINE__, getNode());
		}
	}
...
TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (getSymbolReference()->getSymbol()->isLabel()) {
		...
		case TR_DebugCounter: {
			TR::DebugCounterBase *counter = cg()->comp()->getCounterFromStaticAddress(getSymbolReference());
			if (counter == NULL) {
				cg()->comp()->failCompilation<TR::CompilationException>("Could not generate relocation "
				                                                        "for debug counter in "
				                                                        "TR::"
...
TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	if (getOpCodeValue() == DQImm64) {
		cg()->addProjectSpecializedRelocation(cursor, (uint8_t *)(uint64_t)getSourceImmediate(),
		        getNode() ? (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_Thunks,
		        __FILE__, __LINE__, getNode());
	}
}
