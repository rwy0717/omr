 ...
 */
void
TR_X86SubtractAnalyser::integerSubtractAnalyserWithExplicitOperands(TR::Node *root, TR::Node *firstChild,
 ...
 */
void
TR_X86SubtractAnalyser::integerSubtractAnalyser(TR::Node *root, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
 ...
 */
TR::Register *
TR_X86SubtractAnalyser::integerSubtractAnalyserImpl(TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
	...
	if (getCopyReg1()) {
		if (firstChild->getReferenceCount() > 1) {
			...
			if (firstChild->getOpCodeValue() == TR::iconst && loadedConst) {
				...
				if (secondChild->getReferenceCount() == 1 && secondRegister != 0 && !needsEflags
				        && (borrow == 0)) {
 ...
 */
TR::Register *
TR_X86SubtractAnalyser::longSubtractAnalyserImpl(TR::Node *root, TR::Node *&firstChild, TR::Node *&secondChild)
{
	TR::Register *firstRegister = firstChild->getRegister();
	TR::Register *secondRegister = secondChild->getRegister();
	TR::Register *targetRegister = NULL;
	...
	if (getCopyReg1()) {
		...
		} else // assert getSubReg3Mem2() == true
		{
			...
			/**
			 * The below code is needed to ensure correct behaviour when the subtract analyser encounters a
			 * lushr bytecode that shifts by 32 bits. This is the only case where the useSecondHighOrder bit
			 * is set. When the first child of the lushr is in a register, code above handles the shift.
			 * When the first child of the lushr is in memory, the below ensures that the upper part of the
			 * first child of the lushr is used as lowMR.
			 */
			if (useSecondHighOrder) {
				TR_ASSERT(secondHighZero,
				        "useSecondHighOrder should be consistent with secondHighZero. "
				        "useSecondHighOrder subsumes secondHighZero");
		...
		if (secondHighZero) {
			...
			generateRegRegInstruction(
			        SBB4RegReg, root, firstRegister->getHighOrder(), secondRegister->getHighOrder(), _cg);
		}
		targetRegister = firstRegister;
	} else // assert getSubReg1Mem2() == true
	{
		 ...
		 * bytecode that shifts by 32 bits. This is the only case where the useSecondHighOrder bit is set. When
		 * the first child of the lushr is in a register, code above handles the shift. When the first child of
		 * the lushr is in memory, the below ensures that the upper part of the first child of the lushr is used
		 * as lowMR.
		 */
		if (useSecondHighOrder)
			lowMR = generateX86MemoryReference(*lowMR, 4, _cg);
