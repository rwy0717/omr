...
{
	return (node->getOpCodeValue() == TR::a2i || node->getOpCodeValue() == TR::a2l)
	        && (node->getReferenceCount() == 1);
}

void
TR_X86CompareAnalyser::integerCompareAnalyser(TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
        ...
        TR_X86OpCodes memRegOpCode)
{
	...
	if (getCmpReg1Reg2()) {
		...
		TR::X86RegMemInstruction *inst =
		        generateRegMemInstruction(regMemOpCode, root, firstRegister, tempMR, cg());
	...
	} else // Must be CmpMem1Reg2
	{
		...
		TR::X86MemRegInstruction *inst =
		        generateMemRegInstruction(memRegOpCode, root, tempMR, secondRegister, cg());
		if (!cg()->getImplicitExceptionPoint())
			cg()->setImplicitExceptionPoint(inst);
		tempMR->decNodeReferenceCounts(cg());
	}
...
void
TR_X86CompareAnalyser::longOrderedCompareAndBranchAnalyser(TR::Node *root, TR_X86OpCodes lowBranchOpCode,
        TR_X86OpCodes highBranchOpCode, TR_X86OpCodes highReversedBranchOpCode)
{
			...
			if (firstOp == TR::su2l || firstOp == TR::bu2l) {
				...
				if (firstOp == TR::lushr) {
					...
					/*
					 ...
					 Apparently, it is possible for the first register (and, probably, the second
					 one, also) to be NULL. For java code below if (end != 0
					     && logEnd >>> 32)
					     < end >>> 32))
					     {
					     logEnd = end;
					...
					*/
					// TR_ASSERT(firstRegister != NULL,"Can the first register be NULL in this
					// case?");
		...
		if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
			if (secondOp == TR::su2l || secondOp == TR::bu2l) {
				...
				if (secondOp == TR::lushr) {
					...
					// TR_ASSERT(secondRegister != NULL,"Can the second register be NULL in this
					// case?");
	...
	if (getCmpReg1Reg2()) {
		...
		if (firstHighZero) {
			if (secondHighZero == false) // if both are ui2l, then we just need an unsigned low order
			                             // compare
			{
...
void
TR_X86CompareAnalyser::longEqualityCompareAndBranchAnalyser(TR::Node *root, TR::LabelSymbol *firstBranchLabel,
        TR::LabelSymbol *secondBranchLabel, TR_X86OpCodes secondBranchOp)
{
	...
	if (getCmpReg1Reg2()) {
		generateRegRegInstruction(
		        CMP4RegReg, root, firstRegister->getLowOrder(), secondRegister->getLowOrder(), cg());
		if (deps != NULL) {
			deps->addPostCondition(firstRegister->getHighOrder(), TR::RealRegister::NoReg, cg());
...
TR::Register *
TR_X86CompareAnalyser::longEqualityBooleanAnalyser(TR::Node *root, TR_X86OpCodes setOpCode, TR_X86OpCodes combineOpCode)
{
	TR::Node *firstChild = root->getFirstChild();
	TR::Node *secondChild = root->getSecondChild();
	TR::Register *firstRegister = firstChild->getRegister();
...
TR::Register *
TR_X86CompareAnalyser::longOrderedBooleanAnalyser(
