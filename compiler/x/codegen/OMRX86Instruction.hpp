...
extern void insertUnresolvedReferenceInstructionMemoryBarrier(TR::CodeGenerator *cg, int32_t barrier,
        TR::Instruction *inst, TR::MemoryReference *mr, TR::Register *srcReg = NULL,
...
public:
	X86PaddingInstruction(uint8_t length, TR::Node *node, TR::CodeGenerator *cg)
	        : TR::Instruction(node, BADIA32Op, cg), _length(length), _properties(TR_NoOpPadding)
	{}

	X86PaddingInstruction(uint8_t length, TR_PaddingProperties properties, TR::Node *node, TR::CodeGenerator *cg)
	        : TR::Instruction(node, BADIA32Op, cg), _length(length), _properties(properties)
	{}

	X86PaddingInstruction(TR::Instruction *precedingInstruction, uint8_t length, TR::CodeGenerator *cg)
	        : TR::Instruction(BADIA32Op, precedingInstruction, cg), _length(length), _properties(TR_NoOpPadding)
	{}

	X86PaddingInstruction(TR::Instruction *precedingInstruction, uint8_t length, TR_PaddingProperties properties,
	        TR::CodeGenerator *cg)
	        : TR::Instruction(BADIA32Op, precedingInstruction, cg), _length(length), _properties(properties)
	{}
...
public:
	...
	X86PaddingSnippetInstruction(
	        uint8_t length, TR_PaddingProperties properties, TR::Node *node, TR::CodeGenerator *cg)
	        : TR::X86PaddingInstruction(length, properties, node, cg), _unresolvedSnippet(NULL)
	{}

	X86PaddingSnippetInstruction(TR::Instruction *precedingInstruction, uint8_t length, TR::CodeGenerator *cg)
	        : TR::X86PaddingInstruction(precedingInstruction, length, cg), _unresolvedSnippet(NULL)
	{}

	X86PaddingSnippetInstruction(TR::Instruction *precedingInstruction, uint8_t length,
	        TR_PaddingProperties properties, TR::CodeGenerator *cg)
	        : TR::X86PaddingInstruction(precedingInstruction, length, properties, cg), _unresolvedSnippet(NULL)
	{}
...
public:
	X86BoundaryAvoidanceInstruction(const TR_AtomicRegion *atomicRegions, uint8_t boundarySpacing,
...
public:
	...
	X86PatchableCodeAlignmentInstruction(const TR_AtomicRegion *atomicRegions, TR::Instruction *patchableCode,
	        ...
	        : TR::X86BoundaryAvoidanceInstruction(sizeOfProtectiveNop, atomicRegions,
	                cg->getInstructionPatchAlignmentBoundary(), cg->getInstructionPatchAlignmentBoundary(),
	                patchableCode, cg)
	{}
...
public:
	X86LabelInstruction(
	        TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym, TR::CodeGenerator *cg, bool b = false);
...
public:
	...
	X86AlignmentInstruction(TR::Node *node, uint8_t boundary, TR::CodeGenerator *cg)
	        : TR::Instruction(node, BADIA32Op, cg), _boundary(boundary), _margin(0), _minPaddingLength(0)
	{}

	X86AlignmentInstruction(
	        TR::Node *node, uint8_t boundary, TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
	        : TR::Instruction(cond, node, BADIA32Op, cg), _boundary(boundary), _margin(0), _minPaddingLength(0)
	{}

	X86AlignmentInstruction(TR::Node *node, uint8_t boundary, uint8_t margin, TR::CodeGenerator *cg)
	        : TR::Instruction(node, BADIA32Op, cg), _boundary(boundary), _margin(margin), _minPaddingLength(0)
	{}
	...
	X86AlignmentInstruction(TR::Node *node, uint8_t boundary, uint8_t margin,
	        ...
	        : TR::Instruction(cond, node, BADIA32Op, cg), _boundary(boundary), _margin(margin), _minPaddingLength(0)
	{}
...
public:
	...
	X86FenceInstruction(
	        TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Node *n, TR::CodeGenerator *cg);
...
class X86ImmInstruction : public TR::Instruction {
	...
	int32_t _adjustsFramePointerBy; // TODO: Rename me.  Calls don't adjust the VFP per se; they adjust the stack
	                                // pointer
	int32_t _reloKind;

public:
	X86ImmInstruction(TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
	        : TR::Instruction(node, op, cg), _sourceImmediate(0), _adjustsFramePointerBy(0), _reloKind(reloKind)
	{}

	X86ImmInstruction(int32_t imm, TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg,
	...
	X86ImmInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm, TR::CodeGenerator *cg,
	        int32_t reloKind = TR_NoRelocation);
...
public:
	...
	X86ImmSymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, int32_t imm,
...
public:
		...
		if (cg->enableRematerialisation() && reg->isDiscardable() && getOpCode().modifiesTarget()) {
			...
			if (debug("dumpRemat")) {
				diagnostic("---> Clobbering %s discardable register %s at instruction %p\n",
				        reg->getRematerializationInfo()->toString(comp), reg->getRegisterName(comp),
				        this);
			}
		}
	}
	...
	X86RegInstruction(
	        TR::Register *reg, TR_X86OpCodes op, TR::Instruction *precedingInstruction, TR::CodeGenerator *cg)
...
public:
	X86RegImmSymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg, int32_t imm,
	        TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
public:
	X86RegRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg, int32_t imm,
	        TR::CodeGenerator *cg);
...
public:
	...
	X86RegRegRegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
	        TR::Register *slreg, TR::Register *srreg, TR::RegisterDependencyConditions *cond,
	        TR::CodeGenerator *cg);
...
public:
	...
	X86MemInstruction(TR::MemoryReference *mr, TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg,
	...
	X86MemInstruction(TR::RegisterDependencyConditions *cond, TR::MemoryReference *mr, TR::Node *node,
	        TR_X86OpCodes op, TR::CodeGenerator *cg, TR::Register *srcReg = NULL)
	...
	X86MemInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, TR::CodeGenerator *cg,
...
public:
	...
	TR::LabelRelocation *getRelocation(ncount_t index)
	{
		TR_ASSERT(0 <= index && index < _numRelocations,
		        "X86MemTableInstruction::getRelocation - index %d is out of range (0-%d)", index,
		        _numRelocations);
...
class X86CallMemInstruction : public TR::X86MemInstruction {
	int32_t _adjustsFramePointerBy; // TODO: Rename me.  Calls don't affect the VFP per se; they affect the stack
	                                // pointer
...
public:
	X86MemImmInstruction(int32_t imm, TR::MemoryReference *mr, TR::Node *node, TR_X86OpCodes op,
	        TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
	...
	X86MemImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, int32_t imm,
	        TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation);

	X86MemImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::MemoryReference *mr,
...
public:
	X86MemRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, TR::Register *sreg,
...
public:
	X86RegMemImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::MemoryReference *mr,
	        int32_t imm, TR::CodeGenerator *cg);
...
public:
	AMD64RegImm64Instruction(uint64_t imm, TR::Register *treg, TR::Node *node, TR_X86OpCodes op,
	        TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
	        : TR::X86RegInstruction(treg, node, op, cg), _sourceImmediate(imm), _reloKind(reloKind)
	{}
	...
	AMD64RegImm64Instruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
...
public:
	...
	AMD64Imm64Instruction(
	        uint64_t imm, TR_X86OpCodes op, TR::Instruction *precedingInstruction, TR::CodeGenerator *cg)
	        : TR::Instruction(op, precedingInstruction, cg), _sourceImmediate(imm)
	{}

	AMD64Imm64Instruction(TR::RegisterDependencyConditions *cond, uint64_t imm, TR::Node *node, TR_X86OpCodes op,
	        TR::CodeGenerator *cg)
	...
	AMD64Imm64Instruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm, TR::RegisterDependencyConditions *cond,
	        TR::CodeGenerator *cg)
	        : TR::Instruction(cond, node, op, cg), _sourceImmediate(imm)
	{}
...
public:
	...
	X86VFPSaveInstruction(TR::Node *node, TR::CodeGenerator *cg) : TR::Instruction(node, AdjustFramePtr, cg) {}
...
public:
	X86VFPRestoreInstruction(TR::Instruction *precedingInstruction, TR::X86VFPSaveInstruction *saveInstruction,
	        TR::CodeGenerator *cg)
	        : _saveInstruction(saveInstruction), TR::Instruction(AdjustFramePtr, precedingInstruction, cg)
	{}

	X86VFPRestoreInstruction(TR::X86VFPSaveInstruction *saveInstruction, TR::Node *node, TR::CodeGenerator *cg)
	        : _saveInstruction(saveInstruction), TR::Instruction(node, AdjustFramePtr, cg)
	{}
...
public:
	X86VFPReleaseInstruction(TR::Instruction *precedingInstruction,
	        ...
	        : _dedicateInstruction(dedicateInstruction), TR::Instruction(AdjustFramePtr, precedingInstruction, cg)
	{}

	X86VFPReleaseInstruction(
	        TR::X86VFPDedicateInstruction *dedicateInstruction, TR::Node *node, TR::CodeGenerator *cg)
	        : _dedicateInstruction(dedicateInstruction), TR::Instruction(node, AdjustFramePtr, cg)
	{}
...
public:
	X86VFPCallCleanupInstruction(TR::Instruction *precedingInstruction, int32_t adjustment, TR::CodeGenerator *cg)
	        : _stackPointerAdjustment(adjustment), TR::Instruction(AdjustFramePtr, precedingInstruction, cg)
	{}

	X86VFPCallCleanupInstruction(int32_t adjustment, TR::Node *node, TR::CodeGenerator *cg)
	        : _stackPointerAdjustment(adjustment), TR::Instruction(node, AdjustFramePtr, cg)
	{}
...
TR::X86RegInstruction *generateRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg);
...
TR::X86RegImmInstruction *generateRegImmInstruction(TR::Instruction *, TR_X86OpCodes op, TR::Register *reg1,
        int32_t imm, TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation);
...
TR::X86MemImmSymInstruction *generateMemImmSymInstruction(TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr,
        int32_t imm, TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
TR::X86LabelInstruction *generateLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg);
...
TR::X86LabelInstruction *generateLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
        TR::Node *glRegDep, List<TR::Register> *popRegs, bool evaluateGlRegDeps, TR::CodeGenerator *cg);
inline TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym, TR::Node *glRegDep,
...
TR::X86LabelInstruction *generateLongLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg);
...
TR::X86VirtualGuardNOPInstruction *generateVirtualGuardNOPInstruction(TR::Instruction *i, TR::Node *,
        TR_VirtualGuardSite *site, TR::RegisterDependencyConditions *deps, TR::LabelSymbol *symbol,
        TR::CodeGenerator *cg);
#endif
...
TR::X86RegRegInstruction *generateRegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
...
TR::X86RegRegRegInstruction *generateRegRegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
        TR::Register *reg2, TR::Register *reg3, TR::CodeGenerator *cg);
...
TR::X86CallMemInstruction *generateCallMemInstruction(TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr,
        TR::RegisterDependencyConditions *, TR::CodeGenerator *cg);
...
TR::AMD64RegImm64SymInstruction *generateRegImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
        uint64_t imm, TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
TR::X86FPST0STiRegRegInstruction *generateFPST0STiRegRegInstruction(
        TR_X86OpCodes op, TR::Node *, TR::Register *reg1, TR::Register *reg2, TR::CodeGenerator *cg);
TR::X86FPSTiST0RegRegInstruction *generateFPSTiST0RegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
        TR::Register *reg2, TR::CodeGenerator *cg, bool forcePop = false);
