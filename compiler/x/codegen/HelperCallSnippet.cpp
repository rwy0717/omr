...
void
TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t *buffer, TR::Node *child)
{
...
TR_Debug::printBody(TR::FILE *pOutFile, TR::X86HelperCallSnippet *snippet, uint8_t *bufferPos)
{
	...
	if (snippet->getStackPointerAdjustment() != 0) {
		...
		trfprintf(pOutFile, "add \t%s, %d\t\t\t%s Temporarily deallocate stack frame",
		        TR::Compiler->target.is64Bit() ? "rsp" : "esp", snippet->getStackPointerAdjustment(),
		        commentString());
		bufferPos += size;
	}
	...
	if (snippet->getCallNode()) {
		if (snippet->getOffset() != -1) {
			...
			bool useDedicatedFrameReg =
			        _comp->cg()->getLinkage()->getProperties().getAlwaysDedicateFramePointerRegister();
			...
			trfprintf(pOutFile, "push\t[%s +%d]\t%s Address of Receiver",
			        useDedicatedFrameReg ? "ebx" : "esp", snippet->getOffset(), commentString());
		...
		for (; i < snippet->getCallNode()->getNumChildren(); i++) {
			...
			if (child->getOpCodeValue() == TR::loadaddr && !child->getRegister()) {
				...
				TR_ASSERT(deps,
				        "null dependencies on restart label of helper call snippet with register args");
				print(pOutFile,
...
TR::X86HelperCallSnippet::getLength(int32_t estimatedSnippetStart)
{
	...
	if (_callNode) {
		...
		if (_offset != -1) {
			bool useDedicatedFrameReg =
			        cg()->getLinkage()->getProperties().getAlwaysDedicateFramePointerRegister();
			if (_offset >= -128 && _offset <= 127)
