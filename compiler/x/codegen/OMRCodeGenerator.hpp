...
public:
	...
	ClobberingInstruction(TR_Memory *m)
	        : _instruction(0), _clobberedRegisters(getTypedAllocator<TR::Register *>(TR::comp()->allocator())){};
	ClobberingInstruction(TR::Instruction *instr, TR_Memory *m)
	        : _instruction(instr), _clobberedRegisters(getTypedAllocator<TR::Register *>(TR::comp()->allocator()))
	{}
...
protected:
	...
	static uint8_t intelMultiBytePaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
...
private:
	...
	std::list<TR::ClobberingInstruction *,
	        TR::typed_allocator<TR::ClobberingInstruction *, TR::Allocator> >::iterator _clobIterator;
	...
	enum TR_X86CodeGeneratorFlags {
		...
		EnableSinglePrecisionMethods = 0x00000008, ///< support changing FPCW to single precision for individual
		                                           ///< methods
		EnableRegisterInterferences = 0x00000010, ///< consider register interferences during register
		                                          ///< assignment
		...
		EnableImplicitDivideCheck = 0x00000100, ///< platform can catch hardware exceptions for divide overflow
		                                        ///< and divide by zero
		GenerateMasmListingSyntax = 0x00000200, ///< generate Masm-style syntax in the debug listings
		MapAutosTo8ByteSlots = 0x00000400, ///< don't round up sizes of autos to an 8-byte slot size when the
		                                   ///< stack is mapped
		EnableTLHPrefetching = 0x00000800, ///< enable software prefetches on TLH allocates
		// Available                             = 0x00001000,
		// Available                             = 0x00002000,
		TargetSupportsSoftwarePrefetches = 0x00004000, ///< target processor and OS both support software
		                                               ///< prefetch instructions
		MethodEnterExitTracingEnabled = 0x00008000, ///< trace method enter/exits
		// Available                             = 0x00010000,
		PushPreservedRegisters = 0x00020000 ///< we've chosen to save/restore preserved regs using push/pop
