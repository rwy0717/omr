    ...
    TR::Register* secondRegister)
{
    ...
    if (getPackage() == kFADDpackage || getPackage() == kDADDpackage || getPackage() == kFMULpackage || getPackage() == kDMULpackage) {
        setCommutative();
    }
}
...
void TR_X86XMMBinaryArithmeticAnalyser::genericXMMAnalyser(TR::Node* root)
{
    ...
    //
    if (targetRegister && targetRegister->getKind() == TR_X87 && sourceRegister && sourceRegister->getKind() == TR_X87) {
        TR_X86FPBinaryArithmeticAnalyser temp(root, _codeGen);
        temp.genericFPAnalyser(root);
        return;
    }

    if (isReversed() || (isCommutative() && canUseRegMemOp() && targetRegister && targetRegister->getKind() == TR_X87 && targetChild->getReferenceCount() == 1 && sourceRegister && sourceRegister->getKind() == TR_FPR)) {
        ...
        if (mustUseRegRegOp()) {
            ...
            if (targetRegister && targetRegister->getKind() == TR_X87 && !targetChild->getOpCode().isMemoryReference()) {
        ...
        if (mustUseRegRegOp()) {
            ...
            if (sourceRegister && sourceRegister->getKind() == TR_X87 && !sourceChild->getOpCode().isMemoryReference()) {
