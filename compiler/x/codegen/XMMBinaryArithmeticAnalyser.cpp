    ...
    TR::Node* firstChild, TR::Register* firstRegister, TR::Node* secondChild, TR::Register* secondRegister)
{
    ...
    if (firstChild->getReferenceCount() == 1) {
        ...
        if (!(_codeGen->useSSEForSinglePrecision() && _codeGen->useSSEForDoublePrecision())
    ...
    if (getPackage() == kFADDpackage || getPackage() == kDADDpackage || getPackage() == kFMULpackage
        || getPackage() == kDMULpackage) {
        setCommutative();
    }
}
...
{
    TR::Node *targetChild = root->getFirstChild(), *sourceChild = root->getSecondChild();
    TR::Register *targetRegister = targetChild->getRegister(), *sourceRegister = sourceChild->getRegister(), *tempReg;
    ...
    if (isReversed()
        || (isCommutative() && canUseRegMemOp() && targetRegister && targetRegister->getKind() == TR_X87
               && targetChild->getReferenceCount() == 1 && sourceRegister && sourceRegister->getKind() == TR_FPR)) {
