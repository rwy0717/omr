...
static TR_RematerializationInfo *generateRematerializationInfo(TR::Node *node, intptrj_t constant,
        TR_RematerializableTypes type, TR::Instruction *instr, TR::CodeGenerator *cg);
...
//
void
setDiscardableIfPossible(TR_RematerializableTypes type, TR::Register *candidate, TR::Node *node, TR::Instruction *instr,
		...
		if (info->isIndirect()) {
			...
			if (debug("dumpRemat")) {
				diagnostic("---> Creating %s discardable register %s (type %d) "
				           "for node [%p] at instruction %p in %s\n",
				        info->toString(comp), candidate->getRegisterName(comp), type, node, instr,
				        comp->signature());
			}
		}
	}
...
void
setDiscardableIfPossible(TR_RematerializableTypes type, TR::Register *candidate, TR::Node *node, TR::Instruction *instr,
        intptrj_t constant, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
...
generateRematerializationInfo(TR::Node *node, TR::MemoryReference *mr, TR_RematerializableTypes type,
...
static TR_RematerializationInfo *
generateRematerializationInfo(TR::Node *node, intptrj_t constant, TR_RematerializableTypes type, TR::Instruction *instr,
        TR::CodeGenerator *cg)
{
...
static TR_RematerializationInfo *
generateRematerializationInfo(TR::Node *node, TR::SymbolReference *symRef, TR_RematerializableTypes type,
        TR::Instruction *instr, TR::CodeGenerator *cg)
{
	TR::Symbol *sym = symRef->getSymbol();
