...
TR::X86DataSnippet::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (_isClassAddress) {
		...
		if (needRelocation && !cg()->comp()->compileRelocatableCode()) {
			cg()->addExternalRelocation(new (TR::comp()->trHeapMemory()) TR::ExternalRelocation(
		...
		if (clazz && cg()->comp()->compileRelocatableCode()
			...
			cg()->addExternalRelocation(
			        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)clazz,
...
TR::X86DataSnippet::printValue(TR::FILE *pOutFile, TR_Debug *debug)
{
	...
	case 8:
		trfprintf(pOutFile, "0x%016llx | %lld | double %g", getData<int64_t>(), getData<int64_t>(),
		        getData<double>());
		break;
	default: trfprintf(pOutFile, "VECTOR VALUE"); break;
	}
}
...
TR::X86DataSnippet::print(TR::FILE *pOutFile, TR_Debug *debug)
{
	...
	const char *toString;
	switch (getDataSize()) {
	case 8: toString = dqString(); break;
	case 4: toString = ddString(); break;
	case 2: toString = dwString(); break;
	default: toString = dbString(); break;
	}
