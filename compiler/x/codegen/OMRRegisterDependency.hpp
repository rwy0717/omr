...
public:
	...
	void orderGlobalRegsOnFPStack(TR::Instruction *cursor, TR_RegisterKinds kindsToBeAssigned,
	        TR_X86RegisterDependencyIndex numberOfRegisters, List<TR::Register> *poppedRegisters,
	        TR::CodeGenerator *cg);
...
class RegisterDependencyConditions : public OMR::RegisterDependencyConditions {
	...
	TR_X86RegisterDependencyIndex unionDependencies(TR_X86RegisterDependencyGroup *deps,
	        TR_X86RegisterDependencyIndex cursor, TR::Register *vr, TR::RealRegister::RegNum rr,
...
public:
	...
	TR::RegisterDependencyConditions *clone(
	        TR::CodeGenerator *cg, TR_X86RegisterDependencyIndex additionalRegDeps = 0);
	        ...
	        uint8_t flag = UsesDependentRegister, bool isAssocRegDependency = false)
	{
		...
		if (newCursor == _addCursorForPre)
			_numPreConditions--; // A vmThread/ebp dependency was displaced, so there is now one less
			                     // condition.
		else
	        ...
	        uint8_t flag = UsesDependentRegister, bool isAssocRegDependency = false)
	{
		TR_X86RegisterDependencyIndex newCursor = unionRealDependencies(
		        _postConditions, _addCursorForPost, vr, rr, cg, flag, isAssocRegDependency);
		TR_ASSERT(newCursor <= _numPostConditions, "Too many dependencies");
		if (newCursor == _addCursorForPost)
			_numPostConditions--; // A vmThread/ebp dependency was displaced, so there is now one less
			                      // condition.
		else
