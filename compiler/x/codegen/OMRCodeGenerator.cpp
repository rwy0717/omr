...
TR_X86ProcessorInfo::initialize()
{
	...
	if (isGenuineIntel()) {
		...
		case 0x06: {
			...
			case 0x55: _processorDescription |= TR_ProcessorIntelSkylake; break;
			case 0x4f: _processorDescription |= TR_ProcessorIntelBroadwell; break;
			case 0x3f:
			case 0x3c: _processorDescription |= TR_ProcessorIntelHaswell; break;
			case 0x3e:
			case 0x3a: _processorDescription |= TR_ProcessorIntelIvyBridge; break;
			case 0x2a:
...
OMR::X86::CodeGenerator::initialize(TR::Compilation *comp)
{
	...
	if (_targetProcessorInfo.supportsTM() && !comp->getOption(TR_DisableTM)) {
		/**
		 * Due to many verions of Haswell and a small number of Broadwell have defects for TM and then disabled
		 * by Intel, we will return false for any versions before Broadwell.
		 *
   ...
   if (TR::Compiler->target.is64Bit())
	...
	if (comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRARegisterStates)) {
		...
		self()->setFPRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(
		        self()->machine(), TR::RealRegister::FirstXMMR, TR::RealRegister::LastXMMR));
	}
	...
	if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
		generateMemInstruction(LDCWMem, startNode,
		        generateX86MemoryReference(
		                self()->findOrCreate2ByteConstant(startNode, SINGLE_PRECISION_ROUND_TO_NEAREST),
		                self()),
		        self());
	}
}
...
{
	return TR::RealRegister::LastAssignableGPR - TR::RealRegister::FirstGPR + 1; // TODO:AMD64: This is including
	                                                                             // rsp
}
 ...
 */
static bool
willNotInlineCompareAndSwapNative(TR::Node *node, int8_t size, TR::Compilation *comp)
{
#ifdef J9_PROJECT_SPECIFIC
 ...
 */
bool
OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node *node, TR::Compilation *comp)
{
	TR::SymbolReference *callSymRef = node->getSymbolReference();
	TR::MethodSymbol *methodSymbol = callSymRef->getSymbol()->castToMethodSymbol();
...
void
OMR::X86::CodeGenerator::clobberLiveDiscardableRegisters(TR::Instruction *instr, TR::MemoryReference *mr)
{
		...
		while (iterator != self()->getLiveDiscardableRegisters().end()) {
			...
			if (registerCursor->getRematerializationInfo()->isRematerializableFromMemory()) {
				...
				if ((rmSymRef->getSymbol() == symbol)
					...
					if (debug("dumpRemat")) {
						diagnostic("---> Clobbering %s discardable register %s at instruction "
						           "%p in %s\n",
				...
				// If the symbols are different but will share the same stack slot then a clobber has
				// occurred.
				//
	...
	while (!worklist.isEmpty()) {
		...
		for (auto iterator = self()->getLiveDiscardableRegisters().begin();
			...
			if (info->isIndirect() && info->getBaseRegister() == baseReg) {
				...
				if (debug("dumpRemat")) {
					diagnostic("---> Clobbering %s discardable register %s at instruction %p in "
					           "%s\n",
...
//
void
OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(TR::Register *baseReg)
{
	TR_Stack<TR::Register *> worklist(self()->trMemory());
	worklist.push(baseReg);
...
//
void
OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(TR::Register *baseReg)
{
	TR_Stack<TR::Register *> worklist(self()->trMemory());
	worklist.push(baseReg);
...
OMR::X86::CodeGenerator::supportsNonHelper(TR::SymbolReferenceTable::CommonNonhelperSymbol symbol)
{
	...
	case TR::SymbolReferenceTable::atomicCompareAndSwapReturnValueSymbol: return true;
	default: return false;
	}
}
...
void
OMR::X86::CodeGenerator::saveBetterSpillPlacements(TR::Instruction *branchInstruction)
{
...
void
OMR::X86::CodeGenerator::removeBetterSpillPlacementCandidate(TR::RealRegister *realReg)
{
	...
	for (info = _betterSpillPlacements, next = NULL; info; info = next) {
		...
		if (info->_freeRealRegs == 0) {
			...
			self()->traceRegisterAssignment(
			        "%R is no longer a candidate for better spill placement.", info->_virtReg);
		}
	}
}

TR::Instruction *
OMR::X86::CodeGenerator::findBetterSpillPlacement(TR::Register *virtReg, int32_t realRegNum)
{
        ...
        TR::X86LabelInstruction *branchInstruction, TR_RegisterKinds kindsToBeAssigned)
{
	...
	if (deps) {
		TR::Instruction *ins = generateLabelInstruction(
		        oi->getFirstInstruction(), LABEL, generateLabelSymbol(self()), deps, self());
	...
	//
	TR_ASSERT(
	        !oi->hasBeenRegisterAssigned(), "outlined instructions should not have been register assigned already");
...
void
OMR::X86::CodeGenerator::prepareForNonLinearRegisterAssignmentAtMerge(TR::X86LabelInstruction *mergeInstruction)
{
	TR::Machine *xm = self()->machine();
	...
	while (clobInstructionCursor && (clobInstructionCursor->getInstruction() == instructionCursor)
		...
		while (regIterator != clobInstructionCursor->getClobberedRegisters().end()) {
			...
			if (!info->isIndirect() || info->getBaseRegister()->getAssignedRegister()) {
				...
				if (debug("dumpRemat")) {
					diagnostic("---> Activating %s discardable register %s at instruction %p in "
					           "%s\n",
					        self()->getDebug()->toString(info),
...
#endif
			{
				// It's safe to protect just the first 2 bytes because we won't patch anything other
				// than a 2-byte jmp
...
#define OVER_ESTIMATION 4
	...
	while (cursorInstruction) {
		...
		self()->setBinaryBufferCursor(cursorInstruction->generateBinaryEncoding());
		TR_ASSERT(cursorInstruction->getEstimatedBinaryLength()
		                >= self()->getBinaryBufferCursor() - instructionStart,
		        "Instruction length estimate must be conservatively large (instr=%s, opcode=%s, estimate=%d, "
		        "actual=%d",
		        self()->getDebug() ? self()->getDebug()->getName(cursorInstruction) : "(unknown)",
		        self()->getDebug() ? self()->getDebug()->getOpCodeName(&cursorInstruction->getOpCode())
		                           : "(unknown)",
	...
	if (self()->comp()->getOption(TR_EnableGCRPatching) && self()->comp()->getRecompilationInfo()
		...
		if (!addrToPatch) {
			...
			self()->comp()->failCompilation<TR::GCRPatchFailure>("Must have updated gcrPatchPointSymbol "
			                                                     "with the correct address by now");
		}
	}
#endif
...
TR::Register *
OMR::X86::CodeGenerator::gprClobberEvaluate(TR::Node *node, TR_X86OpCodes movRegRegOpCode)
{
	...
	if (self()->comp()->getOption(TR_TraceCG) && sourceRegister->needsLazyClobbering())
		traceMsg(self()->comp(), "LAZY CLOBBERING: node %s register %s refcount=%d canClobber=%s\n",
		        self()->getDebug()->getName(node), self()->getDebug()->getName(sourceRegister),
...
OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node *node)
{
	...
	TR_ASSERT(
	        !node->getOpCode().isRef() || TR::Compiler->target.is32Bit() || self()->comp()->useCompressedPointers(),
...
OMR::X86::CodeGenerator::findOutlinedInstructionsFromLabel(TR::LabelSymbol *label)
{
...
TR_OutlinedInstructions *
OMR::X86::CodeGenerator::findOutlinedInstructionsFromMergeLabel(TR::LabelSymbol *label)
{
	auto oiIterator = self()->getOutlinedInstructionsList().begin();
	while (oiIterator != self()->getOutlinedInstructionsList().end()) {
...
OMR::X86::CodeGenerator::findOrCreateConstantDataSnippet(TR::Node *n, void *c, size_t s)
{
...
int32_t
OMR::X86::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart)
{
...
bool
OMR::X86::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate *rc, TR::Node *node)
{
...
void
OMR::X86::CodeGenerator::apply32BitLoadLabelRelativeRelocation(TR::Instruction *movInstruction,
        TR::LabelSymbol *startLabel, TR::LabelSymbol *endLabel, int32_t deltaToStartLabel)
{
	...
	cursor += movInstructionX86->getOpCode().length(movInstructionX86->rexBits())
...
void
OMR::X86::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
...
bool
OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(TR::Node *node, TR::Node *parent, TR_RegisterPressureState *state)
{
...
uint8_t
OMR::X86::CodeGenerator::nodeResultGPRCount(TR::Node *node, TR_RegisterPressureState *state)
{
	...
	if (node->getOpCode().isLoadConst()
	        ...
	        && !(self()->simulatedNodeState(node, state)._keepLiveUntil != NULL // Check if parent will become a
	                                                                            // RegStore that keeps this node
	                                                                            // live
...
uint8_t OMR::X86::CodeGenerator::old32BitPaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                                        [PADDING_TABLE_MAX_ENCODING_LENGTH] = {
                                                                ...
                                                                {0x8d, 0x04, 0x20}, // 3: lea exx, [exx]           (with
                                                                                    // SIB)
                                                                {0x8d, 0x44, 0x20, 0x00}, // 4: lea exx, [exx+00] (with
                                                                                          // SIB)
                                                                {0x3e, 0x8d, 0x44, 0x20, 0x00}, // 5: lea exx,
                                                                                                // DS:[exx+00]     (with
                                                                                                // SIB) (with prefix)
                                                                {0x8d, 0x80, 0x00, 0x00, 0x00, 0x00}, // 6: lea exx,
                                                                                                      // [exx+00000000]
...
uint8_t OMR::X86::CodeGenerator::k8PaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                                  [PADDING_TABLE_MAX_ENCODING_LENGTH] = {
                                                          {0x90},
                                                          {0x66, 0x90},
                                                          {0x66, 0x66, 0x90},
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable = TR_X86PaddingTable(7,
        (uint8_t ***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding, TR_X86PaddingTable::registerMatters, 0x8b,
        0x20);
...
uint8_t *
OMR::X86::CodeGenerator::generatePadding(uint8_t *cursor, intptrj_t length, TR::Instruction *neighborhood,
        TR_PaddingProperties properties, bool recursive)
{
	...
	if (length <= _paddingTable->_biggestEncoding) {
		...
		const intptrj_t jmpThreshold = 100; // Beyond this length, a jmp is faster than a sequence of no-op
		                                    // instructions
	...
	if (!recursive && self()->comp()->getOptions()->enableDebugCounters()) {
		if (neighborhood) {
			...
			if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
				...
				switch (vg->getKind()) {
				case TR_NoGuard: guardKind = "NoGuard"; break;
				case TR_ProfiledGuard: guardKind = "ProfiledGuard"; break;
				case TR_InterfaceGuard: guardKind = "InterfaceGuard"; break;
				case TR_AbstractGuard: guardKind = "AbstractGuard"; break;
				case TR_HierarchyGuard: guardKind = "HierarchyGuard"; break;
				case TR_NonoverriddenGuard: guardKind = "NonoverriddenGuard"; break;
				case TR_SideEffectGuard: guardKind = "SideEffectGuard"; break;
				case TR_DummyGuard: guardKind = "DummyGuard"; break;
				case TR_HCRGuard: guardKind = "HCRGuard"; break;
				case TR_MutableCallSiteTargetGuard: guardKind = "MutableCallSiteTargetGuard"; break;
				case TR_MethodEnterExitGuard: guardKind = "MethodEnterExitGuard"; break;
				case TR_DirectMethodGuard: guardKind = "DirectMethodGuard"; break;
				case TR_InnerGuard: guardKind = "InnerGuard"; break;
				case TR_ArrayStoreCheckGuard: guardKind = "ArrayStoreCheckGuard"; break;
				default: guardKind = "Unknown"; break;
				}
				TR::DebugCounter::incStaticDebugCounter(self()->comp(),
				        TR::DebugCounter::debugCounterName(self()->comp(), "nopCount/%d/%s/%s",
				                blockFrequency, neighborhood->description(), guardKind));
				if (length > 0) {
					TR::DebugCounter::incStaticDebugCounter(self()->comp(),
					        TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s/%s",
					                blockFrequency, neighborhood->description(), guardKind));
					...
					for (TR::Instruction *ninst = neighborhood->getNext(); ninst;
						...
						if (ninst->isVirtualGuardNOPInstruction()) {
							...
							TR::DebugCounter::incStaticDebugCounter(self()->comp(),
							        TR::DebugCounter::debugCounterName(self()->comp(),
							                "vgnopNoPatchReason/%d/vgnop", blockFrequency));
							break;
						}
						...
						if (ninst->getEstimatedBinaryLength() > 0) {
							TR::DebugCounter::incStaticDebugCounter(self()->comp(),
							        TR::DebugCounter::debugCounterName(self()->comp(),
							                "vgnopNoPatchReason/%d/%s_%s", blockFrequency,
				...
				TR::DebugCounter::incStaticDebugCounter(self()->comp(),
				        TR::DebugCounter::debugCounterName(self()->comp(), "nopCount/%d/%s",
				                blockFrequency, neighborhood->description()));
				if (length > 0)
					TR::DebugCounter::incStaticDebugCounter(self()->comp(),
					        TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s",
					                blockFrequency, neighborhood->description()));
			}
		} else {
...
bool
OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(
...
{
	if (delta == 1)
		return generateMemInstruction(cursor, INC4Mem,
...
TR::Instruction *
OMR::X86::CodeGenerator::generateDebugCounterBump(TR::Instruction *cursor, TR::DebugCounterBase *counter,
        TR::Register *deltaReg, TR::RegisterDependencyConditions *cond)
{
	if (deltaReg)
		return generateMemRegInstruction(cursor, ADD4MemReg,
		        generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()), self()), deltaReg,
		        self());
	else
		return cursor;
}
...
//
void
OMR::X86::CodeGenerator::dumpPreFPRegisterAssignment(TR::Instruction *instructionCursor)
{
...
void
OMR::X86::CodeGenerator::dumpPreGPRegisterAssignment(TR::Instruction *instructionCursor)
{
	if (self()->comp()->getOutFile() == NULL)
		return;
