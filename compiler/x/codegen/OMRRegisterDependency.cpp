...
OMR::X86::RegisterDependencyConditions::RegisterDependencyConditions(TR::Node *node, TR::CodeGenerator *cg,
        ...
        : _numPreConditions(-1), _numPostConditions(-1), _addCursorForPre(0), _addCursorForPost(0)
{
	...
	for (i = node->getNumChildren() - 1; i >= 0; i--) {
		...
		if (globalReg->getKind() == TR_GPR || globalReg->getKind() == TR_VRF || XMM_GPRS_USE_DISTINCT_NUMBERS) {
			...
			//
			realRegNum = (TR::RealRegister::RegNum)(cg->getGlobalRegister(globalRegNum)
			        - TR::RealRegister::FirstFPR + TR::RealRegister::FirstXMMR);

			// Find the global register that has been allocated in this XMMR, if any.
			//
			TR::Register *xmmGlobalReg =
			        cg->machine()->getXMMGlobalRegister(realRegNum - TR::RealRegister::FirstXMMR);
			if (xmmGlobalReg)
		...
		if (registers.find(globalReg)) {
			if (globalReg->getKind() == TR_GPR) {
				...
				// child->getGlobalRegisterNumber(), getDebug()->getName(globalReg));
				...
				for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
					...
					if ((reg == globalReg) && (i != fpRegNum)) {
						// dumpOptDetails("FP reg num %d global reg num %d global reg %s\n", i,
						// child->getGlobalRegisterNumber(), getDebug()->getName(copyReg));
			...
			if (child->getOpCodeValue() == TR::PassThrough && globalReg->getKind() == TR_X87) {
				...
				if ((fpChild->getDataType() == TR::Float) || (fpChild->getDataType() == TR::Double)) {
					...
					// if the child's register is live on another stack slot, dont change the
					// register.
...
OMR::X86::RegisterDependencyConditions::unionDependencies(TR_X86RegisterDependencyGroup *deps,
		...
		for (TR_X86RegisterDependencyIndex candidate = 0; candidate < cursor; candidate++) {
			...
			if (dep->getRegister() == vr) {
				...
				if (min == TR::RealRegister::NoReg) {
					...
					// TR_ASSERT(min == max, "Specific register dependency is only compatible with
					// itself");
					if (min != max)
						continue;
...
void
OMR::X86::RegisterDependencyConditions::unionNoRegPostCondition(TR::Register *reg, TR::CodeGenerator *cg)
{
	unionPostCondition(reg, TR::RealRegister::NoReg, cg);
}

TR_X86RegisterDependencyIndex
OMR::X86::RegisterDependencyConditions::unionRealDependencies(TR_X86RegisterDependencyGroup *deps,
        TR_X86RegisterDependencyIndex cursor, TR::Register *vr, TR::RealRegister::RegNum rr, TR::CodeGenerator *cg,
        uint8_t flag, bool isAssocRegDependency)
{
	TR::Machine *machine = cg->machine();
...
OMR::X86::RegisterDependencyConditions::clone(TR::CodeGenerator *cg, TR_X86RegisterDependencyIndex additionalRegDeps)
{
...
void
OMR::X86::RegisterDependencyConditions::useRegisters(TR::Instruction *instr, TR::CodeGenerator *cg)
{
	int32_t i;
...
void
TR_X86RegisterDependencyGroup::assignRegisters(TR::Instruction *currentInstruction, TR_RegisterKinds kindsToBeAssigned,
        TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg)
{
	...
	for (i = 0; i < numberOfRegisters; i++) {
		...
		if (virtReg && (kindsToBeAssigned & virtReg->getKindAsMask())
		        && dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::ByteReg
...
void
TR_X86RegisterDependencyGroup::setDependencyInfo(TR_X86RegisterDependencyIndex index, TR::Register *vr,
...
TR::RealRegister *
OMR::X86::RegisterDependencyConditions::getRealRegisterFromVirtual(TR::Register *virtReg, TR::CodeGenerator *cg)
{
	TR::Machine *machine = cg->machine();
...
void
TR_X86RegisterDependencyGroup::assignFPRegisters(TR::Instruction *prevInstruction, TR_RegisterKinds kindsToBeAssigned,
        TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg)
{
		...
		for (i = 0; i < numberOfRegisters; i++) {
			...
			if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
				if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
					...
					//
					if (_dependencies[i].getGlobalFPRegister()) {
						int32_t reqdStackHeight = _dependencies[i].getRealRegister()
			...
			if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
				if (_dependencies[i].getGlobalFPRegister()) {
					...
					if (assignedRegister) {
						int32_t reqdStackHeight = _dependencies[i].getRealRegister()
						        - TR::RealRegister::FirstFPR;
						int32_t currentStackHeight = machine->getFPTopOfStack()
...
//
void
TR_X86RegisterDependencyGroup::orderGlobalRegsOnFPStack(TR::Instruction *cursor, TR_RegisterKinds kindsToBeAssigned,
		...
		if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
			if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
				...
				if (_dependencies[i].getGlobalFPRegister()) {
					int32_t reqdStackHeight = _dependencies[i].getRealRegister()
					        - TR::RealRegister::FirstFPR;
					int32_t currentStackHeight = topOfStack
					        - assignedRegister->getFPStackRegisterNumber();
			...
			if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
				...
				if (_dependencies[i].getGlobalFPRegister()) {
					...
					while ((reqdStackHeight != currentStackHeight) && (reqdStackHeight >= 0)) {
						...
						assignedRegister =
						        toX86FPStackRegister(virtReg->getAssignedRealRegister());
						int32_t tempCurrentStackHeight = topOfStack
						        - assignedRegister->getFPStackRegisterNumber();
...
uint32_t
OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(TR::CodeGenerator *cg)
{
	TR::Machine *machine = cg->machine();
	uint32_t i, total = 0;
	TR::Register *reg;
...
uint32_t
OMR::X86::RegisterDependencyConditions::numReferencedGPRegisters(TR::CodeGenerator *cg)
{
	uint32_t i, total = 0;
	TR::Register *reg;
...
TR::RegisterDependencyConditions *
generateRegisterDependencyConditions(TR::Node *node, TR::CodeGenerator *cg,
        TR_X86RegisterDependencyIndex additionalRegDeps, List<TR::Register> *popRegisters)
{
