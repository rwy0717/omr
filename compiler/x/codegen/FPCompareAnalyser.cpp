...
isUnevaluatedZero(TR::Node *child)
{
	if (child->getRegister() == NULL) {
		...
		case TR::iconst:
		case TR::sconst:
		case TR::bconst: return (child->getInt() == 0);
		case TR::lconst: return (child->getLongInt() == 0);
		case TR::fconst:
			return (child->getFloatBits() == 0 || child->getFloatBits() == 0x80000000); // hex for ieee
			                                                                            // float -0.0
		case TR::dconst:
		...
		case TR::b2d:
		case TR::l2d:
		case TR::f2d: return isUnevaluatedZero(child->getFirstChild());
...
TR::Register *
TR_X86FPCompareAnalyser::fpCompareAnalyser(TR::Node *root, TR_X86OpCodes cmpRegRegOpCode, TR_X86OpCodes cmpRegMemOpCode,
	...
	bool mustSwapOperands =
	        (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple || cmpOp == TR::iffcmpgtu || cmpOp == TR::ifdcmpgtu
	                ...
	                || (useFCOMIInstructions
	                        && (cmpOp == TR::iffcmplt || cmpOp == TR::ifdcmplt || cmpOp == TR::iffcmpgeu
...
void
TR_X86FPCompareAnalyser::setInputs(TR::Node *firstChild, TR::Register *firstRegister, TR::Node *secondChild,
...
TR::Register *
TR_IA32XMMCompareAnalyser::xmmCompareAnalyser(
        TR::Node *root, TR_X86OpCodes cmpRegRegOpCode, TR_X86OpCodes cmpRegMemOpCode)
{
	...
	// code needed to evaluate the result of the comparison.
	//
	bool mustSwapOperands = (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple || cmpOp == TR::iffcmpgtu
	                                || cmpOp == TR::ifdcmpgtu || cmpOp == TR::fcmple || cmpOp == TR::dcmple
	                                || cmpOp == TR::fcmpgtu || cmpOp == TR::dcmpgtu || cmpOp == TR::iffcmplt
	                                || cmpOp == TR::ifdcmplt || cmpOp == TR::iffcmpgeu || cmpOp == TR::ifdcmpgeu
	...
	// code needed to evaluate the result of the comparison.
	//
	bool preventOperandSwapping = (cmpOp == TR::iffcmpltu || cmpOp == TR::ifdcmpltu || cmpOp == TR::iffcmpge
	                                      || cmpOp == TR::ifdcmpge || cmpOp == TR::fcmpltu || cmpOp == TR::dcmpltu
	                                      || cmpOp == TR::fcmpge || cmpOp == TR::dcmpge || cmpOp == TR::iffcmpgt
	                                      || cmpOp == TR::ifdcmpgt || cmpOp == TR::iffcmpleu
	                                      || cmpOp == TR::ifdcmpleu || cmpOp == TR::fcmpgt || cmpOp == TR::dcmpgt
	                                      || cmpOp == TR::fcmpleu || cmpOp == TR::dcmpleu)
	        ? true
	        : false;
