...
TR_Debug::printx(TR::FILE *pOutFile, TR::Instruction *instr)
{
	...
	case TR::Instruction::IsLabel: print(pOutFile, (TR::X86LabelInstruction *)instr); break;
	case TR::Instruction::IsPadding: print(pOutFile, (TR::X86PaddingInstruction *)instr); break;
	case TR::Instruction::IsAlignment: print(pOutFile, (TR::X86AlignmentInstruction *)instr); break;
	case TR::Instruction::IsBoundaryAvoidance: print(pOutFile, (TR::X86BoundaryAvoidanceInstruction *)instr); break;
	case TR::Instruction::IsPatchableCodeAlignment:
		print(pOutFile, (TR::X86PatchableCodeAlignmentInstruction *)instr);
		break;
#ifdef J9_PROJECT_SPECIFIC
	case TR::Instruction::IsVirtualGuardNOP: print(pOutFile, (TR::X86VirtualGuardNOPInstruction *)instr); break;
#endif
	case TR::Instruction::IsFence: print(pOutFile, (TR::X86FenceInstruction *)instr); break;
	case TR::Instruction::IsImm: print(pOutFile, (TR::X86ImmInstruction *)instr); break;
	case TR::Instruction::IsImm64: print(pOutFile, (TR::AMD64Imm64Instruction *)instr); break;
	case TR::Instruction::IsImm64Sym: print(pOutFile, (TR::AMD64Imm64SymInstruction *)instr);
	case TR::Instruction::IsImmSnippet: print(pOutFile, (TR::X86ImmSnippetInstruction *)instr); break;
	case TR::Instruction::IsImmSym: print(pOutFile, (TR::X86ImmSymInstruction *)instr); break;
	case TR::Instruction::IsReg: print(pOutFile, (TR::X86RegInstruction *)instr); break;
	case TR::Instruction::IsRegReg: print(pOutFile, (TR::X86RegRegInstruction *)instr); break;
	case TR::Instruction::IsRegRegReg: print(pOutFile, (TR::X86RegRegRegInstruction *)instr); break;
	case TR::Instruction::IsRegRegImm: print(pOutFile, (TR::X86RegRegImmInstruction *)instr); break;
	...
	case TR::Instruction::IsFPRemainderRegReg: print(pOutFile, (TR::X86FPRegRegInstruction *)instr); break;
#ifdef TR_TARGET_64BIT
	case TR::Instruction::IsRegImm64:
	case TR::Instruction::IsRegImm64Sym: print(pOutFile, (TR::AMD64RegImm64Instruction *)instr); break;
#endif
	case TR::Instruction::IsRegImm:
	case TR::Instruction::IsRegImmSym: print(pOutFile, (TR::X86RegImmInstruction *)instr); break;
	case TR::Instruction::IsRegMem: print(pOutFile, (TR::X86RegMemInstruction *)instr); break;
	case TR::Instruction::IsRegMemImm: print(pOutFile, (TR::X86RegMemImmInstruction *)instr); break;
	case TR::Instruction::IsRegRegMem: print(pOutFile, (TR::X86RegRegMemInstruction *)instr); break;
	case TR::Instruction::IsFPRegMem: print(pOutFile, (TR::X86FPRegMemInstruction *)instr); break;
	case TR::Instruction::IsFPReg: print(pOutFile, (TR::X86FPRegInstruction *)instr); break;
	case TR::Instruction::IsMem:
	case TR::Instruction::IsMemTable:
	case TR::Instruction::IsCallMem: print(pOutFile, (TR::X86MemInstruction *)instr); break;
	case TR::Instruction::IsMemImm:
	case TR::Instruction::IsMemImmSym:
	case TR::Instruction::IsMemImmSnippet: print(pOutFile, (TR::X86MemImmInstruction *)instr); break;
	case TR::Instruction::IsMemReg: print(pOutFile, (TR::X86MemRegInstruction *)instr); break;
	case TR::Instruction::IsMemRegImm: print(pOutFile, (TR::X86MemRegImmInstruction *)instr); break;
	case TR::Instruction::IsFPMemReg: print(pOutFile, (TR::X86FPMemRegInstruction *)instr); break;
	case TR::Instruction::IsVFPSave: print(pOutFile, (TR::X86VFPSaveInstruction *)instr); break;
	case TR::Instruction::IsVFPRestore: print(pOutFile, (TR::X86VFPRestoreInstruction *)instr); break;
	case TR::Instruction::IsVFPDedicate: print(pOutFile, (TR::X86VFPDedicateInstruction *)instr); break;
	case TR::Instruction::IsVFPRelease: print(pOutFile, (TR::X86VFPReleaseInstruction *)instr); break;
	case TR::Instruction::IsVFPCallCleanup: print(pOutFile, (TR::X86VFPCallCleanupInstruction *)instr); break;
	default:
...
TR_Debug::printFullRegisterDependencyInfo(TR::FILE *pOutFile, TR::RegisterDependencyConditions *conditions)
{
	...
	if (conditions->getNumPreConditions() > 0) {
		printDependencyConditions(
		        conditions->getPreConditions(), conditions->getNumPreConditions(), "Pre", pOutFile);
	}
...
void
TR_Debug::dumpDependencyGroup(TR::FILE *pOutFile, TR_X86RegisterDependencyGroup *group, int32_t numConditions,
...
TR_Debug::print(TR::FILE *pOutFile, TR::X86VirtualGuardNOPInstruction *instr)
{
	...
	printPrefix(pOutFile, instr);
	trfprintf(pOutFile, "%s Site:" POINTER_PRINTF_FORMAT ", ", getMnemonicName(&instr->getOpCode()),
	        instr->getSite());
...
TR_Debug::print(TR::FILE *pOutFile, TR::X86ImmInstruction *instr)
{
	...
	if (instr->getOpCode().isCallImmOp() && instr->getNode()->getSymbolReference()) {
		...
		if (symRef->isUnresolved())
			...
			trfprintf(pOutFile, " (" POINTER_PRINTF_FORMAT ")", instr->getSourceImmediate()); // TODO:AMD64:
			                                                                                  // Target
			                                                                                  // address
			                                                                                  // gets
			                                                                                  // truncated
		...
		printIntConstant(
		        pOutFile, instr->getSourceImmediate(), 16, getImmediateSizeFromInstruction(instr), true);
		printInstructionComment(pOutFile, 2, instr);
	}
	...
	if (sym->getMethodSymbol() && name) {
		...
		if (sym->getLabelSymbol()->getSnippet())
			trfprintf(pOutFile, "%-24s%s %s (%s)", name, commentString(),
			        getOpCodeName(&instr->getOpCode()), getName(sym->getLabelSymbol()->getSnippet()));
		else
			trfprintf(pOutFile, "%-24s%s %s (" POINTER_PRINTF_FORMAT ")", name, commentString(),
			        getOpCodeName(&instr->getOpCode()), instr->getSourceImmediate());
	} else {
...
int32_t
TR_Debug::printPrefixAndMnemonicWithoutBarrier(TR::FILE *pOutFile, TR::Instruction *instr, int32_t barrier)
{
...
TR_Debug::printIntConstant(TR::FILE *pOutFile, int64_t value, int8_t radix, TR_RegisterSizes size, bool padWithZeros)
{
	...
	switch (radix) {
	case 10: return printDecimalConstant(pOutFile, value, width, padWithZeros);

	case 16: return printHexConstant(pOutFile, value, width, padWithZeros);

	default: TR_ASSERT(0, "Can't print in unimplemented radix %d\n", radix); break;
	}
...
TR_Debug::print(TR::FILE *pOutFile, TR::RealRegister *reg, TR_RegisterSizes size)
{
	...
	case TR_WordReg:
	case TR_FloatReg:
	case TR_DoubleReg: trfprintf(pOutFile, "%s", getName(reg)); break;
	case TR_QuadWordReg:
	case TR_DoubleWordReg:
	case TR_HalfWordReg:
	case TR_ByteReg: trfprintf(pOutFile, "%s", getName(reg, size)); break;
	default: break;
	}
}
...
TR_Debug::getName(uint32_t realRegisterIndex, TR_RegisterSizes size)
{
	...
	case TR::RealRegister::ByteReg: return "byteReg";
	case TR::RealRegister::BestFreeReg: return "bestFreeReg";
	...
	case TR::RealRegister::vfp:
		...
		case 3:
		case -1: return "vfp";
		default: return unknownRegisterName('v');
		} // 3 is for AMD64
	case TR::RealRegister::st0:
		switch (size) {
		case 2:
		case -1: return "st(0)";
		default: return unknownRegisterName('s');
		}
	...
	case TR::RealRegister::mm0:
		...
		case 3:
		case -1: return "mm0";
		default: return unknownRegisterName('m');
		}
...
void
TR_Debug::dumpInstructionWithVFPState(TR::Instruction *instr, const TR_VFPState *prevState)
{
	if (_file != NULL) {
		...
		if (prevState && (vfpState != *prevState)) {
			trfprintf(_file, "\n\t%s VFP=%s+%d", commentString(), getName(vfpState._register),
...
void
TR_Debug::printRegMemInstruction(
...
void
TR_Debug::printMemRegInstruction(
...
void
TR_Debug::printMemImmInstruction(
...
getInterpretedMethodNameHelper(TR::MethodSymbol *methodSymbol, TR::DataType type)
{
	...
	switch (type) {
	case TR::NoType: return "interpreterVoidStaticGlue";
	case TR::Int8:
	case TR::Int16:
	case TR::Int32: return "interpreterIntStaticGlue";
	case TR::Address: return "interpreterLongStaticGlue";
	case TR::Int64: return "interpreterLongStaticGlue";
	case TR::Float: return "interpreterFloatStaticGlue";
	case TR::Double: return "interpreterDoubleStaticGlue";
	default: TR_ASSERT(0, "Unsupported data type: %d", type); return "UNKNOWN interpreted method type";
	}
}
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		for (count = 0; count < callNode->getNumChildren(); count++) {
			...
			case TR::Int32:
				...
				trfprintf(pOutFile,
				        "mov \tdword ptr[rsp+%d], %s\t\t#save registers for interpreter call snippet",
			...
			case TR::Int64:
				...
				trfprintf(pOutFile,
				        "mov \tqword ptr[rsp+%d], %s\t\t#save registers for interpreter call snippet",
				        offset, callRegName64[count]);
				offset -= 8;
				bufferPos += 5;
				break;
			default: TR_ASSERT(0, "unknown data type: %d", type); break;
			}
		}
...
#ifdef TR_TARGET_64BIT
uint8_t *
TR_Debug::printArgumentFlush(TR::FILE *pOutFile, TR::Node *callNode,
