...
TR_OutlinedInstructions::generateOutlinedInstructionsDispatch()
{
	...
	if (_targetReg) {
		...
		TR_ASSERT((targetRegPair == NULL && resultRegPair == NULL)
		        ...
		        "OutlinedInstructions: targetReg and resultReg must be both register pairs or neither register "
		        "pairs.");
		if (targetRegPair) {
			generateRegRegInstruction(MOVRegReg(), _callNode, targetRegPair->getLowOrder(),
			        resultRegPair->getLowOrder(), _cg);
			generateRegRegInstruction(MOVRegReg(), _callNode, targetRegPair->getHighOrder(),
			        resultRegPair->getHighOrder(), _cg);
		} else {
			TR_X86OpCodes mov = BADIA32Op;
			switch (resultReg->getKind()) {
			case TR_GPR: mov = MOVRegReg(); break;
			case TR_FPR:
			case TR_VRF: mov = MOVDQURegReg; break;
			default: TR_ASSERT(false, "OutlinedInstructions: unsupported result register kind."); break;
			}
...
OMR::RegisterUsage *
TR_OutlinedInstructions::findInRegisterUsageList(TR::list<OMR::RegisterUsage *> *rul, TR::Register *virtReg)
{
...
TR::Node *
TR_OutlinedInstructions::createOutlinedCallNode(TR::Node *callNode, TR::ILOpCodes callOp)
{
	int32_t i;
	TR::Node *child;
	...
	TR::Node *newCallNode = TR::Node::createWithSymRef(
	        callNode, callOp, callNode->getNumChildren(), callNode->getSymbolReference());
	...
	for (i = 0; i < callNode->getNumChildren(); i++) {
		...
		if (child->getRegister() != NULL) {
			...
			if ((child->getOpCodeValue() == TR::loadaddr) && (child->getSymbolReference()->getSymbol())
				...
				// Be very conservative at this point, even though it is possible to make it less so.
				// For example, this will catch the case of an unevaluated argument not persisting
				// outside of the outlined region even though one of its subtrees will.
				//
				(void)_cg->evaluate(child);

				// Do not decrement the reference count here.  It will be decremented when the call node
				// is evaluated again in the helper instruction stream.
				//
				newCallNode->setAndIncChild(i, child);
			}
		}
