...
ConvertToPatchableMemoryReference(TR::MemoryReference *mr, TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::SIMDRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::SIMDRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	TR::Register *globalReg = cg->evaluate(child);
	cg->machine()->setXMMGlobalRegister(
	        node->getGlobalRegisterNumber() - cg->machine()->getNumGlobalGPRs(), globalReg);
	cg->decReferenceCount(child);
	return globalReg;
}

TR::Register *
OMR::X86::TreeEvaluator::SIMDloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	switch (node->getSize()) {
	case 16: opCode = MOVDQURegMem; break;
	default:
...
TR::Register *
OMR::X86::TreeEvaluator::SIMDstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR_X86OpCodes opCode = BADIA32Op;
	switch (node->getSize()) {
	case 16: opCode = MOVDQUMemReg; break;
	default:
		if (cg->comp()->getOption(TR_TraceCG))
			traceMsg(cg->comp(), "Unsupported fill size: Node = %p\n", node);
		TR_ASSERT(false, "Unsupported fill size");
...
TR::Register *
OMR::X86::TreeEvaluator::SIMDsplatsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::VectorInt32:
		...
		generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, resultReg, 0x00, cg); // 00 00 00 00
		                                                                                      // shuffle xxxA to
		                                                                                      // AAAA
		break;
	case TR::VectorInt64:
		...
		generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, resultReg, 0x44, cg); // 01 00 01 00
		                                                                                      // shuffle xxBA to
		                                                                                      // BABA
		break;
	case TR::VectorFloat:
		generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, childReg, 0x00, cg); // 00 00 00 00
		                                                                                     // shuffle xxxA to
		                                                                                     // AAAA
		break;
	case TR::VectorDouble:
		generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, childReg, 0x44, cg); // 01 00 01 00
		                                                                                     // shuffle xxBA to
		                                                                                     // BABA
		break;
	default:
...
TR::Register *
OMR::X86::TreeEvaluator::SIMDgetvelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (4 == elementCount) {
			...
			/*
			 * the value to be read (indicated by shufconst) from srcVectorReg is splatted into all 4 slots
			 * in the dstReg this puts the value we want in the least significant bits and the other bits
			 * should never be read. for float, dstReg and resReg are the same because PSHUFD can work
			 * directly with TR_FPR registers for Int32, the result needs to be moved from the dstReg to a
			 * TR_GPR resReg.
			 */
		...
		{
			/*
			 * for double, dstReg and resReg are the same because PSHUFD can work directly with TR_FPR
			 * registers for Int64, the result needs to be moved from the dstReg to a TR_GPR resReg.
			 */
			 ...
			 * if elem = 0, the value we want is in the most significant 64 bits and needs to be splatted
			 * into the least significant 64 bits (the other bits affected by the splat are never read) if
			 * elem = 1, the value we want is already in the least significant 64 bits as a result, a mov
			 * instruction is good enough and splatting the value is unnecessary
			 */
			if (1 == elem) {
				generateRegRegInstruction(MOVDQURegReg, node, dstReg, srcVectorReg, cg);
			} else // 0 == elem
			...
			if (TR::VectorInt64 == firstChild->getDataType()) {
				if (TR::Compiler->target.is32Bit()) {
					...
					generateRegRegImmInstruction(PSHUFDRegRegImm1, node, dstReg, srcVectorReg,
					        (0 == elem) ? 0x03 : 0x01, cg);
