...
OMR::X86::TreeEvaluator::ifxcmpoEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if ((opCode == TR::ificmno) || (opCode == TR::ificmnno) || (opCode == TR::iflcmno)
	        || (opCode == TR::iflcmnno)) {
...
TR::Register *
OMR::X86::TreeEvaluator::intOrLongClobberEvaluate(TR::Node *node, bool nodeIs64Bit, TR::CodeGenerator *cg)
{
	...
	if (nodeIs64Bit) {
		...
		TR_ASSERT(
		        !TR::TreeEvaluator::getNodeIs64Bit(node, cg), "nodeIs64Bit must be consistent with node size");
		return cg->intClobberEvaluate(node);
	}
}

TR::Instruction *
OMR::X86::TreeEvaluator::compareGPMemoryToImmediate(
        TR::Node *node, TR::MemoryReference *mr, int32_t value, TR::CodeGenerator *cg)
{
	...
	bool is64Bit = TR::Compiler->target.is64Bit() ? TR::TreeEvaluator::getNodeIs64Bit(node->getFirstChild(), cg)
	                                              : false;
...
void
OMR::X86::TreeEvaluator::compareGPRegisterToImmediate(
...
TR::Instruction *
OMR::X86::TreeEvaluator::insertLoadConstant(TR::Node *node, TR::Register *target, intptrj_t value,
	...
	if (currentInstruction) {
		...
		if (is64Bit) {
			if (cg->constantAddressesCanChangeSize(node) && node && node->getOpCodeValue() == TR::aconst
			        && (node->isClassPointerConstant() || node->isMethodPointerConstant())) {
				movInstruction =
				        generateRegImm64Instruction(MOV8RegImm64, node, target, value, cg, reloKind);
		...
		if (target && node && node->getOpCodeValue() == TR::aconst && node->isMethodPointerConstant()
		        && (cg->fe()->isUnloadAssumptionRequired(
		                    cg->fe()->createResolvedMethod(cg->trMemory(),
		...
		if (value == 0 && canClobberEFLAGS) {
			...
			if (is64Bit) {
				...
				} else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we could get RIP
				                                   // too?
				{
					movInstruction = generateRegImmInstruction(
				...
				movInstruction =
				        generateRegImmInstruction(ops[opsRow][MOV], node, target, value, cg, reloKind);
			}
...
TR::Register *
OMR::X86::TreeEvaluator::loadConstant(TR::Node *node, intptrj_t value, TR_RematerializableTypes type,
...
TR::Instruction *
OMR::X86::TreeEvaluator::insertLoadMemory(TR::Node *node, TR::Register *target, TR::MemoryReference *tempMR,
        TR_RematerializableTypes type, TR::CodeGenerator *cg, TR::Instruction *currentInstruction)
{
	TR::Compilation *comp = cg->comp();
...
//
void
OMR::X86::TreeEvaluator::padUnresolvedDataReferences(TR::Node *node, TR::SymbolReference &symRef, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::loadMemory(TR::Node *node, TR::MemoryReference *sourceMR, TR_RematerializableTypes type,
...
TR::Register *
OMR::X86::TreeEvaluator::performIload(TR::Node *node, TR::MemoryReference *sourceMR, TR::CodeGenerator *cg)
{
...
OMR::X86::TreeEvaluator::aloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR::Register *reg = TR::TreeEvaluator::loadMemory(
	        node, sourceMR, TR_RematerializableAddress, node->getOpCode().isIndirect(), cg);
...
//
bool
OMR::X86::TreeEvaluator::genNullTestSequence(
...
OMR::X86::TreeEvaluator::bloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
	TR::Register *reg = TR::TreeEvaluator::loadMemory(
	        node, sourceMR, TR_RematerializableByte, node->getOpCode().isIndirect(), cg);
...
OMR::X86::TreeEvaluator::sloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
	TR::Register *reg = TR::TreeEvaluator::loadMemory(
	        node, sourceMR, TR_RematerializableShort, node->getOpCode().isIndirect(), cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
		...
		if (!usingCompressedPointers && cg->isMemoryUpdate(node)) {
			if (valueChild->getFirstChild()->getReferenceCount() == 1) {
				...
				if (comp->getOption(TR_OptimizeForSpace))
					...
					if (numRegs >= TR::RealRegister::LastAssignableGPR - 2) // -1 for VM thread reg,
					                                                        // -1 fudge
						valueChild->setDirectMemoryUpdate(true);
				}
			}
		...
		if (valueReg) {
			...
			if (cg->enableRematerialisation() && !valueReg->getRematerializationInfo()) {
				...
				if (originalValueChild == valueChild) {
					...
					case TR::Int8: type = TR_RematerializableByte; break;
					case TR::Int16: type = TR_RematerializableShort; break;
					case TR::Int32: type = TR_RematerializableInt; break;
					case TR::Int64: type = TR_RematerializableLong; break;
					case TR::Address: {
						if (node && node->getOpCode().hasSymbolReference() && node->getSymbol()
						        && node->getSymbol()->isClassObject())
							(TR::Compiler->om.generateCompressedObjectHeaders()
					...
					default: TR_ASSERT(0, "Unsupported dataType for integerStoreEvaluator"); break;
					}
	...
	if (tempMR)
		...
		// Direct Memory Update, we need to dec the first child ref count here, since this was not done by the
		// child evaluator in its direct memory update logic.
...
TR::Register *
OMR::X86::TreeEvaluator::arraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::SSE2ArraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateMemRegInstruction(
	        CMP1MemReg, node, generateX86MemoryReference(s2Reg, deltaReg, 0, cg), s2ByteVer1Reg, cg);
	...
	generateRegMemInstruction(
	        L1RegMem, node, s2ByteVer2Reg, generateX86MemoryReference(s2Reg, equalTestReg, 0, cg), cg);
	generateMemRegInstruction(
	        CMP1MemReg, node, generateX86MemoryReference(s2Reg, deltaReg, 0, cg), s2ByteVer2Reg, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::SSE2ArraycmpLenEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *s1AddrNode = node->getChild(0);
	TR::Node *s2AddrNode = node->getChild(1);
	TR::Node *lengthNode = node->getChild(2);
...
bool
OMR::X86::TreeEvaluator::stopUsingCopyRegAddr(TR::Node *node, TR::Register *&reg, TR::CodeGenerator *cg)
{
	if (node != NULL) {
...
bool
OMR::X86::TreeEvaluator::stopUsingCopyRegInteger(TR::Node *node, TR::Register *&reg, TR::CodeGenerator *cg)
{
	if (node != NULL) {
		reg = cg->evaluate(node);
		TR::Register *copyReg = NULL;
...
TR::Block *
OMR::X86::TreeEvaluator::getOverflowCatchBlock(TR::Node *node, TR::CodeGenerator *cg)
{
...
void
OMR::X86::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(TR::Node *node, TR::CodeGenerator *cg)
{
	/*
	 *overflowCHK
	 *   =>child1(Operation)
	...
	// add group
	case TR::ladd:
	case TR::iadd: op = ADDRegReg(nodeIs64Bit); break;
	case TR::sadd: op = ADD2RegReg; break;
	case TR::badd: op = ADD1RegReg; break;
	// sub group
	case TR::lsub:
	case TR::isub: op = SUBRegReg(nodeIs64Bit); break;
	case TR::ssub: op = SUB2RegReg; break;
	case TR::bsub: op = SUB1RegReg; break;
	// mul group
	case TR::imul: op = IMULRegReg(nodeIs64Bit); break;
	case TR::lmul:
		...
		// TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser Therefore the usual way of only detecting the
		// OF for the last instruction of sequence won't work for this case. The implementation needs to detect
		// OF flags after all the instructions involving higher parts of the registers for both operands and
		// intermediate results.
		TR_ASSERT(
		        TR::Compiler->target.is64Bit(), "overflowCHK for lmul on 32 bits is not currently supported\n");
	...
	// we need to do the operation again when the Operation node has been evaluated already under a different
	// treetop
	// TODO: there is still a chance that the flags might still be available and we could detect it and avoid
...
TR::Register *
OMR::X86::TreeEvaluator::overflowCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_X86OpCodes opcode;
...
static void
generateRepMovsInstruction(TR_X86OpCodes repmovs, TR::Node *node, TR::Register *sizeRegister,
        TR::RegisterDependencyConditions *dependencies, TR::CodeGenerator *cg)
{
	switch (repmovs) {
	case REPMOVSQ: generateRegImmInstruction(SHRRegImm1(), node, sizeRegister, 3, cg); break;
	case REPMOVSD: generateRegImmInstruction(SHRRegImm1(), node, sizeRegister, 2, cg); break;
	case REPMOVSW: generateRegInstruction(SHRReg1(), node, sizeRegister, cg); break;
	case REPMOVSB: break;
	default: TR_ASSERT(false, "Invalid REP MOVS opcode [%d]", repmovs); break;
	}
        ...
        TR::Node *node, TR::Register *dstReg, TR::Register *srcReg, TR::Register *sizeReg, TR::CodeGenerator *cg)
{
	...
	} else // decide direction during runtime
	{
		...
		generateRegMemInstruction(
		        LEARegMem(), node, scratch, generateX86MemoryReference(srcReg, sizeReg, 0, -8, cg), cg);
		...
		generateRegMemInstruction(
		        LEARegMem(), node, scratch, generateX86MemoryReference(NULL, scratch, 1, -1, cg), cg);
	}
        ...
        TR::Node *node, TR::Register *dstReg, TR::Register *srcReg, TR::Register *sizeReg, TR::CodeGenerator *cg)
{
	...
	} else // decide direction during runtime
	{
		...
		generateRegMemInstruction(LEARegMem(), node, dstReg, generateX86MemoryReference(dstReg, srcReg, 0, cg),
		        cg); // dst = dst + src
...

		{
			...
			generateRegMemInstruction(
			        LEARegMem(), node, srcReg, generateX86MemoryReference(srcReg, sizeReg, 0, -2, cg), cg);
			generateRegMemInstruction(
			        LEARegMem(), node, dstReg, generateX86MemoryReference(dstReg, sizeReg, 0, -2, cg), cg);
 ...
 */
static void
arrayCopyDefault(TR::Node *node, uint8_t elementSize, TR::Register *dstReg, TR::Register *srcReg, TR::Register *sizeReg,
        TR::CodeGenerator *cg)
{
	TR::RegisterDependencyConditions *dependencies =
	        generateRegisterDependencyConditions((uint8_t)3, (uint8_t)3, cg);
	...
	switch (elementSize) {
	case 8: repmovs = REPMOVSQ; break;
	case 4: repmovs = REPMOVSD; break;
	case 2: repmovs = REPMOVSW; break;
	default: repmovs = REPMOVSB; break;
	}
	if (node->isForwardArrayCopy()) {
 ...
 */
static void
generateArrayElementStore(TR::Node *node, TR::Register *addressReg, int32_t index, TR::Register *valueReg, uint8_t size,
        TR::CodeGenerator *cg)
{
	TR_X86OpCodes storeOpcode;
	if (valueReg->getKind() == TR_FPR) {
		switch (size) {
		case 4: storeOpcode = MOVDMemReg; break;
		case 8: storeOpcode = MOVQMemReg; break;
		case 16: storeOpcode = MOVDQUMemReg; break;
		default: TR_ASSERT(0, "Unsupported size in generateArrayElementStore, size: %d", size); break;
		}
	} else if (valueReg->getKind() == TR_GPR) {
		switch (size) {
		case 1: storeOpcode = S1MemReg; break;
		case 2: storeOpcode = S2MemReg; break;
		case 4: storeOpcode = S4MemReg; break;
		case 8: storeOpcode = S8MemReg; break;
		default: TR_ASSERT(0, "Unsupported size in generateArrayElementStore, size: %d", size); break;
		}
	} else {
 ...
 */
static void
generateArrayElementLoad(TR::Node *node, TR::Register *valueReg, uint8_t size, TR::Register *addressReg, int32_t index,
        TR::CodeGenerator *cg)
{
	TR_X86OpCodes loadOpCode;
	if (valueReg->getKind() == TR_FPR) {
		switch (size) {
		case 4: loadOpCode = MOVDRegMem; break;
		case 8: loadOpCode = MOVQRegMem; break;
		case 16: loadOpCode = MOVDQURegMem; break;
		default: TR_ASSERT(0, "Unsupported size in generateArrayElementLoad, size: %d", size); break;
		}
	} else if (valueReg->getKind() == TR_GPR) {
		switch (size) {
		case 1: loadOpCode = L1RegMem; break;
		case 2: loadOpCode = L2RegMem; break;
		case 4: loadOpCode = L4RegMem; break;
		case 8: loadOpCode = L8RegMem; break;
		default: TR_ASSERT(0, "Unsupported size in generateArrayElementLoad, size: %d", size); break;
		}
	} else {
...
static void
arraycopyForShortConstArrayWithDirection(
 ...
 */
static void
arraycopyForShortConstArrayWithoutDirection(
        TR::Node *node, TR::Register *dstReg, TR::Register *srcReg, uint32_t size, TR::CodeGenerator *cg)
{
	uint32_t totalSize = size;
	...
	} else if (residue != 0) // 1 gpr + 1 xmm
	{
		...
		generateArrayElementLoad(
		        node, reg2, secondLoadSizeForCase4, srcReg, residue - secondLoadSizeForCase4, cg);
		residueCase = 4;
	}
	...
	switch (residueCase) {
	case 0: break;
	case 1: generateArrayElementStore(node, dstReg, totalSize - residue, reg1, residue, cg); break;
	case 2:
	...
	case 4:
		...
		generateArrayElementStore(
		        node, dstReg, totalSize - secondLoadSizeForCase4, reg2, secondLoadSizeForCase4, cg);
		cg->stopUsingRegister(reg1);
		break;
	}
}
...
TR::Register *
OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// There are two variants of TR::arraycopy: one has 5 children, the other has 3 children. Details can be found
	// from compiler/il/ILOpCodeProperties.hpp
	//
	// In most, if not all, cases, the 5-child variant requires language specific semantics, which OMR is not aware
	// of. The fact that a 5-child arraycopy is generated indicates at least one of the first two children must be
	// needed when performing the copy; otherwise a 3-child arraycopy should be generated instead. Interpreting the
	// meanings of the first two children definitely requires language specific semantics. For example, the first
	// two children may be for dealing with an arraycopy where the Garbage Collector may need to be notified about
	// the copy or something to that affect.
	//
	...
	static bool ignoreDirectionForConstantLengthArrayCopy = feGetEnv("TR_"
...
TR::Register *
OMR::X86::TreeEvaluator::arraytranslateEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	//
	...
	if (sourceByte) {
		...
		if (arraytranslateOT) {
			...
			helper = TR::Compiler->target.is64Bit() ? TR_AMD64arrayTranslateTROTNoBreak
...
static void
packUsingShift(TR::Node *node, TR::Register *tempReg, TR::Register *sourceReg, int32_t size, TR::CodeGenerator *cg)
{
        ...
        TR::Node *node, TR::Register *XMMReg, TR::Register *sourceReg, int8_t size, TR::CodeGenerator *cg)
{
	...
	case 8:
		...
		break;
	default: TR_ASSERT(0, "Arrayset Evaluator: unsupported pack size"); break;
	}
}

static void
arraySetToZeroForShortConstantArrays(TR::Node *node, TR::Register *addressReg, uintptrj_t size, TR::CodeGenerator *cg)
{
...
static void
arraySetForShortConstantArrays(TR::Node *node, uint8_t elementSize, TR::Register *addressReg, TR::Register *valueReg,
        uintptrj_t size, TR::CodeGenerator *cg)
{
	...
	if (totalSize < 16) {
		...
		// First check if we are able to do it by just doing equal or less than notWorthPacking movs of the
		// valueReg
        ...
        TR::Node *node, TR::Register *addressReg, TR::Register *valueReg, TR::Register *sizeReg, TR::CodeGenerator *cg)
{
	...
	case TR_GPR:
		...
		break;
	case TR_FPR: generateRegRegInstruction(MOVDQURegReg, node, XMM, valueReg, cg); break;
	default: TR_ASSERT(0, "Arrayset Evaluator: unsupported register type"); break;
	}
	...
	generateLabelInstruction(LABEL, node, loopLabel, cg);
	generateMemRegInstruction(
	        MOVQMemReg, node, generateX86MemoryReference(addressReg, sizeReg, 3, -8, cg), XMM, cg);
	generateLabelInstruction(LOOP1, node, loopLabel, cg);
...
static void
arraySetDefault(TR::Node *node, uint8_t elementSize, TR::Register *addressReg, TR::Register *valueReg,
        TR::Register *sizeReg, TR::CodeGenerator *cg)
{
	...
	switch (valueReg->getKind()) {
	case TR_GPR: movOpcode = MOVRegReg(elementSize == 8); break;
	case TR_FPR: movOpcode = elementSize == 8 ? MOVQReg8Reg : MOVDReg4Reg; break;
	default: TR_ASSERT(0, "Arrayset Evaluator: unsupported register type"); break;
	}
	...
	case 8:
		...
		shiftAmount = 3;
		break;
	default: TR_ASSERT(0, "Arrayset Evaluator: unsupported fill size"); break;
	}
...
TR::Register *
OMR::X86::TreeEvaluator::arraysetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// arrayset
	//    address
	//    fill value
	...
	const uint8_t elementSize = valueNode->getOpCode().isRef() ? TR::Compiler->om.sizeofReferenceField()
	                                                           : valueNode->getSize();
	...
	if (isShortConstantArrayWithZero) {
		...
		if (valueNode->getOpCode().isFloat() || valueNode->getOpCode().isDouble()) {
			...
			generateRegRegInstruction(
			        elementSize == 8 ? MOVQReg8Reg : MOVDReg4Reg, node, valueReg, tempReg, cg);
			cg->stopUsingRegister(tempReg);
		} else {
...
bool
OMR::X86::TreeEvaluator::constNodeValueIs32BitSigned(TR::Node *node, intptrj_t *value, TR::CodeGenerator *cg)
{
...
intptrj_t
OMR::X86::TreeEvaluator::integerConstNodeValue(TR::Node *node, TR::CodeGenerator *cg)
{
	if (TR::TreeEvaluator::getNodeIs64Bit(node, cg)) {
		return (intptrj_t)node->getLongInt(); // Cast to satisfy 32-bit compilers, even though they never take
		                                      // this path
	} else {
		TR_ASSERT(node->getSize() <= 4,
		        "For efficiency on IA32, only call integerConstNodeValue for 32-bit constants");
		return node->getInt();
	}
}

TR::Register *
OMR::X86::TreeEvaluator::performCall(TR::Node *node, bool isIndirect, bool spillFPRegs, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::performHelperCall(TR::Node *node, TR::SymbolReference *helperSymRef,
 ...
 */
static TR::Register *
inlineAtomicMemoryUpdate(TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
{
	TR_ASSERT((!TR_X86OpCode(op).hasLongSource() && !TR_X86OpCode(op).hasLongTarget())
 ...
 */
static TR::Register *
inline64BitAtomicCompareAndMemoryUpdateOn32Bit(TR::Node *node, bool returnValue, TR::CodeGenerator *cg)
{
 ...
 */
static TR::Register *
inlineAtomicCompareAndMemoryUpdate(TR::Node *node, bool returnValue, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	TR::SymbolReference *SymRef = node->getSymbolReference();
...
TR::Register *
OMR::X86::TreeEvaluator::indirectCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// If the method to be called is marked as an inline method, see if it can
	// actually be generated inline.
	//
...
TR::Register *
OMR::X86::TreeEvaluator::generateLEAForLoadAddr(TR::Node *node, TR::MemoryReference *memRef,
...
TR::Register *
OMR::X86::TreeEvaluator::loadaddrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::SymbolReference *symRef = node->getSymbolReference();
...
TR::Register *
OMR::X86::TreeEvaluator::aRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::integerRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
	TR::Compilation *comp = cg->comp();
	if (globalReg == NULL) {
...
TR::Register *
OMR::X86::TreeEvaluator::iRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::lRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	TR::Register *globalReg = cg->evaluate(child);
	cg->decReferenceCount(child);
...
TR::Register *
OMR::X86::TreeEvaluator::GlRegDepsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::passThroughEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
...
OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR::Instruction *fence = generateFenceInstruction(FENCE, node,
	...
	cg->generateDebugCounter(
	        (node->getBlock()->isExtensionOfPreviousBlock()) ? "cg.blocks/extensions" : "cg.blocks", 1,
...
TR::Register *
OMR::X86::TreeEvaluator::conversionAnalyser(
        TR::Node *node, TR_X86OpCodes memoryToRegisterOp, TR_X86OpCodes registerToRegisterOp, TR::CodeGenerator *cg)
{
	...
	if (child->getReferenceCount() > 1) {
		...
		if (node->parentSupportsLazyClobber() && registerToRegisterOp == MOVZXReg8Reg4
			...
			// removal of sign/zero-extension in array accesses in JCL methods where we don't emit bound
			// checks.
			//
		...
		if (child->getRegister() == NULL && child->getOpCode().isMemoryReference()) {
			...
			// iu2l
			//   iiload  <- where this node was materialized by lowering an iaload in compressedPointers
			//   mode
			//
...
TR::Register *
OMR::X86::TreeEvaluator::sbyteswapEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateMemRegInstruction(
	        TR::Compiler->target.isSMP() ? LCMPXCHGMemReg(nodeIs64Bit) : CMPXCHGMemReg(nodeIs64Bit), node, memRef,
	        replaceReg, deps, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::PrefetchEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::atomicorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
OMR::X86::TreeEvaluator::tstartEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	*/
	...
	// in case user will make transientFailure goto persistenFailure, in which case the label will mess up at this
	// point we'd better re-generate the label and set it to persistentFailure node again.
...
static const TR_X86OpCodes BinaryArithmeticOpCodesForReg[TR::NumOMRTypes][NumBinaryArithmeticOps] = {
        ...
        {BADIA32Op, PADDDRegReg, PSUBDRegReg, PMULLDRegReg, BADIA32Op, PANDRegReg, PORRegReg,
        ...
        {BADIA32Op, ADDPDRegReg, SUBPDRegReg, MULPDRegReg, DIVPDRegReg, BADIA32Op, BADIA32Op,
...
static const TR_X86OpCodes BinaryArithmeticOpCodesForMem[TR::NumOMRTypes][NumBinaryArithmeticOps] = {
        ...
        {BADIA32Op, PADDDRegMem, PSUBDRegMem, PMULLDRegMem, BADIA32Op, PANDRegMem, PORRegMem,
        ...
        {BADIA32Op, ADDPDRegMem, SUBPDRegMem, MULPDRegMem, DIVPDRegMem, BADIA32Op, BADIA32Op,
                BADIA32Op}, // VectorDouble
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op}, // Aggregate
};
...
TR::Register *
OMR::X86::TreeEvaluator::FloatingPointAndVectorBinaryArithmeticEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::fadd:
	case TR::dadd:
	case TR::vadd: arithmetic = BinaryArithmeticAdd; break;
	case TR::fsub:
	case TR::dsub:
	case TR::vsub: arithmetic = BinaryArithmeticSub; break;
	case TR::fmul:
	case TR::dmul:
	case TR::vmul: arithmetic = BinaryArithmeticMul; break;
	case TR::fdiv:
	case TR::ddiv:
	case TR::vdiv: arithmetic = BinaryArithmeticDiv; break;
	case TR::vand: arithmetic = BinaryArithmeticAnd; break;
	case TR::vor: arithmetic = BinaryArithmeticOr; break;
	case TR::vxor: arithmetic = BinaryArithmeticXor; break;
	default: TR_ASSERT(false, "Unsupported OpCode");
	}
	...
	TR_X86OpCodes opCode = useRegMemForm ? BinaryArithmeticOpCodesForMem[type][arithmetic]
	                                     : BinaryArithmeticOpCodesForReg[type][arithmetic];
	TR_ASSERT(opCode != BADIA32Op,
	        "FloatingPointAndVectorBinaryArithmeticEvaluator: unsupported data type or arithmetic.");
