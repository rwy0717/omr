    ...
    if (kindsToBeAssigned & TR_X87_Mask) {
        ...
        if (result & (kSourceCanBePopped | kTargetCanBePopped)) {
            ...
            if ((result & (kSourceCanBePopped | kTargetCanBePopped)) == (kSourceCanBePopped | kTargetCanBePopped)) {
                ...
                //
                TR_ASSERT(0,
                    "TR::X86FPCompareRegRegInstruction::assignRegisters() ==> source operand should not be dead after "
                    "compare!");
            }
...
bool TR::X86FPCompareRegRegInstruction::swapOperands()
{
    ...
    if ( // !debug("saveFXCHs")       ||
        ...
        || swappedOp == TR::iffcmpgtu || swappedOp == TR::ifdcmpgtu || swappedOp == TR::fcmpgtu
...
void TR::X86FPCompareEvalInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & TR_GPR_Mask) {
        TR_ASSERT(
            _accRegister != NULL, "TR::X86FPCompareEvalInstruction::assignRegisters() ==> _accRegister is NULL\n");
        ...
        case TR::fcmpgt:
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
            break;
        ...
        case TR::fcmpgtu:
            TR_ASSERT(0,
                "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> shouldn't be able to generate this "
                "condition!\n");
            break;
        ...
        case TR::fcmpge:
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x0500, cg());
            break;
        ...
        case TR::fcmpgeu:
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, CMP2RegImm2, accRegister, 0x0100, cg());
            break;
        ...
        case TR::fcmpeq:
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
            cursor
                = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(cursor, CMP2RegImm2, accRegister, 0x4000, cg());
            break;
        ...
        default:
            TR_ASSERT(
                0, "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> invalid comparison op: %d\n", cmpOp);
            break;
        }
...
TR::X86LabelInstruction* generateLabelInstruction(TR_X86OpCodes op, TR::Node* node, TR::LabelSymbol* sym,
    TR::RegisterDependencyConditions* cond, TR::CodeGenerator* cg)
{
...
TR::X86LabelInstruction* generateLongLabelInstruction(TR_X86OpCodes op, TR::Node* node, TR::LabelSymbol* sym,
    TR::RegisterDependencyConditions* cond, TR::CodeGenerator* cg)
{
...
TR::X86LabelInstruction* generateLongLabelInstruction(
    TR_X86OpCodes op, TR::Node* node, TR::LabelSymbol* sym, bool needsVMThreadRegister, TR::CodeGenerator* cg)
{
