...
//
TR::RealRegister *
assign8BitGPRegister(TR::Instruction *instr, TR::Register *virtReg, TR::CodeGenerator *cg)
{
				...
				if (classObjectRegister != NULL && !classObjectRegister->getRealRegister()) {
					...
					if (assignedClassObjectRegister == NULL
						...
						if ((assignedClassObjectRegister = machine->findBestFreeGPRegister(
						             this, classObjectRegister, TR_WordReg))
						        != NULL) {
							machine->coerceGPRegisterAssignment(this, classObjectRegister,
							        assignedClassObjectRegister->getRegisterNumber());

							// No use of this register here in the original stream.  So,
							// need to bump its total use count without bumping its future
							// use count so that it doesn't look like the first time a
							// register is seen.  That is make it fail the
							// getTotalUseCount() == getFutureUseCount() test used to detect
							// the first reference to a register in the assignment direction
							//
...
TR::X86ImmInstruction::X86ImmInstruction(
        ...
        : TR::Instruction(node, op, cg), _sourceImmediate(imm), _adjustsFramePointerBy(0), _reloKind(reloKind)
{}
...
TR::X86ImmInstruction::X86ImmInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
        ...
        : TR::Instruction(cond, node, op, cg), _sourceImmediate(imm), _adjustsFramePointerBy(0), _reloKind(reloKind)
{}
...
TR::X86ImmSymInstruction::X86ImmSymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, int32_t imm,
        ...
        : TR::X86ImmInstruction(imm, op, precedingInstruction, cg), _symbolReference(sr)
{}
...
TR::X86ImmSymInstruction::X86ImmSymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, int32_t imm,
        ...
        : TR::X86ImmInstruction(cond, imm, op, precedingInstruction, cg), _symbolReference(sr)
{}
...
TR::X86RegInstruction::X86RegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
...
TR::X86RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	...
	if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
		...
		TR_RegisterSizes firstRequestedRegSize = getOpCode().hasByteTarget()
		        ? TR_ByteReg
		        : getOpCode().hasXMMTarget() ? TR_QuadWordReg : TR_WordReg;

		TR_RegisterSizes secondRequestedRegSize = getOpCode().hasByteSource()
		        ? TR_ByteReg
		...
		if (assignedSecondRegister == NULL) {
			...
			assignedSecondRegister->setState(
			        TR::RealRegister::Assigned, secondRegister->isPlaceholderReg());
...
TR::X86RegImmInstruction::X86RegImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
        TR::Register *treg, int32_t imm, TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg,
        int32_t reloKind)
        : TR::X86RegInstruction(cond, treg, op, precedingInstruction, cg), _sourceImmediate(imm), _reloKind(reloKind)
{}

void
TR::X86RegImmInstruction::adjustVFPState(TR_VFPState *state, TR::CodeGenerator *cg)
{
	...
	if (state->_register == targetReal) {
		...
		case ADD4RegImm4:
		case ADD8RegImms:
		case ADD8RegImm4: state->_displacement -= getSourceImmediate(); break;
		case SUB4RegImms:
		case SUB4RegImm4:
		case SUB8RegImms:
		case SUB8RegImm4: state->_displacement += getSourceImmediate(); break;
		default:
			if (cg->getDebug())
...
TR::X86RegImmSymInstruction::X86RegImmSymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg, int32_t imm,
        TR::SymbolReference *sr, TR::CodeGenerator *cg)
...
void
padUnresolvedReferenceInstruction(TR::Instruction *instr, TR::MemoryReference *mr, TR::CodeGenerator *cg)
{
	mr->getUnresolvedDataSnippet()->setDataReferenceInstruction(instr);
	generateBoundaryAvoidanceInstruction(
	        TR::X86BoundaryAvoidanceInstruction::unresolvedAtomicRegions, 8, 8, instr, cg);
}

void
insertUnresolvedReferenceInstructionMemoryBarrier(TR::CodeGenerator *cg, int32_t barrier, TR::Instruction *inst,
        TR::MemoryReference *mr, TR::Register *srcReg, TR::MemoryReference *anotherMr)
{
	...
	if (is5ByteFence) {
		...
		fenceInst = new (cg->trHeapMemory())
		        TR::X86MemImmInstruction(padInst, fenceOp.getOpCodeValue(), espMemRef, 0, cg);
	} else {
...
TR::X86MemRegImmInstruction::X86MemRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
...
TR::X86RegMemImmInstruction::X86RegMemImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
...
TR::X86RegRegMemInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	if ((cg()->getAssignmentDirection() == cg()->Backward)) {
		...
		TR_RegisterSizes requestedRegSize = getOpCode().hasByteTarget()
		        ? TR_ByteReg
		        : getOpCode().hasXMMTarget() ? TR_QuadWordReg : TR_WordReg;
		...
		if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
			...
			if (assignedSource2ndRegister == NULL) {
				assignedSource2ndRegister =
				        assignGPRegister(this, getSource2ndRegister(), requestedRegSize, cg());
			}
...
void
TR::X86FPST0ST1RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
...
void
TR::X86FPSTiST0RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	...
	if (kindsToBeAssigned & TR_X87_Mask) {
		...
		TR_ASSERT((result & (kSourceCanBePopped | kTargetCanBePopped))
		        ...
		        "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> both source and target cannot be "
		        "popped!");

		// Safety valve: we currently don't support stores into an arbitrary FP stack register without a pop.
		//
		// TR_ASSERT( result & (kSourceCanBePopped | kTargetCanBePopped),
		//        "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> at least one of source and target must
		//        be popped!");
...
void
TR::X86FPST0STiRegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
...
void
TR::X86FPArithmeticRegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	...
	if (kindsToBeAssigned & TR_X87_Mask) {
		...
		// Dead tree elimination doesn't always eliminate dead trees and we can eventually pop the target as
		// well, TODO: ensure proper stack balance. TR_ASSERT( !(result & kTargetCanBePopped),
...
void
TR::X86FPCompareRegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	...
	if (kindsToBeAssigned & TR_X87_Mask) {
		...
		if (result & (kSourceCanBePopped | kTargetCanBePopped)) {
			...
			if ((result & (kSourceCanBePopped | kTargetCanBePopped))
				...
				//
				TR_ASSERT(0,
				        "TR::X86FPCompareRegRegInstruction::assignRegisters() ==> source operand "
				        "should not be dead after compare!");
			}
...
TR::X86FPCompareRegRegInstruction::swapOperands()
{
	...
	if ( // !debug("saveFXCHs")       ||
	        ...
	        swappedOp == TR::iffcmple || swappedOp == TR::ifdcmple || swappedOp == TR::fcmple
	        || swappedOp == TR::dcmple || swappedOp == TR::iffcmpgtu || swappedOp == TR::ifdcmpgtu
	        ...
	        swappedOp == TR::iffcmplt || swappedOp == TR::ifdcmplt || swappedOp == TR::fcmplt
	        || swappedOp == TR::dcmplt || swappedOp == TR::iffcmpgeu || swappedOp == TR::ifdcmpgeu
	...
	if (cursor != NULL) {
		...
		TR_ASSERT(0,
		        "\nTR::X86FPCompareRegRegInstruction::swapOperands() ==> branch or set instruction not found "
		        "for FP comparison!\n");
	}
...
void
TR::X86FPCompareEvalInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	TR::Instruction *cursor = this;
	TR::Node *node = getNode();
	...
	if (kindsToBeAssigned & TR_GPR_Mask) {
		TR_ASSERT(_accRegister != NULL,
		        "TR::X86FPCompareEvalInstruction::assignRegisters() ==> _accRegister is NULL\n");
		...
		case TR::fcmpgt:
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
			break;
		...
		case TR::fcmpgtu:
			TR_ASSERT(0,
			        "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> shouldn't be able to "
		...
		case TR::fcmpge:
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x0500, cg());
			break;
		...
		case TR::fcmpgeu:
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, CMP2RegImm2, accRegister, 0x0100, cg());
			break;
		...
		case TR::fcmpeq:
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, AND2RegImm2, accRegister, 0x4500, cg());
			cursor = new (cg()->trHeapMemory())
			        TR::X86RegImmInstruction(cursor, CMP2RegImm2, accRegister, 0x4000, cg());
			break;
		...
		default:
			TR_ASSERT(0,
			        "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> invalid comparison op: %d\n",
			        cmpOp);
			break;
		}
...
TR_X86OpCodes
getBranchOrSetOpCodeForFPComparison(TR::ILOpCodes cmpOp, bool useFCOMIInstructions)
{
	TR_X86OpCodes op;

	switch (cmpOp) {
	case TR::iffcmpeq:
	case TR::ifdcmpeq: op = JE4; break;

	case TR::fcmpeq:
	case TR::dcmpeq: op = SETE1Reg; break;

	case TR::iffcmpneu:
	case TR::ifdcmpneu:
	case TR::iffcmpne:
	case TR::ifdcmpne: op = JNE4; break;

	case TR::fcmpneu:
	case TR::dcmpneu:
	case TR::fcmpne:
	case TR::dcmpne: op = SETNE1Reg; break;

	case TR::iffcmpleu:
	case TR::ifdcmpleu: op = useFCOMIInstructions ? JBE4 : JNE4; break;

	case TR::fcmpleu:
	case TR::dcmpleu: op = useFCOMIInstructions ? SETBE1Reg : SETNE1Reg; break;

	case TR::iffcmpgt:
	case TR::ifdcmpgt: op = useFCOMIInstructions ? JA4 : JE4; break;

	case TR::fcmpgt:
	case TR::dcmpgt: op = useFCOMIInstructions ? SETA1Reg : SETE1Reg; break;

	case TR::iffcmpltu:
	case TR::ifdcmpltu: op = useFCOMIInstructions ? JB4 : JNE4; break;

	case TR::fcmpltu:
	case TR::dcmpltu: op = useFCOMIInstructions ? SETB1Reg : SETNE1Reg; break;

	case TR::iffcmpge:
	case TR::ifdcmpge: op = useFCOMIInstructions ? JAE4 : JE4; break;

	case TR::fcmpge:
	case TR::dcmpge: op = useFCOMIInstructions ? SETAE1Reg : SETE1Reg; break;

	case TR::iffcmplt:
	case TR::ifdcmplt: op = useFCOMIInstructions ? JB4 : JE4; break;

	case TR::fcmplt:
	case TR::dcmplt: op = useFCOMIInstructions ? SETB1Reg : SETE1Reg; break;

	case TR::iffcmpgeu:
	case TR::ifdcmpgeu: op = useFCOMIInstructions ? JAE4 : JNE4; break;

	case TR::fcmpgeu:
	case TR::dcmpgeu: op = useFCOMIInstructions ? SETAE1Reg : SETNE1Reg; break;
...
#endif /* DEBUG */
	default: TR_ASSERT(0, "getBranchOrSetOpCodeForFPComparison() ==> invalid comparison op: %d\n", cmpOp);
	}
...
void
TR::X86FPRemainderRegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{

	if (kindsToBeAssigned & TR_GPR_Mask) // TODO: Move this code generation in FPTreeEvaluator.cpp rather than doing
	                                     // it here
	{
		OMR::X86::Instruction::assignRegisters(kindsToBeAssigned);
	...
	if (kindsToBeAssigned & TR_X87_Mask) {
		...
		if (assignedRegister == NULL) {
			...
			if (sourceRegister->getTotalUseCount() != sourceRegister->getFutureUseCount()) {
				...
				diagnostic("TR::X86FPMemRegInstruction::assignRegisters ==> first reference of source "
				           "operand!");
			}
		} else {
			if (!machine->isFPRTopOfStack(sourceRegister)) {
...
void
TR::X86VFPDedicateInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
	if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
...
void
TR::X86VFPReleaseInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
...
TR_AtomicRegion TR::X86PatchableCodeAlignmentInstruction::spinLoopAtomicRegions[] = {{0x00, 2}, // Spin loops just need
                                                                                                // two
...
TR_AtomicRegion TR::X86BoundaryAvoidanceInstruction::unresolvedAtomicRegions[] = {{0x00, 8}, // The first 8 bytes of an
                                                                                             // unresolved reference
                                                                                             // must be atomic
        {0, 0}};

void
TR::X86BoundaryAvoidanceInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
...
TR::AMD64RegImm64SymInstruction::AMD64RegImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
...
TR::Instruction *
generateInstruction(TR_X86OpCodes op, TR::Node *node, TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86MemInstruction *
generateMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86MemTableInstruction *
generateMemTableInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, ncount_t numEntries,
...
generateMemImmSymInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, int32_t imm,
        TR::SymbolReference *sr, TR::CodeGenerator *cg)
{
...
TR::X86ImmInstruction *
generateImmInstruction(TR::Instruction *prev, TR_X86OpCodes op, int32_t imm, TR::RegisterDependencyConditions *cond,
        TR::CodeGenerator *cg)
{
...
TR::X86RegInstruction *
generateRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::RegisterDependencyConditions *cond,
        TR::CodeGenerator *cg)
{
...
generateBoundaryAvoidanceInstruction(const TR_AtomicRegion *atomicRegions, uint8_t boundarySpacing, uint8_t maxPadding,
...
generatePatchableCodeAlignmentInstructionWithProtectiveNop(const TR_AtomicRegion *atomicRegions,
        TR::Instruction *patchableCode, int32_t sizeOfProtectiveNop, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory())
...
generatePaddingInstruction(uint8_t length, TR::Node *node, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86PaddingInstruction(length, node, cg);
}

TR::X86PaddingInstruction *
generatePaddingInstruction(TR::Instruction *precedingInstruction, uint8_t length, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86PaddingInstruction(precedingInstruction, length, cg);
}

TR::X86PaddingSnippetInstruction *
generatePaddingSnippetInstruction(uint8_t length, TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86LabelInstruction(op, node, sym, cg);
}

TR::X86LabelInstruction *
generateLabelInstruction(TR::Instruction *pred, TR_X86OpCodes op, TR::LabelSymbol *sym, TR::CodeGenerator *cg)
{
...
TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym, TR::RegisterDependencyConditions *cond,
        TR::CodeGenerator *cg)
{
...
TR::X86LabelInstruction *
generateLabelInstruction(TR::Instruction *prev, TR_X86OpCodes op, TR::LabelSymbol *sym,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86LabelInstruction *
generateLongLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes opCode, TR::Node *node, TR::LabelSymbol *label, TR::Node *glRegDep,
        List<TR::Register> *popRegisters, bool evaluateGlRegDeps, TR::CodeGenerator *cg)
{
...
static TR_AtomicRegion longBranchAtomicRegions[] = {{0x0, 6}, // The entire long branch instruction; TODO: JMP
                                                              // instructions are only 5 bytes
        {0, 0}};

TR::X86LabelInstruction *
generateJumpInstruction(TR_X86OpCodes opCode, TR::Node *jumpNode, TR::CodeGenerator *cg, bool needsVMThreadRegister,
        bool evaluateGlRegDeps)
{
	...
	(jumpNode->getNumChildren() > 0) ? inst = generateLabelInstruction(opCode, jumpNode, destinationLabel,
	                                           jumpNode->getFirstChild(), 0, evaluateGlRegDeps, cg)
...
TR::X86VirtualGuardNOPInstruction *
generateVirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site, TR::RegisterDependencyConditions *deps,
...
TR::X86VirtualGuardNOPInstruction *
generateVirtualGuardNOPInstruction(TR::Instruction *i, TR::Node *node, TR_VirtualGuardSite *site,
        TR::RegisterDependencyConditions *deps, TR::LabelSymbol *label, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory())
	        TR::X86VirtualGuardNOPInstruction(i, VirtualGuardNOP, node, site, deps, cg, label);
}
...
generateRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, int32_t imm,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86RegImmInstruction *
generateRegImmInstruction(TR::Instruction *instr, TR_X86OpCodes op, TR::Register *treg, int32_t imm,
...
generateRegImmSymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, int32_t imm, TR::SymbolReference *sr,
        TR::CodeGenerator *cg)
{
...
TR::X86RegMemInstruction *
generateRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::MemoryReference *mr,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86RegRegInstruction *
generateRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
generateRegRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg1, TR::Register *reg2,
...
generateRegRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg1, TR::Register *reg2,
...
TR::X86MemImmInstruction *
generateMemImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::MemoryReference *mr, int32_t imm,
        TR::CodeGenerator *cg, int32_t reloKind)
{
...
generateMemRegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::MemoryReference *mr,
...
TR::X86MemRegInstruction *
generateMemRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, TR::Register *sreg,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86ImmSymInstruction *
generateImmSymInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm, TR::SymbolReference *sr,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
generateRegMemImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg, TR::MemoryReference *mr, int32_t imm,
        TR::CodeGenerator *cg)
{
...
generateCallMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86CallMemInstruction *
generateCallMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, TR::CodeGenerator *cg)
{
...
generateHelperCallInstruction(TR::Instruction *cursor, TR_RuntimeHelper index, TR::CodeGenerator *cg)
{
	...
	return new (cg->trHeapMemory()) TR::X86ImmSymInstruction(
	        cursor, CALLImm4, (uintptrj_t)helperSymRef->getMethodAddress(), helperSymRef, cg);
}
...
generateVFPSaveInstruction(TR::Instruction *precedingInstruction, TR::CodeGenerator *cg)
{
...
TR::X86VFPRestoreInstruction *
generateVFPRestoreInstruction(TR::X86VFPSaveInstruction *saveInstruction, TR::Node *node, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86VFPRestoreInstruction(saveInstruction, node, cg);
}

TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(
...
TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(TR::RealRegister *framePointerReg, TR::Node *node, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86VFPDedicateInstruction(framePointerReg, node, cg);
}

TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(TR::Instruction *precedingInstruction, TR::RealRegister *framePointerReg,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(TR::RealRegister *framePointerReg, TR::Node *node,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
generateVFPReleaseInstruction(TR::Instruction *precedingInstruction, TR::X86VFPDedicateInstruction *dedicateInstruction,
        TR::CodeGenerator *cg)
{
...
TR::X86VFPReleaseInstruction *
generateVFPReleaseInstruction(TR::X86VFPDedicateInstruction *dedicateInstruction, TR::Node *node, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86VFPReleaseInstruction(dedicateInstruction, node, cg);
}

TR::X86VFPCallCleanupInstruction *
generateVFPCallCleanupInstruction(TR::Instruction *precedingInstruction, int32_t adjustment, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::X86VFPCallCleanupInstruction(precedingInstruction, adjustment, cg);
}

TR::X86VFPCallCleanupInstruction *
generateVFPCallCleanupInstruction(int32_t adjustment, TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::X86FPCompareEvalInstruction *
generateFPCompareEvalInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *accRegister,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::X86FPRemainderRegRegInstruction *
generateFPRemainderRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
...
generateFPReturnImmInstruction(
        TR_X86OpCodes op, TR::Node *node, int32_t imm, TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
TR::AMD64RegImm64Instruction *
generateRegImm64Instruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg, uint64_t imm,
        TR::CodeGenerator *cg, int32_t reloKind)
{
...
TR::AMD64RegImm64Instruction *
generateRegImm64Instruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg, uint64_t imm,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg, int32_t reloKind)
{
...
generateImm64Instruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::AMD64Imm64Instruction(op, node, imm, cg);
}

TR::AMD64Imm64Instruction *
generateImm64Instruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, uint64_t imm, TR::CodeGenerator *cg)
{
	return new (cg->trHeapMemory()) TR::AMD64Imm64Instruction(precedingInstruction, op, imm, cg);
}

TR::AMD64Imm64Instruction *
generateImm64Instruction(
...
TR::AMD64Imm64Instruction *
generateImm64Instruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, uint64_t imm,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
...
generateRegImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg, uint64_t imm,
        TR::SymbolReference *sr, TR::CodeGenerator *cg)
{
...
TR::AMD64RegImm64SymInstruction *
generateRegImm64SymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *reg, uint64_t imm,
        TR::SymbolReference *sr, TR::CodeGenerator *cg)
{
...
TR::AMD64Imm64SymInstruction *
generateImm64SymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op, uint64_t imm,
        TR::SymbolReference *sr, TR::CodeGenerator *cg)
{
...
TR::AMD64Imm64SymInstruction *
generateImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm, TR::SymbolReference *sr,
        TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
{
