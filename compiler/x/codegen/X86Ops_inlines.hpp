...
TR_X86OpCode::OpCode_t::encode(typename TBuffer::cursor_t cursor, uint8_t rexbits) const
{
	...
	if (supportsAVX() && TR::CodeGenerator::getX86ProcessorInfo().supportsAVX()) {
		...
		switch (prefixes) {
		case PREFIX___: break;
		case PREFIX_66: buffer.append('\x66'); break;
		case PREFIX_F2: buffer.append('\xf2'); break;
		case PREFIX_F3: buffer.append('\xf3'); break;
		default: break;
		}
		// REX
		...
		// OpCode escape
		switch (escape) {
		case ESCAPE_____: break;
		case ESCAPE_0F__: buffer.append('\x0f'); break;
		case ESCAPE_0F38:
			buffer.append('\x0f');
			buffer.append('\x38');
			break;
...
TR_X86OpCode::CheckAndFinishGroup07(uint8_t *cursor) const
{
	if (info().isGroup07()) {
		...
		case XEND:
			...
			break;
		default: TR_ASSERT(false, "INVALID OPCODE."); break;
		}
	}
}
