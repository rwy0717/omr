...
OMR::X86::Instruction::Instruction(
        ...
        : OMR::Instruction(cg, precedingInstruction, op, node), _conditions(0), _rexRepeatCount(0)
{}

void
OMR::X86::Instruction::initialize(
...
{
	TR_ASSERT(!(TR::Compiler->target.is64Bit() && self()->getOpCode().isIA32Only()),
...
void
OMR::X86::Instruction::adjustVFPStateForCall(TR_VFPState *state, int32_t vfpAdjustmentForCall, TR::CodeGenerator *cg)
{
...
OMR::X86::Instruction::clobberRegsForRematerialisation()
{
	...
	if (self()->cg()->enableRematerialisation() && self()->getDependencyConditions()
	        && (self()->getOpCodeValue() != ASSOCREGS) // reg associations aren't really instructions, so they don't
	                                                   // modify anything
	        && (self()->getOpCodeValue() != LABEL) // labels must already be handled properly for a variety of
	                                               // reasons
	        && (!self()->getOpCode().isShiftOp()) && (!self()->getOpCode().isRotateOp()) // shifts and rotates often
	                                                                                     // have a postcondition on
		...
		for (uint32_t i = 0; i < self()->getDependencyConditions()->getNumPostConditions(); i++) {
			...
			if (reg->isDiscardable()) {
				...
				if (debug("dumpRemat"))
					diagnostic("---> Clobbering %s discardable postcondition register %s at "
					           "instruction %s\n",
...
uint8_t
OMR::X86::Instruction::operandSizeRexBits() // Rex bits indicating 32/64-bit operand sizes
{
