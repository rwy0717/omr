...
bool OMR::X86::Instruction::registerRefKindApplies(TR_UpperHalfRefConditions when)
{
    ...
    case TR_ifModifies32or64bitTarget:
        return self()->getOpCode().modifiesTarget()
            && (self()->getOpCode().hasLongTarget() || self()->getOpCode().hasIntTarget());
    case TR_ifModifies32or64bitSource:
        return self()->getOpCode().modifiesSource()
            && (self()->getOpCode().hasLongSource() || self()->getOpCode().hasIntSource());
    default:
...
void OMR::X86::Instruction::aboutToAssignRegDeps(
    TR_UpperHalfRefConditions usesUpperHalf, TR_UpperHalfRefConditions defsUpperHalf)
{
    if (!self()->getDependencyConditions())
        return;
...
void OMR::X86::Instruction::clobberRegsForRematerialisation()
{
    ...
    if (self()->cg()->enableRematerialisation() && self()->getDependencyConditions()
        ...
        && (!self()->getOpCode()
                   .isRotateOp()) // shifts and rotates often have a postcondition on ecx but don't clobber it
    ) {
...
bool OMR::X86::Instruction::isLabel() { return self()->getOpCodeValue() == LABEL; }

void OMR::X86::Instruction::aboutToAssignRegister(
    TR::Register* reg, TR_UpperHalfRefConditions usesUpperHalf, TR_UpperHalfRefConditions defsUpperHalf)
{
