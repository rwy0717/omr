...
void
OMR::X86::MemoryReference::initialize(TR::SymbolReference *symRef, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	TR::Symbol *symbol = symRef->getSymbol();
...
//
void
OMR::X86::MemoryReference::checkAndDecReferenceCount(TR::Node *node, rcount_t refCount, TR::CodeGenerator *cg)
{
...
void
OMR::X86::MemoryReference::useRegisters(TR::Instruction *instr, TR::CodeGenerator *cg)
{
	if (_baseRegister != NULL) {
...
int32_t
OMR::X86::MemoryReference::getStrideForNode(TR::Node *node, TR::CodeGenerator *cg)
{
	int32_t stride = 0;
...
void
OMR::X86::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg, TR::Node *parent)
{
...
void
OMR::X86::MemoryReference::consolidateRegisters(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	if (comp->getOption(TR_TraceRegisterPressureDetails)) {
...
void
OMR::X86::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
	...
	if (_baseRegister != NULL) {
		if (_baseRegister == cg->machine()->getRealRegister(TR::RealRegister::vfp)) {
			...
			if (assignedBaseRegister == NULL) {
				...
				// generateX86MemoryReference(OMR::X86::MemoryReference  &, intptrj_t, TR::CodeGenerator
				// *cg).
				TR_ASSERT(!_baseRegister->getRealRegister(),
				        "_baseRegister is a Real Register already, are you reusing a Memory "
				        "Reference?");
	...
	if (self()->getBaseRegister()) {
		...
		if (registerNumber == TR::RealRegister::vfp) {
			TR_ASSERT(cg->machine()->getRealRegister(registerNumber)->getAssignedRealRegister(),
			        "virtual frame pointer must be assigned before estimating instruction length lower "
			        "bound!\n");
			registerNumber = toRealRegister(
			        cg->machine()->getRealRegister(registerNumber)->getAssignedRealRegister())
			                         ->getRegisterNumber();
		}
	}
...
OMR::X86::MemoryReference::enlarge(TR::CodeGenerator *cg, int32_t requestedEnlargementSize, int32_t maxEnlargementSize,
	...
	if (!self()->isForceWideDisplacement()) {
		...
		if (potentialPatchGrowth > 0
		        ...
		        && performTransformation(cg->comp(),
		                "O^O Enlarging memory reference by %d bytes by forcing wide displacement - "
		                "allowpartial was %d",
        ...
        uint32_t addressTypes, uint8_t *cursor, TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case 2: {
		...
		{
			cg->jitAdd32BitPicToPatchOnClassRedefinition((void *)(uintptr_t) * (int32_t *)cursor,
			        (void *)cursor, self()->getUnresolvedDataSnippet() != NULL);
		}
		...
		if (symbol) {
			...
			if (staticSym) {
				if (self()->getUnresolvedDataSnippet() == NULL) {
					if (symbol->isConst()) {
						...
						if (cg->wantToPatchClassPointer(NULL, cursor)) // might not point to
						                                               // beginning of class
						{
						...
						if (staticSym->isCountForRecompile()) {
							cg->addExternalRelocation(
							        new (cg->trHeapMemory()) TR::ExternalRelocation(cursor,
							                (uint8_t *)TR_CountForRecompile, TR_GlobalValue,
							                cg),
							        __FILE__, __LINE__, node);
							...
							if (counter == NULL) {
								cg->comp()->failCompilation<TR::CompilationException>(
								        "Could not generate relocation for debug "
								        "counter in "
								        "OMR::X86::MemoryReference::"
			...
			if (label != NULL) {
				if (TR::Compiler->target.is64Bit()) {
					...
					// TODO:AMD64: Would it be cleaner to have some kind of "isRelative" flag rather
					// than "is64BitTarget"?
					cg->addRelocation(new (cg->trHeapMemory())
					                TR::LabelRelative32BitRelocation(cursor, label));
				} else {
					cg->addRelocation(
					        new (cg->trHeapMemory()) TR::LabelAbsoluteRelocation(cursor, label));
					cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
					                                  cursor, 0, TR_AbsoluteMethodAddress, cg),
					        __FILE__, __LINE__, node);
				}
	...
	case 5: {
		...
		if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp()
			...
			if (self()->getSymbolReference().getSymbol()
			        && self()->getSymbolReference().getSymbol()->isClassObject()
			        && cg->wantToPatchClassPointer(NULL, cursor)) // possibly unresolved, may not point to
			                                                      // beginning of class
			{
				cg->jitAdd32BitPicToPatchOnClassRedefinition((void *)(uintptr_t) * (int32_t *)cursor,
				        (void *)cursor, self()->getUnresolvedDataSnippet() != NULL);
        ...
        uint8_t *modRM, TR::Instruction *containingInstruction, TR::CodeGenerator *cg)
{
	...
	case 1: {
		...
		if (baseRegisterNumber == TR::RealRegister::vfp) {
			...
			base = toRealRegister(
			        cg->machine()->getRealRegister(baseRegisterNumber)->getAssignedRealRegister());
			baseRegisterNumber = base->getRegisterNumber();
			...
			if (label != NULL) {
				if (TR::Compiler->target.is64Bit()) {
					...
					*(int32_t *)cursor = -(int32_t)(intptrj_t)(
					        cursor + 4 + containingInstruction->getOpCode().info().ImmediateSize());
				} else {
			...
			if (debug("traceInstructionSelection")) {
				if (TR::Compiler->target.is64Bit())
					diagnostic("\nRematerializing aladd [" POINTER_PRINTF_FORMAT
					           "] with [" POINTER_PRINTF_FORMAT "] at [" POINTER_PRINTF_FORMAT "]",
