...
TR::Register *
TR_X86IntegerMultiplyDecomposer::decomposeIntegerMultiplier(int32_t &tempRegArraySize, TR::Register **tempRegArray)
{
	...
	if (decompositionIndex != -1) {
		...
		if (_multiplier != absMultiplier) // treats TR::getMinSigned<TR::Int64>() and
		                                  // TR::getMinSigned<TR::Int32>() properly, _multiplier < 0 does not
		{
		...
		decompositionIndex =
		        findDecomposition((uint64_t)absMultiplier >> shiftAmount); // cast needed so
		...
		if (decompositionIndex != -1) {
			...
			if (composition._subsequentShiftTooExpensive == false) {
				...
				if (shiftAmount < 3 && !cg()->getX86ProcessorInfo().isIntelCore2()
				        ...
				        && !cg()->getX86ProcessorInfo().isAMDOpteron()) // TODO:: P3 should go straight
				                                                        // to else and use shift always
				{
				...
				if (_multiplier != absMultiplier) // treats TR::getMinSigned<TR::Int64>() properly,
		...
		if (onesCount == 2) // sum of 2 powers of 2
		{
			...
			if (_multiplier != absMultiplier) // treats TR::getMinSigned<TR::Int64>() properly, _multiplier
			                                  // < 0 does not
			{
				generateRegInstruction(NEGReg(nodeIs64Bit), _node, target, _cg);
			}
			...
			return target;
		} else if (onesCount + trailingZeroes(absMultiplier) + leadingZeroes(absMultiplier) == 64) // difference
		                                                                                           // of 2
		                                                                                           // powers of
		                                                                                           // two
		{
...
TR_X86IntegerMultiplyDecomposer::findDecomposition(int64_t multiplier)
{
	...
	if ((i < NUM_CONSTS_DECOMPOSED) && (_integerMultiplySolutions[i]._multiplier == multiplier)) {
		...
		int32_t registerCostOfDecomposition = composition._numAdditionalRegistersNeeded
        ...
        int32_t index, int32_t &tempRegArraySize, TR::Register **tempRegArray)
{
	...
	for (int32_t cursor = 0; cursor < MAX_NUM_COMPONENTS; ++cursor) {
		...
		case shlRegImm:
			generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, registerMap[component._target],
			        component._baseOrImmed, _cg);
			break;
		case addRegReg:
			generateRegRegInstruction(ADDRegReg(nodeIs64Bit), _node, registerMap[component._target],
			        registerMap[component._baseOrImmed], _cg);
			break;
		case subRegReg:
			generateRegRegInstruction(SUBRegReg(nodeIs64Bit), _node, registerMap[component._target],
			        registerMap[component._baseOrImmed], _cg);
			break;
		case movRegReg:
			generateRegRegInstruction(MOVRegReg(nodeIs64Bit), _node, registerMap[component._target],
			        registerMap[component._baseOrImmed], _cg);
			break;
		case leaRegReg2:
