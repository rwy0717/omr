    ...
    TR_RegisterSizes requestedRegSize, bool considerUnlatched)
{
    ...
    TR_ASSERT(
        (virtReg && (virtReg->getKind() == TR_GPR || virtReg->getKind() == TR_FPR || virtReg->getKind() == TR_VRF)),
        ...
        if (info->isRematerializableFromMemory()) {
            ...
            if (info->getDataType() == TR_RematerializableFloat) {
                TR::MemoryReference* tempMR = generateX86MemoryReference(
                    self()->cg()->findOrCreate4ByteConstant(currentInstruction->getNode(), info->getConstant()),
                    self()->cg());
                instr = generateRegMemInstruction(currentInstruction, MOVSSRegMem, best, tempMR, self()->cg());
#ifdef DEBUG
                self()->cg()->incNumRematerializedXMMRs();
    ...
    if (targetRegister->getState() == TR::RealRegister::Free) {
        ...
        if (currentAssignedRegister != NULL) {
            ...
            if (targetRegister != candidate) {
                if (virtualRegister->getKind() == TR_VRF) {
                    instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
                        currentInstruction, MOVDQURegReg, targetRegister, candidate, self()->cg());
                } else if (currentTargetVirtual->isSinglePrecision()) {
                    instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
                        currentInstruction, MOVAPSRegReg, targetRegister, candidate, self()->cg());
                } else {
                    instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
                        currentInstruction, MOVAPDRegReg, targetRegister, candidate, self()->cg());
                }
...
TR::Register** OMR::X86::Machine::captureRegisterAssociations()
{
    ...
    if (_registerAssociations[TR::RealRegister::vfp] != NULL) {
        registerAssociationsClone[TR::RealRegister::vfp]
            = (TR::Register*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::Register), heapAlloc);
        memcpy(registerAssociationsClone[TR::RealRegister::vfp], _registerAssociations[TR::RealRegister::vfp],
            sizeof(TR::Register));
    } else {
...
TR::Instruction* OMR::X86::Machine::fpSpillFPR(TR::Instruction* prevInstruction, TR::Register* vreg)
{
    ...
    if (fpReg != NULL) {
        ...
        cursor = new (self()->cg()->trHeapMemory()) TR::X86FPMemRegInstruction(cursor,
            isFloat ? FSTPMemReg : DSTPMemReg, tempMR, self()->fpMapToStackRelativeRegister(vreg), self()->cg());
#ifdef DEBUG
        self()->cg()->incNumSpilledRegisters();
#endif
    }
