...
TR::RealRegister *
OMR::X86::Machine::findBestFreeGPRegister(TR::Instruction *currentInstruction, TR::Register *virtReg,
        TR_RegisterSizes requestedRegSize, bool considerUnlatched)
{
	...
	case TR_QuadWordReg:
		...
		break;
	default: TR_ASSERT(0, "unknown register size requested\n");
	}
	...
	for (i = first; i <= last; i++) {
		...
		TR::Register *associatedRegister = useRegisterAssociations
...
TR::RealRegister *
OMR::X86::Machine::freeBestGPRegister(TR::Instruction *currentInstruction, TR::Register *virtReg,
        TR_RegisterSizes requestedRegSize, TR::RealRegister::RegNum targetRegister, bool considerVirtAsSpillCandidate)
{
	...
	for (i = first; i <= last; i++) {
		...
		if (realReg->getState() == TR::RealRegister::Assigned) {
			...
			if (targetRegister == i) {
				if (useRegisterInterferences && (interference & (1 << (i - 1)))) // TODO:AMD64: Use the
				                                                                 // proper mask value
	...
	for (cursor = currentInstruction->getPrev(); cursor; cursor = cursor->getPrev()) {
		...
		for (i = 0; i < numCandidates; i++) {
			...
			interferes = interference & (1 << (registerNumber - 1)); // TODO:AMD64: Use the proper mask
			                                                         // value

			if (cursor->refsRegister(candidates[i])) {
				if (distance > FREE_BEST_REGISTER_MINIMUM_CANDIDATE_DISTANCE) {
					TR_RematerializationInfo *info = candidates[i]->isDiscardable()
		...
		for (i = 0; i < NumBestRegisters; i++) {
			if (bestRegisters[i])
				self()->cg()->traceRegisterAssignment("   (best %s %R at distance %d)",
				        bestRegisters[i], bestRegisterTypes[i], bestDistances[i]);
		}
	...
	TR::Register *bestDiscardableRegister =
	        (bestRegister->isDiscardable() && bestRegister->getRematerializationInfo()->isActive()) ? bestRegister
	                                                                                                : NULL;
	...
	if (enableRematerialisation && bestDiscardableRegister) {
		...
		if (info->isRematerializableFromMemory()) {
			...
			if (info->getDataType() == TR_RematerializableFloat) {
				...
				instr = TR::TreeEvaluator::insertLoadConstant(0, best, info->getConstant(),
				        info->getDataType(), self()->cg(), currentInstruction);
			}
		}
		...
		if ((bestRegister->getKind() == TR_FPR)) {
			...
			if (containsInternalPointer) {
				...
				if (bestRegister->getBackingStorage()) {
					...
					if (self()->cg()->getDebug())
						self()->cg()->traceRegisterAssignment("find or create free backing "
						                                      "store (%p) for %s with initial "
						                                      "max spill depth of %d and "
						                                      "adding to list\n",
						        location, self()->cg()->getDebug()->getName(bestRegister),
						        self()->cg()->getCurrentPathDepth());
				} else {
					location =
					        self()->cg()->allocateSpill(TR::Compiler->om.sizeofReferenceAddress(),
		...
		TR::MemoryReference *tempMR =
		        generateX86MemoryReference(location->getSymbolReference(), offset, self()->cg());
		...
		TR_X86OpCodes op;
		if (bestRegister->getKind() == TR_FPR) {
			op = (bestRegister->isSinglePrecision()) ? MOVSSRegMem
...
TR::RealRegister *
OMR::X86::Machine::reverseGPRSpillState(TR::Instruction *currentInstruction, TR::Register *spilledRegister,
        TR::RealRegister *targetRegister, TR_RegisterSizes requestedRegSize)
{
	...
	if (targetRegister == NULL) {
		...
		if (targetRegister == NULL) {
			targetRegister =
			        self()->freeBestGPRegister(currentInstruction, spilledRegister, requestedRegSize);
		}
	}
	...
	if (self()->cg()->enableBetterSpillPlacements()) {
		if (spilledRegister->hasBetterSpillPlacement()) {
			TR::Instruction *betterInstruction = self()->cg()->findBetterSpillPlacement(
	...
	if (spilledRegister->getKind() == TR_FPR) {
		instr = new (self()->cg()->trHeapMemory()) TR::X86MemRegInstruction(currentInstruction,
		        spilledRegister->isSinglePrecision() ? MOVSSMemReg : MOVSDMemReg, tempMR, targetRegister,
		        self()->cg());
...
void
OMR::X86::Machine::coerceGPRegisterAssignment(TR::Instruction *currentInstruction, TR::Register *virtualRegister,
	...
	if (targetRegister->getState() == TR::RealRegister::Free) {
		...
		if (currentAssignedRegister != NULL) {
			instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(currentInstruction,
			        XCHGRegReg(), currentAssignedRegister, targetRegister, self()->cg());
			...
			TR::RealRegister *candidate =
			        self()->findBestFreeGPRegister(currentInstruction, currentTargetVirtual);
			...
			if (candidate) {
				...
				candidate = self()->freeBestGPRegister(currentInstruction, currentTargetVirtual,
				        TR_WordReg, registerNumber, coerceToSatisfyRegDeps);
			}

			if ((targetRegister != candidate) && (candidate != currentTargetVirtual)) {
				instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
				        currentInstruction, MOVRegReg(), targetRegister, candidate, self()->cg());
				...
				self()->cg()->traceRAInstruction(instr);
				self()->cg()->resetRegisterAssignmentFlag(TR_RegisterSpilled); // the spill (if any) has
				                                                               // been traced
			}
...
void
OMR::X86::Machine::coerceXMMRegisterAssignment(TR::Instruction *currentInstruction, TR::Register *virtualRegister,
	...
	if (targetRegister->getState() == TR::RealRegister::Free) {
		if (currentAssignedRegister == NULL) {
			...
			if (virtualRegister->getKind() == TR_VRF) {
				instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(currentInstruction,
				        MOVDQURegReg, currentAssignedRegister, targetRegister, self()->cg());
			} else if (virtualRegister->isSinglePrecision()) {
				instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(currentInstruction,
				        MOVAPSRegReg, currentAssignedRegister, targetRegister, self()->cg());
			} else {
				instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(currentInstruction,
				        MOVAPDRegReg, currentAssignedRegister, targetRegister, self()->cg());
			}
			currentAssignedRegister->setState(TR::RealRegister::Free);
			currentAssignedRegister->setAssignedRegister(NULL);
		}
		...
		if (currentAssignedRegister != NULL) {
			...
			if (candidate) {
				...
				self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
				candidate = self()->freeBestGPRegister(currentInstruction, currentTargetVirtual,
				        TR_QuadWordReg, regNum, coerceToSatisfyRegDeps);
			}
		...
		if (currentAssignedRegister != NULL) {
			...
			if (targetRegister != candidate) {
				...
				self()->cg()->traceRegAssigned(currentTargetVirtual, candidate);
				self()->cg()->traceRAInstruction(instr);
				self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled); // the spill (if any) has
				                                                             // been traced
			}
...
void
OMR::X86::Machine::swapGPRegisters(
...
{
	TR::RealRegister *candidate;
	if ((candidate = self()->findBestFreeGPRegister(currentInstruction, virtualRegister, requestedRegSize))
	        == NULL) {
		self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
...
OMR::X86::Machine::setGPRWeightsFromAssociations()
{
	...
	for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; ++i) {
		...
		if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i)
			...
			assocReg->setAssociation(i);
			if (assocReg->isPlaceholderReg()) { // placeholder register and is only needed at the specific
				                            // dependency site (usually a killed register on a call) so
				                            // defer this register's weight to that of registers where
				                            // the associated register has a longer life
...
OMR::X86::Machine::createRegisterAssociationDirective(TR::Instruction *cursor)
{
	...
	for (int i = 0; i < TR::RealRegister::LastAssignableGPR; i++) {
		...
		associations->addPostCondition(
		        self()->getVirtualAssociatedWithReal(regNum), regNum, self()->cg(), 0, true);
	}
...
OMR::X86::Machine::initializeRegisterFile(const struct TR::X86LinkageProperties &properties)
{
	...
	if (!dontUseEBXasGPR) {
		...
		_registerFile[TR::RealRegister::ebx] = new (self()->cg()->trHeapMemory())
		        TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::ebx,
	...
	_registerFile[TR::RealRegister::ebp] = new (self()->cg()->trHeapMemory())
	        TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::ebp,
	...
	_registerFile[TR::RealRegister::esp] = new (self()->cg()->trHeapMemory())
	        TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::esp,
	...
	_registerFile[TR::RealRegister::vfp] = new (self()->cg()->trHeapMemory())
	        TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::vfp,
	...
	for (reg = TR::RealRegister::r8; reg <= TR::RealRegister::LastAssignableGPR; reg++) {
		_registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(TR_GPR,
		        properties.isPreservedRegister((TR::RealRegister::RegNum)reg) ? PRESERVED_WEIGHT
		                                                                      : NONPRESERVED_WEIGHT,
...
OMR::X86::Machine::cloneRegisterFile(TR::RealRegister **registerFile, TR_AllocationKind allocKind)
{
	...
	for (i = TR::RealRegister::FirstGPR; i <= endReg;
		...
		registerFileClone[i] = (TR::RealRegister *)self()->cg()->trMemory()->allocateMemory(
	...
	// assignRegisters check if (_baseRegister == cg()->machine()->getRealRegister(TR::RealRegister::vfp) works
	// correctly
...
TR::RegisterDependencyConditions *
OMR::X86::Machine::createCondForLiveAndSpilledGPRs(bool calledForDeps, TR::list<TR::Register *> *spilledRegisterList)
{
	int32_t i, c = 0;
		...
		for (i = TR::RealRegister::FirstGPR; i <= endReg;
			...
			if (realReg->getState() == TR::RealRegister::Assigned) {
				...
				TR_ASSERT(!spilledRegisterList
				                || !(std::find(spilledRegisterList->begin(), spilledRegisterList->end(),
				                             virtReg)
...
OMR::X86::Machine::captureRegisterAssociations()
{
	...
	for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
		...
		if (_registerAssociations[i] != NULL) {
			registerAssociationsClone[i] = (TR::Register *)self()->cg()->trMemory()->allocateMemory(
...
void
OMR::X86::Machine::adjustRegisterUseCountsUp(TR::list<OMR::RegisterUsage *> *rul, bool adjustFuture)
{
	if (!rul)
		return;
	TR::Compilation *comp = self()->cg()->comp();
	for (auto iter = rul->begin(); iter != rul->end(); ++iter) {
		if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
			traceMsg(comp,
			        ...
			        (*iter)->virtReg->getTotalUseCount(), adjustFuture, (*iter)->useCount);
		}
...
void
OMR::X86::Machine::adjustRegisterUseCountsDown(TR::list<OMR::RegisterUsage *> *rul, bool adjustFuture)
{
	if (!rul)
		return;
	TR::Compilation *comp = self()->cg()->comp();
...
OMR::X86::Machine::fpMapToStackRelativeRegister(TR::Register *vreg)
{
	...
	return _registerFile[self()->getFPTopOfStack() - fpReg->getFPStackRegisterNumber()
...
//
TR::Instruction *
OMR::X86::Machine::fpStackFXCH(TR::Instruction *prevInstruction, TR::Register *vreg, bool generateCode)
{
...
//
TR::Instruction *
OMR::X86::Machine::fpStackFXCH(TR::Instruction *prevInstruction, int32_t stackReg)
{
	TR_X86FPStackRegister *pTop = _fpStack[_fpTopOfStack];
        ...
        TR::Instruction *prevInstruction, TR::Register *X, TR::Register *Y, bool strict)
{
	...
	case 1: (void)self()->fpStackFXCH(prevInstruction, X); break;
	...
	default: diagnostic("fpCoerceRegistersToTopOfStack() ==> Invalid stack configuration!"); break;
	}
}
...
OMR::X86::Machine::freeBestFPRegister(TR::Instruction *prevInstruction)
{
	...
	while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != LABEL
	        && cursor->getOpCodeValue() != RET && cursor->getOpCodeValue() != RETImm2
	        && cursor->getOpCodeValue() != ReturnMarker && !cursor->getOpCode().isBranchOp()) {
...
//
TR::Instruction *
OMR::X86::Machine::fpSpillFPR(TR::Instruction *prevInstruction, TR::Register *vreg)
{
	...
	if (fpReg != NULL) {
		...
		cursor = new (self()->cg()->trHeapMemory())
		        TR::X86FPMemRegInstruction(cursor, isFloat ? FSTPMemReg : DSTPMemReg, tempMR,
...
//
TR::Instruction *
OMR::X86::Machine::reverseFPRSpillState(TR::Instruction *prevInstruction, TR::Register *spilledRegister)
{
	TR::Instruction *cursor = prevInstruction;
...
#if defined(DEBUG)
static void
printOneRegisterStatus(TR_FrontEnd *fe, TR::FILE *pOutFile, TR_Debug *debug, TR::RealRegister *reg)
{
	trfprintf(pOutFile, "                      ");
...
void
OMR::X86::Machine::printGPRegisterStatus(TR_FrontEnd *fe, TR::RealRegister **registerFile, TR::FILE *pOutFile)
{
	if (pOutFile == NULL)
		return;
