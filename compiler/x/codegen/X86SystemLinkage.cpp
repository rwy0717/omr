...
TR::X86SystemLinkage::X86SystemLinkage(TR::CodeGenerator *cg) : TR::Linkage(cg) {}
...
int32_t
TR::X86SystemLinkage::computeMemoryArgSize(TR::Node *callNode, int32_t first, int32_t last, int8_t direction)
{
...
TR::X86SystemLinkage::copyParametersToHomeLocation(TR::Instruction *cursor)
{
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		if (lri == NOT_LINKAGE) // It's on the stack
		{
			...
			} else // Method body expects it to be in the ai register
			{
				...
				// ai := stack
				loadCursor = generateRegMemInstruction(loadCursor,
				        TR::Linkage::movOpcodes(RegMem, movDataType), machine->getRealRegister(ai),
		...
		} else // It's in a linkage register
		{
			...
			if (ai == NOT_ASSIGNED || hasToBeOnStack(paramCursor)) {
				if (comp()->getOption(TR_TraceCG))
					traceMsg(comp(),
					        "copyToHomeLocation param %p, linkage reg index %d, allocated index "
					        "%d, parameter offset %d, hasToBeOnStack %d, "
					        "parm->isParmHasToBeOnStack() %d.\n",
			...
			if (ai != NOT_ASSIGNED && ai != sourceIndex) {
				...
				//
				TR_ASSERT(
				        movStatus[ai].sourceReg == noReg, "Each target reg must have only one source");
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		if (movStatus[paramReg].targetReg == 0) {
			...
			//
			TR_ASSERT(noReg == 0, "noReg must be zero (not %d) for zero-filled initialization to work",
			        noReg);
			...
			while (movStatus[regCursor].sourceReg != noReg) {
				...
				if (debug("traceCopyParametersToHomeLocation"))
					diagnostic(
					        "copyParametersToHomeLocation: Moving %d to %d\n", source, regCursor);
...
TR::X86SystemLinkage::savePreservedRegisters(TR::Instruction *cursor)
{
	...
	if (_properties.getUsesPushesForPreservedRegs()) {
		...
		for (int32_t pindex = getProperties().getMaxRegistersPreservedInPrologue() - 1; pindex >= 0; pindex--) {
			TR::RealRegister::RegNum idx = _properties.getPreservedRegister((uint32_t)pindex);
			TR::RealRegister *reg =
			        machine()->getRealRegister(getProperties().getPreservedRegister((uint32_t)pindex));
			if (reg->getHasBeenAssignedInMethod() && reg->getState() != TR::RealRegister::Locked) {
				cursor = generateMemRegInstruction(cursor,
				        TR::Linkage::movOpcodes(MemReg, fullRegisterMovType(reg)),
				        generateX86MemoryReference(
				                machine()->getRealRegister(TR::RealRegister::vfp), offsetCursor, cg()),
				        reg, cg());
...
TR::X86SystemLinkage::createPrologue(TR::Instruction *cursor)
{
	...
	if (_properties.getOutgoingArgAlignment() && !cg()->isLeafMethod()) {
		// AMD64 SysV spec requires: The end of the input argument area shall be aligned on a 16 (32, if __m256
		// is passed on stack) byte boundary. In other words, the value (%rsp + 8) is always a multiple of 16
		// (32) when control is transferred to the function entry point.
		TR_ASSERT(_properties.getOutgoingArgAlignment() == 16 || _properties.getOutgoingArgAlignment() == 4,
		        "AMD64 SysV linkage require outgoingArgAlignment be 16/32 bytes aligned, while IA32 linkage "
		        "require 4 bytes aligned.  We currently haven't support 32 bytes alignment for AMD64 SysV ABI "
		        "yet.\n");
	...
	if (comp()->getOption(TR_TraceCG)) {
		traceMsg(comp(),
		        "create prologue using system linkage, after copyParametersToHomeLocation, cursor is %p.\n",
		        cursor);
	}
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		debugFrameSlotInfo = new (trHeapMemory())
		        TR_DebugFrameSegmentInfo(comp(), paramCursor->getOffset(), paramCursor->getSize(), "Parameter",
		                debugFrameSlotInfo, (ai == NOT_ASSIGNED) ? NULL : machine()->getRealRegister(ai));
	}

	if (properties.getAlwaysDedicateFramePointerRegister()) {
		debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(),
		        properties.getOffsetToFirstLocal(), properties.getGPRWidth(), "EBP-save", debugFrameSlotInfo);
	}
	...
	debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
	        comp(), 0, properties.getRetAddressWidth(), "Return address", debugFrameSlotInfo);
	debugFrameSlotInfo = new (trHeapMemory())
	...
	debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(),
	        properties.getOffsetToFirstLocal() - localSize - preservedRegsSize - adjust, adjust,
	        "adjust size for stack alignment", debugFrameSlotInfo);
	debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(),
...
TR::X86SystemLinkage::restorePreservedRegisters(TR::Instruction *cursor)
{
	...
	if (cg()->pushPreservedRegisters()) {
		...
		for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1; pindex >= 0; pindex--) {
			...
			if (reg->getHasBeenAssignedInMethod()) {
				cursor = generateRegMemInstruction(cursor,
				        TR::Linkage::movOpcodes(RegMem, fullRegisterMovType(reg)), reg,
				        generateX86MemoryReference(
				                machine()->getRealRegister(TR::RealRegister::vfp), offsetCursor, cg()),
				        cg());
				offsetCursor -= pointerSize;
...
TR::X86SystemLinkage::createEpilogue(TR::Instruction *cursor)
{
	...
	if (comp()->getOption(TR_TraceCG)) {
		traceMsg(comp(),
		        "create epilogue using system linkage, after restorePreservedRegisters, cursor is %x.\n",
		        cursor);
	}
	...
	if (_properties.getAlwaysDedicateFramePointerRegister()) {
		...
		//
		cursor = new (trHeapMemory()) TR::X86RegRegInstruction(cursor, MOVRegReg(), espReal,
...
void
TR::X86SystemLinkage::copyGlRegDepsToParameterSymbols(TR::Node *bbStart, TR::CodeGenerator *cg)
{
	...
	if (bbStart->getNumChildren() > 0) {
		...
		TR_ASSERT(
		        glRegDeps->getOpCodeValue() == TR::GlRegDeps, "First child of first Node must be a GlRegDeps");
...
int32_t
TR::X86SystemLinkage::getParameterStartingPos(int32_t &dataCursor, uint32_t align)
{
...
int32_t
TR::X86SystemLinkage::layoutTypeOnStack(TR::DataType type, int32_t &dataCursor, TR::parmLayoutResult &layoutResult)
{
	uint32_t typeAlign = getAlignment(type);
	layoutResult.offset = getParameterStartingPos(dataCursor, typeAlign);
	switch (type) {
	case TR::Float: dataCursor += 4; break;
	case TR::Double: dataCursor += 8; break;
	case TR::Int8: dataCursor += 1; break;
	case TR::Int16: dataCursor += 2; break;
	case TR::Int32: dataCursor += 4; break;
	case TR::Int64: dataCursor += 8; break;
	case TR::Address: dataCursor += TR::Compiler->target.is32Bit() ? 4 : 8; break;
	case TR::Aggregate:
	default:
