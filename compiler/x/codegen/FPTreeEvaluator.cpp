...
TR::Register *
OMR::X86::TreeEvaluator::coerceFPRToXMMR(TR::Node *node, TR::Register *fpRegister, TR::CodeGenerator *cg)
{
	...
	if (fpRegister->isSinglePrecision()) {
		...
		generateRegMemInstruction(
		        MOVSSRegMem, node, xmmRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
	} else {
...
void
OMR::X86::TreeEvaluator::coerceFPOperandsToXMMRs(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::coerceXMMRToFPR(TR::Node *node, TR::Register *xmmRegister, TR::CodeGenerator *cg)
{
	...
	if (xmmRegister->isSinglePrecision()) {
		...
		generateFPRegMemInstruction(
		        FLDRegMem, node, fpRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
	} else {
		fpRegister = cg->allocateRegister(TR_X87);
		TR::MemoryReference *tempMR = cg->machine()->getDummyLocalMR(TR::Double);
		generateMemRegInstruction(MOVSDMemReg, node, tempMR, xmmRegister, cg);
		generateFPRegMemInstruction(
		        DLDRegMem, node, fpRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
	}
...
void
OMR::X86::TreeEvaluator::insertPrecisionAdjustment(TR::Register *reg, TR::Node *root, TR::CodeGenerator *cg)
{
	TR::DataType dt;
	...
	if (cg->useSSEForSinglePrecision()) {
		...
		if (node->getFloatBits() == 0) {
			...
			TR::Instruction *instr =
			        generateRegMemInstruction(MOVSSRegMem, node, targetRegister, tempMR, cg);
			setDiscardableIfPossible(TR_RematerializableFloat, targetRegister, node, instr,
			        (intptrj_t)node->getFloatBits(), cg);
		}
	} else {
...
TR::Register *
OMR::X86::TreeEvaluator::performFload(TR::Node *node, TR::MemoryReference *sourceMR, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister;
	TR::Instruction *instr;
	if (cg->useSSEForSinglePrecision()) {
...
TR::Register *
OMR::X86::TreeEvaluator::performDload(TR::Node *node, TR::MemoryReference *sourceMR, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister;
	TR::Instruction *instr;
	if (cg->useSSEForDoublePrecision()) {
...
TR::Register *
OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if ((valueChild->getOpCodeValue() == TR::ibits2f || valueChild->getOpCodeValue() == TR::lbits2d)
		...
		TR::Node::recreate(
		        node, integerOpCodes[nodeIsIndirect][(valueChild->getOpCodeValue() == TR::ibits2f ? 0 : 1)]);
	...
	if (valueChild->getOpCode().isLoadConst()) {
		if (nodeIs64Bit) {
			if (TR::Compiler->target.is64Bit()) {
				...
				generateMemImmInstruction(S4MemImm4, node, generateX86MemoryReference(*tempMR, 4, cg),
				        valueChild->getLongIntHigh(), cg);
			}
		} else {
			exceptionPoint =
			        generateMemImmInstruction(S4MemImm4, node, tempMR, valueChild->getFloatBits(), cg);
		}
		...
		if (sourceRegister->getKind() == TR_FPR) {
			if (TR::Compiler->target.is64Bit() && tempMR->getSymbolReference().isUnresolved()) {
				...
				if (!tempMR->getSymbolReference().getSymbol()->isShadow()
					...
					// The 64-bit static case does not require the LEA instruction as we can resolve
					// the address in the MOV reg, imm  instruction preceeding the store.
					//
...
TR::Register *
OMR::X86::TreeEvaluator::fpReturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (comp->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
		if (cg->useSSEForDoublePrecision())
			comp->setReturnInfo(
			        (returnRegister->isSinglePrecision()) ? TR_FloatXMMReturn : TR_DoubleXMMReturn);
		else
...
TR::Register *
OMR::X86::TreeEvaluator::fpBinaryArithmeticEvaluator(TR::Node *node, bool isFloat, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::fpUnaryMaskEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (value->getKind() != TR_FPR) // Legacy supported for X87, to be deleted
	{
		...
		if (result != value) {
			generateFPST0STiRegRegInstruction(
			        value->isSinglePrecision() ? FLDRegReg : DLDRegReg, node, result, value, cg);
		}
		...
		generateRegMemInstruction(opcode, node, result,
		        generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, mask), cg), cg);
	}
...
OMR::X86::TreeEvaluator::fpSqrtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (value->getKind() != TR_FPR) // Legacy supported for X87, to be deleted
	{
		...
		if (value->isSinglePrecision()) {
			...
			generateFPRegMemInstruction(
			        FLDRegMem, node, result, generateX86MemoryReference(*tempMR, 0, cg), cg);
		} else {
			...
			generateFPRegMemInstruction(
			        DLDRegMem, node, result, generateX86MemoryReference(*tempMR, 0, cg), cg);
		}
		cg->stopUsingRegister(tmp);
	} else {
		generateRegRegInstruction(
		        value->isSinglePrecision() ? SQRTSSRegReg : SQRTSDRegReg, node, result, value, cg);
	}
...
OMR::X86::TreeEvaluator::fpRemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (cg->useSSEForDoublePrecision()) // Note: both float and double helpers use SSE2
	{
		...
		if (TR::Compiler->target.is64Bit()) {
			...
			TR::SymbolReference *helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
			        nodeIsDouble ? TR_IA32doubleRemainderSSE : TR_IA32floatRemainderSSE, false, false,
			        false);
			targetRegister = TR::TreeEvaluator::performHelperCall(
			        node, helperSymRef, nodeIsDouble ? TR::dcall : TR::fcall, false, cg);
		}
...
TR::Register *
OMR::X86::TreeEvaluator::commonFPRemEvaluator(TR::Node *node, TR::CodeGenerator *cg, bool isDouble)
{
	TR::Node *divisor = node->getSecondChild();
	TR::Node *dividend = node->getFirstChild();
	TR::Compilation *comp = cg->comp();
...
OMR::X86::TreeEvaluator::i2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		if (cg->useSSEForSinglePrecision()) {
			switch (node->getOpCodeValue()) {
			case TR::b2f: generateRegRegInstruction(MOVSXReg4Reg1, node, intReg, intReg, cg); break;
			case TR::bu2f: generateRegRegInstruction(MOVZXReg4Reg1, node, intReg, intReg, cg); break;
			case TR::s2f: generateRegRegInstruction(MOVSXReg4Reg2, node, intReg, intReg, cg); break;
			case TR::su2f: generateRegRegInstruction(MOVZXReg4Reg2, node, intReg, intReg, cg); break;
			case TR::i2f: break;
			default: TR_ASSERT(0, "INVALID OP CODE"); break;
			}
			target = cg->allocateSinglePrecisionRegister(TR_FPR);
			...
			generateFPRegMemInstruction(
			        FILDRegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
			target->setMayNeedPrecisionAdjustment();
			target->setNeedsPrecisionAdjustment();
		}
		cg->decReferenceCount(child);
...
OMR::X86::TreeEvaluator::i2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		if (cg->useSSEForDoublePrecision()) {
			switch (node->getOpCodeValue()) {
			case TR::b2d: generateRegRegInstruction(MOVSXReg4Reg1, node, intReg, intReg, cg); break;
			case TR::bu2d: generateRegRegInstruction(MOVZXReg4Reg1, node, intReg, intReg, cg); break;
			case TR::s2d: generateRegRegInstruction(MOVSXReg4Reg2, node, intReg, intReg, cg); break;
			case TR::su2d: generateRegRegInstruction(MOVZXReg4Reg2, node, intReg, intReg, cg); break;
			case TR::i2d: break;
			default: TR_ASSERT(0, "INVALID OP CODE"); break;
			}
			target = cg->allocateRegister(TR_FPR);
			...
			generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
			generateFPRegMemInstruction(
			        DILDRegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
		}
		cg->decReferenceCount(child);
	}
...
//
TR::Register *
OMR::X86::TreeEvaluator::fpConvertToInt(TR::Node *node, TR::SymbolReference *helperSymRef, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
		...
		loadInstr = generateRegMemInstruction(
		        L4RegMem, node, resultReg, generateX86MemoryReference(*tempMR, 0, cg), cg);
	...
	cg->addSnippet(new (cg->trHeapMemory())
	                TR::X86FPConvertToIntSnippet(reStartLabel, snippetLabel, helperSymRef, loadInstr, cg));
...
//
TR::Register *
OMR::X86::TreeEvaluator::fpConvertToLong(TR::Node *node, TR::SymbolReference *helperSymRef, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
		...
		TR::LabelSymbol *CallLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg); // label where long
		                                                                              // (64-bit) conversion
		                                                                              // will start
		TR::LabelSymbol *StartLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
		...
		loadHighInstr = generateRegMemInstruction(
		        L4RegMem, node, highReg, generateX86MemoryReference(*convertedLongMR, 4, cg), cg);

		loadLowInstr = generateRegMemInstruction(
		        L4RegMem, node, lowReg, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
...
OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	if (cg->useSSEForSinglePrecision()) {
		...
		case TR::d2l:
			...
			longTarget = true;
			break;
		default: TR_ASSERT(0, "Unknown opcode value in f2iEvaluator"); break;
		}
		...
		if (sourceRegister->getKind() == TR_X87 && child->getReferenceCount() == 1) {
			...
			generateRegMemInstruction(
			        CVTTSS2SIReg4Mem, node, targetRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
		} else {
...

		{
			...
			// at this point, target is set to -INF and there can only be THREE possible results: -INF,
			// +INF, NaN compare source with ZERO
			generateRegMemInstruction(doubleSource ? UCOMISDRegMem : UCOMISSRegMem, node, sourceRegister,
			        generateX86MemoryReference(doubleSource ? cg->findOrCreate8ByteConstant(node, 0)
			                                                : cg->findOrCreate4ByteConstant(node, 0),
			                cg),
			        cg);
			// load max int if source is positive, note that for long case, LLONG_MAX << 1 is loaded as it
			...
			generateRegMemInstruction(CMOVARegMem(longTarget), node, targetRegister,
			        generateX86MemoryReference(longTarget
			                        ? cg->findOrCreate8ByteConstant(node, LLONG_MAX << 1)
			...
			generateRegMemInstruction(CMOVPRegMem(longTarget), node, targetRegister,
			        generateX86MemoryReference(longTarget ? cg->findOrCreate8ByteConstant(node, 0)
			                                              : cg->findOrCreate4ByteConstant(node, 0),
			                cg),
			        cg);
		...
		return TR::TreeEvaluator::fpConvertToInt(node,
		        cg->symRefTab()->findOrCreateRuntimeHelper(
		                node->getOpCodeValue() == TR::f2i ? TR_IA32floatToInt : TR_IA32doubleToInt, false,
		                false, false),
		        cg);
	}
}
...
OMR::X86::TreeEvaluator::f2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (cg->useSSEForDoublePrecision()) {
		...
		if (cg->useSSEForSinglePrecision() && sourceRegister->getKind() == TR_FPR) {
			...
			generateFPRegMemInstruction(
			        FLDRegMem, node, targetRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
		} else {
...
OMR::X86::TreeEvaluator::d2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (cg->useSSEForDoublePrecision()) {
		...
		if (cg->useSSEForSinglePrecision()) {
			...
			generateFPMemRegInstruction(FSTMemReg, node, tempMR, sourceRegister, cg);
			generateRegMemInstruction(
			        MOVSSRegMem, node, targetRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
		} else {
			targetRegister = cg->doubleClobberEvaluate(child);
			targetRegister->setMayNeedPrecisionAdjustment();
			targetRegister->setNeedsPrecisionAdjustment();
...
OMR::X86::TreeEvaluator::ibits2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getRegister() == NULL && child->getOpCode().isLoadVar()) {
		...
		if (cg->useSSEForDoublePrecision()) {
			...
			generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
			generateRegMemInstruction(
			        MOVSSRegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
		} else {
			target = cg->allocateSinglePrecisionRegister(TR_X87);
			tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
			generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
			generateFPRegMemInstruction(
			        FLDRegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
		}
	}
...
TR::Register *
OMR::X86::TreeEvaluator::fRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
...
TR::Register *
OMR::X86::TreeEvaluator::dRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	TR::Register *globalReg = node->getRegister();
	if (globalReg == NULL) {
...
TR::Register *
OMR::X86::TreeEvaluator::fRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	...
	if (cg->useSSEForSinglePrecision()) {
		...
		machine->setFPStackRegister(
		        globalRegNum - machine->getNumGlobalGPRs(), toX86FPStackRegister(globalReg));
		...
		} else if (childGlobalReg && childGlobalReg != globalReg) {
			{
				...
				for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
					if (i != fpStackSlot) {
						...
						// else
						//   dumpOptDetails("Global reg %d Stack slot %d iglobalreg %d reg %s
						//   childreg %s\n", globalRegNum, fpStackSlot,
						...
						if (reg == childGlobalReg) {
							...
							generateFPSTiST0RegRegInstruction(FSTRegReg, node,
							        childGlobalReg, childGlobalReg, cg, true);
...
TR::Register *
OMR::X86::TreeEvaluator::dRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	int32_t globalRegNum = node->getGlobalRegisterNumber();
	TR::Machine *machine = cg->machine();
			...
			// if (fpGlobalReg)
			{
				...
				for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
					if (i != fpStackSlot) {
						...
						if (reg == childGlobalReg) {
							TR::Register *popReg = cg->machine()->getFPStackRegister(i);
							generateFPSTiST0RegRegInstruction(DSTRegReg, node,
							        childGlobalReg, childGlobalReg, cg, true);
							cg->stopUsingRegister(childGlobalReg);
							cg->machine()
							        ->getFPStackRegisterNode(fpStackSlot)
	...
	if (accRegister == NULL
		...
		if (generateBranch) {
			...
			generateLabelInstruction(
			        JPE4, node, node->getBranchDestination()->getNode()->getLabel(), deps1, cg);
			generateLabelInstruction(
			        JNE4, node, node->getBranchDestination()->getNode()->getLabel(), deps, cg);
		} else {
		...
		if (generateBranch) {
			generateLabelInstruction(
			        op, node, node->getBranchDestination()->getNode()->getLabel(), deps, cg);
		} else {
			targetRegister = cg->allocateRegister();
			cg->getLiveRegisters(TR_GPR)->setByteRegisterAssociation(targetRegister);
...
//
TR::Register *
OMR::X86::TreeEvaluator::compareFloatOrDoubleForOrder(TR::Node *node, TR_X86OpCodes fpCmpRegRegOpCode,
        ...
        TR_X86OpCodes xmmCmpRegMemOpCode, bool useFCOMIInstructions, TR::CodeGenerator *cg)
{
	if (((TR_X86OpCode::singleFPOp(fpCmpRegRegOpCode) && cg->useSSEForSinglePrecision())
		...
		return temp.fpCompareAnalyser(
		        node, fpCmpRegRegOpCode, fpCmpRegMemOpCode, fpCmpiRegRegOpCode, useFCOMIInstructions);
	}
}

TR::Register *
OMR::X86::TreeEvaluator::generateFPCompareResult(TR::Node *node, TR::Register *accRegister, TR::CodeGenerator *cg)
{
	if (accRegister != NULL) {
		TR::RegisterDependencyConditions *accRegDep = generateRegisterDependencyConditions((uint8_t)1, 1, cg);
		accRegDep->addPreCondition(accRegister, TR::RealRegister::eax, cg);
...
bool
OMR::X86::TreeEvaluator::canUseFCOMIInstructions(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::ILOpCodes cmpOp = node->getOpCodeValue();

	return (!cg->getX86ProcessorInfo().supportsFCOMIInstructions() || cmpOp == TR::iffcmpneu
	               || cmpOp == TR::iffcmpeq || cmpOp == TR::ifdcmpneu || cmpOp == TR::ifdcmpeq
	               || cmpOp == TR::fcmpneu || cmpOp == TR::dcmpneu || cmpOp == TR::fcmpeq || cmpOp == TR::dcmpeq)
	        ? false
	        : true;
}

TR::Register *
OMR::X86::TreeEvaluator::compareFloatAndBranchEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::compareDoubleAndBranchEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
	TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
...
TR::Register *
OMR::X86::TreeEvaluator::compareFloatAndSetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
	TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
	        node, FCOMRegReg, FCOMRegMem, FCOMIRegReg, UCOMISSRegReg, UCOMISSRegMem, useFCOMIInstructions, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::compareDoubleAndSetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
	TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
	        node, DCOMRegReg, DCOMRegMem, DCOMIRegReg, UCOMISDRegReg, UCOMISDRegMem, useFCOMIInstructions, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::compareFloatEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
	TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
	        node, FCOMRegReg, FCOMRegMem, FCOMIRegReg, UCOMISSRegReg, UCOMISSRegMem, useFCOMIInstructions, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::compareDoubleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
	TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
	        node, DCOMRegReg, DCOMRegMem, DCOMIRegReg, UCOMISDRegReg, UCOMISDRegMem, useFCOMIInstructions, cg);
