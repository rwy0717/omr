            ...
            if (targetRegister
                ...
                if (computesCarry) {
                    ...
                    generateRegImmInstruction(
                        AddRegImm4(nodeIs64Bit, isWithCarry), node, targetRegister, constValue, cg);
                } else {
                ...
                if (constValue >= -128 && constValue <= 127) {
                    if (computesCarry) {
                        if (isMemOp)
                            instr = generateMemImmInstruction(
                                AddMemImms(nodeIs64Bit, isWithCarry), node, tempMR, constValue, cg);
                        else
                            instr = generateRegImmInstruction(
                                AddRegImms(nodeIs64Bit, isWithCarry), node, targetRegister, constValue, cg);
                    ...
                    if (isMemOp)
                        ...
                        instr = generateRegImmInstruction(
                            SUBRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
                } else {
                    // comp()->useCompressedPointers
                    // ladd
                    //    iu2l
                    ...
                    if (isMemOp)
                        instr = generateMemImmInstruction(
                            AddMemImm4(nodeIs64Bit, isWithCarry), node, tempMR, constValue, cg);
                    else
                        instr = generateRegImmInstruction(
                            AddRegImm4(nodeIs64Bit, isWithCarry), node, targetRegister, constValue, cg);
                }
        ...
        if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (computesCarry) {
                    if (isMemOp)
                        instr = generateMemImmInstruction(
                            SubMemImms(nodeIs64Bit, isWithBorrow), node, tempMR, constValue, cg);
                    else
                        instr = generateRegImmInstruction(
                            SubRegImms(nodeIs64Bit, isWithBorrow), node, targetRegister, constValue, cg);
                } else if (constValue == 1) {
                    if (isMemOp)
                        instr = generateMemInstruction(DECMem(nodeIs64Bit), node, tempMR, cg);
                    else
                    ...
                    if (isMemOp)
                        ...
                        instr
                            = generateRegImmInstruction(SUBRegImms(nodeIs64Bit), node, targetRegister, constValue, cg);
                }
            } else if ((constValue == 128) && !computesCarry) {
                if (isMemOp)
                    ...
                    instr
                        = generateRegImmInstruction(ADDRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
            } else {
                if (isMemOp)
                    instr = generateMemImmInstruction(
                    ...
                    instr = generateRegImmInstruction(
                        SubRegImm4(nodeIs64Bit, isWithBorrow), node, targetRegister, constValue, cg);
            }
...
TR::Register* OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
        ...
        temp.integerSubtractAnalyser(
            node, SUB4RegReg, SUB2RegMem, MOV4RegReg); // Use MOV4 to avoid unnecessary size prefix
        return node->getRegister();
    }
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                ...
                divideInstr
                    = generateRegMemInstruction(DIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        } else {
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = generateRegRegInstruction(
                        IDIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
            } else {
                TR::MemoryReference* tempMR = generateX86MemoryReference(secondChild, cg);
                if (node->getFirstChild()->isNonNegative() || node->getOpCode().isUnsigned())
                    generateRegRegInstruction(XOR4RegReg, node, edxRegister, edxRegister, edxDeps, cg);
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr
                        = generateRegMemInstruction(IDIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        }
        ...
        if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i || op == TR::l2i) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
                ...
                if (secondChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1
                    ...
                    || TR::Compiler->target.is64Bit()) // cannot use a reigster pair as the shift amount
                {
                    secondChild->decReferenceCount();
                    secondChild = grandChild;
                    if (reportShiftAmount)
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
        ...
        if (node->getOpCode().isXor() && constValue == -1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                if (isMemOp)
                    instr = generateMemImmInstruction(
                        package[TR::TreeEvaluator::logicalByteMemImmOp], node, tempMR, constValue, cg);
                else
                    instr = generateRegImmInstruction(
                ...
                if (isMemOp)
                    instr = generateMemImmInstruction(
                        package[TR::TreeEvaluator::logicalMemImmOp], node, tempMR, constValue, cg);
                else
                    instr = generateRegImmInstruction(
                        package[TR::TreeEvaluator::logicalImmOp], node, targetRegister, constValue, cg);
            }
        }
