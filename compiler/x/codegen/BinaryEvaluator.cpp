...
//
static void
forceSize(TR::Node *node, TR::Register *reg, bool is64Bit, TR::CodeGenerator *cg)
{
		...
		if (firstOp.isAdd()
			...
			if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1
				...
				leaMR = generateX86MemoryReference(cg->evaluate(lrChild),
				        cg->evaluate(llChild->getFirstChild()), stride, displacement, cg);
				...
				leaMR = generateX86MemoryReference(cg->evaluate(llChild),
				        cg->evaluate(lrChild->getFirstChild()), stride, displacement, cg);
...
OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (indexNode) {
		...
		if (constNode) {
			...
			leaMR = generateX86MemoryReference(cg->evaluate(baseNode->getFirstChild()), indexRegister,
			        stride, TR::TreeEvaluator::integerConstNodeValue(constNode, cg), cg);
		...
		bool isInternal = node->getOpCode().isArrayRef() && node->isInternalPointer()
		...
		return true;
	        ...
	        && (firstChild->getRegister() == NULL) && (firstChild->getSecondChild()->getOpCode().isLoadConst())
...
TR::Register *
OMR::X86::TreeEvaluator::integerDualAddOrSubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
		...
		if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
		        ...
		        && performTransformation(comp,
		                "O^O analyseAddForLEA: checking that second node is a memory reference %x\n",
		                constValue)) {
			if (!isMemOp)
			...
			if (targetRegister
				...
				if (constValue >= -128 && constValue <= 127) {
					if (computesCarry) {
						if (isMemOp)
							...
							instr = generateRegImmInstruction(
							        AddRegImms(nodeIs64Bit, isWithCarry), node,
							        targetRegister, constValue, cg);
					} else if (constValue == 1) {
						if (isMemOp)
							instr = generateMemInstruction(
							        INCMem(nodeIs64Bit), node, tempMR, cg);
						else
							instr = generateRegImmInstruction(
						...
						if (isMemOp)
							instr = generateMemInstruction(
							        DECMem(nodeIs64Bit), node, tempMR, cg);
						else
							instr = generateRegImmInstruction(
					...
					if (isMemOp)
						instr = generateMemImmInstruction(AddMemImm4(nodeIs64Bit, isWithCarry),
						        node, tempMR, constValue, cg);
					else
						instr = generateRegImmInstruction(AddRegImm4(nodeIs64Bit, isWithCarry),
						        node, targetRegister, constValue, cg);
				}
				if (debug("traceMemOp"))
					diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d", node,
					        constValue);
			}
		} else if (isMemOp) {
			instr = generateMemRegInstruction(
			        AddMemReg(nodeIs64Bit, isWithCarry), node, tempMR, cg->evaluate(secondChild), cg);
			if (debug("traceMemOp"))
		...
		if (isMemOp) {
			...
			temp.integerAddAnalyser(node, AddRegReg(nodeIs64Bit, isWithCarry),
			        AddRegMem(nodeIs64Bit, isWithCarry), computesCarry, 0);
			targetRegister = node->getRegister();
		}
	}
...
OMR::X86::TreeEvaluator::caddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL
	        && performTransformation(comp,
	                "O^O CaddEvaluator: checking that the store has not happened yet. Target register:  %x\n",
	                testRegister)) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	const TR::ILOpCodes opCode = node->getOpCodeValue();
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	TR::Node *firstChild = node->getFirstChild();
	...
	if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
	        && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg)
	        && performTransformation(comp,
	                "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 bit constant. "
	                "Register value: %x\n",
	                testRegister)) {
		...
		if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
			...
			if (constValue >= -128 && constValue <= 127) {
				if (computesCarry) {
					if (isMemOp)
						instr = generateMemImmInstruction(SubMemImms(nodeIs64Bit, isWithBorrow),
						        node, tempMR, constValue, cg);
					else
						instr = generateRegImmInstruction(SubRegImms(nodeIs64Bit, isWithBorrow),
						        node, targetRegister, constValue, cg);
				} else if (constValue == 1) {
					if (isMemOp)
				...
				if (isMemOp)
					instr = generateMemImmInstruction(
					        SubMemImm4(nodeIs64Bit, isWithBorrow), node, tempMR, constValue, cg);
				else
					instr = generateRegImmInstruction(SubRegImm4(nodeIs64Bit, isWithBorrow), node,
					        targetRegister, constValue, cg);
			}
		...
		instr = generateMemRegInstruction(
		        SubMemReg(nodeIs64Bit, isWithBorrow), node, tempMR, cg->evaluate(secondChild), cg);
		if (debug("traceMemOp"))
		...
		temp.integerSubtractAnalyser(node, SubRegReg(nodeIs64Bit, isWithBorrow),
		        SubRegMem(nodeIs64Bit, isWithBorrow), MOVRegReg(nodeIs64Bit), computesCarry, 0);
...
OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
		...
		TR_X86SubtractAnalyser temp(cg);
		temp.integerSubtractAnalyser(node, SUB4RegReg, SUB2RegMem, MOV4RegReg); // Use MOV4 to avoid unnecessary
		                                                                        // size prefix
		return node->getRegister();
	}
...
OMR::X86::TreeEvaluator::csubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL
	        && performTransformation(comp,
	                "O^O CSubEvaluator: checking that the store has not happened yet. Target register: %x\n",
	                testRegister)) {
		int32_t value = secondChild->getConst<uint16_t>();
		if (!isMemOp)
			targetRegister = cg->evaluate(firstChild);
...
TR::Register *
OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->isDualCyclic() || needsHighMulOnly) {
		...
		// (or result/by-product may be reversed, if lumulh encountered first, and RAX may not be required if
		// lumulh is on its own)
		...
		generateRegRegInstruction(
		        MUL8AccReg, node, lmulTargetRegister, lumulhTargetRegister, multDependencies, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	...
	if (secondChild->getOpCode().isLoadConst() && TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) == 0) {
		...
		if (firstChild->getReferenceCount() > 1)
			cg->evaluate(firstChild); // Respect the evaluation point of the child -- TODO: this could be
			                          // sub-optimal and is likely best handled elsewhere
		cg->recursivelyDecReferenceCount(firstChild);
		cg->decReferenceCount(secondChild);
...
TR::Register *
OMR::X86::TreeEvaluator::signedIntegerDivOrRemAnalyser(TR::Node *node, TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	...
	if (isPowerOf2(dvalue)) {
		...
		if (rootOpCode.isRem()) {
			...
			if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
				...
				if (dvalue == CONSTANT64(0x100000000)) {
					...
					// Set the appropriate bits, making sure the Imm4 has its sign bit clear to
					// leave the top 33 bits alone
					generateRegImmInstruction(OR8RegImm4, node, tempRegister,
		...
		if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
			...
			if (dvalue == 2) // special case when working with 2
			{
				...
				TR_ASSERT(mask >= 0,
				        "AMD64: mask must have sign bit clear so that, when sign-extended, it clears "
				        "the upper 32 bits of tempRegister");
				generateRegImmInstruction(
		...
		generateRegImmInstruction(
		        SARRegImm1(nodeIs64Bit), node, tempRegister, trailingZeroes(dvalue), cdqDependencies, cg);
		...
		generateRegRegInstruction(
		        IMULAccReg(nodeIs64Bit), node, eaxRegister, dividendRegister, multDependencies, cg);
		...
		if (rootOpCode.isRem()) {
			...
			generateRegRegInstruction(
			        SUBRegReg(nodeIs64Bit), node, dividendRegister, edxRegister, multDependencies, cg);
			cg->stopUsingRegister(edxRegister);
...
TR::Register *
OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	...
	if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
		...
		if (needsExplicitOverflowCheck) {
			...
			// Only TR::getMinSigned<TR::Int32>() (or TR::getMinSigned<TR::Int64>()) will overflow when
			// compared with 1
			//
			generateLabelInstruction(LABEL, node, startLabel, cg);
		...
		if (!nodeIs64Bit && node->isUnsigned()) {
			...
			if (divisorRegister) {
				divideInstr = generateRegRegInstruction(
				        DIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
			} else {
			...
			if (divisorRegister) {
				...
				if (node->getOpCode().isUnsigned()
					...
					divideInstr = generateRegRegInstruction(DIVAccReg(nodeIs64Bit), node,
					        eaxRegister, divisorRegister, eaxEdxDeps, cg);
				else
		...
		if (needsExplicitOverflowCheck) {
			...
			cg->addSnippet(new (cg->trHeapMemory()) TR::X86DivideCheckSnippet(restartLabel,
			        overflowSnippetLabel, divisionLabel, node->getOpCode(), node->getType(),
...
OMR::X86::TreeEvaluator::generateRegisterShift(
        TR::Node *node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		intptrj_t shiftAmount = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg)
			...
			if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
				...
				if (secondChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1
					...
					if (reportShiftAmount)
				        ...
				        && grandChild->getReferenceCount() == 1 && grandChild->getRegister() == 0) {
					...
					if (reportShiftAmount)
						diagnostic("shift: removed sign or zero extend to shift amount in "
						           "memory in method %s\n",
						        comp->signature());
				} else if (op != TR::l2i || TR::Compiler->target.is64Bit()) // cannot use a reigster
		...
		instr = generateRegRegInstruction(
		        regShiftOpCode, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
	}
...
OMR::X86::TreeEvaluator::generateMemoryShift(
        TR::Node *node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	...
	if (node->isDirectMemoryUpdate()) {
		TR::TreeEvaluator::generateMemoryShift(node, SHLMemImm1(nodeIs64Bit), SHLMemCL(nodeIs64Bit), cg);
	        ...
	        && (shiftAmount = TR::TreeEvaluator::integerConstNodeValue(shiftAmountNode, cg)
	                        & TR::TreeEvaluator::shiftMask(nodeIs64Bit),
...
TR::Register *
OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *targetRegister = NULL;
	TR::Node *secondChild = node->getSecondChild();
	TR::Node *firstChild = node->getFirstChild();
	...
	if (secondChild->getOpCode().isLoadConst()) {
		intptrj_t rotateAmount = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg)
		...
		generateRegRegInstruction(
		        ROLRegCL(nodeIs64Bit), node, targetRegister, rotateAmountReg, rotateDependencies, cg);
	}
...
TR::Register *
OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	if (node->isDirectMemoryUpdate()) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	TR::Compilation *comp = cg->comp();
...
OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (isMemOp) {
		...
		if (firstChild->getReferenceCount() > 1) {
	        ...
	        && performTransformation(comp,
	                "O^O BUSHREvaluator: first child is not an 8-bit signed two's complement, or an 8 bit unsigned "
	                "%x\n",
	                testOpcode1)) {
		targetRegister = cg->allocateRegister();
...
TR::Register *
OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node, TR_X86OpCodes package[], TR::CodeGenerator *cg)
{
	bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
	TR::Register *targetRegister = NULL;
	TR::Node *firstChild = node->getFirstChild();
	...
	if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
		...
		case TR::Int64: constValue = secondChild->getLongInt(); break;
		case TR::Int32: constValue = secondChild->getInt(); break;
		case TR::Int16: constValue = secondChild->getShortInt(); break;
		case TR::Int8: constValue = secondChild->getByte(); break;
		default:
			TR_ASSERT(
			        0, "Unexpected data type for logical node [" POINTER_PRINTF_FORMAT "]\n", secondChild);
		}
	}
	...
	if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
	        && performTransformation(comp,
	                "O^O checking that the store has not happened yet. Target register: %x\n", testRegister)) {
		...
		if (node->getOpCode().isXor() && constValue == -1) {
			...
			if (constValue >= -128 && constValue <= 127) {
				if (isMemOp)
					...
					instr = generateRegImmInstruction(package[TR::TreeEvaluator::logicalByteImmOp],
					        node, targetRegister, constValue, cg);
			} else {
				if (isMemOp)
					instr = generateMemImmInstruction(package[TR::TreeEvaluator::logicalMemImmOp],
					        node, tempMR, constValue, cg);
				else
					instr = generateRegImmInstruction(package[TR::TreeEvaluator::logicalImmOp],
					        node, targetRegister, constValue, cg);
			}
		}
