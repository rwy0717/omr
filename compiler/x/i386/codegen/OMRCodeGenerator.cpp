...
OMR::X86::I386::CodeGenerator::pickRegister(TR_RegisterCandidate *rc, TR::Block **allBlocks,
	...
	if (node->getOpCode().isIf()) {
		...
		if (node->getFirstChild()->getType().isInt64()) {
			if (node->getOpCode().isBranch()) {
				...
				if ((firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isLoadVarDirect())
					...
					extraRegsAvailable += 0; // TODO: put it back to 2 when looking at GRA, GRA
					                         // pushes allocation of 8 registers
		...
		if (node->getFirstChild()->getOpCodeValue() == TR:: instanceof) {
			if (!TR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(node->getFirstChild(), self())
				...
				return self()->getNumberOfGlobalGPRs() - 4; // ebp plus three other regs if vft masking
				                                            // is enabled
			else
				return 0;
		}

		// All other conditional branches, we usually need one reg for the compare and possibly one for the
		// vmthread
