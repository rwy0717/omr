...
TR::IA32SystemLinkage::IA32SystemLinkage(TR::CodeGenerator *cg) : TR::X86SystemLinkage(cg)
{
	...
	_properties._preservedRegisterMapForGC = ((1 << (TR::RealRegister::ebx - 1))
	        | (1 << (TR::RealRegister::esi - 1)) | (1 << (TR::RealRegister::edi - 1))
	        | (1 << (TR::RealRegister::ebp - 1)) | (1 << (TR::RealRegister::esp - 1)));
...
int32_t
TR::IA32SystemLinkage::layoutParm(TR::Node *parmNode, int32_t &dataCursor, uint16_t &intReg, uint16_t &floatReg,
...
int32_t
TR::IA32SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol, int32_t &dataCursor, uint16_t &intReg,
...
TR::Register *
TR::IA32SystemLinkage::buildVolatileAndReturnDependencies(TR::Node *callNode, TR::RegisterDependencyConditions *deps)
{
	...
	case TR::Int8:
	case TR::Int16:
	case TR::Int32: returnReg = integerReturnReg = cg()->allocateRegister(); break;
	case TR::Address: returnReg = integerReturnReg = cg()->allocateCollectedReferenceRegister(); break;
	case TR::Float: returnReg = fpReturnReg = cg()->allocateSinglePrecisionRegister(TR_X87); break;
	case TR::Double: returnReg = fpReturnReg = cg()->allocateRegister(TR_X87); break;
	case TR::Int64:
		returnReg = longReturnReg =
		        (TR::Register *)cg()->allocateRegisterPair(cg()->allocateRegister(), cg()->allocateRegister());
		break;
	case TR::Aggregate:
	default: TR_ASSERT(false, "return type still not supported");
	}
...
int32_t
TR::IA32SystemLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *deps)
{
	...
	for (int i = callNode->getNumChildren() - 1; i >= firstArg; i--) {
		...
		case TR::Int64:
			...
			break;
		case TR::Aggregate:
		default: TR_ASSERT(0, "Attempted to push unknown type"); break;
		}
	}
	return argSize;
}
...
TR::IA32SystemLinkage::buildDirectDispatch(TR::Node *callNode, bool spillFPRegs)
{
	...
	TR::X86ImmInstruction *instr = generateImmSymInstruction(
	        CALLImm4, callNode, (uintptr_t)methodSymbol->getMethodAddress(), methodSymRef, cg());
	...
	if (cg()->getProperties().getCallerCleanup() && argSize > 0) {
		...
		generateRegImmInstruction(
		        (argSize <= 127) ? ADD4RegImms : ADD4RegImm4, callNode, stackPointerReg, argSize, cg());
	}
