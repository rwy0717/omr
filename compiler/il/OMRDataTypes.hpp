...
#define TR_IGNORED_SIGN_CODE \
	(-1) // a special marker value for when a setSign operation can set any or no sign code value (as it will be
	     // ignored by consumer)

#define TR_BCD_SIGN_CHAR_SIZE 1 // one byte for leading leading +/-/u char
#define TR_BCD_STRLEN \
	(TR_MAX_DECIMAL_PRECISION + TR_BCD_SIGN_CHAR_SIZE + 1) // +1 for sign, +1 for terminating null -- for pretty
	                                                       // printed string e.g. "+1234"
...
enum TR_RawBCDSignCode {
	...
	raw_bcd_sign_0xd = 2,
	raw_bcd_sign_0xf = 3,
	last_raw_bcd_sign = raw_bcd_sign_0xf, // NOTE: the TR_RawBCDSignCode fields in Node::DecimalInfo are 3 bits so
	                                      // the max value for a raw_bcd_sign_* is 7 (enum range is -8->7)
