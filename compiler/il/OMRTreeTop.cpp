...
OMR::TreeTop::duplicateTree()
{
	...
	return self()->create(
	        comp, self()->getNode()->duplicateTree(), self()->getPrevTreeTop(), self()->getNextTreeTop());
}

TR::TreeTop *
OMR::TreeTop::createIncTree(TR::Compilation *comp, TR::Node *node, TR::SymbolReference *symRef, int32_t incAmount,
        TR::TreeTop *precedingTreeTop, bool isRecompCounter)
{
...
TR::TreeTop *
OMR::TreeTop::createResetTree(TR::Compilation *comp, TR::Node *node, TR::SymbolReference *symRef, int32_t resetAmount,
        TR::TreeTop *precedingTreeTop, bool isRecompCounter)
{
	TR::StaticSymbol *symbol = symRef->getSymbol()->castToStaticSymbol();
	TR::DataType type = symbol->getDataType();
	...
	if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() && type != TR::Address) {
		...
		storeNode = TR::Node::createWithSymRef(
		        TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0, resetAmount), symRef);
	}
	return precedingTreeTop == NULL ? TR::TreeTop::create(comp, storeNode)
	                                : TR::TreeTop::create(comp, precedingTreeTop, storeNode);
}
...
OMR::TreeTop::TreeTop(TR::TreeTop *precedingTreeTop, TR::Node *node, TR::Compilation *c) : _pNode(node)
{
...
OMR::TreeTop::adjustBranchOrSwitchTreeTop(TR::Compilation *comp, TR::TreeTop *oldTargetTree, TR::TreeTop *newTargetTree)
{
	...
	if (opCode.isBranch()) {
		...
		for (int32_t i = 0; i < self()->getNode()->getNumChildren() - 1; ++i) // we don't care about the last
		                                                                      // monitor child
		{
			TR::TreeTop *branchTreeTop = self()->getNode()->getChild(i)->getBranchDestination();
			if (branchTreeTop == oldTargetTree) {
...
{
	TR::Node *defNode = self()->getNode()->getOpCodeValue() == TR::treetop ? self()->getNode()->getFirstChild()
	                                                                       : self()->getNode();
