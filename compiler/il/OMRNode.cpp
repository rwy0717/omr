        ...
        , _unionPropertyA()
{
	...
	TR_ASSERT(self()->hasSymbolReference() + self()->hasRegLoadStoreSymbolReference()
	        ...
	        self()->hasRegLoadStoreSymbolReference(), self()->hasBranchDestinationNode(), self()->hasBlock(),
	        self()->hasArrayStride(), self()->hasPinningArrayPointer(), self()->hasDataType());
}
...
OMR::Node::copyValidProperties(TR::Node *fromNode, TR::Node *toNode)
{
...
#endif
			break;
		case HasBranchDestinationNode: toNode->setBranchDestination(fromNode->getBranchDestination()); break;
		case HasBlock: toNode->setBlock(fromNode->getBlock()); break;
		case HasArrayStride: toNode->setArrayStride(fromNode->getArrayStride()); break;
		case HasPinningArrayPointer: toNode->setPinningArrayPointer(fromNode->getPinningArrayPointer()); break;
		case HasDataType: toNode->setDataType(fromNode->getDataType()); break;
		default:
			/* HasNoUnionPropertyA */
			break;
		}
	...
	bool fromConst = fromNode->getOpCode().isLoadConst()
	        && (fromNode->getType().isIntegral() || fromNode->getType().isAddress());
	bool toConst = toNode->getOpCode().isLoadConst()
	...
	if (toNode->_numChildren == 0 || !toNode->hasNodeExtension()) {
		/* if (fromNode->getOpCode().getOpCodeValue() == TR::allocationFence)
		   self()->setAllocation(NULL); */
		{
			// may be using other property that is unioned with _unionBase._children but do not know which
			// one so have to copy it forward irregardless of whether it is valid or not
			toNode->_unionBase = fromNode->_unionBase;
		}
	}

	// nameSymRef & callNodeData, if set on formNode, is already set for toNode (its looked up via globalIndex which
	// is unchanged)
...
{
	TR_ASSERT_FATAL(
	        TR::Node::isNotDeprecatedUnsigned(op), "Trying to use a deprecated unsigned opcode: #%d \n", op);
	...
	// we can't delete the node, otherwise we will potentially lose the poolIndex (and other properties), but we do
	// need to deallocate any objects it has created. This is one of them. It will be recreated if necessary on
	// createInternal. other properties such as nameSymRef & callNodeData, are associated with the _globalIndex and
	// are not freed and realloced.
        ...
        TR::Node *originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *originalNode)
{
	...
	if (!originalNode)
		...
		const TR_ByteCodeInfo byteCodeInfo = originalNode->getByteCodeInfo(); // copy bytecode info into
		                                                                      // temporary variable
...
{
	TR_ASSERT(
	        op != TR::bconst && op != TR::cconst && op != TR::sconst, "Invalid constructor for 8/16-bit constants");
...
TR::Node *
OMR::Node::createWithSymRef(TR::Node *originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
...
TR::Node *
OMR::Node::recreateWithoutProperties(
        TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::SymbolReference *symRef)
{
	...
	if (symRef != NULL || node->hasSymbolReference() || node->hasRegLoadStoreSymbolReference()) {
		// don't attempt to set this if the symRef is NULL and the node isn't a symRef type; otherwise attempt
		// to do so
		node->setSymbolReference(symRef);
	}
	return node;
...
TR::Node *
OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
        uintptr_t extraChildren, TR::SymbolReference *symRef)
{
	TR::Node *node = TR::Node::createWithSymRef(op, numChildren, first, extraChildren, symRef); // explictly cast to
	                                                                                            // avoid warning
...
TR::Node *
OMR::Node::recreateWithoutSymRef_va_args(
...
TR::Node *
OMR::Node::createWithoutSymRef(TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...)
{
	va_list args;
...
TR::Node *
OMR::Node::recreateWithoutSymRef(
...
TR::Node *
OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
	va_list args;
	va_start(args, numChildArgs);
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::Node *fourth, TR::Node *fifth)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::Node *sixth)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::Node *sixth, TR::Node *seventh)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
	return TR::Node::recreateWithoutSymRef(
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::SymbolReference *symRef)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::SymbolReference *symRef)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::Node *fourth, TR::SymbolReference *symRef)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
	return TR::Node::recreateWithSymRefWithoutProperties(
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
        TR::Node *second, TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::SymbolReference *symRef)
{
	TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
	return TR::Node::recreateWithSymRefWithoutProperties(
...
TR::Node *
OMR::Node::createWithRoomForOneMore(TR::ILOpCodes op, uint16_t numChildren, void *symbolRefOrBranchTarget,
        TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth, TR::Node *fifth)
{
	...
	if (symbolRefOrBranchTarget != NULL || node->hasSymbolReference() || node->hasBranchDestinationNode()) {
		// don't attempt to set this if the symbolRefOrBranchTarget is NULL and the node isn't one of the valid
		// types
		if (node->hasSymbolReference())
			...
			TR_ASSERT(false,
			        "attempting to access _branchDestinationNode or _symbolReference field for node %s %p "
			        "that does not have one of them",
...
{
	// So far extra 3rd child could be added to the following nodes: if, variableNewArray, arraystorechk or a node
	// with a padding address these nodes are created with room for three children
	TR::Node *node;
	...
	TR_ASSERT(node->getOpCode().isIf() || (node->getOpCodeValue() == TR::variableNewArray)
	        ...
	        "createWithRoomForThree should only be used to create an ifcmp, arraystorechk, variableNewArray node "
	        "or a node with a padding address");
#else
	TR_ASSERT(node->getOpCode().isIf() || (node->getOpCodeValue() == TR::variableNewArray),
	        "createWithRoomForThree should only be used to create an ifcmp, arraystorechk, variableNewArray node "
	        "or a node with a padding address");
...
TR::Node *
OMR::Node::createWithRoomForFive(TR::ILOpCodes op, TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
...
OMR::Node::createConstZeroValue(TR::Node *originatingByteCodeNode, TR::DataType dt)
{
	...
	switch (dt) {
	case TR::Int8: constZero = TR::Node::bconst(originatingByteCodeNode, 0); break;
	case TR::Int16: constZero = TR::Node::sconst(originatingByteCodeNode, 0); break;
	case TR::Int32: constZero = TR::Node::iconst(originatingByteCodeNode, 0); break;
	case TR::Int64: constZero = TR::Node::lconst(originatingByteCodeNode, 0); break;
	case TR::Address: constZero = TR::Node::aconst(originatingByteCodeNode, 0); break;
	case TR::Float: {
	...
	default: TR_ASSERT_SAFE_FATAL(false, "datatype %s not supported for createConstZeroValue\n", dt.toString());
	}
	return constZero;
}
...
OMR::Node::createConstOne(TR::Node *originatingByteCodeNode, TR::DataType dt)
{
	...
	switch (dt) {
	case TR::Int8: constOne = TR::Node::bconst(originatingByteCodeNode, 1); break;
	case TR::Int16: constOne = TR::Node::sconst(originatingByteCodeNode, 1); break;
	case TR::Int32: constOne = TR::Node::iconst(originatingByteCodeNode, 1); break;
	case TR::Int64: constOne = TR::Node::lconst(originatingByteCodeNode, 1); break;
	case TR::Address: constOne = TR::Node::aconst(originatingByteCodeNode, 1); break;
	case TR::Float:
	...
	case TR::Double:
		...
		break;
	default: TR_ASSERT_SAFE_FATAL(false, "datatype %s not supported for createConstOne\n", dt.toString());
	}
	return constOne;
}
...
OMR::Node::createConstDead(TR::Node *originatingByteCodeNode, TR::DataType dt, intptrj_t extraData)
{
	...
	memset(buf, 0, 20);
	switch (dt) {
	case TR::Int8: result = TR::Node::bconst(originatingByteCodeNode, dead8); break;
	case TR::Int16: result = TR::Node::sconst(originatingByteCodeNode, dead16); break;
	case TR::Int32: result = TR::Node::iconst(originatingByteCodeNode, dead32); break;
	case TR::Int64: result = TR::Node::lconst(originatingByteCodeNode, dead64); break;
	case TR::Address:
	...
	case TR::Double:
		...
		break;
	default: TR_ASSERT_SAFE_FATAL(false, "datatype %s not supported for createConstDead\n", dt.toString());
	}
	return result;
}
...
OMR::Node::duplicateTree(bool duplicateChildren)
{
	...
	// Would like to enable this, but some optimizations (e,g. Field Privatization) seem to rely on the old
	// behaviour of duplicateTree TR::Node *safeNode = safeDuplicateTree(duplicateChildren);

	// traceMsg(TR::comp(), "--------------\n duplicateTree duplicating %p unsafeNode %p safeNode %p
...
OMR::Node::isConstZeroBytes()
{
	...
	case TR::Int8: return self()->getByte() == 0;
	case TR::Int16: return self()->getShortInt() == 0;
	case TR::Int32: return self()->getInt() == 0;
	case TR::Int64: return self()->getLongInt() == 0;
	case TR::Address: return self()->getAddress() == 0;
	case TR::Float: return self()->getFloatBits() == 0;
	case TR::Double: return self()->getDoubleBits() == 0;
	case TR::VectorInt8:
	case TR::VectorInt16:
	case TR::VectorInt32:
	case TR::VectorInt64:
	case TR::VectorDouble:
	default: TR_ASSERT(false, "Unrecognized const node %s can't be checked for zero bytes"); return false;
	}
}
...
OMR::Node::isConstZeroValue()
{
	...
	case TR::VectorInt64:
	case TR::VectorDouble:
	default: TR_ASSERT(false, "Unrecognized constant node can't be checked for zero"); return false;
	}
}
...
OMR::Node::computeIsInternalPointer()
{
	TR_ASSERT(self()->getOpCode().hasPinningArrayPointer(),
	        "Opcode %s is not supported, node is " POINTER_PRINTF_FORMAT, self()->getOpCode().getName(), self());
	return self()->computeIsCollectedReference();
}
...
recordProcessedNodeResult(TR::Node *node, TR_YesNoMaybe collectedness, TR::NodeChecklist &processedNodesCollected,
        TR::NodeChecklist &processedNodesNotCollected)
{
	switch (collectedness) {
	case TR_yes: processedNodesCollected.add(node); break;
	case TR_no: processedNodesNotCollected.add(node); break;
	case TR_maybe:
		processedNodesCollected.add(node);
		processedNodesNotCollected.add(node);
		break;
	default: TR_ASSERT_FATAL(false, "Invalid collectedness result for Node %p\n", node); break;
	}
        ...
        TR::NodeChecklist &processedNodesCollected, TR::NodeChecklist &processedNodesNotCollected)
{
	...
	// -- The node is not contained in either collected or uncollected checklists - first time encounter, process
	// the node
	...
	while (curNode) {
		...
		if (op.isLoadVar() || op.isLoadAddr() || op.isLoadReg()) {
			TR_ASSERT(curNode->hasSymbolReference() || curNode->hasRegLoadStoreSymbolReference(),
			        "node " POINTER_PRINTF_FORMAT "(%s) should have symbol reference", curNode,
			        op.getName());
			...
			if (opValue == TR::aloadi && symbol == TR::comp()->getSymRefTab()->findGenericIntShadowSymbol())
				...
				return recordProcessedNodeResult(receiverNode,
				        (symbol->isCollectedReference() ? TR_yes : TR_no), processedNodesCollected,
...
OMR::Node::mayModifyValue(TR::SymbolReference *symRef)
{
	...
	if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monexit
		...
		//
		return !symbol->isAutoOrParm() && !(symbol->isStatic() && symbol->isConst())
	...
	if (node->getOpCode().isStore()) {
		...
		if (symbol->isStatic()) {
			...
			if (storeSymbol->isStatic()) {
				...
				if (symRef->isUnresolved() || storeRef->isUnresolved())
					return TR::Compiler->cls.jitStaticsAreSame(c, storeRef->getOwningMethod(c),
					        storeRef->getCPIndex(), symRef->getOwningMethod(c),
					        symRef->getCPIndex());
		...
		if (symbol->isShadow() && storeSymbol->isShadow()) {
			...
			TR::Compilation *c = TR::comp();
			return TR::Compiler->cls.jitFieldsAreSame(c, storeRef->getOwningMethod(c),
			        storeRef->getCPIndex(), symRef->getOwningMethod(c), symRef->getCPIndex(), false);
		}
	}
	...
	if ((((constNode->isClassPointerConstant())
	            ...
	            || ((constNode->isMethodPointerConstant()) && !c->compileRelocatableCode()
	                    && !TR::Compiler->cls.sameClassLoaders(c,
	                            c->fe()->createResolvedMethod(c->trMemory(),
...
{
	switch (self()->getDataType()) {
	case TR::Int8: return TR::getMaxSignedPrecision<TR::Int8>(); break;
...
OMR::Node::nodeMightKillCondCode()
{
	...
	if (opcode.isArrayRef() && self()->getSecondChild()->getOpCode().isLoadConst()
	        ...
	        && self()->getSecondChild()->get64bitIntegralValue() < 4096) // address increment can use LA or fold
	                                                                     // into MemoryReference
		return false;
#endif
...
OMR::Node::extractTheNullCheck(TR::TreeTop *prevTreeTop)
{
	...
	TR::Node *newNullCheckNode =
	        TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passThru, self()->getSymbolReference());
...
OMR::Node::exceptionsRaised()
{
	...
	case TR::ZEROCHK: possibleExceptions |= TR::Block::CanCatchEverything; break;
	case TR::DIVCHK: possibleExceptions |= TR::Block::CanCatchDivCheck; break;
	case TR::BNDCHK:
	case TR::BNDCHKwithSpineCHK:
	case TR::ArrayCopyBNDCHK: possibleExceptions |= TR::Block::CanCatchBoundCheck; break;
	case TR::ArrayStoreCHK:
	case TR::ArrayCHK: possibleExceptions |= TR::Block::CanCatchArrayStoreCheck; break;
	case TR::arraycopy:
	...
	case TR::irsstore:
	case TR::iristore:
	case TR::irlstore: break;
#endif
	case TR::arraycmp: // does not throw any exceptions
		break;
	case TR::checkcast: possibleExceptions |= TR::Block::CanCatchCheckCast; break;
	case TR::checkcastAndNULLCHK:
		possibleExceptions |= TR::Block::CanCatchCheckCast;
		possibleExceptions |= TR::Block::CanCatchNullCheck;
		break;
	case TR::New: possibleExceptions |= TR::Block::CanCatchNew; break;
	case TR::newarray:
	case TR::anewarray:
	case TR::multianewarray: possibleExceptions |= TR::Block::CanCatchArrayNew; break;
	case TR::MergeNew:
		possibleExceptions |= TR::Block::CanCatchNew;
		possibleExceptions |= TR::Block::CanCatchArrayNew;
		break;
	case TR::monexit: possibleExceptions |= TR::Block::CanCatchMonitorExit; break;
	case TR::monexitfence: possibleExceptions |= TR::Block::CanCatchMonitorExit; break;
	case TR::athrow: possibleExceptions |= TR::Block::CanCatchUserThrows; break;
#ifdef J9_PROJECT_SPECIFIC
	case TR::BCDCHK: possibleExceptions |= TR::Block::CanCatchUserThrows; break;
#endif
	default:
...
OMR::Node::createStoresForVar(TR::SymbolReference *&nodeRef, TR::TreeTop *insertBefore, bool simpleRef)
{
	...
	if (isInternalPointer && self()->getOpCode().isArrayRef()
		...
		if (!storesNeedToBeCreated) {
            ...
            {
				TR::SymbolReference *newIntOrLongRef = comp->getSymRefTab()->createTemporary(
				        comp->getMethodSymbol(), secondChild->getDataType());
	...
	if (storesNeedToBeCreated) {
		...
		if (isInternalPointer) {
			...
			if (self()->getOpCode().isArrayRef()) {
				...
				pinningArray = self()->getSymbolReference()
				                       ->getSymbol()
				                       ->castToInternalPointerAutoSymbol()
				                       ->getPinningArrayPointer();
...
OMR::Node::recursivelyDecReferenceCount()
{
	...
	if (self()->getReferenceCount() > 0)
		...
		TR_ASSERT(self()->getOpCode().isTreeTop(),
		        "OMR::Node::recursivelyDecReferenceCount() invoked for nontreetop node "
		        "%s " POINTER_PRINTF_FORMAT " with count == 0",
...
OMR::Node::get64bitIntegralValue()
{
	...
	if (type.isInt8())
		...
		return self()->getAddress(); // TR::Compiler->target.is64Bit() ? getUnsignedLongInt() :
		                             // getUnsignedInt();
	else {
...
{
	TR_ASSERT(
	        (self()->getOpCodeValue() == TR::allocationFence), "Opcode must be TR::allocationFence in this method");
	...
	if (numElems > NUM_DEFAULT_CHILDREN) {
		if (self()->hasNodeExtension())
			self()->copyNodeExtension(
			        _unionBase._extension.getExtensionPtr(), numElems, self()->sizeOfExtension());
		else
			self()->createNodeExtension(numElems);
	}
...
OMR::Node::getSymbolReferenceOfAnyType()
{
	TR_ASSERT(self()->hasSymbolReferenceOfAnyType(),
	        "attempting to access a regLoad/regStore or normal symbolReference for node %s %p that does not have "
	        "it",
	        self()->getOpCode().getName(), this);
	if (self()->hasRegLoadStoreSymbolReference())
		return self()->getRegLoadStoreSymbolReference();
...
OMR::Node::getSymbol()
{
	...
	if (self()->hasRegLoadStoreSymbolReference())
		return self()->getRegLoadStoreSymbolReference() ? self()->getRegLoadStoreSymbolReference()->getSymbol()
		                                                : NULL;
#else
...
OMR::Node::getArrayStride()
{
	TR_ASSERT(self()->hasArrayStride(),
	        "attempting to access _arrayStride field for node %s %p that does not have it",
	        self()->getOpCode().getName(), this);
...
OMR::Node::computeDataType()
{
	...
	if (self()->getNumChildren() > 0) {
		...
		if (_opCode.isVector()) {
			// Vector comparison returning resultant vector should return vector bool int type, WCode ACR
			// 265
			if (_opCode.isBooleanCompare())
				_unionPropertyA._dataType =
				        self()->getFirstChild()->getDataType().getVectorIntegralType().getDataType();
			else if (_opCode.isVectorReduction())
				_unionPropertyA._dataType =
				        self()->getFirstChild()->getDataType().getVectorElementType().getDataType();
		...
		if (_opCode.getOpCodeValue() == TR::getvelem)
			return _unionPropertyA._dataType =
			               self()->getFirstChild()->getDataType().vectorToScalar().getDataType();
	}
...
OMR::Node::setDirectMemoryUpdate(bool v)
{
	TR::Compilation *c = TR::comp();
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting directMemoryUpdate flag on node %p to %d\n", self(), v))
...
OMR::Node::setIsNonNegative(bool v)
{
	TR::Compilation *c = TR::comp();
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting nodeIsNonNegative flag on node %p to %d\n", self(), v))
...
OMR::Node::setIsNonPositive(bool v)
{
	TR::Compilation *c = TR::comp();
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting nodeIsNonPositive flag on node %p to %d\n", self(), v))
...
OMR::Node::setStoredValueIsIrrelevant(bool v)
{
	...
	TR_ASSERT(c->getOption(TR_EnableOSR),
	        "Currently this OMR::Node flag means different things on different front ends. If we wanted to reuse "
	        "this query across front ends other than Java (which uses these kinds of stores for OSR) then we "
	        "probably need to map it to a different bit in the OMR::Node flags");
	TR_ASSERT((self()->getOpCode().isStore() && self()->getSymbolReference()->getSymbol()->isAutoOrParm()),
	        "Opcode must be store to a stack slot");
...
OMR::Node::setHeapificationStore(bool v)
{
	...
	TR_ASSERT((self()->getOpCodeValue() == TR::astore), "Opcode must be astore");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting HeapificationStore flag on node %p to %d\n", self(), v))
...
OMR::Node::setHeapificationAlloc(bool v)
{
	...
	TR_ASSERT(self()->getOpCode().isNew(), "Opcode must be isNew");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting HeapificationAlloc flag on node %p to %d\n", self(), v))
...
OMR::Node::setDesynchronizeCall(bool v)
{
	...
	TR_ASSERT(self()->getOpCode().isCall(), "Opcode must be a call");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting desynchronizeCall flag on node %p to %d\n", self(), v))
...
OMR::Node::setTermCharNodeIsHint(bool v)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate, "Opcode must be arraytranslate");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting termCharNodeIsHint flag on node %p to %d\n", self(), v))
	...
	if (v) {
		...
		if (self()->isHalfWordElementArrayCopy()
		        && performNodeTransformation2(c,
		                "O^O NODE FLAGS: Setting halfWordElementArrayCopy flag on node %p to %d\n", self(), v))
...
OMR::Node::setRarePathForwardArrayCopy(bool v)
{
	...
	if (v != self()->isRarePathForwardArrayCopy()
		...
		_flags.setValue(
		        arraycopyDirectionMask, v ? arraycopyDirectionForwardRarePath : arraycopyDirectionUnknown);
	if (v)
		...
		TR_ASSERT(
		        !self()->isRarePathForwardArrayCopy() && (wasForwardArrayCopy == self()->isForwardArrayCopy()),
		        "assertion failure");
}
...
{
	TR_ASSERT(self()->getOpCodeValue() == TR::bitOpMem || self()->getOpCodeValue() == TR::bitOpMemND,
...
OMR::Node::setIsNonHeapObjectWrtBar(bool v)
{
	...
	TR_ASSERT(self()->getOpCode().isWrtBar(), "Opcode must be wrtbar");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting nonHeapObjectWrtBar flag on node %p to %d\n", self(), v))
...
OMR::Node::setNeedsSignExtension(bool b)
{
	...
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting needsSignExtension flag on node %p to %d\n", self(), b))
...
OMR::Node::setSkipSignExtension(bool b)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::iRegLoad || self()->getOpCodeValue() == TR::iadd
	        ...
	        "assertion failure");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting skipSignExtension flag on node %p to %d\n", self(), b))
...
{
	TR_ASSERT(self()->canChkNodeCreatedByPRE(), "Unexpected opcode %s in chkNodeCreatedByPRE()",
	        self()->getOpCode().getName());
...
OMR::Node::setIsMutableCallSiteTargetGuard()
{
	...
	TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
	if (performNodeTransformation1(
	            c, "O^O NODE FLAGS: Setting mutableCallSiteTargetGuard flag on node %p\n", self()))
...
OMR::Node::setIsMethodEnterExitGuard(bool b)
{
	...
	if (b
	        && performNodeTransformation2(
	                c, "O^O NODE FLAGS: Setting methodEnterExitGuard flag to %d on node %p\n", b, self()))
...
OMR::Node::setIsDirectMethodGuard(bool b)
{
	...
	if (b
	        && performNodeTransformation2(
	                c, "O^O NODE FLAGS: Setting directMethodGuard flag to %d on node %p\n", b, self()))
...
OMR::Node::setSeenRealReference(bool b)
{
	...
	TR_ASSERT(self()->getOpCode().isLoadReg(), "assertion failure");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting seenRealReference flag on node %p to %d\n", self(), b))
...
OMR::Node::setSyncMethodMonitor(bool v)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::monent || self()->getOpCodeValue() == TR::monexit,
	        "Opcode must be monent or monexit");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting syncMethodMonitor flag on node %p to %d\n", self(), v))
...
OMR::Node::setLocalObjectMonitor(bool v)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::monent || self()->getOpCodeValue() == TR::monexit,
	        "Opcode must be monent or monexit");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting localObjectMonitor flag on node %p to %d\n", self(), v))
...
OMR::Node::setEscapesInColdBlock(bool v)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr, "Opcode must be loadaddr");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting escapesInColdBlock flag on node %p to %d\n", self(), v))
...
OMR::Node::setCannotTrackLocalStringUses(bool v)
{
	...
	TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr, "Opcode must be loadaddr (flag shared on PLX)");
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting cannotTrackString flag on node %p to %d\n", self(), v))
...
{
	TR_ASSERT((self()->getOpCode().isLeftShift() || self()->getOpCode().isRightShift()
...
{
	return (self()->getOpCode().isLeftShift() || self()->getOpCode().isRightShift())
...
OMR::Node::setUnneededConversion(bool v)
{
	...
	if (performNodeTransformation2(
	            c, "O^O NODE FLAGS: Setting unneededConversion flag on node %p to %d\n", self(), v))
...
{
	TR_ASSERT(self()->getOpCode().isConversion(), "Opcode must be a conv");
	TR_ASSERT(self()->getReferenceCount() <= 1,
	        "Lazy clobber requires node reference count of 1 (actual value is %d)", self()->getReferenceCount());
