...
bool OMR::Node::isTernaryHigh()
{
    if (((self()->getOpCodeValue() == TR::luaddc) || (self()->getOpCodeValue() == TR::lusubb))
        ...
        if ((ccOpValue == TR::computeCC) && (((self()->getOpCodeValue() == TR::luaddc) && (pairOpValue == TR::luadd)) || ((self()->getOpCodeValue() == TR::lusubb) && (pairOpValue == TR::lusub))))
            return true;
    }
    return false;
}
...
bool OMR::Node::nodeMightKillCondCode()
{
    ...
    if (((opcode.isLoad() || opcode.isStore() || opcode.isLoadReg() || opcode.isStoreDirectOrReg() || opcode.isLoadAddr())
            && (self()->getSize() == 4 || self()->getSize() == 8 || self()->getSize() == 2 || (self()->getSize() == 1 && TR::Compiler->target.cpu.getS390SupportsZ9()) || self()->getSize() == 0)))
        return false;
...
bool OMR::Node::isEligibleForHighWordOpcode()
{
    ...
    case TR::idiv:
        if (firstChild->getIsHPREligible()) {
            if (secondChild->isPowerOfTwo() || (secondChild->getOpCodeValue() == TR::iconst && ((secondChild->getInt() % 2) == 0))) {
