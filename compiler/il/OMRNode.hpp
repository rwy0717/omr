...
public:
	...
	static TR::Node *create(
	        TR::Node *originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node *first);
	...
	static TR::Node *createWithSymRef(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first,
...
private:
	...
	template <class... ChildrenAndSymRefType>
	static TR::Node *recreateWithSymRefWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op,
	        uint16_t numChildren, uint16_t numChildArgs, TR::Node *first,
...
public:
	...
	static TR::Node *create(
	        TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second, TR::Node *third);
	static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
	        TR::Node *third, TR::Node *fourth);
	static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
	        TR::Node *third, TR::Node *fourth, TR::Node *fifth);
	static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
	        TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::Node *sixth);
	static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
	        TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::Node *sixth, TR::Node *seventh);
	static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren, TR::Node *first, TR::Node *second,
	        TR::Node *third, TR::Node *fourth, TR::Node *fifth, TR::Node *sixth, TR::Node *seventh,
	        TR::Node *eighth);
	...
	static TR::Node *createWithRoomForOneMore(TR::ILOpCodes op, uint16_t numChildren,
	        void *symbolRefOrBranchTarget = 0, TR::Node *first = 0, TR::Node *second = 0, TR::Node *third = 0,
...
public:
	...
	bool containsNode(TR::Node *searchNode, vcount_t visitCount); // Careful how you use this: it doesn't account
	                                                              // for aliasing
	...
	TR::TreeTop *createStoresForVar(
	        TR::SymbolReference *&nodeRef, TR::TreeTop *insertBefore, bool simpleRef = false);
	...
	/** \brief
	 *     Determines whether this node should be sign/zero extended to 32-bit at the point of evaluation (source)
	 * by checking for the signExtendTo32BitAtSource or zeroExtendTo32BitAtSource flags.
	 */
	bool isExtendedTo32BitAtSource();

	/** \brief
	 *     Determines whether this node should be sign/zero extended to 64-bit at the point of evaluation (source)
	 * by checking for the signExtendTo64BitAtSource or zeroExtendTo64BitAtSource flags.
	 */
	bool isExtendedTo64BitAtSource();

	/** \brief
	 *     Determines whether this node should be sign extended at the point of evaluation (source) by checking for
	 * the signExtendTo32BitAtSource or signExtendTo64BitAtSource flags.
	 */
	bool isSignExtendedAtSource();

	/** \brief
	 *     Determines whether this node should be zero extended at the point of evaluation (source) by checking for
	 * the zeroExtendTo32BitAtSource or zeroExtendTo64BitAtSource flags.
	 */
	...
	void rotateChildren(int32_t first, int32_t last); ///< @note when finsihed, last child ends up where first child
	                                                  ///< used to be
...
protected:
	...
	union UnionPropertyA {
		...
		TR::DataTypes _dataType; ///< hasDataType()  TODO: Change to TR::DataType once all target compilers
		                         ///< support it
...
protected:
	enum {
		...
		invalid8BitGlobalRegister = 0x00000010, ///< value is in a global register which cannot be used on an 8
		                                        ///< bit instruction
		...
		arraysetLengthMultipleOfPointerSize = 0x00020000, ///< flag used by TR::arrayset to guarantee that
		                                                  ///< checking for lengths smaller than pointer size is
		                                                  ///< not needed (leading to more efficient code being
		                                                  ///< generated, e.g. REP STOSQ on X86
		...
		/** \brief
		 *     Represents that a load must be sign/zero extended to a particular width at the point of
		 * evaluation.
		 *
		 *  \details
		 *     This flag is used by load nodes to signal the code generator to emit a load and sign/zero extend
		 *     instructions for the evaluation of this particular load. These flags are used in conjunction with
		 * the unneededConv flag to avoid sign/zero extension conversions which the respective load feeds into.
		 */
		 ...
		 *
		 *  \details
		 *     This flag is used by conversion nodes to signal the code generator that the respective conversion
		 * is not needed because the value the conversion is acting on has been cleansed prior to reaching this
		 * evaluation point.
		 */
		unneededConv = 0x00000400, ///< Flag used by TR::x2y
		ParentSupportsLazyClobber = 0x00002000, ///< Tactical x86 codegen flag.  Only when refcount <= 1.
		                                        ///< Indicates that parent will consult the register's node
		                                        ///< count before clobbering it (not just the node's refcount).
		...
		referenceIsNonNull = 0x00008000, ///< Sometimes we can't mark the child as non-null (because it's not
		                                 ///< provably non-null everywhere) but we know it's non-null by the
		                                 ///< time we run this parent
