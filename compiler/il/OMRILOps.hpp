...
public:
	...
	ILOpCode(TR::ILOpCodes opCode) : _opCode(opCode) { TR_ASSERT(opCode <= TR::LastTROp, "assertion failure"); }
	...
	bool isArithmetic() const
	{
		return isAdd() || isSub() || isMul() || isDiv() || isRem() || isLeftShift() || isRightShift()
		        || isShiftLogical() || isAnd() || isXor() || isOr() || isNeg() || isTernaryAdd()
		        || isTernarySub();
	}
	...
	bool isLongCompare() const
	{
		return (getOpCodeValue() == TR::lcmp) || (getOpCodeValue() == TR::lcmpeq)
		        || (getOpCodeValue() == TR::lcmpne) || (getOpCodeValue() == TR::lcmplt)
		        || (getOpCodeValue() == TR::lcmpge) || (getOpCodeValue() == TR::lcmpgt)
		        || (getOpCodeValue() == TR::lcmple) || (getOpCodeValue() == TR::lucmp)
		        || (getOpCodeValue() == TR::lucmplt) || (getOpCodeValue() == TR::lucmpge)
		        || (getOpCodeValue() == TR::lucmpgt) || (getOpCodeValue() == TR::lucmple);
	}
	...
	{
		return getOpCodeValue() == TR::ificmpgt || getOpCodeValue() == TR::iflcmpgt
		        || getOpCodeValue() == TR::ifiucmpgt || getOpCodeValue() == TR::ifacmpgt
	...
	bool isMemToMemOp()
	{
		return getOpCodeValue() == TR::bitOpMem || getOpCodeValue() == TR::arrayset
		        || getOpCodeValue() == TR::arraycmp || getOpCodeValue() == TR::arraycopy;
	}
	...
	{
		switch (op) {
		case TR::icmpeq: return TR::icmpeq;
		case TR::icmpne: return TR::icmpne;
		...
		case TR::ificmpeq: return TR::ificmpeq;
		case TR::ificmpne: return TR::ificmpne;
		...
		case TR::lcmpeq: return TR::lcmpeq;
		case TR::lcmpne: return TR::lcmpne;
		...
		case TR::iflcmpeq: return TR::iflcmpeq;
		case TR::iflcmpne: return TR::iflcmpne;
	...
	static TR::ILOpCodes convertScalarToVector(TR::ILOpCodes op)
	{
		...
		case TR::lloadi:
		case TR::floadi:
		case TR::dloadi: return TR::vloadi;
		case TR::bstore:
		case TR::cstore:
		case TR::sstore:
		case TR::istore:
		case TR::lstore:
		case TR::fstore:
		case TR::dstore: return TR::vstore;
		case TR::bstorei:
		case TR::cstorei:
		case TR::sstorei:
		case TR::istorei:
		case TR::lstorei:
		case TR::fstorei:
		case TR::dstorei: return TR::vstorei;
		case TR::badd:
		case TR::cadd:
		case TR::sadd:
		case TR::iadd:
		...
		case TR::lsub:
		case TR::fsub:
		case TR::dsub: return TR::vsub;
		case TR::bmul:
		case TR::smul:
		case TR::imul:
		case TR::lmul:
		case TR::fmul:
		case TR::dmul: return TR::vmul;
		case TR::bdiv:
		case TR::sdiv:
		case TR::idiv:
		case TR::ldiv:
		case TR::fdiv:
		case TR::ddiv: return TR::vdiv;
		case TR::bconst:
		case TR::cconst:
		case TR::sconst:
		case TR::iconst:
		case TR::lconst:
		case TR::fconst:
		case TR::dconst: return TR::vsplats;
		case TR::brem:
		case TR::srem:
		case TR::irem:
		case TR::lrem:
		case TR::frem:
		case TR::drem: return TR::vrem;
		case TR::bneg:
		case TR::sneg:
		case TR::ineg:
		case TR::lneg:
		case TR::fneg:
		case TR::dneg: return TR::vneg;
		case TR::bor:
		case TR::sor:
		case TR::ior:
		case TR::lor: return TR::vor;
		case TR::band:
		case TR::sand:
		case TR::iand:
		case TR::land: return TR::vand;
		case TR::bxor:
		case TR::sxor:
		case TR::ixor:
		case TR::lxor: return TR::vxor;
		case TR::bushr:
		case TR::sushr:
		case TR::iushr:
		case TR::lushr: return TR::vushr;
		case TR::l2d: return TR::vl2vd;
		default: return TR::BadILOp;
		}
		return TR::BadILOp;
	}
	...
	{
		switch (type) {
		case TR::Int32: return TR::irol;
		case TR::Int64: return TR::lrol;
		default: return TR::BadILOp;
		}
		return TR::BadILOp;
	}
