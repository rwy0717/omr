...
OMR::Symbol::Symbol(TR::DataType d) : _size(0), _name(0), _flags(0), _flags2(0), _localIndex(0)
{
	self()->setDataType(d);
}

OMR::Symbol::Symbol(TR::DataType d, uint32_t size) : _name(0), _flags(0), _flags2(0), _localIndex(0)
{
	self()->setDataType(d);
	_size = size;
}
...
template <typename AllocatorType>
TR::Symbol *
OMR::Symbol::createNamedShadow(AllocatorType m, TR::DataType d, uint32_t s, char *name)
{
