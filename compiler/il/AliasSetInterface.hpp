...
public:
	...
	TR_SymAliasSetInterface(bool shares_symbol, TR::SymbolReference *symRef, bool isDirectCall = false,
	...
	void setAliases_impl(
	        TR::SparseBitVector &aliasesToModify, bool value, bool isDirectCall, bool includeGCSafePoint)
	{
		TR::Compilation *comp = TR::comp();
...
struct TR_UseDefAliasSetInterface : public TR_SymAliasSetInterface<useDefAliasSet> {
	TR_UseDefAliasSetInterface(
	        TR::SymbolReference *symRef, bool isDirectCall = false, bool includeGCSafePoint = false)
	        : TR_SymAliasSetInterface<useDefAliasSet>(symRef, isDirectCall, includeGCSafePoint)
	{}

	TR_UseDefAliasSetInterface(bool shares_symbol, TR::SymbolReference *symRef, bool isDirectCall = false,
	        bool includeGCSafePoint = false)
        ...
        TR::SymbolReference *symRef2, bool value, bool isDirectCall, bool includeGCSafePoint)
{
...

	{
		if (_aliasSetType == useDefAliasSet) {
			...
			if (_symbolReference->getSymbol()->isMethod() && !symRef2->getSymbol()->isMethod()) { // foo
				                                                                              // uses x,
				                                                                              // same as
				                                                                              // x kills
				                                                                              // foo
...
template <>
inline TR_BitVector *
TR_SymAliasSetInterface<useDefAliasSet>::getTRAliases_impl(bool isDirectCall, bool includeGCSafePoint)
{
	if (_symbolReference) {
		if (_shares_symbol)
...
template <>
inline TR_BitVector *
TR_SymAliasSetInterface<UseOnlyAliasSet>::getTRAliases_impl(bool isDirectCall, bool includeGCSafePoint)
{
	if (!_symbolReference)
...
CountUseDefAliases(T &t, const TR::SparseBitVector &syms)
{
...

	{
		...
		TR_BitVector *tr_syms = new (comp->trStackMemory())
		        TR_BitVector(syms.PopulationCount(), comp->trMemory(), stackAlloc, growable);
		*tr_syms = syms;
