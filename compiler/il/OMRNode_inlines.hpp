...
uint16_t
OMR::Node::addChildrenAndSymRef(uint16_t childIndex, TR::Node *child, ChildrenAndSymRefType... childrenAndSymRef)
{
...
template <class... ChildrenAndSymRefType>
TR::Node *
OMR::Node::recreateWithSymRefWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
        uint16_t numChildArgs, TR::Node *first, ChildrenAndSymRefType... childrenAndSymRef)
{
	...
	TR_ASSERT(numChildArgs == numChildArgsActual,
	        "numChildArgs = %d does not match actual number of child args = %d", numChildArgs, numChildArgsActual);
	return node;
}

template <class... Children>
TR::Node *
OMR::Node::recreateWithoutSymRef(TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs,
...
template <class... ChildrenAndSymRefType>
TR::Node *
OMR::Node::createWithSymRefInternal(TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, TR::Node *first,
...
template <class... ChildrenAndSymRefType>
TR::Node *
OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, TR::Node *first,
        ChildrenAndSymRefType... childrenAndSymRef)
{
...
OMR::Node::mayKill(bool gcSafe)
{
	if (self()->getOpCode().hasSymbolReference()
	        && (self()->getOpCode().isLikeDef() || self()->mightHaveVolatileSymbolReference())) // we want the old
	                                                                                            // behavior in these
	                                                                                            // cases
	{
...
OMR::Node::getChild(int32_t c)
{
	if (!self()->hasNodeExtension()) {
		TR_ASSERT(c < NUM_DEFAULT_CHILDREN,
		        "getChild(%d) called on node " POINTER_PRINTF_FORMAT " with %d children.", c, self(),
		        self()->getNumChildren());
...
{
	TR_ASSERT(self()->getNumChildren() >= 1,
	        "getLastChild() called on node " POINTER_PRINTF_FORMAT " with no children", self());
...
OMR::Node::getIntegerNodeValue()
{
	...
	case TR::iconst:
	case TR::iuconst: length = (T)self()->getUnsignedInt(); break;
	case TR::lconst:
	case TR::luconst: length = (T)self()->getUnsignedLongInt(); break;
	case TR::iloadi: TR_ASSERT(false, "Invalid type for length node."); break;
	default: TR_ASSERT(false, "Invalid type for length node %p", this); break;
	}
	return length;
}
...
OMR::Node::canCheckReferenceIsNonNull()
{
	...
	case TR:: instanceof: return true;
