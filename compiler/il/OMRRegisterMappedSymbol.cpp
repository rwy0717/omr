...
OMR::RegisterMappedSymbol::RegisterMappedSymbol(int32_t o) : TR::Symbol(), _mappedOffset(o), _GCMapIndex(-1)
{
	self()->setLiveLocalIndexUninitialized();
}

OMR::RegisterMappedSymbol::RegisterMappedSymbol(TR::DataType d) : TR::Symbol(d), _mappedOffset(0), _GCMapIndex(-1)
{
	self()->setLiveLocalIndexUninitialized();
}
...
OMR::RegisterMappedSymbol::setLiveLocalIndex(uint16_t i, TR_FrontEnd *fe)
{
	...
	if (self()->isLiveLocalIndexUninitialized()) {
		...
		TR::comp()->failCompilation<TR::CompilationException>("OMR::RegisterMappedSymbol::_liveLocalIndex == "
		                                                      "USHRT_MAX");
	}
}
