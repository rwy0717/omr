...
OMR::LabelSymbol::LabelSymbol(TR::CodeGenerator *codeGen)
        : TR::Symbol(), _instruction(NULL), _codeLocation(NULL), _estimatedCodeLocation(0), _snippet(NULL)
{
	self()->setIsLabel();
...
{
	if (TR::comp()->cg()
	        && TR::comp()->cg()->getCodeGeneratorPhase() <= TR::CodeGenPhase::InstructionSelectionPhase)
		return TR_maybe;
