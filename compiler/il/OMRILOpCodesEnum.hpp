...
FirstOMROp,
        ...
        compressedRefs, // no-op anchor providing optimizable subexpressions used for compression/decompression.  First
                        // child is address load/store, second child is heap base displacement
        ...
        instanceof, // instanceof - symref is the class object, cp index is in the
                  ...
                  variableNewArray, // new array - type not known at compile time, type must be a j9class, do not use
                                    // type enums
                  ...
                  luaddh, // add 2 unsigned long integers (the high parts of prior luadd) as high part of 128bit
                          // addition.
                  ...
                  lookup, // lookupswitch (child1 is selector expression, child2 the default destination, subsequent
                          // children are case nodes
                  trtLookup, // special lookupswitch (child1 must be trt, child2 the default destination, subsequent
                             // children are case nodes) The internal control flow is similar to lookup, but each CASE
                             // represents a special semantics associated with a flag on it
                  Case, // case nodes that are children of TR_switch.  Uses the branchdestination and the int const
                        // field
                  table, // tableswitch (child1 is the selector, child2 the default destination, subsequent children are
                         // the branch targets (the last child may be a branch table address, use
                         // getCaseIndexUpperBound() when iterating over branch targets)
                  exceptionRangeFence, // (J9) SymbolReference is the aliasing effect, initializer is where the code
                  ...
                  ResolveAndNULLCHK, // Resolve check a static, field or method and Null check the underlying pointer.
                                     // child 1 is reference to be resolved. Symbolref indicates failure
                                     // action/destination
                  DIVCHK, // Divide by zero check. child 1 is the divide. Symbolref indicates failure action/destination
                  OverflowCHK, // Overflow check. child 1 is the operation node(add, mul, sub). Child 2 and child 3 are
                               // the operands of the operation of the operation. Symbolref indicates failure
                               // action/destination
                  UnsignedOverflowCHK, // UnsignedOverflow check. child 1 is the operation node(add, mul, sub). Child 2
                                       // and child 3 are the operands of the operation of the operation. Symbolref
                                       // indicates failure action/destination
                  BNDCHK, // Array bounds check, checks that child 1 > child 2 >= 0 (child 1 is bound, 2 is index).
                          // Symbolref indicates failure action/destination
                  ArrayCopyBNDCHK, // Array copy bounds check, checks that child 1 >= child 2. Symbolref indicates
                  ...
                  arraytranslateAndTest, // Inline code for scanning of part of an array for a particular 8-bit
                                         // character
                  ...
                  icmpset, // icmpset(pointer,c,r): compare *pointer with c, if it matches, replace with r.  Returns 0
                           // on match, 1 otherwise
                  lcmpset, // the operation is done atomically - return type is int for both [il]cmpset
                  bztestnset, // bztestnset(pointer,c): atomically sets *pointer to c and returns the original value of
                              // *p (represents Test And Set on Z)
