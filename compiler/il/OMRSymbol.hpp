...
public:
	...
	enum {
		...
		SlotSharedByRefAndNonRef = 0x00400000, ///< used in fsd to indicate that an reference symbol shares a
		...
		SpillTempLoaded = 0x04000000, ///< share bit with loadaddr because spill temps will never have their
		                              ///< address taken. Used to remove store to spill if never loaded
		// Available              = 0x02000000,
		VariableSizeSymbol = 0x01000000, ///< non-java only?: specially managed automatic symbols that contain
		                                 ///< both an activeSize and a size
		...
		NonLinear = 0x01000000, // TAROK and temporary.  This bit is used in conjunction with
