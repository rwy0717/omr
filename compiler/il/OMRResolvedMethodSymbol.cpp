...
TR::ResolvedMethodSymbol *
OMR::ResolvedMethodSymbol::create(AllocatorType m, TR_ResolvedMethod *rm, TR::Compilation *comp)
{
 ...
 */
TR::TreeTop *
OMR::ResolvedMethodSymbol::genInduceOSRCallNode(
	...
	if (stashedArgs) {
		for (int32_t i = 0; i < numChildren; ++i)
			induceOSRCallNode->setAndIncChild(i,
			        TR::Node::createLoad(induceOSRCallNode,
 ...
 */
bool
OMR::ResolvedMethodSymbol::induceOSRAfterAndRecompile(TR::TreeTop *insertionPoint, TR_ByteCodeInfo induceBCI,
        TR::TreeTop *branch, bool extendRemainder, int32_t offset, TR::TreeTop **lastTreeTop)
{
	...
	TR_ASSERT_FATAL(
	        induceOSRCallNode->getOpCode().isCall() && symRef->getReferenceNumber() == TR_induceOSRAtCurrentPC,
	        "induceOSRCallNode %p (n%dn) under induceOSRCallTree %p should be a call node with "
	        "TR_induceOSRAtCurrentPC helper call",
...
{
	return self()->induceOSRAfterImpl(insertionPoint, induceBCI, branch, extendRemainder, offset, lastTreeTop)
	        != NULL;
}
 ...
 */
TR::TreeTop *
OMR::ResolvedMethodSymbol::induceOSRAfterImpl(TR::TreeTop *insertionPoint, TR_ByteCodeInfo induceBCI,
        TR::TreeTop *branch, bool extendRemainder, int32_t offset, TR::TreeTop **lastTreeTop)
{
	...
	if (self()->supportsInduceOSR(induceBCI, block, self()->comp())) {
		...
		if (remainderTree->getNode()->getOpCodeValue() != TR::BBEnd) {
			if (extendRemainder) {
				...
				if (self()->comp()->getOption(TR_TraceOSR))
					traceMsg(self()->comp(),
					        "  Split of block_%d at n%dn produced block_%d which is an extension\n",
 ...
 */
TR::TreeTop *
OMR::ResolvedMethodSymbol::genInduceOSRCall(TR::TreeTop *insertionPoint, int32_t inlinedSiteIndex, int32_t numChildren,
        ...
        TR::CFG *callerCFG)
{
	...
	// shouldn't have been able to induce without an OSR point in the first place. This should be removed, as well
	// as the genOSRHelperCall's cfg argument.
	if (firstOSRPoint) {
 ...
 */
int
OMR::ResolvedMethodSymbol::matchInduceOSRCall(
...
OMR::ResolvedMethodSymbol::genAndAttachOSRCodeBlocks(int32_t currentInlinedSiteIndex)
{
	...
	if (self()->comp()->isOutermostMethod() && self()->comp()->getHCRMode() == TR::osr) {
		...
		TR::Block *OSRCatchBlock =
		        osrMethodData->findOrCreateOSRCatchBlock(self()->getFirstTreeTop()->getNode());
	...
	for (TR::TreeTop *tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
		...
		if (self()->comp()->isPotentialOSRPoint(tt->getNode())) {
			...
			// Add an exception edge from the current block to the OSR catch block if there isn't already
			// one
...
static void
getNeighboringSymRefLists(int32_t index, TR_Array<List<TR::SymbolReference> > *listArray,
...
void
OMR::ResolvedMethodSymbol::genOSRHelperCall(
        int32_t currentInlinedSiteIndex, TR::SymbolReferenceTable *symRefTab, TR::CFG *cfg)
{
		...
		for (TR::SymbolReference *symRef = ppsIt.getFirst(); symRef; symRef = ppsIt.getNext(), symRefOrder++) {
			...
			if (sharesSlot) {
				...
				if (self()->comp()->getOption(TR_DisableOSRSharedSlots)) {
					self()->comp()->failCompilation<TR::ILGenFailure>("Pending push slot sharing "
					                                                  "detected");
				}
			}
	...
	for (i = 0; autosListArray && i < autosListArray->size(); ++i) {
		...
		for (TR::SymbolReference *symRef = autosIt.getFirst(); symRef;
			...
			if (sharesSlot) {
				if (trace)
					traceMsg(
					        self()->comp(), "#%d shares auto slot\n", symRef->getReferenceNumber());
				if (self()->comp()->getOption(TR_DisableOSRSharedSlots)) {
					self()->comp()->failCompilation<TR::ILGenFailure>("Auto/parm slot sharing "
					                                                  "detected");
				}
			}
	...
	if (self()->comp()->cg()->getSupportsTM() && !self()->comp()->getOption(TR_DisableTLE) && !disableOSRwithTM) {
		...
		tabortNode->setSymbolReference(self()->comp()->getSymRefTab()->findOrCreateTransactionAbortSymbolRef(
	...
	if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
		...
		TR::Node *osrReturnAddress = TR::Node::createLoad(
		        firstNode, self()->comp()->getSymRefTab()->findOrCreateOSRReturnAddressSymbolRef());
        ...
        TR_FrontEnd *fe, TR::Compilation *comp, TR::SymbolReferenceTable *symRefTab, TR::IlGenRequest &customRequest)
{
	...
	try {
		if (!_firstTreeTop || !comp->isPeekingMethod()) {
			...
			//_automaticList.setListHead(0); // what's the point ? sym ref tab can use the sym refs anyway
			// by using methodSymbol's auto sym refs and pending push sym refs list; this only confuses
			// analyses into thinking these autos cannot be used when they actually can be
			...
			if (_methodFlags.testAny(IlGenSuccess)) {
				if (!comp->isPeekingMethod()) {
					if (self()->catchBlocksHaveRealPredecessors(comp->getFlowGraph(), comp)) {
						comp->failCompilation<TR::CompilationException>("Catch blocks have "
						                                                "real predecessors");
					}
				}

				// If OSR is enabled, need to create an OSR helper call to keep the pending pushes,
				// parms, and locals alive. Only do this in non HCR mode since under HCR we will be more
				// selective about where to add OSR points.
				//
...
bool
OMR::ResolvedMethodSymbol::supportsInduceOSR(
	...
	while (osrMethodData->getInlinedSiteIndex() > -1) {
		...
		if (!cannotAttemptOSR) {
			...
			if (!osrMethodData) {
				if (comp->getOption(TR_TraceOSR))
					traceMsg(comp,
					        "Cannot attempt OSR as OSR method data for caller of callee %d is "
					        "NULL\n",
					        callSite);
			...
			if (!osrCodeBlock || osrCodeBlock->isUnreachable()) {
				if (comp->getOption(TR_TraceOSR))
					traceMsg(comp,
					        "Cannot attempt OSR as OSR code block for site index %d is absent\n",
			...
			if (callSiteInfo._byteCodeInfo.doNotProfile() && comp->getOSRMode() == TR::voluntaryOSR) {
				if (comp->getOption(TR_TraceOSR))
					traceMsg(comp,
					        "Cannot attempt OSR during caller bytecode index %d:%d as it did not "
					        "exist at ilgen\n",
					        callSite, byteCodeIndex);
				cannotAttemptOSR = true;
				break;
 ...
 */
bool
OMR::ResolvedMethodSymbol::cannotAttemptOSRAt(TR_ByteCodeInfo &bci, TR::Block *blockToOSRAt, TR::Compilation *comp)
{
	...
	if (blockToOSRAt && (!osrCatchBlock || !blockToOSRAt->hasExceptionSuccessor(osrCatchBlock))) {
		if (comp->getOption(TR_TraceOSR)) {
			if (osrCatchBlock)
				traceMsg(comp,
				        "Cannot attempt OSR as block_%d is missing an edge to OSR catch block: "
				        "block_%d\n",
				        blockToOSRAt->getNumber(), osrCatchBlock->getNumber());
			else
				traceMsg(comp,
				        "Cannot attempt OSR as call site index %d lacks an OSR catch block for "
				        "block_%d\n",
	...
	if (!allCallersOSRCodeBlocksAreStillInCFG) {
		...
		while (cursorOsrMethodData != finalOsrMethodData) {
			...
			if (cursorOsrMethodData->getInlinedSiteIndex() > -1)
				cursorOsrMethodData =
				        comp->getOSRCompilationData()->findCallerOSRMethodData(cursorOsrMethodData);
			else
        ...
        TR::Compilation *comp, TR_ByteCodeInfo &byteCodeInfo, TR::TreeTop *insertTree, bool keepStashedArgsLive)
{
	...
	if (comp->getOption(TR_TraceOSR))
		traceMsg(comp,
		        "Inserting stores for dead stack slots in method at caller index %d and bytecode index %d for "
		        "induceOSR call %p\n",
	...
	while (bvi.hasMoreElements()) {
		...
		if (!nextSymRef->getSymbol()->isParm()
		        && performTransformation(comp,
		                "OSR LIVE RANGE ANALYSIS : Local %d is reset before tree [%p] (caller index %d "
		                "bytecode index %d)\n",
			...
			TR::Node *storeNode = TR::Node::createWithSymRef(
			        ...
			        TR::Node::createConstDead(
			                insertTree->getNode(), nextSymRef->getSymbol()->getDataType()),
			        nextSymRef);
        ...
        TR::Compilation *comp, int32_t inlinedSiteIndex, TR_ByteCodeInfo &byteCodeInfo, TR::TreeTop *induceOSRTree)
{
	if (!comp->osrStateIsReliable()) {
		traceMsg(comp,
		        "OSR state may not be reliable enough to trust liveness info computed at IL gen time; so "
...
OMR::ResolvedMethodSymbol::setTempIndex(int32_t index, TR_FrontEnd *fe)
{
	if ((_tempIndex = index) < 0) {
		self()->comp()->failCompilation<TR::CompilationException>("TR::ResolvedMethodSymbol::_tempIndex "
		                                                          "overflow");
	}
	return index;
}
...
OMR::ResolvedMethodSymbol::removeTree(TR::TreeTop *tt)
{
	...
	if (prev != NULL)
		...
		TR_ASSERT(_firstTreeTop == tt,
		        "treetops are not forming a single doubly linked list: node address = %p, global index = %d",
				...
				if (!excepSucc.empty()) {
					for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
						...
						if (dest == node) {
							...
							// if the catch block unlocks a monitor, then the new catch
							// block created below must also unlock the monitor
							//
							...
							// handler may be prepended with an asynccheck and pending
							// pushes These should be retained in the copy, so skip them
							// when ripping out trees
							if (comp->isOSRTransitionTarget(TR::postExecutionOSR)
							        || comp->getOSRMode() == TR::involuntaryOSR) {
								TR::TreeTop *next = retain->getNextTreeTop();
								if (next
								        && next->getNode()->getOpCodeValue()
								                == TR::asynccheck)
							...
							if (containsMonexit) {
								TR::TreeTop *storeTT = TR::TreeTop::create(comp,
								        (TR::Node::create(
								                clonedCatch->getEntry()->getNode(),
								                TR::monexitfence, 0)));
								dumpOptDetails(comp,
								        "\tInserted monitor exit fence %p into cloned "
								        "catch %d\n",
							...
							TR::Node *gotoNode = TR::Node::create(
							        dest->getEntry()->getNextTreeTop()->getNode(), TR::Goto,
							        0, gotoBlock->getEntry());
	...
	for (TR::CFGNode *node = cfg->getFirstNode(); node; node = node->getNext()) {
		if (!node->getExceptionPredecessors().empty()) {
			...
			if (!node->getPredecessors().empty()) {
				dumpOptDetails(
				        comp, "detected catch block_%d with real predecessors\n", node->getNumber());
				return true;
			}
		}
	}
 ...
 */
void
OMR::ResolvedMethodSymbol::setProfilerFrequency(int32_t bytecodeIndex, int32_t frequency)
{
	for (auto itr = _bytecodeProfilingOffsets.begin(), end = _bytecodeProfilingOffsets.end(); itr != end; ++itr) {
		if (itr->first >= bytecodeIndex)
...
static bool
compareProfilingOffsetInfo(const std::pair<int32_t, int32_t> &first, const std::pair<int32_t, int32_t> &second)
{
 ...
 */
void
OMR::ResolvedMethodSymbol::addProfilingOffsetInfo(int32_t startBCI, int32_t endBCI)
{
	for (auto itr = _bytecodeProfilingOffsets.begin(), end = _bytecodeProfilingOffsets.end(); itr != end; ++itr) {
		if (itr->first >= endBCI) {
