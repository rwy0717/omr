...
OMR::SymbolReference::getUseonlyAliasesBV(TR::SymbolReferenceTable *symRefTab)
{
	...
	case TR::Symbol::IsMethod: {
		...
		default: return &symRefTab->aliasBuilder.defaultMethodUseAliases();
		}
	}
	case TR::Symbol::IsResolvedMethod: {
		...
		if (!TR::comp()->getOption(TR_EnableHCR)) {
			...
			case TR::java_lang_StrictMath_copySign_D: return NULL;
#endif
			default: break;
			}
		}
...
OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall, bool includeGCSafePoint)
{
	...
	case TR::Symbol::IsMethod: {
		...
		switch (self()->getReferenceNumber()) {
		case TR_methodTypeCheck:
		case TR_nullCheck: return &symRefTab->aliasBuilder.defaultMethodDefAliasesWithoutImmutable();
		...
		case TR_jitProfileParseBuffer: return 0;
		...
		if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) || _symbol->isVolatile()
			...
			if (symRefTab->aliasBuilder.unsafeArrayElementSymRefs().get(self()->getReferenceNumber())) {
				TR_BitVector *aliases = new (aliasRegion)
				        TR_BitVector(bvInitialSize, aliasRegion, growability);
				*aliases |=
				        comp->getSymRefTab()->aliasBuilder.defaultMethodDefAliasesWithoutImmutable();
...
OMR::SymbolReference::getTypeSignature(int32_t &len, TR_AllocationKind allocKind, bool *isFixed)
{
	...
	case TR::Symbol::IsParameter: return _symbol->castToParmSymbol()->getTypeSignature(len);

	case TR::Symbol::IsShadow:
		return (_cpIndex > 0 ? self()->getOwningMethod(comp)->fieldSignatureChars(_cpIndex, len) : 0);

	case TR::Symbol::IsStatic: return self()->getOwningMethod(comp)->staticSignatureChars(_cpIndex, len);
 ...
 */
void
OMR::SymbolReference::makeIndependent(TR::SymbolReferenceTable *symRefTab, TR::SymbolReference *symRef)
{
