...
OMR::Block::Block(TR::TreeTop *entry, TR::TreeTop *exit, TR_Memory *m) : TR::CFGNode(m)
{
...
OMR::Block::Block(TR::TreeTop *entry, TR::TreeTop *exit, TR::CFG &cfg) : TR::CFGNode(cfg.getInternalRegion())
{
	self()->init(entry, exit);
	self()->setFrequency(-1);
	self()->setUnrollFactor(0);
...
OMR::Block::doesNotNeedLabelAtStart()
{
	...
	for (auto thisEdge = self()->getPredecessors().begin(); thisEdge != self()->getPredecessors().end();
	        ++thisEdge) {
 ...
 */
void
OMR::Block::redirectFlowToNewDestination(
	...
	if (from->getEntry() == NULL) {
		...
		if (lastNode->getOpCode().isBranch() && lastNode->getBranchDestination() == origTo->getEntry()) {
			...
			if (lastNode->getOpCode().hasBranchChildren()) {
				traceMsg(
				        comp, "Jump with multple targets, with non fall through path to empty block\n");
				TR::TreeTop *origToEntry = origTo->getEntry();
				TR::TreeTop *newToEntry = newTo->getEntry();
				if (origToEntry)
				...
				for (int32_t i = 0; i < lastNode->getNumChildren() - 1; i++) {
					...
					traceMsg(comp, "considering node %p with branch destination %p \n", child,
					        child->getBranchDestination() ? child->getBranchDestination()->getNode()
					                                      : 0);
					if (child->getBranchDestination() == origToEntry) {
						child->setBranchDestination(newToEntry);
						if (!from->hasSuccessor(newTo))
		...
		// from falls through to origTo
		{
			...
			if (useGotoForFallThrough) {
				...
				if (lastNode->getOpCode().isBranch()) {
					...
					TR::Block::insertBlockAsFallThrough(comp, from, newGotoBlock); // edge won't be
					                                                               // duplicated
...
OMR::Block::findVirtualGuardBlock(TR::CFG *cfg)
{
	...
	for (auto nextEdge = self()->getPredecessors().begin(); nextEdge != self()->getPredecessors().end();
	        ++nextEdge) {
...
OMR::Block::changeBranchDestination(TR::TreeTop *newDestination, TR::CFG *cfg, bool callerFixesRegdeps)
{
	...
	if (!self()->hasSuccessor(newDestinationBlock)) {
		...
		if (prevDestinationBlock->getFrequency() > 0) {
			int32_t newEdgeFrequency = (prevEdgeFrequency * newDestinationBlock->getFrequency())
...
void
OMR::Block::uncommonNodesBetweenBlocks(
...
OMR::Block::isTargetOfJumpWhoseTargetCanBeChanged(TR::Compilation *comp)
{
	...
	for (auto predEdge = self()->getPredecessors().begin(); predEdge != self()->getPredecessors().end();
	        ++predEdge) {
...
TR::Block *
OMR::Block::splitWithGivenMethodSymbol(TR::ResolvedMethodSymbol *methodSymbol, TR::TreeTop *startOfNewBlock,
 ...
 */
static void
replaceNodesInSubtree(TR::Node *node, NodeTable *nodeInfo, TR::NodeChecklist &checklist)
{
 ...
 */
static void
replaceNodesInTrees(TR::Compilation *comp, TR::TreeTop *start, NodeTable *nodeInfo)
{
...
findAvailableRegister(TR::Compilation *comp, TR::Node *node, TR_BitVector &unavailableRegisters)
{
	...
	if (dt.isIntegral() || dt.isAddress()) {
		...
		TR_ASSERT_FATAL(false,
		        "Unknown data type %s encountered when trying to pick a register for postGRA block splitting!",
		        dt.toString());
 ...
 */
static bool
checkIfRegisterIsAvailable(TR::Compilation *comp, TR::Node *node, TR_BitVector &unavailableRegisters)
{
	// Following assert is to make sure that this function is called with node which have Global Registers
	// associated with it.
 ...
 */
static bool
checkStoreRegNodeListForNode(TR::Node *passThroughNode, List<TR::Node> *storeNodeList)
{
 ...
 */
static void
gatherUnavailableRegisters(TR::Compilation *comp, TR::Node *regDeps, TR::TreeTop *currentTT, NodeTable *nodeInfo,
        StoreRegNodeTable *storeNodeInfo, List<TR::Node> *storeRegNodePostSplitPoint,
        TR_BitVector &unavailableRegisters)
{
	...
	for (int i = 0; i < regDeps->getNumChildren(); i++) {
		...
		if (dep->getOpCodeValue() == TR::PassThrough) {
			...
			// We need to check if the we can use the information to allocate the register for the node or
			// not.
			if (nodeInfoEntry != nodeInfo->end()) {
				...
				if (nodeInfoEntry->second.second == NULL) {
					...
					if (storeRegNodeInfoEntry == storeNodeInfo->end()
						...
						// after split point. In this case we do not have to do anything as when
						// we replace the node post split point, it will be replaced under
						// corresponding regstore as well.
						needToCreateRegStore = false;
					} else {
						// Node under passthrough is stored into some set of registers (Not
						// necessarily same set of registers as we only record last regStore for
						// the node) and if those set of registers are available, then use them
						...
						if (checkIfRegisterIsAvailable(comp, storeNode, unavailableRegisters)) {
							// Whether the PassThrough uses same register or not, use the
							// last recorded regStore to assign register to the node.
							TR::Node *regLoad = TR::Node::create(value,
							        comp->il.opCodeForRegisterLoad(value->getDataType()));
							...
							// Course of action to analyze reg store for block splitter is
							// to check for the registers used for the node post split point
							// before we check the regStore information pre split point. As
							// regStore post split point is most likely to be used later.
							// This will reduce number of shuffles. So if we get the
							// candidate from before split point, we do not need to check
							// list of RegStores post split point.
				...
				if (nodeInfoEntry->second.second != NULL
					...
					// Passthrough uses same register as the replacement, do not need to do anything
				        ...
				        && (!needToCheckStoreRegPostSplitPoint
				                || !checkStoreRegNodeListForNode(dep, storeRegNodePostSplitPoint))) {
					// We have either used different register for replacement to value node after
					// split point or Passthrough has corresponding regStore before spilt point, but
					// registers were unavailable. In these cases we need to create a regStore for
					// the PassThrough. If we have replacement then need to store the replacement to
					// register, else, value needs to be stored.
					TR::Node *nodeToBeStored = nodeInfoEntry->second.second != NULL
					        ? nodeInfoEntry->second.second
					        : value;
					TR::Node *regStore = TR::Node::create(value,
					        comp->il.opCodeForRegisterStore(value->getDataType()), 1,
					        nodeToBeStored);
			...
			TR_ASSERT_FATAL(false,
			        "Expected to find only PassThrough and regLoad operations under a GlRegDepNode, but "
			        "got %s\n",
		...
		if (isInternalPointer) {
			...
			if (value->getOpCode().isArrayRef()) {
				...
				if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
					...
					// If the node we are uncommoning is internal pointer and while iterating a
					// child we found a node that is not Array Reference, There are only two
					// possibilities.
					// 1. It is  internal poiter which was commoned means could be stored into
					// register or on temp slot.
					...
					if (valueChildSymRef != NULL
						...
						if (valueChildSymRef->getSymbol()
							...
							pinningArray =
							        valueChildSymRef->getSymbol()->castToAutoSymbol();
 ...
 */
TR::Block *
OMR::Block::splitPostGRA(TR::TreeTop *startOfNewBlock, TR::CFG *cfg, bool copyExceptionSuccessors,
        TR::ResolvedMethodSymbol *methodSymbol)
{
...

	{
		...
		/**
		 * Step-1 : Analyze the trees before split point to record nodes which will be referenced in new block
		 * and needs replacement after split It collects following information. a. Node which are referenced
		 * after split point in nodeInfo b. Node which are referenced after split point and stored into register
		 * in storeNodeInfo
		 */
		for (TR::PostorderNodeOccurrenceIterator iter(start, comp, "FIX_POSTGRA_SPLIT_COMMONING"); iter != end;
			...
			if (entry == nodeInfo->end() && node->getReferenceCount() > 1) {
				(*nodeInfo)[node] =
				        std::make_pair<int32_t, TR::Node *>(node->getReferenceCount() - 1, NULL);
		...
		for (auto iter = nodeInfo->begin(), end = nodeInfo->end(); iter != end; ++iter) {
			...
			if (node->getOpCode().isLoadReg()) {
				...
				// If node is constant load or loadaddr, do not waste a slot/register for that. Instead
				// create new node.
				iter->second.second = TR::Node::copy(node);
				iter->second.second->setReferenceCount(0);
			}
		 ...
		 *          then create a replacement node for that node using the register and mark used register
		 * unavailable
		 *       2. If node under regStore is not required to be uncommoned then mark the used register
		 * unavailable. b. PassThrough Node (Analysis of these kind of nodes are done in
		 * gatherUnavailableRegisters)
		 *       1. If node under PassThrough node is stored into the same register after split point, we do not
		 * have to do anything.
		 *       2. If node is stored into same register before split point and if register is available, then
		 * use the information to store the node. In other cases where register used in the PassThrough is not
		 * available, then create a Store into a register same as PassThrough before the tree containing the
		 * passThrough node.
		 */
		...
		for (TR::TreeTop *iter = newBlock->getEntry(); iter != nextExtendedBlockEntry;) {
			...
			if (node->getOpCode().isStoreReg()) // Case - 1 : TreeTop Node is RegStore
			{
				...
				// If storedNode is needed to be uncommoned and we do not have found replacement node
				// yet and it is stored in register which is still available then use the register to
				// store the node.
				if (nodeInfoEntry != nodeInfo->end() && nodeInfoEntry->second.second == NULL
					...
					TR::Node *regLoad = TR::Node::create(
					        storedNode, comp->il.opCodeForRegisterLoad(storedNode->getDataType()));
			...
			} else if (node->getOpCode().isBranch() && node->getNumChildren() > 0) // Case - 2 : TreeTop
			                                                                       // Node is Branch
			{
			...
			if (nextTreeTop == nextExtendedBlockEntry) // TreeTop Node is BBExit and next tree top is
			                                           // nextExtendedBlockEntry
			{
				TR::Node *regDeps = iter->getNode()->getNumChildren() > 0 ? iter->getNode()->getChild(0)
				                                                          : NULL;
				if (regDeps && regDeps->getOpCodeValue() == TR::GlRegDeps) {
					// If the previous tree top of BBEnd contains branch, we need to make sure any
					// regStore we add are added before that.
		...
		for (auto iter = nodeInfo->begin(), end = nodeInfo->end(); iter != end; ++iter) {
			...
			// For all the nodes in the nodeTable for which we can not find replacement yet, create
			// replacement node using available registers.
			if (!iter->second.second) {
				...
				if (regInfo.first > -1) {
					...
					TR::Node *regStore = TR::Node::create(
					        value, comp->il.opCodeForRegisterStore(value->getDataType()), 1, value);
				...
				// If we can not find a register, store it into temp slot.
					...
					// In case for a given node, if the data type of stack load/store is not same as
					// node's data type, then we need to convert the node to the type which can be
					// stored onto the stack. Also we need to make sure that replacement node is a
					// node that is converted back to original type of the node.
					...
					self()->getExit()->insertBefore(TR::TreeTop::create(comp,
					        TR::Node::createWithSymRef(value,
					                comp->il.opCodeForDirectStore(convertedValDataType), 1,
			...
			// Now as we have created a node to replace all the references of value node after split point,
			// add all nodes which are not stored into the temp slot to the list for entry and exit
			// dependency which will be used to create GlRegDeps for exit node of the original block and
			// entry node of new block.
		...
		if (depCount > 0) {
			...
			TR::Node *entryGlRegDeps =
			        TR::Node::create(newBlock->getEntry()->getNode(), TR::GlRegDeps, depCount);
			int childIdx = 0;
        ...
        TR::ResolvedMethodSymbol *methodSymbol)
{
	...
	TR::Block *block2 = new (comp->trHeapMemory())
	        TR::Block(TR::TreeTop::create(comp, TR::Node::create(startNode, TR::BBStart)), self()->getExit(),
	                comp->trMemory());
	...
	if (structure) {
		...
		if (thisBlockStructure) {
			...
			TR_StructureSubGraphNode *blockStructureNode2 = new (cfg->structureRegion())
...
TR::Block *
OMR::Block::splitBlockAndAddConditional(
 ...
 */
TR::Block *
OMR::Block::createConditionalSideExitBeforeTree(TR::TreeTop *tree, TR::TreeTop *compareTree, TR::TreeTop *exitTree,
 ...
 */
TR::Block *
OMR::Block::createConditionalBlocksBeforeTree(TR::TreeTop *tree, TR::TreeTop *compareTree, TR::TreeTop *ifTree,
        TR::TreeTop *elseTree, TR::CFG *cfg, bool changeBlockExtensions, bool markCold)
{
	auto comp = TR::comp();
...
void
OMR::Block::collectReferencedAutoSymRefsIn(
        ...
        TR::Block *from, TR::Block *to, TR::Compilation *c, TR::TreeTop **lastTreeTop, bool findOptimalInsertionPoint)
{
	...
	if (entryTree) {
		...
		if (isExceptionEdge) {
			...
			TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
			        c, TR::Node::create(from->getExit()->getNode(), TR::Goto, 0, to->getEntry()));
	...
	// splitting the block at the point of the return or throw is the safest way to do this
		...
		TR::ILOpCode &opCode = lastTree->getNode()->getOpCodeValue() == TR::treetop
...
TR_BlockCloner::cloneBlocks(TR_LinkHeadAndTail<BlockMapper> *bMap)
{
	...
	// for a path of blocks we're cloning that doesn't follow the fall-through path, the branches will be set to
	// fall through node (incorrectly) by cloneNode.  This code patches up the branches so they point to the right
	// target (the old fall through node) we don't mess with the last node as the connection is handled by whoever
	// called us
	//
	for (BlockMapper *itr = bMap->getFirst(); itr->getNext(); itr = itr->getNext()) {
		...
		if (lastRealNode->getOpCode().isIf()
			...
			if (_cloneBranchesExactly)
				...
				newDest = getToBlock(itr->_from->getExit()->getNextTreeTop()->getNode()->getBlock())
				                  ->getEntry();
...
TR_BlockCloner::doBlockClone(TR_LinkHeadAndTail<BlockMapper> *bMap)
{
	...
	//
	// Some Extra remarks about this code.  This cfg code looks insufficient to really handle the case when you are
	// not cloning along a fall through path, and you are trying to preserve branches exactly (ie
	// _cloneBranchesExactly is set to true Luckily, so far no code has ever not done that.  Most code that
	// clonesBranchesExactly clones a single block, and the few cases that don't appear to be cloning along a fall
	// through path
	//
	for (m = _blockMappings.getFirst(); m; m = m->getNext()) {
		_cfg->addNode(m->_to);
	...
	if (from->getOpCode().isBranch()) {
		if (_cloneBranchesExactly)
			...
			to->setBranchDestination(
			        getToBlock(from->getBranchDestination()->getNode()->getBlock())->getEntry());
	}
