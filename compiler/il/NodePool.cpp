...
TR::NodePool::NodePool(TR::Compilation *comp, const TR::Allocator &allocator)
        : _comp(comp), _disableGC(true), _globalIndex(0), _nodeRegion(comp->trMemory()->heapMemoryRegion())
{}
...
{
	TR::Node *newNode =
	        static_cast<TR::Node *>(_nodeRegion.allocate(sizeof(TR::Node))); //_pool.ElementAt(poolIndex);
