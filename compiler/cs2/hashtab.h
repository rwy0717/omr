...
inline HashValue
Hash_FNV(const unsigned char *s, uint32_t len, HashValue v = CS2_FNV_OFFSETBASIS)
{
...
public:
	...
	HashTable(uint32_t ignored, const Allocator &a = Allocator())
	        : Allocator(a), fTable(NULL), fMask(0), fNextFree(0), fTableSize(0), fHighestIndex(0)
	{}
...
CS2_HT_TEMP inline CS2_HTE_DECL::HashTableEntry(const typename CS2_HTE_DECL &entry)
        : fKey(entry.fKey), fData(entry.fData), fHashCode(entry.fHashCode), fChain(entry.fChain)
{}
...
CS2_HT_TEMP inline CS2_HTC_DECL::Cursor(const CS2_HT_DECL &inputTable) : fTable(inputTable), fIndex(0) {}

CS2_HT_TEMP inline CS2_HTC_DECL::Cursor(const typename CS2_HTC_DECL &other) : fTable(other.fTable), fIndex(other.fIndex)
{}
...
inline CS2_HT_DECL::HashTable(const CS2_HT_DECL &table)
        : Allocator(table)
        , fTable(table.fTableSize > 0 ? (HashTableEntry *)Allocator::allocate(table.fTableSize * sizeof(HashTableEntry))
                                      : NULL)
