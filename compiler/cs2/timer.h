...
public:
	...
	PhaseMeasuringNode(const char *name = NULL, const Allocator &a = Allocator())
	        : Allocator(a), fName(NULL), fParent(0), fChildMap(a), fTotalMetric(0), fCount(0), fRunning(false)
	{
		if (name) {
	...
	void Dump(ostream &out, uint32_t indent = 0, Metric total = 0, bool running = false,
...
public:
	...
	void Start(ListIndex child)
	{
		CS2Assert(fNodes[child].Parent() == GetCurrentMeter(),
		        ("Incorrect meter nesting for meters %s %s", fNodes[GetCurrentMeter()].Name(),
		                fNodes[child].Name()));
		fNodes[child].Start();
...
template <class ostream>
inline void
PhaseMeasuringNode<Meter, Allocator>::Dump(
...
template <class ostream>
inline void
PhaseMeasuringSummary<Meter, Allocator>::DumpSummaryNode(
...
template <class ostream>
inline void
PhaseMeasuringSummary<Meter, Allocator>::DumpSummary(ostream &out, bool alternativeFormat, bool csv)
{
	...
	if (csv) {
		...
		offset = sprintf(line, "Phase                           %s  |count *=active",
...
public:
	...
	LexicalBlockProfiler(
	        const char *phase, uint32_t count, PhaseMeasuringSummary &s = PhaseMeasuringSummary::Global())
...
public:
	PhaseTimingSummary(
	        const char *name = "Total elapsed time", const Allocator &a = Allocator(), bool collect = true)
