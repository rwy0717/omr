...
public:
	ListOf(const Allocator &a = Allocator())
	        : BaseArrayOf<AElementType, Allocator, segmentBits>(a), fNextAvailable(0), fInitialSize()
	{}
...
CS2_LI_TEMP inline CS2_LI_DECL::ListOf(const CS2_LI_DECL &list)
        : CS2_AR_DECL(list, list.allocator()), fNextAvailable(list.fNextAvailable), fInitialSize(list.fInitialSize)
{
	ListIndex listIndex;
