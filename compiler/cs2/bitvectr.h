...
public:
	explicit ABitVector(const Allocator &a = Allocator()) : Allocator(a), fNumBits(0), fBitWords(NULL) {}
	...
	public:
		Cursor(const ABitVector &adoptVector)
		        : fVector(adoptVector), fWord(0), fIndex(0), fWordCount(fVector.SizeInWords())
		{}
		...
		bool SetToNextOne()
		{
			...
			if (fWord == 0) {
				BitWord wordIndex;
				for (wordIndex = (fIndex + kBitWordSize - 1) / kBitWordSize; wordIndex < fWordCount;
				        wordIndex += 1) {
	...
	public:
		iterator(const ABitVector &adoptVector, BitIndex index = 0) : Cursor(adoptVector)
		{
		...
		// Dereference operator - identical to cast to index operator - note it returns the index, not a
		// reference to it.
...
private:
	...
	class BitRef {
	public:
		BitRef(const ABitVector &vector, BitIndex index) : fIndex(index), fVector(vector) {}

		BitRef(ABitVector &vector, BitIndex index) : fIndex(index), fVector(vector) {}
...
template <class Allocator>
inline void
ABitVector<Allocator>::GrowTo(BitIndex newBitSize, bool geometric, bool forceGeometric)
{
...
template <class Allocator>
inline BitIndex
ABitVector<Allocator>::ClearLastOneIfThereIsOneInRange(BitIndex low, BitIndex high, bool &foundone)
{
...
ABitVector<Allocator>::PopulationCount(uint32_t numBits) const
{
		...
		for (wordIndex = 0; wordIndex < streamWordSize; wordIndex += WORDS_IN_16_BIT_ACCUM) {
			...
			for (BitIndex i2 = wordIndex; i2 < wordIndex + WORDS_IN_16_BIT_ACCUM;
				...
				for (BitIndex i1 = i2; i1 < i2 + WORDS_IN_8_BIT_ACCUM; i1++) {
					c1 += __popcntb(fBitWords[i1]); // count the number of 1's in each byte of the
					                                // word at wordIndex index
					c1a += __popcntb(fBitWords[streamWordSize + i1]); // count the number of 1's in
					                                                  // each byte of the word at
					                                                  // wordIndex+streamWordSize
					                                                  // index
					c1b += __popcntb(
					        fBitWords[streamWordSize * 2 + i1]); // count the number of 1's
					                                             // in each byte of the
					                                             // word at
					                                             // wordIndex+2*streamWordSize
					                                             // index
					c1c += __popcntb(
					        fBitWords[streamWordSize * 3 + i1]); // count the number of 1's
					                                             // in each byte of the
					                                             // word at
	...
	streamWordSize = ((lastWord - wordIndex) - ((lastWord - wordIndex) % (WORDS_IN_8_BIT_ACCUM * 2)))
	        / 2; // for 2 streams
	...
	if (lastWord - wordIndex >= WORDS_IN_8_BIT_ACCUM * 2) {
		...
		for (; wordIndex < wordIndex + streamWordSize; wordIndex += WORDS_IN_8_BIT_ACCUM) {
			...
			for (BitIndex i1 = wordIndex; i1 < wordIndex + WORDS_IN_8_BIT_ACCUM; i1++) {
				c1 += __popcntb(fBitWords[i1]); // count the number of 1's in each byte of the word at
				                                // wordIndex index
				c1a += __popcntb(fBitWords[streamWordSize + i1]); // count the number of 1's in each
...
template <class Allocator>
inline bool
ABitVector<Allocator>::And(const ABitVector<Allocator> &inputVector, ABitVector<Allocator> &outputVector) const
{
...
template <class Allocator>
inline bool
ABitVector<Allocator>::Andc(const ABitVector<Allocator> &inputVector, ABitVector<Allocator> &outputVector) const
{
	uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
	BitWord inputWord, thisWord, newWord;
	bool changed = false;
...
template <class Allocator>
inline bool
ABitVector<Allocator>::Or(const ABitVector<Allocator> &inputVector, ABitVector<Allocator> &outputVector) const
{
	uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
	BitWord inputWord, thisWord, newWord;
	bool changed = false;
...
template <class Allocator>
inline bool
ABitVector<Allocator>::Xor(const ABitVector<Allocator> &inputVector, ABitVector<Allocator> &outputVector) const
{
	uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
	BitWord inputWord, thisWord, newWord;
	bool changed = false;
