...
CS2_ARTEMP inline CS2_BASEARDECL::BaseArrayOf(const Allocator &a) : Allocator(a)
{
	CS2Assert(sizeof(AElementType) <= SegmentSize(),
	        ("Segment size %d must be larger than size of element %d.", (int)SegmentSize(),
...
CS2_ARTEMP inline CS2_BASEARDECL::BaseArrayOf(const CS2_BASEARDECL &inputArray, const Allocator &a)
        : Allocator(a), fMaxSegments(inputArray.fMaxSegments), fNumberOfSegments(inputArray.fNumberOfSegments)
{
	size_t segmentIndex;
...
CS2_BASEARDECL::operator=(const CS2_BASEARDECL &inputArray)
{
	...
	{
		firstSegment = (fNumberOfSegments < inputArray.fNumberOfSegments) ? fNumberOfSegments
...
CS2_BASEARDECL::GrowTo(size_t newSize)
{
	...
	if (segmentMapIndex >= fMaxSegments) {
		if (fSegmentMap == NULL) {
			...
			fSegmentMap =
			        (DerivedElement **)Allocator::allocate(updatedMaxSegments * sizeof(DerivedElement *));
			fMaxSegments = updatedMaxSegments;
		} else {
			size_t maxSegments = segmentMapIndex + (fMaxSegments >> 1) + 1;
			void *newSegmentMapAllocation = Allocator::reallocate(maxSegments * sizeof(DerivedElement *),
			        fSegmentMap, fMaxSegments * sizeof(DerivedElement *));
...
public:
	...
	public:
		...
		typename ArrayOf<AElementType, Allocator, segmentBits, Initializer>::DerivedElement *DerivedElement()
		        const
		{
