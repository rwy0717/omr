...
TR::ARM64HelperCallSnippet::emitSnippetBody()
{
	...
	intptr_t distance = (intptr_t)(getDestination()->getSymbol()->castToMethodSymbol()->getMethodAddress())
	        - (intptr_t)cursor;
	...
	*(int32_t *)cursor = TR::InstOpCode::getOpCodeBinaryEncoding(TR::InstOpCode::bl)
	...
	if (_restartLabel != NULL) {
		...
		if (constantIsSignedImm28(distance)) {
			// b distance
			*(int32_t *)cursor = TR::InstOpCode::getOpCodeBinaryEncoding(TR::InstOpCode::b)
			        | ((distance >> 2) & 0x3ffffff); // imm26
			cursor += ARM64_INSTRUCTION_LENGTH;
		} else {
...
TR_Debug::print(TR::FILE *pOutFile, TR::ARM64HelperCallSnippet *snippet)
{
	...
	trfprintf(
	        pOutFile, "bl \t" POINTER_PRINTF_FORMAT "\t\t; %s%s", target, getName(snippet->getDestination()), info);
