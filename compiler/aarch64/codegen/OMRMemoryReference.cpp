        ...
        TR::Node *node, TR::SymbolReference *ref, TR::Register *reg, TR::MemoryReference *mr, TR::CodeGenerator *cg)
{
	...
	if (ref->isUnresolved() || comp->compileRelocatableCode()) {
		...
		if (symbol->isCountForRecompile()) {
			...
			mr->setUnresolvedSnippet(new (cg->trHeapMemory())
...
void
OMR::ARM64::MemoryReference::setSymbol(TR::Symbol *symbol, TR::CodeGenerator *cg)
{
...
void
OMR::ARM64::MemoryReference::addToOffset(TR::Node *node, intptrj_t amount, TR::CodeGenerator *cg)
{
		...
		if (_baseRegister != NULL) {
			if (node->getOpCode().isLoadConst() && node->getRegister()) {
				...
				if (constantIsUnsignedImm12(displacement)) {
					generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addimmx, node, newBase,
					        _baseRegister, displacement);
				} else {
...
void
OMR::ARM64::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
        ...
        TR::Register *srcReg, TR::Node *srcTree, bool srcModifiable, TR::CodeGenerator *cg)
{
	...
	if (self()->getUnresolvedSnippet() != NULL) {
		...
		if (_indexRegister != NULL) {
			...
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::addx, srcTree, tempTargetRegister, _baseRegister, _indexRegister);
			...
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::addx, srcTree, tempTargetRegister, _baseRegister, srcReg);
...
void
OMR::ARM64::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
