...
public:
        ...
        , _sourceImmediate8Two(sourceImmediateTwo)
    {
        ...
        if (op == TR::InstOpCode::RISBG || op == TR::InstOpCode::RISBGN) {
            TR_ASSERT((sourceImmediateOne & 0xC0) == 0, "Bits 0-1 in the I3 field for %s must be 0",
                getOpCodeValue() == TR::InstOpCode::RISBG ? "RISBG" : "RISBGN");
            TR_ASSERT((sourceImmediateTwo & 0x40) == 0, "Bit 1 in the I4 field for %s must be 0",
                getOpCodeValue() == TR::InstOpCode::RISBG ? "RISBG" : "RISBGN");
        ...
        , _sourceImmediate8Two(sourceImmediateTwo)
    {
        ...
        // S390RegInstruction constructor call
        useSourceRegister(sourceRegister);
        if ((op == TR::InstOpCode::RISBG || op == TR::InstOpCode::RISBGN) && cg->supportsHighWordFacility()
...
class S390NOPInstruction : public TR::Instruction {
    ...
    intptrj_t _estimatedOffset; ///<  Save estimated offset for conservative distance calc to Call Descriptor Snippet
                                ///<  (XPLINK zOS31).
...
public:
    ...
    S390NOPInstruction(
        TR::InstOpCode::Mnemonic op, int32_t numbytes, TR::Snippet* ts, TR::Node* n, TR::CodeGenerator* cg)
        : TR::Instruction(op, n, cg)
    {
        setTargetSnippet(ts);
        setBinaryLength(numbytes);
    ...
    S390NOPInstruction(TR::InstOpCode::Mnemonic op, int32_t numbytes, TR::Snippet* ts, TR::Node* n,
        TR::Instruction* precedingInstruction, TR::CodeGenerator* cg)
        : TR::Instruction(op, n, precedingInstruction, cg)
    {
        setTargetSnippet(ts);
    ...
    /** Fastlink flavor */
    S390NOPInstruction(TR::InstOpCode::Mnemonic op, int32_t numbytes, int32_t argumentsLengthOnCall, TR::Node* n,
        TR::CodeGenerator* cg)
        : TR::Instruction(op, n, cg)
    {
        setTargetSnippet(NULL);
    ...
    S390PseudoInstruction* setCallDescInstr(S390PseudoInstruction* cdi) { return _callDescInstr = cdi; }

    int8_t getCallType() { return _callType; }
    void setCallType(uint8_t callType) { _callType = callType; }

    void setArgumentsLengthOnCall(int32_t argumentsLengthOnCall) { _argumentsLengthOnCall = argumentsLengthOnCall; }
