    ...
    uint32_t extranum, TR::Instruction** cursorPtr)
{
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && cg->machine()->getHPRFromGlobalRegisterNumber(child->getGlobalRegisterNumber())) {
    ...
    TR::RealRegister::RegNum& regNum)
{
    ...
    if (foundLow || foundHigh) {
        ...
        case TR_GPR:
            ...
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && (!reg->is64BitReg() || child->getOpCodeValue() == TR::icall || child->getOpCodeValue() == TR::icalli || TR::RealRegister::isHPR(regNum))) {
    ...
    for (int32_t i = 0; i < numOfDependencies; i++) {
        ...
        if (isHighWordSupported) {
            ...
            if (virtRegI && virtRegI->getKind() != TR_FPR && virtRegI->getKind() != TR_VRF) {
                ...
                if (TR::Compiler->target.is64Bit() && !(currentInstruction->isLabel() && toS390LabelInstruction(currentInstruction)->getLabelSymbol()->isStartOfColdInstructionStream()) && (realRegI == cg->getReturnAddressRegister() || realRegI == cg->getEntryPointRegister())) {
 ...
 */
void TR_S390RegisterDependencyGroup::checkRegisterDependencyDuplicates(TR::CodeGenerator* cg,
    const uint32_t numOfDependencies)
{
    ...
    for (uint32_t i = 0; i < numOfDependencies - 1; ++i) {
        ...
        for (uint32_t j = i + 1; j < numOfDependencies; ++j) {
            ...
            if (isHighWordSupported && realRegI == realRegJ + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR) && virtRegI->isPlaceholderReg() && virtRegJ->is64BitReg() && virtRegJ->getKind() != TR_FPR && virtRegJ->getKind() != TR_VRF) {
            ...
            if (isHighWordSupported && realRegJ == realRegI + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR) && virtRegJ->isPlaceholderReg() && virtRegI->is64BitReg() && virtRegI->getKind() != TR_FPR && virtRegI->getKind() != TR_VRF) {
    ...
    if (!comp->getOption(TR_DisableOOL)) {
        for (i = 0; i < numOfDependencies; i++) {
            ...
            if (dependentRegNum == TR::RealRegister::SpilledReg && !_dependencies[i].getRegister()->getRealRegister()) {
                ...
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF && virtReg->getAssignedRealRegister() == NULL) {
                ...
                if (virtReg->getAssignedRealRegister() || spilledHPR) {
                    ...
                    if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF) {
                    ...
                    if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && virtReg->is64BitReg() && rk != TR_FPR && rk != TR_VRF) {
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (virtReg->getAssignedRealRegister() != NULL) {
            if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
                ...
                if (_dependencies[i].getRealRegister() == assignedRegNum || (map.getDependencyWithTarget(assignedRegNum) && ((virtReg->getKind() != TR_GPR || haveSpareGPRs) && (virtReg->getKind() != TR_HPR || haveSpareHPRs) && (virtReg->getKind() != TR_FPR || haveSpareFPRs) && (virtReg->getKind() != TR_VRF || haveSpareVRFs)))) {
                    virtReg->block();
                }
            }
        }
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (dependentRegister->getRegisterPair() == NULL && dependentRegister->getAssignedRegister() != NULL && getRegisterDependency(i)->getRealRegister() != TR::RealRegister::SpilledReg) {
            ...
            if (highAssignedRegister && dependentRegisterHigh->getFutureUseCount() == 0) {
                ...
                // check if need to free HW
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                    if (dependentRegisterHigh->is64BitReg() && dependentRegisterHigh->getKind() != TR_FPR && dependentRegisterHigh->getKind() != TR_VRF) {
            ...
            if (lowAssignedRegister && dependentRegisterLow->getFutureUseCount() == 0) {
                ...
                // check if need to free HW
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                    if (dependentRegisterLow->is64BitReg() && dependentRegisterLow->getKind() != TR_FPR && dependentRegisterLow->getKind() != TR_VRF) {
                        toRealRegister(assignedRegister)->getHighWordRegister()->setState(TR::RealRegister::Unlatched);
