    ...
    TR::Register*& copyReg, TR::Register*& highCopyReg, TR::Instruction*& iCursor, TR::RealRegister::RegNum& regNum)
{
    ...
    if (foundLow || foundHigh) {
        ...
        case TR_GPR:
            ...
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)
                && (!reg->is64BitReg() || child->getOpCodeValue() == TR::icall || child->getOpCodeValue() == TR::icalli
 ...
 **/
void TR_S390RegisterDependencyGroup::checkRegisterPairSufficiencyAndHPRAssignment(TR::CodeGenerator* cg,
    TR::Instruction* currentInstruction, const uint32_t availableGPRMap, uint32_t numOfDependencies)
{
    TR::Machine* machine = cg->machine();
    ...
    for (int32_t i = 0; i < numOfDependencies; i++) {
        ...
        if (isHighWordSupported) {
            ...
            if (virtRegI && virtRegI->getKind() != TR_FPR && virtRegI->getKind() != TR_VRF) {
                ...
                if (TR::Compiler->target.is64Bit()
                    ...
                    && (realRegI == cg->getReturnAddressRegister() || realRegI == cg->getEntryPointRegister())) {
    ...
    if (!isRegPairSufficient) {
        ...
        comp->failCompilation<TR::CompilationException>(
            "checkDependencyGroup::Insufficient available pairs to satisfy all reg deps, abort compilation\n");
    }
}
 ...
 */
void TR_S390RegisterDependencyGroup::checkRegisterDependencyDuplicates(
    TR::CodeGenerator* cg, const uint32_t numOfDependencies)
{
    TR::Compilation* comp = cg->comp();
    ...
    for (uint32_t i = 0; i < numOfDependencies - 1; ++i) {
        ...
        for (uint32_t j = i + 1; j < numOfDependencies; ++j) {
            ...
            if (virtRegI == virtRegJ && realRegI != TR::RealRegister::SpilledReg
                && realRegJ != TR::RealRegister::SpilledReg) {
                TR_ASSERT(false, "Virtual register duplicate found in a register dependency\n");
            }

            if (realRegI == realRegJ && TR::RealRegister::isRealReg(realRegI)) {
                TR_ASSERT(false, "Real register duplicate found in a register dependency\n");
            }

            // highword RA: remove duplicated GPR/HPR deps
            if (isHighWordSupported && realRegI == realRegJ + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR)
                && virtRegI->isPlaceholderReg() && virtRegJ->is64BitReg() && virtRegJ->getKind() != TR_FPR
            ...
            if (isHighWordSupported && realRegJ == realRegI + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR)
                && virtRegJ->isPlaceholderReg() && virtRegI->is64BitReg() && virtRegI->getKind() != TR_FPR
...
uint32_t TR_S390RegisterDependencyGroup::checkDependencyGroup(
    TR::CodeGenerator* cg, TR::Instruction* currentInstruction, uint32_t numOfDependencies)
{
    ...
    checkRegisterPairSufficiencyAndHPRAssignment(cg, currentInstruction, availableGPRMap, numOfDependencies);
    ...
    TR_RegisterKinds kindToBeAssigned, uint32_t numOfDependencies, TR::CodeGenerator* cg)
{
        ...
        for (i = 0; i < numOfDependencies; i++) {
            ...
            if (dependentRegNum == TR::RealRegister::SpilledReg && !_dependencies[i].getRegister()->getRealRegister()) {
                TR_ASSERT(virtReg->getBackingStorage(),
                    "should have a backing store if dependentRegNum == spillRegIndex()\n");
                ...
                if (virtReg->getAssignedRealRegister() || spilledHPR) {
                    ...
                    if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR
                        && rk != TR_VRF) {
                ...
                // now we are leaving the OOL sequence, anything that was previously spilled in OOL hot path or main
                // line should be considered as mainline spill for the next OOL sequence.
                virtReg->getBackingStorage()->setMaxSpillDepth(1);
            }
            // we also need to free up all locked backing storage if we are exiting the OOL during backwards RA
            // assignment
    ...
    TR_ASSERT(numHPRs <= (TR::RealRegister::LastHPR - TR::RealRegister::FirstHPR + 1
                             - machine->getNumberOfLockedRegisters(TR_HPR)),
        "Too many HPR dependencies, unable to assign");
    ...
    if (numGPRs
        ...
        haveSpareGPRs = false;
    if (numHPRs
        == (TR::RealRegister::LastHPR - TR::RealRegister::FirstHPR + 1 - machine->getNumberOfLockedRegisters(TR_HPR)))
        haveSpareHPRs = false;
    if (numFPRs
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (_dependencies[i].getRealRegister() == TR::RealRegister::KillVolHighRegs && cg->use64BitRegsOn32Bit()) {
                ...
                if (comp->getOption(TR_EnableTrueRegisterModel) && _dependencies[i].getDefsRegister()
                    ...
                    if (cg->insideInternalControlFlow())
                        machine->reverseSpillState(
                            cg->getInstructionAtEndInternalControlFlow(), virtReg, toRealRegister(realReg));
                    else
                    ...
                    if (assignedRegister != NULL && assignedRegister->getState() == TR::RealRegister::Free) {
                        ...
                        fprintf(stdout, "COERCE3 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), aRealNum-1);
                                ...
                                if (comp->getOption(TR_EnableTrueRegisterModel)
                                    && _dependencies[lcount].getDefsRegister() && depReg->isPendingSpillOnDef()) {
                                    TR::RealRegister* realReg = machine->getRealRegister(aRealNum);
                                    if (cg->insideInternalControlFlow())
                                        machine->reverseSpillState(cg->getInstructionAtEndInternalControlFlow(), depReg,
        ...
        if (_dependencies[i].getRealRegister() == TR::RealRegister::AssignAny) {
            ...
            if (virtReg->isUsedInMemRef()) {
                ...
                if (comp->getOption(TR_EnableTrueRegisterModel) && _dependencies[i].getDefsRegister()
                    ...
                    if (cg->insideInternalControlFlow())
                        machine->reverseSpillState(
                            cg->getInstructionAtEndInternalControlFlow(), virtReg, toRealRegister(targetReg));
                    else
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (dependentRegister->getRegisterPair() == NULL
            && (dependentRegister->getAssignedRegister()
                   || (comp->getOption(TR_EnableTrueRegisterModel) && dependentRegister->isLive()))
            ...
            if ((dependentRegister->getFutureUseCount() == 0)
                ...
                if (assignedRealRegister != NULL && cg->supportsHighWordFacility()
                    ...
                    if (dependentRegister->is64BitReg() && dependentRegister->getKind() != TR_FPR
                        && dependentRegister->getKind() != TR_VRF) {
                        toRealRegister(assignedRealRegister)
                            ->getHighWordRegister()
                            ->setState(TR::RealRegister::Unlatched);
                        cg->traceRegFreed(
                            dependentRegister, toRealRegister(assignedRealRegister)->getHighWordRegister());
                    }
                }
            ...
            if (highAssignedRegister && dependentRegisterHigh->getFutureUseCount() == 0) {
                ...
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                    if (dependentRegisterHigh->is64BitReg() && dependentRegisterHigh->getKind() != TR_FPR
                        ...
                        cg->traceRegFreed(
                            dependentRegisterHigh, toRealRegister(assignedRegister)->getHighWordRegister());
                    }
                }
            }
            ...
            if (lowAssignedRegister && dependentRegisterLow->getFutureUseCount() == 0) {
                ...
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                    if (dependentRegisterLow->is64BitReg() && dependentRegisterLow->getKind() != TR_FPR
                        ...
                        toRealRegister(assignedRegister)->getHighWordRegister()->setState(TR::RealRegister::Unlatched);
                        cg->traceRegFreed(
                            dependentRegisterLow, toRealRegister(assignedRegister)->getHighWordRegister());
                    }
                }
            }
        }
