        ...
        TR::CodeGenerator *cg, TR::Node *node, uint32_t extranum, TR::Instruction **cursorPtr)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		if (child->getHighGlobalRegisterNumber() > -1) {
			highRegNum =
			        (TR::RealRegister::RegNum)cg->getGlobalRegister(child->getHighGlobalRegisterNumber());
		}
...
void
OMR::Z::RegisterDependencyConditions::resolveSplitDependencies(TR::CodeGenerator *cg, TR::Node *node, TR::Node *child,
        TR::RegisterPair *regPair, List<TR::Register> &regList, TR::Register *&reg, TR::Register *&highReg,
...
OMR::Z::RegisterDependencyConditions::clone(TR::CodeGenerator *cg, int32_t additionalRegDeps)
{
...
uint32_t
TR_S390RegisterDependencyGroup::genBitMapOfAssignableGPRs(TR::CodeGenerator *cg, uint32_t numberOfRegisters)
{
	TR::Machine *machine = cg->machine();

	// TODO: This looks like it can be cached as it should be the same value every time in a single compilation.
	// Should also investigate whether we even need this? Don't we already block registers which are part of a
	// dependency? I suppose with a mask we can avoid shuffling things into GPRs which are part of the dependency
	// and may need to get other virtual registers shuffled into such GPRs. Need to think about this. Are other
	// codegens doing this?
...
void
TR_S390RegisterDependencyGroup::assignRegisters(TR::Instruction *currentInstruction, TR_RegisterKinds kindToBeAssigned,
        uint32_t numOfDependencies, TR::CodeGenerator *cg)
{
		...
		for (i = 0; i < numOfDependencies; i++) {
			...
			if (dependentRegNum == TR::RealRegister::SpilledReg
				...
				if (virtReg->getAssignedRealRegister()) {
					// this happens when the register was first spilled in main line path then was
					// reverse spilled and assigned to a real register in OOL path. We protected the
					// backing store when doing the reverse spill so we could re-spill to the same
					// slot now
					...
					TR::MemoryReference *tempMR = generateS390MemoryReference(
					        currentNode, virtReg->getBackingStorage()->getSymbolReference(), cg);
					TR::InstOpCode::Mnemonic opCode;
					...
					case TR_GPR:
						...
						break;
					case TR_FPR: opCode = TR::InstOpCode::LD; break;
					case TR_VRF: opCode = TR::InstOpCode::VL; break;
					...
					TR::Instruction *inst = isVector
					        ? generateVRXInstruction(cg, opCode, currentNode, assignedReg, tempMR,
					                0, currentInstruction)
					        : generateRXInstruction(cg, opCode, currentNode, assignedReg, tempMR,
					                currentInstruction);
				...
				// now we are leaving the OOL sequence, anything that was previously spilled in OOL hot
				// path or main line should be considered as mainline spill for the next OOL sequence.
				virtReg->getBackingStorage()->setMaxSpillDepth(1);
			}
			// we also need to free up all locked backing storage if we are exiting the OOL during backwards
			// RA assignment
	...
	for (i = 0; i < numOfDependencies; i++) {
		...
		if (dependentRegNum != TR::RealRegister::SpilledReg
			...
			case TR_GPR: ++numGPRs; break;
			case TR_FPR: ++numFPRs; break;
			case TR_VRF: ++numVRFs; break;
			default: break;
			}
		}
	}
	...
	for (i = 0; i < numOfDependencies; i++) {
		...
		if (virtReg->getAssignedRealRegister() != NULL) {
			if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
				...
				// Always block if the required register and assigned register match or if the assigned
				// register is required by another dependency but only if there are any spare registers
				// left so as to avoid blocking all existing registers
	...
	for (i = 0; i < numOfDependencies; i++) {
		...
		if (dependentRegNum == TR::RealRegister::LegalEvenOfPair) {
			...
			//
			if (assRealReg == NULL
			        || (!machine->isLegalEvenRegister(toRealRegister(assRealReg), ALLOWBLOCKED))) {
		...
		if (dependentRegNum == TR::RealRegister::LegalOddOfPair) {
			...
			//
			if (assRealReg == NULL
			        || (!machine->isLegalOddRegister(toRealRegister(assRealReg), ALLOWBLOCKED))) {
		...
		if (dependentRegNum == TR::RealRegister::LegalFirstOfFPPair) {
			...
			//
			if (assRealReg == NULL
			        || (!machine->isLegalFirstOfFPRegister(toRealRegister(assRealReg), ALLOWBLOCKED))) {
		...
		if (dependentRegNum == TR::RealRegister::LegalSecondOfFPPair) {
			...
			//
			if (assRealReg == NULL
			        || (!machine->isLegalSecondOfFPRegister(toRealRegister(assRealReg), ALLOWBLOCKED))) {
					...
					if (assignedRegister != NULL
						...
						for (int32_t lcount = 0; lcount < numOfDependencies; lcount++) {
							...
							if (aRealNum != TR::RealRegister::NoReg
							        ...
							        && assignedRegister
							                == machine->getRealRegister(aRealNum)) {
#if 0
                        fprintf(stdout, "COERCE3 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), aRealNum-1);
								...
								// Coerce the assignment
								TR::Register *depReg =
								        _dependencies[lcount].getRegister();
...
bool
OMR::Z::RegisterDependencyConditions::doesConditionExist(TR_S390RegisterDependencyGroup *regDepArr, TR::Register *vr,
        TR::RealRegister::RegNum rr, uint32_t flag, uint32_t numberOfRegisters, bool overwriteAssignAny)
{
        ...
        TR::Register *vr, TR::RealRegister::RegDep rr, uint8_t flag)
{
	int32_t pos = -1;
	if (_preConditions != NULL
	        && (pos = _preConditions->searchForRegisterPos(vr, RefsAndDefsDependentRegister, _addCursorForPre, _cg))
	                < 0) {
        ...
        TR::Register *vr, TR::RealRegister::RegDep rr, uint8_t flag)
{
	...
	if (_postConditions != NULL
	        && (pos = _postConditions->searchForRegisterPos(
	                    vr, RefsAndDefsDependentRegister, _addCursorForPost, _cg))
	                < 0) {
		if (!(vr->getRealRegister() && vr->getRealRegister()->getState() == TR::RealRegister::Locked))
