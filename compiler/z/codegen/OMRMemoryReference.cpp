 ...
 */
void
OMR::Z::MemoryReference::setupCausesImplicitNullPointerException(TR::CodeGenerator *cg)
{
 ...
 */
void
OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node *node, int32_t offset, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	if (offset < 0) {
		if (cg->traceBCDCodeGen())
			traceMsg(comp,
			        "\taddToTemporaryNegativeOffset : %s (%p) new offset %d, existing mr offset %d "
			        "(existing mr hasTempNegOffset = %s)\n",
			        node ? node->getOpCode().getName() : "NULL", node, offset, self()->getOffset(),
			        self()->hasTemporaryNegativeOffset() ? "yes" : "no");
		if (self()->getOffset() < 0 && !self()->hasTemporaryNegativeOffset()) // if there is a 'real' negative
		                                                                      ...
		                                                                      // temporary negative offset
		{
			if (cg->traceBCDCodeGen())
				traceMsg(comp,
				        "\t\texisting mr->offset %d < 0 so call enforceSSFormatLimits to clear this up "
				        "before setting HasTemporaryNegativeOffset\n",
				        self()->getOffset());
			self()->enforceSSFormatLimits(node, cg, NULL); // call SSFormatLimits to also take this chance
			                                               // to fold in an index register if needed
		}
 ...
 */
bool
OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator *cg, TR::Node *addressChild)
{
	...
	if (!addressChild->getOpCode().isAdd()) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) is not an add type - most "
			        "likely cannot fold\n",
			        addressChild->getOpCode().getName(), addressChild);
		}
		return false;
	...
	if (!addressChild->getSecondChild()->getOpCode().isLoadConst()) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) second child is not a "
			        "load const - cannot fold\n",
			        addressChild->getOpCode().getName(), addressChild);
		}
		return false;
	}
	...
	if (addressChild->getSecondChild()->getRegister() != NULL) {
		...
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) const child %s (%p) has "
			        "register %s, cannot fold.\n",
			        addressChild->getOpCode().getName(), addressChild,
	...
	if (addressChild->getReferenceCount() != 1) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) has ref count %d, cannot "
			        "fold, need value in register.\n",
	...
	while (eventualNonConversion->getOpCode().isConversion()) {
		...
		allConvNodesHaveRefCount1 = allConvNodesHaveRefCount1
	...
	if (haveToEvalConvIntoRegister) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. "
			        "with ref count > 1 and no register, must evaluate - setForceFolding()\n",
			        addressChild->getOpCode().getName(), addressChild);
		}
	...
	if (!allConvNodesAreUnneeded) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. "
			        "which are needed, folding after evaluation.\n",
			        addressChild->getOpCode().getName(), addressChild);
		}
		cg->evaluate(addressChild->getFirstChild());
		self()->setForceFolding();
	...
	if (foundRegister) {
		...
		//    iconst
		if (cg->traceBCDCodeGen()) {
			TR::Node *dispNode = eventualNonConversion->getRegister() ? eventualNonConversion
			                                                          : foundConvNodeWithRegister;

			traceMsg(comp,
			        "\t\taddressChild %s (%p) node %s (%p) is evaluatedReg+const (%s + %lld) so %s=true\n",
			        addressChild->getOpCode().getName(), addressChild, dispNode->getOpCode().getName(),
			        dispNode, cg->getDebug()->getName(dispNode->getRegister()),
	...
	bool eventualNonConversionIsSuitableForFolding = (eventualNonConversion->getOpCodeValue() == TR::loadaddr
	        || eventualNonConversion->getOpCode().isLoad() || eventualNonConversion->getRegister()
	        || eventualNonConversion->getOpCode().isLoadReg());

	if (!eventualNonConversionIsSuitableForFolding) {
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) eventual non conversion "
			        "%s (%p) is not suitable for folding.\n",
			        addressChild->getOpCode().getName(), addressChild,
	...
	if (!eventualNonConversion->getOpCode().isIndirect()) {
		...
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register "
			        "and refCount==1 and is a aload+const so setForceFolding=true\n",
			        eventualNonConversion->getOpCode().getName(), eventualNonConversion);
		}
		self()->setForceFolding();
		return true;
	} else // if (eventualNonConversion->getReferenceCount() == 1)
	{
		...
		if (cg->traceBCDCodeGen()) {
			traceMsg(comp,
			        " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register "
			        "and refCount==1 and is an iaload+const so eval(%s - %p) and setForceFolding=true\n",
			        eventualNonConversion->getOpCode().getName(), eventualNonConversion,
 ...
 */
void
recursivelyDecrementIncrementedNodesIfUnderRegister(TR::CodeGenerator *cg, TR::Node *cur, List<TR::Node> &_incNodesList,
        ...
        , _incrementedNodesList(cg->comp()->trMemory())
{
	...
	_listingSymbolReference = TR::MemoryReference::shouldLabelForRAS(_symbolReference, cg) ? _symbolReference
	                                                                                       : NULL;
	...
	if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
		...
		if (symbol && symbol->isStatic()) {
			...
			if (!rootLoadOrStore->getOpCode().isLoad() && !rootLoadOrStore->getOpCode().isStore()) {
				// this is a valid path through this constructor, despite what the variable name might
				// mean
				//
				// for example, calling generateMemoryReference([node], cg) on an aiadd node will arrive
				// here.
        ...
        , _incrementedNodesList(cg->comp()->trMemory())
{
	...
	if (base) {
		...
		// TR_ASSERT(cg->supportsOnDemandLiteralPool()==true, "May not be here with Literal Pool On Demand
		// disabled\n");
	} else {
 ...
 */
void
noteAllNodesWithRefCountNotOne(TR_Array<TR::Node *> &nodeArray, TR::Node *n, TR::Compilation *comp)
{
...
void
ArtificiallyInflateReferenceCountWhenNecessary(TR::MemoryReference *mr, const char *functionString,
	...
	for (uint32_t i = 0; i < nodeArray.size(); i++) {
		...
		if ((nodeArray[i]->getReferenceCount() == 1) && (nodeArray[i]->getRegister() != NULL)) {
			if (comp->getOption(TR_TraceCG)) {
				traceMsg(comp,
				        " ArtificiallyInflateReferenceCountWhenNecessary: bug potential with node %p "
				        "(reg %p %s%s): ref count after %s is 1.\n",
			...
			if (incrementedNodesList) {
				// This node's reference count went from N -> 1 during "evaluation" in one of the
				// populate* functions.
				//
				// Check if this node's reference count was inflated artificially somewhere else - look
				// for the reference count changing like so:
				//
				// 1 -> N -> 1
				//
				...
				// 1 -> N -> 1 -> 2
				//
				// as this can have performance implications - the reference count came into this scope
				// at 1, and by inflating it we are saying that we can't evaluate it destructively.
				//
				...
				if (listNode == nodeArray[i]) {
					// Skip processing the current node because the reference count was already
					// artificially inflated somewhere else.
					if (comp->getOption(TR_TraceCG)) {
						traceMsg(comp,
						        ...
						        nodeArray[i], nodeArray[i]->getRegister(),
						        nodeArray[i]->getRegister()
						                ? (nodeArray[i]->getRegister()->isPlaceholderReg() ? "D"
						                                                                     "_"
						                                                                   : "")
						                : "?",
			...
			// Add to a stack so that these nodes will have their reference counts decreased after
			// evaluation.
			if (cg->AddArtificiallyInflatedNodeToStack(nodeArray[i])) {
				// Artificially inflate reference count by 1. This will not allow destructive evaluation
				// of this node.
...
OMR::Z::MemoryReference::populateAddTree(TR::Node *subTree, TR::CodeGenerator *cg)
{
	...
	if ((integerChild->getOpCodeValue() == TR::isub || integerChild->getOpCodeValue() == TR::lsub)) {
		 ...
		 * This is needed to prevent Zero Address Detection events on z/OS which happen when the base register
		 * of an instruction is zero. When we fold the lsub the lRegLoad becomes the _baseRegister and the
		 * lconst becomes offset. The important point is that in this case the _indexRegister is NULL. Thus we
		 * need to take care to select the appropriate register in the code below.
		 */
...
bool
OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator *cg, TR::Node *loadStore, TR::Node *topAdd)
{
...
OMR::Z::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
	...
	if (self()->doEvaluate(subTree, cg)) {
		...
		// Add this memory reference to a list of "folded" memory references, because it is not the doEvaluate
		// branch and there is the possibility that the mem ref will not be loaded into a register.
		cg->AddFoldedMemRefToStack(self());
...
TR::Register *
OMR::Z::MemoryReference::swapBaseRegister(TR::Register *br, TR::CodeGenerator *cg)
{
...
TR::Instruction *
OMR::Z::MemoryReference::handleLargeOffset(TR::Node *node, TR::MemoryReference *interimMemoryReference,
        TR::Register *tempTargetRegister, TR::CodeGenerator *cg, TR::Instruction *preced)
{
	...
	if (_offset > MINLONGDISP && _offset < MAXLONGDISP) {
		...
		if (preced)
			preced =
			        generateRXInstruction(cg, op, node, tempTargetRegister, interimMemoryReference, preced);
		else
...
OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	// e.g. _offset=4088 and the node is 10 byte in size so the max possible alignment is 10-1 = 9 giving a max
	// possible offset of 4088+9 = 4097 (>= MAXDISP of 4096)
	//
	if (_offset < MAXDISP && // skip if going to fixup naturally anyway
	        self()->getFixedSizeForAlignment() > 0 && // i.e. do not check temps -- these will have an offset of 0
	                                                  // anyway at instruction selection time
	        (self()->rightAlignMemRef() || self()->leftAlignMemRef()
	                || TR::MemoryReference::typeNeedsAlignment(node))) {
		int32_t maxAlignmentBump = self()->getFixedSizeForAlignment() - 1; // alignment is
		                                                                   // getFixedSizeForAlignment() -
		                                                                   // (length or leftMostByte) and each
		                                                                   // of these must be at least 1
		if (_offset + maxAlignmentBump >= MAXDISP) {
			if (cg->traceBCDCodeGen())
				traceMsg(cg->comp(),
				        "\tz^z : node %s (%p) : _offset %d + maxAlignmentBump %d >= MAXDISP %d : (%d "
				        ">= %d) -- force 4K fixup\n",
				        node ? node->getOpCode().getName() : "NULL", node, _offset, maxAlignmentBump,
				        MAXDISP, _offset + maxAlignmentBump, MAXDISP);
			return true;
		}
	}
	return false;
 ...
 */
TR::Instruction *
OMR::Z::MemoryReference::enforce4KDisplacementLimit(
        TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *preced, bool forcePLXFixup, bool forceFixup)
{
	...
	TR_ASSERT(!forcePLXFixup,
	        "This logic is only used for markAndAdjustForLongDisplacementIfNeeded. You probably want the other "
	        "flag.");
 ...
 */
TR::Instruction *
OMR::Z::MemoryReference::separateIndexRegister(TR::Node *node, TR::CodeGenerator *cg, bool enforce4KDisplacementLimit,
        TR::Instruction *preced, bool forceDueToAlignmentBump)
{
	TR::Compilation *comp = cg->comp();
	if (_indexRegister != NULL) {
		if (_baseRegister == NULL) {
			// if baseRegister happens to be NULL, we can just set that to be the old index register and be
			// done
		...
		if (!self()->ignoreNegativeOffset() && enforce4KDisplacementLimit
			...
			preced =
			        self()->handleLargeOffset(node, interimMemoryReference, tempTargetRegister, cg, preced);
			_offset = 0;
		} else {
			if (preced)
				preced = generateRXInstruction(cg, TR::InstOpCode::LA, node, tempTargetRegister,
...
OMR::Z::MemoryReference::propagateAlignmentInfo(OMR::Z::MemoryReference *newMemRef)
{
	...
	// when a new interim symRef is created for large/negative displacements then all information that will be used
	for later
...
OMR::Z::MemoryReference::canUseTargetRegAsScratchReg(TR::Instruction *instr)
{
	...
	case TR::InstOpCode::LGF: {
		...
		TR::RealRegister *targetReg =
		        (TR::RealRegister *)((TR::S390RegInstruction *)instr)->getRegisterOperand(1);
...
OMR::Z::MemoryReference::getTotalSizeForAlignment()
{
	if (self()->getOriginalSymbolReference() && self()->getOriginalSymbolReference()->getSymbol()
		...
		TR_ASSERT(_fixedSizeForAlignment > 0,
...
OMR::Z::MemoryReference::getRightAlignmentBump(TR::Instruction *instr, TR::CodeGenerator *cg)
{
	...
	case TR::Instruction::IsSS1: length = toS390SS1Instruction(instr)->getLen() + 1; break;
	case TR::Instruction::IsSS2:
		...
		break;
	case TR::Instruction::IsRSL: length = toS390RSLInstruction(instr)->getLen() + 1; break;
	case TR::Instruction::IsRSLb: length = toS390RSLbInstruction(instr)->getLen() + 1; break;
	case TR::Instruction::IsVSI:
		length = static_cast<TR::S390VSIInstruction *>(instr)->getImmediateField3() + 1; // I3 is operand 2
		                                                                                 // length code
		break;
	default: break;
	}
	...
	// a negative bump means that the allocated symbol size is not big enough to contain the encoded operation and
...
OMR::Z::MemoryReference::calcDisplacement(uint8_t *cursor, TR::Instruction *instr, TR::CodeGenerator *cg)
{
	...
	if (self()->rightAlignMemRef()) {
		...
		if (cg->traceBCDCodeGen())
			traceMsg(comp,
			        "instr %p : right aligning memRef with symRef #%d (%s, isTemp %s) _offset %d, "
			        "totalSize %d : bump = ",
	...
	if (self()->leftAlignMemRef()) {
		...
		if (cg->traceBCDCodeGen())
			traceMsg(comp,
			        "instr %p : left aligning memRef with symRef #%d (%s, isTemp %s), _offset %d : bump = "
			        "totalSize - leftMostByte = %d - %d = ",
			        instr, self()->getSymbolReference()->getReferenceNumber(),
			        cg->getDebug()->getName(self()->getSymbolReference()->getSymbol()),
			        self()->getSymbolReference()->isTempVariableSizeSymRef() ? "yes" : "no", _offset,
	...
	if (self()->getSymbolReference()->isTempVariableSizeSymRef()) {
		if (cg->traceBCDCodeGen())
			traceMsg(comp,
			        "instr %p : bumping tempVariableSizeSymRef memRef with symRef #%d (%s), _offset %d : "
			        "bump = symSize - totalSize = %d - %d = ",
			        instr, self()->getSymbolReference()->getReferenceNumber(),
	...
	if (snippet != NULL) {
		{
			...
			if (cursor != NULL) {
				cg->addRelocation(new (cg->trHeapMemory())
				                TR::LabelRelative12BitRelocation(cursor, snippet->getSnippetLabel()));
			}
		}
	} else if (symbol != NULL) {
		if (symbol->isRegisterMappedSymbol()) {
        ...
        TR::CodeGenerator *cg, TR::Node *node, TR::Register *targetRegister, intptr_t value, TR::Instruction *cursor)
{
	...
	if (value >= (int32_t)0xFFFF8000 && value <= (int32_t)0x00007FFF) {
		...
		if (TR::Compiler->target.is64Bit()) {
			cursor = generateRSInstruction(cg, TR::InstOpCode::SLLG, node, targetRegister, targetRegister,
		...
		cursor = generateRXInstruction(cg, TR::InstOpCode::LA, node, targetRegister, // LA  Rscrtch,
		                                                                             // value&0xFFF(,Rscrtch)
...
OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t *cursor, TR::CodeGenerator *cg, TR::Instruction *instr)
{
	...
	TR::RealRegister *LongDispSlotBaseReg = useNormalSP ? (cg->getLinkage())->getNormalStackPointerRealRegister()
	                                                    : spReg;
	...
	// In most cases we will know at instruction selection that the displacement is large. But it is still possible
	// for us to reach binary encoding without knowing a displacement will be large. A typical example of this is
	// when referencing stack symbols whose displacement will be unknown until the stack is mapped; an operation
	// that happens after instruction selection and before binary encoding.
	if (displacement < MINDISP || displacement >= MAXDISP) {
		auto longDisplacementMnemonic =
		        TR::InstOpCode::getEquivalentLongDisplacementMnemonic(instr->getOpCodeValue());
		...
		if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
			...
			// The corresponding RX/RS/SI `generateBinaryEncoding()` function will "upgrade" the instruction
			// to the corresponding long displacement version. Note that the logic here must stay
			// synchronized with the following evaluators which we add here as eyecatchers for future
			// modifications:
			//
			...
			// TODO (#2848): We should set this to the actual `longDisplacementMnemonic` format once we
			// unify instruction formats and S390*Instruction formats
			instructionFormat = TR::Instruction::IsRXY;
			...
			if (TR::MemoryReference::canUseTargetRegAsScratchReg(instr)) {
				...
				TR_ASSERT(!self()->isMemRefMustNotSpill(),
				        "OMR::Z::MemoryReference::generateBinaryEncoding -- This memoryReference must "
				        "not spill");
				if (!is2ndSSMemRef) {
			...
			TR_ASSERT(scratchReg != NULL,
			        "OMR::Z::MemoryReference::generateBinaryEncoding -- A scratch reg should always be "
			        "found.");

			largeDisp = true;
			if (spillNeeded) {
				currInstr = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node,
				        scratchReg,
				        generateS390MemoryReference(LongDispSlotBaseReg, offsetToLongDispSlot, cg),
				        prevInstr);
			...
			} else if (index == NULL) // Since index reg is free, use it directly
			{
				...
				index = scratchReg;
				self()->setIndexRegister(index);
			} else // base is NULL                                       // Since base reg is free, use it
			       // directly
			{
				TR_ASSERT_FATAL(instructionFormat == TR::Instruction::IsRX
				                || instructionFormat == TR::Instruction::IsRXY
	...
	if (largeDisp) {
		TR_ASSERT(!instr->getOpCode().isBranchOp(),
		        "We don't handle long disp for branch instruction [%p] using long displacement stack slot",
		        instr);
	}
	...
	if (!largeDisp && self()->getCheckForLongDispSlot() && displacement == offsetToLongDispSlot
		...
		TR_ASSERT(false,
		        "Bad use of long displacement slot (%d) on the stack for instruction [%p] corresponding to "
		        "node [%p]",
        ...
        uint8_t *cursor, TR::CodeGenerator *cg, TR::Instruction *instr)
{
	...
	if (TR::MemoryReference::canUseTargetRegAsScratchReg(instr)) {
		...
		TR_ASSERT(!self()->isMemRefMustNotSpill(),
		        "OMR::Z::MemoryReference::generateBinaryEncodingTouchUpForLongDisp -- This memoryReference "
		        "must not spill");
		if (!is2ndSSMemRef) {
	...
	TR_ASSERT(scratchReg != NULL,
	        "OMR::Z::MemoryReference::generateBinaryEncodingTouchUpForLongDisp -- A scratch reg should always be "
	        "found.");

	// Restore the scratch register
	if (largeDisp && spillNeeded) {
		TR::Instruction *nextInstr = instr->getNext();
		TR::Instruction *currInstr = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(),
		        instr->getNode(), scratchReg,
...
OMR::Z::MemoryReference::doEvaluate(TR::Node *subTree, TR::CodeGenerator *cg)
{
	...
	if (self()->forceFirstTimeFolding()) {
		if (cg->traceBCDCodeGen())
			traceMsg(comp,
			        "\tforceFirstTimeFolding=true for subTree %s (%p) refCount %d, reg %s (reset "
			        "firstTimeFlag for next doEvaluate)\n",
...
{
	// The TR::MemoryReference(TR::Node*, TR::CodeGenerator*, bool); constructor (below) is made for store or load
	// nodes that have symbol references. A symbol reference is needed to adjust memory reference displacement
	// (TR::MemoryReference::calcDisplacement). If the node has no sym ref, this memory reference's gets a NULL
	// symbol reference, which can lead to crashes in displacement calculations.
...
findBestSymRefForArrayCopy(TR::CodeGenerator *cg, TR::Node *arrayCopyNode, TR::Node *srcNode)
{
...
generateS390MemoryReference(TR::CodeGenerator *cg, TR::Node *node, TR::Node *baseNode, int32_t disp, bool forSS)
{
	...
	mr->setIsOriginalSymRefForAliasingOnly(); // do not use this symref to derive any memref offsets but use it only
	                                          // for aliasing for scheduling
...
TR::MemoryReference *
generateS390ConstantAreaMemoryReference(
...
generateS390ConstantAreaMemoryReference(TR::CodeGenerator *cg, TR::Node *addrNode, bool forSS)
{
	...
	TR::MemoryReference *mr = generateS390MemoryReference(
	        cg, comp->getSymRefTab()->findOrCreateConstantAreaSymbolReference(), addrNode, 0);
	if (forSS)
...
TR::MemoryReference *
reuseS390MemoryReference(
        TR::MemoryReference *baseMR, int32_t offset, TR::Node *node, TR::CodeGenerator *cg, bool enforceSSLimits)
{
	// if we are reusing memory reference and offset is not 0, it will generate new memory reference based on
	// current offset and return New MR. This will prevent original Memory reference being modified even if it
...
static TR::MemoryReference *
generateS390AlignedMemoryReference(
...
TR::MemoryReference *
generateS390LeftAlignedMemoryReference(TR::MemoryReference &baseMR, TR::Node *node, int32_t offset,
