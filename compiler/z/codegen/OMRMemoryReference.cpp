    ...
    , _incrementedNodesList(cg->comp()->trMemory())
{
    ...
    if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
        ...
        // storageReference cases may contain symbol sizes from 1->64 and in any case this condition is now enforced
        // when the subsequent generate*Instruction routine is called with this memRef
        if ((rootLoadOrStore->getType().isInt64() || (symbol && symbol->getSize() == 8))
            && !(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) && !canUseRX
        ...
        if (symbol && symbol->isStatic()) {
            ...
            if (symRef && symRef->isUnresolved()) {
                ...
                genLoadAddressConstant(cg, rootLoadOrStore,
                    (uintptrj_t)symRef->getSymbol()->getStaticSymbol()->getStaticAddress(), _baseRegister);
                cg->stopUsingRegister(tempReg);
            }
        } else if (!symRef) {
