    ...
    , _incrementedNodesList(cg->comp()->trMemory())
{
    ...
    if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
        ...
        if ((rootLoadOrStore->getType().isInt64() || (symbol && symbol->getSize() == 8)) && !(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) && !canUseRX && (storageReference == NULL)) {
...
void OMR::Z::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (self()->doEvaluate(subTree, cg)) {
        ...
        } else if (subTree->getOpCodeValue() == TR::aload && cg->isAddressOfStaticSymRefWithLockedReg(subTree->getSymbolReference())) {
            self()->populateAloadTree(subTree, cg, false);
        } else if (subTree->getOpCodeValue() == TR::aload && cg->isAddressOfPrivateStaticSymRefWithLockedReg(subTree->getSymbolReference())) {
...
void OMR::Z::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
        ...
        if (_baseRegister->decFutureUseCount() == 0) {
            ...
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && _baseRegister->is64BitReg()) {
...
bool OMR::Z::MemoryReference::doEvaluate(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (subTree->getOpCodeValue() == TR::aload && cg->isAddressOfStaticSymRefWithLockedReg(subTree->getSymbolReference()))
        return false;

    if (subTree->getOpCodeValue() == TR::aload && cg->isAddressOfPrivateStaticSymRefWithLockedReg(subTree->getSymbolReference()))
        return false;
