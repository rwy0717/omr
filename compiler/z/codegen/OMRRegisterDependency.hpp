...
public:
    ...
    void checkRegisterPairSufficiencyAndHPRAssignment(TR::CodeGenerator* cg, TR::Instruction* currentInstruction,
        const uint32_t availableGPRMap, uint32_t numOfDependencies);

    void checkRegisterDependencyDuplicates(TR::CodeGenerator* cg, const uint32_t numOfDependencies);

    uint32_t checkDependencyGroup(
