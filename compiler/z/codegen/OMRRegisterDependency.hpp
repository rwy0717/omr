...
public:
	...
	TR::Register *searchForRegister(
	        TR::Register *vr, uint8_t flag, uint32_t numberOfRegisters, TR::CodeGenerator *cg)
	{
...
public:
	...
	RegisterDependencyConditions(uint16_t numPreConds, uint16_t numPostConds, TR::CodeGenerator *cg)
	        ...
	        , _postConditions(
	                  TR_S390RegisterDependencyGroup::create(numPostConds + NUM_VM_THREAD_REG_DEPS, cg->trMemory()))
	        , _numPreConditions(numPreConds)
	        , _addCursorForPre(0)
	...
	{
		TR_ASSERT(
		        !getIsUsed(), "ERROR: cannot add pre conditions to an used dependency, create a copy first\n");
		...
		if (_addCursorForPre >= _numPreConditions) {
			...
			_cg->comp()->failCompilation<TR::CompilationException>("addPreCondition list overflow, abort "
			                                                       "compilation\n");
		}
	...
	{
		TR_ASSERT(
		        !getIsUsed(), "ERROR: cannot add post conditions to an used dependency, create a copy first\n");
		...
		if (_addCursorForPost >= _numPostConditions) {
			...
			_cg->comp()->failCompilation<TR::CompilationException>("addPostCondition list overflow, abort "
			                                                       "compilation\n");
		}
	...
	TR::Register *searchPreConditionRegister(
	        TR::RealRegister::RegNum rr, uint8_t flag = RefsAndDefsDependentRegister)
	{
		return _preConditions == NULL ? NULL
		                              : _preConditions->searchForRegister(rr, flag, _addCursorForPre, _cg);
	}
	...
	int32_t searchPostConditionRegisterPos(TR::Register *vr, uint8_t flag = RefsAndDefsDependentRegister)
	{
		return _postConditions == NULL
		        ? -1
		        : _postConditions->searchForRegisterPos(vr, flag, _addCursorForPost, _cg);
	}
	...
	bool doesConditionExist(TR_S390RegisterDependencyGroup *regDepArr, TR::Register *vr,
