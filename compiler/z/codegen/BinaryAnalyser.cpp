...
void
TR_S390BinaryAnalyser::remapInputs(
...
void
TR_S390BinaryAnalyser::genericAnalyser(TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
	...
	if (getCopyReg1()) {
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
			if (getBinaryReg3Reg2() || secondRegister != NULL) {
				if (regToRegOpCode == TR::InstOpCode::SR) {
					generateRRRInstruction(cg(), TR::InstOpCode::SRK, root, thirdReg, firstRegister,
					        secondRegister);
					done = true;
		...
		if (!done) {
			...
			if (getBinaryReg3Reg2() || (secondRegister != NULL)) {
				...
				TR::Node *loadBaseAddr = is16BitMemory2Operand ? secondChild->getFirstChild()
				                                               : secondChild;
