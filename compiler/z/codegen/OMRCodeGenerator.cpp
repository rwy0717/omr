...
void
OMR::Z::CodeGenerator::lowerTreeIfNeeded(TR::Node *node, int32_t childNumberOfNode, TR::Node *parent, TR::TreeTop *tt)
{
	...
	if (TR::Compiler->target.cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore())
		...
		if (add1 && add2 && const1
			...
			// traceMsg(comp(), "&&& Found pattern root=%llx add1=%llx add2=%llx const1=%llx sub=%llx
        ...
        , _afterRA(false)
{
	...
	if (self()->supportsBranchPreloadForCalls()) {
		_callsForPreloadList = new (self()->trHeapMemory()) TR::list<TR_BranchPreloadCallData *>(
	...
	if (comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRARegisterStates)) {
		...
		self()->setFPRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(
		        self()->machine(), TR::RealRegister::FirstFPR, TR::RealRegister::LastFPR));
		self()->setVRFRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(
		        self()->machine(), TR::RealRegister::FirstVRF, TR::RealRegister::LastVRF));
	}
...
OMR::Z::CodeGenerator::prepareForGRA()
{
	...
	if (!_globalRegisterTable) {
		...
		// We call init here because getNumberOfGlobal[FG]PRs() is initialized during the call to initialize()
		// above.
		//
		...
		for (grn = self()->getFirstGlobalGPR(); grn <= self()->getLastGlobalGPR(); grn++) {
			...
			if (globalReg != -1
			        && self()->getS390Linkage()->getPreserved((TR::RealRegister::RegNum)globalReg)) {
		...
		if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
			for (int32_t i = 0; i < TR_numSpillKinds; i++)
				_globalRegisterBitVectors[i].init(
				        self()->getNumberOfGlobalRegisters(), self()->trMemory());
			...
			for (grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
				...
				TR_ASSERT(reg != -1,
				        "Register pressure simulator doesn't support gaps in the global register "
				        "table; reg %d must be removed",
				        grn);
...
bool
OMR::Z::CodeGenerator::mulDecompositionCostIsJustified(
	...
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
		...
		if (trace)
			if (numCycles <= 9)
				...
				traceMsg(
				        self()->comp(), "MulDecomp cost is too high. numCycle=%i(max:10)\n", numCycles);
		return numCycles <= 9;
	} else {
...
OMR::Z::CodeGenerator::beginInstructionSelection()
{
	...
	if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
		...
		if (self()->comp()->getJittedMethodSymbol()->isJNI()
			...
			if (TR::Compiler->target.is64Bit()) {
				cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(TR::InstOpCode::DC,
				        startNode, LOWER_4_BYTES(jniMethodTargetAddress), cursor, self());
			}
		}
...
OMR::Z::CodeGenerator::insertInstructionPrefetches()
{
	if (self()->supportsBranchPreload()) {
		if (_hottestReturn._frequency > 6) {
			...
			// if we encountered a call, insert after the call or after the label that follows the call
			// instr.
			if (first != cursor) {
				cursor = real;
			}
			// if we didn't encounter a call or label, but there were real instructions, insert before a
			// real instruction
			else if (real != NULL) {
			...
			cursor = generateRXInstruction(
			        self(), TR::InstOpCode::getExtendedLoadOpCode(), node, tempReg, tempMR, cursor);
...
OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(TR_BranchPreloadCallData *data)
{
	...
	if (true) //! isInLoop || !bppMaxPath)
	{
		...
		while (first != cursor) {
			...
			if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND
				...
				real = cursor;
				if (cursor->getNext() && first != cursor->getNext()) // op.getOpCodeValue() ==
				                                                     // TR::InstOpCode::BPRP ||
				                                                     // op.getOpCodeValue() ==
				                                                     // TR::InstOpCode::BPP)
				{
		...
		for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
			...
			while (first != cursor) {
				...
				if (!op.isAdmin() && !op.isLabel()) {
					count2++;
					if (canReachWithBPRP
					        && op.getOpCodeValue() == TR::InstOpCode::BPRP) // ||
					                                                        // op.getOpCodeValue()
					                                                        // ==
					                                                        // TR::InstOpCode::BPP)
					{
			...
			if (bppCall && (!canReachWithBPRP || bppOnly)) {
				...
				cursor = new (self()->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL, node,
				        tempReg, (data->_callSymRef)->getSymbol(), data->_callSymRef, cursor, self());

				tempMR = generateS390MemoryReference(tempReg, 0, self());
				cursor = generateS390BranchPredictionPreloadInstruction(self(), TR::InstOpCode::BPP,
				        node, data->_callLabel, (int8_t)0xD, tempMR, cursor);
				cursor->setNext(second);
				self()->stopUsingRegister(tempReg);

			} else {
				cursor = generateS390BranchPredictionRelativePreloadInstruction(self(),
				        TR::InstOpCode::BPRP, node, data->_callLabel, (int8_t)0xD, data->_callSymRef,
				        cursor);
				cursor->setNext(second);
			}
		}
	...
	if (!bppMaxPath) {
		...
		if (bppCall && (!canReachWithBPRP || bppOnly)) {
			...
			cursor = generateS390BranchPredictionRelativePreloadInstruction(self(), TR::InstOpCode::BPRP,
			        node, data->_callLabel, (int8_t)0xD, data->_callSymRef, cursor);
			cursor->setNext(second);
		}
	}
}
...
void
OMR::Z::CodeGenerator::recordRegisterAssignment(TR::Register *assignedReg, TR::Register *virtualReg)
{
...
OMR::Z::CodeGenerator::prepareRegistersForAssignment()
{
	...
	if (!self()->isOutOfLineColdPath() && _extentOfLitPool < 0) {
		// ILGRA will call prepareRegistersForAssignment so when localRA calls it again lit pool will be set up
		// already
		...
		if (self()->comp()->getOption(TR_Randomize)) {
			if (randomizer.randomBoolean(300)
			        && performTransformation(self()->comp(),
			                "O^O Random Codegen - Added 6000 dummy slots to  Literal Pool frame to test  "
	...
	// We go from the end since in most cases, we want to test functionality of the first half i.e 0-15 for overlap
	// with FPR etc
		...
		if (instructionCursor->getOpCodeValue() == TR::InstOpCode::FENCE
			...
			for (int8_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1; i++) {
				if (self()->machine()
					...
					regMask |= (1 << (7 - ((i - 1) >> 1))); // bit 0 = GPR0/1, GPR0=1, GPR15=16.
					                                        // 'Or' with bit [(i-1)>>1]
			}
						...
						if (instructionCursor->getNode()->getBranchDestination() != NULL) {
							if (instructionCursor->getNode()
								...
								if ((instructionCursor->getNode()
								                    ...
								                    ->getBlock()
								            != NULL)
								        && (instructionCursor->getNode()->getBlock()
								                != NULL)) {
									// if branching to the same basic block then the
									// branch should be located inside ICF otherwise
									// it's potentially not a case where the branch
									// has to be inside Internal Control Flow
									if (instructionCursor->getNode()
									                ->getBranchDestination()
									                ->getNode()
									                ->getBlock()
									                ->getNumber()
									        == instructionCursor->getNode()
									                   ->getBlock()
									                   ->getNumber()) {
										TR_ASSERT(1,
										        "ASSERTION - Branch found "
										        "outside Internal Control Flow "
										        "that should be located inside "
...
OMR::Z::CodeGenerator::printStats(int32_t instCount)
{
	...
	fprintf(stderr,
	        ...
	        "[spills:%4.4d,xfers:%4.4d,moves:%4.4d] cold blocks with [spills:%4.4d,xfers:%4.4d,moves:%4.4d] for "
	        "%s\n",
	        self()->comp()->getHotnessName(self()->comp()->getMethodHotness()), (double)(totHot / instCount),
	        _totalHotSpills, _totalHotRegisterXfers, _totalHotRegisterMoves, _totalColdSpills,
	        _totalColdRegisterXfers, _totalColdRegisterMoves, self()->comp()->signature());
}
#endif
	...
	if (_stackOfMemoryReferencesCreatedDuringEvaluation.contains(mr)) {
		for (size_t i = 0; i < _stackOfMemoryReferencesCreatedDuringEvaluation.size(); i++) {
			if (_stackOfMemoryReferencesCreatedDuringEvaluation.element(i) == mr) {
				// note: this does not change the top of stack variable taken at the beginning of
				// evaluation:
				// ...
				// ...
				...
				// an evaluator so that a memory reference's registers can ostensibly "be killed"
				// earlier than they would be by this function.
				//
				// Any stack entries pushed after mr should not exist earlier in the stack because
				// memory references should not be allowed to escape their evaluators - reuse is not
				// possible. This means that there should be no way for remove(i) to delete something
				// before the top of stack before evaluation mark.
...
OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(int32_t topOfMemRefStackBeforeEvaluation)
{
	while (_stackOfMemoryReferencesCreatedDuringEvaluation.topIndex() > topOfMemRefStackBeforeEvaluation) {
		...
		// 2) AddFoldedMemRefToStack is only defined for OMR::Z::CodeGenerator, and requires TR::MemoryReference
		// instead of TR_MemoryReference.
		//
		...
		if (self()->comp()->getOption(TR_TraceCG)) {
			self()->comp()->getDebug()->trace(" _stackOfMemoryReferencesCreatedDuringEvaluation.pop() %p, "
...
OMR::Z::CodeGenerator::anyLitPoolSnippets()
{
	...
	for (auto constantiterator = _constantList.begin(); constantiterator != _constantList.end();
	        ++constantiterator) {
...
OMR::Z::CodeGenerator::doBinaryEncoding()
{
		...
		if (reg) {
			if (reg->getRegisterPair()) {
				...
				if (instr->getOpCodeValue() == TR::InstOpCode::LM
					...
					uint8_t numRegs = (lowReg->getRegisterNumber() - highReg->getRegisterNumber())
					        - 1;
					if (numRegs > 0) {
						for (uint8_t i = highReg->getRegisterNumber() + 1; i++; i <= numRegs) {
							self()->getS390Linkage()
							        ->getRealRegister(REGNUM(i))
							        ->setModified(true);
						}
					}
				}
	...
	while (data.cursorInstruction) {
		...
		if (data.cursorInstruction->getOpCodeValue() == TR::InstOpCode::RET) {
			...
			if (skipOneReturn == false) {
				...
				/* inserted before TR::InstOpCode::RET.  If no epilog is generated, TR::InstOpCode::RET
				 * will stay               */
				if (data.cursorInstruction->getOpCodeValue() != TR::InstOpCode::RET)
					skipOneReturn = true;
			} else {
	...
	while (data.cursorInstruction) {
		...
		if (isPrivateLinkage && data.cursorInstruction == data.jitTojitStart) {
			...
			// first word is the code size of intrepeter-to-jit glue in bytes and the second word is the
			// return info
	...
	if (!self()->comp()->getOption(TR_DisableOOL)) {
		...
		while (oiIterator != self()->getS390OutOfLineCodeSectionList().end()) {
			...
			if (needsETE && block && !block->getExceptionSuccessors().empty()) {
				uint32_t startOffset = (*oiIterator)->getFirstInstruction()->getBinaryEncoding()
				        - self()->getCodeStart();
				uint32_t endOffset = (*oiIterator)->getAppendInstruction()->getBinaryEncoding()
				        - self()->getCodeStart();
	...
	if (node) {
		...
		if (node->getOpaquePseudoRegister()) {
			...
			if (self()->traceBCDCodeGen())
				traceMsg(self()->comp(),
				        "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - "
		...
		if (node->safeToDoRecursiveDecrement()) {
			...
			// i.e. a direct load that has no future refs or a indirect load with no future refs that itself
			// has a child that is safeToRecDec
			self()->recursivelyDecReferenceCount(node);
		} else {
			self()->evaluate(node);
			...
			if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
				...
				if (self()->traceBCDCodeGen())
					traceMsg(self()->comp(),
					        "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) "
					        "reg %s - handle extra ref to addr child (ref is node based %s - %s "
					        "%p)\n",
...
{
	uint8_t numDeps = 8 + extraDeps; // TODO: only 6 deps are created below so why is there is always an implicit 2
	                                 // extra deps created?
...
OMR::Z::CodeGenerator::ssrClobberEvaluate(TR::Node *node, TR::MemoryReference *sourceMR) // sourceMR can be NULL
{
	...
	if (self()->traceBCDCodeGen())
		traceMsg(self()->comp(),
		        "\tneedsClobberEval : %s (%p), storageRef #%d (%p) (isBCD = %s, isReadOnlyTemp = %s, "
		        "tempRefCount = %d)\n",
	...
	if (node->getReferenceCount() > 1) {
		...
		if (self()->traceBCDCodeGen())
			traceMsg(self()->comp(),
			        "\t\t-->needsClobber=true due to reuse of readOnlyTemp %s with refCount %d > 1\n",
			        self()->getDebug()->getName(srcStorageReference->getSymbol()),
			        srcStorageReference->getTemporaryReferenceCount());
		// all these needsClobberDueToReuse cases should now be caught by the more general
		// getOwningRegisterCount() > 1 above
	...
	if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
		TR_ASSERT(srcRegister->isInitialized(),
		        "expecting srcRegister to be initialized in bcdClobberEvaluate\n");
		// if the srcStorageReference is not a temp or a hint then the recursive dec in setStorageReference()
		// will be wrong
		TR_ASSERT(srcStorageReference->isTemporaryBased() || srcStorageReference->isNodeBasedHint(),
		        "expecting the srcStorageReference to be either a temporary or a node based hint in "
		        "bcdClobberEvaluate\n");
		if (self()->traceBCDCodeGen())
			traceMsg(self()->comp(),
			        "ssrClobberEvaluate: node %s (%p) with srcReg %s (srcRegSize %d, srcReg->validSymSize "
			        ...
			        srcRegister->getSize(), srcRegister->getValidSymbolSize(),
			        srcRegister->getLiveSymbolSize(), srcStorageReference->getReferenceNumber(),
			        self()->getDebug()->getName(srcStorageReference->getSymbol()),
		...
		if (srcStorageReference->isReadOnlyTemporary() && srcRegister->getRightAlignedIgnoredBytes() > 0) {
			// The MVC must copy over the entire source field including any ignored (but still valid) bytes
			// (but dead bytes are not included as these have been invalidated/clobbered). The subsequent
			// uses of this copied field (via copyStorageReference) will adjust based on the owning
			// registers' ignoredBytes/deadBytes setting
			int32_t regIgnoredBytes = srcRegister->getRightAlignedIgnoredBytes();
			if (self()->traceBCDCodeGen())
				traceMsg(self()->comp(),
				        "\tremove %d ignoredBytes from sourceMR->_offset (%d->%d) for readOnlyTemp #%d "
				        "(%s)\n",
		...
		// the srcRegister->setStorageReference call will set the temp refCount to the final correct value based
		// on the node's refCount
		copyStorageReference->setTemporaryReferenceCount(0);
		TR_OpaquePseudoRegister *targetRegister = bcdSrcRegister
		        ? self()->allocatePseudoRegister(bcdSrcRegister)
		        : self()->allocateOpaquePseudoRegister(srcRegister);
		targetRegister->setStorageReference(srcStorageReference, NULL);

		int32_t digitsInSrc = isBCD ? TR::DataType::getBCDPrecisionFromSize(node->getDataType(), byteLength)
		                            : 0;
		...
		if (self()->traceBCDCodeGen())
			traceMsg(self()->comp(), "\tupdate srcRegister %s with copy ref #%d (%s)\n",
			        ...
			        self()->getDebug()->getName(copyStorageReference->getSymbol()));
		srcRegister->setStorageReference(copyStorageReference, node); // clears leftAlignedZeroDigits so query
		                                                              // getDigitsToClear above
		if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
			traceMsg(self()->comp(), "\tsrcRegister %s had leftAlignedZeroDigits %d on original ref #%d\n",
			        ...
			        srcStorageReference->getReferenceNumber());
			traceMsg(self()->comp(),
			        "\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s with new copy ref "
			        "#%d (digitsInSrc - digitsToClear = %d - %d = %d)\n",
			        alreadyClearedDigits, self()->getDebug()->getName(srcRegister),
			        copyStorageReference->getReferenceNumber(), digitsInSrc, digitsToClear,
		...
		if (srcStorageReference->getNodesToUpdateOnClobber()) {
			...
			//
			//       AP    t1,t2 (pdadd)
			//       ZAP   t1,t1 mark t1 as readOnly, add pdcleanA (w/regA) to list (pdcleanA), increment t1
			//       refCount by 2 (pdaddA nodeRefCount) nop         mark t1 as readOnly (redundant), add
			//       pdcleanB (w/regB) to list (pdcleanB), increment t1 refCount by 3 (pdcleanA
			//       nodeRefCount) UNPK  t3,t1 non-clobber operation, t1 is still alive and readOnly (pd2zd)
			//    ...
			//    pd2zd
			//       =>pdcleanA
			//
			...
			//       =>pdcleanB regB
			//
			//       MVC   t4,t1  clobber eval for pdadd parent, regB has other use so updated above, go
			//       through list and update regA with t4 (on clobber eval for pdaddB) AP    t1,tx
			//    ...
			...
			for (TR::Node *listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
				...
				if (listReg != srcRegister) {
					if (listReg->getStorageReference() != srcStorageReference) {
						...
						//
						// ipdstore p=29 symA (store then does a ZAP and updates child VTS_2 to
						// symA)
						//    aiuadd
						//       aload
						//       iconst
						...
						// ipdstore p=30
						//    addr
						//    pdModPrecB   p=30 (non-passThrough, needs NI, so needs clobberEval
						//    with copy ref VTS_3 and goes to update pdModPrecA symA -> VTS_3)
						//       =>pdaddOverflowMessage p=31 (VTS_2)
						//
						// prevent the symA -> VTS_3 update as symA != the orig VTS_2
						// if the update is done then the last pdModPrecA ref is now using VTS_3
						// and the final decReferenceCount of the symA aload is never done (live
						// reg alive bug)
						//
						if (self()->traceBCDCodeGen())
							traceMsg(self()->comp(),
							        "\ty^y : skip update on reg %s node %s (%p), "
							        "storageRef has already changed -- was #%d (%s), now "
							        "#%d (%s)\n",
						...
						if (listNode->getReferenceCount() > 0)
							listReg->setStorageReference(copyStorageReference,
							        listNode); // TODO: safe to transfer
							                   // alreadyClearedDigits in this case too?
					}
				}
			}
...
bool
OMR::Z::CodeGenerator::useMVCLForMemcpyWithPad(TR::Node *node, TR_MemCpyPadTypes type)
{
	...
	if (maxLen != 0 && maxLen < MVCL_THRESHOLD) // 0 is an unknown maxLength so have to be conservative and not use
	                                            // MVCL
	{
		useMVCL = true;
...
OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label, bool isCheckDisp)
{
	...
	if (s != NULL
		...
		TR_ASSERT(
		        0, "apply12BitLabelRelativeRelocation -- Unexpected use of 12bit rellocation by [%p]\n", label);
	}
}
        ...
        int32_t *cursor, TR::LabelSymbol *label, int8_t addressDifferenceDivisor, bool isInstrOffset)
{
	if (isInstrOffset) {
		...
		if ((*(uint16_t *)label_cursor) == bos(0x1800))
			label_cursor += 1;
		*(int16_t *)cursor =
		        (int16_t)(((intptrj_t)label_cursor - (((intptrj_t)cursor) - addressDifferenceDivisor)) / 2);
	} else
		*(int16_t *)cursor = (int16_t)(
		        ((intptrj_t)label->getCodeLocation() - (((intptrj_t)cursor) - addressDifferenceDivisor))
...
{
	*(int32_t *)(((uint8_t *)cursor) + 2) =
	        (int32_t)(((intptrj_t)(label->getCodeLocation()) - ((intptrj_t)cursor)) / 2);
}
...
OMR::Z::CodeGenerator::findS390OutOfLineCodeSectionFromLabel(TR::LabelSymbol *label)
{
	auto oiIterator = self()->getS390OutOfLineCodeSectionList().begin();
	while (oiIterator != self()->getS390OutOfLineCodeSectionList().end()) {
...
OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node *node, void *c, uint16_t size)
{
	...
	TR::SymbolReference *symRef = (node && node->getOpCode().hasSymbolReference()) ? node->getSymbolReference()
	                                                                               : NULL;
...
OMR::Z::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
	...
	TR::Compilation *comp = self()->comp();
	switch (lc) {
	case TR_Helper: linkage = new (self()->trHeapMemory()) TR::S390zLinuxSystemLinkage(self()); break;
...
OMR::Z::CodeGenerator::createLiteralPoolSnippet(TR::Node *node)
{
	...
	case TR::DecimalDouble:
		...
		break;
	default: TR_ASSERT(0, "createLiteralPoolSnippet: Unknown type.\n"); break;
	}
 ...
 */
bool
OMR::Z::CodeGenerator::isActiveCompareCC(TR::InstOpCode::Mnemonic opcd, TR::Register *tReg, TR::Register *sReg)
{
 ...
 */
bool
OMR::Z::CodeGenerator::signExtendedHighOrderBits(
        TR::Node *node, TR::Register *targetRegister, TR::Register *srcRegister, uint32_t numberOfBits)
{
	switch (numberOfBits) {
	case 16: generateRRInstruction(self(), TR::InstOpCode::LHR, node, targetRegister, targetRegister); break;
	...
	default: TR_ASSERT(0, "No appropriate instruction to use for sign extend this number of bits!\n"); break;
	}
 ...
 */
bool
OMR::Z::CodeGenerator::clearHighOrderBits(
        TR::Node *node, TR::Register *targetRegister, TR::Register *srcRegister, uint32_t numberOfBits)
{
	switch (numberOfBits) {
	case 8:
	...
	default: TR_ASSERT(0, "No appropriate instruction to use for clearing this number of bits!\n"); break;
	}
        ...
        TR_Array<TR_BitVector> &liveOnEntryUsage, TR_Array<TR_BitVector> &liveOnExitUsage)
{
	...
	do {
		...
		while (bvi.hasMoreElements()) {
			...
			int32_t globalRegNum =
			        self()->machine()->getGlobalReg((TR::RealRegister::RegNum)(regIndex + 1));
        ...
        TR_RegisterCandidate *rc, TR::Block **blocks, TR_BitVector &availableRegisters)
{
	...
	while (loe.hasMoreElements()) {
		...
		if (lastTreeTopNode->getNumChildren() > 0
		        ...
		        && lastTreeTopNode->getFirstChild()->getOpCode().isJumpWithMultipleTargets()) // ensure calls
		                                                                                      // that have
		                                                                                      // multiple
		                                                                                      // targets remove
		                                                                                      // all volatile
...
void
OMR::Z::CodeGenerator::genMemCpy(TR::MemoryReference *targetMR, TR::Node *targetNode, TR::MemoryReference *sourceMR,
 ...
 */
void
OMR::Z::CodeGenerator::genCopyFromLiteralPool(TR::Node *node, int32_t bytesToCopy, TR::MemoryReference *targetMR,
...
TR::Instruction *
OMR::Z::CodeGenerator::genLoadAddressToRegister(
        TR::Register *reg, TR::MemoryReference *origMR, TR::Node *node, TR::Instruction *preced)
{
	...
	if (offset >= TR_MIN_RX_DISP && offset <= TR_MAX_RX_DISP) {
		...
		preced = generateS390ImmOp(
		        cg, TR::InstOpCode::getAddOpCode(), node, reg, reg, (int64_t)offset, NULL, NULL);
	}
	return preced;
}
...
OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node *node)
{
	...
	if (node->getOpCode().isPackedRightShift()) {
		// The most efficient way to perform a packed decimal right shift by a non-zero even amount when the
		// round amount is also zero is to use an MVN instruction. However since this instruction will perform
		// the 'shift' by moving the sign code to the left there are right aligned dead bytes and implied left
		// aligned zero bytes introduced. These dead and implied bytes cannot be within a user variable as the
		// actual full value must be materialized therefore require a temporary to be used for these even right
		// shifts.
		//
...
OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node *node)
{
	...
	if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
		...
		if (node->getRegister() == NULL && endHint) {
			// do not have to fully descend a tree looking for overlapping operations in the special (and
			// actually quite common) cases where a node will initialize to a new storageReference
			// (endHintOnOperation check) and one of the conditions below is true 1) a conversion between
			// types that use storageRefs (BCD and Aggr) and types that don't (e.g. int,float) -- i2pd,i2o
			// etc. It is always safe to end the search
			//    because one side does not use storagRefs (no chance for overlap) and the other will always
			//    get a new storageRef.
			// 2) a conversion between types that both use storageRefs AND the evaluator has been fixed up
			// to detect the overlap conditions and privatize to a new temp (pd2zd,ud2pd etc)
			//
			//    zdstore "a"
			//       pd2zd
			//          pdload "b"
			...
			// endAccumulatorSearchOnOperation will return true to allow the pd2zd to UNPK (accumulate)
			// right to the store location "a" as the pd2zd will check and deal with any "a" and "b" overlap
			// if any exists.
			//

			if (node->getOpCode().isConversion()
			                && (self()->isStorageReferenceType(node)
			                        && !self()->isStorageReferenceType(node->getFirstChild()))
			        || // i2pd
			        (!self()->isStorageReferenceType(node)
			                && self()->isStorageReferenceType(node->getFirstChild()))) // pd2i
			{
				...
				// case2: evaluators below have been fixed up to privatize only if needed during
				// instruction selection (privatizeBCDRegisterIfNeeded)
...
#ifdef J9_PROJECT_SPECIFIC
	else if (node->getOpCode().isPackedLeftShift() && isOdd(node->getDecimalAdjust())) // odd left shifts need a new
	                                                                                   // temp
	{
		// odd left shifts may use an MVO and to ensure that the operands don't overlap a new temp is used
		// return false here to avoid a child of the shift from getting a doomed store hint (one that will have
		// to be moved to another new temp before the actual store is completed). pdstore <a>
		//    pdshl    <- end hint after (below) this node
		//       pdadd <- if pdadd is tagged with store hint <a> then it will need to be copied to a new temp
		//       *and* the pdshl cannot use <a> either iconst 3 (some oddShiftAmount)
		return true;
	}
#endif
	else {
...
bool
isMatchingStoreRestore(TR::Instruction *cursorLoad, TR::Instruction *cursorStore, TR::CodeGenerator *cg)
{
	return false;
}
...
getRegMaskFromRange(TR::Instruction *inst)
{
	...
	TR_ASSERT(
	        lowRegNum >= 0 && lowRegNum <= 15 && highRegNum >= 0 && highRegNum <= 15, "Unexpected register number");
...
handleLoadWithRegRanges(TR::Instruction *inst, TR::CodeGenerator *cg)
{
	...
	if (!(opCodeToUse == TR::InstOpCode::LM || opCodeToUse == TR::InstOpCode::LMG
	            || opCodeToUse == TR::InstOpCode::LMH || opCodeToUse == TR::InstOpCode::LMY
	...
	TR::Register *lowReg = isVector ? toS390VRSInstruction(inst)->getFirstRegister()
	                                : toS390RSInstruction(inst)->getFirstRegister();
	TR::Register *highReg = isVector ? toS390VRSInstruction(inst)->getSecondRegister()
	...
	for (uint32_t i = lowRegNum; i != highRegNum; i = ((i == (isVector ? numVRFs - 1 : 15)) ? 0 : i + 1)) {
		...
		// Since we are called post RA for the LM, we check if assigned registers are last used on the LM so we
		// can avoid needlessly spilling them. (ex. LM(GPR00F,GPR14P,PSALCCAV->LCCAEMS0); ! PSALCCAV needs a reg
		// and only needed on LM)
		...
		if (reg->getState() == TR::RealRegister::Assigned && inst != virtualReg->getEndOfRange()) {
			...
			// register is defined before it was stored, spill or shuffle it to a free reg before load
			// clobbers it start of range will be NULL in some cases, we are cautious and spill (def#
			// 100246)
			if (!found) {
				cg->traceRegisterAssignment(
				        "trying to free %R for killed reg %R by loadmultiple \n", virtualReg, reg);
				TR::RealRegister *shuffle =
				        cg->machine()->shuffleOrSpillRegister(inst, virtualReg, availForShuffle);
				if (shuffle != NULL) {
					// if the LM instruction uses this reg we just shuffled, it will have the wrong
					// real reg, need to fix this up
...
TR::Instruction *
OMR::Z::CodeGenerator::generateDebugCounterBump(TR::Instruction *cursor, TR::DebugCounterBase *counter,
...
bool
OMR::Z::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode, TR::DataType dt)
{
...
OMR::Z::CodeGenerator::possiblyConflictingNode(TR::Node *node)
{
	...
	if (node->getReferenceCount() > 1) {
		if (node->getRegister() == NULL && node->getOpCode().canHaveStorageReferenceHint()) {
			if (self()->traceBCDCodeGen())
				traceMsg(self()->comp(),
				        "\t\t\tpossiblyConflicting=true %s (%p) : reg==NULL and storageRefHint_Case, "
				        "refCount %d\n",
			...
			if (self()->traceBCDCodeGen())
				traceMsg(self()->comp(),
				        "\t\t\tpossiblyConflicting=true %s (%p) : BCDOrAggrType_Case, reg=%s, refCount "
				        "%d\n",
				        node->getOpCode().getName(), node,
...
OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node *node1, TR::Node *node2)
{
	...
	if (node1->getSize() != node2->getSize()) {
		if (self()->comp()->getOption(TR_TraceVIP))
			traceMsg(self()->comp(),
			        "\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d and "
			        "node2 %s (%p) size = %d\n",
			        node1->getOpCode().getName(), node1, node1->getSize(), node2->getOpCode().getName(),
			        node2, node2->getSize());
		return false;
	}
...
{
	TR::ILOpCode op = node->getOpCode();
	if (op.isBCDStore() && !node->chkCleanSignInPDStoreEvaluator()) // most common case of needing so put up-front
	                                                                // -- the value escapes
		return true;
...
bool
OMR::Z::CodeGenerator::loadAndStoreMayOverlap(TR::Node *store, size_t storeSize, TR::Node *load, size_t loadSize)
{
...
{
	// The pattern below leads to use index reg in addressing, LAY for example, that costs extra for instructions
	// like MVC.
	//   aiadd
	//     load-addr
	//     laod-non-const
	...
	if (loadNode->getOpCode().isIndirect() && loadNode->getFirstChild()->getNumChildren() > 1
		...
		return false;
	} else if (loadNode->getOpCode().isLoadConst() && loadNode->getReferenceCount() == 1) { // The const may be
		                                                                                // loaded into reg and
		                                                                                // shared. So dont do
		                                                                                // it.
		return true;
...
OMR::Z::CodeGenerator::getStorageDestructiveOverlapInfo(
        TR::Node *src, size_t srcLength, TR::Node *dst, size_t dstLength)
{
	...
	if (srcLength == 1 && dstLength == 1) {
		...
		case TR_NoOverlap: overlapInfo = TR_DefinitelyNoDestructiveOverlap; break;
		case TR_DestructiveOverlap: overlapInfo = TR_DefinitelyDestructiveOverlap; break;
		case TR_MayOverlap:
...
OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node *node, vcount_t visitCount)
{
	...
	// Make sure that the only uses of the add/sub node, so far, are the three we expect: under the iRegStore, under
	// the compare, and in the GLRegDeps under the compare. futureUseCount should've been initialized to be
	// referenceCount - 1 (which will have to be an iRegStore in order for us to see the add node under GLRegDeps
	// here). The compare node here will account for the other two uses, so as long as there's no other uses between
	// the iRegStore and the ificmpgt, all is well.
