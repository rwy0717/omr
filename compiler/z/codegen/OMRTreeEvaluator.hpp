...
public:
	 ...
	 *
	 *    \param shiftAmount
	 *      The number of bits we should shift the result of the logical AND. A positive value represents a left
	 * shift, a negative value represents a right shift, and a value of zero represents no shift. Note that if the
	 * absolute value of number is greater than 6 bits, then this optimization will not work since the RISBG
	 * instruction will only hold 6 bits for the shift operand.
	 *
	 *   \param isSignedShift
	...
	static void tableEvaluatorCaseLabelHelper(TR::Node *node, TR::CodeGenerator *cg,
	...
	static TR::Register *checkAndAllocateReferenceRegister(
	 ...
	 *
	 *     The z Systems code generator requires that <c>isForwardArrayCopy()</c> really means that a forward array
	 * copy is safe and that if it is not set it really means that a backward array copy is safe. The optimizer
	 * should have generated the correct tests around the arraycopy to enforce this assumption.
	 *
	 *     Furthermore note that the z Systems code generator does not need to check for a particular frequency on
	 * the length, again because the optimizer will have generated a test for a particular length, if one is quite
	...
	/** \brief
	 *     Evaluates a primitive arraycopy node by generating an MVC memory-memory copy for a forward arraycopy and
	 a
	 ...
	 *
	 */
	static void generateLoadAndStoreForArrayCopy(TR::Node *node, TR::CodeGenerator *cg,
	        TR::MemoryReference *srcMemRef, TR::MemoryReference *dstMemRef, TR_S390ScratchRegisterManager *srm,
	        TR::DataType elenmentType, bool needsGuardedLoad, TR::RegisterDependencyConditions *deps = NULL);
	...
	static TR::RegisterDependencyConditions *generateMemToMemElementCopy(TR::Node *node, TR::CodeGenerator *cg,
	        ...
	        bool genStartICFLabel = false, TR::RegisterDependencyConditions *deps = NULL);
	 ...
	 *     Register depdendecy conditions containg registers allocated within Internal Control Flow
	 */
	static TR::RegisterDependencyConditions *backwardArrayCopySequenceGenerator(TR::Node *node,
	...
	static TR::Register *arraycmpHelper(TR::Node *node, TR::CodeGenerator *cg, bool isWideChar,
	        bool isEqualCmp = false, int32_t cmpValue = 0, TR::LabelSymbol *compareTarget = NULL,
...
private:
	...
	/** \brief
	 *     Inlines an intrinsic for calls to atomicFetchAndAddSymbol which are represented by a call node of the
	 * form for 32-bit (64-bit similar):
	 *
	 *     \code
