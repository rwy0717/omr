...
TR::S390zOSSystemLinkage::createPrologue(TR::Instruction *cursor)
{
	...
	// DSA size is the frame size aligned to 32-bytes which means it's least significant 5 bits are zero and are
	// used to represent the flags which are always 0 for OMR as we do not support leaf frames or direct calls to
	// alloca()
 ...
 */
TR::Register *
TR::S390zOSSystemLinkage::callNativeFunction(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
...
void
TR::S390zOSSystemLinkage::generateInstructionsForCall(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
        intptrj_t targetAddress, TR::Register *methodAddressReg, TR::Register *javaLitOffsetReg,
        TR::LabelSymbol *returnFromJNICallLabel, TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint)
{
	TR::CodeGenerator *codeGen = cg();
	...
	if (callNode->getOpCode().isIndirect()) {
		generateRRInstruction(cg(), InstOpCode::BASR, callNode, systemReturnAddressRegister,
		        systemEntryPointRegister, preDeps);
...
TR::Instruction *
TR::S390zOSSystemLinkage::genCallNOPAndDescriptor(
        TR::Instruction *cursor, TR::Node *node, TR::Node *callNode, TR_XPLinkCallTypes callType)
{
	if (TR::Compiler->target.is32Bit()) {
		uint32_t callDescriptorValue =
		        TR::XPLINKCallDescriptorSnippet::generateCallDescriptorValue(this, callNode);
		...
		cg()->addRelocation(new (cg()->trHeapMemory())
		                XPLINKCallDescriptorRelocation(cursor, callDescriptor->getSnippetLabel()));
	} else {
		// TODO: Once we support generic TR::InstOpCode::DC of any size we need to modify this line to use it,
		// similarly to what we do above for the 31-bit case.
...
TR::Instruction *
TR::S390zOSSystemLinkage::addImmediateToRealRegister(TR::RealRegister *targetReg, int32_t value,
        TR::RealRegister *tempReg, TR::Node *node, TR::Instruction *cursor, bool *checkTempNeeded)
{
	...
	if (smallPositiveValue) {
		if (!checkTempNeeded)
			cursor = generateRXInstruction(cg(), TR::InstOpCode::LA, node, targetReg,
		...
		if (checkTempNeeded)
			*checkTempNeeded = true;
			...
			cursor = generateRRInstruction(
			        cg(), TR::InstOpCode::getAddRegOpCode(), node, targetReg, tempReg, cursor);
		}
	} else {
		if (!checkTempNeeded)
	...
	if (firstSaved >= 0) {
		...
		if (firstSaved == lastSaved) {
			...
			cursor = restorePreservedRegs(REGNUM(firstSaved + TR::RealRegister::FirstGPR),
			        REGNUM(lastSaved + TR::RealRegister::FirstGPR), blockNumber, cursor, currentNode, spReg,
			        rsa, getNormalStackPointerRegister());
		}
	}

	if (!(GPRSaveMask & (1 << GPREGINDEX(getNormalStackPointerRegister())))) { // GPR4 not restored by previous and
		                                                                   // needs restoring
...
TR::S390zOSSystemLinkage::spillGPRsInPrologue(TR::Node *node, TR::Instruction *cursor)
{
	...
	if ((frameType == TR_XPLinkStackCheckFrame)
	        || ((frameType == TR_XPLinkIntermediateFrame) && needAddTempReg)) { // GPR4 is saved but indirectly via
		                                                                    // GPR0 in stack check or medium
		                                                                    // case - so don't save GPR4 via STM
	...
	case TR_XPLinkIntermediateFrame:
		if (needAddTempReg) {
			...
			if (saveTempReg || needAddTempReg) {
				cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real,
				        spReg, cursor); // LR R0,R4
			}
			if (saveTempReg) {
				gpr3ParmRef = generateS390MemoryReference(spReg, gpr3ParmOffset, cg()); // used for
				                                                                        // temporary
				                                                                        // register
				cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), node, gpr3Real,
				        gpr3ParmRef, cursor); // ST R3,parmRef(R4)
			}
			...
			if (saveTempReg) {
				cursor = generateRRInstruction(
				        cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr3Real, gpr0Real, cursor);
				gpr3ParmRef = generateS390MemoryReference(gpr3Real, gpr3ParmOffset, cg());
				cursor = generateRXInstruction(
				        cg(), TR::InstOpCode::getLoadOpCode(), node, gpr3Real, gpr3ParmRef, cursor);
			}
			...
			if (needAddTempReg) // LR R0,R4
				cursor = generateRRInstruction(
				        cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, spReg, cursor);
	...
	case TR_XPLinkStackCheckFrame:
		...
		if (saveTempReg) {
			gpr3ParmRef = generateS390MemoryReference(spReg, gpr3ParmOffset, cg()); // used for temporary
			                                                                        // register
			cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), node, gpr3Real,
			        gpr3ParmRef, cursor); // ST R3,parmRef(R4)
		}
		...
		if (TR::Compiler->target.is64Bit()) {
			...
			cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), node, gpr3Real,
			        extenderRef, cursor); //  L R3,extender(R12)
		} else {
	...
	default: TR_ASSERT(0, "invalid xplink frame type"); break;
	}
...
TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::apply(TR::CodeGenerator *cg)
{
	...
	if (offsetToCallDescriptor < std::numeric_limits<int16_t>::min()
		...
		cg->comp()->failCompilation<TR::ExcessiveComplexity>("Unable to encode offset to XPLINK call "
		                                                     "descriptor");
	}
