...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isHCRGuard() && !node->isOSRGuard()
        && !(comp->performVirtualGuardNOPing() && comp->isVirtualGuardNOPingRequired(virtualGuard))
        && virtualGuard->canBeRemoved()) {
        return false;
    }

    if (node->getOpCodeValue() != TR::ificmpne && node->getOpCodeValue() != TR::iflcmpne
...
static TR::Register* generateS390lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic brOpCode,
    TR::InstOpCode::S390BranchCondition condCmpHighFalse, TR::InstOpCode::S390BranchCondition condCmpHighTrue,
    TR::InstOpCode::S390BranchCondition condCmpLowTrue, bool isBoolean)
{
    ...
    if (!isBoolean && numGlobalDeps < 8 && //  Needs to be tuned
        ...
        if (cg->canClobberNodesRegister(firstChild, 1, &data) && data.canClobberHighWord()) {
        ...
        && (firstChild->getOpCodeValue() == TR::land || firstChild->getOpCodeValue() == TR::lor
        ...
        tempLogicalOp.setInputs(firstLogicalOpChild, firstLogicalOpRegister, secondLogicalOpChild,
            secondLogicalOpRegister, false, false, cg->comp());
        ...
        tempLogicalOp.remapInputs(
            firstLogicalOpChild, firstLogicalOpRegister, secondLogicalOpChild, secondLogicalOpRegister);

        if (tempLogicalOp.getOpReg1Reg2()) {
            generateRRInstruction(cg, logicalRR, firstChild, firstLogicalOpRegister->getHighOrder(),
            ...
            generateRRInstruction(cg, logicalRR, firstChild, secondLogicalOpRegister->getHighOrder(),
            ...
            TR::RegisterPair* tempReg
                = cg->allocateConsecutiveRegisterPair(cg->allocateRegister(), cg->allocateRegister());

            generateRRInstruction(
                cg, TR::InstOpCode::LR, firstChild, tempReg->getHighOrder(), firstLogicalOpRegister->getHighOrder());
            generateRRInstruction(
                cg, TR::InstOpCode::LR, firstChild, tempReg->getLowOrder(), firstLogicalOpRegister->getLowOrder());

            generateRRInstruction(
                cg, logicalRR, firstChild, tempReg->getHighOrder(), secondLogicalOpRegister->getHighOrder());
            ...
            if (tempLogicalOp.getOpReg3Mem2() || tempLogicalOp.getOpReg1Mem2()) {
                ...
                if (tempLogicalOp.getOpReg3Mem2()) {
                    tempReg = cg->allocateConsecutiveRegisterPair(cg->allocateRegister(), cg->allocateRegister());
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        deps = temp.longOrderedCompareAndBranchAnalyser(node, brOpCode, condCmpLowTrue, condCmpHighTrue,
            condCmpHighFalse, trueTarget, falseTarget, internalControlFlowStarted);
    }
...
{
    TR_ASSERT(
        TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "lcmpEvaluator64() is for 64bit code-gen only!");
    TR::RegisterDependencyConditions* deps = NULL;
    TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
    TR::LabelSymbol* isTrue = generateLabelSymbol(cg);
...
TR::Register* lcmpHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
            ...
            // Set the CC using a comp.
            generateRRInstruction(
                cg, TR::InstOpCode::CR, node, src1RegPair->getHighOrder(), src2RegPair->getHighOrder());
        }
        // If LT we are done
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
        cFlowRegionStart->setStartInternalControlFlow();
...
TR::Register* OMR::Z::TreeEvaluator::ibranchEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return NULL; }
TR::Register* OMR::Z::TreeEvaluator::mbranchEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return NULL; }
...
TR::Register* OMR::Z::TreeEvaluator::returnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && returnValRegister != NULL
    ...
    case TR::iureturn:
        ...
        if (linkage->isNeedsWidening())
            new (cg->trHeapMemory())
                TR::S390RRInstruction(TR::InstOpCode::LLGFR, node, returnValRegister, returnValRegister, cg);
        break;
    case TR::lreturn:
    case TR::lureturn:
...
static inline void generateMergedGuardCodeIfNeeded(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
            ...
            if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq
                || node->getOpCodeValue() == TR::ifacmpeq || node->getOpCodeValue() == TR::iflcmpeq)
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_NOP,
            TR::InstOpCode::COND_BE, CMP4CONTROLFLOW);
    }
    generateMergedGuardCodeIfNeeded(node, cg);
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, TR::InstOpCode::COND_BNE,
            TR::InstOpCode::COND_BNE, CMP4CONTROLFLOW);
    }
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL,
            TR::InstOpCode::COND_BNH, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL,
            TR::InstOpCode::COND_BL, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH,
            TR::InstOpCode::COND_BNL, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH,
            TR::InstOpCode::COND_BH, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::iucmpeq
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
            ...
            if (node->getOpCodeValue() != TR::acmpeq && TR::Compiler->target.is64Bit()
                ...
                && performTransformation(
                       cg->comp(), "O^O CODE GENERATION:  ===>   Use RXSBG to perform icmpeq  <==\n")) {
...
TR::Register* OMR::Z::TreeEvaluator::lcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE,
            TR::InstOpCode::COND_NOP, TR::InstOpCode::COND_BE, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP,
            TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH,
            TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BNH, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH,
            TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BL, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL,
            TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BNL, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL,
            TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BH, CMP4BOOLEAN);
    }
}
...
TR::Register* OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    case AddressTable32bit: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CL, node, selectorReg, numBranchTableEntries,
                TR::InstOpCode::COND_BNL, node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), false,
    ...
    case AddressTable64bitLongLookup: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CLG, node, selectorReg, numBranchTableEntries,
                TR::InstOpCode::COND_BNL, node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), false,
                false);
        generateRSInstruction(cg, TR::InstOpCode::SLLG, node, selectorReg, selectorReg, 3);
        generateRRInstruction(cg, TR::InstOpCode::LGFR, node, selectorReg, selectorReg);
        ...
        if (node->getOpCodeValue() == TR::trtLookup) {
            ...
            if (type.isInt64()) {
                if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())
                    ...
                    generateS390ImmOp(cg, TR::InstOpCode::C, node, selectorReg->getHighOrder(),
                        selectorReg->getHighOrder(), (int32_t)((child->getCaseConstant() >> 32) & 0x00000000FFFFFFFFl));
                    generateS390BranchInstruction(
                        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, skipLoCompareLabel);
                    generateS390ImmOp(cg, TR::InstOpCode::C, node, selectorReg->getLowOrder(),
                        selectorReg->getLowOrder(), (int32_t)(child->getCaseConstant() & 0x00000000FFFFFFFFl));
                }
            } else
                    ...
                    if (nextTopNode) {
                        if (nextTopNode->getOpCode().isBndCheck()) {
                            if ((nextTopNode->getOpCode().isSpineCheck() && (nextTopNode->getChild(2) == n))
                                ...
                                traceMsg(comp, "\nMerging NULLCHK [%p] and BNDCHK [%p] of load child [%p]", node,
                                    nextTopNode, n);
                            }
    ...
    if (needExplicitCheck) {
        ...
        if (cg->getHasResumableTrapHandler()) {
            ...
            if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)
                && reference->getOpCode().isLoadVar() && reference->getRegister() == NULL) {
                targetRegister = cg->allocateCollectedReferenceRegister();
                appendTo = generateRXYInstruction(cg, TR::InstOpCode::getLoadAndTrapOpCode(), node, targetRegister,
                ...
                && reference->getRegister() == NULL && comp->useCompressedPointers()
                && reference->getOpCodeValue() == TR::l2a && reference->getFirstChild()->getOpCodeValue() == TR::iu2l
            ...
            if (!firstChild->getOpCode().isCall() && reference->getOpCode().isLoadVar()
                ...
                appendTo = generateRXYInstruction(
                    cg, TR::InstOpCode::getLoadTestOpCode(), reference, targetRegister, tempMR, appendTo);
                tempMR->stopUsingMemRefRegister(cg);
...
{
    // Sanity check
    TR_ASSERT(cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10),
        "ternary IL only supported on z10 and up");
    ...
    if (comp->getOption(TR_TraceCG))
        traceMsg(comp,
            "Starting evaluation of ternary node %p condition %p (in reg %p) trueVal %p (in reg %p) falseVal %p (in "
            "reg %p)\n",
    ...
    if (condition->getOpCode().isBooleanCompare()
        ...
        TR::InstOpCode::S390BranchCondition bc
            = TR::TreeEvaluator::getBranchConditionFromCompareOpCode(condition->getOpCodeValue());
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
            ...
            if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
                ...
                generateRRFInstruction(cg, mnemonic, node, trueReg->getHighOrder(), falseReg->getHighOrder(),
                    getMaskForBranchCondition(TR::InstOpCode::COND_BER), true);
                generateRRFInstruction(cg, mnemonic, node, trueReg->getLowOrder(), falseReg->getLowOrder(),
                    getMaskForBranchCondition(TR::InstOpCode::COND_BER), true);
            } else {
                generateRRFInstruction(cg,
                    trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR, node, trueReg,
                    falseReg, getMaskForBranchCondition(TR::TreeEvaluator::mapBranchConditionToLOCRCondition(bc)),
                    true);
            }
        } else {
            TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
            ...
            if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
                ...
                generateRRInstruction(cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LGR : TR::InstOpCode::LR,
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
            if (comp->getOption(TR_TraceCG)) {
                traceMsg(comp, "Emmitting a LOCR with resulReg = %p falseReg->getRegister = %p\n", trueReg,
                    falseReg->getRegister());
            }

            if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
                // Sanity check
                TR_ASSERT(trueReg->getRegisterPair() && falseReg->getRegisterPair(),
                    "Both false and true registers should be register pairs");

                auto mnemonic = trueReg->getKind() == TR_GPR64 || cg->use64BitRegsOn32Bit() ? TR::InstOpCode::LOCGR
                                                                                            : TR::InstOpCode::LOCR;
                ...
                generateRRFInstruction(cg, mnemonic, node, trueReg, falseReg->getRegister(),
                    getMaskForBranchCondition(TR::InstOpCode::COND_BER), true);
            }
        } else {
            TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
...
TR::Register* OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (conditionNode->getOpCode().isCompareDouble()) {
        ...
        // To handle unordered compares, we switch the compare operation, since the vector compare instructions return
        // false for unordered. For example, if the original compare is dcmpgeu:
        ...
        // The above operation returns 0's for not equals, so we must swap the true and false values to select the
        // correct "true value" for cmpne operations Swap the values for unordered comparisons also (see above)
        if (ifOp == TR::dcmpne || conditionNode->getOpCode().isCompareTrueIfUnordered())
            generateVRReInstruction(
                cg, TR::InstOpCode::VSEL, node, returnReg, falseValueReg, trueValueReg, vectorSelReg);
        else
            generateVRReInstruction(
                cg, TR::InstOpCode::VSEL, node, returnReg, trueValueReg, falseValueReg, vectorSelReg);
 ...
 */
TR::Instruction* generateAlwaysTrapSequence(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions** retDeps)
{
    // Sanity check
    TR_ASSERT(cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10),
        "Compare and trap instructions only supported on z10 and up");
    ...
    TR::RegisterDependencyConditions* regDeps = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 1, cg);
    regDeps->addPostCondition(zeroReg, TR::RealRegister::AssignAny);
    cursor = new (cg->trHeapMemory()) TR::S390RRFInstruction(
        TR::InstOpCode::CLRT, node, zeroReg, zeroReg, getMaskForBranchCondition(TR::InstOpCode::COND_BER), true, cg);
