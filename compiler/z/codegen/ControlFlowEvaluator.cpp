...
virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((!node->isNopableInlineGuard() && !node->isHCRGuard() && !node->isOSRGuard()) || !cg->getSupportsVirtualGuardNOPing()) {
        return false;
    }

    TR_VirtualGuard* virtualGuard = comp->findVirtualGuardInfo(node);
    if (!node->isHCRGuard() && !node->isOSRGuard() && !(comp->performVirtualGuardNOPing() && comp->isVirtualGuardNOPingRequired(virtualGuard)) && virtualGuard->canBeRemoved()) {
        return false;
    }
    ...
    TR::InstOpCode::S390BranchCondition condCmpLowTrue, bool isBoolean)
{
    ...
    if (!isBoolean && numGlobalDeps < 8 && //  Needs to be tuned
        (node->getOpCodeValue() == TR::iflcmpne || node->getOpCodeValue() == TR::iflcmpeq || node->getOpCodeValue() == TR::iflucmpne || node->getOpCodeValue() == TR::iflucmpeq) && secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0) {
        ...
        (node->getOpCodeValue() == TR::iflcmpne || node->getOpCodeValue() == TR::iflcmpeq || node->getOpCodeValue() == TR::iflucmpne || node->getOpCodeValue() == TR::iflucmpeq) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0 && secondChild->getRegister() == NULL && (firstChild->getOpCodeValue() == TR::land || firstChild->getOpCodeValue() == TR::lor || firstChild->getOpCodeValue() == TR::lxor)) {
        splitLogicalOp = true;
...
static inline void generateMergedGuardCodeIfNeeded(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
            ...
            if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq || node->getOpCodeValue() == TR::ifacmpeq || node->getOpCodeValue() == TR::iflcmpeq)
...
OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq || node->getOpCodeValue() == TR::ifacmpeq) {
...
OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::iucmpeq || node->getOpCodeValue() == TR::acmpeq) {
...
TR::Register* OMR::Z::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(TR::Node* node, bool needsResolve, TR::CodeGenerator* cg)
{
    ...
    if (needExplicitCheck) {
        ...
        if (cg->getHasResumableTrapHandler()) {
            ...
            // i.e.  o.f == NULL
            if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12) && reference->getOpCode().isLoadVar() && reference->getRegister() == NULL) {
            ...
            } else if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12) && reference->getRegister() == NULL && comp->useCompressedPointers() && reference->getOpCodeValue() == TR::l2a && reference->getFirstChild()->getOpCodeValue() == TR::iu2l && reference->getFirstChild()->getFirstChild()->getOpCode().isLoadVar() && TR::Compiler->om.compressedReferenceShiftOffset() == 0) {
            ...
            if (!firstChild->getOpCode().isCall() && reference->getOpCode().isLoadVar() && reference->getOpCode().hasSymbolReference() && reference->getRegister() == NULL) {
