...
void
killRegisterIfNotLocked(TR::CodeGenerator *cg, TR::RealRegister::RegNum reg, TR::Instruction *instr,
...
static TR::Register *
generateS390lcmpEvaluator64(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic brOp,
        TR::InstOpCode::S390BranchCondition brCond, bool isBoolean)
{
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
		...
		generateS390ImmOp(cg, isUnsigned ? TR::InstOpCode::CLG : TR::InstOpCode::CG, node, cmpRegister,
		        cmpRegister, long_value);
		...
		generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
		generateS390CompareAndBranchInstruction(cg, isUnsigned ? TR::InstOpCode::CLGR : TR::InstOpCode::CGR,
		        node, cmpRegister, srcReg, brCond, isTrue, false, false);
	}
...
OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->getOpCodeValue() == TR::fcmpg || node->getOpCodeValue() == TR::dcmpg) {
		generateS390BranchInstruction(
		        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK1, node, cFlowRegionEnd);
	}
...
generateS390Compare(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic branchOp,
        TR::InstOpCode::S390BranchCondition fBranchOpCond, TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
	// Generate compare code, find out if ops were reversed
	...
	if (node->getOpCodeValue() == TR::imax || node->getOpCodeValue() == TR::imin) {
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
			...
			generateS390CompareAndBranchInstruction(
			        cg, TR::InstOpCode::CR, node, lhsReg, rhsReg, bc, cFlowRegionEnd, false);
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
			...
			generateS390CompareAndBranchInstruction(
			        cg, TR::InstOpCode::CGR, node, lhsReg, rhsReg, bc, cFlowRegionEnd, false);
...
OMR::Z::TreeEvaluator::gotoEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->getNumChildren() > 0) {
		...
		generateS390BranchInstruction(
		        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, temp->getLabel());
	}
...
OMR::Z::TreeEvaluator::returnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::dereturn:
		...
		dependencies->addPostCondition(
		        returnValRegister->getHighOrder(), linkage->getLongDoubleReturnRegister0());
		dependencies->addPostCondition(
		        returnValRegister->getLowOrder(), linkage->getLongDoubleReturnRegister2());
		break;
#endif
	case TR::Return: comp->setReturnInfo(TR_VoidReturn); break;
	}
...
bool
OMR::Z::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(TR::Node *node)
{
	...
	if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
		...
		if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
			...
			traceMsg(comp, "generateMergedGuardCodeIfNeeded for %s %s\n",
			        comp->getDebug() ? comp->getDebug()->getVirtualGuardKindName(virtualGuard->getKind())
			                         : "???Guard",
 ...
 */
TR::Register *
OMR::Z::TreeEvaluator::ifFoldingHelper(TR::Node *node, TR::CodeGenerator *cg, bool &handledBIF)
{
	handledBIF = true;
	TR::Node *firstChild = node->getFirstChild(), *secondChild = node->getSecondChild();
...
OMR::Z::TreeEvaluator::ificmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return generateS390CompareBranch(
	        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH);
}
...
OMR::Z::TreeEvaluator::ificmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return generateS390CompareBranch(
	        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH);
}
...
OMR::Z::TreeEvaluator::ificmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return generateS390CompareBranch(
	        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL);
}
...
OMR::Z::TreeEvaluator::ificmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	return generateS390CompareBranch(
	        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL);
}
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
			...
			if (node->getOpCodeValue() != TR::acmpeq && TR::Compiler->target.is64Bit()
			        ...
			        && performTransformation(cg->comp(),
			                "O^O CODE GENERATION:  ===>   Use RXSBG to perform icmpeq  <==\n")) {
				...
				if ((val1 & (val1 - 1)) == 0) {
					...
					generateRIEInstruction(
					        cg, TR::InstOpCode::RXSBG, node, target, source, 63, 128 + 63, rotBy);
...
OMR::Z::TreeEvaluator::icmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!node->getOpCode().isUnsignedCompare() && firstChild->getReferenceCount() == 1
	        && firstChild->isNonNegative() && secondChild->isNonNegative()
	        && performTransformation(
	                cg->comp(), "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmplt  <==\n")) {
...
OMR::Z::TreeEvaluator::icmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!node->getOpCode().isUnsignedCompare() && secondChild->getReferenceCount() == 1
	        && firstChild->isNonNegative() && secondChild->isNonNegative()
	        && performTransformation(
	                cg->comp(), "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmpgt  <==\n")) {
...
void
OMR::Z::TreeEvaluator::tableEvaluatorCaseLabelHelper(TR::Node *node, TR::CodeGenerator *cg,
...
OMR::Z::TreeEvaluator::tableEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	 ...
	 * branch to it, and an address table, where we load an address out of the table and branch to it since cases
	 * are indexed by 1, we need to shift by either 4 or 8 to get the next value in memory.
	 */
...
OMR::Z::TreeEvaluator::lookupEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	for (int32_t ii = 2; ii < numChildren; ii++) {
		...
		if (node->getOpCodeValue() == TR::trtLookup) {
			...
			if (type.isInt64()) {
				generateS390ImmOp(cg, TR::InstOpCode::CG, node, selectorReg, selectorReg,
...
OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	TR_S390OutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
	        TR_S390OutOfLineCodeSection(node, TR::call, NULL, slowPathOOLLabel, doneLabel, cg);
...
void
OMR::Z::TreeEvaluator::addToRegDep(TR::RegisterDependencyConditions *daaDeps, TR::Register *reg, bool mightUseRegPair)
{
	if (!reg)
		return;
        ...
        TR::Node *node, TR::RegisterDependencyConditions *daaDeps, TR::Instruction *daaInstr, TR::CodeGenerator *cg)
{
	...
	case TR::InstOpCode::VSP: break;
	default: TR_ASSERT(0, "Target Instruction is not used by DAA.\n"); break;
	}
	...
	default: TR_ASSERT(0, "Inconsistent DAA Instruction format.\n"); break;
	}
}

TR::Node *
OMR::Z::TreeEvaluator::DAAAddressPointer(TR::Node *callNode, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Z::TreeEvaluator::inlineIfButestEvaluator(
...
TR::Register *
OMR::Z::TreeEvaluator::inlineIfBifEvaluator(
        TR::Node *ifNode, TR::CodeGenerator *cg, TR::Node *cmpOpNode, TR::Node *cmpValNode)
{
	...
	if (ifNode->getFirstChild()->getOpCodeValue() == TR::bu2i) {
		...
		//      cmpOpNode
		//    cmpValNode
		// We have previously verified that cmpOpNode isSingleRefUnevaluated(), so therefore the bu2i must also
		// be isSingleRefEvaluated() and we should dec ref count here
 ...
 */
TR::InstOpCode::S390BranchCondition
OMR::Z::TreeEvaluator::mapBranchConditionToLOCRCondition(TR::InstOpCode::S390BranchCondition incomingBc)
{
	switch (incomingBc) {
...
OMR::Z::TreeEvaluator::getCompareOpFromNode(TR::CodeGenerator *cg, TR::Node *node)
{
...
OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (comp->getOption(TR_TraceCG))
		traceMsg(comp,
		        "Starting evaluation of ternary node %p condition %p (in reg %p) trueVal %p (in reg %p) "
		        "falseVal %p (in reg %p)\n",
	...
	if (TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal)
		...
		if (comp->getOption(TR_TraceCG))
			traceMsg(comp,
			        "Calling evaluate (instead of clobber evaluate for node %p because all other uses are "
			        "in the compare tree)\n",
			        trueVal);
	...
	if (condition->getOpCode().isBooleanCompare()
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
			...
			TR::RegisterDependencyConditions *conditions = conditions = new (cg->trHeapMemory())
			...
			generateRRInstruction(cg,
			        trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LGR : TR::InstOpCode::LR, node,
			        trueReg, falseReg);
			...
			if (comp->getOption(TR_TraceCG)) {
				traceMsg(comp,
				        "firstReg = %p secondReg = %p trueReg = %p falseReg = %p trueReg->is64BitReg() "
				        "= %d falseReg->is64BitReg() = %d isLongCompare = %d\n",
...
OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (conditionNode->getOpCode().isCompareDouble()) {
		...
		case TR::dcmpneu:
		case TR::dcmpeq:
		case TR::dcmpequ: compareOp = TR::InstOpCode::VFCE; break;
		case TR::dcmplt:
		case TR::dcmpgt:
		case TR::dcmpleu:
		case TR::dcmpgeu: compareOp = TR::InstOpCode::VFCH; break;
		case TR::dcmpltu:
		case TR::dcmpgtu:
		case TR::dcmple:
		case TR::dcmpge: compareOp = TR::InstOpCode::VFCHE; break;
		}

		// To handle unordered compares, we switch the compare operation, since the vector compare instructions
		// return false for unordered. For example, if the original compare is dcmpgeu:
		...
		// The above operation returns 0's for not equals, so we must swap the true and false values to select
		// the correct "true value" for cmpne operations Swap the values for unordered comparisons also (see
		// above)
