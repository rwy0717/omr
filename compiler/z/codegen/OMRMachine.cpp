...
///////////////////////////////////////////////////////////////////////////////
TR::Instruction *
OMR::Z::Machine::registerCopy(TR::CodeGenerator *cg, TR_RegisterKinds rk, TR::RealRegister *targetReg,
        TR::RealRegister *sourceReg, TR::Instruction *precedingInstruction)
{
	...
	TR_ASSERT_FATAL(sourceReg->getAssignedRegister() != NULL || targetReg->getAssignedRegister() != NULL,
	        "Attempting register copy with source (%s) and target (%s) real registers not corresponding to any "
	        "virtual register",
	...
	case TR_FPR:
		cursor = generateRRInstruction(
		        cg, TR::InstOpCode::LDR, node, targetReg, sourceReg, precedingInstruction);
		break;
	case TR_VRF:
		cursor = generateVRRaInstruction(
		        cg, TR::InstOpCode::VLR, node, targetReg, sourceReg, precedingInstruction);
		break;
	}
 ...
 */
TR::Instruction *
OMR::Z::Machine::registerExchange(TR::CodeGenerator *cg, TR_RegisterKinds rk, TR::RealRegister *targetReg,
        TR::RealRegister *sourceReg, TR::RealRegister *middleReg, TR::Instruction *precedingInstruction)
{
	...
	if (rk == TR_FPR) {
		if (middleReg != NULL) {
			...
			TR::MemoryReference *tempMR =
			        generateS390MemoryReference(currentNode, location->getSymbolReference(), cg);
			location->getSymbolReference()->getSymbol()->setSpillTempLoaded();

			currentInstruction = generateRXInstruction(
			        cg, TR::InstOpCode::STD, currentNode, targetReg, tempMR, currentInstruction);
			cg->traceRAInstruction(currentInstruction);
			currentInstruction = generateRRInstruction(
			        cg, TR::InstOpCode::LDR, currentNode, targetReg, sourceReg, currentInstruction);
			cg->traceRAInstruction(currentInstruction);
			TR::MemoryReference *tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
			currentInstruction = generateRXInstruction(
			        cg, TR::InstOpCode::LD, currentNode, sourceReg, tempMR2, currentInstruction);
			cg->traceRAInstruction(currentInstruction);
	...
	// exchange vector registers
		...
		//
		if (middleReg == NULL) {
			// Nasty OSC... should happen very rarely.  Only in cases where deps reach critical mass in reg
			// table and last dep resolution requires a regexch from 'blocked' state.
			//
			TR::Instruction *currentInstruction = precedingInstruction;
			TR_BackingStore *location;
		...
		cg->generateDebugCounter(
		        precedingInstruction, "RegisterAllocator/Exchange/GPR", 1, TR::DebugCounter::Free);
	}
...
bool
OMR::Z::Machine::isLegalFirstOfFPRegister(
...
bool
OMR::Z::Machine::isLegalSecondOfFPRegister(
        TR::RealRegister *reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
	// Is the register assigned
	if (reg == NULL) {
        ...
        TR::Register *targetRegister, TR::Instruction *currInst, bool doBookKeeping, uint64_t availRegMask)
{
	...
	if (rp) {
		...
		returnRegister =
		        self()->assignBestRegisterSingle(targetRegister, currInst, doBookKeeping, availRegMask);
	}
        ...
        TR::Instruction *currentInstruction, TR::Register *currentAssignedRegister, uint64_t availRegMask)
{
	...
	if ((newRegister = self()->findBestFreeRegister(
	             currentInstruction, kindOfRegister, currentAssignedRegister, availRegMask))
	        == NULL) {
	...
	self()->cg()->traceRegisterAssignment("%R is assigned to invalid register in this instruction. Shuffling %R => "
	                                      "%R...",
...
*/
TR::RealRegister *
OMR::Z::Machine::shuffleOrSpillRegister(TR::Instruction *currInst, TR::Register *toFreeReg, uint64_t availRegMask)
{
	...
	if (bestRegister == NULL)
		...
		TR::Instruction *cursor = self()->registerCopy(
		        self()->cg(), toFreeReg->getKind(), assignedRegister, bestRegister, currInst);
        ...
        TR::Register *targetRegister, TR::Instruction *currInst, bool doBookKeeping, uint64_t availRegMask)
{
	...
	if (kindOfRegister != TR_FPR && kindOfRegister != TR_VRF && assignedRegister != NULL) {
		// In case we need a register shuffle, if the register we are assigning is a source register of the
		// instruction, the shuffling instruction should happen before this instruction
		...
		if ((assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
			...
			TR::Instruction *cursor = self()->registerCopy(self()->cg(), kindOfRegister,
			        toRealRegister(assignedRegister), newAssignedRegister, appendInst);
		...
		TR::Instruction *cursor = self()->registerCopy(
		        self()->cg(), kindOfRegister, assignedRegister, newAssignedRegister, currInst);
	...
	if (doBookKeeping && (assignedRegister->getState() != TR::RealRegister::Locked)) {
		...
		TR_ASSERT(targetRegister->getFutureUseCount() >= 0,
		        "\nRegister assignment: register [%s] futureUseCount should not be negative (for node [%s], "
		        "ref count=%d) !\n",
		...
		// OOL: if a register is first defined (became live) in the hot path, no matter how many futureUseCount
		// left (in the code path) the register is considered as dead now in the hot path, so GC map contains
		...
		if (self()->cg()->isOutOfLineHotPath() && targetRegister->getStartOfRange() == currInst) {
			...
			// We assign target registers first, but in this case we do not want to kill the virtual
			// register because the source register (which has the same virtual reg) is not yet assigned
 ...
 */
TR::Register *
OMR::Z::Machine::assignBestRegisterPair(
        TR::Register *regPair, TR::Instruction *currInst, bool doBookKeeping, uint64_t availRegMask)
{
	...
	self()->cg()->traceRegisterAssignment(
	        "attempt to assign components of register pair ( %R %R )", firstReg, lastReg);
	...
	TR::RegisterPair *assignedRegPair = new (self()->cg()->trHeapMemory(), TR_MemoryBase::RegisterPair)
	...
	if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF)
	        ...
	        || ((regPair->getKind() == TR_FPR)
	                && self()->isLegalFPPair(freeRegisterHigh, freeRegisterLow, availRegMask))) {
		...
		if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
			...
			if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
				...
				if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow,
					...
					self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPairKind,
		...
		if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
			...
			self()->coerceRegisterAssignment(currInst, firstReg,
			        (TR::RealRegister::RegNum)(
			                toRealRegister(freeRegisterLow->getSiblingRegister())->getRegisterNumber()));
			...
			// Assign a real register pair for
			//
			if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(),
			            currInst, availRegMask)) {
				self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(),
				        currInst, availRegMask);
			}
	...
	if (rk != TR_FPR && rk != TR_VRF) {
		...
		for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
			...
			if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free
			        ...
			        && (_registerFile[i + 1]->getState() == TR::RealRegister::Free
			                || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched)
			        && (_registerFile[i + 0]->getWeight() + _registerFile[i + 1]->getWeight())
			                < bestWeightSoFar) {
		...
		for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
			...
			if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free
			        ...
			        && (_registerFile[i + 0]->getWeight() + _registerFile[i + 2]->getWeight())
			                < bestWeightSoFar) {
				freeRegisterHigh = _registerFile[i];
...
void
OMR::Z::Machine::freeBestFPRegisterPair(
	...
	if (bestVirtCandidateLow != NULL) {
		...
		if (!comp->getOption(TR_DisableOOL)) {
			if (!self()->cg()->isOutOfLineColdPath()) {
				...
				if (debugObj)
					self()->cg()->traceRegisterAssignment("OOL: adding reg pair low %s to the "
					                                      "spilledRegisterList, maxSpillDepth = "
					                                      "%d\n",
	...
	if (bestVirtCandidateHigh != NULL) {
		...
		if (!comp->getOption(TR_DisableOOL)) {
			if (!self()->cg()->isOutOfLineColdPath()) {
				...
				if (debugObj)
					self()->cg()->traceRegisterAssignment("OOL: adding reg pair high %s to the "
					                                      "spilledRegisterList, maxSpillDepth = "
					                                      "%d\n",
...
void
OMR::Z::Machine::freeBestRegisterPair(TR::RealRegister **firstReg, TR::RealRegister **lastReg, TR_RegisterKinds rk,
        ...
        TR::Instruction *currentInstruction, TR_RegisterKinds rk, TR::Register *virtualReg, uint64_t availRegMask)
{
	...
	if (virtualReg->getKind() == TR_VRF && self()->cg()->getSupportsVectorRegisters()
	...
	if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair) {
		...
		if ((virtualReg->getSiblingRegister())
			...
			uint64_t sibRegMask = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())
			                              ->getRealRegisterMask();
		...
		// If preference register is also marked as an interference at some point, check to see if the
	...
	for (int32_t i = first; i <= last; i++) {
		...
		if ((_registerFile[i]->getState() == TR::RealRegister::Free
			...
			if (comp->getOption(TR_Randomize)) {
				...
				if (performTransformation(comp,
				            "O^O Random Codegen - Randomizing Weight for %s, Original bestWeightSoFar: "
				            "%x randomized to: %x\n",
				            self()->cg()->getDebug()->getName(_registerFile[i]), bestWeightSoFar,
				            randomWeight)) {
        ...
        TR::Instruction *currentInstruction, TR::Register *virtReg, TR_RegisterKinds rk, bool allowNullReturn)
{
	...
	for (int32_t i = first; i <= last; i++) {
		...
		// TODO: This assert was added as it existed in a path which was guarded by is64BitReg. I'm fairly
		// certain this assert should never fire because otherwise we would be trying to free the best register
		// when a free register already exists, meaning that the caller is to blame for not checking if a free
		// register was available. An alternative would be to just return the free register right away? In
		// either event we'll leave this assert here for a little while and we can remove it once it has had
		// time to bake.
		TR_ASSERT_FATAL(realReg->getState() != TR::RealRegister::Free,
		        "Attempting to free best register for virtual register (%s) when a free register (%s) already "
		        "exists",
	...
	if (numCandidates == 0) {
		if (!allowNullReturn) {
			...
			TR_ASSERT_FATAL(false, "Ran out of register candidates to free on instruction [%p]",
			        currentInstruction);
		}
...
OMR::Z::Machine::spillRegister(TR::Instruction *currentInstruction, TR::Register *virtReg)
{
	...
	case TR_GPR:
		if (!comp->getOption(TR_DisableOOL)
			...
			if (debugObj)
				self()->cg()->traceRegisterAssignment("\nOOL: Reuse backing store (%p) for %s inside "
				                                      "OOL\n",
	...
	case TR_FPR:
		if (!comp->getOption(TR_DisableOOL)
			...
			location = self()->cg()->allocateSpill(8, false, NULL, true); // TODO: Use 4 for
		...
		break;
	case TR_VRF:
		// Spill of size 16 has never been done before. The call hierarchy seems to support it but this should
		// be watched closely.
	...
	if (!comp->getOption(TR_DisableOOL) && location != NULL) {
		if (!self()->cg()->isOutOfLineColdPath()) {
			...
			if (debugObj)
				self()->cg()->traceRegisterAssignment("OOL: adding %s to the spilledRegisterList, "
				                                      "maxSpillDepth = %d\n",
        ...
        TR::Instruction *currentInstruction, TR::Register *spilledRegister, TR::RealRegister *targetRegister)
{
	...
	if (targetRegister == NULL) {
		...
		targetRegister =
		        self()->findBestFreeRegister(currentInstruction, spilledRegister->getKind(), spilledRegister);
		if (targetRegister == NULL) {
	...
	if (true) // Check to see if we should free spill location
	{
		if (comp->getOption(TR_DisableOOL)) {
			...
			if (self()->cg()->isOutOfLineColdPath()) {
				...
				// OOL: only free the spill slot if the register was spilled in the same or less
				// dominant path ex: spilled in cold path, reverse spill in hot path or main line we
				// have to spill this register again when we reach OOL entry point due to post
				// conditions. We want to guarantee that the same spill slot will be protected and
				// reused. maxSpillDepth: 3:cold path, 2:hot path, 1:main line
				if (location->getMaxSpillDepth() == 0 || location->getMaxSpillDepth() == 3
					...
					if (debugObj)
						self()->cg()->traceRegisterAssignment("\nOOL: reverse spill %s in less "
						                                      "dominant path (%d / 3), protect "
						                                      "spill slot (%p)\n",
						        debugObj->getName(spilledRegister),
				...
				// Any registers reverse spilled before entering OOL should be removed from the spilled
				// list
				if (debugObj)
					self()->cg()->traceRegisterAssignment("\nOOL: removing %s from the "
					                                      "spilledRegisterList)\n",
					        debugObj->getName(spilledRegister));
				...
				if (location->getMaxSpillDepth() == 2) {
					...
					if (debugObj)
						self()->cg()->traceRegisterAssignment("\nOOL: reverse spilling %s in "
						                                      "less dominant path (%d / 2), "
	...
	if (opCode == TR::InstOpCode::VST)
		...
		cursor = generateRXInstruction(
		        self()->cg(), opCode, currentNode, targetRegister, tempMR, currentInstruction);
...
OMR::Z::Machine::isAssignable(TR::Register *virtReg, TR::RealRegister *realReg)
{
	if (virtReg->isUsedInMemRef() && realReg == _registerFile[TR::RealRegister::GPR0]) {
		...
		// TODO: This is needlessly restrictive. The registerExchange API cannot handle register exchanges with
		// one 32-bit and one 64-bit register and the code below effectively guards against calling the
		// registerExchange API in such situations. This can definitely be relaxed and the registerExchange API
		// taught how to handle those cases. If you look at the places this API (isAssignable) is used you will
		// see we effectively handle it there already. That code needs to be cleaned up and consolidated into
		// the registerExchange API.
        ...
        TR::Instruction *currentInstruction, TR::Register *virtualRegister, TR::RealRegister::RegNum registerNumber)
{
	...
	// the target reg is free
		...
		if (virtualRegister->isPlaceholderReg())
			targetRegister->setIsAssignedMoreThanOnce(); // Register is killed invalidate it for moving
			                                             // spill out of loop
		...
		if (currentAssignedRegister != NULL) {
			if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)) {
				...
				cursor = self()->registerExchange(self()->cg(), rk, targetRegister,
				        currentAssignedRegister, spareReg, currentInstruction);
	...
	// the target reg is assigned
		...
		if (currentAssignedRegister != NULL) {
			...
			if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)
				...
				if (spareReg == NULL) {
					...
					if (virtualRegister->is64BitReg()) {
						...
						spareReg = self()->freeBestRegister(currentInstruction,
						        currentTargetVirtual, currentTargetVirtual->getKind());
					} else {
						spareReg = self()->freeBestRegister(currentInstruction,
						        currentTargetVirtual, currentTargetVirtual->getKind(), true);
					}

					// For some reason (blocked/locked regs etc), we couldn't find a spare reg so
					// spill the virtual in the target and use it for coercion
					if (spareReg == NULL) {
				...
				if (targetRegister->getRegisterNumber() != spareReg->getRegisterNumber()
				        && !doNotRegCopy) {
					self()->cg()->traceRegAssigned(currentTargetVirtual, spareReg);
...
int32_t
OMR::Z::Machine::addGlobalRegLater(TR::RealRegister::RegNum reg, int32_t tableIndex)
{
	if (reg == TR::RealRegister::NoReg)
		return tableIndex;
	if (self()->getRealRegister(reg)->getState() == TR::RealRegister::Locked)
...
OMR::Z::Machine::initGlobalVectorRegisterMap(uint32_t vectorOffset)
{
	...
	// Colouring will make sure to map global virtual FPRs only to the first 16 and vector global virtuals can map
	// to all 32 registers.
...
#define addToGRAMap(regNum) _globalRegisterNumberToRealRegisterMap[vectorOffset++] = TR::RealRegister::regNum
	...
	// to ensure we do things right i.e if any of the 16 fpr grn's are assigned, remove the corresponding reg in
	// the 32.
	...
	for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
		...
		if (_registerAssociationsSnapShot[i])
			self()->setVirtualAssociatedWithReal(
			        (TR::RealRegister::RegNum)(i), _registerAssociationsSnapShot[i]);
		if (_registerFile[i]->getState() == TR::RealRegister::Free) {
			if (_registerFile[i]->getAssignedRegister() != NULL) {
				if (_registerFile[i]->getAssignedRegister()->getAssignedRegister()
				        == _registerFile[i]) {
					// clear the Virt -> Real reg assignment if we restored the Real reg state to
					// FREE
			...
			if (_registerFile[i]->getAssignedRegister() != NULL
				...
				// If the virtual register associated with the _registerFile[i] is not assigned to the
				// current real register it must have been updated by prior _registerFile.... Do NOT
				// Clear.
				//   Ex:
				//     RegFile starts as:
				...
				//  When we handled _registerFile[12], we would have updated the assignment of GPR_3545
				//  (currently to GPR15) to GPR12. When we subsequently handle _registerFile[15], we
				//  cannot blindly reset GPR_3545's assigned register to NULL, as that will incorrectly
				//  break the assignment to GPR12.
				if (_registerFile[i]->getAssignedRegister()->getAssignedRegister()
				        == _registerFile[i]) {
					// clear the Virt -> Real reg assignment for any newly assigned virtual register
					// (due to spills) in the hot path
					_registerFile[i]->getAssignedRegister()->setAssignedRegister(NULL);
				}
			}
		}
		...
		// the dead in a later OOL hot path) and has never been used in any OOL cold path or main line, then we
		// need to make sure to not include this register in the snapshot once the register is dead, otherwise
...
OMR::Z::Machine::createDepCondForLiveGPRs(TR::list<TR::Register *> *spilledRegisterList)
{
	int32_t i, c = 0;
