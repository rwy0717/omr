    ...
    flags32_t instFlags)
{
    ...
    bool enableHighWordRA = cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF;

    switch (rk) {
    case TR_GPR64:
        currentInstruction = generateRRInstruction(cg, TR::InstOpCode::LGR, currentNode, targetReg, sourceReg, precedingInstruction);
        ...
        break;
    case TR_GPR:
        currentInstruction = generateRRInstruction(cg, (comp->getOption(TR_ForceLargeRAMoves)) ? TR::InstOpCode::LGR : TR::InstOpCode::getLoadRegOpCode(), currentNode, targetReg, sourceReg, precedingInstruction);
        ...
        if (middleReg == NULL) {
            ...
            if (enableHighWordRA && rk != TR_GPR64) {
                if (srcRegIsHPR != middleRegIsHPR) {
                    currentInstruction = generateRRInstruction(cg, srcRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR, currentNode, sourceReg, middleReg, precedingInstruction);
                } else {
                    currentInstruction = generateRRInstruction(cg, srcRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR, currentNode, sourceReg, middleReg, precedingInstruction);
                }
                cg->traceRAInstruction(currentInstruction);
                if (debugObj) {
                ...
                if (srcRegIsHPR != tgtRegIsHPR) {
                    currentInstruction = generateRRInstruction(cg, tgtRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR, currentNode, targetReg, sourceReg, precedingInstruction);
                } else {
                    currentInstruction = generateRRInstruction(cg, tgtRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR, currentNode, targetReg, sourceReg, precedingInstruction);
                }
                cg->traceRAInstruction(currentInstruction);
                if (debugObj) {
                    debugObj->addInstructionComment(toS390RRInstruction(currentInstruction), REG_EXCHANGE);
                ...
                if (middleRegIsHPR != tgtRegIsHPR) {
                    currentInstruction = generateRRInstruction(cg, middleRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR, currentNode, middleReg, targetReg, precedingInstruction);
                } else {
                    currentInstruction = generateRRInstruction(cg, middleRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR, currentNode, middleReg, targetReg, precedingInstruction);
                }
...
checkOOLInterference(TR::Instruction* currentInstruction, TR::Register* virtReg)
{
    ...
    while (cursor != virtReg->getStartOfRange()) {
        ...
        if (cursor->getNode() != NULL && cursor->getNode()->getOpCodeValue() == TR::BBStart && !cursor->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    ...
    uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && kindOfRegister != TR_FPR && kindOfRegister != TR_VRF;
    ...
    if (enableHighWordRA && assignedRegister != NULL) {
        ...
        if (targetRegister->is64BitReg()) {
            if ((toRealRegister(assignedRegister)->getRealRegisterMask() & availRegMask) == 0) {
            ...
            } else if (toRealRegister(assignedRegister)->isLowWordRegister() && toRealRegister(assignedRegister)->getHighWordRegister()->getAssignedRegister() != targetRegister) {
                ...
                } else if (toRealRegister(assignedRegister)->getHighWordRegister()->getState() == TR::RealRegister::Blocked || toRealRegister(assignedRegister)->getHighWordRegister()->getState() == TR::RealRegister::Locked) {
            ...
            if ((toRealRegister(assignedRegister))->isLowWordRegister() && targetRegister->assignToHPR()) {
                ...
                self()->addToUpgradedBlockedList(assignedRegister) ? assignedRegister->setState(TR::RealRegister::Blocked) : assignedRegister->setState(TR::RealRegister::Free);
    ...
    uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && regPairKind != TR_FPR && regPairKind != TR_VRF;
    ...
    TR_RegisterKinds rk, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF;
    ...
    if (rk != TR_FPR && rk != TR_VRF) {
        ...
        for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
            // Don't consider registers that can't be assigned.
            if ((_registerFile[i + 0]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 1]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 0]->getRealRegisterMask() & availRegMask) == 0 || (_registerFile[i + 1]->getRealRegisterMask() & availRegMask) == 0) {
                continue;
            }
            ...
            if (enableHighWordRA && rk == TR_GPR64) {
                ...
                if ((_registerFile[i + 0]->getHighWordRegister()->getState() == TR::RealRegister::Free || _registerFile[i + 0]->getHighWordRegister()->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 1]->getHighWordRegister()->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getHighWordRegister()->getState() == TR::RealRegister::Unlatched))
                    highWordPairIsFree = true;
            }

            // See if this pair is available, and better than the prev
            if (highWordPairIsFree && (_registerFile[i + 0]->getState() == TR::RealRegister::Free || _registerFile[i + 0]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 1]->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 0]->getWeight() + _registerFile[i + 1]->getWeight()) < bestWeightSoFar) {
    ...
    } else // if FP Reg Pair
    {
        ...
        for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
            ...
            // Don't consider registers that can't be assigned
            if ((_registerFile[i + 0]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 2]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 0]->getRealRegisterMask() & availRegMask) == 0 || (_registerFile[i + 2]->getRealRegisterMask() & availRegMask) == 0) {
                continue;
            }

            // See if this pair is available and better than the previous
            if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 2]->getState() == TR::RealRegister::Free || _registerFile[i + 2]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 0]->getWeight() + _registerFile[i + 2]->getWeight()) < bestWeightSoFar) {
                freeRegisterHigh = _registerFile[i];
    ...
    uint64_t availRegMask)
{
    ...
    if (bestVirtCandidateLow != NULL) {
        ...
        if (comp->getOption(TR_ForceLargeRAMoves) || bestVirtCandidateLow->is64BitReg() || bestVirtCandidateLow->getKind() == TR_GPR64) {
        ...
        if (enableHighWordRA && rk == TR_GPR64 && bestCandidateLow->getHighWordRegister()->getAssignedRegister() != NULL) {
            // if the candidate is assigned on both GPR and HPR, need to spill HPR
            self()->cg()->traceRegisterAssignment("spilling HPR %R for reg pair low",
                bestCandidateLow->getHighWordRegister()->getAssignedRegister());
            currInst = self()->freeHighWordRegister(currInst, bestCandidateLow->getHighWordRegister(), 0);
    ...
    if (bestVirtCandidateHigh != NULL) {
        ...
        if (comp->getOption(TR_ForceLargeRAMoves) || bestVirtCandidateHigh->is64BitReg() || bestVirtCandidateHigh->getKind() == TR_GPR64) {
        ...
        if (enableHighWordRA && rk == TR_GPR64 && bestCandidateHigh->getHighWordRegister()->getAssignedRegister() != NULL) {
            // if the candidate is assigned on both GPR and HPR, need to spill HPR
            self()->cg()->traceRegisterAssignment("spilling HPR %R for reg pair high",
                bestCandidateHigh->getHighWordRegister()->getAssignedRegister());
            currInst = self()->freeHighWordRegister(currInst, bestCandidateHigh->getHighWordRegister(), 0);
    ...
    bool needsHighWord)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && (rk == TR_GPR || rk == TR_GPR64);
    ...
    if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair && !needsHighWord) {
       ...
       */
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && virtualReg->is64BitReg()) {
            if (bestRegister != NULL && (bestRegister->getState() == TR::RealRegister::Free || bestRegister->getState() == TR::RealRegister::Unlatched) && (bestRegister->getHighWordRegister()->getState() == TR::RealRegister::Free || bestRegister->getHighWordRegister()->getState() == TR::RealRegister::Unlatched)) {
        ...
        // HighWord RA stuff
            ...
            if (virtualReg->is64BitReg() && !needsHighWord) {
                ...
                if (preference != 0 && (prefRegMask & availRegMask) && _registerFile[preference] != NULL) {
                    candidateLWFree = (_registerFile[preference]->getState() == TR::RealRegister::Free) || (_registerFile[preference]->getState() == TR::RealRegister::Unlatched);
                    candidateHWFree = (_registerFile[preference]->getHighWordRegister()->getState() == TR::RealRegister::Free) || (_registerFile[preference]->getHighWordRegister()->getState() == TR::RealRegister::Unlatched);
                }
                ...
                if ((prefRegMask & availRegMask) && candidateLWFree && candidateHWFree && _registerFile[preference] != NULL) {
                    ...
                    if (bestRegister != NULL && bestRegister->getHighWordRegister()->getState() == TR::RealRegister::Unlatched) {
                        bestRegister->getHighWordRegister()->setAssignedRegister(NULL);
                        bestRegister->getHighWordRegister()->setState(TR::RealRegister::Free);
                    }
                    self()->cg()->setRegisterAssignmentFlag(TR_ByAssociation);
                ...
                if (candidate != NULL && (prefRegMask & availRegMask) && ((candidate->getState() == TR::RealRegister::Free) || (candidate->getState() == TR::RealRegister::Unlatched))) {
                    bestWeightSoFar = 0x0fffffff;
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (!enableHighWordRA) {
            ...
            if (virtualReg->is64BitReg() && !needsHighWord) {
                bool candidateLWFree = (candidate->getState() == TR::RealRegister::Free) || (candidate->getState() == TR::RealRegister::Unlatched);
                bool candidateHWFree = (candidate->getHighWordRegister()->getState() == TR::RealRegister::Free) || (candidate->getHighWordRegister()->getState() == TR::RealRegister::Unlatched);

                // Don't consider registers that can't be assigned.
                if ((candidate->getState() == TR::RealRegister::Locked) || (candidate->getHighWordRegister()->getState() == TR::RealRegister::Locked) || ((tRegMask & availRegMask) == 0)) {
                    continue;
                }
                //self()->cg()->traceRegWeight(candidate, candidate->getWeight());

                iNew = interference & (1 << (i - maskI));
                if (candidateLWFree && candidateHWFree && (freeRegister == NULL || (iOld && !iNew) || ((iOld || !iNew) && candidate->getWeight() < bestWeightSoFar))) {
                    iOld = iNew;
                ...
                iNew = interference & (1 << (i - maskI));
                if ((candidate->getState() == TR::RealRegister::Free || (candidate->getState() == TR::RealRegister::Unlatched)) && (freeRegister == NULL || (iOld && !iNew) || ((iOld || !iNew) && candidate->getWeight() < bestWeightSoFar))) {
                    iOld = iNew;
...
void OMR::Z::Machine::allocateUpgradedBlockedList(TR_Stack<TR::RealRegister*>* mem)
{
...
void OMR::Z::Machine::spillAllVolatileHighRegisters(TR::Instruction* currentInstruction)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (volatileReg && realReg->getState() == TR::RealRegister::Assigned && (virtReg = realReg->getAssignedRegister()) && virtReg->getKind() == TR_GPR64) {
    ...
    uint64_t availRegMask, bool allowNullReturn, bool doNotSpillToSiblingHPR)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (!enableHighWordRA) {
            if (realReg->getState() == TR::RealRegister::Assigned) {
                ...
                if (currentInstruction->getDependencyConditions() && currentInstruction->getDependencyConditions()->searchPostConditionRegister(associatedVirtual)) {
        ...
        // HighWord RA stuff
            ...
            */
                ...
                if ((realReg->getState() == TR::RealRegister::Assigned || realReg->getState() == TR::RealRegister::Free) && (realRegHW->getState() == TR::RealRegister::Assigned || realRegHW->getState() == TR::RealRegister::Free) && !doNotSpillHPR) {
    ...
    while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != TR::InstOpCode::LABEL && cursor->getOpCodeValue() != TR::InstOpCode::PROC) {
        for (int32_t i = 0; i < numCandidates; i++) {
            ...
            // also need to check HW. if it's assigned, push the candidate back
                ...
                TR::RealRegister* candidateHW = toRealRegister(candidates[i]->getAssignedRegister())->getHighWordRegister();
    ...
    // todo fix this
    else if (enableHighWordRA && virtReg->is64BitReg() && best->isLowWordRegister() && best->getHighWordRegister()->getAssignedRegister() && best->getHighWordRegister()->getAssignedRegister() != candidates[0]) {
...
void OMR::Z::Machine::freeRealRegister(TR::Instruction* currentInstruction, TR::RealRegister* targetReal, bool is64BitReg)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && targetReal->getKind() != TR_FPR && targetReal->getKind() != TR_VRF;
 ...
 *  A GPRX, ....      // use GPRX
 */
void OMR::Z::Machine::spillRegister(TR::Instruction* currentInstruction, TR::Register* virtReg, uint32_t availHighWordRegMap)
{
    TR::Compilation* comp = self()->cg()->comp();
    ...
    if (virtReg->containsCollectedReference() && TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && TR::Compiler->vm.heapBaseAddress() == 0 && TR::Compiler->om.compressedReferenceShift() == 0 && !checkOOLInterference(currentInstruction, virtReg)) {
        canSpillPointerToHPR = true;
    }

    // try to spill to highword
    if (enableHighWordRA && !comp->getOption(TR_DisableHPRSpill) && ((!virtReg->is64BitReg() && !virtReg->containsCollectedReference()) || canSpillPointerToHPR) && best->isLowWordRegister() && !virtReg->containsInternalPointer() && !self()->cg()->isOutOfLineColdPath()) {
        ...
        if (freeHighWordReg) {
            ...
            if (debugHPR) {
                printf("in %s ,0x%x, %s", comp->signature(), availHighWordRegMap, comp->getHotnessName(comp->getMethodHotness()));
                fflush(stdout);
            }
...
bool OMR::Z::Machine::isAssignable(TR::Register* virtReg, TR::RealRegister* realReg)
{
    if (virtReg->isUsedInMemRef() && realReg == _registerFile[TR::RealRegister::GPR0]) {
        ...
        if (self()->cg()->supportsHighWordFacility() && !self()->cg()->comp()->getOption(TR_DisableHighWordRA) && virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF) {
            if ((virtReg->is64BitReg() && realReg->getLowWordRegister()->getAssignedRegister() == realReg->getHighWordRegister()->getAssignedRegister()) || (!virtReg->is64BitReg() && realReg->getLowWordRegister()->getAssignedRegister() != realReg->getHighWordRegister()->getAssignedRegister())) {
                return true;
            } else {
                return false;
            }
    ...
    flags32_t instFlags)
{
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            if (targetRegister->isHighWordRegister() && !virtualRegister->isPlaceholderReg()) {
                ...
                if (virtualRegister->getAssignedRegister() == NULL && currentHighWordReg) {
                    ...
                    if (currentAssignedRegister == NULL) {
                        if (virtualRegister->getTotalUseCount() != virtualRegister->getFutureUseCount() && virtualRegister->getBackingStorage() != NULL) {
    ...
    // the target reg is blocked so we need to guarantee it stays in a reg
        ...
        if (enableHighWordRA && currentTargetVirtual) {
            if (targetRegister->isHighWordRegister() && targetRegister->getLowWordRegister()->getAssignedRegister() != currentTargetVirtual) {
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            ...
            if (targetRegisterHW->getState() != TR::RealRegister::Free && targetRegisterHW->getState() != TR::RealRegister::Unlatched && targetRegisterHW->getAssignedRegister() != virtualRegister) {
                if (targetRegisterHW->getAssignedRegister() && targetRegisterHW->getAssignedRegister() != targetRegister->getAssignedRegister()) {
    ...
    // the target reg is assigned
        ...
        if (enableHighWordRA && currentTargetVirtual->getAssignedRegister() == NULL && targetRegister->isHighWordRegister()) {
        ...
        if (currentAssignedRegister != NULL) {
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)
                ...
                if (spareReg == NULL) {
                    ...
                    if (targetRegister->isHighWordRegister() && currentTargetVirtual->is64BitReg() && targetRegister->getRegisterNumber() == spareReg->getHighWordRegister()->getRegisterNumber())
                        doNotRegCopy = true;
                    virtualRegister->unblock();
                }
            ...
            if (spareReg == NULL && virtualRegister->isPlaceholderReg()) {
                ...
                if (enableHighWordRA && virtualRegister->is64BitReg()) {
                    ...
                    if (targetRegisterHW->getState() != TR::RealRegister::Free && targetRegisterHW->getAssignedRegister() != NULL && targetRegisterHW->getAssignedRegister() != targetRegister->getAssignedRegister()) {
...
OMR::Z::Machine::getGPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg)
{
    ...
    if (firstGlobalHPR != -1 && reg >= firstGlobalGPR && reg <= firstGlobalHPR && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
...
OMR::Z::Machine::getHPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg)
{
    ...
    if (firstGlobalHPR != -1 && reg >= firstGlobalHPR && reg <= lastGlobalHPR && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
