...
///////////////////////////////////////////////////////////////////////////////
TR::Instruction* OMR::Z::Machine::registerCopy(TR::Instruction* precedingInstruction, TR_RegisterKinds rk,
    TR::Register* targetReg, TR::Register* sourceReg, TR::CodeGenerator* cg, flags32_t instFlags)
{
    ...
    bool enableHighWordRA
        = cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF;
    ...
    case TR_GPR:
        currentInstruction = generateRRInstruction(cg,
            (comp->getOption(TR_ForceLargeRAMoves)) ? TR::InstOpCode::LGR : TR::InstOpCode::getLoadRegOpCode(),
    ...
    case TR_HPR:
        ...
        if (sourceRealReg && sourceRealReg->isLowWordRegister())
            currentInstruction = generateExtendedHighWordInstruction(
                currentNode, cg, TR::InstOpCode::LHLR, targetReg, sourceReg, 0, precedingInstruction);
        else
            currentInstruction = generateExtendedHighWordInstruction(
                currentNode, cg, TR::InstOpCode::LHHR, targetReg, sourceReg, 0, precedingInstruction);
        cg->traceRAInstruction(currentInstruction);
#ifdef DEBUG
        if (debug("traceMsg90GPR") || debug("traceGPRStats")) {
            cg->incTotalRegisterMoves();
    ...
    case TR_GPRL:
        ...
        if (sourceRealReg && sourceRealReg->isLowWordRegister())
            currentInstruction = generateRRInstruction(
                cg, TR::InstOpCode::LR, currentNode, targetReg, sourceReg, precedingInstruction);
        else
            currentInstruction = generateExtendedHighWordInstruction(
                currentNode, cg, TR::InstOpCode::LLHFR, targetReg, sourceReg, 0, precedingInstruction);
        cg->traceRAInstruction(currentInstruction);
#ifdef DEBUG
        if (debug("traceMsg90GPR") || debug("traceGPRStats")) {
            cg->incTotalRegisterMoves();
    ...
    case TR_FPR:
        currentInstruction
            = generateRRInstruction(cg, TR::InstOpCode::LDR, currentNode, targetReg, sourceReg, precedingInstruction);
        cg->traceRAInstruction(currentInstruction);
        break;
    case TR_VRF:
        currentInstruction
 ...
 */
TR::Instruction* OMR::Z::Machine::registerExchange(TR::Instruction* precedingInstruction, TR_RegisterKinds rk,
    TR::RealRegister* targetReg, TR::RealRegister* sourceReg, TR::RealRegister* middleReg, TR::CodeGenerator* cg,
    flags32_t instFlags)
{
        ...
        if (middleReg == NULL) {
            ...
            if (rk == TR_GPR64) {
                ...
                location = cg->allocateSpill(
                    TR::Compiler->om.sizeofReferenceAddress(), false, NULL); // No chance of a gcpoint
            }
            TR::MemoryReference* tempMR = generateS390MemoryReference(currentNode, location->getSymbolReference(), cg);
            location->getSymbolReference()->getSymbol()->setSpillTempLoaded();
            ...
            if (enableHighWordRA && rk != TR_GPR64) {
                if (srcRegIsHPR != middleRegIsHPR) {
                    currentInstruction
                        = generateRRInstruction(cg, srcRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR, currentNode,
                            sourceReg, middleReg, precedingInstruction);
                } else {
                    currentInstruction = generateRRInstruction(cg, srcRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR,
                ...
                if (srcRegIsHPR != tgtRegIsHPR) {
                    currentInstruction
                        = generateRRInstruction(cg, tgtRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR, currentNode,
                            targetReg, sourceReg, precedingInstruction);
                } else {
                    currentInstruction = generateRRInstruction(cg, tgtRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR,
                        currentNode, targetReg, sourceReg, precedingInstruction);
                }
                cg->traceRAInstruction(currentInstruction);
                if (debugObj) {
                ...
                if (middleRegIsHPR != tgtRegIsHPR) {
                    currentInstruction
                        = generateRRInstruction(cg, middleRegIsHPR ? TR::InstOpCode::LHLR : InstOpCode::LLHFR,
                            currentNode, middleReg, targetReg, precedingInstruction);
                } else {
                    currentInstruction
                        = generateRRInstruction(cg, middleRegIsHPR ? TR::InstOpCode::LHHR : InstOpCode::LR, currentNode,
                            middleReg, targetReg, precedingInstruction);
                }
 ...
 */
static bool checkOOLInterference(TR::Instruction* currentInstruction, TR::Register* virtReg)
{
    ...
    while (cursor != virtReg->getStartOfRange()) {
        ...
        if (cursor->isLabel() && toS390LabelInstruction(cursor)->getLabelSymbol()->isEndOfColdInstructionStream()) {
            cursor->cg()->traceRegisterAssignment(
                "Found %R used in OOL [0x%p]: will not spill to HPR", virtReg, cursor);
            return true;
        }
...
uint8_t OMR::Z::Machine::getGPRSize() { return TR::Compiler->target.is64Bit() ? 8 : 4; }
...
OMR::Z::Machine::Machine(TR::CodeGenerator* cg)
    ...
    , _lastGlobalHPRRegisterNumber(-1)
    ...
    TR::Instruction* currentInstruction, TR::Register* currentAssignedRegister, uint64_t availRegMask)
{
    ...
    if ((newRegister
        ...
        == NULL) {
        newRegister
            = self()->freeBestRegister(currentInstruction, currentAssignedRegister, kindOfRegister, availRegMask);
    }
    ...
    TR::Instruction* currInst, TR::Register* toFreeReg, uint64_t availRegMask)
{
    ...
    if (bestRegister == NULL)
        ...
        TR::Instruction* cursor
            = self()->registerCopy(currInst, toFreeReg->getKind(), assignedRegister, bestRegister, self()->cg(), 0);
    ...
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (assignedRegister == NULL) {
        if (self()->cg()->insideInternalControlFlow()) {
            TR_ASSERT_FATAL(0,
                "ASSERTION assignBestRegisterSingle inside Internal Control Flow on currInst=%p.\n"
    ...
    if (enableHighWordRA && assignedRegister != NULL) {
        ...
        if (targetRegister->is64BitReg()) {
            if ((toRealRegister(assignedRegister)->getRealRegisterMask() & availRegMask) == 0) {
                ...
                TR::Instruction* cursor = self()->registerCopy(
                    appendInst, kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, self()->cg(), 0);
                assignedRegister->setAssignedRegister(NULL);
                ...
                self()->cg()->traceRegisterAssignment(
                    "%R is 64bit but HPR is assigned to a different vreg, freeing up HPR", targetRegister);
                if (toRealRegister(assignedRegister)->getHighWordRegister()->getState() == TR::RealRegister::Assigned) {
                    ...
                    TR::Instruction* cursor = self()->freeHighWordRegister(
                        currInst, toRealRegister(assignedRegister)->getHighWordRegister(), 0);
                    assignedRegister->unblock();
                    self()->cg()->traceRAInstruction(cursor);
                } else if (toRealRegister(assignedRegister)->getHighWordRegister()->getState()
                ...
                self()->cg()->traceRegisterAssignment(
                    "%R is 64bit but currently assigned to HPR, shuffling", targetRegister);
                ...
                TR::RealRegister* assignedRegister64
                    = self()->findBestRegisterForShuffle(currInst, targetRegister, availRegMask);
                assignedRegister->unblock();
                TR::Instruction* cursor = generateExtendedHighWordInstruction(currInst->getNode(), self()->cg(),
                    TR::InstOpCode::LHLR, assignedRegister, assignedRegister64, 0, appendInst);
                self()->cg()->traceRAInstruction(cursor);
                assignedRegister->setAssignedRegister(NULL);
                assignedRegister->setState(TR::RealRegister::Free);
                assignedRegister = assignedRegister64;
                // todo: in regdepds make sure to do the other half: if the register comes in as 64-bit but requires an
                // HPR, need to do shuffling
            }
            ...
            if ((toRealRegister(assignedRegister))->isLowWordRegister() && targetRegister->assignToHPR()) {
                ...
                if ((assignedHighWordRegister
                        = self()->findBestFreeRegister(currInst, kindOfRegister, targetRegister, availRegMask))
                    == NULL) {
                    // assignedRegister->block();
                    assignedHighWordRegister
                        = self()->freeBestRegister(currInst, targetRegister, kindOfRegister, availRegMask);
                    // assignedRegister->unblock();
                }

                TR::Instruction* cursor = generateExtendedHighWordInstruction(currInst->getNode(), self()->cg(),
                    TR::InstOpCode::LLHFR, assignedRegister, assignedHighWordRegister, 0, appendInst);
                ...
                if (currInst->getOpCodeValue() == TR::InstOpCode::RISBG
                    ...
                    TR::Instruction* cursor = generateExtendedHighWordInstruction(currInst->getNode(), self()->cg(),
                        TR::InstOpCode::LHLR, assignedRegister, assignedLowWordRegister, 0, appendInst);
        ...
        && (toRealRegister(assignedRegister)->getRealRegisterMask() & availRegMask) == 0) {
        ...
        assignedRegister->unblock();
        TR::Instruction* cursor = self()->registerCopy(
            currInst, kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, self()->cg(), 0);
    ...
    if (assignedRegister == NULL) {
        ...
        if ((comp->getOption(TR_EnableTrueRegisterModel)) && targetRegister->isLive()
        ...
        if (!comp->getOption(TR_EnableTrueRegisterModel)
            && targetRegister->getTotalUseCount() != targetRegister->getFutureUseCount()) {
            ...
            if ((assignedRegister
                ...
                if (enableHighWordRA && targetRegister->is64BitReg()) {
                    ...
                    assignedRegister
                        = self()->freeBestRegister(currInst, targetRegister, kindOfRegister, availRegMask, false, true);
                } else {
        ...
        if (enableHighWordRA && targetRegister->is64BitReg() && !reverseSpilled) {
            // TR_ASSERTC(comp, toRealRegister(assignedRegister)->getHighWordRegister()->getState() ==
    ...
    if (defsRegister && targetRegister->isPendingSpillOnDef()) {
        ...
        if (self()->cg()->insideInternalControlFlow())
            self()->reverseSpillState(self()->cg()->getInstructionAtEndInternalControlFlow(), targetRegister,
    ...
    if (doBookKeeping && (assignedRegister->getState() != TR::RealRegister::Locked)) {
        ...
        if ((targetRegister->getFutureUseCount() == 0) || killOOLReg
            ...
            if (comp->getOption(TR_EnableTrueRegisterModel) && currInst->startOfLiveRange(targetRegister)
                ...
                traceMsg(comp,
                    "Start of live range for a non global virtual yet its future count is not zero. Reg=%s "
                    "Instr=[%p]\n",
                    self()->cg()->getDebug()->getName(targetRegister), currInst);
            }
#endif
    ...
    TR::Register* regPair, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (!comp->getOption(TR_EnableTrueRegisterModel) && freeRegisterLow == NULL
        && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount()) && freeRegisterHigh == NULL
    ...
    // True register model will mark register with a pending restoreSpillState and only when we see a def of this
    // register will we store to spill.
    if (freeRegisterLow == NULL && (comp->getOption(TR_EnableTrueRegisterModel)) && lastReg->isLive()
        && (lastReg->isValueLiveOnExit() || !lastReg->isNotUsedInThisBB()))
        lastReg->setPendingSpillOnDef();
    if (freeRegisterHigh == NULL && (comp->getOption(TR_EnableTrueRegisterModel)) && firstReg->isLive()
    ...
    // the same, and there is no assigned reg, we must have spilled the reg, so invert
    // the spill state to get the reg back
    if (freeRegisterHigh == NULL && !comp->getOption(TR_EnableTrueRegisterModel)
        && (firstReg->getTotalUseCount() != firstReg->getFutureUseCount())) {
        if (freeRegisterLow) {
        ...
        if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
                ...
                self()->coerceRegisterAssignment(
                    currInst, firstReg, (toRealRegister(tfreeRegisterHigh)->getRegisterNumber()), PAIRREG);
                self()->coerceRegisterAssignment(
                    currInst, lastReg, (toRealRegister(tfreeRegisterLow)->getRegisterNumber()), PAIRREG);
    ...
    if (currInst->defsAnyRegister(firstReg) && firstReg->isPendingSpillOnDef()) {
        if (self()->cg()->insideInternalControlFlow())
            self()->reverseSpillState(
                self()->cg()->getInstructionAtEndInternalControlFlow(), firstReg, toRealRegister(freeRegisterHigh));
        else
    ...
    if (currInst->defsAnyRegister(lastReg) && lastReg->isPendingSpillOnDef()) {
        if (self()->cg()->insideInternalControlFlow())
            self()->reverseSpillState(
                self()->cg()->getInstructionAtEndInternalControlFlow(), lastReg, toRealRegister(freeRegisterLow));
        else
    ...
    TR_RegisterKinds rk, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    if (rk != TR_FPR && rk != TR_VRF) {
        ...
        for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
            ...
            if (highWordPairIsFree
                && (_registerFile[i + 0]->getState() == TR::RealRegister::Free
                       || _registerFile[i + 0]->getState() == TR::RealRegister::Unlatched)
                && (_registerFile[i + 1]->getState() == TR::RealRegister::Free
                       || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched)
    ...
    if (enableHighWordRA && rk == TR_GPR64) {
        ...
        if (freeRegisterHigh != NULL
            && freeRegisterHigh->getHighWordRegister()->getState() == TR::RealRegister::Unlatched) {
        ...
        if (freeRegisterLow != NULL
            && freeRegisterLow->getHighWordRegister()->getState() == TR::RealRegister::Unlatched) {
    ...
    TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    if (bestCandidateHigh == NULL && bestCandidateLow == NULL) {
        ...
        traceMsg(
            comp, "OMR::Z::Machine::freeBestRegisterPair -- Ran out of regs to use as pair on Inst %p.\n", currInst);

        TR_ASSERT(
            0, "OMR::Z::Machine::freeBestRegisterPair -- Ran out of regs to use as a pair on Inst %p.\n", currInst);
    }
    ...
    if (bestVirtCandidateLow != NULL) {
        ...
        if (comp->getOption(TR_ForceLargeRAMoves) || bestVirtCandidateLow->is64BitReg()
            ...
            if (bestVirtCandidateLow->getKind() == TR_GPR64) {
                ...
                if (enableHighWordRA) {
                    if (bestVirtCandidateLow->is64BitReg()) {
                        ...
                        locationLow = self()->cg()->allocateSpill(
                            4, bestVirtCandidateLow->containsCollectedReference(), NULL, true);
                        opCodeLow = TR::InstOpCode::L;
                    }
                } else {
                    locationLow = self()->cg()->allocateSpill(TR::Compiler->om.sizeofReferenceAddress(),
        ...
        if (enableHighWordRA && rk == TR_GPR64
            ...
            // if the candidate is assigned on both GPR and HPR, need to spill HPR
            self()->cg()->traceRegisterAssignment(
                "spilling HPR %R for reg pair low", bestCandidateLow->getHighWordRegister()->getAssignedRegister());
            currInst = self()->freeHighWordRegister(currInst, bestCandidateLow->getHighWordRegister(), 0);
        }
    }
    ...
    if (bestVirtCandidateHigh != NULL) {
        ...
        if (comp->getOption(TR_ForceLargeRAMoves) || bestVirtCandidateHigh->is64BitReg()
            ...
            if (bestVirtCandidateHigh->getKind() == TR_GPR64) {
                ...
                if (enableHighWordRA) {
                    if (bestVirtCandidateHigh->is64BitReg()) {
                        ...
                        locationHigh = self()->cg()->allocateSpill(
                            4, bestVirtCandidateHigh->containsCollectedReference(), NULL, true);
                        opCodeHigh = TR::InstOpCode::L;
                    }
                } else {
                    locationHigh = self()->cg()->allocateSpill(TR::Compiler->om.sizeofReferenceAddress(),
        ...
        if (enableHighWordRA && rk == TR_GPR64
            ...
            // if the candidate is assigned on both GPR and HPR, need to spill HPR
            self()->cg()->traceRegisterAssignment(
                "spilling HPR %R for reg pair high", bestCandidateHigh->getHighWordRegister()->getAssignedRegister());
            currInst = self()->freeHighWordRegister(currInst, bestCandidateHigh->getHighWordRegister(), 0);
        }
    }
    ...
    TR::Register* virtualReg, uint64_t availRegMask, bool needsHighWord)
{
    ...
    if (comp->getOption(TR_Randomize)) {
        ...
        if (TR::RealRegister::isHPR((TR::RealRegister::RegNum)preference)) {
            randomPreference
                = self()->cg()->randomizer.randomInt(TR::RealRegister::FirstHPR, TR::RealRegister::LastHPR);
        ...
        // HighWord RA stuff
            ...
            if (virtualReg->is64BitReg() && !needsHighWord) {
                ...
                if (preference != 0 && (prefRegMask & availRegMask) && _registerFile[preference] != NULL) {
                    candidateLWFree = (_registerFile[preference]->getState() == TR::RealRegister::Free)
                        || (_registerFile[preference]->getState() == TR::RealRegister::Unlatched);
                    candidateHWFree
                ...
                if ((prefRegMask & availRegMask) && candidateLWFree && candidateHWFree
        ...
        if (!enableHighWordRA) {
            ...
            if ((_registerFile[i]->getState() == TR::RealRegister::Free
                ...
                if (comp->getOption(TR_Randomize)) {
                    ...
                    if (performTransformation(comp,
                            "O^O Random Codegen - Randomizing Weight for %s, Original bestWeightSoFar: %x randomized "
                            "to: %x\n",
            ...
            if (virtualReg->is64BitReg() && !needsHighWord) {
                bool candidateLWFree = (candidate->getState() == TR::RealRegister::Free)
                    || (candidate->getState() == TR::RealRegister::Unlatched);
                bool candidateHWFree = (candidate->getHighWordRegister()->getState() == TR::RealRegister::Free)
                ...
                if (candidateLWFree && candidateHWFree
                    ...
                    if (comp->getOption(TR_Randomize)) {
                        randomWeight = self()->cg()->randomizer.randomInt(0, 0xFFF);
                        if (performTransformation(comp,
                                "O^O Random Codegen - Randomizing Weight for %s, Original bestWeightSoFar: %x "
                                "randomized to: %x\n",
                                self()->cg()->getDebug()->getName(_registerFile[i]), bestWeightSoFar, randomWeight)) {
                            bestWeightSoFar = randomWeight;
                        }
 ...
 */
TR::RealRegister* OMR::Z::Machine::findVirtRegInHighWordRegister(TR::Register* virtReg)
{
...
void OMR::Z::Machine::allocateUpgradedBlockedList(TR_Stack<TR::RealRegister*>* mem) { _blockedUpgradedRegList = mem; }
...
bool OMR::Z::Machine::addToUpgradedBlockedList(TR::RealRegister* reg)
{
    if (reg == NULL)
        return false;
    self()->cg()->traceRegisterAssignment(
        "Adding %s (0x%p) to blocked reg list", getRegisterName(reg, self()->cg()), reg);
...
TR::RealRegister* OMR::Z::Machine::getNextRegFromUpgradedBlockedList()
{
    ...
    TR_ASSERT(reg->getAssignedRegister() == NULL, "Register %s (0x%p) from Blocked-list has assigned reg %p",
        getRegisterName(reg, self()->cg()), reg, reg->getAssignedRegister());
...
void OMR::Z::Machine::spillAllVolatileHighRegisters(TR::Instruction* currentInstruction)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (volatileReg && realReg->getState() == TR::RealRegister::Assigned
            && (virtReg = realReg->getAssignedRegister()) && virtReg->getKind() == TR_GPR64) {
...
TR::RealRegister* OMR::Z::Machine::freeBestRegister(TR::Instruction* currentInstruction, TR::Register* virtReg,
    TR_RegisterKinds rk, uint64_t availRegMask, bool allowNullReturn, bool doNotSpillToSiblingHPR)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (!enableHighWordRA) {
            if (realReg->getState() == TR::RealRegister::Assigned) {
                ...
                if (currentInstruction->getDependencyConditions()
                    ...
                    traceMsg(self()->cg(),
                        "  Reg[@%d] associatedVirtual[%s] was excluded from spill target because of reg dependency\n",
        ...
        // HighWord RA stuff
            ...
            if (virtReg->assignToHPR()) {
                ...
                bool assignedToTwoRegs
                    = false; // LW and HW are assigned to 2 different virtRegs, least preferred candidate
                if (realReg->getState() == TR::RealRegister::Assigned
                ...
                bool doNotSpillHPR
                    = realRegHW->getAssignedRegister() == virtReg && realReg->getAssignedRegister() != virtReg;

                /*
                // do not to spill an HPR that contains a compressed ref when shift !=0.
                // We have to first decompress the pointer, which means we need an extra 64-bit register or at least the
                low word register.
    ...
    while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != TR::InstOpCode::LABEL
        ...
        for (int32_t i = 0; i < numCandidates; i++) {
            ...
            // also need to check HW. if it's assigned, push the candidate back
                ...
                if (candidateHW->getState() == TR::RealRegister::Assigned && candidateHW->getAssignedRegister()) {
    ...
    // todo fix this
    else if (enableHighWordRA && virtReg->is64BitReg() && best->isLowWordRegister()
        ...
        self()->cg()->traceRegisterAssignment(
            "HW RA: freeBestReg Spill %R for fullsize reg: %R ", best->getHighWordRegister(), virtReg);
    ...
    if (enableHighWordRA && (virtReg->is64BitReg() || doNotSpillToSiblingHPR)) {
        uint32_t availHighWordRegMap
            = ~(toRealRegister(best->getHighWordRegister())->getRealRegisterMask()) & availRegMask & 0xffff0000;
 ...
 */
void OMR::Z::Machine::freeRealRegister(
    TR::Instruction* currentInstruction, TR::RealRegister* targetReal, bool is64BitReg)
{
    TR::Compilation* comp = self()->cg()->comp();
    ...
    if (virtReg) {
        if (enableHighWordRA && is64BitReg) {
            uint32_t availHighWordRegMap
                = ~(toRealRegister(targetReal->getHighWordRegister())->getRealRegisterMask()) & 0xffff0000;
...
TR::Instruction* OMR::Z::Machine::freeHighWordRegister(
    TR::Instruction* currentInstruction, TR::RealRegister* targetRegisterHW, flags32_t instFlags)
{
    ...
    if (virtRegHW->getAssignedRegister() == NULL && targetRegisterHW->getState() == TR::RealRegister::Assigned) {
        self()->cg()->traceRegisterAssignment(
            " HW RA %R was spilled to %R, now need to spill again", virtRegHW, targetRegisterHW);

        // try to find another free HPR
        spareReg = self()->findBestFreeRegister(
            currentInstruction, rk, virtRegHW, self()->cg()->getAvailableHPRSpillMask(), true);

        if (spareReg) {
            cursor
                = self()->registerCopy(currentInstruction, TR_HPR, targetRegisterHW, spareReg, self()->cg(), instFlags);
    ...
    if (targetRegisterHW->getState() == TR::RealRegister::Blocked) {
        if (spareReg == NULL) {
            virtRegHW->block();
            spareReg
                = self()->freeBestRegister(currentInstruction, virtRegHW, rk, self()->cg()->getAvailableHPRSpillMask());
            virtRegHW->unblock();
        }
        ...
        if (spareReg) {
            cursor
                = self()->registerCopy(currentInstruction, TR_HPR, targetRegisterHW, spareReg, self()->cg(), instFlags);
            // TR_ASSERTC( spareReg->isHighWordRegister(),self()->cg()->comp(), "\nfreeHighWordRegister: spareReg is not
            // an HPR?\n");
            spareReg->setAssignedRegister(virtRegHW);
            spareReg->setState(TR::RealRegister::Assigned);
            virtRegHW->setAssignedRegister(spareReg);
 ...
 *  A GPRX, ....      // use GPRX
 */
void OMR::Z::Machine::spillRegister(
    TR::Instruction* currentInstruction, TR::Register* virtReg, uint32_t availHighWordRegMap)
{
    TR::Compilation* comp = self()->cg()->comp();
    ...
    if (virtReg->containsCollectedReference() && TR::Compiler->target.is64Bit() && comp->useCompressedPointers()
        && TR::Compiler->vm.heapBaseAddress() == 0 && TR::Compiler->om.compressedReferenceShift() == 0
    ...
    if (enableHighWordRA && !comp->getOption(TR_DisableHPRSpill)
        ...
        && best->isLowWordRegister() && !virtReg->containsInternalPointer() && !self()->cg()->isOutOfLineColdPath()) {
        ...
        if (freeHighWordReg) {
            ...
            if (alreadySpilledToHPR) {
                cursor = generateExtendedHighWordInstruction(
                    currentNode, self()->cg(), TR::InstOpCode::LHHR, best, freeHighWordReg, 0, currentInstruction);
            } else {
                if (virtReg->containsCollectedReference()) {
                    if (debugHPR) {
                        printf("(collected) ");
                    ...
                    if (compressShift == 0) {
                        cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LLHFR,
                            best, freeHighWordReg, 0, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                        cursor
                            = generateRILInstruction(self()->cg(), TR::InstOpCode::IIHF, currentNode, best, 0, cursor);
                        self()->cg()->traceRAInstruction(cursor);
                    } else {
                        cursor = generateRIEInstruction(self()->cg(), TR::InstOpCode::RISBLG, currentNode, best,
                            freeHighWordReg, 0, 31 + 0x80 - compressShift, 32 + compressShift, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                        cursor = generateRIEInstruction(self()->cg(), TR::InstOpCode::RISBHG, currentNode, best,
                            freeHighWordReg, 32 - compressShift, 31 + 0x80, 32 + compressShift, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                    }
                } else {
                    cursor = generateExtendedHighWordInstruction(
                        currentNode, self()->cg(), TR::InstOpCode::LLHFR, best, freeHighWordReg, 0, currentInstruction);
                }
            }
            if (debugHPR) {
...

    {
        ...
        if (enableHighWordRA && alreadySpilledToHPRCollectible) {
            ...
            cursor
                = generateRXYInstruction(self()->cg(), TR::InstOpCode::LFH, currentNode, best, mr, currentInstruction);
    ...
    TR::Instruction* currentInstruction, TR::Register* spilledRegister, TR::RealRegister* targetRegister)
{
    ...
    if (freeHighWordReg) {
        if (targetRegister->isHighWordRegister()) {
            cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LHHR,
                freeHighWordReg, targetRegister, 0, currentInstruction);
        } else {
            if (spilledRegister->containsCollectedReference()) {
                uint32_t compressShift = TR::Compiler->om.compressedReferenceShift();
                // need to assert heapbase, offset = 0
                if (compressShift == 0) {
                    cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LHLR,
                        freeHighWordReg, targetRegister, 0, currentInstruction);
                } else {
                    cursor = generateRIEInstruction(self()->cg(), TR::InstOpCode::RISBHG, currentNode, freeHighWordReg,
                        targetRegister, 0, 31 + 0x80, 32 - compressShift, currentInstruction);
                }
            } else {
                cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LHLR,
                    freeHighWordReg, targetRegister, 0, currentInstruction);
    ...
    if (needMVHI) {
        ...
        self()->cg()->traceRAInstruction(cursor);
        cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::STFH, currentNode, targetRegister,
        ...
        if (opCode == TR::InstOpCode::VST)
            cursor = generateVRXInstruction(
                self()->cg(), opCode, currentNode, targetRegister, tempMR, 0, currentInstruction);
        else
            cursor
                = generateRXInstruction(self()->cg(), opCode, currentNode, targetRegister, tempMR, currentInstruction);
...
TR::Instruction* OMR::Z::Machine::coerceRegisterAssignment(TR::Instruction* currentInstruction,
    TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber, flags32_t instFlags)
{
    ...
    TR_RegisterKinds currentAssignedRegisterRK
        = rk; // used for register Copy, todo: use it for freeBestReg and findFreeReg
    ...
    // the target reg is free
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            if (targetRegister->isHighWordRegister() && !virtualRegister->isPlaceholderReg()) {
                // this could happen for OOL, when the reg deps on the top of slow path dictates that a collectible
                // register must be spilled to a specific HPR
                TR_ASSERT(
                    virtualRegister->containsCollectedReference(), " OOL HPR spill: spilling a 64 bit scalar into HPR");
                ...
                if (virtualRegister->getAssignedRegister() == NULL && currentHighWordReg) {
                    ...
                    cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LHHR,
                        currentHighWordReg, targetRegister, 0, currentInstruction);
                    self()->cg()->traceRAInstruction(cursor);
                    ...
                    if (currentAssignedRegister == NULL) {
                        if (virtualRegister->getTotalUseCount() != virtualRegister->getFutureUseCount()
                            ...
                            TR::MemoryReference* tempMR = generateS390MemoryReference(
                                currentNode, virtualRegister->getBackingStorage()->getSymbolReference(), self()->cg());

                            // is the offset correct?  +4 big endian?
                            TR::MemoryReference* mr = generateS390MemoryReference(*tempMR, 4, self()->cg());

                            cursor = generateSILInstruction(
                                self()->cg(), TR::InstOpCode::MVHI, currentNode, tempMR, 0, currentInstruction);
                            self()->cg()->traceRAInstruction(cursor);
                            cursor = generateRXInstruction(
                                self()->cg(), TR::InstOpCode::STFH, currentNode, targetRegister, mr, cursor);
                            self()->cg()->traceRAInstruction(cursor);

                            // fix up states
                            // don't need to worry about protecting backing storage because we are leaving cold path OOL
                            // now
                            ...
                            TR_ASSERT(comp,
                                " OOL HPR spill: currentAssignedRegister is NULL but virtual reg is not spilled?");
                        }
                    } else {
                        ...
                        TR_ASSERT(currentAssignedRegister->isLowWordRegister(),
                            " OOL HPR spill: 64-bit reg assigned to HPR and is not spilled to HPR");
                        cursor = generateExtendedHighWordInstruction(currentNode, self()->cg(), TR::InstOpCode::LLHFR,
                            currentAssignedRegister, targetRegister, 0, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                        cursor = generateRILInstruction(
                            self()->cg(), TR::InstOpCode::IIHF, currentNode, currentAssignedRegister, 0, cursor);
                        self()->cg()->traceRAInstruction(cursor);
        ...
        if (currentAssignedRegister == NULL) {
            ...
            cursor = self()->registerCopy(currentInstruction, currentAssignedRegisterRK, currentAssignedRegister,
                targetRegister, self()->cg(), instFlags);
            if (enableHighWordRA && virtualRegister->is64BitReg()) {
                currentAssignedRegisterHW->setState(TR::RealRegister::Free);
                currentAssignedRegisterHW->setAssignedRegister(NULL);
            }
    ...
    // the target reg is blocked so we need to guarantee it stays in a reg
        ...
        if (spareReg == NULL) {
            ...
            if (enableHighWordRA && virtualRegister->is64BitReg()) {
                // if we end up spilling currentTargetVirtual to HPR, make sure to not pick the HPR of targetRegister
                // since it will not free up the full 64 bit register
                ...
                // we can allow freeBestRegister() to return NULL here, as long as we can perform register exchange
                // later via memory and take a bad OSC penalty todo: fix HW RA to enable register exchange later
                spareReg = self()->freeBestRegister(
                    currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind(), 0xffff, true);
            }
            virtualRegister->unblock();
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            ...
            // HPRx and we are attempting to coerce it to GPRx. In this case we do not need to spill HPRx, rather just
            // copy it
            if (targetRegisterHW->getState() != TR::RealRegister::Free
                && targetRegisterHW->getState() != TR::RealRegister::Unlatched
                ...
                if (targetRegisterHW->getAssignedRegister()
                    ...
                    // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),self()->cg()->comp(), "currentTargetVirtual is
                    // not LWOnly but HW is clobbered by another vreg?");
                    virtualRegister->block();
                    currentTargetVirtual->block();
        ...
        if (currentAssignedRegister != NULL) {
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)) {
                if (enableHighWordRA && spareReg == NULL && currentTargetVirtualRK != currentAssignedRegisterRK) {
                    ...
                    cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LG, currentNode,
                        currentAssignedRegister, tempMR, currentInstruction);
                    self()->cg()->traceRAInstruction(cursor);
                    cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LGR, currentNode, targetRegister,
                        currentAssignedRegister, currentInstruction);
                    self()->cg()->traceRAInstruction(cursor);
                    TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, self()->cg());
                    cursor = generateRXInstruction(
                        self()->cg(), TR::InstOpCode::STG, currentNode, targetRegister, tempMR2, currentInstruction);
                    self()->cg()->traceRAInstruction(cursor);
                    ...
                    //
                    // hence the statement in the if block below would set currentTargetVirtual to point to the
                    // High-Word of the currentAssignedRegister which is not what we want (since it is a 64-bit value).
                    ...
                    cursor = self()->registerCopy(
                        currentInstruction, currentTargetVirtualRK, targetRegister, spareReg, self()->cg(), instFlags);
                    cursor = self()->registerCopy(currentInstruction, currentAssignedRegisterRK,
                    ...
                    if (enableHighWordRA && currentTargetVirtual->is64BitReg()) {
                        ...
                        spareReg->getHighWordRegister()->setState(
                            TR::RealRegister::Assigned); // no need to block the HW half, we already blocked the Lw
                ...
                if (enableHighWordRA) {
                    cursor = self()->registerExchange(currentInstruction, currentAssignedRegisterRK, targetRegister,
                        currentAssignedRegister, spareReg, self()->cg(), instFlags);
                    if (currentTargetVirtual->is64BitReg()) {
                        // currentAssignedRegister->getHighWordRegister()->setState(TR::RealRegister::Blocked);  //not
                        // necessary
                    ...
                    cursor = self()->registerExchange(currentInstruction, rk, targetRegister, currentAssignedRegister,
    ...
    // the target reg is assigned
        ...
        if (enableHighWordRA && currentTargetVirtual->getAssignedRegister() == NULL
            && targetRegister->isHighWordRegister()) {
            self()->cg()->traceRegisterAssignment(
                " HW RA %R was spilled to %R, now need to spill again", currentTargetVirtual, targetRegister);
            ...
            if (virtualRegister->is64BitReg() && !virtualRegister->isPlaceholderReg()) {
                ...
                if (currentAssignedRegister == NULL) {
                    ...
                    if (currentHighWordReg) {
                        ...
                        TR_ASSERT(virtualRegister->getBackingStorage(),
            ...
            if (enableHighWordRA && virtualRegister->is64BitReg())
                spareReg
                    = self()->findBestFreeRegister(currentInstruction, rk, currentTargetVirtual, availHighWordRegMap);
            else
        ...
        if (currentAssignedRegister != NULL) {
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister) || enableHighWordRA) {
                ...
                if (spareReg == NULL) {
                    ...
                    virtualRegister->block();
                    if (enableHighWordRA && virtualRegister->is64BitReg()) {
                        // if we end up spilling currentTargetVirtual to HPR, make sure to not pick the HPR of
                        // targetRegister since it will not free up the full 64 bit register
                        spareReg = self()->freeBestRegister(currentInstruction, currentTargetVirtual,
                            currentTargetVirtual->getKind(), availHighWordRegMap, false, true);
                    } else {
                        spareReg = self()->freeBestRegister(currentInstruction, currentTargetVirtual,
                    ...
                    // we must not generate register move of LGR targetRegister,spareReg because they will be the same
                    // register and we must leave currentTargetVirtual spilled instead of assigning it to spareReg.
                ...
                if (enableHighWordRA && virtualRegister->is64BitReg()) {
                    if (targetRegisterHW->getState() != TR::RealRegister::Free
                        ...
                        if (targetRegisterHW->getAssignedRegister()
                            && targetRegisterHW->getAssignedRegister() != targetRegister->getAssignedRegister()) {
                            // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),self()->cg()->comp(),
                            // "currentTargetVirtual is not LWOnly but HW is clobbered by another vreg?");
                            virtualRegister->block();
                            currentTargetVirtual->block();
                            spareReg->block(); // to do: is this necessary? only need to block HPR?
                            // free up this highword register by either spilling it or moving it
                ...
                if (spareReg == NULL) {
                    ...
                    cursor = self()->registerCopy(
                        currentInstruction, rk, currentAssignedRegister, targetRegister, self()->cg(), instFlags);
                    currentAssignedRegister->setState(TR::RealRegister::Unlatched);
                    currentAssignedRegister->setAssignedRegister(NULL);
                } else {
                    cursor = self()->registerExchange(currentInstruction, rk, targetRegister, currentAssignedRegister,
            ...
            if (spareReg == NULL && virtualRegister->isPlaceholderReg()) {
                ...
                if (spareReg == NULL) {
                    ...
                    // If currentTargetVirtual is 64-bit, spareReg will be a GPR (targetRegister's low word)
                    // however, targetRegister could be an HPR because we might only need its high word. In this case
                    // we must not generate register move of LGR targetRegister,spareReg because they are the same
                    // register and we must leave currentTargetVirtual spilled instead of assigning it to spareReg.
                    if (targetRegister->isHighWordRegister() && currentTargetVirtual->is64BitReg()
                        && targetRegister->getRegisterNumber() == spareReg->getHighWordRegister()->getRegisterNumber())
                        doNotRegCopy = true;
...
void OMR::Z::Machine::initializeRegisterFile()
{
    ...
    _registerFile[TR::RealRegister::HPR0] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR0, TR::RealRegister::HPR0Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR1] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR1, TR::RealRegister::HPR1Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR2] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR2, TR::RealRegister::HPR2Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR3] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR3, TR::RealRegister::HPR3Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR4] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR4, TR::RealRegister::HPR4Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR5] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR5, TR::RealRegister::HPR5Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR6] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR6, TR::RealRegister::HPR6Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR7] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR7, TR::RealRegister::HPR7Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR8] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR8, TR::RealRegister::HPR8Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR9] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR9, TR::RealRegister::HPR9Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR10] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR10, TR::RealRegister::HPR10Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR11] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR11, TR::RealRegister::HPR11Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR12] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR12, TR::RealRegister::HPR12Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR13] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR13, TR::RealRegister::HPR13Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR14] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR14, TR::RealRegister::HPR14Mask, self()->cg());

    _registerFile[TR::RealRegister::HPR15] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR15, TR::RealRegister::HPR15Mask, self()->cg());
...
uint32_t* OMR::Z::Machine::initializeGlobalRegisterTable()
{
    ...
    //
    // [1]
    // https://github.com/eclipse/omr/blob/9d1d8cf3048781bc6d87e6a1079167586cc5aa4d/compiler/codegen/CodeGenRA.cpp#L2691-L2702
    // [2]
    // https://github.com/eclipse/omr/blob/9d1d8cf3048781bc6d87e6a1079167586cc5aa4d/compiler/codegen/CodeGenRA.cpp#L2889-L2903
    // [3]
    // https://github.com/eclipse/omr/blob/9d1d8cf3048781bc6d87e6a1079167586cc5aa4d/compiler/z/codegen/ControlFlowEvaluator.cpp#L1098-L1102
...
TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalHPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
...
TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalHPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
...
TR::Register* OMR::Z::Machine::getGPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg)
{
    auto firstGlobalGPR = self()->getFirstGlobalGPRRegisterNumber();
    auto firstGlobalHPR = self()->getFirstGlobalHPRRegisterNumber();

    if (firstGlobalHPR != -1 && reg >= firstGlobalGPR && reg <= firstGlobalHPR
...
TR::Register* OMR::Z::Machine::getHPRFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg)
{
    auto firstGlobalHPR = self()->getFirstGlobalHPRRegisterNumber();
    auto lastGlobalHPR = self()->getLastGlobalHPRRegisterNumber();

    if (firstGlobalHPR != -1 && reg >= firstGlobalHPR && reg <= lastGlobalHPR
        && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
...
void OMR::Z::Machine::createRegisterAssociationDirective(TR::Instruction* cursor)
{
    ...
    if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        ...
        associations = new (self()->cg()->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
            TR::RegisterDependencyConditions(0, lastHPR, self()->cg());
    } else {
        associations = new (self()->cg()->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
    ...
    TR::list<TR::Register*>* spilledRegisterList)
{
    ...
    if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR; i++) {
            if (self()->getRealRegister(i)->getState() == TR::RealRegister::Assigned
                && self()->getRealRegister(i)->getAssignedRegister()) {
                if (self()->getRealRegister(i)->getAssignedRegister()
                    != self()->getRealRegister(i)->getLowWordRegister()->getAssignedRegister())
                    c++;
                // if a HPR is assigned to a virtReg but a virtReg is not assigned to HPR, we must have spilled the
                // virtReg to HPR
        ...
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR; i++) {
                if (self()->getRealRegister(i)->getState() == TR::RealRegister::Assigned
                    ...
                    if (self()->getRealRegister(i)->getAssignedRegister()
                        != self()->getRealRegister(i)->getLowWordRegister()->getAssignedRegister()) {
                        deps->addPostCondition(self()->getRealRegister(i)->getAssignedRegister(),
                            self()->getRealRegister(i)->getRegisterNumber());
                        self()->getRealRegister(i)->getAssignedRegister()->incFutureUseCount();
                    }
                    // if a HPR is assigned to a virtReg but a virtReg is not assigned to HPR, we must have spilled the
                    // virtReg to HPR
                    if (!self()->getRealRegister(i)->getAssignedRegister()->getAssignedRegister()) {
                        // this is not a conflicting dependency rather a directive to tell RA to mark the virt reg as a
                        // HPR spill
