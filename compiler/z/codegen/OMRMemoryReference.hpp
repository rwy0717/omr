...
private:
	 ...
	 * instructions use of the symbol. In these cases the size from the original rootLoadOrStore node's symRef is
...
public:
	 ...
	 *
	 * \note
	 *    The offset may not represent the final displacement which will be used to encode the memory reference
	 * until the binary encoding of this memory reference is generated.
	 */
	 ...
	 * underlying loadaddr symRef. This loadaddr symRef does not reflect the actual size of the symbol being loaded
	 * so the fixed _fixedSizeForAlignment must be used instead.
	 */
	 ...
	 *
	 * \return
	 *    `true` if this memory reference requires long displacement; `false` if long displacement may not be
	 * required.
	 *
	 * \note
	 *    A return value of `false` does not imply long displacement is not required. This is because the
	 * displacement of this memory reference is not fully known until binary encoding. Only after the binary
	 * encoding of this memory reference will a return value of `false` indicate that long displacement is not
	 * required. The offset may increase by some amount during code generation, however it will never decrease. This
	 * is why a return value of `true` always implies long displacement will be required.
	 */
	 ...
	 *
	 * \return
	 *    `true` if this memory reference requires huge displacement; `false` if huge displacement may not be
	 * required.
	 *
	 * \note
	...
	TR::Instruction *separateIndexRegister(TR::Node *subTree, TR::CodeGenerator *cg,
	        bool enforce4KDisplacementLimit, TR::Instruction *preced, bool forceDueToAlignmentBump = false);
	...
	TR::Instruction *enforceVRXFormatLimits(
	        TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *preced); // revert
