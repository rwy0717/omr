...
public:
    ...
    virtual TR::RealRegister::RegNum setEnvironmentPointerRegister(TR::RealRegister::RegNum r)
    {
    ...
    {
        return getRealRegister(_environmentPointerRegister);
    }
...
public:
    ...
    {
        return (getFrameType() == TR_XPLinkStackLeafFrame) || (getFrameType() == TR_XPLinkNoStackLeafFrame);
    }
    virtual int32_t getOutgoingParameterBlockSize();

    virtual TR::Register* loadEnvironmentIntoRegister(
        TR::Node* callNode, TR::RegisterDependencyConditions* deps, bool loadIntoEnvironmentReg);
    ...
    static uint32_t shiftFloatParmDescriptorFlag(uint32_t fieldVal, int32_t floatParmNum)
    {
        return (fieldVal) << (6 * (3 - floatParmNum));
    } // accepts floatParmNum values 0,1,2,3
    bool updateFloatParmDescriptorFlags(uint32_t* parmDescriptorFields, TR::Symbol* funcSymbol, int32_t parmCount,
        int32_t argSize, TR::DataType dataType, int32_t* floatParmNum, uint32_t* lastFloatParmAreaOffset,
        uint32_t* parmAreaOffset);

    static uint32_t getFloatParmDescriptorFlag(uint32_t descriptorFields, int32_t floatParmNum)
    {
