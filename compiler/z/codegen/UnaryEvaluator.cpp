...
TR::Register* lconstHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "lconstHelper64() is for 64bit only!");
...
#ifdef J9_PROJECT_SPECIFIC
TR::Register* OMR::Z::TreeEvaluator::o2xEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::x2oEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* destMR
        = generateS390MemRefFromStorageRef(node, targetStorageReference, cg, false); // enforceSSLimits=false

    if (cg->traceBCDCodeGen())
        traceMsg(comp, "\tx2oEval : %s (%p) srcSize = %d (srcAddrPrec %d), destNode->aggrSize = %d\n",
            node->getOpCode().getName(), node, srcNode->getSize(),
            srcNode->getType().isAddress() ? (TR::Compiler->target.is64Bit() ? 8 : 4) : -1, dstAggrSize);
    ...
    if (storeSize > srcSize) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tisWidening=true : clear upper %lld bytes, set storeSize to %d, add %lld to destMR offset "
                "(dstAggrSize %d > srcIntSize %d)\n",
    ...
    case 3:
        generateRSInstruction(
            cg, TR::InstOpCode::STCM, node, srcIsRegPair ? srcRegLow : srcReg64, (uint32_t)0x7, destMR);
        break;
    case 4:
    ...
    case 6:
    case 7:
        generateRSYInstruction(cg, srcIsRegPair ? TR::InstOpCode::STCM : TR::InstOpCode::STCMH, node,
            srcIsRegPair ? srcRegHi : srcReg64, (uint32_t)((1 << (dstAggrSize - 4)) - 1), destMR);
        generateRXInstruction(cg, TR::InstOpCode::ST, node, srcIsRegPair ? srcRegLow : srcReg64,
...
TR::Register* OMR::Z::TreeEvaluator::labsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        TR_ASSERT(
            targetRegister->getRegisterPair() != NULL, "in 32-bit mode, a 64-bit value is not in a register pair");
