...
// Non-Java use
void TR::S390SystemLinkage::initParamOffset(TR::ResolvedMethodSymbol* method, int32_t stackIndex, List<TR::ParameterSymbol>* parameterList)
{
 ...
 * determined.
 */
bool TR::S390zOSSystemLinkage::updateFloatParmDescriptorFlags(uint32_t* parmDescriptorFields, TR::Symbol* funcSymbol, int32_t parmCount, int32_t argSize, TR::DataType dataType, int32_t* floatParmNum, uint32_t* lastFloatParmAreaOffset, uint32_t* parmAreaOffset)
{
 ...
 */
void TR::S390SystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps, intptrj_t targetAddress,
 ...
 * Front-end customization of callNativeFunction
 */
void TR::S390zLinuxSystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps, intptrj_t targetAddress,
    TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg, TR::LabelSymbol* returnFromJNICallLabel,
    TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
    TR::CodeGenerator* codeGen = cg();
...
void TR::S390SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if ((ft == noStackLeafFrame || ft == StackLeafFrame || ft == noStackForwardingFrame) && performTransformation(comp(), "O^O Generating method as leaf of type %d\n", ft)) {
        setFrameType(ft);
...
{
    return isFastLinkLinkageType() && (aggregateLenth > (getNumIntegerArgumentRegisters() * cg()->machine()->getGPRSize()));
}
