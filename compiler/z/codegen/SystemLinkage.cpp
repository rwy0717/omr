...
TR::SystemLinkage* TR::S390SystemLinkage::self() { return static_cast<TR::SystemLinkage*>(this); }

void TR::S390SystemLinkage::initS390RealRegisterLinkage()
{
...
// Non-Java use
void TR::S390SystemLinkage::initParamOffset(
 ...
 */
TR::Instruction* TR::S390SystemLinkage::getputFPRs(
    TR::InstOpCode::Mnemonic opcode, TR::Instruction* cursor, TR::Node* node, TR::RealRegister* spReg)
{
    int16_t FPRSaveMask;
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(
                cg(), opcode, node, getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), rsa, cursor);
            j++;
        }
    }
...
TR::Instruction* TR::S390SystemLinkage::getputHPRs(
    TR::InstOpCode::Mnemonic opcode, TR::Instruction* cursor, TR::Node* node, TR::RealRegister* spReg)
{
    int16_t HPRSaveMask;
    int32_t offset, firstSaved, lastSaved, beginSaveOffset;
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        ...
        if (previousWasOne && (i == lastSaved || !(HPRSaveMask & (1 << i)))) {
            ...
            cursor = generateRSInstruction(cg(), opcode, node, getRealRegister(REGNUM(k + TR::RealRegister::FirstGPR)),
 ...
 */
TR::Instruction* TR::S390SystemLinkage::addImmediateToRealRegister(TR::RealRegister* targetReg, int32_t value,
 ...
 */
uint16_t TR::S390SystemLinkage::flipBitsRegisterSaveMask(uint16_t mask)
{
 ...
 */
uint32_t TR::S390zOSSystemLinkage::calculateInterfaceMappingFlags(TR::ResolvedMethodSymbol* method)
{
    ...
    while ((parmCursor != NULL) && !done) {
        ...
        done = updateFloatParmDescriptorFlags(&parmDescriptorFields, funcSymbol, parmCount, argSize, dataType,
 ...
 */
uint32_t TR::S390zOSSystemLinkage::calculateReturnValueAdjustFlag(TR::DataType dataType, int32_t aggregateLength)
{
 ...
 * determined.
 */
bool TR::S390zOSSystemLinkage::updateFloatParmDescriptorFlags(uint32_t* parmDescriptorFields, TR::Symbol* funcSymbol,
    int32_t parmCount, int32_t argSize, TR::DataType dataType, int32_t* floatParmNum, uint32_t* lastFloatParmAreaOffset,
...
TR::Register* TR::S390zOSSystemLinkage::loadEnvironmentIntoRegister(
    TR::Node* callNode, TR::RegisterDependencyConditions* deps, bool loadIntoEnvironmentRegister)
{
    ...
    if (loadIntoEnvironmentRegister) {
        TR::Register* systemEnvironmentRegister
            = deps->searchPostConditionRegister(getEnvironmentPointerRegister()); // gpr 5
 ...
 */
void TR::S390SystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
 ...
 */
TR::Register* TR::S390SystemLinkage::callNativeFunction(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
    return 0;
...
#if defined(PYTHON) || defined(OMR_RUBY) || defined(JITTEST)
    {
        ...
        if (callNode->getOpCode().isIndirect()) {
            ...
            generateRRInstruction(
                codeGen, TR::InstOpCode::BASR, callNode, systemReturnAddressRegister, targetReg, deps);
        } else {
...
void TR::S390SystemLinkage::mapStack(TR::ResolvedMethodSymbol* method)
{
    {
        mapStack(method, 0);
    }
 ...
 */
void TR::S390SystemLinkage::mapStack(TR::ResolvedMethodSymbol* method, uint32_t stackIndex)
{
    int32_t i;
...
TR::Instruction* TR::S390SystemLinkage::saveGPRsInPrologue2(TR::Instruction* cursor)
{
    ...
    for (i = lastReg = firstReg = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR - 2;
    ...
    // Original Strategy --- use 1 store multiple operation that goes from 1st to last preserved reg.  We will preserve
    // regs in between even if they aren't assigned.
    ...
    // r14 is a special case.  It is the return value register.  Since we will have to save/restore it often, we will
    // try to reduce the range above by just emiting its own save/restore
    ...
    if (getPreserved(REGNUM(i)) && (getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getStoreOpCode(), firstNode, getRealRegister(REGNUM(i)), retAddrMemRef, cursor);
    }

    // r15 (stack pointer) might need to be put in to the register mask as well.  Other code will deal with how we
    // save/restore it.
...
TR::Instruction* TR::S390SystemLinkage::saveGPRsInPrologue(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    ...
    if (!hasSavedSP && !getIsLeafRoutine()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getStoreOpCode(), firstNode, getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    }
...
TR::Linkage::FrameType TR::S390zLinuxSystemLinkage::checkLeafRoutine(
    int32_t stackFrameSize, TR::Instruction** callInstruction)
{
    ...
    if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(), "stackFrameSize = %d  default 0 arg and 0 local stack frame size = %d\n", stackFrameSize,
            zeroArgsAndLocalsStackFrameSize);
    ...
    if (spReg->isUsedInMemRef()
        || getHPRSaveMask()) // don't need to check gprmask or hpr mask as these are saved in caller stack.
    {
        if (comp()->getOption(TR_TraceCG))
...
#define ZLINUX_SYSTEMLINK_LEAF_INSTRUCTION_CHECK_THRESHOLD 150
    ...
    bool hasCalls
        = findPossibleCallInstruction(cursor, numToCheck, callInstruction, regs, TR::RealRegister::NumRegisters);
...
bool OMR::Z::Linkage::checkPreservedRegisterUsage(bool* regs, int32_t regsSize)
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR - 2;
         ++i) // special case for r14, the return value
    {
        // TR_ASSERT(regs[i] == (getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod(), "Mismatch between register
    ...
    // check r14 usage -- don't check if it has been assigned because it could be used in a call. the regs array passed
    // in will take this in to account.
...
void TR::S390SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
            "initial stackFrameSize: stack size %d = offset to first parm %d + arg size %d + localSize %d, rounded up "
            "to 8\n",
    ...
    if (!(getFrameType() == noStackForwardingFrame || getFrameType() == noStackLeafFrame
        ...
        if (!largeStack) {
            ...
            cursor = generateRXYInstruction(cg(), TR::InstOpCode::LAY, firstNode, spReg,
            ...
            if (largeStack) {
                ...
                cursor
                    = generateRRInstruction(cg(), TR::InstOpCode::getAddRegOpCode(), firstNode, spReg, tempReg, cursor);
            } else {
                cursor = generateRIInstruction(
                    cg(), TR::InstOpCode::getAddHalfWordImmOpCode(), firstNode, spReg, (stackFrameSize) * -1, cursor);
            }
        }
    } else {
        if (comp()->getOption(TR_TraceCG))
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(cg(), TR::InstOpCode::STD, firstNode,
                getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), retAddrMemRef, cursor);
        }
    }
...
TR::Instruction* TR::S390SystemLinkage::restoreGPRsInEpilogue2(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    int32_t offset = 0;
    ...
    // Original Strategy --- use 1 load multiple operation that goes from 1st to last preserved reg.  We will preserve
    // regs in between even if they aren't assigned.
    ...
    if (lastReg - firstReg == 0)
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), nextNode, getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
    else if (lastReg > firstReg)
    ...
    if (getPreserved(REGNUM(i)) && (getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset + stackFrameSize, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), nextNode, getRealRegister(REGNUM(i)), retAddrMemRef, cursor);
    }

    return cursor;
}

TR::Instruction* TR::S390SystemLinkage::restoreGPRsInEpilogue(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    ...
    if (!hasRestoreSP && !getIsLeafRoutine()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset + stackFrameSize, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), nextNode, getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    }
    return cursor;
}
...
void TR::S390SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(cg(), TR::InstOpCode::LD, nextNode,
                getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), retAddrMemRef, cursor);
    ...
    cursor = generateS390RegInstruction(
        cg(), TR::InstOpCode::BCR, nextNode, getRealRegister(REGNUM(TR::RealRegister::GPR14)), cursor);
 ...
 */
void OMR::Z::Linkage::setUsedRegisters(TR::Instruction* instruction, bool* regs, int32_t regsSize)
{
    if (!regs || regsSize == 0)
        return;
    ...
    if (instruction->getRegisters(usedRegs)) {
        for (auto cursor = usedRegs.begin(); cursor != usedRegs.end(); ++cursor) {
            if ((*cursor) && (*cursor)->getRealRegister()) {
                ...
                TR_ASSERT(
                    regNum < regsSize, "Trying to set regNum %d to true when max regsSize = %d\n", regNum, regsSize);
 ...
 *        On Output: Instruction found or instruction just outside window (or NULL if no more -- this is the successful
 * indicator that this may be a leaf routine)
...
bool OMR::Z::Linkage::findPossibleCallInstruction(
    TR::Instruction*& cursor, int32_t& numToCheck, TR::Instruction** callInstruction, bool* regs, int32_t regsSize)

{
    ...
    if (result && canReplaceCallWithJump && *callInstruction && n < numToCheck) {
        ...
        if (opCode == TR::InstOpCode::BASR) // don't know which instruction to use for a virtual dispatch (yet).  In
                                            // theory, BCR.
        {
            canReplaceCallWithJump = false;
...
bool TR::S390zLinuxSystemLinkage::isSymbolPassedByReference(TR::Symbol* sym)
{
    ...
    if (sym->getDataType() != TR::Aggregate) // any non-aggregate symbols that have a data type that can be passed by
                                             // reference *will* be passed by reference
        return true;
...
bool TR::Z::ZOSBaseSystemLinkage::isAggregateReturnedInIntRegistersAndMemory(int32_t aggregateLenth)
{
    return isFastLinkLinkageType()
        && (aggregateLenth > (getNumIntegerArgumentRegisters() * cg()->machine()->getGPRSize()));
}

bool TR::S390zOSSystemLinkage::isAggregateReturnedInIntRegisters(int32_t aggregateLenth)
{
 ...
 */
uint32_t TR::S390zOSSystemLinkage::calculateCallDescriptorFlags(TR::Node* callNode)
{
#define XPLINK_FLOAT_PARM_UNPROTYPED_CALL 0x08
    ...
    if (!isPrototyped) {
        parmDescriptorFields = shiftFloatParmDescriptorFlag(
            XPLINK_FLOAT_PARM_UNPROTYPED_CALL, 0); // FPR0 field has special value for unprototped call
        callDescValue |= parmDescriptorFields;
        return callDescValue;
    }
