    ...
    void* cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol>* parameterList)
{
    ...
    bool enableHighWordRA
        = self()->cg()->supportsHighWordFacility() && !self()->comp()->getOption(TR_DisableHighWordRA);
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (ai >= 0 && loadOpCode == TR::InstOpCode::L && enableHighWordRA
            && self()->getRealRegister(REGNUM(ai))->isHighWordRegister())
        ...
        if (lri >= 0) {
            ...
            if (!self()->getIsLeafRoutine()
                && ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor)
                       || unconditionalSave)) {
                ...
                case TR::VectorDouble:
                    if (genBinary) {
                        cursor = (void*)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::VST, (uint8_t*)cursor,
                            self()->getRealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
            ...
            if (ai >= 0 && !InPreProlog) {
                ...
                if (regNum != ai || ((dtype == TR::Int64) && self()->cg()->use64BitRegsOn32Bit())) {
                    ...
                    if (freeScratchable.isSet(ai)) {
                        if (dtype == TR::Float || dtype == TR::Double
                            ...
                            cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                                ...
                                self()->cg()->allocateFPRegisterPair(self()->getRealRegister(REGNUM(regNum + 2)),
                            ...
                            if ((dtype == TR::Int64) && self()->cg()->use64BitRegsOn32Bit()) {
                                ...
                                if (enableHighWordRA && self()->getRealRegister(REGNUM(ai))->isHighWordRegister()) {
                                    cursor = generateExtendedHighWordInstruction(firstNode, self()->cg(),
                                        TR::InstOpCode::LHLR, self()->getRealRegister(REGNUM(ai)),
            ...
            if (!(target < 0)
                ...
                case 0: // Reg 2 Reg
                    if (enableHighWordRA && self()->getRealRegister(REGNUM(target))->isHighWordRegister()) {
                        cursor = generateExtendedHighWordInstruction(firstNode, self()->cg(), TR::InstOpCode::LHLR,
                            self()->getRealRegister(REGNUM(target)), self()->getRealRegister(REGNUM(source)), 0,
                            (TR::Instruction*)cursor);
                    } else {
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                ...
                case 1: // Int or Addr  Reg from Stack Slot
                {
                    ...
                    if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
                        ...
                        cursor = generateRXYInstruction(self()->cg(), TR::InstOpCode::LFH, firstNode,
    ...
    while ((paramCursor != NULL)
        ...
        case TR::DecimalFloat:
            ...
            if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
                if (self()->isFloatParmDescriptors()) { // XPLink: handle large separation of float args constraint
                    int32_t val = TR::S390zOSSystemLinkage::getFloatParmDescriptorFlag(
        ...
        case TR::DecimalLongDouble:
            ...
            if ((numFloatArgs & 0x1)
                != 0) // if there are odd number of fp args in before a long double arg, need to skip one
                numFloatArgs++;
            if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
                if (self()->isFloatParmDescriptors()) { // XPLink: handle large separation of float args constraint
                    int32_t val = TR::S390zOSSystemLinkage::getFloatParmDescriptorFlag(
...
TR::Register* OMR::Z::Linkage::copyArgRegister(TR::Node* callNode, TR::Node* child, TR::Register* argRegister)
{
    ...
    if (TR::Compiler->target.is32Bit() && self()->cg()->use64BitRegsOn32Bit() && child->getDataType() == TR::Int64
        ...
        if (TR::Compiler->target.is32Bit() && child->getDataType() == TR::Int64) {
            TR_ASSERT_FATAL(!self()->cg()->use64BitRegsOn32Bit(),
                "Long values should not be passed in register pairs, they can be stored in a single GPR");
            TR::Register* tempRegH = self()->cg()->allocateRegister();
            TR::Register* tempRegL = self()->cg()->allocateRegister();

            cursor = generateRRInstruction(
                self()->cg(), copyOpCode, callNode, tempRegH, argRegister->getRegisterPair()->getHighOrder());
            if (debugObj) {
                debugObj->addInstructionComment(toS390RRInstruction(cursor), REG_PARAM);
            }

            cursor = generateRRInstruction(
                self()->cg(), copyOpCode, callNode, tempRegL, argRegister->getRegisterPair()->getLowOrder());
            if (debugObj) {
                debugObj->addInstructionComment(toS390RRInstruction(cursor), REG_PARAM);
            }
    ...
    int64_t killMask, TR::Register*& vftReg, bool PassReceiver)
{
    ...
    if (self()->isFloatParmDescriptors()) { // these flags are used to help determine if float parm is needs to be put
                                            // in memory
    ...
    for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
        ...
        {
            if (self()->isFloatParmDescriptors()
                && (numFloatArgs < self()->getNumFloatArgumentRegisters())) { // XPLink: handle large separation of
                                                                              // float args constraint
                uint32_t val
                    = TR::S390zOSSystemLinkage::getFloatParmDescriptorFlag(xplinkCallDescriptorFlags, numFloatArgs);
    ...
    if (self()->cg()->supportsHighWordFacility() && !self()->comp()->getOption(TR_DisableHighWordRA)) {
        ...
        self()->killAndAssignRegister(
            killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR0), self()->cg(), true, true);
        self()->killAndAssignRegister(
            killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR1), self()->cg(), true, true);
        self()->killAndAssignRegister(
            killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR2), self()->cg(), true, true);
        self()->killAndAssignRegister(
            killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR3), self()->cg(), true, true);
        self()->killAndAssignRegister(
            killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR14), self()->cg(), true, true);

        // consider all HPR volatile on 31-bit
        if (TR::Compiler->target.is32Bit()) {
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR6), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR7), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR8), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR9), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR10), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR11), self()->cg(), true, true);
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR12), self()->cg(), true, true);
        }

        if (TR::Compiler->target.isZOS()) {
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR15), self()->cg(), true, true);
            if (self()->cg()->supportsJITFreeSystemStackPointer()) {
                self()->killAndAssignRegister(
                    killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR4), self()->cg(), true, true);
            }
        } else {
            self()->killAndAssignRegister(
                killMask, dependencies, &dummyReg, REGNUM(TR::RealRegister::HPR4), self()->cg(), true, true);
...
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg, int32_t toreg)
{
    ...
    bool checkHPR = (self()->cg()->supportsHighWordFacility() && !self()->comp()->getOption(TR_DisableHighWordRA)
    ...
    for (int32_t i = fromreg; i <= toreg; ++i) {
        if ((self()->getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()
            || (checkHPR
                   && (self()->getRealRegister(REGNUM(i)))->getHighWordRegister()->getHasBeenAssignedInMethod())) {
    ...
    TR::RealRegister* spReg, TR::MemoryReference* rsa, TR::RealRegister::RegNum spRealReg)
{
    ...
    } else // 32-bit (try to use bumps)
    {
        ...
        rsa = generateS390MemoryReference(
            spReg, rsa->getOffset() + self()->cg()->machine()->getGPRSize(), self()->cg());
        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
...
{
    return (self()->getFrameType() == StackLeafFrame) || (self()->getFrameType() == noStackLeafFrame)
        || (self()->getFrameType() == noStackForwardingFrame);
}

bool OMR::Z::Linkage::isOSLinkageType() { return (self()->getExplicitLinkageType() & TR_SystemOS_MASK) != 0; }

bool OMR::Z::Linkage::isXPLinkLinkageType() { return self()->getExplicitLinkageType() == TR_SystemXPLink; }

bool OMR::Z::Linkage::isFastLinkLinkageType() { return self()->getExplicitLinkageType() == TR_SystemFastLink; }

bool OMR::Z::Linkage::isZLinuxLinkageType() { return self()->getExplicitLinkageType() == TR_SystemLinux; }
