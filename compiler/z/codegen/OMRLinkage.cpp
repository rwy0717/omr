			...
			if (disp == stackOffset) {
				...
				if (opCode == TR::InstOpCode::L) {
					newInst = generateRRInstruction(
					        self()->cg(), TR::InstOpCode::LR, current->getNode(), tReg, sReg, prev);
					if (self()->comp()->getOption(TR_TraceCG)) {
						traceMsg(self()->comp(),
				...
				if (newInst
				        && performTransformation(
				                self()->comp(), "O^O : Prolog peeking removing [%p]\n", current)) {
					...
					if (!mustReplace && tReg == sReg) {
						...
						if (self()->comp()->getOption(TR_TraceCG))
							traceMsg(self()->comp(),
							        "deleting instruction as sreg = treg\n");
					}
        ...
        void *cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol> *parameterList)
{
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		if (self()->comp()->getOption(TR_TraceCG)) {
			traceMsg(self()->comp(),
			        "save argument: %s, lri: %d, ai: %d, offset: %d, isReferenced: %d, hasToBeOnStack: "
			        "%d\n",
			        param_name, lri, ai, offset, paramCursor->isReferencedParameter(),
		...
		if (lri >= 0) {
			...
			if (type.isFloatingPoint()) // float/double/long double
			{
				...
				if (TR::Compiler->target.is32Bit() && paramCursor->getSize() > 4) {
					fullLong = true; // On 31 bit this larger aggregate will be treated like a 64
					                 // bit long
					lastFreeIntArgIndex++;
				}
			...
			// This query here is actually not ideal.  originally isReferencedParameter (which actually
			// means isReferencedParameterAtIlGenTime) was replaced with isParmHasToBeOnStack, which would
			// make sense. However, this broke some java modes (OSR,HCR).  Likely, those modes should rely
			// on 'unconditionalSave' to ensure we always store out to the stack for whenever we need all
			// variables on the stack. This investigation will be a future todo .
			...
			if ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor)
				...
				{
					if (genBinary) {
						cursor = (void *)TR::S390CallSnippet::storeArgumentItem(storeOpCode,
						        (uint8_t *)cursor, self()->getRealRegister(regNum), offset,
						        self()->cg());
					} else {
						...
						if (storeOpCode == TR::InstOpCode::STCM)
							cursor = generateRSInstruction(self()->cg(),
							        TR::InstOpCode::STCM, firstNode,
							        self()->getRealRegister(regNum), opcodeMask, mr,
							        (TR::Instruction *)cursor);

						else
							cursor = generateRXInstruction(self()->cg(), storeOpCode,
							        firstNode, self()->getRealRegister(regNum), mr,
							        (TR::Instruction *)cursor);
					...
					if (secondStore && fullLong && TR::Compiler->target.is32Bit()) {
						if (genBinary) {
							...
							cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::ST,
							        firstNode, self()->getRealRegister(REGNUM(regNum + 1)),
				...
				case TR::DecimalDouble:
					...
					if (genBinary) {
						...
						TR::MemoryReference *mr = generateS390MemoryReference(
						        stackPtr, offset, self()->cg(), param_name);
						cursor = (void *)generateRXInstruction(self()->cg(),
						        TR::InstOpCode::STD, firstNode, self()->getRealRegister(regNum),
						        mr, (TR::Instruction *)cursor);
						((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
						if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
							self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
				...
				if (regNum != ai || (dtype == TR::Int64 && TR::Compiler->target.is32Bit())) {
					...
					if (freeScratchable.isSet(ai)) {
						if (dtype == TR::Float || dtype == TR::Double
							...
							cursor = generateRRInstruction(self()->cg(),
							        TR::InstOpCode::LDR, firstNode,
							...
							int32_t ai_l =
							        paramCursor
							                ->getAssignedLowGlobalRegisterIndex(); //  low
							                                                       //  reg
							                                                       //  of a
							                                                       //  pair
							TR_ASSERT((ai_l == (ai + 2)),
							...
							if (dtype == TR::Int64 && TR::Compiler->target.is32Bit()) {
								...
								// Figure out if the low word is passed in a reg or in
								// memory
								if (fullLong) {
									cursor = generateRRInstruction(self()->cg(),
									        TR::InstOpCode::LR, firstNode,
									        self()->getRealRegister(REGNUM(ai)),
									        self()->getRealRegister(
									                REGNUM(regNum + 1)),
									        (TR::Instruction *)cursor);
									...
									cursor = generateRXInstruction(self()->cg(),
									        TR::InstOpCode::L, firstNode,
									        self()->getRealRegister(REGNUM(ai)),
									        generateS390MemoryReference(stackPtr,
									                offset + 4, self()->cg()),
									        (TR::Instruction *)cursor);
								}

								// Don't do anything for the low word later on as it was
								// handled here
								//
								skipLong = true;
							} else {
								cursor = generateRRInstruction(self()->cg(),
								        TR::InstOpCode::getLoadRegOpCode(), firstNode,
								        self()->getRealRegister(REGNUM(ai)),
								        self()->getRealRegister(regNum),
								        (TR::Instruction *)cursor);
			...
			if (ai_l >= 0 && !InPreProlog && !skipLong) {
				...
				//  - complex:  can pass real part in FP reg (or FP reg pair for complex long double)
				//  and imag part on stack
				        ...
				        if (!fullLong) // if long int lower word pass in stack ...
				{
					if (freeScratchable.isSet(ai_l)) {
						cursor = generateRXInstruction(self()->cg(),
						        TR::InstOpCode::getLoadOpCode(), firstNode,
						        self()->getRealRegister(REGNUM(ai_l)),
		...
		//
			...
			case TR::Aggregate: // Should only happen on zLinux
				if (freeScratchable.isSet(ai)) {
					...
					if (loadOpCode == TR::InstOpCode::ICM)
						cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM,
						        firstNode, self()->getRealRegister(REGNUM(ai)), opcodeMask, mr,
						        (TR::Instruction *)cursor);
					else
			...
			case TR::DecimalLongDouble:
				...
				break;
#endif
			default: TR_ASSERT(0, "saveArguments: unhandled parameter type");
			} // switch(dtype)
			...
			if (!(target < 0)
				...
				case 0: // Reg 2 Reg
				{
					cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(),
					        firstNode, self()->getRealRegister(REGNUM(target)),
				...
				case 1: // Int or Addr  Reg from Stack Slot
				{
					...
					if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
						cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM,
						        firstNode, self()->getRealRegister(REGNUM(target)),
						        busyMoves[4][i1], mr, (TR::Instruction *)cursor);

					else if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::LFH)
						cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LFH,
						        firstNode, self()->getRealRegister(REGNUM(target)), mr,
						        (TR::Instruction *)cursor);

					else
						cursor = generateRXInstruction(self()->cg(),
						        (TR::InstOpCode::Mnemonic)busyMoves[3][i1], firstNode,
...
OMR::Z::Linkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method, List<TR::ParameterSymbol> &parmList)
{
	...
	while ((paramCursor != NULL)
		...
		case TR::DecimalLongDouble:
			...
			if ((numFloatArgs & 0x1) != 0) // if there are odd number of fp args in before a long double
		...
		if (self()->isFastLinkLinkageType()) {
			if ((numFloatArgs == 1) || (numIntArgs >= self()->getNumIntegerArgumentRegisters())) {
				// force fastlink ABI condition of only one float parameter for fastlink parameter and
				// it must be within first slots
				numFloatArgs = self()->getNumFloatArgumentRegisters(); // no more float args possible
				                                                       // now
			}
		}
	}
...
TR::Register *
OMR::Z::Linkage::pushLongArg32(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs, int32_t numFloatArgs,
        int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
	...
	case TR::java_lang_invoke_MethodHandle_invokeWithArgumentsHelper: isStorePair = true; break;
#endif
	default: {
...
TR::Register *
OMR::Z::Linkage::pushArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs, int32_t numFloatArgs,
        int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
...
#ifdef J9_PROJECT_SPECIFIC
	case TR::java_lang_invoke_ComputedCalls_dispatchJ9Method:
	case TR::java_lang_invoke_MethodHandle_invokeWithArgumentsHelper: isStoreArg = true; break;
#endif
	default: {
	...
	if (isStoreArg) {
		...
		if (argRegister != NULL) {
			...
			if (child->getDataType() == TR::DecimalLongDouble)
				self()->storeLongDoubleArgumentOnStack(callNode, child->getDataType(), storeOp,
				        argRegister, stackOffsetPtr, stackRegister);
			else
#endif
	...
	if (isFloatShadowedToIntRegs) {
		TR::Register *stackRegister = self()->getStackRegisterForOutgoingArguments(
		        callNode, dependencies); // delay (possibly) creating this till needed
		self()->loadIntArgumentsFromStack(callNode, dependencies, argDataType, (*stackOffsetPtr) - argSize,
		        argSize, numIntegerArgs, stackRegister);
	}
...
TR::Register *
OMR::Z::Linkage::pushJNIReferenceArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs, int32_t numFloatArgs,
        int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
	...
	if (isNull) {
		...
		generateRRInstruction(
		        self()->cg(), TR::InstOpCode::getXORRegOpCode(), child, pushRegister, pushRegister);
		...
		if (!self()->cg()->canClobberNodesRegister(child, 0)) {
			pushRegister = self()->cg()->allocateRegister();
			generateRRInstruction(
			        self()->cg(), TR::InstOpCode::getLoadRegOpCode(), child, pushRegister, addrReg);
		} else {
		...
		generateS390BranchInstruction(
		        self()->cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, child, nonNullLabel);
		generateRRInstruction(
		        self()->cg(), TR::InstOpCode::getXORRegOpCode(), child, pushRegister, pushRegister);
	...
	return self()->pushArg(
	        callNode, child, numIntegerArgs, numFloatArgs, stackOffsetPtr, dependencies, pushRegister);
}

TR::Register *
OMR::Z::Linkage::pushVectorArg(TR::Node *callNode, TR::Node *child, int32_t numVectorArgs, int32_t pindex,
        int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
	TR::DataType argType = child->getType();
...
TR::Register *
OMR::Z::Linkage::pushAggregateArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs, int32_t numFloatArgs,
        int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
#ifdef J9_PROJECT_SPECIFIC
 ...
 */
void
OMR::Z::Linkage::loadIntArgumentsFromStack(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
...
#ifdef J9_PROJECT_SPECIFIC
			if (argType == TR::DecimalFloat && TR::Compiler->target.is64Bit()) // special case for DecFloat
			                                                                   // on 64bit
 ...
 */
int32_t
OMR::Z::Linkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies, bool isFastJNI,
        int64_t killMask, TR::Register *&vftReg, bool PassReceiver)
{
	...
	for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
		...
		case TR::Int16:
		case TR::Int32:
			argRegister = self()->pushArg(
			        callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
			numIntegerArgs++;
			break;
		case TR::Int64:
			if (TR::Compiler->target.is32Bit()) {
				argRegister = self()->pushLongArg32(
				        callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
				numIntegerArgs += 2;
			} else {
				argRegister = self()->pushArg(
	...
	case TR::DecimalLongDouble: {
		...
		dependencies->addPostCondition(
		        highReg, self()->getLongDoubleReturnRegister0(), DefinesDependentRegister);
		self()->cg()->setRealRegisterAssociation(lowReg, self()->getLongDoubleReturnRegister2());
		dependencies->addPostCondition(
		        lowReg, self()->getLongDoubleReturnRegister2(), DefinesDependentRegister);
	...
	case TR::Int64:
		if (TR::Compiler->target.is32Bit()) {
			...
			dependencies->addPostCondition(
			        resultRegLow, self()->getLongLowReturnRegister(), DefinesDependentRegister);
...
OMR::Z::Linkage::replaceCallWithJumpInstruction(TR::Instruction *callInstruction)
{
	...
	TR_ASSERT(opCode == TR::InstOpCode::BASSM || opCode == TR::InstOpCode::BASR || opCode == TR::InstOpCode::BALR
	                || opCode == TR::InstOpCode::BAS || opCode == TR::InstOpCode::BAL
	                || opCode == TR::InstOpCode::BRAS || opCode == TR::InstOpCode::BRASL,
...
TR::Instruction *
OMR::Z::Linkage::storeArgumentOnStack(TR::Node *callNode, TR::InstOpCode::Mnemonic opCode, TR::Register *argReg,
        int32_t *stackOffsetPtr, TR::Register *stackRegister)
{
	...
	TR::MemoryReference *argMemRef =
	        generateS390MemoryReference(stackRegister, storeOffset + padding, self()->cg());
 ...
 */
TR::Instruction *
OMR::Z::Linkage::storeLongDoubleArgumentOnStack(TR::Node *callNode, TR::DataType argType,
...
int64_t
OMR::Z::Linkage::killAndAssignRegister(int64_t killMask, TR::RegisterDependencyConditions *deps,
        TR::Register **virtualRegPtr, TR::RealRegister *realReg, TR::CodeGenerator *codeGen, bool isAllocate,
        bool isDummy)
{
...
void
OMR::Z::Linkage::generateDispatchReturnLable(TR::Node *callNode, TR::CodeGenerator *codeGen,
        ...
        TR::Node *GlobalRegDeps)
{
	...
	TR::RegisterDependencyConditions *postDeps = new (self()->trHeapMemory()) TR::RegisterDependencyConditions(
	        NULL, deps->getPostConditions(), 0, deps->getAddCursorForPost(), self()->cg());
	...
	if (javaReturnRegister) {
		deps->stopUsingDepRegs(self()->cg(),
		        javaReturnRegister->getRegisterPair() == NULL ? javaReturnRegister
...
void
OMR::Z::Linkage::setupBuildArgForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
...
void
OMR::Z::Linkage::performCallNativeFunctionForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
        ...
        TR::RegisterDependencyConditions *&deps, bool isFastJNI, bool isPassReceiver, bool isJNIGCPoint)
{
	...
	/* Build Arguments               */
	/*********************************/
	self()->setupBuildArgForLinkage(callNode, dispatchType, deps, isFastJNI, isPassReceiver, killMask,
	...
	/*********************************/
	self()->generateDispatchReturnLable(
	        callNode, self()->cg(), deps, javaReturnRegister, hasGlRegDeps, GlobalRegDeps);
...
OMR::Z::Linkage::needsAlignment(TR::DataType dt, TR::CodeGenerator *cg)
{
	...
	case TR::DecimalLongDouble:
#endif
	case TR::Aggregate: return true;
	case TR::Address: return TR::Compiler->om.sizeofReferenceAddress() == 8 ? true : false;
	default: break;
	}
	return false;
}
...
TR::Instruction *
OMR::Z::Linkage::restorePreservedRegs(TR::RealRegister::RegNum firstUsedReg, TR::RealRegister::RegNum lastUsedReg,
        int32_t blockNumber, TR::Instruction *cursor, TR::Node *nextNode, TR::RealRegister *spReg,
        TR::MemoryReference *rsa, TR::RealRegister::RegNum spRealReg)
{
...
{
	switch (kind) {
	case TR_GPR: return self()->getNumIntegerArgumentRegisters();
	case TR_FPR: return self()->getNumFloatArgumentRegisters();
	case TR_VRF: return self()->getNumVectorArgumentRegisters();
	default: return 0;
	}
	return 0;
}
