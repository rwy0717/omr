...
MemToMemVarLenMacroOp::generateLoop()
{
	...
	if (useEXForRemainder()) {
		...
		if (_lengthMinusOne)
			generateRRInstruction(_cg, TR::InstOpCode::LTR, _rootNode, _regLen,
			        _regLen); // Because
			                  // transformLengthMinusOneForMemoryOps
			                  // uses TR::iadd
	...
	generateRXInstruction(_cg, TR::InstOpCode::LA, _srcNode, _srcReg,
	        new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg, 256, _cg));
	if (_srcReg != _dstReg) {
...
MemToMemConstLenMacroOp::generateLoop()
{
	...
	// TR_ASSERT(((uint64_t)INT_MAX*8) > _length, "_length should not exceed the maximum array size in
	// bytes,INT_MAX*8 = 0x%x%x, _length = 0x%x%x",(((uint64_t)INT_MAX*8) >>32), (uint32_t)((uint64_t)INT_MAX*8),
	...
	// if a series of instructions can be done instead of a loop of them, do so, but only if it will not exceed the
	// 4K displacement on XC
	if (largeCopies != 0 && largeCopies < _maxCopies) {
		...
		if (largeCopies * 256 + _offset + srcMROffset >= 4096) {
			// this early LA isn't needed for correctness as the later generated SS instructions will
			// enforce the limits but doing this upfront instead saves possibly multiple fixup LAs later on
			if (_srcReg == NULL)
		...
		if (_srcReg == NULL || _dstReg == NULL || _srcReg != _dstReg) {
			...
			if (largeCopies * 256 + dstMROffset >= 4096) {
				// this early LA isn't needed for correctness as the later generated SS instructions
				// will enforce the limits but doing this upfront instead saves possibly multiple fixup
				// LAs later on
				if (_dstReg == NULL)
	...
	// Notice that within the small range [4K*i-7, 4K*i], MVCL is significantly degraded. (3 times slower than
	// normal detected) In order to use MVCL, we have also to make sure that the use is safe. i.e., src and dst are
	// NOT aliased.
	...
	if (inRange && (aliasingPattern || !_cg->storageMayOverlap(_srcNode, len, _dstNode, len))) {
		...
		TR::Instruction *cursor =
		        generateRRInstruction(_cg, opCode, _rootNode, targetPairRegister, sourcePairRegister);
...
BitOpMemVarLenMacroOp::getHelper()
{
	...
	case TR::InstOpCode::OC:
		return (intptrj_t)_cg->symRefTab()
		        ...
		        ->getMethodAddress();
	default: TR_ASSERT(0, "not support");
	}
	return 0;
}
...
{
	switch (_opcode) {
	case TR::InstOpCode::XC: return _cg->getSymRef(TR_S390arrayXORHelper);
	case TR::InstOpCode::NC: return _cg->getSymRef(TR_S390arrayANDHelper);
	case TR::InstOpCode::OC: return _cg->getSymRef(TR_S390arrayORHelper);
	default: TR_ASSERT(0, "not support");
	}
	return 0;
}
	...
	if (needDependencies()) {
		...
		if (_inNestedICF) {
			TR_ASSERT(!_nestedICFDeps,
			        "needDependencies should not be true if _nestedICFDeps have already been created");
			_nestedICFDeps = dependencies;
		}
	}
...
MemToMemVarLenMacroOp::generateRemainder()
{
	if (useEXForRemainder()) {
		...
		if (_doneLabel)
			_cursor = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _rootNode,
...
MemInitVarLenMacroOp::generateRemainder()
{
	...
	if (checkLengthAfterLoop()) {
		...
		if (_firstByteInitialized)
			generateRIInstruction(_cg,
			        TR::Compiler->target.is64Bit() ? TR::InstOpCode::AGHI : TR::InstOpCode::AHI, _rootNode,
			        _regLen, -1);
		...
		generateRIInstruction(_cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::AGHI : TR::InstOpCode::AHI,
		        _rootNode, _regLen, -1);
		...
		generateS390BranchInstruction(
		        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, _rootNode, _doneLabel);
	...
	_cursor = cursor;
	if (_doneLabel)
		_cursor = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _rootNode, _doneLabel); // required
		                                                                                           // as main
		                                                                                           // logic
		                                                                                           // branches
		                                                                                           // here
...
MemClearVarLenMacroOp::generateRemainder()
{
	...
	_cursor = cursor;
	_cursor = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _rootNode, _doneLabel); // required as main
	                                                                                           // logic branches
	                                                                                           // here
...
MemInitConstLenMacroOp::generateInstruction(int32_t offset, int64_t length, TR::Instruction *cursor1)
{
	...
	if (length == 1 && !_useByteVal) {
		...
		cursor =
		        generateSS1Instruction(_cg, TR::InstOpCode::MVC, _rootNode, length - 1, _dstMR, _srcMR, cursor);
	}
...
MemClearConstLenMacroOp::generateInstruction(int32_t offset, int64_t length, TR::Instruction *cursor1)
{
	...
	if (_dstNode == _srcNode) {
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) && length <= 8
			...
			case 8:
				cursor =
				        generateSILInstruction(_cg, TR::InstOpCode::MVGHI, _rootNode, srcMR, 0, cursor);
				return cursor;
			case 4:
			...
			case 2:
				cursor =
				        generateSILInstruction(_cg, TR::InstOpCode::MVHHI, _rootNode, srcMR, 0, cursor);
				return cursor;
			case 1:
...
MemCmpConstLenMacroOp::generateInstruction(int32_t offset, int64_t length, TR::Instruction *cursor1)
{
	...
	if (!inRemainder()) {
		cursor = generateS390BranchInstruction(
		        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, _rootNode, _falseLabel);
	}
...
generateCmpResult(TR::CodeGenerator *cg, TR::Node *rootNode, TR::Register *dstReg, TR::Register *srcReg,
        TR::Register *resultReg, TR::LabelSymbol *falseLabel, TR::LabelSymbol *trueLabel, TR::LabelSymbol *doneLabel)
{
...
TR::Instruction *
MemCmpConstLenMacroOp::generate(
        TR::Register *dstReg, TR::Register *srcReg, TR::Register *tmpReg, int32_t offset, TR::Instruction *cursor)
{
	TR::Instruction *cursorBefore = cursor;
	_resultReg = _cg->allocateRegister();
...
static TR::Instruction *
generateCmpSignResult(TR::CodeGenerator *cg, TR::Node *rootNode, TR::Register *dstReg, TR::Register *srcReg,
        TR::Register *resultReg, TR::LabelSymbol *falseLabel, TR::LabelSymbol *gtLabel, TR::LabelSymbol *trueLabel,
...
TR::Instruction *
MemCmpVarLenSignMacroOp::generate(
        TR::Register *dstReg, TR::Register *srcReg, TR::Register *tmpReg, int32_t offset, TR::Instruction *cursor)
{
	TR::Instruction *cursorBefore = cursor;
	_resultReg = _cg->allocateRegister();
...
TR::Instruction *
MemCmpConstLenSignMacroOp::generate(
        TR::Register *dstReg, TR::Register *srcReg, TR::Register *tmpReg, int32_t offset, TR::Instruction *cursor)
{
	TR::Instruction *cursorBefore = cursor;
	_resultReg = _cg->allocateRegister();
...
MemToMemTypedVarLenMacroOp::shiftSize()
{
	...
	case TR::Float:
	case TR::Int32: return 2;
	case TR::Double:
	case TR::Int64: return 3; break;
	default:
		if (TR::Compiler->target.is64Bit())
			return 3;
		else
...
MemToMemTypedVarLenMacroOp::strideSize()
{
	...
	case TR::Float:
	case TR::Int32: return 4;
	case TR::Double:
	case TR::Int64: return 8; break;
	default:
...
MemToMemTypedVarLenMacroOp::generateLoop()
{
	...
	if (_isForward) {
		...
		generateRIInstruction(
		        _cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), _dstNode, _strideReg, strideSize());
		...
		if (_srcReg != _startReg) {
			...
			generateRRInstruction(
			        _cg, TR::InstOpCode::getAddRegWidenOpCode(), _srcNode, _srcReg, _strideReg);
		}
...
MemInitVarLenTypedMacroOp::generateInstruction()
{
	...
	switch (_destType) {
	case TR::Int16: cursor = generateRXInstruction(_cg, TR::InstOpCode::STH, _dstNode, _initReg, newMR); break;
	case TR::Int32: cursor = generateRXInstruction(_cg, TR::InstOpCode::ST, _dstNode, _initReg, newMR); break;
	case TR::Int64: cursor = generateRXInstruction(_cg, TR::InstOpCode::STG, _dstNode, _initReg, newMR); break;
	case TR::Float: cursor = generateRXInstruction(_cg, TR::InstOpCode::STE, _dstNode, _initReg, newMR); break;
	case TR::Double: cursor = generateRXInstruction(_cg, TR::InstOpCode::STD, _dstNode, _initReg, newMR); break;
	default: cursor = generateRXInstruction(_cg, TR::InstOpCode::STG, _dstNode, _initReg, newMR); break;
	}
...
MemCpyVarLenTypedMacroOp::generateInstruction()
{
	...
	case TR::Address:
			...
			if (_needsGuardedLoad) {
				...
				if (shiftAmount != 0) {
					cursor = generateRSInstruction(
					        _cg, TR::InstOpCode::SRLG, _srcNode, _workReg, _workReg, shiftAmount);
				}
			} else {
				cursor = generateRXInstruction(_cg, TR::InstOpCode::L, _srcNode, _workReg, srcMR);
			}
		...
		break;
	default: TR_ASSERT_FATAL(false, "_destType of invalid type\n"); break;
	}
...
TR::Instruction *
MemCpyAtomicMacroOp::generateConstLoop(TR::InstOpCode::Mnemonic loadOp, TR::InstOpCode::Mnemonic storeOp)
{
...
TR::Instruction *
MemCpyAtomicMacroOp::generateSTXLoop(
        int32_t strideSize, TR::InstOpCode::Mnemonic loadOp, TR::InstOpCode::Mnemonic storeOp, bool unroll)
{
	...
	if (!_isForward) {
		...
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _srcReg, -1 * strideSize);
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _endReg, -1 * strideSize);
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _startReg, -1 * strideSize);
		cursor = generateRRInstruction(
		        _cg, TR::InstOpCode::getSubstractRegOpCode(), _dstNode, _startReg, _lenReg);
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), _dstNode, _strideReg, -1 * strideSize);
	} else {
		// Set _startReg to the last element to be copied (i.e. start + length - stride)
		cursor = generateRIInstruction(_cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _startReg,
		        -1 * _unrollFactor * strideSize);
		cursor = generateRRInstruction(_cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _lenReg);
		cursor = generateRIInstruction(_cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), _dstNode, _strideReg,
...
TR::Instruction *
MemCpyAtomicMacroOp::generateSTXLoopLabel(TR::LabelSymbol *oolStartLabel, TR::LabelSymbol *doneCopyLabel,
        int32_t strideSize, TR::InstOpCode::Mnemonic loadOp, TR::InstOpCode::Mnemonic storeOp)
{
	TR::Compilation *comp = _cg->comp();
	if (_trace)
	...
	cursor = generateS390BranchInstruction(
	        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, _srcNode, doneCopyLabel);
...
TR::Instruction *
MemCpyAtomicMacroOp::generateOneSTXthenSTYLoopLabel(TR::LabelSymbol *oolStartLabel, TR::LabelSymbol *doneCopyLabel,
        int32_t strideSize1, TR::InstOpCode::Mnemonic loadOp1, TR::InstOpCode::Mnemonic storeOp1, int32_t strideSize2,
        TR::InstOpCode::Mnemonic loadOp2, TR::InstOpCode::Mnemonic storeOp2)
{
	...
	cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::getCmpOpCode(), _lenNode, _lenReg,
	        strideSize1, TR::InstOpCode::COND_BL, skipRoutineLabel);
	...
	if (false && !_isForward) {
		...
		cursor = generateRRInstruction(_cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _lenReg);
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), _dstNode, _strideReg, strideSize1);
	}
	...
	if (false && !_isForward)
		...
		cursor = generateRIInstruction(
		        _cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _endReg, strideSize1);
...
MemCpyAtomicMacroOp::generateLoop()
{
	...
	case 8:
		...
		break;
	default: TR_ASSERT(0, "bad stride size in array copy loop\n");
	}
	...
	if (_destType == TR::NoType) {
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
			...
			cursor = generateRRInstruction(
			        _cg, TR::InstOpCode::getLoadRegOpCode(), _srcNode, _alignedReg, _srcReg);
			cursor = generateRRInstruction(
			        _cg, TR::InstOpCode::getOrRegOpCode(), _srcNode, _alignedReg, _startReg);
		}
		...
		cursor = generateS390BranchInstruction(_cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, _srcNode,
		...
		cursor = generateRIInstruction(_cg, TR::InstOpCode::NILL, _srcNode, _alignedReg, 0x3); // 0x3 == 11,
		                                                                                       // last 2 bits
		...
		cursor = generateRIInstruction(_cg, TR::InstOpCode::NILL, _srcNode, _alignedReg, 0x1); // 0x1 == 1, last
		                                                                                       // bit
		...
		if (_destType == TR::Int16) {
			if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
				...
				cursor = generateRRInstruction(
				        _cg, TR::InstOpCode::getLoadRegOpCode(), _srcNode, _alignedReg, _srcReg);
				cursor = generateRRInstruction(
				        _cg, TR::InstOpCode::getAndRegOpCode(), _srcNode, _alignedReg, _startReg);
			}

			cursor = generateRILInstruction(_cg, TR::InstOpCode::NILF, _srcNode, _alignedReg, 0x7);

			cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::C, _srcNode, _alignedReg,
			        0x2, TR::InstOpCode::COND_BE, oolStartLabel2); // 0x2 = 0x010
			cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel2, preDoneCopyLabel2, 2,
			        TR::InstOpCode::LH, TR::InstOpCode::STH, 4, TR::InstOpCode::L, TR::InstOpCode::ST);
			...
			cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::C, _srcNode, _alignedReg,
			        0x6, TR::InstOpCode::COND_BE, oolStartLabel3); // 0x6 = 0x110
			cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel3, preDoneCopyLabel3, 2,
			        TR::InstOpCode::LH, TR::InstOpCode::STH, 4, TR::InstOpCode::L, TR::InstOpCode::ST);
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
			...
			cursor = generateRRInstruction(
			        _cg, TR::InstOpCode::getLoadRegOpCode(), _srcNode, _alignedReg, _srcReg);
			cursor = generateRRInstruction(
			        _cg, TR::InstOpCode::getAddRegOpCode(), _srcNode, _alignedReg, _startReg);
		}
		...
		cursor = generateSTXLoopLabel(
		        oolStartLabel4, preDoneCopyLabel4, 8, TR::InstOpCode::LG, TR::InstOpCode::STG);
		cursor = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel4);
		cursor = generateS390BranchInstruction(
		        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, _srcNode, remainderLabel);
		if (_destType == TR::Int16) {
			cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::C, _srcNode, _alignedReg,
			        0x4, TR::InstOpCode::COND_BE, oolStartLabel5); // 0x4 = 0x100
			cursor = generateSTXLoopLabel(
			        oolStartLabel5, preDoneCopyLabel5, 4, TR::InstOpCode::L, TR::InstOpCode::ST);
		...
		cursor = generateSTXLoopLabel(
		        oolStartLabel6, preDoneCopyLabel6, strideSize(), unalignedLoadOp, unalignedStoreOp);
	...
	if (generateRemainder && _isForward) {
		switch (_destType) {
		case TR::Int8: TR_ASSERT(0, "Non backwards 8 bit element array should be using MVC\n"); break;
		case TR::Int16: cursor = generateSTXLoop(2, TR::InstOpCode::LH, TR::InstOpCode::STH, false); break;
		case TR::Float:
		case TR::Int32: cursor = generateSTXLoop(4, TR::InstOpCode::L, TR::InstOpCode::ST, false); break;
		case TR::Int64:
		case TR::Double: cursor = generateSTXLoop(8, TR::InstOpCode::LG, TR::InstOpCode::STG, false); break;
		case TR::Address:
...
MemCpyAtomicMacroOp::MemCpyAtomicMacroOp(TR::Node *rootNode, TR::Node *dstNode, TR::Node *srcNode,
        TR::CodeGenerator *cg, TR::DataType destType, TR::Register *lenReg, TR::Node *lenNode, bool isForward,
        bool unroll, int32_t constLength)
        : MemToMemTypedVarLenMacroOp(rootNode, dstNode, srcNode, cg, destType, lenReg, lenNode, isForward)
{
	...
	if ((bool)unrollFactor) {
		...
		case '8': _unrollFactor = 8; break;
		case '7': _unrollFactor = 7; break;
		case '6': _unrollFactor = 6; break;
		case '5': _unrollFactor = 5; break;
		case '4': _unrollFactor = 4; break;
		case '3': _unrollFactor = 3; break;
		case '2': _unrollFactor = 2; break;
		case '1': _unrollFactor = 1; break;
		default: TR_ASSERT(0, "MemCpyAtomicMacroOp: Unacceptable unroll factor\n");
		}
	} else
		_unrollFactor = 3;
...
MemCpyAtomicMacroOp::allocWorkReg()
{
	...
	if (_unroll && _isForward) {
		if (_unrollFactor) {
			...
			case 8: _workReg8 = _cg->allocateRegister();
			case 7: _workReg7 = _cg->allocateRegister();
			case 6: _workReg6 = _cg->allocateRegister();
			case 5: _workReg5 = _cg->allocateRegister();
			case 4: _workReg4 = _cg->allocateRegister();
			case 3: _workReg3 = _cg->allocateRegister();
			case 2: _workReg2 = _cg->allocateRegister();
			}
		}
	}
	_workReg = _cg->allocateRegister();
...
MemCpyAtomicMacroOp::createLoopDependencies(TR::Instruction *cursor)
{
	...
	if (_unroll && _isForward) {
		if (_unrollFactor) {
			switch (_unrollFactor) {
			case 8: loopDep->addPostCondition(_workReg8, TR::RealRegister::AssignAny);
			case 7: loopDep->addPostCondition(_workReg7, TR::RealRegister::AssignAny);
			case 6: loopDep->addPostCondition(_workReg6, TR::RealRegister::AssignAny);
			case 5: loopDep->addPostCondition(_workReg5, TR::RealRegister::AssignAny);
			case 4: loopDep->addPostCondition(_workReg4, TR::RealRegister::AssignAny);
			case 3: loopDep->addPostCondition(_workReg3, TR::RealRegister::AssignAny);
			case 2: loopDep->addPostCondition(_workReg2, TR::RealRegister::AssignAny);
			}
		}
	}
