...
OMR::Z::CodeGenPhase::performPreRAPeepholePhase(TR::CodeGenerator *cg, TR::CodeGenPhase *phase)
{
	...
	if (comp->getOption(TR_TraceCG))
		comp->getDebug()->dumpMethodInstrs(
		        comp->getOutFile(), "Pre Register Assignment Peephole Instructions", false);
}
...
OMR::Z::CodeGenPhase::performPeepholePhase(TR::CodeGenerator *cg, TR::CodeGenPhase *phase)
{
	...
	if (comp->getOption(TR_TraceCG))
		comp->getDebug()->dumpMethodInstrs(
		        comp->getOutFile(), "Post Register Assignment Peephole Instructions", false);
}
...
{
	switch (phase) {
	case markLoadAsZeroOrSignExtension: return "markLoadAsZeroOrSignExtension";
	case SetBranchOnCountFlagPhase: return "SetBranchOnCountFlagPhase";
	case PreRAPeepholePhase: return "PreRegisterAllocationPeepholePhase";
	case PeepholePhase: return "PostRegisterAllocationPeepholePhase";
	default:
