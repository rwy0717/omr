    ...
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::MemoryReference* memRef)
{
    ...
    if (!memRef->hasTemporaryNegativeOffset()
        && ((displacement > MINLONGDISP && displacement < MINDISP)
 ...
 */
TR::Instruction* generateS390CompareAndBranchInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic compareOpCode,
    TR::Node* node, TR::Register* first, int32_t second, TR::InstOpCode::S390BranchCondition bc,
...
TR::Instruction* generateRXYInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
...
TR::Instruction* generateRXYbInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint8_t mask,
    TR::MemoryReference* mf, TR::Instruction* preced)
{
...
TR::Instruction* generateRSYInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* treg, uint32_t mask, TR::MemoryReference* mf, TR::Instruction* preced)
{
...
TR::Instruction* generateSIYInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::MemoryReference* mf, uint32_t imm, TR::Instruction* preced)
{
        ...
        if (node->isClassUnloadingConst()) {
            ...
            if (isMethod) {
                unloadableClass = (TR_OpaqueClassBlock*)cg->fe()
                                      ->createResolvedMethod(cg->trMemory(), (TR_OpaqueMethodBlock*)(intptr_t)imm,
 ...
 */
TR::Instruction* generateRegLitRefInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node,
    TR::Register* treg, uintptrj_t imm, int32_t reloType, TR::RegisterDependencyConditions* cond,
    TR::Instruction* preced, TR::Register* base)
{
    ...
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        if (op == TR::InstOpCode::LG || op == TR::InstOpCode::L) {
            ...
            AOTcgDiag4(comp,
                "generateRegLitRefInstruction constantDataSnippet=%x symbolReference=%x symbol=%x reloType=%x\n",
                targetSnippet, targetSnippet->getSymbolReference(), targetSnippet->getSymbolReference()->getSymbol(),
                reloType);

            cursor = (TR::S390RILInstruction*)generateRILInstruction(cg,
                (op == TR::InstOpCode::LG) ? TR::InstOpCode::LGRL : TR::InstOpCode::LRL, node, treg, targetSnippet,
                preced);
            return cursor;
        }
    }
                ...
                if (isMethod) {
                    unloadableClass = (TR_OpaqueClassBlock*)cg->fe()
                                          ->createResolvedMethod(
                                              cg->trMemory(), (TR_OpaqueMethodBlock*)imm, comp->getCurrentMethod())
                                          ->classOfMethod();
                    if (!TR::Compiler->cls.sameClassLoaders(
                            comp, unloadableClass, comp->getCurrentMethod()->classOfMethod()))
...
TR::Instruction* generateSerializationInstruction(TR::CodeGenerator* cg, TR::Node* node, TR::Instruction* preced)
{
    ...
    TR::InstOpCode::S390BranchCondition cond = cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)
 ...
 */
TR::Instruction* generateExtendedHighWordInstruction(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op,
    TR::Register* targetReg, TR::Register* srcReg, int8_t imm8, TR::Instruction* preced)
{
    ...
    if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        ...
        case TR::InstOpCode::LHHR: // Load (High <- High)
            cursor
                = generateRIEInstruction(cg, TR::InstOpCode::RISBHG, node, targetReg, srcReg, 0, 31 + 0x80, 0, preced);
            COMMENT = "LHHR : Load (High <- High)";
            break;
        case TR::InstOpCode::LHLR: // Load (High <- Low)
            cursor
                = generateRIEInstruction(cg, TR::InstOpCode::RISBHG, node, targetReg, srcReg, 0, 31 + 0x80, 32, preced);
            COMMENT = "LHLR : Load (High <- Low)";
            break;
        case TR::InstOpCode::LLHFR: // Load (Low <- High)
            cursor
                = generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetReg, srcReg, 0, 31 + 0x80, 32, preced);
            COMMENT = "LLHFR : Load (Low <- High)";
            break;
        case TR::InstOpCode::LLHHHR: // Load Logical Halfword (High <- High)
            cursor
                = generateRIEInstruction(cg, TR::InstOpCode::RISBHG, node, targetReg, srcReg, 16, 31 + 0x80, 0, preced);
 ...
 */
bool canThrowDecimalOverflowException(TR::InstOpCode::Mnemonic op)
{
    switch (op) {
    ...
    TR::CodeGenerator* cg, TR::Node* n, TR::Instruction* instr, TR::InstOpCode::Mnemonic op, bool hasNOP)
{
    ...
    if (BCDCHKNode && BCDCHKNode->getOpCodeValue() == TR::BCDCHK) {
        ...
        TR::InstOpCode::S390BranchCondition bc
            = canThrowDecimalOverflowException(op) ? TR::InstOpCode::COND_CC3 : TR::InstOpCode::COND_NOP;
