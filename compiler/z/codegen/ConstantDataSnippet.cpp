...
TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	case TR_ClassObject: {
		AOTcgDiag3(comp, "add relocation (%d) cursor=%x symbolReference=%x\n", reloType, cursor,
		        getSymbolReference());
		TR::SymbolReference *reloSymRef = (reloType == TR_ClassAddress) ? getNode()->getSymbolReference()
		                                                                : getSymbolReference();
		if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
			TR_ASSERT_FATAL(getDataAs8Bytes(), "Static Sym can not be NULL");

			cg()->addExternalRelocation(
			        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)getDataAs8Bytes(),
			                (uint8_t *)TR::SymbolType::typeClass, TR_SymbolFromManager, cg()),
			        __FILE__, __LINE__, getNode());

		} else {
			cg()->addExternalRelocation(
			        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)reloSymRef,
	...
	case TR_ClassPointer:
		...
		if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
			...
			TR::Relocation *relo;
			// for optimizations where we are trying to relocate either profiled j9class or getfrom
			// signature we can't use node to get the target address so we need to pass it to relocation in
			// targetaddress2 for now two instances where use this relotype in such way are: profile
			...
			relo = new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)getNode(),
			        targetAdress2, (TR_ExternalRelocationTargetKind)reloType, cg());
	...
	case TR_DebugCounter: {
		TR::DebugCounterBase *counter =
		        cg()->comp()->getCounterFromStaticAddress(getNode()->getSymbolReference());
		if (counter == NULL) {
	...
	default: TR_ASSERT(0, "relocation type not handled yet");
	}
	...
	if (getUnresolvedDataSnippet() != NULL) {
		...
		TR_ASSERT(udsPatchLocation != NULL,
...
TR::S390ConstantDataSnippet::emitSnippetBody()
{
	...
	case TR_ClassAddress:
	case TR_ClassObject: {
		uintptrj_t romClassPtr = TR::Compiler->cls.persistentClassPointerFromClassPointer(
...
TR_Debug::print(TR::FILE *pOutFile, TR::S390ConstantDataSnippet *snippet)
{
	...
	if (snippet->getKind() == TR::Snippet::IsWritableData) {
		...
		// Cold Eyecatcher is used for padding of endPC so that Return Address for exception snippets will never
		// equal the endPC.
