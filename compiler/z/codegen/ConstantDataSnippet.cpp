...
void TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    case TR_ClassPointer: {
        ...
        // node to get the target address so we need to pass it to relocation in targetaddress2 for now two instances
        // where use this relotype in such way are: profile checkcast and arraystore check object check optimiztaions
        ...
        if (std::find(comp->getMethodSnippetsToBePatchedOnClassUnload()->begin(),
            ...
            void* classPointer = (void*)cg()
                                     ...
                                     ->createResolvedMethod(cg()->trMemory(),
                                         (TR_OpaqueMethodBlock*)(*(uintptrj_t*)cursor), comp->getCurrentMethod())
                                     ->classOfMethod();
