...
uint8_t* TR::S390LabelInstruction::generateBinaryEncoding()
{
    ...
    if (tryToUseLabelTargetNOPs) {
        ...
        if (tryToUseLabelTargetNOPs) {
            ...
            // the offset that matters is the offset relative to the start of the object (the object start is 256 byte
            // aligned but programs in this object may only be 8 byte aligned) instructionStart-cg()->getCodeStart() is
            // the offset just for this one routine or nested routine so have to add in the object size for any already
            // compiled separate programs (batchObjCodeSize) and any offset for any already compiled parent programs for
            // the nested case (getCodeSize)
            ...
            if (traceLabelTargetNOPs) {
                ...
                traceMsg(comp,
                    ...
                    isSkipForLabelTargetNOPs() ? "true" : "false", offsetForLabelTargetNOPs, cg()->getCodeStart());
    ...
    if (tryToUseLabelTargetNOPs) {
        ...
        if (offset > labelTargetNOPLimit) {
            ...
            if (traceLabelTargetNOPs)
                traceMsg(comp,
                    "\tset useLabelTargetNOPs = true : offsetForLabelTargetNOPs=%p > labelTargetNOPLimit %d, "
                    "offset=%lld, cursor %p -> %p, labelTargetBytesInserted=%d\n",
                    offsetForLabelTargetNOPs, labelTargetNOPLimit, offset, cursor, newInstructionStart,
    ...
    if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
        ...
        if (doUseLabelTargetNOPs) {
            if (traceLabelTargetNOPs)
                traceMsg(comp,
                    "\tfinished NOP insertion : setBinaryLength to 0, addAccumError = estBinLen %d - bytesInserted %d "
                    "= %d, setBinaryEncoding to 0x%x\n",
            ...
            TR_ASSERT(getEstimatedBinaryLength() >= labelTargetBytesInserted,
                "lable inst %p : estimatedBinaryLength %d must be >= labelTargetBytesInserted %d inserted\n", this,
                getEstimatedBinaryLength(), labelTargetBytesInserted);
...
bool TR::S390LabelInstruction::considerForLabelTargetNOPs(bool inEncodingPhase)
{
    ...
    if (isSkipForLabelTargetNOPs()) {
        ...
        // encoding phases and therefore this routine would return false during estimation and true during encoding and
    ...
    if (comp->getOption(TR_EnableLabelTargetNOPs)) {
        if (considerForLabelTargetNOPs(false)) // inEncodingPhase=false (in estimating phase now)
        {
            ...
            if (getLabelSymbol() != NULL) {
                if (traceLabelTargetNOPs)
                    traceMsg(comp,
                        "\tupdate label %s (%p) estimatedCodeLocation with estimate %d + maxNOPBump %d = %d\n",
...
int32_t TR::S390RILInstruction::adjustCallOffsetWithTrampoline(int32_t offset, uint8_t* currentInst)
{
    ...
    if (cg()->comp()->getOption(TR_StressTrampolines)
        || (!CHECK_32BIT_TRAMPOLINE_RANGE(getTargetPtr(), (uintptrj_t)currentInst))) {
        ...
        if (getSymbolReference()->getReferenceNumber() < TR_S390numRuntimeHelpers)
            targetAddr
                = cg()->fe()->indexedTrampolineLookup(getSymbolReference()->getReferenceNumber(), (void*)currentInst);
        else
...
uint8_t* TR::S390RSInstruction::generateBinaryEncoding()
{
    ...
    if (getMemoryReference() != NULL) {
        ...
        if ((displacement > MINLONGDISP && displacement < 0)
...
uint8_t* TR::S390RXInstruction::generateBinaryEncoding()
{
    ...
    if ((displacement > MINLONGDISP && displacement < MINDISP)
        || (displacement >= MAXDISP && displacement < MAXLONGDISP)) {
        auto longDisplacementMnemonic = TR::Instruction::opCodeCanBeAdjustedTo(getOpCodeValue());
...
uint8_t* TR::S390NOPInstruction::generateBinaryEncoding()
{
    ...
    if (getKindNOP() == FastLinkCallNOP) { // assumes 4 byte length
        ...
        if (getTargetSnippet() != NULL) {
            ...
            // The offset is a 16 bit field containg the offset in double worlds from the call site to the call
            // descriptor. Our snippet is always emitted later than our NOP.
            ...
            intptrj_t distance
                = snippetOffset - currentOffset + 8; // Add 8 to conservatively estimate the alginment of NOP.
            ...
            if (distance < 0x3FFF8 && distance > 0) {
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR::LabelRelative16BitRelocation(cursor + 2, getTargetSnippet()->getSnippetLabel(), 8));
            } else {
                ...
                getCallDescInstr()->setCallDescValue(
                    ((TR::S390ConstantDataSnippet*)getTargetSnippet())->getDataAs8Bytes(), cg()->trMemory());
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR::LabelRelative16BitRelocation(cursor + 2, getCallDescInstr()->getCallDescLabel(), 8));
            }
        }
        cursor += 4;
...
TR::MemoryReference* getFirstReadWriteMemoryReference(TR::Instruction* i)
{
    ...
    default:
        TR_ASSERT(0,
            "This type of instruction needs to be told how to get memory reference if any. opcode=%s kind=%d\n",
