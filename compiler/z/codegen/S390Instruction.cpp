				...
				if (region) {
					...
					for (auto e = entry->getPredecessors().begin();
						...
						for (currInstr = firstInstr;; currInstr = currInstr->getNext()) {
							if (currInstr == NULL) {
								// We might be here if the loop structure isn't
...
TR::S390LabelInstruction::generateBinaryEncoding()
{
	...
	if (tryToUseLabelTargetNOPs) {
		...
		if (tryToUseLabelTargetNOPs) {
			...
			// the offset that matters is the offset relative to the start of the object (the object start
			// is 256 byte aligned but programs in this object may only be 8 byte aligned)
			// instructionStart-cg()->getCodeStart() is the offset just for this one routine or nested
			// routine so have to add in the object size for any already compiled separate programs
			// (batchObjCodeSize) and any offset for any already compiled parent programs for the nested
			// case (getCodeSize)
			...
			if (traceLabelTargetNOPs)
				traceMsg(comp,
				        "\toffsetForLabelTargetNOPs = absOffset 0x%p + batchObjCodeSize 0x%p + "
				        "objCodeSize 0x%p = 0x%p\n",
			...
			if (traceLabelTargetNOPs) {
				...
				traceMsg(comp,
				        "\tTR::S390LabeledInstruction %p (%s) at cursor %p (skipThisOne=%s, offset = "
				        "%p, codeStart %p)\n",
				        this, thisInst->getOpCode().getMnemonicName(), instructionStart,
				        isSkipForLabelTargetNOPs() ? "true" : "false", offsetForLabelTargetNOPs,
				        cg()->getCodeStart());
	...
	} else // must be real LABEL instruction
	{
		...
		if (_alignment != 0) {
			if (tryToUseLabelTargetNOPs && traceLabelTargetNOPs)
				traceMsg(comp,
				        "force tryToUseLabelTargetNOPs to false because _alignment already needed on "
				        "inst %p\n",
				        this);
	...
	if (tryToUseLabelTargetNOPs) {
		...
		if (traceLabelTargetNOPs)
			traceMsg(comp,
			        "\tcomparing offset %lld >? labelTargetNOPLimit %d (offsetForLabelTargetNOPs = %p)\n",
		...
		if (offset > labelTargetNOPLimit) {
			...
			if (traceLabelTargetNOPs)
				traceMsg(comp,
				        "\tset useLabelTargetNOPs = true : offsetForLabelTargetNOPs=%p > "
				        "labelTargetNOPLimit %d, offset=%lld, cursor %p -> %p, "
	...
	if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
		...
		if (offset < 238) {
			...
			instr = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK15,
			        getNode(), getLabelSymbol(), prevInstr);
		...
		if (doUseLabelTargetNOPs) {
			if (traceLabelTargetNOPs)
				traceMsg(comp,
				        "\tfinished NOP insertion : setBinaryLength to 0, addAccumError = estBinLen %d "
				        "- bytesInserted %d = %d, setBinaryEncoding to 0x%x\n",
				        getEstimatedBinaryLength(), labelTargetBytesInserted,
				        getEstimatedBinaryLength() - labelTargetBytesInserted, instructionStart);
			setBinaryLength(0);
			TR_ASSERT(getEstimatedBinaryLength() >= labelTargetBytesInserted,
			        "label inst %p : estimatedBinaryLength %d must be >= labelTargetBytesInserted %d "
			        "inserted\n",
...
TR::S390LabelInstruction::considerForLabelTargetNOPs(bool inEncodingPhase)
{
	...
	if (traceLabelTargetNOPs)
		traceMsg(comp, "considerForLabelTargetNOPs check during %s phase : %p (%s), prev  %p (%s)\n",
		        inEncodingPhase ? "encoding" : "estimating", this, thisInst->getOpCode().getMnemonicName(),
		        prevInst, prevInst->getOpCode().getMnemonicName());
	...
	if (isSkipForLabelTargetNOPs()) {
		...
		// this is the case where some instructions (e.g. SCHEDON/SCHEDOFF) have been inserted between the
		// estimate and encoding phases and therefore this routine would return false during estimation and true
		// during encoding and the estimate bump for NOPs would have been missed
		if (traceLabelTargetNOPs)
			traceMsg(comp,
			        "\t%p (%s) marked with wasEstimateDoneForLabelTargetNOPs = false : set "
			        "doConsider=false\n",
			        this, thisInst->getOpCode().getMnemonicName());
		doConsider = false;
	} else {
	...
	if (comp->getOption(TR_EnableLabelTargetNOPs)) {
		if (considerForLabelTargetNOPs(false)) // inEncodingPhase=false (in estimating phase now)
		{
			...
			if (getLabelSymbol() != NULL) {
				if (traceLabelTargetNOPs)
					traceMsg(comp,
					        "\tupdate label %s (%p) estimatedCodeLocation with estimate %d + "
					        "maxNOPBump %d = %d\n",
					        cg()->getDebug()->getName(getLabelSymbol()), getLabelSymbol(),
					        currentEstimate, maxNOPBump, currentEstimate + maxNOPBump);
				getLabelSymbol()->setEstimatedCodeLocation(currentEstimate + maxNOPBump);
			}
			if (traceLabelTargetNOPs)
				traceMsg(comp, "\tupdate estimatedSize %d by maxNOPBump %d -> %d\n", estimatedSize,
				        maxNOPBump, estimatedSize + maxNOPBump);
...
TR::S390BranchInstruction::generateBinaryEncoding()
{
	...
	if (doRelocation) {
		if (shortRelocation) {
			...
			cg()->addRelocation(
			        new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(relocationPoint, label));
		} else {
			AOTcgDiag1(comp, "add TR_AbsoluteMethodAddress cursor=%x\n", cursor);
			cg()->addRelocation(
			        new (cg()->trHeapMemory()) TR::LabelAbsoluteRelocation(relocationPoint, label));
			cg()->addProjectSpecializedRelocation(
		...
		if (doRelocation) {
			if (shortRelocation) {
				...
				cg()->addProjectSpecializedRelocation(relocationPoint, NULL, NULL,
				        TR_AbsoluteMethodAddress, __FILE__, __LINE__, getNode());
			}
		}
	}
	toRealRegister(getRegisterOperand(1))->setRegisterField((uint32_t *)instructionStart);
	...
	if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
		...
		*(int16_t *)(cursor + 2) |= bos(distance);
		...
		TR_ASSERT_FATAL(false,
		        "Cannot encode branch on index instruction because distance (%d) is out of range", distance);
	}
...
TR::S390DebugCounterBumpInstruction::generateBinaryEncoding()
{
	...
	if (spillNeeded) {
		*(int32_t *)cursor =
		        boi(0xE3005000 | (offsetToLongDispSlot & 0xFFF)); // STG Rscrtch,offToLongDispSlot(,GPR5)
	...
	*(int16_t *)cursor = TR::Compiler->target.is64Bit() ? bos(0x007A) : bos(0x006A); // On 32-bit platforms
	...
	if (spillNeeded) {
		*(int32_t *)cursor =
		        boi(0xE3005000 | (offsetToLongDispSlot & 0xFFF)); // LG Rscrtch,offToLongDispSlot(,GPR5)
		scratchReg->setRegisterField((uint32_t *)cursor); //
		cursor += 4; //
...
TR::S390Imm2Instruction::generateBinaryEncoding()
{
	...
	memset((void *)cursor, 0, getEstimatedBinaryLength());
	TR_ASSERT(
	        getOpCode().getOpCodeValue() == TR::InstOpCode::DC2, "ImmInstruction is for TR::InstOpCode::DC2 only!");
	TR::Compilation *comp = cg()->comp();
...
TR::S390RRFInstruction::generateBinaryEncoding()
{
	...
	case TR::Instruction::IsRRD: // nnnn 1x32 e.g. MSDBR/MSEBR/MSER/MSDR
		...
		if (!isSrc2Pair)
			toRealRegister(getRegisterOperand(3))->setRegisterField((uint32_t *)cursor, 1); // encode src2
			                                                                                // in R3
			                                                                                // position
		else
	...
	case TR::Instruction::IsRRF: // nnnn 3x12 e.g. IEDTR/IEXTR
		...
		if (!isSrc2Pair)
			toRealRegister(getRegisterOperand(3))->setRegisterField((uint32_t *)cursor, 3); // encode src2
			                                                                                // in R3
			                                                                                // position
		else
	...
	case TR::Instruction::IsRRF5: // .,  M4, R1, R2
		...
		setMaskField((uint32_t *)cursor, getMask3(), 3);
		break;
	default: TR_ASSERT(0, "Unsupported RRF format!");
	}
...
TR::S390RILInstruction::generateBinaryEncoding()
{
	...
	if (getTargetSnippet() && getTargetSnippet()->getKind() == TR::Snippet::IsConstantData) {
	        ...
	        || getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
		{
			...
			{
				TR_ASSERT(
				        TR::Compiler->target.is64Bit(), "We should only be here on 64-bit platforms\n");
				...
				//
				if (spillNeeded) {
					*(int32_t *)cursor = boi(0xE3005000
					        | (offsetToLongDispSlot & 0xFFF)); // STG scratchReg,
				...
				if (((uint64_t)addr) >> 32) {
					...
					*(int16_t *)cursor = bos(0xFFFF & (uint16_t)(((uint64_t)addr) >> 32));
					...
					if (addr & 0xFFFFFFFF) //  LLIHF does zero out low word... so only do IILF if
					                       //  non-NULL
					{
				...
				if (spillNeeded) {
					*(int32_t *)cursor = boi(0xE3005000
					        | (offsetToLongDispSlot & 0xFFF)); // LG scratchReg,
					                                           // offToLongDispSlot(,GPR5)
					scratchReg->setRegisterField((uint32_t *)cursor);
					cursor += 4;
					*(int16_t *)cursor = bos(0x0004);
		...
		if (getTargetSnippet() != NULL) {
			cg()->addRelocation(new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(
			        cursor, getTargetSnippet()->getSnippetLabel()));
		} else if (getTargetLabel() != NULL) {
			cg()->addRelocation(
			        new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(cursor, getTargetLabel()));
		}
		...
		if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
			...
			doRegisterPIC = true;
		} else if (std::find(comp->getStaticMethodPICSites()->begin(), comp->getStaticMethodPICSites()->end(),
		                   this)
		...
		TR::Symbol *sym = (getNode() && getNode()->getOpCode().hasSymbolReference()) ? getNode()->getSymbol()
		                                                                             : NULL;
			...
			if (getTargetSnippet() != NULL) {
				...
				if (isHelper) {
					...
					cg()->addProjectSpecializedRelocation(cursor + 2,
					        (uint8_t *)getSymbolReference(), NULL, TR_HelperAddress, __FILE__,
					        __LINE__, getNode());
				}
			}
		}
			...
			if (comp->isRecursiveMethodTarget(getTargetSymbol())) {
				...
				*(int32_t *)(cursor + 2) = boi(((intptrj_t)jitTojitStart - (intptrj_t)cursor) / 2);
				...
				if (getTargetPtr() == 0 && callSymbol) {
					i2 = (int32_t)(
					        ((uintptrj_t)(callSymbol->getMethodAddress()) - (uintptrj_t)cursor)
					        / 2);
				}
#if defined(TR_TARGET_64BIT)
#if defined(J9ZOS390)
				...
				TR::ResolvedMethodSymbol *resolvedMethodSym = sym ? sym->getResolvedMethodSymbol()
				                                                  : NULL;
				...
				if (sym && sym->castToMethodSymbol()->isHelper()) {
					...
					cg()->addProjectSpecializedRelocation(cursor + 2,
					        (uint8_t *)getSymbolReference()->getMethodAddress(), NULL,
...
TR::S390RSInstruction::generateBinaryEncoding()
{
	...
	if (getMemoryReference() != NULL) {
		longDispTouchUpPadding =
		        getMemoryReference()->generateBinaryEncodingTouchUpForLongDisp(cursor, cg(), this);
	}
...
TR::S390RIEInstruction::generateBinaryEncoding()
{
	...
	if (getBranchDestinationLabel()) {
		...
		if ((distance / 2) >= MIN_IMMEDIATE_VAL && (distance / 2) <= MAX_IMMEDIATE_VAL) {
			if (doRelocation) {
				cg()->addRelocation(
				        new (cg()->trHeapMemory()) TR::LabelRelative16BitRelocation(cursor, label));
			} else {
			...
			if (isRR || isRRE) {
				// add the register number for the second register to the buffer after the first
				// register.
				TR::Register *srcReg = getRegForBinaryEncoding(getRegisterOperand(2));
				toRealRegister(srcReg)->setRegister2Field((uint32_t *)cursor);
				cursor += 2;
			} else if (isRI) {
				// TR::InstOpCode::CHI/TR::InstOpCode::CGHI are signed so sign extend the 8 bit
				// immediate value to 16 bits
				(*(int16_t *)(cursor)) = bos((int16_t)getSourceImmediate8());
				cursor += 2;
			} else if (isRIL) {
				// TR::InstOpCode::CLGFI/TR::InstOpCode::CLFI are unsigned so zero extend the 8 bit
				// immediate value to 32 bits
			...
			*(cursor + 1) |= (uint8_t)getMask();
			...
			if (doRelocation) {
				cg()->addRelocation(
				        new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(cursor, label));
			} else {
...
TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void)
{
	...
	if (isRRE || isRR)
		currInstr = generateRRInstruction(
		        cg(), cmpOp, getNode(), getRegisterOperand(1), getRegisterOperand(2), prevInstr);
	else if (isRI)
	...
	// inserted BRC/BRCL so the main cg generateBinaryEncoding loop does not encode the new CHI and BRC/BRCL
...
TR::S390VRRInstruction::getExtendedMnemonicName()
{
	...
	case TR::InstOpCode::VSTRC:
		...
		ccMask = getM6();
		break;
	case TR::InstOpCode::VFAE: printM5 = true; break;

	// handle Vector FP extended mnemonics
	case TR::InstOpCode::VLDE: useBInsteadOfDB = true;
	case TR::InstOpCode::VFSQ: singleSelector = getM4();
	case TR::InstOpCode::WFC:
	case TR::InstOpCode::WFK: printM4 = false; break;
	case TR::InstOpCode::VCDG:
	case TR::InstOpCode::VCDLG:
	case TR::InstOpCode::VCGD:
	case TR::InstOpCode::VCLGD:
	case TR::InstOpCode::VLED: useBInsteadOfDB = true;
	...
	case TR::InstOpCode::VFMS: printM6 = false; break;
...
TR::S390MemMemInstruction::generateBinaryEncoding()
{
	...
	if (getMemoryReference2())
		longDispTouchUpPadding +=
		        getMemoryReference2()->generateBinaryEncodingTouchUpForLongDisp(cursor, cg(), this);
...
TR::S390VirtualGuardNOPInstruction::generateBinaryEncoding()
{
	...
	if (guardForPatching != this) {
		...
		if (label->getCodeLocation() == NULL) {
			cg()->addRelocation(new (cg()->trHeapMemory())
			                TR::LabelAbsoluteRelocation((uint8_t *)(&_site->getDestination()), label));
		} else {
	...
	if (performEmptyPatch) {
		// so at this point we know we are an stop the world guard and that there may be other stop the world
		// guards after us that will use us as their patch point. We now calculate how much space we are sure to
		// have to overwrite to inform us about what to do next
	...
	if (performEmptyPatch && (sumEstimatedBinaryLength > 0)
	        && performTransformation(
	                comp, "O^O CODE GENERATION: Generating empty patch for guard node %p\n", getNode())) {
		doRelocation = false;
...
getFirstReadWriteMemoryReference(TR::Instruction *i)
{
	...
	case TR::Instruction::IsRSY: mr = toS390RSInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsSMI: mr = toS390SMIInstruction(i)->getMemoryReference(); break;
	...
	case TR::Instruction::IsRXYb: mr = toS390MemInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsRSLb: mr = toS390RSLbInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsRX:
	case TR::Instruction::IsRXE:
	case TR::Instruction::IsRXY:
	case TR::Instruction::IsSSF: mr = toS390RXInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsRXF: mr = toS390RXFInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRSa: mr = toS390VRSaInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRSb: mr = toS390VRSbInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRSc: mr = toS390VRScInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRSd: mr = static_cast<TR::S390VRSdInstruction *>(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRV: mr = toS390VRVInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVRX: mr = toS390VRXInstruction(i)->getMemoryReference(); break;
	case TR::Instruction::IsVSI: mr = static_cast<TR::S390VSIInstruction *>(i)->getMemoryReference(); break;
	...
	case TR::Instruction::IsOpCodeOnly: mr = NULL; break;
	default:
		TR_ASSERT(0,
		        "This type of instruction needs to be told how to get memory reference if any. opcode=%s "
		        "kind=%d\n",
