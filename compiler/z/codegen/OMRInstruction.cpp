...
{
    TR_ASSERT(cg->getS390ProcessorInfo()->supportsArch(_opcode.getMinimumALS()),
...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register* reg, TR::Register* instReg)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility()
        && !self()->cg()->comp()->getOption(TR_DisableHighWordRA) && (reg->getKind() != TR_FPR)
        && (instReg->getKind() != TR_FPR) && (reg->getKind() != TR_VRF) && (instReg->getKind() != TR_VRF);
...
bool OMR::Z::Instruction::startOfLiveRange(TR::Register* reg)
{
    ...
    if ((self()->getOpCodeValue() == TR::InstOpCode::SRDA || self()->getOpCodeValue() == TR::InstOpCode::SRDL)
    ...
    if ((self()->getOpCodeValue() == TR::InstOpCode::SLDA || self()->getOpCodeValue() == TR::InstOpCode::SLDL)
        && toS390RSInstruction(self())->getSourceImmediate() >= 32) {
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::RISBG || self()->getOpCodeValue() == TR::InstOpCode::RISBGN
        || self()->getOpCodeValue() == TR::InstOpCode::RISBHG || self()->getOpCodeValue() == TR::InstOpCode::RISBLG) {
        ...
        for (i = 0; i < _targetRegSize; ++i) {
            if (_targetReg[i]->usesRegister(reg) && !self()->usesOnlyRegister(reg)) {
                ...
                if (self()->getOpCodeValue() == TR::InstOpCode::LTR
                    ...
                    return false;
                if (_targetReg[i]->getKind() == TR_GPR64 && self()->getOpCode().is32bit()
...
bool OMR::Z::Instruction::getUsedRegisters(TR::list<TR::Register*>& usedRegs)
{
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::BCR && _sourceRegSize == 1
        && _sourceReg[0]->getRealRegister() == machine->getRealRegister(TR::RealRegister::GPR0))
        return false;
    ...
    if (self()->getOpCode().usesTarget()) {
        for (i = 0; i < _targetRegSize; ++i) {
            ...
            if (rp) {
                ...
                if (i == 0
                    && (self()->getOpCodeValue() == TR::InstOpCode::RISBG
...
bool OMR::Z::Instruction::getDefinedRegisters(TR::list<TR::Register*>& defedRegs)
{
    ...
    if (!((self()->getOpCodeValue() == TR::InstOpCode::LTR || self()->getOpCodeValue() == TR::InstOpCode::LTGR)
            && self()->getRegisterOperand(1) == self()->getRegisterOperand(2))) {
        TR::Register** _targetReg = self()->targetRegBase();
        for (i = 0; i < _targetRegSize; ++i) {
            TR::Register* r = _targetReg[i];
...
bool OMR::Z::Instruction::getKilledRegisters(TR::list<TR::Register*>& killedRegs)
{
    ...
    // A function call via address in a register uses register r1 to hold the target address but this register is also
    // killed.
            ...
            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                for (int32_t j = 0; j < TR::RealRegister::LastHPR - TR::RealRegister::FirstHPR; ++j) {
                    ...
                    machine->setVirtualAssociatedWithReal(
                        (TR::RealRegister::RegNum)(j + TR::RealRegister::FirstHPR), virtReg);
                }
            }
...
uint32_t OMR::Z::Instruction::useSourceRegister(TR::Register* reg)
{
    ...
    if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        ...
        if (reg->getRegisterPair()) {
            ...
            //        cg()->getDebug()->getName(reg->getLowOrder()), cg()->getDebug()->getName(reg->getHighOrder()),
            //        self());
        }
    }
...
TR::Register* OMR::Z::Instruction::assignRegisterNoDependencies(TR::Register* reg)
{
    ...
    if (reg->getRealRegister() != NULL) {
        ...
        if (!skipBookkeeping && realReg->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtReg
            ...
            if (enableHighWordRA && virtReg->getKind() != TR_FPR && virtReg->is64BitReg()
    ...
    if ((reg->getRegisterPair()
        ...
        if (realRegHigh->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegHigh
            ...
            if (enableHighWordRA && virtRegHigh->getKind() != TR_FPR && virtRegHigh->is64BitReg()
                && virtRegHigh->getKind() != TR_VRF) {
        ...
        if (realRegLow->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegLow
            ...
            if (enableHighWordRA && virtRegLow->getKind() != TR_FPR && virtRegLow->is64BitReg()
                && virtRegLow->getKind() != TR_VRF) {
    ...
    for (i = 0; i < _targetRegSize; ++i) {
        ...
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            ...
            if (_targetReg[i]->getRegisterPair()
                ...
                availHPRMask &= ~(toRealRegister(_targetReg[i]->getRegisterPair()->getHighOrder())
                                      ->getHighWordRegister()
                                      ->getRealRegisterMask());
                availHPRMask &= ~(toRealRegister(_targetReg[i]->getRegisterPair()->getLowOrder())
                                      ->getHighWordRegister()
                                      ->getRealRegisterMask());
        ...
        for (i = 0; i < _targetRegSize; ++i) {
            ...
            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                ...
                if ((self()->getOpCodeValue() == TR::InstOpCode::RISBLG
                        || self()->getOpCodeValue() == TR::InstOpCode::RISBHG)
                    && ((TR::S390RIEInstruction*)self())->getExtendedHighWordOpCode().getOpCodeValue()
                        != TR::InstOpCode::BAD) {
            ...
            if (firstNonPairSourceRegister) {
                ...
                if ((self()->getOpCodeValue() == TR::InstOpCode::RISBLG
                    ...
                    firstNonPairSourceRegister->setAssignToHPR(
                        ((TR::S390RIEInstruction*)self())->getExtendedHighWordOpCode().isOperandHW(registerOperandNum));
                } else
...
{
    TR::Compilation* comp = self()->cg()->comp();
    if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF && self()->cg()->supportsHighWordFacility()
...
bool OMR::Z::Instruction::isRet() { return self()->getOpCodeValue() == TR::InstOpCode::RET; }
