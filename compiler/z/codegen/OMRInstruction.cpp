...
#define CTOR_INITIALIZER_LIST \
	_conditions(NULL), _operands(NULL), _useRegs(NULL), _defRegs(NULL), _sourceUsedInMemoryReference(NULL), \
	        _flags(0), _longDispSpillReg1(NULL), _longDispSpillReg2(NULL), _binFreeRegs(0), _targetRegSize(0), \
	        _sourceRegSize(0), _sourceMemSize(0), _targetMemSize(0), _sourceStart(-1), _targetStart(-1)
...
OMR::Z::Instruction::Instruction(
        ...
        : OMR::Instruction(cg, precedingInstruction, op, node), CTOR_INITIALIZER_LIST
{
	TR_ASSERT_FATAL(TR::Compiler->target.cpu.getSupportsArch(_opcode.getMinimumALS()),
	        "Processor detected does not support instruction %s\n", _opcode.getMnemonicName());
        ...
        TR::Instruction *precedingInstruction, bool instFlag, TR::RegisterDependencyConditions *cond, bool condFlag)
{
	...
	if (cg->getCondCodeShouldBePreserved())
		TR_ASSERT((_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() || _opcode.setsSignFlag()
	...
	if (cg->comp()->getOption(TR_EnableEBBCCInfo)) {
		((self()->isLabel() || self()->isCall()) || (instFlag && precedingInstruction == NULL))
		        ? self()->clearCCInfo()
		        : self()->setCCInfo();
	} else {
	...
	if (_opcode.setsCC()) {
		...
		if (!self()->cg()->comp()->getOption(TR_EnableEBBCCInfo)) {
			if (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() || _opcode.setsSignFlag()
				...
				self()->cg()->setHasCCOverflow(
				        _opcode.setsOverflowFlag() && !self()->getNode()->cannotOverflow());
			...
			// track Ops that setsCompareFlag for compare ops as well, so that we may remove redudent
			// compare ops; note this will not intruduce mismatching for compare immediate ops with CC from
			// compare Ops, because compare immed ops looks for setsSignedFlag or setsZeroFlags - compare
			// ops does not sets these flags
			{
				// we have detailed information on the CC setting
				self()->cg()->setHasCCInfo(true);
				self()->cg()->setHasCCOverflow(
				...
				// the CC setting is generic, such as Load & Test Reg, Test Under Mask etc are not
				// handled; or, there is no node associated with this instruction, play it safe and
...
OMR::Z::Instruction::useRegister(TR::Register *reg, bool isDummy)
{
	...
	if (regPair) {
		...
		// S390RegisterDependency also calls useRegister, in this case we do not want to reset the dummy status
		// of these regs
		//
...
static bool
isInternalControlFlowOneEntryOneExit(TR::Instruction *regionEnd, TR::Compilation *comp)
{
...
OMR::Z::Instruction::setupThrowsImplicitNullPointerException(TR::Node *n, TR::MemoryReference *mr)
{
	...
	if (self()->cg()->getSupportsImplicitNullChecks()) {
		...
		// 5. This is the first instruction in the evaluation of this null check node to have met all the
		// conditions
		...
		if (n != NULL & mr != NULL && mr->getCausesImplicitNullPointerException()
			...
			if (comp->useCompressedPointers() && firstChild->getOpCodeValue() == TR::l2a) {
				...
				nullCheckReference =
				        self()->cg()->getCurrentEvaluationTreeTop()->getNode()->getNullCheckReference();
...
OMR::Z::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
	...
	if (self()->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
		...
		// if (kindToBeAssigned == TR_GPR)
		{
			...
			// Step 1 : First traverse the existing associations and remove them so that they don't
			...
			// Step 2 : loop through and set up the new associations (both on the machine and by associating
			// the virtual registers with their real dependencies)
	...
	// we must register assign the instruction object hanging off of the memory reference's
	// constantInstructionSnippet
	TR::Instruction *outOfLineEXInstr = self()->getOutOfLineEXInstr();
	if (outOfLineEXInstr) {
		TR::Instruction *savePrev = outOfLineEXInstr->getPrev();

		outOfLineEXInstr->setPrev(self()->getPrev()); // Temporarily set Prev() instruction of snippet to Prev()
		                                              // of EX just in case we insert LR_move on
		                                              // assignRegisters()
		...
		if (deps) // merge the dependency into the EX deps
		{
			...
			TR::RegisterDependencyConditions *newDeps = new (self()->cg()->trHeapMemory())
		...
		// Java and other languages probably need the value field to be set to the binary encoding of the
		// instruction This can be done any time after register assignment If the value is set here, the snippet
		// can be handled in an identical fashion to a normal constantdataSnippet
	}
	...
	if (self()->getOpCodeValue() == TR::InstOpCode::TBEGIN || self()->getOpCodeValue() == TR::InstOpCode::TBEGINC) {
		...
		for (int32_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1; i++) {
			if (0 != self()->cg()->getS390Linkage()->getPreserved((TR::RealRegister::RegNum)i)) {
				// linkageBasedSaveMask is 8 bit mask where each bit represents consecutive even/odd
				// regpair to save.
				linkageBasedSaveMask |= (1 << (7 - ((i - 1) >> 1))); // bit 0 = GPR0/1, GPR0=1,
	...
	if (self()->isOutOfLineEX()) {
		if (self()->getOpCodeValue() == TR::InstOpCode::EX) {
			...
			TR::S390ConstantInstructionSnippet *cis =
			        (TR::S390ConstantInstructionSnippet *)tempMR->getConstantDataSnippet();
			TR_ASSERT(
			        cis != NULL, "Out of line EX instruction doesn't have a constantInstructionSnippet\n");
			...
			TR::S390ConstantInstructionSnippet *cis =
			        (TR::S390ConstantInstructionSnippet *)((TR::S390RILInstruction *)self())
			                ->getTargetSnippet();
			TR_ASSERT(cis != NULL,
			        "Out of line EXRL instruction doesn't have a constantInstructionSnippet\n");
			if (cis->getInstruction()->containsRegister(reg))
				return true;
		}
	}
...
OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
	...
	// 2nd pass: S390 REAL assigned to S390 REAL (instead of being skipped because it was assigned on the first
	// pass)
	//
	...
	// "The solution is to unblock all _targetRegs before PreCond assignment and reblock them right after (required
	// for srcReg assignment)." "We only unblock those target registers that are not alive anymore. Block them again
...
OMR::Z::Instruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
	...
	if (self()->getOpCode().isBranchOp() && self()->getKind() == IsRIE
		...
		//
		TR_S390OutOfLineCodeSection *oi = self()->cg()->findS390OutOfLineCodeSectionFromLabel(
...
OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies)
{
	...
	if (self()->cg()->getCodeGeneratorPhase() == TR::CodeGenPhase::PeepholePhase) {
		// In Peephole Phase, UseDefRegisters need to reset. The check on non Peephole Phase needs to be avoided
		// here.
	...
	if (loadOrStoreMultiple >= 0) {
		...
		if (lastReg->getRealRegister() && firstReg->getRealRegister()) {
			...
			if (numRegs > 0) {
				...
				for (uint32_t i = lowRegNum; i != highRegNum; i = ((i == 15) ? 0 : i + 1)) // wrap
				                                                                           // around to
				                                                                           // 0 at 15
				{
					TR::RealRegister *reg =
					        self()->cg()->machine()->getRealRegister(i + TR::RealRegister::GPR0);
...
void
OMR::Z::Instruction::setMaskField(uint32_t *instruction, int8_t mask, int8_t nibbleIndex)
{
