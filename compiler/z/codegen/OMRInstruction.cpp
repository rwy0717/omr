...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register* reg, TR::Register* instReg)
{
    ...
    bool enableHighWordRA = self()->cg()->supportsHighWordFacility() && !self()->cg()->comp()->getOption(TR_DisableHighWordRA) && (reg->getKind() != TR_FPR) && (instReg->getKind() != TR_FPR) && (reg->getKind() != TR_VRF) && (instReg->getKind() != TR_VRF);
...
OMR::Z::Instruction::useTargetRegister(TR::Register* reg)
{
    ...
    // mark used bit for HW/LW virtual regs
    if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        if (_opcode.is64bit() || _opcode.is32to64bit() || (TR::Compiler->target.is64Bit() && (self()->getOpCodeValue() == TR::InstOpCode::LA || self()->getOpCodeValue() == TR::InstOpCode::LAY || self()->getOpCodeValue() == TR::InstOpCode::LARL || self()->getOpCodeValue() == TR::InstOpCode::BASR || self()->getOpCodeValue() == TR::InstOpCode::BRASL))) {
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    for (i = 0; i < _targetRegSize; ++i) {
        ...
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            ...
            if (_targetReg[i]->getRegisterPair() && _targetReg[i]->getRegisterPair()->getHighOrder()->getKind() != TR_FPR && _targetReg[i]->getRegisterPair()->getHighOrder()->getKind() != TR_VRF) {
    ...
    if (_targetReg) {
        for (i = 0; i < _targetRegSize; ++i) {
            ...
            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                ...
                if ((self()->getOpCodeValue() == TR::InstOpCode::RISBLG || self()->getOpCodeValue() == TR::InstOpCode::RISBHG) && ((TR::S390RIEInstruction*)self())->getExtendedHighWordOpCode().getOpCodeValue() != TR::InstOpCode::BAD) {
            ...
            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && _targetReg[i]->getKind() != TR_FPR && _targetReg[i]->getKind() != TR_VRF) {
...
{
    TR::Compilation* comp = self()->cg()->comp();
    if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF && self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
