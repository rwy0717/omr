...
TR::S390zLinuxSystemLinkage::S390zLinuxSystemLinkage(TR::CodeGenerator *cg) : TR::SystemLinkage(cg, TR_SystemLinux)
{
	...
	setRegisterFlag(TR::RealRegister::GPR6, IntegerArgumentAddToPost); // I only need to do this for r6 because r6
	                                                                   // is the only preserved linkage register on
	                                                                   // zlinux.
	...
	setRegisterFlag(TR::RealRegister::GPR14, Preserved); // Setting the return address register as preserved so that
	                                                     // way we do not always have to emit stores in prologues /
	                                                     // epilogues This possibly assumes that we don't shuffle
	                                                     // the return address register around elsewhere
...
TR::S390zLinuxSystemLinkage::createPrologue(TR::Instruction *cursor)
{
	...
	if (cg()->isLiteralPoolOnDemandOn() != true && firstSnippet != NULL)
		cursor = new (trHeapMemory()) TR::S390RILInstruction(
		        TR::InstOpCode::LARL, node, getLitPoolRealRegister(), firstSnippet, cursor, cg());
#endif
...
void
TR::S390zLinuxSystemLinkage::generateInstructionsForCall(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
        ...
        TR::LabelSymbol *returnFromJNICallLabel, TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint)
{
...
#if defined(JITTEST)
	{
		...
		if (callNode->getOpCode().isIndirect()) {
			...
			TR::Instruction *callInstr = new (trHeapMemory()) TR::S390RILInstruction(
			        TR::InstOpCode::BRASL, callNode, fpReg, callSymbol, callSymRef, codeGen);
 ...
 */
TR::Register *
TR::S390zLinuxSystemLinkage::callNativeFunction(TR::Node *callNode, TR::RegisterDependencyConditions *deps,
        intptrj_t targetAddress, TR::Register *methodAddressReg, TR::Register *javaLitOffsetReg,
        TR::LabelSymbol *returnFromJNICallLabel, TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint)
{
	/********************************/
	...
	case TR::icalli:
	case TR::acall:
	case TR::acalli: returnRegister = deps->searchPostConditionRegister(getIntegerReturnRegister()); break;
	case TR::lcall:
	case TR::lcalli: {
		if (TR::Compiler->target.is64Bit()) {
	...
	case TR::vcall:
	case TR::vcalli: returnRegister = deps->searchPostConditionRegister(getVectorReturnRegister()); break;
	default: returnRegister = NULL; TR_ASSERT(0, "Unknown direct call Opcode %d.", callNode->getOpCodeValue());
	}
	...
	while (parmCursor != NULL) {
		...
		case TR::Int64:
			if (TR::Compiler->target.is32Bit()) {
				// make sure that we can fit the entire value in registers, not half in registers half
				// on the stack
...
TR::S390zLinuxSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
	...
	case TR::RealRegister::GPR1:
		TR_ASSERT(false, "ERROR: TR::zLinuxSystemLinkage::getRegisterSaveOffset called for volatile reg: %d",
		        srcReg);
		return -1;
		...
		if (getPreserved(REGNUM(i))) {
			...
			if (!findStartReg
				...
				if (lastReg - firstReg == 0)
					...
					cursor = generateRSInstruction(cg(), TR::InstOpCode::getLoadMultipleOpCode(),
					        node, getRealRegister(REGNUM(firstReg)),
