...
getIntToFloatLogicalConversion(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic convertOpCode)
{
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
		switch (convertOpCode) {
		case TR::InstOpCode::CEFBR: return TR::InstOpCode::CELFBR; break;

		case TR::InstOpCode::CDFBR: return TR::InstOpCode::CDLFBR; break;

		case TR::InstOpCode::CXFBR: return TR::InstOpCode::CXLFBR; break;

		case TR::InstOpCode::CEGBR: return TR::InstOpCode::CELGBR; break;

		case TR::InstOpCode::CDGBR: return TR::InstOpCode::CDLGBR; break;

		case TR::InstOpCode::CXGBR: return TR::InstOpCode::CXLGBR; break;
 ...
 */
static TR::InstOpCode::Mnemonic
getFloatToIntLogicalConversion(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic convertOpCode)
{
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
		switch (convertOpCode) {
		case TR::InstOpCode::CFEBR: return TR::InstOpCode::CLFEBR; break;

		case TR::InstOpCode::CFDBR: return TR::InstOpCode::CLFDBR; break;

		case TR::InstOpCode::CFXBR: return TR::InstOpCode::CLFXBR; break;

		case TR::InstOpCode::CGEBR: return TR::InstOpCode::CLGEBR; break;

		case TR::InstOpCode::CGDBR: return TR::InstOpCode::CLGDBR; break;

		case TR::InstOpCode::CGXBR: return TR::InstOpCode::CLGXBR; break;
...
inline TR::Register *
unaryEvaluator(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic regToRegOpCode,
        TR::Register *newTargetRegister = NULL)
{
	TR::Node *firstChild = node->getFirstChild();

	TR::Register *srcRegister = (newTargetRegister != NULL) ? cg->evaluate(firstChild)
...
genLogicalConversionForInt(TR::Node *node, TR::CodeGenerator *cg, TR::Register *targetRegister, int8_t shift_amount)
{
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)) {
		generateRIEInstruction(cg, TR::InstOpCode::RISBGN, node, targetRegister, targetRegister, shift_amount,
		        (int8_t)(63 | 0x80), 0);
	} else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
		generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, targetRegister, targetRegister, shift_amount,
		        (int8_t)(63 | 0x80), 0);
	} else {
...
static void
generateValueConversionToIntType(TR::CodeGenerator *cg, TR::Node *node, TR::Register *targetRegister)
{
	int32_t nshift = 0;
...
convertToFixed(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::Double:
		...
		MSBIntConstInFP = (int64_t)CONSTANT64(0x41e0000000000000);
		break;
	default: TR_ASSERT(false, "Invalid source floating point type\n"); return NULL;
	}
	...
	if (isUnsignedConversion) {
		if (floatToIntLogicalConverted) {
			 ...
			 * *short floating point must use long form for enough precision bits when we subtract by MSB (4
			 * bytes) 0) short to long, if required 1) fp round to 0 2) sub MSB (of target integer, in float
			 * value) 3) convert to fixed if  max<a<+inf, result will be max signed int (MSB-1) if
			 * 0<=a<=max,  result will be converted, fixed value if  min<=a<0,   result will be converted,
			 * fixed value (MSB unset) if -inf<a<min,  result will be min signed int (MSB) 4) XOR MSB  (will
			 * fix sign-bit) 5) Convert the targetRegister value to appropriate integer type, if needed (for
			 * size < Int32)
			 *
			 * The idea behind this is below
			 * (int)(a - MSB) xor MSB = (unsigned int)a
			 *
			 * Why this works: We will subtract by MSB just before conversion. This means that MSB will be 0
			 * for value >= 2^31. If value < 2^31, MSB will be 1 due to borrow and lower bits will NOT be
			 * touched. The XOR MSB test will put the sign back if original exceeded max integral value. xor
			 * will take care of -inf<a<min case (result is min signed value as above) by unsetting MSB,
			 * which gives 0 as required by XLC lang ref.
			 */
			...
			if (firstChild->getDataType() == TR::Float) {
				generateRRInstruction(cg, TR::InstOpCode::LDEBR, node, srcRegCpy, srcRegCpy); // short
				                                                                              // to long
				                                                                              // FP
			...
			// 1) fp round to 0
			//   todo: the only reason this is placed here is because we can specify rounding mode
			//   explicitly to "round to zero".
			//         convertOp for hex doesn't allow setting rounding mode mask, but IEEE does, so we may
			//         not need this here for IEEE
			generateRRFInstruction(
			        cg, roundToIntOp, node, srcRegCpy, srcRegCpy, (uint8_t)0x5 /*round to zero*/, true);
...
convertFromFixed(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::Float:
		...
		isFloatTgt = true;
		break;
	case TR::Double: convertOpCode = TR::InstOpCode::CDFBR; break;
	default: TR_ASSERT(false, "should be unreachable"); return NULL;
	}
	...
	//       We can also take advantage of logical conversions that will save us a branch and add/load lengthen
	//       insns
	...
	if (isUint32Src) {
		...
		case TR::Double:
			...
			break;
		default: TR_ASSERT(false, "unknown target fp type"); break;
		}
		if (isFloatTgt)
...
OMR::Z::TreeEvaluator::floatRemHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	// callNode is used as dangling node used to pass to buildSystemLInkageDispatch to generate a call to fmod
	// helper
	...
	if (node->getDataType() == TR::Float) {
		...
		generateRRFInstruction(
		        cg, TR::InstOpCode::DIEBR, node, targetRegister, secondRegister, tempRegister, 0x5);
		generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
		cFlowRegionStart->setStartInternalControlFlow();
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK3, node,
		        labelNotExact); // remainder not exact
		generateRXEInstruction(cg, TR::InstOpCode::TCEB, node, targetRegister,
		        generateS390MemoryReference(0x800, cg), 0); // c is +0 ?
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, labelOK); // it is
		                                                                                                // not
		                                                                                                // +0
		generateRXEInstruction(cg, TR::InstOpCode::TCEB, node, firstRegister,
		        generateS390MemoryReference(0x555, cg), 0); // a is < +0 ?
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, labelOK); // it is
		                                                                                                // not <
		                                                                                                // +0
		generateRRInstruction(cg, TR::InstOpCode::LCEBR, node, targetRegister, targetRegister); // negate answer
		                                                                                        // to be -0
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelOK); // it
		                                                                                                 // is
		                                                                                                 // not
		                                                                                                 // < +0
		...
		generateRRFInstruction(
		        cg, TR::InstOpCode::DIDBR, node, targetRegister, secondRegister, tempRegister, 0x5);
		generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
		cFlowRegionStart->setStartInternalControlFlow();
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK3, node,
		        labelNotExact); // remainder not exact
		...
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, labelOK); // it is
		                                                                                                // not <
		                                                                                                // +0
		generateRRInstruction(cg, TR::InstOpCode::LCDBR, node, targetRegister, targetRegister); // negate answer
		                                                                                        // to be -0
		generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelOK); // it
		                                                                                                 // is
		                                                                                                 // not
...
l2dHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196) && node->getOpCodeValue() == TR::lu2d) {
		generateRRFInstruction(cg, TR::InstOpCode::CDLGBR, node, targetFloatRegister, longRegister,
...
l2fHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (IntToFloatLogicalConverted)
		generateRRFInstruction(
		        cg, convertOpCode, node, targetFloatRegister, longRegister, (uint8_t)0x5, (uint8_t)0x0);
	else
...
d2lHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->getOpCodeValue() == TR::d2lu)
		generateRRFInstruction(
		        cg, TR::InstOpCode::CLGDBR, node, targetRegister, floatRegister, (int8_t)0x5, true);
	else
		generateRRFInstruction(
		        cg, TR::InstOpCode::CGDBR, node, targetRegister, floatRegister, (int8_t)0x5, true);
