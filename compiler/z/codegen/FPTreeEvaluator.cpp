    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* targetRegister, int8_t shift_amount)
{
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
            generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetRegister, targetRegister, 32 + shift_amount,
                (int8_t)(63 | 0x80), 0);
        } else {
...
TR::Register* commonLong2FloatEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::RegisterDependencyConditions* deps
        = depsNeeded > 0 ? new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, depsNeeded, cg) : NULL;
    ...
    if (isUnsigned) {
        ...
        if (is128) {
            ...
            two_pow_64_hi
                = generateS390MemoryReference((int64_t)CONSTANT64(0x403f000000000000), TR::Int64, cg, litBase);
            two_pow_64_lo = generateS390MemoryReference((int64_t)0, TR::Int64, cg, litBase);
        } else { // float add -2^63 + 2^64 triggers inexact exception in 31bit. Workaround is lengthen to double and add

            deps->addPostCondition(targetReg, TR::RealRegister::AssignAny);
            two_pow_64_hi
                = generateS390MemoryReference((int64_t)CONSTANT64(0x43f0000000000000), TR::Int64, cg, litBase);
        }
        ...
        // the fixed to float convertOp only handles signed source operands so if the converted value is negative and
        // the source was an unsigned number then add 2^64 to correct the value
        TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart, deps);
        TR::Instruction* cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, node, cFlowRegionEnd);
        if (is128) {
...
TR::Register* OMR::Z::TreeEvaluator::lbits2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            TR_ASSERT(false,
                "must ensure sourceReg->getHighOrder() is a 64BitReg in SLLG so local RA will use 64 bit "
                "instructions\n");
            TR::Register* targetReg = cg->allocateRegister(TR_FPR);
            ...
            generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, sourceReg->getHighOrder(), sourceReg->getHighOrder(), 32);
...
TR::Register* OMR::Z::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        node->setSymbolReference(
            cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitMathHelperConvertLongToFloat, false, false, false));
...
{
    return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? l2dHelper64(node, cg)
...
TR::Register* f2lHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRXEInstruction(
        cg, TR::InstOpCode::TCEB, node, tempFloatRegister, generateS390MemoryReference(0xCCF, cg), 0);
    // if result 0, done
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BM, node, label7);
    TR::MemoryReference* evenMRcopy
        = generateS390MemoryReference(*evenMR, 0, cg); // prevent duplicate use of memory reference
    generateRXInstruction(cg, TR::InstOpCode::STE, node, tempFloatRegister, evenMRcopy);
    // Test if NaN
    generateRXEInstruction(
        cg, TR::InstOpCode::TCEB, node, tempFloatRegister, generateS390MemoryReference(0x030, cg), 0);
    // If NaN , go to label6
    ...
    TR::MemoryReference* oddMRcopy
        = generateS390MemoryReference(*oddMR, 0, cg); // prevent duplicate use of memory reference
    ...
    TR::MemoryReference* evenMRcopy2
        = generateS390MemoryReference(*evenMR, 0, cg); // prevent duplicate use of memory reference
    ...
    TR::MemoryReference* evenMRcopy3
        = generateS390MemoryReference(*evenMR, 0, cg); // prevent duplicate use of memory reference
...
TR::Register* d2lHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRXEInstruction(
        cg, TR::InstOpCode::TCDB, node, tempFloatRegister, generateS390MemoryReference(0xCCF, cg), 0);
    // if 0 then done
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    ...
    generateRXEInstruction(
        cg, TR::InstOpCode::TCDB, node, tempFloatRegister, generateS390MemoryReference(0x030, cg), 0);
    // If NaN, go to label6
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, node, label6);
    // Load as positive
...
{
    TR_ASSERT(
        TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "d2lHelper64(...) is for 64bit code-gen only");
    bool checkNaN = true;
