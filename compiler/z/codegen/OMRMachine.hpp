...
class OMR_EXTENSIBLE Machine : public OMR::Machine {
    ...
    /** Used to keep track of blocked registers (HPR/GPR) that upgrades/spill's etc should not use. Typical stores ~0-3
     * registers. */
...
public:
    ...
    TR::Instruction* registerCopy(TR::Instruction* precedingInstruction, TR_RegisterKinds rk, TR::Register* targetReg,
        TR::Register* sourceReg, TR::CodeGenerator* cg, flags32_t instFlags);
    TR::Instruction* registerExchange(TR::Instruction* precedingInstruction, TR_RegisterKinds rk,
    ...
    TR::RealRegister* findBestFreeRegister(TR::Instruction* currentInstruction, TR_RegisterKinds rk,
        TR::Register* virtualReg = NULL, uint64_t availRegMask = 0xffffffff, bool needsHighWord = false);

    TR::RealRegister* freeBestRegister(TR::Instruction* currentInstruction, TR::Register* virtReg, TR_RegisterKinds rk,
        uint64_t availRegMask = 0xffffffff, bool allowNullReturn = false, bool doNotSpillToSiblingHPR = false);
    ...
    void freeRealRegister(TR::Instruction* currentInstruction, TR::RealRegister* targetReal, bool is64BitReg);

    TR::Instruction* freeHighWordRegister(
    ...
    TR::Instruction* coerceRegisterAssignment(TR::Instruction* currentInstruction, TR::Register* virtualRegister,
    ...
    TR_GlobalRegisterNumber getLastGlobalHPRRegisterNumber() { return _lastGlobalHPRRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getFirstGlobalHPRRegisterNumber() { return _firstGlobalHPRRegisterNumber; }
