...
#define TR_PACKED_TO_DECIMAL_FLOAT_PRECISION_ARCH11 \
	16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CDPT to DFP -- df2pd
	   ///< and dd2pd use same instruction
...
#define TR_DECIMAL_FLOAT_TO_PACKED_PRECISION_ARCH11 \
	16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CPDT -- df2pd and dd2pd
...
public:
	...
	TR::Register *assignBestRegisterPair(TR::Register *regPair, TR::Instruction *currInst, bool doBookKeeping,
	...
	bool isLegalEvenOddPair(
	        TR::RealRegister *evenReg, TR::RealRegister *oddReg, uint64_t availRegMask = 0x0000ffff);
	...
	TR::Register *assignBestRegisterSingle(TR::Register *targetRegister, TR::Instruction *currInst,
	        bool doBookKeeping, uint64_t availRegMask = 0xffffffff);
	...
	TR::RealRegister *freeBestRegister(TR::Instruction *currentInstruction, TR::Register *virtReg,
	        TR_RegisterKinds rk, bool allowNullReturn = false);

	TR::RealRegister *findBestRegisterForShuffle(TR::Instruction *currentInstruction,
	        TR::Register *currentAssignedRegister, uint64_t availRegMask = 0xffffffff);

	TR::RealRegister *shuffleOrSpillRegister(
