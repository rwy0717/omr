...
uint32_t
TR::XPLINKCallDescriptorSnippet::generateCallDescriptorValue(TR::S390zOSSystemLinkage *linkage, TR::Node *callNode)
{
	uint32_t result = 0;
	...
	if (TR::Compiler->target.is32Bit()) {
		...
		switch (dataType) {
		case TR::NoType: returnValueAdjust = XPLINK_RVA_RETURN_VOID_OR_UNUSED; break;

		case TR::Int8:
		case TR::Int16:
		case TR::Int32:
		case TR::Address: returnValueAdjust = XPLINK_RVA_RETURN_INT32_OR_LESS; break;

		case TR::Int64: returnValueAdjust = XPLINK_RVA_RETURN_INT64; break;

		case TR::Float: returnValueAdjust = XPLINK_RVA_RETURN_FLOAT4; break;

		case TR::Double: returnValueAdjust = XPLINK_RVA_RETURN_FLOAT8; break;

		default:
			TR_ASSERT_FATAL(
			        false, "Unknown datatype (%s) for call node (%p)", dataType.toString(), callNode);
			break;
		}
		...
		for (int32_t i = firstArgumentChild; (i <= to) && !done; i++, parmCount++) {
			...
			case TR::DecimalDouble:
				...
				break;
#ifdef J9_PROJECT_SPECIFIC
			case TR::DecimalLongDouble: break;
#endif
			}
