...
TR::PPA1Snippet::emitSnippetBody()
{
	...
	*reinterpret_cast<uint16_t *>(cursor) = _linkage->getGPRSaveMask();
	...
	cg()->addRelocation(new (cg()->trHeapMemory())
	                PPA1OffsetToPPA2Relocation(cursor, _linkage->getPPA2Snippet()->getSnippetLabel()));
	...
	*reinterpret_cast<uint8_t *>(cursor) = prologueSize / 2;
	...
	if (_linkage->getStackPointerUpdateLabel() != NULL) {
		offsetToStackPointerUpdate =
		        _linkage->getStackPointerUpdateLabel()->getInstruction()->getBinaryEncoding()
	...
	*reinterpret_cast<uint32_t *>(cursor) = cg()->getAppendInstruction()->getBinaryEncoding()
