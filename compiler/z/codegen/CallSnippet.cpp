...
uint8_t *
TR::S390CallSnippet::storeArgumentItem(
...
uint8_t *
TR::S390CallSnippet::S390flushArgumentsToStack(
		...
		case TR::Float:
			...
			if (floatArgNum < linkage->getNumFloatArgumentRegisters()) {
				buffer = storeArgumentItem(TR::InstOpCode::STE, buffer,
				        machine->getRealRegister(linkage->getFloatArgumentRegister(floatArgNum)),
				        offset, cg);
			}
			floatArgNum++;
			if (rightToLeft) {
...
TR_RuntimeHelper
TR::S390CallSnippet::getInterpretedDispatchHelper(TR::SymbolReference *methodSymRef, TR::DataType type)
{
...
TR::S390CallSnippet::getLength(int32_t estimatedSnippetStart)
{
	...
	return (instructionCountForArguments(getNode(), cg()) + getPICBinaryLength(cg()) + 3 * sizeof(uintptrj_t)
	        + getRuntimeInstrumentationOnOffInstructionLength(cg()) + sizeof(uintptrj_t)); // the last item is for
	                                                                                       // padding
}
