...
void
TR_S390BinaryCommutativeAnalyser::remapInputs(TR::Node *firstChild, TR::Register *firstRegister, TR::Node *secondChild,
        TR::Register *secondRegister, bool nonClobberingDestination)
{
	if (!cg()->useClobberEvaluate()) {
		...
		if (firstChild == secondChild && firstChild->getReferenceCount() == 2 && firstRegister
			...
			// isRegisterClobberable(firstRegister,1) returns false in this latter case as firstRegister is
			// attached to more than one unique node
			setClob1();
			setClob2();
		}
	}
...
void
TR_S390BinaryCommutativeAnalyser::genericAnalyser(TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
        ...
        TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
	...
	if (getOpReg1Reg2()) {
		if (targetLabel != NULL) {
			finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, firstRegister,
			        secondRegister, getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel,
			        false, true);
			...
			if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14)) {
				...
				} else if (root->getOpCodeValue() == TR::imul && firstRegister != NULL
				        && secondRegister != NULL) {
				...
				if (z14OpCode != TR::InstOpCode::BAD) {
					...
					generateRRFInstruction(
					        cg(), z14OpCode, root, nodeReg, firstRegister, secondRegister, 0, 0);
				}
			}
			...
			if (nodeReg == NULL) {
				...
				if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
				        ...
				        && performTransformation(comp,
				                "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: "
					...
					nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any
					                         // more;
				        ...
				        && performTransformation(comp,
				                "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: "
				                "reuse CC from ccInst [%p].",
				                ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
				                debugObj->getName(secondRegister), ccInst)) {
					nodeReg = secondRegister;
					notReversedOperands(); // CCInfo already exists; don't generate compare op any
					                       // more;
				} else {
		...
		if (targetLabel != NULL) {
			finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, secondRegister,
			        firstRegister, (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel,
			        false, true);
			nodeReg = secondRegister;
			notReversedOperands();
		} else {
			...
			if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
			        && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister)
			        && performTransformation(comp,
			                "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, %s]: reuse CC "
			                "from ccInst [%p].",
			                ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
			                debugObj->getName(secondRegister), ccInst)) {
				nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
			        ...
			        && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister)
			        && performTransformation(comp,
			                "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t %s, %s]: reuse CC "
			                "from ccInst [%p].",
			                ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
			                debugObj->getName(secondRegister), ccInst)) {
				nodeReg = secondRegister;
		...
		if (targetLabel != NULL)
			finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, nodeReg,
			        secondRegister, getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel,
			        false, true);
		else
	...
	if (!finalInstr && targetLabel != NULL) {
		generateS390BranchInstruction(cg(), TR::InstOpCode::BRC,
		        getReversedOperands() ? rBranchOpCond : fBranchOpCond, root, targetLabel);
	}
...
TR_S390BinaryCommutativeAnalyser::conversionIsRemoved(TR::Node *root, TR::Node *&child)
{
	if (((root->getDataType() == TR::Int64 && child->getOpCodeValue() == TR::a2l && TR::Compiler->target.is64Bit())
	        ...
	        && child->getFirstChild()->getRegister() == NULL && child->getFirstChild()->getReferenceCount() == 1
 ...
 */
void
TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
			if (regToRegOpCode == TR::InstOpCode::AR) {
				cursor = generateRRRInstruction(
				        cg(), TR::InstOpCode::ARK, root, tempReg, secondRegister, firstRegister);
				done = true;
			} else if (regToRegOpCode == TR::InstOpCode::ALR) {
				cursor = generateRRRInstruction(
				        cg(), TR::InstOpCode::ALRK, root, tempReg, secondRegister, firstRegister);
				done = true;
			} else if (regToRegOpCode == TR::InstOpCode::AGR) {
				cursor = generateRRRInstruction(
				        cg(), TR::InstOpCode::AGRK, root, tempReg, secondRegister, firstRegister);
				done = true;
			} else if (regToRegOpCode == TR::InstOpCode::ALGR) {
				cursor = generateRRRInstruction(
				        cg(), TR::InstOpCode::ALGRK, root, tempReg, secondRegister, firstRegister);
				done = true;
			}
		}
