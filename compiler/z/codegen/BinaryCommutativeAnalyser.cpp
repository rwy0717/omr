            ...
            if (nodeReg == NULL) {
                ...
                if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                    ...
                    && performTransformation(comp,
                           "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: reuse CC from ccInst "
                           "[%p].",
                    ...
                    && performTransformation(comp,
                           "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: reuse CC from ccInst "
                           "[%p].",
                           ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
                           debugObj->getName(secondRegister), ccInst)) {
...
void TR_S390BinaryCommutativeAnalyser::genericLongAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic lowRegToRegOpCode,
    ...
    TR::InstOpCode::Mnemonic highMemToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (root->getOpCodeValue() == TR::land || root->getOpCodeValue() == TR::lor || root->getOpCodeValue() == TR::lxor) {
    ...
    setInputs(firstChild, firstRegister, secondChild, secondRegister, false, false, cg()->comp());
    ...
    } else // RX instruction
    {
        ...
        if (getOpReg1Mem2() || getOpReg3Mem2()) {
            if (!secondHighZero && secondChild->getSymbolReference()->isUnresolved()) {
                ...
                ...
                For trees with one of the children having lushr by 32 bits and in highWordZero when useSecondHighOrder
                or useFirstHighOrder are set and that child is a mem ref, we need to use high order
                 */
            ...
            if (getOpReg3Mem2()) {
                if (firstHighZero) {
                    ...
                    cursor = generateRRInstruction(
                        cg(), copyOpCode, root, targetReg->getLowOrder(), firstRegister->getLowOrder());
                    if (debugObj) {
                        debugObj->addInstructionComment(toS390RRInstruction(cursor), CLOBBER_EVAL);
                    }

                    cursor = generateRRInstruction(
                        cg(), copyOpCode, root, targetReg->getHighOrder(), firstRegister->getHighOrder());
                    if (debugObj) {
                        debugObj->addInstructionComment(toS390RRInstruction(cursor), CLOBBER_EVAL);
                    }
                }
            ...
            if (getOpReg3Mem1()) {
                if (secondHighZero) {
                    ...
                    targetReg = cg()->allocateConsecutiveRegisterPair(lowRegister, highRegister);
                    cursor = generateRRInstruction(
                        cg(), copyOpCode, root, targetReg->getLowOrder(), secondRegister->getLowOrder());
                    if (debugObj) {
                        debugObj->addInstructionComment(toS390RRInstruction(cursor), CLOBBER_EVAL);
                    }

                    cursor = generateRRInstruction(
                        cg(), copyOpCode, root, targetReg->getHighOrder(), secondRegister->getHighOrder());
                    if (debugObj) {
                        debugObj->addInstructionComment(toS390RRInstruction(cursor), CLOBBER_EVAL);
                    }
                }
        ...
        if (memHighZero) {
            ...
            if (secondOperandIsInRegister) {
                cursor
                    = generateRRInstruction(cg(), highRegToRegOpCode, root, targetHigh, longRegister->getHighOrder());
            } else {
    ...
    TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (TR::Compiler->target.cpu.getS390SupportsZ14() && secondChild->getOpCodeValue() == TR::s2l
    ...
    if (getOpReg1Reg2()) {
        ...
        if (!done) {
            if (firstRegister->getKind() == TR_GPR64
                || ((firstChild->getType().isInt64() || TR::Compiler->target.is64Bit())
                ...
                if (cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                    cursor = generateRRInstruction(
                        cg(), TR::InstOpCode::getLoadRegOpCodeFromNode(cg(), root), root, tempReg, firstRegister);
                } else {
                    cursor
                        = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), root, tempReg, firstRegister);
                }
            }
            if (debugObj) {
                debugObj->addInstructionComment(toS390RRInstruction(cursor), CLOBBER_EVAL);
...
void TR_S390BinaryCommutativeAnalyser::longAddAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (root->getOpCodeValue() == TR::luaddc && TR_S390ComputeCC::setCarryBorrow(root->getChild(2), false, cg())) {
        // use ALCR rather than ALR
        //     ALC rather than AL
    ...
    } else // RX instruction
    {
        ...
        if ((ENABLE_ZARCH_FOR_32 && zArchTrexSupported) || setsOrReadsCC) {
            ...
            if (secondOperandIsInRegister) {
                cursor
                    = generateRRInstruction(cg(), TR::InstOpCode::ALCR, root, targetHigh, longRegister->getHighOrder());
            } else {
