    ...
    TR::InstOpCode::Mnemonic highRegToRegOpCode, TR::InstOpCode::Mnemonic lowMemToRegOpCode, TR::InstOpCode::Mnemonic highMemToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (firstChild->isHighWordZero()) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
            ...
            if (firstOp == TR::iu2l || (firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && (firstChild->getSecondChild()->getInt() & LONG_SHIFT_MASK) == 32)) {
                firstChild = firstChild->getFirstChild();
                initFirstChild = firstChild;
    ...
    if (secondChild->isHighWordZero()) {
        ...
        if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
            // bu2l, su2l, c2l need sign-extention
            if (secondOp == TR::iu2l || (secondOp == TR::lushr && secondChild->getSecondChild()->getOpCodeValue() == TR::iconst && (secondChild->getSecondChild()->getInt() & LONG_SHIFT_MASK) == 32)) {
                secondChild = secondChild->getFirstChild();
                initSecondChild = secondChild;
...
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic regToRegOpCode, TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    */
    ...
    if (TR::Compiler->target.cpu.getS390SupportsZ14() && secondChild->getOpCodeValue() == TR::s2l && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->isSingleRefUnevaluated() && secondChild->getFirstChild()->isSingleRefUnevaluated()) {
        setMem2();
    ...
    if (getOpReg1Reg2()) {
        ...
        if (!done) {
            if (firstRegister->getKind() == TR_GPR64 || ((firstChild->getType().isInt64() || TR::Compiler->target.is64Bit()) && cg()->use64BitRegsOn32Bit())) {
