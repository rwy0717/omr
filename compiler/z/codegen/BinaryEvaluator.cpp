...
laddConst(TR::Node* node, TR::CodeGenerator* cg, TR::RegisterPair* targetRegisterPair, int64_t value, TR::RegisterDependencyConditions* deps)
{
    ...
    if (ENABLE_ZARCH_FOR_32 && performTransformation(comp, "O^O Use AL/ALC to perform long add.\n")) { // wrong carry bit to be set
        if (l_value != 0) {
...
generic32BitAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        bool useLA = node->isNonNegative() && !cg->use64BitRegsOn32Bit() && ((value < MAXLONGDISP && value > MINLONGDISP) || (value >= 0 && value <= MAXDISP)) && (TR::Compiler->target.is64Bit() || node->isNonNegative());
...
generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196) && firstChild->getRegister() && !cg->canClobberNodesRegister(firstChild) && ((-value) >= MIN_IMMEDIATE_VAL && (-value) <= MAX_IMMEDIATE_VAL)) {
...
lDivRemGenericEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
    ...
    if (handleConstDenominator) {
        ...
        else if ((shiftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(denominator)) > 0 && performTransformation(comp, "O^O Denominator is powerOfTwo (%d) for ldir/lrem.\n", denominator)) {
    ...
    if (performTransformation(comp, "O^O Using 64bit ldiv on 32bit driver.\n")) {
        ...
        if (!node->getOpCode().isUnsigned() && !node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative() && !(secondChild->getOpCodeValue() == TR::lconst && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
        ...
        {
            if (!node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative() && !(secondChild->getOpCodeValue() == TR::lconst && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
...
lDivRemGenericEvaluator64(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
    ...
    if (!doConditionalRemainder && !node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative() && !(secondChild->getOpCodeValue() == TR::lconst && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
        //  We have to check for a special case where we do 0x8000000000000000/0xffffffffffffffff
        //  JAVA requires we return 0x8000000000000000 rem 0x00000000
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        if (shiftAmount != 0) {
            if (trgReg != srcReg && canUseAltShiftOp) {
                if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && srcReg->assignToHPR() && (altShiftOp == TR::InstOpCode::SLLK || altShiftOp == TR::InstOpCode::SRLK)) {
...
genericRotateAndInsertHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        ...
        if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift()) && firstChild->getRegister() == NULL && firstChild->getReferenceCount() <= 1 && secondChild->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
            ...
            // instructions
            if (cg->supportsHighWordFacility() && firstChild->getFirstChild()->getRegister() && firstChild->getFirstChild()->getRegister()->assignToHPR()) {
                return NULL;
            }
...
lmulHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (performTransformation(comp, "O^O Using 64bit lmul on 32bit platform.\n")) {
        ...
        if (!node->getOpCode().isUnsigned()) // Signed Multiply
        {
            ...
            bool useMGHI = secondChildIsConstant && value >= MIN_IMMEDIATE_VAL && value <= MAX_IMMEDIATE_VAL && secondChild->getRegister() == NULL;
...
lmulHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChildIsConstant) {
        ...
        if (firstChild->getRegister() != NULL && value == 2 && !cg->use64BitRegsOn32Bit()) // 3 way AGEN LA is cracked on zG
        {
            create_LA = true;
        }
...
OMR::Z::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) && ((targetRegister = genericRotateAndInsertHelper(node, cg)) || (targetRegister = TR::TreeEvaluator::tryToReplaceLongAndWithRotateInstruction(node, cg)))) {
        node->setRegister(targetRegister);
...
OMR::Z::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    if ((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) && (targetRegister = genericRotateLeft(node, cg))) {
        node->setRegister(targetRegister);
