 ...
 */
TR::RegisterPair *
lnegFor32Bit(TR::Node *node, TR::CodeGenerator *cg, TR::RegisterPair *targetRegisterPair,
 ...
 */
TR::RegisterPair *
lnegFor128Bit(TR::Node *node, TR::CodeGenerator *cg, TR::RegisterPair *targetRegisterPair,
        TR::RegisterDependencyConditions *dep = 0)
{
	TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
	TR::LabelSymbol *cFlowRegionEnd = generateLabelSymbol(cg);
...
laddConst(TR::Node *node, TR::CodeGenerator *cg, TR::RegisterPair *targetRegisterPair, int64_t value,
	...
	if (performTransformation(comp, "O^O Use AL/ALC to perform long add.\n")) { // wrong carry bit to be set
		...
		if (l_value != 0) {
			...
			generateS390BranchInstruction(
			        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK12, node, cFlowRegionEnd);
         ...
         !node->isNonZero() */ )
      {
		...
		if (!isNonZero) {
			...
			if (addOrSubNode->getFirstChild()->getOpCode().isShift()
				...
				generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
				generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node,
				        targetRegister, 0, TR::InstOpCode::COND_BE, skipAdd);
			}
...
laddHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	bool isCompressionSequence = TR::Compiler->target.is64Bit() && comp->useCompressedPointers()
	        && node->containsCompressionSequence();
	...
	if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
		...
		// we assume that the analyser will use the opcodes provided and does not generate alternative
		// instructions which do not produce a carry.
...
generic32BitAddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (useAHIK) {
			...
			if (!canClobberReg) {
				generateRRInstruction(
				        cg, TR::InstOpCode::getLoadRegOpCode(), node, targetRegister, childTargetReg);
			}
		...
		temp.integerAddAnalyser(node, TR::InstOpCode::AR,
		        type == TR::Int16 ? TR::InstOpCode::AH : TR::InstOpCode::A, TR::InstOpCode::LR);
		targetRegister = node->getRegister();
	}
...
generic32BitSubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	} else // The second child of the sub node isn't a loadConstant type.
	{
		...
		temp.intBinaryAnalyser(
		        node, TR::InstOpCode::SR, type == TR::Int16 ? TR::InstOpCode::SH : TR::InstOpCode::S);
		targetRegister = node->getRegister();
	}
...
lDivRemGenericEvaluator64(TR::Node *node, TR::CodeGenerator *cg, bool isDivision)
{
	...
	if (isUnsigned && isConstDenominator) {
		...
		handleConstDenominator = (denominator == 1) ||
		        ...
		        (denominator > 0 && TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(denominator)
		                && isDivision);
	}
	...
	if (handleConstDenominator) {
		...
		if (denominator == -1) {
			...
			if (!isDivision) // remainder
			{
				...
				// special case, need to check if dividend is 0x8000000000000000 - but need to check it
				// in 2 steps
		...
		// case A/(+-)2^x - division is done using shifts
			...
			if (isDivision) {
				...
				// This adjustment only applies to signed division as we are effectively trying to skirt
				// the sign bit during the shift operation below
				if (!isUnsigned && !firstChild->isNonNegative()) {
					...
					generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node,
					        firstRegister, 0, TR::InstOpCode::COND_BNL, skipSet);
			...
			} else // remainder
			{
				...
				if (!isUnsigned && !firstChild->isNonNegative()) {
					...
					generateShiftAndKeepSelected64Bit(node, cg, firstRegister, firstRegister,
					        64 - shiftAmnt, 63, 0, true, false);
				}
	...
	if (doConditionalRemainder) {
		...
		if (firstChild->isNonNegative()) {
			...
			// LGPR is needed so negative numbers do not always take the slow path after the logical compare
			// (but functionally it is not needed)
		...
		if (value <= TR::getMaxSigned<TR::Int32>()) {
			generateRILInstruction(
			        cg, TR::InstOpCode::CLGFI, node, absDividendReg, static_cast<int32_t>(value));
		} else {
		...
		cFlowRegionStart->setStartInternalControlFlow();
		generateS390BranchInstruction(
		        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK4, node, skipDiv); // branch
		                                                                             // to
		                                                                             // done
		                                                                             // on
		                                                                             // <
	...
	if (isUnsigned) {
		if (doConditionalRemainder)
			generateRRInstruction(
			        cg, TR::InstOpCode::LGR, node, quoRegister, remRegister); // in
			                                                                  // doConditionalRemainder
			                                                                  // case this is
			                                                                  // the slow path
			                                                                  // so do move
			                                                                  // here
		...
		if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
			...
			if (node->getOpCodeValue() == TR::lshl) {
				if (firstChild->getOpCodeValue() == TR::i2l && firstChild->isSingleRefUnevaluated()
					...
					auto mnemonic = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
	...
	if (node->getOpCodeValue() == TR::bushr) {
		if (node->getFirstChild()->getOpCodeValue() == TR::bconst && node->getReferenceCount() == 1
			...
			generateLoad32BitConstant(
			        cg, node->getFirstChild(), node->getFirstChild()->getByte() & 0xFF, srcReg, true);
		} else {
	...
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
		if (node->getOpCodeValue() == TR::lor) {
			...
			if (lushr && lushr->getOpCodeValue() == TR::lushr && lushr->getReferenceCount() == 1
			        && lushr->getRegister() == NULL && lshl->getOpCodeValue() == TR::lshl
		...
		if (node->getOpCodeValue() == TR::lor && andChild && andChild->getRegister() == NULL
			...
			if (mask1 && mask1 == (~mask2) && shiftBy <= 63
				...
				generateRIEInstruction(
				        cg, TR::InstOpCode::RISBG, node, targetReg, fromReg, bitPos, bitPos, shiftBy);
				...
				if (mask1 == mask2
					...
					TR::Register *targetReg = cg->allocateRegister();
					generateRRInstruction(cg, TR::InstOpCode::LGR, node, targetReg, toReg);
					generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, targetReg, fromReg, 0,
					        bitPos, shiftBy);
...
genericRotateAndInsertHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
		...
		if (firstChild->getOpCode().isConversion()) {
			...
			if (!firstChild->getFirstChild()->getType().isIntegral()) // only skip integer to integer
			                                                          // conversions
			{
				return NULL;
			}
		...
		if (node->getOpCode().isAnd()
			...
			case TR::Int64: value = (uint64_t)secondChild->getLongInt(); break;
			case TR::Int32:
				value = ((uint64_t)secondChild->getInt()) & 0x00000000FFFFFFFFL; // avoid sign extension
				break;
			case TR::Int16: value = ((uint64_t)secondChild->getShortInt()) & 0x000000000000FFFFL; break;
			case TR::Int8: value = ((uint64_t)secondChild->getByte()) & 0x00000000000000FFL; break;
			default: TR_ASSERT(0, "genericRotateAndInsertHelper: Unexpected Type\n"); break;
			}
			...
			if (shiftMsBit > msBit) // Conditions for when we don't need any sign extension
			{
				if (firstChild->getFirstChild()->isNonNegative() || firstChild->chkUnsigned()
					...
					traceMsg(comp,
					        "Cannot use RISBG, number could be negative, no sign extension "
					        "available for RISBG\n");
					return NULL;
				}
			}
			...
			if (comp->getOption(TR_TraceCG)) {
				traceMsg(comp,
				        "[%p] and/sh[r,l] => rotated-and-insert: tZeros %d, lZeros %d, popCnt %d\n",
				        node, tZeros, lZeros, popCnt);
				traceMsg(comp,
				        "\t               => rotated-and-insert: shiftMsBit %d, shiftLsBit %d \n",
			...
			if (popCnt == (64 - lZeros - tZeros)) {
				...
				if (msBit > lsBit) {
					...
					generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetReg, sourceReg,
					...
					generateRIEInstruction(cg, mnemonic, node, targetReg, sourceReg, msBit,
					        0x80 + lsBit, shiftAmnt);
				}
        ...
        TR::Node *node, TR::CodeGenerator *cg, int32_t shiftAmount, bool isSignedShift)
{
	if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
		...
		if (node->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst()) {
			...
			// RISBG instructions perform unsigned rotation, so if we are selecting the sign bit via the
			// logical AND we cannot perform a signed shift because we must preserve the sign bit. In
			// general we cannot handle this case because we are not certain our input will be non-negative,
			// hence we must conservatively disallow the optimization in such a case.
			if (longConstValue >= 0x8000000000000000LL && shiftAmount != 0 && isSignedShift) {
				return NULL;
			}
			// The Shift amount operand in RISBG only holds 6 bits so the optimization will not work on
			// shift amounts >63 or < 63.
			if (shiftAmount > 63 || shiftAmount < -63) {
				return NULL;
			}
			// If the node entered is a common node, then there is no guarantee that the child of the common
			// node will still be alive. And when we try to access the value in that node, we might receive
			// values from a node that might've been killed already. Thus, the optimization will not work.
			...
			if (popCnt == (64 - lZeros - tZeros)) {
				...
				if (firstChild->getReferenceCount() > 1 || lZeros > 31 || tZeros > 31
					...
					// [1] If the resultant shifted msb in a right shift is shifted "too far" to the
					// right, then we are basically zeroing the register. This optimization will not
					// be used. [2] Same case if the resultant lsb of a left shift is shifted out of
					// range. Eg[1]: 000...0011 >> 3. msb = 62, lsb = 63. If we right shift, then
					// msb and lsb will both be > 63 (out of range) Eg[2]: 11100...00 << 10. msb =
					// 0, lsb = 2. If we left shift, then msb and lsb will both be < 0 (out of
					// range)
				...
				if ((firstChild->getReferenceCount() > 1 || (lOnes < 32 && tOnes < 32))
				        && shiftAmount == 0) {
					// Similar to above, if the shift amount is NOT zero, then we must account for
					// the shifted range
					if ((shiftAmount < 0 && msBit - shiftAmount > 63)
					        || (shiftAmount > 0 && lsBit - shiftAmount < 0)) {
						return NULL;
			...
			if (doTransformation
			        && performTransformation(comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
				TR::Register *targetReg = cg->allocateRegister();
				...
				// If the shift amount is zero, this instruction sets the rotation factor to 0 and sets
				// the zero bit(0x80). So it's effectively zeroing out every bit except the inclusive
				// range of lsBit to msBit. The bits in that range are preserved as is.
				//
				// If the shift amount is NOT zero, then we must account for the shifted range
				// [1] If the shifted lsb of a left shift is still in range, but the msb is out of
				// range, then we can still "capture" the shifted bits by upperbounding the msb. [2]
				// Similar case if the msb of a right shift is still in range but the lsb is out of
				// range.
				//
				// Eg[1]: 00...0111 >> 2. msb = 61, lsb = 63. If we right shift, we get 000...0001, so
				// we can still preserve the last bit. Eg[2]: 1110...00 << 2. msb = 0, lsb = 2. If we
				// left left, we can still preserve the first bit.
				...
				generateRIEInstruction(cg, opCode, node, targetReg, sourceReg, rangeStart,
				        0x80 + rangeEnd, shiftAmount);
				cg->decReferenceCount(firstChild);
				cg->decReferenceCount(secondChild);
				return targetReg;
			}
...
lmulHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChildIsConstant) {
		...
		if (firstChild->getRegister() != NULL && value == 2 && TR::Compiler->target.is64Bit()) // 3 way AGEN LA
		                                                                                       // is cracked on
		                                                                                       // zG
		{
			create_LA = true;
...
OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getInt() == 0) {
		...
		if (isUnsigned) {
			...
			cursor =
			        generateRRInstruction(cg, TR::InstOpCode::MR, node, targetRegisterPair, secondRegister);
		}
	}
...
OMR::Z::TreeEvaluator::idivEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (value == -1 && !node->getOpCode().isUnsigned()) {
			...
			generateS390ImmOp(
			        cg, TR::InstOpCode::C, node, targetRegister, targetRegister, (int32_t)0x80000000);
			...
			if (!firstChild->isNonNegative() && !node->getOpCode().isUnsigned()) {
				...
				generateS390ImmOp(cg, TR::InstOpCode::A, node, targetRegister, targetRegister,
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (!isUnsigned && (value == -1)) {
			...
			if (!isUnsigned && !firstChild->isNonNegative()) {
				...
				generateRILInstruction(
				        cg, TR::InstOpCode::NILF, node, tempRegister1, 0xFFFFFFFF - absValue + 1);
				generateRRInstruction(cg, TR::InstOpCode::SR, node, targetRegister, tempRegister1);
				cg->stopUsingRegister(tempRegister1);
				cg->stopUsingRegister(tempRegister2);
			} else {
				generateShiftAndKeepSelected31Bit(node, cg, targetRegister, targetRegister,
				        0x3f & (32 - shftAmnt), 31, 0, true, false);
			}
			generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, done, deps);
		} else {
			// Division with no exception check
...
OMR::Z::TreeEvaluator::integerRolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (rotateAmount == 0) {
			...
			generateRSInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL, node,
