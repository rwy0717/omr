...
#define PRINT_ME(string, node, cg) \
    TR::Delimiter evalDelimiter(TR::comp(), TR::comp()->getOption(TR_TraceCG), "EVAL", string)
#else
...
inline TR::Register* generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        case TR::Address:
            TR_ASSERT(
                TR::Compiler->target.is32Bit(), "generic32BitSubEvaluator(): unexpected data type for 64bit code-gen!");
        case TR::Int32: {
            PRINT_ME("iconst", node, cg);
            value = secondChild->getInt();
            break;
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196) && firstChild->getRegister()
 ...
 */
inline TR::Register* lDivRemGenericEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Compilation* comp = cg->comp();
    ...
    if (handleConstDenominator) {
        ...
        // case A/-1 Q=-A, R=0
        if (denominator == -1 && performTransformation(comp, "O^O Denominator is -1 for ldir/lrem.\n")) {
            if (!isDivision) // remainder
            {
                ...
                generateS390ImmOp(cg, TR::InstOpCode::C, node, dividendPair->getHighOrder(),
                ...
                generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node, dividendPair->getLowOrder(),
                    (signed char)0, TR::InstOpCode::COND_BE, cFlowRegionEnd);
        ...
        // case A/1 Q=A, R=0
        else if (denominator == 1 && performTransformation(comp, "O^O Denominator is 1 for ldir/lrem.\n")) {
            if (!isDivision) // remainder
            {
                generateLoad32BitConstant(cg, node, 0, dividendPair->getLowOrder(), true);
                generateLoad32BitConstant(cg, node, 0, dividendPair->getHighOrder(), true);
        ...
        // case A/(+-)2^x - division is done using shifts
            ...
            if (isDivision) {
                if (!firstChild->isNonNegative()) {
                    ...
                    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
                    cFlowRegionStart->setStartInternalControlFlow();
                    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node, dividendPair->getHighOrder(),
                        (signed char)0, TR::InstOpCode::COND_BNL, skipSet);
            ...
            } else // remainder
            {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    TR::RegisterPair* tempRegisterPair1
                        = cg->allocateConsecutiveRegisterPair(tempRegister1h, tempRegister1l);
                    TR::RegisterPair* tempRegisterPair2
                        = cg->allocateConsecutiveRegisterPair(tempRegister2h, tempRegister2l);
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, tempRegisterPair2->getLowOrder(), dividendPair->getLowOrder());
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, tempRegisterPair2->getHighOrder(), dividendPair->getHighOrder());
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, tempRegisterPair1->getLowOrder(), dividendPair->getLowOrder());
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, tempRegisterPair1->getHighOrder(), dividendPair->getHighOrder());
                    generateRSInstruction(cg, TR::InstOpCode::SRDA, node, tempRegisterPair2, 63);
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegisterPair2->getLowOrder(),
                        static_cast<int32_t>(absValueOfDenominator - 1));
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegisterPair2->getHighOrder(),
                        static_cast<int32_t>((absValueOfDenominator - 1) >> 32));
                    generateRRInstruction(
                        cg, TR::InstOpCode::ALR, node, dividendPair->getLowOrder(), tempRegisterPair2->getLowOrder());
                    generateRRInstruction(cg, TR::InstOpCode::ALCR, node, dividendPair->getHighOrder(),
                        tempRegisterPair2->getHighOrder());
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, dividendPair->getLowOrder(),
                    ...
                    if (absValueOfDenominator != static_cast<int32_t>(absValueOfDenominator))
                        generateRILInstruction(cg, TR::InstOpCode::NILF, node, dividendPair->getHighOrder(),
                            static_cast<int32_t>((CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenominator + 1) >> 32));
                    generateRRInstruction(
                        cg, TR::InstOpCode::SLR, node, tempRegisterPair1->getLowOrder(), dividendPair->getLowOrder());
                    generateRRInstruction(cg, TR::InstOpCode::SLBR, node, tempRegisterPair1->getHighOrder(),
                        dividendPair->getHighOrder());
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, dividendPair->getLowOrder(), tempRegisterPair1->getLowOrder());
                    generateRRInstruction(
                        cg, TR::InstOpCode::LR, node, dividendPair->getHighOrder(), tempRegisterPair1->getHighOrder());
    ...
    if (performTransformation(comp, "O^O Using 64bit ldiv on 32bit driver.\n")) {
        ...
        if (!node->getOpCode().isUnsigned() && !node->isSimpleDivCheck() && !firstChild->isNonNegative()
        ...
        generateRSInstruction(
            cg, TR::InstOpCode::SLLG, node, dividendPair->getHighOrder(), dividendPair->getHighOrder(), 32);
        ...
        generateRSInstruction(
            cg, TR::InstOpCode::SLLG, node, divisorPair->getHighOrder(), divisorPair->getHighOrder(), 32);
        ...
        {
            if (!node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative()
                && !(secondChild->getOpCodeValue() == TR::lconst
                       && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
                ...
                generateS390ImmOp(cg, TR::InstOpCode::CG, node, dividendPair->getHighOrder(),
                    dividendPair->getHighOrder(), (int64_t)(CONSTANT64(0x8000000000000000)), dependencies, litPoolBase);
                ...
                if (!isDivision) {
                    cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node,
                        divisorPair->getHighOrder(), (signed char)-1, TR::InstOpCode::COND_BNE, gotoDiv);
                    ...
                    if (compareInstr && compareInstr->getNumRegisterOperands() == 2) {
                        dependencies->addPostCondition(
                            compareInstr->getRegisterOperand(2), TR::RealRegister::AssignAny);
                    }
                    ...
                    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, divisorPair->getHighOrder(),
                        (signed char)-1, TR::InstOpCode::COND_BE, dontDiv);
                }
        ...
        } else // Unsigned Divide Code
        {
            ...
            generateRRInstruction(
                cg, TR::InstOpCode::XGR, node, dividendPair->getHighOrder(), dividendPair->getHighOrder());
        ...
        if (isDivision) {
            generateRSInstruction(
                cg, TR::InstOpCode::SRLG, node, dividendPair->getHighOrder(), dividendPair->getLowOrder(), 32);
        } else // remainder
        {
            generateRRInstruction(
                cg, TR::InstOpCode::LR, node, dividendPair->getLowOrder(), dividendPair->getHighOrder());

            generateRSInstruction(
                cg, TR::InstOpCode::SRLG, node, dividendPair->getHighOrder(), dividendPair->getHighOrder(), 32);
        }
        ...
        if (isDivision) {
            cursor = generateDirectCall(cg, node, false,
                cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390longDivide, false, false, false), dependencies);
        } else {
            cursor = generateDirectCall(cg, node, false,
                cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390longRemainder, false, false, false), dependencies);
        }
...
inline TR::Register* lDivRemGenericEvaluator64(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
            ...
            if (isDivision) {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    generateS390CompareAndBranchInstruction(
                        cg, TR::InstOpCode::CG, node, firstRegister, (signed char)0, TR::InstOpCode::COND_BNL, skipSet);
            ...
            } else // remainder
            {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    generateShiftAndKeepSelected64Bit(
                        node, cg, firstRegister, firstRegister, 64 - shiftAmnt, 63, 0, true, false);
                }
    ...
    if (!doConditionalRemainder && !node->isSimpleDivCheck() && !firstChild->isNonNegative()
        && !secondChild->isNonNegative()
        && !(secondChild->getOpCodeValue() == TR::lconst
               && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
        //  We have to check for a special case where we do 0x8000000000000000/0xffffffffffffffff
    ...
    TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR)
{
    ...
    //
    // Note:  D and DR require that R1=0 before executing the division
    bool needCheck = (!node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative());
    ...
    if (needCheck) {
        ...
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::C, node, sourceRegister, (signed char)-1, TR::InstOpCode::COND_BNE, doDiv);
 ...
 */
inline TR::Register* genericLongShift(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    TR_ASSERT(
        TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit(), "genericLongShift is for 32bit code-gen only! ");
...
inline TR::Register* genericLongShiftSingle(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
            ...
            if (node->getOpCodeValue() == TR::lshl) {
                if (firstChild->getOpCodeValue() == TR::i2l && firstChild->isSingleRefUnevaluated()
                    ...
                    auto mnemonic = cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)
        ...
        if ((shiftOp == TR::InstOpCode::SRLG || shiftOp == TR::InstOpCode::SLLG)
            && secondChild->isSingleRefUnevaluated() && secondChild->getOpCode().isAnd()
            && secondChild->getOpCode().isInteger() && secondChild->getSecondChild()->getOpCode().isLoadConst()
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp, TR::InstOpCode::Mnemonic altShiftOp)
{
    ...
    if (canUseAltShiftOp && (TR::InstOpCode(altShiftOp).is64bit() || TR::InstOpCode(altShiftOp).is32to64bit()))
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::addrAddHelper(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* genericRotateAndInsertHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        ...
        if (node->getOpCode().isAnd()
            ...
            // instructions
            if (cg->supportsHighWordFacility() && firstChild->getFirstChild()->getRegister()
            ...
            if (shiftMsBit > msBit) // Conditions for when we don't need any sign extension
            {
                if (firstChild->getFirstChild()->isNonNegative() || firstChild->chkUnsigned()
                    ...
                    traceMsg(
                        comp, "Cannot use RISBG, number could be negative, no sign extension available for RISBG\n");
                    return NULL;
                }
            }
            if (lsBit > shiftLsBit)
...
TR::Register* OMR::Z::TreeEvaluator::tryToReplaceLongAndWithRotateInstruction(TR::Node* node, TR::CodeGenerator* cg)
{
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        TR::Node* firstChild = node->getFirstChild();
        TR::Node* secondChild = node->getSecondChild();
        ...
        if (node->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst()) {
            ...
            if (doTransformation && performTransformation(comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
                ...
                TR::InstOpCode::Mnemonic opCode
                    = cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12) ? TR::InstOpCode::RISBGN
                                                                                               : TR::InstOpCode::RISBG;
...
{
    TR_ASSERT(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "Call lsubHelper(...) for 32bit code-gen!");
...
TR::Register* lmulHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChildIsConstant) {
        ...
        if (isNonNegativePowerOf2(src2Value)) {
            if (!noShift) {
                ...
                TR_ASSERT(
                    shiftAmount <= 63 && shiftAmount > 0, "lmulEvaluator - unexpected shift amount for Power of 2.");
    ...
    if (performTransformation(comp, "O^O Using 64bit lmul on 32bit platform.\n")) {
        ...
        if (!node->getOpCode().isUnsigned()) // Signed Multiply
        {
            ...
            bool useMGHI = secondChildIsConstant && value >= MIN_IMMEDIATE_VAL && value <= MAX_IMMEDIATE_VAL
                && secondChild->getRegister() == NULL;
            ...
            cursor = generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, trgtRegPair->getHighOrder(), trgtRegPair->getHighOrder(), 32);

            cursor = generateRRInstruction(
                cg, TR::InstOpCode::LR, node, trgtRegPair->getHighOrder(), trgtRegPair->getLowOrder());
            ...
            if (!useMGHI) {
                ...
                cursor = generateRSInstruction(
                    cg, TR::InstOpCode::SLLG, node, src2RegPair->getHighOrder(), src2RegPair->getHighOrder(), 32);

                cursor = generateRRInstruction(
                    cg, TR::InstOpCode::LR, node, src2RegPair->getHighOrder(), src2RegPair->getLowOrder());

                // do multiplication
                cursor = generateRRInstruction(
                    cg, TR::InstOpCode::MSGR, node, trgtRegPair->getHighOrder(), src2RegPair->getHighOrder());
            } else {
            ...
            cursor = generateRRInstruction(
                cg, TR::InstOpCode::LR, node, trgtRegPair->getLowOrder(), trgtRegPair->getHighOrder());
        ...
        } else // Unsigned Multiply Support
        {
            ...
            // put value in low order because the MLGR instruction will look for the multiplicand in register R1+1 (the
            // odd in the pair)
            cursor = generateRRInstruction(
                cg, TR::InstOpCode::LGR, node, trgtRegPair->getLowOrder(), trgtRegPair->getHighOrder());
            ...
            // shift product back into original trgtRegPair
            cursor = generateRRInstruction(
                cg, TR::InstOpCode::LGR, node, trgtRegPair->getHighOrder(), trgtRegPair->getLowOrder());
        }

        // Common signed / unsigned clean-up code

        cursor = generateRSInstruction(
            cg, TR::InstOpCode::SRLG, node, trgtRegPair->getHighOrder(), trgtRegPair->getHighOrder(), 32);
        ...
        generateRRInstruction(
            cg, TR::InstOpCode::LTR, node, tempRegisterPair->getLowOrder(), trgtRegPair->getLowOrder());
        ...
        generateRRInstruction(
            cg, TR::InstOpCode::ALR, node, trgtRegPair->getHighOrder(), tempRegisterPair->getLowOrder());
        ...
        // Prod += Al*Bh<<32
        cursor = generateRRInstruction(
            cg, TR::InstOpCode::ALR, node, trgtRegPair->getHighOrder(), src2RegPair->getLowOrder());
 ...
 */
TR::Register* dualMulHelper32on64(TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    bool needsUnsignedHighMulOnly = (lmulNode == NULL);
    ...
    // TR::RegisterDependencyConditions * dependencies = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 6,
    // cg);
    TR::RegisterDependencyConditions* dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 3, cg);
    bool highWordRADisabled = !cg->supportsHighWordFacility() || comp->getOption(TR_DisableHighWordRA);
    ...
    cursor = generateRSInstruction(
        cg, TR::InstOpCode::SLLG, node, lmulTargetRegister->getHighOrder(), lmulTargetRegister->getHighOrder(), 32);

    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LR, node, lmulTargetRegister->getHighOrder(), lmulTargetRegister->getLowOrder());

    // put value in low order because the MLGR instruction will look for the multiplicand in register R1+1 (the odd in
    // the pair)
    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LGR, node, lmulTargetRegister->getLowOrder(), lmulTargetRegister->getHighOrder());

    // shift all 64 bits of multiplier into lumulhTargetRegister->getHighOrder()
    cursor = generateRSInstruction(
        cg, TR::InstOpCode::SLLG, node, lumulhTargetRegister->getHighOrder(), lumulhTargetRegister->getHighOrder(), 32);

    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LR, node, lumulhTargetRegister->getHighOrder(), lumulhTargetRegister->getLowOrder());

    // do multiplication
    cursor = generateRRInstruction(
        cg, TR::InstOpCode::MLGR, node, lmulTargetRegister, lumulhTargetRegister->getHighOrder());
    ...
    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LGR, node, lumulhTargetRegister->getHighOrder(), lmulTargetRegister->getHighOrder());

    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LGR, node, lumulhTargetRegister->getLowOrder(), lmulTargetRegister->getHighOrder());
    ...
    cursor = generateRSInstruction(
        cg, TR::InstOpCode::SRLG, node, lumulhTargetRegister->getHighOrder(), lumulhTargetRegister->getHighOrder(), 32);

    // make sure low part of product is in the lumulhTargetRegister, both its highOrder and lowOrder.
    cursor = generateRRInstruction(
        cg, TR::InstOpCode::LGR, node, lmulTargetRegister->getHighOrder(), lmulTargetRegister->getLowOrder());
    ...
    cursor = generateRSInstruction(
        cg, TR::InstOpCode::SRLG, node, lmulTargetRegister->getHighOrder(), lmulTargetRegister->getHighOrder(), 32);
 ...
 */
TR::Register* dualMulHelper32on32(TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::dualMulHelper32(
    TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
...
{
    TR_ASSERT(
        TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "lmulHelper64() is for 64bit code-gen only!");
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CG, node, sourceRegister, (signed char)0, TR::InstOpCode::COND_BNL, posMulh);
    ...
    if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit()) {
        ...
        //
        generateRRInstruction(
            cg, TR::InstOpCode::LGR, node, targetRegisterPair->getLowOrder(), targetRegisterPair->getHighOrder());

        TR::Instruction* cursor;
        cursor = generateRSInstruction(
            cg, TR::InstOpCode::SRLG, node, targetRegisterPair->getHighOrder(), targetRegisterPair->getHighOrder(), 32);
        cursor->setDependencyConditions(dependencies);
...
TR::Register* OMR::Z::TreeEvaluator::integerRolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool useRegPairs
        = TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && node->getOpCodeValue() == TR::lrol;
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (rotateAmount == 0) {
            ...
            // generateRSYInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL,
            // node,targetRegister,0,0); generateRegImmInstruction(ROLRegImm1(nodeIs64Bit), node, targetRegister,
            // rotateAmount , cg);
...
TR::Register* OMR::Z::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            temp.genericLongAnalyser(node, TR::InstOpCode::NR, TR::InstOpCode::NR, TR::InstOpCode::N, TR::InstOpCode::N,
...
TR::Register* OMR::Z::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            temp.genericLongAnalyser(node, TR::InstOpCode::OR, TR::InstOpCode::OR, TR::InstOpCode::O, TR::InstOpCode::O,
                TR::InstOpCode::getLoadRegOpCode());
        }
        targetRegister = node->getRegister();
    }
...
TR::Register* OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            temp.genericLongAnalyser(node, TR::InstOpCode::XR, TR::InstOpCode::XR, TR::InstOpCode::X, TR::InstOpCode::X,
                TR::InstOpCode::getLoadRegOpCode());
        }
        targetRegister = node->getRegister();
    }
