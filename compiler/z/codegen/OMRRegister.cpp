...
{
    return (!_flags.testAny(IsNotHighWordUpgradable) && !self()->isUsedInMemRef() && !self()->containsCollectedReference() && !self()->containsInternalPointer() && !self()->getRegisterPair() && (self()->getKind() == TR_GPR) && !self()->is64BitReg());
}
