...
bool OMR::Z::Register::assignToHPR() { return (!(self()->is64BitReg()) && _flags.testAny(AssignToHPR)); }

bool OMR::Z::Register::assignToGPR() { return (!(self()->is64BitReg()) && !_flags.testAny(AssignToHPR)); }

bool OMR::Z::Register::isHighWordUpgradable()
{
    return (!_flags.testAny(IsNotHighWordUpgradable) && !self()->isUsedInMemRef()
        && !self()->containsCollectedReference() && !self()->containsInternalPointer() && !self()->getRegisterPair()
