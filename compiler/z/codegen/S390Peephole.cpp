    ...
    if (_cursor->getNext()->getOpCodeValue() == storeOpCode) {
        ...
        if (performTransformation(
            ...
            TR::Instruction* newInst
                = generateSS1Instruction(_cg, TR::InstOpCode::MVC, comp()->getStartTree()->getNode(), size - 1,
                    storeInst->getMemoryReference(), loadInst->getMemoryReference(), _cursor->getPrev());
...
{
    while (
        inst && (inst->getKind() == TR::Instruction::IsPseudo || inst->getKind() == TR::Instruction::IsNotExtended)) {
        inst = forward ? inst->getNext() : inst->getPrev();
    }
...
bool TR_S390PostRAPeephole::replaceGuardedLoadWithSoftwareReadBarrier()
{
    ...
    if (shouldPreserveLoadReg) {
        TR::MemoryReference* gsIntermediateAddrMemRef
            = generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetGSIntermediateResultOffset(comp()), _cg);
        _cursor = generateRXInstruction(
            _cg, TR::InstOpCode::STG, load->getNode(), loadTargetReg, gsIntermediateAddrMemRef, prev);
        prev = _cursor;
    }
    ...
    if (load->getOpCodeValue() == TR::InstOpCode::LGG) {
        ...
        _cursor = generateRSInstruction(_cg, TR::InstOpCode::SLLG, load->getNode(), loadTargetReg, loadTargetReg,
            TR::Compiler->om.compressedReferenceShift(), _cursor);
    }

    // Check if concurrent scavange is in progress and if object pointer is in the evacuate space
    TR::MemoryReference* privFlagMR = generateS390MemoryReference(
        vmReg, TR::Compiler->vm.thisThreadGetConcurrentScavengeActiveByteAddressOffset(comp()), _cg);
    _cursor = generateSIInstruction(_cg, TR::InstOpCode::TM, load->getNode(), privFlagMR, 0x00000002, _cursor);
    _cursor = generateS390BranchInstruction(
        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_CC0, load->getNode(), concurrentScavangeNotActiveLabel, _cursor);

    _cursor = generateRXInstruction(_cg, TR::InstOpCode::CG, load->getNode(), loadTargetReg,
        generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetEvacuateBaseAddressOffset(comp()), _cg),
        _cursor);
    _cursor = generateS390BranchInstruction(
        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_CC1, load->getNode(), concurrentScavangeNotActiveLabel, _cursor);

    _cursor = generateRXInstruction(_cg, TR::InstOpCode::CG, load->getNode(), loadTargetReg,
        generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetEvacuateTopAddressOffset(comp()), _cg),
        _cursor);
    _cursor = generateS390BranchInstruction(
        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_CC2, load->getNode(), concurrentScavangeNotActiveLabel, _cursor);
    ...
    TR::MemoryReference* gsOperandAddrMemRef
        = generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetGSOperandAddressOffset(comp()), _cg);
    _cursor
        = generateRXInstruction(_cg, TR::InstOpCode::STG, load->getNode(), loadTargetReg, gsOperandAddrMemRef, _cursor);
    ...
    TR::MemoryReference* gsHelperAddrMemRef
        = generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetGSHandlerAddressOffset(comp()), _cg);
    ...
    _cursor = new (_cg->trHeapMemory())
        TR::S390RRInstruction(TR::InstOpCode::BASR, load->getNode(), raReg, raReg, _cursor, _cg);
    _cursor = generateRRInstruction(_cg, TR::InstOpCode::LGR, load->getNode(), raReg, loadTargetReg, _cursor);

    if (shouldPreserveLoadReg) {
        TR::MemoryReference* restoreBaseRegAddrMemRef
            = generateS390MemoryReference(vmReg, TR::Compiler->vm.thisThreadGetGSIntermediateResultOffset(comp()), _cg);
        _cursor = generateRXInstruction(
            _cg, TR::InstOpCode::LG, load->getNode(), loadTargetReg, restoreBaseRegAddrMemRef, _cursor);
    }
    ...
    if (load->getOpCodeValue() == TR::InstOpCode::LGG) {
        _cursor
            = generateRXInstruction(_cg, TR::InstOpCode::LG, load->getNode(), loadTargetReg, updateLoadMemRef, _cursor);
    } else {
        _cursor = generateRXInstruction(
            _cg, TR::InstOpCode::LLGF, load->getNode(), loadTargetReg, updateLoadMemRef, _cursor);
        _cursor = generateRSInstruction(_cg, TR::InstOpCode::SLLG, load->getNode(), loadTargetReg, loadTargetReg,
            TR::Compiler->om.compressedReferenceShift(), _cursor);
    }

    _cursor = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, load->getNode(), concurrentScavangeNotActiveLabel, _cursor);
    ...
    if (lgrTargetReg == lgrSourceReg
        ...
        if ((prev->getOpCodeValue() == TR::InstOpCode::LR && lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR)
            ...
            if ((lgrTargetReg == prevTargetReg || lgrTargetReg == prevSourceReg)
                && performTransformation(comp(),
                       "\nO^O S390 PEEPHOLE: Transforming load register into load and test register and removing "
                       "current at %p\n",
                       _cursor)) {
                TR::Instruction* newInst
        ...
        TR::Instruction* next = _cursor->getNext();
        // try to remove redundant LTR, LTGR when we can reuse the condition code of an arithmetic logical operation,
        // ie. Add/Subtract Logical this is also done by isActiveLogicalCC, and the end of
        // generateS390CompareAndBranchOpsHelper when the virtual registers match but those cannot handle the case when
        // the virtual registers are not the same but we do have the same restricted register which is why we are
        // handling it here when all the register assignments are done, and the redundant LR's from the clobber evaluate
        // of the add/sub logical are cleaned up as well.
        ...
        if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag()
            && prev->getRegisterOperand(1) == lgrTargetReg && next->getOpCodeValue() == TR::InstOpCode::BRC) {
        ...
        if (curOpCode.getOpCodeValue() == lgrOpCode.getOpCodeValue() && current->getKind() == TR::Instruction::IsRR) {
            ...
            if (((curSourceReg == lgrTargetReg && curTargetReg == lgrSourceReg)
                ...
                if ((!lgrSetCC || !(setCC || useCC))) {
                    ...
                    if (performTransformation(
                            comp(), "O^O S390 PEEPHOLE: Duplicate LR/CPYA removal at %p\n", current)) {
                        if (comp()->getOption(TR_TraceCG)) {
                            printInfo("\nDuplicate LR/CPYA:");
                            printInstr(comp(), current);
                            char tmp[50];
    ...
    if (curOpCode == TR::InstOpCode::LGFR || curOpCode == TR::InstOpCode::LLGTR) {
        ...
        if (llcTgtReg == curSrcReg && llcTgtReg == curTgtReg) {
            ...
            if (performTransformation(comp(), "O^O S390 PEEPHOLE: Transforming LLC/%s to LLGC at %p.\n",
                ...
                _cursor = generateRXInstruction(_cg, TR::InstOpCode::LLGC, comp()->getStartTree()->getNode(), llcTgtReg,
                    memRef, _cursor->getPrev());
                _cg->replaceInst(oldCursor, _cursor);
                performed = true;
            }
        }
    ...
    if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
        ...
        if (performTransformation(
            ...
            _cg->replaceInst(loadInst,
                _cursor = generateRXYInstruction(_cg, LZOpCode, comp()->getStartTree()->getNode(), loadTargetReg,
...
{
    // z10 specific
    if (!_cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
        || _cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
        return false;
    }
...
bool TR_S390PostRAPeephole::trueCompEliminationForCompareAndBranch()
{
    ...
    if (backwardBranch) {
        if ((prev && btar) && (prev->usesRegister(compReg) || btar->usesRegister(compReg))
            ...
            if (performTransformation(comp(),
                    "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 1 at %p.\n", curr)) {
                swapOperands(trueReg, compReg, curr);
                if (next && next->usesRegister(trueReg))
                    insertLoad(comp(), _cg, next->getPrev(), compReg);
                return true;
        ...
        if ((prev && next) && (prev->usesRegister(compReg) || next->usesRegister(compReg))
            ...
            if (!isWCodeCmpEqSwap
                && performTransformation(comp(),
                       "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 2 at %p.\n", curr)) {
                swapOperands(trueReg, compReg, curr);
    ...
    if (_cursor->throwsImplicitException()) {
        ...
        if (hasPadding) {
            if (nextInst->getOpCodeValue() == TR::InstOpCode::NOP)
                ...
                TR_ASSERT(
                    0, "S390 PEEPHOLE: DAA instruction padding was not found following instruction %p.\n", nextInst);
        ...
        if ((nextInst->getOpCodeValue() == TR::InstOpCode::BRC || nextInst->getOpCodeValue() == TR::InstOpCode::BRCL)
            ...
            if (OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0)
                if (performTransformation(comp(),
                        "O^O S390 PEEPHOLE: Eliminating redundant DAA NOP BRC/BRCL to OOL path %p.\n", OOLbranchInst)) {
...
bool TR_S390PostRAPeephole::DAAHandleMemoryReferenceSpill(bool hasPadding)
{
    ...
    if (_cursor->throwsImplicitException()) {
        ...
        if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC && nextInst->getOpCodeValue() != TR::InstOpCode::BRCL) {
            if (performTransformation(comp(),
                    "O^O S390 PEEPHOLE: Eliminating DAA intrinsic due to a memory reference spill near instruction "
                    "%p.\n",
                    currInst)) {
                currInst = _cursor;
                nextInst = _cursor->getNext();
    ...
    if (performTransformation(comp(),
        ...
        if (instr->getSourceImmediate()) {
            ...
            newInstr = new (_cg->trHeapMemory()) TR::S390RSInstruction(
                newOpCode, instr->getNode(), instr->getRegisterOperand(1), memRef, instr->getPrev(), _cg);
        } else {
...
bool TR_S390PostRAPeephole::inlineEXtargetHelper(TR::Instruction* inst, TR::Instruction* instrB4EX)
{
    if (performTransformation(comp(), "O^O S390 PEEPHOLE: Converting LARL;EX into EXRL instr=[%p]\n", _cursor)) {
        ...
        // generate EXRL
        TR::S390RILInstruction* newEXRLInst = new (_cg->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::EXRL,
            _cursor->getNode(), _cursor->getRegisterOperand(1), ssInstrLabel, _cursor->getPrev(), _cg);
        ...
        if (current->usesRegister(lgrTargetReg)) {
            if (current->defsRegister(lgrTargetReg)) {
                ...
                case TR::InstOpCode::AR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::ARK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::AGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::AGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::ALR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::ALRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::ALGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::ALGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::AHI: {
                    int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
                    newInstr = generateRIEInstruction(
                        _cg, TR::InstOpCode::AHIK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                case TR::InstOpCode::AGHI: {
                    int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
                    newInstr = generateRIEInstruction(
                        _cg, TR::InstOpCode::AGHIK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                case TR::InstOpCode::NR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::NRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::NGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::NGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::XR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::XRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::XGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::XGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::OR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::ORK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::OGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::OGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::SLA: {
                    ...
                    if (mf != NULL) {
                        ...
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SLAK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
                    } else
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SLAK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                case TR::InstOpCode::SLL: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SLLK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
                    } else
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SLLK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                ...
                case TR::InstOpCode::SRA: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SRAK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
                    } else
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SRAK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                case TR::InstOpCode::SRL: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SRLK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
                    } else
                        newInstr = generateRSInstruction(
                            _cg, TR::InstOpCode::SRLK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
                    break;
                }
                case TR::InstOpCode::SR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::SRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::SGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::SGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::SLR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::SLRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                case TR::InstOpCode::SLGR: {
                    newInstr = generateRRRInstruction(
                        _cg, TR::InstOpCode::SLGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
                    break;
                }
                default:
                    return false;
...
void TR_S390PostRAPeephole::reloadLiteralPoolRegisterForCatchBlock()
{
    ...
    if (TR::Compiler->target.isZOS() && !isZ10 && !_cg->isLiteralPoolOnDemandOn()) {
        ...
        if ((_cg->getLinkage())->setupLiteralPoolRegister(firstSnippet) > 0) {
            // the imm. operand will be patched when the actual address of the literal pool is known at binary encoding
            // phase
        ...
        while (nextInstruction != NULL && !nextInstruction->getOpCode().readsCC()) {
            if (nextInstruction->getOpCode().setsCC() || nextInstruction->getNode()->getOpCodeValue() == TR::BBEnd) {
                ...
                TR::Instruction* xrInstruction
                    = generateRRInstruction(_cg, TR::InstOpCode::XR, lhiInstruction->getNode(),
...
void TR_S390PostRAPeephole::perform()
{
    ...
    while (_cursor != NULL) {
        ...
        case TR::InstOpCode::LHI: {
            // This optimization is disabled by default because there exist cases in which we cannot determine whether
            // this transformation is functionally valid or not. The issue resides in the various runtime patching
            // sequences using the LHI instruction as a runtime patch point for an offset. One concrete example can be
            // found in the virtual dispatch sequence for unresolved calls on 31-bit platforms where an LHI instruction
            // is used and is patched at runtime.
            //
            // TODO (Issue #255): To enable this optimization we need to implement an API which marks instructions that
            // will be patched at runtime and prevent ourselves from modifying such instructions in any way.
            //
            // ReduceLHIToXR();
        } break;
        ...
        case TR::InstOpCode::EX: {
            ...
            if (_cursor->isOutOfLineEX() && !comp()->getCurrentBlock()->isCold() && !(bool)disableEXRLDispatch
                && _cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10))
                inlineEXtarget();
            break;
        }
                    ...
                    if (instructionsMatch) {
                        if (cast_na->matchesTargetRegister(cast_nb->getRegisterOperand(1))
                            ...
                            //
                                ...
                                && ((cast_nb->getSourceImmediate() & cast_na->getSourceImmediate())
