 ...
 */
extern "C" void
_patchVirtualGuard(uint8_t *locationAddr, uint8_t *destinationAddr, int32_t smpFlag)
{
	...
	if (debugTrace) {
		printf("####> Patching VGNOP at locationAddr %p (%x), destinationAddr %p (%x), smpFlag: %d\n",
		        volatileLocationAddr, *reinterpret_cast<volatile intptrj_t *>(volatileLocationAddr),
		        destinationAddr, *reinterpret_cast<intptrj_t *>(destinationAddr), smpFlag);
	}
	...
	// we are patching it. Because the caller / codegen ensured this branch will never execute if it is to be
	// patched asynchronously, modifying the displacement first should always be valid. Thus we modify the
	// displacement first, and then activate the branch mask to avoid any potential race conditions.
