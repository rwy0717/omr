...
TR::OptionTable OMR::Options::_jitOptions[] = {
    ...
    // For PLX debug use
    { "disableLongRegAllocation", "O\tdisable allocation of 64-bit regs on 32-bit",
        SET_OPTION_BIT(TR_Disable64BitRegsOn32Bit), "F" },
    { "disableLongRegAllocationHeuristic", "O\tdisable heuristic for long register allocation",
    ...
    { "disablePrefetchInsertion", "O\tdisable prefetch insertion", TR::Options::disableOptimization, prefetchInsertion,
        0, "P" },
    ...
    { "disableRedundantBCDSignElimination", "I\tdisable redundant BCD sign elimination",
    ...
    { "disableStoreAnchoring",
        "O\tin trivialStoreSinking disable store child anchoring and therefore more aggressively duplicate trees",
    ...
    { "disableTrivialStoreSinking", "O\tdisable trivial store sinking", RESET_OPTION_BIT(TR_EnableTrivialStoreSinking),
        "F" },
    { "disableTrueRegisterModel",
        "C\tdisable use of true liveness model in local RA instead of Future Use Count (Dangerous use unless no Global "
        "Virtual Regs used)",
        RESET_OPTION_BIT(TR_EnableTrueRegisterModel), "F" },
    { "disableUncountedUnrolls", "O\tdisable GLU from unrolling uncoutned loops ",
    ...
    { "enableJProfilingInProfilingCompilations", "O\tuse jprofiling instrumentation in profiling compilations",
        SET_OPTION_BIT(TR_EnableJProfilingInProfilingCompilations), "F" },
    ...
    { "enableLateCleanFolding", "O\tfold pdclean flags into pdstore nodes right before codegen",
        SET_OPTION_BIT(TR_EnableLateCleanFolding), "F" },
    { "enableLinkagePreserveStrategy2", "O\tenable linkage strategy 2", SET_OPTION_BIT(TR_LinkagePreserveStrategy2),
        "F" },
    ...
    { "enableLongRegAllocationHeuristic", "O\tenable heuristic for long register allocation",
    ...
    { "enableMethodTrampolineReservation",
        "O\tReserve method trampolines even if they are not needed; only applicable on x86 and zLinux",
    ...
    { "enableTrivialStoreSinking", "O\tenable trivial store sinking", SET_OPTION_BIT(TR_EnableTrivialStoreSinking),
        "F" },
    { "enableTrueRegisterModel", "C\tUse true liveness model in local RA instead of Future Use Count",
    ...
    { "sinkAllBlockedStores",
        "O\tin trivialStoreSinking sink all stores that are blocked by a killed sym by creating an anchor",
        SET_OPTION_BIT(TR_SinkAllBlockedStores), "F" },
    { "sinkAllStores",
        "O\tin trivialStoreSinking sink all stores possible by agressively creating anchors for indirect loads and "
        "killed syms",
    ...
    { "traceRegDepCopyRemoval", "L\ttrace register dependency copy removal", TR::Options::traceOptimization,
        regDepCopyRemoval, 0, "P" },
    { "traceRegisterITF", "L\ttrace register interference graph (basic)", TR::Options::setBitsFromStringSet,
        offsetof(OMR::Options, _traceRegisterITF), TR_TraceRegisterITFBasic, "F" },
    { "traceRegisterITF=",
        "L{regex}\tlist of additional register interference graph build options: basic, listing, results, build, "
        "details, live, colourability",
    ...
    { "traceTrivialStoreSinking", "L\ttrace trivial store sinking", TR::Options::traceOptimization, trivialStoreSinking,
        0, "P" },
    ...
    if (_optFileName) {
        ...
        if (_debug) {
            ...
            if (_customStrategy) {
                ...
                TR_VerboseLog::write(
                    "<JIT: WARNING: ignoring optFile option; unable to read opts from '%s'\n", _optFileName);
            }
        }
    }
        ...
        *((int32_t*)((char*)base + entry->parm1)) = 0; // Reset all bits to zero when this function is called
        ...
        if (!regex)
            ...
            if (*((int32_t*)((char*)base + entry->parm1)) == 0x00000000)
                TR_VerboseLog::write(
                    "<JIT: Address enumeration option not found.  No address enumeration option was set.>");
        }
    }
...
OMR::Options::TR_OptionStringToBit OMR::Options::_optionStringToBitMapping[] = {
    ...
    { "deps", TR_TraceRADependencies }, { "details", TR_TraceRADetails },
    { "preRA", TR_TraceRAPreAssignmentInstruction }, { "spillTemps", TR_TraceRASpillTemps },
    { "states", TR_TraceRARegisterStates }, { "listing", TR_TraceRAListing },
    ...
    { "basic", TR_TraceRegisterITFBasic }, { "build", TR_TraceRegisterITFBuild },
...
int32_t OMR::Options::getJitMethodEntryAlignmentBoundary(TR::CodeGenerator* cg)
{
