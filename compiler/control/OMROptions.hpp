...
enum TR_CompilationOptions {
	...
	TR_UnresolvedAreNotColdAtCold = 0x00004000 + 4, // cold block marker marks unresolved blocks as cold at hotness
	                                                // cold or less
	...
	TR_ReservingLocks = 0x00004000 + 6, // Can be merged with TR_DisableLockResevation when lock reservation is
	...
	TR_DisableVSSStackCompaction = 0x00100000 + 17, // never compact variable size symbols -- if not force or
	                                                // disable then use heuristic based on opt level and VSS total
	                                                // size
	...
	TR_DisableOutlinedNew = 0x00200000 + 18, // Expand object allocation logic inline instead of using a fast jit
	                                         // helper
	...
	TR_DisableClearCodeCacheFullFlag = 0x00000200 + 21, // Disable the re-enabling of the code cache when a method
...
public:
	...
	TR_FilterBST *find(const char *methodName, int32_t methodNameLen, const char *methodClass,
...
public:
	...
	static int32_t _lowCodeCacheThreshold; // Turn off Iprofiler if available code cache space is lower than this
	                                       // value
...
public:
	...
	TR_MCTLogs(int32_t compThreadID, TR::Options *options)
	        : _next(NULL), _compThreadID(compThreadID), _options(options), _logFile(NULL)
	{}
