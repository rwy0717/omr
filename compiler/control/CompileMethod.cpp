...
writePerfToolEntry(void *start, uint32_t size, const char *name)
{
	...
	if (firstAttempt) {
		...
		int numCharsWritten = snprintf(perfFilename, maxPerfFilenameSize, "/tmp/perf-%" OMR_PRId64 ".map",
	...
	if (perfFile) {
		...
		// j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t) getMetadata()->startPC,
		// getMetadata()->endWarmPC - getMetadata()->startPC,
		...
		//                  TR::comp()()->signature(),
		//                  getCompilation()->getHotnessName(getCompilation()->getMethodHotness())); // should
...
static bool
methodCanBeCompiled(OMR::FrontEnd *fe, TR_ResolvedMethod &method, TR_FilterBST *&filter, TR_Memory *trMemory)
{
...
uint8_t *
compileMethod(OMR_VMThread *omrVMThread, TR_ResolvedMethod &compilee, TR_Hotness hotness, int32_t &rc)
{
        ...
        OMR_VMThread *omrVMThread, TR::IlGeneratorMethodDetails &details, TR_Hotness hotness, int32_t &rc)
{
	...
	TR_ASSERT(TR::comp() == NULL,
	        "there seems to be a current TLS TR::Compilation object %p for this thread. At this point there should "
	        "be no current TR::Compilation object",
	        TR::comp());
	...
	try {
		...
		if (rc == COMPILATION_SUCCEEDED) // success!
		{
			...
			if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance)) {
				...
				TR_VerboseLog::writeLine(TR_Vlog_COMP,
				...
				if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
					TR_VerboseLog::write(" time=%llu mem=%lluKB", translationTime,
					        static_cast<unsigned long long>(scratchSegmentProvider.bytesAllocated())
					                / 1024);
				}
