        ...
        for (TR::PreorderNodeIterator iter(comp->getStartTree(), comp); iter != NULL; ++iter) {
            ...
            if (node->getOpCode().isNullCheck()) {
                ...
                if (nodeBeingNullChkd) {
                    ...
                    if (firstChild->getOpCode().isCall() && firstChild->getOpCode().isIndirect()) {
                        parentOfNullChkdNode = firstChild->getFirstChild();
                    } else {
                        parentOfNullChkdNode = firstChild;
                    }

                    if (parentOfNullChkdNode && parentOfNullChkdNode->getOpCodeValue() == TR::aloadi && parentOfNullChkdNode->getNumChildren() > 0 && parentOfNullChkdNode->getFirstChild() == nodeBeingNullChkd) {
                ...
                if (node->getOpCodeValue() == TR::aloadi && !unAnchorableAloadiNodes->isSet(node->getGlobalIndex())) {
                    shouldAnchorNode = true;
                } else if (node->getOpCodeValue() == TR::aload && node->getSymbol()->isStatic() && node->getSymbol()->isCollectedReference()) {
                    shouldAnchorNode = true;
                }
