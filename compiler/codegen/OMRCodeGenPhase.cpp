...
void OMR::CodeGenPhase::performSetupForInstructionSelectionPhase(TR::CodeGenerator* cg, TR::CodeGenPhase* phase)
{
    ...
    if (TR::Compiler->target.cpu.isZ() && TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads()) {
        ...
        std::map<TR::TreeTop*, TR::TreeTop*, std::less<TR::TreeTop*>,
            TR::typed_allocator<std::pair<TR::TreeTop* const, TR::TreeTop*>, TR::Allocator> >
        ...
        for (TR::PreorderNodeIterator iter(comp->getStartTree(), comp); iter != NULL; ++iter) {
            ...
            if (node->getOpCode().isNullCheck()) {
                ...
                if (nodeBeingNullChkd) {
                    ...
                    if (parentOfNullChkdNode && parentOfNullChkdNode->getOpCodeValue() == TR::aloadi
                ...
                } else if (node->getOpCodeValue() == TR::aload && node->getSymbol()->isStatic()
