...
struct TR_RegisterPressureState {
	...
	uint32_t _pressureRiskUntilEnd; // Nonzero if a value has been stored to the candidate, the value would not be
	                                // in a register without GRA, and the candidate is live on an exit we haven't
	                                // reached yet
	uint32_t _numLiveCandidateLoads; // Number of isCandidateLoad nodes that have been evaluated, but whose refcount
	                                 // is not yet zero
	uint32_t _rematNestDepth; // How many nodes between current node and current treetop will be duplicated by
	                          // rematerialization
	vcount_t _visitCountForInit; // Used to indicate that a node's entry in the node state array has been
	                             // initialized
...
struct TR_RegisterPressureSummary {
	...
	{
		accumulate(other._gprPressure, other._fprPressure, other._vrfPressure, other._linkageConventionMask,
		        other._spillMask);
	}
...
private:
	        ...
	        uint32_t linkageConventionMask = 0, uint32_t spillMask = 0)
	{
		...
		/*
		         TR_ASSERT(_gprPressure <= INT_MAX/2 && gprPressure <= INT_MAX/2, "gprPressure is getting too
		   high!"); TR_ASSERT(_fprPressure <= INT_MAX/2 && fprPressure <= INT_MAX/2, "fprPressure is getting too
		   high!"); TR_ASSERT(_vrfPressure <= INT_MAX/2 && vrfPressure <= INT_MAX/2, "vrfPressure is getting too
		   high!");
...
struct TR_SimulatedNodeState {
	...
	TR::TreeTop *_keepLiveUntil; // Register pressure contribution should not be subtracted until this tree has been
	                             // simulated
...
void simulateNodeInitialization(TR::Node *node, TR_RegisterPressureState *state); // ok to call if node is already
                                                                                  // initialized
