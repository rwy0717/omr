...
public:
	...
	LabelRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::Relocation(p), _label(l) {}
...
public:
	...
	LabelRelative8BitRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::LabelRelocation(p, l) {}
	virtual void apply(TR::CodeGenerator *codeGen);
};
...
class LabelRelative16BitRelocation : public TR::LabelRelocation {
	// field _instructionOffser is used for 16-bit relocation when we need to specify where relocation starts
...
public:
	LabelRelative16BitRelocation() : TR::LabelRelocation(), _addressDifferenceDivisor(1) {}
...
public:
	...
	LabelRelative24BitRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::LabelRelocation(p, l) {}
	virtual void apply(TR::CodeGenerator *codeGen);
};
...
public:
	...
	LabelRelative32BitRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::LabelRelocation(p, l) {}
	virtual void apply(TR::CodeGenerator *codeGen);
};
...
public:
	 ...
	 *
	 *  \param divisor
	 *     The width of each unit of the encoding. For example if \p divisor is 2 then the value which will be
	 * encoded at \p cursor + \p offset will be the number of half-words between \p cursor and \p l
	 */
...
public:
	InstructionAbsoluteRelocation(uint8_t *updateLocation, TR::Instruction *i, bool useEndAddr) /* specified if the
	                                                                                               start or the end
	                                                                                               address of the
	                                                                                               instruction is
	                                                                                               required */
...
public:
	...
	LabelAbsoluteRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::LabelRelocation(p, l) {}
	virtual void apply(TR::CodeGenerator *codeGen);
};
...
public:
	...
	LoadLabelRelative16BitRelocation(
	        TR::Instruction *i, TR::LabelSymbol *start, TR::LabelSymbol *end, int32_t delta)
	        : TR::Relocation(NULL)
	        , _lastInstruction(i)
	        , _startLabel(start)
	        , _endLabel(end)
...
public:
	...
	IteratedExternalRelocation(uint8_t *target, uint8_t *target2, TR_ExternalRelocationTargetKind k,
...
public:
	...
	ExternalRelocation(
	        uint8_t *p, uint8_t *target, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator *codeGen)
	        : TR::Relocation(p), _targetAddress(target), _targetAddress2(NULL), _kind(kind), _relocationRecord(NULL)
	{}
...
public:
	...
	BeforeBinaryEncodingExternalRelocation(TR::Instruction *instr, uint8_t *target,
	        ...
	        : TR::ExternalRelocation(NULL, target, kind, codeGen), _instruction(instr)
	{}

	BeforeBinaryEncodingExternalRelocation(TR::Instruction *instr, uint8_t *target, uint8_t *target2,
	        TR_ExternalRelocationTargetKind kind, TR::CodeGenerator *codeGen)
	        : TR::ExternalRelocation(NULL, target, target2, kind, codeGen), _instruction(instr)
	{}
...
public:
	...
	LabelTable32BitRelocation(uint8_t *p, TR::LabelSymbol *l) : TR::LabelRelocation(p, l) {}
	virtual void apply(TR::CodeGenerator *codeGen);
};
