	...
	if (comp->cg()->needGuardSitesEvenWhenGuardRemoved() && ifNode->isTheVirtualGuardForAGuardedInlinedCall()) {
		...
		if (virtualGuard->getKind() == TR_HCRGuard) {
			if (comp->getOption(TR_TraceReloCG))
				traceMsg(comp,
				        "createGuardSiteForRemovedGuard: removing HCRGuard, no need to add AOTNOPsite, "
				        "node %p\n",
				        ifNode);
			return;
		}
		...
		if (virtualGuard->getKind() == TR_BreakpointGuard) {
			if (comp->getOption(TR_TraceReloCG))
				traceMsg(comp,
				        "createGuardSiteForRemovedGuard: removing BreakpointGuard, no need to add "
				        "AOTNOPsite, node %p\n",
				        ifNode);
			return;
		}
		...
		case TR_DirectMethodGuard:
			...
			break;
		default:
			TR_ASSERT(false, "removed virtual guard type %d on node %p not supported yet.",
		...
		if (comp->getOption(TR_TraceAll))
			traceMsg(comp,
			        "createGuardSiteForRemovedGuard: removedGuardKind %d, removedGurad %p, _callNode %p, "
			        "_guardNode %p, _thisClass %p, _calleeIndex %d, _byteCodeIndex %d, addedAOTNopSite "
			        "%p\n",
