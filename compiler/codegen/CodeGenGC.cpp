...
OMR::CodeGenerator::buildGCMapForInstruction(TR::Instruction *instr)
{
	...
	for (auto location = self()->getCollectedSpillList().begin(); location != self()->getCollectedSpillList().end();
		...
		if ((*location)->containsCollectedReference()
			...
			//
			if ((TR::Compiler->target.cpu.isPower() || TR::Compiler->target.cpu.isZ())
				...
				if (self()->getDebug())
					traceMsg(comp,
					        ...
					        map, s->getGCMapIndex(),
					        self()->getDebug()->getName((*location)->getSymbolReference()), instr);
				continue;
			}
	...
	if (instr->needsGCMap()) {
		map = instr->getGCMap();
	        ...
	        && (instr->getNode()->getByteCodeInfo().getCallerIndex()
	                ...
	                || 0)) {
		// find the previous instruction that contains a map and copy it with the current instruction's byte
		// code info
		//
