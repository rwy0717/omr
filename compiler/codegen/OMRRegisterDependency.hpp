...
public:
	 ...
	 *  \note
	 *     This constructor does not actually populate the cache. This is because it is very common to loop through
	 the
	 ...
	 */
	RegisterDependencyMap(TR::RegisterDependency *deps, uint32_t numDeps) : deps(deps)
	{
		TR_ASSERT(numDeps <= SENTINEL, "Number of dependencies supplied (%d) cannot exceed %d!", numDeps,
		        SENTINEL);
	 ...
	 *
	 *  \return
	 *     The dependency whose source (virtual register) is currently assigned to \param regNum if such a
	...
	TR::RegisterDependency *getDependencyWithTarget(TR::RealRegister::RegNum regNum)
	{
		TR_ASSERT(regNum >= 0 && regNum < TR::RealRegister::NumRegisters,
		        "Register number (%d) used as index but out of range!", regNum);
		TR_ASSERT(regNum != TR::RealRegister::NoReg,
	 ...
	 *
	 *  \return
	 *     The source (virtual register) whose dependent target (real register) is \param regNum if such a
	 * dependency exists; <c>NULL</c> otherwise.
	 */
