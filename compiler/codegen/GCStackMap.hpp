...
public:
	...
	TR_InternalPointerPair(TR::AutomaticSymbol *pinningArrayPtr, int32_t internalPtrRegNum)
	        : _pinningArrayPtr(pinningArrayPtr), _internalPtrRegNum(internalPtrRegNum)
	{}
	...
	{
		TR_ASSERT(!internalPointerAuto || internalPointerAuto->isInternalPointerAuto(),
...
public:
	...
	TR_InternalPointerMap(TR_Memory *m)
	        : _trMemory(m), _numInternalPtrs(0), _numDistinctPinningArrays(0), _internalPtrPairs(m), _size(0)
	{}
...
public:
	...
	void *operator new(size_t s, TR_HeapMemory m, uint32_t numberOfSlotsToMap)
	{
		if (numberOfSlotsToMap > DEFAULT_NUMBER_OF_SLOTS_MAPPED) {
			s += ((numberOfSlotsToMap - DEFAULT_NUMBER_OF_SLOTS_MAPPED) + 7) >> 3; // round up to the next
...
public:
	...
	TR_GCStackAllocMap(uint32_t slots) : _numberOfSlotsMapped(slots) { clearBits(); }
