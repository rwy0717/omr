...
// an equality test
bool
OMR::TreeEvaluator::instanceOfOrCheckCastNeedEqualityTest(TR::Node *node, TR::CodeGenerator *cg)
{
...
bool
OMR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *castClassNode = node->getSecondChild();
	...
	if (!TR::TreeEvaluator::isStaticClassSymRef(castClassSymRef)) {
		...
		// changes to platform code. The benefit is little at this point (shows up from reference arraycopy
		// reductions)
...
bool
OMR::TreeEvaluator::nodeIsIArithmeticOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
...
bool
OMR::TreeEvaluator::nodeIsIAddOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
	...
	static TR_Pattern *pattern = pb.choice(pb(TR::ificmpge), pb(TR::ificmplt),
	        pb.children(
	                pb(TR::iand,
	                        pb(TR::ixor,
	                                pb(TR::iadd, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
	                                pb(u, u->leftChild)),
...
bool
OMR::TreeEvaluator::nodeIsISubOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
	TR::Compilation *comp = TR::comp();
	if (comp->getOption(TR_DisableTreePatternMatching))
		return false;
	...
	static TR_Pattern *pattern = pb.choice(pb(TR::ificmpge), pb(TR::ificmplt),
	        pb.children(
	                pb(TR::iand,
	                        pb(TR::ixor,
	                                pb(TR::isub, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
	                                pb(u, u->leftChild)),
...
bool
OMR::TreeEvaluator::nodeIsLArithmeticOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
...
bool
OMR::TreeEvaluator::nodeIsLAddOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
	TR::Compilation *comp = TR::comp();
	if (comp->getOption(TR_DisableTreePatternMatching))
		return false;
	...
	static TR_Pattern *pattern = pb.choice(pb(TR::iflcmpge), pb(TR::iflcmplt),
	        pb.children(
	                pb(TR::land,
	                        pb(TR::lxor,
	                                pb(TR::ladd, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
	                                pb(u, u->leftChild)),
...
bool
OMR::TreeEvaluator::nodeIsLSubOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
	TR::Compilation *comp = TR::comp();
	if (comp->getOption(TR_DisableTreePatternMatching))
		return false;
	...
	static TR_Pattern *pattern = pb.choice(pb(TR::iflcmpge), pb(TR::iflcmplt),
	        pb.children(
	                pb(TR::land,
	                        pb(TR::lxor,
	                                pb(TR::lsub, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
	                                pb(u, u->leftChild)),
...
//
void
OMR::TreeEvaluator::evaluateNodesWithFutureUses(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	if (node->getFutureUseCount() > 0) {
		...
		if (actualLoadOrStoreChild->getOpCode().isStore() || actualLoadOrStoreChild->getOpCode().isLoadConst()
			...
			if (comp->getOption(TR_TraceCG)) {
				traceMsg(comp,
				        "avoiding escaping commoned subtree %p [RealLoad/Store: %p], but processing "
				        "its children: node is ",
			...
			// Do not decrement the reference count here.  It will be decremented when the call node is
			// evaluated again in the helper instruction stream.
			//
...
//
void
OMR::TreeEvaluator::initializeStrictlyFutureUseCounts(TR::Node *node, vcount_t visitCount, TR::CodeGenerator *cg)
{
	if (node->getRegister() != NULL) {
		// Node has already been evaluated outside this tree.
		//
