	...
	if (opcode != TR::BBStart && node->getRegister()) {
		...
		while (_stackOfArtificiallyInflatedNodes.topIndex() > topOfNodeStackBeforeEvaluation) {
			...
			if (artificiallyInflatedNode->getReferenceCount() == 1) {
				...
				//
				// 1a) N is never evaluated, so the ref count never goes down to 1. (node was not
				// commoned in another subtree)
				//
				//     - no tree difference should be seen in this case.
				//
				// 1b) N is evaluated, so the ref count then goes down to 1. (node was commoned in
				// another subtree)
				//
				//     - register shuffling _could_ be seen in this case.
				//     - but a bug might have been avoided: partial and complete evaluation of a
				//     commoned node occurred.
				//
				if (self()->comp()->getOption(TR_TraceCG)) {
					self()->comp()->getDebug()->trace(" _stackOfArtificiallyInflatedNodes.pop(): "
					                                  "node %p part of commoned case, might have "
					                                  "avoided a bug!\n",
			...
			if (self()->comp()->getOption(TR_TraceCG)) {
				self()->comp()->getDebug()->trace(" _stackOfArtificiallyInflatedNodes.pop() %p, "
	...
	if (reg && reg->getOpaquePseudoRegister()) {
		...
		if (node->getReferenceCount() == 1) {
			...
			if (self()->traceBCDCodeGen())
				traceMsg(self()->comp(),
				        "\tdecrement owningRegisterCount %d->%d on ref #%d (%s) for reg %s as %s (%p) "
				        "refCount == 1 (going to 0)\n",
