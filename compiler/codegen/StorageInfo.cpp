...
TR_StorageInfo::TR_StorageInfo(TR::Node *node, size_t length, TR::Compilation *c)
        : _node(node), _length(length), _address(NULL), _symRef(NULL), _offset(0), _class(TR_UnknownClass), _comp(c)
{
...
TR_StorageInfo::print()
{
	traceMsg(comp(), "\t\t\t%s (%p) len %d: addr %s (%p) symRef #%d, offset %d, class %s\n",
	        _node->getOpCode().getName(), _node, _length, _address ? _address->getOpCode().getName() : "NULL",
	        _address, _symRef ? _symRef->getReferenceNumber() : -1, _offset, getName());
}
...
TR_StorageInfo::mayOverlapWith(TR_StorageInfo *info)
{
	...
	// lengths and offset do not matter in these first two cases as disjointness is known from the storage classes
	// alone
	...
	if (symbolMismatch && _class == TR_DirectMappedAuto && info->_class == TR_DirectMappedAuto) {
		...
		return TR_NoOverlap; // directly mapped autos from loadaddrs or direct stores cannot interfere with
		                     // static addresses note that aload from autos and static addresses certainly can
		                     // as the aload may contain a static address
	}
	...
	// TR_StaticBaseAddress points to the start of the the static area and TR_DirectMappedStatic points to a
	// particular static symbol
	...
	if (!compareRanges && _address && info->_address && comp()->cg()->nodeMatches(_address, info->_address)) {
		if (comp()->cg()->traceBCDCodeGen())
			traceMsg(comp(), "\t\t\tcompareRanges : nodes match case (%s (%p) and %s (%p))\n",
			        _address->getOpCode().getName(), _address, info->_address->getOpCode().getName(),
			        info->_address);
		compareRanges = true;
	}
	...
	if (compareRanges) {
		...
		if (overlapStart >= overlapEnd) // i.e. is this an impossible (>) or trivial range (==) with start >=
		                                // end? If so then there is no overlap
		{
			overlap = TR_NoOverlap;
		} else {
			// If overlapStart < overlapEnd then this is a valid range and there is overlap
			// (overlapStart->overlapEnd is the co-incident)
			if (startOne == startTwo) {
				...
				if (startOne + std::min(_length, info->_length) - 1 >= overlapStart) {
					...
					//    :   abababab  (i.e. result not the same as if a temp was used in which
					//    case answer would be ababcd)
		...
		if (comp()->cg()->traceBCDCodeGen())
			traceMsg(comp(), "\t\toverlap=%s (%s) : overlap range %d->%d is %spossible\n",
			        overlap ? "true" : "false", getName(overlap), overlapStart, overlapEnd,
			        overlap ? "" : "im");
