...
TR::Node* generatePoisonNode(TR::Compilation* comp, TR::Block* currentBlock, TR::SymbolReference* liveAutoSymRef)
{
    ...
    if (comp->getOption(TR_TraceCG) && comp->getOption(TR_PoisonDeadSlots)) {
        if (poisoned)
            traceMsg(comp,
                "POISON DEAD SLOTS --- Live local %d  from parent block %d going dead .... poisoning slot with node "
                "0x%x .\n",
                liveAutoSymRef->getReferenceNumber(), currentBlock->getNumber(), storeNode);
        else
            traceMsg(comp,
                "POISON DEAD SLOTS --- Live local %d of unsupported type from parent block %d going dead .... "
                "poisoning skipped.\n",
...
void OMR::CodeGenerator::addSnippet(TR::Snippet* s) { _snippetList.push_front(s); }
...
// J9
//
TR::Node* OMR::CodeGenerator::createOrFindClonedNode(TR::Node* node, int32_t numChildren)
{
    TR_HashId index;
...
uint8_t* OMR::CodeGenerator::allocateCodeMemory(
    uint32_t warmSize, uint32_t coldSize, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    uint8_t* warmCode;
...
uint8_t* OMR::CodeGenerator::allocateCodeMemory(uint32_t size, bool isCold, bool isMethodHeaderNeeded)
{
    uint8_t* coldCode;
    if (isCold) {
...
#define TOINT64(x) (*((int64_t*)&x))
    static uint32_t div64BitMagicValues[NUM_64BIT_MAGIC_VALUES][6] =
        ...
        { { 3, 0, 0x55555556, 0x55555555, 0, 0 }, { 5, 0, 0x66666667, 0x66666666, 1, 0 },
            { 7, 0, 0x24924925, 0x49249249, 1, 0 }, { 9, 0, 0x71c71c72, 0x1c71c71c, 0, 0 },
            { 10, 0, 0x66666667, 0x66666666, 2, 0 }, { 12, 0, 0xaaaaaaab, 0x2aaaaaaa, 1, 0 } };
...
void OMR::CodeGenerator::dumpSpillStats(TR_FrontEnd* fe)
{
    if (debug("spillStats")) {
        ...
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "%8d indirect accesses rematerialized", _totalNumRematerializedIndirects);
...
void OMR::CodeGenerator::addAOTRelocation(TR::Relocation* r, const char* generatingFileName,
    uintptr_t generatingLineNumber, TR::Node* node, TR::AOTRelocationPositionRequest where)
{
    self()->addExternalRelocation(
        r, generatingFileName, generatingLineNumber, node, static_cast<TR::ExternalRelocationPositionRequest>(where));
}

void OMR::CodeGenerator::addAOTRelocation(
    TR::Relocation* r, TR::RelocationDebugInfo* info, TR::AOTRelocationPositionRequest where)
{
