...
OMR::CodeGenerator::setUpForInstructionSelection()
{
	...
	for (tt = self()->comp()->getStartTree(); tt; prev = tt, tt = tt->getNextTreeTop()) {
		...
		if ((node->getOpCodeValue() == TR::treetop) || node->getOpCode().isAnchor()
			...
			if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
				...
				if (tt == self()->getCurrentEvaluationTreeTop()) {
					...
					for (TR::TreeTop *dumptt = tt;
					        dumptt != self()->getCurrentEvaluationTreeTop()->getNextTreeTop();
...
OMR::CodeGenerator::initializeLinkage()
{
	...
	linkage = linkage ? linkage
	                  : self()->createLinkage(self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
...
bool
OMR::CodeGenerator::mulDecompositionCostIsJustified(
...
void
OMR::CodeGenerator::apply8BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
	*(int8_t *)cursor += (int8_t)(intptrj_t)label->getCodeLocation();
}
void
OMR::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label, bool isCheckDisp)
{
	TR_ASSERT(0, "unexpected call to OMR::CodeGenerator::apply12BitLabelRelativeRelocation");
}
void
OMR::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
	*(int16_t *)cursor += (int16_t)(intptrj_t)label->getCodeLocation();
}
void
OMR::CodeGenerator::apply16BitLabelRelativeRelocation(
        int32_t *cursor, TR::LabelSymbol *label, int8_t d, bool isInstrOffset)
{
	*(int16_t *)cursor += (int16_t)(intptrj_t)label->getCodeLocation();
}
void
OMR::CodeGenerator::apply24BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *)
{
...
void
OMR::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *)
{
	TR_ASSERT(0, "unexpected call to OMR::CodeGenerator::apply32BitLabelRelativeRelocation");
}
void
OMR::CodeGenerator::apply32BitLabelTableRelocation(int32_t *cursor, TR::LabelSymbol *)
{
...
void
OMR::CodeGenerator::traceRegAssigned(TR::Register *virtReg, TR::Register *realReg)
{
	if (self()->getDebug())
...
void
OMR::CodeGenerator::traceRegAssigned(TR::Register *virtReg, TR::Register *realReg, TR_RegisterAssignmentFlags flags)
{
	if (self()->getDebug())
...
void
OMR::CodeGenerator::traceRegInterference(TR::Register *virtReg, TR::Register *interferingVirtual, int32_t distance)
{
	if (self()->getDebug())
...
{
	if ((node2->getOpCode().isLoadVarOrStore() || node2->getType().isAddress()) && // node1 is usually an always
	                                                                               // valid store so check node2
	                                                                               // first
		...
		if (self()->traceBCDCodeGen())
			traceMsg(self()->comp(),
			        "overlap=true : node1 %s (%p) and/or node2 %s (%p) are not valid load/store/address "
			        "nodes\n",
...
{
	return TR::CodeGenerator::defaultArrayTranslateMinimumNumberOfIterations("arrayTranslateMinimumNumberOfElement"
	                                                                         "s");
}
...
{
	return TR::CodeGenerator::defaultArrayTranslateMinimumNumberOfIterations("arrayTranslateAndTestMinimumNumberOfI"
	                                                                         "terations");
}
...
OMR::CodeGenerator::isAliasedGRN(TR_GlobalRegisterNumber n)
{
	...
	return n >= _firstOverlappedGlobalFPR && n <= _lastOverlappedGlobalFPR; // _lastOverlappedFPR includes
...
OMR::CodeGenerator::getOverlappedAliasForGRN(TR_GlobalRegisterNumber n)
{
	...
	if (n >= _firstOverlappedGlobalFPR && n < _firstOverlappedGlobalVRF) // n is a a float grn but not a vector grn
	{
		...
		TR_ASSERT(self()->isGlobalVRF(aliasedGrn), "Aliased FPR %d did not give an a valid VRF %d", n,
		        aliasedGrn);
	} else if (n >= _firstOverlappedGlobalVRF && n <= _lastOverlappedGlobalVRF) // n is a pure overlapped vector grn
	{
		aliasedGrn = n - self()->getOverlapOffsetBetweenAliasedGRNs();
		TR_ASSERT(self()->isGlobalVRF(aliasedGrn), "Aliased VRF %d did not give an a valid FPR %d", n,
		        aliasedGrn);
	} else {
...
bool
OMR::CodeGenerator::uniqueAddressOccurrence(TR::Node *addr1, TR::Node *addr2, bool addressesUnderSameTreeTop)
{
	...
	if (duringEvaluation) {
		...
		if (addr1->getReferenceCount() <= 1 && // refCounts may be zero when called during ilgen and the nodes
		                                       // are freshly popped
	...
	if (addr1 == addr2) {
		...
		if (op1.getOpCodeValue() == op2.getOpCodeValue() && op1.isLoadVar() && op1.getDataType() == TR::Address
			...
			// aloadi, ardbari etc
			        ...
			        && self()->nodeMatches(
			                addr1->getFirstChild(), addr2->getFirstChild(), addressesUnderSameTreeTop))
				foundMatch = true;
		}
	}
...
OMR::CodeGenerator::additionsMatch(TR::Node *addr1, TR::Node *addr2, bool addressesUnderSameTreeTop)
{
	...
	if (self()->nodeMatches(addr1ChildOne, addr2ChildOne, addressesUnderSameTreeTop)) {
		...
		foundMatch = true;
	        ...
	        && addr1ChildTwo->getOpCode().isLoadConst() && addr2ChildTwo->getOpCode().isLoadConst()) {
		// aladd
		//    aladd       addr1ChildOne
		//       aload
		//       lconst x
		...
		int64_t offsetOne = addr1ChildOne->getSecondChild()->get64bitIntegralValue()
		        + addr1ChildTwo->get64bitIntegralValue();
		int64_t offsetTwo = addr2ChildTwo->get64bitIntegralValue();
		/*
		      traceMsg(comp(),"adding offsets from %p and %p (%lld + %lld) and %p (%lld) (found base match %s
...
OMR::CodeGenerator::addressesMatch(TR::Node *addr1, TR::Node *addr2, bool addressesUnderSameTreeTop)
{
		...
		if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
			if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
				...
				if (firstChildrenMatch && addr1->getSecondChild() && addr2->getSecondChild()
					...
					secondChildrenMatch = self()->additionsMatch(addr1->getSecondChild(),
					        addr2->getSecondChild(), addressesUnderSameTreeTop);
				}
	...
	if (!foundMatch && addressesUnderSameTreeTop) {
		...
		// 1) before evaluation : refCounts of both nodes are 1 (a more refined test, that requires scanning
		// ahead in the block, is if both are first references) 2) during evaluation : registers are both NULL
		// (i.e. both first ref)
		...
		if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)
			...
			if (traceInspect())
				traceMsg(comp(),
				        "\t\tfound possibly matching additions : update addr1 to %s (%p), addr2 to %s "
...
uint8_t *
OMR::CodeGenerator::allocateCodeMemory(
        uint32_t warmCodeSizeInBytes, uint32_t coldCodeSizeInBytes, uint8_t **coldCode, bool isMethodHeaderNeeded)
{
	uint8_t *warmCode;
...
OMR::CodeGenerator::alignBinaryBufferCursor()
{
	...
	if (self()->supportsJitMethodEntryAlignment() && boundary > 1) {
		...
		alignedBinaryBufferCursor = reinterpret_cast<uint8_t *>(
		        OMR::align(reinterpret_cast<size_t>(alignedBinaryBufferCursor), boundary));

		TR_ASSERT_FATAL(OMR::aligned(reinterpret_cast<size_t>(alignedBinaryBufferCursor), boundary),
		        "alignedBinaryBufferCursor [%p] is not aligned to the specified boundary (%d)",
		...
		TR_ASSERT_FATAL(threshold <= boundary,
		        "JIT method entry alignment threshold (%d) definition is violated as it is larger than the "
		        "boundary (%d)",
		        threshold, boundary);
	...
	for (auto iterator = _snippetList.begin(); iterator != _snippetList.end(); ++iterator) {
		...
		if (codeOffset != NULL) {
			TR_ASSERT(
			        ...
			        (*iterator)->getLength(
			                self()->getBinaryBufferCursor() - self()->getBinaryBufferStart()),
...
bool
OMR::CodeGenerator::areMergeableGuards(TR::Instruction *earlierGuard, TR::Instruction *laterGuard)
{
...
OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction *vgdnop)
{
	TR_ASSERT(vgdnop->isVirtualGuardNOPInstruction(),
	        "getGuardForPatching called with non VirtualGuardNOPInstruction [%p] - this only works for guards!",
	        vgdnop);
	...
	if (toReturn != vgdnop) {
		...
		if (self()->comp()->getOption(TR_TraceCG))
			traceMsg(self()->comp(),
			        "vgdnop instruction [%p] begins scanning for patch instructions for mergeable guard "
			        "[%p]\n",
			        vgdnop, toReturn);
	}
	return toReturn;
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(const char *name, TR::RegisterDependencyConditions &cond, int32_t delta,
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(const char *name, TR::Register *deltaReg,
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(const char *name, TR_ScratchRegisterManager &srm, int32_t delta,
        int8_t fidelity, int32_t staticDelta, TR::Instruction *cursor)
{
	if (!cursor)
		cursor = self()->getAppendInstruction();
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(const char *name, TR::Register *deltaReg, TR_ScratchRegisterManager &srm,
        int8_t fidelity, int32_t staticDelta, TR::Instruction *cursor)
{
	if (!cursor)
		cursor = self()->getAppendInstruction();
...
{
	self()->setReferencedRegisterList(new (self()->trHeapMemory()) TR::list<OMR::RegisterUsage *>(
...
OMR::CodeGenerator::addRelocation(TR::Relocation *r)
{
	if (r->isExternalRelocation()) {
		TR_ASSERT(false,
		        "Cannot use addRelocation to add an AOT relocation. Please use addExternalRelocation here");
	} else {
...
void
OMR::CodeGenerator::addExternalRelocation(TR::Relocation *r, const char *generatingFileName,
        ...
        TR::Relocation *r, TR::RelocationDebugInfo *info, TR::ExternalRelocationPositionRequest where)
{
	if (self()->comp()->compileRelocatableCode()) {
		...
		r->setDebugInfo(info);
		switch (where) {
		case TR::ExternalRelocationAtFront: _externalRelocationList.push_front(r); break;

		case TR::ExternalRelocationAtBack: _externalRelocationList.push_back(r); break;

		default: TR_ASSERT_FATAL(false, "invalid TR::ExternalRelocationPositionRequest %d", where); break;
		}
	}
}
...
{
	if (self()->comp()->compileRelocatableCode()) // We don't want to store values using HI_VALUE at compile time,
	                                              // otherwise, we do this a 2nd time when we relocate (and new
	                                              // value is based on old one)
...
void
OMR::CodeGenerator::addToOSRTable(int32_t instructionPC, TR_ByteCodeInfo &bcInfo)
{
...
TR::RegisterPair *
OMR::CodeGenerator::allocateSinglePrecisionRegisterPair(TR::Register *lo, TR::Register *ho)
{
...
TR::RegisterPair *
OMR::CodeGenerator::allocateFloatingPointRegisterPair(TR::Register *lo, TR::Register *ho)
{
	TR::RegisterPair *temp = new (self()->trHeapMemory()) TR::RegisterPair(lo, ho);
	temp->setKind(TR_FPR);
	self()->addAllocatedRegisterPair(temp);
