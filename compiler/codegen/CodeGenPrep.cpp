		...
		if (secondChild->getOpCode().isLoadConst()) {
			...
			if (shftAmnt > 0) {
				...
				} else // negative value of the multiply constant
				{
					if (secondChild->getReferenceCount() == 1) {
						TR::Node *newChild =
						        TR::Node::create(parent, is32BitOp ? TR::ishl : TR::lshl, 2);
						;
...
void
OMR::CodeGenerator::lowerTreeIfNeeded(TR::Node *node, int32_t childNumberOfNode, TR::Node *parent, TR::TreeTop *tt)
{
			...
			if (!storeLocal->holdsMonitoredObject() && !storeLocal->isInternalPointer()
				...
				if ((storeIGNode != NULL) && (loadIGNode != NULL)) {
					...
					if (colourLoad == colourStore && colourLoad != UNCOLOURED
					        && performTransformation(self()->comp(),
					                "%sCoalescing locals by removing store tree %p, load = %d, "
					                "store = %d \n",
				...
				if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i || childOp == TR::bu2i))
					...
					if ((childOp == TR::b2i || childOp == TR::s2i)
					        ...
					        && performTransformation(self()->comp(),
					                ...
					                child)) {
						TR::Node::recreate(
						        child, (storeType == TR::Int8 ? TR::bu2i : TR::su2i));
	...
	for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
		...
		if (node->getOpCodeValue() == TR::BBStart) {
			...
			if (block->isCold()) {
				...
				TR::DebugCounter::prependDebugCounter(self()->comp(),
				        TR::DebugCounter::debugCounterName(self()->comp(),
				                "coldBlocks/byJittedBody/(%s)/%s/=%d", self()->comp()->signature(),
				                self()->comp()->getHotnessName(self()->comp()->getMethodHotness()),
				                block->getNumber()),
				        tt->getNextTreeTop(), 1, TR::DebugCounter::Free);
			} else {
				...
				TR::DebugCounter::prependDebugCounter(self()->comp(),
				        TR::DebugCounter::debugCounterName(self()->comp(),
				                "warmBlocks/byJittedBody/(%s)/%s/=%d", self()->comp()->signature(),
				                self()->comp()->getHotnessName(self()->comp()->getMethodHotness()),
				                block->getNumber()),
				        tt->getNextTreeTop());
			}
			...
			if (block->getPredecessors().size() == 1) {
				...
				if (predBlock->getEntry() && predBlock->getExit()) {
					...
					if (predNode->getOpCode().isSwitch()) {
						...
						TR::DebugCounter::prependDebugCounter(self()->comp(),
						        TR::DebugCounter::debugCounterName(self()->comp(),
						                "branchtargets/%s/(%s)/%s/%d/%d", opName,
						                self()->comp()->signature(),
		...
		if (node->getOpCode().isNew()) {
			...
			if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::anewarray) {
				...
				char typeName[30];
				switch (type) {
				case 4: sprintf(typeName, "boolean"); break;
				case 8: sprintf(typeName, "byte");
				case 5: sprintf(typeName, "char"); break;
				default: sprintf(typeName, "non-char"); break;
				}
