...
OMR::CodeGenerator::checkForLiveRegisters(TR_LiveRegisters *liveRegisters)
{
	...
	if (liveRegisters && liveRegisters->getFirstLiveRegister()) {
		...
		for (TR_LiveRegisterInfo *p = liveRegisters->getFirstLiveRegister(); p; p = p->getNext()) {
			if (p->getNode())
				traceMsg(self()->comp(),
				        "Virtual register %s (for node [%s], ref count=%d) is live at end of method\n",
				        debug->getName(p->getRegister()), debug->getName(p->getNode()),
				        p->getNode()->getReferenceCount());
			else
				traceMsg(self()->comp(),
				        "Virtual register %s %p is live at end of method with node count %d\n",
...
void
OMR::CodeGenerator::estimateRegisterPressure(TR::Node *node, int32_t &registerPressure, int32_t &maxRegisterPressure,
        ...
        TR::SymbolReference *symRef, bool &symRefIsLive, bool checkForIMuls, bool &vmThreadUsed)
{
	...
	for (childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
		self()->estimateRegisterPressure(node->getChild(childCount), registerPressure, maxRegisterPressure,
		        maxRegisters, valuesInGlobalRegs, isCold, visitCount, symRef, symRefIsLive, checkForIMuls,
		        vmThreadUsed);
				...
				if (highRegisterPressureOpCode
					...
					if (!isCold) {
						if (registerPressure >= (maxRegisters - NUM_REGS_USED_BY_COMPLEX_OPCODES
						            - 1)) // Only allow if register pressure is known to be very
						                  // low
						{
...
int32_t
OMR::CodeGenerator::estimateRegisterPressure(TR::Block *block, vcount_t visitCount, int32_t maxStaticFrequency,
        ...
        TR::SymbolReference *symRef, bool checkForIMuls)
{
	...
	while (treeTop) {
		...
		self()->estimateRegisterPressure(node, currRegisterPressure, maxRegisterPressure,
		        self()->getMaximumNumbersOfAssignableGPRs(), valuesInGlobalRegs, isCold, visitCount, symRef,
		        symRefIsLive, checkForIMuls, vmThreadUsed);

		if (vmThreadUsed && (maxRegisterPressure >= self()->getMaximumNumbersOfAssignableGPRs())) {
			// dumpOptDetails("For global register candidate %d reg pressure reached limit %d at block_%d
...
OMR::CodeGenerator::allocateSpill(int32_t dataSize, bool containsCollectedReference, int32_t *offset, bool reuse)
{
	...
	//
	bool try8ByteSpills = (dataSize < 16)
	        && ((TR::Compiler->om.sizeofReferenceAddress() == 8) || !containsCollectedReference);
	...
	if (self()->getTraceRAOption(TR_TraceRASpillTemps))
		traceMsg(self()->comp(), "\nallocateSpill returning (%s(%d%d), %d) ",
		        ...
		        spill->firstHalfIsOccupied() ? 1 : 0, spill->secondHalfIsOccupied() ? 1 : 0,
		        offset ? *offset : 0);
	return spill;
}
...
OMR::CodeGenerator::freeSpill(TR_BackingStore *spill, int32_t dataSize, int32_t offset)
{
	...
	if (self()->getTraceRAOption(TR_TraceRASpillTemps)) {
		traceMsg(self()->comp(), "\nfreeSpill(%s(%d%d), %d, %d, isLocked=%d)",
		        self()->getDebug()->getName(spill->getSymbolReference()->getSymbol()),
		        spill->firstHalfIsOccupied() ? 1 : 0, spill->secondHalfIsOccupied() ? 1 : 0, dataSize, offset,
	...
	if (spill->getSymbolReference()->getSymbol()->getAutoSymbol()->isInternalPointer()) {
		...
		if (spill->isEmpty()) {
			...
			// TODO: Once every caller can cope with nonzero offsets, we should add first-half-occupied
			// symbols into _spill4FreeList.
			if (self()->getTraceRAOption(TR_TraceRASpillTemps))
				traceMsg(self()->comp(),
				        "\n -> first half is still occupied; conservatively keeping out of "
				        "spill4FreeList");
		} else {
		...
		if (isSafePoint) {
			...
			if (safePointNode->getVisitCount() != self()->comp()->getVisitCount()) {
				...
				if (!_liveReferenceList.empty()) {
					self()->spillLiveReferencesToTemps(
					        tt->getPrevTreeTop(), firstAvailableSpillTemp);
				}
			}
...
void
OMR::CodeGenerator::spillLiveReferencesToTemps(TR::TreeTop *insertionTree,
...
OMR::CodeGenerator::pickRegister(TR_RegisterCandidate *rc, TR::Block **allBlocks, TR_BitVector &availableRegisters,
        TR_GlobalRegisterNumber &highRegisterNumber, TR_LinkHead<TR_RegisterCandidate> *candidatesAlreadyAssigned)
{
	...
	if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
		...
		if (self()->terseSimulateTreeEvaluation()) {
			...
			if (debug("dumpAllSpillKinds")) {
				for (int32_t i = 0; i < TR_numSpillKinds; i++) {
					...
					traceMsg(self()->comp(),
					        "            %s regs: ", self()->getDebug()->getSpillKindName(sk));
		...
		while (blockIterator.hasMoreElements()) {
			...
			if (!block->isExtensionOfPreviousBlock() && !blockIsIgnorablyCold(block, self())) {
				...
				//
				static int32_t meniscus = feGetEnv("TR_registerPressureMeniscus")
				...
				TR_RegisterPressureState blockEntryState(NULL, false, alreadyAssignedOnEntry,
				        alreadyAssignedOnExit, candidatesAlreadyAssigned,
				...
				// block, and if the block is "uncacheable" we could just not use the cache for that
				// one.
				//
		...
		while (blockIterator.hasMoreElements()) {
			...
			if (block != self()->comp()->getFlowGraph()->getEnd() && !blockIsIgnorablyCold(block, self())) {
				...
				//
				int32_t blockWeight = 1; // Note that we use a static loop-nesting-depth heuristic here
				...
				if (blockWeight > hottestCopiedRegisterWeight) {
					...
					for (TR::TreeTop *tt = block->getEntry(); tt != stopTree;
						...
						TR_RegisterCandidate *candidate =
						        self()->findCoalescenceForRegisterCopy(
						                node, rc, &isUnpreferred);
							...
							if (!isUnpreferred) {
								...
								if (remainingRegisters.isSet(preferredRegister)) {
									...
									if (self()->traceSimulateTreeEvaluation()) {
										traceMsg(self()->comp(),
										        "            found copy "
										        "to/from #%d, register %d "
										        "(%s), weight %d, at %s\n",
										        candidate->getSymbolReference()
										                ->getReferenceNumber(),
										        hottestCopiedRegister,
										        self()->getDebug()
										                ->getGlobalRegisterName(
										                        hottestCopiedRegister),
		...
		if (hottestCopiedRegister == -1) {
			...
			if (unpreferredRegisters != remainingRegisters) {
				if (self()->traceSimulateTreeEvaluation() && !unpreferredRegisters.isEmpty()) {
					...
					self()->getDebug()->print(
					        self()->comp()->getOptions()->getLogFile(), &unpreferredRegisters);
					traceMsg(self()->comp(), "\n");
				}
		...
		if (remainingRegisters.isEmpty()) {
			...
			while (blockIterator.hasMoreElements() && !availableRegisters.isEmpty()) // TODO: Can
			                                                                         // availableRegisters
			                                                                         // possibly be empty
			                                                                         // here?
			{
	...
	if (1) {
		TR::DataType dtype = rc->getDataType();
		uint8_t regsWithheld = (dtype == TR::Float || dtype == TR::Double
#ifdef J9_PROJECT_SPECIFIC
		                               || dtype == TR::DecimalFloat || dtype == TR::DecimalDouble
		                               || dtype == TR::DecimalLongDouble
#endif
...
OMR::CodeGenerator::findCoalescenceForRegisterCopy(TR::Node *node, TR_RegisterCandidate *rc, bool *isUnpreferred)
{
	...
	if (node->getOpCode().isStoreDirect() && node->getFirstChild()->getOpCode().isLoadVarDirect()) {
		...
		if (storedCand) {
			...
			// unpreferred if the candidate for coalescing does not match the candidate rc, currently being
			// evaluated
			*isUnpreferred = !(loadedSymRefNum == rc->getSymbolReference()->getReferenceNumber());
			candidate = storedCand;
		}

		TR_RegisterCandidate *loadedCand = self()->comp()->getGlobalRegisterCandidates()->find(
...
void
OMR::CodeGenerator::computeSimulatedSpilledRegs(
        TR_BitVector *spilledRegisters, int32_t blockNum, TR::SymbolReference *candidate)
{
	...
	if (symRefNum == _blockAndCandidateRegisterPressureCacheTags[blockNum]) {
		if (traceSimulateTreeEvaluation())
			traceMsg(comp(),
			        "Using candidate-specific regsiter pressure simulation cache for block_%d, sym #%d\n",
			        blockNum, symRefNum);
...
void
OMR::CodeGenerator::TR_RegisterPressureState::addVirtualRegister(TR::Register *reg)
{
	if (reg == NULL)
		return;
...
void
OMR::CodeGenerator::TR_RegisterPressureState::removeVirtualRegister(TR::Register *reg)
{
	if (reg == NULL)
		return;
	if (reg->getRealRegister())
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::setLinkagePresent(TR_LinkageConventions lc, TR::CodeGenerator *cg)
{
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::spill(TR_SpillKinds kind, TR::CodeGenerator *cg)
{
	_spillMask |= (1 << kind);
	if (cg->traceSimulateTreeEvaluation())
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::dumpSpillMask(TR::CodeGenerator *cg)
{
	if (cg->traceSimulateTreeEvaluation()) {
		for (int32_t i = 0; i < TR_numSpillKinds; i++) {
			TR_SpillKinds spillKind = (TR_SpillKinds)i;
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::dumpLinkageConventionMask(TR::CodeGenerator *cg)
{
	if (cg->traceSimulateTreeEvaluation()) {
		for (int32_t i = 0; i < TR_NumLinkages; i++) {
			TR_LinkageConventions lc = (TR_LinkageConventions)i;
...
bool
OMR::CodeGenerator::isCandidateLoad(TR::Node *node, TR::SymbolReference *candidate)
{
	return node->getOpCode().isLoadVarDirect() && node->getSymbolReference() == candidate;
}

bool
OMR::CodeGenerator::isCandidateLoad(TR::Node *node, TR_RegisterPressureState *state)
{
	return state->_candidate ? self()->isCandidateLoad(node, state->_candidate->getSymbolReference()) : false;
}

bool
OMR::CodeGenerator::isLoadAlreadyAssignedOnEntry(TR::Node *node, TR_RegisterPressureState *state)
{
...
{
	if (type.isAggregate())
		return (TR::Compiler->target.is32Bit() && !self()->use64BitRegsOn32Bit() && size > 4 && size <= 8) ? 2
		                                                                                                   : 1;
...
inline void
standardNodeSimulationAnnotations(OMR::CodeGenerator::TR_RegisterPressureState *state, TR::Compilation *comp)
{
	if (state->_candidate)
...
inline void
leaveSpaceForRegisterPressureState(OMR::CodeGenerator::TR_RegisterPressureState *state, TR::Compilation *comp)
{
...
findCandidate(TR::SymbolReference *symRef, TR_LinkHead<TR_RegisterCandidate> *candidates,
        TR_RegisterCandidate *anotherCandidate = NULL)
{
	...
	for (result = candidates->getFirst(); result && result->getSymbolReference() != symRef;
        ...
        OMR::CodeGenerator::TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
	if (state->_alreadyAssignedOnExit.isSet(symRef->getReferenceNumber())
		...
		TR_RegisterCandidate *candidate =
		        findCandidate(symRef, state->_candidatesAlreadyAssigned, state->_candidate);
...
static void
keepMostRecentValueAliveIfLiveOnEntryToSuccessor(TR_RegisterCandidate *candidate, TR::TreeTop *exitPoint,
	...
	while (tt && !state->mustAbort()) {
		...
		if (tt->getNode()->getOpCode().isStoreDirect())
			rememberMostRecentValue(
			        tt->getNode()->getSymbolReference(), tt->getNode()->getFirstChild(), state, self());
        ...
        TR::TreeTop *tt, TR::Node *node, TR_RegisterPressureState *state, TR_RegisterPressureSummary *summary)
{
	...
	if (node->getOpCode().isStoreDirect()) {
		...
		for (auto e = node->getBlock()->getSuccessors().begin(); e != node->getBlock()->getSuccessors().end();
		        ++e) {
			if (state->_candidate)
			...
			if (state->_candidatesAlreadyAssigned)
				for (TR_RegisterCandidate *candidate = state->_candidatesAlreadyAssigned->getFirst();
...
static bool
nodeGotFoldedIntoMemref(TR::Node *node, OMR::CodeGenerator::TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
...
void
OMR::CodeGenerator::simulateTreeEvaluation(
        TR::Node *node, TR_RegisterPressureState *state, TR_RegisterPressureSummary *summary)
{
	...
	if (self()->getHasResumableTrapHandler()
		...
		//
		bool isResolvedLoadVar = (op.isLoadVar() && child->getFutureUseCount() == 1
	...
	if (nodeState.hasRegister()) {
		...
		if (self()->traceSimulateTreeEvaluation()) {
			...
			if (self()->isCandidateLoad(node, state))
				...
				if (findCandidate(node->getSymbolReference(), state->_candidatesAlreadyAssigned)
				                ->getLastLoad()
				        == node)
					tagChar = 'A';
				else
		...
		if (self()->nodeWillBeRematerialized(node, state)) {
			// TR_ASSERT(node->getFutureUseCount() >= 2, "Node %s must have refcount >= 2 to be
			// rematerialized", getDebug()->getName(node));
		...
		if (state->candidateIsLiveAfterGRA()) {
			...
			if (isCall) {
				...
				summary->setLinkagePresent(
				        node->getSymbol()->castToMethodSymbol()->getLinkageConvention(), self());
			}
...
void
OMR::CodeGenerator::simulateNodeEvaluation(
        TR::Node *node, TR_RegisterPressureState *state, TR_RegisterPressureSummary *summary)
{
	...
	//
	flags32_t childrenInRegisters(0); // (Don't bother with a bitvector on the slight chance that some child beyond
	                                  // the first 32 will get in a register)
	...
	if (nodeGotFoldedIntoMemref(node, state, self())
	        && self()->simulatedNodeState(node)._willBeRematerialized == 0 // remat nodes have already had their
	                                                                       // children incremented, so we must
	                                                                       // decrement them
	) {
	...
	if (state->_currentMemref) {
		if (op.isArrayRef())
			...
			if (self()->getSupportsConstantOffsetInAddressing() && (op.isAdd() || op.isSub()) && isByConst)
				result = true;
			else if (self()->getSupportsScaledIndexAddressing() && (op.isMul() || op.isLeftShift())
			        && isByConst)
				result = true;
		}
	}
...
void
OMR::CodeGenerator::simulateMemoryReference(TR_SimulatedMemoryReference *memref, TR::Node *node,
...
static int64_t
powerOfTwoScaleFactor(TR::Node *node) // INT_MAX means the node doesn't represent a fixed-point mul by a power of 2
{
	TR::ILOpCode &op = node->getOpCode();
...
OMR::CodeGenerator::nodeCanBeFoldedIntoMemref(TR::Node *node, TR_RegisterPressureState *state)
{
	...
	bool isByIntOrLong = secondChild ? (secondChild->getType().isInt32() || secondChild->getType().isInt64())
	                                 : false;
...
OMR::CodeGenerator::nodeResultGPRCount(TR::Node *node, TR_RegisterPressureState *state)
{
	if (self()->nodeResultConsumesNoRegisters(node, state))
		...
		if (result == 2 && !node->getType().isAggregate() && node->isHighWordZero()
...
void
OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(TR::Node *node, TR_RegisterPressureState *state,
        uint32_t &numGPRTemporaries, uint32_t &numFPRTemporaries, uint32_t &numVRFTemporaries)
{
...

	{
		...
		bv |= *self()->getGlobalRegisters(TR_vmThreadSpill,
		        self()->comp()->getJittedMethodSymbol()->getLinkageConvention()); // vmthread, when present, is
		                                                                          // also a virtual reg not
		                                                                          // represented in trees
		bv &= *self()->getGlobalRegisters(
		        TR_gprSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
		...
		bv = *self()->getGlobalRegisters(
		        TR_volatileSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
		bv &= *self()->getGlobalRegisters(
		        TR_fprSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
		numFPRVolatileRegs = bv.elementCount();
		bv &= *self()->getGlobalRegisters(
		        TR_linkageSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
		numFPRLinkageRegs = bv.elementCount();
		bv &= *self()->getGlobalRegisters(
		        TR_vrfSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
	...
	for (int32_t i = node->getFirstArgumentIndex(); i < node->getNumChildren(); i++) {
		TR::Node *child = node->getChild(i);
		numGPRArgs =
		        std::min(numGPRLinkageRegs, numGPRArgs + self()->simulatedNodeState(child, state)._liveGPRs);
		numFPRArgs =
		        std::min(numFPRLinkageRegs, numFPRArgs + self()->simulatedNodeState(child, state)._liveFPRs);
		numVRFArgs =
		        std::min(numVRFLinkageRegs, numVRFArgs + self()->simulatedNodeState(child, state)._liveVRFs);
	}
...
OMR::CodeGenerator::simulateNodeGoingDead(TR::Node *node, TR_RegisterPressureState *state)
{
	...
	if (self()->isCandidateLoad(node, state) && state->candidateIsLiveOnEntry()
		...
		if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
	        ...
	        && (node
	                != findCandidate(node->getSymbolReference(), state->_candidatesAlreadyAssigned)
	                           ->getLastLoad())) {
	...
	if (self()->isCandidateLoad(node, state) && nodeState._liveCandidateLoad) {
		...
		nodeState._liveGPRs = nodeState._liveFPRs = nodeState._liveVRFs = nodeState._liveSSRs =
		        nodeState._liveARs = 0;
		TR_ASSERT(!nodeState.hasRegister(), "assertion failure");
	}
}
			...
			if (nodeState._keepLiveUntil) // && !simulatedNodeState(node, state)._willBeRematerialized)
			{
				...
				if (state->_candidate && node == state->_candidate->getMostRecentValue()) {
					...
					traceMsg(self()->comp(), " keeping:%s",
					        self()->getDebug()->getName(node)); // Some node other than the
					                                            // candidate's most recent value is
					                                            // being kept alive
			} else {
				if (nodeState._isCausingPressureRiskUntilEnd) {
					state->_pressureRiskUntilEnd--;
        ...
        TR::Node *node, TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
	...
	_numRegisters += 1;
	//_numRegisters += cg->simulatedNodeState(node)._liveGPRs; // Fix this once we find a platform that can put
	// other reg kinds in a memref
}
