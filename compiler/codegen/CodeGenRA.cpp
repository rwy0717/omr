...
TR_BackingStore* OMR::CodeGenerator::allocateInternalPointerSpill(TR::AutomaticSymbol* pinningArrayPointer)
{
    ...
    if (!spill) {
        ...
        TR::AutomaticSymbol* spillSymbol
            = TR::AutomaticSymbol::createInternalPointer(self()->trHeapMemory(), TR::Address,
    ...
    if (rk == TR_GPR
        && (!self()->getDebug() || !self()->getTraceRAOption(TR_TraceRASpillTemps)
               || performTransformation(self()->comp(), "O^O SPILL TEMPS: Set UpperHalfIsDead on %s\n",
                      self()->getDebug()->getName(temp)))) // allocateRegister is called for the vmthread during
                                                           // initialization when getDebug has not been initialized yet
    {
    ...
    TR_LinkHead<TR_RegisterCandidate>* candidatesAlreadyAssigned)
{
    ...
    if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
                ...
                if (cacheIsInconclusive) {
                    if (canAffordFullSimulation) {
                        ...
                        if (enableHighWordGRA) {
                            ...
                            if (dtype == TR::Int8 || dtype == TR::Int16 || dtype == TR::Int32) {
        ...
        if (enableHighWordGRA) {
            TR_BitVector HPRMasks
                = *self()->getGlobalRegisters(TR_hprSpill, self()->comp()->getMethodSymbol()->getLinkageConvention());
    ...
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (enableHighWordGRA) {
        ...
        if (state->_candidate && node->getOpCode().hasSymbolReference()) {
            if (node->getOpCode().isRef() && node->getSymbolReference() == state->_candidate->getSymbolReference()) {
                // not HPR candidate
                state->setCandidateIsHPREligible(false);
                summary->spill(TR_hprSpill, self());
            }
