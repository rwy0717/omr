...
TR::ELFGenerator::initializeELFHeaderForPlatform(void)
{
	...
	for (auto b = EI_PAD; b < EI_NIDENT; b++)
		_header->e_ident[b] = 0;
	_header->e_ident[EI_OSABI] = ELFOSABI_LINUX; // Current support for Linux only. AIX would use the macro
	                                             // ELFOSABI_AIX.
...
void
TR::ELFGenerator::initializeTextSection(uint32_t shName, ELFAddress shAddress, ELFOffset shOffset, uint32_t shSize)
{
...
TR::ELFExecutableGenerator::initializePHdr(void)
{
	...
	_programHeader->p_offset = sizeof(ELFEHeader); // offset of program header from the first byte of file to be
	                                               // loaded
...
TR::ELFExecutableGenerator::buildSectionHeaders(void)
{
	...
	uint32_t symbolsStartOffset = trailerStartOffset + (sizeof(ELFSectionHeader) * /* #shdr */ 5)
	        + shStrTabNameLength;
...
bool
TR::ELFExecutableGenerator::emitELF(
...
TR::ELFRelocatableGenerator::initializeELFHeader(void)
{
	...
	_header->e_shoff = sizeof(ELFEHeader) + _codeSize; // start of the section header table in bytes from the first
	                                                   // byte of the ELF file
...
TR::ELFRelocatableGenerator::buildSectionHeaders(void)
{
	...
	uint32_t symbolsStartOffset = trailerStartOffset + (sizeof(ELFSectionHeader) * /* # shdr */ 6)
	        + shStrTabNameLength;
	uint32_t symbolNamesStartOffset = symbolsStartOffset + (_numSymbols + /* UNDEF */ 1) * sizeof(ELFSymbol);
...
bool
TR::ELFRelocatableGenerator::emitELF(const char *filename, CodeCacheSymbol *symbols, uint32_t numSymbols,
