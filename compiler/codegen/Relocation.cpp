...
void TR::Relocation::apply(TR::CodeGenerator* codeGen) { TR_ASSERT(0, "Should never get here"); }
...
uint8_t TR::ExternalRelocation::collectModifier()
{
    ...
    if (TR::Compiler->target.cpu.isPower()
        && (_kind == TR_ArrayCopyHelper || _kind == TR_ArrayCopyToc || _kind == TR_RamMethod || _kind == TR_GlobalValue
               || _kind == TR_BodyInfoAddressLoad || _kind == TR_DataAddress || _kind == TR_JNISpecialTargetAddress
               || _kind == TR_JNIStaticTargetAddress || _kind == TR_JNIVirtualTargetAddress
               || _kind == TR_StaticRamMethodConst || _kind == TR_VirtualRamMethodConst
               || _kind == TR_SpecialRamMethodConst || _kind == TR_DebugCounter)) {
...
void TR::ExternalRelocation::addExternalRelocation(TR::CodeGenerator* codeGen)
{
    ...
    for (r = aot.getFirst(); r != 0; r = r->getNext()) {
        if (r->getTargetAddress2())
            ...
            AOTcgDiag5(comp,
                "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " kind=%x modifier=%x\n", r,
    ...
    if (_targetAddress2)
        ...
        AOTcgDiag5(comp,
            ...
            temp->full(), temp->getTargetAddress(), temp->getTargetKind(), temp->getModifierValue());
