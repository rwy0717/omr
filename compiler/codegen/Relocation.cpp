...
TR::InstructionLabelRelative16BitRelocation::InstructionLabelRelative16BitRelocation(
        ...
        : TR::LabelRelocation(NULL, l), _cursor(cursor), _offset(offset), _divisor(divisor)
{}
...
TR::InstructionAbsoluteRelocation::apply(TR::CodeGenerator *codeGen)
{
	...
	AOTcgDiag2(codeGen->comp(),
	        "TR::InstructionAbsoluteRelocation::apply cursor=" POINTER_PRINTF_FORMAT " instruction"
...
{
	AOTcgDiag2(codeGen->comp(),
	        "TR::LoadLabelRelative64BitRelocation::apply lastInstruction=" POINTER_PRINTF_FORMAT
...
TR::ExternalRelocation::addExternalRelocation(TR::CodeGenerator *codeGen)
{
	...
	for (r = aot.getFirst(); r != 0; r = r->getNext()) {
		if (r->getTargetAddress2())
			AOTcgDiag6(comp,
			        "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT
			        " target2=" POINTER_PRINTF_FORMAT ", kind=%x modifier=%x\n",
			        r, r->full(), r->getTargetAddress(), r->getTargetAddress2(), r->getTargetKind(),
			        r->getModifierValue());
		else
			AOTcgDiag5(comp,
			        "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " kind=%x "
			        "modifier=%x\n",
		...
		if (r->full() == false && r->getTargetAddress() == _targetAddress
		        && r->getTargetAddress2() == _targetAddress2 && r->getTargetKind() == _kind
			...
			r->setSizeOfRelocationData(
			        r->getSizeOfRelocationData() + (r->needsWideOffsets() ? wideSize : narrowSize));
			_relocationRecord = r;
			if (r->getTargetAddress2())
				AOTcgDiag6(comp,
				        "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT
	...
	TR::IteratedExternalRelocation *temp = _targetAddress2
	        ...
	        : new (codeGen->trHeapMemory())
	                TR::IteratedExternalRelocation(_targetAddress, _kind, modifier, codeGen);
	...
	if (_targetAddress2)
		...
		AOTcgDiag5(comp,
		        "temp=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " kind=%x modifier=%x\n",
	...
	temp->setSizeOfRelocationData(
	        temp->getSizeOfRelocationData() + (temp->needsWideOffsets() ? wideSize : narrowSize));
...
TR::ExternalRelocation::apply(TR::CodeGenerator *codeGen)
{
	TR::Compilation *comp = codeGen->comp();
	AOTcgDiag1(
	        comp, "TR::ExternalRelocation::apply updateLocation=" POINTER_PRINTF_FORMAT " \n", getUpdateLocation());
	uint8_t *relocatableMethodCodeStart = (uint8_t *)comp->getRelocatableMethodCodeStart();
...
TR::ExternalOrderedPair32BitRelocation::collectModifier()
{
	...
	if (TR::Compiler->target.cpu.isPower()
	        && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethod
...
char *TR::ExternalRelocation::_globalValueNames[TR_NumGlobalValueItems] = {"not used (0)", "TR_CountForRecompile (1)",
        "TR_HeapBase (2)", "TR_HeapTop (3)", "TR_HeapBaseForBarrierRange0 (4)", "TR_ActiveCardTableBase (5)",
...
TR::IteratedExternalRelocation::IteratedExternalRelocation(uint8_t *target, uint8_t *target2,
        TR_ExternalRelocationTargetKind k, flags8_t modifier, TR::CodeGenerator *codeGen)
        : TR_Link<TR::IteratedExternalRelocation>()
        , _numberOfRelocationSites(0)
        , _targetAddress(target)
