    ...
    TR::Block* block, TR::Simplifier* s, int32_t maxExponent)
{
    ...
    if (s->comp()->cg()->multiplyIsDestructive()) {
        ...
        int32_t maxCeilingExp
            = trailingZeroes(maxCeiling); // (2^x=maxCeiling) so if maxCeiling = 32 then x=maxCeilingExp=5
        TR::Node** subTrees
    ...
    if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0
        ...
        if (isPowAndReasonableIntExponent
            ...
            if (0 == absPowExponent) {
                ...
                TR_ASSERT(
                    absPowExponent >= TR::getMinSigned<TR::Int32>() && absPowExponent <= TR::getMaxSigned<TR::Int32>(),
                    "exponent should not exceed integer limits\n"); // checked above
                int32_t maxExponent
                    = isExponentNegative ? (int32_t)kMaxNegativeExponent : (int32_t)kMaxPositiveExponent;
