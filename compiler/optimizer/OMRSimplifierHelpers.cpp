...
TR::Node *
_gotoSimplifier(TR::Node *node, TR::Block *block, TR::TreeTop *curTree, TR::Optimization *s)
{
...
void
foldIntConstant(TR::Node *node, int32_t value, TR::Simplifier *s, bool anchorChildrenP)
{
...
void
foldUIntConstant(TR::Node *node, uint32_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
void
foldLongIntConstant(TR::Node *node, int64_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
void
foldByteConstant(TR::Node *node, int8_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
void
foldShortIntConstant(TR::Node *node, int16_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
bool
swapChildren(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
...
//
void
orderChildren(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
...
TR::Node *
foldRedundantAND(TR::Node *node, TR::ILOpCodes andOpCode, TR::ILOpCodes constOpCode, int64_t andVal, TR::Simplifier *s)
{
	...
	if (constChild->getOpCodeValue() == constOpCode) {
		switch (constOpCode) {
		case TR::sconst:
		case TR::cconst: val = constChild->getShortInt(); break;
		case TR::iconst: val = constChild->getInt(); break;
		case TR::lconst: val = constChild->getLongInt(); break;
		default: val = 0;
		}
	} else
		return 0;
		...
		if (rhsNode->getOpCode().isLoadConst()) {
			...
			if (lhsNode->getOpCode().isZeroExtension()
				...
				TR_ASSERT(node->getSize() > extensionValueNode->getSize(),
				        "Extended value datatype size must be smaller than the parent's datatype "
				        "size.");

				// Produce a mask of 1 bits of the same width as the value being extended
				int64_t mask = (1ll << (8ll * static_cast<int64_t>(extensionValueNode->getSize())))
				        - 1ll;

				if ((rhsNode->getConstValue() & mask) == 0) {
					if (performTransformation(simplifier->comp(),
...
//
void
makeConstantTheRightChild(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
...
void
makeConstantTheRightChildAndSetOpcode(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
	if (firstChild->getOpCode().isLoadConst() && !secondChild->getOpCode().isLoadConst()) {
...
TR::Node *
replaceChild(int32_t childIndex, TR::Node *node, TR::Node *newChild, TR::Simplifier *s)
{
...
TR::Node *
postWalkLowerTreeSimplifier(TR::TreeTop *tt, TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	TR::TreeTop *newTree = s->comp()->cg()->lowerTree(node, tt);
	if (newTree != s->_curTree)
		s->_curTree = newTree = newTree->getPrevTreeTop(); // set it to the previous treetop so simplier will
		                                                   // next walk the new tree
...
bool
isIntegralExponentInRange(TR::Node *parent, TR::Node *exponent, int64_t maxNegativeExponent,
        int64_t maxPositiveExponent, TR::Simplifier *s)
{
	...
	TR_ASSERT(parent->getOpCode().isExponentiation(),
...
TR::Node *
reduceExpTwoAndGreaterToMultiplication(int32_t exponentValue, TR::Node *baseNode, TR::ILOpCodes multOp,
        TR::Block *block, TR::Simplifier *s, int32_t maxExponent)
{
	...
	if (s->comp()->cg()->multiplyIsDestructive()) {
		int32_t bitPosOfLeftMostOne = 32 - leadingZeroes(exponentValue) - 1; // bitPos=0 is the least
		                                                                     // significant bit so for
		                                                                     // exponentValue=7 : 32 - 29 - 1 =
		                                                                     // 2
		resultNode = baseNode;
		if (bitPosOfLeftMostOne != 0) {
			for (int32_t i = bitPosOfLeftMostOne - 1; i >= 0; i--) {
				...
				dumpOptDetails(s->comp(),
				        "%screated %s [" POINTER_PRINTF_FORMAT "] operation for exponentiation "
				        "strength reduction "
				        "(algorithmA/caseA)\n",
		...
		int32_t maxCeilingExp = trailingZeroes(maxCeiling); // (2^x=maxCeiling) so if maxCeiling = 32 then
		                                                    // x=maxCeilingExp=5
		TR::Node **subTrees = (TR::Node **)s->comp()->trMemory()->allocateStackMemory(
		...
		for (i = 1; exponentValue >= (CONSTANT64(1) << i); ++i) // i can reach maxCeilingExp+1
		{
			...
			dumpOptDetails(s->comp(),
			        "%screated %s [" POINTER_PRINTF_FORMAT "] operation for exponentiation strength "
...
TR::Node *
replaceExpWithMult(TR::Node *node, TR::Node *valueNode, TR::Node *exponentNode, TR::Block *block, TR::Simplifier *s)
{
	...
	// The current expansion below generates the divide ignorant of any of these rules so it cannot be enabled
	// globally.
	...
	if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0
		...
		default: isPowAndReasonableIntExponent = false;
		}

		if (isPowAndReasonableIntExponent
		        && performTransformation(s->comp(),
		                "%sStrength reduce %s [" POINTER_PRINTF_FORMAT "] with power = %d to a series of "
		                "multiplications\n",
			...
			if (0 == absPowExponent) {
				...
				if (isExponentNegative) {
					valueNode->incReferenceCount(); // keep node alive across prepareToReplaceNode
					                                // call
...
removePaddingNode(TR::Node *node, TR::Simplifier *s)
{
	...
	if (paddingNode) {
		...
		dumpOptDetails(s->comp(), "remove paddingNode %s (0x%p) from %s (0x%p) and dec numChildren %d->%d\n",
		        paddingNode->getOpCode().getName(), paddingNode, node->getOpCode().getName(), node,
		        oldNumChildren, oldNumChildren - 1);
	}
}
