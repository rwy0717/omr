    ...
    TR::SymbolReference* symRef, vcount_t visitCount, bool& doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
        ...
        && performTransformation(comp(),
               "%s   Local Common Subexpression Elimination propagating local #%d in node : %p PARENT : %p from node "
               "%p\n",
    ...
    TR::Node* parent, TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, bool canBeAvailable)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        // common volatiles that are based on an indirection chain of such final non volatiles (or autos or parms that
        // are not global by definition) Any non volatile field that could change is a problem since it could be proven
        // that a later volatile load based on an indirection chain involving a non final, non volatile variable must be
        // different than an earlier volatile load based on the same indirection chain.
        //
        if ((!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber()))
...
bool OMR::LocalCSE::killExpressionsIfVolatileLoad(
    TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, TR_NodeKillAliasSetInterface& UseDefAliases)
{
