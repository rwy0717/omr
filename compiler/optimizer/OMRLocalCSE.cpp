...
bool
OMR::LocalCSE::shouldCopyPropagateNode(TR::Node *parent, TR::Node *node, int32_t childNum, TR::Node *storeNode)
{
...
OMR::LocalCSE::perform()
{
	...
	for (tt = comp()->getStartTree(); tt; tt = exitTreeTop->getNextTreeTop()) {
		...
		if (doExtraPassForVolatiles()) {
			if (trace())
				traceMsg(comp(),
				        "LocalCSE entering 2 pass mode for volatile elimination - pass 1 for volatiles "
				        "ONLY\n");
			_volatileState = VOLATILE_ONLY;
			transformBlock(tt, exitTreeTop);
			if (trace())
				traceMsg(comp(),
				        "LocalCSE volatile only pass 1 complete - pass 2 for non-volatiles ONLY\n");
...
OMR::LocalCSE::prePerformOnBlocks()
{
	...
	_simulatedNodesAsArray =
	        (TR::Node **)trMemory()->allocateStackMemory(comp()->getNodeCount() * sizeof(TR::Node *));
...
OMR::LocalCSE::transformBlock(TR::TreeTop *entryTree, TR::TreeTop *exitTree)
{
	...
	for (currentTree = entryTree->getNextRealTreeTop(); currentTree != exitTree->getNextTreeTop();
		...
		examineNode(currentNode, seenAvailableLoadedSymbolReferences, NULL, -1, &nextNodeIndex,
		        &nodeCanBeAvailable, 0);
	}
...
void
OMR::LocalCSE::examineNode(TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences, TR::Node *parent,
        int32_t childNum, int32_t *nextLoadIndex, bool *parentCanBeAvailable, int32_t depth)
{
	...
	// If register pressure is high at this point, then nothing should be commoned or copy propagated across this
	// point To achieve that, we clear all the commoning and copy propagation related information here
	...
	for (int32_t i = 0; i < node->getNumChildren(); i++)
		examineNode(node->getChild(i), seenAvailableLoadedSymbolReferences, node, i, nextLoadIndex,
		        &nodeCanBeAvailable, depth + 1);
	...
	if (symRef && node->getOpCode().isLoadVar()) {
		...
		if (result != _storeMap->end()
			...
			// Case #2: symrefs don't match, but underlying objects match by checking sizes, types, offsets,
			// etc.
			for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end; ++itr) {
				...
				if (storeNode->getOpCode().isStoreIndirect()
					...
					if (doCopyPropagationIfPossible(node, parent, childNum, storeNode, symRef,
					            visitCount, doneCopyPropagation))
						break;
				}
			}
		}
	...
	if (node->getOpCode().isStore() || hasAliases) {
		...
		if (hasAliases) {
			...
			if (UseDefAliases.containsAny(tmp, comp())) {
				// we want to keep the last load of a particular sym ref alive but no earlier loads
				// should survive note that volatile processing already handles killing symbols when
				...
				if (alreadyKilledAtVolatileLoad) // we want to keep the last load of a particular sym
				                                 // ref alive but no earlier loads should surivive
					seenAvailableLoadedSymbolReferences.set(symRef->getReferenceNumber());
			}
		} else {
	...
	if (verboseProcessing) {
		traceMsg(
		        comp(), "  after n%dn [%p] seenAvailableLoadedSymbolReferences:", node->getGlobalIndex(), node);
		seenAvailableLoadedSymbolReferences.print(comp());
		traceMsg(comp(), "\n");
	}
}
...
void
OMR::LocalCSE::doCommoningAgainIfPreviouslyCommoned(TR::Node *node, TR::Node *parent, int32_t childNum)
{
	for (int32_t i = 0; i < _nextReplacedNode; i++) {
		...
		if (_replacedNodesAsArray[i] == node && shouldCommonNode(parent, node)
		        && performTransformation(comp(),
		                "%s   Local Common Subexpression Elimination commoning node : %p again\n",
...
void
OMR::LocalCSE::doCommoningIfAvailable(TR::Node *node, TR::Node *parent, int32_t childNum, bool &doneCommoning)
{
		...
		if (!node->getOpCode().hasSymbolReference()
			...
			if (parent != NULL) {
				...
				if (node->getOpCode().isResolveOrNullCheck()) {
					...
					manager()->setAlteredCode(true);
					optimizer()->prepareForNodeRemoval(node);
					for (int32_t index = 0; index < _treeBeingExamined->getNode()->getNumChildren();
					        index++)
						_treeBeingExamined->getNode()
						        ->getChild(index)
						        ->recursivelyDecReferenceCount();
...
bool
OMR::LocalCSE::doCopyPropagationIfPossible(TR::Node *node, TR::Node *parent, int32_t childNum, TR::Node *storeNode,
        TR::SymbolReference *symRef, vcount_t visitCount, bool &doneCopyPropagation)
{
	...
	if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
	        ...
	        && performTransformation(comp(),
	                "%s   Local Common Subexpression Elimination propagating local #%d in node : %p PARENT : %p "
	                "from node %p\n",
...
//
void
OMR::LocalCSE::commonNode(TR::Node *parent, int32_t childNum, TR::Node *node, TR::Node *replacingNode)
{
	...
	if ((node->getOpCode().isLoadVar() || node->getOpCode().isLoadReg()) && node->isDontMoveUnderBranch()
		...
		// dumpOptDetails(comp(), "propagating dontMoveUnderBranch flag from node [%p] to replacing node
		// [%p]\n", node, replacingNode);
		...
		if (symRef->getReferenceNumber() == copySymbolReference->getReferenceNumber()) {
			...
			if (
				...
				TR_ASSERT(convOp != TR::BadILOp,
				        "BadIlOp for conversion from node %p (%s) to node %p (%s)", rhsOfStoreDefNode,
				        rhsOfStoreDefNode->getOpCode().getName(), node, node->getOpCode().getName());

				TR::Node *convNode = NULL;
				if (convOp == TR::v2v)
					convNode = TR::Node::createVectorConversion(
					        rhsOfStoreDefNode, node->getDataType());
				else
				...
				if (rhsOfStoreDefNode->getType().isBCD()) {
					if (defNode && defNode->getDataType() == TR::PackedDecimal
						...
						TR_ASSERT(rhsOfStoreDefNode->getDataType() == TR::PackedDecimal,
						        "rhsOfStoreDefNode %p (type %s) must be pd type if defNode %p "
						        "is pd\n",
						        rhsOfStoreDefNode, rhsOfStoreDefNode->getDataType().toString(),
						        defNode);
						// clean should only have been folded into defNode if rhsOfStoreDefNode
						// prec <= 31
						TR_ASSERT(rhsOfStoreDefNode->getDecimalPrecision()
						        ...
						        rhsOfStoreDefNode, rhsOfStoreDefNode->getDecimalPrecision(),
						        TR::DataType::getMaxPackedDecimalPrecision());
						...
						rhsOfStoreDefNode = TR::Node::create(origRhsOfStoreDefNode,
						        ...
						        1);
						rhsOfStoreDefNode->setChild(0, origRhsOfStoreDefNode); // already inc'ed
						                                                       // above
					...
					if (overrideNodePrecision != 0) {
						...
						if (comp()->cg()->traceBCDCodeGen() || trace())
							traceMsg(comp(),
							        "using overrideNodePrecision %d instead of node %s "
							        "(%p)\n",
							        overrideNodePrecision, node->getOpCode().getName(),
							        node);
					} else {
						TR_ASSERT(node->getType().isBCD(),
						        "node %s (%p) must be a BCD type if overrideNodePrecision == "
						        "0\n",
					...
					if (rhsOfStoreDefNode->getDecimalPrecision() != nodePrecision) {
						...
						// replaced then a size changing pdshl must be prepended in case the new
						// parent requires explicit widening (such as if the parent is a call).
						TR::Node *origRhsOfStoreDefNode = rhsOfStoreDefNode;
						TR::ILOpCodes modifyPrecisionOp = TR::ILOpCode::modifyPrecisionOpCode(
        ...
        TR::Node *parent, TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences, bool canBeAvailable)
{
	...
	if (node->getOpCode().hasSymbolReference()) {
		...
		// common volatiles that are based on an indirection chain of such final non volatiles (or autos or
		// parms that are not global by definition) Any non volatile field that could change is a problem since
		// it could be proven that a later volatile load based on an indirection chain involving a non final,
		// non volatile variable must be different than an earlier volatile load based on the same indirection
		// chain.
		//
		if ((!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber()))
		        || ((_volatileState == VOLATILE_ONLY) && !node->getSymbol()->isAutoOrParm()
...
//
bool
OMR::LocalCSE::isAvailableNullCheck(TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences)
{
	if (node->getOpCode().hasSymbolReference()) {
		if (!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber()))
			return false;
...
void
OMR::LocalCSE::killAllInternalPointersBasedOnThisPinningArray(TR::SymbolReference *symRef)
{
	ListIterator<TR::Node> arrayRefNodesIt(_arrayRefNodes);
	TR::Node *arrayRefNode = arrayRefNodesIt.getFirst();
	for (; arrayRefNode; arrayRefNode = arrayRefNodesIt.getNext()) {
...
void
OMR::LocalCSE::killAvailableExpressionsUsingBitVector(HashTable *hashTable, TR_BitVector &vec)
{
...
void
OMR::LocalCSE::killAvailableExpressionsUsingAliases(TR_UseDefAliasSetInterface &UseDefAliases)
{
	TR_BitVector tmp(_availableLoadExprs);
...
void
OMR::LocalCSE::killAllDataStructures(TR_BitVector &seenAvailableLoadedSymbolReferences)
{
	_storeMap->clear();
...
//
bool
OMR::LocalCSE::areSyntacticallyEquivalent(TR::Node *node1, TR::Node *node2, bool *remove)
{
	...
	//
	// allowBCDSignPromotion=true below (if node1 has 'better' sign state then node2 then consider equivalent as
	// well)
		...
		if (child1 != child2) {
			if (child1->getOpCode().isArrayRef() && child2->getOpCode().isArrayRef()) {
				...
				for (int32_t j = 0; j < child1->getNumChildren(); j++) {
			        ...
			        && node1->getOpCode().isCommutative() && node1->getNumChildren() == 2
			        && node2->getNumChildren() == 2
