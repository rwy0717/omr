	...
	for (tt = comp()->getStartTree(); tt != NULL; tt = tt->getNextTreeTop()) {
		...
		case TR::BBStart:
			if (!node->getBlock()->isExtensionOfPreviousBlock()) {
				if (trace())
					traceMsg(comp(),
					        "clearing remembered node choices at start of extended block at "
					        "block_%d\n",
...
TR::RegDepCopyRemoval::selectNodesToReuse(TR::NodeChecklist &usedNodes)
{
	for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
		...
		TR_ASSERT(
		        !usedNodes.contains(prevChoice.selected), "attempted to reuse the same node more than once\n");
...
TR::RegDepCopyRemoval::updateRegDeps(TR::NodeChecklist &usedNodes)
{
	for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
		RegDepInfo &dep = getRegDepInfo(reg);
		switch (dep.state) {
		case REGDEP_NODE_ORIGINAL: rememberNodeChoice(reg, dep.value); break;
		case REGDEP_NODE_REUSE_COPY: reuseCopy(reg); break;
		case REGDEP_NODE_FRESH_COPY: makeFreshCopy(reg); break;
		case REGDEP_IGNORED: discardNodeChoice(reg); break;
		case REGDEP_ABSENT: {
		...
		case REGDEP_UNDECIDED:
			TR_ASSERT(
			        false, "undecided preference for %s survived to updateRegDeps()\n", registerName(reg));
			break;
		default: TR_ASSERT(false, "%s has bad RegDepInfo state %d\n", registerName(reg), (int)dep.state); break;
		}
	}
}
...
TR::RegDepCopyRemoval::reuseCopy(TR_GlobalRegisterNumber reg)
{
	...
	TR_ASSERT(
	        prevChoice.selected != dep.value, "previous copy is the same as original for %s\n", registerName(reg));
	if (performTransformation(comp(), "%schange %s in GlRegDeps n%un to use previous copy n%un of n%un\n",
	            ...
	            prevChoice.selected->getGlobalIndex(), prevChoice.original->getGlobalIndex())) {
...
TR::RegDepCopyRemoval::makeFreshCopy(TR_GlobalRegisterNumber reg)
{
	...
	if (!performTransformation(comp(), "%schange %s in GlRegDeps n%un to an explicit copy of n%un\n",
	            optDetailString(), registerName(reg), _regDeps->getGlobalIndex(), dep.value->getGlobalIndex()))
		return;
...
TR::RegDepCopyRemoval::generateRegcopyDebugCounter(const char *category)
{
	...
	const char *fullName = TR::DebugCounter::debugCounterName(comp(),
	        "regcopy/RegDepCopyRemoval/%s/(%s)/%s/block_%d", category, comp()->signature(),
	        comp()->getHotnessName(comp()->getMethodHotness()), _treetop->getEnclosingBlock()->getNumber());
