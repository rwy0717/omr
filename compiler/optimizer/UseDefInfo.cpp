...
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation *comp, TR::CFG *cfg, TR::Optimizer *optimizer, bool requiresGlobals,
        bool prefersGlobals, bool loadsShouldBeDefs, bool cannotOmitTrivialDefs, bool conversionRegsOnly,
        ...
        bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs, bool conversionRegsOnly)
{
	...
	if (_hasLoadsAsDefs && !cannotOmitTrivialDefs && (comp()->getMethodHotness() < hot)) {
		for (int32_t j = 0; j < numSymRefs; j++) {
			if (symRefTab->getSymRef(j) && symRefTab->getSymRef(j)->getSymbol()->isAutoOrParm()) {
				aux._onceReadSymbolsIndices[j].GrowTo(comp()->getNodeCount()); /* no effect for sparse
				                                                                  bit vectors, except to
				                                                                  make non-null */
			}
		}
	}
	...
	if (_hasLoadsAsDefs && !cannotOmitTrivialDefs) {
		for (int32_t j = 0; j < numSymRefs; j++) {
			if (symRefTab->getSymRef(j) && symRefTab->getSymRef(j)->getSymbol()->isAutoOrParm()) {
				aux._onceWrittenSymbolsIndices[j].GrowTo(comp()->getNodeCount()); /* no effect for
				                                                                     sparse bit vectors,
				                                                                     except to make
				                                                                     non-null */
			}
	...
	if (requiresGlobals || prefersGlobals) {
		...
		if (requiresGlobals && !canBuild) {
			...
			if (trace())
				traceMsg(comp(),
				        "Use/Def info: cannot build global use/def info as it failed in "
				        "indexSymbolsAndNodes\n");
			return;
		}
	}
	...
	if (!canBuild) {
		...
		if (trace())
			traceMsg(comp(),
			        "Use/Def info: cannot build global nor local use/def info as it failed in "
			        "indexSymbolsAndNodes\n");
		return;
	}
...
TR_UseDefInfo::performAnalysis(AuxiliaryData &aux)
{
	if (!_isUseDefInfoValid)
		// use-def info hasn't been successfully generated so performing reaching definition analysis is not
		// possible
		return false;
	if (trace())
...
bool
TR_UseDefInfo::_runReachingDefinitions(TR_ReachingDefinitions &reachingDefinitions, AuxiliaryData &aux)
{
	// stack memory is prohibited in UseDefInfo, but here we have to use it as reachingDefinitions use stack memory
	// for _blockAnalysisInfo
...
void
TR_UseDefInfo::setVolatileSymbolsIndexAndRecurse(TR::BitVector &volatileSymbols, int32_t symRefNum)
{
...
void
TR_UseDefInfo::findAndPopulateVolatileSymbolsIndex(TR::BitVector &volatileSymbols)
{
...
TR_UseDefInfo::fillInDataStructures(AuxiliaryData &aux)
{
	...
	comp()->incVisitCount();
	TR::Block *block = NULL;
	TR::SparseBitVector expandedLIndexes(comp()->allocator()); // expanded localIndex for variables defined by
	                                                           // function
	for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop != NULL; treeTop = treeTop->getNextTreeTop()) {
		if (treeTop->getNode()->getOpCodeValue() == TR::BBStart) {
			block = treeTop->getNode()->getBlock();
	...
	if (_uniqueIndexForDefsOnEntry) {
		for (int32_t i = getFirstDefIndex(); i < _numDefsOnEntry; i++) {
			...
			for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
				...
				// traceMsg(comp(),"\n _numDefsOnEntry=%d getNumExpandedDefNodes()=%d j=(u%d s#%d)
				// i=(u%d s#%d)",_numDefsOnEntry, getNumExpandedDefNodes(), j,
				// aliasedSymRef->getReferenceNumber(),i, symRef->getReferenceNumber());
			}
		}
	}
}
...
void
TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node *node, TR::TreeTop *treeTop, AuxiliaryData &aux)
{
...
bool
TR_UseDefInfo::isTrivialUseDefSymRef(TR::SymbolReference *symRef, AuxiliaryData &aux)
{
	// if (!_hasLoadsAsDefs)
	//   return false;
...
TR_UseDefInfo::findMemorySymbols(TR::Node *node)
{
	...
	if ((node->getOpCode().isLoadIndirect() || node->getOpCode().isStoreIndirect())
	        ...
	        && _valueNumberInfo->getNext(node->getFirstChild()) != node->getFirstChild()) // value number is shared
	                                                                                      // with some other node
	{
		...
		if (trace())
			traceMsg(comp(), "Node %p has memory symbol index %d (%d:%d:%d)\n", node,
			        _valueNumbersToMemorySymbolsMap[valueNumber]->front()._localIndex, valueNumber, size,
			        offset);
	}
}
...
bool
TR_UseDefInfo::shouldIndexVolatileSym(TR::SymbolReference *ref, AuxiliaryData &aux)
{
...
TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData &aux)
{
		...
		if (symRef) {
			...
			if (sym) {
				...
				if (!shouldIndexVolatileSym(symRef, aux)) {
					if (trace())
						traceMsg(comp(),
						        "Ignoring Symbol [%p] because it is volatile %d or aliased to "
						        "a volatile %d\n",
						        sym, sym->isVolatile(),
	...
	// A cleared symRefToLocalIndexMap is required for each call to indexSymbolsAndNodes - so we create it as a
	// local variable here, and pass it down to findUseDefNodes, as findUseDefNodes is recursive.
	...
	int32_t maxDefByUseNodes = ((comp()->getMethodHotness() >= hot)) ? MAX_DEF_BY_USE_NODES_FOR_HIGH_OPT
	...
	if (getNumExpandedDefNodes() >= maxExpandedDefOrUseNodes) {
		dumpOptDetails(
		        comp(), "   use/def failed, too many expanded def nodes (%d)\n", getNumExpandedDefNodes());
		return false;
	}
	if (getNumExpandedUseNodes() >= maxExpandedDefOrUseNodes) {
		dumpOptDetails(
		        comp(), "   use/def failed, too many expanded use nodes (%d)\n", getNumExpandedUseNodes());
		return false;
	}
	...
	if (getExpandedTotalNodes() > MAX_EXPANDED_TOTAL_NODES) {
		dumpOptDetails(
		        comp(), "   use/def failed, too many expanded nodes in total (%d)\n", getExpandedTotalNodes());
		return false;
	}
...
bool
TR_UseDefInfo::skipAnalyzingForCompileTime(TR::Node *node, TR::Block *block, TR::Compilation *comp, AuxiliaryData &aux)
{
...
bool
TR_UseDefInfo::findUseDefNodes(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
        AuxiliaryData &aux, TR::deque<uint32_t, TR::Region &> &symRefToLocalIndexMap, bool considerImplicitStores)
{
	...
	for (int32_t i = 0; i < node->getNumChildren(); i++) {
		bool shouldConsiderLoadAddrChildrenImplicitStores = parentCouldHaveImplicitStoreChildren
	...
	if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
		if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
			...
			if (num_aliases > 0) {
				if (symRefToLocalIndexMap[symRef->getReferenceNumber()] == 0) {
					...
					// opCode.isFence(), node->getSymbolReference()->getSymbol()->isMethod());
					useDefIndex = _numDefUseNodes++;
			...
			if (isTrivialUseDefNode(node, aux)) {
				if (symRef
					...
					if (aux._onceReadSymbols[symRef->getReferenceNumber()] == NULL)
						aux._onceReadSymbols[symRef->getReferenceNumber()] = new (aux._region)
						        TR_BitVector(aux._region);
		...
		localIndex = _numExpandedDefOnlyNodes;
		_numExpandedDefOnlyNodes += TR::NumTypes + 1; // == arraylet shadows + read barrier....encode this
...
bool
TR_UseDefInfo::assignAdjustedNodeIndex(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
        AuxiliaryData &aux, bool considerImplicitStores)
{
	...
	if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
		if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
			...
			//         traceMsg(comp(), "node %p setting adjustment to %d _numDefOnlyNodes = %d,
	...
	if (symRef && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].ValueAt(node->getGlobalIndex())) {
		if (aux._onceWrittenSymbols[symRef->getReferenceNumber()] == NULL)
			aux._onceWrittenSymbols[symRef->getReferenceNumber()] = new (aux._region)
			        TR_BitVector(aux._region);
...
bool
TR_UseDefInfo::childIndexIndicatesImplicitStore(TR::Node *node, int32_t childIndex)
{
	return false;
}

void
TR_UseDefInfo::insertData(TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop, AuxiliaryData &aux,
        TR::SparseBitVector &expandedLIndexes, bool considerImplicitStores)
{
	vcount_t visitCount = comp()->getVisitCount();
	if (visitCount == node->getVisitCount())
		return;
...
bool
isSymRefFromInlinedMethod(TR::Compilation *comp, TR::ResolvedMethodSymbol *currentMethod, TR::SymbolReference *symRef)
{
	while (true) {
...
TR_UseDefInfo::buildUseDefs(void *vblockInfo, AuxiliaryData &aux)
{
	...
	if (trace()) {
		traceMsg(comp(),
		        ...
		        "getNumDefsOnEntry()=%d\n",
		        getFirstDefIndex(), getLastDefIndex(), getFirstUseIndex(), getLastUseIndex(),
		        getNumDefsOnEntry());
	}
...
//
int32_t
TR_UseDefInfo::setSingleDefiningLoad(
        int32_t useIndex, TR_UseDefInfo::BitVector &nodesLookedAt, TR_UseDefInfo::BitVector &loadDefs)
{
	TR_UseDefInfo::BitVector &useDefInfo = _useDefInfo[useIndex];
	nodesLookedAt[useIndex] = true;
        ...
        TR::Node *node, void *vanalysisInfo, TR_BitVector &nodesToBeDereferenced, TR::Node *parent, AuxiliaryData &aux)
{
	...
	if (node->getOpCode().hasSymbolReference() && isTrivialUseDefNode(node, aux)) {
		...
		if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
			...
			if (aux._neverWrittenSymbols.get(node->getSymbolReference()->getReferenceNumber())) {
			...
			} else if (!aux._onceReadSymbolsIndices[node->getSymbolReference()->getReferenceNumber()]
			                    .IsNull()) {
				if (trace())
					traceMsg(comp(), "Use node %p is of a symbol read only once\n", node);

				TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex = %d\n", realIndex);
				//            traceMsg(comp(), "UDI: oring in _useDefInfo[realIndex=%d] with
				//            ",realIndex);
				//           *(comp()) <<
			...
			} else if (!aux._onceWrittenSymbolsIndices[node->getSymbolReference()->getReferenceNumber()]
			                    .IsNull()) {
				if (trace())
					traceMsg(comp(), "Use node %p is of a symbol write only once\n", node);

				TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex = %d\n", realIndex);
				//           traceMsg(comp(), "UDI: oring2 in _useDefInfo[realIndex=%d] with
				//           ",realIndex);
				//           *(comp()) <<
				...
				//   _onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()]; TR::Node
				//   *storeNode = storeTree->getNode(); if (!storeNode->getOpCode().isStore() &&
				...
				//   TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex = %d\n",
				//   realIndex); _useDefInfo[realIndex][storeNode->getUseDefIndex()] = true;
				//    }
			}
		}
			...
			while (cursor.hasMoreElements()) {
				...
				if (i >= _numDefsOnEntry) {
					...
					if (memSymIndex == -1 || getMemorySymbolIndex(defNode) != memSymIndex
						...
						//              _useDefInfo[realIndex=%d][j=%d] to true\n",realIndex,j);
					...
					//          traceMsg(comp(), "UDI: setting _useDefInfo[realIndex=%d][i=%d] to
					//          true\n",realIndex,i);
...
const TR_UseDefInfo::BitVector &
TR_UseDefInfo::getUseDef_ref_body(int32_t useIndex, TR_BitVector *visitedDefs, TR_UseDefInfo::BitVector *defs)
{
...
bool
TR_UseDefInfo::getUsesFromDef(BitVector &usesFromDef, int32_t defIndex, bool loadAsDef)
{
...
TR_UseDefInfo::getNode(int32_t index)
{
	...
	if (ud->isDef()) {
		...
		if ((node->getOpCode().isCheck() || node->getOpCodeValue() == TR::treetop)) {
			...
			// But we must save and restore the number of children here, because if VP deletes unreachable
			// code, it may expect the number of children to be set to zero.
			//
	...
	for (int32_t i = getFirstUseIndex(); i <= getLastUseIndex(); i++) {
		...
		if (!defs.IsZero()) {
			...
			for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
				...
				TR_ASSERT((defIndex < getNumDefNodes()),
				        "USEDEF: found def which is not store or call: useIndex = %d, defIndex = %d", i,
				        defIndex);
		...
		if (loadAsDef) {
			...
			if (!loadDefs.IsZero()) {
				...
				for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
					int32_t defIndex = cursor;
					// TR_ASSERT((defIndex < getNumDefNodes()), "USEDEF: found def which is not
					// store or call %d", defIndex);
