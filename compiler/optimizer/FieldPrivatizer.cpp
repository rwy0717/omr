...
TR_FieldPrivatizer::isFieldAliasAccessed(TR::SymbolReference *symRef)
{
	...
	if (symRef->getUseDefAliases().hasAliases()) {
		...
		TR_ASSERT(
		        _allSymRefs[symRef->getReferenceNumber()] == true, "symRef should have been seen in the loop");
        ...
        TR_Structure *loopStructure, TR_BitVector **optSetInfo, int32_t bitVectorSize)
{
	...
	if (loopInvariantBlock
		...
		int32_t isPredictableLoop =
		        checkLoopForPredictability(loopStructure, loopInvariantBlock->getBlock(), 0);
		...
		if (_stringPeepholeTree && !_stringSymRef->getUseonlyAliases().hasAliases()
			...
			_appendSymRef = 0;
			_stringBufferClass =
			        fe()->getClassFromSignature("java/lang/StringBuffer", 22, comp()->getCurrentMethod());
					...
					for (auto edge = pred->getSuccessors().begin();
						...
						if (next != invariantBlock) {
							...
							if (next->getSuccessors().size() == 1) {
								TR::Block *join = next->getSuccessors()
								                          .front()
								                          ->getTo()
								                          ->asBlock();
								for (auto predEdge = join->getPredecessors().begin();
								        ...
								        ++predEdge) {
									if (blocksInRegion.find((*predEdge)
									                                ->getFrom()
									                                ->asBlock())) {
...
bool
TR_FieldPrivatizer::containsEscapePoints(TR_Structure *structure, bool &containsStringPeephole)
{
	bool result = false;
...
bool
TR_FieldPrivatizer::isStringPeephole(TR::Node *currentNode, TR::TreeTop *currentTree)
{
...
void
TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR_Structure *structure, vcount_t visitCount)
{
	if (structure->asBlock() != 0) {
		TR_BlockStructure *blockStructure = structure->asBlock();
		TR::Block *block = blockStructure->getBlock();
...
void
TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR::Node *node, vcount_t visitCount)
{
...
void
TR_FieldPrivatizer::privatizeNonEscapingLoop(TR_Structure *structure, TR::Block *entryBlock, vcount_t visitCount)
{
	if (structure->asBlock() != 0) {
		TR_BlockStructure *blockStructure = structure->asBlock();
		TR::Block *block = blockStructure->getBlock();
...
void
TR_FieldPrivatizer::privatizeFields(TR::Node *node, bool postDominatesEntry, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
	node->setVisitCount(visitCount);
	...
	if (opCode.isStore() || opCode.isLoadVar()) {
		...
		if (!_fieldsThatCannotBePrivatized->get(symRef->getReferenceNumber())) {
			...
			bool canPrivatizeBasedOnThisNode = performTransformation(comp(),
			        "%s Field access %p using sym ref %d privatized ", optDetailString(), node,
				...
				if (opCode.isIndirect()) {
					...
					if (opCode.isStore()) {
						...
						if (node->getOpCodeValue() == TR::awrtbar) {
							...
							node->setNumChildren(1);
							TR::Node::recreate(
							        node, comp()->il.opCodeForDirectStore(nodeDataType));
						}
					}
				}
			}
...
TR_FieldPrivatizer::canPrivatizeFieldSymRef(TR::Node *node)
{
	...
	//    1. If the symref is in _privatizedFields then we add this node to the list of those that can't be
	//    privatized.
	...
	while (currentNodeElem) {
		...
		if (duplicateNode->getOpCode().isStore()) {
			if (duplicateNode->getOpCode().isIndirect()) {
				...
				TR::Node::recreate(
				        duplicateNode, comp()->il.opCodeForDirectLoad(duplicateNode->getDataType()));
		...
		// currentSymRefElem->getData()->getReferenceNumber()))
		{
			TR::Node *initNode = TR::Node::createWithSymRef(
			        comp()->il.opCodeForDirectStore(duplicateNode->getDataType()), 1, 1, duplicateNode,
			        currentSymRef);
...
void
TR_FieldPrivatizer::placeStoresBackInExits(List<TR::Block> *exitBlocks, List<TR::Block> *blocksInLoop)
{
	TR::CFG *cfg = comp()->getFlowGraph();
		...
		for (auto edge = exitBlock->getSuccessors().begin(); edge != exitBlock->getSuccessors().end();) {
			...
			if (!blocksInsideLoop->get(next->getNumber())
				...
				if (exitBlock->getExit()->getNextTreeTop() == next->getEntry()) {
					...
					// The block nnext must be appended to the end before, now we can move it to
					// here-pefect
					else {
				...
				// D191793, there are cases while we goto next block, so update branchDestination
				// conditionaly for safety
				...
				if (trace()) {
					...
					traceMsg(comp(), "new edge %d(%d) -> %d(%d) freq %d\n", from->getNumber(),
					        from->getFrequency(), to->getNumber(), to->getFrequency(),
...
TR_FieldPrivatizer::addPrivatizedRegisterCandidates(TR_Structure *structure)
{
	...
	while (privatizedCandidate) {
		if (performTransformation(comp(),
		            ...
		            optDetailString(),
		            privatizedCandidate->getData()->getSymbolReference()->getReferenceNumber(),
		            structure->getNumber()))
...
TR_FieldPrivatizer::addStringInitialization(TR::Block *block)
{
	...
	TR::Node *classObject =
	        TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, stringBufferSymRef);
	TR::Node *initNode = TR::Node::createWithSymRef(
	...
	TR::SymbolReference *initSymRef = _initSymRef
	        ? getSymRefTab()->findOrCreateMethodSymbol(
	                ...
	                _initSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
	...
	TR::Node *initCall = TR::Node::create(TR::treetop, 1,
	        TR::Node::createWithSymRef(TR::call, 2, 2, initNode->getFirstChild(),
	                TR::Node::createWithSymRef(initNode->getFirstChild(), TR::aload, 0, _stringSymRef),
	                initSymRef));
...
void
TR_FieldPrivatizer::placeStringEpilogueInExits(List<TR::Block> *exitBlocks, List<TR::Block> *blocksInLoop)
{
#ifdef J9_PROJECT_SPECIFIC
	TR::CFG *cfg = comp()->getFlowGraph();
	TR_BitVector *seenExitBlocks = new (trStackMemory())
...
void
TR_FieldPrivatizer::placeStringEpiloguesBackInExit(TR::Block *block, bool placeAtEnd)
{
	...
	TR::SymbolReference *toStringSymRef = _toStringSymRef
	        ? comp()->getSymRefTab()->findOrCreateMethodSymbol(
	                ...
	                _toStringSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
