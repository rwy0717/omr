    ...
    for (TR::TreeTop* tt = comp()->getMethodSymbol()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (loadOrStoreNode && isNodeInCorrectForm(loadOrStoreNode)) {
            traceMsg(comp(), "Adding node %p tt %p with vn %d to list of candidates to privatize\n", loadOrStoreNode,
                tt, _valueNumberInfo->getValueNumber(loadOrStoreNode->getFirstChild()));
...
void TR_FieldPrivatizer::privatizeElementCandidates()
{
    ...
    for (auto cursor = _epCandidates.begin(); cursor != _epCandidates.end(); ++cursor) {
        ...
        if (tempMap.Locate(candidate.valueNum, index)) {
            ...
            if (offset < 0) // This means that candidate has a value number and some other node will have the addr
                            // calculation, we just have to wait till it gets added to the hashtable
            {
                difficultCandidates.push_back(candidate);
                continue;
            }

            tempSymRef = comp()->getSymRefTab()->createCoDependentTemporary(comp()->getMethodSymbol(),
                candidate.node->getDataType(), false, candidate.node->getSize(), candidate.node->getSymbol(), 0);
        ...
        TR::TreeTop* anchorTT
            = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, candidate.node->getFirstChild()));
        candidate.rootTT->insertBefore(anchorTT);

        // step 2, convert the indirect store in to a direct store.

        TR_ASSERT(candidate.node->getReferenceCount() == 1,
    ...
    for (auto difficult_cursor = difficultCandidates.begin(); difficult_cursor != difficultCandidates.end();
         ++difficult_cursor) {
        TR::SymbolReference* tempSymRef = 0;
...
bool TR_FieldPrivatizer::isNodeInCorrectForm(TR::Node* node)
{
    ...
    TR_ASSERT(node->getOpCode().isStoreIndirect() || node->getOpCode().isLoadIndirect(),
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::loadaddr) {
        ...
        && ((node->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::loadaddr
                && node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst())
               || (node->getFirstChild()->getFirstChild()->getOpCode().isLoadConst()
                      && node->getFirstChild()->getSecondChild()->getOpCodeValue() == TR::loadaddr))) {

        int64_t offset = getOffSetFromAddressNode(node->getFirstChild());

        if (offset < 0
            || offset > 20) // a basic heuristic to not do this for massively huge arrays.  Maybe I don't need this.
            return false;
