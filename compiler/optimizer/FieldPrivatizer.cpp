...
bool TR_FieldPrivatizer::isNodeInCorrectForm(TR::Node* node)
{
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::loadaddr) {
        return true;
    } else if (node->getFirstChild()->getOpCode().isAdd() && ((node->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) || (node->getFirstChild()->getFirstChild()->getOpCode().isLoadConst() && node->getFirstChild()->getSecondChild()->getOpCodeValue() == TR::loadaddr))) {
