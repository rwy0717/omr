...
TR_LiveVariableInformation::TR_LiveVariableInformation(TR::Compilation* c, TR::Optimizer* optimizer,
    TR_Structure* rootStructure, bool splitLongs, bool includeParms, bool includeMethodMetaDataSymbols,
    bool ignoreOSRUses)
    : _compilation(c)
    ...
    TR_BitVector* commonedLoads, bool belowCommonedNode)
{
    ...
    } else if (!belowCommonedNode && _includeMethodMetaDataSymbols && node->getOpCode().isLoadIndirect()
                    ...
                    for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                        ...
                        if (usedSymbol && usedSymbol->getDataType() != TR::NoType) {
                            ...
                            if (usedSymbolIndex != INVALID_LIVENESS_INDEX && usedSymbolIndex < _numLocals
                                ...
                                if (traceLiveVarInfo())
                                    traceMsg(comp(),
                                        "       setting symIdx %d (from symRef %d) on usedSymbols (in liveness)\n",
...
TR_OSRLiveVariableInformation::TR_OSRLiveVariableInformation(TR::Compilation* c, TR::Optimizer* optimizer,
    TR_Structure* rootStructure, bool splitLongs, bool includeParms, bool includeMethodMetaDataSymbols,
    bool ignoreOSRUses)
    : TR_LiveVariableInformation(
          c, optimizer, rootStructure, splitLongs, includeParms, includeMethodMetaDataSymbols, ignoreOSRUses)
{
