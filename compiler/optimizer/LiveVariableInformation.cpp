...
void
TR_LiveVariableInformation::initializeGenAndKillSetInfo(TR_BitVector **regularGenSetInfo,
				...
				if (localIndex != INVALID_LIVENESS_INDEX) {
					...
					if (!seenException) {
						...
						if (_splitLongs && local->getType().isInt64()) {
							...
							if (_cachedExceptionKillSetInfo)
								_cachedExceptionKillSetInfo[blockNum]->set(
								        localIndex + 1);
						}
					}
				}
...
void
TR_LiveVariableInformation::findCommonedLoads(
...
void
TR_LiveVariableInformation::findAllUseOfLocal(TR::Node *node, int32_t blockNum, TR_BitVector *genSetInfo,
...
void
TR_LiveVariableInformation::findLocalUsesInBackwardsTreeWalk(TR::Node *node, int32_t blockNum, TR_BitVector *genSetInfo,
        TR_BitVector *killSetInfo, TR_BitVector *commonedLoads, vcount_t visitCount)
{
	TR_ASSERT(commonedLoads != NULL,
	        "can't do a backward tree walk properly unless recording commoned loads for tree");
	visitTreeForLocals(node, &genSetInfo, killSetInfo, false, true, visitCount, commonedLoads, false);
}

void
TR_LiveVariableInformation::findUseOfLocal(TR::Node *node, int32_t blockNum, TR_BitVector **genSetInfo,
        TR_BitVector **killSetInfo, TR_BitVector *commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
	visitTreeForLocals(node, genSetInfo + blockNum, killSetInfo[blockNum], movingForwardThroughTrees, false,
...
//    efficiently
void
TR_LiveVariableInformation::visitTreeForLocals(TR::Node *node, TR_BitVector **blockGenSetInfo,
        ...
        TR_BitVector *commonedLoads, bool belowCommonedNode)
{
	...
	if (traceLiveVarInfo()) {
		if (movingForwardThroughTrees)
			traceMsg(comp(),
			        "         Looking forward for uses in node %p has visitCount = %d and comp() "
			        "visitCount = %d\n",
			        node, node->getVisitCount(), visitCount);
		else
			traceMsg(comp(),
			        "         Looking backward for uses in node %p has future use count = %d and reference "
			        "count = %d\n",
	...
	if (movingForwardThroughTrees) {
		...
		if (!belowCommonedNode) {
			...
			if (node->getVisitCount() == visitCount) {
				if (visitEntireTree)
					...
					// this node has already been visited and we're not supposed to visit the entire
					// tree
					return;
			} else
		...
		if (!belowCommonedNode) {
			...
			if (node->getFutureUseCount() > 0) {
				...
				// traversal should either end here (for regular traversal) or track that we're below a
				// commoned node
				if (visitEntireTree)
					belowCommonedNode = true;
				else
		...
		// visit count used to detect commoned nodes WITHIN each tree: visit count must be different for each
		// tree in an EBB for this to work properly ...IF the visit count isn't different for each tree, this
	...
	if (local || !indirectMethodMetaUses.IsZero() || !regStarAsmUses.IsZero()) {
		...
		if (belowCommonedNode) {
			...
			if (_liveCommonedLoads != NULL) {
				if (traceLiveVarInfo())
					traceMsg(
					        comp(), "              Marking %d as live commoned load\n", localIndex);
...

		{
			...
			if (!movingForwardThroughTrees || blockKillSetInfo == NULL
			        || (local && !blockKillSetInfo->get(localIndex)) || !regStarAsmUses.IsZero()
					...
					for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
						...
						if (usedSymbol && usedSymbol->getDataType() != TR::NoType) {
							...
							if (usedSymbolIndex != INVALID_LIVENESS_INDEX
								...
								if (traceLiveVarInfo())
									traceMsg(comp(),
									        "       setting symIdx %d (from symRef "
									        "%d) on usedSymbols (in liveness)\n",
									        usedSymbolIndex,
 ...
 */
void
TR_OSRLiveVariableInformation::findUseOfLocal(TR::Node *node, int32_t blockNum, TR_BitVector **genSetInfo,
        TR_BitVector **killSetInfo, TR_BitVector *commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
	...
	if (comp()->isPotentialOSRPoint(node)) {
		...
		if (comp()->getOption(TR_TraceOSR)) {
			traceMsg(comp(),
			        "liveSymbols introduced by real uses at OSRPoint node n%dn:", node->getGlobalIndex());
...
TR_OSRLiveVariableInformation::getLiveSymbolsInInterpreter(TR_ByteCodeInfo &byteCodeInfo)
{
	...
	if (bcLiveSymbolMaps[callerIndex + 1] == NULL) {
		bcLiveSymbolMap = new TR_BCLiveSymbolsMap(
		        TR_BCLiveSymbolsMapComparator(), TR_BCLiveSymbolsMapAllocator(comp()->region()));
        ...
        : TR_LiveVariableInformation(c, optimizer, rootStructure, splitLongs, includeParms, ignoreOSRUses)
{
	...
	bcLiveSymbolMaps = (TR_BCLiveSymbolsMap **)comp()->trMemory()->allocateHeapMemory(
