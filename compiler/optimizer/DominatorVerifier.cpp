...
TR_DominatorVerifier::TR_DominatorVerifier(TR_Dominators &findDominators) : _compilation(findDominators.comp())
{
...
bool
TR_DominatorVerifier::isExpensiveAlgorithmCorrect(TR_DominatorsChk &expensiveAlgorithm)
{
	int32_t i, j;
		...
		for (j = 0; j < _numBlocks - 1; j++) {
			if (bucket->get(j)) // dominator according to algorithm
			{
				...
				if (!dominates(_dominatorsChkInfo[j + 1]._block,
				            _dominatorsChkInfo[i]._block)) // dominator
				                                           // according
				                                           // to
				                                           // the
				                                           // CFG
				{
					if (debug("traceVER")) {
						...
						dumpOptDetails(comp(),
						        "   But [%p] is not an the dominator of [%p] as per the "
						        "Control Flow Graph",
...
//
void
TR_DominatorVerifier::compareWithPredsOf(TR::Block *dominator, TR::Block *dominated)
{
