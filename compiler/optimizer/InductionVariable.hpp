...
public:
	...
	// bool replaceAllInductionVariableComputations(TR::Block *loopInvariantBlock, TR_Structure *,
	...
	TR::Node *placeNewInductionVariableIncrementTree(TR_BlockStructure *, TR::SymbolReference *,
	        TR::SymbolReference *, int32_t, TR::SymbolReferenceTable *, TR::Node *, int32_t);
	...
	TR::Node *duplicateAdditiveTermNode(int32_t k, TR::Node *node, TR::DataType type)
	{
		...
		if (new_node->getDataType() != type)
			new_node = TR::Node::create(TR::ILOpCode::getProperConversion(new_node->getDataType(), type,
	...
	typedef std::map<uint32_t, List<TR_StoreTreeInfo> *, StoreTreeMapComparator, StoreTreeMapAllocator>
	        StoreTreeMap;
...
private:
	...
	void examineOpCodesForInductionVariableUse(TR::Node *node, TR::Node *parent, int32_t &childNum, int32_t &index,
	        ...
	        TR::AutomaticSymbol *pinningArrayPointer, int64_t differenceInAdditiveConstants,
	        bool &isInternalPointer, bool &downcastNode, bool &usingAladd);
...
public:
	...
	TR_PrimaryInductionVariable(TR_BasicInductionVariable *biv, TR::Block *branchBlock, TR::Node *exitBound,
	        TR::ILOpCodes exitOp, TR::Compilation *comp, TR::Optimizer *opt, bool usesUnchangedValueInLoopTest,
	        bool trace);
...
public:
	TR_ALLOC(TR_Memory::InductionVariableAnalysis);
	TR_DerivedInductionVariable(
	        TR::Compilation *c, TR_BasicInductionVariable *biv, TR_PrimaryInductionVariable *piv)
...
private:
	...
	public:
		TR_ALLOC(TR_Memory::InductionVariableAnalysis);
		DeltaInfo(int32_t delta) : _delta(delta), _unknown(false), _kind(Identity) {}
		DeltaInfo(DeltaInfo *other) : _delta(other->_delta), _unknown(other->_unknown), _kind(other->_kind) {}
