...
void
collectArraylengthNodes(TR::Node *node, vcount_t visitCount, List<TR::Node> *arraylengthNodes)
{
...
OMR::ValuePropagation::createRelationship(int32_t relative, TR::VPConstraint *constraint)
{
...
OMR::ValuePropagation::createStoreRelationship(TR::Symbol *symbol, Relationship *firstRel)
{
...
void
OMR::ValuePropagation::freeStoreRelationships(TR_LinkHead<StoreRelationship> &list)
{
...
bool
OMR::ValuePropagation::propagateConstraint(
        TR::Node *node, int32_t valueNumber, Relationship *first, Relationship *rel, ValueConstraints *valueConstraints)
{
	...
	if (rel->relative != AbsoluteConstraint) {
		...
		if (!addConstraintToList(node, rel->relative, valueNumber,
		            rel->constraint->asRelation()->getComplement(this), valueConstraints))
			return false;
	}
...
//
TR::VPConstraint *
OMR::ValuePropagation::addConstraintToList(TR::Node *node, int32_t valueNumber, int32_t relative,
        TR::VPConstraint *constraint, ValueConstraints *valueConstraints, bool replaceExisting)
{
			...
			if (updateStoreConstraint) {
				...
				if (!storeRel) {
					...
					if (TR::VPConstraint::isSpecialClass((uintptrj_t)constraint->getClass())) {
						...
						storeConstraint = TR::VPClass::create(this, NULL,
						        constraint->getClassPresence(), constraint->getPreexistence(),
						        constraint->getArrayInfo(), constraint->getObjectLocation());
					} else
	...
	if (newOrChanged) {
		if (trace() && node) {
			traceMsg(comp(), "   n%in %s gets new constraint:", node->getGlobalIndex(),
			        node->getOpCode().getName());
			rel->print(this, valueNumber, 1);
			traceMsg(comp(),
			        "type of constraint - longConstraint: %p, intConstraint: %p, shortConstraint: %p\n",
		...
		if (valueNumber < _firstUnresolvedSymbolValueNumber) {
			...
			if (!propagateConstraint(
			            node, valueNumber, cur->relationships.getFirst(), rel, valueConstraints))
				propFailed = true;
...
TR::VPConstraint *
OMR::ValuePropagation::addGlobalConstraint(TR::Node *node, TR::VPConstraint *constraint, TR::Node *relative)
{
        ...
        TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint, int32_t relative)
{
	...
	// The assume below is informational in nature and should therefore not be triggered if production builds
	// include assumes (as on WCode or binopt). This condition should rarely show up in WCode or binopt (as
	// Simplifier transformations will homogenize the types) but can happen, for example, when an aggregate symbol
	// is referenced in one place on an oload and another as an iload. MG: Update assert?
	...
	if (newOrChanged) {
		...
		if (trace() && node) {
			traceMsg(comp(), "   %s [%p] found existing global constraint value number %d (%p): ",
...
TR::VPConstraint *
OMR::ValuePropagation::addEdgeConstraint(
...
TR::VPConstraint *
OMR::ValuePropagation::addBlockConstraint(
...
TR::VPConstraint *
OMR::ValuePropagation::addBlockOrGlobalConstraint(
...
void
OMR::ValuePropagation::mergeRelationships(TR_LinkHead<Relationship> &fromList, TR_LinkHead<Relationship> &toList,
	...
	while (from || to) {
		if (!to || (from && from->relative < to->relative)) {
			...
			if (preserveStoreRelationships && mergingStore && (from->relative == AbsoluteConstraint)
			        && (!inBothLists /* || !isToEmpty */)) // commented out the !isToEmpty because it is not
			                                               // guaranteed to be safe to take an absolute
			                                               // constraint that only exists on one side
			{
...
void
OMR::ValuePropagation::mergeStoreRelationships(ValueConstraint *fromvc, ValueConstraint *tovc, bool preserveFrom)
{
	...
	while (from || to) {
		...
		if (!fromRel) {
			...
			mergeRelationships(*fromRel, *toRel, valueNumber, preserve, prev /*mergingStore*/,
			        &storeSymbols, inBothLists);
		}
	}
}

void
OMR::ValuePropagation::mergeValueConstraints(ValueConstraint *fromvc, ValueConstraint *tovc, bool preserveFrom)
{
...
void
OMR::ValuePropagation::mergeEdgeConstraints(EdgeConstraints *fromEdge, EdgeConstraints *toEdge)
{
	// Merge (i.e. logical OR) the "from" list into the "to" list.
...
void
OMR::ValuePropagation::mergeConstraintIntoEdge(ValueConstraint *constraint, EdgeConstraints *edge)
{
...
OMR::ValuePropagation::GlobalConstraint::create(TR::Compilation *c, int32_t valueNumber)
{
...
OMR::ValuePropagation::EdgeConstraints::create(TR::Compilation *c, TR::CFGEdge *edge)
{
...
OMR::ValuePropagation::EdgeConstraints *
OMR::ValuePropagation::createEdgeConstraints(TR::CFGEdge *edge, bool keepBlockList)
{
	if (!_isGlobalPropagation)
		return NULL;
...
void
OMR::ValuePropagation::removeConstraint(int32_t valueNumber, ValueConstraints &valueConstraints, int32_t relative)
{
...
OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreRelationship(TR_LinkHead<StoreRelationship> &list, TR::Symbol *symbol)
{
...
OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findConstraintInList(TR_LinkHead<Relationship> &list, int32_t relative)
{
...
OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findValueConstraint(int32_t valueNumber, ValueConstraints &valueConstraints, int32_t relative)
{
	// Find the constraint for the given value number in the given value
	// constraint list.
	//
...
OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findEdgeConstraint(int32_t valueNumber, EdgeConstraints *edge, int32_t relative)
{
...
OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreConstraint(int32_t valueNumber, TR::Symbol *symbol)
{
	return findStoreValueConstraint(valueNumber, symbol, _curConstraints);
}

OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreEdgeConstraint(int32_t valueNumber, TR::Symbol *symbol, EdgeConstraints *edge)
{
	return findStoreValueConstraint(valueNumber, symbol, edge->valueConstraints);
}

OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findGlobalConstraint(int32_t valueNumber, int32_t relative)
{
	GlobalConstraint *gc = findGlobalConstraint(valueNumber);
	if (!gc)
		return NULL;
...
TR::VPConstraint *
OMR::ValuePropagation::applyGlobalConstraints(
        TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint, int32_t relative)
{
...
OMR::ValuePropagation::getStoreConstraint(TR::Node *node, TR::Node *relative)
{
	...
	if (rel) {
		if (trace()) {
			traceMsg(
			        comp(), "   %s [%p] has existing store constraint:", node->getOpCode().getName(), node);
			rel->print(this, valueNumber, 1);
		}
...
bool
OMR::ValuePropagation::hasBeenStored(int32_t valueNumber, TR::Symbol *symbol, ValueConstraints &valueConstraints)
{
...
void
OMR::ValuePropagation::checkTypeRelationship(
...
void
OMR::ValuePropagation::invalidateParmConstraintsIfNeeded(TR::Node *node, TR::VPConstraint *constraint)
{
	if (_isGlobalPropagation)
	...
	if (symRef && symRef->getSymbol()->getParmSymbol()) {
		...
		if (parmConstraint) {
			...
			if (!result) // there is some incompatibility
			{
				if (trace())
					traceMsg(comp(),
					        "   Store node %p to parm %d is not compatible with rhs, invalidating "
					        "_parms entry %p\n",
...
TR::VPConstraint *
OMR::ValuePropagation::mergeDefConstraints(TR::Node *node, int32_t relative, bool &isGlobal, bool forceMerge)
{
	...
	if (_defMergedNodes->get(node->getGlobalIndex())) {
		if (trace())
			traceMsg(comp(),
			        "Node n%dn has already been processed by mergeDefConstraints - returning NULL\n",
			        node->getGlobalIndex());
		return NULL;
	}
	...
	if (!forceMerge && defNode) {
		if (trace())
			traceMsg(comp(), "   %s [%p] has singleDefiningLoad [%p] ", node->getOpCode().getName(), node,
			        defNode);
	...
	for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
		...
		if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
			if (trace())
				traceMsg(comp(),
				        "   %s [%p] looking at method entry def point: ", node->getOpCode().getName(),
				        node);
			...
			// before we can look at def nodes to build up a set of contraints we had better know that set
			// of defs dominates the use we are considering - if not we treat the use as an unknown
			...
			if (trace())
				traceMsg(comp(),
				        "   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(),
				        node, getValueNumber(defNode), defNode->getOpCode().getName(), defNode);

			if (defNode->getOpCode().isStore()) {
				// TODO : This can be improved to return a value by checking which byte(s) are written
				// to and taking the rhs and applying it properly to get the correct value for
				// (potentially wider) datatype of the use
				//
				...
				if (matchingDef) {
					...
					if (defValueConstraint) {
						...
						if (!isDefInUnreachableBlock(defIndex)) {
							...
							if (trace())
								traceMsg(comp(),
								        "def is unreachable (defInUnreachableBlock), "
								        "ignored\n");
							continue;
						}
#if 0
						...
						if (!symRef->isUnresolved()
							...
							if ((method->getRecognizedMethod()
								...
								if ((thisChild == node)
									...
									if (thisChild != node) {
										uint16_t thisUseDefIndex =
										        thisChild->getUseDefIndex();
										if (!thisUseDefIndex
										        || !_useDefInfo->isUseIndex(
										                thisUseDefIndex))
											avoidMerge = true;

										TR_UseDefInfo::BitVector thisDefs(
										        comp()->allocator());
										if (!_useDefInfo->getUseDef(
										            thisDefs, thisUseDefIndex)
										        || (!(thisDefs == defs)))
											avoidMerge = true;
									}
	...
	if (relative != AbsoluteConstraint) {
		if (trace())
			traceMsg(comp(),
			        "we are processing a relative constraint and merging with backedge constraints is not "
			        "supported\n");
		return NULL;
	}
	...
	for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
		...
		if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound
			...
			if (trace())
				traceMsg(comp(),
				        "symref %d is not defined on all paths - consulting backedge constraints\n",
			...
			for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent) {
				...
				if (!hasBeenStored(
				            defValueNumber, sym, loopInfo->_backEdgeConstraints->valueConstraints)) {
					if (trace())
				...
				rel = NULL;
				defValueConstraint =
				        findStoreEdgeConstraint(defValueNumber, sym, loopInfo->_backEdgeConstraints);
				// if (defValueConstraint)
				...
				if (mergeConstraint) {
					...
					if (constraint) {
						//
						// If its an induction var, check the direction of increment and do not
						// change the bound in the direction opposite to the increment
						//
							...
							if (ivInfo->_symbol->getType().isInt32()) {
								if (!defConstraint->isUnsigned()) {
									if (ivInfo->_increment->asIntConst()) {
										...
										if ((ivInfo->_increment->getLowInt()
										            > 0)
										        && (ivInfo->_increment
										                        ->getHighInt()
										                > 0))
											positiveIncrement = true;
										else if ((ivInfo->_increment
										                         ->getLowInt()
										                 < 0)
										        && (ivInfo->_increment
										                        ->getHighInt()
								...
								if (!defConstraint->isUnsigned()) {
									if (ivInfo->_increment->asLongConst()) {
										if (ivInfo->_increment->getLowLong()
										        > 0)
											positiveIncrement = true;
										else
											negativeIncrement = true;
								...
								if (!defConstraint->isUnsigned()) {
									if (ivInfo->_increment->asShortConst()) {
										if (ivInfo->_increment->getLowShort()
										        > 0)
											positiveIncrement = true;
										else
											negativeIncrement = true;
							...
							if (positiveIncrement) {
								if (defConstraint->isUnsigned()) {
									if ((uint64_t)defConstraint->getHighLong()
										...
										if ((uint64_t)defConstraint
										                        ->getLowLong()
										                < (uint64_t)constraint
										                          ->getLowLong()
										        && (uint64_t)constraint
										                        ->getLowLong()
										                <= (uint64_t)defConstraint
										                           ->getHighLong()) {
											TR::VPConstraint *newDefConstraint =
...
void
OMR::ValuePropagation::replaceByConstant(TR::Node *node, TR::VPConstraint *constraint, bool isGlobal)
{
	if (isGlobal)
	...
	case TR::Address:
		...
		break;
	default: TR_ASSERT(0, "Bad type for convert to constant"); break;
	}
...
OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node *node)
{
		...
		if (!_useDefInfo->getUseDef(defs, useIndex))
			...
			for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
				...
				if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
					...
					if (trace())
						traceMsg(comp(),
						        "      ivInfo is invalid for sym [%p] as method entry is def "
						        "point\n",
						        sym);
					break;
				}
				...
				if (defNode->getOpCode().isStore()) {
					...
					if (defTree && comp()->getFlowGraph()->getStructure()) {
						if (defTree->getEnclosingBlock()->getStructureOf()->getContainingLoop()
						        != _loopInfo->_loop)
							notInLoop = true;
					}
				}
			...
			for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
				...
				if (entryVN >= 0) {
					...
					if (_loopInfo->_seenDefs->get(defVN)) {
						...
						if (trace())
							traceMsg(comp(),
							        "      ivInfo is invalid for sym [%p], def [%d] "
				...
				if ((defVN != entryVN) && notInLoop) {
					...
					if (trace())
						traceMsg(comp(),
						        "      ivInfo is invalid for sym [%p], def [%d] node %p is "
						        "likely in some inner loop\n",
						        sym, defVN, defNode);
					break;
				}
			}
	...
	if (iv) {
		...
		if (maybeInductionVariable) {
			if (checkLoopTestBlock(sym)) {
				...
				traceMsg(comp(),
				        "      Could not guess the increment for sym [%p], not marked as induction "
				        "variable\n",
				        sym);
		}
	...
	//
	if (iv->_entryDef) {
		// select the right type of constraint for induction variables to prevent merging different types of
		// constraints together
		if (node->getOpCode().isLong())
			addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
			        TR::VPLongConst::create(this, incrementVN), &_curConstraints, true);
		else if (node->getOpCode().isShort())
			addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
			        TR::VPShortConst::create(this, incrementVN), &_curConstraints, true);
		else
			addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
			        TR::VPIntConst::create(this, incrementVN), &_curConstraints, true);
	}
}
...
OMR::ValuePropagation::checkLoopTestBlock(TR::Symbol *sym)
{
	...
	if (!isWellFormedLoop) {
		...
		for (TR_StructureSubGraphNode *sgNode = snIt.getCurrent(); sgNode && (branchNode == NULL);
			...
			for (auto edge = sgNode->getSuccessors().begin(); edge != sgNode->getSuccessors().end();
			        ++edge) {
...
OMR::ValuePropagation::checkBackEdgeCoverage()
{
	...
	for (iv = _loopInfo->_inductionVariables.getFirst(); iv; iv = next) {
		...
		if (iv->_entryDef) {
			...
			if (rel) {
				TR_ASSERT(((rel->constraint->asIntConst()
				                   && rel->constraint->asIntConst()->getInt() == iv->_incrementVN)
				                  || (rel->constraint->asLongConst()
				                          && rel->constraint->asLongConst()->getLong()
				                                  == iv->_incrementVN)
				                  || (rel->constraint->asShortConst()
				                          && rel->constraint->asShortConst()->getShort()
				                                  == iv->_incrementVN)),
	...
	while ((ivInfo = _loopInfo->_inductionVariables.pop())) {
		...
		if (ivInfo->_symbol->getType().isInt32()) {
			...
			if (ivInfo->_increment->asShortConst())
				...
				incr = new (trHeapMemory()) TR::VPShortRange(
				        ivInfo->_increment->getLowShort(), ivInfo->_increment->getHighShort());
			if (exitConstraint) {
		...
		TR_InductionVariable *iv = new (trHeapMemory()) TR_InductionVariable(
		        ivInfo->_symbol->castToRegisterMappedSymbol(), entry, exit, incr, isSigned);
...
TR::GlobalValuePropagation::perform()
{
	...
	if (skipBlocksThatCannotReachNormalPaths) {
		...
		TR_CanReachGivenBlocks(comp(), _blocksToProcess).perform(_blocksToProcess); // Bitvector is changed
		                                                                            // in-place
	}
...
void
TR::GlobalValuePropagation::propagateOutputConstraints(TR_StructureSubGraphNode *node, bool lastTimeThrough,
...
void
TR::GlobalValuePropagation::processStructure(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
...
void
TR::GlobalValuePropagation::processAcyclicRegion(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
	if (trace())
...
void
TR::GlobalValuePropagation::processNaturalLoop(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
	TR_RegionStructure *region = node->getStructure()->asRegion();
			...
			if ((*itr)->getTo()->getNumber() == edge->getTo()->getNumber()) {
				...
				if (parentEdgeDefinedOnAllPaths != NULL) {
					*parentEdgeDefinedOnAllPaths &= (*(*_definedOnAllPaths)[edge]);
					...
					parentEdgeDefinedOnAllPaths = new (trStackMemory())
					        TR_BitVector(0, trMemory(), stackAlloc);
...
void
TR::GlobalValuePropagation::processImproperLoop(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
	if (trace())
		printStructureInfo(node->getStructure(), true, lastTimeThrough);
...
void
OMR::ValuePropagation::generalizeStores(ValueConstraints &stores, ValueConstraints *vC)
{
...
void
OMR::ValuePropagation::findStoresInBlock(TR::Block *block, ValueConstraints &stores)
{
	...
	for (treeTop = block->getEntry(); treeTop && treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
		...
		if (node->getOpCode().isStore()) {
			...
			for (rel = vc->storeRelationships.getFirst(), prev = NULL;
			        rel && rel->symbol < node->getSymbol(); prev = rel, rel = rel->getNext()) {
			}
...
void
TR::GlobalValuePropagation::getImproperRegionStores(TR_StructureSubGraphNode *node, ValueConstraints &stores)
{
	TR_RegionStructure *region = node->getStructure()->asRegion();
	if (region) {
...
void
TR::GlobalValuePropagation::processRegionNode(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
...
TR_BitVector *
TR::GlobalValuePropagation::mergeDefinedOnAllPaths(TR_StructureSubGraphNode *node)
{
...
void
TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
	TR::CFGEdge *edge;
	...
	if (_blocksToProcess && !_blocksToProcess->isSet(block->getNumber())) {
		if (performTransformation(
		            comp(), "%sSkip block_%d deemed unimportant\n", OPT_DETAILS, block->getNumber()))
			return;
	}
	...
	// Create temporary List structures to pass to this function. Will be eliminated once getExitEdges has migrated
	// to STL
...
void
OMR::ValuePropagation::printStructureInfo(TR_Structure *s, bool starting, bool lastTimeThrough)
{
...
void
OMR::ValuePropagation::Relationship::print(OMR::ValuePropagation *vp, int32_t valueNumber, int32_t indent)
{
	if (vp->comp()->getOutFile() == NULL)
		return;
...
void
OMR::ValuePropagation::StoreRelationship::print(OMR::ValuePropagation *vp, int32_t valueNumber, int32_t indent)
{
	if (vp->comp()->getOutFile() == NULL)
		return;
...
void
OMR::ValuePropagation::ValueConstraint::print(OMR::ValuePropagation *vp, int32_t indent)
{
	if (vp->comp()->getOutFile() == NULL)
		return;
	for (Relationship *rel = relationships.getFirst(); rel; rel = rel->getNext())
...
bool
OMR::ValuePropagation::removeConstraints(int32_t valueNumber, ValueConstraints *vc, bool findStores)
{
	if (trace()) {
...
//
bool
OMR::ValuePropagation::removeConstraints(int32_t valueNumber, ValueConstraints *valueConstraints)
{
	if (trace())
		traceMsg(comp(), "   Intersection of constraints failed for valueNumber [%d], removing constraints\n",
		        valueNumber);
...
bool
OMR::ValuePropagation::removeStoreConstraints(ValueConstraints *valueConstraints, int32_t valueNumber, int32_t relative)
{
	ValueConstraint *vc = _vcHandler.find(valueNumber, *valueConstraints);
	if (vc) {
...
TR::TreeTop *
TR::ArraycopyTransformation::createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
...
TR::TreeTop *
TR::ArraycopyTransformation::createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef, TR::Node *len, TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef,
        bool isForward)
{
	TR::Node *root = tree->getNode()->getFirstChild();
	TR::Node *src = NULL;
...
int64_t
TR::ArraycopyTransformation::arraycopyHighFrequencySpecificLength(TR::Node *arrayCopyNode)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::TreeTop *
TR::ArraycopyTransformation::createRangeCompareNode(
...
TR::TreeTop *
TR::ArraycopyTransformation::createMultipleArrayNodes(TR::TreeTop *arrayTree, TR::Node *node)
{
	...
	if (!alreadyForwardArrayCopy) {
		...
		cfg->removeEdge(
		        outerElseBlock->getSuccessors(), outerElseBlock->getNumber(), followOnBlock->getNumber());
	...
	if (specificLength >= 0)
		specializeForLength(outerArraycopyTree, node, (uintptrj_t)specificLength, srcRef, dstRef, lenRef,
		        srcObjRef, dstObjRef);
...
TR::TreeTop *
TR::ArraycopyTransformation::tryToSpecializeForLength(TR::TreeTop *tt, TR::Node *arraycopyNode)
{
	...
	if (!arraycopyNode->isForwardArrayCopy())
		return tt; // can only specialize forward arraycopies because the rare path flag is
		           // "isRarePathForwardArrayCopy"

	int64_t specificLength = arraycopyHighFrequencySpecificLength(arraycopyNode); // TODO: What about 64-bit lengths
	                                                                              // on 64-bit platforms?
	if (specificLength >= 0
...
TR::TreeTop *
TR::ArraycopyTransformation::specializeForLength(TR::TreeTop *tt, TR::Node *arraycopyNode, uintptrj_t lengthInBytes,
        ...
        TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef)
{
	...
	requestOpt(OMR::basicBlockOrdering); // Block frequency is reset, so run order blocks optimization to reorder
...
OMR::ValuePropagation::createNewBlockInfoForVersioning(TR::Block *block)
{
	...
	if (!block->isCatchBlock() && !_bndChecks->isEmpty() && !_bndChecks->isSingleton()) {
		...
		if (prepareForBlockVersion(&arrayLengthsLocal)) {
			TR_LinkHead<ArrayLengthToVersion> *arrayLengths = new (trStackMemory())
			...
			for (fromTT = fromTT->getNextTreeTop(); fromTT != tempBlock->getExit();
				...
				if (fromTT->getNode()->getOpCode().isBranch())
					fromTT->getNode()->setBranchDestination(
					        cloner.getToBlock(fromTT->getNode()
		...
		for (auto edge = lastBlock->getSuccessors().begin(); edge != lastBlock->getSuccessors().end(); ++edge) {
				...
				if (!((lastNode->getBranchDestination() == succ->getEntry())
				            || (succ->getEntry()
				                    && (lastNode->getBranchDestination()
				                            == cloner.getToBlock(
				                                             succ->getEntry()->getNode()->getBlock())
				                                       ->getEntry()))))
					fallsThrough = true;
			}
			...
			if (!fallsThrough) {
				...
				TR::Block *newGotoBlock =
				        TR::Block::createEmptyBlock(lastNode, comp(), succ->getFrequency(), succ);
				if (trace())
			...
			if (comp()->useCompressedPointers()) {
				for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
					...
					if (shouldBeCompressed) {
						TR::TreeTop *translateTT = TR::TreeTop::create(comp(),
						        TR::Node::createCompressedRefsAnchor(objectRef), NULL, NULL);
...
bool
OMR::ValuePropagation::prepareForBlockVersion(TR_LinkHead<ArrayLengthToVersion> *arrayLengths)
{
	bool isGlobal;
	...
	for (TR::Node *bndchkNode = iter.getFirst(); bndchkNode; bndchkNode = iter.getNext()) {
		...
		if (!arrayLen->getOpCode().isLoadDirect()) {
			...
			if (arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aloadi) {
				...
				int32_t len;
				char *sig = symRefField->getOwningMethod(comp())->classNameOfFieldOrStatic(
		...
		if (bndchkNode->getSecondChild()->getOpCode().isLoadConst()) {
			...
			// not a const
			static const char *disableExtendedBoundCheckRemoval = feGetEnv("TR_"
			                                                               "DisableExtendedBoundCheckRemova"
			                                                               "l");
			...
			if (index) {
				// check if invariant
				TR::SymbolReference *symRef =
				        index->getOpCode().hasSymbolReference() ? index->getSymbolReference() : 0;
				if (symRef) {
					...
					if (baseNode) // invarinat or trackableNonInvariant
					{
						...
						if (bndchkNode->getSecondChild() == baseNode)
							...
							if (getConstraint(
								...
								c = getConstraint(bndchkNode->getSecondChild(),
								        isGlobal, baseNode)
								            ->asEqual()
								            ->increment();
							else
		...
		if (arrayIndexInfoFound) {
			...
			if (baseNode) {
				if (arrayIndexInfoFound->_max == arrayIndexInfoFound->_min) {
					...
					// The following tests, using c and max (distance from 0) guarantees delta
					// positive.
...
void
OMR::ValuePropagation::buildBoundCheckComparisonNodes(BlockVersionInfo *blockInfo, List<TR::Node> *comparisonNodes)
{
	...
	for (ArrayLengthToVersion *arrayLength = blockInfo->_arrayLengths->getFirst(); arrayLength;
		...
		for (ArrayIndexInfo *arrayIndex = arrayLength->_arrayIndicesInfo->getFirst(); arrayIndex;
			...
			if (performTransformation(comp(),
			            "%s Creating tests outside block_%d for versioning arraylenth %p \n", OPT_DETAILS,
		...
		if (arrayLengthVersioned) {
			if (arrayLength->_arrayLen->getOpCode().isArrayLength()) {
				...
				if (objectRefChild && !objectRefChild->isNonNull()) {
					dumpOptDetails(comp(),
					        "%s Creating test for nullCheck of %p outside block_%d for versioning "
					        "arraylenth %p \n",
					        OPT_DETAILS, objectRefChild, blockInfo->_block->getNumber(),
					        arrayLength->_arrayLen);
					TR::Node *nullTestNode = TR::Node::createif(TR::ifacmpeq,
					        objectRefChild->duplicateTree(), TR::Node::aconst(objectRefChild, 0));
				...
				if (arrayLength->_instanceOfClass && objectRefChild
					...
					dumpOptDetails(comp(),
					        "%s Creating test for instanceof of %p outside block_%d for versioning "
					        "arraylenth %p \n",
					        OPT_DETAILS, objectRefChild, blockInfo->_block->getNumber(),
					        arrayLength->_arrayLen);
					TR::Node *duplicateClassPtr = TR::Node::createWithSymRef(objectRefChild,
					        TR::loadaddr, 0,
					...
					TR::Node *instanceofNode = TR::Node::createWithSymRef(
					        TR:: instanceof, 2, 2, objectRefChild->duplicateTree(),
					        duplicateClassPtr,
				...
				if (!objectRef->isNonNull()) {
					dumpOptDetails(comp(),
					        ...
					        arrayLength->_arrayLen);
					TR::Node *nullTestNode = TR::Node::createif(TR::ifacmpeq,
					        objectRef->duplicateTree(), TR::Node::aconst(objectRef, 0));
					comparisonNodes->add(nullTestNode);
				}
			}
...
void
OMR::ValuePropagation::removeBndChecksFromFastVersion(BlockVersionInfo *blockInfo)
{
...
TR::Node *
OMR::ValuePropagation::findVarOfSimpleFormOld(TR::Node *node) // ArrayIndexNodeFromOffset(TR::Node *node, int32_t
                                                              // stride)
{
...
TR::Node *
OMR::ValuePropagation::findVarOfSimpleForm(TR::Node *node) // ArrayIndexNodeFromOffset(TR::Node *node, int32_t stride)
{
	if (node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference()
	        && ((node->getOpCode().getOpCodeValue() == TR::iload)
	                || ((node->getOpCode().getOpCodeValue() == TR::iloadi)
...
void
OMR::ValuePropagation::createNewBucketForArrayIndex(ArrayLengthToVersion *array,
			...
			if (opCode.isStore()) {
				...
				if (opCode.getOpCodeValue() == TR::istore) {
					...
					if (load
						...
						if (!firstLoadWasSeen) {
							FirstLoadOfNonInvariant *newFirstLoad = new (trStackMemory())
							        FirstLoadOfNonInvariant;
...
TR::TreeTop *
OMR::ValuePropagation::createReferenceArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
...
void
OMR::ValuePropagation::transformReferenceArrayCopyWithoutCreatingStoreTrees(TR_TreeTopWrtBarFlag *arrayTree,
        TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef, TR::SymbolReference *lenRef)
{
	...
	if ((arrayTree->_flag & NEED_WRITE_BARRIER) == 0)
		...
		createReferenceArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcObjRef,
...
void
OMR::ValuePropagation::transformReferenceArrayCopy(TR_TreeTopWrtBarFlag *arrayTree)
{
	TR::Node *node = arrayTree->_treetop->getNode();
	if (node->getOpCodeValue() != TR::arraycopy)
		node = node->getFirstChild();
...
void
OMR::ValuePropagation::transformUnknownTypeArrayCopy(TR_TreeTopWrtBarFlag *arrayTree)
{
#ifdef J9_PROJECT_SPECIFIC
	TR::Node *node = arrayTree->_treetop->getNode();
	if (node->getOpCodeValue() != TR::arraycopy)
	...
	// TR_ASSERT(dstRef, "No destination address ref\n");
	createPrimitiveArrayNodeWithoutFlags(
	        arrayTree->_treetop, arraycopyPrimitive, srcRef, dstRef, lenRef, true, false);
	...
	if ((arrayTree->_flag & NEED_ARRAYSTORE_CHECK) == 0)
		...
		transformReferenceArrayCopyWithoutCreatingStoreTrees(
		        new (trStackMemory()) TR_TreeTopWrtBarFlag(arraycopyReference, arrayTree->_flag), srcObjRef,
		        dstObjRef, srcRef, dstRef, lenRef);
#endif
}

static void
changeBranchToGoto(OMR::ValuePropagation *vp, TR::Node *guardNode, TR::Block *guard)
{
...
bool
OMR::ValuePropagation::checkAllUnsafeReferences(TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return true;
...
OMR::ValuePropagation::doDelayedTransformations()
{
	...
	if (_edgesToBeRemoved) {
		for (i = _edgesToBeRemoved->size() - 1; i >= 0; --i) {
			...
			// NB: this following transformation is not conditional - it must be done otherwise the CFG
			// could be incorrect (for example, if a conditional branch was converted to a goto, you have to
			// remove the extra edge in the CFG or else madness will ensue.
			removedEdgeSources.push_back(toBlock(edge->getFrom()));
			if (std::find(edge->getTo()->getPredecessors().begin(), edge->getTo()->getPredecessors().end(),
			            edge)
					...
					if (!guardForDifferentCall) {
						...
						if (!guard) {
							...
							if (ci->_block->getPredecessors().size() == 1)
								foldedGuard = ci->_block->getPredecessors()
								                      .front()
								                      ->getFrom()
								                      ->asBlock();
						...
						if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()) {
							...
							if (guardNode && guardNode->isProfiledGuard()) {
								...
								if (classTypeNode->getOpCodeValue() == TR::aconst
									...
									TR_YesNoMaybe isVPClassDerivedFromGuardClass =
									        TR_no;
									TR_YesNoMaybe isGuardClassDerivedFromVPClass =
									        TR_yes;
									if (ci->_thisType
										...
										isVPClassDerivedFromGuardClass =
										        comp()->fe()->isInstanceOf(
										                ci->_thisType,
										                typeClass, true);
										if (isVPClassDerivedFromGuardClass
										        == TR_no)
									...
									if ((result == TR_no)
										...
										typeCompatibleStatus =
										        TR_no; // types incompatible
									} else if ((result == TR_yes)
									        && (ci->_thisType == typeClass)
									        && canBeRemoved) {
										typeCompatibleStatus =
										        TR_yes; // types compatible
									} else
										callUnreachable = false;

									if (trace())
										traceMsg(comp(),
										        "typeCompatibleStatus [%p] "
										        "%d\n",
										        guardNode,
										        typeCompatibleStatus);
								} else {
									TR_ASSERT(
									        (classTypeNode->getOpCodeValue()
									                        == TR::aconst
									                && classTypeNode
									                           ->isMethodPointerConstant()),
									        "Devirtualization of guarded profiled "
									        "calls only works currently for "
									        "vft-guards or method compare guards");
									void *profiledMethod =
									        (void *)classTypeNode->getAddress();
									void *devirtualizedMethod =
									        callNode->getSymbolReference()
									                ->getSymbol()
									                ->castToResolvedMethodSymbol()
									                ->getResolvedMethod()
								...
								if (typeCompatibleStatus == TR_no) {
									...
									if ((guardType == 1)
									        && performTransformation(comp(),
									                "%sChanging branch (guard) %p "
									                "in block_%d to goto due to "
									                "devirtualization\n",
									                OPT_DETAILS, guardNode,
									                guard->getNumber())) {
										// change the if to goto
										changeBranchToGoto(
										        this, guardNode, guard);
										callUnreachable = false;
									} else if ((guardType == 0)
									        && performTransformation(comp(),
									                "%sRemoving branch %p in "
									                "block_%d due to "
									                "devirtualization\n",
									                OPT_DETAILS, guardNode,
									                guard->getNumber())) {
						...
						if (!newInlineCall.inlineCall(ci->_tt, ci->_thisType, true,
							...
							if (!callNode->getSymbolReference()
							        ...
							        || callNode->getSymbolReference()
							                   ...
							                   ->isJITInternalNative()) {
								if (callNode->getOpCode().isCallIndirect()) {
									// printf("XXX Added devirtualized call info in
									// %s for %x\n", comp()->signature(), callNode);
									comp()->findOrCreateDevirtualizedCall(
									        callNode, ci->_thisType);
								}
							}
						}
#endif
	...
	for (ClassInitInfo *cii = _classesToCheckInit.getFirst(); cii; cii = cii->getNext()) {
		...
		if ((origFirst->getOpCodeValue() == TR::aload) && origFirst->getSymbol()->isStaticField()) {
			...
			if ((staticName && (staticNameLen > 0)
			            && (!strncmp(staticName, "com/ibm/websphere/ras/TraceComponent.fineTracingEnabled",
			                        55)
			                    || !strncmp(
			                            staticName, "com/ibm/ejs/ras/TraceComponent.anyTracingEnabled", 48)
			                    || !strncmp(staticName, "java/lang/String.compressionFlag", 32)))
			        || ((cii->_len == 41)
			                && !strncmp(cii->_sig, "Lcom/ibm/websphere/ras/TraceEnabledToken;", cii->_len))
			        || ((cii->_len == 35)
			                && !strncmp(cii->_sig, "Lcom/ibm/ejs/ras/TraceEnabledToken;", cii->_len))
			        || ((cii->_len == 40)
			                && !strncmp(
			                        cii->_sig, "Ljava/lang/String$StringCompressionFlag;", cii->_len))) {
		...
		TR::Node *guard =
		        comp()->createSideEffectGuard(comp(), block->getEntry()->getNode(), splitBlock2->getEntry());
...
void
constrainRangeByPrecision(const int64_t low, const int64_t high, const int32_t precision, int64_t &lowResult,
...
void
OMR::ValuePropagation::ValueConstraintHandler::empty(ValueConstraints &valueConstraints)
{
	ValueConstraint *vc;
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::copyAll(ValueConstraints &valueConstraints)
{
...
void
OMR::ValuePropagation::ValueConstraintHandler::setRoot(ValueConstraints &list, ValueConstraint *vc)
{
	list.setFirst(vc);
}

OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::find(int32_t key, ValueConstraints &list)
{
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::findOrCreate(int32_t key, ValueConstraints &list)
{
	ValueConstraint *prev, *cur;
	for (cur = list.getFirst(), prev = NULL; cur; prev = cur, cur = cur->getNext()) {
		if (cur->getValueNumber() < key)
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::remove(int32_t key, ValueConstraints &list)
{
	ValueConstraint *prev, *cur;
	for (cur = list.getFirst(), prev = NULL; cur; prev = cur, cur = cur->getNext()) {
		if (cur->getValueNumber() < key)
