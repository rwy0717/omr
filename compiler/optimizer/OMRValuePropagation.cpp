        ...
        if (!(arrayLen->getOpCode().isLoadConst()
                || (arrayLen->getOpCode().isArrayLength()
                       && (((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aloadi)
                               && ((arrayLen->getFirstChild()->getFirstChild()->getOpCode().getOpCodeValue()
                                       == TR::aload)
...
void OMR::ValuePropagation::transformStringConcats(VPStringCached* stringCached)
{
    if (!performTransformation(comp(), "%sSimplified String Concatenation:(StringCache) [%p] \n", OPT_DETAILS,
            stringCached->_treetop1, stringCached->_treetop2)
        && getStringCacheRef())
        return;
    ...
    TR::SymbolReference* symRef = getStringCacheRef()
        ? comp()->getSymRefTab()->findOrCreateMethodSymbol(stringNode->getSymbolReference()->getOwningMethodIndex(), -1,
              getStringCacheRef()->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
...
void OMR::ValuePropagation::transformStringCtors(VPTreeTopPair* treeTopPair)
{
    ...
    if (!performTransformation(comp(), "%sSimplified String Concatenation:(StringCache) [%p] \n", OPT_DETAILS,
            treeTopPair->_treetop1, treeTopPair->_treetop2)
        && getStringCacheRef())
        return;
    ...
    if (!(base1 && base1->isConstString() && base2 && base2->isConstString())) {
        traceMsg(
            comp(), "%p, %p not Constant Strings, returning from StringCtors", appendedString[0], appendedString[1]);
        return;
    }
    ...
    /* Before									After
      ...
      treetop2								treetop2
          ...
          new										treetop
          loadaddr								acall
      Method[java/lang/String.cachedConstantString] aload aload iconst
    */
    ...
    TR::SymbolReference* symRef = getStringCacheRef()
        ? comp()->getSymRefTab()->findOrCreateMethodSymbol(
              treeTopPair->_treetop2->getNode()->getFirstChild()->getSymbolReference()->getOwningMethodIndex(), -1,
              getStringCacheRef()->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(),
              TR::MethodSymbol::Static)
        : 0;
