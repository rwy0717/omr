    ...
    TR_VirtualGuardSelection* guard)
{
    ...
    if (guard->_type == TR_VftTest) {
        ...
        return TR_VirtualGuard::createRubyInlineGuard(
            guard->_kind, comp(), calleeIndex, callNode, destination, guard->_thisClass);
    ...
    TR_VirtualGuardSelection* guard, TR::TreeTop** induceOSRCallTree, TR_CallTarget* calltarget)
{
    ...
    if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard)
        && callNode->getSymbolReference()->getOwningMethodSymbol(comp())->supportsInduceOSR(
        ...
        if (shouldUseOSR
            ...
            // Late inlining may result in callerSymbol not being the resolved method that actually calls the inlined
            // method This is problematic for linking OSR blocks
...
TR_ParameterToArgumentMapper::TR_ParameterToArgumentMapper(TR::ResolvedMethodSymbol* callerSymbol,
    TR::ResolvedMethodSymbol* calleeSymbol, TR::Node* callNode, List<TR::SymbolReference>& temps,
...
void TR_ParameterToArgumentMapper::printMapping()
{
    ...
    for (TR_ParameterMapping* pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
        debugTrace(tracer(),
            ...
            pm->_replacementSymRef2 ? pm->_replacementSymRef2->getReferenceNumber() : -1,
            pm->_replacementSymRef3 ? pm->_replacementSymRef3->getReferenceNumber() : -1, pm->_argIndex,
...
void TR_HandleInjectedBasicBlock::printNodesWithMultipleReferences()
{
    ...
    for (MultiplyReferencedNode* mn = _multiplyReferencedNodes.getFirst(); mn; mn = mn->getNext()) {
        debugTrace(tracer(),
            ...
            mn->_replacementSymRef2 ? mn->_replacementSymRef2->getReferenceNumber() : -1,
            mn->_replacementSymRef3 ? mn->_replacementSymRef3->getReferenceNumber() : -1, mn->_referencesToBeFound,
            mn->_isConst, mn->_symbolCanBeReloaded);
    }
}
...
void TR_HandleInjectedBasicBlock::replaceNodesReferencedFromAbove(TR::Block* block, vcount_t visitCount)
{
...
TR_HandleInjectedBasicBlock::MultiplyReferencedNode::MultiplyReferencedNode(
    ...
    , _replacementSymRef2(0)
    , _replacementSymRef3(0)
    , _isConst(false)
{}
...
bool TR_InlinerBase::callMustBeInlinedRegardlessOfSize(TR_CallSite* callsite) { return false; }
    ...
    TR_CallStack* callStack, TR_CallTarget* calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t)
{
    ...
    TR_ParameterToArgumentMapper pam(
        callerSymbol, calleeSymbol, callNode, tempList, _availableTemps, _availableBasicBlockTemps, this);
    ...
    */
    ...
    if (guard->_kind != TR_NoGuard) {
        ...
        TR::Block* blockOfCallerInCalleeCFG
...
TR_CallSite::TR_CallSite(TR_ResolvedMethod* callerResolvedMethod, TR::TreeTop* callNodeTreeTop, TR::Node* parent,
    TR::Node* callNode, TR_Method* interfaceMethod, TR_OpaqueClassBlock* receiverClass, int32_t vftSlot,
