    ...
    TR::Node* callNode, TR::Node* replacementNode, rcount_t& numberOfReferencesToFind)
{
    ...
    //
    if ((_inliningAsWeWalk && node->getOpCode().isCall() && (node->getVisitCount() == _visitCount)) || (node->getVisitCount() == comp()->getVisitCount()))
        return;
...
void TR_TransformInlinedFunction::transform()
{
    ...
    //
    if (comp()->getOption(TR_EnableJProfiling) || (firstBlock->getPredecessors().size() > 1) || firstBlock->hasExceptionSuccessors() || comp()->fe()->isMethodEnterTracingEnabled(calleeResolvedMethod->getPersistentIdentifier()) || TR::Compiler->vm.canMethodEnterEventBeHooked(comp())) {
