        ...
        TR_ResolvedMethod *feMethod, TR::ResolvedMethodSymbol *methodSymbol, TR::Compilation *comp)
{
	...
	if (!comp->getOption(TR_DisableAdaptiveDumbInliner)) {
		if (methodSymbol && !methodSymbol->mayHaveInlineableCall() && size <= 5) // favor the inlining of
		                                                                         // methods that are very small
			size = 1;
	}
...
TR_TrivialInliner::TR_TrivialInliner(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
TR_InlinerBase::setInlineThresholds(TR::ResolvedMethodSymbol *callerSymbol)
{
	...
	if (comp()->getNodeCount() > _nodeCountThreshold) {
		_nodeCountThreshold = comp()->getNodeCount() * (1.05F); // allow a little bit of inlining anyways to get
		                                                        // smaller methods
	}
	...
	heuristicTrace(tracer(),
	        ...
	        "\n\tMethod size threshold (_methodByteCodeSizeThreshold) = %d \n\tMethod size threshold for warm (and "
	        "above) compiles (_methodInWarmBlockByteCodeSizeThreshold) = %d"
        ...
        TR::Node *callNode, bool allConsts)
{
	...
	if (block) {
		...
		if (allowBiggerMethods()) {
			...
			if (blockFrequency > borderFrequency) {
				...
				if (comp()->trace(OMR::inlining))
					heuristicTrace(tracer(),
					        "exceedsSizeThreshold: Scaled down size for call block_%d from %d to "
					        "%d",
	...
	if (inlineThreshold && (uint32_t)bytecodeSize > inlineThreshold) {
		...
		if (block) {
			TR::DebugCounter::prependDebugCounter(comp(),
			        "inliner.callSites/failed/calleeHasTooManyBytecodes", block->getFirstRealTreeTop());
			TR::DebugCounter::prependDebugCounter(comp(),
	...
	heuristicTrace(tracer(), "### Did not exceed size threshold, bytecodeSize %d <= inlineThreshold %d ",
	        bytecodeSize, inlineThreshold);
...
OMR_InlinerPolicy::tryToInlineGeneral(TR_CallTarget *calltarget, TR_CallStack *callStack, bool toInline)
{
	...
	// consider the global dontInline
	{
		if (!toInline) {
			...
			if (inlineFilters) {
				bool inclusive = comp()->getDebug()->methodSigCanBeFound(signature, inlineFilters,
				        filterInfo, calltarget->_calleeMethod->convertToMethod()->methodType());
...
TR_CallStack::~TR_CallStack()
{
	...
	TR_ASSERT(std::uncaught_exception()
	                || (_autos.isEmpty() && _temps.isEmpty() && _injectedBasicBlockTemps.isEmpty()),
...
TR_CallStack::isAnywhereOnTheStack(TR_ResolvedMethod *method, int32_t occurrences)
{
	...
	for (TR_CallStack *cs = this; cs; cs = cs->getNext()) {
		if (cs->_method->isSameMethod(method) && ++counter == occurrences) {
			...
			for (int16_t callerIndex = cs->_currentCallNode->getByteCodeInfo().getCallerIndex();
			        callerIndex != -1;) {
...
bool
TR_InlineCall::inlineCall(TR::TreeTop *callNodeTreeTop, TR_OpaqueClassBlock *thisClass, bool recursiveInlining,
        TR_PrexArgInfo *argInfo, int32_t initialMaxSize)
{
	...
	for (int32_t i = 0; i < callsite->numTargets(); i++) {
		...
		if (initialMaxSize > 0
			...
			heuristicTrace(tracer(),
			        "Failed at Inlining. getMaxByteCodeIndex of %d exceeds initialMaxSize of %d",
			        getPolicy()->getInitialBytecodeSize(calltarget->_calleeSymbol, comp()), initialMaxSize);
			return false; // nothing to callStack::commit yet
		}
...
bool
TR_DumbInliner::inlineCallTargets(
        TR::ResolvedMethodSymbol *callerSymbol, TR_CallStack *prevCallStack, TR_InnerPreexistenceInfo *innerPrexInfo)
{
		        ...
		        (int32_t)_initialSize
		                - callerSize * (int32_t)_initialSize
		                        / comp()->getOptions()->getDumbInlinerBytecodeSizeCutoff()
		                                 :
		                                 // If I inlined a bigger method, my budget should decrease by a bigger
		                                 // amount
		        prevCallStack->_maxCallSize
		                - callerSize / comp()->getOptions()->getDumbInlinerBytecodeSizeDivisor();
	} else {
	...
	TR_CallStack callStack(
	        comp(), callerSymbol, callerSymbol->getResolvedMethod(), prevCallStack, maxCallSize, true);
	if (innerPrexInfo)
		callStack._innerPrexInfo = innerPrexInfo;
	...
	for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
		...
		if (parent->getOpCodeValue() == TR::BBStart) {
			...
			if (comp()->getMethodHotness() <= cold) {
				veryColdBorderFrequency = comp()->getOption(TR_DisableConservativeColdInlining) ? 0
				                                                                                : 1500;
		...
		if (parent->getNumChildren()) {
			...
			if (node->getOpCode().isCall() && (!node->getOpCode().isJumpWithMultipleTargets())
				...
				if ((isCold
				        ...
				        && !alwaysWorthInlining(
				                node->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod(),
				                node))
	...
	for (int32_t i = 0; i < callsite->numTargets(); i++) {
		...
		if ((byteCodeSize > maxBCSize)) {
			if (tryToInline("overriding getMaxBytecodeIndex check", calltarget)) {
				...
				if (comp()->trace(OMR::inlining))
					traceMsg(comp(),
					        "inliner: overriding getMaxBytecodeIndex check because it's always "
					        "worth inlining\n");
			} else {
				if (comp()->trace(OMR::inlining))
				...
				if (comp()->cg()->traceBCDCodeGen()) {
					traceMsg(comp(),
					        "q^q : failing to inline %s into %s (callNode %p on line_no=%d) due to "
					        "wcode size\n",
					        tracer()->traceSignature(calltarget->_calleeSymbol),
...
static TR::Node *
findPotentialDecompilationPoint(TR::ResolvedMethodSymbol *calleeSymbol, TR::Compilation *comp)
{
 ...
 */
void
TR_InlinerBase::replaceCallNode(TR::ResolvedMethodSymbol *callerSymbol, TR::Node *resultNode,
 ...
 */
void
TR_InlinerBase::replaceCallNodeReferences(TR::Node *node, TR::Node *parent, uint32_t childIndex, TR::Node *callNode,
        TR::Node *replacementNode, rcount_t &numberOfReferencesToFind, TR::NodeChecklist &visitedNodes)
{
	...
	if (!replacedNode) {
		for (int32_t i = 0; i < node->getNumChildren() && numberOfReferencesToFind; ++i)
			replaceCallNodeReferences(node->getChild(i), node, i, callNode, replacementNode,
			        numberOfReferencesToFind, visitedNodes);
	}
}

TR::Node *
TR_InlinerBase::createVirtualGuard(TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol, TR::TreeTop *destination,
        int16_t calleeIndex, TR_OpaqueClassBlock *thisClass, bool favourVftCompareGuard,
        TR_VirtualGuardSelection *guard)
{
	...
	TR::TreeTop *debugCounterInsertionPoint = destination->getNextTreeTop(); // destination should always be a
	                                                                         // BBStart
	...
	case TR_ProfiledGuard:
		if (guard->isHighProbablityProfiledGuard())
			fidelity = TR::DebugCounter::Free; // The taken path is marked isCold, so it had better never
			                                   // happen
		else
			fidelity = TR::DebugCounter::Moderate;
		break;
	case TR_MethodEnterExitGuard: fidelity = TR::DebugCounter::Cheap; break;
	default:
	...
	TR::DebugCounter::prependDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "virtualGuards.byJittedBody/%s/(%s)/(%s)/%s/bcinfo=%d.%d",
	                ...
	                calleeSymbol->signature(trMemory()), tracer()->getGuardKindString(guard),
 ...
 */
void
TR_InlinerBase::rematerializeCallArguments(TR_TransformInlinedFunction &tif, TR_VirtualGuardSelection *guard,
        TR::Node *callNode, TR::Block *block1, TR::TreeTop *rematPoint)
{
	debugTrace(tracer(),
	        "privatizedInlinerArg - consider arguments for rematerialization for guard [%p] of call node [n%dn]",
	...
	bool suitableForRemat = !comp()->getOption(TR_DisableGuardedCallArgumentRemat)
	...
	if (suitableForRemat) {
		...
		for (prevTree = NULL, argStoreTree = tif.getParameterMapper().firstTempTreeTop();
			...
			if (argStore->chkIsPrivatizedInlinerArg()) {
				debugTrace(tracer(), "  considering priv arg store node [%p] - %d - for remat",
				...
				TR_YesNoMaybe result = RematTools::gatherNodesToCheck(comp(), argStore,
				        argStore->getFirstChild(), scanTargets, argSymRefsToCheck,
				        tracer()->debugLevel());
				if (result == TR_yes) {
					debugTrace(tracer(), "    priv arg remat may be possible for node [%p] - %d",
					        argStore, argStore->getGlobalIndex());
		...
		if (failedArgs.size() > 0) {
			RematTools::walkTreeTopsCalculatingRematFailureAlternatives(comp(),
			        ...
			        tif.getParameterMapper().firstTempTreeTop()->getNextTreeTop(), failedArgs, scanTargets,
			        argSafetyInfo, tracer()->debugLevel());
			...
			for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
				...
				TR::DebugCounter::incStaticDebugCounter(comp(),
				        TR::DebugCounter::debugCounterName(comp(),
				                "privatizedInlinerArgs.byMethod/Failed/(%s)/%s", comp()->signature(),
				                argStore->getFirstChild()->getOpCode().getName()));
				TR::DebugCounter::incStaticDebugCounter(comp(),
		...
		if (!scanTargets.IsZero()) {
			RematTools::walkTreesCalculatingRematSafety(comp(), block1->getFirstRealTreeTop(),
			        tif.getParameterMapper().lastTempTreeTop()->getNextTreeTop(), scanTargets,
			        unsafeSymRefs, tracer()->debugLevel());
		}
		...
		for (uint32_t i = 0; i < argSafetyInfo.size(); ++i) {
			...
			if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
				TR::DebugCounter::incStaticDebugCounter(comp(),
				        TR::DebugCounter::debugCounterName(comp(),
				                "privatizedInlinerArgs.byMethod/(%s)/Succeeded", comp()->signature()));
				TR::DebugCounter::incStaticDebugCounter(comp(),
				...
				// otherwise we are doing a partial remat where we are going to load from another temp
					...
					TR::Node *duplicateStore = TR::Node::createStore(argStore->getSymbolReference(),
					        TR::Node::createLoad(
					                argStore, rematTree->getNode()->getSymbolReference()));
					...
					if (performTransformation(comp(),
					            ...
					            rematTree->getNode()->getSymbolReference()->getReferenceNumber())) {
						rematPoint = TR::TreeTop::create(comp(), rematPoint, duplicateStore);
					}
				}
			} else {
				debugTrace(tracer(),
				        " priv arg remat failed for node [%p] - %d - due to data dependencies",
...
void
TR_InlinerBase::addAdditionalGuard(TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
        ...
        TR_VirtualGuardKind kind, TR_VirtualGuardTestType type, bool favourVFTCompare, TR::CFG *callerCFG)
{
	...
	TR::TreeTop *tt = guardBlock->append(TR::TreeTop::create(comp(),
	        createVirtualGuard(callNode, calleeSymbol, slowPath->getEntry(),
	                calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(), thisClass, favourVFTCompare,
	                guard)));
...
TR::TreeTop *
TR_InlinerBase::addGuardForVirtual(TR::ResolvedMethodSymbol *callerSymbol, TR::ResolvedMethodSymbol *calleeSymbol,
        TR::TreeTop *callNodeTreeTop, TR::Node *callNode, TR_OpaqueClassBlock *thisClass,
        TR::TreeTop *startOfInlinedCall, TR::TreeTop *previousBBStartInCaller, TR::TreeTop *nextBBEndInCaller,
        TR_TransformInlinedFunction &tif, List<TR::SymbolReference> &tempList, TR_VirtualGuardSelection *guard,
        TR::TreeTop **induceOSRCallTree, TR_CallTarget *calltarget)
{
	...
	if (guard->_kind != TR_InnerGuard)
		virtualGuard = block1->append(TR::TreeTop::create(comp(),
		        createVirtualGuard(callNode, calleeSymbol, block4->getEntry(),
		                calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(), thisClass,
		                tif.favourVftCompare(), guard)));
	...
	if (!disableFSDGuard && comp()->getOption(TR_FullSpeedDebug) && guard->_kind != TR_BreakpointGuard) {
		addAdditionalGuard(callNode, calleeSymbol, thisClass, block1, block2, block4, TR_BreakpointGuard,
		        TR_FSDTest, false /*favourVftCompare*/, callerCFG);
	}
	...
	if (comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
		...
		while (cursor && comp()->getMethodSymbol()->isOSRRelatedNode(cursor->getNode(), bci)) {
			if (cursor->getNode()->getOpCode().isStoreDirect()
				...
				debugTrace(tracer(),
				        "  virtual call node pps: Pending push store of call [%p] found: [%p]",
			...
			debugTrace(tracer(),
			        "  virtual call node pps: Skipping node [%p] whilst searching for store of call [%p]",
	...
	if (tif.simpleCallReferenceTreeTop())
		cloneAndReplaceCallNodeReference(
		        tif.simpleCallReferenceTreeTop(), 0, 0, block4->getLastRealTreeTop(), comp());
	...
	if (block4->getLastRealTreeTop()->getNode()->getOpCode().isReturn()) {
		...
		lastTreeInBlock4 = block4->append(
		        TR::TreeTop::create(comp(), TR::Node::create(callNode, TR::Goto, 0, block3->getEntry())));
	...
	if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard)
		...
		bool shouldUseOSR =
		        heuristicForUsingOSR(callNode, calleeSymbol, callerSymbol, createdHCRAndVirtualGuard);
		...
		if (shouldUseOSR
			...
			TR::TreeTop *induceTree = callingMethod->genInduceOSRCall(guardedCallNodeTreeTop,
...
bool
TR_InlinerBase::heuristicForUsingOSR(TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
        TR::ResolvedMethodSymbol *callerSymbol, bool isIndirectCall)
{
	...
	static int32_t calleeIndirectThresh = (OSRIndirectThreshold = feGetEnv("TR_OSRIndirectCalleeThreshold"))
	...
	static int32_t callerThresh = (OSRThreshold2 = feGetEnv("TR_OSRCallerThreshold")) ? atoi(OSRThreshold2)
	                                                                                  : 100000;
	...
	while (osrMethodData) {
		...
		int32_t osrCallerNumStackSlots = osrCallerMethod->numberOfParameterSlots()
		        + osrCallerMethod->numberOfTemps() + osrCallerMethod->numberOfPendingPushes();
		...
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "OSR caller at inlined site index %d has %d bytecodes and %d stack slots, total "
			        "callers bytecodes %d total callers stack slots %d total callers live stack slots %d "
			        "total pending push slots %d\n",
			        osrMethodData->getInlinedSiteIndex(), osrCallerSize, osrCallerNumStackSlots,
			        totalOSRCallersSize, totalOSRCallersStackSlots, totalOSRCallersLiveStackSlots,
...
TR_ParameterToArgumentMapper::printMapping()
{
	...
	for (TR_ParameterMapping *pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
		debugTrace(tracer(),
		        "Mapping at addr %p:\n\tparmSymbol = %p (offset %d) \treplacementSymRef = %d\t_parameterNode = "
		        "%p\n"
...
TR_ParameterToArgumentMapper::initialize(TR_CallStack *callStack)
{
	...
	if (_callNode->getOpCode().isCallIndirect()) {
		...
		if (arg->getReferenceCount() > 1) {
			...
			_firstTempTreeTop = _lastTempTreeTop = OMR_InlinerUtil::storeValueInATemp(comp(), arg,
			        _vftReplacementSymRef, 0, _calleeSymbol, _tempList, _availableTemps, &_availableTemps2,
			        false, &newStoreTreeTop);
		...
		if (parmMap->_parmSymbol->getParameterOffset() == argOffset) {
			...
			if (!parmMap->_parmIsModified
				...
				if (parmMap->_addressTaken) {
					...
					debugTrace(tracer(),
					        "Setting parameterNode to n%in %s, argOffset=%d, argIndex=%d, "
					        "_callNode n%in",
						...
						if ((arg->getOpCode().isLoadVarDirect()
						        ...
						        && ((arg->getReferenceCount() == 1)
						                || (_callNode->getOpCode().isCallIndirect()
						                        ...
						                        && (_callNode->getFirstChild()->getNumChildren()
						                                > 0)
						                        && (arg
						                                == _callNode->getFirstChild()
						                                           ->getFirstChild())
			...
			if (parmMap->_isConst) {
				// create treetop for constant arg so that inliner can later store the arg into correct
				// register
				...
				if (parmMap->_replacementSymRef)
					...
					const static bool disableUseKnownObjectTempsForParms = feGetEnv("TR_"
					                                                                "DisableUseKnow"
					                                                                "nObjectTempsFo"
					                                                                "rParmsInCalle"
					                                                                "e")
					...
					tt = OMR_InlinerUtil::storeValueInATemp(comp(), arg, symRef, 0, _calleeSymbol,
					        _tempList, _availableTemps, &_availableTemps2, false,
					...
					if (!hasStaticCallStack && !neverNeedPrivatizedArguments
					        && tt->getNode()->getOpCode().isStoreDirectOrReg()) // compjazz 53912:
					                                                            // PLX sometimes
					                                                            // privatizes using
					                                                            // indirect stores
					{
					...
					if (!disableUseKnownObjectTempsForParms && prexArgument
					        && !parmMap->_parmIsModified
					        && TR_PrexArgument::knowledgeLevel(prexArgument) == KNOWN_OBJECT
					        && prexArgument->isTypeInfoForInlinedBody()) // use known object temp in
					                                                     // inlined body only
					{
						...
						debugTrace(tracer(),
						        "created tree n%dn to store the priv arg #%d into a known "
						        "object temp #%d on entry of the inlined body\n",
		...
		for (; parmMap; parmMap = parmMap->getNext())
			if (symbol == parmMap->_parmSymbol) {
				if (parmMap->_isConst) {
					...
					TR::DebugCounter::incStaticDebugCounter(comp(),
					        TR::DebugCounter::debugCounterName(comp(),
					                "osrCallSiteRemat/mapParm/const/(%s)", comp()->signature()));
				} else if (loadSymRef->getOffset() > 0) {
					comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, NULL);
					TR::DebugCounter::incStaticDebugCounter(comp(),
					        TR::DebugCounter::debugCounterName(comp(),
					                "osrCallSiteRemat/mapParm/addr/(%s)", comp()->signature()));
				} else {
					comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, parmMap->_replacementSymRef);
					TR::DebugCounter::incStaticDebugCounter(comp(),
					        TR::DebugCounter::debugCounterName(comp(),
					                "osrCallSiteRemat/mapParm/success/(%s)", comp()->signature()));
				}
				break;
			}
	...
	if (opcode.hasSymbolReference()) {
		...
		if (symbol->isParm()) {
			...
			if (newNode && newNode != node) {
				if (newNode->getOpCode().isLoadConst() && newNode->getType().isInt32()
				        && node->getType().isInt8()) {
					// do not create bstore/iconst -- work-around for minor type incorrectness in
					// spec2006 hmmer (type mismatch but iconst actually fits in a byte)
					...
					newNode->setReferenceCount(1);
					dumpOptDetails(comp(),
					        "%screate %s (0x%p) to resolve type mismatch between %s (%p) and %s "
					        "(%p)\n",
	...
	if (returnNode->getNumChildren() && _callNode->getReferenceCount() > 1) {
		...
		if (_simpleCallReferenceTreeTop) {
			...
			OMR_InlinerUtil::storeValueInATemp(comp(), returnValue, _resultTempSymRef,
			        _currentTreeTop->getPrevTreeTop(), _callerSymbol, _tempList, _availableTemps,
			        &_availableTemps2);
		else
        ...
        , _mappings(mappings)
{
	...
	if (!_tracer)
		_tracer = new (comp->trHeapMemory()) TR_InlinerTracer(comp, comp->fe(), NULL); // if I wasn't provided a
		                                                                               // tracer, just use a
		                                                                               // dummy one.  No tracing
		                                                                               // will occur.
}
...
TR_HandleInjectedBasicBlock::printNodesWithMultipleReferences()
{
	...
	for (MultiplyReferencedNode *mn = _multiplyReferencedNodes.getFirst(); mn; mn = mn->getNext()) {
		debugTrace(tracer(),
		        ...
		        mn->_replacementSymRef ? mn->_replacementSymRef->getReferenceNumber() : -1,
...
TR_HandleInjectedBasicBlock::createTemps(bool replaceAllReferences)
{
	...
	for (MultiplyReferencedNode *ref = _multiplyReferencedNodes.getFirst(); ref; ref = ref->getNext()) {
		...
		if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
			...
			// The real underlying issue is that just because a symref is on a parmMap, it doesn't mean you
			// can automatically add it to a list of injected basic block temps When I enabled the
			// ParmTempOpt, this is what happened.  An auto symbol got added to the injectedBasicBlockTemps
			// list, and then was reused in a very bad manner.
			if (enabletempCreationOpt) {
				for (TR_ParameterMapping *parmMap = _mappings; parmMap; parmMap = parmMap->getNext()) {
					if (parmMap->_parameterNode == ref->_node) {
						...
						debugTrace(_tracer,
						        "\nadding %d to injected basic block temps for node %p is in "
						        "temp list %d",
			...
			if (!symRef) {
				...
				OMR_InlinerUtil::storeValueInATemp(comp(), value, symRef, tt, _methodSymbol,
				        _injectedBasicBlockTemps, _availableTemps, 0);
			}
...
void
TR_HandleInjectedBasicBlock::replace(
        MultiplyReferencedNode *ref, TR::TreeTop *tt, TR::Node *parent, uint32_t childIndex)
{
	...
	if (ref->_isConst) {
		...
		// Convert the node just loaded back to the original type in case the FE required us to store it as
		// something else
	...
	if (!parent->getChild(childIndex)->getByteCodeInfo().doNotProfile())
		replacementNode->getByteCodeInfo().setDoNotProfile(0); // allow profiling of replacement node if the
		                                                       // original could be profiled
	parent->setChild(childIndex, replacementNode);
}
...
static TR::TreeTop *
findPinningArrayStore(TR::Compilation *comp, TR::TreeTop *prevTT, TR::Node *value)
{
	TR::TreeTop *tt = NULL;
 ...
 */
TR::TreeTop *
OMR_InlinerUtil::storeValueInATemp(TR::Compilation *comp, TR::Node *value, TR::SymbolReference *&tempSymRef,
        TR::TreeTop *prevTreeTop, TR::ResolvedMethodSymbol *methodSymbol, List<TR::SymbolReference> &tempList,
        List<TR::SymbolReference> &availableTemps, List<TR::SymbolReference> *availableTemps2, bool behavesLikeTemp,
        TR::TreeTop **newStoreValueTreeTop, bool isIndirect, int32_t offset)
{
			...
			if (value->getOpCode().isArrayRef()) {
				...
				if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
					...
					if (valueChild->getOpCode().isLoadVarDirect()
						...
						if (!newValueArrayRef->getSymbol()->isParm()) // newValueArrayRef could
						                                              // contain a parm symbol
						{
							pinningArray =
							        newValueArrayRef->getSymbol()->castToAutoSymbol();
							pinningArray->setPinningArrayPointer();
						}
						...
						if (!prevTreeTop) {
							if (!newStoreValueTreeTop)
								TR_ASSERT(0,
								        "no previous treetop and no treetop pointer "
								        "passed in");
	...
	TR_ASSERT(
	        comp->il.opCodeForDirectStore(dataType) != TR::BadILOp, "Inlining, unexpected data type for temporary");
	...
	if (comp->cg()->traceBCDCodeGen())
		traceMsg(comp,
		        "\tcreate storeNode %p of tempSymRef #%d (possibly for node uncommoning during "
		        "opcodeExpansion)\n",
...
static TR::TreeTop *
cloneAndReplaceCallNodeReference(TR::TreeTop *callNodeReference, TR::Node *callNode, TR::Node *replacementNode,
        TR::TreeTop *insertAfterTreeTop, TR::Compilation *comp)
{
...
static void
cloneAndReplaceReturnValueReference(TR::TreeTop *ifCmpTreeTop, TR::TreeTop *rvStoreTreeTop, TR::Block *curBlock,
...
bool
TR_TransformInlinedFunction::findCallNodeInTree(TR::Node *callNode, TR::Node *node)
{
	if (node == callNode)
		return true;
...
bool
TR_TransformInlinedFunction::findReturnValueInTree(TR::Node *rvStoreNode, TR::Node *node, TR::Compilation *comp)
{
...
bool
TR_TransformInlinedFunction::onlyMultiRefNodeIsCallNode(TR::Node *callNode, TR::Node *node)
{
	if (node != callNode) {
		if (node->getReferenceCount() > 1)
			return false;
...
TR::TreeTop *
TR_TransformInlinedFunction::findSimpleCallReference(TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
	// If the call node's only other reference is in the next tree top
...
bool
TR_CallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
...
bool
TR_IndirectCallSite::tryToRefineReceiverClassBasedOnResolvedTypeArgInfo(TR_InlinerBase *inliner)
{
	if (hasResolvedTypeArgInfo()) {
		...
		if (_receiverClass
			...
			heuristicTrace(inliner->tracer(), "refining _receiverClass %p to %p", _receiverClass,
			        classFromArgInfo);
...
bool
TR_IndirectCallSite::findCallTargetUsingArgumentPreexistence(TR_InlinerBase *inliner)
{
...
TR_IndirectCallSite::addTargetIfMethodIsNotOverriden(TR_InlinerBase *inliner)
{
	...
	if (_initialCalleeMethod && !_initialCalleeMethod->virtualMethodIsOverridden()) {
		if (comp()->compileRelocatableCode() && !TR::Options::getCmdLineOptions()->allowRecompilation())
			return false; // CHTable is not used when recompilation is disabled, hence we cannot use
			              // assumptions for AOT
...
bool
TR_IndirectCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
...
bool
TR_DirectCallSite::findCallSiteTarget(TR_CallStack *callStack, TR_InlinerBase *inliner)
{
...
void
TR_InlinerBase::getSymbolAndFindInlineTargets(TR_CallStack *callStack, TR_CallSite *callsite, bool findNewTargets)
{
	...
	if (callsite->_initialCalleeSymbol) {
		...
		if (comp()->fe()->isInlineableNativeMethod(comp(), callsite->_initialCalleeSymbol)) {
			...
			callsite->addTarget(trMemory(), this, guard,
			        callsite->_initialCalleeSymbol->getResolvedMethod(), callsite->_receiverClass);
		}
			...
			if (type
			        && (!callsite->_receiverClass
			                || (type != callsite->_receiverClass
			                        && fe()->isInstanceOf(type, callsite->_receiverClass, true, true)
			                                == TR_yes)))
	...
	for (int32_t i = 0; i < callsite->numTargets(); i++) {
		...
		if (!target->_calleeSymbol
			...
			target->_calleeMethod->setOwningMethod(callsite->_callNode->getSymbolReference()
			                                               ...
			                                               ->getResolvedMethod()); // need to point to the
			                                                                       // owning method to the
			                                                                       // same one that got
			                                                                       // ilgen'd d157939

			target->_calleeSymbol =
			        comp()->getSymRefTab()
			                ->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1,
...
void
TR_InlinerBase::applyPolicyToTargets(TR_CallStack *callStack, TR_CallSite *callsite)
{
	for (int32_t i = 0; i < callsite->numTargets(); i++) {
		TR_CallTarget *calltarget = callsite->getTarget(i);
		...
		if (calltarget->_calleeSymbol
		        && strstr(calltarget->_calleeSymbol->signature(trMemory()), "FloatingDecimal")) {
			bytecodeSize >>= 1;

			if (comp()->trace(OMR::inlining))
				traceMsg(comp(),
				        "Reducing bytecode size to %d because it's method of FloatingDecimal\n",
				        bytecodeSize);
		}
		...
		if (!forceInline(calltarget)
		        && exceedsSizeThreshold(callsite, bytecodeSize,
		                (callsite->_callerBlock != NULL)
		                        ...
		                        : (callsite->_callNodeTreeTop ? callsite->_callNodeTreeTop->getEnclosingBlock()
		                                                      : 0),
...
TR_CallSite *
TR_InlinerBase::findAndUpdateCallSiteInGraph(TR_CallStack *callStack, TR_ByteCodeInfo &bcInfo, TR::TreeTop *tt,
        TR::Node *parent, TR::Node *callNode, TR_CallTarget *calltarget)
{
	...
	if (foundCall && callNode->getSymbolReference()->isUnresolved() && !callsite->isInterface()) {
		debugTrace(
		        tracer(), "findAndUpdateCallsiteInGraaph: call at node %p is unresolved. Bail out", callNode);
		return 0;
	}
	...
	if (callNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
		heuristicTrace(
		        tracer(), "findAndUpdateCallSiteInGraph: this call %p is on a cold side. Bail out", callNode);
		return 0;
	}
	...
	if (comp()->getOptions()->enableDebugCounters()) {
		if (!foundCall) {
			for (callsite = calltarget->_deletedCallees.getFirst(); callsite;
				...
				debugTrace(tracer(),
				        "considering deleted callee %p callNode->getByteCodeIndex = %d "
	...
	if (!foundCall && !foundDeadCall) {
		if (tracer()->debugLevel())
			tracer()->dumpCallSite(calltarget->_myCallees.getFirst(),
			        "findAndUpdateCallsiteInGraaph: could not match call form IL to call in graph for the "
			        "following callsite. callTarget %s, numCallees = %d numDeletedCallees = %d "
			        "falureReason = %d",
			        calltarget->_calleeSymbol->getResolvedMethod()->signature(comp()->trMemory()),
	...
	if (foundDeadCall) {
		...
		for (k = 0; k < callsite->numRemovedTargets(); k++) {
			tracer()->insertCounter(callsite->getRemovedTarget(k)->getCallTargetFailureReason(),
	...
	if (callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isInterface()
	        && callsite->_initialCalleeSymbol)
		debugTrace(tracer(),
		        "findAndUpdateCallSiteInGraph: BAD: Interface call has an initialCalleeSYmbol %p for calNode "
		        "%p",
		...
		if (callsite->numTargets() > 0) {
			if (callsite->isIndirectCall()) // local opts pre-existence has been performed
			{
				if (comp()->compileRelocatableCode())
					...
					callsite->getTarget(i)->_guard->_kind = TR_NoGuard; // do not need a guard
					                                                    // anymore
				...
				if (!callsite->getTarget(i)->_calleeMethod->isSameMethod(
				            callsite->_initialCalleeSymbol->getResolvedMethod())) {
					callsite->getTarget(i)->_myCallees.setFirst(0); // preventing furhter inlining
					                                                // as we now don't know what's
					                                                // beyond the new target
					callsite->getTarget(i)->_numCallees = 0; // may want to improve this eventually
					callsite->getTarget(i)->_partialInline = 0; // Can't do a partial inline anymore
					                                            // as the blocks are wrong.
		...
		if (callsite->getTarget(i)->_guard->_kind == TR_InterfaceGuard
			...
			if (tracer()->debugLevel())
				tracer()->dumpCallSite(callsite,
				        "findAndUpdateCallSiteInGraph: BAD: Interface call has an initialCalleeSYmbol "
				        "%p for calNode %p",
				        callsite->_initialCalleeSymbol, callNode);
        ...
        TR_CallStack *prevCallStack, TR_CallTarget *calltarget, TR_InnerPreexistenceInfo *innerPrexInfo)
{
	...
	for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
		...
		if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount
			...
			bool oldIsCold = isCold; // added to account for the fact that isCold is changed by
			                         // refineColdness call
			getUtil()->refineColdness(node, isCold);

			if (!isCold) {
				debugTrace(tracer(),
				        "inlineFromGraph:: about to call findAndUpdateCallSiteInGraph on call target "
				        "%p for call at node %p",
				        calltarget, node);
				TR_CallSite *site = findAndUpdateCallSiteInGraph(
				        &callStack, node->getByteCodeInfo(), tt, parent, node, calltarget);
				debugTrace(tracer(),
				        "inlineFromGraph: found a call at parent %p child %p, findAndUpdateCallsite "
				        "returned callsite %p",
	...
	debugTrace(tracer(),
	        "Done First Tree Iteration for updating PrexInfo.  Now starting inline Loop for calltarget %p",
	        calltarget);
	...
	for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
		...
		for (TR_CallTarget *target = iter.getFirst(); target; target = iter.getNext()) {
			if (target->_myCallSite->_callNodeTreeTop == tt && !target->_alreadyInlined) {
				debugTrace(tracer(),
				        "inlineFromGraph: Calling inlinecallTarget on calltarget %p in callsite %p",
        ...
        TR::ResolvedMethodSymbol *callerSymbol, TR::ResolvedMethodSymbol *calleeSymbol, TR::Optimizer *optimizer)
{
	...
	//
	static char *disablePropagatingCalleeFlagsToTopLevelMethod = feGetEnv("TR_"
	                                                                      "disablePropagatingCalleeFlagsToTopLevelM"
	                                                                      "ethod");
...
bool
TR_InlinerBase::tryToInlineTrivialMethod(TR_CallStack *callStack, TR_CallTarget *calltarget)
{
	return getPolicy()->tryToInlineTrivialMethod(callStack, calltarget);
}

bool
OMR_InlinerPolicy::tryToInlineTrivialMethod(TR_CallStack *callStack, TR_CallTarget *calltarget)
{
	return false;
}
        ...
        TR_CallStack *callStack, TR_CallTarget *calltarget, TR::TreeTop **cursorTreeTop, bool inlinefromgraph, int32_t)
{
	...
	int32_t numberOfNodesBefore = comp()->getNodeCount(); // want to know the number of nodes created before and
	                                                      // after genIL
	...
	if (innerPrexInfo->hasInnerAssumptions() && guard->_kind == TR_NoGuard) {
		if (comp()->trace(OMR::inlining)) {
			traceMsg(comp(),
			        "%sIPREX virtualize the non-virtual call %s because of inner preexistence for inner "
			        "calls\n",
	...
	if (argInfo) {
		if (comp()->usesPreexistence()) // Mark the preexistent arguments (maybe redundant if VP has already
		                                // done that and provided the info in argInfo)
		{
	...
	TR_TransformInlinedFunction *tif = getUtil()->getTransformInlinedFunction(callerSymbol, calleeSymbol,
	        blockContainingTheCall, callNodeTreeTop, callNode, pam, guard, tempList, _availableTemps,
	        _availableBasicBlockTemps);
	...
	*/
	...
	if (lastCalleeBlock) {
		...
		while ((n = calleeCFG->getNodes().pop())) {
			...
			if (!n->asBlock()->isOSRCatchBlock() && !n->asBlock()->isOSRCodeBlock())
				...
				debugTrace(tracer(),
				        "\ndon't add exception edges from callee OSR block(block_%d) to caller catch "
				        "blocks\n",
				        n->getNumber());
		}
	}
	...
	if (guard->_kind != TR_NoGuard) {
		virtualGuard = addGuardForVirtual(callerSymbol, calleeSymbol, callNodeTreeTop, callNode,
		        calltarget->_receiverClass, startOfInlinedCall, previousBBStartInCaller, nextBBEndInCaller,
		        *tif, tempList, guard, &induceOSRCallTree, calltarget);

		TR_VirtualGuard *vGuard = comp()->findVirtualGuardInfo(virtualGuard->getNode());
		heuristicTrace(tracer(),
		        "calltarget %p vGuard %p calltarget->_receiverClass %p callsite->_receiverClass %p\n",
		...
		// No guard is generated, caller's trees immediately after the call node is merged into inlined callee's
		// last block. The OSR exception edge from block containing the call hasn't been copied onto the
		// callee's last block. Check if the block has to be splitted for distinct exception edges.
		//
		...
		if (osrMethodData && osrMethodData->getOSRCatchBlock()) {
			 ...
			 * inliner behaviour, as the existance of a catch block will result in the required temporary
			 * being created.
			 */
			...
			// mergedBlock may have exception successors from the callee and non-OSR exception successors
			// from the caller, after split, `blockOfTreesAfterTheCall` only contains trees from the caller,
			// should not have exception successors from the callee.
			...
			// The inlined callee has no OSR point, but the trees after the call from caller may have OSR
			// points. Have to copy the exception edge from caller block containing the call to the merged
			// block, otherwise OSR points from the caller will become invalid.
	...
	if (callNode->getReferenceCount() > 0)
		replaceCallNode(callerSymbol, tif->resultNode(), originalCallNodeReferenceCount, callNodeTreeTop,
		        parent, callNode);
	...
	if (induceOSRCallTree) {
		...
		induceOSRCallNode->setNumChildren(numOfChildren - firstArgChild); // tricky "swelling" of the node but
		                                                                  // we allocated it of the right size
		                                                                  // when we inserted the induce OSR
		                                                                  // call to begin with
	...
	if (calltarget->_myCallSite->numTargets() > 1
		...
		if (lastNodeInCallBlock->getOpCode().isReturn() || lastNodeInCallBlock->getOpCode().isGoto()) {
			...
			nextBBEndInCaller->getNode()->getBlock()->getPrevBlock()->getExit()->insertBefore(
			        TR::TreeTop::create(comp(),
			                TR::Node::create(virtualGuard->getNode(), TR::Goto, 0,
			                        nextBBEndInCaller->getNode()->getBlock()->getEntry()))); // goto belongs
			                                                                                 // to the
			                                                                                 // callee
			                                                                                 // (?)
			...
			// cursorTreeTop is used to communicate to the caller the treeTop from which it should continue
			// iteration
			if (cursorTreeTop) {
	...
	int32_t bytecodeSize =
	        getPolicy()->getInitialBytecodeSize(calleeSymbol->getResolvedMethod(), calleeSymbol, comp());
	TR_ResolvedMethod *reportCalleeMethod = calleeSymbol->getResolvedMethod();
	int32_t numberOfLocalsInCallee = reportCalleeMethod->numberOfParameterSlots()
...
void
TR_CallSite::tagcalltarget(int32_t index, TR_InlinerTracer *tracer, TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::tagcalltarget(TR_CallTarget *calltarget, TR_InlinerTracer *tracer, TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::removecalltarget(int32_t index, TR_InlinerTracer *tracer, TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::removecalltarget(TR_CallTarget *calltarget, TR_InlinerTracer *tracer, TR_InlinerFailureReason reason)
{
	bool foundCallTarget = false;
	for (int32_t i = 0; i < _mytargets.size(); ++i) {
		if (_mytargets[i] == calltarget) {
...
void
TR_CallSite::removeTargets(TR_InlinerTracer *tracer, int index, TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::removeAllTargets(TR_InlinerTracer *tracer, TR_InlinerFailureReason reason)
{
...
TR_CallTarget *
TR_CallSite::addTarget(TR_Memory *mem, TR_InlinerBase *inliner, TR_VirtualGuardSelection *guard,
        TR_ResolvedMethod *implementer, TR_OpaqueClassBlock *receiverClass, TR_AllocationKind allocKind, float ratio)
{
	...
	if (inliner->tracer()->heuristicLevel()) {
		char name[1024];
		heuristicTrace(inliner->tracer(),
		        "Creating a call target %p for callsite %p using a %s and %s .  Signature %s", result, this,
		        inliner->tracer()->getGuardKindString(guard), inliner->tracer()->getGuardTypeString(guard),
		        _comp->fe()->sampleSignature(
	...
	for (calltarget = targets->getFirst(); calltarget; calltarget = calltarget->getNext()) {
		...
		do {
			...
			traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s", currentTarget->_myCallSite->getDepth(),
			        ...
			        currentTarget->_isPartialInliningCandidate ? currentTarget->_partialSize
			                                                   : currentTarget->_fullSize,
			        getFailureReasonString(currentTarget->_failureReason));
			traceMsg(comp(), "\t\t%d\t\t%s\n", currentTarget->_myCallSite->_byteCodeIndex,
			        comp()->fe()->sampleSignature(currentTarget->_calleeMethod->getPersistentIdentifier(),
...
TR_InlinerTracer::dumpDeadCalls(TR_LinkHead<TR_CallSite> *sites)
{
	...
	for (callsite = sites->getFirst(); callsite; callsite = callsite->getNext()) {
		traceMsg(comp(), "^^^ Top Level Dead CallSite %p Node %p bcIndex %p Failure Reason: %s\n", callsite,
		        callsite->_callNode, callsite->_byteCodeIndex,
		        getFailureReasonString(callsite->_failureReason));

		TR_CallTarget *calltarget;
		if (callsite->numTargets())
			traceMsg(comp(),
			        "\tCall Targets\n\tDepth\tP.I.\tcalltarget\tsize\tfailure reason\t\t\tbc "
			        "index\t\tSignature\n");
		for (int32_t i = 0; i < callsite->numTargets(); i++) {
			...
			traceMsg(comp(), "\t\t%d\t\t%s\n", calltarget->_myCallSite->_byteCodeIndex,
			        comp()->fe()->sampleSignature(calltarget->_calleeMethod->getPersistentIdentifier(),
			                name, SIGNATURE_SIZE, trMemory()));
		}
...

	{
		...
		if (callsite->_callerResolvedMethod) {
			...
			traceMsg(comp(), "\t CALLER signature from method = %s\n",
			        fe()->sampleSignature(callsite->_callerResolvedMethod->getPersistentIdentifier(),
...
TR_InlinerTracer::dumpCallTarget(TR_CallTarget *calltarget, const char *fmt, ...)
{
	...
	if (calltarget->_calleeMethod) {
		...
		traceMsg(comp(), "\t\t signature from method = %s\n",
		        fe()->sampleSignature(
		                calltarget->_calleeMethod->getPersistentIdentifier(), calleeName, 1024, trMemory()));
	} else
	...
	traceMsg(comp(),
	        ...
	        "_partialInline = %p\n",
	        calltarget->_callGraphAdjustedWeight, calltarget->_frequencyAdjustment,
			...
			if (currentTarget->_failureReason == InlineableTarget
				...
				traceMsg(comp(), "\t\t\t%s\n",
				        comp()->fe()->sampleSignature(
				                currentTarget->_calleeMethod->getPersistentIdentifier(), nameBuffer,
...
TR_InlinerDelimiter::TR_InlinerDelimiter(TR_InlinerTracer *tracer, char *tag) : _tracer(tracer), _tag(tag)
{
...
TR_InlinerTracer::dumpPrexArgInfo(TR_PrexArgInfo *argInfo)
{
...

	{
		...
		if (arg && arg->getClass()) {
			...
			traceMsg(comp(),
			        "<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d class=%p className= "
			        "%s/>\n",
...
OMR_InlinerPolicy::OMR_InlinerPolicy(TR::Compilation *comp) : TR::OptimizationPolicy(comp) {}

OMR_InlinerUtil::OMR_InlinerUtil(TR::Compilation *comp) : TR::OptimizationUtil(comp) {}

/**
 * replace the existing guard with a better one of more specific type if possible
 */
void
OMR_InlinerUtil::refineInlineGuard(TR::Node *callNode, TR::Block *&block1, TR::Block *&block2, bool &appendTestToBlock1,
...
void
OMR_InlinerUtil::estimateAndRefineBytecodeSize(
...
TR_TransformInlinedFunction *
OMR_InlinerUtil::getTransformInlinedFunction(TR::ResolvedMethodSymbol *callerSymbol,
...
OMR_InlinerUtil::createInnerPrexInfo(TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol,
