...
protected:
	TR_BlockOrderingOptimization(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
public:
	TR_BlockShuffling(TR::OptimizationManager *manager) : TR_BlockOrderingOptimization(manager) {}
	static TR::Optimization *create(TR::OptimizationManager *manager)
	{
...
private:
	...
	void riffle(TR::Block **blocks); // Break into two parts and interleave them randomly.  Blocks generally end up
	                                 // in the same order, but with cold interleaved with hot.
