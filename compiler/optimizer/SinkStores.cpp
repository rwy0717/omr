...
// are not precise enough to deal with some cases where a particular store contains the same commoned symbol under
...
//          =>iload sym1 <a>
//
// The bitvectors alone only provides the info that the istore contains sym1 as a commoned load but not how many times
// it contains sym1. This is a problem because then detecting when a particular sym1 reference is killed by another
// store or when the first reference to a particular symbol is impossible using the bitvectors alone. While the
// bitvectors are useful as a hint to the commoned symbols a more precise mechanism is required to track exactly what
// commoned references a particular store contains. Each movable store will contain a list of commoned loads beneath it.
// The loads will be wrapped in a class to mark the load as satisfied and/or killed and the symIdx that the load
// references. A commoned load is satisifed when the first reference to this load has been seen (regardless of whether
// the store has been killed first). A commoned load is killed when a store to this symbol is seen before the load has
// been satisified. After visiting all the nodes every commoned load will have been satisfied and some may have been
// killed. The savedLiveCommoned bitvector can be more precisely refined to removed symbols when no active movable store
// contains a particular symbol as an unsatisfied commoned load. This refinement will cause fewer walks of the movable
// stores to occur.
    ...
    , _isLoadStatic(false)
{
    ...
    if (_s->enablePreciseSymbolTracking() && _commonedLoadsUnderTree && !_commonedLoadsUnderTree->isEmpty()) {
        ...
        _commonedLoadsCount
            = initCommonedLoadsList(_useOrKillInfo->_tt->getNode()->getFirstChild(), comp()->incVisitCount());
        if (_s->trace())
        ...
        // A single symbol may be found in more than one load therefore commonedLoadsFound should be greater or equal to
        // the number of symbols set in _commonedLoadsUnderTree NOTE: _commonedLoadsCount <
        // _commonedLoadsUnderTree->elementCount() when a node is commoned within the tree but not outside of it istore
        //   ixor
        //      iload sym1
        //      =>iload sym1
        // In this case the futureUseCount on the node is 0 so it isn't added to the _commonedLoadsList
        // TR_ASSERT(_commonedLoadsCount >= _commonedLoadsUnderTree->elementCount(),"did not find the right number of
        // commonedLoads (found %d, actual %d)\n",_commonedLoadsCount,_commonedLoadsUnderTree->elementCount());
...
int32_t TR_MovableStore::initCommonedLoadsList(TR::Node* node, vcount_t visitCount)
{
    int32_t increment = 0;
    ...
    if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
        ...
        if (node->getFutureUseCount() > 0 && symIdx != INVALID_LIVENESS_INDEX) {
            TR_ASSERT(
                _commonedLoadsUnderTree->isSet(symIdx), "symIdx %d should be set in _commonedLoadsUnderTree\n", symIdx);
...
bool TR_MovableStore::containsSatisfiedAndNotKilledCommonedLoad(TR::Node* node)
{
    TR_ASSERT(_commonedLoadsList, "_commonedLoadsList is NULL\n");
    for (ListElement<TR_CommonedLoad>* le = _commonedLoadsList->getListHead(); le; le = le->getNextElement())
...
bool TR_MovableStore::satisfyCommonedLoad(TR::Node* node)
{
    ...
    for (ListElement<TR_CommonedLoad>* le = _commonedLoadsList->getListHead(); le; le = le->getNextElement()) {
        ...
        if ((node == commonedLoad->getNode()) && !commonedLoad->isSatisfied()) {
            if (_s->trace())
                traceMsg(comp(),
                    "      satisfyCommonedLoad (store %p) symIdx %d setting commonedLoad %p with node %p satisfied "
                    "(isKilled = %d, isSatisfied = %d)\n",
                    this->_useOrKillInfo->_tt->getNode(), commonedLoad->getSymIdx(), commonedLoad,
                    commonedLoad->getNode(), commonedLoad->isKilled(), commonedLoad->isSatisfied());
...
{
    // should never have satisfied more commoned loads then exist under the store, must have made too many calls to
    // satisfyCommonedLoad
...
bool TR_MovableStore::killCommonedLoadFromSymbol(int32_t symIdx)
{
    ...
    for (ListElement<TR_CommonedLoad>* le = _commonedLoadsList->getListHead(); le; le = le->getNextElement()) {
        ...
        if (!commonedLoad->isSatisfied() && (commonedLoad->getSymIdx() == symIdx)) {
            if (_s->trace())
                traceMsg(comp(),
                    "      killCommonedLoadFromSymbol (store %p) symIdx %d setting commonedLoad %p with node %p "
                    "killed\n",
...

    {
        ...
        if (trace() && sinkMethodMetaDataStores()) {
            ...
            for (auto* m = methodMetaDataSymbols.getFirst(); m != NULL; m = methodMetaDataSymbols.getNext()) {
                ...
                // If this assume fires than the order the method meta symbols were added in LiveVariableInformation has
                // changed and the mapping of live index to symbol name given below is wrong
                TR_ASSERT(m->getLiveLocalIndex() == localCount,
                    "Index mismatch for MethodMetaDataSymbol therefore this tracing output is wrong\n");
        ...
        if (block->getEntry() != NULL) {
            ...
            do {
                ...
                if (enablePreciseSymbolTracking()) {
                    ...
                    // must or in treeCommonedLoads *after* subtracting out satisfiedLiveCommonedLoads in case this
                    // store contains both a first use and a commoned load from the same symbol
                    (*savedLiveCommonedLoads) |= (*treeCommonedLoads);
                    satisfiedLiveCommonedLoads->empty();
                } else {
                    // remember the currently live commoned loads so that we can figure out later if we can move this
                    // store
                    (*savedLiveCommonedLoads) = (*_liveVarInfo->liveCommonedLoads());
                }
                ...
                // TR_MethodMetaData symbols are thread local and may be killed/used by any arbitrary symRef
                else if (sinkMethodMetaDataStores() && (node->getOpCode().isCallDirect() || node->getOpCode().isStore())
                    ...
                    if (!usedef.IsZero()) {
                        ...
                        if (1) {
                            ...
                            if (trace()) {
                                traceMsg(comp(),
                                    "       symRef [%d] on node " POINTER_PRINTF_FORMAT " implicitly kills symrefs:  ",
                            ...
                            for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                                ...
                                if (killedSymbol->getLiveLocalIndex() < numLocals
                                    && killedSymbol->getLiveLocalIndex() != INVALID_LIVENESS_INDEX) {
                                    if (trace())
                                    ...
                                    if (savedLiveCommonedLoads->get(killedSymbol->getLiveLocalIndex())) {
                                        if (trace())
                                            traceMsg(comp(),
                                                "      updating killedLiveCommonedLoads with savedLiveCommonedLoads, "
                                                "setting index %d\n",
                                    ...
                                    if (node->getOpCode().isCallDirect()) {
                                        if (trace())
                                            traceMsg(comp(),
                                                "       setting symIdx %d (from symRef %d) on usedSymbols\n",
                                                killedSymbol->getLiveLocalIndex(), killedSymRef->getReferenceNumber());
                    ...
                    while (storeElement != NULL) {
                        ...
                        if (trace()) {
                            ...
                            if (store->_movable && store->_commonedLoadsUnderTree
                                ...
                                traceMsg(comp(),
                                    "            store->containsUnsatisfedLoadFromSymbol(%d) = %d so %s temp\n",
                                    killedSymIdx, store->containsUnsatisfedLoadFromSymbol(killedSymIdx),
                                    store->containsUnsatisfedLoadFromSymbol(killedSymIdx) ? "needs" : "doesn't need");
                            }
                        }
                        if (store->_movable && store->_commonedLoadsUnderTree
                            && store->_commonedLoadsUnderTree->get(killedSymIdx)
                ...
                //
                //
                // The goal of the code below is to identify cases like the above where the iload sym1 in b1 is the
                // first use the commoned sym1 in b4 (and to know that the iload sym1 in b3 is *not* the first use)
                // Precise first reference locations are required in case an anchor needs be created when replicating
                // the istore in b4 to a location outside the current EBB. The anchored value of sym1 must be from just
                // above the iload reference in b1 because an incorrect anchor in b3 would cause the wrong value to be
                // used in the replicated store.
                //
                    ...
                    if (!firstRefsToCommonedSymbols->isEmpty()) {
                        ...
                        while (storeElement != NULL) {
                            ...
                            if (store->_movable && store->_commonedLoadsUnderTree
                                ...
                                if (trace())
                                    traceMsg(comp(),
                                        "            examining store for first uses [" POINTER_PRINTF_FORMAT "]\n",
                                        store->_useOrKillInfo->_tt->getNode());
                                TR_BitVector* firstRefsOfCommonedSymbolsUnderThisStore
                                    = new (trStackMemory()) TR_BitVector(*store->_commonedLoadsUnderTree);
                                (*firstRefsOfCommonedSymbolsUnderThisStore) &= (*firstRefsToCommonedSymbols);
                                _searchMarkCalls++;
                                searchAndMarkFirstUses(
                                    node, tt, store, block, firstRefsOfCommonedSymbolsUnderThisStore);
                                ...
                                while (bvi.hasMoreElements()) {
                                    ...
                                    if (!store->containsUnsatisfedLoadFromSymbol(firstRefSymIdx)) {
                                        if (trace())
                                            traceMsg(comp(),
                                                "            symIdx %d now satisfied increment "
                                        ...
                                        traceMsg(comp(),
                                            "            symIdx %d is not yet satisfied do increment "
                                ...
                                if ((*firstRefsOfCommonedSymbolsUnderThisStore) != (*firstRefsToCommonedSymbols)) {
                                    TR_BitVector* firstRefsOfCommonedSymbolsNotUnderThisStore
                                        = new (trStackMemory()) TR_BitVector(*firstRefsToCommonedSymbols);
                                    ...
                                    // any firstRef symbols not under this store as a commoned load are trivially
                                    // satisfied
                                    while (bvi.hasMoreElements()) {
                                        int32_t firstRefSymIdx = bvi.getNextElement();
                                        if (trace())
                                            traceMsg(comp(),
                                                "            symIdx %d not under this store increment "
                                                "satisfiedCommonedSymCount %d -> %d\n",
                                                firstRefSymIdx, satisfiedCommonedSymCount[firstRefSymIdx],
                                                satisfiedCommonedSymCount[firstRefSymIdx] + 1);
                                        satisfiedCommonedSymCount[firstRefSymIdx]++;
                                ...
                                while (bvi.hasMoreElements()) {
                                    ...
                                    if (trace())
                                        traceMsg(comp(),
                                            "            symIdx %d not at all under this store increment "
                                            "satisfiedCommonedSymCount %d -> %d\n",
                                            firstRefSymIdx, satisfiedCommonedSymCount[firstRefSymIdx],
                                            satisfiedCommonedSymCount[firstRefSymIdx] + 1);
                                    satisfiedCommonedSymCount[firstRefSymIdx]++;
                        ...
                        // A symIdx can be removed from the savedLiveCommonedLoads bitvector (using
                        // satisfiedLiveCommonedLoads) when it no longer appears as unsatisfied in *any* store
                        ...
                        // savedLiveCommonedLoads is refined by subtracting (taking the set difference) of
                        // satisfiedLiveCommonedLoads from savedLiveCommonedLoads
                        ...
                        while (firstRefsItAfter.hasMoreElements()) {
                            ...
                            if (satisfiedCommonedSymCount[firstRefSymIdx] == storeCount) {
                                if (trace())
                                    traceMsg(comp(),
                                        "            satisfiedCommonedSymCount[%d] = storeCount = %d set in "
                                        "satisfiedLiveCommonedLoads\n",
                                        firstRefSymIdx, storeCount);
                                satisfiedLiveCommonedLoads->set(firstRefSymIdx);
                            } else if (trace()) {
                                traceMsg(comp(),
                                    "            satisfiedCommonedSymCount[%d] = %d and storeCount = %d do not set in "
                                    "satisfiedLiveCommonedLoads\n",
                ...
                if (!enablePreciseSymbolTracking()) {
                    // When the commoned symbols are tracked precisely it is enough to run the above code that
                    // intersects used with savedLiveCommonedLoads to find the first uses of symbols With this
                    // information an anchor or temp store can be created when/if it is actually needed in during
                    // sinkStorePlacement()
                        ...
                        while (storeElement != NULL) {
                            ...
                            if (store->_movable && store->_needTempForCommonedLoads) {
                                if (store->_needTempForCommonedLoads->intersects(*firstRefsToKilledSymbols)) {
                                    ...
                                    // actually be placed here see the comment in isCorrectCommonedLoad for more
                                    // information
                ...
                if (local != NULL && node->getOpCode().isStoreDirect()) {
                    ...
                    bool sinkWithIndirectLoads
                        = sym && comp()->getOption(TR_SinkAllStores) && sinkStoresWithIndirectLoads();
                    canMoveStore = true;
                    ...
                    // It is not legal to sink AR symbols as exceptions points are implicit uses of these symbols
                        ...
                        canMoveStore = false;
                    } else if (!storeIsSinkingCandidate(block, node, symIdx, sinkWithIndirectLoads, indirectLoadCount,
                ...
                if (canMoveStore) {
                    ...
                    if (enablePreciseSymbolTracking() && myCommonedLoads) {
                        ...
                        if (myCommonedLoads->isSet(symIdx)) {
                            ...
                            if (trace())
                                traceMsg(comp(),
                                    "      store kills its own commoned symbol (%d), mark the store as needing a temp "
                                    "load\n",
                                    symIdx);
                            moveStoreWithTemps->set(symIdx);
                            killedLiveCommonedLoads->set(
                                symIdx); // otherwise the anchor/temp store is never created when the first ref is found
                        }
                        if (myCommonedLoads && !myCommonedLoads->isEmpty()) {
                            ...
                            // So the tree walk above to discover nodes does not skip any nodes that were visited by
                            // TR_MovableStore ctor increment treeVisitCount and highestVisitCount here.
                            treeVisitCount++;
                            highestVisitCount++;
                        }
                    }
...
void TR_SinkStores::searchAndMarkFirstUses(TR::Node* node, TR::TreeTop* tt, TR_MovableStore* movableStore,
    TR::Block* currentBlock, TR_BitVector* firstRefsOfCommonedSymbolsForThisStore)
{
    if (0 && trace())
        traceMsg(comp(), "      node %p, futureUseCount %d, refCount %d\n", node, node->getFutureUseCount(),
    ...
    if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
        ...
        if (symIdx != INVALID_LIVENESS_INDEX && firstRefsOfCommonedSymbolsForThisStore->isSet(symIdx)) {
            ...
            if (trace() && commonedLoad)
                traceMsg(comp(),
                    "      movableStore %p containsCommonedLoad (node %p, symIdx %d, isSatisfied = %d, isKilled = "
                    "%d)\n",
                    movableStore->_useOrKillInfo->_tt->getNode(), commonedLoad->getNode(), commonedLoad->getSymIdx(),
                    commonedLoad->isSatisfied(), commonedLoad->isKilled());
            else if (trace())
                traceMsg(comp(), "      commonedLoad is NULL for node %p with symIdx %d\n", node, symIdx);
            if ((node->getFutureUseCount() == 0) && movableStore->satisfyCommonedLoad(node)) {
                if (0 && trace())
                    traceMsg(comp(), "      node %p, futureUseCount %d, refCount %d\n", node, node->getFutureUseCount(),
                        node->getReferenceCount());
                if (!findFirstUseOfLoad(node)) {
                    TR_FirstUseOfLoad* firstUse
                        = new (trStackMemory()) TR_FirstUseOfLoad(node, tt, currentBlock->getNumber());
                    TR_HashId addID = 0;
                    _firstUseOfLoadMap->add(node, addID, firstUse);
                    if (trace())
                        traceMsg(comp(),
                            "      searchAndMarkFirstUses creating and adding firstUse %p with node %p and anchor "
                            "treetop %p to hash\n",
    ...
    if (_indirectLoadAnchors) {
        ...
        for (TR_IndirectLoadAnchor* loadAnchor = it.getFirst(); loadAnchor; loadAnchor = it.getNext()) {
            ...
            if (anchoredNode->decFutureUseCount() == 0) {
                ...
                //    - low enough that the child of the indirect load (the translateAddress node) has already been
                //    evaluated
                ...
                if (trace())
                    traceMsg(comp(),
                        "anchoring indirect load %p after node %p at start of block_%d anchor tt node is %p\n",
                        anchoredNode, anchorBlock->getEntry()->getNode(), anchorBlock->getNumber(),
                        anchorTree->getNode());
...
TR::RegisterMappedSymbol* TR_SinkStores::getSinkableSymbol(TR::Node* node)
{
    ...
    if (symbol->isAutoOrParm() || (sinkMethodMetaDataStores() && symbol->isMethodMetaData()))
...
bool TR_TrivialSinkStores::storeIsSinkingCandidate(TR::Block* block, TR::Node* node, int32_t symIdx,
    ...
    vcount_t& highVisitCount)
{
    ...
    return (symIdx >= 0 && block->getNextBlock() && block->getNextBlock()->isExtensionOfPreviousBlock()
    ...
    vcount_t& highVisitCount)
{
    ...
    // treeIsSinkable store checks the visitCount as part of the enablePreciseSymbolTracking work so when this is
    // enabled then the call below must pass in comp()->incVisitCount() instead of comp()->getVisitCount() and
    // treeVisitCount and highVisitCount must be incremented
    if (enablePreciseSymbolTracking()) {
        treeVisitCount++;
        highVisitCount++;
    }
    TR_ASSERT(!enablePreciseSymbolTracking(),
        "visitCount arg treeIsSinkableStore is ignored, should use incVisitCount() instead\n");
    comp()->setCurrentBlock(block);
    return (symIdx >= 0 && _liveOnNotAllPaths->_outSetInfo[b]->get(symIdx)
        && treeIsSinkableStore(node, sinkIndirectLoads, indirectLoadCount, depth, isLoadStatic,
               enablePreciseSymbolTracking() ? comp()->incVisitCount() : comp()->getVisitCount()));
}
    ...
    List<TR_IndirectLoadAnchor>* indirectLoadAnchorsForThisStore, vcount_t visitCount)
{
    ...
    if (indirectLoadAnchorsForThisStore && nodeOrig->getOpCode().isLoadIndirect()) {
        ...
        if (nodeParentCopy) {
            // if the node is being replaced then there is no need to search below because the side exit tree being
            // created can now not reference anything below this point in the fall through case (where the node is not
            // being replaced) then the children of this indirect load must be searched in case there are first uses of
            // commoned or blocked symbols that should be anchored
    ...
    /*
          ...
          // If they are only first evaluated below the anchor then the register may be commoned and this would be
       illegal
    ...
    */
        ...
        // Only need an anchor for killedCommoned loads when calling this function for sideExit store trees (when
        // nodeCopy == NULL) In the case where the current store is the one being moved to the lowest fall through path
        // we are not duplicating and therefore no uncommoning takes place and therefore no temp anchor is required as
        // the original, naturally anchored, load will be used.
        if (nodeCopy && needTempForCommonedLoads && needTempForCommonedLoads->isSet(symIdx)
            ...
            if (trace())
                traceMsg(comp(),
                    "      insertAnchoredNodes needTemp for symIdx %d found firstUse %p with node %p and anchor "
                    "treetop %p\n",
                    symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
        } else if (blockingCommonedSymbols && blockingCommonedSymbols->isSet(symIdx)
            ...
            if (trace())
                traceMsg(comp(),
                    "      insertAnchoredNodes commonedBlocked for symIdx %d found firstUse %p with node %p and anchor "
                    "treetop %p\n",
                    symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
        } else if (blockingUsedSymbols && blockingUsedSymbols->isSet(symIdx)
            && !store->containsCommonedLoad(nodeOrig)) {
            firstUseOfLoad = findFirstUseOfLoad(nodeOrig);
            // A first use may already exist for a blocking used symbol load if this node itself is commoned somewhere
            // below
            if (!firstUseOfLoad) {
                ...
                if (trace())
                    traceMsg(comp(),
                        "      insertAnchoredNodes usedBlocked create/add for symIdx %d  firstUse %p with node %p and "
                        "anchor treetop\n",
                        symIdx, firstUseOfLoad, nodeOrig, store->_useOrKillInfo->_tt->getNode());
            } else {
                if (trace())
                    traceMsg(comp(),
                        "      insertAnchoredNodes usedBlocked found for symIdx %d firstUse %p with node %p and anchor "
                        "treetop %p\n",
                        symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
            }
        }
    ...
    for (int32_t i = nodeOrig->getNumChildren() - 1; i >= 0; i--) {
        increment += insertAnchoredNodes(store, nodeCopy ? nodeCopy->getChild(i) : NULL, nodeOrig->getChild(i),
            nodeCopy, i, needTempForCommonedLoads, blockingUsedSymbols, blockingCommonedSymbols, currentBlock,
...
// TR_TrivialSinkStores is less restrictive in its conditions below as it allows a used symbol to interfere with a
// killed symbol below. In this an anchor to the symbol will be created above its first use.
...
TR::TreeTop* TR_TrivialSinkStores::genSideExitTree(TR::TreeTop* storeTree, TR::Block* exitBlock, bool isFirstGen)
{
    ...
    if (isFirstGen) {
        ...
        // It is necessary to anchor the grandchildren of the computeCC instead of just the child because lowerTrees
        // will uncommon the computeCC node anyway as cc computations require that the opNode operation is done right
        // before the cc sequence is generated. Therefore the grandchildren must be anchored. bustore      <- storeNode
        //   computeCC
        //     opNode (iadd/isub/iand... etc)
        //       child1   <- anchor this node (except for const nodes, these are uncommoned -- duplicated in the
        //       sideExitTree) child2   <- anchor this node (except for const nodes, these are uncommoned -- duplicated
        //       in the sideExitTree)
        for (int32_t i = 0; i < opNode->getNumChildren(); i++) {
            if (!opNode->getChild(i)->getOpCode().isLoadConst()) {
                ...
                if (trace())
                    traceMsg(comp(),
                        "      genSideExitTree anchoring computeCC grandchild %p in new node %p before node %p\n",
                ...
                traceMsg(
                    comp(), "      genSideExitTree not anchoring const computeCC grandchild %p\n", opNode->getChild(i));
            }
        }
    }
    ...
    if (trace())
        traceMsg(comp(),
            "      genSideExitTree creating opNodeClone %p (refCount = %d) from opNode %p (refCount = %d)\n",
            opNodeClone, opNodeClone->getReferenceCount(), opNode, opNode->getReferenceCount());
    return TR::TreeTop::create(comp(), exitBlock->getEntry(),
        TR::Node::createWithSymRef(storeNode->getOpCodeValue(), 1, 1, computeCCNode, storeNode->getSymbolReference()));
}

bool TR_TrivialSinkStores::passesAnchoringTest(
    TR_MovableStore* store, bool storeChildIsAnchored, bool nextStoreWasMoved)
{
    ...
    // istore sym1
    //   =>opNode
    // In this case the istore sym1 will not be sunk but we do want to sink the bustore cc. Because the bustore contains
    // both an indirectLoad and a used sym that is killed below (sym1) two anchors are required. Instead we might as
    // well just anchor the two children of the opNode to better deal with the case where they are more complex
    // expressions. Blindly duplicating the entire tree and just inserting the minimally required anchors will cause
    // lots of uncommoning to occur as a result of the duplication.
    TR::TreeTop* storeTree = store->_useOrKillInfo->_tt;
    TR::Node* storeNode = storeTree->getNode();

    if (!comp()->getOption(TR_DisableStoreAnchoring) && store->_useOrKillInfo->_indirectLoadCount
        && // if it contains an indirectLoad will need at least one anchor
        ...
        && storeTree->getNextTreeTop()->getNode()->containsNode(
               storeNode->getFirstChild()->getFirstChild(), comp()->incVisitCount())) {
        return true;
    } else {
        return false;
    }
...
TR::TreeTop* TR_TrivialSinkStores::duplicateTreeForSideExit(TR::TreeTop* tree)
{
    ...
    return TR::TreeTop::create(
        comp(), duplicateNodeForSideExit(tree->getNode()), tree->getPrevTreeTop(), tree->getNextTreeTop());
}
...
TR::Node* TR_TrivialSinkStores::duplicateNodeForSideExit(TR::Node* node)
{
    ...
    // In all cases for a sideExitStoreTree insertAnchoredNodes will replace newRoot with a reference to an anchored
    // mainline node
    if (node->getOpCode().isLoadIndirect()) {
        if (0 && trace())
...
bool TR_TrivialSinkStores::sinkStorePlacement(TR_MovableStore* store, bool nextStoreWasMoved)

{
    ...
    if (commonedSymbols) {
        if (trace())
            traceMsg(
                comp(), "         commonedSymbols = true so set allUsedSymbols to usedSymbols | commonedSymbols\n");
    ...
    while (fallThroughBlock && fallThroughBlock->isExtensionOfPreviousBlock()) {
        ...
        if (trace()) {
            ...
            traceMsg(comp(),
                "         Four storeCanMoveThroughBlock conditions, if any are true then the store cannot be sunk:\n");
        ...
        if (storeCanMoveThroughBlock(
                _symbolsKilledInBlock[currentBlockNumber], _symbolsUsedInBlock[currentBlockNumber], symIdx)) {
            TR::Node* storeNode = storeTree->getNode();
            ...
            if (_symbolsKilledInBlock[currentBlockNumber]->intersects(*usedSymbols)) {
                ...
                if (comp()->getOption(TR_SinkAllStores) || 1) {
                    TR_BitVector* blockingUsedSymbolsForThisBlock
                        = new (trStackMemory()) TR_BitVector(*_symbolsKilledInBlock[currentBlockNumber]);
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "         not sinking store %p any further as it is not a cc and there are blocking used "
                            "symbols\n",
                            storeNode);
                    break;
                }
                ...
                // if (comp()->getOption(TR_SinkAllStores) || 1)
                {
                    ...
                    if (blockingCommonedSymbols)
                        ...
                        blockingCommonedSymbols
                            = new (trStackMemory()) TR_BitVector(*blockingCommonedSymbolsForThisBlock);
                    if (trace()) {
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "         not sinking store %p any further as it is not a cc and there are blocking "
                            "commoned symbols\n",
                            store);
                    break;
                }
            ...
            bool copyStore = true; // when the store is going sunk to the side exit should it be copied first and then
                                   // sunk or just sunk
            ...
            if (!(currentBlock->getSuccessors().size() == 1)) {
                ...
                if (lastTree->getNode()->getOpCode().isBranch()) {
                    if (trace())
                        traceMsg(comp(),
                            "               found branch node [" POINTER_PRINTF_FORMAT "] as last node in block_%d\n",
                            lastTree->getNode(), currentBlock->getNumber());
                    // test !anchoredByBranch below instead of !storeChildIsAnchored to catch this case
                    // S
                    // ST
                    // BC
                    // if !storeChildIsAnchored is used then the cc computation is duplicated on the BC side exit
                    // instead of being anchored above the branch
                    if (passesAnchoringTest(store, storeChildIsAnchored, nextStoreWasMoved)) {
                        ...
                        if (trace())
                            traceMsg(comp(),
                                "                  store anchor branch first - create new store %p with commoned "
                                "store->firstGrandChild %p on sideExit block_%d\n",
                                sideExitStoreTree->getNode(), storeNode->getFirstChild()->getFirstChild(),
                                exitBlock->getNumber());
                        ...
                        if (trace())
                            traceMsg(comp(), "                  store has %d indirect load%s\n", indirectLoadCount,
                                indirectLoadCount > 1 ? "s" : "");
                    } else {
                        canMoveToAllPaths = true;
                        if (trace())
                            traceMsg(comp(),
                                "               store child %p is not anchored under the branch or a subsequent "
                                "store\n",
                        ...
                        if (anchoredByBranch) {
                            ...
                            TR_ASSERT(anchoredByBranchNode,
                                "anchoredByBranchNode should have been created already when anchoredByBranch was set "
                                "to true\n");
                            sideExitStoreTree = TR::TreeTop::create(comp(), exitBlock->getEntry(),
                                TR::Node::createWithSymRef(storeNode->getOpCodeValue(), 1, 1,
                                    storeNode->getFirstChild(), storeNode->getSymbolReference()));
                            if (trace())
                                traceMsg(comp(),
                                    "                  branch anchor next - create new store %p with commoned "
                            ...
                            if (trace())
                                traceMsg(comp(),
                                    "                  store anchor branch next - create new store %p with commoned "
                                    "store->firstChild %p on sideExit block_%d\n",
                                    sideExitStoreTree->getNode(), storeNode->getFirstChild(), exitBlock->getNumber());
                        } else {
                            sideExitStoreTree = storeTree;
                            if (trace()) {
                                traceMsg(comp(),
                                    "                  cc child is not already anchored and should not be anchored "
                                    "here\n");
                ...
                if (trace())
                    traceMsg(comp(),
                        "               found exception node [" POINTER_PRINTF_FORMAT "] as last node in block_%d\n",
                        lastTree->getNode(), currentBlock->getNumber());
                // For now just support a single exception successor.
                if (currentBlock->getExceptionSuccessors().size() == 1) {
                    // If the last node of an exception throwing block is not the one and only node that raises the
                    // exception then we will have to split the block as the store may not be live on all exception
                    // paths out of the block For now just require that an exception raising node must end a block. This
                    // is the excepted behaviour in zEmulator control flow graphs.
                    ...
                    if (exceptingNodes == 1) {
                        ...
                        if (passesAnchoringTest(store, storeChildIsAnchored, nextStoreWasMoved)) {
                            ...
                            if (trace())
                                traceMsg(comp(),
                                    "                  store anchor exception first - create new store %p with "
                                    "commoned store->firstGrandChild %p on sideExit block_%d\n",
                                    sideExitStoreTree->getNode(), storeNode->getFirstChild()->getFirstChild(),
                                    exitBlock->getNumber());
                        } else if (!storeChildIsAnchored && indirectLoadCount) {
                            sideExitStoreTree = storeTree;
                            ...
                            if (anchoredByBranch) {
                                ...
                                copyStore = false;
                                TR_ASSERT(anchoredByBranchNode,
                                    "anchoredByBranchNode should have been created already when anchoredByBranch was "
                                    "set to true\n");
                                sideExitStoreTree = TR::TreeTop::create(comp(), exitBlock->getEntry(),
                                    TR::Node::createWithSymRef(storeNode->getOpCodeValue(), 1, 1,
                                        storeNode->getFirstChild(), storeNode->getSymbolReference()));
                                if (trace())
                                    traceMsg(comp(),
                                        "                  create new store %p with commoned store->firstChild %p on "
                                        "sideExit block_%d\n",
                                        sideExitStoreTree->getNode(), storeNode->getFirstChild(),
                                        exitBlock->getNumber());
                            } else if (anchoredByStore) {
                                if (trace())
                                    traceMsg(comp(), "                  cc grand child already anchored\n");
                                ...
                                if (trace())
                                    traceMsg(comp(),
                                        "                  store anchor exception next - create new store %p with "
                                        "commoned store->firstChild %p on sideExit block_%d\n",
                                        sideExitStoreTree->getNode(), storeNode->getFirstChild(),
                                        exitBlock->getNumber());
                            } else {
                                sideExitStoreTree = storeTree;
                        ...
                        if (trace() || comp()->getOption(TR_TraceOptDetails))
                            traceMsg(comp(),
                                "            missed a store sinking opportunity as a current block_%d with a single "
                                "exception succ does not have exactly 1 excepting node (%d excepting nodes)\n",
                    ...
                    if (trace() || comp()->getOption(TR_TraceOptDetails))
                        traceMsg(comp(),
                            "            not sinking store as there is > 1 exception successor for current block_%d\n",
            ...
            if (canMoveToAllPaths) {
                ...
                if (sideExitStoreTree) {
                    TR_StoreInformation* storeInfo
                        = new (trStackMemory()) TR_StoreInformation(sideExitStoreTree, copyStore);
                    if (copyStore && store->_depth > MAX_TREE_DEPTH_TO_DUP) {
                        if (trace())
                            traceMsg(comp(),
                                "no anchor found and depth %d is > than max duplication depth of %d so do not sink "
                                "this store\n",
                                store->_depth, MAX_TREE_DEPTH_TO_DUP);
                        break;
                    }
                    if (copyStore
                        && (indirectLoadCount || killedCommonedSymbols || blockingUsedSymbols
                               || blockingCommonedSymbols)) {
                        storeInfo->_storeTemp = duplicateTreeForSideExit(sideExitStoreTree);
                        storeInfo->_needsDuplication = false;
                        if (trace())
                            traceMsg(comp(),
                                "                  call insertAnchoredNodes dupTree is %p, copyStore = %d, "
                                "indirectLoadCount = %d, killedCommonedSymbols = %p, blockingUsedSymbols = %p, "
                                "blockingCommonedSymbols = %p\n",
                                storeInfo->_storeTemp->getNode(), copyStore, indirectLoadCount, killedCommonedSymbols,
                                blockingUsedSymbols, blockingCommonedSymbols);
                        uint32_t indirectLoadsReplaced
                            = insertAnchoredNodes(store, storeInfo->_storeTemp->getNode()->getFirstChild(), // copy
                                sideExitStoreTree->getNode()->getFirstChild(), // original
                                storeInfo->_storeTemp->getNode(), // parent node of copy
                                0, // child index of copy
                                killedCommonedSymbols, blockingUsedSymbols, blockingCommonedSymbols, currentBlock,
                                indirectLoadAnchorsForThisStore, comp()->incVisitCount());

                        // replaced must be >= count because indirectLoadCount is the unique count of indirect loads but
                        // the tree might contain multiple references to the same indirect load and they all must be
                        // replaced
                        TR_ASSERT(indirectLoadsReplaced >= indirectLoadCount,
                            "did not find and replace all the indirect loads (replaced %d, found %d)\n",
                            indirectLoadsReplaced, indirectLoadCount);
                    } else {
                        storeInfo->_storeTemp = sideExitStoreTree;
                        if (trace())
                            traceMsg(comp(),
                                "                  do not call insertAnchoredNodes dupTree is %p, copyStore = %d, "
                                "indirectLoadCount = %d, killedCommonedSymbols = %p, blockingUsedSymbols = %p, "
                                "blockingCommonedSymbols = %p\n",
                                storeInfo->_storeTemp->getNode(), copyStore, indirectLoadCount, killedCommonedSymbols,
                                blockingUsedSymbols, blockingCommonedSymbols);
                    ...
                    if (exitBlock) {
                        TR_BlockStorePlacement* newBlockPlacement
                            = new (trStackMemory()) TR_BlockStorePlacement(storeInfo, exitBlock, trMemory());
            ...
            if (trace() || comp()->getOption(TR_TraceOptDetails))
                traceMsg(
                    comp(), "            Can't push sym %d to end of current block_%d\n", symIdx, currentBlockNumber);
            fallThroughBlock = 0;
        }
    } // end while
    ...
    if ((trace() || comp()->getOption(TR_TraceOptDetails)) && fallThroughBlock
        ...
        traceMsg(comp(),
            "            fallThroughBlock %d is not part of same superblock as currentBlock %d so will not try to sink "
            "further\n",
    ...
    if (sourceBlock != currentBlock) {
        ...
        // indirect loads have to be anchored right at their original location otherwise if the parent node is swung
        // down then the indirect load may get moved passed a store to the same memory location (or loads will get
        // reordered which is also not allowed). This happens, among other places, in the IL for TS where the iiload
        // under the bustore cc gets swung below the iistore to the same memory location.
        if (anchorIndirectLoads || blockingUsedSymbols || blockingCommonedSymbols) {
            if (trace())
                traceMsg(comp(),
                    "                  call insertAnchoredNodes !storeChildisAnchored = %d, indirectLoadCount = %d, "
                    "killedCommonedSymbols = %p, blockingUsedSymbols = %p, blockingCommonedSymbols = %p\n",
                    !storeChildIsAnchored, indirectLoadCount, killedCommonedSymbols, blockingUsedSymbols,
                    blockingCommonedSymbols);
            ...
            uint32_t indirectLoadsReplaced = insertAnchoredNodes(store,
                ...
                0, // child index of copy
                killedCommonedSymbols, blockingUsedSymbols, blockingCommonedSymbols, currentBlock,
                anchorIndirectLoads ? indirectLoadAnchorsForThisStore : NULL, comp()->incVisitCount());
            ...
            if (trace())
                traceMsg(comp(),
                    "                  do not call insertAnchoredNodes !storeChildisAnchored = %d, indirectLoadCount = "
                    "%d, killedCommonedSymbols = %p, blockingUsedSymbols = %p, blockingCommonedSymbols = %p\n",
                    !storeChildIsAnchored, indirectLoadCount, killedCommonedSymbols, blockingUsedSymbols,
                    blockingCommonedSymbols);
        }
        TR_BlockStorePlacement* newBlockPlacement
            = new (trStackMemory()) TR_BlockStorePlacement(storeInfo, currentBlock, trMemory());
        blockPlacementsForThisStore.add(newBlockPlacement);
        sunkStore = true;
    }
    ...
    // The stores are examined in reverse order but the exit blocks are encountered in forward order as the store is
    // pushed down. So for the program order stores (S) and exits (E) S_a E_1a E_2a E_3a S_b E_1b E_2b E_3b The stores
    // are visited in the order S_b, S_a but for each store the exit blocks are visited in the order E_1,E_2,E_3. So the
    // futureUseCounts will work correctly so the indirect load is anchored before its first use we want the master list
    // of _indirectLoadAnchors to be in reverse order (the first loadAnchor in program order is the last one dealt with
    // in doSinking)
    //
    ...
    // Before the indirectLoadAnchorsForThisStore for S_b are added the master list is empty and after they are appended
    // the master list is: L_3b,L_2b,L_1b And after the indirectLoadAnchorsForThisStore for S_a are appended to the
    // master list: L_3b,L_2b,L_1b,L_3a,L_2a,L_1a and this list is now in the reverse order we need for the
    // futureUseCounts to work correctly in doSinking.
    //
    // How doSinking uses the futureUseCounts:
    // Assuming each side exit tree references the same indirect load anchor then after all the exits have been seen the
    // futureUseCount will be 6. When doSinking processes the list it will decrement the futureUseCount each time it
    // encounters a regStore and then finally anchor it when the futureUseCount has reached zero.  Because L_1a shows up
    // last in the list doSinking will correctly anchor the indirect load before this location
...
const char* TR_TrivialSinkStores::optDetailString() const throw() { return "O^O TRIVIAL SINK STORES: "; }
