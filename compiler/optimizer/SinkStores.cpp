	...
	for (TR::CFGNode *block = comp()->getFlowGraph()->getFirstNode(); block != NULL; block = block->getNext()) {
		...
		for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
			...
			if (liveOnSomePaths->_blockAnalysisInfo[toBlockNumber]) {
				...
				int isBackEdge =
				        (block->getForwardTraversalIndex() >= toBlock->getForwardTraversalIndex());
				if (isBackEdge) {
					...
					if (trace)
						traceMsg(comp(),
						        "    Adding backedge live vars from block_%d to LiveOnAllPaths "
						        "for block_%d\n",
						        toBlockNumber, b);
...
TR_OrderedBlockList::addInTraversalOrder(TR::Block *block, bool forward, TR::CFGEdge *edge)
{
	...
	while (ptr != NULL) {
		...
		int32_t ptrTraversalIndex = (forward) ? ptrBlock->getForwardTraversalIndex()
...
TR_OrderedBlockList::removeBlockFromList(TR::Block *block, TR::CFGEdge *edge)
{
	...
	while (ptr != NULL) {
		...
		// if block is already in list: remove pred from its list of preds, decrement its count, remove it from
		// the list if resultant count is 0
...
TR_GeneralSinkStores::TR_GeneralSinkStores(TR::OptimizationManager *manager) : TR_SinkStores(manager)
{
...
TR_EdgeInformation *
TR_SinkStores::findEdgeInformation(TR::CFGEdge *edge, List<TR_EdgeInformation> &edgeList)
{
...
void
TR_SinkStores::recordPlacementForDefAlongEdge(TR_EdgeStorePlacement *edgePlacement)
{
	...
	if (trace())
		traceMsg(comp(),
		        "            RECORD placement along edge (%d->%d), for tt [" POINTER_PRINTF_FORMAT "] "
		        "(copy=%d)"
		        "\n",
	...
	if (_placementsForEdgesToBlock[toBlockNumber] != NULL) {
		...
		while (ptr != NULL) {
			...
			if (placementEdgeInfo != NULL) {
				...
				// NOTE that since stores are encountered in backwards order and "add" just puts new
				// stores at the beginning of the list then when we ultimately place the stores on this
				// edge, they will be stored in the correct order (i.e. a store X that originally
				// appeared before another store Y in the code will be on _placement->_stores list in
				// the order X, Y
	...
	if (ptr == NULL) {
		...
		if (from->isGotoBlock(comp())) {
			...
			if (_placementsForEdgesToBlock[toBlockNumber] == NULL)
				_placementsForEdgesToBlock[toBlockNumber] = new (trStackMemory())
				        TR_EdgeStorePlacementList(trMemory());
...
void
TR_SinkStores::recordPlacementForDefInBlock(TR_BlockStorePlacement *blockPlacement)
{
	...
	if (trace())
		traceMsg(comp(),
		        "            RECORD placement at beginning of block_%d for tt [" POINTER_PRINTF_FORMAT "] "
		        "(copy=%"
		        "d)\n",
	...
	// NOTE: we should already have visited these blocks, so we should only have to add these symbols to kills and
	// uses
	...
	TR_ASSERT(_symbolsKilledInBlock[blockNumber] != NULL, "_symbolsKilledInBlock[%d] should have been initialized!",
	        blockNumber);
	//_symbolsKilledInBlock[blockNumber] = new (trStackMemory()) TR_BitVector(_liveVarInfo->numLocals(),
	// trMemory());
	if (trace()) {
...
TR_SinkStores::performStoreSinking()
{
...

	{
		...
		// these arrays are used to maintain the stores placed in each block or in edges directed at each
		// particular block
		...
		if (sinkStoresWithIndirectLoads()) {
			...
			_indirectLoadAnchorMap = new (trStackMemory()) TR_HashTab(
			        comp()->trMemory(), stackAlloc, comp()->getFlowGraph()->getNextNodeNumber() / 2);
			if (trace())
				traceMsg(comp(), "creating _firstUseOfLoadMap with initSize = %d\n",
				        comp()->getFlowGraph()->getNextNodeNumber() / 4);
			_firstUseOfLoadMap = new (trStackMemory()) TR_HashTab(
			        comp()->trMemory(), stackAlloc, comp()->getFlowGraph()->getNextNodeNumber() / 4);
		...
		// lookForSinkableStores anyway (after the block loop is done).  Having whole block info like this is
		// probably not needed because we are always interested in just what is below the current node and not
		// in the whole block
		//
		//   if (usesDataFlowAnalysis())
		//      {
...
TR_SinkStores::lookForSinkableStores()
{
	...
	for (int32_t i = 0; i < cfg->getBackwardTraversalLength(); i++) {
		...
		if (block->getEntry() != NULL) {
			...
			do {
				// FIXME: It is legal in trivialStoreSinking to reset the foundException flag at the
				// start of each loop so return false for isExceptionFlagIsSticky() (more sinking
				// opportunities are found) Is it also legal to reset the flag given the more general
				// control flow found in generalStoreSinking?
				...
				// remove any commoned loads where the first reference to the load has been seen (it is
				// 'satisfied') satisfiedLiveCommonedLoads was initialized, if required, on the previous
				// node examined

				// remember the currently live commoned loads so that we can figure out later if we can
				// move this store
				(*savedLiveCommonedLoads) = (*_liveVarInfo->liveCommonedLoads());
				...
				_liveVarInfo->findLocalUsesInBackwardsTreeWalk(tt->getNode(), blockNumber, usedSymbols,
				        killedSymbols, treeCommonedLoads, treeVisitCount);
				if (trace()) {
				...
				// killedSymbols should have the same syms as _symbolsKilledInBlock[blockNumber] at this
				// point
				if (local != NULL) {
				...
				// check to see if we can find a store to a local that is used later in the EBB via a
				// commoned reference
				...
				while (bvi.hasMoreElements()) {
					...
					if (trace())
						traceMsg(comp(),
						        "      symbol stores to a live commoned reference, searching "
						        "stores below that load this symbol via commoned reference "
						        "...\n");
					_killMarkWalks++;

					// find any stores that use this symbol (via a commoned reference) and mark them
					// unmovable
					...
					while (storeElement != NULL) {
						...
						if (trace()) {
							...
							if (store->_movable && store->_commonedLoadsUnderTree
								...
								traceMsg(comp(),
								        "            killedSymIdx = %d and temp is not "
								        "needed\n",
								        killedSymIdx);
							}
						}
						if (store->_movable && store->_commonedLoadsUnderTree
							...
							if (store->_depth >= MIN_TREE_DEPTH_TO_MOVE) {
								if (!store->_needTempForCommonedLoads)
									store->_needTempForCommonedLoads = new (
									        trStackMemory())
									        TR_BitVector(numLocals, trMemory());
				...
				// When the commoned symbols are tracked precisely it is enough to run the above code
				// that intersects used with savedLiveCommonedLoads to find the first uses of symbols
				// With this information an anchor or temp store can be created when/if it is actually
				// needed in during sinkStorePlacement()
				bool usedSymbolKilledBelow = usedSymbols->intersects(*killedLiveCommonedLoads);
				if (usedSymbolKilledBelow) {
					TR_BitVector *firstRefsToKilledSymbols = new (trStackMemory())
					...
					while (storeElement != NULL) {
						...
						if (store->_movable && store->_needTempForCommonedLoads) {
							if (store->_needTempForCommonedLoads->intersects(
								...
								// looking at bit vectors alone may not be enough to
								// guarantee that a temp should actually be placed here
								// see the comment in isCorrectCommonedLoad for more
								// information
								TR_BitVector *commonedLoadsUsedAboveStore =
								        new (trStackMemory()) TR_BitVector(
								...
								if (trace()) {
									traceMsg(comp(),
									        "            found store "
									        "[" POINTER_PRINTF_FORMAT "] below "
									        "that may "
									        "require a "
									        "temp\n",
									        store->_useOrKillInfo->_tt->getNode());
									traceMsg(comp(),
									        "               killed live commoned "
									        "loads used above store:");
									commonedLoadsUsedAboveStore->print(comp());
									traceMsg(comp(), "\n");
								}
								genStoreToTempSyms(tt, node,
								        commonedLoadsUsedAboveStore,
				...
				if (local != NULL && node->getOpCode().isStoreDirect()) {
					...
					// It is not legal to sink AR symbols as exceptions points are implicit uses of
					// these symbols
					if (sym && sym->isMethodMetaData()
						...
						if (trace())
							traceMsg(
							        comp(), "         can't move store with side effect\n");
						canMoveStore = false;
					} else if (!storeIsSinkingCandidate(block, node, symIdx, false,
					                   indirectLoadCount, treeDepth, isLoadStatic, treeVisitCount,
					...
					// if we move this store, we will probably change the value of a symbol used
					// below it unless commoned syms are replaced by temps (saved value of the
					// commoned load)
					...
					if (canMoveStore && usedSymbols->intersects(*killedLiveCommonedLoads)) {
						// simple store-load tree won't be considered because of the overhead of
						// creating temp store
						if (treeDepth >= MIN_TREE_DEPTH_TO_MOVE) {
							moveStoreWithTemps = new (trStackMemory())
							        TR_BitVector(*usedSymbols);
							...
							if (trace())
								traceMsg(comp(),
								        "         store will not be moved because its "
				...
				// Note that separate lists are maintained for potentially movable stores and all other
				// trees that alter the used and killed symbols. This is done to avoid having to go
				// through the typically much larger used and killed list when we are only interested in
				// the potentially movable stores
				...
				// TR_MovableStore is used to track the specific information about the potential movable
				// store so that we can decide later on whether to move this store or not
				TR_MovableStore *movableStore = NULL;
				if (canMoveStore) {
					if (trace())
						traceMsg(comp(),
						        "      store is potentially movable, collecting commoned loads "
						        "and adding to list\n");
					TR_BitVector *myCommonedLoads = treeCommonedLoads->isEmpty()
					        ? 0
					        : new (trStackMemory()) TR_BitVector(*treeCommonedLoads);
					...
					if (!savedLiveCommonedLoads->isEmpty() && movedStorePinsCommonedSymbols()) {
						commonedLoadsAfter = new (trStackMemory())
						        TR_BitVector(*savedLiveCommonedLoads);
						(*commonedLoadsAfter) &= (*usedSymbols);
					}

					movableStore = new (trStackMemory()) TR_MovableStore(this, useOrKill, symIdx,
					        myCommonedLoads, commonedLoadsAfter, treeDepth, moveStoreWithTemps);
				...
				//   since the kill is below this use, even if we move a store higher up, this use will
				//   preserve he old value of the local
				...
				// must skip over any temp stores that were created for this treetop so we do not
				// attempt to sink these temp stores as well. The number of temp stores created for this
				// treetop is numTemps-prevNumTemps
		...
		if (!block->isExtensionOfPreviousBlock()) {
			...
			while (!useOrKillInfoList.isEmpty()) {
				...
				//_usedSymbolsToMove = useOrKill->_usedSymbols;  // this should be initialized in
				// sinkStorePlacement and after
				...
				if (isMovableStore) {
					...
					// Example potentiallyMovableStores list and useOrKillInfoList should look like
					// below relative to each other: useOrKillInfoList potentiallyMovableStores
					// head: UseOrKill_1             MovableStore_1
					//       UseOrKill_2             MovableStore_2
					//       MovableStore_1
					//       UseOrKill_3
					//       MovableStore_2
					// If the assume below is fired then the lists do not look like the above
					// example lists
					TR_ASSERT(store == useOrKill->_movableStore,
					        "mismatch in the order of nodes in potentiallyMovableStores and "
					        "useOrKillInfoList!");
					...
					if (performTransformation(comp(),
					            "%s Finding placements for store [" POINTER_PRINTF_FORMAT "] %s "
					            "with "
					            "tree "
					            "depth "
					            "%d\n",
						...
						if (sinkStorePlacement(store, movedPreviousStore)) {
							if (trace())
								traceMsg(comp(),
								        "        Successfully sunk past a non-live "
								        "path\n");
					...
					if (trace())
						traceMsg(comp(), " is not movable (isStore = %s)\n",
						        isStore ? "true" : "false");
					// if a once movable store was later marked as unmovable must remove it from the
					// list now
					if (isStore
				...
				if (movedStore) {
					// Even though the store is pushed outside of the Ebb, we should still set its
					// used sym to be used if it is commoned within the Ebb. For now, just set the
					// sym to be if it is commoned below so that stores from earlier node can push
					// thru this block. Perhaps a better way to do this is to allow earlier store to
					// be sunk along the Ebb path
					if (store->_commonedLoadsAfter) {
						if (usesDataFlowAnalysis()) {
							...
							if (0 && trace()) {
								traceMsg(comp(),
								        "      Update for movable store due to "
								        "commonedLoad: LOAP->blockInfo[%d]=",
								        blockNumber);
								_liveOnAllPaths->_blockAnalysisInfo[blockNumber]->print(
								        comp());
								traceMsg(comp(), "\n");
							}
						}
					...
					// we didn't move it, so add it to block's treeKilled set and add uses to
					// block's symbolsUsed set
					...
					TR_ASSERT(!isStore
					        ...
					        "_symbolsKilledInBlock[%d] is inconsistent with for this store "
					        "(symIdx=%d)!",
					        blockNumber, useOrKill->_movableStore->_symIdx);

					// need to update LOSP and LOAP as well, because the previously moved store may
					// wiped off some locals away which is not correct at *this* point and up
					if (usesDataFlowAnalysis() && (block->hasExceptionSuccessors() == false)) {
						...
						if (0 && trace()) {
							traceMsg(comp(),
							        ...
							        blockNumber);
							_liveOnSomePaths->_blockAnalysisInfo[blockNumber]->print(
							        comp());
							traceMsg(comp(), "\n");
							traceMsg(comp(),
...
TR_SinkStores::coalesceSimilarEdgePlacements()
{
	...
	while (firstEdgePtr != NULL) {
		...
		// such edges can be combined into a single placement and only one of the edges must be split (others
		// just redirected) not really expecting there to be a large number of edges on this list, so just do
		// the stupidest N^2 thing for now
		ListElement<TR_EdgeStorePlacement> *secondEdgePtr = firstEdgePtr->getNextElement();
		TR::Block *firstToBlock = firstEdge->getTo()->asBlock();

		if (trace()) {
			traceMsg(comp(),
			        "  Examining edge placement (%d,%d) with stores:", firstEdge->getFrom()->getNumber(),
			...
			while (firstStorePtr != NULL) {
				traceMsg(comp(), " [" POINTER_PRINTF_FORMAT "](copy=%d)",
				        firstStorePtr->getData()->_store->getNode(), firstStorePtr->getData()->_copy);
		...
		while (secondEdgePtr != NULL) {
			...
			if (trace()) {
				traceMsg(comp(), "    Comparing to edge placement (%d,%d) with stores:",
				        secondEdge->getFrom()->getNumber(), secondToBlock->getNumber());
			...
			if (secondToBlock->getNumber() == firstToBlock->getNumber()) {
				...
				// the stores should be added to the lists in the same order, so first mismatch in a
				// parallel walk is the exit test
				...
				// if both pointers are NULL, we walked the whole list so they're the same and we should
				// coalesce them
				if (firstStorePtr == NULL && secondStorePtr == NULL) {
					...
					if (trace())
						traceMsg(
						        comp(), "      store lists are different so cannot coalesce\n");
					lastPtr = secondEdgePtr;
				}
			} else {
				if (trace())
					traceMsg(comp(),
					        "      destination blocks are different (%d,%d) so cannot coalesce\n",
...
void
TR_SinkStores::placeStoresInBlock(List<TR_StoreInformation> &stores, TR::Block *placementBlock)
{
	// place stores into placementBlock
	// NOTE: this list should be properly ordered so that if a store X was earlier than another store Y in the
	// original code,
	...
	while (storePtr != NULL) {
		...
		if (copyStore) {
			...
			if (storePtr->getData()->_needsDuplication) // the tree may have already been duplicated in
			                                            // sinkStorePlacement
		...
		if (0 && trace())
			traceMsg(comp(),
			        "        PLACE new store [" POINTER_PRINTF_FORMAT "] (original store "
			        "[" POINTER_PRINTF_FORMAT "]) at "
			        "beginning "
			        "of "
			        "block_%"
			        "d\n",
...
void
TR_SinkStores::placeStoresAlongEdges(List<TR_StoreInformation> &stores, List<TR_EdgeInformation> &edges)
{
	TR::CFG *cfg = comp()->getFlowGraph();
	...
	if (!to->getExceptionPredecessors().empty()) {
		...
		// to->getOwningMethod()); wrong placementBlock->setHandlerInfo(0, to->getInlineDepth(),
		// to->getHandlerIndex(), to->getOwningMethod()); wrong
		// placementBlock->setHandlerInfo(to->getCatchType(), to->getInlineDepth(), 0, to->getOwningMethod());
		// fail modena 113380
		...
		//                                    TR::Node::create(firstStore->getNode(), TR::BBStart)),
		//                                    TR::TreeTop::create(comp(),
		...
		TR::Node *excpNode = TR::Node::createWithSymRef(
		        firstStore->getNode(), TR::aload, 0, symRefTab->findOrCreateExcpSymbolRef());
	...
	if (_indirectLoadAnchors) {
		...
		for (TR_IndirectLoadAnchor *loadAnchor = it.getFirst(); loadAnchor; loadAnchor = it.getNext()) {
			...
			if (anchoredNode->decFutureUseCount() == 0) {
				...
				//    - low enough that the child of the indirect load (the translateAddress node) has
				...
				if (trace())
					traceMsg(comp(),
					        "anchoring indirect load %p after node %p at start of block_%d anchor "
					        "tt node is %p\n",
		...
		if (!placement->_stores.isEmpty()) {
			...
			while (!placement->_stores.isEmpty()) {
				...
				if (storeInfo->_copy) {
					...
					if (trace())
						traceMsg(comp(),
						        "    adding store [" POINTER_PRINTF_FORMAT "] to movedStores "
						        "(block "
						        "placement)\n",
						        store);
					TR_ASSERT(!movedStores.find(store),
...
bool
TR_SinkStores::treeIsSinkableStore(TR::Node *node, bool sinkIndirectLoads, uint32_t &indirectLoadCount, int32_t &depth,
        bool &isLoadStatic, vcount_t visitCount)
{
	...
	if (numChildren == 0) {
		if (!(node->getOpCode().isLoadConst() || node->getOpCode().isLoadVarDirect())) //||
		                                                                               // node->getOpCodeValue()
		                                                                               //== TR::loadaddr))
		{
			if (trace())
		...
		if (node->getOpCode().isLoadIndirect()) {
			if (sinkIndirectLoads) {
				...
				if (trace())
					traceMsg(comp(),
					        "      found %s at %p so do not search children indirectLoadCount = "
					        "%d\n",
					        "indirect load", node, indirectLoadCount);
				return true; // indirectLoads are anchored in the mainline path so no need to search
				             // below them
			} else {
				if (trace()) {
					if (sinkStoresWithIndirectLoads())
		...
		if (node->getOpCode().isStoreDirect() && node->isPrivatizedInlinerArg()) {
			if (trace())
				traceMsg(
				        comp(), "         store is privatized inliner argument, not safe to move it\n");
			return false;
		}
	...
	for (int32_t c = 0; c < numChildren; c++) {
		...
		if (!treeIsSinkableStore(
		            child, sinkIndirectLoads, indirectLoadCount, childDepth, isLoadStatic, visitCount))
			return false;
		if (childDepth > depth)
			depth = childDepth;
	}
...
bool
TR_GeneralSinkStores::storeIsSinkingCandidate(TR::Block *block, TR::Node *node, int32_t symIdx, bool sinkIndirectLoads,
        ...
        int symIdx, TR::CFGNode *block, TR::CFGNode *succBlock, TR_BitVector *allEdgeInfoUsedOrKilledSymbols)
{
	...
	if (_placementsForEdgesToBlock[succBlockNumber] != NULL) {
		...
		for (TR_EdgeStorePlacement *placement = edgeIt.getFirst(); placement != NULL;
		        placement = edgeIt.getNext()) {
			// all edge placements should have exactly one edge at this point, so no need to iterate over
			// edges
...
bool
TR_SinkStores::isSymUsedInEdgePlacement(TR::CFGNode *block, TR::CFGNode *succBlock)
{
	int32_t succBlockNumber = succBlock->getNumber();
	if (_placementsForEdgesToBlock[succBlockNumber] != NULL) {
		ListIterator<TR_EdgeStorePlacement> edgeIt(_placementsForEdgesToBlock[succBlockNumber]);
			...
			if (from == block) {
				if (edgeInfo->_symbolsUsedOrKilled->intersects(*_killedSymbolsToMove)) {
					if (trace()) {
						...
						traceMsg(comp(), "\n");
						traceMsg(comp(),
						        "              Killed symbols used in store placement along "
						        "edge (%d->%d)\n",
...
bool
TR_SinkStores::shouldSinkStoreAlongEdge(int symIdx, TR::CFGNode *block, TR::CFGNode *succBlock,
        int32_t sourceBlockFrequency, bool isLoadStatic, vcount_t visitCount,
        TR_BitVector *allEdgeInfoUsedOrKilledSymbols)
{
	int32_t maxFrequency = (sourceBlockFrequency * 110) / 100; // store can be sunk along BBs with 10% tolerance in
	                                                           // hotness
...
bool
TR_SinkStores::checkLiveMergingPaths(TR_BlockListEntry *blockEntry, int32_t symIdx)
{
	...
	// TR_ASSERT(numPredsLONAP <= blockEntry->_count, "block has fewer LONAP predecessors than edges def was
	// propagated along");
...
void
TR_SinkStores::genStoreToTempSyms(TR::TreeTop *storeLocation, TR::Node *node, TR_BitVector *commonedLoadsUnderStore,
	...
	if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
		...
		// FIXME: does isCorrectCommonedLoad need to be called for all symbols or just TR_MethodMetaData symbols
		{
			...
			if (performTransformation(comp(),
				...
				storeLocation->insertBefore(store0_tt); // insert before as the current treetop may be
				                                        // the one killing the sym
				TR_HashId hashIndex = 0; // not used, initialized here to avoid compiler warning
				_tempSymMap->add(node, hashIndex, symRef0);
				_numTemps++;
			} else {
				// one or more potentially movable stores below may require that this temp store is
				// created. Find and mark these dependent movable stores as unmovable.
...
uint32_t
TR_SinkStores::insertAnchoredNodes(TR_MovableStore *store, TR::Node *nodeCopy, TR::Node *nodeOrig,
        ...
        List<TR_IndirectLoadAnchor> *indirectLoadAnchorsForThisStore, vcount_t visitCount)
{
	...
	//    anchored nodes. In this case, nodeParentCopy != NULL and all nodes must be visited so duplicated
	//    references within the tree are all replaced. To avoid very deep recursion MAX_TREE_DEPTH_TO_DUP is checked
	//    before calling this function.
	// 2. When creating anchors for the fall through store. In this case, nodeParentCopy == NULL and all nodes need
	// only be
	//    visited once because only a single anchor has to be created even if the fall through store tree has
	//    multiple references to the same node (the multiple references will naturally refer to the single anchored
	//    copy)
	...
	// In this case only anchors for blockingUsedSymbols and blockingCommonedSymbols need to be created as indirect
	// load nodes are naturally commoned correctly for the fall through sunk store
	if (indirectLoadAnchorsForThisStore && nodeOrig->getOpCode().isLoadIndirect()) {
		...
		if (trace())
			traceMsg(comp(),
			        "create loadAnchored for node %p at currentBlock %d with nodeOrig %p for store node "
			        "%p\n",
			        anchoredNode, currentBlock->getNumber(), nodeOrig,
			        store->_useOrKillInfo->_tt->getNode());
		indirectLoadAnchorsForThisStore->add(new (trStackMemory())
		                TR_IndirectLoadAnchor(anchoredNode, currentBlock, store->_useOrKillInfo->_tt));
		if (trace())
		...
		if (nodeParentCopy) {
			// if the node is being replaced then there is no need to search below because the side exit
			// tree being created can now not reference anything below this point in the fall through case
			// (where the node is not being replaced) then the children of this indirect load must be
	...
	*/
		...
		if (blockingUsedSymbols && blockingUsedSymbols->isSet(symIdx)) {
			...
			// A first use may already exist for a blocking used symbol load if this node itself is commoned
			// somewhere below
			if (!firstUseOfLoad) {
				firstUseOfLoad = new (trStackMemory()) TR_FirstUseOfLoad(nodeOrig,
				        store->_useOrKillInfo->_tt, store->_useOrKillInfo->_block->getNumber());
				TR_HashId addID = 0;
				_firstUseOfLoadMap->add(nodeOrig, addID, firstUseOfLoad);
				if (trace())
					traceMsg(comp(),
					        "      insertAnchoredNodes usedBlocked create/add for symIdx %d  "
					        "firstUse %p with node %p and anchor treetop\n",
					        symIdx, firstUseOfLoad, nodeOrig,
					        store->_useOrKillInfo->_tt->getNode());
			} else {
				if (trace())
					traceMsg(comp(),
					        "      insertAnchoredNodes usedBlocked found for symIdx %d firstUse %p "
					        "with node %p and anchor treetop %p\n",
		...
		if (firstUseOfLoad) {
			if (!firstUseOfLoad->isAnchored()) {
				...
				// This anchor creates a new use of symIdx in the anchoring block so update
				// _symbolsUsedInBlock so earlier stores are not incorrectly sunk past this anchor
				...
				if (trace()) {
					...
					traceMsg(comp(),
					        "               update _symbolsUsedInBlock[%d] for anchor set symIdx "
					        "%d\n",
			...
			if (nodeCopy) {
				TR_ASSERT(nodeParentCopy,
				        "if nodeCopy is not NULL then nodeParentCopy should be not NULL too\n");
				if (trace())
	...
	for (int32_t i = nodeOrig->getNumChildren() - 1; i >= 0; i--) {
		increment += insertAnchoredNodes(store, nodeCopy ? nodeCopy->getChild(i) : NULL, nodeOrig->getChild(i),
		        nodeCopy, i, needTempForCommonedLoads, blockingUsedSymbols, blockingCommonedSymbols,
		        currentBlock, indirectLoadAnchorsForThisStore, visitCount);
	}
	if (nodeCopy)
...
bool
TR_SinkStores::storeCanMoveThroughBlock(TR_BitVector *blockKilledSet, TR_BitVector *blockUsedSet, int symIdx,
        TR_BitVector *allBlockUsedSymbols, TR_BitVector *allBlockKilledSymbols)
{
	bool canMove = (blockKilledSet == NULL
	                       || (!blockKilledSet->intersects(*_usedSymbolsToMove) && !blockKilledSet->get(symIdx)))
	        && (blockUsedSet == NULL
	                || (!blockUsedSet->intersects(*_killedSymbolsToMove) && !blockUsedSet->get(symIdx)));
...
bool
TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore *movableStore, bool nextStoreWasMoved)
{
	...
	// allUsedSymbols will include the ones that are commoned if temp is not used; this is used for dataflow
	// analysis when the store is attempted to be sunk out of ebb
	if (commonedSymbols) {
	...
	if (!storeCanMoveThroughBlock(_symbolsKilledInBlock[sourceBlockNumber], _symbolsUsedInBlock[sourceBlockNumber],
		...
		if (trace())
			traceMsg(comp(),
			        "            TT[" POINTER_PRINTF_FORMAT "] Can't push sym %d to end of source "
			        "block_%d\n",
			        tt->getNode(), symIdx, sourceBlock->getNumber());
		return false;
		...
		if (trace())
			traceMsg(comp(),
			        "            TT[" POINTER_PRINTF_FORMAT "] LONAP is false for sym %d at the end of "
			        "source block_%d\n",
			        tt->getNode(), symIdx, sourceBlock->getNumber());
		return false;
	}
	...
	bool canSinkOutOfEBB = !commonedSymbols
	...
	for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext()) {
		...
		if (_liveOnSomePaths->_blockAnalysisInfo[succBlock->getNumber()]->get(symIdx)
			...
			if (!canSinkOutOfEBB && !isSuccBlockInEBB) {
				if (trace())
					traceMsg(comp(),
					        "              attempt to sink out of Ebb but commoned syms are killed "
			...
			if (shouldSinkStoreAlongEdge(symIdx, sourceBlock, succBlock, sourceBlockFrequency,
				...
				TR_StoreInformation *storeInfo = new (trStackMemory())
				        TR_StoreInformation(tt, copyStore);
				...
				TR_BitVector *symsReferenced = new (trStackMemory())
			...
			// Though symbol is not live on this path, it could be used in previous sink store recorded on
			// this edge. need check if symbol is used in this edge.
			if (isSymUsedInEdgePlacement(sourceBlock, succBlock)) {
				placeDefInBlock = true;
				if (trace())
					traceMsg(
					        comp(), "              symbol is used in early edge store placement\n");
				break;
			}
		}
	}
	...
	while (!worklist.isEmpty()) {
		...
		if (!checkLiveMergingPaths(blockEntry, symIdx)) {
			// def could not be propagated along some incoming LONAP edge, so propagating def to this block
			// will cause def
			...
			// NOTE: def should go along edge (not at end of pred) because each pred may have other
			// successors to which def was
			//       safely propagated...so if we put it at end of pred, then def may occur more than once
			//       along those edges (which may be unsafe)
			if (trace())
				traceMsg(comp(),
				        "              didn't reach from all live predecessors, splitting edges it did "
				        "reach on\n");
			...
			for (TR::CFGEdge *predEdgeLONAP = predItLONAP.getFirst(); predEdgeLONAP != NULL;
				...
				// recordPlacementForDefAlongEdge(tt, predEdgeLONAP, true); // block can't be an
				// extension of source block so copy
				TR_StoreInformation *storeInfo = new (trStackMemory())
				        TR_StoreInformation(tt, copyStore);
				// init _storeTemp field
				if (!needTempForCommonedLoads)
			...
			// and if we've reached this block, then any symbols used by the store will be live at the
			// beginning of this block the liveness of the symbols used by the store can be determined from
			...
			// NOTE: we check that none of the symbols used or killed by the code to move are used or killed
			// in the block
			//       this has an important sublety: it means that, if we allow the store to move through
			//       this block, then
			//        it is guaranteed that the store can move to all exception successors regardless of
			//        where in the block the exception flow occurs.  Otherwise, a store might be live at an
			//        exception successor from some exception points within the block but not others, which
			//        would force us to split the block to safely push the store to
			...
			if (_liveOnNotAllPaths->_inSetInfo[blockNumber]->get(symIdx)
			        && storeCanMoveThroughBlock(_symbolsKilledInBlock[blockNumber],
			                _symbolsUsedInBlock[blockNumber], symIdx, allBlockUsedSymbols,
				...
				// 2) it's safe to push this store to the end of block and the store is live on som
				// successor paths (but not all) so: push the store down to those successors where it is
				// live if we can't push it all the way to the beginning of the successor block, then
				// place def along the edge
				if (trace())
				...
				for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext()) {
					...
					if (_liveOnSomePaths->_blockAnalysisInfo[succBlockNumber]->get(symIdx)
						...
						if (!_sinkThruException && succBlock->asBlock()->isCatchBlock()
							...
							if (trace())
								traceMsg(comp(),
								        "              attempt to sink at the catch "
								        "block => don't sink\n");
							placeDefInBlock = true;
							break;
						}
						if (shouldSinkStoreAlongEdge(symIdx, block, succBlock,
						            sourceBlockFrequency, movableStore->_isLoadStatic,
						...
						//   }
							...
							if (!needTempForCommonedLoads)
								...
								replaceLoadsWithTempSym(
								        storeInfo->_storeTemp->getNode(), tt->getNode(),
								        needTempForCommonedLoads);
							}
							TR_BitVector *symsReferenced = new (trStackMemory())
							        TR_BitVector(*_killedSymbolsToMove);
				...
				// clean up all the mess that we have made: remove the edge placements and worklist
				// blocks previously added for successors
				if (placeDefInBlock) {
					...
					if (lastAddedEntry) {
						...
						for (auto tempSuccEdge = outEdges.getFirst(); tempSuccEdge;
							...
							worklist.removeBlockFromList(
							        tempSuccEdge->getTo()->asBlock(), tempSuccEdge);
					}
				}
			}
			...
			if (placeDefInBlock) {
				...
				// 2) store can move thru this block but the store has commoned load which is killed in
				// this block and
				//    we attempt to sink along a succ path which is out of this Ebb
				if (trace())
					traceMsg(comp(), "              store can't go through block\n");

				// be careful if sourceBlock and block are in the same extended block: there may be
				// commoned references
				//   in the store's tree that will get moved down if we make a copy of the store and
	...
	if (sunkPastNonLivePath || _usedSymbolsToMove->isEmpty() || _sinkAllStores) {
		...
		// because we moved the store across all these blocks, so liveness of lhs went away and liveness of rhs
		// entered NOTE: LONAP,LOAP, and LOSP will no longer be consistent after this, but all we need to ensure
		// is that
		//       no other store to any of those symbols will be sunk into any of these blocks
		//       It's true that we could miss an opportunity to move a store that should be LONAP because we
		//       moved *this* store, but let's leave that case on the table until there's a need to catch it
		while (!blocksVisitedWhileSinking.isEmpty()) {
			...
			bool isVisitedBlockInEBB =
			        (visitedBlock->startOfExtendedBlock() == sourceBlock->startOfExtendedBlock());
			...
			// remove 0 for more detail tracing
			if (0 && trace()) {
				traceMsg(comp(), "            Update for the movable store: LOSP->blockInfo[%d]",
				        visitedBlockNumber);
				_liveOnSomePaths->_blockAnalysisInfo[visitedBlockNumber]->print(comp());
				traceMsg(comp(), "\n");
				traceMsg(comp(), "            Update for the movable store: LOAP->blockInfo[%d]",
				        visitedBlockNumber);
