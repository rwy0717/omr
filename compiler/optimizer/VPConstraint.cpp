        ...
        OMR::ValuePropagation *vp, const char *sig, int32_t len, TR_ResolvedMethod *method, bool isFixedClass)
{
	...
	switch (sig[0]) {
	case 'L':
	case '[': return TR::VPClassType::create(vp, sig, len, method, isFixedClass);
	case 'B': return TR::VPIntRange::create(vp, TR::Int8, TR_no /* signed */);
	case 'Z':
		return TR::VPIntRange::create(vp, TR::Int8, TR_yes /* unsigned */); // FIXME: we can probably do 0/1
		                                                                    // here
	case 'C': return TR::VPIntRange::create(vp, TR::Int16, TR_yes /* unsigned */);
	case 'S': return TR::VPIntRange::create(vp, TR::Int16, TR_no /* signed */);
	}
	return NULL;
}
...
TR::VPShortConstraint *
TR::VPShortRange::create(OMR::ValuePropagation *vp, int16_t low, int16_t high, TR_YesNoMaybe canOverflow)
{
...
TR::VPShortConstraint *
TR::VPShortRange::createWithPrecision(OMR::ValuePropagation *vp, int32_t precision, bool isNonNegative)
{
	int64_t lo, hi;
...
TR::VPIntConstraint *
TR::VPIntRange::create(OMR::ValuePropagation *vp, int32_t low, int32_t high, TR_YesNoMaybe canOverflow)
{
	if (low == TR::getMinSigned<TR::Int32>() && high == TR::getMaxSigned<TR::Int32>())
		return NULL;

	//    if (isUnsigned &&
	//          ((uint32_t)low == TR::getMinUnsigned<TR::Int32>() && (uint32_t)high ==
...
TR::VPIntConstraint *
TR::VPIntRange::create(OMR::ValuePropagation *vp, TR::DataTypes dt, TR_YesNoMaybe isUnsigned)
{
	return TR::VPIntRange::createWithPrecision(vp, dt, VP_UNDEFINED_PRECISION, isUnsigned);
}

TR::VPIntConstraint *
TR::VPIntRange::createWithPrecision(
        OMR::ValuePropagation *vp, TR::DataType dt, int32_t precision, TR_YesNoMaybe isUnsigned, bool isNonNegative)
{
	...
	if (isUnsigned == TR_no) {
		if (dt == TR::Int8)
			constrainRangeByPrecision(TR::getMinSigned<TR::Int8>(), TR::getMaxSigned<TR::Int8>(), precision,
			        lo, hi, isNonNegative);
		else if (dt == TR::Int16)
			constrainRangeByPrecision(TR::getMinSigned<TR::Int16>(), TR::getMaxSigned<TR::Int16>(),
			        precision, lo, hi, isNonNegative);
		...
		if (dt == TR::Int8)
			constrainRangeByPrecision(TR::getMinSigned<TR::Int8>(), TR::getMaxUnsigned<TR::Int8>(),
			        precision, lo, hi, isNonNegative);
		else if (dt == TR::Int16)
			constrainRangeByPrecision(TR::getMinSigned<TR::Int16>(), TR::getMaxUnsigned<TR::Int16>(),
			        precision, lo, hi, isNonNegative);
	}
...
TR::VPConstraint *
TR::VPClass::create(OMR::ValuePropagation *vp, TR::VPClassType *type, TR::VPClassPresence *presence,
...
TR::VPClassType *
TR::VPClassType::create(OMR::ValuePropagation *vp, const char *sig, int32_t len, TR_ResolvedMethod *method,
        bool isFixed, TR_OpaqueClassBlock *classObject)
{
#ifdef J9_PROJECT_SPECIFIC
	if (!classObject)
...
TR::VPResolvedClass *
TR::VPResolvedClass::create(OMR::ValuePropagation *vp, TR_OpaqueClassBlock *klass)
{
...
TR::VPKnownObject *
TR::VPKnownObject::create(OMR::ValuePropagation *vp, TR::KnownObjectTable::Index index, bool isJavaLangClass)
{
	...
	if (!knot)
		return NULL;
	if (knot->isNull(index)) // No point in worrying about the NULL case because existing constraints handle that
	                         // optimally
		return NULL;
...
TR::VPConstString *
TR::VPConstString::create(OMR::ValuePropagation *vp, TR::SymbolReference *symRef)
{
#ifdef J9_PROJECT_SPECIFIC
	...
	if (vpConstStringCriticalSection.hasVMAccess()) {
		...
		for (entry = vp->_constraintsHashTable[hash]; entry; entry = entry->next) {
			...
			if (constraint) {
				uintptrj_t constraintStaticAddr = (uintptrj_t)constraint->_symRef->getSymbol()
				                                          ->castToStaticSymbol()
				                                          ->getStaticAddress();
				if (string
				        == vp->comp()->fej9()->getStaticReferenceFieldAtAddress(constraintStaticAddr)) {
					return constraint;
				}
			}
		}
		constraint = new (vp->trStackMemory())
		        TR::VPConstString(vp->comp()->getStringClassPointer(), vp->comp(), symRef);
		vp->addConstraint(constraint, hash);
		return constraint;
	}
#endif
...
TR::VPConstString::charAt(int32_t i, TR::Compilation *comp)
{
	...
	if (charAtCriticalSection.hasVMAccess()) {
		uintptrj_t stringStaticAddr =
		        (uintptrj_t)_symRef->getSymbol()->castToStaticSymbol()->getStaticAddress();
...
bool
TR::VPConstString::getFieldByName(TR::SymbolReference *symRef, void *&val, TR::Compilation *comp)
{
	return TR::Compiler->cls.getStringFieldByName(comp, _symRef, symRef, val);
}

TR::VPUnresolvedClass *
TR::VPUnresolvedClass::create(OMR::ValuePropagation *vp, const char *sig, int32_t len, TR_ResolvedMethod *method)
{
	// If the constraint does not already exist, create it
	//
...
TR::VPPreexistentObject *
TR::VPPreexistentObject::create(OMR::ValuePropagation *vp, TR_OpaqueClassBlock *c)
{
...
TR::VPArrayInfo *
TR::VPArrayInfo::create(OMR::ValuePropagation *vp, int32_t lowBound, int32_t highBound, int32_t elementSize)
{
	// If the constraint does not already exist, create it
	//
...
TR::VPMergedConstraints *
TR::VPMergedConstraints::create(OMR::ValuePropagation *vp, TR::VPConstraint *first, TR::VPConstraint *second)
{
	// If the constraint does not already exist, create it
	//
...
TR::VPObjectLocation *
TR::VPObjectLocation::create(OMR::ValuePropagation *vp, VPObjectLocationKind kind)
{
...
TR::VPMergedConstraints *
TR::VPMergedConstraints::create(OMR::ValuePropagation *vp, ListElement<TR::VPConstraint> *list)
{
	// If the constraint does not already exist, create it
	//
	int32_t hash = 0;
...
TR::VPConstraint *
TR::VPShortConstraint::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPMergedConstraints::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPMergedConstraints::shortMerge(
...
TR::VPConstraint *
TR::VPMergedConstraints::intMerge(
        TR::VPConstraint *other, ListElement<TR::VPConstraint> *otherNext, OMR::ValuePropagation *vp)
{
...
TR::VPConstraint *
TR::VPMergedConstraints::longMerge(
        TR::VPConstraint *other, ListElement<TR::VPConstraint> *otherNext, OMR::ValuePropagation *vp)
{
	TR_ScratchList<TR::VPConstraint> result(vp->trMemory());
	ListElement<TR::VPConstraint> *next = _constraints.getListHead();
				...
				if (lastResultEntry) {
					...
					if (cur
						...
						if (lastResult->getHigh() == TR::getMaxSigned<TR::Int64>()
							...
							lastResultEntry =
							        result.addAfter(otherCurLong, lastResultEntry);
						}
						if (otherNext) {
							otherCur = otherNext->getData()->asIntConstraint();
							// TR_ASSERT(otherCur, "Expecting long constraints in
							// longMerge");
							otherNext = otherNext->getNextElement();
						} else
							otherCur = NULL;
...
TR::VPConstraint *
TR::VPLessThanOrEqual::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPGreaterThanOrEqual::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint::intersect(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	...
	if (!other) {
		if (vp->trace())
			traceMsg(vp->comp(),
			        "setIntersectionFailed to true because NULL constraint found this = 0x%p, other = "
			        "0x%p\n",
			        this, other);
...
TR::VPConstraint *
TR::VPShortConstraint::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPIntConstraint::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPLongConstraint::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
			...
			if (TR::VPConstraint::isSpecialClass((uintptrj_t)type->getClass()))
				...
				if (_location && _location->isClassObject() == TR_yes) {
					if (otherType->asResolvedClass()) {
						...
						// e.g. <fixedClass, classObject> with <resolvedClass, classObject>
							...
							if (rc
							        && rc->getClass()
							                == vp->fe()->getClassClassPointer(
							                        rc->getClass())) {
								if (vp->trace()) {
									traceMsg(vp->comp(),
									        "   Current type is a class object\n");
									this->print(
									        vp->comp(), vp->comp()->getOutFile());
									traceMsg(vp->comp(), "\n");
								}
				...
				// e.g. <fixedClass, non-null> with <resolvedClass, classObject>
					...
					if (rc && rc->getClass() == vp->fe()->getClassClassPointer(rc->getClass())) {
						if (vp->trace()) {
							traceMsg(vp->comp(),
							        "   2Intersecting type is a class object\n");
			...
			// e.g. <fixedClass, classObject> with <resolvedClass>
				...
				if (otherType && otherType->asResolvedClass()) {
					...
					if (rc->getClass() == vp->fe()->getClassClassPointer(rc->getClass())) {
						if (vp->trace()) {
							traceMsg(
							        vp->comp(), "   Intersecting type is a class object\n");
							otherType->print(vp->comp(), vp->comp()->getOutFile());
							traceMsg(vp->comp(), "\n");
						}
...
TR::VPConstraint *
TR::VPResolvedClass::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
	...
	if (otherRes) {
		...
		if (classObject != TR_maybe) {
			location = TR::VPObjectLocation::create(vp,
...
TR::VPConstraint *
TR::VPKnownObject::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
	...
	if (otherKnownObject) {
		if (getIndex() == otherKnownObject->getIndex())
			return other; // Provably the same object.  Return "other" because it is no less strict than
			              // "this"
		else
...
TR::VPConstraint *
TR::VPConstString::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
#ifdef J9_PROJECT_SPECIFIC
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPUnresolvedClass::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
	...
	if (other->asClassPresence()) {
		...
		return TR::VPClass::create(vp, this, NULL, NULL, NULL, other->asObjectLocation()); // FIXME: unless
		                                                                                   // rtresolve, should
		                                                                                   // be on heap
	return this;
}
...
TR::VPConstraint *
TR::VPNonNullObject::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPPreexistentObject::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPObjectLocation::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPMergedConstraints::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPMergedConstraints::intIntersect(
        TR::VPConstraint *other, ListElement<TR::VPConstraint> *otherNext, OMR::ValuePropagation *vp)
{
	...
	if (otherCur) {
		...
		while (cur && otherCur) {
			...
			if ((int64_t)curHigh < otherLow)
				...
				// Put the intersection of the two current ranges into the result list
				//
				int32_t resultLow = ((int64_t)curLow > (int64_t)otherLow) ? (int32_t)curLow
				                                                          : (int32_t)otherLow;
				int32_t resultHigh = ((int64_t)curHigh < (int64_t)otherHigh) ? (int32_t)curHigh
				                                                             : (int32_t)otherHigh;
				lastResultEntry = result.addAfter(
				        TR::VPIntRange::create(vp, resultLow, resultHigh), lastResultEntry);
...
TR::VPConstraint *
TR::VPMergedConstraints::longIntersect(
        TR::VPConstraint *other, ListElement<TR::VPConstraint> *otherNext, OMR::ValuePropagation *vp)
{
	TR_ScratchList<TR::VPConstraint> result(vp->trMemory());
	ListElement<TR::VPConstraint> *next = _constraints.getListHead();
...
TR::VPConstraint *
TR::VPLessThanOrEqual::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPGreaterThanOrEqual::intersect1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TRACER(vp, this, other);
...
TR::VPConstraint *
TR::VPConstraint::add(TR::VPConstraint *other, TR::DataType type, OMR::ValuePropagation *vp)
{
	return NULL;
}

TR::VPConstraint *
TR::VPConstraint::subtract(TR::VPConstraint *other, TR::DataType type, OMR::ValuePropagation *vp)
{
	return NULL;
}

TR::VPConstraint *
TR::VPShortConstraint::add(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
	TR::VPShortConstraint *otherShort = other->asShortConstraint();
	if (!otherShort)
		return NULL;
...
TR::VPConstraint *
TR::VPIntConstraint::add(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
...
TR::VPConstraint *
TR::VPShortConstraint::subtract(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
	TR::VPShortConstraint *otherShort = other->asShortConstraint();
...
TR::VPConstraint *
TR::VPIntConstraint::subtract(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
	TR::VPIntConstraint *otherInt = other->asIntConstraint();
	if (!otherInt)
		return NULL;
...
TR::VPConstraint *
TR::VPShortConstraint::getRange(
        int16_t low, int16_t high, bool lowCanOverflow, bool highCanOverflow, OMR::ValuePropagation *vp)
{
	// This function returns a VP constraint range, with the appropriate setting of the canOverflow bit, for either
	// an addition or subtraction operation. The function takes the results of the lower- and upper-bound
	// calculations for the operation, both performed in the same precision arithmetic as that of the underlying
	// operation, along with two flags indicated whether or not the operations producing these two bounds
	// overflowed. From these values, a constraint can be created, with the overflow flags being set as
	...
	if (lowCanOverflow && highCanOverflow) { // Both sides overflow
		...
		if ((high ^ low) < 0)
			// high and low have different signs, thus high has overflowed above range, low has overflowed
			// below range, result is full range
			return NULL;
...
TR::VPConstraint *
TR::VPLongConstraint::add(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
	// TODO - handle add and subtract for merged constraints
	//
	TR::VPLongConstraint *otherLong = other->asLongConstraint();
...
TR::VPConstraint *
TR::VPLongConstraint::subtract(TR::VPConstraint *other, TR::DataType dt, OMR::ValuePropagation *vp)
{
	TR::VPLongConstraint *otherLong = other->asLongConstraint();
	if (!otherLong)
		return NULL;
...
bool
TR::VPConstraint::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return false;
}

bool
TR::VPShortConst::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
TR::VPClass::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	...
	if (!_preexistence && !_arrayInfo && _type && _type->isFixedClass() && isNonNullObject() && other && otherClass
	        ...
	        && otherClass->getClassType()->isFixedClass() && otherClass->isNonNullObject()
	        && (isClassObject() == TR_yes) && (other->isClassObject() == TR_yes)) {
...
bool
TR::VPNullObject::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return other->isNullObject();
}

bool
TR::VPConstString::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
bool
TR::VPKnownObject::mustBeEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
bool
TR::VPKnownObject::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
bool
TR::VPConstString::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (other->getKnownObject())
...
bool
TR::VPConstraint::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (isNullObject()) {
		if (other->isNonNullObject())
			return true;
...
bool
TR::VPShortConstraint::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TR::VPShortConstraint *otherShort = other->asShortConstraint();
	if (otherShort) {
...
bool
TR::VPIntConstraint::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TR::VPIntConstraint *otherInt = other->asIntConstraint();
	if (otherInt) {
...
bool
TR::VPLongConstraint::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	TR::VPLongConstraint *otherLong = other->asLongConstraint();
	if (otherLong)
...
bool
TR::VPMergedConstraints::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
bool
TR::VPNullObject::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return other->isNonNullObject();
}

bool
TR::VPNonNullObject::mustBeNotEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return other->isNullObject();
}

bool
TR::VPConstraint::mustBeLessThan(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return false;
}

bool
TR::VPShortConstraint::mustBeLessThan(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
...
bool
TR::VPIntConstraint::mustBeLessThan(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (isUnsigned() && other->isUnsigned())
...
bool
TR::VPLongConstraint::mustBeLessThan(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return getHigh() < other->getLowLong();
}

bool
TR::VPMergedConstraints::mustBeLessThan(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (_type.isInt16()) {
...
bool
TR::VPConstraint::mustBeLessThanOrEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return false;
}

bool
TR::VPShortConstraint::mustBeLessThanOrEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (isUnsigned() && other->isUnsigned())
		return ((uint16_t)getHigh() <= (uint16_t)other->getLowShort());
	return getHigh() <= other->getLowShort();
...
bool
TR::VPIntConstraint::mustBeLessThanOrEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (isUnsigned() && other->isUnsigned())
		return ((uint32_t)getHigh() <= (uint32_t)other->getLowInt());
	return getHigh() <= other->getLowInt();
...
bool
TR::VPLongConstraint::mustBeLessThanOrEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	return getHigh() <= other->getLowLong();
}

bool
TR::VPMergedConstraints::mustBeLessThanOrEqual(TR::VPConstraint *other, OMR::ValuePropagation *vp)
{
	if (_type.isInt64())
		return getHighLong() <= other->getLowLong();
	if (_constraints.getLastElement()->getData()->isUnsigned())
...
TR::VPConstraint *
TR::VPRelation::propagateAbsoluteConstraint(TR::VPConstraint *constraint, int32_t relative, OMR::ValuePropagation *vp)
{
...
TR::VPConstraint *
TR::VPLessThanOrEqual::propagateAbsoluteConstraint(
...
TR::VPConstraint *
TR::VPEqual::propagateAbsoluteConstraint(TR::VPConstraint *constraint, int32_t relative, OMR::ValuePropagation *vp)
{
...
TR::VPConstraint *
TR::VPNotEqual::propagateAbsoluteConstraint(TR::VPConstraint *constraint, int32_t relative, OMR::ValuePropagation *vp)
{
	// x != y + I and x == N           ==> y != (N-I)
	//
	if (vp->trace()) {
	...
	if (constraint->asLongConst()) {
		...
		if (excludedValue != TR::getMaxSigned<TR::Int64>()) {
			if (newConstraint)
				newConstraint = newConstraint->merge(
				        TR::VPLongRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int64>()),
				        vp);
			else
				newConstraint =
				        TR::VPLongRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int64>());
		...
		//             {
		//             newConstraint = TR::VPIntRange::create(vp, TR::getMinUnsigned<TR::Int32>(),
		//             excludedValue-1, true);
		//             }
		...
		if (excludedValue != TR::getMaxSigned<TR::Int32>()) {
			if (newConstraint)
				newConstraint = newConstraint->merge(
				        TR::VPIntRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int32>()),
				        vp);
			else
				newConstraint =
				        TR::VPIntRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int32>());
...
TR::VPConstraint *
TR::VPLessThanOrEqual::propagateRelativeConstraint(
        TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp)
{
	...
	// newIncr cannot be TR::getMinSigned<TR::Int32>(), because when we try to negate TR::getMinSigned<TR::Int32>(),
	// we get TR::getMinSigned<TR::Int32>() again
...
TR::VPConstraint *
TR::VPGreaterThanOrEqual::propagateRelativeConstraint(
        TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp)
{
	// x >= y + M and x <= z + N    ==> y <= z + (N-M)
	//
...
TR::VPConstraint *
TR::VPEqual::propagateRelativeConstraint(
        TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp)
{
	// x == y + M and x <= z + N    ==> y <= z + (N-M)
	// x == y + M and x >= z + N    ==> y >= z + (N-M)
	...
	if (other->asLessThanOrEqual()) {
		...
		if (vp->trace()) {
			traceMsg(vp->comp(), "      Propagating V == value %d %+d and V <= value %d %+d", relative,
			        increment(), otherRelative, other->increment());
			traceMsg(vp->comp(), " ... value %d <= value %d %+d\n", relative, otherRelative, newIncr);
		}
...
TR::VPConstraint *
TR::VPNotEqual::propagateRelativeConstraint(
        TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp)
{
	// x != y + M and x == z + N    ==> y != z + (N-M)
	//
...
void
TR::VPConstraint::print(TR::Compilation *comp, TR::FILE *outFile, int32_t relative)
{
	if (outFile == NULL)
		return;
...
void
TR::VPLessThanOrEqual::print(TR::Compilation *comp, TR::FILE *outFile, int32_t relative)
{
	if (outFile == NULL)
		return;
...
void
TR::VPGreaterThanOrEqual::print(TR::Compilation *comp, TR::FILE *outFile, int32_t relative)
{
	if (outFile == NULL)
		return;
...
void
TR::VPNotEqual::print(TR::Compilation *comp, TR::FILE *outFile, int32_t relative)
{
	if (outFile == NULL)
		return;
