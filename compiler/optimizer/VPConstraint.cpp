    ...
    int16_t low, int16_t high, bool lowCanOverflow, bool highCanOverflow, OMR::ValuePropagation* vp)
{
    ...
    // the operation, both performed in the same precision arithmetic as that of the underlying operation, along with two
    // flags indicated whether or not the operations producing these two bounds overflowed. From these values, a
    ...
    TR::VPConstraint* constraint, int32_t relative, OMR::ValuePropagation* vp)
{
    ...
    if (constraint->asLongConst()) {
        ...
        if (excludedValue != TR::getMaxSigned<TR::Int32>()) {
            if (newConstraint)
                newConstraint = newConstraint->merge(
                    TR::VPIntRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int32>()), vp);
            else
