...
public:
	...
	virtual TR_Structure *cloneStructure(
	        TR::Block **, TR_StructureSubGraphNode **, List<TR_Structure> *, List<TR_Structure> *) = 0;
	...
	void setNestingDepth(int16_t f)
	{
		if (f > SHRT_MAX - 1) {
			...
			comp()->failCompilation<TR::CompilationException>("nesting depth must be less than or equal to "
			                                                  "SHRT_MAX-1");
		}
		_nestingDepth = f;
	}
	...
	void setMaxNestingDepth(int16_t f)
	{
		if (f > SHRT_MAX - 1) {
			...
			comp()->failCompilation<TR::CompilationException>("max nesting depth must be less than or "
			                                                  "equal to SHRT_MAX-1");
		}
		_maxNestingDepth = f;
	}
	...
	virtual void hoistInvariantsOutOfNestedLoops(
	        TR_LocalTransparency *, TR_BitVector **, bool, TR_BlockStructure *, TR_RegionStructure *, int32_t) = 0;
...
public:
	...
	TR_StructureSubGraphNode(TR_Structure *s)
	        : TR::CFGNode(s->getNumber(), s->cfg()->structureRegion()), _structure(s)
	{
		s->setSubGraphNode(this);
	}

	TR_StructureSubGraphNode(int32_t n, TR::Region &region) : TR::CFGNode(n, region), _structure(0) {}
...
private:
	...
	TR_YesNoMaybe _isSigned; // FIXME// CRITICAL Value Propgation should set this up.. right now this is not being
	                         // done
};
