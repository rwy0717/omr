...
void
TR_GlobalAnticipatability::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *, TR_BitVector *)
{}
...
static bool
nodeCanSurvive(TR::Node *nextNode, TR::Node *lastNodeFirstChild, TR::Node *lastNodeSecondChild, TR::Compilation *comp,
        bool trace)
{
	if (isFieldAccess(nextNode)) {
		...
		if (seenSimilarAccess) {
			...
			int32_t otherLen;
			const char *otherSig = otherSymRef->getOwningMethod(comp)->classNameOfFieldOrStatic(
...
bool
TR_GlobalAnticipatability::isExceptionalInBlock(
        TR::Node *node, int32_t blockNumber, ContainerType *alreadyInBlock, vcount_t visitCount)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		if (isExceptionalInBlock(child, blockNumber, alreadyInBlock, visitCount)) {
			node->setVisitCount(visitCount - 1); // so that node gets visited again for another expression
			                                     // in this block
			return true;
		}
	}
...
//
void
TR_GlobalAnticipatability::analyzeTreeTopsInBlockStructure(TR_BlockStructure *blockStructure)
{
	...
	if (block != comp()->getFlowGraph()->getEnd()) {
		...
		if (analyzedSucc) {
			...
			{
				_scratch->empty();
				for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end();
				        ++edge) {
					next = toBlock((*edge)->getTo());
					ExtraAnalysisInfo *analysisInfo = getAnalysisInfo(next->getStructureOf());
					bool rare = isRareEdge(comp(), *edge);
					...
					if (!rare && blockWeight > 1) {
						dumpOptDetails(comp(),
						        "Ignoring ifcmp within loop in block_%d and adding available "
						        "expressions from block_%d\n",
