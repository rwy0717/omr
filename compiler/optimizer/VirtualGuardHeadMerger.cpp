 ...
 */
static bool
anchorCommonNodes(TR::Compilation *comp, TR::Node *node, TR::TreeTop *tt, TR::NodeChecklist &checklist)
{
 ...
 */
static void
collectDirectLoads(TR::Node *node, TR_BitVector &loadSymRefs, TR::NodeChecklist &checklist)
{
	if (checklist.contains(node))
		return;
	checklist.add(node);
			...
			if (tt->getNode()->chkIsPrivatizedInlinerArg()
			        && (disablePrivArgMovement ||
			                ...
			                (guardCandidate->getNode()->getInlinedSiteIndex() > -1 &&
			                        // if priv arg store does not have the same inlined site index as the
			                        // guard's caller, that means it is not a priv arg for this guard, then
			                        // we cannot move the guard and its priv args up across other calls'
			                        // priv args
			                        tt->getNode()->getInlinedSiteIndex()
			                                != TR::comp()
			                                           ->getInlinedCallSite(guardCandidate->getNode()
			                                                                        ->getInlinedSiteIndex())
	...
	for (TR::Block *block = optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock(); block;
		...
		if (isMergeableGuard(guard1)) {
			...
			if (!disableVGHeadMergerTailSplitting && (cold1->getSuccessors().size() == 1)
				...
				if ((block->getNextBlock()->getSuccessors().size() == 1
				        ...
				        && performTransformation(comp(),
				                "%sCloning block_%d and placing clone after block_%d to reduce HCR "
				                "guard nops\n",
			...
			for (TR::Block *nextBlock = block->getNextBlock(); nextBlock;
				...
				// It is not possible to shift an OSR guard unless the destination is already an OSR
				// point as the necessary OSR state will not be available
				if (guard2->isOSRGuard() && !guard1->isOSRGuard())
					break;

				TR::Block *insertPoint = isStopTheWorldGuard(guard2) ? HCRIns : runtimeIns;
				if (!safeToMoveGuard(
				            insertPoint, guard2Tree, guard1->getBranchDestination(), privArgSymRefs))
					break;
				...
				if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
					...
					if (trace())
						traceMsg(comp(),
						        "  Guard1 [%p] is the caller of Guard2 [%p] - proceeding with "
						        "guard merging\n",
						        guard1, guard2);
					...
					if (trace())
						traceMsg(comp(),
						        ...
						        "destination block_%d as its only successor - proceeding with "
						        "guard merging\n",
						        cold1->getNumber(), cold2->getNumber());
				} else {
					if (trace())
						traceMsg(comp(),
						        "  Cold1 block_%d and cold2 block_%d of guard2 [%p] in unknown "
						        "relationship - abandon the merge attempt\n",
						        cold1->getNumber(), cold2->getNumber(), guard2);
					break;
				}

				// Runtime guards will shift their privargs, so it is necessary to check such a move is
				// safe This is possible if a privarg temp was recycled for the inner call site, with a
				// prior use as an argument for the outer call site. As the privargs for the inner call
				// site must be evaluated before both guards, this would result in the recycled temp
				...
				if (!performTransformation(comp(),
				            ...
				            OPT_DETAILS, isStopTheWorldGuard(guard2) ? "stop the world" : "runtime",
				...
				if (guard2Tree != guard2Block->getFirstRealTreeTop()) {
					...
					// 1, it might have side effect to runtime guards after it, moving it up might
					// cause us to falsely merge
					//    the subsequent runtime guards
					// 2, it might contain live monitor, moving it up above a guard can affect the
					// monitor's live range
					if (!isStopTheWorldGuard(guard2)) {
						// the block created above guard2 contains only privarg treetops or
						// monitor stores if guard2 is a runtime-patchable guard and is safe to
						// merge. We need to move the priv args up to the runtime insert point
						// and leave the monitor stores in place It's safe to do so because
						// there is no data dependency between the monitor store and the priv
						// arg store, because the priv arg store does not load the value from
						// the temp holding the monitored object
						...
						if (privargBlock != guard2Block) {
							...
							// refresh the insertPoint since it could be stale after the
							// above updates
...
TR_VirtualGuardHeadMerger::tailSplitBlock(TR::Block *block, TR::Block *cold1)
{
	...
	if (tailSplitEnd->getOpCode().isGoto()) {
		...
		gotoBlock->append(TR::TreeTop::create(comp(),
		        TR::Node::create(
		                tailSplitEnd, TR::Goto, 0, block->getNextBlock()->getExit()->getNextTreeTop())));
