            ...
            if (tt->getNode()->chkIsPrivatizedInlinerArg()
                && (disablePrivArgMovement ||
                       ...
                       (guardCandidate->getNode()->getInlinedSiteIndex() > -1 &&
                           // if priv arg store does not have the same inlined site index as the guard's caller, that
                           // means it is not a priv arg for this guard, then we cannot move the guard and its priv args
                           // up across other calls' priv args
