...
TR::Node* constrainAnyIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (dataType == TR::Int16 && node->getOpCode().isIndirect() && node->getSymbol()->isArrayShadowSymbol()
        ...
        if (index->getOpCode().isLoadConst() && array->getOpCode().isIndirect()) {
            ...
            if (baseVPConstraint && baseVPConstraint->isConstString()) {
                ...
                uint16_t ch
                    = constString->charAt(((TR::Compiler->target.is64Bit() ? index->getLongIntLow() : index->getInt())
                    ...
                    if (classOfStatic == NULL) {
                        ...
                        if (classNameOfFieldOrStatic) {
                            ...
                            TR_OpaqueClassBlock* curClass = vp->fe()->getClassFromSignature(
                                classNameOfFieldOrStatic, len, symRef->getOwningMethod(vp->comp()));
                    ...
                    if (classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
                        ...
                        if (constrainAloadCriticalSection.hasVMAccess()) {
                            ...
                            uintptrj_t arrayObject
                                = vp->comp()->fej9()->getStaticReferenceFieldAtAddress(arrayStaticAddress);
                            if (arrayObject != 0) {
                                ...
                                if (firstDimension >= 0) {
                                    ...
                                    if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                                        ...
                                        if (elementSize != 0) {
                                            if (vp->trace())
                                                traceMsg(vp->comp(),
                                                    "Using class lookahead info to find out non null, array dimension, "
                                                    "and object location\n");
                                            vp->addGlobalConstraint(node,
                    ...
                    if (jlClass) {
                        if (classBlock != jlClass) {
                            constraint = TR::VPClassType::create(
                                vp, sig, len, symRef->getOwningMethod(vp->comp()), isFixed, classBlock);
                            if (*sig == '[' || sig[0] == 'L') {
                                int32_t elementSize = arrayElementSize(sig, len, node, vp);
                                if (elementSize != 0) {
        ...
        if (base && node->getOpCode().hasSymbolReference()
            ...
            if (knot && symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
                ...
                TR::KnownObjectTable::Index knownObjectIndex = knot->getIndexAt(
                    (uintptrj_t*)(base->getClass() + fej9->getOffsetOfJavaLangClassFromClassField()));
                vp->addBlockOrGlobalConstraint(node,
                    TR::VPClass::create(vp, TR::VPKnownObject::createForJavaLangClass(vp, knownObjectIndex),
                        TR::VPNonNullObject::create(vp), NULL, NULL,
                        TR::VPObjectLocation::create(vp, TR::VPObjectLocation::JavaLangClassObject)),
                ...
                if (callSiteConstraint && callSiteConstraint->getKnownObject()) {
                    ...
                    if (bypassOnThisRun) {
                        ...
                        if (bypassLocation
                            ...
                            node->setSymbolReference(vp->comp()->getSymRefTab()->createKnownStaticRefereneceSymbolRef(
                ...
                if (sym->isStatic() && sym->isFinal()) {
                    ...
                    if ((classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
                        ...
                        if (constrainIaloadCriticalSection.hasVMAccess()) {
                            ...
                            if (arrayObject != 0) {
                                /*
                                 * This line is left commented because of the commented lines further below which make
                                 * use of the arrLength variable. This can be removed once it is determined that the
                                 * commented out lines below will never be needed.
                                 */
                                // int32_t arrLength = TR::Compiler->om.getArrayLengthInElements(vp->comp(),
                                // arrayObject);
                                ...
                                if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                                    ...
                                    if (elementSize != 0) {
                                        // vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
                                        // vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp, arrLength,
                                        // arrLength, elementSize));
                                        foundInfo = true;
                                        isFixed = true;
                                    }
                                }
...
void canRemoveWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    TR::VPConstraint* constraint = vp->getConstraint(node, isGlobal);
    if (constraint) {
        if (constraint->isNullObject() && !vp->comp()->getOptions()->alwaysCallWriteBarrier()
...
TR::Node* constrainWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (doOpt && ((gcMode == TR_WrtbarCardMarkAndOldCheck) || (gcMode == TR_WrtbarOldCheck))
            ...
            */
            ...
            if (constraint && constraint->getKnownObject() && constraint->isClassObject() == TR_yes
                ...
                if (j9class && TR::Compiler->cls.isClassInitialized(vp->comp(), j9class)
                    && performTransformation(vp->comp(),
                           "%s Remove TR::sun_misc_Unsafe_ensureClassInitialized call, class already initialized "
                           "[%p]\n",
                           OPT_DETAILS, node)) {
...
#ifdef J9_PROJECT_SPECIFIC
void getHelperSymRefs(OMR::ValuePropagation* vp, TR::Node* curCallNode, TR::SymbolReference*& getHelpersSymRef,
    TR::SymbolReference*& helperSymRef, char* helperSig, int32_t helperSigLen, TR::MethodSymbol::Kinds helperCallKind)
{
...
TR::Node* setCloneClassInNode(OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* constraint, bool isGlobal)
{
    ...
    if (constraint && constraint->getClass()) {
        ...
        if (performTransformation(
                vp->comp(), "%sSetting type on Object.Clone acall node [%p] to [%p]\n", OPT_DETAILS, node, clazz))
        ...
        if (!node->getOpCode().isIndirect()) {
            ...
            if (method->getRecognizedMethod() == TR::java_lang_Object_clone
                ...
                if (constraint) {
                    ...
                    if (constraint->getClass() && !vp->comp()->fej9()->isCloneable(constraint->getClass())) {
                        ...
                        TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                            TR::DebugCounter::debugCounterName(vp->comp(), "inlineClone/unsuitable/(%s)/%s/block_%d",
                ...
                if (!constraint
                    ...
                    TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                        TR::DebugCounter::debugCounterName(vp->comp(), "inlineClone/miss/(%s)/%s/block_%d",
                            vp->comp()->signature(), vp->comp()->getHotnessName(vp->comp()->getMethodHotness()),
                            vp->_curTree->getEnclosingBlock()->getNumber()));
                else
                    TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                        TR::DebugCounter::debugCounterName(vp->comp(), "inlineClone/hit/(%s)/%s/block_%d",
                            vp->comp()->signature(), vp->comp()->getHotnessName(vp->comp()->getMethodHotness()),
                            vp->_curTree->getEnclosingBlock()->getNumber()));
                ...
                TR::VPConstraint* newConstraint = TR::VPClass::create(
                    vp, newTypeConstraint, cloneResultNonNull, NULL, cloneResultArrayInfo, cloneResultOnHeap);
                ...
                TR_OpaqueClassBlock* classObject
                    = vp->fe()->getClassFromSignature("java/math/BigDecimal", 20, owningMethod);
                if (classObject) {
    ...
    if (powerOfTen) {
        ...
        if (!isUnsigned && fcPrecision <= refinedPrecision
            && performTransformation(vp->comp(),
                   ...
                   node->getFirstChild(), fcPrecision, refinedPrecision, constValue)) {
        ...
        if (mask == 0) {
                ...
                if ((firstChild->getOpCodeValue() == TR::iloadi || firstChild->getOpCodeValue() == TR::lloadi)
                    ...
                    if (vftLoad->getOpCodeValue() == TR::aloadi || vftLoad->getOpCodeValue() == TR::loadaddr) {
                        ...
                        if (baseExpressionConstraint && baseExpressionConstraint->getClassType()
                            ...
                            TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                                TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/hit/(%s)/%s",
                                    vp->comp()->signature(),
                                    vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
                        } else {
                            TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                                TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/miss/(%s)/%s",
                                    vp->comp()->signature(),
                                    vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
                        }
                    } else {
                        TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                            TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/unsuitable/(%s)/%s",
                                vp->comp()->signature(), vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
                    }
                }
            }
            ...
            if (!constraint) {
                if (mask != 0xffffffff && isNonNegativePowerOf2(mask + 1)) {
                    ...
                    // else
                    constraint = TR::VPIntRange::create(
                        vp, TR::getMinSigned<TR::Int32>(), mask & TR::getMaxSigned<TR::Int32>());
                }
            }
        }
    } else {
...
static TR::VPConstraint* passingTypeTestObjectConstraint(
    OMR::ValuePropagation* vp, TR::VPConstraint* classConstraint, bool testingForFixedType)
{
    TR_ASSERT_FATAL(
    ...
    TR_OpaqueClassBlock* objectClass)
{
    ...
    if (vp->trace()) {
        traceMsg(
            vp->comp(), "P2O: oldGuard %p newGuard %p currentTree %p\n", oldVirtualGuard, newGuardNode, vp->_curTree);
    }
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if (((node->getOpCodeValue() == TR::ifacmpne || node->getOpCodeValue() == TR::ifacmpeq
        ...
        if (vGuard && !vGuard->canBeRemoved()) {
            if (vp->trace())
                traceMsg(
                    vp->comp(), "   found virtual guard node %p with inner assumptions (cannot be removed)\n", node);
    ...
    if ((!vp->comp()->compileRelocatableCode() || vp->comp()->getOption(TR_UseSymbolValidationManager))
        && vp->lastTimeThrough() && vp->comp()->performVirtualGuardNOPing() && !vp->_curBlock->isCold()
        && !vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock() && (node->getNumChildren() == 2)
            ...
            case TR_MethodTest: {
                ...
                /*(vtableEntryNode->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
                {
                    ...
                    if (ignoreVirtualGuard && classConstraint && classConstraint->isFixedClass()) {
                        ...
                        bool testForEquality = (node->getOpCodeValue() == TR::ifacmpeq);
                        traceMsg(vp->comp(),
                            "TR_MethodTest: node=%p, vtableEntryNode=%p, clazz=%p, vftOffset=%d, vftEntry=%p, "
                    ...
                    if (vtableEntryNode->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef()) {
                        TR_OpaqueClassBlock* classFromMethod = vp->comp()->fe()->getClassFromMethodBlock(
    ...
    if (instanceofConstraint && instanceofOnBranch) {
        TR::VPConstraint* newConstraint
            = passingTypeTestObjectConstraint(vp, instanceofConstraint, instanceofDetectedAndFixedType);
    ...
    if (cannotBranch
        ...
        // lhsChild->getFirstChild()->getFirstChild() is not NULL, since non-NULL constraintFromMethodPointer implies the
        // following trees:
        // n235n     ifacmpne --> block_25 BBStart at n230n (inlineProfiledGuard ) [0x7fffd5110a40] bci=[0,0,314] rc=0
        // vc=56 vn=49 sti=- udi=- nc=2 flg=0x1020 n233n       iaload <vtable-entry-symbol> [#552 Shadow +768] [flags
        // 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56 vn=7 sti=- udi=- nc=1 n232n iaload
        // <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ] (X>=0 )                  [0x7fffd5110968] bci=[-1,0,2302]
        // rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100 n3n             ==>aload n234n       aconst 0x7fd070
        // (methodPointerConstant )                                          [0x7fffd51109f8] bci=[0,0,314] rc=1 vc=56
        // vn=5 sti=- udi=- nc=0 flg=0x2000
        ...
        if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
            ...
            if (rhs && !objectClass && lhs && lhs->getClassType()
                ...
                // if we ask for a rammethod in such a class we would trigger an assert ("no ram method in the vft
                // slot") to avoid this we would to insert another isInstanceOf (objectClass, callClass)
                    ...
                    == TR_yes) /*the object class may be less specific than callClass*/
            {
                ...
                if (methodSymbol
                    ...
                    int32_t cpIndexOrVftSlot = TR::Compiler->cls.isInterfaceClass(vp->comp(), objectClass)
                        ? symRef->getCPIndex()
                        : symRef->getOffset();
                    ...
                    // we should treat this case conservatively meaning we assume that there are more than two concrete
                    // classes in the given hierarchy most likely rvm will also be NULL in this case, but it is better to
                    // not rely on this implicit connection between different CH queries.
                    ...
                    /*
                      ...
                      Namely, we only do a transformation if there is no more than one instantiable class in the
                      hierarchy starting from objectClass. If there is more than one concrete class in hierarchy
                      overriding methods in objectClass and if any call was devirtualized by
                      invariantargumentpreexisence downstream we will produce the functionally incorrect code by
                      swapping a profiled test with a method test as the method test is insufficient for the
                    ...
                    */
                    ...
                    if (searchSucceeded && numConcreteClasses(&subClasses) < 2 && rvm) {
                        ...
                        TR_VirtualGuardKind guardKind = methodSymbol->isInterface()
                            ? TR_InterfaceGuard
                            : TR::Compiler->cls.isAbstractClass(vp->comp(), objectClass) ? TR_AbstractGuard
                                                                                         : TR_HierarchyGuard;
            ...
            if (sig && sig[0] == '[') {
                ...
                //
                    ...
                    if (isInstance == TR_yes) {
                        ...
                        && !(vp->comp()->getOption(TR_DisableArrayStoreCheckOpts)) && arrayComponentClass && objectClass
