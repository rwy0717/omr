...
{
    if (vp->comp()->getSymRefTab()->findDLPStaticSymbolReference(node->getSymbolReference()) || node->getSymbolReference()->isLitPoolReference())
        return true;
...
TR::Node* constrainAnyIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (dataType == TR::Int16 && node->getOpCode().isIndirect() && node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isAdd()) {
...
TR::Node* constrainWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (doOpt && ((gcMode == TR_WrtbarCardMarkAndOldCheck) || (gcMode == TR_WrtbarOldCheck)) && (node->getOpCodeValue() == TR::awrtbari) && !node->skipWrtBar()) {
    ...
    if (method) {
        if (!node->getOpCode().isIndirect()) {
            ...
            if (method->getRecognizedMethod() == TR::java_lang_Object_clone
                ...
                if (!constraint || (!constraint->isFixedClass() && (enableDynamicObjectClone && !(constraint->getClassType() && constraint->getClassType()->asResolvedClass() && constraint->getClassType()->isArray() == TR_no))))
            ...
            if ((method->getRecognizedMethod() == TR::java_math_BigDecimal_add) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_subtract) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_multiply)) {
                ...
                if (constraint && bigDecimalClass && constraint->isFixedClass() && (bigDecimalClass == constraint->getClass())) {
...
TR::Node* constrainIand(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (rhs && lrhs && lrhs->asIntConst()) {
        ...
        if (rhs->asIntRange()) {
            ...
            int32_t low = range->getLowInt();
            int32_t high = range->getHighInt();
            if ((node->getFirstChild()->getOpCodeValue() == TR::imul || node->getFirstChild()->getOpCodeValue() == TR::iumul) && ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
                while (!(mask & 0x1)) {
                    shift++;
                    mask >>= 1;
                }
                if (high < 1 << shift)
                    constraint = TR::VPIntConst::create(vp, 0);
            } else if ((node->getFirstChild()->getOpCodeValue() == TR::ishl || node->getFirstChild()->getOpCodeValue() == TR::iushl) && ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
                if (high < 1 << mask)
                    constraint = TR::VPIntConst::create(vp, 0);
            }
        } else if (rhs->asIntConst()) {
            int32_t iandMask = rhs->asIntConst()->getInt();
            if ((node->getFirstChild()->getOpCodeValue() == TR::imul || node->getFirstChild()->getOpCodeValue() == TR::iumul) && ((iandMask & 0x80000000) == 0)) {
                while (!(mask & 0x1)) {
                    shift++;
                    mask >>= 1;
                }
                if (iandMask < 1 << shift)
                    constraint = TR::VPIntConst::create(vp, 0);
            } else if ((node->getFirstChild()->getOpCodeValue() == TR::ishl || node->getFirstChild()->getOpCodeValue() == TR::iushl) && ((iandMask & 0x80000000) == 0)) {
                if (iandMask < 1 << mask)
                    constraint = TR::VPIntConst::create(vp, 0);
            }
        }
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if ((!vp->comp()->compileRelocatableCode() || vp->comp()->getOption(TR_UseSymbolValidationManager)) && vp->lastTimeThrough() && vp->comp()->performVirtualGuardNOPing() && !vp->_curBlock->isCold() && !vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock() && (node->getNumChildren() == 2) && ((node->getOpCodeValue() == TR::ifacmpeq) || (node->getOpCodeValue() == TR::ifacmpne))) {
