...
static void
checkForNonNegativeAndOverflowProperties(OMR::ValuePropagation *vp, TR::Node *node, TR::VPConstraint *constraint = NULL)
{
	if (!constraint) {
		bool isGlobal;
...
static int32_t
arrayElementSize(const char *signature, int32_t len, TR::Node *node, OMR::ValuePropagation *vp)
{
	if (signature[0] != '[')
		return 0;
	...
	if (signature[0] == '[') {
		...
		case 'L':
		default: return TR::Compiler->om.sizeofReferenceField();
		}
	}
...
constrainBaseObjectOfIndirectAccess(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	char *baseObjectSig =
	        method->classNameOfFieldOrStatic(node->getSymbolReference()->getCPIndex(), baseObjectSigLen);
	if (baseObjectSig)
...
static bool
tryFoldCompileTimeLoad(OMR::ValuePropagation *vp, TR::Node *node, bool &isGlobal)
{
	isGlobal = true;
	...
	if (node->getOpCode().isIndirect()) {
		...
		for (; !baseExpression;) {
			...
			if (!constraint) {
				...
				if (prevNodeSym->isArrayShadowSymbol() &&
				        // UnsafeShadow responds true to above question, but the type of the array
				        // element might not match the type of the symbol, it needs to be derived from
				        // the array
					...
					if (constraint->getKnownObject()->isArrayWithConstantElements(vp->comp())) {
						// Use the DataType of the symbol because it represents the element type
						// of the array
						...
						// Alias info of calls are computed in createAliasInfo, which is only
						// called if alias info is not available or is invalid. Invalidate alias
						// info so that it can be recomputed in the next optimization that needs
						// it
				...
				break;
			        ...
			        && constraint->getClassType() && constraint->getClassType()->asFixedClass()
			        && constraint->getClass()) {
				if (vp->trace())
				...
				if (nodeToRemove) {
					TR_ASSERT(didSomething,
					        "How can there be a node to remove if transformIndirectLoadChain "
					        "didn't do something??");
				...
				if (curNode->getOpCodeValue() == TR::loadaddr
					...
					if (vp->trace())
						traceMsg(vp->comp(),
						        "  - FAIL: %s %p is not an indirect load and has insufficient "
						        "constraints\n",
						        curNode->getOpCode().getName(), curNode);
					break;
				}
				...
				if (vp->trace())
					traceMsg(vp->comp(),
					        " - FAIL: %s %p points to a j9class but has insufficient constraints\n",
					        curNode->getOpCode().getName(), curNode);
				baseExpression = NULL;
				break;
		...
		if (baseExpression) {
			...
			if (nodeToRemove) {
				TR_ASSERT(didSomething,
				        "How can there be a node to remove if transformIndirectLoadChain didn't do "
				        "something??");
				vp->removeNode(nodeToRemove, true);
			}
			return didSomething;
			...
			if (constraint->isNullObject()) {
				...
				if (constraint->getKnownObject()) {
					...
					if (node->getOpCode().isLoadIndirect()
						...
						TR::KnownObjectTable *knot = vp->comp()->getKnownObjectTable();
						TR_ASSERT(knot,
						        "Can't have a known-object constraint without a known-object "
						        "table");
						TR::SymbolReference *improvedSymRef =
						...
						if (improvedSymRef->hasKnownObjectIndex()) {
							if (performTransformation(vp->comp(),
							            "%sUsing known-object specific symref #%d for "
							            "obj%d at [%p]\n",
							            OPT_DETAILS, improvedSymRef->getReferenceNumber(),
	...
	if (node->getSymbolReference()->getSymbol()->isShadow()) {
		...
		if (node->getSymbolReference() == vp->getSymRefTab()->findInstanceShapeSymbolRef()
		        ...
		        node->getSymbolReference()
		                == vp->getSymRefTab()->findClassFromJavaLangClassAsPrimitiveSymbolRef()
		        ||
#endif
...
bool
constraintFitsInIntegerRange(OMR::ValuePropagation *vp, TR::VPConstraint *constraint)
{
	if (constraint == NULL)
		return false;
...
bool
canMoveLongOpChildDirectly(TR::Node *node, int32_t numChild, TR::Node *arithNode)
{
...
bool
reduceLongOpToIntegerOp(OMR::ValuePropagation *vp, TR::Node *node, TR::VPConstraint *nodeConstraint)
{
	...
	// Adding this check because it is likely suboptimal on machines where you can use long registers to convert
	// things to integers and insert conversion trees. Conversion trees are not free. LoadExtensions (and other
	// codegen peepholes) should in theory catch uneeded conversions (like widening) as described in the case below
	// on 64 bit platforms.
		...
		for (int32_t i = 0; i < node->getNumChildren(); i++) {
			...
			if (canMoveLongOpChildDirectly(node, i, arithNode)) {
				...
				dumpOptDetails(vp->comp(), "    Creating new %s (0x%p) above child %d %s (0x%p)\n",
				        newChild->getOpCode().getName(), newChild, i, child->getOpCode().getName(),
				        child);
			}
		}
...
constrainAConst(OMR::ValuePropagation *vp, TR::Node *node, bool isGlobal)
{
	...
	if (node->getAddress() == 0) {
		...
		if (node->isClassPointerConstant()) {
			TR::VPConstraint *typeConstraint = TR::VPClass::create(vp,
			        TR::VPFixedClass::create(vp, (TR_OpaqueClassBlock *)node->getAddress()), NULL, NULL,
...
static void
constrainIntAndFloatConstHelper(OMR::ValuePropagation *vp, TR::Node *node, int32_t value, bool isGlobal)
{
...
constrainBCDSign(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (node->hasKnownSignCode()) // TODO: track assumed sign codes too? -- these are tracked on load operations
	                              // already so not much benefit
	{
	...
	if (sign == TR::DataType::getInvalidSignCode() || sign == TR::DataType::getIgnoredSignCode()) // if no specific
	                                                                                              // sign code see
	                                                                                              // if a clean or
	                                                                                              // preferred sign
	                                                                                              // state can be
		...
		if (constraintType == TR_Sign_Minus && node->hasKnownCleanSign()) {
			if (vp->trace())
				traceMsg(vp->comp(), "\tpromote constraintType %s->%s as node %s (%p) is clean\n",
				        ...
				        TR::VP_BCDSign::getName(TR_Sign_Minus_Clean), node->getOpCode().getName(),
				        node);
		...
		if (index->getOpCode().isLoadConst()) {
			...
			if (dataType == TR::Int16 && array->getOpCode().isIndirect()) {
				...
				if (baseVPConstraint && baseVPConstraint->isConstString()) {
					...
					uintptrj_t chIdx = (offset
					                           - (uintptrj_t)TR::Compiler->om
					...
					if (kobj->isArrayWithConstantElements(vp->comp())
						...
						if (constrainAnyIntLoadCriticalSection.hasVMAccess()) {
							...
							uintptrj_t lengthInElements =
							        TR::Compiler->om.getArrayLengthInElements(
							                vp->comp(), arrayObj);
							TR::DataType elementType = kobj->getPrimitiveArrayDataType();
							uintptrj_t lengthInBytes = lengthInElements
							...
							if (validStart <= offset && offset < accessEnd
								...
								case TR::Int8: {
									...
									vp->replaceByConstant(node,
									        TR::VPIntConst::create(vp, value),
									        isGlobal);
									return node;
								}
								case TR::Int16: {
									...
									vp->replaceByConstant(node,
									        TR::VPShortConst::create(vp, value),
									        isGlobal);
									return node;
								}
								case TR::Int32: {
								...
								case TR::Int64: {
									...
									vp->replaceByConstant(node,
									        TR::VPLongConst::create(vp, value),
									        isGlobal);
									return node;
								}
								}
...
addKnownObjectConstraints(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (objectReferenceLocation) {
...

		{
			...
			uintptrj_t objectReference = vp->comp()->fej9()->getStaticReferenceFieldAtAddress(
			...
			if (isFixedJavaLangClass) {
				// A FixedClass constraint means something different when the class happens to be
				// java/lang/Class. Must add constraints pertaining to the class that the
				// java/lang/Class object represents.
				//
		...
		} else if (jlClass) // without a jlClass, we can't tell what kind of constraint to add
		{
			...
			if (isFixedJavaLangClass) {
				if (performTransformation(vp->comp(),
				            "%sAdd ClassObject constraint to %p based on known java/lang/Class %s "
				            "=obj%d\n",
				...
				if (performTransformation(vp->comp(),
				            "%sAdd known-object constraint to %p based on known object obj%d of class "
				            "%s\n",
	...
	if (node->getOpCode().hasSymbolReference()) {
		...
		if (symRef->getSymbol()->isLocalObject()) {
			vp->addGlobalConstraint(
			        node, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::StackObject));
		}
		if (symRef->getSymbol()->isClassObject()) {
			vp->addGlobalConstraint(
			        node, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
		}
		...
		if (!symRef->getSymbol()->isArrayShadowSymbol()) {
			...
			if (haveClassLookaheadInfo) {
				...
				if (sym->isStatic() && sym->isFinal()) {
					...
					if (classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
					        && (type == TR::Address)) {
						...
						if (constrainAloadCriticalSection.hasVMAccess()) {
							...
							if (arrayObject != 0) {
								...
								if (sig && (len > 0)
								        && (sig[0] == '[' || sig[0] == 'L')) {
									int32_t elementSize =
									        arrayElementSize(sig, len, node, vp);
									if (elementSize != 0) {
										vp->addGlobalConstraint(node,
										        TR::VPNonNullObject::create(
										                vp));
										vp->addGlobalConstraint(node,
				...
				if (!foundInfo) {
					...
					if (classInfo && classInfo->getFieldInfo()) {
						TR_PersistentFieldInfo *fieldInfo =
						        classInfo->getFieldInfo()->findFieldInfo(
						                vp->comp(), node, false);
						if (fieldInfo) {
							...
							if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
								int32_t firstDimension =
								        arrayFieldInfo->getDimensionInfo(0);
								if (firstDimension >= 0) {
									int32_t len;
									const char *sig =
									        getFieldSignature(vp, node, len);
									if (sig && (len > 0)
									        && (sig[0] == '[' || sig[0] == 'L')) {
										int32_t elementSize = arrayElementSize(
										        sig, len, node, vp);
										if (elementSize != 0) {
											if (vp->trace())
												traceMsg(vp->comp(),
				...
				if (classBlock) {
					...
					if (jlClass) {
						if (classBlock != jlClass) {
							constraint = TR::VPClassType::create(vp, sig, len,
							        symRef->getOwningMethod(vp->comp()), isFixed,
							        classBlock);
							if (*sig == '[' || sig[0] == 'L') {
								int32_t elementSize =
								        arrayElementSize(sig, len, node, vp);
								if (elementSize != 0) {
									constraint = TR::VPClass::create(vp,
									        (TR::VPClassType *)constraint, NULL,
									        NULL,
...
static TR::Node *
findArrayLengthNode(OMR::ValuePropagation *vp, TR::Node *node, List<TR::Node> *arraylengthNodes)
{
	...
	if (base && base->isConstString()) {
		...
		if (success) {
			...
			if (!base->isNonNullObject() && vp->getCurrentParent()->getOpCodeValue() == TR::NULLCHK) {
				...
				TR::TreeTop *newTree =
				        TR::TreeTop::create(vp->comp(), TR::Node::create(TR::treetop, 1, node));
...
constrainIaload(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (node->getOpCode().hasSymbolReference()) {
		...
		case TR::SymbolReferenceTable::componentClassAsPrimitiveSymbol:
			...
			break;
		default: break; // OK to attempt compile-time load
		}
		...
		if (base && node->getOpCode().hasSymbolReference()
			...
			vp->addBlockOrGlobalConstraint(
			        node, TR::VPClass::create(vp, type, nonnull, NULL, NULL, loc), isGlobal);
		...
		// hence can be treated as known object.
		{
						...
						if (recognizedClazz
							...
							if (0 != fieldObject) {
								TR_OpaqueClassBlock *fieldObjectClazz =
								        TR::Compiler->cls.objectClass(
								                vp->comp(), fieldObject);
								if (!TR::Compiler->cls.isClassArray(
								            vp->comp(), fieldObjectClazz)) {
									traceMsg(vp->comp(),
									        "Recognized known object field node %p "
									        "\n",
									        node);
									TR::KnownObjectTable::Index
									        fieldObjectKnotIndex =
									                knot->getIndex(fieldObject);
									// Global constraints should work here, as field
									// loads get fresh value numbers
									constraint = TR::VPClass::create(vp,
									        TR::VPKnownObject::create(
									                vp, fieldObjectKnotIndex),
									        TR::VPNonNullObject::create(vp), NULL,
									        NULL,
									        TR::VPObjectLocation::create(vp,
									                TR::VPObjectLocation::
									                        HeapObject));
									...
									int32_t arrLength =
									        TR::Compiler->om
									                .getArrayLengthInElements(
									                        vp->comp(),
									                        fieldObject);
									traceMsg(vp->comp(),
									        "Recognized known array field node %p "
									        "length %d\n",
									        node, arrLength);
									// Global constraints should work here, as field
									// loads get fresh value numbers
									vp->addGlobalConstraint(node,
									        TR::VPFixedClass::create(
									                vp, fieldObjectClazz));
									vp->addGlobalConstraint(
									        node, TR::VPNonNullObject::create(vp));
									vp->addGlobalConstraint(node,
									        TR::VPArrayInfo::create(vp, arrLength,
									                arrLength,
									                TR::Compiler->om
									                        .getArrayElementWidthInBytes(
									                                vp->comp(),
									                                fieldObject)));
								}
							}
...

				{
					...
					if (constrainIaloadCriticalSection.hasVMAccess()) {
						...
						if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
							...
							// invalid assumptions on seeing this fixed class constraint so
							// be conservative and don't let the world know if we aren't
							// sure the elements match this type
			...
			if (knot && symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
				...
				TR::KnownObjectTable::Index knownObjectIndex =
				        knot->getIndexAt((uintptrj_t *)(base->getClass()
				...
				// constraint so be conservative and don't let the world know if we aren't sure the
			...
			vp->addGlobalConstraint(
			        node, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::JavaLangClassObject));
			vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
			node->setIsNonNull(true);
					...
					if (bypassOnThisRun) {
...

						{
							...
							if (constrainIaloadCriticalSection.hasVMAccess()) {
								uintptrj_t *siteLocation =
								        vp->comp()
								                ->getKnownObjectTable()
								                ->getPointerLocation(callSiteKOI);
								bypassLocation =
								        vp->comp()
								                ->fej9()
								                ->mutableCallSite_bypassLocation(
								                        *siteLocation);
								if (!bypassLocation
								        && performTransformation(vp->comp(),
								                "%s[%p] create CallSite bypass for "
								                "obj%d\n",
								...
								if (vp->trace())
									traceMsg(vp->comp(),
									        "[%p] Unable to acquire VM access.  "
									        "Not attempting to bypass CallSite\n",
									        node);
							}
						}
						...
						if (bypassLocation
							...
							if (targetConstraint && targetConstraint->getKnownObject())
								targetKOI =
								        targetConstraint->getKnownObject()->getIndex();
							...
							node->setSymbolReference(
							        vp->comp()
							                ->getSymRefTab()
							                ->createKnownStaticReferenceSymbolRef(
				...
				if (sym->isStatic() && sym->isFinal()) {
					...
					if ((classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
						...
						if (constrainIaloadCriticalSection.hasVMAccess()) {
							...
							if (arrayObject != 0) {
								/*
								 * This line is left commented because of the commented
								 * lines further below which make use of the arrLength
								 * variable. This can be removed once it is determined
								 * that the commented out lines below will never be
								 * needed.
								 */
								// int32_t arrLength =
							...
							if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
								...
								if (arrLength >= 0 && sig && (len > 0)
									...
									if (elementSize != 0) {
										if (vp->trace())
											traceMsg(vp->comp(),
											        "Using class lookahead "
											        "info to find out non "
											        "null, array "
											        "dimension\n");
										// vp->addGlobalConstraint(node,
										// TR::VPNonNullObject::create(vp));
						...
						if (jlClass) {
							if (clazz != jlClass) {
								...
								if (fieldName && (fieldNameLen > 0)
								        && !strncmp(fieldName,
								                "java/util/TreeMap.valuesCollection",
								                34)) {
									TR_OpaqueClassBlock *clazz =
									        vp->fe()->getClassFromSignature("Ljava/"
									                                        "util/"
									                                        "TreeMa"
									                                        "p$2;",
									                21, method);
			...
			if (jlClass) {
				if (classBlock != jlClass) {
					...
					if (*sig == '[' || sig[0] == 'L') {
						...
						if (elementSize != 0) {
							constraint = TR::VPClass::create(vp,
							        (TR::VPClassType *)constraint, NULL, NULL,
							        TR::VPArrayInfo::create(vp, 0,
	...
	if (node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol()
		...
		if (constraint && constraint->getClass()) {
			...
			if (arraySig && *arraySig == '[') {
					...
					if (prefetchOffset >= 0) {
						if (vp->comp()->findPrefetchInfo(node) < 0) {
							TR_Pair<TR::Node, uint32_t> *prefetchInfo =
							        new (vp->comp()->trHeapMemory())
							                TR_Pair<TR::Node, uint32_t>(node,
							                        (uint32_t *)(intptr_t)prefetchOffset);
				...
				For example, the following two snippets are both rejected by javac but accepted by a
				bytecode verifier. Both throw an ArrayStoreException at runtime.
				...
				this line staticField = new Object(); won't produce any exceptions if staticField is
				Serializable but it will be rejected by the bytecode verifier if staticField is of a
				class type.
				*/
				if (clazz) {
					...
					if (jlClass) {
						if (clazz != jlClass) {
							...
							if (fieldName && (fieldNameLen > 0)
							        && (!strncmp(fieldName, "java/math/BigDecimal.CACHE0",
							                    27)
							                || !strncmp(fieldName,
							                        "java/math/BigDecimal.CACHE1", 27)
							                || !strncmp(fieldName,
							                        "java/math/BigDecimal.CACHE2", 27)))
	...
	if (node->getSymbolReference()) {
		if (!node->getSymbolReference()->isUnresolved()
			...
			uint16_t classNameLen =
			        node->getSymbolReference()->getOwningMethod(vp->comp())->classNameLength();
			if (fieldName && !strncmp(fieldName, "this$0", 6) && className
				...
				if (vp->trace())
					traceMsg(vp->comp(),
					        ...
					        node->getGlobalIndex(), classNameLen, className, fieldSigLen, fieldSig,
...
constrainWrtBar(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none) {
		...
		//     x.a = new A();
		//
		// Most of the time both newly allocated objects would be in the nursery and there is no need for a
		// generational barrier. However, in a rare case that a GC occurs in between the two allocations above
		// the premise can be violated (by tenuring the first, but not the second object). The GC will
		// compensate for the lack of the barrier by auto-remembering all objects that are tenured and directly
		// stack referenced.
		//
		// This optimization is currently disabled when read barriers are required as it is currently unknown
		// whether this optimization is safe under such a GC mode.
		doOpt = false;
	}
						...
						for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
							...
							if (defNode && (defNode->getOpCodeValue() == TR::astore)
								...
								if ((defChild->getOpCodeValue() == TR::New)
									...
									removeWrtBar = true;
								/*
								 else if (defChild && (defChild->getOpCodeValue() ==
								 TR::aload))
								    {
					...
					if (useDefInfo->getUseDef(defs, useIndex)) {
						...
						for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
							...
							if (defNode && (defNode->getOpCodeValue() == TR::astore)
								...
								if (defChild
								        && (defChild->getOpCodeValue() == TR::loadaddr)
								        && defChild->getSymbolReference()
								                   ->getSymbol()
								                   ->isAuto()
								        && defChild->getSymbolReference()
									...
									if (useDefInfo->isUseIndex(useIndexA)) {
										TR_UseDefInfo::BitVector defsA(
										        vp->comp()->allocator());
										if (useDefInfo->getUseDef(
										            defsA, useIndexA)) {
											TR_UseDefInfo::BitVector::Cursor
											        cursorA(defsA);
											for (cursorA.SetToFirstOne();
											        cursorA.Valid();
												...
												if (defIndexA
												        < useDefInfo
												                  ->getFirstRealDefIndex())
													continue;

												TR::TreeTop *defTreeA =
												        useDefInfo->getTreeTop(
												                defIndexA);
												TR::Node *defNodeA =
												        defTreeA->getNode();
												if (defNodeA
												        && (defNodeA->getOpCodeValue()
												                == TR::astore)
												        && (defNodeA->getNumChildren()
												                > 0)) {
													TR::Node *defChildA =
													        defNodeA->getFirstChild();
													if (defChildA
													        && (defChildA->getOpCodeValue()
													                == TR::loadaddr)
													        && defChildA
													                   ->getSymbolReference()
													                   ->getSymbol()
	...
	if (objectConstraint) {
		...
		if (objectConstraint->isNullObject()) {
			result = 0;
		        ...
		        && objectConstraint->isNonNullObject() && objectConstraint->isClassObject() != TR_yes) {
			result = 1;
			...
			if (castConstraint) {
				if (objectConstraint->asClass() && castConstraint->asClass()) {
					...
					if ((castIsClassObject == TR_no) && !objectConstraint->getClassType() &&
						...
						if (vp->trace())
							traceMsg(vp->comp(),
							        "object is a classobject but cast is not "
							        "java/lang/Class\n");
						...
						if (vp->trace())
							traceMsg(vp->comp(),
							        "object is not a classobject but cast is "
							        "java/lang/Class\n");
					}
					...
					//
						...
						if (objectConstraint->isNonNullObject()) {
							...
							vp->removeNode(node->getChild(1), true);
							node->setAndIncChild(
							        1, TR::Node::create(node, TR::aconst, 0, 0));
							vp->addGlobalConstraint(
							        node->getChild(1), TR::VPNullObject::create(vp));
						}
			...
			if (castConstraint && objectConstraint->isNonNullObject()) {
				if (objectConstraint->asClass() && castConstraint->asClass()) {
					...
					if (objectConstraint->asClassType() && castConstraint->asClass()
						...
						if (objectConstraint->isClassObject() == TR_yes)
							intersectConstraint = TR::VPClass::create(vp, dilutedType, NULL,
							        NULL, NULL,
							        TR::VPObjectLocation::create(
			...
			else ////if (isInstance == TR_yes)
			{
					...
					if (jlClass) {
						...
						if (classType->getClass() == jlClass) {
							...
							if ((classType->isClassObject() == TR_maybe)
							        && (objectConstraint
							                && (objectConstraint->isClassObject()
							                        == TR_yes))) {
								...
								newClassType = TR::VPResolvedClass::create(vp,
								        classType->getClass()); // Do not want to get
								                                // the 'fixed' property
						}
					}
				} else {
				...
				if (applyType) {
					...
					if ((objectConstraint && (objectConstraint->isClassObject() == TR_yes))
					        || castIsClassObject)
						newTypeConstraint = TR::VPClass::create(vp, newClassType, NULL, NULL,
						        NULL,
						        TR::VPObjectLocation::create(
						                vp, TR::VPObjectLocation::JavaLangClassObject));
				...
				if (useDefInfo->isUseIndex(useIndex)) {
					...
					if (useDefInfo->getUseDef(defs, useIndex)) {
						...
						for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
							...
							if (defNode && (defNode->getOpCodeValue() == TR::astore)) {
								TR_OpaqueMethodBlock *ownMethod =
								        defNode->getOwningMethod();
								char buf[512];
...
constrainNewArray(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (sizeConstraint)
		...
		vp->addGlobalConstraint(
		        node, TR::VPArrayInfo::create(vp, 0, TR::getMaxSigned<TR::Int32>(), elementSize));
	...
	if (constraint && constraint->getClassType() && constraint->getClassType()->asFixedClass()
		...
		if (TR::Compiler->cls.isPrimitiveClass(vp->comp(), elementType)) {
			...
			TR::Node *typeConst = TR::Node::create(
			        TR::iconst, 0, vp->comp()->fe()->getNewArrayTypeFromClass(constraint->getClass()));
			...
			if (typeNode->getOpCodeValue() != TR::loadaddr) {
				TR::Node *loadaddr = TR::Node::createWithSymRef(TR::loadaddr, 0,
				        vp->comp()->getSymRefTab()->findOrCreateClassSymbol(
				                typeNode->getSymbolReference()->getOwningMethodSymbol(vp->comp()), 0,
				                elementType));
				vp->_curTree->insertBefore(
				        OMR::TreeTop::create(vp->comp(), TR::Node::create(TR::treetop, 1, typeNode)));
...
constrainMultiANewArray(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	int32_t maxSize = (int32_t)TR::Compiler->om.maxArraySizeInElementsForAllocation(
	        node, vp->comp()); // int32_t is ok because multiANewArray is only useful in Java, where array sizes are
	                           // bound by TR::getMaxSigned<TR::Int32>()
	int64_t maxHeapSize;
...
void
addValidRangeBlockOrGlobalConstraint(OMR::ValuePropagation *vp, TR::Node *node, FUNC createRange, FUNC2 processValue,
        T low, T high, bool childGlobal)
{
...
static TR::Node *
constrainHighestOneBitAndLeadingZerosHelper(OMR::ValuePropagation *vp, TR::Node *node, A getConst, B getRange,
        C getValue, D getValues, E createConst, F createRange, G processValue, T MIN_VALUE, T MAX_VALUE)
{
	...
	if (childConstraint) {
		if (getConst(childConstraint)) {
			...
			if (low < 0 && high < 0) {
				...
				if (vp->trace()) {
					traceMsg(vp->comp(),
					        "Constraint %lld .. %lld of %p 's first child is negative and folded "
					        "into %lld \n",
...
template <typename A, typename B, typename C, typename D, typename E, typename F, typename G, typename T>
static TR::Node *
constrainLowestOneBitAndTrailingZerosHelper(OMR::ValuePropagation *vp, TR::Node *node, A getConst, B getRange,
        C getValue, D getValues, E createConst, F createRange, G processValue, T MIN_VALUE, T MAX_VALUE)
{
...
{
	return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
	        createIntConstConstraint, createIntRangeConstraint, integerNumberOfLeadingZeros, (int32_t)0,
	        (int32_t)-1);
}
...
{
	return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getLongConst, getLongRange, getLong,
	        getLowHighLongs, createLongConstConstraint,
...
{
	return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getLongConst, getLongRange, getLong,
	        getLowHighLongs, createIntConstConstraint, createIntRangeConstraint, longNumberOfLeadingZeros,
	        (int64_t)0, (int64_t)-1);
}
...
{
	return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
	        createIntConstConstraint, createIntRangeConstraint, integerNumberOfTrailingZeros, (int32_t)0,
	        (int32_t)-1);
}
...
{
	return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong,
	        getLowHighLongs, createLongConstConstraint, createLongRangeConstraint, longToPowerOf2,
	        (int64_t)TR::getMaxSigned<TR::Int64>(), (int64_t)TR::getMinSigned<TR::Int64>());
}
...
{
	return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong,
	        getLowHighLongs, createIntConstConstraint, createIntRangeConstraint, longNumberOfTrailingZeros,
	        (int64_t)0, (int64_t)-1);
}
...
{
	return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong,
	        getLowHighLongs, createIntConstConstraint, createIntRangeConstraint, longBitCount, (int64_t)0,
	        (int64_t)-1);
}
...
constrainArraylength(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (!(node->getOpCodeValue() == TR::contigarraylength || node->getOpCodeValue() == TR::discontigarraylength)
		...
		if (node->getOpCodeValue() == TR::contigarraylength) {
			int32_t maxContiguousArraySizeInElements = TR::Compiler->om.maxContiguousArraySizeInBytes()
			        >> shiftAmount;
...
static void
refineSymbolReferenceForProfiledToOverridden(
        OMR::ValuePropagation *vp, TR::Node *callNode, TR::SymbolReference *newSymRef)
{
	for (OMR::ValuePropagation::VirtualGuardInfo *cvg = vp->_convertedGuards.getFirst(); cvg;
	        cvg = cvg->getNext()) {
...
refineMethodSymbolInCall(OMR::ValuePropagation *vp, TR::Node *node, TR::SymbolReference *symRef,
...
devirtualizeCall(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (!methodSymbol->firstArgumentIsReceiver()) {
		if (vp->trace())
			traceMsg(vp->comp(),
			        "Not attempting to de-virtualize call [%p] without first argument receiver\n", node);
		return;
	}
	...
	if ((constraint->asClass() && (constraint->asClass()->isClassObject() == TR_yes))
		...
		if (!thisType) {
			if (vp->trace())
				traceMsg(vp->comp(),
				        "Not attempting to de-virtualize call [%p] with class object receiver\n", node);
			return;
		}
		constraint = TR::VPFixedClass::create(vp, thisType);
#endif
	...
	if (argNode->getSymbol()) {
		...
		if (argSym->isParm()) {
			if (!vp->isParmInvariant(argSym)) {
				if (vp->trace())
					traceMsg(vp->comp(),
					        "Not attempting to de-virtualize call [%p] because receiver [%p] is "
					        "not invariant\n",
					        node, argNode);
				return;
			}
	...
	if (methodSymbol->isComputed()) {
			...
			if (constraint && constraint->getKnownObject() && knot) {
				...
				if (resolvedMethod) {
					...
					if (vp->optimizer()
						...
						if (vp->trace())
							traceMsg(vp->comp(),
							        "Not inlining call [%p] because the "
							        "MethodHandle.invoke inlining group will do a better "
							        "job\n",
							        node);
						return;
					}
				...
				if (vp->trace())
					traceMsg(vp->comp(),
					        "Not inlining call [%p] because unable to substitute object-specific "
					        "method symbol\n",
					        node);
				return;
			}
		...
		if (TR::Compiler->cls.isInterfaceClass(vp->comp(), thisType)) {
			if (vp->trace())
				traceMsg(vp->comp(),
				        "Not attempting to de-virtualize interface call [%p] with interface-class "
				        "receiver\n",
				        node);
			return;
		}
		...
		if (!originalMethodClass) {
			if (vp->trace())
				traceMsg(vp->comp(),
				        "Not attempting to de-virtualize interface call [%p] with receiver class %.*s "
				        "not yet loaded\n",
				        node, len, s);
			return;
		}
		...
		if (!resolvedMethod) {
			if (vp->trace())
				traceMsg(vp->comp(),
				        "Not attempting to de-virtualize interface call [%p] with no resolved method\n",
				        node, len, s);
			return;
		}
	...
	if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
		...
		if (constraint->isFixedClass() || constraint->isPreexistentObject()) {
			...
			int32_t numChildren = node->getNumChildren();
			for (int32_t i = 1; i < numChildren; i = (++i & 0x0000ffff)) // this & with 0x0000ffff is to
			                                                             // work around an opt bug in the
			                                                             // ibm build on ia32
				...
				if (constr->asKnownObject() && constr->isNonNullObject()) {
					...
					if (tracePrex) {
						...
						traceMsg(vp->comp(),
						        ...
						        argument, argInfo->get(c - firstArgIndex), clazz, sig);
					}
	...
	if ((vp->lastTimeThrough() || !node->getOpCode().isIndirect())
	        && (vp->_isGlobalPropagation || !vp->getLastRun()) && !methodSymbol->isInterface())
		vp->_devirtualizedCalls.add(
		        new (vp->trStackMemory()) OMR::ValuePropagation::CallInfo(vp, thisType, argInfo));
	vp->invalidateUseDefInfo();
	vp->invalidateValueNumberInfo();
}
...
constrainCall(OMR::ValuePropagation *vp, TR::Node *node)
{
...

	{
		...
		if (operand
			...
			vp->_unsafeCallsToInline.add(
			        new (vp->trStackMemory()) OMR::ValuePropagation::CallInfo(vp, NULL, NULL));
	...
	if (node->getOpCode().isIndirect() && !debug("dontDevirtualize")) {
		...
		if (node->getSymbolReference()->isUnresolved() && symbol->isInterface()) {
			...
			if (existingConstraint && existingConstraint->intersect(constraint, vp)) {
				...
				// dumpOptDetails(vp->comp(), "First time an interface call made using receiver at node
	...
	if (node->getType().isInt32() && vp->comp()->getOption(TR_AllowVPRangeNarrowingBasedOnDeclaredType)) {
		...
		if (method) {
			...
			if (dataType == TR::Int8 || dataType == TR::Int16 || dataType == TR::Int32
			        || dataType == TR::Int64) {
	...
	if (symbol) {
		...
		if (isIntegerAbs || isLongAbs) {
			...
			// value of the range (TR::getMinSigned<TR::Int32>() or TR::getMinSigned<TR::Int64>()) then the
			// return value of the ABS could be that same minimum value (yes, that's right, it could be
			// negative).
			bool operandGlobal;
			...
			if (isIntegerAbs) {
				...
				couldBeMinValue =
				        (operand == NULL || operand->getLowInt() == TR::getMinSigned<TR::Int32>());
			} else {
				...
				couldBeMinValue =
				        (operand == NULL || operand->getLowLong() == TR::getMinSigned<TR::Int64>());
			}
			...
			if (couldBeMinValue) {
				...
				if (isIntegerAbs)
					...
					TR_ASSERT(isLongAbs, ""); // sanity check that no other cases get in here by
					                          // mistake
					minValueConstraint = TR::VPLongConst::create(vp, TR::getMinSigned<TR::Int64>());
				}
				absConstraint =
				        TR::VPMergedConstraints::create(vp, minValueConstraint, positiveConstraint);
			} else
							...
							for (cursor.SetToFirstOne(); cursor.Valid();
								...
								if (defNode
									...
									if (defChild->getOpCodeValue() == TR::New) {
										...
										break;
									} else if (defChild->getOpCodeValue()
									        == TR::aload) {
										uint16_t useIndexC =
										        defChild->getUseDefIndex();
										if (useDefInfo->isUseIndex(useIndexC)) {
											TR_UseDefInfo::BitVector defsC(
											        vp->comp()
											                ->allocator());
											if (useDefInfo->getUseDef(
											            defsC, useIndexC)) {
												TR_UseDefInfo::BitVector::
												        Cursor cursorC(
												                defsC);
												for (cursorC.SetToFirstOne();
												        cursorC.Valid();
													...
													if (defNodeC
													        && (defNodeC->getOpCodeValue()
													                == TR::astore)) {
														TR::Node *defChildC =
														        defNodeC->getFirstChild();
														if (defChildC
														        && defChildC->getOpCodeValue()
														                == TR::New) {
															definitionNode =
															        defChildC;
															break;
			...
			if (jitHelpersClass && TR::Compiler->cls.isClassInitialized(vp->comp(), jitHelpersClass)) {
				...
				if (vp->comp()->getOption(TR_EnableJITHelpershashCodeImpl) && hashCodeMethodSymRef
				        && getHelpersSymRef
				        && performTransformation(vp->comp(),
			...
			/*
			n572n       vcall  sun/misc/Unsafe.ensureClassInitialized(Ljava/lang/Class;)V[#631  final native
			virtual Method -728] n577n         aload  java/lang/invoke/MethodHandle$UnsafeGetter.myUnsafe
			   n579n         iaload  java/lang/invoke/MethodHandle.defc

			Because Unsafe.ensureClassInitialized is JNI, can't use direct JNI, otherwise can't optimized it
			at compile time.
			*/
			...
			if (constraint && constraint->getKnownObject() && constraint->isClassObject() == TR_yes
				...
				if (j9class && TR::Compiler->cls.isClassInitialized(vp->comp(), j9class)
				        && performTransformation(vp->comp(),
				                "%s Remove TR::sun_misc_Unsafe_ensureClassInitialized call, class "
...
constrainAdd(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && rhs) {
		...
		if (constraint) {
			...
			// If we reduced long to integer, node is now an i2l; not safe to do any further
			// addition-related things with node
			if (didReduction)
				return node;
		}
	}
...
constrainSubtract(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && rhs) {
		...
		if (constraint) {
			...
			vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
			// If we reduced long to integer, node is now an i2l; not safe to do any further
			// subtract-related things with node
			if (didReduction)
				return node;
		}
	}
...
constrainImul(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && rhs) {
		...
		if (lhs->asIntConst() && rhs->asIntConst()) {
			...
			// else
			constraint =
			        TR::VPIntConst::create(vp, lhs->asIntConst()->getInt() * rhs->asIntConst()->getInt());
		} else {
			...
			if (lowest < TR::getMinSigned<TR::Int32>() || highest > TR::getMaxSigned<TR::Int32>()) {
				...
				constraint =
				        TR::VPIntRange::create(vp, (int32_t)lowest, (int32_t)highest /*, isUnsigned*/);
			}
		}
		if (constraint) {
			if (constraint->asIntConst()) {
...
can64BitUnsignedMultiplyOverflow(uint64_t a, uint64_t b, uint64_t maxValue)
{
	...
	// LOWWORD(b)) If we compute the highword of the above expression in a 64-bit variable, a value >= 2^32
	// indicates overflow
	...
	if (lhs && rhs) {
		...
		if (lhs->asLongConst() && rhs->asLongConst()) {
			...
			if (!(can64BitMultiplyOverflow(
			              lhs->getLowLong(), rhs->getLowLong(), node->getOpCode().isUnsigned())
			            || can64BitMultiplyOverflow(
			                    lhs->getLowLong(), rhs->getHighLong(), node->getOpCode().isUnsigned())
			            || can64BitMultiplyOverflow(
			                    lhs->getHighLong(), rhs->getLowLong(), node->getOpCode().isUnsigned())
			            || can64BitMultiplyOverflow(
...
bool
constrainIntegerDivisionRange(int64_t lhsLow, int64_t lhsHigh, int64_t rhsLow, int64_t rhsHigh, int64_t rangeMin,
        int64_t rangeMax, int64_t &resultLow, int64_t &resultHigh, bool allowZeroInDivisorRange)
{
	...
	// integer division by 0 is meaningless and should be handled by an exception when executed. In other words,
	// this method determines the range of the result assuming the divisor is valid.
	if (rhsLow == 0)
		rhsLow = 1;
	...
	if (lhs && rhs) {
		if (lhs->asIntConst() && rhs->asIntConst()) {
			...
			if (lhsConst == TR::getMinSigned<TR::Int32>() && rhsConst == -1 && !isUnsigned)
				...
				if (isUnsigned)
					constraint =
					        TR::VPIntConst::create(vp, ((uint32_t)lhsConst / (uint32_t)rhsConst));
				else
			...
			if (lhsInt && rhsInt) {
				...
				if (isUnsigned) {
					...
					range_min = 0;
					range_max = TR::getMaxSigned<TR::Int32>(); // we don't support unsigned range
...
constrainLdiv(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
		...
		if (vp->lastTimeThrough() && !isUnsigned && lhsConstraint
		        ...
		        && performTransformation(vp->comp(),
		                "%sChange node [" POINTER_PRINTF_FORMAT "] ldiv->i2l of idiv\n", OPT_DETAILS, node)) {
			...
			if (rhsLow >= 1 && rhsLow <= rhsHigh && lhsLow <= lhsHigh) {
				...
				vp->addBlockOrGlobalConstraint(node,
				        TR::VPLongRange::create(vp, lhsLow / rhsHigh, lhsHigh / rhsLow), lhsGlobal);
			}
		}
	}
...
TR::Node *
removeRedundantREM(OMR::ValuePropagation *vp, TR::Node *node, TR::VPConstraint *nodeConstraint,
        TR::VPConstraint *firstChildConstraint, TR::VPConstraint *secondChildConstraint)
{
	...
	if (powerOfTen) {
		bool isUnsigned = node->getOpCode().isUnsigned();
		// the max value for a remainder result is the divisor value - 1 so when the divisor is a power of ten
		// then -1 this value is one decimal precision value less (e.g. 100 - 1 = 99)
		...
		if (!isUnsigned && fcPrecision <= refinedPrecision
		        && performTransformation(vp->comp(),
		                ...
		                node->getFirstChild()->getOpCode().getName(), node->getFirstChild(), fcPrecision,
		                refinedPrecision, constValue)) {
			// in this case based on the child prec and the divisor constant value that the irem is not
			// needed (999 < 1000) so can remove the irem completely irem
			//    child prec=3 (i.e. max value is 999)
			//    iconst 1000
			//
			// this doesn't work for unsigned (iurem) as these ops treat the operands as unsigned so, for
			// example, a prec=1 dividend that is negative when interpreted as unsigned will actually have
			// 10 digits of precision (-1 would be 0xFFffFFff = 4,294,967,295)
			//
...
constrainLrem(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
		...
		if (lhsConst == TR::getMinSigned<TR::Int64>() && rhsConst == -1)
			...
			constraint =
			        TR::VPLongConst::create(vp, TR::Compiler->arith.longRemainderLong(lhsConst, rhsConst));
		if (constraint)
			vp->replaceByConstant(node, constraint, lhsGlobal);
	...
	if (child) {
		if (child->asIntConst()) {
			...
			if (low == TR::getMinSigned<TR::Int32>()) {
				lowConstraint = TR::VPIntRange::create(
				        vp, TR::getMinSigned<TR::Int32>(), TR::getMinSigned<TR::Int32>(), TR_yes);
				low++;
			}

			if (high == TR::getMinSigned<TR::Int32>()) {
				highConstraint = TR::VPIntRange::create(
				        vp, TR::getMinSigned<TR::Int32>(), TR::getMinSigned<TR::Int32>(), TR_yes);
				high++;
			}
	...
	if (child) {
		if (child->asLongConst()) {
			...
			if (low == TR::getMinSigned<TR::Int64>()) {
				lowConstraint = TR::VPLongRange::create(
				        vp, TR::getMinSigned<TR::Int64>(), TR::getMinSigned<TR::Int64>(), TR_yes);
				low++;
			}

			if (high == TR::getMinSigned<TR::Int64>()) {
				highConstraint = TR::VPLongRange::create(
				        vp, TR::getMinSigned<TR::Int64>(), TR::getMinSigned<TR::Int64>(), TR_yes);
				high++;
			}
...
constrainIshl(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (lhs && lhs->asIntConst() && rhs && rhs->asIntConst()) {
		...
		int32_t rhsConst = rhs->asIntConst()->getInt();
		TR::VPConstraint *constraint =
		        TR::VPIntConst::create(vp, lhsConst << (rhsConst & 0x01F) /*, isUnsigned*/);
		vp->replaceByConstant(node, constraint, lhsGlobal);
	}
...
static TR::Node *
distributeShift(OMR::ValuePropagation *vp, TR::Node *node, int32_t shiftAmount, TR::VPConstraint *&constraint)
{
		...
		if (replaceNode) {
			...
			if (lhsORrhs) {
				...
				if ((low <= 0) && (((int64_t)(high + 1) & (pow2Val - 1))) == 0) {
					...
					if (vp->trace())
						traceMsg(vp->comp(),
						        "failed additive expr constraint is not within range, node "
						        "[%p] replaceNode [%p]\n",
						        node, replaceNode);
					replaceNode = NULL;
				}
	...
	if (rhs && rhs->asIntConst()) {
		...
		if (constraint) {
			...
			if (longShr) {
				TR::Node *secondChild = node->getSecondChild();
				didReduction = reduceLongOpToIntegerOp(vp, node, constraint);
				// If the shift amount is between 32 and 63, inclusive, then if we convert the node to
				// an integer shift we must reduce this shift value. Integer shifts are masked by 0x1f
				// in java, and if the first child is integer constrained, a shift by an amount in this
				// range is effectively just a shift by 31.
...
constrainIushr(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (rhs && rhs->asIntConst()) {
		...
		if (low == high)
			...
			constraint = TR::VPIntRange::create(
			        vp, ((uint32_t)low) >> shiftAmount, ((uint32_t)high) >> shiftAmount /*, isUnsigned*/);
...
constrainIand(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (rhs && rhs->asIntConst()) {
		...
		if (mask == 0) {
			...
			// if (isUnsigned)
			//   constraint = TR::VPIntConst::create(vp, ((uint32_t)lhs->asIntConst()->getInt() &
			//   (uint32_t)mask), isUnsigned);
			// else
				...
				if ((firstChild->getOpCodeValue() == TR::iloadi
					...
					if (vftLoad->getOpCodeValue() == TR::aloadi
						...
						if (baseExpressionConstraint && baseExpressionConstraint->getClassType()
							...
							if (baseExpressionConstraint->getClassType()->isArray()
							        == TR_yes)
								constraint = TR::VPIntConst::create(
								        vp, rhs->asIntConst()->getLowInt());
							else
								constraint = TR::VPIntConst::create(vp, 0);

							TR::DebugCounter::incStaticDebugCounter(vp->comp(),
							        TR::DebugCounter::debugCounterName(vp->comp(),
							                "isArrayTest/hit/(%s)/%s",
							                vp->comp()->signature(),
			...
			if (!constraint) {
				if (mask != 0xffffffff && isNonNegativePowerOf2(mask + 1)) {
					...
					// if (isUnsigned)
					//   constraint = TR::VPIntRange::create(vp, TR::getMinUnsigned<TR::Int32>(),
					//   mask, isUnsigned);
					// else
					constraint = TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(),
	...
	if (rhs && rhs->asLongConst()) {
		...
		if (mask == 0L) {
			...
			if (mask != -1L && isNonNegativePowerOf2(mask + 1)) {
				...
				constraint = TR::VPLongRange::create(
				        vp, TR::getMinSigned<TR::Int64>(), mask & TR::getMaxSigned<TR::Int64>());
			}
		}
...
static TR::Node *
constrainWidenToLong(OMR::ValuePropagation *vp, TR::Node *node, int64_t low, int64_t high, bool isUnsigned)
{
	if (findConstant(vp, node))
		return node;
	constrainChildren(vp, node);
...
replaceWithSmallerType(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (performTransformation(comp,
	            "%sReplace [" POINTER_PRINTF_FORMAT "] and [" POINTER_PRINTF_FORMAT "] with store and load of a "
	            "smaller type\n",
		...
		// create new temp
		TR::SymbolReference *symRef =
		        comp->getSymRefTab()->createTemporary(vp->comp()->getMethodSymbol(), newType);
...
static TR::Node *
constrainNarrowIntValue(OMR::ValuePropagation *vp, TR::Node *node, int32_t low, int32_t high)
{
	if (findConstant(vp, node))
		return node;
	constrainChildren(vp, node);
	...
	if (constraint) {
		if (constraint->asIntConstraint() || constraint->asMergedConstraints()) {
			...
			if (constraint->getLowLong() < 0 && constraint->getLowLong() == constraint->getHighLong())
				low = high = max
				        + (int32_t)constraint->getLowLong() % (TR::getMaxUnsigned<TR::Int16>() + 1);
		}
	}
...
static bool
constrainWidenToInt(OMR::ValuePropagation *vp, TR::Node *&node, int32_t low, int32_t high, bool isUnsigned,
        TR::ILOpCodes correspondingNarrowingOpCode)
{
	...
	*/
	...
	if ( // p &&
		...
		// printf("found one (sign extension conversion removal) in method %s, node %p, low was %d, high was
...
static void
changeConditionalToGoto(OMR::ValuePropagation *vp, TR::Node *node, TR::CFGEdge *branchEdge)
{
#ifdef J9_PROJECT_SPECIFIC
...
static void
removeConditionalBranch(OMR::ValuePropagation *vp, TR::Node *node, TR::CFGEdge *branchEdge)
{
#ifdef J9_PROJECT_SPECIFIC
	createGuardSiteForRemovedGuard(vp->comp(), node);
#endif
...
static TR::Node *
getOriginalCallNode(TR_VirtualGuard *vGuard, TR::Compilation *comp, TR::Node *guardNode)
{
...
static void
addDelayedConvertedGuard(TR::Node *node, TR::Node *callNode, TR::ResolvedMethodSymbol *methodSymbol,
        ...
        TR_OpaqueClassBlock *objectClass)
{
	...
	if (!callNode->getFirstChild()->getOpCode().hasSymbolReference()
	        ...
	        || !callNode->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect() || // I guess, having a
	                                                                                         // vft symbol implies
	                                                                                         // that there is
	                                                                                         // something underneath
	                                                                                         // it?
	        !callNode->getSecondChild()->getOpCode().isLoadVarDirect() || // it also implies, there should be a
	                                                                      // receiver argument?
	...
	TR::Node *newGuardNode = TR_VirtualGuard::createMethodGuardWithReceiver(guardKind, vp->comp(),
	        oldVirtualGuard->getCalleeIndex(), callNode, node->getBranchDestination(), methodSymbol,
	        objectClass /*oldVirtualGuard->getThisClass()*/, newReceiver);

	if (vp->trace()) {
		traceMsg(vp->comp(), "P2O: oldGuard %p newGuard %p currentTree %p\n", oldVirtualGuard, newGuardNode,
		        vp->_curTree);
	}
...
#ifdef J9_PROJECT_SPECIFIC
TR_ResolvedMethod *
findSingleImplementer(TR_OpaqueClassBlock *thisClass, int32_t cpIndexOrVftSlot, TR_ResolvedMethod *callerMethod,
        TR::Compilation *comp, bool locked, TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
...
//
static TR::Node *
constrainIfcmpeqne(OMR::ValuePropagation *vp, TR::Node *node, bool branchOnEqual)
{
	constrainChildren(vp, node);
		...
		if (rhsConst == 0 || rhsConst == 1) {
			...
			if (lhsOp.isInt() || lhsOp.isLong() || lhsOp.isRef()) {
				...
				if (performTransformation(vp->comp(), "%sChanging n%un (%s (%s ...) %d) to (%s ...)\n",
				            OPT_DETAILS, node->getGlobalIndex(), node->getOpCode().getName(),
				            lhsCmp.getName(), rhsConst, newIfOp.getName())) {
	...
	if (((node->getOpCodeValue() == TR::ifacmpne || node->getOpCodeValue() == TR::ifacmpeq
		...
		if (vGuard && !vGuard->canBeRemoved()) {
			if (vp->trace())
				traceMsg(vp->comp(),
				        "   found virtual guard node %p with inner assumptions (cannot be removed)\n",
				        node);
	...
	if (vp->getValueNumber(lhsChild) == vp->getValueNumber(rhsChild) && ignoreVirtualGuard) {
		if (vp->trace())
			traceMsg(vp->comp(), "   cmp children have same value number: %p = %p = %d\n", lhsChild,
			        rhsChild, vp->getValueNumber(lhsChild));
		if (branchOnEqual)
	...
	//
		...
		if (lhs && rhs) {
			...
			if (lhs->mustBeEqual(rhs, vp) && ignoreVirtualGuard) {
				...
				if (vp->trace())
					traceMsg(vp->comp(),
					        "   cmp children must be not equal by absolute constraints: %p != %p\n",
					        lhsChild, rhsChild);
				if (branchOnEqual)
					cannotBranch = true;
				else
	...
	if (!cannotBranch && !cannotFallThrough) {
		...
		if (rel && ignoreVirtualGuard) {
			...
			if (rel->mustBeEqual()) {
				...
				if (vp->trace())
					traceMsg(vp->comp(),
					        "   cmp children must be not equal by relative constraint: %p != %p\n",
					        lhsChild, rhsChild);
				if (branchOnEqual)
					cannotBranch = true;
				else
				...
				if (typeConstraint) {
					...
					if (resolvedTypeConstraint) {
						...
						if (!classInfo || !classInfo->isInitialized()) {
							int32_t len;
							const char *sig =
							        resolvedTypeConstraint->getClassSignature(len);
						...
						if (unresolvedTypeConstraint) {
							...
							if (owningMethod) {
								...
								if (!clazz) {
									...
									if (vp->trace())
										traceMsg(vp->comp(),
										        "MyDebug: clazz %p classInfo "
										        "%p classInfo->isInitialized() "
										        "%d\n",
										        clazz, classInfo,
										        classInfo
										                ? classInfo
					...
					if (clazzToBeInitialized
						...
						char *clazzToBeInitializedCopy = (char *)vp->trMemory()->allocateMemory(
						        clazzNameLen + 1, heapAlloc);
	...
	if (node->getOpCodeValue() != TR::ifacmpeq && node->getOpCodeValue() != TR::ifacmpne) {
		if (branchOnEqual) {
			...
			if (!vp->addEdgeConstraint(
			            lhsChild, TR::VPNotEqual::create(vp, 0), edgeConstraints, rhsChild)) {
				if (!vp->intersectionFailed())
					cannotBranch = true;
				else
					vp->setIntersectionFailed(false);
	...
	if (cannotBranch
		...
		changeConditionalToGoto(vp, node, edge);
	        ...
	        && lhsChild->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect()) {
		// if constraintFromMethodPointer is set we should be able to assume that
		// lhsChild->getFirstChild()->getFirstChild() is not NULL, since non-NULL constraintFromMethodPointer
		// implies the following trees:
		// n235n     ifacmpne --> block_25 BBStart at n230n (inlineProfiledGuard ) [0x7fffd5110a40]
		// bci=[0,0,314] rc=0 vc=56 vn=49 sti=- udi=- nc=2 flg=0x1020 n233n       iaload <vtable-entry-symbol>
		// [#552 Shadow +768] [flags 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56 vn=7
		// sti=- udi=- nc=1 n232n         iaload <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ] (X>=0 )
		// [0x7fffd5110968] bci=[-1,0,2302] rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100 n3n             ==>aload
		// n234n       aconst 0x7fd070 (methodPointerConstant ) [0x7fffd51109f8] bci=[0,0,314] rc=1 vc=56 vn=5
		...
		if (intersected) {
			if (!branchOnEqual) {
				...
				vp->addEdgeConstraint(
				        lhsChild->getFirstChild()->getFirstChild(), intersected, edgeConstraints);
			}
		}
		...
		if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
			...
			// interfaces are weird we would create an UnresolvedClass Constraint and then wrap it in a
			// Class constraint
			if (rhs && !objectClass && lhs && lhs->getClassType()
				...
				// if we ask for a rammethod in such a class we would trigger an assert ("no ram method
				// in the vft slot") to avoid this we would to insert another isInstanceOf (objectClass,
				// callClass)
			...
			// if all the conditions below are satisfied, especially the last one, we can cut some corners
			// and use findSingleImplementer to get a concrete implementation regardless of the type(e.g.
			// virtual vs interface) of a call and the type of a receiver (e.g. abstract, interface, normal
			// classes) if findSingleImplementer returns a method than it must be the implementation Inliner
			// used since it would be the only one available
			                ...
			                == TR_yes) /*the object class may be less specific than callClass*/
			{
				...
				if (methodSymbol
					...
					// in AOT or when CHOpts are disabled findClassInfoAfterLocking returns NULL
					// unconditionally we should treat this case conservatively meaning we assume
					// that there are more than two concrete classes in the given hierarchy most
					// likely rvm will also be NULL in this case, but it is better to not rely on
					// this implicit connection between different CH queries.
					...
					/*
					  ...
					  Namely, we only do a transformation if there is no more than one instantiable
					  class in the hierarchy starting from objectClass. If there is more than one
					  concrete class in hierarchy overriding methods in objectClass and if any call
					  was devirtualized by invariantargumentpreexisence downstream we will produce
					  the functionally incorrect code by swapping a profiled test with a method test
					  as the method test is insufficient for the devirtualizations done by
						...
						TR::ResolvedMethodSymbol *cMethodSymbol =
						        vp->comp()
						                ...
						                ->findOrCreateMethodSymbol(
						                        symRef->getOwningMethodIndex(), -1, rvm,
						                        methodKind)
						                ->getSymbol()
...
void
getLimits(OMR::ValuePropagation *vp, int32_t *zLow, int32_t *zHigh, TR::Node *symNode, bool isGlobal)
{
...
void
getLimits(OMR::ValuePropagation *vp, int64_t *zLow, int64_t *zHigh, TR::Node *symNode, bool isGlobal)
{
	TR::VPConstraint *zConstraint = vp->getConstraint(symNode, isGlobal);
...
TR::Node *
makeNewRhsNode(OMR::ValuePropagation *vp, TR::Node *node, TR::Node *symNodeY, int32_t constDiff)
{
...
TR::Node *
makeNewRhsNode(OMR::ValuePropagation *vp, TR::Node *node, TR::Node *symNodeY, int64_t constDiff)
{
...
static TR::Node *
simplifyInequality(OMR::ValuePropagation *vp, TR::Node *node, TR::Node *lhsChild, TR::Node *rhsChild, bool isGlobal,
        bool childrenReversed)
{
	...
	if ((lhsChild->getOpCode().isAdd() || lhsChild->getOpCode().isSub())
		...
		if ( // no overflow in x + a
			...
			// Values larger than that limit are used for other purposes, so there can't be any overlap.
			// It's not safe to call constrainChildren on a newly-created node
			// constrainChildren(vp, node);
		}
	}
	return node;
...
//
static TR::Node *
constrainIfcmplessthan(OMR::ValuePropagation *vp, TR::Node *node, TR::Node *lhsChild, TR::Node *rhsChild, bool orEqual)
{
	...
	if (!cannotBranch && !cannotFallThrough && !isUnsigned) {
		...
		if (rel) {
			...
			if (rel->asEqual() && !cannotFallThrough && !cannotBranch && (lhs || rhs)) {
				// If we know lhs == rhs+k, and rhs+k can't overflow, we can tell which way the
				// comparison will go. Likewise, since lhs-k == rhs, if we know lhs-k can't overflow,
				// we're also good to go.

				TR::VPEqual *relation = rel->asEqual();
				int32_t increment = relation->increment();

				int64_t maxIncrement = 0; // If absIncrement is more than this, overflow may occur, and
				...
				if (absIncrement < 0) {
					...
					if (rhs) {
						if (rhsChild->getOpCode().isLong())
							maxIncrement = clippedDifference(
							        rhs->getLowLong(), TR::getMinSigned<TR::Int64>());
						else
							maxIncrement = clippedDifference(
							        rhs->getLowInt(), TR::getMinSigned<TR::Int32>());
					} else {
						if (lhsChild->getOpCode().isLong())
							maxIncrement = clippedDifference(
							        TR::getMaxSigned<TR::Int64>(), lhs->getHighLong());
						else
							maxIncrement = clippedDifference(
							        TR::getMaxSigned<TR::Int32>(), lhs->getHighInt());
					}
				} else {
					if (rhs) {
						if (rhsChild->getOpCode().isLong())
							maxIncrement = clippedDifference(
							        TR::getMaxSigned<TR::Int64>(), rhs->getHighLong());
						else
							maxIncrement = clippedDifference(
							        TR::getMaxSigned<TR::Int32>(), rhs->getHighInt());
					} else {
						if (lhsChild->getOpCode().isLong())
							maxIncrement = clippedDifference(
							        lhs->getLowLong(), TR::getMinSigned<TR::Int64>());
						else
							maxIncrement = clippedDifference(
							        lhs->getLowInt(), TR::getMinSigned<TR::Int32>());
					}
				}
				...
				// -TR::getMinSigned<TR::Int32>(), and maxIncrement comes from clippedDifference, so by
				// the definition of clippedDifference, we can compare them with the expected results.
				//
	...
	if (lhsChild->getOpCode().isLong()) {
		...
		if (longBoundary != TR::getMinSigned<TR::Int64>() && !isUnsigned) {
			if (!vp->addEdgeConstraint(rhsChild,
			            TR::VPLongRange::create(vp, longBoundary, TR::getMaxSigned<TR::Int64>()),
			            edgeConstraints)) {
				if (!vp->intersectionFailed())
					cannotBranch = true;
				else
	...
	if (lhsChild->getOpCode().isLong() && !isUnsigned) {
		...
		if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
			if (!vp->addBlockConstraint(rhsChild,
			            TR::VPLongRange::create(vp, TR::getMinSigned<TR::Int64>(), longBoundary), NULL,
			            false)) {
				if (!vp->intersectionFailed())
					cannotFallThrough = true;
				else
		...
		if (longBoundary != TR::getMinSigned<TR::Int64>()) {
			if (!vp->addBlockConstraint(lhsChild,
			            TR::VPLongRange::create(vp, longBoundary, TR::getMaxSigned<TR::Int64>()), NULL,
			            false)) {
				if (!vp->intersectionFailed())
					cannotFallThrough = true;
				else
				...
				if (firstChild->getOpCodeValue() == node->getOpCodeValue()) {
					...
					if (secondGrandChild->getOpCode().isLoadConst() && firstGrandIsBool
						...
						if (constValue == firstGrandConstValue) {
							if (performTransformation(vp->comp(),
							            "%sReduced 2 NOTs of bool in node "
								...
								if (firstGrandChild->getOpCode().isLong())
									newChild = TR::Node::create(
									        TR::l2i, 1, firstGrandChild);
...
//
static TR::Node *
constrainCmplessthan(OMR::ValuePropagation *vp, TR::Node *node, TR::Node *lhsChild, TR::Node *rhsChild, bool orEqual)
{
	...
	if (result >= 0) {
		if ((lhsGlobal || vp->lastTimeThrough())
		        && performTransformation(vp->comp(), "%sChanging node [%p] %s into constant %d\n", OPT_DETAILS,
		                node, node->getOpCode().getName(), result)) {
			vp->removeChildren(node);
	...
	if (node->getOpCodeValue() != TR::table) {
		...
		if (!isInt64 && constraint && constraint->asIntConstraint()) {
			...
			if (low >= minCase && high <= maxCase)
				node->setCannotOverflow(true); // Let evaluator know value range of selector is covered
				                               // by min and max case
		} // if IntConstraint
...
static void
constrainClassObjectLoadaddr(OMR::ValuePropagation *vp, TR::Node *node, bool isGlobal)
{
...
constrainLoadaddr(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (symbol->isAddressOfClassObject()) {
		...
		TR::VPConstraint *constraint = TR::VPClass::create(vp, TR::VPClassType::create(vp, symRef, false, true),
		        NULL, NULL, NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
		vp->addGlobalConstraint(node, constraint);
...
void
constrainNewlyFoldedConst(OMR::ValuePropagation *vp, TR::Node *node, bool isGlobal)
{
	switch (node->getOpCodeValue()) {
	case TR::iconst: constrainIntConst(vp, node, isGlobal); break;

	case TR::lconst: constrainLongConst(vp, node, isGlobal); break;

	case TR::aconst: constrainAConst(vp, node, isGlobal); break;
...
handleNullCheck(OMR::ValuePropagation *vp, TR::Node *node, bool resolveChkToo)
{
	...
	if (!resolveChkToo) {
		...
		/// vp->createValueConstraint(referenceChild, vp->AbsoluteConstraint, TR::VPNullObject::create(vp)),
		/// node);
		vp->createExceptionEdgeConstraints(TR::Block::CanCatchNullCheck, NULL, node);
	}
...
handleResolveCheck(OMR::ValuePropagation *vp, TR::Node *node, bool nullChkToo)
{
	...
	if (nullChkToo)
		vp->createExceptionEdgeConstraints(
		        TR::Block::CanCatchNullCheck | TR::Block::CanCatchResolveCheck, NULL, node);
	else
...
constrainNullChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (nullCheckResult == 1
	        && performTransformation(
	                vp->comp(), "%sRemoving redundant null check node [%p]\n", OPT_DETAILS, node)) {
...
constrainResolveChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (removeTheCheck
	        && performTransformation(
	                vp->comp(), "%sRemoving redundant resolve check node [%p]\n", OPT_DETAILS, node)) {
		...
		if (child->getOpCode().isTreeTop()) {
			if (vp->comp()->useCompressedPointers() && child->getOpCode().isStoreIndirect()) {
				...
				TR_ASSERT(
				        child->getReferenceCount() == 1, "Treetop child of ResolveCHK has other uses");
...
constrainResolveNullChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (handleNullCheck(vp, node, !removeTheCheck) == 1) {
		...
		if (performTransformation(vp->comp(), "%sChanging ResolveAndNULLCHK node into a NULLCHK node [%p]\n",
			...
			node->setSymbolReference(vp->comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(
...
constrainBndChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (sizeBefore && index) {
		if (index->getLowInt() >= 0 && index->getHighInt() < sizeBefore->getLowInt()
		        && performTransformation(
		                vp->comp(), "%sRemoving unnecessary bound check node [%p]\n", OPT_DETAILS, node)) {
...
constrainBndChkWithSpineChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (sizeConstraint && !vp->comp()->generateArraylets()) {
		...
		if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(sizeConstraint->getHighInt(), elementSize)
		        ...
		        && performTransformation(vp->comp(),
		                "%sRemoving unnecessary spine check from composite node [%p] : min arraylength = %d, "
		                "max arraylength=%d, element size=%d\n",
		                OPT_DETAILS, node, sizeConstraint->getLowInt(), sizeConstraint->getHighInt(),
		                elementSize)) {
			removeSpineChk = true;
		}
	}
...
constrainArrayStoreChk(OMR::ValuePropagation *vp, TR::Node *node)
{
			...
			if (sig && sig[0] == '[') {
				...
				//
					...
					if (arrayComponentClass)
						isInstance = vp->fe()->isInstanceOf(objectClass, arrayComponentClass,
						        object->isFixedClass(), array->isFixedClass());
					// isInstance = vp->fe()->isInstanceOf(object->getClass(), arrayComponentClass,
					// object->isFixedClass(), array->isFixedClass());
					...
					if (isInstance == TR_yes) {
						...
						mustFail = true;
					        ...
					        && !TR::Compiler->cls.isClassArray(vp->comp(), arrayComponentClass)
						...
						if (vp->trace())
							traceMsg(vp->comp(),
							        "Setting arrayComponentClass on ArrayStoreChk node "
							        "[%p] to [%p]\n",
	...
	if (canBeRemoved) {
		...
		if (performTransformation(
			...
			//
			TR::Node::recreate(node, TR::treetop);
			// DemandLiteralPool can add extra aload to arraystore check node, but treetop can only have one
			// child
...
constrainArrayChk(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (removeThisNode
	        && performTransformation(
	                vp->comp(), "%sRemoving redundant array check node [%p]\n", OPT_DETAILS, node)) {
...
constrainArraycopy(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (node->getNumChildren() == 5) {
		...
		if (elementType != TR::NoType
		        && performTransformation(
		                vp->comp(), "%sTransforming arraycopy node [%p]\n", OPT_DETAILS, node)) {
	...
	if (node->getNumChildren() == 3) {
		...
		if (!alreadyAdded)
			vp->_scalarizedArrayCopies.add(new (vp->comp()->trStackMemory())
...
constrainBCDToIntegral(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (node->getType().isInt64()) {
		// If a node has precision < the max precision (for its data type) and has the isNonNegative (x >= 0)
		// flag set, we know that it can't possibly be represented as a negative number, so the min value for
		// the constraint must be at least 0
		if (p < node->getMaxIntegerPrecision() && node->isNonNegative())
			constrainRangeByPrecision(
			        TR::getMinSigned<TR::Int64>(), TR::getMaxSigned<TR::Int64>(), p, lo, hi, true);
		else
			constrainRangeByPrecision(
			        TR::getMinSigned<TR::Int64>(), TR::getMaxSigned<TR::Int64>(), p, lo, hi);
	} else {
		if (p < node->getMaxIntegerPrecision() && node->isNonNegative())
			constrainRangeByPrecision(
			        TR::getMinSigned<TR::Int32>(), TR::getMaxSigned<TR::Int32>(), p, lo, hi, true);
		else
			constrainRangeByPrecision(
			        TR::getMinSigned<TR::Int32>(), TR::getMaxSigned<TR::Int32>(), p, lo, hi);
	}
