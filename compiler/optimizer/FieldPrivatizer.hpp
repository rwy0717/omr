...
public:
    ...
    // This Optimization looks to convert locally declared arrays/structs/etc in languages like C/C++ which are accessed
    // using indirect loads/stores with direct loads/stores Ex: iiload  #57 shadow sym m[]
    //   aadd
    ...
    // the second part of this optimization will be an alias refinement which will try to remove m and m[] from alias
    // sets of temps if we are able to eliminate all references to m and m[] in the trees.
...
private:
    ...
    CS2::ABitVector<TR::Allocator>
        _epValueNumbers; // this bit vector is to keep track of value numbers that we know are 'safe' to transform.
                         // Allowing us to catch move opportunities.
};
