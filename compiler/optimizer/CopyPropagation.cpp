...
template <class T>
TR::Node *
getNodeParent(TR::Node *lookForNode, TR::Node *curNode, vcount_t visitCount, T &ci)
{
...
bool
foundInterferenceBetweenCurrentNodeAndPropagation(TR::Compilation *comp, bool trace, TR::Node *currentNode,
        TR::Node *storeNode, TR::list<TR::Node *> &checkNodes, TR::SparseBitVector &refsToCheckIfKilled)
{
	...
	if (trace) {
		comp->getDebug()->trace("foundInterferenceBetweenCurrentNodeAndPropagation: currentNode %p storeNode "
		                        "%p checkNodes [",
		        currentNode, storeNode);
		for (auto checkNodesCursor = checkNodes.begin(); checkNodesCursor != checkNodes.end();
		        ++checkNodesCursor) {
...
private:
	...
	{
		_parentOfTargetNode = getNodeParent(
		        _targetNode, _targetTreeTop->getNode(), comp()->incOrResetVisitCount(), _targetChildIndex);

		TR_ASSERT(_parentOfTargetNode != 0,
		        "could not find _targetNode " POINTER_PRINTF_FORMAT " parent, _targetTreeTop root "
	        ...
	        TR::TreeTop *targetTreeTop, TR::Node *targetNode, TR::Node *sourceTree)
	{
		while (targetTreeTop->getNode()->containsNode(targetNode, comp()->incOrResetVisitCount())) {
			size_t childIndex;
			TR::Node *parentNode = getNodeParent(
			        targetNode, targetTreeTop->getNode(), comp()->incOrResetVisitCount(), childIndex);
	...
	void fixupBCDPrecisionIfRequired()
	{
		...
		if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
			...
			if (newNode->getNumChildren() == 2) {
				...
				// for a oldNode with prec=30 and an newNode with prec=31 then if the symbol sizes match
				// and the other checks below pass then can reduce the newPrecision to 30 so a pdModPrec
				// correction is not needed (as the propagated symbol will not have the top nibble
				// 30->31 set).
				if (newNode->getType().isAnyPacked()) {
					        ...
					        oldSize == newSymSize) // 16 == 16
					{
						...
						newPrecision = oldPrecision; // 31->30 -- no correction needed for same
			...
			TR_ASSERT(newPrecision >= oldPrecision,
			...
			if (needsClean || needsPrecisionCorrection) {
				dumpOptDetails(comp(),
				        "node %p precision %d != propagated symRef #%d precision %d and/or needsClean "
				        "(%s)\n",
				        oldNode, oldPrecision, newNode->getSymbolReference()->getReferenceNumber(),
				...
				if (needsPrecisionCorrection) {
					...
					pdModPrecNode->setDecimalPrecision(oldPrecision); // set to existing node's
					                                                  // precision
	...
	void addConversionIfRequired()
	{
		...
		if (newNode->getType().getDataType() != oldNode->getType().getDataType()) {
			...
			TR::Node *convNode =
			        TR::Node::create(convOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
	...
	void propagateThroughToCommoningPoints()
	{
		...
		if (oldNode->getReferenceCount() > 0) {
			// Step 1: Propagate this RHS change to anything with a commoned old node further down the
			// block. Common the rematerialization instead.

			// The idea is that we have replaced the RHS of some node, but other nodes further down the
			// block could have commoned the old node. We want to replace those commonings with a commoning
			// to the new RHS of the node because the value is the same.

			// Get a list of load sym refs for the new rematerialized RHS - use this to see if any part of
			// the new node is killed during tree traversal.
			...
			// Start at _targetTreeTop to replace other instances of _targetNode in _targetTreeTop as well
			// as to the end of the block.
			...
			while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
				...
				if (oldNode->getReferenceCount() == 0) {
					...
					if (trace()) {
						comp()->getDebug()->trace("%s   Propagating new "
						                          "RHS " POINTER_PRINTF_FORMAT " stops because "
						                          "oldNode ref "
						                          "count = 0\n",
						        OPT_DETAILS, newNode);
					}
...
TR_CopyPropagation::perform()
{
...

	{
		...
		typedef TR::typed_allocator<std::pair<TR::Node *const, TR::deque<TR::Node *, TR::Region &> *>,
		        TR::Region &>
		...
		StoreNodeMap storeMap(
		        (StoreNodeMapComparator()), StoreNodeMapAllocator(trMemory()->currentStackRegion()));
		...
		for (TR::TreeTop *tree = comp()->getStartTree(); tree; tree = tree->getNextTreeTop()) {
			...
			if (storeLookup != storeMap.end()) {
				for (auto itr = storeLookup->second->begin(), end = storeLookup->second->end();
				        itr != end; ++itr) {
					...
					if (prevDefSlot < 0) // pending pushes
						prevSymRefIsPreferred = true;
					        ...
					        < comp()->getOwningMethodSymbol(prevDefNode->getSymbolReference()
					...
					if (prevDefNode->getType().isBCD()) {
						TR_ASSERT(defNode->getType().isBCD(),
						        "expecting defNode to be a BCD type (dt=%s) when prevDefNode "
						        "is a BCD type (dt=%s)\n",
						...
						if (defNode->getType().isBCD()
							...
							dumpOptDetails(comp(),
							        "%s   Def nodes %p (precision = %d) and %p (precision "
							        "= %d) are equivalent\n",
		...
		for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end(); itr != end; ++itr) {
			...
			for (useCursor.SetToFirstOne(); useCursor.Valid(); useCursor.SetToNextOne()) {
				...
				if (performTransformation(comp(),
					...
					comp()->setOsrStateIsReliable(false); // could check if the propagation was done
					                                      // to a child of the OSR helper call here
		...
		for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
			...
			if (useDefInfo->getUseDef(defs, i)) {
				...
				if (defTree->getEnclosingBlock()->startOfExtendedBlock()
					...
					// PR62462: LocalCSE is a fine and trustworthy opt, and we're not trying to do
					// its job here. Our analysis (particularly of RegLoads) assumes it is
					// propagating copies into successor blocks, and doesn't handle propagation
					// within an extended block properly.
					//
					continue;
				}
				...
				TR::Node *rhsOfStoreDefNode = defNode->getOpCode().isStoreIndirect()
				        ...
				        : defNode->getFirstChild();
				TR::SymbolReference *copySymbolReference = defNode->getOpCode().hasSymbolReference()
				...
				if (performTransformation(comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n",
					...
					if (!isRegLoad && (loadNode->getOpCodeValue() != TR::loadaddr) && // loadaddr
					                                                                  // doesn't
					                                                                  // need to be
					                                                                  // anchored
					        (loadNode->getOpCode().hasSymbolReference())
					        && (loadNode->getSymbolReference() == copySymbolReference)
					        && (rhsOfStoreDefNode != loadNode)) // Modifications to this condition
					                                            // should be mirrored in the NOTEMPS
					                                            // check above.
					{
					...
					if (_propagatingWholeExpression) {
						...
						if (!optimizer()->areSyntacticallyEquivalent(rematExpression,
						            newExpression, comp()->incOrResetVisitCount())) {
						...
						replaceCopySymbolReferenceByOriginalIn(copySymbolReference,
						        rhsOfStoreDefNode, useNode, defNode, baseAddr, baseAddrAvail);
						...
						if (bbstartNode->getNumChildren() > 0) {
							for (int i = 0;
							        ...
							        i++) {
								TR::Node *child =
								        bbstartNode->getFirstChild()->getChild(i);
							...
							if (regLoadNode) {
								...
								TR::TreeTop *exitTree =
								        cursorTree->getExtendedBlockExitTreeTop()
								                ->getNextTreeTop();
...
void
TR_CopyPropagation::collectUseTrees(TR::TreeTop *tree, TR::Node *node, TR::NodeChecklist &checklist)
{
	...
	while (currentTree) {
		...
		if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress())
		        ...
		        && performTransformation(comp(),
		                "%s   Rematerializing indirect load from auto in node %p (temp #%d)\n", OPT_DETAILS,
		                nextNode->getFirstChild(), nextNode->getSymbolReference()->getReferenceNumber())) {
			comp()->setOsrStateIsReliable(false); // could check if the propagation was done to a child of
			                                      // the OSR helper call here
			TR::Node *newChild =
			        TR::Node::create(node, comp()->il.opCodeForIndirectLoad(node->getDataType()), 1);
...
*/
void
TR_CopyPropagation::replaceCopySymbolReferenceByOriginalIn(TR::SymbolReference *copySymbolReference, TR::Node *origNode,
        TR::Node *node, TR::Node *defNode, TR::Node *baseAddrNode, bool baseAddrAvail)
{
		...
		if (node->getOpCode().hasSymbolReference()) {
...
#endif
			{
				...
				if (origNode->getOpCode().isLoadIndirect()) {
					...
					if (trace() || comp()->cg()->traceBCDCodeGen())
						traceMsg(comp(),
						        "node %s (%p) #%d->#%d, origNode %s (%p) #%d, defNode %s (%p) "
						        "#%d,copySymRef #%d\n",
					...
					if (origNode->getNumChildren() == 2) {
						...
						// pass then can reduce the newPrecision to 30 so a pdModPrec correction
						// is not needed (as the propagated symbol will not have the top nibble
						// 30->31 set).
						...
						if (node->getType().isAnyPacked()) {
							if (origSymPrecision > nodePrecision && // 31 > 30
							        nodePrecision == origNode->getDecimalPrecision()
							        && // 30 == 30
							        ...
							        nodeSize == origSymSize) // 16 == 16
							{
								if (trace() || comp()->cg()->traceBCDCodeGen())
									traceMsg(comp(),
									        "reduce newPrecision %d->%d for odd to "
									        "even truncation (origNode %s (%p) "
									        ...
									        node->getOpCode().getName(), node,
									        nodePrecision);
								newPrecision = nodePrecision; // 31->30 -- no correction
								                              // needed for same byte
								                              // odd to even truncation
								                              // if the origNode was
								                              // only storing out the
								                              // even $ of digits
							}
						}
					}

					// if newPrecision < currentPrecision then one or more digits will be lost
					// during the copyPropagation -- the isCorrect* routines disallow this
					...
					if (trace() || comp()->cg()->traceBCDCodeGen())
						traceMsg(comp(),
						        "needsClean = %s, needsPrecisionCorrection = %s (cur %d, new "
						        "%d)\n",
						        needsClean ? "yes" : "no",
					...
					//
					if (needsClean || needsPrecisionCorrection) {
						// If the copy propagation is going to change the size of the node being
						// replaced then a modifyPrecision node must be prepended in case the
						// new parent requires explicit widening (such as if the parent is a
						// call). Also if the defNode was going to clean must also insert a
						...
						dumpOptDetails(comp(),
						        "node %p precision %d != propagated symRef #%d precision %d "
						        "and/or needsClean (%s) so create nodeCopy %p "
						        "(isTwoChildrenCase=%s)\n",
						...
						if (needsPrecisionCorrection) {
							                ...
							                // op (the coming clean) do this
							{
								...
								dumpOptDetails(comp(),
								        "create %s (0x%p) to correct precision "
								        "%d->%d\n",
								...
								node->setFlags(0);
								dumpOptDetails(comp(),
								        "modify load 0x%p to %s to correct precision "
								        "%d->%d\n",
								        node, node->getOpCode().getName(), newPrecision,
								        currentPrecision);

								node = nodeCopy; // so the recursion continues from the
								                 // correct point below
							}
						}
						...
						dumpOptDetails(comp(),
						        "attach new first %s (%p) and second %s (%p) to node %s (%p) "
						        "in the twoChildren=true case\n",
...
*/
void
TR_CopyPropagation::replaceCopySymbolReferenceByOriginalRegLoadIn(TR::Node *regLoadNode, TR::Node *useNode,
...
bool
TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node *useNode, TR::Node *storeNode, TR::Node *loadNode)
{
#ifdef J9_PROJECT_SPECIFIC
	if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
		dumpOptDetails(comp(),
		        "isLoadNodeSuitableForPropagation : useNode %p (prec %d), loadNode %p (prec %d) -- "
		        "isCorrect=%s (only correct when useNodePrec <= loadNodePrec)\n",
		...
		if (useNode->getDecimalPrecision() > loadNode->getDecimalPrecision()) {
			...
			// have to disallow this case because will not be able to clean useNode after propagation for
			// prec > max
			dumpOptDetails(comp(),
			        "isLoadNodeSuitableForPropagation=false for useNode %s (%p) prec %d > max %d and "
			        "mustClean store %p\n",
...
void
TR_CopyPropagation::adjustUseDefInfo(TR::Node *defNode, TR::Node *useNode, TR_UseDefInfo *useDefInfo)
{
...
TR::Node *
TR_CopyPropagation::areAllDefsInCorrectForm(TR::Node *useNode, const TR_UseDefInfo::BitVector &defs,
        TR_UseDefInfo *useDefInfo, int32_t firstRealDefIndex, TR::Node *&loadNode, int32_t &regNumber,
        TR::Node *&baseNode)
{
	...
	//...exception point that could reach a catch
	// catch block
	// load b  //  copy propagating "a" here is incorrect since we will end up propagating rhs2 value (in "a")
	// instead of rhs1 value (in "b") changing program semantics
	//
	...
	for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
		...
		TR::Node *rhsOfStoreDefNode = defNode->getOpCode().isStoreIndirect() ? defNode->getChild(1)
		                                                                     : defNode->getChild(0);
...

		{
			...
			if (regNumber == -1) {
				...
				collectNodesForIsSafeChecks(
				        rhsOfStoreDefNode, anchorCheckNodes, comp()->incOrResetVisitCount(), false);
	...
	if (lastDefNode != NULL) {
		...
		collectNodesForIsCorrectChecks(
		        loadNode, checkNodes, refsToCheckIfKilled, comp()->incOrResetVisitCount());
		...
		if (!isCorrectToPropagate(
		            useNode, lastDefNode, checkNodes, refsToCheckIfKilled, regNumber, defs, useDefInfo)) {
			return NULL;
		}
	}
...
TR_CopyPropagation::isSafeToPropagate(TR::Node *storeNode, TR::Node *loadNode)
{
	...
	if (lookup != _storeTreeTops.end()) {
		...
		if (loadNode) {
			...
			// Walk the tree backward to first treetop inside extended basic block, reject if org sym is
			// killed
...
bool
TR_CopyPropagation::containsNode(TR::Node *node1, TR::Node *node2, bool searchClones)
{
	vcount_t curVisit = comp()->getVisitCount();
...
bool
TR_CopyPropagation::containsLoadOfSymbol(TR::Node *node, TR::SymbolReference *symRef, TR::Node **loadNode)
{
	vcount_t curVisit = comp()->getVisitCount();
...
bool
TR_CopyPropagation::isCorrectToPropagate(TR::Node *useNode, TR::Node *storeNode, TR::list<TR::Node *> &checkNodes,
        ...
        TR_UseDefInfo *useDefInfo)
{
	...
	for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
		...
		if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
			...
			if (isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode, next->getExit(),
...
bool
TR_CopyPropagation::isRedefinedBetweenStoreTreeAnd(TR::list<TR::Node *> &checkNodes,
        ...
        const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
	...
	static char *useRecursiveIsRedefinedBetweenStoreTreeAnd = feGetEnv("TR_"
	                                                                   "useRecursiveIsRedefinedBetweenStoreTreeAn"
	                                                                   "d");
	...
	while (currentTree != NULL) {
		...
		while (!(currentTree->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)) {
			...
			// TR_ASSERT(_storeTree->getNode()->getOpCode().isStoreDirect(), "Def node should be a direct
			// store\n");
...
#endif
			{
				...
				if (regNumber == -1) {
					if (foundInterferenceBetweenCurrentNodeAndPropagation(comp(), trace(),
					            currentNode, storeNode, checkNodes, refsToCheckIfKilled)) {
...
bool
TR_CopyPropagation::recursive_isRedefinedBetweenStoreTreeAnd(TR::list<TR::Node *> &checkNodes,
        TR::SparseBitVector &refsToCheckIfKilled, TR::Node *storeNode, TR::TreeTop *exitTree, int32_t regNumber,
        const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
	TR::TreeTop *currentTree = exitTree;
	...
	for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
		...
		if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
		        && ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
			if (recursive_isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode,
			            next->getExit(), regNumber, defs, useDefInfo))
				return true;
		}
	}
...
bool
TR_CopyPropagation::isCorrectToReplace(
        TR::Node *useNode, TR::Node *storeNode, const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
	...
	if (useNode->getType().isBCD() && storeNode->getType().isBCD()) {
		dumpOptDetails(comp(),
		        "isCorrectToReplace : useNode %p (prec %d), storeNode %p (prec %d) -- isCorrect=%s (only "
...
TR::Node *
TR_CopyPropagation::isIndirectLoadFromRegister(TR::Node *node, TR::Node *&baseNode)
{
	baseNode = NULL;
...
TR::Node *
TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node *defNode, TR::Node *node)
{
	...
	//    aiadd (internalPtr )                                                            [0x2B016B20]
	//    bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=- nc=2 addr=4 flg=0x8000
	//      loadaddr <auto slot 40> [id=152:"PSA"] [#1.63 Auto] [flags 0x4000008 0x0 ]    [0x2B016AD0]
	//      bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=- nc=0 addr=4 iconst 1212 (X!=0 X>=0 ) [0x2B016B70]
	//      bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=- nc=0 flg=0x104
	//
	if (node->getOpCode().isLoadIndirect()) {
...
bool
TR_CopyPropagation::isNodeAvailableInBlock(TR::TreeTop *useTree, TR::Node *loadNode)
{
...
TR::Node *
TR_CopyPropagation::isValidRegLoad(TR::Node *rhsOfStoreDefNode, TR::TreeTop *defTree, int32_t &regNumber)
{
	regNumber = -1;
							...
							if (rhsChild == rhsOfStoreDefNode) {
								if (regNumber == -1) {
									...
									isRegLoad = true;
								        ...
								        == regChild->getGlobalRegisterNumber())
									isRegLoad = true;
