...

    {
        ...
        for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
            ...
            if (useDefInfo->getUseDef(defs, i)) {
                ...
                // Bail out if we're in a NOTEMPS method and we're going to create a temp.  This conditional should
                // match the conditional that covers the anchoring code below.
                if (!isRegLoad && (loadNode->getOpCodeValue() != TR::loadaddr)
                    ...
                    && (loadNode->getSymbolReference() == copySymbolReference) && (rhsOfStoreDefNode != loadNode)
