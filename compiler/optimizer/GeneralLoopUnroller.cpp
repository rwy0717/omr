...
getConstOpCode(TR::DataType type)
{
	...
	default: TR_ASSERT(false, "unsupported type"); return TR::iconst;
	}
...
static TR_ScratchList<TR::CFGEdge> *
findCorrespondingCFGEdges(TR_Structure *from, TR_Structure *to, TR::Compilation *comp)
{
	if (from->asBlock()) {
...
{
	TR::Block *block = subNode->getStructure()->asBlock() != NULL ? subNode->getStructure()->asBlock()->getBlock()
	                                                              : NULL;
...
bool
TR_LoopUnroller::shouldConnectToNextIteration(TR_StructureSubGraphNode *subNode, TR_RegionStructure *loop)
{
...
void
TR_LoopUnroller::unrollLoopOnce(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode, bool finalUnroll)
{
	...
	for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
		...
		TR_StructureSubGraphNode *clonedSubNode = new (_cfg->structureRegion())
		        TR_StructureSubGraphNode(clonedSubStruct);
				...
				if (_iteration != 1) // for all iterations except the first iteration
				{
					if (shouldConnectToNextIteration(toStructureSubGraphNode(subNode), loop)) {
						addEdgeAndFixEverything(loop, *edge, GET_PREV_CLONE_NODE(subNode),
						        newEntryNode, false, false, false, BackEdgeFromPrevGeneration);
					} else {
						if (trace())
							traceMsg(comp(),
							        ...
							        subNode->getNumber());
						addEdgeAndFixEverything(loop, *edge, GET_PREV_CLONE_NODE(subNode),
						        loop->getEntry(), false, false, true, BackEdgeToEntry);
					}
				}
			} else // Type D
			{
...
void
TR_LoopUnroller::redirectBackEdgeToExitDestination(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode,
        TR_StructureSubGraphNode *fromNode, bool notLoopBranchNode)
{
	...
	addEdgeForSpillLoop(loop, exitEdge, fromNode,
	        findNodeInHierarchy(
	                loop->getParent()->asRegion(), toStructureSubGraphNode(exitEdge->getTo())->getNumber()),
	        false, BackEdgeFromLastGenerationCompleteUnroll, notLoopBranchNode);
}

void
TR_LoopUnroller::modifyBranchTree(
        TR_RegionStructure *loop, TR_StructureSubGraphNode *loopNode, TR_StructureSubGraphNode *branchNode)
{
	...
	if (originalLimitNode->getOpCode().isLoadConst() && getTestIsUnsigned()) {
		...
		TR_ASSERT(!originalLimitNode->getType().isAddress(),
		        "Counted loop with a constant address limit node is unexpected, test node is %p",
		        getTestNode());
	...
	if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
		...
		pBlock->append(TR::TreeTop::create(
		        comp(), TR::Node::createif(opCode, originalLimitClone, c2, spillBlock->getEntry())));
	...
	if (1) /// moveEntryNode
	{
		...
		TR::TreeTop *iterGuard = TR::TreeTop::create(
		        comp(), TR::Node::createif(opCode, firstChild, dupNewLimit, spillBlock->getEntry()));
		...
		if (lastTree->getNode()->getOpCodeValue() != TR::Goto) {
			...
			for (auto edge = pBlock->getSuccessors().begin(); edge != pBlock->getSuccessors().end();
			        ++edge) {
			...
			TR::CFGEdge::createEdge(
			        newSubNode, parent->findSubNodeInRegion(nextBlock->getNumber()), trMemory());

			_cfg->removeEdge(edgeToRemove);
			parent->removeEdge(
			        predBlock, parent->findSubNodeInRegion(nextBlock->getNumber())->getStructure());
			_loopIterTestBlock = newBlock;
		} else {
			// loop invariant block ends with a goto
			// create a new block - move the goto in the new block and add append an if to the loop inv
			// block
			//
...
void
TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode)
{
		...
		if (!loop->isExitEdge(current)) {
			...
			if (predNode->getNumber() < _numNodes) {
				if (shouldConnectToNextIteration(predNode, loop)) {
					addEdgeAndFixEverything(loop, current, predNode, _firstEntryNode, true, true,
					        true, BackEdgeFromPrevGeneration);
					loop->removeEdge(predNode->getStructure(), loop->getEntry()->getStructure());
				} else {
					if (trace())
						traceMsg(comp(),
						        "Redirecting cold edge from block_%d to unrolled loop header "
						        "rather than going to the next iteration\n",
						        predNode->getNumber());
					addEdgeAndFixEverything(loop, current, predNode, loop->getEntry(), true, false,
					        true, BackEdgeToEntry);
				}
			}
		}
	}
	...
	if (_spillLoopRequired) {
		...
		if (1) {
			...
			//
			if (1) {
				TR::Block *branchDestination =
				        branchTreeTop->getNode()->getBranchDestination()->getNode()->getBlock();
					...
					if (loop->getPrimaryInductionVariable()
						...
						if (!getTestIsUnsigned()) {
							...
							if (is64BitComparison) {
								...
								constNode = TR::Node::lconst(
								        boundNode, (int64_t)getLoopStride());
							} else {
								addOp = isAddress ? TR::aiadd : TR::iadd;
								constNode =
								        TR::Node::iconst(boundNode, getLoopStride());
							}
				...
				TR::Block *gotoBlock = TR::Block::createEmptyBlock(gotoNode, comp(),
				...
				_cfg->addEdge(
				        TR::CFGEdge::createEdge(gotoBlock, spillLoop->getEntryBlock(), trMemory()));

				TR_StructureSubGraphNode *gotoSubNode = new (_cfg->structureRegion())
				        TR_StructureSubGraphNode(new (_cfg->structureRegion())
			...
			if (!e) // there do not remain an edge in L going to X
			{
				if (exitNode->getStructure()->getParent() == parent) {
					bool found = (std::find(loopNode->getSuccessors().begin(),
					                      loopNode->getSuccessors().end(), correspondingExitEdge)
					...
					found = (std::find(exitNode->getPredecessors().begin(),
					                 exitNode->getPredecessors().end(), correspondingExitEdge)
	...
	if (isCountedLoop() && _unrollKind != CompleteUnroll) {
		...
		dumpOptDetails(comp(), "%schanged entry node of region %d [%x] to %d\n", OPT_DETAILS,
		        cursor->getNumber(), cursor, newNumber);
...
void
TR_LoopUnroller::removeExternalEdge(TR_RegionStructure *parent, TR_StructureSubGraphNode *from, int32_t to)
{
...
void
TR_LoopUnroller::getLoopPreheaders(TR_RegionStructure *loop, TR_ScratchList<TR::Block> *preheaders)
{
...
static bool
isBiv(TR_RegionStructure *loop, TR::SymbolReference *symRef, TR_BasicInductionVariable *&biv)
{
				...
				if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub())
					...
					if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
						...
						if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
						        && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
							if (firstChild->getSecondChild()->getOpCodeValue()
							        == TR::iconst)
								offsetValue =
						...
						if (firstChild->getOpCode().isLoad()) {
							...
							if (canMatch || canMatchAsIP) {
								if (trace())
									traceMsg(comp(),
									        "\tFound internal pointer %p with iv "
									        "%d in offset node %p\n",
									        node,
								...
								IntrnPtr *intrnPtr =
								        (IntrnPtr *)trMemory()->allocateStackMemory(
								                sizeof(IntrnPtr));
								intrnPtr->symRefNum = node->getSymbolReference()
								                              ->getReferenceNumber();
								if (canMatch) {
									intrnPtr->bivNum = -1;
									intrnPtr->biv = biv;
...
TR_LoopUnroller::examineArrayAccesses()
{
	...
	for (ListsOfArrayAccesses *loaa = it.getFirst(); loaa; loaa = it.getNext()) {
		...
		for (aa1 = it1.getFirst(); aa1 && (aa2 = it1.getNext()); aa1 = aa2) {
			...
			IntrnPtr *intrnPtr1 =
			        findIntrnPtr(aa1->intrnPtrNode->getSymbolReference()->getReferenceNumber());
			IntrnPtr *intrnPtr2 =
			        findIntrnPtr(aa2->intrnPtrNode->getSymbolReference()->getReferenceNumber());
		...
		{ // and it doesn't have an internal pointer
			if (trace())
				traceMsg(comp(),
				        "List of array accesses with sym ref %d does not qualify for aliasing "
				        "refinement\n",
				        loaa->symRefNum);
...
TR_LoopUnroller::refineArrayAliasing()
{
	...
	if (_listOfListsOfArrayAccesses.isEmpty()
	        || !performTransformation(
	                comp(), "%sRefine array aliasing in loop %d\n", OPT_DETAILS1, _loop->getNumber()))
		return;
...
int32_t
TR_LoopUnroller::unroll(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode)
{
...
void
TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode)
{
	// Reset the mappers.
	_iteration = 0;
	memset(_blockMapper[CURRENT_MAPPER], 0, _numNodes * sizeof(TR::Block *));
...
void
TR_LoopUnroller::addEdgeForSpillLoop(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
	...
	for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
		...
		// FALLS INTO
			...
			if (newFromNextBlock != newTo) // but the copy does not fall through
			{
				...
				gotoNode->setLocalIndex(CREATED_BY_GLU);
				TR::Block *gotoBlock =
				        TR::Block::createEmptyBlock(lastNode, comp(), newFrom->getFrequency(), newFrom);
...
void
TR_LoopUnroller::addExitEdgeAndFixEverything(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
...
void
TR_LoopUnroller::addEdgeAndFixEverything(TR_RegionStructure *region, TR::CFGEdge *originalEdge,
...
bool
TR_LoopUnroller::cfgEdgeAlreadyExists(TR::Block *from, TR::Block *to, EdgeContext edgeContext)
{
	for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge) {
		...
		if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
			...
			if (firstRealTree == lastRealTree && firstRealTree->getNode()->getOpCodeValue() == TR::Goto) {
				TR::Block *dest =
				        firstRealTree->getNode()->getBranchDestination()->getNode()->getBlock();
...
bool
TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode *from, TR_StructureSubGraphNode *to)
{
	for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge)
...
bool
TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode *from, int32_t toNum)
{
	for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge)
...
void
TR_LoopUnroller::cloneBlocksInRegion(TR_RegionStructure *region, bool isSpillLoop)
{
	TR_ScratchList<TR::Block> blocksInRegion(trMemory());
...
{
	TR_RegionStructure *clonedRegion = new (_cfg->structureRegion()) TR_RegionStructure(comp(), 0xDeadF00d); // for
	                                                                                                         // now
...
void
TR_LoopUnroller::fixExitEdges(TR_Structure *s, TR_Structure *clone, TR_StructureSubGraphNode *branchNode)
{
	if (s->asBlock())
		return;
...
{
	if (comp->getOption(TR_DisableTLE) || !comp->cg()->getSupportsTM()) // if TLE is not supported or on, it is not
	                                                                    // possible
		return false;
...
bool
TR_LoopUnroller::isWellFormedLoop(TR_RegionStructure *loop, TR::Compilation *comp, TR::Block *&loopInvariantBlock)
{
	...
	if (loopNode && (loopNode->getPredecessors().size() == 1)) {
		...
		if (predBlock && predBlock->isLoopInvariantBlock()) {
			...
			for (auto e = entryNode->getPredecessors().begin(); e != entryNode->getPredecessors().end();
			        ++e) {
        ...
        , _listOfListsOfArrayAccesses(c->trMemory())
{
	...
	_branchToExit = !loop->contains(
	        branch->getBranchDestination()->getNode()->getBlock()->getStructureOf(), loop->getParent());
}

// ::unrollCountedLoop
bool
TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop, TR_PrimaryInductionVariable *piv,
        UnrollKind unrollKind, int32_t unrollCount, int32_t peelCount, TR::Optimizer *optimizer)
{
	...
	if (1) {
		...
		if (unroller._spillLoopRequired) {
			...
			if (!backEdgeFound) {
				...
				if (loopCanBeUnrolled) {
					// Either the entry block has the branch condition or the only 2 blocks in the
					// loop are the entry and the branch block Further the only code other from the
					// entry block in the loop must be the branch (and an optional async check)
					//
				} else {
					dumpOptDetails(comp,
					        "Cannot unroll loop %d: exit condition is not in a block containing a "
					        "backedge\n",
					        loop->getNumber());
					return false;
				}
		...
		if (1) {
			...
			if (!foundExitEdge) {
				dumpOptDetails(comp,
				        "Cannot unroll loop %d: eq loop with branch not an exit out of the loop\n",
				        loop->getNumber());
				return false;
			}
		}
		...
		if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
			if (piv->getDeltaOnExitEdge() != 1 && piv->getDeltaOnExitEdge() != -1) {
				dumpOptDetails(comp,
				        "Cannot unroll loop %d: eq loop with non unit inc/dec not supported\n",
				        loop->getNumber());
				return false;
			}
			...
			if ((TR::ILOpCode::isEqualCmp(op) && !unroller._branchToExit)
				...
				dumpOptDetails(
				        comp, "Cannot unroll loop %d: stange controlling test\n", loop->getNumber());
				return false;
			}
	...
	if (unrollKind == CompleteUnroll) {
		...
		if (!loop->contains(destStruct, loop->getParent())) {
			dumpOptDetails(comp,
			        "Cannot unroll loop %d: complete unroll of a loop with a reversed branch\n",
			        loop->getNumber());
			return false;
		}
	}
	...
	if (!performTransformation(comp,
	            ...
	            unrollKind == UnrollWithResidue ? "yes" : "no", unrollKind == CompleteUnroll ? "yes" : "no")) {
		return false;
	}
...
bool
TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop, int32_t unrollCount, int32_t peelCount,
	...
	if (unroller._spillLoopRequired) {
		...
		if (!backEdgeFound) {
			...
			if (loopCanBeUnrolled) {
				...
				// Further the only code other from the entry block in the loop must be the branch (and
...
TR_GeneralLoopUnroller::TR_GeneralLoopUnroller(TR::OptimizationManager *manager) : TR_LoopTransformer(manager)
{
...
void
TR_GeneralLoopUnroller::gatherStatistics(TR_Structure *str, int32_t &numNodes, int32_t &numBlocks, int32_t &numBranches,
...
void
TR_GeneralLoopUnroller::countNodesAndSubscripts(
...
static bool
exitsLoop(TR::Compilation *comp, TR_RegionStructure *loop, TR_StructureSubGraphNode *source)
{
...
// Returns the weight
int32_t
TR_GeneralLoopUnroller::weighNaturalLoop(TR_RegionStructure *loop, TR_LoopUnroller::UnrollKind &unrollKind,
        int32_t &unrollCount, int32_t &peelCount, int32_t &cost)
{
	...
	if (trace()) {
		traceMsg(comp(), "numUsed = %i, numUseOnly = %i(min:%i), numUseDefd = %i(max:%i)\n", numUsed,
		        numUseOnly, minNumUseOnly, numUseDefd, maxNumUseDefd);
	...
	if (piv) {
		...
		dumpOptDetails(
		        comp(), "loop %d has primary iv and %d legitimate branches\n", loop->getNumber(), numBranches);
		if (numBranches > 1) {
			...
			for (auto e = entryNode->getPredecessors().begin(); e != entryNode->getPredecessors().end();
				...
				if (source->getStructure()->asBlock()) {
					...
					if (!branchNode->getOpCode().isGoto()
						...
						dumpOptDetails(comp(),
						        "backedge branch [%p] is controlled using a biv, so unrolling "
						        "as non-counted\n",
						        branchNode);
						loop->setPrimaryInductionVariable(NULL);
						break;
	...
	if (_haveProfilingInfo && outerLoop) {
		...
		if (maxCount > 0) {
			...
			{
				dumpOptDetails(comp(),
				        "\trejecting loop because its not warm enough compared to the outer loop\n");
				return -1;
			}
	...
	if (piv) {
				...
				if (!multipleOfPreferred && !multipleOf2 && !multipleOf3) {
					...
					if (comp()->getOption(TR_Randomize)) {
						...
						/*unrollCount = (multipleOf4 ? randomInt(0,3) :
						       ...
						       traceMsg(comp(), "\nTR_Randomize
						   Enabled||TR_GeneralLoopUnroller::weighNaturalLoop(), unrollCount:%d
						   Original unrollCount:%d", unrollCount, originalUnrollCount);*/
					} else {
						unrollCount = (multipleOfPreferred
						                ? preferredUnrollFactor - 1
						                : multipleOf4 ? 3
						                              : multipleOf3 ? 2 : multipleOf2 ? 1 : 0);
					}
					if (unrollCount > 0) {
		...
		if (iterCount <= 0 || unrollKind == TR_LoopUnroller::NoUnroll) {
			if (iterCount <= 0)
				dumpOptDetails(comp(),
				        "\twe have a primary induction variable with unknown itercount stride=%d\n",
		...
		if ((unrollKind == TR_LoopUnroller::ExactUnroll) && (iterCount == (unrollCount + 1))) {
			...
			unrollKind = TR_LoopUnroller::CompleteUnroll;
			dumpOptDetails(comp(),
			        "\tloop unroll count %d iterCount %d, making complete unroll out of exact unroll\n",
	...
	if (cg()->getSizeOfCombinedBuffer()) {
		...
		case TR::sun_nio_cs_ISO_8859_1_Encoder_encodeArrayLoop: forceUnrollCount = true; break;
...
bool
TR_GeneralLoopUnroller::canUnrollUnCountedLoop(
				...
				if (node->getOpCodeValue() == TR::NULLCHK && (firstChild = node->getFirstChild())
					...
					for (TR::TreeTop *tt1 = tt; tt1 != exit; tt1 = tt1->getNextRealTreeTop()) {
						...
						if (n->getOpCode().isBooleanCompare()
							...
							if (trace()) {
								traceMsg(comp(),
								        "\tLoop %d can be unrolled because of common "
...
bool
TR_GeneralLoopUnroller::branchContainsInductionVariable(TR::Node *node, TR::SymbolReference *ivSymRef)
{
