...
OMR::CFGSimplifier::CFGSimplifier(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
{
	...
	if (compareNode->getOpCodeValue() == TR::ificmpeq) {
		...
		if (trace())
			traceMsg(comp(),
			        "   Found an ificmpeq of an instanceof against zero which throws on taken size\n");
		throwBlock = _next2;
		...
		if (trace())
			traceMsg(comp(),
			        "   Found an ificmpne of an instance of against zero which throws on the fallthrough "
			        "path\n");
		throwBlock = _next1;
	...
	TR::Node *checkcastAndNULLCHKNode = TR::Node::createWithSymRef(compareNode->getFirstChild(),
	        TR::checkcastAndNULLCHK, 2,
	        comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(comp()->getMethodSymbol()));
	TR_Pair<TR_ByteCodeInfo, TR::Node> *bcInfo = new (trHeapMemory()) TR_Pair<TR_ByteCodeInfo, TR::Node>(
	        &compareNode->getFirstChild()->getByteCodeInfo(), checkcastAndNULLCHKNode);
...
static bool
containsIndirectOperationImpl(TR::Node *node, TR::NodeChecklist *visited, int32_t depth)
{
...
OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
{
	...
	while (treeCursor->getNode()->getOpCode().isStoreDirect() && !treeCursor->getNode()->getOpCode().isWrtBar()) {
		...
		TR::Node *select =
		        TR::Node::create(storeNode, comp()->il.opCodeForTernarySelect(storeNode->getDataType()), 3);
		if (trace())
...
OMR::CFGSimplifier::simplifySimpleStore(bool needToDuplicateTree)
{
	...
	if (!performTransformation(comp(),
	            "%sReplace conditional store with store of an appropriate ternary at node [%p]\n", OPT_DETAILS,
	            compareNode))
		return false;
...
OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
{
	...
	if (!performTransformation(comp(),
	            "%sReplace ifacmpeq/ifacmpne of NULL node n%dn [%p] to a blcok ending in throw with a NULLCHK to a "
	            "catch which goes to block_%d\n",
...
OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
{
	...
	if (value1->getOpCode().isLoadConst()) {
		...
		// Is this code really valid when the trees guarded by the if rely on the condition checked in the 'if'
		// (e.g. we could have an indirect access without any checking in guarded code, because the test checked
		// if the value was NULL, in which case by performing the simplification we could end up with a crash
		// when the object is dereferenced)
		// else if (int1 == 0)
		//   swapCompare = true;
		else {
			return false;
	...
	if (getFallThroughBlock(_block) != joinBlock) {
		...
		_block->append(
		        TR::TreeTop::create(comp(), TR::Node::create(compareNode, TR::Goto, 0, joinBlock->getEntry())));
	}
	if (trace())
 ...
 */
bool
OMR::CFGSimplifier::simplifyCondCodeBooleanStore(
        TR::Block *joinBlock, TR::Node *branchNode, TR::Node *store1Node, TR::Node *store2Node)
{
	...
	if (NULL != child1 && child1->getOpCode().isInteger()) {
		...
		if (child1->getOpCode().isAnd()) {
			// First child must be a load; second child must be a const equal to the non-zero const that we
			// stored
	...
	if (compareNode->getOpCode().isCompareForEquality()) {
		if (compareNode->getOpCode().isCompareTrueIfEqual() && compareConst == value1) // if (cond), x == a; if
		                                                                               // (x == b) jump
			needToSwap = true;
		else if (!compareNode->getOpCode().isCompareTrueIfEqual() && compareConst == value2) // if (cond), x ==
		                                                                                     // a; f (x != a),
		                                                                                     // jump
			needToSwap = true;
	} else
	// TODO: Handle cases where there's a test for order; sometimes normal, sometimes swap, sometimes can guarantee
	// that only one path will be taken
	{
	...
	// Everything matches
	if (!performTransformation(comp(),
	            "%sReplace (branch on condition code [%p] -> boolean stores -> branch-and-compare using stored "
	            "boolean) with single branch on condition code\n",
	...
	if (needToSwap && !canReverseBranchMask()) {
		...
		TR::Block *newGotoBlock =
		        TR::Block::createEmptyBlock(lastNode, comp(), fallthroughBlock->getFrequency(), NULL);
		...
		_cfg->addEdge(TR::CFGEdge::createEdge(newGotoBlock, fallthroughBlock, trMemory())); // Add edge from G
		                                                                                    // to E
