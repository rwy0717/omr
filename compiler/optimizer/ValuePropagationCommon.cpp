    ...
    TR::Node* node, int32_t& srcLength, int32_t& dstLength, int32_t& elementSize, TR::DataType& type)
{
    ...
    if (srcSig && srcSigLength >= 2 && *srcSig == '[') {
        ...
        srcLength
            = srcArrayNode->getFirstChild()->getOpCode().isLoadConst() ? srcArrayNode->getFirstChild()->getInt() : -1;
    } else {
        srcType = -1;
    }
    ...
    if (dstSig && dstSigLength >= 2 && *dstSig == '[') {
        ...
        dstLength
            = dstArrayNode->getFirstChild()->getOpCode().isLoadConst() ? dstArrayNode->getFirstChild()->getInt() : -1;
    } else {
        dstType = -1;
    }
