...
OMR::ValuePropagation::initialize()
{
	...
	if (_isGlobalPropagation) {
		...
		_edgeConstraintsHashTable = (EdgeConstraints **)trMemory()->allocateStackMemory(
	...
	if (_isGlobalPropagation)
		_definedOnAllPaths = new (trStackMemory())
		        DefinedOnAllPathsMap(std::less<TR::CFGEdge *>(), trMemory()->currentStackRegion());
	else
	...
	//
	{
		...
		_firstUnresolvedSymbolValueNumber = e ? atoi(e)
	...
	if (((comp()->getMethodHotness() > warm) || !comp()->isOptServer())
		...
		if (!comp()->getFlowGraph()->getStructure()
		        && (((comp()->mayHaveLoops() && optimizer()->getLastRun(OMR::loopVersioner))
		                    || !comp()->mayHaveLoops())
...
#ifdef OPT_TIMING
				statStructuralAnalysisTiming.update((double)myTimer.timeTaken() * 1000.0
		...
		if (comp()->getFlowGraph()->getStructure()) {
			...
			// The bitVector is growable /bc when we devirtaulize calls we add symbol references for the
		...
		if (node->getOpCode().isStore()) {
			...
			if (_constNodeInfo.Locate(value, index)) {
				...
				for (auto nodeCursor = nodeList->begin();
				        (result == NULL) && (nodeCursor != nodeList->end()); ++nodeCursor) {
...
TR::VPConstraint *
OMR::ValuePropagation::getConstraint(TR::Node *node, bool &isGlobal, TR::Node *relative)
{
	/*
	...
	if (rel) {
		if (trace()) {
			traceMsg(comp(), "   %s [%p] has existing global constraint:", node->getOpCode().getName(),
			        node);
			rel->print(this, valueNumber, 1);
		}
		isGlobal = true;
...
void
OMR::ValuePropagation::processTrees(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
	TR::TreeTop *treeTop;
...
bool
OMR::ValuePropagation::canTransformArrayCopyCallForSmall(
        TR::Node *node, int32_t &srcLength, int32_t &dstLength, int32_t &elementSize, TR::DataType &type)
{
	...
	// check if both arrays are primitive types and of the same type, if so then can transform the call for small
	// (ie a primitive array copy)
...
findResolvedClassMethod(TR::Compilation *comp, char *className, char *methodName, char *methodSig)
{
...
createHdrSizeNode(TR::Compilation *comp, TR::Node *n)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		hdrSize = TR::Node::create(
		        n, TR::iconst, 0, (int32_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes());

	return hdrSize;
}

TR::Node *
generateLenForArrayCopy(TR::Compilation *comp, int32_t elementSize, TR::Node *stride, TR::Node *srcObjNode,
...
OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node)
{
	...
	if (srcVN == dstVN) {
		...
		case gc_modron_wrtbar_cardmark_incremental: break;
		default: needWriteBarrier = false; break;
		}
	}
	...
	if ((primitiveTransform || referenceTransform) && !comp()->getOption(TR_DisableArrayCopyOpts)
	        ...
	        && !(dstObject && dstObject->isNullObject()) && srcOffHigh >= 0 && dstOffHigh >= 0
	        && copyLenHigh >= 0) {
				...
				if (referenceArray2) {
					if (dstObject && dstObject->getClass()) {
						...
						if (sig && sig[0] == '[') {
							...
							// If the object's class is resolved too, see if we can prove
		...
		if (isStringCompressedArrayCopy) {
			...
			// VP may not know anything about the types of the objects we are copying, hence primitiveArray1
			// and and primitiveArray2 would both return false. However because we are dealing with a
			// recognized method we know that the types must be primitive. As such we we can safely set the
			// following two variables without repercussions.
	...
	if (transformTheCall
		...
		if (comp()->requiresSpineChecks()) {
			...
			_arrayCopySpineCheck.add(
			        new (trStackMemory()) TR_ArrayCopySpineCheck(arraycopyTree, node->getSymbolReference(),
			                srcObjNode, srcOffNode, dstObjNode, dstOffNode, copyLenNode));
		}
		...
		if (!comp()->generateArraylets()) {
			...
			src = generateArrayletAddressTree(comp(), node, type, srcOff, srcObjNode, spineShiftNode,
			        shiftNode, strideShiftNode, hdrSize);

			// -------------------------------------------------------------------
			// Calculate destination address node
			// -------------------------------------------------------------------

			dst = generateArrayletAddressTree(comp(), node, type, dstOff, dstObjNode, spineShiftNode,
			        shiftNode, strideShiftNode, hdrSize);
			...
			if (comp()->useCompressedPointers()) {
				...
				prevTree = TR::TreeTop::create(
				        comp(), prevTree, TR::Node::createCompressedRefsAnchor(src->getFirstChild()));
				prevTree = TR::TreeTop::create(
				        comp(), prevTree, TR::Node::createCompressedRefsAnchor(dst->getFirstChild()));
			}
		}
		...
		if (changeExistingNodeToPrimitiveCopy) {
			...
			if (existingNodeIsReferenceArraycopy)
				...
				case 8: newType = TR::Int64; break;
				case 4: newType = TR::Int32; break;
				case 2: newType = TR::Int16; break;
				case 1: newType = TR::Int8; break;
				default: TR_ASSERT(0, "Unhandled element size\n");
				}
...
TR::Node *
generateArrayAddressTree(TR::Compilation *comp, TR::Node *node, int32_t offHigh, TR::Node *offNode, TR::Node *objNode,
        int32_t elementSize, TR::Node *&stride, TR::Node *hdrSize)
{
...
TR::TreeTop *
OMR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
...
TR::TreeTop *
createStoresForArraycopyChildren(TR::Compilation *comp, TR::TreeTop *arrayTree, TR::SymbolReference *&srcObjRef,
        TR::SymbolReference *&dstObjRef, TR::SymbolReference *&srcRef, TR::SymbolReference *&dstRef,
...
#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::generateArrayTranslateNode(TR::TreeTop *callTree, TR::TreeTop *arrayTranslateTree,
        ...
        TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, TR::SymbolReference *tableRef, bool hasTable)
{
	...
	TR::Node *srcOffNode = srcOffRef ? TR::Node::createLoad(callNode, srcOffRef)
	                                 : callNode->getChild(childId++)->duplicateTree();
	TR::Node *srcOff = srcOffNode->createLongIfNeeded();
	...
	TR::Node *dstObj = dstRef ? TR::Node::createLoad(callNode, dstRef)
	                          : callNode->getChild(childId++)->duplicateTree();
	TR::Node *dstOffNode = dstOffRef ? TR::Node::createLoad(callNode, dstOffRef)
	                                 : callNode->getChild(childId++)->duplicateTree();
	TR::Node *dstOff = dstOffNode->createLongIfNeeded();
	...
	*/
	...
	if (encode && !isSBCSEncoder) {
		...
		} else // z
		{
			...
			if (genSIMD) {
				tableNode = TR::Node::create(callNode, TR::aconst, 0, 0); // dummy table node, it's not
				                                                          // gonna be used
			} else {
		...
		if (cg()->getSupportsArrayTranslateTROTNoBreak() || cg()->getSupportsArrayTranslateTROT()) { // X or P
			...
			tableNode = TR::Node::create(callNode, TR::iconst, 0, 0); // dummy table node, it's not gonna be
			                                                          // used
		} else { // Z
			bool genSIMD = comp()->cg()->getSupportsVectorRegisters()
			        && !comp()->getOption(TR_DisableSIMDArrayTranslate);
	...
	} else if (isSBCSEncoder) // only z
	{
		...
		TR::Node *tableNodeAddr = tableRef ? TR::Node::createLoad(callNode, tableRef)
		                                   : callNode->getChild(childId++)->duplicateTree();
	...
	if (isEncodeUtf16) {
		TR::SymbolReference *transformedCallSymRef = comp()->getSymRefTab()->methodSymRefFromName(
		        ...
		        const_cast<char *>(TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big == rm ? "transformedEncodeUTF16"
		                                                                                "Big"
		                                                                              : "transformedEncodeUTF16"
		                                                                                "Little"),
		        "(JJI)I", TR::MethodSymbol::Static);

		arrayTranslateNode =
		        TR::Node::createWithSymRef(callNode, callNode->getOpCodeValue(), 3, transformedCallSymRef);
	}
...
TR::TreeTop *
OMR::ValuePropagation::createConverterCallNodeAfterStores(TR::TreeTop *tree, TR::TreeTop *origTree,
        TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
...
//-------------------------- realtime support
TR::TreeTop *
OMR::ValuePropagation::buildSameLeafTest(TR::Node *offset, TR::Node *len, TR::Node *spineShiftNode)
{
...
TR::Node *
generateArrayletAddressTree(TR::Compilation *comp, TR::Node *vcallNode, TR::DataType type, TR::Node *off, TR::Node *obj,
        TR::Node *spineShiftNode, TR::Node *shiftNode, TR::Node *strideShiftNode, TR::Node *hdrSize)
{
	bool is64BitTarget = TR::Compiler->target.is64Bit() ? true : false;
	...
	if (is64BitTarget) {
		...
		leafOffset =
		        TR::Node::create(vcallNode, TR::iconst, 0, (int32_t)comp->fe()->getArrayletMask(elementSize));
	}
...
void
OMR::ValuePropagation::generateRTArrayNodeWithoutFlags(TR_RealTimeArrayCopy *rtArrayCopy, TR::TreeTop *dupArraycopyTree,
        TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *srcOffRef,
        TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, bool primitive)
{
	...
	TR::Node *spineShiftNode =
	        TR::Node::create(node, TR::iconst, 0, (int32_t)fe()->getArraySpineShift(elementSize));
...
TR::TreeTop *
OMR::ValuePropagation::createArrayCopyVCallNodeAfterStores(TR::TreeTop *tree, TR::SymbolReference *srcRef,
...
TR::TreeTop *
createStoresForConverterCallChildren(TR::Compilation *comp, TR::TreeTop *callTree, TR::SymbolReference *&srcRef,
        TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef, TR::SymbolReference *&dstOffRef,
        TR::SymbolReference *&lenRef, TR::SymbolReference *&thisRef, TR::TreeTop *insertBefore)
{
...
TR::TreeTop *
createStoresForArraycopyVCallChildren(TR::Compilation *comp, TR::TreeTop *vcallTree, TR::SymbolReference *&srcRef,
        TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef, TR::SymbolReference *&dstOffRef,
        TR::SymbolReference *&lenRef, TR::TreeTop *insertBefore)
{
...
//
TR::TreeTop *
OMR::ValuePropagation::createSpineCheckNode(TR::Node *node, TR::SymbolReference *objSymRef)
{
...
//
TR::TreeTop *
OMR::ValuePropagation::createAndInsertStoresForArrayCopySpineCheck(TR_ArrayCopySpineCheck *checkInfo)
{
...
TR::TreeTop *
OMR::ValuePropagation::createArrayCopyCallForSpineCheck(TR_ArrayCopySpineCheck *checkInfo)
{
	...
	if (acNode->getOpCodeValue() != TR::arraycopy) {
		...
		//      TR_ASSERT((acNode->getOpCodeValue() == TR::arraycopy) || acNode->getOpCode().isStore(),
		//      "unexpected opcode");
	}
...
void
OMR::ValuePropagation::transformArrayCopySpineCheck(TR_ArrayCopySpineCheck *checkInfo)
{
...
void
OMR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy *rtArrayCopyTree)
{
#ifdef J9_PROJECT_SPECIFIC
	TR::CFG *cfg = comp()->getFlowGraph();
	...
	if (type != TR::Address || !comp()->getOptions()->realTimeGC()) {
		...
		if (TR::Compiler->target.is64Bit()) {
			...
			ifTree = TR::TreeTop::create(comp(),
			        TR::Node::createif(
			                TR::iflcmpne, globalFragment, TR::Node::create(vcallNode, TR::lconst, 0, 0)));
		} else {
			TR::Node *globalFragment = TR::Node::createWithSymRef(TR::iloadi, 1, 1, fragmentParent,
			        comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
			ifTree = TR::TreeTop::create(comp(),
			        TR::Node::createif(
			                TR::ificmpne, globalFragment, TR::Node::create(vcallNode, TR::iconst, 0, 0)));
		}
	...
	// if this is tarok, depending on wrtbar we create a primitive arraycopy or reference arraycopy with no store
	// check flag. for realtime the fast path is primitive (if we got there there is no need for wrtbar)
	...
	if (type == TR::Address && comp()->getOptions()->realTimeGC() && doMultiLeafArrayCopy) {
		...
		slowArraycopyBlock->append(TR::TreeTop::create(
		        comp(), TR::Node::create(origDupVCallTree->getNode(), TR::Goto, 0, succ->getEntry())));
	...
	if ((type == TR::Address) && (rtArrayCopyTree->_flag & NEED_ARRAYSTORE_CHECK)) {
		...
		// We know there is no wrtbar needed, so the fast path will be arraycopy with 3 children, while the slow
		// will be arraycopy with 5 children with arrayStore check.
		...
		TR::Block *referenceArraycopyBlock = TR::Block::createEmptyBlock(
		        dupRefArraycopyNode, comp(), origCallBlock->getFrequency(), origCallBlock);
		if (!(rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC
		            || rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_DST)) {
...
#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::transformRTMultiLeafArrayCopy(TR_RealTimeArrayCopy *rtArrayCopyTree)
{
	...
	TR::Node *newCallNode = TR::Node::createWithSymRef(TR::call, 7, 7, vcallNode->getChild(0),
	        vcallNode->getChild(1), vcallNode->getChild(2), vcallNode->getChild(3), vcallNode->getChild(4),
...
transformedTargetName(TR::RecognizedMethod rm)
{
	...
	default: return "arraytranslate";
	}
}

#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::transformObjectCloneCall(TR::TreeTop *callTree, OMR::ValuePropagation::ObjCloneInfo *cloneInfo)
{
	...
	if (!performTransformation(comp(),
	            "%sInlining object clone call [%p] as new object and JITHelpers object copy\n", OPT_DETAILS,
	            callNode))
		return;

	TR::DebugCounter::prependDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "inlineClone.location/object/(%s)", comp()->signature()),
	        callTree);
	int32_t classNameLength;
	char *className = TR::Compiler->cls.classNameChars(comp(), j9class, classNameLength);
	TR::DebugCounter::prependDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "inlineClone.type/(%s)/(%s)/%s", className,
	...
	prepareToReplaceNode(callNode); // This will remove the usedef info, valueNumber info and all children of the
	                                // node

	if (callTree->getNode()->getOpCode().isCheck()) {
		TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
		        "it only makes sense to do this object clone transform for checks with a NULLCHK - how did we "
		        "get here otherwise??");
	...
	if (!cloneInfo->_isFixed) {
		...
		slowPath->append(
		        TR::TreeTop::create(comp(), TR::Node::create(callNode, TR::Goto, 0, remainder->getEntry())));
	...
	TR::SymbolReference *helperAccessor = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(),
	        "com/ibm/jit/JITHelpers", const_cast<char *>("jitHelpers"),
	        const_cast<char *>("()Lcom/ibm/jit/JITHelpers;"), TR::MethodSymbol::Static);
	TR::SymbolReference *objCopySymRef = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(),
	        "com/ibm/jit/JITHelpers",
	        const_cast<char *>(
	                TR::Compiler->target.is64Bit() ? "unsafeObjectShallowCopy64" : "unsafeObjectShallowCopy32"),
...
void
OMR::ValuePropagation::transformArrayCloneCall(TR::TreeTop *callTree, OMR::ValuePropagation::ArrayCloneInfo *cloneInfo)
{
	...
	TR::DebugCounter::prependDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "inlineClone.location/array/(%s)", comp()->signature()),
	        callTree);
	int32_t classNameLength;
	...
	if (callTree->getNode()->getOpCode().isCheck()) {
		TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
		        "it only makes sense to do this array clone transform for checks with a NULLCHK - how did we "
		        "get here otherwise??");
		TR::Node *passthrough = TR::Node::create(callNode, TR::PassThrough, 1, objNode);
		callTree->insertBefore(TR::TreeTop::create(comp(),
		        TR::Node::createWithSymRef(callNode, callTree->getNode()->getOpCodeValue(), 1, passthrough,
	...
	if (isPrimitiveClass) {
		...
		if (isFixedClass) {
			...
			// Load the component class of the cloned array as 2nd child of anewarray as expected by the
			// opcode.
	...
	TR::Node *lengthInBytes = TR::Compiler->target.is64Bit()
	        ? TR::Node::create(callNode, TR::lmul, 2, TR::Node::create(callNode, TR::i2l, 1, lenNode),
	...
	TR::Node *srcStart = TR::Compiler->target.is64Bit()
	        ? TR::Node::create(callNode, TR::aladd, 2, objNode,
	                TR::Node::lconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
	        : TR::Node::create(callNode, TR::aiadd, 2, objNode,
	                TR::Node::iconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
	TR::Node *destStart = TR::Compiler->target.is64Bit()
	        ? TR::Node::create(callNode, TR::aladd, 2, newArray,
	                TR::Node::lconst(newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
	...
	if (isPrimitiveClass) {
		...
		case 4:
		case 8: arraycopy->setWordElementArrayCopy(true); break;
		}
	} else {
...
OMR::ValuePropagation::transformConverterCall(TR::TreeTop *callTree)
{
	...
	if (!performTransformation(comp(), "%sChanging call %s [%p] to %s \n", OPT_DETAILS,
	            callNode->getOpCode().getName(), callNode, transformedTargetName(rm)))
		return;
	...
	dumpOptDetails(
	        comp(), "Insert runtime tests for converter call transformation to arraytranslate (%p)\n", callNode);
	...
	TR::TreeTop *ifTreec = TR::TreeTop::create(comp(),
	        TR::Node::createif(
	                TR::ifacmpeq, TR::Node::createLoad(callNode, srcRef), TR::Node::aconst(callNode, 0)));
	...
	TR::TreeTop *ifTreed = TR::TreeTop::create(comp(),
	        TR::Node::createif(
	                TR::ifacmpeq, TR::Node::createLoad(callNode, dstRef), TR::Node::aconst(callNode, 0)));
	...
	if (TR::Compiler->target.cpu.isZ()) {
		...
		if (threshold > 0) {
			...
			createAndInsertTestBlock(
			        comp(), thresholdTestTree, callTree, origCallBlock, slowArraytranslateBlock);

			TR_InlineCall inlineFallback(optimizer(), this);
			// Inline the duplicate call as this block is potentially not cold depending on input string
			// length
...
TR::LocalValuePropagation::LocalValuePropagation(TR::OptimizationManager *manager) : TR::ValuePropagation(manager)
{
...
TR::LocalValuePropagation::performOnBlock(TR::Block *block)
{
	...
	if ((_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
		dumpOptDetails(
		        comp(), "Can't do Local Value Propagation on block %d - too many nodes\n", block->getNumber());
	} else {
...
void
OMR::ValuePropagation::launchNode(TR::Node *node, TR::Node *parent, int32_t whichChild)
{
