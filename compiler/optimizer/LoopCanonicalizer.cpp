 ...
 */
TR_LoopCanonicalizer::TR_LoopCanonicalizer(TR::OptimizationManager *manager) : TR_LoopTransformer(manager) {}
...
TR_LoopCanonicalizer::perform()
{
...

	{
		...
		detectWhileLoops(
		        whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _rootStructure, true);
		...
		/*
		   for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop =
		   whileLoopsIt.getNext())
		      {
		      TR_RegionStructure *naturalLoop = nextWhileLoop->asRegion();
		      TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),"Loop canonicalizer, expecting natural
		   loop"); if (naturalLoop->getEntryBlock()->isCold()) continue;
...
void
TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(ListAppender<TR_Structure> &whileLoopsInnerFirst,
        ...
        TR_RegionStructure *region, vcount_t visitCount, TR_BitVector *pendingList, bool innerFirst)
{
	...
	for (auto pred = rootNode->getPredecessors().begin(); pred != rootNode->getPredecessors().end(); ++pred) {
		...
		if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
			detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst,
			        doWhileLoops, predStructure, predNode, region, visitCount, pendingList, innerFirst);
			return;
		}
	}
	...
	for (auto succ = rootNode->getSuccessors().begin(); succ != rootNode->getSuccessors().end(); ++succ) {
		...
		if ((!isExitEdge) && pendingList->get(succStructure->getNumber())) {
			...
			_nodesInCycle->empty();
			detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst,
			        doWhileLoops, succStructure, succNode, region, visitCount, pendingList, innerFirst);
		}
	}
        ...
        bool innerFirst)
{
	while (!_analysisStack.isEmpty() && _analysisStack.top()->getStructure() != region) {
		...
		_nodesInCycle->empty();
		detectWhileLoops(
		        whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops, root, innerFirst);
		pendingList->reset(root->getNumber());
		_analysisStack.pop();
...
void
TR_LoopTransformer::createWhileLoopsList(TR_ScratchList<TR_Structure> *whileLoops)
{
	TR_ScratchList<TR::CFGNode> newEmptyBlocks(trMemory());
	TR_ScratchList<TR::CFGNode> newEmptyExceptionBlocks(trMemory());
 ...
 */
void
TR_LoopTransformer::detectWhileLoops(ListAppender<TR_Structure> &whileLoopsInnerFirst, List<TR_Structure> &whileLoops,
        ...
        bool innerFirst)
{
	...
	// if (debug("nonRecursiveBVA"))
	{
		...
		detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst,
		        doWhileLoops, region, visitCount, pendingList, innerFirst);
	}
	/*
     else
			...
			if ((hdrBlock->hasExceptionPredecessors() == false)
				...
				if (entryNode->getSuccessors().size() == 2) {
					...
					if (succ1Internal ^ succ2Internal) {
						...
						static const bool skipTt = feGetEnv("TR_canonicalizerStopAtTreetop")
						        == NULL;
						static const bool skipCRAnchor = feGetEnv("TR_"
						                                          "canonicalizerStopAtCompresse"
						                                          "dRefsAnchor")
						        == NULL;
						while ((firstTree
						        && (firstTree->getNode()->getOpCodeValue() == TR::asynccheck
						                || (skipTt
						                        && firstTree->getNode()->getOpCodeValue()
						                                == TR::treetop)
						                || (skipCRAnchor
						                        && firstTree->getNode()->getOpCodeValue()
						                                == TR::compressedRefs)
						...
						if (firstTree == hdrBlock->getLastRealTreeTop()
							...
							if (!exitBlocks.isSingleton()) {
								TR::TreeTop *hdrBlockLoopTestTree =
								        hdrBlock->getLastRealTreeTop();
								bool foundInductionVariable = findMatchingIVInRegion(
								        hdrBlockLoopTestTree, region);
								if (!foundInductionVariable) {
									ListIterator<TR::Block> blocksIt(&exitBlocks);
									TR::Block *exitBlock;
									for (exitBlock = blocksIt.getCurrent();
									        exitBlock;
									        exitBlock = blocksIt.getNext()) {
										if (exitBlock != hdrBlock) {
											if (exitBlock->getSuccessors()
											                .size()
											        == 2) {
												auto it =
												        exitBlock
												...
												bool successor2Internal = region->contains(
												        toBlock(exitSucc2
												                ...
												                ->getStructureOf(),
												        region->getParent());
												if (successor1Internal
												        ^ successor2Internal) {
													TR::TreeTop *exitBlockLoopTestTree =
													        exitBlock
													                ->getLastRealTreeTop();
													if (exitBlockLoopTestTree
													                ->getNode()
													                ->getOpCode()
													                .isCompBranchOnly()
													        || exitBlockLoopTestTree
													                   ->getNode()
													                   ->getOpCode()
													                   .isJumpWithMultipleTargets())
														continue;
...
bool
TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop *loopTestTree, TR_RegionStructure *region)
{
...
TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure *whileLoop)
{
	...
	if (!_blocksToBeCleansed.find(loopHeader))
		...
		fallThroughBlock =
		        loopHeader->getLastRealTreeTop()->getNode()->getBranchDestination()->getNode()->getBlock();
	}
	...
	TR::Block *clonedHeader =
	        TR::Block::createEmptyBlock(entryNode, comp(), loopHeader->getFrequency(), loopHeader);
	...
	*/
	...
	if (loopBody->isCold()) {
		if (trace())
			traceMsg(
			        comp(), "Setting s2 block_%d cold because loop body is cold\n", splitter2->getNumber());
	...
	for (auto edge = entryGraphNode->getSuccessors().begin(); edge != entryGraphNode->getSuccessors().end();
	        ++edge) {
...
bool
TR_LoopCanonicalizer::isLegalToSplitEdges(TR_RegionStructure *doWhileLoop, TR::Block *blockAtHeadOfLoop)
{
 ...
 */
bool
TR_LoopCanonicalizer::modifyBranchesForSplitEdges(TR_RegionStructure *doWhileLoop, TR::Block *blockAtHeadOfLoop,
		...
		if ((!doWhileLoop->contains(pred->getStructureOf())) && (pred != loopInvariantBlock)) {
			*sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
			if (pred != _cfg->getStart()) {
				...
				// it is okay to reach here if checking only, as the preparation work will not have been
				// done
				TR_ASSERT(isCheckOnly,
				        "Cannot reach here if preparation work for the loopInvariantBlock has been "
				        "done correctly\n");
				if (!isCheckOnly) {
					TR::TreeTop *targetEntry = targetBlock->getEntry();
					TR::TreeTop *targetExit = targetBlock->getExit();
...
TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure *doWhileLoop)
{
	...
	if (!performTransformation(
	            comp(), "%sCanonicalizing do while loop %d\n", OPT_DETAILS, doWhileLoop->getNumber()))
		return;
	...
	if (!comp()->getOption(TR_DisableEmptyPreHeaderCheck)) // Skip adding another pre-header if you don't need to.
	{
		for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin();
			...
			if (ipred->getStructureOf()->isLoopInvariantBlock() && ipred->isEmptyBlock()
				...
				return; // if there is an empty pre-header which is not the entry block of a region,
				        // don't add another.
			}
		}
	}
...
void
TR_LoopCanonicalizer::eliminateRedundantInductionVariablesFromLoop(TR_RegionStructure *naturalLoop)
{
	...
	if (primaryInductionVariable && bivIterator.getFirst()) {
		...
		for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
			...
			if ((nextIndVar->getSymRef()->getSymbol()->getDataType()
				...
				// comp()->incVisitCount();
				bool incrementsInLockStep = incrementedInLockStep(naturalLoop, derivedInductionVar,
				        symRefInCompare, nextIndVarIncr, primaryIncr,
				...
				if (incrementsInLockStep
					...
					incrementsInLockStep = checkIfOrderOfBlocksIsKnown(naturalLoop, entryBlock,
					        loopTestBlock, &derivedInductionVarIncrementBlocks,
		...
		if (alteredCode) {
			if (loopInvariantBlock->getBlock()->getLastRealTreeTop()->getNode()->getOpCodeValue()
			        != TR::BBStart) {
				...
				TR::Block *newLoopInvariantBlock = loopInvariantBlock->getBlock()->split(
				        loopInvariantBlock->getBlock()->getLastRealTreeTop(), comp()->getFlowGraph(),
				        false);
...
bool
TR_LoopCanonicalizer::replaceInductionVariableComputationsInExits(TR_Structure *structure, TR::Node *node,
		...
		if (!structure->contains(block->getStructureOf())) {
			...
			TR::Node *subNode = TR::Node::create((dataType == TR::Int32) ? TR::iadd : TR::ladd, 2,
			        TR::Node::createWithSymRef(
			                node, comp()->il.opCodeForDirectLoad(dataType), 0, newSymbolReference),
			        TR::Node::createWithSymRef(
...
void
TR_LoopCanonicalizer::findIncrements(TR::Node *currentNode, vcount_t visitCount,
        ...
        int64_t &derivedInductionVarIncrement, int64_t &primaryInductionVarIncrement, bool &unknownIncrement)
{
	...
	for (i = 0; i < currentNode->getNumChildren(); i++) {
		...
		if (child->getOpCode().isStore())
			findIncrements(child, visitCount, derivedInductionVar, primaryInductionVar,
			        derivedInductionVarIncrement, primaryInductionVarIncrement, unknownIncrement);
	}
}

bool
TR_LoopCanonicalizer::incrementedInLockStep(TR_Structure *structure, TR::SymbolReference *derivedInductionVar,
...
bool
TR_LoopCanonicalizer::checkIfOrderOfBlocksIsKnown(TR_RegionStructure *naturalLoop, TR::Block *entryBlock,
...
bool
TR_LoopCanonicalizer::checkComplexInductionVariableUseNode(TR::Node *node, bool addrExpression)
{
	...
	if (opCode.isStoreIndirect()) {
		...
		if (node->getOpCodeValue() == TR::imul) {
			...
			if (node->getFirstChild()->getOpCode().hasSymbolReference()
				...
				traceMsg(comp(),
				        "\tAvoiding induction variable replacement because of address mode complexity. "
				        "Sym Ref. = %p\n",
...
bool
TR_LoopCanonicalizer::examineTreeForInductionVariableUse(TR::Block *loopInvariantBlock, TR::Node *parent,
        int32_t childNum, TR::Node *node, vcount_t visitCount, TR::SymbolReference **newSymbolReference)
{
	...
	if (node->getOpCode().hasSymbolReference() && !node->getOpCode().isStore()) {
		if ((node->getSymbolReference() == _symRefBeingReplaced)
			...
			loadOfPrimaryInductionVar = TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2,
			        loadOfPrimaryInductionVar,
			        TR::Node::createWithSymRef(
			                node, comp()->il.opCodeForDirectLoad(dataType), 0, *newSymbolReference));
			...
			if (_primaryIncrementedFirst == 0) {
				if (_derivedInductionVarStoreInBlock) {
					if (!_primaryInductionVarStoreInBlock) {
						TR::Node *constChild = _derivedInductionVarStoreInBlock->getFirstChild()
						                               ->getSecondChild();
				...
				} else if (0) // Do not need adjustment in this case since we have obtained the
				              // correctly commoned node in this case
				{
					if (_primaryInductionVarStoreInBlock) {
						TR::Node *constChild = _primaryInductionVarStoreInBlock->getFirstChild()
						                               ->getSecondChild();
						int64_t additiveConstant;
						if (constChild->getOpCodeValue() == TR::lconst)
							additiveConstant = constChild->getLongInt();
				...
				if (_primaryIncrementedFirst == 1) {
					if (((_primaryInductionIncrementBlock != _currentBlock)
						...
						nodeSetUp = true;
						TR::Node::recreate(
						        node, ((dataType == TR::Int32) ? TR::iadd : TR::ladd));
						node->setNumChildren(2);
						node->setAndIncChild(0, loadOfPrimaryInductionVar);
						node->setAndIncChild(1, adjustmentConstChild);
					}
			...
			//                                   TR::Node::create(node,
			//                                   comp()->il.opCodeForDirectLoad(dataType), 0,
...
void
TR_LoopCanonicalizer::placeInitializationTreeInLoopPreHeader(TR::Block *b, TR::Node *node,
        TR::SymbolReference *newSymbolReference, TR::SymbolReference *primaryInductionVar,
        TR::SymbolReference *derivedInductionVar)
{
	TR::DataType dataType = newSymbolReference->getSymbol()->getDataType();
	...
	TR::Node *storeNode = TR::Node::createWithSymRef(
	        comp()->il.opCodeForDirectStore(dataType), 1, 1, subNode, newSymbolReference);
	TR::TreeTop *newStoreTree = TR::TreeTop::create(comp(), storeNode, NULL, NULL);
 ...
 */
void
TR_LoopCanonicalizer::rewritePostToPreIncrementTestInRegion(TR_RegionStructure *region)
{
...
void
TR_LoopCanonicalizer::rewritePostToPreIncrementTestInBlock(TR::Block *const block)
{
...
bool
TR_LoopTransformer::replaceAllInductionVariableComputations(TR::Block *loopInvariantBlock, TR_Structure *structure,
        TR::SymbolReference **newSymbolReference, TR::SymbolReference *inductionVarSymRef)
{
	...
	if (structure->asBlock() != NULL) {
		...
		while (currentTree != exitTree) {
			...
			if (examineTreeForInductionVariableUse(loopInvariantBlock, NULL, -1, currentNode,
...
//
TR::Node *
TR_LoopTransformer::duplicateExact(TR::Node *node, List<TR::Node> *seenNodes, List<TR::Node> *duplicateNodes)
{
	vcount_t visitCount = comp()->getVisitCount();
	...
	//        treeTop = exitTreeTop->getNextTreeTop())
	{
		...
		if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
			...
			// if (targetTree->getPrevTreeTop() != NULL)
			{
				...
				prevTreeOfTarget =
				        targetTree->getPrevTreeTop()->getNode()->getBlock()->getLastRealTreeTop();
			}
			...
			if ((prevTreeOfTarget->getNode()->getOpCode().isBranch()
				...
				// ((prevTreeOfTarget->getNode()->getOpCode().getOpCodeValue() == TR::Goto) ||
				// (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree)))
				{
	...
	//        treeTop = exitTreeTop->getNextTreeTop())
	{
		...
		if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
			...
			if ((prevTreeOfTarget->getNode()->getOpCode().isBranch()
			        ...
			        || (prevTreeOfTarget->getNode()->getOpCodeValue() == TR::athrow)) {
				// if (isPrevTreeOfTargetReturn || (prevTreeOfTarget->getNode()->getBranchDestination()
				// != targetTree))
				{
					...
					if ((exitTreeTop->getNextTreeTop() != targetTree) && allPredsBranchExplicitly) {
						if (trace())
							traceMsg(comp(),
							        "Moving invariant block_%d to fall through into loop "
							        "%d\n",
							        block->getNumber(),
...
bool
TR_LoopTransformer::isStoreInRequiredForm(int32_t symRefNum, TR_Structure *loopStructure)
{
...
int32_t
TR_LoopTransformer::checkLoopForPredictability(TR_Structure *loopStructure, TR::Block *loopInvariantBlock,
...
void
TR_LoopTransformer::updateInfo(TR::Node *node, vcount_t visitCount, updateInfo_tables &uinfo)
{
...
void
TR_LoopTransformer::collectSymbolsWrittenAndReadExactlyOnce(TR_Structure *structure, vcount_t visitCount)
{
	updateInfo_tables uinfo(comp()->allocator());
...
TR_LoopTransformer::getSymbolDefUseStateInSubTree(TR::Node *node, TR::RegisterMappedSymbol *indVarSym)
{
...
TR_LoopTransformer::TR_TransformerDefUseState
TR_LoopTransformer::getSymbolDefUseStateInBlock(TR::Block *block, TR::RegisterMappedSymbol *indVarSym)
{
	for (TR::TreeTop *tree = block->getFirstRealTreeTop(); tree != block->getExit();
...
TR::Node *
TR_LoopTransformer::getCorrectNumberOfIterations(TR::Node *node, TR::Node *constNode)
{
	TR::Node *subNode;
...
TR::Node *
TR_LoopTransformer::updateLoadUsedInLoopIncrement(TR::Node *node, int32_t inductionVariable)
{
	TR::ILOpCode &opCode = node->getOpCode();
...
TR::Node *
TR_LoopTransformer::containsOnlyInductionVariableAndAdditiveConstant(TR::Node *node, int32_t inductionVariable)
{
	TR::ILOpCode &opCode = node->getOpCode();
...
void
TR_LoopTransformer::checkIfIncrementInDifferentExtendedBlock(TR::Block *block, int32_t inductionVariable)
{
...
TR_LoopInverter::TR_LoopInverter(TR::OptimizationManager *manager) : TR_LoopTransformer(manager) {}
				        ...
				        && (entryVal->getLowInt() == 0
				                || (exitVal && exitVal->asIntConst()
				                        && (llabs((long long)(exitVal->getLowInt())
				                                   - (long long)(entryVal->getLowInt())))
				                                < INT_MAX))
				        &&
				        // We cannot safely guess exit value of IV if it doesn't land on
				        // comparand(RhsOfStoreNode). Workaround is only allow abs(incr)==1.   This is
				        // also a hard requirement for unsigned IV to not introduce wraparound. For
				        // signed case, we may allow abs(incr)>1, but exit value calc is not just
				...
				for (auto edge = nextBlock->getSuccessors().begin();
					...
					if (!blocksInRegion.find(next)) {
						...
						if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
							storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
							        rhsOfStoreNode->duplicateTree(), nextInductionSymRef);
						else {
							if (origOpCodeWasAdd)
								storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
 ...
 */
bool
TR_LoopInverter::checkIfSymbolIsReadInKnownTree(
