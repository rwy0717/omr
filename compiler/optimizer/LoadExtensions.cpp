...
TR_LoadExtensions::TR_LoadExtensions(TR::OptimizationManager *manager)
        : TR::Optimization(manager), excludedNodes(NULL), loadExtensionPreference(NULL)
{
...
TR_LoadExtensions::perform()
{
	...
	if (comp()->getOptLevel() >= hot && !optimizer()->cantBuildGlobalsUseDefInfo() && enableGRALoadExtensions) {
		...
		TR::LexicalMemProfiler memoryProfiler(
		        "Load Extensions: Usedef calculation", comp()->phaseMemProfiler());
...
const bool
TR_LoadExtensions::canSkipConversion(TR::Node *conversion, TR::Node *child, bool &forceExtension)
{
	bool result = false;
	...
	if (isSupportedType(child) && excludedNodes->count(child) == 0) {
		...
		if (isSupportedLoad(child) &&
		        ...
		        (TR::Compiler->target.is64Bit() || comp()->cg()->use64BitRegsOn32Bit()
		                || conversion->getSize() != 8)
		        &&
			...
			if (trace()) {
				traceMsg(comp(),
				        "\t\tDetected sign extension pattern on widening conversion %s [%p] and load "
				        "%s [%p]\n",
				        conversion->getOpCode().getName(), conversion, child->getOpCode().getName(),
				        child);
			}
		...
		if (conversion->getSize() < child->getSize()) {
			// TODO (Issue #2213): Determine whether this case is ever needed and why? Shouldn't the
			// simplifier have eliminated such IL?
			if (child->getOpCode().isConversion()) {
				TR::Node *grandChild = child->getFirstChild();

				if (isSupportedLoad(grandChild) &&

				        // Conversion is narrowing down to the original width (i.e. stacked conversion
				        // which is a NOP)
				        conversion->getSize() == grandChild->getSize()) {
					if (trace()) {
						traceMsg(comp(),
						        "\t\tDetected sign extension pattern on narrowing conversion "
						        "%s [%p] and load %s [%p]\n",
						        conversion->getOpCode().getName(), conversion,
	...
	if (isSupportedType(parent) && parentOpCode.isConversion()) {
		...
		if (isSupportedType(child) && parent->getSize() != child->getSize()) {
			if (isSupportedLoad(child)) {
				...
				// If we have usedef info we can traverse all defs of this particular use and if all the
				// defs are stores of supported counted loads then we can count such loads as well. If
				// this criteria is not met then there exists at least one def (store) of this
				// particular use which feeds from a non-load operation (an addition for example). These
				// are not candidates for skipping extension because we cannot easily extend a non-load
				// operation.
				if (useDefInfo != NULL && useDefInfo->infoIsValid() && useRegLoad->getUseDefIndex() != 0
					...
					if (useDefInfo->getUseDef(info, useRegLoad->getUseDefIndex())) {
						if (trace()) {
							traceMsg(comp(),
							        "\t\tPeeking through RegLoad %p for conversion %s "
							        "[%p]\n",
						...
						for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
							...
							if (defRegLoad != NULL) {
								...
								if (defRegLoad->getOpCode().isStoreReg()
									...
									if (trace()) {
										traceMsg(comp(),
										        "\t\tPeeked through use %s "
										        "[%p] and found def %s [%p] "
										        "with child %s [%p] - Counting "
										        "[%p]\n",
										        useRegLoad->getOpCode()
										                .getName(),
										        useRegLoad,
										        defRegLoad->getOpCode()
										                .getName(),
										        defRegLoad,
										        defRegLoadChild->getOpCode()
										                .getName(),
										        defRegLoadChild,
										        defRegLoadChild);
									}
				...
				if (childOpCode.isLoadReg()
					...
					if (useDefInfo != NULL && useDefInfo->infoIsValid()
						...
						if (useDefInfo->getUseDef(info, useRegLoad->getUseDefIndex())) {
							...
							for (cursor.SetToFirstOne();
							        cursor.Valid() && canSkipConversion;
							        cursor.SetToNextOne()) {
								int32_t defIndex = cursor;
								...
								if (defRegLoad != NULL) {
									TR::Node *defRegLoadChild =
									        defRegLoad->getFirstChild();

									bool forceExtension = false;
									canSkipConversion =
									        TR_LoadExtensions::canSkipConversion(
									                parent, defRegLoadChild,
									                forceExtension);

									forceExtensionOnAnyLoads |= forceExtension;
									forceExtensionOnAllLoads &= forceExtension;

									// If we have to force extension on any loads
									// which feed a def of this use ensure we must
									// also force extension on all such loads.
									// Conversely the conversion can be skipped if
									// none of the loads feeding the def of this use
									// need to be extended. This ensures either all
									// loads feeding into defs of this use should be
									// extended or none of them.
									canSkipConversion &= forceExtensionOnAllLoads
									        == forceExtensionOnAnyLoads;

									if (trace()) {
										traceMsg(comp(),
										        "\t\tPeeked through %s [%p] "
										        "and found %s [%p] with child "
										        "%s [%p] - conversion %s be "
										        "skipped\n",
										        useRegLoad->getOpCode()
										                .getName(),
							...
							if (canSkipConversion
								...
								if (forceExtensionOnAllLoads) {
									...
									if (useDefInfo->getUseDef(info,
										...
										TR_UseDefInfo::BitVector::Cursor cursor(
										        info);
										...
										for (cursor.SetToFirstOne();
											...
											// We've examined all the defs
											// of this particular use
											if (defIndex >= firstUseIndex) {
												break;
											}

											// Do not consider defs that
											// correspond to method
											// arguments as we cannot force
											// extension on those
											if (defIndex < firstDefIndex) {
												continue;
											}
											...
											if (defRegLoad != NULL) {
												...
												if (preference > 0) {
													...
													if (parent->getSize()
														...
														defRegLoadChild
														        ->setSignExtendTo64BitAtSource(
														                true);
													} else {
														defRegLoadChild
														        ->setSignExtendTo32BitAtSource(
														                true);
													}
												}
												...
												if (preference < 0) {
													...
													if (parent->getSize()
													        ...
													        || parent->useSignExtensionMode()) {
														defRegLoadChild
														        ->setZeroExtendTo64BitAtSource(
														                true);
													} else {
														defRegLoadChild
														        ->setZeroExtendTo32BitAtSource(
														                true);
													}
												}
											}
								...
								if (parent->getType().isInt64()
									...
									if (trace()) {
										traceMsg(comp(),
										        "\t\t\tSet global register %s "
										        "in "
										        "getExtendedToInt64GlobalRegist"
										        "ers for child %s [%p] with "
										        "parent node %s [%p]\n",
									...
									// getExtendedToInt64GlobalRegisters is used by
									// the evaluators to force a larger virtual
									// register to be used when evaluating the
									// regload so any instructions generated by
									// local RA are the correct size to preserve the
									// upper bits
	...
	if (conversion->isZeroExtension() || conversion->getOpCode().isUnsigned()) {
		if (trace()) {
			traceMsg(comp(), "\t\tCounting unsigned load %s [%p] under %s [%p]\n",
			        load->getOpCode().getName(), load, conversion->getOpCode().getName(), conversion);
		}
