...
correctDecimalPrecision(TR::Node *store, TR::Node *child, TR::Compilation *comp)
{
	if (child->getType().isBCD() && child->getDecimalPrecision() != store->getDecimalPrecision()) {
		...
		if (comp->cg()->traceBCDCodeGen())
			traceMsg(comp,
			        "%screating %s (%p) to correctDecimalPrecision (%d->%d : isTruncation=%s) on node %s "
			        "(%p)\n",
			        OPT_DETAILS, modPrecNode->getOpCode().getName(), modPrecNode,
			        child->getDecimalPrecision(), store->getDecimalPrecision(), isTruncation ? "yes" : "no",
...
TR_PartialRedundancy::TR_PartialRedundancy(TR::OptimizationManager *manager) : TR::Optimization(manager)
{
...
TR_PartialRedundancy::perform()
{
	...
	if (comp()->getProfilingMode() == JitProfiling && comp()->getHCRMode() != TR::none
	        && _numProfilingsAllowed == 0)
		return 0;
	...
	//
	TR_LocalAnticipatability &localAnticipatability = _isolatedness->_latestness->_delayedness->_earliestness
	...
	for (bvi1.SetToFirstOne(); bvi1.Valid(); bvi1.SetToNextOne()) {
		...
		if (nextRedundantComputation != 0) {
			supportedNodesAsArray[nextRedundantComputation] =
			        supportedNodesAsArray[nextRedundantComputation]->duplicateTreeWithCommoning(
			                comp()->allocator());
	...
	for (bvi3.SetToFirstOne(); bvi3.Valid(); bvi3.SetToNextOne()) {
		...
		if (cost > benefit) {
			if (trace())
				traceMsg(comp(),
				        "Cost-benefit analysis (cost %d > benefit %d) said ignore expression #%d\n",
...
void
TR_PartialRedundancy::invalidateOptimalComputation(int32_t nextOptimalComputation)
{
	TR::CFGNode *nextNode;
	TR::CFG *cfg = comp()->getFlowGraph();
	for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
...
//
TR::Node *
TR_PartialRedundancy::duplicateExact(
...
void
TR_PartialRedundancy::calculateGlobalRegisterWeightsBasedOnStructure(TR_Structure *structure, int32_t *currentWeight)
{
...
void
TR_PartialRedundancy::processReusedNode(
...
TR::TreeTop *
TR_PartialRedundancy::placeComputationsOptimally(TR::Block *block, TR::Node ***supportedNodesAsArray)
{
	...
	for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement];
			...
			while (placeToInsertOptimalComputations != exitTree) {
				if (nextOptimalNode->getOpCode().isCheck()) {
					...
					if (nextNode->getOpCode().isCheck()) {
						if (nextNode->getLocalIndex() == nextOptimalComputation) {
							if (nextNode->getOpCode().isNullCheck()) {
								if (nextNode->getFirstChild()->exceptionsRaised()
								        != 0) {
									...
									// comp()->incVisitCount(); List<TR::Node>
									// seenNodes, duplicateNodes; TR::Node
									...
									TR::Node *passThroughNode = TR::Node::create(
									        TR::PassThrough, 1,
									        nextNode->getNullCheckReference());
									TR::Node *nodeInTreeTop =
									        TR::Node::createWithSymRef(TR::NULLCHK,
									                1, 1, passThroughNode,
									                newSymRef);
									...
									if (!shouldAllowLimitingOfOptimalPlacement(
									            comp())
									        || performTransformation(comp(),
									                "%sPlacing NULLCHK optimally : "
									                "%p\n",
										...
										if (comp()->useCompressedPointers()) {
											TR::Node *reference =
											        passThroughNode
											                ->getFirstChild();
											if (reference->getOpCode()
											                .isLoadIndirect()
											        && reference->getDataType()
											                == TR::Address
											        && TR::TransformUtil::
											                fieldShouldBeCompressed(
											                        reference,
											                        comp())) {
										...
										// if comp()->useCompressedPointers()
										// place the translateTT after the
										// nullchk
										//
										if (translateTT) {
					...
					TR::Node *optimalNode = getAlreadyPresentOptimalNode(
					        placeToInsertOptimalComputations->getNode(), nextOptimalComputation,
					        comp()->getVisitCount(), rhsOfStore);
					...
					if (optimalNode
						...
						if (!shouldAllowLimitingOfOptimalPlacement(comp())
							...
							if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType())
								...
								convertedOptimalNode = TR::Node::create(
								        conversionOpCode, 1, optimalNode);
							}

							TR::TreeTop *translateTT = NULL;
							if (comp()->useCompressedPointers()) {
								if (convertedOptimalNode->getOpCode().isLoadIndirect()
								        && convertedOptimalNode->getDataType()
								                == TR::Address
                          ...
                          (placeHolderNode->exceptionsRaised() != 0)) */)
                         {
								...
								placeToInsertOptimalComputations = translateTT
								        ? translateTT
								        : duplicateOptimalTree;
								_prevTree = translateTT ? translateTT
								                        : duplicateOptimalTree;
							} else {
								...
								// if comp()->useCompressedPointers()
								if (translateTT) {
									placeToInsertOptimalComputations->join(
									        translateTT);
		...
		if (nextOptimalNode->getOpCode().isStore()) {
			...
			////TR::TreeTop *duplicateOptimalTree = TR::TreeTop::create(comp(), duplicateOptimalNode, NULL,
			/// NULL); /dumpOptDetails(comp(), "%sPlacing store optimally : %p\n", OPT_DETAILS,
			/// duplicateOptimalNode); /setAlteredCode(true);
				...
				if (!replacedNullCheckReference) {
					if (trace())
						traceMsg(comp(), "Trying to replace OPTIMAL SUBNODES in : %p\n",
						        nextOptimalNode);
					int32_t k;
					for (k = 0; k < nextOptimalNode->getNullCheckReference()->getNumChildren();
					        k++) {
					...
					if (comp()->useCompressedPointers()) {
						...
						if (reference->getOpCode().isLoadIndirect()
						        ...
						        && TR::TransformUtil::fieldShouldBeCompressed(
						                reference, comp())) {
							translateTT = TR::TreeTop::create(comp(),
							        TR::Node::createCompressedRefsAnchor(reference), NULL,
							        NULL);
						}
					}
					manager()->setAlteredCode(true);
				...
				for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
					...
					placeToInsertOptimalComputations =
					        replaceOptimalSubNodes(placeToInsertOptimalComputations,
					                nextOptimalNode, nextOptimalNode->getChild(k), k,
					                duplicateOptimalNode, duplicateOptimalNode->getChild(k), false,
					                block->getNumber(), visitCount4);
				}
			...
			TR::Node *duplicateOptimalNode =
			        duplicateExact(nextOptimalNode, &seenNodes, &duplicateNodes, visitCount2);
			...
			for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
				vcount_t visitCount4 = comp()->incVisitCount();
				placeToInsertOptimalComputations = replaceOptimalSubNodes(
				        placeToInsertOptimalComputations, nextOptimalNode, nextOptimalNode->getChild(k),
				        k, duplicateOptimalNode, duplicateOptimalNode->getChild(k), false,
				        block->getNumber(), visitCount4);
			}
			...
			if (fe()->dataTypeForLoadOrStore(duplicateOptimalNode->getDataType())
				...
				TR::ILOpCodes conversionOpCode =
				        TR::ILOpCode::getProperConversion(duplicateOptimalNode->getDataType(),
			...
			TR::ILOpCodes storeOp = type == TR::NoType ? TR::treetop
			...
			if (duplicateOptimalNode->getOpCode().isCall()
				...
				TR::TreeTop *anchorTT = TR::TreeTop::create(
				        comp(), TR::Node::create(TR::treetop, 1, duplicateOptimalNode), NULL, NULL);
...
static bool
isExpressionRedundant(TR::Node *node, TR_PartialRedundancy::ContainerType *redundantComputations,
        ...
        TR::Block *block, TR::Node **supportedNodesAsArray, ContainerType **rednSetInfo, TR::TreeTop *startTree)
{
...

	{
				...
				if (blockStructure->isLoopInvariantBlock())
					...
					while (parent) {
						...
						if (region->isNaturalLoop() && piv) {
							...
							if (callerIndex != -1)
								do {
									method = comp()->getInlinedResolvedMethod(
									        callerIndex);
									...
									callerIndex =
									        site._byteCodeInfo.getCallerIndex();
								} while (callerIndex != -1);
							else {
		...
		while (currentTree != exitTree) {
			...
			//
			eliminateRedundantSupportedNodes(NULL, currentTree->getNode(), false, currentTree,
			        block->getNumber(), visitCount,
			        shouldHaveRedundantComputations ? redundantComputations : NULL, anticipatabilityInfo,
			        supportedNodesAsArray);

			// Now handle stores
			//
			// Stores could be either treetops themselves or be under a TR::treetop or a null check/resolve
			// check
			//
			if ((firstOpCodeInTree.isStore()
				...
				if (isExpressionRedundant(
				            currentTree->getNode(), redundantComputations, anticipatabilityInfo)) {
					...
					if (!currentTree->getNode()->getOpCode().isStore()) {
						                                                      ...
						                                                      // version 1.296
						{
							if (firstOpCodeInTree.getOpCodeValue() == TR::NULLCHK)
								...
								TR::Node::recreate(
								        currentTree->getNode(), TR::ResolveCHK);
		      ...
		      currentTree->getNode()) */) {
							...
							TR::ILOpCodes opCodeValue = comp()->il.opCodeForDirectStore(
							        redundantNode->getDataType());

							TR::Node *storeForCommonedNode = NULL;
							if (currentTree->getNode()->getOpCode().isStoreIndirect()) {
								TR::Node *rhsOfStore =
								        currentTree->getNode()->getSecondChild();
								...
								storeForCommonedNode =
								        TR::Node::createWithSymRef(opCodeValue, 1, 1,
								                currentTree->getNode()->getFirstChild(),
								                newSymbolReference);
#ifdef J9_PROJECT_SPECIFIC
							correctDecimalPrecision(storeForCommonedNode,
							        storeForCommonedNode->getFirstChild(), comp());
#endif
				...
				if ((currentTree->getNode()->getNumChildren() > 0)
				        && isExpressionRedundant(currentTree->getNode()->getFirstChild(),
				                redundantComputations, anticipatabilityInfo)
				        && currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
		  ...
		  performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()->getFirstChild()) */)
                 {
						...
						if (currentTree->getNode()
						                ...
						                ->getOpCode()
						                .isStoreIndirect()) {
							TR::Node *rhsOfStore = currentTree->getNode()
							                               ->getFirstChild()
							                               ->getSecondChild();
							TR::Node *convertedOptimalNode = rhsOfStore;
							...
							storeForCommonedNode = TR::Node::createWithSymRef(opCodeValue,
							        1, 1, convertedOptimalNode, newSymbolReference);
						} else
							storeForCommonedNode = TR::Node::createWithSymRef(opCodeValue,
							        1, 1,
					...
					if (firstOpCodeInTree.isResolveOrNullCheck()) {
						...
						if (performTransformation(comp(),
							...
							for (nextAnchor = anchorIt.getFirst(); nextAnchor != NULL;
								...
								TR::TreeTop *anchorTreeTop = TR::TreeTop::create(comp(),
								        TR::Node::create(TR::treetop, 1, nextAnchor));
								TR::TreeTop *tempPrevTree =
								        currentTree->getPrevTreeTop();
...
//
void
TR_PartialRedundancy::collectAllNodesToBeAnchored(List<TR::Node> *anchoredNodes, TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
...
//
bool
TR_PartialRedundancy::eliminateRedundantSupportedNodes(TR::Node *parent, TR::Node *node, bool firstComputation,
	...
	if (isSupportedOpCode(node, parent)) {
		...
		if (_profilingWalk
			...
			if (comp()->isProfilingCompilation()
			        && ((node->getType().isInt32()) || ((node->getType().isInt64())))
			        && /* &&
 ...
 && */
		...
		if (isExpressionRedundant(node, redundantComputations, anticipatabilityInfo)) {
			...
			if (!isNodeAnImplicitNoOp(node)) {
				...
				if (newSymbolReference
				        && performTransformation(comp(),
				                "%sEliminating redundant computation (%s) : %p in block_%d, visit "
				                "count %d\n",
				                OPT_DETAILS, node->getOpCode().getName(), node, blockNum,
				                node->getVisitCount())) {
					if (node->getDataType() == TR::NoType) {
						...
						if (parent && parent->getOpCode().isConversion()
							...
							TR::Node *newLoad = TR::Node::createWithSymRef(node,
							        comp()->il.opCodeForDirectLoad(node->getDataType()), 0,
							        newSymbolReference);
							...
							if (conversionOpCode == TR::v2v) {
								node = TR::Node::createVectorConversion(
								        newLoad, node->getDataType());
							} else {
						...
						processReusedNode(node, comp()->il.opCodeForDirectLoad(nodeDataType),
						        newSymbolReference, 0);
					}
...
//
TR::TreeTop *
TR_PartialRedundancy::replaceOptimalSubNodes(TR::TreeTop *curTree, TR::Node *parent, TR::Node *node, int32_t childNum,
        TR::Node *duplicateParent, TR::Node *duplicateOptimalNode, bool isNullCheck, int32_t blockNum,
        vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return curTree;
	...
	if (isSupportedOpCode(node, parent)
		...
		if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0))
			...
			// We do not want to do optimal subnode replacement under tracePRE because it can cause spurious
			// failures as a result of earlier optimal node placement not having been done (because of opt
			// transformation index) that interfere with last opt transformation index based search. This is
			// done because optimal sub node replacement is rarely the cause of bugs on it's own and this
			// issue is just making it harder to debug problems in other PRE transformations. We created
			// another option TR_TracePREForOptimalSubNodeReplacement to continue to allow optimal sub node
			// replacement to be done regardless but in order to make this more useful we turn off limiting
			// optimal placements when this new option is in effect (so that it becomes safe to narrow down
			// to a optimal sub node replacement in the unlikely event that this is the cause of some
			// failure)
			//
			if (shouldAllowOptimalSubNodeReplacement(comp())
				...
				if (trace())
					traceMsg(comp(),
					        "Duplicate parent %p had its old child %p replaced by %p with symRef "
					        "#%d\n",
				...
				if (trace())
					traceMsg(comp(),
					        "Note : Duplicate parent %p wanted to replace its child %p by possibly "
				...
				for (i = 0; i < node->getNumChildren(); i++) {
					...
					curTree = replaceOptimalSubNodes(curTree, node, child, i, duplicateOptimalNode,
					        duplicateChild, isNullCheck, blockNum, visitCount);
				}
...
int32_t
TR_ExceptionCheckMotion::areExceptionSuccessorsIdentical(TR::CFGNode *node1, TR::CFGNode *node2)
{
	int32_t result = -1;
	_firstSucc->empty();
	_secondSucc->empty();
...
void
TR_ExceptionCheckMotion::copyListFromInto(List<TR::Node> *fromList, List<TR::Node> *toList)
{
	if (!fromList->isEmpty()) {
...
void
TR_ExceptionCheckMotion::composeLists(
	...
	if (!sourceList->isEmpty() && !targetList->isEmpty()) {
		...
		for (newNode = sourceList->getListHead(); newNode != NULL;) {
			...
			if ((oldNode == NULL) || (newNode == NULL)
				...
				ListElement<TR::Node> *cursorNode = cutoffNode ? cutoffNode->getNextElement()
				                                               : markerNode;
				while (cursorNode) {
					if (_definitelyNotKilled->get(cursorNode->getData()->getLocalIndex())
					        || ((cursorNode->getData()->getOpCode().isIndirect()
					                ...
					                && availableExprs->get(cursorNode->getData()
					                                               ->getFirstChild()
					                                               ->getLocalIndex())))
				...
				while (oldNode
					...
					if (!_composeHelper->get(oldNode->getData()->getLocalIndex())) {
						...
						if (prevCursorNode) {
							...
							// dumpOptDetails(comp(), "prevCursorNode %x %x oldNode %x
...
void
TR_ExceptionCheckMotion::appendLists(List<TR::Node> *firstList, List<TR::Node> *secondList)
{
...
bool
TR_ExceptionCheckMotion::compareLists(List<TR::Node> *firstList, List<TR::Node> *secondList)
{
	bool same = true;
...
void
TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo *info, TR_Structure *s)
{
...
void
TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo *info, TR::Block *block)
{
...
void
TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo *info, TR_RegionStructure *region)
{
	...
	if (region != comp()->getFlowGraph()->getStructure()) {
		...
		for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
			...
			if (!exitNodes->get(toStructureNumber)) {
				info->_outList[toStructureNumber] = new (trStackMemory())
	      ...
	      !checkIfNodeCanSurvive(node->getNullCheckReference(), _indirectAccessesThatSurvive))
                 ...
                 nodeKilledByChild)
              {
				...
				if ((!_optimisticRednSetInfo[blockNum]->get(
				        ...
				        && !checkIfNodeCanSurvive(
				                node->getFirstChild()->getSecondChild(), _dividesThatSurvive)) {
					_killedGenExprs[blockNum]->set(node->getLocalIndex());
					_nullCheckKilled->set(blockNum);
					_resolveCheckKilled->set(blockNum);
					_boundCheckKilled->set(blockNum);
                 ...
                 nodeKilledByChild)
              {
				...
				if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
                    ...
                    nodeKilledByChild)
                 {
						...
						//
						markNodeAsSurvivor(
						        node->getNullCheckReference(), _indirectAccessesThatSurvive);
					}
				}
			} else {
				_seenImmovableExceptionPoint = true;
...
void
TR_ExceptionCheckMotion::createAndAddListElement(TR::Node *node, int32_t blockNum)
{
...
bool
TR_ExceptionCheckMotion::isNodeKilledByChild(TR::Node *parent, TR::Node *child, int32_t blockNum)
{
...
bool
TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node *node, vcount_t visitCount, int32_t blockNum)
{
	if (node->getVisitCount() == visitCount) {
...
bool
TR_ExceptionCheckMotion::analyzeRegionStructure(TR_RegionStructure *regionStructure, bool checkForChange)
{
	...
	/////if (regionStructure != comp()->getFlowGraph()->getStructure())
	{
		...
		for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
			...
			if (analysisInfo->_outList[toStructureNumber] != NULL) {
				copyListFromInto(
				        _currentList[toStructureNumber], analysisInfo->_outList[toStructureNumber]);
			}
        ...
        TR::CFGNode *cfgNode, TR_RegionStructure *regionStructure, ContainerType *pendingList, ContainerType *exitNodes)
{
	...
	while (_analysisQueue.getListHead()
		...
		if (_analysisInterrupted) {
			comp()->failCompilation<TR::CompilationInterrupted>("interrupted in forward bit vector "
			                                                    "analysis");
		}
				...
				if (!regionStructure->isExitEdge(*succ)) {
					...
					if (!(isNaturalLoop && (succNode == regionStructure->getEntry()))) {
						...
						if (!(pendingList->get(succNode->getNumber())
							...
							copyListFromInto(
							        succInList, _currentList[succNode->getNumber()]);
						} else
					...
					if (!(isNaturalLoop && (succNode == regionStructure->getEntry()))) {
						...
						if (!(pendingList->get(succNode->getNumber())
							...
							List<TR::Node> *esuccInList =
							        getAnalysisInfo(succStructure)->_inList;
							copyListFromInto(
							        esuccInList, _currentList[succNode->getNumber()]);
						} else
							_currentList[succNode->getNumber()]->deleteAll();
					}
		...
		for (auto pred = nodeStructure->getPredecessors().begin();
			...
			if (pendingList->get(predStructure->getNumber()) || inSetChanged) {
				// comp()->incVisitCount();
				_nodesInCycle->empty();
				// if (analyzeNodeIfSuccessorsAnalyzed(pred->getFrom(), regionStructure, pendingList,
				// exitNodes))
				//   changed = true;
...
bool
TR_ExceptionCheckMotion::analyzeBlockStructure(TR_BlockStructure *blockStructure, bool checkForChange)
{
	...
	*_tempContainer &= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
	...
	for (bvi2.SetToFirstOne(); bvi2.Valid(); bvi2.SetToNextOne()) {
		...
		if (opCode.getOpCodeValue() == TR::NULLCHK) {
			markNodeAsSurvivor(
			        nextAvailableNode->getNullCheckReference(), _knownIndirectAccessesThatSurvive);
			...
			markNodeAsSurvivor(
			        nextAvailableNode->getFirstChild()->getSecondChild(), _knownDividesThatSurvive);
	...
	for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL; listElem = listElem->getNextElement()) {
		...
		if (!_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) {
			...
			if (placedEarlierCheckOptimally) {
				...
				if (_exprsContainingIndirectAccess->get(node->getLocalIndex())) {
					if (indirectAccessesKilled) {
						if (!checkIfNodeCanSomehowSurvive(node, _indirectAccessesThatSurvive)) {
							...
							if (!opCode.isCheck())
								removeFromList(
								        listElem, _blockInfo[blockNum], prevElem);
						}
					}
				}
				...
				if (_exprsContainingUnresolvedAccess->get(node->getLocalIndex())) {
					if (unresolvedAccessesKilled) {
						if (!checkIfNodeCanSomehowSurvive(
						            node, _unresolvedAccessesThatSurvive)) {
							checkKilledByChild = true;
							if (!opCode.isCheck())
								removeFromList(
								        listElem, _blockInfo[blockNum], prevElem);
			...
			if (_exprsContainingIndirectAccess->get(listElem->getData()->getLocalIndex())
				...
				canPlaceOptimally = false;
			else if (_exprsContainingDivide->get(listElem->getData()->getLocalIndex())
			...
			if (checkElement) {
				...
				if (opCodeValue == TR::NULLCHK) {
					...
					arrayStoreCheckKilled = true;
					divCheckKilled = true;
				} else if (listElem->getData()->getOpCode().isCheckCast()) /// opCodeValue ==
				                                                           /// TR::checkcast)
				{
					nullCheckKilled = true;
					resolveCheckKilled = true;
			...
			if (canPlaceOptimally
				...
				if (trace())
					traceMsg(comp(),
					        "IN ORDER Expr %d (representative) Node %p (listElem %p) in Block : "
					        "%d\n",
					        listElem->getData()->getLocalIndex(), listElem->getData(), listElem,
					        blockNum);
				...
				if (!_orderedOptList[blockNum])
					_orderedOptList[blockNum] = new (trStackMemory())
					        TR_ScratchList<TR::Node>(trMemory());
...
bool
TR_ExceptionCheckMotion::checkIfNodeCanSomehowSurvive(TR::Node *node, ContainerType *nodesThatSurvive)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		} else if ((i == 1) && (node->getOpCode().isDiv() || node->getOpCode().isRem())
...
bool
TR_ExceptionCheckMotion::checkIfNodeCanSurvive(TR::Node *node, ContainerType *nodesThatSurvive)
{
	if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) {
		//
...
void
TR_ExceptionCheckMotion::markNodeAsSurvivor(TR::Node *node, ContainerType *nodesThatSurvive)
{
	if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) {
		nodesThatSurvive->set(node->getLocalIndex());
	}
...
void
TR_ExceptionCheckMotion::removeFromList(
...
void
TR_RedundantExpressionAdjustment::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *, ContainerType *)
{}
...
TR_RedundantExpressionAdjustment::initializeGenAndKillSetInfo()
{
	...
	for (i = 1; i < _numberOfNodes; ++i) {
		...
		if (actualOptSetInfo[i]) {
			...
			*_regularGenSetInfo[i] &= *_partialRedundancy->_isolatedness->_latestness->_delayedness
			                                   ->_earliestness->_globalAnticipatability
			...
			*_optSetHelper -= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
			...
			//              reallocating a larger array thus resulting an a stale assumption which produces
			//              an AIOOB.
			// *_optSetHelper -=
...
bool
TR_RedundantExpressionAdjustment::analyzeBlockStructure(TR_BlockStructure *blockStructure, bool checkForChange)
{
	...
	if (_exceptionCheckMotion->getTrySecondBestSolution()) {
		...
		for (listElem = genSetInfo[blockNum]->getListHead(); listElem != NULL;
			...
			*/
			...
			if (trace())
				traceMsg(comp(),
				        "CONSIDERING Expr %d (representative) Node %p (listElem %p) in Block : %d\n",
