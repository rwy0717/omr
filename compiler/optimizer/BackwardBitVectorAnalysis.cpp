...
TR_BackwardDFSetAnalysis<Container *>::initializeDFSetAnalysis()
{
	...
	_currentOutSetInfo =
	        (Container **)this->trMemory()->allocateStackMemory(this->_numberOfNodes * sizeof(Container *));
	_originalOutSetInfo =
	        (Container **)this->trMemory()->allocateStackMemory(this->_numberOfNodes * sizeof(Container *));
...
template <class Container>
bool
TR_BackwardDFSetAnalysis<Container *>::canGenAndKillForStructure(TR_Structure *nodeStructure)
{
...
template <class Container>
void
TR_BackwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForBlock(TR_BlockStructure *s)
{
        ...
        TR_RegionStructure *regionStructure, TR_BitVector &pendingList, TR_BitVector &exitNodes, bool lastIteration)
{
	while (this->_analysisQueue.getListHead()
		...
		if (this->_analysisInterrupted) {
			...
			comp->failCompilation<TR::CompilationInterrupted>("interrupted in backward bit vector "
			                                                  "analysis");
		}
		...
		for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
			...
			TR_LinkHead<typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>
			        ...
			        *_regionKillSetInfo = normalSucc ? regionAnalysisInfo->_regularKillSetInfo
			        ...
			        *_regionGenSetInfo = normalSucc ? regionAnalysisInfo->_regularGenSetInfo
			...
			if (!seenSuccNodes.get(nodeNumber)) {
				...
				if (node == regionStructure->getEntry()) {
					...
					pair = analysisInfo->getContainerNodeNumberPair(
					        _currentKillSetInfo, nodeNumber);
					if (!pair) {
						pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
						        Container *>::TR_ContainerNodeNumberPair(NULL, nodeNumber);
		...
		if (traceBBVA()) {
			...
			if (analysisInfo->_currentExceptionKillSetInfo) {
				traceMsg(this->comp(),
				        "\nCurrent Kill Set Info for Region or Block : %p numbered %d is : \n",
				        nodeStructure->getStructure(), nodeStructure->getStructure()->getNumber());
				for (pair = analysisInfo->_currentExceptionKillSetInfo->getFirst(); pair;
				        pair = pair->getNext()) {
					if (pair->_container) {
						traceMsg(
						        this->comp(), "Exit or Succ numbered %d : ", pair->_nodeNumber);
...
template <class Container>
void
TR_BackwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure *region)
{
	...
	while (changed) {
		...
		for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
			if (noExitEdges
			        || (subNode->getSuccessors().empty() && subNode->getExceptionSuccessors().empty())) {
...
template <class Container>
bool
TR_BackwardDFSetAnalysis<Container *>::analyzeRegionStructure(TR_RegionStructure *regionStructure, bool checkForChange)
{
	...
	while (this->_analysisQueue.getListHead()
				...
				for (pair = regionAnalysisInfo->_outSetInfo->getFirst(); pair; pair = pair->getNext()) {
					...
					for (otherPair = analysisInfo->_outSetInfo->getFirst(); otherPair;
						...
						if (pair->_nodeNumber == otherPair->_nodeNumber) {
							compose(_currentOutSetInfo[pair->_nodeNumber],
							        pair->_container);
							break;
						}
					}
		...
		if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
			...
			for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
				...
				Container *killBitVector = analysisInfo->getContainer(normalSucc
				                ? analysisInfo->_regularKillSetInfo
				                : analysisInfo->_exceptionKillSetInfo,
				        nodeNumber);
				Container *genBitVector = analysisInfo->getContainer(normalSucc
...
template <class Container>
bool
TR_BackwardDFSetAnalysis<Container *>::analyzeBlockStructure(TR_BlockStructure *blockStructure, bool checkForChange)
{
...
template <class Container>
void
TR_BackwardDFSetAnalysis<Container *>::analyzeTreeTopsInBlockStructure(TR_BlockStructure *blockStructure)
{
	TR::Block *block = blockStructure->getBlock();
