...
void
TR_RegionAnalysis::simpleIterator(TR_Stack<int32_t> &workStack, StructureBitVector &vector, WorkBitVector &regionNodes,
        WorkBitVector &nodesInPath, bool &cyclesFound, TR::Block *hdrBlock, bool doThisCheck)
{
	...
	while (cursor.hasMoreElements()) {
		...
		// so if we come across the same node the second time (nodesInPath[next]) we could say that we are done
		// with this particular node and move on
...
void
TR_RegionAnalysis::StructInfo::initialize(TR::Compilation *comp, int32_t index, TR::Block *block)
{
 ...
 */
TR_Structure *
TR_RegionAnalysis::getRegions(TR::Compilation *comp, TR::ResolvedMethodSymbol *methSym)
{
 ...
 */
TR_RegionStructure *
TR_RegionAnalysis::findNaturalLoop(StructInfo &node, WorkBitVector &regionNodes, WorkBitVector &nodesInPath)
{
	regionNodes.empty();
	...
	TR_RegionStructure *region = new (_structureRegion)
...
void
TR_RegionAnalysis::addNaturalLoopNodesIterativeVersion(StructInfo &node, WorkBitVector &regionNodes,
...
void
TR_RegionAnalysis::addNaturalLoopNodes(StructInfo &node, WorkBitVector &regionNodes, WorkBitVector &nodesInPath,
 ...
 */
TR_RegionStructure *
TR_RegionAnalysis::findRegion(StructInfo &node, WorkBitVector &regionNodes, WorkBitVector &nodesInPath)
{
...
void
TR_RegionAnalysis::addRegionNodesIterativeVersion(StructInfo &node, WorkBitVector &regionNodes,
        WorkBitVector &nodesInPath, bool &cyclesFound, TR::Block *hdrBlock)
{
	// We will be pushing elements backwards to emulate processing in a depth-first order
	// Also, we will be pushing the same node twice in order to update nodesInPath correctly
...
void
TR_RegionAnalysis::addRegionNodes(StructInfo &node, WorkBitVector &regionNodes, WorkBitVector &nodesInPath,
        bool &cyclesFound, TR::Block *hdrBlock)
{
	int32_t index = node._nodeIndex;
 ...
 */
void
TR_RegionAnalysis::buildRegionSubGraph(TR_RegionStructure *region, StructInfo &entryNode, WorkBitVector &regionNodes,
