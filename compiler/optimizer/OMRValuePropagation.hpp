...
private:
	...
	TR::TreeTop *createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
	        TR::SymbolReference *dstRef, TR::Node *lenRef, TR::SymbolReference *srcObjRef,
	        TR::SymbolReference *dstObjRef, bool isForward);
...
public:
	...
	public:
		ValueConstraint(int32_t valueNumber) : TREE_NODE<ValueConstraint>(valueNumber) {}
	...
	void mergeRelationships(TR_LinkHead<Relationship> &fromList, TR_LinkHead<Relationship> &toList,
	        int32_t valueNumber, bool preserveFrom = false, StoreRelationship *mergingStore = NULL,
	        List<TR::Symbol> *storeSymbols = NULL, bool inBothLists = false);
	...
	Relationship *findValueConstraint(
	        int32_t valueNumber, ValueConstraints &valueConstraints, int32_t relative = AbsoluteConstraint);
	Relationship *findEdgeConstraint(
	        int32_t valueNumber, EdgeConstraints *edge, int32_t relative = AbsoluteConstraint);
	...
	TR::VPConstraint *mergeDefConstraints(
	        TR::Node *node, int32_t relative, bool &isGlobal, bool forceMerge = false);
	...
	struct TR_TreeTopNodePair {
		...
		TR_TreeTopNodePair(TR::TreeTop *treetop, TR::Node *n) : _treetop(treetop), _node(n) {}
	...
	struct TR_TreeTopWrtBarFlag {
		...
		TR_TreeTopWrtBarFlag(TR::TreeTop *treetop, uint8_t b) : _treetop(treetop), _flag(b) {}
	...
	struct TR_ArrayCopySpineCheck {
		...
		TR_ArrayCopySpineCheck(TR::TreeTop *arraycopyTree, TR::SymbolReference *arraycopySymRef,
		        TR::Node *srcObjNode, TR::Node *srcOffNode, TR::Node *dstObjNode, TR::Node *dstOffNode,
		        TR::Node *copyLenNode)
	...
	struct ObjCloneInfo {
		...
		bool _isFixed;
		ObjCloneInfo(TR_OpaqueClassBlock *clazz, bool isFixed) : _clazz(clazz), _isFixed(isFixed) {}
	};
	...
	struct ArrayCloneInfo {
		...
		TR_OpaqueClassBlock *_clazz;
		bool _isFixed;
		ArrayCloneInfo(TR_OpaqueClassBlock *clazz, bool isFixed) : _clazz(clazz), _isFixed(isFixed) {}
	};
	...
	void generateArrayTranslateNode(TR::TreeTop *vcallTree, TR::TreeTop *arrayTranslateTree,
	        ...
	        TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, TR::SymbolReference *tableRef,
	        bool hasTable);
#endif
	...
	struct LoopInfo : public TR_Link<LoopInfo> {
		LoopInfo(TR_RegionStructure *region, LoopInfo *parent)
		        : _loop(region), _parent(parent), _entryBlock(NULL), _backEdgeConstraints(NULL)
		{}
	...
	struct VirtualGuardInfo : public TR_Link<VirtualGuardInfo> {
		VirtualGuardInfo(OMR::ValuePropagation *vp, TR_VirtualGuard *vgOld, TR_VirtualGuard *vgNew,
	...
	void createNewBucketForArrayIndex(ArrayLengthToVersion *, TR_LinkHead<ArrayLengthToVersion> *, int32_t,
	        TR::Node *, TR::Node *, TR_OpaqueClassBlock *);
