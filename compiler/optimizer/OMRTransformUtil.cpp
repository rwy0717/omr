...
TR::Node *
OMR::TransformUtil::scalarizeArrayCopy(TR::Compilation *comp, TR::Node *node, TR::TreeTop *tt, bool useElementType,
        bool &didTransformArrayCopyNode, TR::SymbolReference *sourceRef, TR::SymbolReference *targetRef,
        bool castToIntegral)
{
	...
	if ((comp->getOptLevel() == noOpt) || !comp->getOption(TR_ScalarizeSSOps)
	        || node->getOpCodeValue() != TR::arraycopy || node->getNumChildren() != 3 || comp->requiresSpineChecks()
	        || !node->getChild(2)->getOpCode().isLoadConst() || cg->getOptimizationPhaseIsComplete())
		return node;
	...
	if (targetRef == NULL || sourceRef == NULL) {
		if (trace)
			traceMsg(comp,
			        "do not scalarizeArrayCopy node %p : targetRef is NULL (%s) or sourceRef is NULL "
			        "(%s)\n",
...
TR::Node *
OMR::TransformUtil::scalarizeAddressParameter(TR::Compilation *comp, TR::Node *address,
...
bool
OMR::TransformUtil::transformIndirectLoadChain(TR::Compilation *comp, TR::Node *node, TR::Node *baseExpression,
...
bool
OMR::TransformUtil::transformIndirectLoadChainAt(TR::Compilation *comp, TR::Node *node, TR::Node *baseExpression,
...
void
OMR::TransformUtil::transformCallNodeToPassThrough(
...
void
OMR::TransformUtil::createConditionalAlternatePath(TR::Compilation *comp, TR::TreeTop *ifTree, TR::TreeTop *thenTree,
        TR::Block *elseBlock, TR::Block *mergeBlock, TR::CFG *cfg, bool markCold)
{
	...
	elseBlock = ifBlock->split(
	        ifTree->getNextTreeTop(), cfg, false /*fixupCommoning*/, true /*copyExceptionSuccessors*/);
