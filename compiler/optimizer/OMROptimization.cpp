...
OMR::Optimization::anchorAllChildren(TR::Node *node, TR::TreeTop *anchorTree)
{
	...
	for (int i = 0; i < node->getNumChildren(); i++) {
		TR::TreeTop *tt =
		        TR::TreeTop::create(self()->comp(), TR::Node::create(TR::treetop, 1, node->getChild(i)));
		if (self()->trace())
			traceMsg(self()->comp(),
		...
		if (prevChild != child) // quite common for anchor to be called with two equal children
		{
			if (self()->nodeIsOrderDependent(child, depth, hasCommonedAncestor)) {
				dumpOptDetails(self()->comp(),
				        "%sanchor child %s [" POINTER_PRINTF_FORMAT "] at depth %d before %s "
 ...
 */
bool
OMR::Optimization::removeOrconvertIfToGoto(TR::Node *&node, TR::Block *block, int takeBranch, TR::TreeTop *curTree,
 ...
 */
TR::CFGEdge *
OMR::Optimization::changeConditionalToUnconditional(
 ...
 */
TR::Node *
OMR::Optimization::replaceNodeWithChild(
        TR::Node *node, TR::Node *child, TR::TreeTop *anchorTree, TR::Block *block, bool correctBCDPrecision)
{
	...
	if (correctBCDPrecision && node->getType().isBCD() && child->getType().isBCD()
		...
		// If the 'child' truncates then there is the danger that this truncating side-effect will be lost
		// unless a bcd modPrecOp is inserted. If correctBCDPrecision=false then the caller has guaranteed that
		// there is no danger of a child truncation or widening causing any problems.
		//
		...
		if (node->getReferenceCount() > 1) {
			TR::Node *newNode =
			        TR::Node::create(TR::ILOpCode::modifyPrecisionOpCode(child->getDataType()), 1, child);
			newNode->setDecimalPrecision(node->getDecimalPrecision());
			dumpOptDetails(self()->comp(),
			        "%sPrecision mismatch when replacing parent %s [" POINTER_PRINTF_FORMAT
			        "] with child %s [" POINTER_PRINTF_FORMAT "] so create new parent %s "
			        "[" POINTER_PRINTF_FORMAT "]\n",
			        self()->optDetailString(), node->getOpCode().getName(), node,
			        child->getOpCode().getName(), child, newNode->getOpCode().getName(), newNode);
			...
			// zd2pd    <- node - change to zdModifyPrecision (must use child type as modPrec is being
...
OMR::Optimization::replaceNode(TR::Node *node, TR::Node *other, TR::TreeTop *anchorTree, bool anchorChildren)
{
	if (!performTransformation(self()->comp(),
		...
		if (other->getReferenceCount() == 0) {
			// Because the replacement node will not be used and it does not currently exist in the
			// compilation unit, it should be destroyed.
