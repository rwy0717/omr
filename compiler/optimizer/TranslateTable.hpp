...
private:
	...
	// can do something smarter if number of tables gets big - expected to be quite small (1 or 2 tables in
	// practice)
	TR_TranslateTableData *matchTable(uint32_t startA, uint32_t endA, uint32_t startB, uint32_t endB,
	        uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue);
};
...
public:
	...
	TR_SetTranslateTable(TR::Compilation *, uint8_t, uint8_t, void *set, int copySize); // input and output size in
	                                                                                    // bits
};
