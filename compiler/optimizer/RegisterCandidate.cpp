...
bool TR_RegisterCandidate::find(TR::Block* b) { return _blocks.find(b->getNumber()); }
...
void TR_RegisterCandidate::removeLoopExitBlock(TR::Block* b) { _loopExitBlocks.remove(b); }
...
bool TR_RegisterCandidate::hasLoopExitBlock(TR::Block* b) { return (_loopExitBlocks.find(b) != 0); }
    ...
    for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next) {
        ...
        if (isFloat) {
            ...
            if (0 && comp()->cg()->spillsFPRegistersAcrossCalls()) {
                ...
                if (!temp.isEmpty()) {
                    ...
                    // rc->getSymbolReference()->getReferenceNumber()); printf("Discarding FP candidate in %s\n",
                    // comp()->getCurrentMethod()->signature());
                    continue;
                }
            }
        ...
        if (true) {
            ...
            while (bvi.hasMoreElements()) {
                ...
                // maxGPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum]);
