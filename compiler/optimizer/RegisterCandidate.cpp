...
void
TR::GlobalSet::collectReferencedAutoSymRefs(
...
void
TR_RegisterCandidate::addAllBlocksInStructure(
...
TR_RegisterCandidates::find(TR::SymbolReference *symRef)
{
	if (symRef->getSymbol()->isAutoOrParm()) {
		TR_RegisterCandidate *rc = (_candidateForSymRefs)
		        ? (*_candidateForSymRefs)[GET_INDEX_FOR_CANDIDATE_FOR_SYMREF(symRef)]
		        : NULL;
		// two symrefs can point to same symbols so it is possible that we have an rc for symref->getSymbol but
		// not have it added to the cache
		if (rc == NULL) {
...
TR_RegisterCandidate::printLoopInfo(TR::Compilation *comp)
{
	for (LoopInfo *li = _loops.getFirst(); li; li = li->getNext()) {
		traceMsg(comp, "Candidate #%d has %d loads and stores in loop %d\n",
		        getSymbolReference()->getReferenceNumber(), li->_numberOfLoadsAndStores,
		        li->_loop->getNumber());
	}
}
...
void
TR_RegisterCandidate::removeAssignedCandidateFromLoop(TR::Compilation *comp, TR_Structure *loop, int32_t regNum,
        TR_BitVector *liveOnEntryUsage, TR_BitVector *liveOnExitUsage, bool trace)
{
	...
	for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
		...
		// handle predecessors
		for (auto pred = nextBlock->getPredecessors().begin(); pred != nextBlock->getPredecessors().end();
		        ++pred) {
...
bool
TR_RegisterCandidates::aliasesPreventAllocation(TR::Compilation *comp, TR::SymbolReference *symRef)
{
        ...
        TR_BitVector &isExtensionOfPreviousBlock)
{
	...
	while (itr.hasMoreElements()) {
		...
		TR_ASSERT(
		        (blockNumber < cfg->getNextNodeNumber()) && (blocks[blockNumber] == b), "blockNumber is wrong");
        ...
        TR_Array<int32_t> &blockGPRCount, TR_Array<int32_t> &blockFPRCount, TR_Array<int32_t> &blockVRFCount)
{
	...
	if (_liveOnEntry.get(blockNum)) {
		...
		for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
			...
			// dumpOptDetails(comp, " pred %d actual gprs %d max gprs %d ", pred->getNumber(), gprCount,
			// maxGPRs);
...
TR_RegisterCandidate::processLiveOnEntryBlocks(TR::Block **blocks, int32_t *blockStructureWeight, TR::Compilation *comp,
        TR_Array<int32_t> &blockGPRCount, TR_Array<int32_t> &blockFPRCount, TR_Array<int32_t> &blockVRFCount,
        TR_BitVector *referencedBlocks, TR_Array<TR::Block *> &startOfExtendedBBForBB, bool callToRemoveUnusedLoops)
{
	...
	// used for MetaData that are live on entry in blocks, just to mark that they can escape via return but there is
	// no real loads so we do not want such live on entry blocks to contribute to weigh of the candidate
	...
	while (lookForBlocksToRemove) {
		...
		while (bvi.hasMoreElements()) {
			...
			if (trace)
				dumpOptDetails(comp,
				        "Examining original live-on-entry block_%d with loads and stores = %d\n",
			...
			for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
				TR_ASSERT(comp->getOptimizer()->cachedExtendedBBInfoValid(),
				        "Incorrect value in _startOfExtendedBBForBB");
				if (_liveOnEntry.get(
				            startOfExtendedBBForBB[toBlock((*e)->getFrom())->getNumber()]->getNumber()))
			...
			if (!referencedInPred)
				for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end();
				        ++e)
			...
			if (!referencedInPred) {
				if (trace)
					dumpOptDetails(
					        comp, "Not referenced in pred, re-setting block_%d \n", blockNumber);
	...
	while (bvi.hasMoreElements()) {
		...
		if (trace)
			dumpOptDetails(comp,
			        "Examining live-on-entry block_%d with loads and stores = %d to find maxFrequency\n",
			        blockNumber, _blocks.getNumberOfLoadsAndStores(blockNumber));
		TR::Block *block = blocks[blockNumber];
		...
		while (lookForBlocksToRemove) {
			...
			if (trace)
				traceMsg(comp, "At frequency %d for %d numBlocks %d numUnreferencedBlocks %d\n",
				        frequency, getSymbolReference()->getReferenceNumber(), numBlocksAtThisFrequency,
				        numUnreferencedBlocksAtThisFrequency);
							...
							if (parent) {
								if (!unreferencedLoops.find(parent)) {
									if (!referencedLoops.find(parent)) {
										List<TR::Block> blocksInReferencedLoop(
										        comp->trMemory());
										parent->getBlocks(
										        &blocksInReferencedLoop);
										ListIterator<TR::Block> blocksIt(
										        &blocksInReferencedLoop);
										TR::Block *nextBlock;
										blocksIt.reset();
										for (nextBlock = blocksIt.getCurrent();
										        nextBlock;
										        nextBlock =
										                blocksIt.getNext()) {
											if (nextBlock->getEntry()) {
												TR_ASSERT(
												        comp->getOptimizer()
												                ->cachedExtendedBBInfoValid(),
												        "Incorrect "
												        "value in "
												        "_startOfExtend"
												        "edBBForBB");
												TR::Block *liveBlock = startOfExtendedBBForBB
												        [nextBlock->getNumber()];
												if (_liveOnEntry.get(
												            liveBlock
												                    ->getNumber())) {
													if ((_blocks.getNumberOfLoadsAndStores(
													             blockNumber)
													            == 0)
													        && (!dontAssignInColdBlocks(
													                    comp)
													                || !nextBlock
													                            ->isCold())) {
														referencedInNearestLoop =
														        true;
														referencedLoops
														        .add(parent);
														break;
							...
							if (!referencedInNearestLoop) {
								...
								if (parent && !unreferencedLoops.find(parent)) {
									...
									ListIterator<TR::CFGEdge> ei(
									        &parent->getExitEdges());
									for (TR::CFGEdge *edge = ei.getCurrent();
									        edge != NULL; edge = ei.getNext()) {
										int32_t toStructureNumber =
										...
										if (toStructureNumber
										        == comp->getFlowGraph()
										                   ->getEnd()
										                   ->getNumber()) {
											// The exit block shouldn't be
											// in any live ranges anyway, so
											// putting it in the resetExits
											// set is unnecessary at best.
											//
										...
										//   for (succ = succIt.getFirst(); succ
										//   != NULL; succ = succIt.getNext())
										//       {
								...
								if (shouldResetThisLoop) {
									if (trace)
										dumpOptDetails(comp,
										        "\n2Loop does not use "
										        "candidate, removing block_%d "
										        "from live ranges of candidate "
										        "#%d \n",
										        blockNumber,
										        getSymbolReference()
										                ->getReferenceNumber());

									unreferencedLoops.add(parent); // should not be
									                               // moved where
									                               // exits are
									                               // removed ?
									_liveOnEntry.reset(blockNumber);
									_liveOnExit.reset(blockNumber);
									resetLiveOnEntry->set(blockNumber); // Added by
									                                    // Nakaike
									resetLiveOnExit->set(blockNumber); // Added by
									                                   // Nakaike
								} else
									referencedLoops.add(parent);
								// dumpOptDetails(comp, "Resetting liveOnEntry at
								// block_%d frequency %d for %d\n", blockNumber,
								// frequency,
								// getSymbolReference()->getReferenceNumber());
	...
	while (bvi.hasMoreElements()) {
		...
		if (trace)
			dumpOptDetails(comp,
			        "Examining live-on-entry block_%d (blockWeight %d maxFrequency %d) numberOfBlocks %d "
			        "loadsAndStores %d",
		...
		for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
			...
			if (((predWeight > blockWeight) && !liveOnExitFromPred && !_liveOnEntry.get(pred->getNumber()))
			        && _blocks.getNumberOfLoadsAndStores(
			                   startOfExtendedBBForBB[pred->getNumber()]->getNumber())
			                == 0
	...
	if (!comp->getOptimizer()->getSeenBlocksGRA())
		comp->getOptimizer()->setSeenBlocksGRA(new (comp->trHeapMemory())
		                TR_BitVector(comp->getFlowGraph()->getNextNodeNumber(), comp->trMemory()));
	else
	...
	while (bvi.hasMoreElements()) {
		...
		if (!_liveOnEntry.get(blockNumber) && (_blocks.getNumberOfLoadsAndStores(blockNumber) > 0)) {
			...
			if (trace)
				traceMsg(comp,
				        "Increased weight of candidate %d by %d (new weight %d) because of live on "
				        "exit from block_%d\n",
					...
					if (((*succ)->getTo() != block->getNextBlock())
						...
						if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
							...
							if (!exclude && blockStructure
								...
								TR_ASSERT(blockNumber == block->getNumber(),
								        "blocks[x]->getNumber() != x");
								comp->getOptimizer()->getSeenBlocksGRA()->set(
								        blockNumber);
								// blockStructure->calculateFrequencyOfExecution(&blockWeight);
								blockWeight = blockStructureWeight[blockNumber];
								int32_t succWeight =
								        blockStructureWeight[(*succ)->getTo()
								                                     ->getNumber()];
								...
								// blockWeight = (blockWeight + succWeight)/2; // maybe
								// the store will be inserted in a split block in
								...
								if (trace)
									traceMsg(comp,
									        "Reduced weight of candidate %d by %d "
									        "(new weight %d) because of end in "
									        "live range at block_%d succ %d\n",
									        getSymbolReference()
									                ->getReferenceNumber(),
									        blockWeight, loadsAndStores,
									        blockNumber,
				...
				for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end();
					...
					if ((*succ)->getTo() != block->getNextBlock()) {
						if (_liveOnEntry.get((*succ)->getTo()->getNumber())) {
							...
							if (!exclude && blockStructure
							        && (/* !dontAssignInColdBlocks(comp) || */ !block
							                        ->isCold())
							        && !comp->getOptimizer()->getSeenBlocksGRA()->get(
							                extBlockNumber)) {
								TR_Structure *containingLoop =
								...
								if (trace)
									traceMsg(comp,
									        "Reduced weight of candidate %d by %d "
									        "(new weight %d) because of start of "
									        "live range at block_%d succ %d\n",
									        getSymbolReference()
									                ->getReferenceNumber(),
									        blockWeight, loadsAndStores,
	...
	bool isPPS = false; // (comp->getOption(TR_EnableOSR) && getSymbol()->isAuto() &&
	                    // (getSymbolReference()->getCPIndex() < 0)); // This hurts in WAS performance; so it is
	                    // disable until I can figure out the issue
		...
		if (!comp->allocateAtThisOptLevel() && getSymbolReference()->getSymbol()->isMethodMetaData())
			...
			if (!callToRemoveUnusedLoops) {
				...
				processLiveOnEntryBlocks(blocks, blockStructureWeight, comp, blockGPRCount,
				        blockFPRCount, blockVRFCount, referencedBlocks, startOfExtendedBBForBB, true);
			} else {
				_weight = 0;
			}
		}
        ...
        TR::Compilation *comp, TR::Block **blocks, TR_Array<TR::Block *> &startOfExtendedBBForBB)
{
	...
	TR_BitVector *blocksReferencing = comp->getGlobalRegisterCandidates()->getBlocksReferencingSymRef(
	        getSymbolReference()->getReferenceNumber());
			...
			for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
				...
				do {
					...
					if (!_liveOnEntry.get(currBlock->getNumber()) && // not set already
						...
						if (trace)
							traceMsg(comp,
							        "\tFor candidate #%d, set live on block_%d entry due "
							        "to live on exit\n",
							        getSymbolReference()->getReferenceNumber(),
							        currBlock->getNumber());
...
bool
TR_RegisterCandidates::prioritizeCandidate(TR_RegisterCandidate *newRC, TR_RegisterCandidate *&first)
{
...
TR_RegisterCandidate *
TR_RegisterCandidates::reprioritizeCandidates(TR_RegisterCandidate *first, TR::Block **blocks,
	...
	for (TR_RegisterCandidate *rc = first; rc; rc = next) {
		...
		if (rc->getBlocksLiveOnEntry().intersects(*successorBits)) {
			if ((!onlyReprioritizeLongs || (rc->getType().isInt64() && TR::Compiler->target.is32Bit()))
				...
				if (trace) {
					dumpOptDetails(comp,
					        "\nBefore repriortization, removing the following blocks from live "
					        "ranges of candidate #%d \n",
					        rc->getSymbolReference()->getReferenceNumber());
					successorBits->print(comp);
				}

				rc->getBlocksLiveOnEntry() -= *successorBits;
				rc->recalculateWeight(blocks, blockStructureWeight, comp, blockGPRCount, blockFPRCount,
				        blockVRFCount, referencedBlocks, _startOfExtendedBBForBB);
			}
		}
...
static void
scanPressureSimulatorCacheForConflicts(TR_RegisterCandidate *rc, TR_BitVector &blocks,
        TR_Array<TR_BitVector> &liveOnEntryConflicts, TR_Array<TR_BitVector> &liveOnExitConflicts,
        TR_Array<TR_BitVector> &entryExitConflicts, int32_t firstRegister, int32_t lastRegister,
        TR_BitVector *spilledRegs, TR::Compilation *comp, TR::Block **all_blocks)
{
...
void
TR_RegisterCandidates::collectCfgProperties(TR::Block **blocks, int32_t numberOfBlocks)
{
	TR::CFG *cfg = comp()->getFlowGraph();
...
static void
assign_candidate_loop_trace_increment(TR::Compilation *comp, TR_RegisterCandidate *rc, unsigned count)
{
	bool trace = comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
	if (trace) {
	...
	for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next) {
		...
		if (comp()->getOption(TR_EnableGRACostBenefitModel)) {
			...
			if ((rc->getWeight() < weightThreshold)
				...
				if (trace)
					traceMsg(comp(),
					        "Leaving candidate because the compile time cost is not worth it\n");
		...
		if (type.isInt64() && cg->getDisableLongGRA()) {
			if (trace)
				traceMsg(comp(),
				        "Leaving candidate because LongGRA is disabled and candidate is 64 bit\n");
			continue;
		}
		...
		if (dt.isVector() && !comp()->cg()->hasGlobalVRF()) {
			if (trace)
				traceMsg(comp(),
				        "Leaving candidate because it has vector type but no global vector registers "
				        "provided\n");
		...
		if (((catchBlockLiveLocalsExist && rc->getSymbolReference()->getSymbol()->isAuto()
			...
			// TODO:GRA Decide if this fix is OK -- without it on zemulator we would have a useonlyalias set
			// for metadata and so we would set this propert
		...
		if (isFloat) {
			...
			if (0 && comp()->cg()->spillsFPRegistersAcrossCalls()) {
				...
				if (!temp.isEmpty()) {
					...
					// rc->getSymbolReference()->getReferenceNumber()); printf("Discarding FP
					// candidate in %s\n", comp()->getCurrentMethod()->signature());
					continue;
				}
			}
		...
		if (true) {
			...
			while (bvi.hasMoreElements()) {
				...
				// dumpOptDetails(comp(), "For block_%d maxGPRs %d maxFPRs %d ext block_%d maxGPRs %d
				// maxFPRs %d maxVRFs %d\n", b->getNumber(), maxGPRsLiveOnExit[b->getNumber()],
				// maxFPRsLiveOnExit[b->getNumber()],  blockNum, maxGPRsLiveOnExit[blockNum],
				// maxFPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum]); dumpOptDetails(comp(),
				// "For block_%d numGPRs %d numFPRs %d ext block_%d numGPRs %d numFPRs %d maxVRFs %d\n",
				// b->getNumber(), totalGPRCount[b->getNumber()], totalFPRCount[b->getNumber()],
				// blockNum, totalGPRCount[blockNum], totalFPRCount[blockNum], totalVRFCount[blockNum]);
					...
					if (totalFPRCount[blockNum]
						...
						for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
							...
							if (hasMultiWayBranch
								...
								if (trace)
									dumpOptDetails(comp(),
									        "FPR count exceeded : resetting live "
				...
				if (isVector) {
					if (totalVRFCount[blockNum] >= maxVRFsLiveOnExit[blockNum]) {
						...
						for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
							TR::Block *b = e->getTo()->asBlock();
							if (hasMultiWayBranch
							        || (totalVRFCountOnEntry[b->getNumber()]
							                >= maxVRFsLiveOnExit[blockNum])) {
								if (trace)
									dumpOptDetails(comp(),
									        "VRF count exceeded : resetting live "
									        "on entry block_%d (because of "
									        "extended block_%d)\n",
									        b->getNumber(), blockNum);
								rc->getBlocksLiveOnEntry().reset(b->getNumber());
					...
					if (totalGPRCount[blockNum]
						...
						for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
							...
							if (hasMultiWayBranch
								...
								if (trace)
									dumpOptDetails(comp(),
									        "GPR count exceeded : resetting live "
									        "on entry block_%d (because of "
									        "extended block_%d)\n",
									        b->getNumber(), blockNum);
								rc->getBlocksLiveOnEntry().reset(b->getNumber());
		...
		if (trace) // Feel this would be useful by default
		{
			dumpOptDetails(comp(), "\nFor candidate %d : trials left %d weight : %d\n",
			        rc->getSymbolReference()->getReferenceNumber(), rc->getReprioritized(),
			        rc->getWeight());
		...
		if (enableVectorGRA) {
			for (int i = firstRegister; i <= lastRegister; ++i) {
				if (vrbVecGRA && (cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i)) {
					...
					dumpOptDetails(comp(),
					        "ffpr %d lfpr %d fopf %d lopr %d fvrf %d lvrf %d fovrf %d lovrf %d\n",
					        cg->getFirstGlobalFPR(), cg->getLastGlobalFPR(),
					        cg->getFirstOverlappedGlobalFPR(), cg->getLastOverlappedGlobalFPR(),
			...
			for (int i = firstRegister; i <= lastRegister; ++i) {
				...
				if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i)
					...
					if (trace) {
						traceMsg(comp(),
						        "removing %s (%d) from available list since it is aliased with "
						        "%s (%d)\n",
		...
		if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
			...
			//
				...
				int32_t maxClearStructures = -1; // largest number of clear structures of the
				                                 // unavailable registers

				intersection.empty();
				List<TR_Structure> regStructureUses(trMemory()); // list of deepest loops where a
				                                                 // register is currently live
				List<TR_Structure> candStructureUses(trMemory()); // list of loops where this candidate
				                                                  // is currently live

				// loop over the blocks where this candidate is live, and add the loops that contain
				// those blocks to candStructureUses
				TR_BitVectorIterator bvi(rc->getBlocksLiveOnEntry());
				while (bvi.hasMoreElements()) {
					int32_t blockNumber = bvi.getNextElement();
					TR::Block *block = blocks[blockNumber];
				...
				if (useRegisterPressureInfo) {
					...
					scanPressureSimulatorCacheForConflicts(rc, liveOnEntryOrExit,
					        _liveOnEntryConflicts, _liveOnExitConflicts, _entryExitConflicts,
					        firstRegister, lastRegister, &spilledRegs, comp(), blocks);
				}

				for (i = firstRegister; i <= lastRegister; ++i) {
					if (trace) {
						traceMsg(comp(),
						        "Searching for blocks/structures with max frequency for reg: "
						        "%d\n",
						        i);
					}
...
#ifdef TRIM_ASSIGNED_CANDIDATES
								if (containingLoop
								        && isInnerLoop(comp(), containingLoop)) {
...
#endif
								if (autosInBlock
								        && autosInBlock->get(block->getNumber())) {
									// blockWeight =
									// blockStructureWeight[block->getNumber()];
									blockWeight =
									        rc->_blocks.getNumberOfLoadsAndStores(
									                block->getNumber());
									...
									if (blockWeight > maxFrequency) {
										...
										// collect all loops with blocks at this
										// frequency where this register is live
										regStructureUses.deleteAll();
										regStructureUses.add(containingLoop);
									} else if (blockWeight
									        >= maxFrequency * freqRatio) {
										numBlocksAtMaxFrequency++;

										// collect loops with blocks at this
										// frequency where this register is live
										if (!regStructureUses.find(
										            containingLoop))
											regStructureUses.add(
											        containingLoop);
									}
								}
							}
					...
					if (needs2Regs) {
						...
						for (j = firstRegister; j < i; ++j)
							if (!availableRegisters.get(j)) {
								// use the _liveOnEntryConflicts array element to
								// maintain the list of block numbers to remove from the
								// candidate's liveOnEntry list.
								//
								...
								if (numBlocksAtMaxFrequency == 0)
									numBlocksAtMaxFrequency =
									        intersection.elementCount();

								int32_t intersectionElemCount =
								        intersection.elementCount();
								if (conflictingRegister != -1) {
									intersection = _liveOnEntryConflicts
									        [conflictingRegister];
									if (secondConflictingRegister != -1)
										intersection &= _liveOnEntryConflicts
										        [secondConflictingRegister];
								}
						...
						// first check for smaller maximum frequency, then try to get as many
						// clear
				...
				if (conflictingRegister != -1) {
...
#ifdef TRIM_ASSIGNED_CANDIDATES
					TR_BitVector blocksToBeFreed(comp()->getFlowGraph()->getNextNodeNumber(),
						...
						while (bvi.hasMoreElements()) {
							...
							TR_RegisterCandidate *cand =
							        block->getGlobalRegisters(comp())[conflictingRegister]
							                .getRegisterCandidateOnEntry();
							if (cand) {
								cand->removeAssignedCandidateFromLoop(comp(), loop,
								        conflictingRegister,
					...
					rc->recalculateWeight(blocks, blockStructureWeight, comp(), totalGPRCount,
		...
		if (needs2Regs) {
			if (!performTransformation(comp(), "%s assign auto #%d to low reg %d and high reg %d (%s)\n",
			            OPT_DETAILS, rc->getSymbolReference()->getReferenceNumber(), registerNumber,
			            highRegisterNumber,
		...
		while (bvi.hasMoreElements()) {
			...
			if (trace)
				traceMsg(comp(), "Register %d in block_%d is candidate %d on entry\n", registerNumber,
				        nextElement, rc->getSymbolReference()->getReferenceNumber());
		...
		while (bvi.hasMoreElements()) {
			...
			if (trace)
				traceMsg(comp(), "Register %d in block_%d is candidate %d on exit\n", registerNumber,
				        blockNumber, rc->getSymbolReference()->getReferenceNumber());
				...
				if ((b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != NULL)
				        ...
				        && (rc
				                != b->getGlobalRegisters(comp())[registerNumber]
				                           .getRegisterCandidateOnEntry())) {
					traceMsg(comp(),
					        "Candidate %d instead of candidate %d is required in register %d on "
					        "exit out of block_%d\n",
					        b->getGlobalRegisters(comp())[registerNumber]
			...
			if (needs2Regs) {
				if (b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit()
				        != rc) {
					if ((b->getGlobalRegisters(comp())[highRegisterNumber]
					        ...
					        && (rc
					                != b->getGlobalRegisters(comp())[highRegisterNumber]
					                           .getRegisterCandidateOnEntry())) {
						traceMsg(comp(),
						        "Candidate %d instead of candidate %d is required in register "
						        "%d on exit out of block_%d\n",
						        b->getGlobalRegisters(comp())[highRegisterNumber]
						                .getRegisterCandidateOnExit()
						                ->getSymbolReference()
						                ->getReferenceNumber(),
			...
			if (isFloat) {
				if (++numberOfFPRsLiveOnExit[blockNumber] == maxFPRsLiveOnExit[blockNumber]) {
					...
					if (!comp()->getOptimizer()->getSuccessorBitsGRA())
						comp()->getOptimizer()->setSuccessorBitsGRA(
						        new (trHeapMemory()) TR_BitVector(
						                numberOfBlocks, trMemory(), heapAlloc, growable));
					else
					...
					next = reprioritizeCandidates(next, blocks, blockStructureWeight,
					        numberOfBlocks, b, comp(), true, false, &referencedBlocks,
			...
			if (isVector) {
				if (++numberOfVRFsLiveOnExit[blockNumber] == maxVRFsLiveOnExit[blockNumber]) {
					...
					next = reprioritizeCandidates(next, blocks, blockStructureWeight,
					        numberOfBlocks, b, comp(), false, false, &referencedBlocks,
					        totalGPRCount, totalFPRCount, totalVRFCount,
					        comp()->getOptimizer()->getSuccessorBitsGRA(), trace);
				}
			} else {
				        ...
				        == maxGPRsLiveOnExit[blockNumber]) // all reg cands need to be reprioritized
				{
					...
					next = reprioritizeCandidates(next, blocks, blockStructureWeight,
				        ...
				        == (maxGPRsLiveOnExit[blockNumber] - 1)) // only long cands need to be
				                                                 // reprioritized
				{
					// static TR_BitVector *successorBits;
					if (!comp()->getOptimizer()->getSuccessorBitsGRA())
					...
					next = reprioritizeCandidates(next, blocks, blockStructureWeight,
					        numberOfBlocks, b, comp(), false, true, &referencedBlocks,
					        totalGPRCount, totalFPRCount, totalVRFCount,
					        comp()->getOptimizer()->getSuccessorBitsGRA(), trace);
				}
			}
...
void
ComputeOverlaps(TR::Node *node, TR::Compilation *comp, TR_RegisterCandidates::Coordinates &overlaps, uint32_t &seqno)
{
...
void
ComputeOverlaps(TR::Block *block, TR::Compilation *comp, TR_RegisterCandidates::Coordinates &overlaps)
{
	uint32_t seqno = 0;
	comp->incVisitCount();
...
void
TR_RegisterCandidates::computeAvailableRegisters(TR_RegisterCandidate *rc, int32_t firstRegister, int32_t lastRegister,
		...
		while (bvi.hasMoreElements()) {
			...
			if (overlapLookup == overlapTable->end()) {
				overlaps = new (comp()->trMemory()->currentStackRegion())
				        Coordinates((CoordinatesComparator()),
