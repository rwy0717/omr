...
TR_StripMiner::collectLoops(TR_Structure *str)
{
	...
	if (region->isNaturalLoop()) {
		...
		// It is ok in case where if the stores are in different paths of the code. Then the value is equal to
		// the deltaOnBackEdge
		...
		if (checkIfIncrementalIncreasesOfPIV(li)) {
			if (trace())
				traceMsg(comp(),
				        "\tReject loop %d ==> multiple store of induction variable were found\n",
				        region->getNumber());
			return;
		}
		...
		if ((li->_offset == -1)
			...
			if (trace())
				traceMsg(comp(),
				        "\tReject loop %d ==> needs a offsetLoop - cannot deal with this now\n",
				        region->getNumber());
		...
		if (iterCount != -1) {
			if (iterCount < li->_stripLen) {
				if (trace())
					traceMsg(comp(),
					        "\tReject loop %d ==> iteration count is less than the strip length\n",
					        region->getNumber());
				return;
			}
		}
...
TR_StripMiner::transformLoop(LoopInfo *li)
{
	...
	bool isInt32 = ltTree->getNode()->getOpCode().isBooleanCompare() && // isCompare only returns true for signed
		...
		if ((li->_increasing && limit < limit + li->_stripLen)
			...
			if (trace())
				traceMsg(comp(), "No overflow test for %s loop %d: limit = %ld, stripLength = %ld\n",
				        li->_increasing ? "increasing" : "decreasing", loop->getNumber(), limit,
				        li->_stripLen);
			testOverflow = false;
		}
	}
	...
	if (li->_needOffsetLoop) {
		...
		if (testOverflow) {
			...
			if (trace())
				traceMsg(comp(),
				        "\t   adding edge: overflow test block [%d] => outer pre-header [%d]\n",
	...
	if (li->_preOffset > 0) {
		...
		if (trace()) {
			traceMsg(comp(),
			        ...
			        outerHeader->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(),
			        pre->getNumber());
		}
...
TR_StripMiner::stripMineLoop(LoopInfo *li, TR::Block *outerHeader)
{
	...
	TR::ILOpCodes updateOpCode = li->_increasing ? (isInt32 ? TR::iadd : TR::ladd)
	...
	if (trace()) {
		traceMsg(comp(),
		        ...
		        branchBlock->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(),
		...
		if ((block != lt) && (block->getNumber() < _nodesInCFG)) {
			for (exitEdge = eItExit.getFirst(); exitEdge; exitEdge = eItExit.getNext()) {
				if (exitEdge->getFrom()->getNumber() == block->getNumber()) {
					...
					for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end();
					        ++e) {
					...
					freq = (block->getFrequency() < dest->getFrequency()) ? block->getFrequency()
					                                                      : dest->getFrequency();
...
TR_StripMiner::createGotoBlock(TR::Block *source, TR::Block *dest)
{
	...
	intptrj_t freq = (source->getFrequency() < dest->getFrequency()) ? source->getFrequency()
	                                                                 : dest->getFrequency();
...
void
TR_StripMiner::redirect(TR::Block *source, TR::Block *oldDest, TR::Block *newDest)
{
	...
	if (found) {
		...
		if (trace()) {
			traceMsg(comp(), "\t      adding edges: source [%d] => goto [%d]; goto [%d] => new dest [%d]\n",
			        source->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(),
			        newDest->getNumber());
		}
	}
}
...
//
void
TR_StripMiner::examineLoop(LoopInfo *li, TR_ClonedLoopType type, bool checkClone)
{
	if (trace())
...
void
TR_StripMiner::examineNode(LoopInfo *li, TR::Node *parent, TR::Node *node, TR::SymbolReference *oldSymRef,
	...
	if (checkClone && node->getSymbolReference() == oldSymRef) {
		if (node->getOpCode().isLoad()) {
			...
			if (foundLoad) {
				...
				type == mainLoop ? li->_mainParentsOfLoads.add(
							...
							if (secondChild->getFirstChild()->getOpCode().isMul()
								...
								if (firstChild->getOpCode().isRightShift()
									...
									if ((accessNode->getOpCodeValue() == TR::iload)
										...
										if (accessNode->getOpCodeValue()
										        != TR::iload) {
											if ((accessNode->getOpCode()
											                    .isAdd()
											            || accessNode
											                       ->getOpCode()
											                       .isSub())
											        && accessNode->getFirstChild()
											                        ->getSymbolReference()
											                == oldSymRef
											        && accessNode->getSecondChild()
											                        ->getOpCodeValue()
											                == TR::iconst) {
												foundArrayAccess = true;
												intptrj_t offset =
												        accessNode->getOpCodeValue()
												                == TR::iadd
												        ? accessNode
												                  ->getSecondChild()
												                  ->getInt()
												...
												if (li->_increasing) {
													if (offset < 0)
														li->_preOffset = std::max(
														        li->_preOffset,
														        -1 * offset);
													else if (offset
													        > 0)
														li->_postOffset = std::max(
														        li->_postOffset,
														        offset);
												} else {
													if (offset > 0)
...
void
TR_StripMiner::replaceLoopPivs(LoopInfo *li, TR::ILOpCodes newOpCode, TR::Node *newConst,
        TR::SymbolReference *newSymRef, TR_ClonedLoopType type)
{
	...
	for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
		...
		if ((parentNode->getOpCode().isRightShift() || parentNode->getOpCode().isDiv()) && skipNode) {
			if (trace())
				traceMsg(comp(),
				        "      replacing load [%p] with new load [%p]: parent [%p], childNum %d\n",
	...
	for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
		...
		if (trace())
			traceMsg(comp(), "      replacing store [%p] with new store [%p]: new symref [%d]\n", node,
			        storeNode, newSymRef->getReferenceNumber());
...
TR_StripMiner::getLoopTest(TR_Structure *str, TR::Block *preHeader)
{
	...
	if (loopTest) {
		if (TR::ILOpCode::isNotEqualCmp(loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue())
			...
			if (trace())
				traceMsg(
				        comp(), "loop %d: found loop with eq/ne test condition\n", region->getNumber());
			loopTest = NULL;
