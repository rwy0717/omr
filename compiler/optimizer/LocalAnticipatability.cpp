...
void
TR_LocalAnticipatability::analyzeBlock(TR::Block *block, vcount_t visitCount1, vcount_t visitCount2, TR_BitVector *temp)
{
	...
	TR_BitVector *allSymbolReferencesInStore = new (trStackMemory())
	...
	while (currentTree != exitTree) {
		...
		if (_isStoreTree) {
			...
			if (_isNullCheckTree) {
				...
				if (downwardExposed
				        && !_downwardExposedBeforeButNotAnymore->get(treeTopNode->getLocalIndex())
				...
				if (isCurrentTreeTopAnticipatable) {
					if (!killedExpressions->get(treeTopNode->getLocalIndex())) {
						...
						if (trace())
							traceMsg(comp(),
							        "\n11Definition #%d is locally anticipatable in "
							        "block_%d\n",
		...
		//
		updateUsesAndDefs(currentTree->getNode(), block, seenDefinedSymbolReferences,
		        seenStoredSymbolReferences, seenUsedSymbolReferences, tempContainer, temp, storeNodes,
		        visitCount2);
...
bool
TR_LocalAnticipatability::updateAnticipatabilityForSupportedNodes(TR::Node *node,
        ...
        ContainerType *storeNodes, vcount_t visitCount)
{
	...
	if (visitCount <= node->getVisitCount()) {
		if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)
			...
			if (opCode.isLoad() || node->getOpCodeValue() == TR::loadaddr) {
				...
				if (!adjustInfoForAddressAdd(node, firstChild, seenDefinedSymbolReferences,
				            seenStoredSymbolReferences, killedExpressions, storeNodes, block))
					return false;
				if (!adjustInfoForAddressAdd(node, secondChild, seenDefinedSymbolReferences,
				            seenStoredSymbolReferences, killedExpressions, storeNodes, block))
					return false;
			} else
				return false;
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		if (!updateAnticipatabilityForSupportedNodes(child, seenDefinedSymbolReferences,
		            seenStoredSymbolReferences, block, killedExpressions, allSymbolReferences,
		            allSymbolReferencesInNullCheckReference, allSymbolReferencesInStore, storeNodes,
		            visitCount))
			flag = false;
	}
 ...
 */
bool
TR_LocalAnticipatability::adjustInfoForAddressAdd(TR::Node *node, TR::Node *child,
		...
		if (killedExpressions->get(child->getLocalIndex())) {
			if (trace())
				(TR::Compiler->target.is64Bit()) ? traceMsg(comp(),
				        "\n330Definition #%d (aladd) is NOT locally anticipatable in block_%d because "
				        "of child\n",
				        node->getLocalIndex(), block->getNumber())
				                                 : traceMsg(comp(),
...
void
TR_LocalAnticipatability::updateUsesAndDefs(TR::Node *node, TR::Block *block,
        ContainerType *seenDefinedSymbolReferences, ContainerType *seenStoredSymbolReferences,
...
void
TR_LocalAnticipatability::killDownwardExposedExprs(TR::Block *block, ContainerType *tempContainer, TR::Node *node)
{
...
void
TR_LocalAnticipatability::killDownwardExposedExprs(TR::Block *block, TR::Node *node)
{
	bool nodeBitShouldBeOn = false;
	bool storeNodeBitShouldBeOn = false;
	if (node && (node->getLocalIndex() != MAX_SCOUNT)) {
