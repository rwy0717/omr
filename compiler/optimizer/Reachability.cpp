...
TR_ReachabilityAnalysis::perform(TR_BitVector *result)
{
	...
	blocknum_t *depthMap =
	        (blocknum_t *)comp()->trMemory()->allocateStackMemory(numBlockIndexes * sizeof(depthMap[0]));
...
void
TR_ReachabilityAnalysis::propagateOneInput(blocknum_t inputBlockNum, blocknum_t blockNum, int32_t depth,
...
void
TR_ReachabilityAnalysis::traverse(
...
void
TR_ForwardReachability::propagateInputs(
...
void
TR_BackwardReachability::propagateInputs(
        blocknum_t blockNum, int32_t depth, blocknum_t *stack, blocknum_t *depth_map, TR_BitVector *closure)
{
	TR::Block *block = getBlock(blockNum);
