...
OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop *treeTop)
{
	...
	for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
		...
		if (!block && _reassociate && comp()->getFlowGraph()->getStructure() != NULL // [99391] getStructureOf()
		                                                                             // only valid if structure
		                                                                             // isn't invalidated
		...
		for (auto cursor = _performLowerTreeNodePairs.begin(); cursor != _performLowerTreeNodePairs.end();
		        ++cursor) {
...
TR::Node *
OMR::Simplifier::unaryCancelOutWithChild(
        TR::Node *node, TR::Node *firstChild, TR::TreeTop *anchorTree, TR::ILOpCodes opcode, bool anchorChildren)
{
	...
	if (firstChild->getOpCodeValue() == opcode
		...
		//     loadaddr size=4
		// This restriction could be loosened to only disallow intermediate truncations (see BCD case above) but
		// then would require a node op that would just correct for size (e.g. addrSizeMod size=3 to replace the
		// o2a/a2o pair)
		//
		// Do allow cases when all three sizes are the same and when the middle node widens but the top and
		// bottom node have the same size, e.g.
		//
		// i2o size=3
		//   o2i size=4
		//     oload size=3
		//
		// Also allow the special case where the grandchild is not really truncated as the 'truncated' bytes are
		// known to be zero (i.e. there really isn't an intermediate truncation of 4->3 even though it appears
		// that way from looking at the sizes alone) o2i
		//   i2o size=3
		//     iushr
		//       x
		//       iconst 8
		...
		if (node->getType().isIntegral() && nodeSize == grandChild->getSize()
			...
			if (grandChild->getOpCode().isRightShift() && grandChild->getOpCode().isShiftLogical()
				...
				if (trace())
					traceMsg(comp(),
					        "do allow unaryCancel of node %s (%p) and firstChild %s (%p) as "
					        "grandChild %s (%p) zeros the %d truncated bytes\n",
					        node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
					        firstChild, grandChild->getOpCode().getName(), grandChild,
					        truncatedBits / 8);
			}
		}
		...
		if (disallow) {
			if (trace())
				traceMsg(comp(),
				        "disallow unaryCancel of node %s (%p) and firstChild %s (%p) due to unequal "
				        "sizes (nodeSize %d, firstChildSize %d, firstChild->childSize %d)\n",
				        node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
				        firstChild, node->getSize(), firstChild->getSize(),
...
OMR::Simplifier::conditionalToUnconditional(TR::Node *&node, TR::Block *block, int takeBranch)
{
	...
	TR::CFGEdge *removedEdge =
	        changeConditionalToUnconditional(node, block, takeBranch, _curTree, optDetailString());
