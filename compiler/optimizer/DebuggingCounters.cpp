 ...
 */
NamedCounterInfo *
TR_DebuggingCounters::getOrCreateNamedCounter(TR::Compilation *comp, const char *name, int32_t d, int32_t bucketSize)
{
 ...
 */
void
TR_DebuggingCounters::insertCounter(const char *name, TR::Compilation *comp, TR::TreeTop *tt, int32_t d)
{
	...
	if (comp->getOptions()->insertDebuggingCounters()) {
		...
		TR::SymbolReference *counterRef =
		        comp->getSymRefTab()->createKnownStaticDataSymbolRef(ctrAddress, TR::Int32);
		...
		TR::Node *addNode = TR::Node::create(TR::iuadd, 2, loadNode, TR::Node::create(node, TR::iconst, 0, 1));
 ...
 */
void
TR_DebuggingCounters::insertInliningCounter(
...
void
TR_DebuggingCounters::insertInliningCounterOK(const char *name, TR::Compilation *comp, TR::TreeTop *origTT,
...
TR_DebuggingCounters::inliningReportForMethod(TR::Compilation *comp)
{
	...
	while (curSite != NULL) {
		if (!curSite->method && !curSite->opaque_method) {
			fprintf(output, "\t%d %d %d %d %d %s \t%s\n", curSite->flag, -1, curSite->frequency,
			        curSite->size, curSite->numLocals, curSite->name, "Unknown");
			...
			if (curSite->opaque_method)
				fprintf(output, "\t%d %d %d %d %d %s \t%s\n", curSite->flag,
				        ...
				        curSite->frequency, curSite->size, curSite->numLocals, curSite->name,
 ...
 */
bool
TR_DebuggingCounters::insertIfMissing(
