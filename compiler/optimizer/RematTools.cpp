...
void
RematSafetyInformation::add(TR::TreeTop *argStore, TR::SparseBitVector &symRefDependencies)
{
 ...
 */
bool
RematTools::getNextTreeTop(TR::TreeTop *&treeTop, TR_BitVector *blocksToVisit, TR::Block *startBlock)
{
 ...
 */
TR_YesNoMaybe
RematTools::gatherNodesToCheck(TR::Compilation *comp, TR::Node *privArg, TR::Node *currentNode,
        ...
        TR::SparseBitVector &visitedNodes)
{
	...
	if (opCode.hasSymbolReference() && !opCode.isLoad()) {
		if (trace)
			traceMsg(comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - non-load with a symref",
			        privArg, currentNode);
		return TR_no;
		...
		if (comp->requiresSpineChecks() && currentNode->getSymbol()->isArrayShadowSymbol()) {
			if (trace)
				traceMsg(comp,
				        "  priv arg remat: Can't fully remat [%p] due to [%p] - array access needs "
				        "spine check",
				        privArg, currentNode);
		...
		for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
			...
			TR_YesNoMaybe childResult = gatherNodesToCheck(comp, privArg, currentNode->getChild(i),
			        childScanTargets, childSymRefsToCheck, trace, visitedNodes);
			if (childResult == TR_no) {
				if (trace)
					traceMsg(comp,
					        "  priv arg remat: Can't fully remat [%p] due to [%p] - unsafe "
					        "arithmetic child %d",
		...
		if (trace)
			traceMsg(comp, "  guarded call remat: Can't fully remat [%p] due to [%p] - unhandled case",
			        privArg, currentNode);
		return TR_no;
	}
}
...
void
RematTools::walkNodesCalculatingRematSafety(TR::Compilation *comp, TR::Node *currentNode,
        ...
        bool trace, TR::SparseBitVector &visitedNodes)
{
	for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
		...
		walkNodesCalculatingRematSafety(comp, currentNode->getChild(i), scanTargets, enabledSymRefs,
		        unsafeSymRefs, trace, visitedNodes);
	}
 ...
 */
bool
RematTools::walkTreesCalculatingRematSafety(TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
        TR::SparseBitVector &scanTargets, TR::SparseBitVector &unsafeSymRefs, TR_BitVector *blocksToVisit, bool trace)
{
	...
	while (start != end) {
		if (start->getNode()) {
			walkNodesCalculatingRematSafety(comp, start->getNode(), scanTargets, enabledSymRefs,
			        unsafeSymRefs, trace, visitedNodes);
		}
 ...
 */
bool
RematTools::walkTreeTopsCalculatingRematFailureAlternatives(TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
        TR::list<TR::TreeTop *> &failedArgs, TR::SparseBitVector &scanTargets, RematSafetyInformation &rematInfo,
        TR_BitVector *blocksToVisit, bool trace)
{
	...
	while (start != end) {
		...
		if (start->getNode() && start->getNode()->getOpCode().isStoreDirect()
		        ...
		        && failedTargets.ValueAt(start->getNode()->getFirstChild()->getGlobalIndex())) {
			if (trace)
				traceMsg(comp, "  priv arg remat: considering partial remat with node [%p]",
				        start->getNode());
			for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
				if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference()
					...
					if (trace)
						traceMsg(comp,
						        "  priv arg remat: attempting partial remat with node [%p] for "
						        "[%p]",
