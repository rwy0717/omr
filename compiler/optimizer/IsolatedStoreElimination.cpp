...
TR_IsolatedStoreElimination::perform()
{
			...
			if (groupOfStores
				...
				while (bviUses.hasMoreElements()) {
					...
					dumpOptDetails(comp(), "%sRemoving %s n%dn [" POINTER_PRINTF_FORMAT "] %s\n",
					        optDetailString(), node->getOpCode().getName(), node->getGlobalIndex(),
					        node,
					...
					_trivialDefs->reset(index); // It's not a trivial def if we're handling it as a
					...
					if (treeTop->getNode()->getOpCode().isSpineCheck()
						...
						TR::Node::recreate(node,
						        comp()->il.opCodeForConst(node->getSymbolReference()
						                                          ->getSymbol()
						                                          ->getDataType()));
						node->setFlags(0);
						...
						if (usr.isUnsafe(child)) {
							...
							if (trace())
								traceMsg(comp(),
								        "  - replace unsafe child %s n%dn with dummy "
								        "%s n%dn\n",
			...
			if (node) {
				if (trace()) {
					traceMsg(comp(), "removing trivial node %p %s n%dn udi=%d\n", node,
					        node->getOpCode().getName(), node->getGlobalIndex(), useDefIndex);
				...
				if (useDefInfo->getUseDef(defsOfRhs, node->getFirstChild()->getUseDefIndex())
					...
					for (cursor1.SetToFirstOne(); cursor1.Valid(); cursor1.SetToNextOne()) {
						...
						for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
							int32_t nextUse = (int32_t)cursor2
							        + useDefInfo->getFirstUseIndex();
	...
	if (eliminatedStore) {
		...
		if (eliminatedBCDStore)
			requestOpt(OMR::treeSimplification); // some of the more complex BCD simplifications work best
			                                     // when refCounts == 1
#endif
...
TR_IsolatedStoreElimination::performWithUseDefInfo()
{
	...
	// bits in "Visited" and also on "ShouldNotBeRemoved". After finding the group set the right bits in
	// "ToBeRemoved" or "ShouldNotBeRemoved and update Visited vector. Also, emtpy InTransit vector
	//*******************************************************************************************

	_defStatus = new (trStackMemory())
	        TR_Array<defStatus>(trMemory(), info->getNumDefOnlyNodes(), true, stackAlloc);
	...
	for (i = info->getNumDefOnlyNodes() - 1; i >= 0; --i) {
		if (_defStatus->element(i) == notVisited) {
			...
			foundGroupOfStores =
			        groupIsolatedStores(i, &currentGroupOfStores, optimizer()->getUseDefInfo());
...
void
TR_IsolatedStoreElimination::collectDefParentInfo(int32_t defIndex, TR::Node *node, TR_UseDefInfo *info)
{
	if (node->getReferenceCount() > 1)
		return;
...
static void
examChildrenForValueNode(TR::TreeTop *tt, TR::Node *parent, TR::Node *valueNode, vcount_t visitCount)
{
	...
	if (tt) {
		...
		if (node->getValueChild()->getOpCode().isBCDLoadVar()
			...
			for (TR::TreeTop *ttTemp = tt->getNextTreeTop();
			        ttTemp && (valueChild->getFutureUseCount() > 0); ttTemp = ttTemp->getNextTreeTop()) {
				...
				if (defNodeHasSymRef
				        && defNode->getSymbolReference()->canKill(valueChild->getSymbolReference())) {
					if (trace())
						traceMsg(comp(),
...
void
TR_IsolatedStoreElimination::examineNode(TR::Node *node, vcount_t visitCount, bool seenMultiplyReferencedParent)
{
...
bool
TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(TR_UseDefInfo *info, TR_Structure *structure,
        vcount_t visitCount, TR_BitVector *nodesInStructure, TR_BitVector *defsInStructure,
        bool *propagateRemovalToParent)
{
	...
	if (structure->asRegion()) {
		...
		if (onlyExitEdge == -1) {
			...
			// If the parent can be removed then it doesn't matter how many exit edges there are for this
			// structure.
		...
		TR_BitVector *nodesInSubStructure = new (trStackMemory())
		        TR_BitVector(0, trMemory(), stackAlloc, growable);
		TR_BitVector *defsInSubStructure = new (trStackMemory())
		        TR_BitVector(0, trMemory(), stackAlloc, growable);
		...
		for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
			...
			if (findStructuresAndNodesUsedIn(info, subStruct, visitCount, nodesInSubStructure,
			            defsInSubStructure, &toPropagateRemoval)) {
           ...
           regionStructure->getParent()->asRegion()->isCanonicalizedLoop() */))
         {
			...
			if (symbolInCompare) {
				...
				for (v = regionStructure->getFirstInductionVariable(); v; v = v->getNext()) {
					if ((v->getLocal()->getDataType() == TR::Int32) || // I have made int checks a
					                                                   // bit weaker.. the code
					                                                   // below doesn't make sense
					                                                   // for unsigned operations //
					                                                   // it might make sense to
					                                                   // guard this with a test
...
static TR::Node *
getLoopIncrementStep(TR::Block *block, TR::SymbolReference *symRef, bool shouldBeSub, int32_t *loopStride)
{
...
void
TR_IsolatedStoreElimination::analyzeSingleBlockLoop(TR_RegionStructure *regionStructure, TR::Block *block)
{
	TR_InductionVariable *v;
						...
						{
							{
								...
								TR::Node *loopIncrement = getLoopIncrementStep(block,
								        inductionSymRef, shouldBeSub, &loopStride);

								if (loopIncrement && (loopStride == 1)
								        && ((loopIncrement->getFirstChild()
								                    == firstChildOfLastTree)
								                || (firstChildOfLastTree->getOpCode()
								                                .isLoadVarDirect()
								                        && ((firstChildOfLastTree
								                                        ->getSymbolReference()
								                                        ->getReferenceNumber()
									...
									if ((lastTreeInLoopHeader->getOpCodeValue()
									            == TR::ificmpgt)
									        || (lastTreeInLoopHeader
									                        ->getOpCodeValue()
									                == TR::ificmpge)) {
										numIterations = TR::Node::create(
										        TR::isub, 2,
										        TR::Node::createWithSymRef(
										                secondChildOfLastTree,
										                TR::iload, 0,
										                inductionSymRef),
										        secondChildOfLastTree
										                ->duplicateTree());
										if (lastTreeInLoopHeader
										                ->getOpCodeValue()
										        == TR::ificmpge)
											numIterations = TR::Node::create(
											        TR::iadd, 2,
        ...
        TR::Node *node, vcount_t visitCount, TR_BitVector *nodesSeen, TR_BitVector *defsSeen)
{
	...
	if (node->getOpCode().isLikeDef() && node->getUseDefIndex()) {
		// we don't need to record defs marked as stored value is irrelevant - there will be no uses of the dead
		// values
