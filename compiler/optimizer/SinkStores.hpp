...
public:
	...
	int32_t _symIdx;
	TR_BitVector *_commonedLoadsUnderTree; // commoned used syms under this store in the IL tree (syms are not first
	                                       // used)
	                                       // - syms that have associated temps are excluded (only when
	                                       // enablePreciseSymbolTracking() == false otherwise all syms are
	                                       // included)
	TR_BitVector *_commonedLoadsAfter; // used syms which are commoned following the store (syms are first used and
	                                   // are commoned later)
	TR::Compilation *comp() { return _comp; }
...
public:
	...
	TR_SideExitStorePlacement(TR::Block *b, TR::TreeTop *t, bool copy) : _block(b), _store(t), _copyStore(copy) {}
...
public:
	...
	bool _copy; // whether original store should be copied or moved
	            //  it should be copied if the store is placed in a block that does NOT extend the original source
	            //  block it should be moved if it is to be placed in a block that extends the original source block
...
public:
	...
	List<TR_EdgeInformation> _edges; // list of cfg edges to _block that should be split to place def
	                                 // NOTE: only one of these edges need be split: others can simply be redirected
...
public:
	...
	TR_BlockStorePlacement(TR_StoreInformation *store, TR::Block *block, TR_Memory *m) : _stores(m)
	{
		_stores.add(store);
		_block = block;
	}
...
public:
	...
	TR_BlockListEntry(TR::Block *block, TR::CFGEdge *pred, TR::Region &region) : _preds(region)
	{
		_block = block;
		if (pred != 0)
			_preds.add(pred);
...
protected:
	...
	bool shouldSinkStoreAlongEdge(int symIdx, TR::CFGNode *block, TR::CFGNode *succBlock,
	...
	enum { UsesDataFlowAnalysis = 0x0001,
		// AVAILABLE                             = 0x0002,
