...
bool
TR_OrderBlocks::needBetterChoice(TR::CFG *cfg, TR::CFGNode *block, TR::CFGNode *bestSucc)
{
	...
	// bestSucc_%d:sec_%d\n",block->getNumber(), block->asBlock()->getSectionNumber(), bestSucc->getNumber(),
	...
	if (bestSucc != comp()->getStartBlock() && bestSucc->asBlock()->isSuperCold()
		...
		if (trace())
			traceMsg(comp(),
			        "\t\tneedBetterChoice: hot block_%d:cold_%d: follows a cold block "
			        "bestSucc_%d:cold_%d\n",
...
// valid hot blocks.
TR::CFGNode *
TR_OrderBlocks::findSuitablePathInList(List<TR::CFGNode> &list, TR::CFGNode *prevBlock)
{
...
bool
TR_OrderBlocks::hasValidCandidate(List<TR::CFGNode> &list, TR::CFGNode *prevBlock)
{
	ListElement<TR::CFGNode> *prev = NULL;
	ListElement<TR::CFGNode> *ptr = list.getListHead();
	TR::CFGNode *block = NULL;
...
bool
TR_OrderBlocks::endPathAtBlock(TR::CFGNode *block, TR::CFGNode *bestSucc, TR::CFG *cfg)
{
	...
	if (bestSuccStructure == NULL || blockStructure == NULL) {
		if (trace())
			traceMsg(comp(), "\t\tEnd path because structure is NULL block_%d=%p to block_%d=%p \n",
			        block->asBlock()->getNumber(), blockStructure, bestSucc->asBlock()->getNumber(),
			        bestSuccStructure);
		return true;
	}
		...
		if (bestSuccLoop != blockLoop) {
			if (trace())
				traceMsg(comp(), "\t\tEnd path because different loop block_%d=%p to block_%d=%p \n",
				        block->asBlock()->getNumber(), blockLoop, bestSucc->asBlock()->getNumber(),
				        bestSuccLoop);
			return true;
		} else
			return false;
...
bool
TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge *candEdge, TR::Compilation *comp)
{
	...
	for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
		...
		for (auto predSuccEdge = predSuccessors.begin(); predSuccEdge != predSuccessors.end(); ++predSuccEdge) {
			...
			// predSuccEdge->getFrom()->getNumber(), predSuccEdge->getTo()->getNumber(),
			// predSuccEdge->getFrequency()); traceMsg(comp, "cand edge %d -> %d freq %d\n",
...
isCandidateTheHottestSuccessor(TR::CFGEdge *candEdge, TR::Compilation *comp)
{
	...
	if (!comp->getFlowGraph()->getStructure()) // we cannot trust below checks if someone invalidated structure
	                                           // earlier in this opt
		return true;
...
bool
TR_OrderBlocks::candidateIsBetterSuccessorThanBest(TR::CFGEdge *candidateEdge, TR::CFGEdge *currentBestEdge)
{
	...
	if (trace())
		traceMsg(comp(), "\tComparing candidate %d(%d) to current best %d(%d) as successor for %d(%d)\n",
		        candidate->getNumber(), candidate->getFrequency(), currentBest->getNumber(),
		        currentBest->getFrequency(), prevBlock->getNumber(), prevBlock->getFrequency());
		...
		if (candidateEdge->getFrequency() >= 0) {
			if (candidateEdge->getFrequency() == currentBestEdge->getFrequency()) {
				...
				if (lexicalSuccessorEntry != NULL
					...
					if (trace())
						traceMsg(comp(),
						        "\t\tis equally hot, but is currently the lexical successor, "
	...
	if (candidate->hasSuccessor(currentBest)) {
		if (candidate->getFrequency() > (prevBlock->getFrequency() - candidate->getFrequency())) {
			if (trace())
				traceMsg(comp(),
				        "\t\thas has current best succ as a succ, detecting an if-then structure and "
				        "making the if block my best choice\n");
			return true;
		}
	}
	...
	if (!_changeBlockOrderBasedOnHWProfile) {
		...
		if (!candidate->asBlock()->isCold() && currentBest->asBlock()->isCold()) {
			if (trace())
				traceMsg(comp(),
				        "\t\tcurrent best choice is cold but this one isn't, making it my best "
				        "choice\n");
			return true;
	...
	if (bestHasHazards && !candHasHazards) {
		if (trace())
			traceMsg(comp(),
			        "\t\tbest choice has hazards but candidate doesn't, making it my best choice\n");
		// numberHazardCandidates++;
		return true;
	}
...
TR::CFGNode *
TR_OrderBlocks::chooseBestFallThroughSuccessor(TR::CFG *cfg, TR::CFGNode *block, int32_t &numCandidates)
{
	...
	for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
		...
		if (block->asBlock()->getExit() && block->asBlock()->getExit()->getNextTreeTop()
			...
			if (trace())
				traceMsg(comp(),
				        "\t\tfound original fall-through successor %d: making it initial best "
				        "successor\n",
				        succBlock->getNumber());
		}
	...
	if (numCandidates == 1) {
		...
		if (!_changeBlockOrderBasedOnHWProfile) {
			...
			if (candBlock->isSuperCold()) {
				if (!_hotPathList.isEmpty()) {
					if (trace())
						traceMsg(comp(),
						        "\tcandidate %d is cold, discard because there are still hot "
						        "choices \n",
						        theCandidate->getNumber());
					return NULL;
					...
					if (trace())
						traceMsg(comp(),
						        "\tcandidate %d is super cold, discard because there are still "
						        "choices which are not so cold \n",
						        theCandidate->getNumber());
					return NULL;
				}
			}
			...
			if (candBlock->getFrequency() == 0 && hasValidCandidate(_hotPathList, block)) {
				if (trace())
					traceMsg(comp(),
					        "HW Profile: candidate %d has freq zero, discard because there are "
					        "still hot choices \n",
					        candBlock->getNumber());
				return NULL;
			}
		...
		// if candBlock is a loop exit but the loop has multiple exits, choose the hottest exit instead of
		// candBlock
							...
							if (edge->getTo()->asBlock()->getVisitCount() != _visitCount) {
								...
								if (trace())
									traceMsg(comp(),
									        ...
									        "current exit frequency %4d\n",
									        edge->getTo()->getNumber(),
									        edge->getFrequency(), exitFrequency);
	...
	for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
		...
		if (pred != block) {
			...
			if (predSuccEdge == successors.end()) {
				if (trace())
					traceMsg(comp(),
					        "\t\tbut it has another predecessor %d for which it is the hottest "
					        "successor\n",
					        pred->getNumber());
				if (trace())
	...
	if (_changeBlockOrderBasedOnHWProfile) {
		...
		if (candBlock->getFrequency() == 0 && hasValidCandidate(_hotPathList, block)) {
			if (trace())
				traceMsg(comp(),
				        "HW Profile: candidate %d has freq zero, discard because there are still hot "
				        "choices \n",
				        candBlock->getNumber());
			return NULL;
		}
...
void
TR_OrderBlocks::addToOrderedBlockList(TR::CFGNode *block, TR_BlockList &list, bool useNumber)
{
...
void
TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode *block, TR::CFGNode *excludeBlock)
{
...
void
TR_OrderBlocks::addRemainingSuccessorsToListHWProfile(TR::CFGNode *block, TR::CFGNode *excludeBlock)
{
	...
	for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
		...
		if (succBlock != excludeBlock && succBlock->getVisitCount() != _visitCount
			...
			if (trace())
				traceMsg(
				        comp(), "\t\tAdding unvisited non-cold successor %d\n", succBlock->getNumber());
			addToOrderedBlockList(succBlock, _hotPathList, false);
		}
	}
...
bool
TR_OrderBlocks::peepHoleGotoToFollowing(TR::CFG *cfg, TR::Block *block, TR::Block *followingBlock, char *title)
{
	// pattern: block has goto to fall-through block
...
// returns TRUE if the pattern was found and replaced
bool
TR_OrderBlocks::peepHoleGotoToGoto(TR::CFG *cfg, TR::Block *block, TR::Node *gotoNode, TR::Block *destOfGoto,
        char *title, TR::BitVector &skippedGotoBlocks)
{
	...
	if (destOfGoto->isGotoBlock(comp(), true)) {
		...
		if (performTransformation(comp(),
		            "%s in block_%d, dest of goto (%d) is also goto block, forwarding destination (%d) back "
		            "into goto\n",
			...
			if (parent && parent->isNaturalLoop() && (parent->getNumber() == destOfGoto->getNumber()))
				cfg->setStructure(0); // to avoid automatic structure repair creating an improper region
...
// returns TRUE if the pattern was found and replaced
bool
TR_OrderBlocks::peepHoleGotoToEmpty(
        TR::CFG *cfg, TR::Block *block, TR::Node *gotoNode, TR::Block *destOfGoto, char *title)
{
	...
	if (destOfGoto->isEmptyBlock()
		...
		if (destFallThroughDest != NULL) {
			...
			if (performTransformation(comp(),
			            "%s in block_%d, dest of goto is empty block, forwarding destination (%d) back "
			            "into goto\n",
				...
				if (parent && parent->isNaturalLoop()
					...
					cfg->setStructure(0); // to avoid automatic structure repair creating an
					                      // improper region due to temporary state after adding the
					                      // first edge
...
static bool
peepHoleGotoToLoopHeader(TR::CFG *cfg, TR::Block *block, TR::Block *dest, char *title)
{
	...
	if (destStructure && blockStructure) {
		...
		if (isSingleton) {
			...
			if (predBlock && predBlock->getStructureOf())
				predBlockIsSibling =
				        (predBlock->getStructureOf()->getParent() == blockStructure->getParent());
		}
		if (parent && parent->asRegion() && destIsSibling && predBlockIsSibling && predBlock
			...
			if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() && (branchDest != dest)
			        && blockIsFallThrough
			        && (branchNode->getOpCodeValue() != TR::ibranch
			                && branchNode->getOpCodeValue() != TR::mbranch)
			        && performTransformation(cfg->comp(),
...
void
TR_OrderBlocks::removeRedundantBranch(TR::CFG *cfg, TR::Block *block, TR::Node *branchNode, TR::Block *takenBlock)
{
...
bool
TR_OrderBlocks::peepHoleBranchToLoopHeader(
	...
	if (destStructure && fallThroughStructure && blockStructure) {
		...
		if (parent && parent->asRegion() && fallThroughIsSibling && !destIsSibling
		        ...
		        performTransformation(comp(),
		                "%s applied loop header peephole for block_%d fall through %d dest %d\n", title,
...
void
TR_OrderBlocks::peepHoleBranchAroundSingleGoto(TR::CFG *cfg, TR::Block *block, char *title)
{
		...
		if (!disableSPMD) {
			...
			if (blockStructure) {
				if (blockStructure->isLoopInvariantBlock()) {
					if (trace())
						traceMsg(comp(),
						        "\t\tavoid redirecting a jump to a pre-header block %d for "
						        "later SPMD optimization\n",
		...
		if (!doNotRemove)
			...
			if (takenBlock == blockAfterFallThrough
				...
				if (fallThroughSuccessors.front()->getTo()->asBlock() != blockAfterFallThrough) {
					...
					if (performTransformation(comp(),
					            "%s in block_%d, branch taken dest (%d) is a block after a single "
					            "goto, so redirecting to its goto's destination (%d)\n",
					            title, block->getNumber(), takenBlock->getNumber(),
					            gotoDestBlock->getNumber())) {
						TR_ASSERT(branchNode->getOpCode().isBranch(),
						        "expected branch at end of prevBlock to reverse, but found "
						        "something else");
						...
						if (!block->hasSuccessor(gotoDestBlock)) {
							...
							block->getSuccessors().front()->setFrequency(std::min(
							        ...
							        fallThroughBlock->getEdge(gotoDestBlock)
							                ->getFrequency()));
						}
...
TR_OrderBlocks::peepHoleBranchBlock(TR::CFG *cfg, TR::Block *block, char *title)
{
	...
	while (takenBlock->isGotoBlock(comp(), true)) {
		...
		if ((!takenBlock->getStructureOf() || !takenBlock->getStructureOf()->isLoopInvariantBlock())
		        && performTransformation(comp(),
		                "%s in block_%d, branch taken dest (%d) is a goto block, so redirecting to its "
		                "destination (%d)\n",
			...
			if (isDoubleton && !block->hasSuccessor(takenGotoDestBlock)) {
				...
				if (trace()) {
					...
					traceMsg(comp(), "\t\t\tinstead of edge (b_%d -> b_%d) freq: %d \n",
					        block->getNumber(), takenBlock->getNumber(), newEdge->getFrequency());
				}
...
bool
TR_OrderBlocks::peepHoleBranchToFollowing(TR::CFG *cfg, TR::Block *block, TR::Block *followingBlock, char *title)
{
...
TR_OrderBlocks::removeEmptyBlock(TR::CFG *cfg, TR::Block *block, char *title)
{
	...
	if (performTransformation(comp(), "%s empty block_%d, redirecting edges around this block then removing it\n",
		...
		if (!block->getSuccessors().empty()) {
			...
			if (trace())
				traceMsg(comp(),
				        "\t\tblock has no successors so no edges to redirect, just removing "
				        "predecessor edges\n");
			for (auto predEdge = block->getPredecessors().begin();
			        predEdge != block->getPredecessors().end();) {
			...
			traceMsg(comp(),
			        "\t\tblock has no successors edges to redirect, no predecessor edges to remove\n");
...
TR_OrderBlocks::doPeepHoleBlockCorrections(TR::Block *block, char *title)
{
	...
	if (block->getPredecessors().empty() && (block->hasExceptionPredecessors() == false)
	        && block->getEntry() != NULL) {
		if (performTransformation(comp(),
	...
	while (madeAChange) {
		...
		if (prevBlock != NULL) {
			...
			// if we made either of the above changes, then prevBlock might be empty now (remove it if it
			// is)
	...
	for (node = cfg->getFirstNode(); node; node = node->getNext()) {
		...
		if (rootStructure) {
			...
			if (block->getStructureOf() != NULL) {
				...
				if (trace())
					traceMsg(
					        comp(), "\t\tLoop nesting depth set to %d\n", block->getNestingDepth());
			}
		}
	}
}
...
TR_OrderBlocks::generateNewOrder(TR_BlockList &newBlockOrder)
{
	...
	while (block != NULL || !(_hotPathList.isEmpty() && _coldPathList.isEmpty())) {
		...
		// else if block is non-NULL and its best successor is in a different structure, then we should see if
...
TR::Block *
TR_BlockOrderingOptimization::insertGotoFallThroughBlock(TR::TreeTop *fallThroughTT, TR::Node *node,
...
void
TR_BlockOrderingOptimization::connectTreesAccordingToOrder(TR_BlockList &newBlockOrder)
{
	TR::CFG *cfg = comp()->getFlowGraph();
	...
	// right now, this code just assumes that it can safely rewrite the trees...should probably put some assumes in
	// here
	...
	while (!newBlockOrder.isEmpty()) {
		...
		if (fallThroughTT != block->getEntry()) {
			...
			// if prevBlock is exit, or prevBlock was originally the last block, then there's no special
			// work to do also, if there is no flow edge from prevBlock to the original fall through block,
			...
			if (prevBlock->getEntry() != NULL && fallThroughTT != NULL
				...
				if (trace())
					traceMsg(comp(),
					        "\t%d did not originally follow %d in trees, might need to move "
					        "trees\n",
					        block->getNumber(), prevBlock->getNumber());
				// 5 possibilities, the first two of which require some fix-up:
				//     1) block is not a successor of prevBlock, but prevBlock's fall-through successor
				//     has already been placed elsewhere 2) block is a different successor block than
				//     the original fall-through
				//        similar to 5, but we can reverse the branch in this case
				//     3) block is the only successor of prevBlock, but prevBlock ends in a goto
				//     (straightening should clean-up later) 4) block is not a successor of prevBlock,
				//     but it begins the path we (arbitrarily?) chose to follow prevBlock 5) block is a
				//     successor of prevBlock, but it's not the original fall through (due to
				//     endPathAtBlock)
				//        and we can't reverse branch (TR::branch and isJumpWithMulitpleTargets), we
				//        need a go to for this case
				TR::Node *branchNode = prevBlock->getLastRealTreeTop()->getNode();
				if (branchNode->getOpCodeValue() == TR::treetop)
					branchNode = branchNode->getFirstChild();

				if (branchNode->getOpCodeValue() != TR::Goto
				        && (!branchNode->getOpCode().isJumpWithMultipleTargets() || // don't worry about
				                                                                    // igotos
				...
				if (trace())
					traceMsg(comp(),
					        "\t%d did not originally follow %d in trees, but no trees manipulation "
					        "needed\n",
					        block->getNumber(), prevBlock->getNumber());
			}
...
TR_OrderBlocks::perform()
{
	...
	// block extension must be the last thing we do...particularly after peephole opts because they can change a
...
checkOrderingConsistency(TR::Compilation *comp)
{
	...
	for (; block; prevBlock = block, block = block->getNextBlock()) {
		...
		if (debugConsistencyCheck && prevFlowsToBlock && prevsHotterSuccessor != NULL) {
			if (rootStructure)
				fprintf(stderr,
				        "Block %d(%d) doesn't look like the best successor compared to %d(%d)\n",
				        block->getNumber(), block->getFrequency(), prevsHotterSuccessor->getNumber(),
				        prevsHotterSuccessor->getFrequency());
			else
				fprintf(stderr,
				        "Block %d(%d,%d) doesn't look like the best successor compared to %d(%d,%d)\n",
...
TR_BlockOrderingOptimization::dumpBlockOrdering(TR::TreeTop *tt, char *title)
{
	...
	while (tt != NULL) {
		...
		if (node && node->getOpCodeValue() == TR::BBStart) {
			...
			traceMsg(comp(), "block_%-4d\t[ " POINTER_PRINTF_FORMAT "]\tfrequency %4d", block->getNumber(),
			        block, block->getFrequency());
...
TR_BlockShuffling::perform()
{
	...
	TR::Block *firstShufflableBlock = firstExecutableBlock->getNextBlock(); // first executable block must be
	                                                                        // physically first
...
TR_BlockShuffling::riffle(TR::Block **blocks)
{
	...
	if (!performTransformation(comp(),
	            ...
	            _numBlocks, blocks[split]->getNumber()))
		return;
...
TR_BlockShuffling::reverse(TR::Block **blocks)
{
	...
	for (upper = 0, lower = _numBlocks - 1; upper < lower; upper++, lower--)
		if (performTransformation(comp(),
		            "O^O BLOCK SHUFFLING:   swap [%3d] and [%3d] (block_%d and block_%d)\n", upper, lower,
		            blocks[upper]->getNumber(), blocks[lower]->getNumber()))
