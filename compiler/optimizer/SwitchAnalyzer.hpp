...
public:
	...
	public:
		SwitchInfo(TR_Memory *m) : _kind(Dense), _count(0), _freq(0), _cost(0), _min(INT_MAX), _max(INT_MIN)
		{
			_chain = new (m->trHeapMemory()) TR_LinkHead<SwitchInfo>();
		}

		SwitchInfo(CASECONST_TYPE value, TR::TreeTop *target, int32_t cost)
		        : _kind(Unique), _count(1), _freq(0), _min(value), _max(value), _target(target), _cost(cost)
		{}
