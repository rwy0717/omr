...
int32_t
TR_RedundantAsyncCheckRemoval::perform(TR_Structure *str, bool insideImproperRegion)
{
...
//
void
TR_RedundantAsyncCheckRemoval::computeCoverageInfo(TR_StructureSubGraphNode *node, TR_StructureSubGraphNode *entry)
{
	if (node->getVisitCount() == comp()->getVisitCount())
		return;
		...
		for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
			...
			if (!succ->getStructure())
				...
				if (succ == entry)
					...
					case NotCovered: notCoveredOnSomePaths = true; break;
					case PartiallyCovered:
						coveredOnSomePaths = true;
						notCoveredOnSomePaths = true;
						break;
					case FullyCovered: coveredOnSomePaths = true; break;
					}
				}
			}
		}
...
bool
TR_RedundantAsyncCheckRemoval::isMaxLoopIterationGuardedLoop(TR_RegionStructure *loop)
{
...
int32_t
TR_RedundantAsyncCheckRemoval::findShallowestCommonCaller(int32_t callSiteIndex1, int32_t callSiteIndex2)
{
...
bool
TR_RedundantAsyncCheckRemoval::originatesFromShortRunningMethod(TR_RegionStructure *region)
{
	...
	// The following is an algorithm to detect such loops. In the algorithm, we assume that if all branches of all
	// basic blocks of a loop have the same owning method, then that loop originates from that method.
...
int32_t
TR_RedundantAsyncCheckRemoval::processNaturalLoop(TR_RegionStructure *region, bool isInsideImproperRegion)
{
	if (trace())
	...
	if (!isInsideImproperRegion) {
		...
		if ((comp()->getMethodHotness() == scorching) && originatesFromShortRunningMethod(region)) {
			...
			if (trace())
				traceMsg(comp(),
				        "Loop %d originates from a trusted method, and therefore, is tagged as short "
				        "running. Skipping\n",
				        region->getNumber());
		}
...
void
TR_RedundantAsyncCheckRemoval::markAncestors(TR_StructureSubGraphNode *node, TR_StructureSubGraphNode *entry)
{
	...
	for (auto edge = node->getPredecessors().begin(); edge != node->getPredecessors().end(); ++edge) {
		...
		//
		if (info->getCoverage() == PartiallyCovered && !(pred->getSuccessors().size() == 1)
		        && !info->isAncestor() && info->canHaveAYieldPoint()) {
...
void
TR_RedundantAsyncCheckRemoval::enqueueSinks(TR_RegionStructure *region, TR_Queue<TR_StructureSubGraphNode> *q, bool in)
{
	if (region->isAcyclic()) {
...
//
bool
TR_RedundantAsyncCheckRemoval::performRegionalBackwardAnalysis(TR_RegionStructure *region, bool inInfo)
{
	...
	while (!q->isEmpty()) {
		...
		for (auto edge = node->getSuccessors().begin();
		        edge != node->getSuccessors().end() && !hasUnanalyzedChildren; ++edge) {
			TR_StructureSubGraphNode *succ = toStructureSubGraphNode((*edge)->getTo());
		...
		bool backedge = false;
		for (auto edge = node->getSuccessors().begin();
		        edge != node->getSuccessors().end() && !notCoveredOnSomePaths; ++edge) {
			TR_StructureSubGraphNode *succ = toStructureSubGraphNode((*edge)->getTo());
		...
		out = out
		        || (info->hasYieldPoint() && !info->hasSoftYieldPoint()
		                && ((region && GET_ASYNC_INFO(region->getEntry())->getReverseCoverageInfo())
		                        || !earlyExit))
...
void
TR_RedundantAsyncCheckRemoval::solidifySoftAsyncChecks(TR_StructureSubGraphNode *node)
{
	...
	if (b) {
		if (GET_ASYNC_INFO(node)->hasSoftYieldPoint()) {
			if (performTransformation(
			            comp(), "%sinserted async check in block_%d\n", OPT_DETAILS, b->getNumber())) {
				TR::Block *block = b->getBlock();
			...
			if (einfo->isUnknownValue() && iinfo->getKind() != Geometric) {
				...
				if (iinfo->getKind() == Geometric) {
					...
					// messed up induction variable info
						...
						int32_t diff = in - lim;
						int32_t iters = (diff < 0) ? ceiling(-diff, incr)
						                           : ceiling(diff, -incr);

						// The above calculation is approximate, it is possible that the number
						// of iterations is calculated to be a negative number, because of
						// commoning. Ignore these cases. Usually the diff is off the actual
						// exact value by a difference of incr.
						//
						if (iters < 0)
							iters = 0;
...
void
TR_LoopEstimator::getLoopIncrements(TR_BitVector &candidates, IncrementInfo **loopIncrements)
{
...
void
TR_LoopEstimator::mergeWithLoopIncrements(TR::Block *block, IncrementInfo **loopIncrements)
{
...
bool
TR_LoopEstimator::isRecognizableExitEdge(
...
{
	IncrementInfo ***array =
	        (IncrementInfo ***)trMemory()->allocateStackMemory(_numBlocks * sizeof(IncrementInfo **));
...
{
	IncrementInfo **array =
	        (IncrementInfo **)trMemory()->allocateStackMemory(_numCandidates * sizeof(IncrementInfo *));
...
//
bool
TR_LoopEstimator::isProgressionalStore(TR::Node *node, TR_ProgressionKind *kind, int32_t *incr)
{
...
//
bool
TR_LoopEstimator::getProgression(TR::Node *expr, TR::SymbolReference **ref, TR_ProgressionKind *prog, int32_t *incr)
{
	int32_t konst;
	TR_ProgressionKind kind;
...
TR_LoopEstimator::EntryInfo *
TR_LoopEstimator::getEntryValue(
        TR::Block *block, TR::SymbolReference *symRef, TR_BitVector &nodesDone, EntryInfo **entryInfos)
{
	...
	if (!entryTree) {
		if (symRef->getSymbol()->isParm()) {
			entryInfos[blockIndex] = myInfo = new (trStackMemory()) EntryInfo(); // parameters have
			                                                                     // unintialized values
		} else
			TR_ASSERT(
			        entryTree, "unintialized local discovered.  Uninitialized locals cannot exist in Java");
			...
			if (ref->getReferenceNumber() == symRef->getReferenceNumber()) {
				if (node->getFirstChild()->getOpCode().isLoadConst())
					entryInfos[blockIndex] = myInfo = new (trStackMemory())
					        EntryInfo(node->getFirstChild()->getInt());
				else
					entryInfos[blockIndex] = myInfo = new (trStackMemory()) EntryInfo(); // unknown
					                                                                     // value
			}
		}
	}
