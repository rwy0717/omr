...
bool
TR_BlockStructure::doDataFlowAnalysis(TR_DataFlowAnalysis *dfa, bool checkForChanges)
{
...
bool
TR_RegionStructure::doDataFlowAnalysis(TR_DataFlowAnalysis *dfa, bool checkForChanges)
{
...
void
TR_RegionStructure::ExitExtraction::extractUnconditionalExits(const TR::list<TR::Block *, TR::Region &> &blocks)
{
	collectWork(blocks);
...
void
TR_RegionStructure::ExitExtraction::collectWork(const TR::list<TR::Block *, TR::Region &> &blocks)
{
...
void
TR_RegionStructure::ExitExtraction::collectWorkFromRegion(TR_RegionStructure *region, const StructureSet &relevant)
{
...
void
TR_RegionStructure::ExitExtraction::extractStructure(TR_Structure *const initialStructure)
{
...
TR_BitVector &
TR_RegionStructure::ExitExtraction::regionContents(TR_RegionStructure *const region)
{
	...
	for (auto e = _allPreds.begin(); e != _allPreds.end(); ++e) {
		...
		if (_trace) {
			traceMsg(_comp, "added exit edge (%d->%d) to region %d:%p\n", src->getNumber(),
			        node->getNumber(), region->getNumber(), region);
		}
	}
...
void
TR_RegionStructure::ExitExtraction::moveOutgoingEdgeToParent(TR_RegionStructure *const region,
...
TR_RegionStructure::replacePart(TR_Structure *from, TR_Structure *to)
{
	...
	TR_ASSERT(node != getEntry() || _nodeIndex == to->getNumber(),
	        "cannot replace the entry node in %d with node %d", _nodeIndex, to->getNumber());
...
void
TR_RegionStructure::collectExitBlocks(List<TR::Block> *exitBlocks, List<TR::CFGEdge> *exitEdges)
{
...
void
TR_BlockStructure::collectExitBlocks(List<TR::Block> *exitBlocks, List<TR::CFGEdge> *exitEdges)
{
	exitBlocks->add(getBlock());
}

void
TR_RegionStructure::addGlobalRegisterCandidateToExits(TR_RegisterCandidate *inductionCandidate)
{
...
static bool
findCycle(TR_StructureSubGraphNode *node, TR_BitVector &regionNodes, TR_BitVector &nodesSeenOnPath,
...
TR_RegionStructure::removeEdgeWithoutCleanup(TR::CFGEdge *edge, bool isExitEdge)
{
	...
	if (std::find(fromNode->getSuccessors().begin(), fromNode->getSuccessors().end(), edge)
		...
		TR_ASSERT((std::find(fromNode->getExceptionSuccessors().begin(),
		                   fromNode->getExceptionSuccessors().end(), edge)
		...
		TR_ASSERT((std::find(toNode->getExceptionPredecessors().begin(),
		                   toNode->getExceptionPredecessors().end(), edge)
...
int32_t
TR_Structure::getMaxNestingDepth(int32_t *currentNestingDepth, int32_t *currentMaxNestingDepth)
{
	return *currentMaxNestingDepth;
}

int32_t
TR_RegionStructure::getMaxNestingDepth(int32_t *currentNestingDepth, int32_t *currentMaxNestingDepth)
{
...
void
TR_Structure::adjustWeightForBranches(
		...
		for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
			...
			if (!subStruct->asRegion()) {
				if (weight > subStruct->getWeight()) {
					...
					// subNode->getNumber(), subStruct, weight);
				}
			} else if (subStruct->asRegion()->isAcyclic())
				adjustWeightForBranches(
				        subStruct->asRegion()->getEntry(), subStruct->asRegion()->getEntry(), &weight);
		}
	}
...
TR_Structure *
TR_BlockStructure::cloneStructure(TR::Block **correspondingBlocks, TR_StructureSubGraphNode **correspodingSubNodes,
        List<TR_Structure> *whileLoops, List<TR_Structure> *correspondingWhileLoops)
{
...
TR_Structure *
TR_RegionStructure::cloneStructure(TR::Block **correspondingBlocks, TR_StructureSubGraphNode **correspondingSubNodes,
        List<TR_Structure> *whileLoops, List<TR_Structure> *correspondingWhileLoops)
{
	...
	for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
		...
		TR_StructureSubGraphNode *clonedSubNode = new (structureRegion)
...
//
List<TR::Block> *
TR_BlockStructure::getBlocks(List<TR::Block> *blocksInRegion, vcount_t visitCount)
{
...
List<TR::Block> *
TR_RegionStructure::getBlocks(List<TR::Block> *blocksInRegion, vcount_t visitCount)
{
	TR_StructureSubGraphNode *subNode;
	TR_Structure *subStruct = NULL;
	TR_RegionStructure::Cursor si(*this);
...
void
TR_BlockStructure::addExternalEdge(TR_Structure *from, int32_t toNumber, bool isExceptionEdge)
{}

void
TR_RegionStructure::addExternalEdge(TR_Structure *from, int32_t toNumber, bool isExceptionEdge)
{
...
TR_RegionStructure::collapseIntoParent()
{
	if (debug("traceCollapseRegion")) {
		diagnostic(
		        "==> Structure before collapsing %d into parent %d\n", getNumber(), getParent()->getNumber());
...
TR_RegionStructure::checkStructure(TR_BitVector *_blockNumbers)
{
	...
	for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
		...
		for (auto edge = node->getPredecessors().begin(); edge != node->getPredecessors().end(); ++edge) {
			...
			isConsistent = false;
			for (auto succEdge = pred->getSuccessors().begin(); succEdge != pred->getSuccessors().end();
			        ++succEdge) {
				if (*succEdge == *edge) {
					isConsistent = true;
					break;
		...
		for (auto edge = node->getExceptionPredecessors().begin();
			...
			TR_ASSERT_FATAL(isConsistent,
			        "exception predecessor of this subGraph node not not contain this node in its "
		...
		for (auto edge = node->getExceptionSuccessors().begin(); edge != node->getExceptionSuccessors().end();
			...
			TR_ASSERT_FATAL(isConsistent,
			        "exception successor of this subGraph node not not contain this node in its exception "
			        "predecessors");
		}
	...
	for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
		...
		for (seenNode = exitNodesIt.getFirst(); seenNode; seenNode = exitNodesIt.getNext()) {
			if ((seenNode->getNumber() == toNode->getNumber()) && (seenNode != toNode))
				TR_ASSERT_FATAL(0,
				        "Exit edges to the same node number %d in region %p must have a unique "
				        "structure subgraph node (node1 %p node2 %p)\n",
	...
	if (isNaturalLoop()) {
		...
		if (numNewHeaders == 1
			...
			for (TR::CFGEdge *splitEdge = seIterator.getFirst(); splitEdge;
				...
				TR::Block *newHeader = TR::Block::createEmptyBlock(entryBlock->getEntry()->getNode(),
				        comp(), splitEdge->getFrequency(), prevHeader);
				...
				if (traceMyself)
					diagnostic("Rerouting edge %d->old header %d to new header %d\n",
					        ...
					        splitEdge->getTo()->asBlock()->getNumber(), newHeader->getNumber());

				// change destination at the end of splitEdge->getFrom()->asBlock() to branch to
				// newHeader rather than entryBlock, updating edges as needed
...
void
TR_RegionStructure::collectCFGEdgesTo(int32_t toNum, List<TR::CFGEdge> *cfgEdges)
{
	ListElement<TR::CFGEdge> *edge;
	for (edge = _exitEdges.getListHead(); edge != NULL; edge = edge->getNextElement()) {
...
void
TR_RegionStructure::updateInvariantExpressions(TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
...
void
TR_BlockStructure::hoistInvariantsOutOfNestedLoops(TR_LocalTransparency *transparencyInfo, TR_BitVector **optSetInfo,
        bool insideLoop, TR_BlockStructure *outerLoopInvariantBlock, TR_RegionStructure *outerLoop,
...
void
TR_RegionStructure::hoistInvariantsOutOfNestedLoops(TR_LocalTransparency *transparencyInfo, TR_BitVector **optSetInfo,
        bool insideLoop, TR_BlockStructure *outerLoopInvariantBlock, TR_RegionStructure *outerLoop,
        int32_t bitVectorSize)
{
	bool isInvariantLoop = false;
	...
	if (isCanonicalizedLoop() && getEntry()->getSuccessors().size() == 2) {
		for (auto succ = getEntry()->getSuccessors().begin(); succ != getEntry()->getSuccessors().end();
		        ++succ) {
		...
		if (loopInvariantBlock) {
			if (loopInvariantNode->getSuccessors().size() == 1) {
				loopStructure = ((TR_StructureSubGraphNode *)loopInvariantNode->getSuccessors()
				                         .front()
				                         ->getTo())
				                        ->getStructure()
				                        ->asRegion();
		...
		if (isInvariantLoop) {
			...
			TR_RegionStructure::Cursor si(*this);
			for (node = si.getCurrent(); node != NULL; node = si.getNext())
				node->getStructure()->hoistInvariantsOutOfNestedLoops(transparencyInfo, optSetInfo,
				        true, loopInvariantBlock, loopStructure, bitVectorSize);
				...
				if (!(optSetInfo[loopInvariantBlock->getNumber()])->isEmpty()) {
					...
					while (bvi.hasMoreElements()) {
						...
						if (!(optSetInfo[outerLoopInvariantBlock->getNumber()])
							...
							if (outerLoop->isExpressionTransparentIn(
							            nextOptimalComputation, transparencyInfo)) {
								TR::Block *innerLoopInvariantBlock =
								        loopInvariantBlock->getBlock();
								TR::TreeTop *exitTree =
								        innerLoopInvariantBlock->getExit();
								TR::TreeTop *currentTree =
								        innerLoopInvariantBlock->getEntry();
								TR::TreeTop *treeToBeHoisted = NULL;
								while (currentTree != exitTree) {
								...
								if (treeToBeHoisted != NULL) {
									TR::TreeTop *prevTree =
									        treeToBeHoisted->getPrevTreeTop();
									TR::TreeTop *nextTree =
									        treeToBeHoisted->getNextTreeTop();
									prevTree->setNextTreeTop(nextTree);
									nextTree->setPrevTreeTop(prevTree);

									TR::TreeTop *outerExitTree =
									        outerLoopInvariantBlock->getBlock()
									                ->getLastRealTreeTop();

									TR::TreeTop *outerPrevTree =
									        outerExitTree->getPrevTreeTop();
									treeToBeHoisted->setNextTreeTop(outerExitTree);
									treeToBeHoisted->setPrevTreeTop(outerPrevTree);
									outerPrevTree->setNextTreeTop(treeToBeHoisted);
									outerExitTree->setPrevTreeTop(treeToBeHoisted);
								...
								dumpOptDetails(comp(),
								        ...
								        "Hoisting loop invariant computation %d over "
								        "nested loops (lifted from inner block_%d to "
								        "outer block_%d)\n",
								        nextOptimalComputation,
...
bool
TR_BlockStructure::isExpressionTransparentIn(int32_t nextOptimalComputation, TR_LocalTransparency *transparencyInfo)
{
...
bool
TR_RegionStructure::isExpressionTransparentIn(int32_t nextOptimalComputation, TR_LocalTransparency *transparencyInfo)
{
	TR_StructureSubGraphNode *node;
	TR_RegionStructure::Cursor si(*this);
	for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
