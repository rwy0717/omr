...
TR_ValueNumberInfo::congruentNodes(TR::Node *node, TR::Node *entryNode)
{
	...
	if (node->getType().isBCD()) {
		if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
			if (trace())
				traceMsg(comp(),
				        "BCD node %s (%p) and BCD entryNode %s (%p) have size/shape mismatch -- do not "
		...
		if (!node->isSignStateEquivalent(entryNode)) {
			if (trace() || comp()->cg()->traceBCDCodeGen())
				traceMsg(comp(),
				        "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- "
				        "do not consider as matching\n",
				        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
			return false;
		}
		...
		if (trace())
			traceMsg(comp(),
			        "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch -- do not "
			        "consider as matching\n",
			        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
		return false;
		...
		if (trace())
			traceMsg(comp(),
			        "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD "
			        "%d -- do not consider as matching\n",
			        node->getOpCode().getName(), node, node->castedToBCD(),
			        entryNode->getOpCode().getName(), entryNode, entryNode->castedToBCD());
		return false;
...
TR_ValueNumberInfo::initializeNode(TR::Node *node, int32_t &negativeValueNumber)
{
	...
	for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
		...
		if (node->getOpCode().isSetSignOnNode() && node->getSetSign() != entryNode->getSetSign()) {
			if (trace())
				traceMsg(comp(),
				        "Nodes %s (%p) and entryNode %s (%p) with setSignOnNode have sign mismatch -- "
				        "do not consider as matching\n",
				        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
			continue;
		}
		...
		if (node->getType().isBCD()) {
			if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
				if (trace())
					traceMsg(comp(),
					        "BCD node %s (%p) and BCD entryNode %s (%p) have size/shape mismatch "
					        "-- do not consider as matching\n",
					        node->getOpCode().getName(), node, entryNode->getOpCode().getName(),
					        entryNode);
				continue;
			}
			...
			if (!node->isSignStateEquivalent(entryNode)) {
				if (trace() || comp()->cg()->traceBCDCodeGen())
					traceMsg(comp(),
					        "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state "
			...
			if (trace())
				traceMsg(comp(),
				        "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch "
				        "-- do not consider as matching\n",
				        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
			continue;
		} else if (node->chkOpsCastedToBCD() && node->castedToBCD() != entryNode->castedToBCD()) {
			if (trace())
				traceMsg(comp(),
				        "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) "
				        "castedToBCD %d -- do not consider as matching\n",
				        node->getOpCode().getName(), node, node->castedToBCD(),
				        entryNode->getOpCode().getName(), entryNode, entryNode->castedToBCD());
			continue;
		} else if (node->getType().isDFP() && node->getOpCode().isModifyPrecision()
			...
			if (trace())
				traceMsg(comp(),
				        "DFP node %s (%p) and entryNode %s (%p) have different precisions -- do not "
				        "consider as matching\n",
				        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
			continue;
		}
	...
	if (defNode) {
		...
		if (baseVN == -1 || (defNode->getOpCode().isIndirect() && baseVN == getVN(defNode->getFirstChild()))) {
			...
			if (value >= 0) {
				...
				if (trace()) {
					traceMsg(comp(),
					        "  Change value number for load %d at [%p] to value number %d of "
					        "dominating load %d at [%p]\n",
					        node->getGlobalIndex(), node, defValue, defNode->getGlobalIndex(),
					        defNode);
				}
				return NULL;
			}
			...
			if (trace()) {
				traceMsg(comp(),
				        ...
				        getVN(defNode), defNode->getGlobalIndex(), defNode, node->getGlobalIndex(),
				        node);
			}
			return defNode;
		}
	...
	if (isNonZero && node->getOpCode().isLoadVarDirect()) {
		...
		if (defValue >= 0) {
			if (value >= 0) {
				...
				if (trace()) {
					traceMsg(comp(),
					        "  2Use value number %d of dominating load %d at [%p] for load %d at "
					        "[%p]\n",
				...
				for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
					...
					if (useNode && (useNode != node) && useNode->getOpCode().isLoadVar()) {
						...
						if (_useDefInfo->getDefiningLoads(otherUseDefiningLoads, useNode)
							...
							if (baseVN == -1
							        || (useNode->getOpCode().isIndirect()
							                && baseVN == getVN(useNode->getFirstChild()))) {
								// If this node now has a value number, we can equate
								// the value numbers of it and its dominating load
								//
								value = getVN(node);
								if (value >= 0) {
									...
									if (trace()) {
										traceMsg(comp(),
										        "  Change value number for "
										        "load %d at [%p] to value "
										        "number %d of load %d at [%p] "
										        "reached by same dominating "
										        "loads (defs)\n",
										        node->getGlobalIndex(), node,
										        useValue,
								...
								if (trace()) {
									traceMsg(comp(),
									        "  Use value number %d of dominating "
									        "load %d at [%p] for load %d at [%p]\n",
									        getVN(useNode),
...
void
TR_HashValueNumberInfo::initializeNode(TR::Node *node, int32_t &negativeValueNumber)
{
	int32_t index = node->getGlobalIndex();
	if (_nodes.ElementAt(index) != NULL) {
		// Node has already been initialized
...
TR_HashValueNumberInfo::allocateValueNumber(TR::Node *node)
{
	...
	for (i = node->getNumChildren() - 1; i >= 0; --i) {
		...
		//         else
		//            {
		//            traceMsg(comp(), "Did not recurse on node %p because getGloblalIndex =
		//            -2\n",node->getChild(i));
		//            }
	}
	...
	//
		...
		if (isValidToLookIntoHash) {
			...
			if (_nodeHash.Locate(nodeKey, loc)) {
				...
				TR_ASSERT((otherNode != NULL),
				        "HASHVN :Non-null nodeTable entry expected for globalIndex:%d", otherNodeIndex);

				//              traceMsg(comp(),"JIAG1: Setting node %p value number to same number at
				//              node %p\n",node,otherNode);
