...
TR_ExpressionsSimplification::TR_ExpressionsSimplification(TR::OptimizationManager *manager) : TR::Optimization(manager)
{}
...
void
TR_ExpressionsSimplification::simplifyInvariantLoopExpressions(ListIterator<TR::Block> &blocks)
{
	...
	if (!loopInfo) {
		...
		if (trace())
			traceMsg(comp(),
			        "Accurate loop info has been found, will try to carry out summation reduction\n");
	...
	for (TR::TreeTop *treeTop = treeTops.getFirst(); treeTop; treeTop = treeTops.getNext()) {
		...
		if (canReduceSummations) {
			usedCandidate =
			        tranformSummationReductionCandidate(treeTop, loopInfo, &isPreheaderBlockInvalid);
		}
...
void
TR_ExpressionsSimplification::setSummationReductionCandidates(TR::Node *node, TR::TreeTop *tt)
{
...
void
TR_ExpressionsSimplification::setStoreMotionCandidates(TR::Node *node, TR::TreeTop *tt)
{
        ...
        TR::TreeTop *treeTop, LoopInfo *loopInfo, bool *isPreheaderBlockInvalid)
{
	...
	if (expNode) {
		...
		if (loopInfo->getNumIterations() > 0 || // make sure that the loop is going to be executed at least once
		        _currentRegion->isCanonicalizedLoop()) // or that the loop is canonicalized, in which case the
		                                               // preheader is
...
void
TR_ExpressionsSimplification::tranformStoreMotionCandidate(TR::TreeTop *treeTop, bool *isPreheaderBlockInvalid)
{
	TR::Node *node = treeTop->getNode();
	...
	if (_currentRegion->isCanonicalizedLoop()) // make sure that the loop is canonicalized, in which case the
	                                           // preheader is
	{ // executed in its first iteration and is protected.
		if (performTransformation(comp(), "%sMove out loop-invariant store [%p] to block_%d\n", OPT_DETAILS,
		            node, preheaderBlock->getNumber())) {
			TR::Node *newNode = node->duplicateTree();
			transformNode(newNode, preheaderBlock);
			TR::TransformUtil::removeTree(comp(), treeTop);
		}
...
TR_ExpressionsSimplification::removeUnsupportedCandidates()
{
	...
	for (TR::TreeTop *candidateTT = candidateTTs.getFirst(); candidateTT; candidateTT = candidateTTs.getNext()) {
		...
		if (!_supportedExpressions->get(candidate->getGlobalIndex())) {
			if (trace())
				traceMsg(comp(),
				        "Removing candidate %p which is unsupported or has unsupported "
				        "subexpressions\n",
				        candidate);
...
TR_ExpressionsSimplification::removeCandidate(TR::Node *node, TR::TreeTop *tt)
{
	...
	for (int32_t i = 0; i < node->getNumChildren(); i++) {
		...
		// candidates child expressions must be invariant and supported. Here we determine if they are
		// supported.
...
TR_ExpressionsSimplification::removeUncertainBlocks(TR_RegionStructure *region, List<TR::Block> *candidateBlocksList)
{
	...
	for (TR::Block *block = blocks.getFirst(); block; block = blocks.getNext()) {
		...
		if (!(cfgNode->getExceptionSuccessors().empty()) || blockHasCalls(block, comp())) {
			if (trace())
				traceMsg(comp(),
				        "An exception can be thrown from block_%d. Removing all the blocks, since we "
				        "cannot know the number of iterations.\n",
				        block->getNumber());
	...
	if (postDominators.isValid()) {
		...
		for (TR::Block *block = blocks.getFirst(); block; block = blocks.getNext()) {
			if (postDominators.dominates(block, entryBlock) == 0) {
				...
				if (trace())
					traceMsg(comp(),
					        "Block_%d is not guaranteed to be executed at least once. Removing it "
					        "from the list.\n",
					        block->getNumber());
			}
		}
...
TR_ExpressionsSimplification::findLoopInfo(TR_RegionStructure *region)
{
	...
	if (!firstChildOfLastTree->getOpCode().hasSymbolReference()) {
		if (trace())
			traceMsg(comp(),
			        "The branch node's first child node %p - its opcode does not have a symbol reference\n",
