    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        if ((versionToRefineAliases || !refineAliases())
            && (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                   ...
                   || arrayStoreCheckTreesWillBeEliminated || specializedNodesWillBeEliminated
                   || invariantNodesWillBeEliminated || versionToRefineAliases || _containsGuard)) {
        ...
        for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL; nextVirtualGuard = guardsIt.getNext()) {
            ...
            if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall()
                ...
                if (!useOldLoopTransfer) {
                    ...
                    if (doIt
                        && performTransformation(comp(), "%sLoop transfer for guard %p in loop %d\n",
...
void TR_LoopVersioner::fixupVirtualGuardTargets(VirtualGuardInfo* vgInfo)
{
    ...
    for (i = 0; i < numNodes; i++)
        if (distinctTargets[i] && distinctTargets[i]->_count != 0) {
                        ...
                        if (from->getNextBlock() == coldGuardLoopEntry) {
                            if (branchNode->getOpCode().isBranch()) {
                                ...
                                if (branchNode->getBranchDestination()->getNode()->getBlock() == coldGuardLoopEntry) {
                                    ...
                                    dumpOptDetails(
                                        comp(), "created goto block_%d to invariant block\n", gotoBlock->getNumber());
                                }
                            } else {
                                TR::Node* gotoNode
                                    = TR::Node::create(branchNode, TR::Goto, 0, invariantBlock->getEntry());
            ...
            if (!loopTempSymRef) {
                ...
                // reset the selector
                //
                clonedHeader->prepend(TR::TreeTop::create(comp(),
            ...
            TR::Node* subNode = TR::Node::create(
                TR::isub, 2, TR::Node::create(node, TR::iconst, 0, distinctTargets[i]->_count), loadTemp);
            ...
            //
            cloneBlock->prepend(TR::TreeTop::create(comp(),
                TR::Node::createWithSymRef(
                    TR::istore, 1, 1, TR::Node::create(innerSwitchNode, TR::iconst, 0, 0), loopTempSymRef)));
            for (nextVG = vgIt.getFirst(); nextVG; nextVG = vgIt.getNext()) {
                if (nextVG->_isGuarded) {
    ...
    //
    TR::Node* defaultStore
        = TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(switchNode, TR::iconst, 0, 0), vgTempSymRef);
            ...
            if (nextVG->_isInsideInnerLoop) {
                ...
                for (LoopTemps* l = selectorTemps.getFirst(); l; l = l->getNext()) {
                    ...
                    TR::Node* resetNode = TR::Node::createWithSymRef(
                        TR::istore, 1, 1, TR::Node::create(hotGuardNode, TR::iconst, 0, l->_maxValue), l->_symRef);
                ...
                for (auto e = nextVG->_coldGuardLoopEntryBlock->getSuccessors().begin();
                     e != nextVG->_coldGuardLoopEntryBlock->getSuccessors().end(); ++e) {
                ...
                storeTemp = TR::Node::createWithSymRef(
                    TR::istore, 1, 1, TR::Node::create(hotGuardNode, TR::iconst, 0, ++caseKonst), vgTempSymRef);
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        while (!(currentTree == exitTree)) {
            ...
            if (!isUnimportant && !_containsCall && !refineAliases()) {
                ...
                {
                    TR_ASSERT(currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr,
                    ...
                    if (!comp()->fe()->isUnloadAssumptionRequired(
                        ...
                        if (trace()) // if we move the checkcast before the inlined body instanceof might fail because
                                     // the class might not exist in the original code
                            traceMsg(comp(),
    ...
    SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    if (_loopConditionInvariant && _asyncCheckTree) {
        ...
        if (!refineAliases() && _canPredictIters && comp()->getProfilingMode() != JitProfiling
            ...
            collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
                loopLimit, &comparisonTrees, clonedLoopInvariantBlock, comp()->incVisitCount());
            ...
            comparisonTrees.add(nextComparisonNode);
            dumpOptDetails(
                comp(), "The node %p has been created for testing if async check is required\n", nextComparisonNode);
    ...
    if (comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() > hotnessThreshold)) {
        if (!nullCheckTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
            buildNullCheckComparisonsTree(nullCheckedReferences, nullCheckTrees, boundCheckTrees, divCheckTrees,
                checkCastTrees, arrayStoreCheckTrees, &comparisonTrees, clonedLoopInvariantBlock);
        }
    }
    ...
    if (!boundCheckTrees->isEmpty()) {
        ...
        if (_loopConditionInvariant
            ...
            reverseBranch = true;
        buildBoundCheckComparisonsTree(nullCheckTrees, boundCheckTrees, divCheckTrees, checkCastTrees,
            arrayStoreCheckTrees, spineCheckTrees, &comparisonTrees, clonedLoopInvariantBlock, reverseBranch);
    }
    ...
    if (shouldOnlySpecializeLoops() && !spineCheckTrees->isEmpty()) {
        buildSpineCheckComparisonsTree(nullCheckTrees, spineCheckTrees, divCheckTrees, checkCastTrees,
    ...
    if (!divCheckTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
        buildDivCheckComparisonsTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
    ...
    //
    if (!awrtbariTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
        buildAwrtbariComparisonsTree(awrtbariTrees, nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
            &comparisonTrees, clonedLoopInvariantBlock);
    }
    ...
    if (!specializedNodes->isEmpty()) {
        ...
        bool specializedLongs
            = buildSpecializationTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
                &comparisonTrees, specializedNodes, clonedLoopInvariantBlock, invariantBlock, symRefs);
    ...
    if (!conditionalTrees->isEmpty())
        buildConditionalTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees, conditionalTrees,
            &comparisonTrees, clonedLoopInvariantBlock, reverseBranchInLoops);

    // Construct the tests for invariant expressions that need
    // to be cast.
    //
    if (!checkCastTrees->isEmpty()) {
        buildCheckCastComparisonsTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
            &comparisonTrees, clonedLoopInvariantBlock);
    ...
    if (!arrayStoreCheckTrees->isEmpty()) {
        buildArrayStoreCheckComparisonsTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
            &comparisonTrees, clonedLoopInvariantBlock);
    ...
    if (invariantNodes && !invariantNodes->isEmpty()) {
        // printf("Reached here in %s\n", comp()->signature());
        buildLoopInvariantTree(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees, &comparisonTrees,
            invariantNodes, /*invariantTranslationNodesList*/ NULL, clonedLoopInvariantBlock, invariantBlock);
        invariantNodes->deleteAll();
    }

    if (comparisonTrees.isEmpty() && _containsGuard
        && performTransformation(comp(), "%s Creating test outside loop for checking if virtual guard is required\n",
               OPT_DETAILS_LOOP_VERSIONER)) {
        ...
        comparisonTrees.add(nextComparisonNode);
        dumpOptDetails(
            comp(), "The node %p has been created for testing if virtual guard is required\n", nextComparisonNode);
    }
        ...
        for (TR::Block* block = blocksIt.getCurrent(); safeToRemoveHCRGuards && block; block = blocksIt.getNext()) {
            for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
                if (tt->getNode()->isHCRGuard()) {
                    ...
                    bool isVirtualCallForHCR
                        = (tt->getNode()->getOpCodeValue() == TR::treetop || tt->getNode()->getOpCode().isCheck())
    ...
    while (currComparisonBlock) {
        ...
        const char* debugCounter
            = TR::DebugCounter::debugCounterName(comp(), "loopVersioner.fail/(%s)/%s/origin=block_%d",
...
void TR_LoopVersioner::buildNullCheckComparisonsTree(List<TR::Node>* nullCheckedReferences,
    List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* divCheckTrees,
    ...
    TR::Block* exitGotoBlock)
{
    ...
    for (; nextNode;) {
        ...
        /// collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        /// nextNode->getData(), comparisonTrees, exitGotoBlock, visitCount);
        collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is null\n",
            ...
            // TR::Node::create(duplicateNullCheckReference, TR::aconst, 0, 0), exitGotoBlock->getEntry());
            TR::Node* nextComparisonNode = TR::Node::createif(TR::ifacmpeq, duplicateNullCheckReference,
                TR::Node::aconst(duplicateNullCheckReference, 0), exitGotoBlock->getEntry());
            comparisonTrees->add(nextComparisonNode);

            dumpOptDetails(
                comp(), "The node %p has been created for testing if null check is required\n", nextComparisonNode);
...
void TR_LoopVersioner::buildAwrtbariComparisonsTree(List<TR::TreeTop>* awrtbariTrees, List<TR::TreeTop>* nullCheckTrees,
    ...
    List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock)
{
    ...
    while (nextTree) {
        ...
        // vcount_t visitCount = comp()->incVisitCount();
        // collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        // divCheckNode->getFirstChild()->getSecondChild(), comparisonTrees, exitGotoBlock, visitCount);
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is awrtbari is required\n",
            ...
            ifNode = TR::Node::createif(TR::ificmpne, TR::Node::create(TR::iand, 2, ifNode1, ifNode2),
                TR::Node::create(duplicateBase, TR::iconst, 0, 0), exitGotoBlock->getEntry());

            comparisonTrees->add(ifNode);

            dumpOptDetails(comp(), "2 The node %p has been created for testing if awrtbari is required\n", ifNode2);

            // printf("Found opportunity for skipping wrtbar %p in %s at freq %d\n", awrtbariNode, comp()->signature(),
            // awrtbariTree->getEnclosingBlock()->getFrequency()); fflush(stdout);
...
void TR_LoopVersioner::buildDivCheckComparisonsTree(List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* divCheckTrees,
    List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* comparisonTrees,
    TR::Block* exitGotoBlock)
{
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is divide by zero\n",
            ...
            if (duplicateDivisor->getType().isInt64())
                ifNode = TR::Node::createif(TR::iflcmpeq, duplicateDivisor,
                    TR::Node::create(duplicateDivisor, TR::lconst, 0, 0), exitGotoBlock->getEntry());
            else
                ifNode = TR::Node::createif(TR::ificmpeq, duplicateDivisor,
                    TR::Node::create(duplicateDivisor, TR::iconst, 0, 0), exitGotoBlock->getEntry());
            comparisonTrees->add(ifNode);
...
void TR_LoopVersioner::buildCheckCastComparisonsTree(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees,
    List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock)
...
void TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees,
    List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock)
{
    ListElement<TR::TreeTop>* nextTree = arrayStoreCheckTrees->getListHead();
    while (nextTree) {
        ...
        visitCount = comp()->incVisitCount();
        // collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
        // arrayNode, comparisonTrees, exitGotoBlock, visitCount);
        ...
        TR::Node* ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
            TR::Node::create(arrayStoreCheckNode, TR::iconst, 0, 0), exitGotoBlock->getEntry());
        comparisonTrees->add(ificmpeqNode);
        dumpOptDetails(
            comp(), "The node %p has been created for testing if arraystorecheck is required\n", ificmpeqNode);
    ...
    TR::Block* exitGotoBlock, TR::Block* loopInvariantBlock)
{
    ...
    while (nextInvariantNode) {
        ...
        if (nextInvariantNode->getData()->_node->getReferenceCount() < 1
            ...
            if (trace())
                traceMsg(comp(),
                    "skipping node %p or its parent %p which were removed as a part of a versioned check (e.g. count < "
                    "1)\n",
        ...
        // loopInvariantBlock->getNumber(), comp()->signature());
        ...
        if (performTransformation(comp(), "%s Creating store outside the loop for loop invariant expression %p\n",
            ...
            if (comp()->useCompressedPointers()) {
                if (duplicateNode->getOpCode().isLoadIndirect() && duplicateNode->getDataType() == TR::Address
                    && TR::TransformUtil::fieldShouldBeCompressed(duplicateNode, comp())) {
                    TR::TreeTop* translateTT
                        = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(duplicateNode), NULL, NULL);
            ...
            TR::Node* storeForInvariantNode = TR::Node::createWithSymRef(
                comp()->il.opCodeForDirectStore(duplicateNode->getDataType()), 1, 1, duplicateNode, newSymbolReference);
    ...
    while (nextInvariantNode) {
        if (nextInvariantNode->getData()->_symRef) {
            ...
            if (nextInvariantNode->getData()->_node->getReferenceCount() > 0
                ...
                if (trace())
                    traceMsg(comp(),
                        "skipping node %p or its parent %p which were removed as a part of a versioned check (e.g. "
                        "count < 1)\n",
                        nextInvariantNode->getData()->_node, nextInvariantNode->getData()->_parent);
            }
        }
...
bool TR_LoopVersioner::buildSpecializationTree(List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* divCheckTrees,
    List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* comparisonTrees,
    List<TR::Node>* specializedNodes, TR::Block* exitGotoBlock, TR::Block* loopInvariantBlock,
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is value profiled\n",
            ...
            if (specializedNode->getType().isInt64()) {
                ...
                comparisonNode = TR::Node::createif(TR::iflcmpne, highInt,
                    TR::Node::create(duplicateNode, TR::lconst, 0, 0), exitGotoBlock->getEntry());
            } else {
                comparisonNode = TR::Node::createif(TR::ificmpne, duplicateNode,
                    TR::Node::create(duplicateNode, TR::iconst, 0, value), exitGotoBlock->getEntry());
            }
...
void TR_LoopVersioner::buildConditionalTree(List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* divCheckTrees,
    List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::TreeTop>* conditionalTrees,
    List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    while (nextTree) {
        ...
        // if (includeComparisonTree)
        {
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is conditional\n",
                ...
                //
                // We pulled out from both on first conditional and all the subsequent conditionals we only pull from
                // fast path This exposed a bug where slow path had an unconditional that removed a valid path. Fix is
                // only allowing unconditioning in both if there is single if conditional tree.
                //
                ...
                if (trace())
                    traceMsg(comp(),
                        "Frequency Test for conditional node [%p], destination Frequency %d, fallThrough frequency "
                        "%d\n",
                ...
                if (reverseBranchInLoops[conditionalNode->getGlobalIndex()]) {
                    if (trace())
                        traceMsg(comp(),
                            "Branch reversed for conditional node [%p], destination Frequency %d, fallThrough "
                            "frequency %d\n",
                            conditionalNode, destBlock->getFrequency(), fallThroughBlock->getFrequency());
                    reverseBranch = true;
                }
                ...
                if (conditionalNode->isVersionableIfWithMaxExpr() || conditionalNode->isVersionableIfWithMinExpr()) {
                    ...
                    if (isInductionVar
                        ...
                        TR::Node* storeNode
                            = _storeTrees[indexNode->getSymbolReference()->getReferenceNumber()]->getNode();
                        ...
                        extraIter = TR::Node::create(
                            TR::iand, 2, extraIter, TR::Node::create(parent, TR::iconst, 0, condValue));
                        ...
                        if (duplicateIndexNode->getOpCode().isLoad()
                            ...
                            replaceInductionVariable(conditionalNode, duplicateIndexNode, indexChildIndex,
                                indexSymRefNum, maxValue, visitCount);
                    }
                }
                ...
                if (replaceIndexWithExpr) {
                    ...
                    if (indexNodeOccursAsSecondChild) {
                        ...
                        if (!reverseBranch)
                            duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(),
                                duplicateIndexNode, conditionalNode->getSecondChild()->duplicateTree(comp()),
                                exitGotoBlock->getEntry());
                        else
                            duplicateComparisonNode = TR::Node::createif(
                                conditionalNode->getOpCode().getOpCodeForReverseBranch(), duplicateIndexNode,
                                conditionalNode->getSecondChild()->duplicateTree(comp()), exitGotoBlock->getEntry());
                    }
                } else {
                    if (!reverseBranch)
                        duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(),
...
void TR_LoopVersioner::buildSpineCheckComparisonsTree(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* spineCheckTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees,
    List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock)
{
    ListElement<TR::TreeTop>* nextTree = spineCheckTrees->getListHead();
    ...
    while (nextTree) {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p has spine\n",
            ...
            TR::Node* nextComparisonNode = TR::Node::createif(TR::ificmpne, contigArrayLength,
                TR::Node::create(spineCheckNode, TR::iconst, 0, 0), exitGotoBlock->getEntry());
...
void TR_LoopVersioner::buildBoundCheckComparisonsTree(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees,
    List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::TreeTop>* spineCheckTrees, List<TR::Node>* comparisonTrees,
    TR::Block* exitGotoBlock, bool reverseBranch)
{
    ListElement<TR::TreeTop>* nextTree = boundCheckTrees->getListHead();
    bool isAddition;
        ...
        if (isIndexInvariant) {
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                ...
                nextComparisonNode = TR::Node::createif(TR::ificmplt, indexNode->duplicateTreeForCodeMotion(),
                    TR::Node::create(boundCheckNode, TR::iconst, 0, 0), exitGotoBlock->getEntry());
                ...
                comparisonTrees->add(nextComparisonNode);
                dumpOptDetails(
                    comp(), "The node %p has been created for testing if exceed bounds\n", nextComparisonNode);
            }
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                ...
                if (boundCheckNode->getOpCodeValue() == TR::BNDCHK
                    ...
                    nextComparisonNode = TR::Node::createif(TR::ificmpge, indexNode->duplicateTreeForCodeMotion(),
                        arrayLengthNode->duplicateTreeForCodeMotion(), exitGotoBlock->getEntry());
                else
            ...
            if (performFirstBoundCheck && performSecondBoundCheck) {
                ...
                if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
                    ...
                    TR::TreeTop* firstNewTree = TR::TreeTop::create(comp(),
                        TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(boundChildIndex)), NULL, NULL);
                    TR::TreeTop* secondNewTree = TR::TreeTop::create(comp(),
                        TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(indexChildIndex)), NULL, NULL);
                    prevTree->join(firstNewTree);
                    firstNewTree->join(secondNewTree);
            ...
            bool performFirstBoundCheck
                = performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                    OPT_DETAILS_LOOP_VERSIONER, boundCheckNode);
            bool performSecondBoundCheck
                = performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                    OPT_DETAILS_LOOP_VERSIONER, boundCheckNode);
            ...
            if (performFirstBoundCheck) {
                if (isSpecialInductionVariable) {
                    ...
                    if ((isAddition && !indVarOccursAsSecondChildOfSub)
                        ...
                        nextComparisonNode = TR::Node::createif(TR::ificmpge, duplicateIndex,
                            boundCheckNode->getChild(boundChildIndex)->duplicateTreeForCodeMotion(),
                            exitGotoBlock->getEntry());
                ...
                comparisonTrees->add(nextComparisonNode);
                dumpOptDetails(
                    comp(), "1: The node %p has been created for testing if exceed bounds\n", nextComparisonNode);
            }
            ...
            if (performSecondBoundCheck) {
                ...
                if (isSpecialInductionVariable) {
                    ...
                    nextComparisonNode = TR::Node::createif(TR::ifiucmpgt, correctCheckNode,
                        boundCheckNode->getChild(boundChildIndex)->duplicateTree(), exitGotoBlock->getEntry());
                    if (trace())
                        traceMsg(comp(), "Special Induction variable added in each iter -> Creating %p (%s)\n",
                            nextComparisonNode, nextComparisonNode->getOpCode().getName());
                } else {
                            ...
                            if (isLoopDrivingInductionVariable) {
                                if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt)
                                    ...
                                    nextComparisonNode = TR::Node::createif(TR::ificmple, correctCheckNode,
                                        TR::Node::create(boundCheckNode, TR::iconst, 0, -1), exitGotoBlock->getEntry());
                            }
                ...
                comparisonTrees->add(nextComparisonNode);
                dumpOptDetails(
                    comp(), "2: The node %p has been created for testing if exceed bounds\n", nextComparisonNode);
                ...
                if (0 && complexButPredictableForm) {
                    ...
                    TR::Node* comparedConstNode = TR::Node::create(
                        boundCheckNode, TR::iconst, 0, additiveConstantInStore - additiveConstantInIndex);
                    nextComparisonNode
                        = TR::Node::createif(TR::ificmpge, iremNode, comparedConstNode, exitGotoBlock->getEntry());
                    ...
                    if (!isIndexChildMultiplied) {
                        ...
                        if (isAddition) {
                            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple)
                                || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))
                                nextComparisonNode = TR::Node::createif(
                                    TR::ificmpgt, firstChild, secondChild, exitGotoBlock->getEntry());
                            else
                                nextComparisonNode = TR::Node::createif(
                                    TR::ificmpge, firstChild, secondChild, exitGotoBlock->getEntry());
                            ...
                            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge)
                                || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))
                                nextComparisonNode = TR::Node::createif(
                                    TR::ificmplt, firstChild, secondChild, exitGotoBlock->getEntry());
                            else
                                nextComparisonNode = TR::Node::createif(
                                    TR::ificmple, firstChild, secondChild, exitGotoBlock->getEntry());
    ...
    TR::Node* node, List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock, vcount_t visitCount)
{
    ...
    if (node->isInternalPointer()
        ...
        if (!node->getFirstChild()->isThisPointer()
            && performTransformation(comp(), "%s Creating test outside loop for checking if %p is null\n",
            ...
            TR::Node* ifacmpeqNode = TR::Node::createif(
                TR::ifacmpeq, duplicateNullCheckReference, TR::Node::aconst(node, 0), exitGotoBlock->getEntry());
        ...
        if (node->isInternalPointer()
            ...
            if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
                ...
                if (!instanceOfReqd) {
                    ...
                    traceMsg(comp(), "failed to find class from field #%d\n", otherSymRef->getReferenceNumber());
        ...
        if (instanceOfReqd) {
            if (otherClassObject) {
                ...
                TR::Node* ificmpeqNode = TR::Node::createif(
                    TR::ificmpeq, instanceofNode, TR::Node::create(node, TR::iconst, 0, 0), exitGotoBlock->getEntry());
                comparisonTrees->add(ificmpeqNode);
                dumpOptDetails(
                    comp(), "The node %p has been created for testing if object is of the right type\n", ificmpeqNode);
                // printf("The node %p has been created for testing if object is of the right type in %s\n",
                // ificmpeqNode, comp()->signature()); fflush(stdout);
            } else if (testIsArray) {
#ifdef J9_PROJECT_SPECIFIC
                ...
                TR::Node* vftLoad = TR::Node::createWithSymRef(
                    TR::aloadi, 1, 1, duplicateCheckedValue, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
                ...
                // comp()->getSymRefTab()->findVtableEntrySymbolRef(node->getSymbolReference())), "Not enough information
                // to emit the instanceof test that is reqd\n"); dumpOptDetails(comp(), "otherClassObject is NULL in node
                // %p\n", node); printf("otherClassObject is NULL in %s\n", comp()->signature()); fflush(stdout);
            }
        }
    } else if (node->getOpCode().isIndirect() && node->getFirstChild()->isInternalPointer()
        && performTransformation(comp(), "%s Creating test outside loop for checking if %p requires bound check\n",
        ...
        if (!duplicateIndex) {
            if (!childInRequiredForm) {
                ...
                duplicateIndex = TR::Node::create(
                    type.isInt32() ? TR::isub : TR::lsub, 2, offset->duplicateTreeForCodeMotion(), constNode);
                if (type.isInt64())
        ...
        // Note: duplicateIndex was already duplicated for code motion, we don't need to duplicate the duplicate for
        // code motion
        TR::Node* ificmpltNode = TR::Node::createif(type.isInt32() ? TR::ificmplt : TR::iflcmplt,
        ...
        && performTransformation(comp(), "%s Creating test outside loop for checking if %p is divide by zero\n",
               OPT_DETAILS_LOOP_VERSIONER)) {
        ...
        if (duplicateDivisor->getType().isInt64())
            ifNode = TR::Node::createif(
                TR::iflcmpeq, duplicateDivisor, TR::Node::create(node, TR::lconst, 0, 0), exitGotoBlock->getEntry());
        else
            ifNode = TR::Node::createif(
                TR::ificmpeq, duplicateDivisor, TR::Node::create(node, TR::iconst, 0, 0), exitGotoBlock->getEntry());
        comparisonTrees->add(ifNode);
        dumpOptDetails(comp(), "The node %p has been created for testing if div check is required\n", ifNode);
        ...
        while (checkCastTree) {
            ...
            if (checkCastNode->getOpCode().isCheckCast()) //(checkCastNode->getOpCodeValue() == TR::checkcast)
            {
                if ((checkCastNode == node)
                    && performTransformation(comp(), "%s Creating test outside loop for checking if %p is casted\n",
                           OPT_DETAILS_LOOP_VERSIONER, node)) {
                    TR::Node* duplicateClassPtr = node->getSecondChild()->duplicateTreeForCodeMotion();
                    TR::Node* duplicateCheckedValue = node->getFirstChild()->duplicateTreeForCodeMotion();
                    if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
                        TR::Node* duplicateNullCheckReference = node->getFirstChild()->duplicateTreeForCodeMotion();
                        TR::Node* ifacmpeqNode = TR::Node::createif(TR::ifacmpeq, duplicateNullCheckReference,
                    ...
                    TR::Node* instanceofNode = TR::Node::createWithSymRef(
                        ...
                        comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
                    TR::Node* ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
                        TR::Node::create(node, TR::iconst, 0, 0), exitGotoBlock->getEntry());
                    comparisonTrees->add(ificmpeqNode);
                    dumpOptDetails(
                        comp(), "The node %p has been created for testing if checkcast is required\n", ificmpeqNode);
                    break;
                }
            }
