...
void TR_LoopVersioner::fixupVirtualGuardTargets(VirtualGuardInfo* vgInfo)
{
    ...
    for (i = 0; i < numNodes; i++)
        if (distinctTargets[i] && distinctTargets[i]->_count != 0) {
            ...
            if (!loopTempSymRef) {
                ...
                // reset the selector
                //
                clonedHeader->prepend(TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0, distinctTargets[i]->_count), loopTempSymRef)));
            ...
            //
            cloneBlock->prepend(TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(innerSwitchNode, TR::iconst, 0, 0), loopTempSymRef)));
            for (nextVG = vgIt.getFirst(); nextVG; nextVG = vgIt.getNext()) {
                if (nextVG->_isGuarded) {
    ...
    // reset the temp on entry to the loop
    //
    clonedHeader->prepend(TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::istore, 1, 1, TR::Node::create(switchNode, TR::iconst, 0, 0), vgTempSymRef)));
    ...
    for (nextVG = vgIt.getFirst(), caseKonst = 0; nextVG; nextVG = vgIt.getNext()) {
        ...
        if (nextVG->_isGuarded) {
            ...
            if (nextVG->_isInsideInnerLoop) {
                ...
                bool addIt = true;
                for (auto e = nextVG->_coldGuardLoopEntryBlock->getSuccessors().begin(); e != nextVG->_coldGuardLoopEntryBlock->getSuccessors().end(); ++e) {
...
bool TR_LoopVersioner::detectInvariantTrees(TR_RegionStructure* whileLoop, List<TR::TreeTop>* trees, bool onlyDetectHighlyBiasedBranches, bool* containsNonInlineGuard, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    for (; nextTree;) {
        ...
        if (includeComparisonTree) {
            ...
            if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode) && node->isTheVirtualGuardForAGuardedInlinedCall() && !node->isHCRGuard()) {
                if (!guardInfo)
                    guardInfo = comp()->findVirtualGuardInfo(node);
                //TR::Node *thisChild = NULL;
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        while (!(currentTree == exitTree)) {
            ...
            if (!isUnimportant && !_containsCall && !refineAliases()) {
                ...
                //SPECpower Work
                if (comp()->getOptions()->getGcMode() == TR_WrtbarCardMarkAndOldCheck || comp()->getOptions()->getGcMode() == TR_WrtbarOldCheck) {
...
void TR_LoopVersioner::versionNaturalLoop(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* spineCheckTrees, List<TR::TreeTop>* conditionalTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* awrtbariTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* specializedNodes, List<TR_NodeParentSymRef>* invariantNodes, List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodesList, List<TR_Structure>* innerWhileLoops, List<TR_Structure>* clonedInnerWhileLoops, bool skipVersioningAsynchk, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        bool disableLT = comp()->getOption(TR_DisableLoopTransfer);
        if (!disableLT && lastTree->getOpCode().isIf() && blocksInWhileLoop.find(lastTree->getBranchDestination()->getNode()->getBlock()) && lastTree->isTheVirtualGuardForAGuardedInlinedCall() && isBranchSuitableToDoLoopTransfer(&blocksInWhileLoop, lastTree, comp())) {
    ...
    if (_loopConditionInvariant && _asyncCheckTree) {
        ...
        if (!refineAliases() && _canPredictIters && comp()->getProfilingMode() != JitProfiling && performTransformation(comp(), "%s Creating test outside loop for deciding if async check is required\n", OPT_DETAILS_LOOP_VERSIONER)) {
    ...
    if (!arrayStoreCheckTrees->isEmpty()) {
        ...
        while (nextTree) {
            ...
            if (child->getOpCodeValue() == TR::awrtbari && comp()->getOptions()->getGcMode() == TR_WrtbarNone && performTransformation(comp(), "%sChanging awrtbari node [%p] to an iastore\n", OPT_DETAILS_LOOP_VERSIONER, child)) {
    ...
    if (comparisonTrees.isEmpty() && _containsGuard && performTransformation(comp(), "%s Creating test outside loop for checking if virtual guard is required\n", OPT_DETAILS_LOOP_VERSIONER)) {
    ...
    TR::Block* exitGotoBlock, TR::Block* loopInvariantBlock)
{
    ...
    while (nextInvariantNode) {
        ...
        if (performTransformation(comp(), "%s Creating store outside the loop for loop invariant expression %p\n", OPT_DETAILS_LOOP_VERSIONER, invariantNode)) {
            ...
            if (comp()->useCompressedPointers()) {
                if (duplicateNode->getOpCode().isLoadIndirect() && duplicateNode->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(duplicateNode, comp())) {
            ...
            for (; otherInvariantNodeElem;) {
                ...
                if (!otherInvariantNodeElem->getData()->_symRef && optimizer()->areNodesEquivalent(otherInvariantNode, invariantNode)) {
                    visitCount = comp()->incVisitCount();
    ...
    while (nextInvariantNode) {
        if (nextInvariantNode->getData()->_symRef) {
            if (nextInvariantNode->getData()->_parent && nextInvariantNode->getData()->_parent->getOpCode().isNullCheck())
                TR::Node::recreate(nextInvariantNode->getData()->_parent, TR::treetop);
