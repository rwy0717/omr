...
TR_LoopSpecializer::TR_LoopSpecializer(TR::OptimizationManager *manager) : TR_LoopVersioner(manager, true) {}
		...
		if (naturalLoop->getParent()) {
			...
			if (loopNode && (loopNode->getPredecessors().size() == 1)) {
				...
				if (loopInvariantNode->getStructure()->asBlock()
					...
					static const char *unimportantLoopCountStr = feGetEnv("TR_"
					                                                      "UnimportantLoopCountThre"
					                                                      "shold");
...
TR_LoopVersioner::performWithoutDominators()
{
	...
	for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
		...
		if (detectCanonicalizedPredictableLoops(naturalLoop, NULL, -1) < 0) // if there was a problem detecting,
		                                                                    // move to next iteration of the
		                                                                    // loop
		{
			_curLoop = NULL;
			continue;
		...
		bool nullChecksMayBeEliminated = detectChecksToBeEliminated(naturalLoop, &nullCheckedReferences,
		        &nullCheckTrees, &numIndirections, &boundCheckTrees, &spineCheckTrees, &numDimensions,
		...
		if (!shouldOnlySpecializeLoops() && !refineAliases()) {
			...
			if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
				...
				conditionalsWillBeEliminated = detectInvariantTrees(naturalLoop, &conditionalTrees,
				        true, &containsNonInlineGuard, reverseBranchInLoops);
			} else {
				conditionalsWillBeEliminated = detectInvariantTrees(naturalLoop, &conditionalTrees,
				        false, &containsNonInlineGuard, reverseBranchInLoops);
		...
		if ((versionToRefineAliases || !refineAliases())
		        && (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
		                ...
		                || spineChecksWillBeEliminated || conditionalsWillBeEliminated
		                || divChecksWillBeEliminated || awrtBarisWillBeEliminated
		                || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated
		                || specializedNodesWillBeEliminated || invariantNodesWillBeEliminated
			...
			versionNaturalLoop(naturalLoop, &nullCheckedReferences, &nullCheckTrees, &boundCheckTrees,
			        &spineCheckTrees, &conditionalTrees, &divCheckTrees, &awrtbariTrees, &checkCastTrees,
			        &arrayStoreCheckTrees, &specializedInvariantNodes, invariantNodes,
			        &invariantTranslationNodesList, &whileLoops, &clonedInnerWhileLoops,
		...
		if (versionedThisLoop) {
			...
			while (versionableInductionVar) {
				...
				// dumpOptDetails(comp(), "%s Adding auto %d as a global register candidate in loop
			...
			for (v = naturalLoop->getFirstInductionVariable(); v; v = v->getNext()) {
				...
				for (symRefNumber = symRefTab->getIndexOfFirstSymRef(); symRefNumber < symRefCount;
				        symRefNumber++) {
				...
				if (inductionSymRef && !inductionVars->get(inductionSymRef->getReferenceNumber())) {
					...
					// inductionSymRef->getReferenceNumber(), naturalLoop->getNumber(),
					// comp()->signature());
					TR_RegisterCandidate *inductionCandidate =
					        comp()->getGlobalRegisterCandidates()->findOrCreate(inductionSymRef);
					// dumpOptDetails(comp(), "%s Adding auto %d as a global register candidate in
					// loop %d\n", OPT_DETAILS_LOOP_VERSIONER,
					// inductionCandidate->getSymbolReference()->getReferenceNumber(),
					// naturalLoop->getNumber());
					inductionCandidate->addAllBlocksInStructure(
			...
			if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall()
				...
				if (comp()->getOptions()->dynamicDebugCounterIsEnabled(debugCounter)) {
					TR::Block *newBlock = hotGuardBlock->splitEdge(hotGuardBlock,
					        coldGuard->getBranchDestination()->getNode()->getBlock(), comp(), NULL,
					        false);
...
//
TR::Block *
TR_LoopVersioner::createClonedHeader(TR::Block *origHeader, TR::TreeTop **endTree)
{
	...
	for (auto e = origHeader->getExceptionSuccessors().begin(); e != origHeader->getExceptionSuccessors().end();
	        ++e) {
		removedEdges.add(*e);
		TR::Block *dest = toBlock((*e)->getTo());
...
TR::Node *
TR_LoopVersioner::createSwitchNode(TR::Block *clonedHeader, TR::SymbolReference *tempSymRef, int32_t numCase)
{
	TR::Node *switchNode =
	        TR::Node::create(clonedHeader->getFirstRealTreeTop()->getNode(), TR::lookup, numCase + 2);
...
TR::Block *
TR_LoopVersioner::createEmptyGoto(TR::Block *source, TR::Block *dest, TR::TreeTop *endTree)
{
...
bool
TR_LoopVersioner::detectInvariantChecks(List<TR::Node> *nullCheckedReferences, List<TR::TreeTop> *nullCheckTrees)
{
...
bool
TR_LoopVersioner::detectInvariantArrayStoreChecks(List<TR::TreeTop> *arrayStoreCheckTrees)
{
	bool foundInvariantChecks = false;
        ...
        List<TR_NodeParentSymRef> *invariantNodes, List<TR_NodeParentSymRefWeightTuple> *invariantTranslationNodes)
{
	...
	for (; nextNode;) {
		...
		// while not technically true, computeCC and overflow compares need to have their children intact and so
		// they can't be marked as invariant a better solution is really needed
...
bool
TR_LoopVersioner::detectInvariantTrees(TR_RegionStructure *whileLoop, List<TR::TreeTop> *trees,
				...
				if (onlyNonInlineGuardConditional) {
					...
					for (; nextTempTree != nextTree;
						...
						if (nextTempTree->getData() == onlyNonInlineGuardConditional) {
							if (prevTempTree) {
								prevTempTree->setNextElement(
								        nextTempTree->getNextElement());
							} else {
		...
		if (includeComparisonTree) {
			...
			if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode)
				...
				if (guardInfo->getKind() == TR_DummyGuard
				        || !guardInfo->getInnerAssumptions().isEmpty()) // inner assumption may have
					...
					if (!b) {
						// if (node->isNonoverriddenGuard())
						{
							if (!(thisChild->getOpCode().hasSymbolReference()
								...
								if (guardInfo->getTestType() == TR_VftTest) {
									if (node->getFirstChild()->getNumChildren()
									        == 0)
										isTreeInvariant = false;
								} else if (guardInfo->getTestType() == TR_MethodTest) {
									if ((node->getFirstChild()->getNumChildren()
						...
						if (node->isVersionableIfWithMaxExpr()
							...
							while (child->getOpCode().isAdd() || child->getOpCode().isSub()
								...
								if (child->getSecondChild()->getOpCode().isLoadConst())
									...
									bool isSecondChildInvariant = isExprInvariant(
							...
							if (!child || !child->getOpCode().isLoadVarDirect()) {
								...
								int32_t symRefNum = child->getSymbolReference()
								                            ->getReferenceNumber();
				...
				if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
					...
					if (prevTree) {
						...
						if (!_postDominators
							...
							if (trace())
								traceMsg(comp(),
								        "Keeping invariant branch  %p (%s) in "
								        "block_%d\n",
								        node, node->getOpCode().getName(),
								        nextTree->getData()
								                ->getEnclosingBlock()
...
TR::Node *
TR_LoopVersioner::isDependentOnInductionVariable(TR::Node *useNode, bool noArithmeticAllowed,
...
TR_LoopVersioner::detectInvariantBoundChecks(List<TR::TreeTop> *boundCheckTrees)
{
	...
	for (; nextTree;) {
		...
		TR::Node *boundNode = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? node->getChild(2)
		                                                                         : node->getFirstChild();
		...
		if (isMaxBoundInvariant) {
			indexChild = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? node->getChild(3)
			                                                                : node->getSecondChild();
                    ...
                    !strncmp(comp()->signature(), "banshee/scan/encoding/Latin1EncodingSupport.load", 48) */)
                  {
						if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1)
							...
							while (childInCompare->getOpCode().isAdd()
								...
								if (childInCompare->getSecondChild()
								                ->getOpCode()
								                .isLoadConst())
									childInCompare =
									        childInCompare->getFirstChild();
								else
									break;
							}

							if (childInCompare->getOpCode().hasSymbolReference()) {
								symbolInCompare = childInCompare->getSymbolReference()
								                          ->getSymbol();
							...
							if (symbolInCompare) {
								TR_InductionVariable *v;
								for (v = _currentNaturalLoop
								                 ->getFirstInductionVariable();
								        v; v = v->getNext()) {
									if ((v->getLocal() == indexSymRef->getSymbol())
									        && (v->getLocal() == symbolInCompare)
									        && (v->getLocal()->getDataType()
									                == TR::Int32)
									        && (v->isSigned())) {
					...
					if (!isInductionVariable) {
						...
						indexChild = isDependentOnInductionVariable(indexChild,
						        changedIndexSymRefAtSomePoint, isIndexChildMultiplied, mulNode,
						        strideNode, indVarOccursAsSecondChildOfSub);
		...
		if (isInductionVariable && indexSymRef) {
			if (!boundCheckUsesUnchangedValue(
			            nextTree->getData(), indexChild, indexSymRef, _currentNaturalLoop)) {
		...
		if ((node->getFirstChild()->getOpCodeValue() == TR::idiv)
			...
			if (!isDivisorInvariant) {
				...
				if (indexSymRef)
				   ...
				   int32_t symRefNum = indexSymRef->getReferenceNumber();
				   ListElement<int32_t> *versionableInductionVar =
				_versionableInductionVariables.getListHead(); while (versionableInductionVar)
				      {
				      if (symRefNum == *(versionableInductionVar->getData()))
				         {
				         isInductionVariable = true;
...
bool
TR_LoopVersioner::isDependentOnAllocation(TR::Node *useNode, int32_t recursionDepth)
{
	if (recursionDepth < 0)
		return false;
				...
				if (isBaseInvariant) {
					if (_checksInDupHeader.find(nextTree->getData())) {
						...
						if (trace())
							traceMsg(comp(), "base invariant 0 in %p\n", baseChild);
						//
						// recursionDepth cannot be changed to > 1 because isExprInvariant used
						// inside isDependentOnAllocation may not give the right answer if the
						// expr is outside the loop isExprInvariant is relevant mostly in the
						// context of a loop is my worry
						//
...
opCodeIsHoistable(TR::Node *node, TR::Compilation *comp)
{
	...
	// CAREFUL: This should probably be a whitelist of side-effect free operators. If we move effectful stuff,
	// things may explode. Boom is bad.
...
bool
TR_LoopVersioner::isExprInvariantRecursive(TR::Node *node, bool ignoreHeapificationStore)
{
...
bool
TR_LoopVersioner::hasWrtbarBeenSeen(List<TR::TreeTop> *awrtbariTrees, TR::Node *awrtbariNode)
{
	ListElement<TR::TreeTop> *nextTree = awrtbariTrees->getListHead();
	for (; nextTree;) {
		TR::Node *node = nextTree->getData()->getNode();
...
bool
TR_LoopVersioner::canPredictIters(TR_RegionStructure *whileLoop, const TR_ScratchList<TR::Block> &blocksInWhileLoop,
...
int32_t *
computeCallsiteCounts(TR_ScratchList<TR::Block> *loopBlocks, TR::Compilation *comp)
{
	int32_t inlinedCount = comp->getNumInlinedCallSites();
	int32_t *callIndexCounts =
	        (int32_t *)comp->trMemory()->allocateHeapMemory(sizeof(int32_t) * (inlinedCount + 1));
	...
	for (TR::Block *block = loopItr.getCurrent(); block; block = loopItr.getNext()) {
		for (OMR::TreeTop *tt = block->getFirstRealTreeTop(); tt != block->getExit();
		        tt = tt->getNextTreeTop()) {
	...
	// methods it inlines
	int32_t *transitiveCounts =
	        (int32_t *)comp->trMemory()->allocateHeapMemory(sizeof(int32_t) * (inlinedCount + 2));
...
bool
TR_LoopVersioner::checkProfiledGuardSuitability(TR_ScratchList<TR::Block> *loopBlocks, TR::Node *guardNode,
        TR::SymbolReference *callSymRef, TR::Compilation *comp)
{
	...
	if (comp->getMethodHotness() >= hot && callSymRef) {
		if (callSymRef->getSymbol() && callSymRef->getSymbol()->castToMethodSymbol()
...
#ifdef J9_PROJECT_SPECIFIC
		        && comp->fej9()->maybeHighlyPolymorphic(comp, callSymRef->getOwningMethod(comp),
		                callSymRef->getCPIndex(), callSymRef->getSymbol()->castToMethodSymbol()->getMethod())
#endif
		) {
			if (trace()) {
			...
			float loopCodeRatio = (float)treeTopCounts[guardNode->getInlinedSiteIndex() + 2]
			        / (float)treeTopCounts[0];
			if (trace())
				traceMsg(comp, "  Loop code ratio %d / %d = %.2f\n",
				        treeTopCounts[guardNode->getInlinedSiteIndex() + 2], treeTopCounts[0],
				        loopCodeRatio);
			if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
				if (trace())
					traceMsg(comp,
					        "Skipping versioning of profiled guard %p because we found more than 2 "
					        "JIT'd implementors at warm or above and the loop code ratio is too "
					        "low\n",
					        guardNode);
				risky = true;
				TR::DebugCounter::incStaticDebugCounter(comp,
				        TR::DebugCounter::debugCounterName(comp,
				                ...
				                "bci=%d.%d",
				                comp->signature(), guardNode->getByteCodeInfo().getCallerIndex(),
			...
			if (trace()) {
				traceMsg(comp,
				        "Found profiled guard %p is for a method subject to phase change - skipping "
				        "versioning\n",
				        guardNode);
			}
			risky = true;
        ...
        TR_ScratchList<TR::Block> *loopBlocks, TR::Node *node, TR::Compilation *comp)
{
	...
	if (node->isProfiledGuard()) {
		bool isGetFieldCacheCounts = !strncmp(comp->signature(),
		        "org/apache/solr/request/SimpleFacets.getFieldCacheCounts(Lorg/apache/solr/search/"
		        "SolrIndexSearcher;Lorg/apache/solr/search/DocSet;Ljava/lang/String;IIIZLjava/lang/"
		        "String;Ljava/lang/String;)Lorg/apache/solr/common/util/NamedList;",
		        60);
			...
			if (valueInfo) {
				...
				if (valueInfo->getTopProbability() >= profiledGuardProbabilityThreshold) {
					suitableForVersioning = checkProfiledGuardSuitability(loopBlocks, node,
					        comp->getInlinedCallerSymRef(node->getByteCodeInfo().getCallerIndex()),
					        comp);
				} else {
					suitableForVersioning = false;
				}
...
bool
TR_LoopVersioner::detectChecksToBeEliminated(TR_RegionStructure *whileLoop, List<TR::Node> *nullCheckedReferences,
        ...
        bool &discontinue)
{
	...
	for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
		...
		if (nextBlock->isCold()) {
			...
			static char *unimportantFrequencyRatioStr = feGetEnv("TR_"
			                                                     "loopVersionerControlUnimportantFrequencyR"
			                                                     "atio");
			...
			static const bool useOuterHeader = feGetEnv("TR_loopVersionerUseOuterHeaderForImportance")
			        != NULL;
			...
			if ((!aggressive || comp()->getMethodHotness() <= warm) && blockFrequency >= 0 // frequency must
			                                                                               // be valid
			        ...
			        && performTransformation(comp(),
			                ...
			                OPT_DETAILS_LOOP_VERSIONER, nextBlock->getNumber(), blockFrequency,
			                loopFrequency, hotBlock->getNumber(), whileLoop->getNumber())) {
				isUnimportant = true;
		...
		while (!(currentTree == exitTree)) {
			...
			updateDefinitionsAndCollectProfiledExprs(NULL, currentNode, visitCount,
			        specializedInvariantNodes, invariantNodes, invariantTranslationNodesList, NULL,
			...
			if ((currentNode->getNumChildren() > 0) && currentNode->getFirstChild()->getOpCode().isCall()) {
				if (!currentNode->getFirstChild()->isTheVirtualCallNodeForAGuardedInlinedCall()) {
					...
					if (!guard) {
						// printf("\t\tnode %p (nextblock = %p) is hte virtual call node for a
						// guarded inlined call without a guard.\n",currentNode,nextBlock);
						_containsUnguardedCall = true;
						_containsGuard = false;
					}
				}
			...
			if (!isUnimportant && !_containsCall && !refineAliases()) {
				if (currentOpCode.isNullCheck()) {
					...
					if (isConditionalTreeCandidateForElimination(currentTree)) {
						...
						if (_postDominators) {
							postDominatesEntry = (_postDominators->dominates(nextBlock,
					...
					if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
						...
						if (!comp()->fe()->isUnloadAssumptionRequired(
							...
							if (trace()) // if we move the checkcast before the inlined body
							             // instanceof might fail because the class might
							             // not exist in the original code
								traceMsg(comp(),
								        "Class for Checkcast %p is not loaded by the "
...
void
TR_LoopVersioner::updateDefinitionsAndCollectProfiledExprs(TR::Node *parent, TR::Node *node, vcount_t visitCount,
        List<TR::Node> *profiledNodes, List<TR_NodeParentSymRef> *invariantNodes,
        List<TR_NodeParentSymRefWeightTuple> *invariantTranslationNodesList, TR::Node *lastFault,
        bool collectProfiledExprs, TR::Block *block, int32_t warmBranchCount)
{
	...
	if (defNode) {
		...
		if (opCode.isStore()) {
			...
			if (!heapificationStore && _writtenAndNotJustForHeapification)
				defAliases.getAliasesAndUnionWith(*_writtenAndNotJustForHeapification); // TODO: avoid
				                                                                        // getting the
				                                                                        // same aliases
				                                                                        // twice
		}
			...
			if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs
				...
				if (valueInfo) {
					if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY)
					        ...
					        // Only collect nodes from unspecialized blocks to avoid specializing
					        // the same nodes twice
					        !block->isSpecialized()) {
						if ((!node->getType().isInt64())
						        || (profileLongParms && (valueInfo->getTopValue() == 0))) {

							// Zero length contiguous array lengths mean further profiling
							// is necessary to find the true array length.
							//
							if (comp()->requiresSpineChecks()
								...
								dumpOptDetails(comp(),
								        ...
								        "contigarraylength node %p with value 0 freq "
								        "%d total freq %d\n",
								        node,
								...
								/////if (debug("traceLVE"))
								dumpOptDetails(comp(),
								        "From value profiling, node %p has value %d "
								        "freq %d total freq %d\n",
	...
	int32_t i;
	for (i = 0; i < node->getNumChildren(); i++)
		updateDefinitionsAndCollectProfiledExprs(node, node->getChild(i), visitCount, profiledNodes,
		        invariantNodes, invariantTranslationNodesList, lastFault, collectProfiledExprs, block,
		        warmBranchCount);
...
void
TR_LoopVersioner::versionNaturalLoop(TR_RegionStructure *whileLoop, List<TR::Node> *nullCheckedReferences,
        ...
        SharedSparseBitVector &reverseBranchInLoops)
{
	...
	for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
		...
		if (!_neitherLoopCold || nextBlock->isCold()) {
			...
			// Set the flag on blocks in the slow version of the loop, so that in the analysis phase, we
			// don't collect these blocks
		...
		while (!(currentTree == exitTree)) {
			...
			for (int32_t childNum = 0; childNum < currentTree->getNode()->getNumChildren(); childNum++) {
				TR::Node *duplicateChildNode = duplicateExact(
				        currentTree->getNode()->getChild(childNum), &seenNodes, &duplicateNodes);
		...
		if (!disableLT && lastTree->getOpCode().isIf()
			...
			if (trace())
				traceMsg(comp(),
				        "virtualGuardPair at guard node %p hotGuardBlock %d coldGuardBlock %d\n",
				        nextBlock->getLastRealTreeTop()->getNode(), nextBlock->getNumber(),
				        nextClonedBlock->getNumber());
	...
	for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
		...
		for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
			...
			if (clonedSucc) {
				...
				if (!(lastOpCode.isBranch()
				            ...
				            || lastOpCode.isReturn() || (lastOpCode.getOpCodeValue() == TR::athrow)
				            || callWithException))
					fallsThrough = true;
		...
		for (auto edge = nextBlock->getExceptionSuccessors().begin();
			...
			if (clonedSucc) {
				_cfg->addEdge(
				        TR::CFGEdge::createExceptionEdge(nextClonedBlock, clonedSucc, trMemory()));
			} else {
	...
	if (_requiresAdditionalCheckForIncrement
	        && performTransformation(comp(),
	                "%s Creating test outside loop for checking if loop driving induction variable is incremented "
	                "by a loop invariant that is greater than 0\n",
	                OPT_DETAILS_LOOP_VERSIONER)) {
		if (_constNode->getType().isInt32()) {
			...
			TR_ASSERT(0,
			        "Unable to create test outside of loop for checking if iv update increment is greater "
			        "than 0 due to unknown type\n");
		}
	}
	...
	if (_loopConditionInvariant && _asyncCheckTree) {
		...
		if (loopLimit->getType().isInt32() && !loopLimit->getByteCodeInfo().doNotProfile()) {
			...
			if (valueInfo) {
				if (valueInfo->getTotalFrequency()) {
					dumpOptDetails(comp(),
					        "From value profiling, loop test node %p has value %d freq %d total "
					        "freq %d, and max value %d\n",
					...
					// if (shouldOnlySpecializeLoops())
					{
						...
						for (TR_ExtraValueInfo *profiledInfo = sortedValuesIt.getFirst();
						        profiledInfo != NULL; profiledInfo = sortedValuesIt.getNext()) {
						...
						if (valueInfo->getMaxValue() > profiledLoopLimit
							...
							dumpOptDetails(comp(),
							        "Maximum profiled value of %d exceeds profiled loop "
							        "limit of %d, and value profiled iterations below "
							        "limit account for only %5.3f of iterations.  Delaying "
			...
			// compjazz 108308 disallow versioning of async checks for long running GPU Loops when forced to
			// compile at scorching without jit value info.
		...
		if (!comp()->getOption(TR_DisableAsyncCheckVersioning) && !refineAliases() && _canPredictIters
		        ...
		        && performTransformation(comp(),
		                "%s Creating test outside loop for deciding if async check is required\n",
		                OPT_DETAILS_LOOP_VERSIONER)) {
	...
	if (comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() > hotnessThreshold)) {
		...
		for (ListElement<TR::TreeTop> *elt = head; elt != NULL; elt = elt->getNextElement()) {
			...
			if (!performTransformation(comp(),
			            "%sOpportunistically attempting to eliminate null check n%un [%p] using existing "
			            "prep %p\n",
	...
	if (comp()->getHCRMode() != TR::none && disableLoopHCR == NULL) {
		...
		LoopBodySearch search(
		        comp(), _curLoop->_memRegion, whileLoop, &removedNodes, &_curLoop->_takenBranches);
		...
		if (!safeToRemoveHCRGuards) {
			...
			// performTransformation() is all-or-nothing.
		                   ...
		                   comp(), "%sCreating versioned HCR guards\n", OPT_DETAILS_LOOP_VERSIONER)) {
	...
	if (comp()->getHCRMode() == TR::osr && disableLoopOSR == NULL) {
		...
		if (seenOSRGuards && safeToRemoveOSRGuards) {
			...
			// improvement.
			if (performTransformation(
			            comp(), "%sCreate versioned OSRGuard\n", OPT_DETAILS_LOOP_VERSIONER)) {
				...
				if (trace())
					traceMsg(comp(),
					        "OSRGuard n%dn has been created to guard against method invalidation\n",
	...
	// lastOptTransformationIndex can now potentially remove all the tests above the 2 versioned loops.  When there
	// are two versions of the loop, it is necessary that there be at least one test at the top.  Therefore, the
	// following is required to ensure that a test is created.
	...
	TR_ASSERT_FATAL(privatizationCount <= comparisonTreesCount,
	        "more privatizations (%d) than entries in comparisonTrees (%d)", privatizationCount,
	...
	if (testCount == 0) {
		...
		comparisonTrees.add(nextComparisonNode);
		dumpOptDetails(comp(),
		        "The comparison tree is empty.  The node %p has been created because a test is required when "
	...
	TR::DebugCounter::prependDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "loopVersioner.coldLoop/%d/(%s)/%s/bcinfo=%d.%d",
	                ...
	                comp()->getHotnessName(comp()->getMethodHotness()), bcInfo.getCallerIndex(),
	...
	while (currComparisonBlock) {
		...
		if (isTest)
			debugCounter = TR::DebugCounter::debugCounterName(comp(),
			        "loopVersioner.fail/(%s)/%s/origin=block_%d", comp()->signature(),
			        comp()->getHotnessName(comp()->getMethodHotness()), currentBlock->getNumber());

		if (nextComparisonBlock)
			_cfg->addEdge(
			        TR::CFGEdge::createEdge(currentBlock, nextComparisonBlock->getData(), trMemory()));
		else
		...
		if (isTest) {
			if (currCriticalEdgeBlock == NULL) {
				_cfg->addEdge(
				        TR::CFGEdge::createEdge(currentBlock, clonedLoopInvariantBlock, trMemory()));
			} else {
				_cfg->addEdge(TR::CFGEdge::createEdge(
				        currentBlock, currCriticalEdgeBlock->getData(), trMemory()));
				_cfg->addEdge(TR::CFGEdge::createEdge(
				        currCriticalEdgeBlock->getData(), clonedLoopInvariantBlock, trMemory()));
	...
	// as appropriate.
	//
	TR_StructureSubGraphNode **correspondingSubNodes = (TR_StructureSubGraphNode **)trMemory()->allocateStackMemory(
	        numNodes * sizeof(TR_StructureSubGraphNode *));
	memset(correspondingSubNodes, 0, numNodes * sizeof(TR_StructureSubGraphNode *));

	TR_RegionStructure *clonedWhileLoop = whileLoop
	...
	TR_StructureSubGraphNode *clonedWhileNode = new (_cfg->structureRegion())
	        TR_StructureSubGraphNode(clonedWhileLoop);
	...
	for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
		if (exitEdge->getTo()->getNumber() == invariantBlockStructure->getNumber()) {
			clonedWhileLoop->replaceExitPart(
			        invariantBlockStructure->getNumber(), properRegion->getNumber());
			break;
		}
	}
	...
	while (nextTree) {
		...
		if (performTransformation(comp(),
			...
			TR::Node *duplicateDivisor =
			        divCheckNode->getFirstChild()->getSecondChild()->duplicateTreeForCodeMotion();
			TR::Node *ifNode;
...
void
TR_LoopVersioner::buildCheckCastComparisonsTree(List<TR::TreeTop> *checkCastTrees)
{
...
TR_LoopVersioner::RemoveCheckCast::improveLoop()
{
	...
	TR::TreeTop *firstNewTree = TR::TreeTop::create(
	        comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getFirstChild()), NULL, NULL);
	TR::TreeTop *secondNewTree = TR::TreeTop::create(
	        comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getSecondChild()), NULL, NULL);
...
void
TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(List<TR::TreeTop> *arrayStoreCheckTrees)
{
...
void
TR_LoopVersioner::convertSpecializedLongsToInts(TR::Node *node, vcount_t visitCount, TR::SymbolReference **symRefs)
{
	if (node->getVisitCount() == visitCount)
		return;
...
bool
TR_LoopVersioner::buildLoopInvariantTree(List<TR_NodeParentSymRef> *invariantNodes)
{
	...
	while (nextInvariantNode) {
		...
		if (performTransformation(comp(), "%s Attempting to hoist n%un [%p] out of the loop\n",
			...
			LoopEntryPrep *prep =
			        createLoopEntryPrep(LoopEntryPrep::PRIVATIZE, invariantNode->duplicateTree());
...
bool
TR_LoopVersioner::buildSpecializationTree(List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
...
#ifdef J9_PROJECT_SPECIFIC
		if (performTransformation(comp(),
		            "%s Creating test outside loop for checking if %p is value profiled\n",
			...
			if (specializedNode->getType().isInt64()) {
				TR::Node *highInt = TR::Node::create(
				        TR::land, 2, duplicateNode, TR::Node::create(duplicateNode, TR::lconst, 0));
			...
			dumpOptDetails(comp(),
			        "The node %p has been created for testing if value profiling check is required\n",
			        comparisonNode);
			...
			if (specializedNode->getType().isInt64()) {
				if (specializedNode->getOpCode().isLoadVar()) {
					...
					TR::Node *newLoad =
					        TR::Node::createWithSymRef(specializedNode, TR::iload, 0, tempSymRef);
...
void
TR_LoopVersioner::buildConditionalTree(List<TR::TreeTop> *conditionalTrees, SharedSparseBitVector &reverseBranchInLoops)
{
	...
	while (nextTree) {
					...
					if (dupThisChild) {
						...
						if (guardInfo->getTestType() == TR_VftTest) {
							...
							conditionalNode->getFirstChild()
							        ->getFirstChild()
							        ->setAndIncChild(0, dupThisChild->duplicateTree());
						}
		...
		if (performTransformation(comp(),
			...
			//
			// We pulled out from both on first conditional and all the subsequent conditionals we only pull
			// from fast path This exposed a bug where slow path had an unconditional that removed a valid
			// path. Fix is only allowing unconditioning in both if there is single if conditional tree.
			//
			...
			if (trace())
				traceMsg(comp(),
				        "Frequency Test for conditional node [%p], destination Frequency %d, "
			...
			if (reverseBranchInLoops[origConditionalNode->getGlobalIndex()]) {
				if (trace())
					traceMsg(comp(),
					        "Branch reversed for conditional node [%p], destination Frequency %d, "
					        "fallThrough frequency %d\n",
			...
			if (conditionalNode->isVersionableIfWithMaxExpr()
				...
				if (isInductionVar
					...
					case TR::ificmpge: condValue = 1; break;
					case TR::ificmple: condValue = 1; break;
					case TR::ificmplt: incrementJ = incrementJ - 1; break;
					case TR::ificmpgt: incrementJ = incrementJ + 1; break;
					default: break;
					}
					...
					if (duplicateIndexNode->getOpCode().isLoad()
						...
						replaceInductionVariable(conditionalNode, duplicateIndexNode,
						        indexChildIndex, indexSymRefNum, maxValue, visitCount);
					}
				}
			}
				...
				if (indexNodeOccursAsSecondChild) {
					if (!reverseBranch)
						duplicateComparisonNode =
						        TR::Node::createif(conditionalNode->getOpCodeValue(),
			...
			if (duplicateComparisonNode->isMaxLoopIterationGuard()) {
				duplicateComparisonNode->setIsMaxLoopIterationGuard(false); // for the outer loop its
				                                                            // not a maxloop itr guard
				                                                            // anymore!
			}
			...
			if (prep != NULL) {
				...
				if (changeConditionalToUnconditionalInBothVersions && !prep->_requiresPrivatization) {
					...
					_curLoop->_loopImprovements.push_back(
					        new (_curLoop->_memRegion) FoldConditional(this, prep,
					                _duplicateConditionalTree, reverseBranch,
					                /* original = */ false));
				}
			}
		}
...
bool
TR_LoopVersioner::replaceInductionVariable(TR::Node *parent, TR::Node *node, int childNum, int loopDrivingInductionVar,
        TR::Node *loopLimit, int visitCount)
{
	if (node->getVisitCount() == visitCount)
		return false;
	...
	int i;
	for (i = 0; i < node->getNumChildren(); i++) {
		if (replaceInductionVariable(
		            node, node->getChild(i), i, loopDrivingInductionVar, loopLimit, visitCount))
			return true;
	}
	return false;
}
...
void
TR_LoopVersioner::buildSpineCheckComparisonsTree(List<TR::TreeTop> *spineCheckTrees)
{
	ListElement<TR::TreeTop> *nextTree = spineCheckTrees->getListHead();
...
TR_LoopVersioner::RemoveSpineCheck::improveLoop()
{
	...
	if (is64BitTarget) {
		...
		leafOffset =
		        TR::Node::create(spineCheckNode, TR::iconst, 0, (int32_t)fe()->getArrayletMask(elementSize));
	}
	...
	if (strideShiftNode) {
		nodeLeafOffset =
		        TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl, 2, nodeLeafOffset, strideShiftNode);
	}
	...
	if (comp()->useCompressedPointers()) {
		compressTree = TR::TreeTop::create(
		        comp(), TR::Node::createCompressedRefsAnchor(node->getFirstChild()), NULL, NULL);
	}
        ...
        List<TR::TreeTop> *boundCheckTrees, List<TR::TreeTop> *spineCheckTrees, bool reverseBranch)
{
	...
	while (nextTree) {
			...
			if (arrayLengthNode->getOpCode().isArrayLength()) {
				...
				if (arrayObject->getOpCode().hasSymbolReference()
					...
					if (invariantArrayObject) {
						...
						arrayLengthNode = TR::Node::create(arrayLengthNode,
						        arrayLengthNode->getOpCodeValue(), 1, dupInvariantArrayObject);
					} else
		...
		bool performBoundCheck = performTransformation(comp(),
		...
		if (isIndexInvariant) {
			...
			if (boundCheckNode->getOpCodeValue() == TR::BNDCHK
				...
				nextComparisonNode = TR::Node::createif(TR::ificmpge,
				        indexNode->duplicateTreeForCodeMotion(),
				        arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
			else
				nextComparisonNode = TR::Node::createif(TR::ificmpgt,
				        indexNode->duplicateTreeForCodeMotion(),
				        arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
						...
						while (indexChild->getOpCode().isAdd()
							...
							if (indexChild->getSecondChild()->getOpCode().isLoadConst()) {
								...
								if (isSecondChildInvariant) {
									if (isIndexChildMultiplied)
										strideNode =
										        indexChild->getSecondChild();
									indexChild = indexChild->getFirstChild();
								} else {
									bool isFirstChildInvariant = isExprInvariant(
									        indexChild->getFirstChild());
									if (isFirstChildInvariant) {
										if (indexChild->getOpCode().isSub()) {
											indVarOccursAsSecondChildOfSub =
											        !indVarOccursAsSecondChildOfSub;
			...
			TR::Node *storeNode =
			        _storeTrees[indexChild->getSymbolReference()->getReferenceNumber()]->getNode();
			...
			while (valueChild->getOpCode().isAdd() || valueChild->getOpCode().isSub()) {
				if (valueChild->getSecondChild()->getOpCode().isLoadConst()) {
					...
					//        visitCount = comp()->incVisitCount();
					//        bool isSecondChildInvariant =
					//        isExprInvariant(indexChild->getSecondChild(), visitCount); if
					//        (isSecondChildInvariant)
					//       indexChild = indexChild->getFirstChild();
					//        else
					//       {
					//           visitCount = comp()->incVisitCount();
					...
					if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
						...
						if (!storeRhs->getOpCode().isLoad()) {
							...
							replaceInductionVariable(NULL, storeRhs, -1,
							        changedIndexSymRefNumForStoreRhs, replacementLoad,
							        visitCount);
						} else
						...
						if (!duplicateIndex->getOpCode().isLoad()) {
							...
							replaceInductionVariable(NULL, duplicateIndex, -1,
							        origIndexSymRef->getReferenceNumber(), storeRhs,
							        visitCount);
						} else
			...
			if (changedIndexSymRefAtSomePoint
				...
				if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
					...
					if (!storeRhs->getOpCode().isLoad()) {
						int visitCount = comp()->incVisitCount();
						replaceInductionVariable(NULL, storeRhs, -1,
						        changedIndexSymRefNumForStoreRhs, replacementLoad, visitCount);
					} else
						storeRhs = replacementLoad;
					if (!duplicateIndex->getOpCode().isLoad()) {
						int visitCount = comp()->incVisitCount();
						replaceInductionVariable(NULL, duplicateIndex, -1,
						        origIndexSymRef->getReferenceNumber(), storeRhs, visitCount);
					} else
			...
			if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
				...
				if (isDerivedInductionVariable) {
					TR::Node *indexChild =
					        _storeTrees[loopDrivingInductionVariable]->getNode()->getFirstChild();
					...
					while (indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub()) {
						if (indexChild->getSecondChild()->getOpCode().isLoadConst()) {
							if (indexChild->getOpCode().isAdd())
								incrementI = incrementI
								        + indexChild->getSecondChild()->getInt();
							else
								incrementI = incrementI
								        - indexChild->getSecondChild()->getInt();
							indexChild = indexChild->getFirstChild();
			...
			if (isSpecialInductionVariable) {
				nextComparisonNode = TR::Node::createif(TR::ifiucmpgt,
				        boundCheckNode->getChild(indexChildIndex)->duplicateTree(),
				        correctCheckNode->duplicateTree(), _exitGotoTarget);
				if (trace())
					traceMsg(comp(),
					        "Special Induction variable added in each iter -> Creating %p (%s)\n",
					        nextComparisonNode, nextComparisonNode->getOpCode().getName());
					...
					if (!indVarOccursAsSecondChildOfSub) {
						if (isLoopDrivingInductionVariable) {
							if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple)
							        || (_loopTestTree->getNode()->getOpCodeValue()
							                == TR::ificmpgt))
								nextComparisonNode = TR::Node::createif(TR::ifiucmpge,
								        correctCheckNode,
						...
						if (isLoopDrivingInductionVariable) {
							if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt)
							        || (_loopTestTree->getNode()->getOpCodeValue()
							                == TR::ificmpge))
								nextComparisonNode = TR::Node::createif(TR::ificmple,
								        correctCheckNode,
								        TR::Node::create(
								                boundCheckNode, TR::iconst, 0, -1),
								        _exitGotoTarget);
						}
				...
				if (changedIndexSymRefAtSomePoint
					...
					if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
						...
						if (!storeRhs->getOpCode().isLoad()) {
							int visitCount = comp()->incVisitCount();
							replaceInductionVariable(NULL, storeRhs, -1,
							        origIndexSymRef->getReferenceNumber(), replacementLoad,
							        visitCount);
						} else
							storeRhs = replacementLoad;
				...
				if (!isIndexChildMultiplied) {
					...
					if ((isAddition && !indVarOccursAsSecondChildOfSub)
						...
						replaceInductionVariable(NULL, duplicateMulNode, -1,
						        indexSymRef->getReferenceNumber(), loopLimit->duplicateTree(),
						        visitCount);
						visitCount = comp()->incVisitCount();
					...
					// Adding multiplicative factor greater than zero check for multiplicative
					// BNDCHKS a.i+b; a>0
					nextComparisonNode = TR::Node::createif(TR::ificmple,
...
void
TR_LoopVersioner::createRemoveBoundCheck(
        TR::TreeTop *boundCheckTree, LoopEntryPrep *prep, List<TR::TreeTop> *spineCheckTrees)
{
	...
	if (op == TR::BNDCHK || op == TR::ArrayCopyBNDCHK) {
		...
		auto insertResult =
		        _curLoop->_boundCheckPrepsWithSpineChecks.insert(std::make_pair(boundCheckNode, prep));
...
TR_LoopVersioner::RemoveBoundCheck::improveLoop()
{
	...
	if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
		...
		TR::TreeTop *firstNewTree = TR::TreeTop::create(
		        comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(0)), NULL, NULL);
		TR::TreeTop *secondNewTree = TR::TreeTop::create(
		        comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(1)), NULL, NULL);
		prevTree->join(firstNewTree);
		firstNewTree->join(secondNewTree);
 ...
 */
void
TR_LoopVersioner::collectAllExpressionsToBeChecked(List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
        List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees, TR::Node *node,
        List<TR::Node> *comparisonTrees, TR::Block *exitGotoBlock, vcount_t visitCount)
{
 ...
 */
void
TR_LoopVersioner::collectAllExpressionsToBeChecked(TR::Node *node, List<TR::Node> *comparisonTrees)
{
...
TR_LoopVersioner::requiresPrivatization(TR::Node *node)
{
	...
	case TR::SymbolReferenceTable::classFromJavaLangClassSymbol: return false;
 ...
 */
bool
TR_LoopVersioner::suppressInvarianceAndPrivatization(TR::SymbolReference *symRef)
{
	...
	case TR::SymbolReferenceTable::excpSymbol: return true;

	// Class flags are mutable.
	case TR::SymbolReferenceTable::isClassAndDepthFlagsSymbol: return true;

#ifdef J9_PROJECT_SPECIFIC
	// These may be updated by the garbage collector at GC points.
	case TR::SymbolReferenceTable::lowTenureAddressSymbol:
	case TR::SymbolReferenceTable::highTenureAddressSymbol: return true;
...
void
TR_LoopVersioner::dumpOptDetailsCreatingTest(const char *description, TR::Node *node)
{
	dumpOptDetails(comp(), "Creating %s test for n%un [%p]\n", description, node->getGlobalIndex(), node);
}

void
TR_LoopVersioner::dumpOptDetailsFailedToCreateTest(const char *description, TR::Node *node)
{
 ...
 */
bool
TR_LoopVersioner::depsForLoopEntryPrep(TR::Node *node, TR::list<LoopEntryPrep *, TR::Region &> *deps,
	...
	if (node->isInternalPointer()
		...
		if (!node->getFirstChild()->isThisPointer()) {
			...
			TR::Node *ifacmpeqNode = TR::Node::createif(
			        TR::ifacmpeq, node->getFirstChild(), TR::Node::aconst(node, 0), _exitGotoTarget);
		...
		if (instanceOfReqd) {
			if (otherClassObject) {
				...
				if (TR::Compiler->target.is32Bit()) {
					classFlag = TR::Node::createWithSymRef(TR::iloadi, 1, 1, vftLoad,
					        comp()->getSymRefTab()->findOrCreateClassAndDepthFlagsSymbolRef());
				} else {
					classFlag = TR::Node::createWithSymRef(TR::lloadi, 1, 1, vftLoad,
					        comp()->getSymRefTab()->findOrCreateClassAndDepthFlagsSymbolRef());
					classFlag = TR::Node::create(TR::l2i, 1, classFlag);
				}
				TR::Node *andConstNode = TR::Node::create(
				        classFlag, TR::iconst, 0, TR::Compiler->cls.flagValueForArrayCheck(comp()));
				TR::Node *andNode = TR::Node::create(TR::iand, 2, classFlag, andConstNode);
				TR::Node *cmp =
				        TR::Node::createif(TR::ificmpne, andNode, andConstNode, _exitGotoTarget);
		...
		if (!indexNode) {
			if (!childInRequiredForm) {
				TR::Node *constNode =
				        TR::Node::create(node, type.isInt32() ? TR::iconst : TR::lconst, 0, 0);
				indexNode =
				        TR::Node::create(type.isInt32() ? TR::isub : TR::lsub, 2, offset, constNode);
				if (type.isInt64())
		...
		TR::Node *ificmpltNode = TR::Node::createif(
		        type.isInt32() ? TR::ificmplt : TR::iflcmplt, indexNode, constNode, _exitGotoTarget);
		if (type.isInt64())
		...
		if (!divisorIsNonzeroConstant) {
			...
			if (divisor->getType().isInt64())
				ifNode = TR::Node::createif(TR::iflcmpeq, divisor,
				        TR::Node::create(node, TR::lconst, 0, 0), _exitGotoTarget);
			else
...
TR_LoopVersioner::addLoopEntryPrepDep(LoopEntryPrep::Kind kind, TR::Node *node,
...
TR::Node *
TR_LoopVersioner::findLoad(TR::Node *node, TR::SymbolReference *symRef, vcount_t origVisitCount)
{
...
bool
TR_LoopVersioner::findStore(TR::TreeTop *start, TR::TreeTop *end, TR::Node *node, TR::SymbolReference *symRef,
        ...
        TR::TreeTop *bndCheckTree, TR::Node *node, TR::SymbolReference *symRef, TR_RegionStructure *loopStructure)
{
	...
	while (flag) {
		...
		for (auto pred = cursorBlock->getPredecessors().begin(); pred != cursorBlock->getPredecessors().end();
		        ++pred) {
        ...
        TR_Structure *loopStructure, TR_BitVector **optSetInfo, int32_t bitVectorSize)
{
	...
	if (loopInvariantBlock) {
		/////if (isInvariantLoop)
		{
			...
			//_writtenAndNotJustForHeapification = new (trStackMemory()) TR_BitVector(symRefCount,
			// trMemory(), stackAlloc);
			...
			int32_t isPredictableLoop =
			        checkLoopForPredictability(loopStructure, loopInvariantBlock->getBlock(), NULL);
			...
			if ((isPredictableLoop > 0)
					...
					if (storeInRequiredForm /* && !_cannotBeEliminated->get(nextInductionVariableNumber) */)
                   {
						...
						*versionableInductionVariable = nextInductionVariableNumber;
...

						{
							...
							_versionableInductionVariables.add(
							        versionableInductionVariable);
							...
							if (trace())
								traceMsg(comp(),
								        "Version loop : %d with respect to induction "
								        "variable %d\n",
								        loopStructure->getNumber(),
								        nextInductionVariableNumber);
						}
						...
						*versionableInductionVariable = nextInductionVariableNumber;
						...
						if (trace())
							traceMsg(comp(),
							        "Version loop : %d with respect to induction variable "
							        "%d\n",
							        loopStructure->getNumber(),
							        nextInductionVariableNumber);
					}
				...
				// clearing the derived inductionVariables when primary induction variable had a
				// non-const increment.
...
bool
TR_LoopVersioner::isStoreInSpecialForm(int32_t symRefNum, TR_Structure *loopStructure)
{
...
bool
TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum, TR_Structure *loopStructure)
{
...
void
TR_LoopVersioner::collectArrayAliasCandidates(TR::Node *node, vcount_t visitCount)
{
	TR_ASSERT(!refineAliases(), "something wrong");
}
void
TR_LoopVersioner::buildAliasRefinementComparisonTrees(List<TR::TreeTop> *nullCheckTrees,
...
void
TR_LoopVersioner::findAndReplaceContigArrayLen(TR::Node *parent, TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;

	if ((node->getOpCodeValue() == TR::contigarraylength)) // future: || (node->getOpCodeValue() ==
 ...
 */
bool
TR_LoopVersioner::initExprFromNode(Expr *expr, TR::Node *node, bool onlySearching)
{
...
TR_LoopVersioner::guardOkForExpr(TR::Node *node, bool onlySearching)
{
	...
	case TR_AbstractGuard: return test == TR_MethodTest;

	case TR_HierarchyGuard: return test == TR_VftTest || test == TR_MethodTest;

	case TR_NonoverriddenGuard: return test == TR_NonoverriddenTest || test == TR_VftTest;

	case TR_DirectMethodGuard: return test == TR_NonoverriddenTest;
	...
	case TR_RemovedInterfaceGuard: return false;
	...
	if (!initExprFromNode(&expr, node, onlySearching)) {
		...
		if (shouldAssert) {
			if (node->isNopableInlineGuard()) {
				TR_VirtualGuard *guard = comp()->findVirtualGuardInfo(node);
				TR_ASSERT_FATAL(false, "n%un [%p] is unrepresentable guard kind=%d, test=%d",
				        node->getGlobalIndex(), node, (int)guard->getKind(), (int)guard->getTestType());
			} else {
 ...
 */
const TR_LoopVersioner::Expr *
TR_LoopVersioner::substitutePrivTemps(TR::TreeTop *tt, TR::Node *node, TR::NodeChecklist *visited)
{
	if (visited->contains(node)) {
		auto cached = _curLoop->_nodeToExpr.find(node);
		if (cached == _curLoop->_nodeToExpr.end())
...
TR_LoopVersioner::emitPrep(LoopEntryPrep *prep, List<TR::Node> *comparisonTrees)
{
	...
	if (prep->_kind == LoopEntryPrep::TEST) {
		...
		//
		TR_ASSERT_FATAL(
		        !value->isInternalPointer(), "prep %p attempting to privatize an internal pointer", prep);
		...
		TR::SymbolReference *temp =
		        comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), tempType);
		...
		dumpOptDetails(comp(), "Emitted prep %p as n%un [%p] storing to temp #%d\n", prep,
		        store->getGlobalIndex(), store, temp->getReferenceNumber());
	}
}
        ...
        const TR::list<LoopEntryPrep *, TR::Region &> &preps, List<TR::Node> *comparisonTrees)
{
	for (auto it = preps.begin(); it != preps.end(); ++it) {
		...
		if (prep->_kind == LoopEntryPrep::TEST) {
			...
			dumpOptDetails(
			        comp(), "Unsafely emitted prep %p as n%un [%p]\n", prep, node->getGlobalIndex(), node);
 ...
 */
void
TR_LoopVersioner::nodeWillBeRemovedIfPossible(TR::Node *node, LoopEntryPrep *prep)
{
 ...
 */
TR_LoopVersioner::LoopEntryPrep *
TR_LoopVersioner::createLoopEntryPrep(
        LoopEntryPrep::Kind kind, TR::Node *node, TR::NodeChecklist *visited, LoopEntryPrep *prev)
{
	...
	if (optDetails) {
		dumpOptDetails(comp(), "Prep for n%un [%p] is prep %p %s expr %p%s, deps: ", node->getGlobalIndex(),
		        node, prep, prep->_kind == LoopEntryPrep::PRIVATIZE ? "PRIVATIZE" : "TEST", prep->_expr,
 ...
 */
TR_LoopVersioner::LoopEntryPrep *
TR_LoopVersioner::createChainedLoopEntryPrep(LoopEntryPrep::Kind kind, TR::Node *node, LoopEntryPrep *prev)
{
	if (prev == NULL)
		return NULL;
...
void
TR_LoopVersioner::LoopBodySearch::enqueueReachableSuccessorsInLoopFrom(TR::CFGEdgeList &outgoingEdges)
{
