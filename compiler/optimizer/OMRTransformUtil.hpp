...
public:
	...
	static TR::Node *scalarizeArrayCopy(TR::Compilation *comp, TR::Node *node, TR::TreeTop *tt, bool useElementType,
	        bool &didTransformArrayCopy, TR::SymbolReference *sourceRef = NULL,
	        TR::SymbolReference *targetRef = NULL, bool castToIntegral = true);

	static TR::Node *scalarizeAddressParameter(TR::Compilation *comp, TR::Node *address,
	        size_t byteLengthOrPrecision, TR::DataType dataType, TR::SymbolReference *ref, bool store);
	...
	/**
	 *    \brief
	 *       Create a conditional alternate implementation path for existing operations. Two blocks will be
	 * inserted, one block with comparison tree (compareBlock) and one block to take when comparison succeeds
	 * (ifBlock). The resulting CFG will look like
	 *
	 *          elseBlock               ifBlock
	 *              |                   /     \
	 *             ...      ===>       /       \
	 ...
	 *   \note
	 *      After transformation, the original elseBlock will contain only one tree, that is the ifTree. Its
	 * original trees will be in a newly created block which is its fall through block after the transformation.
	 */
