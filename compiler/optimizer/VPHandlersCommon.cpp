...
constrainVcall(OMR::ValuePropagation *vp, TR::Node *node)
{
	...
	if (node->getSymbolReference() == finalizeSymRef) {
		...
		// by virtue of (indirectly) calling bool J9::ObjectModel::canAllocateInlineClass(TR_OpaqueClassBlock
		// *block) Doesn't make sense to call jitCheckIfFinalizeObject for a stack allocated object, so optimize
		...
		if (canBeRemoved
			...
			/// vp->comp()->signature());fflush(stdout); traceMsg(vp->comp(), "found opportunity to remove
			/// call %p to checkfinalize\n", node);
