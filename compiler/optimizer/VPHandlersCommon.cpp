...
TR::TreeTop* searchForToStringCall(OMR::ValuePropagation* vp, TR::TreeTop* tt, TR::TreeTop* exitTree,
    TR::Node* newBuffer, vcount_t visitCount, TR::TreeTop** toStringTree, bool useStringBuffer)
{
                ...
                if (call->getFirstChild() == newBuffer) {
                    if (vp->getConstraint(call->getSecondChild(), isGlobal)) {
                        if (vp->getConstraint(call->getSecondChild(), isGlobal)->getClassType()) {
                            const char* sigAppend = vp->getConstraint(call->getSecondChild(), isGlobal)
                                                        ->getClassType()
...
static int cacheStringAppend(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (checkMethodSignature(vp, node->getSymbolReference(), sigInit)) {
        ...
        if (strncmp(m->signatureChars(), "(Ljava/lang/String;Ljava/lang/String;)V", m->signatureLength()) == 0) {
            vp->_cachedStringPeepHolesVcalls.add(
                new (vp->comp()->trStackMemory()) OMR::ValuePropagation::VPTreeTopPair(tt, tt->getPrevRealTreeTop()));
        }
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    while (1) {
        ...
        int visitCount = 0;
        if (useStringBuffer)
            tt = searchForStringAppend(vp, "java/lang/StringBuffer.append(", startTree, exitTree, TR::acall, child,
                visitCount, appendedString + stringCount);
        else
            tt = searchForStringAppend(vp, "java/lang/StringBuilder.append(", startTree, exitTree, TR::acall, child,
                visitCount, appendedString + stringCount);
    ...
    vp->_cachedStringBufferVcalls.add(new (vp->comp()->trStackMemory()) OMR::ValuePropagation::VPStringCached(
        appendTree[0], appendTree[1], appendedString[0], appendedString[1], newTree, toStringTree));
}
