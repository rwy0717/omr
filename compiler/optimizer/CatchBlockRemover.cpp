...
TR_CatchBlockRemover::TR_CatchBlockRemover(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...

	{
		...
		for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
			...
			for (treeTop = block->getEntry(); treeTop != block->getExit();
				...
				if (treeTop->getNode()->getOpCodeValue() == TR::monexitfence) // for live monitor
				                                                              // metadata
			...
			for (auto edge = block->getExceptionSuccessors().begin();
				...
				if (!reachedExceptions
				        && performTransformation(comp(),
				                "%sRemove redundant exception edge from block_%d at [%p] to catch "
				                "block_%d at [%p]\n",
				                optDetailString(), block->getNumber(), block, catchBlock->getNumber(),
				                catchBlock)) {
