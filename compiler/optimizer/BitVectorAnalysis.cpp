...
template <class Container>
bool
TR_BasicDFSetAnalysis<Container *>::performAnalysis(TR_Structure *rootStructure, bool checkForChanges)
{
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::allocateContainer(Container **result, bool, bool)
{
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::allocateBlockInfoContainer(Container **result, bool, bool)
{
	*result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::allocateBlockInfoContainer(Container **result, Container *)
{
	*result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::allocateTempContainer(Container **result, Container *)
{
	*result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
TR_BasicDFSetAnalysis<Container *>::initializeBasicDFSetAnalysis()
{
	...
	if (supportsGenAndKillSets()) {
		...
		if (!_hasImproperRegion) {
			...
			if (traceBVA())
				dumpOptDetails(comp(),
				        "\n ************** Completed initialization of gen and kill sets for all "
				        "structures ************* \n");
		}
	} else {
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForStructure(TR_Structure *s)
{
...
template <class Container>
void
TR_BasicDFSetAnalysis<Container *>::initializeGenAndKillSetInfoPropertyForStructure(TR_Structure *s, bool inLoop)
{
...
template <class Container>
bool
TR_ForwardDFSetAnalysis<Container *>::canGenAndKillForStructure(TR_Structure *nodeStructure)
{
...
template <class Container>
void
TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure *region)
{
	...
	if (region->isNaturalLoop()) {
		...
		typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair = new (
		        this->trStackMemory())
	...
	if (region->isNaturalLoop()) {
		...
		Container *killBitVector =
		        analysisInfo->getContainer(analysisInfo->_regularKillSetInfo, region->getNumber());
		Container *genBitVector =
		        analysisInfo->getContainer(analysisInfo->_regularGenSetInfo, region->getNumber());
	...
	while (this->_analysisQueue.getListHead()
		...
		if (this->traceBVA())
			traceMsg(this->comp(), "Gen : Begin analyzing node %p numbered %d in region %p (%d)\n",
			        nodeStructure->getStructure(), node->getNumber(), regionStructure,
		...
		if (node != regionStructure->getEntry()) {
			...
			for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
				        ...
				        && !predStructure->hasBeenAnalyzedBefore()) //_firstIteration)
				{
					...
					int32_t nodeNumber = predStructure->asRegion() ? node->getNumber()
					                                               : predNode->getNumber();
					TR_LinkHead<
					        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>
					        *predCurrentGenSetInfo = normalPred
					        ? predInfo->_currentRegularGenSetInfo
				...
				if (this->traceBVA()) {
					dumpOptDetails(this->comp(), "Node %p (%d) pred %p (%d)\n",
					        ...
					        nodeStructure->getStructure()->getNumber(), predStructure,
		...
		*this->_temp2 = *_currentRegularKillSetInfo;
		...
		for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
			...
			*_currentRegularKillSetInfo = *this->_temp2;
			...
			int32_t nodeNumber = nodeStructure->getStructure()->asRegion() ? succNode->getNumber()
			                                                               : node->getNumber();
			...
			if (regionStructure->isExitEdge(succ) || (succNode == regionStructure->getEntry())) {
				...
				if (!pair) {
					...
					TR_ASSERT(
					        (pair->_nodeNumber == node->getNumber()), "Node number inconsistent\n");
				}
				...
				TR_LinkHead<typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>
				        *_currentGenSetInfo = normalSucc ? nodeInfo->_currentRegularGenSetInfo
				                                         : nodeInfo->_currentExceptionGenSetInfo,
				        *_currentKillSetInfo = normalSucc ? nodeInfo->_currentRegularKillSetInfo
				                                          : nodeInfo->_currentExceptionKillSetInfo;
				if (!_currentGenSetInfo)
					TR_ASSERT(
					        0, "Gen set info should have been allocated in initialization step\n");
...
template <class Container>
void
TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForBlock(TR_BlockStructure *s)
{
...
{
	typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo = new (this->trStackMemory())
	        typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo;
...
template <class Container>
bool
TR_ForwardDFSetAnalysis<Container *>::analyzeRegionStructure(TR_RegionStructure *regionStructure, bool checkForChange)
{
	...
	if (regionStructure != this->_cfg->getStructure()) {
		...
		for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
			...
			Container *fromBitVector =
			        fromAnalysisInfo->getContainer(fromAnalysisInfo->_outSetInfo, toStructureNumber);
        ...
        TR_RegionStructure *regionStructure, TR_BitVector &pendingList)
{
	...
	while (this->_analysisQueue.getListHead()
			...
			for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
				...
				if (bitVector) {
					*_info -= *bitVector;
					if (this->traceBVA()) {
						traceMsg(this->comp(),
						        "\n%sKill Set Info for Region or Block : %p numbered %d and "
						        "exit %d is : \n",
				...
				if (bitVector) {
					*_info |= *bitVector;
					if (this->traceBVA()) {
						traceMsg(this->comp(),
						        "\n%sGen Set Info for Region or Block : %p numbered %d and "
						        "exit %d is : \n",
						        (normalSucc ? "" : "E"), nodeStructure->getStructure(),
						        nodeStructure->getStructure()->getNumber(),
						        succNode->getNumber());
						bitVector->print(this->comp());
						traceMsg(this->comp(),
						        "\n1%s Info for Region or Block : %p numbered %d and exit %d "
						        "is : \n",
						        (normalSucc ? "Normal" : "Exception"),
						        nodeStructure->getStructure(),
						        nodeStructure->getStructure()->getNumber(),
				...
				bitVector =
				        analysisInfo->getContainer(analysisInfo->_outSetInfo, succNode->getNumber());
				// Don't copy vectors if they are the same since comparison is much cheaper and can save
				// compile time
				if (*_info != *bitVector) {
		...
		for (auto succ = node->getExceptionSuccessors().begin(); succ != node->getExceptionSuccessors().end();
		        ++succ) {
			TR::CFGNode *succNode = (*succ)->getTo();
			if ((!regionStructure->isExitEdge(*succ))
			        && (needToIterate || (succNode != regionStructure->getEntry()))
...
template <class Container>
bool
TR_ForwardDFSetAnalysis<Container *>::analyzeBlockStructure(TR_BlockStructure *blockStructure, bool checkForChange)
{
	if (this->supportsGenAndKillSets() && canGenAndKillForStructure(blockStructure)) {
		...
		if (!this->_blockAnalysisInfo[blockStructure->getNumber()])
			this->allocateBlockInfoContainer(
			        &this->_blockAnalysisInfo[blockStructure->getNumber()], _currentInSetInfo);
		...
		if (blockStructure->getNumber() == 0) {
			...
			for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
				outSetInfo = analysisInfo->getContainer(
	...
	if (blockStructure->getNumber() == 0) {
		...
		if (this->_regularGenSetInfo) {
			...
			this->copyFromInto(
			        analysisInfo->_inSetInfo, this->_blockAnalysisInfo[blockStructure->getNumber()]);
		} else {
	...
	for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
		...
		Container *outSetInfo =
		        analysisInfo->getContainer(analysisInfo->_outSetInfo, succ->getTo()->getNumber());
		Container *_info = normalSucc ? this->_regularInfo : this->_exceptionInfo;
...
template <class Container>
void
TR_ForwardDFSetAnalysis<Container *>::analyzeBlockZeroStructure(TR_BlockStructure *blockStructure)
{
	analyzeTreeTopsInBlockStructure(blockStructure);
}
...
template <class Container>
void
TR_ForwardDFSetAnalysis<Container *>::analyzeTreeTopsInBlockStructure(TR_BlockStructure *blockStructure)
{
	TR::Block *block = blockStructure->getBlock();
