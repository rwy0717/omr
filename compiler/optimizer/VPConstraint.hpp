...
protected:
	...
	type_t subWithOverflow(type_t a, type_t b, bool &overflow)
	{
		...
		// The overflow flag is set when the arithmetic used to compute the difference overflows. This happens
		// exactly when both operand bounds have a differing sign and the resulting value has the same sign as
		// the first operand. A non-negative result from xoring two numbers indicates that their sign bits are
		// either both set, or both unset, meaning they have the same sign, while a negative result from xoring
		// two numbers means that exactly one of them has its sign bit set, meaning the signs of the two numbers
		// differ.
...
public:
	...
	static TR::VPIntConstraint *create(OMR::ValuePropagation *vp, TR::DataTypes dt,
	        TR_YesNoMaybe isUnsigned); // Takes a TR::DataTypes instead of TR::DataType to work around ambiguous
	                                   // overloads
...
public:
	...
	static TR::VPLongConstraint *create(OMR::ValuePropagation *vp, int64_t low, int64_t high,
	        bool powerOfTwo = false, TR_YesNoMaybe canOverflow = TR_no);
...
public:
	...
	static TR::VPClassType *create(OMR::ValuePropagation *vp, const char *sig, int32_t len,
	        TR_ResolvedMethod *method, bool isFixed, TR_OpaqueClassBlock *j9class = 0);
...
public:
	VPPreexistentObject(TR_OpaqueClassBlock *c = NULL) : TR::VPConstraint(ClassPreexistencePriority)
	{
		_assumptionClass = c;
	}
...
public:
	...
	virtual TR::VPConstraint *propagateRelativeConstraint(
	        TR::VPRelation *other, int32_t relative, int32_t otherRelative, OMR::ValuePropagation *vp) = 0;
...
public:
	VPLessThanOrEqual(int32_t incr) : TR::VPRelation(incr, LessThanOrEqualPriority) {}
...
public:
	VPGreaterThanOrEqual(int32_t incr) : TR::VPRelation(incr, GreaterThanOrEqualPriority) {}
