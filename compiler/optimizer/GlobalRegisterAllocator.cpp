    ...
    TR_BitVector* symsThatShouldNotBeAssignedInCurrentLoop, bool hasCatchBlock)
{
    ...
    if (enableNewSignExtGRA) {
        ...
        if (!canSkip
                && ((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub)
                       || (node->getOpCodeValue() == TR::iand) || (node->getOpCodeValue() == TR::iloadi)
                ...
                if (TR::Compiler->target.is64Bit() && !_rejectedSignExtNodes.find(node)) {
                    if ((node->getOpCodeValue() == TR::iload)
                        ...
                        TR::TreeTop* cursorTree
                            = _candidates->_startOfExtendedBBForBB[block->getNumber()]->getEntry()->getNextTreeTop();
                        while (cursorTree) {
                ...
                if (comp()->getFlowGraph()->getStructure() && blockStructure) {
                    ...
                    if (containingLoop) {
                        ...
                        if (loopInvariantBlock) {
                            ...
                            if (_candidatesSignExtendedInThisLoop->get(storeSymRef->getReferenceNumber())) {
                                ...
                                signExtStore = TR::Node::createWithSymRef(
                                    ...
                                    TR::Node::createWithSymRef(node,
                                        comp()->il.opCodeForDirectLoad(storeSymRef->getSymbol()->getDataType()), 0,
                                        storeSymRef),
                                    storeSymRef);
                            ...
                            TR_ASSERT(
                                signExtStore, "Store for sign extension must be found or created in loop pre-header\n");
                            ...
                            TR_NodeTriple* nodeTriple = new (comp()->trHeapMemory())
                                TR_NodeTriple(parent, signExtStore, (loadNode ? parent : storeNode), childNum);
            ...
            if (TR::Compiler->target.is64Bit() && info
                ...
                if (info->getUseDef(defs, useIndex)) {
                    ...
                    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                        ...
                        if (defIndex
                            < info->getFirstRealDefIndex()) // def is  unseen--can't mark this node as skippable
                        {
        ...
        if (TR::Compiler->target.is64Bit() && parent && (parent->getOpCodeValue() == TR::i2l)
            && ((*currentArrayAccess != parent) && (grandParent->getOpCodeValue() != TR::l2i))
        ...
        if (!needStoreAndItExists
            ...
            if (*currentArrayAccess) {
                if (TR::Compiler->target.is64Bit()
                    && (parent->getOpCodeValue() == TR::i2l) /* && node->isNonNegative() */
                    && (*currentArrayAccess == parent) && !_rejectedSignExtNodes.find(node)) {
                    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
                        "Incorrect value in _startOfExtendedBBForBB");
                    TR::TreeTop* cursorTree
                        = _candidates->_startOfExtendedBBForBB[block->getNumber()]->getEntry()->getNextTreeTop();
                            ...
                            if (containingLoop) {
                                ...
                                if (loopInvariantBlock) {
                                    ...
                                    TR::Node* signExtStore = TR::Node::createWithSymRef(
                                        comp()->il.opCodeForDirectStore(node->getDataType()), 1, 1, node, _storeSymRef);
