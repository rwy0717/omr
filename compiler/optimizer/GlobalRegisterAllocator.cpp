    ...
    bool hasCatchBlock)
{
    ...
    if (enableNewSignExtGRA) {
        ...
        if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isArrayShadowSymbol())) {
            ...
            if (child->getOpCodeValue() == TR::aladd) {
                ...
                if (((offsetChild->getOpCodeValue() == TR::ladd) || (offsetChild->getOpCodeValue() == TR::lsub)) && (offsetChild->getSecondChild()->getOpCodeValue() == TR::lconst))
                ...
                if ((offsetChild->getOpCodeValue() == TR::lmul) && (offsetChild->getSecondChild()->getOpCodeValue() == TR::lconst) && (sizeOfElem == offsetChild->getSecondChild()->getLongInt())) {
                    TR::Node* longIndexNode = offsetChild->getFirstChild();
                    if (longIndexNode->getOpCodeValue() == TR::i2l) {
                        *currentArrayAccess = longIndexNode;
                    }
                } else if ((offsetChild->getOpCodeValue() == TR::i2l) && (sizeOfElem == 1)) {
        ...
        bool canSkip = false;
        if (0 && ((node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isAuto()) || (((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub)) && node->getFirstChild()->getOpCode().isLoadVarDirect() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() && node->getSecondChild()->getOpCode().isLoadConst() /* &&
        ...
        if (!canSkip && ((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub) || (node->getOpCodeValue() == TR::iand) || (node->getOpCodeValue() == TR::iloadi) || (node->getOpCodeValue() == TR::ixor)) || ((node->getOpCodeValue() == TR::iload) /* && !node->getSymbolReference()->getSymbol()->isAutoOrParm() */)) {
            TR::Node* storeNode = NULL;
            TR::Node* loadNode = NULL;
            if (*currentArrayAccess) {
                if (TR::Compiler->target.is64Bit() && !_rejectedSignExtNodes.find(node)) {
                    if ((node->getOpCodeValue() == TR::iload) && (node->getSymbolReference()->getSymbol()->isAutoOrParm())) {
                        loadNode = node;
                        canSkip = true;
                    } else if (false) {
                        TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
                        ...
                        while (cursorTree) {
                            ...
                            if (cursorNode->getOpCode().isStoreDirect() && cursorNode->getSymbolReference()->getSymbol()->isAuto() && (cursorNode->getFirstChild() == node)) {
                    ...
                    if (containingLoop) {
                        ...
                        if (loopInvariantBlock) {
                            ...
                            if (_candidatesSignExtendedInThisLoop->get(storeSymRef->getReferenceNumber())) {
                                ...
                                while (cursorTree) {
                                    TR::Node* cursorNode = cursorTree->getNode();
                                    if ((cursorNode->getOpCodeValue() == comp()->il.opCodeForDirectStore(node->getDataType())) && (cursorNode->getSymbolReference() == storeSymRef)) {
                                        if (cursorNode->getFirstChild()->getOpCode().isLoadVar() && (cursorNode->getFirstChild()->getSymbolReference() == cursorNode->getSymbolReference()) && cursorNode->needsSignExtension()) {
                            ...
                            TR_NodeTriple* nodeTriple = new (comp()->trHeapMemory()) TR_NodeTriple(parent, signExtStore, (loadNode ? parent : storeNode), childNum);
        ...
        if (0 && *currentArrayAccess && canSkip) {
            ...
            if (TR::Compiler->target.is64Bit() && info && (parent->getOpCodeValue() == TR::i2l) /* && node->isNonNegative() */ && (*currentArrayAccess == parent) && enableSignExtGRA) {
                TR_BlockStructure* blockStructure = block->getStructureOf();
                node->setSkipSignExtension(true);
        ...
        if (TR::Compiler->target.is64Bit() && parent && (parent->getOpCodeValue() == TR::i2l) && ((*currentArrayAccess != parent) && (grandParent->getOpCodeValue() != TR::l2i)) && node->skipSignExtension()) {
    ...
    if (!enableNewSignExtGRA && /* !hasCatchBlock && */ node->getOpCode().isLoadVarDirect() && (node->getSymbolReference()->getSymbol()->isAuto())) {
        TR_UseDefInfo* info = optimizer()->getUseDefInfo();
    ...
    if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect()) {
        ...
        TR::SymbolReference* symRef = node->getSymbolReference();
        if ((symRef->getSymbol()->isAutoOrParm() || (0) && symRef->getSymbol()->getDataType() != TR::NoType)
    ...
    if (enableNewSignExtGRA && node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isArrayShadowSymbol()))
    ...
    vcount_t visitCount, bool hasCatchBlock)
{
    ...
    if (enableNewSignExtGRA) {
        ...
        if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isArrayShadowSymbol())) {
            ...
            if (child->getOpCodeValue() == TR::aladd) {
                ...
                if ((offsetChild->getOpCodeValue() == TR::lmul) && (offsetChild->getSecondChild()->getOpCodeValue() == TR::lconst) && (sizeOfElem == offsetChild->getSecondChild()->getLongInt())) {
                    ...
                    if (longIndexNode->getOpCodeValue() == TR::i2l)
                        *currentArrayAccess = longIndexNode;
                } else if ((offsetChild->getOpCodeValue() == TR::i2l) && (node->getOpCode().getSize() == 1)) {
                    *currentArrayAccess = offsetChild;
                }
            }
        }
        ...
        if (!needStoreAndItExists && ((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub) || (node->getOpCodeValue() == TR::iand) || (node->getOpCodeValue() == TR::iloadi) || (node->getOpCodeValue() == TR::ixor) || ((node->getOpCodeValue() == TR::iload) && !node->getSymbolReference()->getSymbol()->isAutoOrParm()))) {
            TR::Node* storeNode = NULL;
            if (*currentArrayAccess) {
                if (TR::Compiler->target.is64Bit() && (parent->getOpCodeValue() == TR::i2l) /* && node->isNonNegative() */ && (*currentArrayAccess == parent) && !_rejectedSignExtNodes.find(node)) {
                    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
                    TR::TreeTop* cursorTree = _candidates->_startOfExtendedBBForBB[block->getNumber()]->getEntry()->getNextTreeTop();
                    while (cursorTree) {
                        TR::Node* cursorNode = cursorTree->getNode();
