...
void
TR_GlobalRegisterAllocator::populateSymRefNodes(TR::Node *node, vcount_t visitCount)
{
 ...
 */
bool
TR_GlobalRegisterAllocator::isSymRefAvailable(TR::SymbolReference *symRef, List<TR::Block> *blocksInLoop)
{
	if (!comp()->cg()->considerTypeForGRA(symRef))
		return false;
...
static void
changeHeapBaseConstToLoad(TR::Compilation *comp, TR::SymbolReference *&autoSymRef, TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
	...
	if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
		...
		if (trace())
			comp()->dumpMethodTrees(
			        "Trees before tactical global register allocator", comp()->getMethodSymbol());
		...
		if (!comp()->getOption(TR_ProcessHugeMethods)) {
			...
			//
			double complexityEstimate = comp()->getFlowGraph()->getNumberOfNodes() * (double)numCands
			        * numCands;
		...
		if (canAffordAssignment) {
			globalFPAssignmentDone = _candidates->assign(
			        cfgBlocks, numberOfBlocks, _firstGlobalRegisterNumber, _lastGlobalRegisterNumber);
				...
				for (nodeTriple = nodeTriplesIt.getFirst(); nodeTriple != NULL;
					...
					if (trace())
						traceMsg(comp(), "node1 %p node2 %p node3 %p \n", nodeTriple->_node1,
						        nodeTriple->_node2, nodeTriple->_node3);
			...
			if (!splitSymRefs.isEmpty()) {
				...
				// If source and destination locals of a copy receive registers, in other words, a
				// register-register copy exists, and a value is modified in one of locals, another
				// local must be indentified as 'valueModified'.
				//
				if (trace)
				...
				// Remove a store generated at the end of the live range when a value is not modified in
				// the live range. If a value is not modified in a local which receives a register, the
				...
				for (tt = itr.getFirst(); tt; tt = itr.getNext()) {
					if (!_valueModifiedSymRefs->isSet(tt->getNode()
						...
						if (trace)
							traceMsg(
							        comp(), "Remove a redundant store %p\n", tt->getNode());
	...
	if (node->getOpCode().hasSymbolReference() || node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg()) {
		if (node->getSymbolReferenceOfAnyType()) {
			...
			TR_RegisterCandidate *oldRc =
			        origSymRef ? (*_registerCandidates)[origSymRef->getReferenceNumber()] : 0;
...
void
TR_GlobalRegisterAllocator::transformNode(TR::Node *node, TR::Node *parent, int32_t childIndex, TR::TreeTop *tt,
	...
	if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch()
		...
		if (value) {
			...
			TR_GlobalRegister *ptrToGr =
			        getGlobalRegisterWithoutChangingCurrentCandidate(symbol, registers, block);
			if (ptrToGr) {
		...
		if (changeNode) {
			...
			if (!value) {
...

				{
					...
					dumpOptDetails(comp(),
					        "%s did not change load var [%p] of symRef#%d to load reg because "
					        "float/double issues\n",
			...
			if (comp()->getOption(TR_CheckGRA)
				...
				TR::Node *zeroIfEqual =
				        TR::Node::create(comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()),
				                2, oldValue, value);
				TR::TreeTop::create(comp(), tt->getPrevTreeTop(),
				        TR::Node::createWithSymRef(TR::BNDCHK, 2, 2,
				                TR::Node::iconst(oldValue, 1), // pretend we have an array of size=1 so
			...
			if (value->getOpCode().isLoadReg()) {
				...
				TR_Array<TR_GlobalRegister> &extRegisters =
				        _candidates->_startOfExtendedBBForBB[block->getNumber()]->getGlobalRegisters(
				                comp());
			...
			if ((!fpStore && !symRef->storeCanBeRemoved())
				...
				if (fpStore && !comp()->cg()->getSupportsJavaFloatSemantics()) {
					...
					for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
						...
						while (b) {
							...
							if (curRegisters[i].getRegisterCandidateOnExit()
							        && (curRegisters[i].getRegisterCandidateOnExit()
							                != rc)) {
								if ((*extRegisters)[i].getValue()
								        == node->getFirstChild()) {
									// gr.setValue(0);
									(*extRegisters)[rc->getGlobalRegisterNumber()]
									        .setValue(0);
				...
				if (_osrCatchSucc || symRef->getSymbol()->dontEliminateStores(comp()) || keepAllStores
					...
					origStoreToMetaData = NULL;
					{
						TR::DataType regStoreType = node->getDataType();
						TR::Node::recreate(
						        node, comp()->il.opCodeForRegisterStore(regStoreType));
					}
 ...
 */
void
TR_GlobalRegisterAllocator::transformMultiWayBranch(TR::TreeTop *exitTreeTop, TR::Node *node, TR::Block *block,
...
void
TR_GlobalRegisterAllocator::transformBlockExit(TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR::Block *block,
        ...
        int32_t i, TR_GlobalRegister *extReg, TR_GlobalRegister *reg, TR::Block *block, bool traceIt)
{
	...
	if (traceIt)
		traceMsg(comp(),
		        "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors block=%d "
	...
	if (traceIt)
		traceMsg(
		        comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors checking extensions\n");
	...
	if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
		if (traceIt)
			traceMsg(comp(),
			        "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors nextBlock=%d\n",
			        nextBlock->getNumber());
		...
		if (nextRc && nextRc != rc) // Not live anymore so leave. Should be caught on previous test on live on
		                            // exit
		{
			if (traceIt)
		...
		if (traceIt)
			traceMsg(comp(),
			        "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors next block is not "
			        "extension\n");
        ...
        TR::TreeTop *appendPoint, TR_Array<TR_GlobalRegister> &registers, TR::Block *throwingBlock)
{
	...
	for (auto nextEdge = throwingBlock->getExceptionSuccessors().begin();
		...
		if (succ->asBlock()->isOSRCatchBlock()) {
			...
			if (trace())
				traceMsg(comp(),
				        "           addStoresForCatchBlockLoads([%p], block_%d) found OSR catch "
				        "block_%d\n",
        ...
        TR_Array<TR::Node *> &regDepNodes, TR::Node *exitNode, TR_Array<TR_GlobalRegister> &registers, TR::Block *block)
{
	...
	for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
		if (regDepNodes[i]) {
			...
			// regDepNodes[i], (candidate ? candidate->getSymbolReference()->getReferenceNumber() : 0));
...
void
TR_GlobalRegisterAllocator::prepareForBlockExit(TR::TreeTop *&exitTreeTop, TR::Node *&exitNode, TR::Block *block,
        TR_Array<TR_GlobalRegister> &registers, TR::Block *successorBlock, TR_Array<TR::Node *> &regDepNodes)
{
	...
	for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
		...
		/// extgr->getValue(), extgr->getAutoContainsRegisterValue());
		//
			...
			if ((!extgr->getAutoContainsRegisterValue() || mustBeLiveAcrossAllPaths)
				...
				if (liveOnSomeSucc && !extgr->getAutoContainsRegisterValue()
					...
					if ((block->getSuccessors().size() == 1)
						...
						extgr->createStoreFromRegister(_visitCount,
						        extgr->optimalPlacementForStore(block, comp()), i, comp());
					} else {
						if (!prevTreeTop)
							prevTreeTop = findPrevTreeTop(
							        exitTreeTop, exitNode, block, successorBlock);
					...
					if (needs2Regs) {
						...
						if (i == highRegNum) {
							...
							if (extRegisters[lowRegNum].getCurrentRegisterCandidate() == rc)
								extRegisters[lowRegNum].setAutoContainsRegisterValue(
								        true);
						} else {
							if (extRegisters[highRegNum].getCurrentRegisterCandidate()
							        == rc)
								extRegisters[highRegNum].setAutoContainsRegisterValue(
								        true);
						}
					}
				}
		...
		// traceMsg(comp(), "exit node %p succ RC %p reg dep nodes %p\n", exitNode, successorRC,
		// regDepNodes[i]);
		if (successorRC && !regDepNodes[i] && !gr->isUnavailable()) // ensure the global reg is currently
		                                                            // available)
		{
			...
			// if (extgr->getCurrentRegisterCandidate() != successorRC)
			{
				...
				if (currRC && currRC->rcNeeds2Regs(comp())) {
					...
					if (i == highRegNum) {
						TR_GlobalRegister *grLow =
						        &extRegisters[currRC->getLowGlobalRegisterNumber()];
						grLow->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(),
						        _visitCount, exitTreeTop->getEnclosingBlock(),
						        currRC->getLowGlobalRegisterNumber(), comp());
						// grLow->copyCurrentRegisterCandidate(gr);
					} else {
						TR_GlobalRegister *grHigh =
						        &extRegisters[currRC->getHighGlobalRegisterNumber()];
						grHigh->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(),
						        _visitCount, exitTreeTop->getEnclosingBlock(),
	...
	if ((successorBlock != originalSuccessorBlock) && (successorBlock->getPredecessors().size() == 1)
		...
		transformBlockExit(
		        succExit, succExit->getNode(), sb, sb->getGlobalRegisters(comp()), originalSuccessorBlock);
	}
}

bool
TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR::Block *block,
        TR_GlobalRegister *extgr, TR::Block *&successorBlock, int32_t i)
{
	...
	if (!predecessors.empty() && !(predecessors.size() == 1)
		...
		if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
			if (trace())
				traceMsg(comp(),
				        "Extended live range of #%d into successor since candidate is available in "
				        "register on all predecessor's exits: block=%d succ=%d allPredFreq=%d "
				        "succFreq=%d\n",
				        rc->getSymbolReference()->getReferenceNumber(), block->getNumber(),
				        successorBlock->getNumber(), allPredFreq, successorBlock->getFrequency());
	...
	bool keepingItAliveAcrossEdgeRequiresExtraGoto = !ttBeforeSuccessor
				...
				if (((block->getNextBlock()->getFrequency() == 0)
				            || ((block->getNextBlock()->getFrequency() > 0)
				                    && ((successorBlock->getFrequency() * 100
				                                / (block->getNextBlock()->getFrequency()))
				                            > 130))))
					branchIsHotter = TR_yes;
			}
		}
	...
	if (branchIsHotter == TR_no) {
		...
		if (!newBlock->getEntry()->getPrevTreeTop()) {
			newBlock->append(TR::TreeTop::create(
			        comp(), TR::Node::create(exitNode, TR::Goto, 0, successorBlock->getEntry())));
...
TR::Block *
TR_GlobalRegisterAllocator::createNewSuccessorBlock(TR::Block *block, TR::Block *successorBlock,
        TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR_RegisterCandidate *rc)
{
					...
					if (numRegistersLiveOnNewSuccessor + (needs2Regs ? 2 : 1)
						...
						if (trace())
							traceMsg(comp(),
							        ...
							        "> "
							        "comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge("
							        "block_%d) %d\n",
							        numRegistersLiveOnNewSuccessor,
							        nextNewBlock->getNumber(), block->getNumber(),
							        comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(
							                block));
						newBlockCanBeReused = false;
					}
	...
	if (!newBlock) {
		...
		for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
			if (successorRegisters[j].getRegisterCandidateOnEntry()) {
				...
				// blockRegisters[j].getRegisterCandidateOnExit()); traceMsg(comp(), "succ on enter
				// %p\n", successorRegisters[j].getRegisterCandidateOnEntry()); traceMsg(comp(), "cand
			...
			if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
				...
				if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
					...
					TR_ASSERT(!symbol->isInGlobalRegister(),
					        "Current candidate is not live on entry and not live on exit from "
					        "basic block\n");
			}
		}
		return extgr;
		...
		if (!firstRc || firstRc->getSymbolReference()->getSymbol() != symbol) {
			...
			if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
				...
				if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
					...
					TR_ASSERT(false,
					        "Current cand is not live on entry and not live on exit from basic "
					        "block\n");
			}
		}
...
void
TR_GlobalRegister::setCurrentRegisterCandidate(TR_RegisterCandidate *rc, vcount_t visitCount, TR::Block *currentBlock,
        int32_t i, TR::Compilation *comp, bool resetOtherHalfOfLong)
{
	if (_rcCurrent != rc) {
		...
		_rcCurrent = rc;
		setValue(0); // regardless of whether or not we created a store we must setValue to NULL, else we can
		             // use the wrong value

		if (currentBlock)
			comp->setCurrentBlock(currentBlock);
		if (rc
		        && (rc->getSymbol()->dontEliminateStores(comp) || keepAllStores
		                || rc->isLiveAcrossExceptionEdge()))
...
TR::Node *
TR_GlobalRegister::createStoreToRegister(TR::TreeTop *prevTreeTop, TR::Node *node, vcount_t visitCount,
...
TR::Node *
TR_GlobalRegister::createStoreFromRegister(
        vcount_t visitCount, TR::TreeTop *prevTreeTop, int32_t i, TR::Compilation *comp, bool storeUnconditionally)
{
	...
	if (i != -1) {
		if (store->requiresRegisterPair(comp))
			dumpOptDetails(comp,
			        "%s create store [%p] from Register %d (low word) and Register %d (high word)\n",
				...
				if (block != comp()->getStartBlock() && block != cfg->getStart()
					...
					while (bvi.hasMoreElements()) {
						...
						if (!rc->_blocks.find(block->getNumber()))
							rc->_blocks.setNumberOfLoadsAndStores(
							        toBlock(block)->getNumber(), 0);
					}
				}
			}
		}
						...
						if (storeNode1 && storeNode1->getOpCode().isStoreDirect()) {
							...
							if ((symRef->getSymbol()->isAutoOrParm())
								...
								for (TR::TreeTop *tt2 = block2->getEntry();
									...
									TR::Node *storeNode2 =
									        tt2->getNode()->getStoreNode();
									if (storeNode2
									        && storeNode2->getOpCode()
									                   .isStoreDirect()
									        && storeNode2->getSymbolReference()
									                == symRef) {
										rc = comp()->getGlobalRegisterCandidates()
										             ->findOrCreate(symRef);
										break;
									}
								}
			...
			if (virtualGuard->isTheVirtualGuardForAGuardedInlinedCall()) {
				...
				for (; tt1 != currBlock->getExit(); tt1 = tt1->getNextTreeTop()) {
					...
					if (storeNode1 && storeNode1->getOpCode().isStoreDirect()) {
						...
						if (rc) {
							int32_t weight = 1;
							if (branchBlock->findFirstReference(
...
void
TR_GlobalRegisterAllocator::offerAllFPAutosAndParmsAsCandidates(TR::Block **cfgBlocks, int32_t numberOfNodes)
{
			...
			if (sym) {
				...
				if ((sym->getDataType() == TR::Float || sym->getDataType() == TR::Double
				        ...
				        && ((sym->isAuto()
				                ...
				                || (sym->isParm()
				                        && methodSymbol->getParameterList().find(
				                                sym->castToParmSymbol())
			...
			for (nextBlock = blocksIt.getFirst(); nextBlock; nextBlock = blocksIt.getNext()) {
				if (nextBlock->getVisitCount() != visitCount) {
					...
					if (((++numIter) & 0x3f) == 0
						...
						comp()->failCompilation<TR::CompilationInterrupted>("interrupted in "
						                                                    "GRA-"
						                                                    "findLoopsAndCorrsp"
						                                                    "ondingAuto-block");
					}
						...
						for (p = pairs.getFirst(); p; p = pairs.getNext()) {
							...
							if (otherSymRef
								...
								if (otherCandidate->countNumberOfLoadsAndStoresInBlocks(
								            &blocksInLoop)
								        < rc->countNumberOfLoadsAndStoresInBlocks(
								                &blocksInLoop))
...
void
TR_GlobalRegisterAllocator::markAutosUsedIn(TR::Node *node, TR::Node *parent, TR::Node *grandParent,
        ...
        TR_BitVector *symsThatShouldNotBeAssignedInCurrentLoop, bool hasCatchBlock)
{
	...
	if (node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isAuto()) {
		...
		if (TR::Compiler->target.is64Bit() && info && (parent->getOpCodeValue() == TR::i2l)
		        && node->isNonNegative() && enableSignExtGRA) {
			...
			if (info->getUseDef(defs, useIndex)) {
				...
				for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
					...
					if (defIndex < info->getFirstRealDefIndex()) // def is  unseen--can't mark this
					                                             // node as skippable
					{
						node->setSkipSignExtension(false);
					...
					if (defNode->getOpCode().isStore()) {
						...
						// TR_ASSERT((sym == node->getSymbolReference()->getSymbol()), "Symbols
						// must match between a use and a def\n"); Comment out the above assume
						// because:
						//   - The assume came from JAVA where there could not be aliasing with
						//   autos
						//   - If there is an alias causing a different sym then we will not
						//   have a candidate anyways so it does not matter
						//   - avoid extra computation if symbols do not match by adding extra
						//   condition below.
	...
	if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect()) {
		...
		if (symRef->getSymbol()->isAutoOrParm() && isSymRefAvailable(symRef, blocksInLoop)) {
			...
			if (block != cfg->getStart()) {
				...
				// because this cost is meaningful only when one of operand (source or destination)
				// receives a register. Note: Although we need to re-calculate the cost when one of
				// operand receives a register, we do not do so currently.
				//
					...
					if (parent->getSecondChild()->getOpCode().isLoadConst())
						...
						if (otherChild->getOpCode().isLoadVarDirect()
						        && (otherChild->getSymbolReference()
						                        ->getSymbol()
						                        ->isAutoOrParm()))
							findOrCreatePairedSymbols(
							        symRef, otherChild->getSymbolReference());
					}
				}
			}
		}
	...
	for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
		...
		markAutosUsedIn(node->getChild(childNum), node, parent, currentArrayAccess, block, blocksInLoop,
		        visitCount, executionFrequency, registerCandidates, assignedAutosInCurrentLoop,
		        symsThatShouldNotBeAssignedInCurrentLoop, hasCatchBlock);
	...
	if (symsThatShouldNotBeAssignedInCurrentLoop && assignedAutosInCurrentLoop
	        && ((opCode.hasSymbolReference() && opCode.isIndirect()) || (opCode.isArrayLength())
	                || !(opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isBooleanCompare()
	                        || opCode.isNullCheck() || opCode.isBndCheck())))
		findSymsUsedInIndirectAccesses(
		        node, symsThatShouldNotBeAssignedInCurrentLoop, assignedAutosInCurrentLoop, true);
}

void
TR_GlobalRegisterAllocator::signExtendAllDefNodes(TR::Node *defNode, List<TR::Node> *defNodes)
{
...
void
TR_GlobalRegisterAllocator::findSymsUsedInIndirectAccesses(TR::Node *node,
...
TR_PairedSymbols *
TR_GlobalRegisterAllocator::findOrCreatePairedSymbols(TR::SymbolReference *symRef1, TR::SymbolReference *symRef2)
{
...
TR_PairedSymbols *
TR_GlobalRegisterAllocator::findPairedSymbols(TR::SymbolReference *symRef1, TR::SymbolReference *symRef2)
{
	ListIterator<TR_PairedSymbols> pairs(&_pairedSymbols);
	TR_PairedSymbols *p;
	for (p = pairs.getFirst(); p; p = pairs.getNext()) {
				...
				if (nextBlock->getVisitCount() != visitCount) {
					...
					while (currentTree != exitTree) {
						...
						createStoresForSignExt(currentNode, NULL, NULL, currentTree,
						        &arrayAccess, nextBlock, &blocksInLoop, visitCount, false);
						currentTree = currentTree->getNextRealTreeTop();
					}
				}
			}
...
void
TR_GlobalRegisterAllocator::createStoresForSignExt(TR::Node *node, TR::Node *parent, TR::Node *grandParent,
        ...
        vcount_t visitCount, bool hasCatchBlock)
{
	...
	int32_t childNum;
	for (childNum = 0; childNum < node->getNumChildren(); childNum++)
		createStoresForSignExt(node->getChild(childNum), node, parent, treetop, currentArrayAccess, block,
		        blocksInLoop, visitCount, hasCatchBlock);
}
		...
		if (regionStructure->isNaturalLoop() && structureNode) {
			...
			if (loopInvariantBlock) {
				...
				SymRefCandidateMap *registerCandidates = new (trStackMemory())
				        SymRefCandidateMap((SymRefCandidateMapComparator()),
				                SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));

				TR_BitVector *replacedAutosInCurrentLoop = new (trStackMemory())
				        TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
				TR_BitVector *autosThatCannotBeReplacedInCurrentLoop = new (trStackMemory())
				        TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
					...
					if (nextBlock->getVisitCount() != visitCount) {
						...
						while (currentTree != exitTree) {
							...
							replaceAutosUsedIn(currentTree, currentNode, NULL, nextBlock,
							        ...
							        autosThatCannotBeReplacedInCurrentLoop, structureNode,
							        loopInvariantBlock);
							currentTree = currentTree->getNextRealTreeTop();
						}
					}
				...
				/// if (trace())
				traceMsg(comp(),
				        "Trying to split unused locals in loop %d with starting sym ref count %d\n",
					...
					if (candidateIsLiveOnExit) {
						...
						if (((!isFloat
							...
							TR_SymRefCandidatePair *correspondingSymRefCandidate =
							        splitAndFixPreHeader(symRef, correspondingSymRefs,
							                loopInvariantBlock,
							...
							fixExitsAfterSplit(symRef, correspondingSymRefCandidate,
							        correspondingSymRefs, loopInvariantBlock, &blocksInLoop,
							        loopInvariantBlock->getEntry()->getNode(),
				...
				_origSymRefCount = comp()->getSymRefCount(); // _origSymRefCount was always zero becasue
...
void
TR_LiveRangeSplitter::replaceAutosUsedIn(TR::TreeTop *currentTree, TR::Node *node, TR::Node *parent, TR::Block *block,
        ...
        TR_StructureSubGraphNode *loop, TR::Block *loopInvariantBlock)
{
		...
		if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp())
					...
					if (!rc) {
						...
						if (!rc) {
							...
							if (!dontCreateCandidatesJustForSplitter
							        && performTransformation(comp(),
							                "%s live range splitter creating candidate for "
							                "#%d in loop %d\n",
							                OPT_DETAILS, origSymRef->getReferenceNumber(),
							                loop->getNumber())) {
								rc = comp()->getGlobalRegisterCandidates()
								             ->findOrCreate(origSymRef);
			...
			if (correspondingSymRef) {
				...
				if (!node->getOpCode().isStoreDirect()
					...
					traceMsg(comp(),
					        " --- created a store to auto #%d adjacent to existing store to auto "
					        "#%d at %s node %p --- \n",
				...
				fixExitsAfterSplit(symRef, correspondingSymRefCandidate, correspondingSymRefs,
				        loopInvariantBlock, blocksInLoop, node, registerCandidates, loop,
				        replacedAutosInCurrentLoop, origSymRef);
			}
		}
	}
	...
	int32_t childNum;
	for (childNum = 0; childNum < node->getNumChildren(); childNum++)
		replaceAutosUsedIn(currentTree, node->getChild(childNum), node, block, blocksInLoop, exitBlocks,
		        visitCount, executionFrequency, registerCandidates, correspondingSymRefs,
		        replacedAutosInCurrentLoop, autosThatCannotBeReplacedInCurrentLoop, loop, loopInvariantBlock);
}

TR_SymRefCandidatePair *
TR_LiveRangeSplitter::splitAndFixPreHeader(TR::SymbolReference *symRef, TR_SymRefCandidatePair **correspondingSymRefs,
        TR::Block *loopInvariantBlock, TR::Node *node)
{
	...
	TR_SymRefCandidatePair *correspondingSymRefCandidate = new (trStackMemory())
	        TR_SymRefCandidatePair(correspondingSymRef, 0);
...
void
TR_LiveRangeSplitter::fixExitsAfterSplit(TR::SymbolReference *symRef,
        ...
        SymRefCandidateMap &registerCandidates, TR_StructureSubGraphNode *loop,
	...
	if (correspondingSymRef) {
		...
		if (!replacedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
			...
			if (rc && !correspondingRc) {
				correspondingRc =
				        comp()->getGlobalRegisterCandidates()->findOrCreate(correspondingSymRef);
				...
				if (parentOfLoop) {
					...
					for (nextBlock = blocksIt.getCurrent(); nextBlock;
						...
						if (!blocksInInnerLoop->get(nextBlock->getNumber())) {
							if (trace())
								traceMsg(comp(),
								        "Adding original candidate #%d in block_%d in "
								        "outer loop %d (%p)\n",
								        rc->getSymbolReference()->getReferenceNumber(),
								        nextBlock->getNumber(),
...
void
TR_LiveRangeSplitter::appendStoreToBlock(
        TR::SymbolReference *storeSymRef, TR::SymbolReference *loadSymRef, TR::Block *block, TR::Node *node)
{
	TR::Node *initNode = TR::Node::createWithSymRef(
	        comp()->il.opCodeForDirectStore(storeSymRef->getSymbol()->getDataType()), 1, 1,
...
void
TR_LiveRangeSplitter::prependStoreToBlock(
        TR::SymbolReference *storeSymRef, TR::SymbolReference *loadSymRef, TR::Block *block, TR::Node *node)
{
...
void
TR_LiveRangeSplitter::placeStoresInLoopExits(TR::Node *node, TR_StructureSubGraphNode *loop,
	...
	for (exitBlock = exitBlocksIt.getCurrent(); exitBlock; exitBlock = exitBlocksIt.getNext()) {
		if ((exitBlock != comp()->getFlowGraph()->getEnd())
			...
			if (!(exitBlock->getPredecessors().size() == 1)) {
				...
				dumpOptDetails(comp(),
				        " place store-back of auto #%d using auto #%d in loop exit block_%d \n",
				        orig->getReferenceNumber(), replacement->getReferenceNumber(),
				        exitBlock->getNumber());
