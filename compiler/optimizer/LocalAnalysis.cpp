        ...
        if (node->getOpCode().isStore() && !node->getSymbolReference()->getSymbol()->isAutoOrParm()) {
            ...
            if (node->getOpCode().isStoreIndirect()) {
                if (seenWriteBarrier) {
                    ...
                    TR::Node::recreate(
                        node, _compilation->il.opCodeForCorrespondingIndirectStore(node->getOpCodeValue()));
                }
        ...
        for (other = cursor.firstNode(); other; other = cursor.nextNode()) {
            ...
            if (other->getOpCode().isStore() && !other->getSymbolReference()->getSymbol()->isAutoOrParm()) {
                ...
                if (other->getOpCode().isStoreIndirect()) {
                    if (seenOtherWriteBarrier) {
                        ...
                        TR::Node::recreate(
                            other, _compilation->il.opCodeForCorrespondingIndirectStore(other->getOpCodeValue()));
                    }
            ...
            if (seenOtherWriteBarrier) {
                ...
                if (other->getOpCode().isIndirect())
                    TR::Node::recreate(
                        other, _compilation->il.opCodeForCorrespondingIndirectLoad(other->getOpCodeValue()));
                else
