...
bool
TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(TR::Node *node, TR::Compilation *comp, TR::Node *parent)
{
...
bool
TR_LocalAnalysis::isSupportedNode(TR::Node *node, TR::Compilation *comp, TR::Node *parent, bool isSupportedStoreNode)
{
...
bool
TR_LocalAnalysis::isSupportedNodeForPREPerformance(TR::Node *node, TR::Compilation *comp, TR::Node *parent)
{
...
bool
TR_LocalAnalysis::isSupportedNodeForFunctionality(
   ...
   comp()->resetVisitCounts(0);
	...
	if (_numNodes < 0) {
		...
		if (_numNullChecks == 0)
			...
			_nullCheckNodesAsArray =
			        (TR::Node **)trMemory()->allocateStackMemory(_numNullChecks * sizeof(TR::Node *));
					...
					if (oldExpressionOnRhs == -1) {
						if (trace()) {
							...
							comp()->getDebug()->print(comp()->getOutFile(),
							        firstNodeInTree->getFirstChild(), 6, true);
						}
...
TR_LocalAnalysis::TR_LocalAnalysis(TR_LocalAnalysisInfo &info, bool trace) : _lainfo(info), _trace(trace)
{
...
//
int
TR_LocalAnalysisInfo::hasOldExpressionOnRhs(TR::Node *node, bool recalcContainsCall, bool storeLhsContainsCall)
{
	//
	...
	/////if (node->getOpCode().isNullCheck())
	{
		...
		if (node->getOpCode().isStore() && !node->getSymbolReference()->getSymbol()->isAutoOrParm()) {
			...
			if (node->getOpCode().isStoreIndirect()) {
				if (seenWriteBarrier) {
					TR::Node::recreate(
					        node, _compilation->il.opCodeForIndirectArrayLoad(node->getDataType()));
				} else {
			...
			if (areSame) {
				if (seenWriteBarrier) {
					...
					if (node->getOpCode().isIndirect())
						...
						TR::Node::recreate(node,
						        _compilation->il.opCodeForDirectStore(node->getDataType()));
				}
	...
	if (seenWriteBarrier) {
		...
		if (node->getOpCode().isIndirect())
			TR::Node::recreate(
			        node, _compilation->il.opCodeForCorrespondingIndirectLoad(node->getOpCodeValue()));
		else
...
TR_LocalAnalysisInfo::isCallLike(TR::Node *node)
{
	...
	if (node->getOpCode().hasSymbolReference()) {
		...
		// Note that this change is only needed because javaLangClassFromClass loads are being skipped for
		// performance (see earlier code in this file) and we want to avoid strange side effects of a parent
		// being commoned but a child is not a candidate for commoning.
		//
		if (node->getOpCode().hasSymbolReference()
...
bool
TR_LocalAnalysisInfo::containsCallInTree(TR::Node *node, bool &containsCallInStoreLhs)
{
...
//
bool
TR_LocalAnalysisInfo::areSyntacticallyEquivalent(TR::Node *node1, TR::Node *node2)
{
						...
						if (grandChild1->getLocalIndex() != grandChild2->getLocalIndex())
							...
							if (grandChild1->getLocalIndex() == MAX_SCOUNT) {
								if ((grandChild1->getOpCode().isLoadConst()
								            || grandChild1->getOpCode()
								                       .isLoadVarDirect())
								        && (grandChild2->getOpCode().isLoadConst()
								                || grandChild2->getOpCode()
								                           .isLoadVarDirect())) {
									if (!_optimizer->areNodesEquivalent(
...
//
bool
TR_LocalAnalysisInfo::countSupportedNodes(TR::Node *node, TR::Node *parent, bool &containsCallInStoreLhs)
{
