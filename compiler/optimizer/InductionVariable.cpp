...
TR_LoopStrider::TR_LoopStrider(TR::OptimizationManager *manager)
        ...
        , _storeTreesSingleton(
                  (StoreTreeMapComparator()), StoreTreeMapAllocator(comp()->trMemory()->currentStackRegion()))
{
...
TR_LoopStrider::perform()
{
	...
	_hoistedAutos = new (stackMemoryRegion)
	        SymRefPairMap((SymRefPairMapComparator()), SymRefPairMapAllocator(stackMemoryRegion));
...
bool
TR_LoopStrider::foundLoad(TR::TreeTop *storeTreeTop, TR::Node *node, int32_t symRefNum, vcount_t visitCount)
{
...
bool
TR_LoopStrider::foundLoad(TR::TreeTop *storeTree, int32_t nextInductionVariableNumber, TR::Compilation *comp)
{
...
static int32_t
countInternalPointerOrPinningArrayStores(TR::Compilation *comp, TR::Block *block)
{
	int32_t count = 0;
...
int32_t
TR_LoopStrider::detectCanonicalizedPredictableLoops(
        TR_Structure *loopStructure, TR_BitVector **optSetInfo, int32_t bitVectorSize)
{
	...
	if (loopInvariantBlock) {
		...
		if (1) {
			...
			// check if there are multiple pre-headers (introduced by previous passes of
		...
		if (isPredictableLoop > 0) {
			...
			_linearEquations =
			        (int64_t **)trMemory()->allocateStackMemory(_numberOfLinearExprs * sizeof(int64_t *));
			...
			for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
				...
				//       as we might produce incorrect trees (wanted old value but will get new
				//       inced/deced value) .
				//
					...
					if (storeInRequiredForm && _storeTreesList) {
						...
						if (lookup != _storeTreesList->end()) {
							...
							for (storeTree = si.getCurrent(); storeTree != NULL;
								...
								storeInRequiredForm = foundLoad(storeTree->_tt,
								        nextInductionVariableNumber, comp());
								if (!storeInRequiredForm) {
				...
				if (_loopTestTree->getNode()->getNumChildren() == 2) {
					...
					if (firstChildOfLoopTestNode->getOpCode().hasSymbolReference()) {
						...
						TR_ASSERT(
						        storeNode->getOpCode().isStore(), "Expected to find a store\n");
						...
						if (firstChildOfLoopTestNode == storeNode->getFirstChild()
							...
							if (_storeTreesList) {
								auto lookup = _storeTreesList->find(
								        nextInductionVariableNumber);
								if (lookup != _storeTreesList->end()) {
									List<TR_StoreTreeInfo> *storeTreesList =
									        lookup->second;
									ListIterator<TR_StoreTreeInfo> si(
									        storeTreesList);
									TR_StoreTreeInfo *storeTree;
									for (storeTree = si.getCurrent();
									        storeTree != NULL;
									        storeTree = si.getNext()) {
										TR::Node *storeNode =
										        storeTree->_tt->getNode();
				...
				if (storeInRequiredForm
					...
					if (replaceAllInductionVariableComputations(loopInvariantBlock->getBlock(),
					            loopStructure, &newSymbolReference, inductionVarSymRef)) {
						...
						for (; k < _nextExpression; k++) {
							...
							// Add the initialization of the derived induction variable
							...
							TR::Node *storeNode = placeNewInductionVariableIncrementTree(
							        loopInvariantBlock, inductionVarSymRef,
							        newSymbolReference, k, symRefTab, placeHolderNode,
							        loopTestHasSymRef);
							...
							// todo: create IV even if non-const additive term? need to
							// check loop-invariance and
							//       known constraints
							...
							if (newSymbolReference->getSymbol()->getDataType()
								...
								if (v) {
									...
									if (newSymbolReference->getSymbol()
									                ->getType()
									                .isInt64()) {
										newEntryVal = genVPLongRange(entryVal,
										        mulConst,
										        getAdditiveTermConst(k));
										newExitVal = genVPLongRange(exitVal,
										        mulConst,
										        getAdditiveTermConst(k));
										newIncr = genVPLongRange(
										        incr, mulConst, 0);
									} else {
										newEntryVal = genVPIntRange(entryVal,
										        mulConst,
										        getAdditiveTermConst(k));
										newExitVal = genVPIntRange(exitVal,
										        mulConst,
										        getAdditiveTermConst(k));
										newIncr = genVPIntRange(
										        incr, mulConst, 0);
									}
									...
									if (bestCandidate == k)
										strideForPIV = iv;
									loopStructure->asRegion()->addInductionVariable(
									        iv);
								}
							}
						...
						if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable
						         ...
						         *                                                         */
						        !(0 && // no-strict IV
						                loopTestNode->getOpCode().isCompareForOrder()
						                && // unsigned inequality
						                loopTestNode->getOpCode().isUnsignedCompare()
						                && (!isAdditiveTermConst(bestCandidate) || // non-const
						                                                           // additive
						                                                           // term
						                        !strideForPIV || // no stride iv
						                        // we have stride, but range not strictly
						                        // non-neg or non-pos.
						                        !(strideForPIV->getEntry()->asLongConstraint()
						                                        ? (strideForPIV->getEntry()
						                                                                ->getLowLong()
						                                                        >= 0
						                                                || strideForPIV->getExit()
						                                                                ->getHighLong()
						                                                        <= 0)
						                                        : (strideForPIV->getEntry()
						                                                                ->getLowInt()
						        ...
						        && performTransformation(comp(),
						                ...
						                comp()->signature())) {
							changeLoopCondition(loopInvariantBlock, usingAladd,
							        bestCandidate, storeOfDerivedInductionVariable);
						}
					} else {
						TR_RegisterCandidate *inductionCandidate =
						        comp()->getGlobalRegisterCandidates()->findOrCreate(
						                inductionVarSymRef);
						if (performTransformation(comp(),
						            "%s Adding auto %d as a global register candidate in loop "
						            "%d\n",
						            OPT_DETAILS,
		...
		for (j = 0; j < symRefCount; j++) {
			...
			TR::SymbolReference *reassociatedAuto = reassociatedAutoSearchResult
			...
			if (reassociatedAuto) {
				...
				TR::Node *newStore =
				        TR::Node::createWithSymRef(TR::astore, 1, 1, arrayRefNode, reassociatedAuto);
						...
						if (usingAladd) {
							...
							arrayRefNode = TR::Node::create(TR::aiadd, 2,
							        TR::Node::createWithSymRef(
							                byteCodeInfoNode, TR::aload, 0, origAuto),
							        TR::Node::create(
							                byteCodeInfoNode, TR::iconst, 0, index));
					} else {
...
TR::VPLongRange *
TR_LoopStrider::genVPLongRange(TR::VPConstraint *cons, int64_t coeff, int64_t additive)
{
	if (cons) {
		int64_t low, high;
...
TR::VPIntRange *
TR_LoopStrider::genVPIntRange(TR::VPConstraint *cons, int64_t coeff, int64_t additive)
{
...
TR_LoopStrider::findOrCreateStoreInfo(TR::TreeTop *tree, int32_t i)
{
	...
	if (lookup != _storeTreesList->end()) {
		...
		lookup->second->add(
		        new (trStackMemory()) TR_StoreTreeInfo(tree, NULL, NULL, NULL, NULL, false, NULL, false));
	} else {
		TR_ScratchList<TR_StoreTreeInfo> *newList = new (trStackMemory())
...
TR::Node *
TR_LoopStrider::getNewLoopIncrement(TR::Node *oldLoad, int32_t k, int32_t symRefNum)
{
...
void
TR_LoopStrider::changeLoopCondition(TR_BlockStructure *loopInvariantBlock, bool usingAladd, int32_t bestCandidate,
        TR::Node *storeOfDerivedInductionVariable)
{
	...
	if (usingAladd || (secondChildOfLoopTestNode->getType().isInt64())) {
		...
		if (!secondChildOfLoopTestNode->getType().isInt64()) {
			...
			mulNode =
			        TR::Node::create(TR::lmul, 2, secondChildOfLoopTestNode->duplicateTree(), constantNode);
	} else
...
bool
TR_LoopStrider::foundValue(TR::Node *node, int32_t symRefNum, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return false;
...
bool
TR_LoopStrider::unchangedValueNeededIn(TR::Block *exitBlock, int32_t nextInductionVariableNumber, bool &seenStore)
{
...
TR::Node *
TR_LoopStrider::genLoad(TR::Node *node, TR::SymbolReference *symRef, bool isInternalPointer)
{
	TR::Node *result;
	result = TR::Node::createWithSymRef(node,
	        isInternalPointer ? TR::aload : (symRef->getSymbol()->getType().isInt64() ? TR::lload : TR::iload), 0,
	        symRef);
...
void
TR_LoopStrider::examineOpCodesForInductionVariableUse(TR::Node *node, TR::Node *parent, int32_t &childNum,
	...
	if ((replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload)
		...
		if (_usesLoadUsedInLoopIncrement) {
			...
			if (newLoad) {
				...
				if ((newLoad->getOpCodeValue() == TR::lload) && downcastNode) {
					// We are replacing oldNode(node) with newLoad. Must recursivelyDec all children
					// of oldNode
		...
		if (downcastNode && node->getOpCodeValue() == TR::lload) {
			...
			if (parent && parent->getType().isInt64() &&
			        ...
			        && !parent->getOpCode().isCall() && !parent->getOpCode().isConversion()
			...
			if (usingAladd) {
				...
				if (replacingNode->getOpCodeValue() == TR::isub) {
					adjustmentNode = TR::Node::create(
					        TR::imul, 2, adjustmentNode, TR::Node::create(node, TR::iconst, 0, -1));
					adjustmentNode->setLocalIndex(~0);
					adjustmentNode->getSecondChild()->setLocalIndex(~0);
				}
			...
			if (adjustmentNode) {
				if (usingAladd || (adjustmentNode->getType().isInt64())) {
					...
					differenceInAdditiveConstantsNode =
					        TR::Node::create(node, TR::iconst, 0, differenceInAdditiveConstants);
					op = replacingNode->getOpCode().isAdd() ? TR::iadd : TR::isub;
				}
				adjustmentNode =
				        TR::Node::create(op, 2, adjustmentNode, differenceInAdditiveConstantsNode);
			} else {
		...
		if (node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aladd) {
			...
			if (!pinningArrayPointer->isInternalPointer()) {
				...
				node->setPinningArrayPointer(pinningArrayPointer->castToInternalPointerAutoSymbol()
				                                     ->getPinningArrayPointer());
		}
		...
		if (canHoistAdditiveTerm) {
			...
			dumpOptDetails(comp(),
			        ...
			        "initialization tree : %p for new loop invariant symRef #%d\n",
		...
		if (downcastNode && newLoad->getType().isInt64()) {
			...
			if (nodeRememberedInLoadUsed) {
				...
				if (_storeTreeInfoForLoopIncrement)
					addLoad(_storeTreeInfoForLoopIncrement, node->getFirstChild()->getFirstChild(),
					        index);
			} else {
...
bool
TR_LoopStrider::examineTreeForInductionVariableUse(TR::Block *loopInvariantBlock, TR::Node *parent, int32_t childNum,
        TR::Node *node, vcount_t visitCount, TR::SymbolReference **newSymbolReference)
{
	...
	if ((node->getOpCodeValue() == TR::iadd || node->getOpCodeValue() == TR::isub)
			...
			if ((node->getFirstChild()->getOpCodeValue() == TR::imul
				...
				if ((replacingNode = findReplacingNode(
					...
					mulTerm = node->getFirstChild()->getSecondChild();
				        ...
				        && (replacingNode = findReplacingNode(
				                    node->getFirstChild()->getSecondChild(), usingAladd, -1))) {
				...
				if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral()
				        || // we were checking for signed here -- i made the test a bit weaker, but I
				           // don't think we can reach here with unsigned
					...
					*newSymbolReference = NULL;
					...
					if (canCreateNewSymRef) {
						...
						*newSymbolReference = comp()->getSymRefTab()->createTemporary(
						...
						if (isInternalPointer && !pinningArrayPointer) {
							...
							createParmAutoPair(comp()->getSymRefTab()->getSymRef(
							                           internalPointerSymbol),
							        newPinningArray);

							TR::Node *newAload = TR::Node::createLoad(node,
							        comp()->getSymRefTab()->getSymRef(
							                internalPointerSymbol));
							newAload->setLocalIndex(~0);
							TR::Node *newStore = TR::Node::createWithSymRef(
							...
							dumpOptDetails(comp(),
							        ...
							        "analysis inserted initialization tree : %p for new "
							        "symRef #%d\n",
					...
					if (node->getSecondChild()->getOpCode().isLoadConst()) {
						...
						if (node->getOpCodeValue() == TR::iadd
							...
							differenceInAdditiveConstants = additiveConstant
							        - getAdditiveTermConst(index);
						else // if (node->getSecondChild()->getOpCodeValue() == TR::isub)
							differenceInAdditiveConstants = (-1 * additiveConstant)
							        - getAdditiveTermConst(index);
					}

					examineOpCodesForInductionVariableUse(node, parent, childNum, index,
					        originalNode, replacingNode, linearTerm, mulTerm, newSymbolReference,
					        loopInvariantBlock, pinningArrayPointer, differenceInAdditiveConstants,
		...
		if (replacingNode) {
			*newSymbolReference = NULL;
			int32_t index =
			        findNewInductionVariable(node, newSymbolReference, false, internalPointerSymbol);
			...
			examineOpCodesForInductionVariableUse(node, parent, childNum, index, originalNode,
			        replacingNode, linearTerm, mulTerm, newSymbolReference, loopInvariantBlock,
			        pinningArrayPointer, differenceInAdditiveConstants, isInternalPointer, downcastNode,
			        usingAladd);
...
void
TR_LoopStrider::populateLinearEquation(TR::Node *node, int32_t loopDrivingInductionVar, int32_t derivedInductionVar,
...
void
TR_LoopStrider::setInternalPointer(TR::Symbol *symbol, TR::AutomaticSymbol *pinningArrayPointer)
{
	_numInternalPointers++;
...
TR::DataType
TR_LoopStrider::findDataType(TR::Node *node, bool usingAladd, bool isInternalPointer)
{
...
void
TR_LoopStrider::createParmAutoPair(TR::SymbolReference *parmSymRef, TR::SymbolReference *autoSymRef)
{
...
TR::Node *
TR_LoopStrider::placeInitializationTreeInLoopInvariantBlock(TR_BlockStructure *loopInvariantBlock,
	...
	if (getAdditiveTermNode(k) != NULL) {
		if (usingAladd) {
			addNode = TR::Node::create(
			        TR::ladd, 2, mulNode, duplicateAdditiveTermNode(k, placeHolderNode, TR::Int64));
		} else {
			addNode = TR::Node::create(mulNode->getType().isInt64() ? TR::ladd : TR::iadd, 2, mulNode,
...
TR::Node *
TR_LoopStrider::placeNewInductionVariableIncrementTree(TR_BlockStructure *loopInvariantBlock,
        TR::SymbolReference *inductionVarSymRef, TR::SymbolReference *newSymbolReference, int32_t k,
        TR::SymbolReferenceTable *symRefTab, TR::Node *placeHolderNode, int32_t loopTestHasSymRef)
{
	...
	if (lookup != _storeTreesList->end()) {
		...
		for (; storeTree != NULL;) {
			...
			TR::Node *newStore = placeNewInductionVariableIncrementTree(loopInvariantBlock,
			        inductionVarSymRef, newSymbolReference, k, symRefTab, placeHolderNode, newLoad,
			        storeTree->getData()->_insertionTreeTop, storeTree->getData()->_constNode,
		...
		returnValue = placeNewInductionVariableIncrementTree(loopInvariantBlock, inductionVarSymRef,
		        newSymbolReference, k, symRefTab, placeHolderNode, newLoad, _insertionTreeTop, _constNode,
		        _isAddition);
	}
        ...
        TR::TreeTop *insertionTreeTop, TR::Node *constNode, bool isAddition)
{
	...
	if (isAddition) {
		...
		if (_linearEquations[k][4] > -1) {
			if (constNode->getOpCode().isLoadConst()) {
				...
				TR::ILOpCodes op = (usingAladd || newMulNode->getType().isInt64()) ? TR::lneg
				                                                                   : TR::ineg;
	...
	dumpOptDetails(comp(),
	        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted loop incremental step tree : "
	        "%p for new symRef #%d\n",
	        newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
	return newStore;
}
...
int32_t
TR_LoopStrider::findNewInductionVariable(
			...
			if (internalPointerParentSame) {
				if (node->getFirstChild()->getOpCodeValue() == TR::imul
					...
					if (isMulTermEquivalentTo(i, node->getFirstChild()->getSecondChild())) {
				        ...
				        || node->getFirstChild()->getOpCodeValue() == TR::lshl) {
					TR_ASSERT(node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst(),
					        "assertion failure");
		...
		for (; i < _nextExpression; i++) {
			...
			if (internalPointerParentSame) {
				if (node->getOpCodeValue() == TR::imul || node->getOpCodeValue() == TR::lmul) {
					...
					TR_ASSERT(
					        node->getSecondChild()->getOpCode().isLoadConst(), "assertion failure");
...
void
TR_LoopStrider::identifyExpressionsLinearInInductionVariables(TR_Structure *structure, vcount_t visitCount)
{
...
bool
TR_LoopStrider::identifyExpressionLinearInInductionVariable(TR::Node *node, vcount_t visitCount)
{
	// for aladds
	bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
	...
	if (node->getOpCode().isLoadVar()) {
		...
		if (isExprLoopInvariant(node->getSecondChild())) {
			if (((node->getFirstChild()->getOpCodeValue() == TR::imul)
				...
				{
					if (usingAladd
					        && node->getFirstChild()->getSecondChild()->getOpCodeValue() == TR::i2l)
		...
		if (usingAladd && node->getFirstChild()->getOpCodeValue() == TR::i2l) // handle both TR::xmul and
		                                                                      // TR::xshl
			linearNode = node->getFirstChild()->getFirstChild();
		else
		...
		{
			if (usingAladd && node->getSecondChild()->getOpCodeValue() == TR::i2l) // handle both TR::xmul
			                                                                       // and TR::xshl
...
bool
TR_LoopStrider::reassociateAndHoistComputations(TR::Block *loopInvariantBlock, TR_Structure *structure)
{
...
bool
TR_LoopStrider::reassociateAndHoistComputations(
			...
			if ((isInternalPointer
				...
				if (_reassociatedAutos->find(originalInternalPointerSymbol)
					...
					if (isInternalPointer && !pinningArrayPointer) {
						...
						dumpOptDetails(comp(),
						        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable "
						        "analysis inserted initialization tree : %p for new symRef "
						        "#%d\n",
						        newStoreTreeTop->getNode(),
...
bool
TR_LoopStrider::isStoreInRequiredForm(int32_t symRefNum, TR_Structure *loopStructure)
{
	...
	if (_storeTreesList) {
		...
		if (lookup != _storeTreesList->end()) {
			...
			for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
				...
				if (storeTree->_tt->getEnclosingBlock()->getStructureOf()->getContainingLoop()
				        != loopStructure)
					return false;
			}
...
void
TR_LoopStrider::checkIfIncrementInDifferentExtendedBlock(TR::Block *block, int32_t inductionVariable)
{
...
TR::Node *
TR_LoopStrider::updateLoadUsedInLoopIncrement(TR::Node *node, int32_t inductionVariable)
{
	...
	if (_indirectInductionVariable && opCode.isLoadVar()
	        ...
	        && ((!_storeTreesList && _writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber()))
	                || (_storeTreesList
	                        && _storeTreesList->find(node->getSymbolReference()->getReferenceNumber())
	                                != _storeTreesList->end()
		...
		if (useDefInfo->getUseDef(defs, useIndex) && (defs.PopulationCount() == 1)) {
			...
			for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
				...
				if (constNode && _storeTrees[defNode->getSymbolReference()->getReferenceNumber()]) {
					checkIfIncrementInDifferentExtendedBlock(useDefInfo->getTreeTop(defIndex)
					                                                 ->getEnclosingBlock()
					                                                 ->startOfExtendedBlock(),
					        inductionVariable);
				} else {
...
bool
TR_LoopStrider::isStoreInRequiredForm(TR::Node *storeNode, int32_t symRefNum, TR_Structure *loopStructure)
{
	/* calls too can kill symRefs via loadaddr */
	if (symRefNum != 0 && _allKilledSymRefs[symRefNum] == true)
		return false;
...
static void
orderSensitiveDescendantsRec(TR::Node *node, TR::NodeChecklist &out, TR::NodeChecklist &visited)
{
 ...
 */
static void
assertSubstPreservesEvalOrder(TR::Node *orig, TR::Node *replacement, const char *operation)
{
	if (enableExpensiveLoopStriderAssertions()) {
		TR_ASSERT_FATAL(
		        substPreservesEvalOrder(orig, replacement), "%s fails to preserve ordering\n", operation);
	}
}
 ...
 */
void
TR_LoopStrider::detectLoopsForIndVarConversion(TR_Structure *loopStructure, TR::NodeChecklist &exLoads)
{
		...
		if (nextBlock->hasExceptionSuccessors()) {
			...
			for (TR::CFGEdge *e = sit.getFirst(); e != NULL; e = sit.getNext()) {
				...
				if (dest->isOSRCatchBlock()) {
					if (trace())
						traceMsg(comp(),
						        "reject loop %d because dest (block_%d) of an exit edge is "
						        "OSRCatchBlock\n",
						...
						if (_isInductionVariableMorphed || stressMode) {
							if (checkStoreOfIndVar(
								...
								widenedIVs.push_back(
								        std::make_pair(nextInductionVariable,
 ...
 */
void
TR_LoopStrider::extendIVsOnLoopEntry(const TR::list<std::pair<int32_t, int32_t> > &ivs, TR::Block *preheader)
{
 ...
 */
void
TR_LoopStrider::truncateIVsOnLoopExit(const TR::list<std::pair<int32_t, int32_t> > &ivs, TR_RegionStructure *loop)
{
 ...
 */
void
TR_LoopStrider::convertIV(TR::Node *bciNode, TR::TreeTop *nextTT, int32_t ivIn, int32_t ivOut, TR::ILOpCodes op)
{
	...
	TR::Node *conv = TR::Node::createStore(
	        bciNode, out, TR::Node::create(bciNode, op, 1, TR::Node::createLoad(bciNode, in)));
        ...
        TR_Structure *loopStructure, TR::SymbolReference *newSymRef, TR::SymbolReference *oldSymRef)
{
	...
	if (v) {
		...
		TR_InductionVariable *newv = new (trHeapMemory())
		        TR_InductionVariable(newSymRef->getSymbol()->castToRegisterMappedSymbol(), newEntryVal,
...
void
TR_LoopStrider::morphExpressionsLinearInInductionVariable(TR_Structure *structure, vcount_t visitCount)
{
	if (structure->asBlock() != NULL) {
		TR_BlockStructure *blockStructure = structure->asBlock();
		TR::Block *block = blockStructure->getBlock();
...
bool
TR_LoopStrider::morphExpressionLinearInInductionVariable(
 ...
 */
void
TR_LoopStrider::walkTreesAndFixUseDefs(
        TR_Structure *loopStructure, TR::SymbolReference *newSymbolReference, TR::NodeChecklist &exLoads)
{
	TR::DebugCounter::incStaticDebugCounter(comp(),
	        TR::DebugCounter::debugCounterName(comp(), "loopStrider.widen/(%s)/%s/loop=%d/iv=%d",
	                comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()),
	                loopStructure->getEntryBlock()->getNumber(), _loopDrivingInductionVar));
	...
	replaceLoadsInStructure(
	        loopStructure, _loopDrivingInductionVar, newIntValue, newSymbolReference, exLoads, visited);
 ...
 */
void
TR_LoopStrider::replaceLoadsInStructure(TR_Structure *structure, int32_t iv, TR::Node *defRHS,
...
void
TR_LoopStrider::replaceLoadsInSubtree(TR::Node *node, int32_t iv, TR::Node *defRHS, TR::SymbolReference *newSR,
 ...
 */
void
TR_LoopStrider::widenComparison(TR::Node *node, int32_t iv, TR::Node *defRHS, TR::NodeChecklist &exLoads)
{
...
void
TR_LoopStrider::eliminateSignExtensionsInSubtree(
        TR::Node *parent, TR::NodeChecklist &exLoads, TR::NodeChecklist &visited, SignExtMemo &signExtMemo)
{
	...
	for (int i = 0; i < parent->getNumChildren(); i++) {
		...
		if (!performTransformation(comp(),
		            ...
		            optDetailString(), node->getGlobalIndex(), extended->getGlobalIndex(), i,
...
TR_LoopStrider::signExtend(TR::Node *node, TR::NodeChecklist &exLoads, SignExtMemo &memo)
{
	...
	case TR::iadd: result = signExtendBinOp(TR::ladd, node, exLoads, memo); break;

	case TR::isub: result = signExtendBinOp(TR::lsub, node, exLoads, memo); break;

	case TR::imul: result = signExtendBinOp(TR::lmul, node, exLoads, memo); break;

	default: break; // just don't populate result
	}
	...
	if (result.extended != NULL) {
		...
		if (trace())
			traceMsg(comp(), "[Sign-Extn] sign-extended n%un %s into n%un %s\n", node->getGlobalIndex(),
			        node->getOpCode().getName(), result.extended->getGlobalIndex(),
...
TR_LoopStrider::SignExtEntry
TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64, TR::Node *node, TR::NodeChecklist &exLoads, SignExtMemo &memo)
{
 ...
 */
void
TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node *intNode, TR::Node *longNode)
{
	...
	if (!performTransformation(comp(), "%s [Sign-Extn] Transmuting n%un %s into l2i of n%un %s\n",
	            optDetailString(), intNode->getGlobalIndex(), intNode->getOpCode().getName(),
...
TR::Node *
TR_LoopStrider::duplicateMulTermNode(int32_t k, TR::Node *node, TR::DataType type)
{
	TR_ASSERT(k < _numberOfLinearExprs, "index k %d exceeds _numberOfLinearExprs %d!\n", k, _numberOfLinearExprs);
	...
	if (new_node->getDataType() != type)
		new_node = TR::Node::create(TR::ILOpCode::getProperConversion(
...
void
TR_InductionVariableAnalysis::gatherCandidates(TR_Structure *s, TR_BitVector *loopLocalDefs, TR_BitVector *allDefs)
{
	if (s->asRegion()) {
...
void
TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure *structure, TR_RegionStructure *loop)
{
		...
		if (node->getOpCode().isStoreDirect()
			...
			if (candidates->isSet(refNum)) {
				...
				if (isProgressionalStore(node, &kind, &delta)) {
					...
					// traceMsg(comp(),"In analyze block loop %d block %d, setting inSymbol %d to
					// unknown value
...
void
TR_InductionVariableAnalysis::analyzeCyclicRegion(TR_RegionStructure *region, TR_RegionStructure *loop)
{
...
void
TR_InductionVariableAnalysis::mergeWithBlock(TR::Block *block, DeltaInfo **inSet, TR_RegionStructure *loop)
{
...
void
TR_InductionVariableAnalysis::mergeWithSet(DeltaInfo **mySet, DeltaInfo **inSet, TR_RegionStructure *loop)
{
	TR_BitVector *candidates = ((AnalysisInfo *)loop->getAnalysisInfo())->getLoopLocalDefs();
...
//
bool
TR_InductionVariableAnalysis::isProgressionalStore(TR::Node *node, TR_ProgressionKind *kind, int64_t *incr)
{
	...
	if (!(cursor->getOpCode().isAdd() || cursor->getOpCode().isSub() || cursor->getOpCode().isLeftShift()
		...
		// traceMsg(comp(),"In isProgressionalStore cursor node %p returning false. its not
		// add/sub/shift\n",cursor);
		return false;
	}
	...
	{
		// traceMsg(comp(),"In isProgressionalStore cursor node %p returning false. symRef = %p(%d) is not equal
		// to storeRef
	...
	if (((int64_t)(int32_t)(*incr)) != *incr) {
		// traceMsg(comp(),"In isProgressionalStore cursor node %p, delta is not representable in
		// 32bits\n",cursor);
		return false;
	}
	return true;
        ...
        TR_RegionStructure *loop, TR::Node *branchNode, TR_Array<TR_BasicInductionVariable *> &basicIVs)
{
	...
	for (iv = it.getFirst(); iv; iv = it.getNext()) {
		...
		if (branchContainsInductionVariable(branchNode, iv->getSymRef(), &nodeBudget)) {
			...
			if (trace())
				traceMsg(
				        comp(), "\tbranchnode [%p] does not contain basiciv [%d]\n", branchNode, index);
		}
	}
	return result;
}
...
//
bool
TR_InductionVariableAnalysis::getProgression(TR::Node *expr, TR::SymbolReference *storeRef, TR::SymbolReference **ref,
	...
	*/
		...
		if (expr->getOpCode().hasSymbolReference()) {
			if (!expr->getSymbolReference()->getSymbol()->isAutoOrParm())
				return false;
			if (storeRef
			        && (expr->getSymbolReference() != storeRef
...
void
TR_InductionVariableAnalysis::analyzeLoopExpressions(TR_RegionStructure *loop, DeltaInfo **loopSet)
{
	TR_BitVector *candidates = ((AnalysisInfo *)loop->getAnalysisInfo())->getLoopLocalDefs();
	...
	while (it.hasMoreElements()) {
		...
		if (info->isUnknownValue()) {
			...
			if (info->getDelta() == 0) {
				...
				if (trace())
					traceMsg(comp(),
					        "====> Found basic linear induction variable symRef #%d[%p] with "
					        "increment %d\n",
			...
			if (trace())
				traceMsg(comp(),
				        "====> Found basic geometric induction variable symRef #%d[%p] with increment "
				        "%d\n",
				        refNum, symRef, info->getDelta());
...
//
static uint16_t
countNodeOccurrencesInSubTree(TR::Node *root, TR::Node *node, vcount_t visitCount, TR::Compilation *comp)
{
	uint16_t count = 0;
...
bool
TR_InductionVariableAnalysis::isIVUnchangedInLoop(
        TR_RegionStructure *loop, TR::Block *loopTestBlock, TR::Node *candidateIV)
{
	...
	if (trace())
		traceMsg(
		        comp(), "\tTrying to make sure that candidate IV hasn't been modified elsewhere in the loop\n");
	...
	while (!workQueue.empty()) {
		...
		for (TR::TreeTop *tt = curBlock->getFirstRealTreeTop(); tt->getNode()->getOpCodeValue() != TR::BBEnd;
			...
			if (curNode->getOpCode().isStoreDirect()
				...
				if (trace())
					traceMsg(comp(),
					        "\t\tFound store %p of symRef %p in block %d, which is not a loop test "
					        "block\n",
        ...
        TR_RegionStructure *loop, TR_BitVector *candidates, TR_Array<TR_BasicInductionVariable *> &basicIVs)
{
	...
	for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
		...
		if (!(node->getFirstChild()->getType().isIntegral()
			...
			if (trace())
				traceMsg(comp(),
				        "\tReject - Branch node is not integral nor ordered comparison between "
				        "internal pointers\n");
			return false;
		}
		...
		if (!node->getOpCode().isJumpWithMultipleTargets()) {
			...
			// TM: a jump with multiple target (this case, a tstart) is not a 'controlling branch to exit'
			// in terms of the temp induction variable, so we can continue
		...
		//      traceMsg(comp(),"Before branchContainsInductionVariable basicIVs->isEmpty() =
		//      %d\n",basicIVs.isEmpty());
		...
		if (valueCouldBeEvaluatedBeforeTest || forceILWalk) {
			...
			if (usesUnchangedValueInLoopTest) {
				if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(), valueNode)) {
					...
					if (trace())
						traceMsg(comp(),
						        "\tReject - IV node has been changed in one of the blocks that "
						        "is not part of the loop test in the loop\n");
					return false;
				}
			}
		}
		...
		for (tt = controllingBranch;; tt = tt->getPrevTreeTop()) {
			...
			if (curNode->getOpCode().isStoreDirect() && curNode->getSymbolReference() == biv->getSymRef()) {
				if (curNode->getFirstChild() != valueNode) {
					if (trace())
						traceMsg(comp(),
						        "\tRejected - exit value %p maybe not same as tested value "
						        "%p\n",
						        curNode, valueNode);
			...
			if (offset->getOpCode().isAdd() || offset->getOpCode().isSub()) {
				if (offset->getSecondChild()->getOpCode().isLoadConst()) {
					...
					if (otherNode->getOpCode().isMul()
						...
						if (loadNode->getOpCode().isLoadVarDirect()) {
							TR::Node *actualValue = findEntryValueForSymRef(
							        loop, loadNode->getSymbolReference());
							if (actualValue && actualValue->getOpCode().isLoadConst()) {
	...
	for (iv = bit.getFirst(); iv; iv = bit.getNext()) {
		if (iv != piv) {
			int32_t index = iv->getSymRef()->getSymbol()->getLocalIndex();
			TR_DerivedInductionVariable *div = new (trHeapMemory())
			        TR_DerivedInductionVariable(comp(), iv, piv);
			basicIVs[index] = div;
...
bool
TR_InductionVariableAnalysis::findEntryValues(TR_RegionStructure *loop, TR_Array<TR_BasicInductionVariable *> &basicIVs)
{
	TR_ArrayIterator<TR_BasicInductionVariable> it(&basicIVs);
...
TR::Node *
TR_InductionVariableAnalysis::findEntryValueForSymRef(TR_RegionStructure *loop, TR::SymbolReference *symRef)
{
...
TR::Node *
TR_InductionVariableAnalysis::getEntryValue(
        TR::Block *block, TR::SymbolReference *symRef, TR_BitVector *nodesDone, TR_Array<TR::Node *> &cachedValues)
{
	...
	if (!entryTree) {
		if (comp()->isLoopTransferDone()) {
			...
			TR::Node *dummyNode =
			        TR::Node::create(NULL, ((dataType == TR::Int32) ? TR::iconst : TR::lconst), 0);
	...
	for (TR::TreeTop *tt = block->getLastRealTreeTop(); tt != entryTree; tt = tt->getPrevRealTreeTop()) {
		...
		if (node->getOpCode().isStoreDirect()) {
			...
			// if you find somewhere a call like  foo(&num);  then this kills the num, and we can no longer
...
TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure *natLoop)
{
	...
	auto intIdxStoreTT = TR::TreeTop::create(cm,
	        TR::Node::createStore(
	                _intIdxSymRef, TR::Compiler->target.is64Bit() ? TR::Node::lconst(0) : TR::Node::iconst(0)));
	...
	auto intIncrementTT = TR::TreeTop::create(cm,
	        TR::Node::createStore(_intIdxSymRef,
	                TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd, 2,
	                        ...
	                        TR::Compiler->target.is64Bit() ? TR::Node::lconst(increment)
	...
	if (astoreNode->getSymbolReference() == firstChildSymRef) // Match if children to original backedgeIf children
	{
		newIf = TR::Node::createif(
		        getIntegralIfOpCode(backEdgeIfNode->getOpCodeValue(), TR::Compiler->target.is64Bit()),
...
TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node *node)
{
	...
	if (child && child->getOpCodeValue() == TR::aload && child->getSymbolReference() == _addrSymRef
		...
		auto arrayRef = TR::Node::recreateWithoutProperties(child,
		        TR::Compiler->target.is64Bit() ? TR::aladd : TR::aiadd, 2, TR::Node::createLoad(_baseSymRef),
		        TR::Node::createLoad(_intIdxSymRef));
	}
	for (int i = 0; i < node->getNumChildren(); i++) {
...
TR_IVTypeTransformer::isInAddrIncrementForm(TR::Node *node, int32_t &increment)
{
	...
	if (node->getOpCodeValue() == TR::astore && node->getChild(0)->getOpCode().isArrayRef()
	        ...
	        && (incChild = node->getChild(0)->getChild(1)) && incChild->getOpCode().isLoadConst()) // TODO:
	                                                                                               // non-const,
	                                                                                               // loop-invariant
	                                                                                               // expr
	{
