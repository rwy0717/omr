...
countReplicationFailure(char *failureReason, int32_t regionNum)
{
	...
	TR::DebugCounter::incStaticDebugCounter(TR::comp(),
	        TR::DebugCounter::debugCounterName(TR::comp(), "LoopReplicator/%s/%s/(%s)/region_%d", failureReason,
	                TR::comp()->getHotnessName(TR::comp()->getMethodHotness()), TR::comp()->signature(),
	                regionNum));
}
...
bool
TR_LoopReplicator::isWellFormedLoop(TR_RegionStructure *region, TR_Structure *condBlock)
{
...
int32_t
TR_LoopReplicator::replicateLoop(TR_RegionStructure *region, TR_StructureSubGraphNode *branchNode)
{
...
static void
collectNonColdInnerLoops(TR::Compilation *c, TR_RegionStructure *region, List<TR_RegionStructure> &innerLoops)
{
...
static void
collectNonColdLoops(TR::Compilation *c, TR_RegionStructure *region, List<TR_RegionStructure> &loops)
{
...
bool
TR_LoopReplicator::checkInnerLoopFrequencies(TR_RegionStructure *region, LoopInfo *lInfo)
{
	...
	for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
		...
		if (isInnerLoopHot) {
			...
			if (!searchList(innerLoopHeader, common, lInfo)) {
				...
				traceMsg(comp(),
				        "not going to replicate loop because hot inner loop %d is not on the trace\n",
				        loop->getNumber());
				return false;
			}
		}
        ...
        TR_RegionStructure *region, LoopInfo *lInfo, TR_ScratchList<TR::Block> *hotInnerLoopHeaders)
{
	...
	for (;;) {
		...
		for (auto e = tracePrefixCursor->getSuccessors().begin(); e != tracePrefixCursor->getSuccessors().end();
		        ++e) {
			...
			if (tracePrefixNext != NULL) {
				...
				countReplicationFailure(
				        "HotInnerLoopHitBranchWithoutColdSideEntry", region->getNumber());
				if (trace())
					traceMsg(comp(),
		...
		if (tracePrefixNext == NULL) {
			...
			if (trace())
				traceMsg(comp(),
				        "Ran out of trace without finding a cold side-entry. Will not replicate.\n");
			return false;
		}
		...
		if (hotInnerLoopHeaders->find(tracePrefixCursor)) {
			...
			if (trace())
				traceMsg(comp(),
				        "Hit a hot inner loop without finding a cold side-entry. Will not "
				        "replicate.\n");
			return false;
		}
	}
...
TR::Block *
TR_LoopReplicator::nextCandidate(TR::Block *X, TR_RegionStructure *region, bool doSucc)
{
...
void
TR_LoopReplicator::processBlock(TR::Block *X, TR_RegionStructure *region, LoopInfo *lInfo)
{
	TR::Block *seed = region->getEntryBlock();
...
bool
TR_LoopReplicator::isBackEdgeOrLoopExit(TR::CFGEdge *e, TR_RegionStructure *region, bool testSGraph)
{
...
TR::Block *
TR_LoopReplicator::bestSuccessor(TR_RegionStructure *region, TR::Block *node, TR::CFGEdge **edge)
{
	TR::Block *cand = NULL;
...
void
TR_LoopReplicator::nextSuccessor(TR_RegionStructure *region, TR::Block **cand, TR::CFGEdge **edge)
{
	...
	for (be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
		      ...
		      _blockMapper[n->getNumber()])*/
		{
			...
			newBlock->setFrequency(
			        (int32_t)((block->getFrequency() * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast
			                                                                                // explicitly
			...
			for (auto e = newBlock->getSuccessors().begin(); e != newBlock->getSuccessors().end(); ++e) {
				...
				(*e)->setFrequency(
				        (int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
			...
			for (auto e = newBlock->getPredecessors().begin(); e != newBlock->getPredecessors().end();
			        ++e) {
				int32_t edgeFreq = (*e)->getFrequency();
				(*e)->setFrequency(
				        (int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
...
TR_LoopReplicator::addBlocksAndFixEdges(LoopInfo *lInfo)
{
	...
	for (BlockEntry *be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
		...
		for (auto e = origBlock->getSuccessors().begin(); e != origBlock->getSuccessors().end(); ++e) {
			...
			else if ((dest->getNumber() < _nodesInCFG) && _blockMapper[dest->getNumber()]
			        && dest != loopHeader) {
					...
					if (branchNode->getOpCode().isBranch()) {
						...
						for (int32_t i = 0; i < branchNode->getNumChildren() - 1; i++) {
							...
							if (target->getNumber() == saveDest->getNumber()) {
								...
								if (trace())
									traceMsg(comp(),
									        "   fixed jumpwithmultipletarget child "
									        "%d -> %d\n",
									        i, dest->getNumber());
							}
						}
	...
	for (ee = (lInfo->_removedEdges).getFirst(); ee; ee = ee->getNext()) {
		...
		if (source->getNextBlock() == dest) {
			...
			if (branchNode->getOpCode().isSwitch()) {
				for (int32_t i = branchNode->getCaseIndexUpperBound() - 1; i > 0; i--) {
					TR::Block *target =
					        branchNode->getChild(i)->getBranchDestination()->getNode()->getBlock();
				...
				if ((branchNode->getOpCode().isGoto() || branchNode->getOpCode().isBranch())
				        && (branchNode->getBranchDestination()->getNode()->getBlock()->getNumber()
				                == dest->getNumber())) {
					// B2 is a goto block
...
TR::Block *
TR_LoopReplicator::createEmptyGoto(TR::Block *source, TR::Block *dest, bool redirect)
{
...
TR_LoopReplicator::fixUpLoopEntry(LoopInfo *lInfo, TR::Block *loopHeader)
{
	...
	for (auto e = loopHeader->getExceptionSuccessors().begin(); e != loopHeader->getExceptionSuccessors().end();
	        ++e) {
					...
					if (target == loopHeader) {
						...
						if (!edgeFixed) {
							...
							_cfg->addEdge(TR::CFGEdge::createEdge(
							        predBlock, clonedHeader, trMemory()));
							edgeFixed = true;
						}
					}
				}
...
TR_LoopReplicator::calculateBlockWeights(TR_RegionStructure *region)
{
	...
	do {
		...
		if (predecessorsNotYetVisited(region, node)) {
			dumpOptDetails(
			        comp(), "predecessors not yet visited for block: %d, requeue\n", node->getNumber());
			blockq.enqueue(node);
			continue;
		}
		// pbr
...
int32_t
TR_LoopReplicator::deriveFrequencyFromPreds(TR_StructureSubGraphNode *curNode, TR_RegionStructure *region)
{
...
bool
TR_LoopReplicator::predecessorsNotYetVisited(TR_RegionStructure *r, TR_StructureSubGraphNode *curNode)
{
...
int32_t
TR_LoopReplicator::getScaledFreq(TR_ScratchList<TR::Block> &blocks, TR::Block *X)
{
	int32_t freq = 0;
