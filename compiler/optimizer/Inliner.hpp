...
protected:
	...
	bool inlineCallTarget(TR_CallStack *, TR_CallTarget *calltarget, bool inlinefromgraph,
	        TR_PrexArgInfo *argInfo = 0, TR::TreeTop **cursorTreeTop = NULL);
	...
	int32_t scaleSizeBasedOnBlockFrequency(int32_t bytecodeSize, int32_t frequency, int32_t borderFrequency,
	        TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode,
	        int32_t coldBorderFrequency = 0); // not virtual because we want base class always calling base version
	                                          // of this
	...
	TR::TreeTop *addGuardForVirtual(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::TreeTop *,
	        TR::Node *, TR_OpaqueClassBlock *, TR::TreeTop *, TR::TreeTop *, TR::TreeTop *,
	        TR_TransformInlinedFunction &, List<TR::SymbolReference> &, TR_VirtualGuardSelection *, TR::TreeTop **,
	        TR_CallTarget *calltarget = 0);

	void addAdditionalGuard(TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
	...
	TR::Node *createVirtualGuard(TR::Node *, TR::ResolvedMethodSymbol *, TR::TreeTop *, int16_t,
	        TR_OpaqueClassBlock *, bool, TR_VirtualGuardSelection *);
...
struct TR_ParameterMapping : TR_Link<TR_ParameterMapping> {
	 ...
	 *
	 * Used for replacing both the parm symbol references in the inlined body and args on the taken side of the
	 * guard.
	 */
...
public:
	...
	TR_ParameterToArgumentMapper(TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::Node *,
	        TR_PrexArgInfo *, List<TR::SymbolReference> &, List<TR::SymbolReference> &, List<TR::SymbolReference> &,
	        TR_InlinerBase *);
...
public:
	...
	static TR::TreeTop *storeValueInATemp(TR::Compilation *comp, TR::Node *, TR::SymbolReference *&, TR::TreeTop *,
	        ...
	        List<TR::SymbolReference> &availableTemps, List<TR::SymbolReference> *moreTemps,
	        bool behavesLikeTemp = true, TR::TreeTop **newStoreValueATreeTop = NULL, bool isIndirect = false,
	        int32_t offset = 0);
	...
	virtual TR_InnerPreexistenceInfo *createInnerPrexInfo(TR::Compilation *c,
	        TR::ResolvedMethodSymbol *methodSymbol, TR_CallStack *callStack, TR::TreeTop *callTree,
	 ...
	 * information of the next call, which is usually the receiver or the method of the next call. Knowing the
	 * receiver of the first call means we can devirtualize all the calls along the chain.
	 *
	 *    Each method except the last one in the chain usually contains simple code that manipulates the arguments
	 * for the next call. The last method in the chain is usually the most important one, it does the real job and
...
private:
	...
	void replaceNodesReferencedFromAbove(
	        TR::TreeTop *, TR::Node *, TR::Node *, uint32_t, TR::NodeChecklist &visitedNodes);
