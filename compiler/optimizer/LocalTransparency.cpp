...
TR_LocalTransparency::TR_LocalTransparency(TR_LocalAnalysisInfo &info, bool t) : TR_LocalAnalysis(info, t)
{
	if (trace())
	 ...
	 */
	{
		...
		while (currentTree) {
			...
			updateUsesAndDefs(currentTree->getNode(), globalDefinedSymbolReferences,
			        ...
			        symRefsUsedAfterDefinedInBlock[blockNumber], startVisitCount, tempContainer, temp,
		...
		*_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
		...
		while (currentTree) {
			...
			updateInfoForSupportedNodes(currentNode, allSymbolReferences,
				...
				for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
					...
					if (!allSymbolReferencesInStore->isEmpty() && storeTreeParticipatesInAnalysis) {
						if (allSymbolReferencesInStore->get(newDefinedSymbolReference)) {
							...
							if (trace())
								traceMsg(comp(),
								        "75757Expression %d killed (%x) by symRef #%d  "
								        "%d\n",
					...
					if (_isNullCheckTree) {
						if (!allSymbolReferencesInNullCheckReference->isEmpty()) {
							...
							if (allSymbolReferencesInNullCheckReference->get(
								...
								if (trace())
									traceMsg(comp(),
									        "76767Expression %d killed (%x) by "
									        "symRef #%d  %d\n",
									        currentNode->getLocalIndex(),
									        currentNode, newDefinedSymbolReference,
									        true);
		...
		for (nextNode = comp()->getFlowGraph()->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
			...
			*(_info[nextBlock->getNumber()]._analysisInfo) &= *_supportedNodes;
			...
			// We need to kill all expressions dependent on a symbol (other than the load/store) being
			// stored into; this is because we cannot compensate for an expression (o.f+1) where o.f is
			// written in the block. We would adjust the load/store of o.f (say with temp t1) but to adjust
			// the value of the temp t2 that corresponds to (o.f+1) is more complicated
			//
			...
			*tempContainer |= *definedSymbolReferencesInBlock[nextBlock->getNumber()];
			...
			for (iter.SetToFirstOne(); iter.Valid(); iter.SetToNextOne()) {
				...
				if (_hasTransparencyInfoFor->get(nextDefinedSymbolReference)) {
					...
					// If the value was not really defined by anything other than a call, ONLY the
					// store/load of the stored value (say o.f) can still survive. All other
					// dependent expressions must be killed though. See discussion above.
					//
					if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(
						...
						for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
							...
							// transparent; i.e. do not kill it because of this store as
							// this store is going to be privatized
							//
							if (_info[nextBlock->getNumber()]._analysisInfo->get(
							            storeNode)) {
								int32_t storeSymRefNum =
								        supportedNodesAsArray[storeNode]
								                ->getSymbolReference()
								                ->getReferenceNumber();
								...
								if (supportedNodesAsArray[storeNode]
									...
									if ((child->getLocalIndex() != MAX_SCOUNT)
										...
										if (!_transparencyInfo[nextDefinedSymbolReference]
										                ->get(child->getLocalIndex()))
											childKilled = true;
									} else
										childKilled = true;
								}
								...
								if (storeSymRefNum == nextDefinedSymbolReference)
									symRefCanSurvive = true;
								                 ...
								                 ->mayKill(true)
								                 .contains(nextDefinedSymbolReference,
								                         comp())) {
									{
										if (!definedSymbolReferencesInBlock
										                [nextBlock->getNumber()]
										                        ->get(storeSymRefNum)
										        || (!symRefsDefinedAfterStoredInBlock
										                        [nextBlock->getNumber()]
										                                ->get(storeSymRefNum)
										                && !symRefsUsedAfterDefinedInBlock
										                            [nextBlock->getNumber()]
										                                    ->get(storeSymRefNum)))
											symRefCanSurvive = true;
									}
								...
								if (symRefCanSurvive && !childKilled) {
									if (trace())
										traceMsg(comp(),
										        "Store node %d survives\n",
										        storeNode);

									if (survivingStoreNodes->isEmpty())
										survivingStoreNodes->set(storeNode);
									else {
										if (survivingStoreNodes->elementCount()
										                > 1
										        || !survivingStoreNodes->get(
										                storeNode)) {
											survivingStoreNodes->empty();
											break;
										}
...
void
TR_LocalTransparency::updateUsesAndDefs(TR::Node *node, ContainerType *globallySeenDefinedSymbolReferences,
        ...
        ContainerType *tempContainer, TR_BitVector *temp, ContainerType *allStoredSymRefsInMethod)
{
	...
	for (i = 0; i < node->getNumChildren(); i++)
		updateUsesAndDefs(node->getChild(i), globallySeenDefinedSymbolReferences, seenDefinedSymbolReferences,
		        seenStoredSymRefs, symRefsDefinedAfterStored, symRefsUsedAfterDefined, visitCount,
...
void
TR_LocalTransparency::updateInfoForSupportedNodes(TR::Node *node, ContainerType *allSymbolReferences,
        ...
        ContainerType *allStoredSymRefsInMethod, vcount_t visitCount)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		updateInfoForSupportedNodes(child, allSymbolReferences, allSymbolReferencesInNullCheckReference,
		        allSymbolReferencesInStore, seenDefinedSymbolReferences, seenStoredSymRefs,
		        allStoredSymRefsInMethod, visitCount);
				...
				if (!_supportedNodes->get(child->getLocalIndex()))
					...
					for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
						if (!_transparencyInfo[i]->get(child->getLocalIndex())
							...
							if (trace())
								traceMsg(comp(),
								        "Expression %d killed by symRef #%d because "
					...
					if (child->getOpCode().hasSymbolReference()
						...
						if (seenDefinedSymbolReferences->get(childSymRefNum)
							...
							for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
								...
								_transparencyInfo[nextAlias]->reset(
								        node->getLocalIndex());
								if (trace())
									traceMsg(comp(),
									        "9999Expression %d killed (%x) by "
									        "symRef #%d  %d\n",
									        node->getLocalIndex(), node, nextAlias,
									        seenDefinedSymbolReferences->get(
									                nextAlias));
							}

							if (trace())
								traceMsg(comp(),
								        "Expression %d killed by symRef #%d (loaded in "
								        "child)\n",
        ...
        TR::Node *node, TR::Node *child, ContainerType *seenDefinedSymbolReferences, ContainerType *seenStoredSymRefs)
{
	...
	if (childHasSupportedOpCode) {
		...
		if (child->getOpCode().isLoad() || child->getOpCodeValue() == TR::loadaddr) {
			...
			if (trace())
				traceMsg(
				        comp(), "Expression %d killed (non supported opcode)\n", node->getLocalIndex());
		}
	}
}
