            ...
            if (firstOp == TR::su2l) {
                if (secondOp == TR::cconst || secondOp == TR::su2l
                    ...
                    if (secondOp == TR::su2l) {
                        ...
                        if (reportCompareDemotions) {
                            dumpOptDetails(s->comp(),
                                "Long Compare Narrower: found child 1 c2l and child 2 lconst in cconst range in method "
                                "%s\n",
                                s->comp()->signature());
                        }
                    } else if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(),
                            "Long Compare Narrower: found child 1 c2l and child 2 cconst in method %s\n",
                            s->comp()->signature());
                    }
                }
        ...
        if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
            if (secondOp == TR::su2i
                ...
                if (secondOp == TR::su2i) {
                    ...
                    if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(),
                            "Integer Compare Narrower: found child 1 c2i and child 2 iconst in cconst range in method "
                            "%s\n",
                            s->comp()->signature());
                    }
                } else if (reportCompareDemotions) {
                    dumpOptDetails(s->comp(),
                        "Integer Compare Narrower: found child 1 c2i and child 2 cconst in method %s\n",
                        s->comp()->signature());
                }
            }
...
static bool checkAndReplaceRotation(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((1 << correctLeftShiftAmount) != mulConst) // Only a rotate if the const values add up to the size of the node
                                                   // type.  ie for ints, this better add up to 32
        return false;
...
static bool isOperationFPCompliant(TR::Node* parent, TR::Node* arithExprNode, TR::Simplifier* s)
{
    ...
    if (mulNonConstChild->getOpCode().isConversion()) {
        ...
        if (bitcount > precisionLimit)
            ...
            traceMsg(
                s->comp(), "Node:%p failed bitcount:%d >= precision:%d\n", mulConstChild, bitcount, precisionLimit);
    }
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (s->comp()->cg()->getSupportsVectorRegisters()) {
        ...
        if ((node->getType().isIntegral() || node->getType().isDouble())
            ...
            && performTransformation(s->comp(), "%sReplace indirect load [" POINTER_PRINTF_FORMAT "] with getvelem",
                   s->optDetailString(), node)) {
            int32_t offset = 0;
...
TR::Node* lushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    //
    if (secondChild->getOpCode().isLoadConst()
        && (firstChild->getOpCodeValue() == TR::lushl || firstChild->getOpCodeValue() == TR::lshl)
        && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
...
TR::Node* lorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // For 32 bit platforms (p and z), evaluator due to historical reasons, evaluator for rotate does not support 64 bit
    // regs on a 32 bit platform by default Need to disable this transform until that support is added.
...
TR::Node* iflcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::iflcmplt) {
        ...
        if (firstChild->getOpCode().isLoadConst()
            && conditionalBranchFold(
                   (((uint64_t)originalFirst->getLongInt()) < ((uint64_t)originalSecond->getLongInt())), node,
...
//
TR::Node* bitOpMemNDSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s) { return node; }
