...
template <typename T>
inline void
setCCAddSigned(T value, T operand1, T operand2, TR::Node *node, TR::Simplifier *s)
{
...
template <typename T>
inline void
setCCSubSigned(T value, T operand1, T operand2, TR::Node *node, TR::Simplifier *s)
{
...
convertToTestUnderMask(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand && node->getChild(1)->getOpCode().isLoadConst()
	        ...
	        && (constVal = iushr->getChild(1)->getConst<int32_t>()) == 24 && !load->getSymbol()->isParm()
	        && load->getOpCode().isLoadIndirect() && // Limit it to non-parm, indirect cases
	        load->getReferenceCount() == 1
	        && performTransformation(cm,
	                "%sTransforming iand/iushr to byte test under mask [" POINTER_PRINTF_FORMAT "]\n",
	                s->optDetailString(), node)) {
		// anchor iand subtree
		...
		auto bloadi =
		        TR::Node::createWithSymRef(TR::bloadi, 1, load->getChild(0), 0, load->getSymbolReference());
		node->setAndIncChild(0,
		        TR::Node::create(
		                TR::butest, 2, bloadi, TR::Node::bconst(iand->getChild(1)->getConst<int8_t>())));
   ...
   //    t1  c1         t1 t2
	...
	if (s->reassociate() && node->getOpCode().isAdd()) {
      ...
      //        t   c2     c1 c2
		        ...
		        && isExprInvariant(region, firstChild) && secondChild->getOpCode().isSub()
      ...
      //                          c2
		        ...
		        (!isExprInvariant(region, secondChild->getFirstChild())
		                || isSmallConstant(secondChild->getFirstChild(), s))
...
addSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		...
		if (node->getOpCode().isUnsigned()) {
			...
			if (node->nodeRequiresConditionCodes())
				setCCAddSigned<T>(
				        value, firstChild->getConst<T>(), secondChild->getConst<T>(), node, s);
		}
	...
	// turn add with other child negated in to a subtract
		...
		if (s->getLastRun() && s->comp()->cg()->codegenMulDecomposition((int64_t)iMulComposerValue)
		        && performTransformation(s->comp(),
		                "%sFactored iadd with distributed imul with a codegen decomposible constant in node "
		                "[%s]\n",
		                s->optDetailString(), node->getName(s->getDebug()))) {
		...
		if (lrChild->getOpCode().isLoadConst()) {
			if (secondChildOp.isLoadConst()) {
				if (!s->reassociate() && // use new rules
				        performTransformation(s->comp(),
				                "%sFound xadd of xconst with xadd or xsub of x and const in node "
				                "[%s]\n",
				                s->optDetailString(), node->getName(s->getDebug()))) {
					if (firstChild->getReferenceCount() > 1) {
...
subSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	// drives inegs up the tree
		...
		if (performTransformation(s->comp(),
		            "%sReduced xsub with negated first child in node [%s] to ineg of xadd\n",
		            s->optDetailString(), node->getName(s->getDebug()))) {
			TR::Node *newFirstChild = firstChild->getFirstChild();
	...
	// turn bitwise complement + 1 into a 2's complement negate
		...
		if (performTransformation(s->comp(),
		            "%sReduced isub of bitwise complement and iconst -1 in node [%s] to 2s complement "
		            "negation\n",
		            s->optDetailString(), node->getName(s->getDebug()))) {
	...
	// reduce imul operations by factoring
		...
		// s->optDetailString(), node->getName(s->getDebug())))
		{
			...
			if (!factorChild
				...
				TR::Node::recreate(node, TR::ILOpCode::getMulOpCode<T>());
				node->setChild(factorChild->getOpCode().isLoadConst() ? 1 : 0, factorChild)
				        ->decReferenceCount();
			...
			if (secondChildOp.isLoadConst()) {
				if (performTransformation(s->comp(),
			       ...
			       //   to fix register pressure problems thta result
			        if ((firstChild->getReferenceCount() == 1)
			                && (performTransformation(s->comp(),
			                        "%sFound xsub of non-xconst with xadd or xsub of x and const in node "
			                        "[%s]\n",
			                        s->optDetailString(), node->getName(s->getDebug())))) {
				// move constants up the tree so they will tend to get merged together
				node->setChild(1, lrChild);
...
static void
normalizeConstantShiftAmount(TR::Node *node, int32_t shiftMask, TR::Node *&secondChild, TR::Simplifier *s)
{
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (shiftValue != secondChild->getInt()) {
			if (performTransformation(s->comp(), "%sReducing constant of node [%s] from %d to %d\n",
			            s->optDetailString(), secondChild->getName(s->getDebug()), secondChild->getInt(),
			            shiftValue)) {
				if (secondChild->getReferenceCount() > 1) {
...
static void
normalizeShiftAmount(TR::Node *node, int32_t normalizationConstant, TR::Simplifier *s)
{
...
static void
orderChildrenByHighWordZero(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
...
static TR::Node *
foldDemotionConversion(TR::Node *node, TR::ILOpCodes opcode, TR::ILOpCodes foldedOpCode, TR::Simplifier *s)
{
...
//
static bool
reduceLongOp(TR::Node *node, TR::Block *block, TR::Simplifier *s, TR::ILOpCodes newConversionOp)
{
	...
	case TR::lshl:
		if (firstChild->getSecondChild()->getOpCode().isLoadConst()) {
			if ((firstChild->getSecondChild()->get64bitIntegralValue() & LONG_SHIFT_MASK) < 32) {
				...
				if (!performTransformation(s->comp(),
				            "%sReducing long operation in node [" POINTER_PRINTF_FORMAT "] to an int "
				            "operation\n",
				            s->optDetailString(), node))
					return false;
...
static void
fold2SmallerIntConstant(TR::Node *node, TR::Node *child, TR::DataType sdt, TR::DataType tdt, TR::Simplifier *s)
{
	int32_t val;
	switch (sdt) {
	case TR::Int16: val = (int32_t)child->getShortInt(); break;
	case TR::Int32: val = (int32_t)child->getInt(); break;
	case TR::Int64: val = (int32_t)child->getLongInt(); break;
	default: TR_ASSERT(0, "invalid source for fold");
	}
	switch (tdt) {
	case TR::Int8: foldByteConstant(node, (val << 24) >> 24, s, false /* !anchorChildren */); break;
	case TR::Int16: foldShortIntConstant(node, (val << 16) >> 16, s, false /* !anchorChildren */); break;
	case TR::Int32: foldIntConstant(node, val, s, false /* !anchorChildren */); break;
	default: TR_ASSERT(0, "invalid target for fold");
	}
}
...
intDemoteSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	TR::ILOpCodes inverseOp1 = TR::ILOpCode::getProperConversion(targetDataType, sourceDataType, false); // sign
	                                                                                                     // extend
	TR::ILOpCodes inverseOp2 = TR::ILOpCode::getProperConversion(targetDataType, sourceDataType, true); // zero
	                                                                                                    // extend
   ...
   // subtree  lconst 22
	...
	if (sourceIs64Bit) {
		...
		if (targetDataType == TR::Int32) {
			...
			reduceOp = TR::ILOpCode::getProperConversion(TR::Int32, targetDataType, false); // appropriate
			                                                                                // integer to
			                                                                                // integer op
		}
...
// 2. TODO: for cmp,
static void
partialRedundantCompareElimination(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	// return;
			...
			for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();) {
				...
				if (gotoTree) {
					...
					if (predEndWithGoto || isImmediatePred) {
						TR::TreeTop *storeTree = predEndWithGoto ? gotoTree->getPrevTreeTop()
						                                         : gotoTree;
						...
						if (storeNode->getOpCode().isStoreDirect() && storeSymRef == symRef
							...
							// ...
							//
							// also consider the case of block_A is the immediate
							// predecessor of block_B check if it is feeding the ifcmp if
							// (num >
							// comp->getOptions()->getLastIpaOptTransformationIndex())
							// return; num++;
							if (performTransformation(comp,
								...
								TR_RegionStructure *parent =
								        predBlock->getCommonParentStructureIfExists(
								                fallThroughBB, cfg);
								TR::Block *branchDestBB = node->getBranchDestination()
								                                  ->getNode()
								                                  ->getBlock();
								...
								if (predEndWithGoto) {
									...
									gotoTree = TR::TreeTop::create(comp,
									        gotoBlock->getEntry(),
									        TR::Node::create(storeNode, TR::Goto, 0,
									                fallThroughTT));
								...
								// Clone every treetop of block, all referenceCount to
								// be 1
								TR::TreeTop *curTree =
								...
								gotoTree->getNode()->setBranchDestination(
								        fallThroughTT);
...
addressCompareConversion(TR::Node *node, TR::Simplifier *s)
{
	...
	if (firstOp == TR::a2i && firstChild->getFirstChild()->getType().isAddress() && TR::Compiler->target.is32Bit()
		...
		if ((secondOp == TR::iconst && secondChild->getInt() == 0) || (secondOp == TR::a2i)) {
	        ...
	        && TR::Compiler->target.is64Bit() && firstChild->getReferenceCount() == 1) {
...
static void
longCompareNarrower(TR::Node *node, TR::Simplifier *s, TR::ILOpCodes intOp, TR::ILOpCodes charOp, TR::ILOpCodes shortOp,
        TR::ILOpCodes byteOp)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
		...
		if ((firstOp == TR::i2l)
			...
			if (secondOp == TR::iconst || secondOp == TR::i2l
				...
				if (secondOp == TR::i2l) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Long Compare Narrower: found child 1 i2l and child 2 lconst "
						        "in iconst range in method %s\n",
						        s->comp()->signature());
					}
				} else if (reportCompareDemotions) {
					dumpOptDetails(s->comp(),
					        "Long Compare Narrower: found child 1 i2l and child 2 iconst in method "
					        "%s\n",
					        s->comp()->signature());
				}
			}
			...
			if (firstOp == TR::su2l) {
				if (secondOp == TR::cconst || secondOp == TR::su2l
					...
					if (secondOp == TR::su2l) {
						...
						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found both children c2l in "
							        "method %s\n",
							        s->comp()->signature());
						}
					} else if (secondOp == TR::lconst) {
						if (secondChild->getReferenceCount() == 1) {
							...
							secondChild->decReferenceCount();
							TR::Node *cConstNode = TR::Node::sconst(
							        node, (uint16_t)secondChild->getLongInt());
							node->setAndIncChild(1, cConstNode);
						}
						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found child 1 c2l and child 2 "
							        "lconst in cconst range in method %s\n",
							        s->comp()->signature());
						}
					} else if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Long Compare Narrower: found child 1 c2l and child 2 cconst "
						        "in method %s\n",
						        s->comp()->signature());
					}
				}
				...
				if (secondOp == TR::sconst || secondOp == TR::s2l
					...
					if (secondOp == TR::s2l) {
						...
						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found both children s2l in "
							        "method %s\n",
							        s->comp()->signature());
						}
					} else if (secondOp == TR::lconst) {
						if (secondChild->getReferenceCount() == 1) {
							...
							secondChild->decReferenceCount();
							TR::Node *sConstNode = TR::Node::sconst(
							        node, (int16_t)secondChild->getLongInt());
							node->setAndIncChild(1, sConstNode);
						}

						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found child 1 s2l and child 2 "
							        "lconst in sconst range in method %s\n",
							        s->comp()->signature());
						}
					} else if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Long Compare Narrower: found child 1 s2l and child 2 sconst "
						        "in method %s\n",
						        s->comp()->signature());
					}
				}
				...
				if (secondOp == TR::bconst || secondOp == TR::b2l
					...
					if (secondOp == TR::b2l) {
						...
						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found both children b2l in "
							        "method %s\n",
							        s->comp()->signature());
						}
					} else if (secondOp == TR::lconst) {
						if (secondChild->getReferenceCount() == 1) {
							...
							secondChild->decReferenceCount();
							TR::Node *bConstNode = TR::Node::bconst(
							        node, (int8_t)secondChild->getLongInt());
							node->setAndIncChild(1, bConstNode);
						}
						if (reportCompareDemotions) {
							dumpOptDetails(s->comp(),
							        "Long Compare Narrower: found child 1 b2l and child 2 "
							        "lconst in bconst range in method %s\n",
							        s->comp()->signature());
						}
					} else if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Long Compare Narrower: found child 1 b2l and child 2 bconst "
						        "in method %s\n",
						        s->comp()->signature());
					}
				}
...
static void
transformToLongDivBy10Bitwise(TR::Node *origNode, TR::Node *node, TR::Simplifier *s)
{
...
static int
decomposeConstant(char bitPosition[], char operationType[], int64_t value, int wordSize)
{
	int32_t count = 0;
	int32_t pos = 0;
...
static TR::Node *
generateDecomposedTree(TR::Node *parentNode, TR::Node *multiplierChild, TR::Simplifier *s, char bitPosition[],
        char operationType[], int from, int to, int tabspace, bool isLong)
{
	int count = to - from;
	int i;
	...
	if (count <= 2) {
					...
					if (bitPosition[from] == 0) {
						...
						TR::Node *constNode1 = isLong
						        ? (TR::Node::lconst(
						                parentNode, (int64_t)1 << (int32_t)(bitPosition[from])))
						        : (TR::Node::iconst(
						                parentNode, 1 << (int32_t)(bitPosition[from])));
						TR::Node *shlNode =
					...
					if (bitPosition[from + 1] == 0) {
						...
						TR::Node *constNode1 = isLong
						        ? (TR::Node::lconst(parentNode,
						                (int64_t)1 << (int32_t)(bitPosition[from + 1])))
						        : (TR::Node::iconst(
						                parentNode, 1 << (int32_t)(bitPosition[from + 1])));
						firstChild = TR::Node::create(mul_op, 2, multiplierChild, constNode1);
					}
		...
		secondChild = generateDecomposedTree(parentNode, multiplierChild, s, bitPosition, operationType,
		        from + mid, to, tabspace + 1, isLong);
...
static void
printTree(TR::Simplifier *s, char bitPosition[], char operationType[], int from, int to, int tabspace, bool isLong)
{
	int count = to - from;
	int i;
...
decomposeMultiply(TR::Node *node, TR::Simplifier *s, bool isLong)
{
	...
	count = decomposeConstant(bitPosition, operationType,
	        isLong ? secondChild->getLongInt() : secondChild->getInt(), isLong ? 64 : 32);
...
static void
foldAddressConstant(TR::Node *node, int64_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
//
TR::Node *
getQuotientUsingMagicNumberMultiply(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	TR::Node *firstChild = node->getFirstChild(), *secondChild = node->getSecondChild();
	...
	if (node->getOpCodeValue() == TR::idiv || node->getOpCodeValue() == TR::irem) {
		...
		if (shiftAmount == 0) {
			if (divisor > 0) {
				node4 = TR::Node::create(
				        TR::iushr, 2, firstChild, TR::Node::create(firstChild, TR::iconst, 0, 31));
			} else {
				node4 = TR::Node::create(
			...
			node3 = TR::Node::create(
			        TR::ishr, 2, node2, TR::Node::create(node2, TR::iconst, 0, shiftAmount));
		...
		if (shiftAmount == 0) {
			if (divisor > 0) {
				node4 = TR::Node::create(
				        TR::lushr, 2, firstChild, TR::Node::create(firstChild, TR::iconst, 0, 63));
			} else {
				node4 = TR::Node::create(
			...
			node3 = TR::Node::create(
			        TR::lshr, 2, node2, TR::Node::create(node2, TR::iconst, 0, shiftAmount));
 ...
 */
static void
changeConverts2Unsigned(TR::Node *node, TR::ILOpCodes searchOp, TR::Simplifier *s)
{
	if (node->getReferenceCount() > 1)
		return;
	TR::ILOpCode op = node->getOpCode();
...
static void
simplifyIntBranchArithmetic(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
	SIMPLIFY_BRANCH_ARITHMETIC(Int, int32_t);
}

static void
simplifyLongBranchArithmetic(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild, TR::Simplifier *s)
{
...
//
static bool
conditionalBranchFold(int32_t takeBranch, TR::Node *&node, TR::Node *firstChild, TR::Node *secondChild,
...
//
static TR::Node *
binaryNanFloatOp(TR::Node *node, TR::Node *firstChild, TR::Node *secondChild, TR::Simplifier *s)
{
...
//
static TR::Node *
binaryNanDoubleOp(TR::Node *node, TR::Node *firstChild, TR::Node *secondChild, TR::Simplifier *s)
{
...
static bool
isLegalToMerge(TR::Node *node, TR::Block *block, TR::Block *nextBlock, TR::CFGEdgeList &nextExcpOut,
...
//
static void
changeBranchDestinationsForMergeBlocks(TR::Block *block, TR::Block *nextBlock, TR::Node *bbStartNode,
...
doubleToFloatOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq: op = TR::iffcmpeq; break;
	case TR::ifdcmpne: op = TR::iffcmpne; break;
	case TR::ifdcmplt: op = TR::iffcmplt; break;
	case TR::ifdcmpge: op = TR::iffcmpge; break;
	case TR::ifdcmpgt: op = TR::iffcmpgt; break;
	case TR::ifdcmple: op = TR::iffcmple; break;
	case TR::ifdcmpequ: op = TR::iffcmpequ; break;
	case TR::ifdcmpneu: op = TR::iffcmpneu; break;
	case TR::ifdcmpltu: op = TR::iffcmpltu; break;
	case TR::ifdcmpgeu: op = TR::iffcmpgeu; break;
	case TR::ifdcmpgtu: op = TR::iffcmpgtu; break;
	case TR::ifdcmpleu: op = TR::iffcmpleu; break;
	default: return op;
	}
...
doubleToIntegerOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq:
	case TR::ifdcmpequ: op = TR::ificmpeq; break;
	case TR::ifdcmpne:
	case TR::ifdcmpneu: op = TR::ificmpne; break;
	case TR::ifdcmplt:
	case TR::ifdcmpltu: op = TR::ificmplt; break;
	case TR::ifdcmpge:
	case TR::ifdcmpgeu: op = TR::ificmpge; break;
	case TR::ifdcmpgt:
	case TR::ifdcmpgtu: op = TR::ificmpgt; break;
	case TR::ifdcmple:
	case TR::ifdcmpleu: op = TR::ificmple; break;
	default: return op;
	}
...
doubleToLongOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq:
	case TR::ifdcmpequ: op = TR::iflcmpeq; break;
	case TR::ifdcmpne:
	case TR::ifdcmpneu: op = TR::iflcmpne; break;
	case TR::ifdcmplt:
	case TR::ifdcmpltu: op = TR::iflcmplt; break;
	case TR::ifdcmpge:
	case TR::ifdcmpgeu: op = TR::iflcmpge; break;
	case TR::ifdcmpgt:
	case TR::ifdcmpgtu: op = TR::iflcmpgt; break;
	case TR::ifdcmple:
	case TR::ifdcmpleu: op = TR::iflcmple; break;
	default: return op;
	}
...
doubleToShortOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq:
	case TR::ifdcmpequ: op = TR::ifscmpeq; break;
	case TR::ifdcmpne:
	case TR::ifdcmpneu: op = TR::ifscmpne; break;
	case TR::ifdcmplt:
	case TR::ifdcmpltu: op = TR::ifscmplt; break;
	case TR::ifdcmpge:
	case TR::ifdcmpgeu: op = TR::ifscmpge; break;
	case TR::ifdcmpgt:
	case TR::ifdcmpgtu: op = TR::ifscmpgt; break;
	case TR::ifdcmple:
	case TR::ifdcmpleu: op = TR::ifscmple; break;
	default: return op;
	}
...
doubleToCharOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq:
	case TR::ifdcmpequ: break;
	case TR::ifdcmpne:
	case TR::ifdcmpneu: break;
	case TR::ifdcmplt:
	case TR::ifdcmpltu: op = TR::ifsucmplt; break;
	case TR::ifdcmpge:
	case TR::ifdcmpgeu: op = TR::ifsucmpge; break;
	case TR::ifdcmpgt:
	case TR::ifdcmpgtu: op = TR::ifsucmpgt; break;
	case TR::ifdcmple:
	case TR::ifdcmpleu: op = TR::ifsucmple; break;
	default: return op;
	}
...
doubleToByteOp(TR::ILOpCodes dOp)
{
	...
	switch (dOp) {
	case TR::ifdcmpeq:
	case TR::ifdcmpequ: op = TR::ifbcmpeq; break;
	case TR::ifdcmpne:
	case TR::ifdcmpneu: op = TR::ifbcmpne; break;
	case TR::ifdcmplt:
	case TR::ifdcmpltu: op = TR::ifbcmplt; break;
	case TR::ifdcmpge:
	case TR::ifdcmpgeu: op = TR::ifbcmpge; break;
	case TR::ifdcmpgt:
	case TR::ifdcmpgtu: op = TR::ifbcmpgt; break;
	case TR::ifdcmple:
	case TR::ifdcmpleu: op = TR::ifbcmple; break;
	default: return op;
	}
...
floatToIntegerOp(TR::ILOpCodes fOp)
{
	...
	switch (fOp) {
	case TR::iffcmpeq:
	case TR::iffcmpequ: op = TR::ificmpeq; break;
	case TR::iffcmpne:
	case TR::iffcmpneu: op = TR::ificmpne; break;
	case TR::iffcmplt:
	case TR::iffcmpltu: op = TR::ificmplt; break;
	case TR::iffcmpge:
	case TR::iffcmpgeu: op = TR::ificmpge; break;
	case TR::iffcmpgt:
	case TR::iffcmpgtu: op = TR::ificmpgt; break;
	case TR::iffcmple:
	case TR::iffcmpleu: op = TR::ificmple; break;
	default: return op;
	}
	return op;
}
...
floatToLongOp(TR::ILOpCodes fOp)
{
	...
	switch (fOp) {
	case TR::iffcmpeq:
	case TR::iffcmpequ: op = TR::iflcmpeq; break;
	case TR::iffcmpne:
	case TR::iffcmpneu: op = TR::iflcmpne; break;
	case TR::iffcmplt:
	case TR::iffcmpltu: op = TR::iflcmplt; break;
	case TR::iffcmpge:
	case TR::iffcmpgeu: op = TR::iflcmpge; break;
	case TR::iffcmpgt:
	case TR::iffcmpgtu: op = TR::iflcmpgt; break;
	case TR::iffcmple:
	case TR::iffcmpleu: op = TR::iflcmple; break;
	default: return op;
	}
	return op;
}
...
floatToShortOp(TR::ILOpCodes fOp)
{
	...
	switch (fOp) {
	case TR::iffcmpeq:
	case TR::iffcmpequ: op = TR::ifscmpeq; break;
	case TR::iffcmpne:
	case TR::iffcmpneu: op = TR::ifscmpne; break;
	case TR::iffcmplt:
	case TR::iffcmpltu: op = TR::ifscmplt; break;
	case TR::iffcmpge:
	case TR::iffcmpgeu: op = TR::ifscmpge; break;
	case TR::iffcmpgt:
	case TR::iffcmpgtu: op = TR::ifscmpgt; break;
	case TR::iffcmple:
	case TR::iffcmpleu: op = TR::ifscmple; break;
	default: return op;
	}
	return op;
}
...
floatToCharOp(TR::ILOpCodes fOp)
{
	...
	switch (fOp) {
	case TR::iffcmpeq:
	case TR::iffcmpequ: break;
	case TR::iffcmpne:
	case TR::iffcmpneu: break;
	case TR::iffcmplt:
	case TR::iffcmpltu: op = TR::ifsucmplt; break;
	case TR::iffcmpge:
	case TR::iffcmpgeu: op = TR::ifsucmpge; break;
	case TR::iffcmpgt:
	case TR::iffcmpgtu: op = TR::ifsucmpgt; break;
	case TR::iffcmple:
	case TR::iffcmpleu: op = TR::ifsucmple; break;
	default: return op;
	}
	return op;
}
...
floatToByteOp(TR::ILOpCodes fOp)
{
	...
	switch (fOp) {
	case TR::iffcmpeq:
	case TR::iffcmpequ: op = TR::ifbcmpeq; break;
	case TR::iffcmpne:
	case TR::iffcmpneu: op = TR::ifbcmpne; break;
	case TR::iffcmplt:
	case TR::iffcmpltu: op = TR::ifbcmplt; break;
	case TR::iffcmpge:
	case TR::iffcmpgeu: op = TR::ifbcmpge; break;
	case TR::iffcmpgt:
	case TR::iffcmpgtu: op = TR::ifbcmpgt; break;
	case TR::iffcmple:
	case TR::iffcmpleu: op = TR::ifbcmple; break;
	default: return op;
	}
	return op;
}
		...
		if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
			if (secondOp == TR::su2i
				...
				if (secondOp == TR::su2i) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found both children c2i in method "
						        "%s\n",
						        s->comp()->signature());
					}
				} else if (secondOp == TR::iconst) {
					if (secondChild->getReferenceCount() > 1) {
						TR::Node *newSecondChild =
						        TR::Node::sconst(secondChild, (uint16_t)secondChild->getInt());
						secondChild->recursivelyDecReferenceCount();
						node->setAndIncChild(1, newSecondChild);
					} else {
						TR::Node::recreate(secondChild, TR::sconst);
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found child 1 c2i and child 2 "
						        "iconst in cconst range in method %s\n",
						        s->comp()->signature());
					}
				} else if (reportCompareDemotions) {
					dumpOptDetails(s->comp(),
					        "Integer Compare Narrower: found child 1 c2i and child 2 cconst in "
					        "method %s\n",
					        s->comp()->signature());
				}
			}
			...
			if (secondOp == TR::s2i
				...
				if (secondOp == TR::s2i) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found both children s2i in method "
						        "%s\n",
						        s->comp()->signature());
					}
				} else if (secondOp == TR::iconst) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found child 1 s2i and child 2 "
						        "iconst in sconst range in method %s\n",
						        s->comp()->signature());
					}
				} else if (reportCompareDemotions) {
					dumpOptDetails(s->comp(),
					        "Integer Compare Narrower: found child 1 s2i and child 2 sconst in "
					        "method %s\n",
					        s->comp()->signature());
				}
			}
			...
			if (secondOp == TR::b2i
				...
				if (secondOp == TR::b2i) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found both children b2i in method "
						        "%s\n",
						        s->comp()->signature());
					}
				} else if (secondOp == TR::iconst) {
					...
					if (reportCompareDemotions) {
						dumpOptDetails(s->comp(),
						        "Integer Compare Narrower: found child 1 b2i and child 2 "
						        "iconst in bconst range in method %s\n",
						        s->comp()->signature());
					}
				} else if (reportCompareDemotions) {
					dumpOptDetails(s->comp(),
					        "Integer Compare Narrower: found child 1 b2i and child 2 bconst in "
					        "method %s\n",
					        s->comp()->signature());
				}
			}
...
static void
unsignedIntCompareNarrower(TR::Node *node, TR::Simplifier *s, TR::ILOpCodes ushortOp, TR::ILOpCodes ubyteOp)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	static char *reportCompareDemotions = feGetEnv("TR_ReportCompareDemotions");
	if (s->cg()->getSupportsEfficientNarrowUnsignedIntComputation()) {
		...
		if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
			if (secondOp == TR::su2i
			        || (secondOp == TR::iuconst && secondChild->getUnsignedInt() <= USHRT_MAX)) {
				node->setAndIncChild(0, firstChild->getFirstChild());
				TR::Node::recreate(node, ushortOp);
				firstChild->recursivelyDecReferenceCount();
				if (secondOp == TR::su2i) {
			...
			if (secondOp == TR::s2i
			        || (secondOp == TR::iconst && secondChild->getUnsignedInt() <= SHRT_MAX)) {
				node->setAndIncChild(0, firstChild->getFirstChild());
				TR::Node::recreate(node, ushortOp);
				firstChild->recursivelyDecReferenceCount();
				if (secondOp == TR::s2i) {
			...
			if (secondOp == TR::b2i
			        || (secondOp == TR::iuconst && secondChild->getUnsignedInt() <= SCHAR_MAX)) {
				node->setAndIncChild(0, firstChild->getFirstChild());
...
//
static TR::Node *
integerToFloatHelper(uint32_t absValue, bool isUnsignedOperation, TR::Node *node, TR::Simplifier *s)
{
	TR::Node *firstChild = node->getFirstChild();
	...
	if (leadingZeroes(absValue) + trailingZeroes(absValue) >= 8) {
		...
		foldFloatConstant(node,
		        isUnsignedOperation ? (float)firstChild->getUnsignedInt() : (float)firstChild->getInt(), s);
		return node;
	} else {
...
static bool
canMovePastTree(TR::TreeTop *tree, TR::SymbolReference *symRef, TR::Compilation *c, bool blockHasCatchers)
{
...
checkAndReplaceRotation(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	TR_ASSERT(node->getOpCode().isOr() || node->getOpCode().isXor(),
	        "Node %p being checked for rotation pattern is not an (x)or!\n", node); // I believe this is general
	                                                                                // enough to allow other
	                                                                                // opcodes, like add's, xor's.
	                                                                                // But just want to be safe for
	                                                                                // now.
	...
	// To simplify my code, I am not going to handle these two cases on the assumption that simplifier will already
	// have simplified these away.
	...
	TR::Node *mulNode = firstChild->getOpCode().isMul() ? firstChild
	                                                    : (secondChild->getOpCode().isMul() ? secondChild : 0);
	TR::Node *shiftNode = firstChild->getOpCode().isMul() ? secondChild
	                                                      : (secondChild->getOpCode().isMul() ? firstChild : 0);

	if (!mulNode || !shiftNode
	        || !(shiftNode->getOpCode().isShiftLogical() && shiftNode->getOpCode().isRightShift()))
		return false;
	...
	if (!mulConstNode || !shiftConstNode || mulNonConstNode != shiftNonConstNode) // requires commoning to have
	...
	T expectedMulConst = correctLeftShiftAmount < static_cast<T>(sizeof(T) * 8) ? 1ULL << correctLeftShiftAmount
	                                                                            : 0;
	...
	TR::Node::recreate(node, TR::ILOpCode::getRotateOpCodeFromDt(mulConstNode->getDataType())); // this line will
	                                                                                            // have to change
	                                                                                            // based on T
...
//
static TR::Node *
longToFloatHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node *node, TR::Simplifier *s)
{
	TR::Node *firstChild = node->getFirstChild();

	if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
		foldFloatConstant(node,
		        isUnsignedOperation ? (float)firstChild->getUnsignedLongInt() : (float)firstChild->getLongInt(),
		        s);
		return node;
	} else {
...
//
static TR::Node *
longToDoubleHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node *node, TR::Simplifier *s)
{
	TR::Node *firstChild = node->getFirstChild();
	...
	if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 53)) {
		foldDoubleConstant(node,
		        isUnsignedOperation ? (double)firstChild->getUnsignedLongInt()
	...
	} else // probably the native compile rounds to ieee-754, but to be sure we round it manually
	{
		...
		if (isUnsignedOperation)
			...
			foldDoubleConstant(
			        node, firstChild->getLongInt() < 0 ? -(double)absValue : (double)absValue, s);
		return node;
	}
}
...
bitTestingOp(TR::Node *node, TR::Simplifier *s)
{
	...
	case TR::Int64:
		foldLongIntConstant(
		        firstChild->getSecondChild(), bitAmount >> shiftAmount, s, false /* !anchorChildren */);
		foldLongIntConstant(secondChild, cmpAmount >> shiftAmount, s, false /* !anchorChildren */);
		break;
	default: TR_ASSERT(false, "should be unreachable");
	}
}
...
static bool
isOperationFPCompliant(TR::Node *parent, TR::Node *arithExprNode, TR::Simplifier *s)
{
	...
	if (mulNode->getDataType() == TR::Double && mulNode->getFirstChild()->getOpCode().isConversion()
	        && mulNode->getSecondChild()->getOpCode().isConversion()) { // long->double is only case where we must
		                                                            // give up
	...
	if (mulNonConstChild->getOpCode().isConversion()) {
		...
		case TR::Double: // rely on fact same storage used for longs
			...
			case TR::Int16:
			case TR::Int32:
			case TR::Int64: TR_ASSERT(0, "We should not reach here for these data types."); return false;
			}
...
public:
	IfxcmpgeToIfxcmpeqReducer(TR::Simplifier *simplifier, TR::Node *node) : _simplifier(simplifier), _node(node) {}
	...
	bool isReducible()
	{
		...
		case TR::ifbcmpge: result = isReducibleSpecific<int8_t>(); break;
		case TR::ifbucmpge: result = isReducibleSpecific<uint8_t>(); break;

		case TR::ifscmpge: result = isReducibleSpecific<int16_t>(); break;
		case TR::ifsucmpge: result = isReducibleSpecific<uint16_t>(); break;

		case TR::ificmpge: result = isReducibleSpecific<int32_t>(); break;
		case TR::ifiucmpge: result = isReducibleSpecific<uint32_t>(); break;

		case TR::iflcmpge: result = isReducibleSpecific<int64_t>(); break;
		case TR::iflucmpge: result = isReducibleSpecific<uint64_t>(); break;
		...
		default: TR_ASSERT(0, "unexpected opcode provided to isReducible()"); break;
		}
...
indirectLoadSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCodeValue() == TR::loadaddr) {
		bool newOType = false;
		TR::DataType loadDataType = newOType ? node->getDataType()
		...
		if (newOType)
			allowLoadSize = (loadSize == loadSymSize)
		...
		if (isSame && (node->getSymbol()->getSize() == firstChild->getSymbol()->getSize())
		        ...
		        && performTransformation(s->comp(),
		                "%sReplace indirect load %s [" POINTER_PRINTF_FORMAT "] with ", s->optDetailString(),
			...
			if (loadDataType != addrDataType) {
				uint8_t precision =
				        node->getType().isAddress() ? (TR::Compiler->target.is64Bit() ? 8 : 4) : 0;
			...
			dumpOptDetails(s->comp(),
			        "%s [" POINTER_PRINTF_FORMAT "] (load %s [" POINTER_PRINTF_FORMAT "])\n",
	...
	if (s->comp()->cg()->getSupportsVectorRegisters()) {
		...
		if ((node->getType().isIntegral() || node->getType().isDouble())
		        ...
		        && performTransformation(s->comp(),
		                "%sReplace indirect load [" POINTER_PRINTF_FORMAT "] with getvelem",
		                s->optDetailString(), node)) {
			int32_t offset = 0;
			...
			TR::SymbolReference *newSymRef = s->comp()->getSymRefTab()->createSymbolReference(
			        TR::Symbol::createShadow(
			                s->comp()->trHeapMemory(), addressNode->getSymbol()->getDataType()),
			        0);
...
indirectStoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCodeValue() == TR::loadaddr) {
		...
		TR::DataType storeDataType = newOType ? node->getDataType()
		                                      : node->getSymbolReference()->getSymbol()->getDataType();
		...
		if (newOType)
			allowStoreSize = (storeSize == storeSymSize)
		...
		if ((storeDataType == addrDataType && isSameType)
		        ...
		        && (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile())
		        && performTransformation(s->comp(),
		                "%sReplace indirect store %s [" POINTER_PRINTF_FORMAT "] with ", s->optDetailString(),
		                node->getOpCode().getName(), node)) {
			...
			dumpOptDetails(
			        s->comp(), "%s [" POINTER_PRINTF_FORMAT "]\n", node->getOpCode().getName(), node);
	...
	if ((node->getOpCodeValue() == TR::awrtbari) && 0
		...
		if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray)
			...
			if (removeWrtBar && !s->comp()->getOptions()->realTimeGC()
			        && performTransformation(s->comp(),
			                "%sFolded indirect write barrier to iastore because GC could not have occurred "
			                "enough times to require iwrtbar [" POINTER_PRINTF_FORMAT "]\n",
			                s->optDetailString(), node)) {
	...
	if (s->comp()->cg()->getSupportsVectorRegisters()) {
		...
		// Can convert to vsetelem if it is a scalar store to a vector, or it's an array ref to a vector with a
		// constant offset
		if ((node->getType().isIntegral() || node->getType().isDouble())
		        ...
		        && ((addressNode->getOpCode().isArrayRef()
		                ...
		                || (addressNode->getOpCode().hasSymbolReference()
		                        && addressNode->getSymbol()->getType().isVector()))
		        && performTransformation(s->comp(),
		                "%sReplace indirect store [" POINTER_PRINTF_FORMAT "] with vsetelem",
		                s->optDetailString(), node)) {
			int32_t offset = 0;
			TR::SymbolReference *oldSymRef = node->getSymbolReference();
			...
			TR::SymbolReference *newSymRef = s->comp()->getSymRefTab()->createSymbolReference(
			        ...
			        0);
			TR::Node *vloadiNode = TR::Node::createWithSymRef(TR::vloadi, 1, 1, addressNode, newSymRef);
			TR::Node *indexNode = TR::Node::iconst(offset / (node->getSize())); // idx = byte offset /
			                                                                    // element size
			TR::Node *vsetelemNode = TR::Node::create(TR::vsetelem, 3, vloadiNode, indexNode, valueNode);
			auto newVStorei =
			        TR::Node::createWithSymRef(TR::vstorei, 2, 2, addressNode, vsetelemNode, newSymRef);
			dumpOptDetails(s->comp(), "[" POINTER_PRINTF_FORMAT "]\n", newVStorei);
...
directStoreSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	// when we find an update form where the Yload has ref count > 1, we'd like to swing this tree down below the
	// node that uses it to avoid ugly register copying (because both the original and incremented value are
	// required) NOTE: this likely won't improve register pressure; it's purely a codegen quality improvement
		...
		bool firstLoadsSymRef = (firstGrandchild->getOpCode().isLoadVar()
		        && symRef == firstGrandchild->getSymbolReference()); // && firstGrandchild->getReferenceCount()
		                                                             // > 1);
		...
		if ((firstLoadsSymRef && secondIsLoadConst) || (firstIsLoadConst && secondLoadsSymRef)) {
			// found a candidate update form tree: now walk trees forward until either something stores to
			// <sym> or end of block remember tree where last use of node was on this walk: insert our
			// update tree following that last use tree

			// first, find the actual update tree (this is obviously our first potential insertion point for
			// the update tree since it's already here)
			TR::TreeTop *updateTree = block->getEntry();
			while (updateTree->getNode() != node) {
				updateTree = updateTree->getNextRealTreeTop();

				// if we reach the end of the block, then the update tree is probably under a ResolveCHK
				// node and we don't really want to move that
			...
			// only proceed if previous tree is a treetop holding an iload of <sym> (more specific pattern
			// match)
			...
			if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
				...
				if (loadNode->getOpCode().isLoadVarDirect()
					...
					// dumpOptDetails(s->comp(), "found load [" POINTER_PRINTF_FORMAT "] of symbol
					// in previous tree\n", loadNode);

					// now, walk trees forward to end of block or a tree that accesses <sym>
					// when we find a tree containing a use of <sym>, update insertion point with
					// tree following the use tree
					...
					// if we found a better insertion point, snip tree out of its original place and
					// insert it after insertionTree
					if (insertionTree != NULL
						...
						// eliminateDeadTrees might subsequently try to undo it by moving the
						// load below the update tree (bad pass!).  We'll allow it to do that
						// (because we can come along and re-swing the update below the load) so
						// long as it doesn't move the load under a branch tree.  If the load
						// gets moved into a branch tree, then there's no place we could put the
						// update tree that's below the load but still in the block (executive
						// summary: all our careful work above would be for naught).  So mark
						// the load here so that eliminateDeadTrees won't make us look (as?)
						// bad.
...
vsetelemSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	for (TR::Node *vsetelemNode = node; vsetelemNode && vsetelemNode->getOpCodeValue() == TR::vsetelem;
		...
		// If we've seen this index before, then the outer set element overrides the inner set element, so it's
		// safe to skip this node
...
iaddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (nodeOp == TR::iadd && secondChildOp == TR::iconst && secondChild->getInt() > 0) {
		if (performTransformation(s->comp(),
		            ...
		            node->getName(s->getDebug()))) {
			TR::Node *newSecondChild = (secondChild->getReferenceCount() == 1)
			        ? secondChild
	...
	// reduce imul operations by factoring
		...
		if (lrChild->getOpCodeValue() == TR::iconst) {
			if (secondChildOp == TR::iconst) {
				if (!s->reassociate() && // use new rules
				        performTransformation(s->comp(),
				                "%sFound iadd of iconst with iadd or isub of x and const in node "
				                "[%s]\n",
				                s->optDetailString(), node->getName(s->getDebug()))) {
					if (firstChild->getReferenceCount() > 1) {
						// it makes sense to uncommon it in this situation
		...
		if ((lrChild->getOpCodeValue() == TR::iconst)
	        ...
	        (node->getSecondChild()->getOpCode().isLoadConst()) && // iadd second child should be const to be pushed
	                                                               // down
		...
		//
		// n1.3083n   (  0)      iadd (in *GPR_6512) (profilingCode ) [0x2BB38260] loc=[-1,2848,1153] rc=0
		// vc=631 vn=- sti=- udi=14723 nc=2 flg=0x80 n1.3081n   (  0)        a2i (in *GPR_6512) (profilingCode
		// unneededConv )                               [0x2BB381C0] loc=[-1,2847,1153] rc=0 vc=631 vn=- sti=-
		// udi=14723 nc=1 addr=4 flg=0x8080 n1.26322n  (  0)          aiadd (in *GPR_6512) (profilingCode X>=0
		// internalPtr )                       [0x374931DC] loc=[-1,2844,1153] rc=0 vc=1859 vn=- sti=- udi=14723
		// nc=2 addr=4 flg=0x8180 n1.3077n   (  2)            ==>aload (in GPR_6513) (profilingCode ) n1.3086n
		// (  0)            ==>iconst 692 (profilingCode X!=0 X>=0 ) n1.3082n   (  0)        iconst -24
		// (profilingCode X!=0 X<=0 )                                          [0x2BB38210] loc=[-1,2848,1153]
		// rc=0 vc=631 vn=- sti=- udi=- nc=0 flg=0x284
		//
		...
		if ((node_aiadd->getSecondChild()->getOpCodeValue() == TR::iconst)
		        ...
		        performTransformation(s->comp(),
		                "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] iconst [%p] below\n",
			...
			if (passesPrecisionTest
			        && performTransformation(s->comp(),
			                "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] second child [%p], "
			                "creating new iadd\n",
			                s->optDetailString(), node, node->getSecondChild(), node_aiadd,
			                node_aiadd->getSecondChild())) {
...
laddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()
		...
		int64_t val1 = (firstChild->getDataType() == TR::Address) ? firstChild->getAddress()
		                                                          : firstChild->getLongInt();
		int64_t val2 = (secondChild->getDataType() == TR::Address) ? secondChild->getAddress()
		                                                           : secondChild->getLongInt();
	...
	if (node->getOpCodeValue() == TR::ladd && secondChildOp == TR::lconst && secondChild->getLongInt() > 0) {
		...
		{
			if (performTransformation(s->comp(),
			            "%sReduced ladd of -1 and an lneg in node [" POINTER_PRINTF_FORMAT "] to bitwise "
			            "complement\n",
			            s->optDetailString(), node)) {
				s->anchorChildren(node, s->_curTree);
		...
		// Turn add with first child negated into a subtract
			...
			if (performTransformation(s->comp(),
			            "%sReduced ladd with negated first child in node [" POINTER_PRINTF_FORMAT "] to "
			            "lsub\n",
			            s->optDetailString(), node)) {
				s->anchorChildren(node, s->_curTree);
				TR::Node::recreate(node, TR::lsub);
	...
	// reduce imul operations by factoring
		...
		if (secondChildOp == TR::lconst) {
			...
			if (lrChild->getOpCodeValue() == TR::lconst) {
				if (performTransformation(s->comp(),
				            "%sFound ladd of lconst with ladd or lsub of x and const in node "
			...
			} else // move constants up the tree so they will tend to get merged together
			{
				if ((firstChild->getReferenceCount() == 1)
				        && performTransformation(s->comp(),
				                "%sFound ladd of non-lconst with ladd or lsub of x and lconst in node "
				                "[" POINTER_PRINTF_FORMAT "]\n",
				                s->optDetailString(), node)) {
					node->setChild(1, lrChild);
					firstChild->setChild(1, secondChild);
	...
	} else if (node->getOpCode().isArrayRef() && firstChild->getOpCode().isArrayRef()
...
faddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldFloatConstant(
		        node, TR::Compiler->arith.floatAddFloat(firstChild->getFloat(), secondChild->getFloat()), s);
		return node;
	}
...
daddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldDoubleConstant(node,
		        TR::Compiler->arith.doubleAddDouble(firstChild->getDouble(), secondChild->getDouble()), s);
		return node;
	}
...
isubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	// drives inegs up the tree
		...
		if (performTransformation(s->comp(),
		            "%sReduced isub with negated first child in node [%s] to ineg of iadd\n",
		            s->optDetailString(), node->getName(s->getDebug()))) {
			TR::Node *newFirstChild = firstChild->getFirstChild();
			...
			if (secondChildOp == TR::iconst) {
				if (performTransformation(s->comp(),
			       ...
			       //   to fix register pressure problems thta result
			        if ((firstChild->getReferenceCount() == 1)
			                && (performTransformation(s->comp(),
			                        "%sFound isub of non-iconst with iadd or isub of x and const in node "
			                        "[%s]\n",
			                        s->optDetailString(), node->getName(s->getDebug())))) {
				// move constants up the tree so they will tend to get merged together
				node->setChild(1, lrChild);
...
lsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		...
		foldLongIntConstant(
		        node, firstChild->getLongInt() - secondChild->getLongInt(), s, false /* !anchorChildren */);
	...
	// drives lnegs up the tree
		...
		if (performTransformation(s->comp(),
		            "%sReduced lsub with negated first child in node [" POINTER_PRINTF_FORMAT "] to lneg of "
		            "ladd\n",
		            s->optDetailString(), node)) {
			TR::Node *newFirstChild = firstChild->getFirstChild();
	...
	{
		if (performTransformation(s->comp(),
		            "%sReduced lsub of bitwise complement and lconst -1 in node [" POINTER_PRINTF_FORMAT "] to "
		            "2s "
		            "compl"
		            "ement"
		            " nega"
	...
	// reduce imul operations by factoring
		...
		if (secondChildOp == TR::lconst) {
			...
			if (lrChild->getOpCodeValue() == TR::lconst) {
				if (performTransformation(s->comp(),
				            "%sFound lsub of lconst with ladd or lsub of x and lconst in node "
				            "[" POINTER_PRINTF_FORMAT "]\n",
				            s->optDetailString(), node)) {
					if (firstChild->getReferenceCount() > 1) {
						// it makes sense to uncommon it in this situation
			...
			} else // move constants up the tree so they will tend to get merged together
			{
				if ((firstChild->getReferenceCount() == 1)
				        && performTransformation(s->comp(),
				                "%sFound lsub of non-lconst with ladd or lsub of x and lconst in node "
				                "[" POINTER_PRINTF_FORMAT "]\n",
				                s->optDetailString(), node)) {
					node->setChild(1, lrChild);
					firstChild->setChild(1, secondChild);
	...
	// second operand is smaller than the first.
		...
		if ((lSubValue & 0xFFFFFFFF) == lSubValue && (lSubValue & (lSubValue + 1)) == 0
		        && lAndValue <= lSubValue) {
			TR::Node::recreate(node, TR::lxor);
			node->setVisitCount(0);
			s->_alteredBlock = true;
	...
	// 184251
		...
		if (shftAmnt > 0
			...
			if ((iValue << shftAmnt) == (int32_t)(lValue << shftAmnt)
			        && performTransformation(s->comp(),
			                "%sFound lsub/lmul of +ve powOf2 lconst with i2l of iadd or isub of x and "
			...
			if (secondChildOp == TR::lconst) {
				...
				if (lrChild->getOpCodeValue() == TR::iconst) {
					if (node->isNonNegative() && i2lNode->isNonNegative()) {
						// need to ensure no overflow happens on current isub operation to be
						// able to make this transformation we can loosen this up in the future
						// if we have more information on children nodes but not the parents
						...
						if (performTransformation(s->comp(),
							...
							if (firstChild->getReferenceCount() > 1) {
								...
								TR::Node *newFirstChild = TR::Node::create(
								        node->getFirstChild(), firstChildOp, 2);
								newFirstChild->setAndIncChild(
								        0, firstChild->getFirstChild());
								newFirstChild->setAndIncChild(
								        1, firstChild->getSecondChild());
								firstChild->recursivelyDecReferenceCount();
...
fsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldFloatConstant(node,
		        TR::Compiler->arith.floatSubtractFloat(firstChild->getFloat(), secondChild->getFloat()), s);
		return node;
	}
...
imulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		if (node->getOpCode().isUnsigned()) {
			...
			foldIntConstant(
			        node, firstChild->getInt() * secondChild->getInt(), s, false /* !anchorChildren*/);
		}
		return node;
	}
...
lmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
		...
		if (lrChild->getOpCodeValue() == TR::lconst) {
			if (secondChildOp == TR::lconst) {
				if (performTransformation(s->comp(),
				            "%sFound lmul of lconst with lmul of x and lconst in node "
				            "[" POINTER_PRINTF_FORMAT "]\n",
				            s->optDetailString(), node)) {
					if (secondChild->getReferenceCount() == 1) {
						secondChild->setLongInt(
						        secondChild->getLongInt() * lrChild->getLongInt());
						setIsHighWordZero(secondChild, s);
					} else {
						TR::Node *foldedConstChild =
						        TR::Node::create(secondChild, TR::lconst, 0);
						node->setAndIncChild(1, foldedConstChild);
						foldedConstChild->setLongInt(
						        secondChild->getLongInt() * lrChild->getLongInt());
						secondChild->recursivelyDecReferenceCount();
			...
			{
				if (performTransformation(s->comp(),
				            "%sFound lmul of non-lconst with lmul of x and lconst in node "
				            "[" POINTER_PRINTF_FORMAT "]\n",
				            s->optDetailString(), node)) {
					node->setChild(1, lrChild);
					firstChild->setChild(1, secondChild);
		...
		if (lrChild->getOpCodeValue() == TR::lconst
		        && performTransformation(s->comp(),
		                "%sDistributed lmul with lconst over lsub or ladd with lconst in node "
		                "[" POINTER_PRINTF_FORMAT "]\n",
		                s->optDetailString(), node)) {
	...
	// the simplification is then invalid
		...
		if (((i2lNode->getFirstChild()->getOpCodeValue() == TR::isub)
			...
			if (lrChild->getOpCodeValue() == TR::iconst
			        && performTransformation(s->comp(),
			                "%sDistributed lmul with lconst over isub or iadd of with iconst in node "
			                "[" POINTER_PRINTF_FORMAT "]\n",
			                s->optDetailString(), node)) {
...
fmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldFloatConstant(node,
		        TR::Compiler->arith.floatMultiplyFloat(firstChild->getFloat(), secondChild->getFloat()), s);
		return node;
	}
...
dmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (s->comp()->getOption(TR_IgnoreIEEERestrictions) && !node->isFPStrictCompliant()
	        && !s->comp()->cg()->supportsNegativeFusedMultiplyAdd()) // we can create x*1.0 when negFma is
	                                                                 // available.  Don't undo it
	{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (divisor != 0) {
			...
			// Design 1790
			        ...
			        && (secondChild->getReferenceCount() == 1)
			        && performTransformation(
			                s->comp(), "%sPwr of 2 idiv opt node %p\n", s->optDetailString(), node)) {
				secondChild->decReferenceCount();
				TR::Node *newNode1;
				if (shftAmnt - 1 != 0) {
					...
					newNode1->setSecond(
					        TR::Node::create(node, TR::iconst, 0, shftAmnt - 1)); // shift
					                                                              // amount,
					                                                              // test
					                                                              // for
					                                                              // 0
				...
				newNode2->setSecond(TR::Node::create(node, TR::iconst, 0, 32 - shftAmnt)); // shift
				                                                                           // amount,
				                                                                           // test for 0
				...
				node->getFirstChild()->incReferenceCount();
			        ...
			        && performTransformation(s->comp(), "%sMagic number idiv opt in node %p\n",
			                s->optDetailString(), node)) {
				// leave idiv as is if the divisor is 2^n. CodeGen generates a fast instruction squence
				// for it. otherwise, expose the magic number squence to allow optimization lowered tree
				// will look like this: iadd (node)
				//   ishr (node3)
				//     iadd (node2)
				//       iload [ dividend ]
				//       imulh (node1)
...
ldivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
		...
		if (divisor != 0) {
			if (firstChild->getOpCode().isLoadConst()) {
				...
				if (firstChild->isNonNegative()) {
					...
					if (divisor < 0) {
						if (performTransformation(s->comp(),
						            "%sReduced ldiv power of 2 - neg nominator with lshr lneg "
						            "in node [%p]\n",
						            s->optDetailString(), node)) {
						...
						if (performTransformation(s->comp(),
						            "%sReduced ldiv power of 2 - neg nominator with lneg lneg "
						            "in node [%p]\n",
						            s->optDetailString(), node)) {
							if (s->cg()->canUseImmedInstruction(divisor - 1)) {
								...
								node->setAndIncChild(0,
								        TR::Node::create(
								                TR::lshr, 2, firstChild, shiftNode));
								...
								node_and = TR::Node::create(
								        TR::land, 2, firstChild, node_divisor_1);

								node_add = TR::Node::create(
								        TR::ladd, 2, node_and, node_divisor_1);
								...
								node_const0or1 =
								        TR::Node::create(TR::lneg, 1, node_sign);
				...
				// Design 1790
				        ...
				        && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(divisor))
				                > 0)
				        && (secondChild->getReferenceCount() == 1)
	...
	if (node->getOpCodeValue() == TR::ldiv) {
		...
		if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10
		        ...
		        && // transformToLongDiv creates a tonne of nodes.  Not good to do this recursively if you have
		           // chained divides
		        performTransformation(
...
fdivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getFloatBits() != FLOAT_POS_ZERO
		...
		if (firstChild->getOpCode().isLoadConst()) {
			foldFloatConstant(node,
			        TR::Compiler->arith.floatDivideFloat(firstChild->getFloat(), secondChild->getFloat()),
			        s);
			return node;
		}
		...
		if (divisor != 0) {
			if (divisor == 1 || (!isUnsigned && (divisor == -1))) {
				...
				return node;
			        ...
			        && (secondChild->getReferenceCount() == 1)
			        && performTransformation(
			                s->comp(), "%sPwr of 2 irem opt node %p\n", s->optDetailString(), node)) {
			...
			} else if (node->getOpCodeValue() == TR::irem && s->cg()->getSupportsLoweringConstIDiv()
			        && !isPowerOf2(divisor) && !skipRemLowering(divisor, s)
			        && performTransformation(s->comp(), "%sMagic number irem opt in node %p\n",
			                s->optDetailString(), node)) {
				// leave irem as is if the divisor is 2^n. CodeGen generates a fast instruction squence
				// for it. otherwise, expose the magic number squence to allow optimization lowered tree
				// will look like this:
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (divisor != 0) {
			...
			// Design 1792
			        ...
			        && (secondChild->getReferenceCount() == 1)
			        && performTransformation(
			                s->comp(), "%sPwr of 2 lrem opt node %p\n", s->optDetailString(), node)) {
				secondChild->decReferenceCount();
				TR::Node *newNode1;
				if (shftAmnt - 1 != 0) {
					newNode1 = TR::Node::create(node, TR::lshr, 2);
...
dremSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldDoubleConstant(node,
		        TR::Compiler->arith.doubleRemainderDouble(firstChild->getDouble(), secondChild->getDouble()),
		        s);
		return node;
	}
	...
	// don't just do -A op B -> -(A op B) as not always a win, so catch opportunity here
		...
		if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
			...
			newMul->setIsFPStrictCompliant(true);
		        ...
		        && performTransformation(s->comp(),
		                "%sTransforming [" POINTER_PRINTF_FORMAT "] -(A*B) -> -((A*B)-0)\n",
		                s->optDetailString(), node)) {
			TR::Node *newConst = TR::Node::create(firstChild, TR::fconst, 0);
			newConst->setFloat(0);
...
ilfdabsSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	static bool AllowFlagBasedTransformationForFloatOrDouble = feGetEnv("TR_"
	                                                                    "AllowFlagBasedTransformationForFloatOrDoub"
	                                                                    "le")
	        != NULL;
	...
	case TR::dabs:
		...
		performFlagBasedTransformation = AllowFlagBasedTransformationForFloatOrDouble;
		break;
	default: TR_ASSERT(0, "unexpected opcode %d\n", node->getOpCodeValue());
	}
...
lshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		//
		performTransformation(s->comp(),
		        "%sCanonicalize long left shift by constant in node [" POINTER_PRINTF_FORMAT "] to long "
		        "multiply by "
		        "power of 2\n",
		        s->optDetailString(), node);
		TR::Node::recreate(node, TR::lmul);
		...
		if (constGrandChild->getOpCodeValue() == TR::iconst
			...
			if (subTree->getReferenceCount() == 1) {
				if (opCode == TR::s2i && rightShiftValue == 16) {
					if (performTransformation(s->comp(),
					            "%sReduced left shift followed by iushr equivalent to zero extend "
					            "short in node [" POINTER_PRINTF_FORMAT "] to su2i\n",
					            s->optDetailString(), node)) {
					...
					if (performTransformation(s->comp(),
					            "%sReduced left shift followed by iushr equivalent to zero extend "
					            "byte in node [" POINTER_PRINTF_FORMAT "] to bu2i\n",
					            s->optDetailString(), node)) {
			...
			//
			if (performTransformation(s->comp(),
			            "%sReduced left shift followed by iushr in node [" POINTER_PRINTF_FORMAT "] to "
			            "iand "
			            "with "
			            "mask\n",
			            s->optDetailString(), node)) {
...
lushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldLongIntConstant(node,
		        ((uint64_t)firstChild->getLongInt()) >> (secondChild->getInt() & LONG_SHIFT_MASK), s,
		        false /* !anchorChildren */);
		return node;
	}
		...
		if (constGrandChild->getOpCodeValue() == TR::lconst
			...
			if (subTree->getReferenceCount() == 1) {
				if (opCode == TR::i2l && rightShiftValue == 32) {
					if (performTransformation(s->comp(),
					            "%sReduced left shift followed by lushr equivalent to zero extend "
					            "int in node [" POINTER_PRINTF_FORMAT "] to iu2l\n",
					            s->optDetailString(), node)) {
						foundZeroExtension = true;
					...
					if (performTransformation(s->comp(),
					            "%sReduced left shift followed by lushr equivalent to zero extend "
					            "byte in node [" POINTER_PRINTF_FORMAT "] to bu2l\n",
					            s->optDetailString(), node)) {
						foundZeroExtension = true;
			...
			//
			if (performTransformation(s->comp(),
			            "%sReduced left shift followed by lushr in node [" POINTER_PRINTF_FORMAT "] to "
			            "land "
			            "with "
			            "mask\n",
			            s->optDetailString(), node)) {
...
sushrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldShortIntConstant(node,
		        firstChild->getUnsignedShortInt() >> (secondChild->getInt() & INT_SHIFT_MASK), s,
		        false /* !anchorChildren */);
		return node;
	}
	BINARY_IDENTITY_OP(Int, 0)
		...
		if (isBitwiseIntComplement(firstChild) && secondChild->getReferenceCount() == 1
			...
			if (performTransformation(s->comp(),
				...
				TR::Node *orNode = TR::Node::create(
				        TR::ior, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
				...
				if (secondChildOp == TR::iconst
					...
					if (performTransformation(s->comp(),
						...
						if (secondChild->getReferenceCount() == 1) {
							...
							TR::Node *foldedConstChild =
							        TR::Node::create(secondChild, TR::iconst, 0);
							foldedConstChild->setInt(
							        secondChild->getInt() & lrChild->getInt());
							node->setSecond(s->replaceNode(
							        secondChild, foldedConstChild, s->_curTree));
						}
	...
	if (firstChild->getReferenceCount() == 1) {
		if (isBitwiseLongComplement(firstChild) && secondChild->getReferenceCount() == 1
			...
			if (performTransformation(s->comp(),
			            ...
			            s->optDetailString(), node)) {
				TR::Node *orNode = TR::Node::create(
				        TR::lor, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
				TR::Node *constNode = firstChild->getSecondChild();
				TR::Node::recreate(node, TR::lxor);
				node->setAndIncChild(0, orNode);
				node->setAndIncChild(1, constNode);
		...
		// look for reassociation to fold constants
			...
			if (lrChild->getOpCodeValue() == TR::lconst) {
				if (secondChildOp == TR::lconst) {
					if (performTransformation(s->comp(),
					            "%sFound land of lconst with land of x and lconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						if (secondChild->getReferenceCount() == 1) {
							secondChild->setLongInt(
				...
				{
					if (performTransformation(s->comp(),
					            "%sFound land of non-lconst with land of x and lconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						node->setChild(1, lrChild);
						firstChild->setChild(1, secondChild);
		...
		// look for opportunities to perform zero extending conversions
			...
			if (maskValue == 255 && firstChildOp == TR::b2i) {
				if (performTransformation(s->comp(),
				            "%sReduced land with lconst 255 in node [" POINTER_PRINTF_FORMAT "] to "
				            "bu2l\n",
				            s->optDetailString(), node)) {
					TR::Node::recreate(node, TR::bu2l);
					foundZeroExtension = true;
				...
				if (performTransformation(s->comp(),
				            "%sReduced land with lconst 65536 in node [" POINTER_PRINTF_FORMAT "] to "
				            "su2l\n",
				            s->optDetailString(), node)) {
					TR::Node::recreate(node, TR::su2l);
					foundZeroExtension = true;
				...
				if (performTransformation(s->comp(),
				            "%sReduced land with lconst 0xffffffff in node [" POINTER_PRINTF_FORMAT
				            "] to iu2l\n",
				            s->optDetailString(), node)) {
					TR::Node::recreate(node, TR::iu2l);
					foundZeroExtension = true;
	...
	if (node->getOpCodeValue() == TR::land && secondChild->getOpCodeValue() == TR::lconst
		...
		if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
			...
			if (firstChildOp == TR::iu2l
			        && performTransformation(s->comp(),
			                "%sReduced land with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT
			                "] to iand\n",
			                s->optDetailString(), node)) {
				...
				TR::Node *iandChild =
				        TR::Node::create(TR::iand, 2, firstChild->getFirstChild(), constChild);
				TR::Node::recreate(node, firstChildOp);
				node->setNumChildren(1);
...
bool
isChildOrConstRedundant(TR::Node *parentOrConstNode, TR::Node *childOrConstNode, TR::Simplifier *s)
{
	...
	// if the result of or'ing the two or constants is the same as the parent (int or) constant then can remove
	// child oor operation
...
//
TR::Node *
removeRedundantIntegralOrPattern1(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (!performTransformation(s->comp(),
	            ...
	            childOrConstNode->get64bitIntegralValueAsUnsigned(), childOr, parentOr->getOpCode().getName(),
...
//
TR::Node *
removeRedundantIntegralOrPattern2(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	TR::Node *parentOr = node;
	TR::Node *firstChild = parentOr->getFirstChild();
	...
	dumpOptDetails(s->comp(),
	        "%sCreate new zero extension conversion %s [" POINTER_PRINTF_FORMAT "] of childOr child %s "
	        "[" POINTER_PRINTF_FORMAT "]\n",
	...
	if (firstChild->getReferenceCount() == 1) {
		if (isBitwiseIntComplement(firstChild) && secondChild->getReferenceCount() == 1
			...
			if (performTransformation(s->comp(),
			            ...
			            "\n",
			            s->optDetailString(), node)) {
				TR::Node *andNode = TR::Node::create(
				        TR::iand, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
				TR::Node *constNode = firstChild->getSecondChild();
				TR::Node::recreate(node, TR::ixor);
			...
			if (lrChild->getOpCodeValue() == TR::iconst) {
				if (secondChildOp == TR::iconst) {
					...
					if ((((~and_const) | or_const) == or_const)
					        && performTransformation(s->comp(),
					                "%sFound ior of iconst with iand of x and iconst in node "
					                "[" POINTER_PRINTF_FORMAT "]\n",
					                s->optDetailString(), node)) {
						node->setAndIncChild(0, firstChild->getFirstChild());
						firstChild->recursivelyDecReferenceCount();
		...
		// look for reassociation to fold constants
			...
			if (lrChild->getOpCodeValue() == TR::iconst) {
				if (secondChildOp == TR::iconst) {
					if (performTransformation(s->comp(),
					            "%sFound ior of iconst with ior of x and iconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						if (secondChild->getReferenceCount() == 1) {
							secondChild->setInt(secondChild->getInt() | lrChild->getInt());
				...
				{
					if (performTransformation(s->comp(),
					            "%sFound ior of non-iconst with ior x and iconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						node->setChild(1, lrChild);
						firstChild->setChild(1, secondChild);
...
lorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getReferenceCount() == 1) {
		if (isBitwiseLongComplement(firstChild) && secondChild->getReferenceCount() == 1
			...
			if (performTransformation(s->comp(),
			            ...
			            s->optDetailString(), node)) {
				TR::Node *andNode = TR::Node::create(
				        TR::land, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
				TR::Node *constNode = firstChild->getSecondChild();
				TR::Node::recreate(node, TR::lxor);
				node->setAndIncChild(0, andNode);
				node->setAndIncChild(1, constNode);
		...
		// look for reassociation to fold constants
			...
			if (lrChild->getOpCodeValue() == TR::lconst) {
				if (secondChildOp == TR::lconst) {
					if (performTransformation(s->comp(),
					            "%sFound lor of lconst with lor of x and lconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						if (secondChild->getReferenceCount() == 1) {
							secondChild->setLongInt(
				...
				{
					if (performTransformation(s->comp(),
					            "%sFound lor of non-lconst with lor of x and lconst in node "
					            "[" POINTER_PRINTF_FORMAT "]\n",
					            s->optDetailString(), node)) {
						node->setChild(1, lrChild);
						firstChild->setChild(1, secondChild);
	...
	if (node->getOpCodeValue() == TR::lor && secondChild->getOpCodeValue() == TR::lconst
		...
		if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
			...
			if (firstChildOp == TR::iu2l
			        && performTransformation(s->comp(),
			                "%sReduced lor with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT "] "
			                "to "
			                "ior"
			                "\n",
			                s->optDetailString(), node)) {
...
ixorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChildOp == TR::ixor && firstChild->getReferenceCount() == 1) {
		...
		if (lrChild->getOpCodeValue() == TR::iconst) {
			...
			{
				if (performTransformation(s->comp(),
				            "%sFound ixor of non-iconst with ixor x and iconst in node [%s]\n",
				            s->optDetailString(), node->getName(s->getDebug()))) {
					node->setChild(1, lrChild);
					firstChild->setChild(1, secondChild);
					node->setVisitCount(0);
...
lxorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChildOp == TR::lxor && firstChild->getReferenceCount() == 1) {
		...
		if (lrChild->getOpCodeValue() == TR::lconst) {
			if (secondChildOp == TR::lconst) {
				if (performTransformation(s->comp(),
				            "%sFound lxor of lconst with lxor of x and lconst in node "
				            "[" POINTER_PRINTF_FORMAT "]\n",
				            s->optDetailString(), node)) {
					if (secondChild->getReferenceCount() == 1) {
						secondChild->setLongInt(
			...
			{
				if (performTransformation(s->comp(),
				            "%sFound lxor of non-lconst with lxor x and lconst in node "
				            "[" POINTER_PRINTF_FORMAT "]\n",
				            s->optDetailString(), node)) {
					node->setChild(1, lrChild);
					firstChild->setChild(1, secondChild);
	...
	if (node->getOpCodeValue() == TR::lxor && secondChild->getOpCodeValue() == TR::lconst
		...
		if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
			...
			if (firstChildOp == TR::iu2l
			        && performTransformation(s->comp(),
			                "%sReduced lxor with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT
			                "] to ixor\n",
			                s->optDetailString(), node)) {
				TR::Node *constChild = NULL;
				...
				TR::Node *ixorChild =
				        TR::Node::create(TR::ixor, 2, firstChild->getFirstChild(), constChild);
				TR::Node::recreate(node, firstChildOp);
				node->setNumChildren(1);
...
sqrtSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (child->getOpCode().isLoadConst()
	        && performTransformation(s->comp(), "%sSimplify sqrt of const child at [" POINTER_PRINTF_FORMAT "]\n",
	                s->optDetailString(), node)) {
		foldConstant<T>(
		        node, TR::Compiler->arith.fpSquareRoot(child->getConst<T>()), s, false /* !anchorChilren */);
	}
	return node;
}
...
i2bSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		foldByteConstant(node,
		        node->getOpCode().isUnsigned() ? (firstChild->getInt() & 0xff)
...
i2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if ((firstChild->getOpCodeValue() == TR::isub || firstChild->getOpCodeValue() == TR::iadd)
	        && (s->comp()->cg()->supports32bitAiadd() || node->isNonNegative()) && // todo: do we know NonNegativity
	                                                                               // in simplifier?
...
l2fSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCodeValue() == TR::lconst) {
		...
		uint64_t absValue = firstChild->getLongInt() >= 0 ? firstChild->getLongInt()
		                                                  : -firstChild->getLongInt();
...
l2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if ((firstChild->getOpCodeValue() == TR::lsub || firstChild->getOpCodeValue() == TR::ladd)
		...
		if (!(addressAddOp == TR::aladd && !is64Bit) && // aladd only allowed in 64bit
			...
			node->setNumChildren(2); // upto 2 children do not require node extension. this should suffice.
			                         // Any more requires Node::addChildren
			node->setAndIncChild(0, firstChild->getFirstChild()->getFirstChild()); // aload

			int64_t newVal = (firstChild->getOpCodeValue() == TR::lsub) ? -constChild->getLongInt()
			                                                            : constChild->getLongInt();
			constChild =
			        TR::Node::create(constChild, (addressAddOp == TR::aladd) ? TR::lconst : TR::iconst, 0);
...
f2bSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		foldByteConstant(
		        node, (floatToInt(firstChild->getFloat(), false) << 24) >> 24, s, false /* !anchorChildren*/);
		return node;
	}
...
d2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		if (node->getOpCode().isUnsigned()) {
			...
			foldIntConstant(
			        node, doubleToInt(firstChild->getDouble(), false), s, false /* !anchorChildren */);
		}
		return node;
	}
...
bu2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCodeValue() == TR::i2b && firstChild->getFirstChild()->getOpCodeValue() == TR::iand
		...
		if (((val & 0xFFUL) == 0UL)
			...
			// if we're going to zero extend a zero, then we can just replace this entire expression with a
			// 0
			s->anchorNode(firstChild->getFirstChild()->getFirstChild(), s->_curTree); // only really need to
			                                                                          // anchor this child,
			                                                                          // everything else is
			                                                                          // guaranteed to not
			                                                                          // be a load
			...
			bool anchorChildren = false; // no loads are being removed (just bu2i/i2b) so there is nothing
		...
		TR::Node *grandChild = firstChild->getFirstChild();
		grandChild->incReferenceCount();
		bool anchorChildren = false; // no loads are being removed (just bu2i/i2b) so there is nothing that
		                             // needs anchoring
		s->prepareToStopUsingNode(node, s->_curTree, anchorChildren);
		s->prepareToStopUsingNode(firstChild, s->_curTree, anchorChildren);
		node->recursivelyDecReferenceCount();
		...
		if (childOp == TR::i2s) {
			...
			if (firstGrandChild->getReferenceCount() == 1
				...
				if (performTransformation(s->comp(),
				            ...
				            "] and i2s child [" POINTER_PRINTF_FORMAT "] to no-op\n",
...
removeArithmeticsUnderIntegralCompare(TR::Node *node, TR::Simplifier *s)
{
	...
	if ((isAddOp || isSubOp) && isSignednessTypeSupported && opNode->getSecondChild()->getOpCode().isLoadConst()
		...
		if (opNode->getOpCode().is1Byte()) {
			...
			if (s->trace())
				traceMsg(s->comp(),
				        "\nEliminating add/sub under compare node n%dn failed due to opcode data "
				        "type\n",
				        node->getGlobalIndex());
		...
		if (!isEqOrNotEqCompare) {
			...
			// arithmnetic operations are defined. Need to make sure that signed integer results don't
			// overflow.
			...
			if (!(canTransformAdd || canTransformSub)) {
				if (s->trace())
					traceMsg(s->comp(),
					        "\nEliminating add/sub under order compare node n%dn failed due to "
					        "overflow\n",
					        node->getGlobalIndex());
		...
		// The IL expects integer wrapping in case of overflow; and this matches unsigned integer behaviors in
		// C++.
...
ificmpeqSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isRightShift() && // First child is a right shift
	        ...
	        && firstChild->getSecondChild()->getInt() >= 0 && // Shift value is a positive constant of reasonable
	                                                          // size
		...
		TR::Node *newSecondChild =
		        TR::Node::create(node, TR::iconst, 0, 1 << firstChild->getSecondChild()->getInt());
		node->setAndIncChild(1, newSecondChild);
	...
	if (firstChild->getOpCode().isBooleanCompare() && (secondChild->getOpCode().isLoadConst())
	        ...
	        && performTransformation(s->comp(),
	                "%sChanging if opcode %p because first child %p is a comparison opcode\n", s->optDetailString(),
	                node, firstChild)) {
...
ificmpltSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (node->getOpCodeValue() == TR::ificmplt) {
		...
		if (firstChild->getOpCode().isLoadConst()
		        && conditionalBranchFold(
		                (originalFirstVal < originalSecondVal), node, firstChild, secondChild, block, s))
			return node;
...
ificmpleSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (node->getOpCodeValue() == TR::ificmple) {
		if (firstChild->getOpCode().isLoadConst()
		        && conditionalBranchFold((originalFirst->getInt() <= originalSecond->getInt()), node,
...
iflcmpltSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (node->getOpCodeValue() == TR::iflcmplt) {
		if (firstChild->getOpCode().isLoadConst()
		        && conditionalBranchFold((firstChild->getLongInt() < secondChild->getLongInt()), node,
		                firstChild, secondChild, block, s))
			return node;
		...
		if (firstChild->getOpCode().isLoadConst()
		        && conditionalBranchFold(
		                (((uint64_t)originalFirst->getLongInt()) < ((uint64_t)originalSecond->getLongInt())),
...
iflcmpleSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (node->getOpCodeValue() == TR::iflcmple) {
		if (firstChild->getOpCode().isLoadConst()
		        && conditionalBranchFold((originalFirst->getLongInt() <= originalSecond->getLongInt()), node,
		                firstChild, secondChild, block, s))
			return node;
...
ifacmpeqSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCodeValue() == TR::aconst
	        && conditionalBranchFold((firstChild->getAddress() == secondChild->getAddress()), node, firstChild,
	                secondChild, block, s))
		return node;
...
ifCmpWithEqualitySimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		...
		default: TR_ASSERT(0, "unexpected opcode %d\n", node->getOpCodeValue()); foldBranch = false;
		}
	...
	if (!disableFoldIfSet && node->getOpCodeValue() == TR::ifbcmpeq) {
		if (secondChild->getOpCodeValue() == TR::bconst && secondChild->getByte() == 0
		        ...
		        && firstChild->getOpCode().isBranch() == false && firstChild->getReferenceCount() == 1) {
			TR::ILOpCodes op = firstChild->getOpCode().convertCmpToIfCmp();
			if (op != TR::BadILOp
			        && performTransformation(s->comp(),
			                "%sFolding ifbcmpeq of bconst 0 to boolean compare at node "
			                "[" POINTER_PRINTF_FORMAT "] to equivalent if?cmp??\n",
			                s->optDetailString(), node)) {
				TR::Node::recreate(node, op);
				TR::Node::recreate(node, node->getOpCode().getOpCodeForReverseBranch());
				secondChild->recursivelyDecReferenceCount();
...
TR::Node *
ifCmpWithoutEqualitySimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	if (removeIfToFollowingBlock(node, block, s) == NULL)
		return NULL;
	simplifyChildren(node, block, s);
...
icmpeqSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if ((firstChild->getOpCode().isBooleanCompare() && firstChild->getOpCode().isInteger())
	        && ((secondChild->getOpCode().isLoadConst()) && secondChild->getInt() == 0)
	        && performTransformation(s->comp(),
	                "%sChanging icmpeq opcode %p because first child %p is an int compare\n", s->optDetailString(),
	                node, firstChild)) {
	...
	if (node->getOpCode().isSignum() && secondChild->getOpCode().isLoadConst()) {
		...
		if (secondChild->getInt() == -1) {
			// Change to an Xcmpl
			switch (firstChild->getOpCodeValue()) {
			case TR::lcmp: newOpCode = TR::lcmplt; break;
			case TR::icmp: newOpCode = TR::icmplt; break;
			case TR::scmp: newOpCode = TR::scmplt; break;
			case TR::bcmp: newOpCode = TR::bcmplt; break;
			default: break;
			}
		} else if (secondChild->getInt() == 0) {
			// Change to a Xcmpeq
			switch (firstChild->getOpCodeValue()) {
			case TR::lcmp: newOpCode = TR::lcmpeq; break;
			case TR::icmp: newOpCode = TR::icmpeq; break;
			case TR::scmp: newOpCode = TR::scmpeq; break;
			case TR::bcmp: newOpCode = TR::bcmpeq; break;
			default: break;
			}
		} else if (secondChild->getInt() == 1) {
			// Change to a Xcmpgt
			switch (firstChild->getOpCodeValue()) {
			case TR::lcmp: newOpCode = TR::lcmpgt; break;
			case TR::icmp: newOpCode = TR::icmpgt; break;
			case TR::scmp: newOpCode = TR::scmpgt; break;
			case TR::bcmp: newOpCode = TR::bcmpgt; break;
			default: break;
			}
		} else {
			// Change to a false
		...
		if (newOpCode != TR::BadILOp
		        && performTransformation(s->comp(),
		                "%sChanging icmpeq opcode %p because first child %p is an %s opcode\n",
...
endBlockSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (cfg) {
		...
		if (rootStructure != NULL) {
			...
			// This is inconsistent with what we do for CFG in the following, it's better for mergeBlock to
			// merge nextBlock to block, this is a TODO
...
a2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		if (firstChild->getType().isAddress()) {
			// on Z the address cast evaluation of a2l of a 4 byte addr child will always zero out bit 32,
			// need to do equivalently here
			if (TR::Compiler->target.cpu.isZ() && node->getFirstChild()->getSize() == 4)
				foldLongIntConstant(
				        node, firstChild->getAddress() & CLEARBIT32, s, false /* !anchorChildren */);
			else
...
//
TR::Node *
switchSimplifier(TR::Node *node, TR::Block *block, bool isTableSwitch, TR::Simplifier *s)
{
	simplifyChildren(node, block, s);
	...
	if (firstChild->getOpCode().isLoadConst()) {
			...
			for (caseIndex = numCases + 1; caseIndex >= 1; caseIndex--) {
				...
				if (dest != target) {
					...
					if (!removedTargets.get(destBlock->getNumber())
					        && performTransformation(s->comp(),
					                "%sRemoving edge block_%d->block_%d in CFG because switch "
					                "value is known\n",
		...
		// If the first child is a shift node (possibly with a conversion node as a parent), we can eliminate
		// the shift by performing the reverse shift on all the case values (eg. instead of left-shifting the
		// first child by two, right-shift all the case values by two instead).
		...
		if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode
			...
			if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
				// In order for the transformation to be accurate, any bits shifted out of either the
				// selector node or the case constants must be 0. For the constants, we can check the
				// bits to be shifted out at compile time. For the shift node, which is non-constant,
				// the only way we know those bits are 0 is if they are explicitly zeroed with an AND
				// node.
				//
				// Example: if the selector is 3 (binary 11) and we shift it right 1, and have case
				// values 0 and 1, we'll match 1 (since 11b >> 1 is 1b) If we shift 0 and 1 left 1 bit,
				// we'll compare 11b to 00b and 10b, and not have a match.
				...
				if (!canTransform
				        || !performTransformation(s->comp(),
				                ...
				                "] from lookup node [" POINTER_PRINTF_FORMAT "], applying shift to "
				                "case constants\n",
				...
				for (int caseIndex = 0; caseIndex < numCases; caseIndex++) {
					TR::Node *caseNode = node->getChild(caseIndex + 2);
					caseNode->setCaseConstant(isLeftShift
	...
	if (convertToGoto) {
		...
		for (int childIndex = 1; childIndex < numChildren; childIndex++) // loop from default to last case
		{
			...
			if (destInst->getOpCodeValue() == TR::Goto) {
				...
				if (!performTransformation(s->comp(),
				            "%sRedirecting switch [" POINTER_PRINTF_FORMAT "] child %d from block_%d "
				            "to block_%d\n",
				...
				if (cfg) {
					...
					// Remove edge between this block and old destination block if this is the only
					// case that leads to the block.
					int otherChildIndex;
					for (otherChildIndex = 1; otherChildIndex < numChildren;
					        ++otherChildIndex) // Look for another case/default that leads to
					                           // destBlock.
						if (otherChildIndex != childIndex) // Another case/default, not this
						                                   // case.
							if (destBlock
					...
					if (otherChildIndex == numChildren) // If another case/default to destBlock
					                                    // wasn't found,
						blocksWereRemoved |= cfg->removeEdge(block, destBlock); // remove the
						                                                        // edge.
				}
			}
		}
...
static void
foldUnsignedLongIntConstant(TR::Node *node, uint64_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
static void
foldUByteConstant(TR::Node *node, uint8_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
static void
foldCharConstant(TR::Node *node, uint16_t value, TR::Simplifier *s, bool anchorChildrenP)
{
	if (!performTransformationSimplifier(node, s))
		return;
...
TR::Node *
checkcastAndNULLCHKSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
...
caddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		foldCharConstant(node, firstChild->getConst<uint16_t>() + secondChild->getConst<uint16_t>(), s,
		        false /* !anchorChildren*/);
		return node;
	}
	orderChildren(node, firstChild, secondChild, s);
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		...
		if (src2 == 0
			...
			if (firstChild->isNegative()
			        && performTransformation(s->comp(),
			                "%sfirst child [%p] of node [%p] is negative, setting the result of imulh to "
			                "-1\n",
				...
				node->setInt(-1);
			        ...
			        && performTransformation(s->comp(),
			                "%sfirst child [%p] of node [%p] is non-negative, setting the result of imulh "
			                "to 0\n",
			                s->optDetailString(), firstChild, node)) {
				s->prepareToReplaceNode(node);
				TR::Node::recreate(node, TR::iconst);
      ...
      // src1  src2      src1 32-n
			...
			while (src2 >>= 1)
				msb++;
			TR::Node::recreate(node, TR::ishr);
			TR::Node *newSecondChild = TR::Node::create(TR::iconst, 0); // Create a new node because
			                                                            // changing the value of the node
			                                                            // might affect trees referencing
			                                                            // this node
...
lmulhSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
		if (performTransformationSimplifier(node, s)) {
			...
			if (node->getOpCode().isUnsigned()) {
				TR::Node::recreate(node, TR::lconst);
				node->setUnsignedLongInt(
				        lmulhu(firstChild->getUnsignedLongInt(), secondChild->getUnsignedLongInt()));
			} else {
				TR::Node::recreate(node, TR::lconst);
...
f2cSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		foldCharConstant(
		        node, floatToInt(firstChild->getFloat(), false) & 0x0000FFFF, s, false /* !anchorChildren*/);
		return node;
	}
...
d2cSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if (firstChild->getOpCode().isLoadConst()) {
		foldCharConstant(
		        node, doubleToInt(firstChild->getDouble(), false) & 0x0000FFFF, s, false /* !anchorChildren*/);
		return node;
	}
...
arraycopybndchkSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	...
	if ((lhsOp == TR::iconst) && (lhsOp == rhsOp) && (lhsChild->getInt() >= rhsChild->getInt()))
		...
		removableBoundCheck = true;
	        ...
	        && (lhsChild->getFirstChild() == rhsChild) && lhsChild->getSecondChild()->getOpCode().isLoadConst()
		...
		if (lengthChild == boundChild || s->isBoundDefinitelyGELength(boundChild, lengthChild)) {
			...
			if (indexChild->getOpCodeValue() == TR::isub) {
				...
				if (boundChild->getOpCodeValue() == TR::iadd
					...
					if (performTransformation(s->comp(),
					            "%sRemoved arraycopy bndchk with arrayLength index in node [%s] "
					            "when bound is sum of length and index arraylengths\n",
					            s->optDetailString(), node->getName(s->getDebug()))) {
						s->removeNode(node, s->_curTree);
						return NULL;
					}
...
//
TR::Node *
bndchkwithspinechkSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s)
{
	simplifyChildren(node, block, s);
