...
static TR::Node *
testNode(TR::Compilation *comp, TR::Node *node, TR::ILOpCodes value, const char *msg)
{
...
static TR::Node *
testUnary(TR::Compilation *comp, TR::Node *parent, TR::ILOpCodes value, const char *msg)
{
	return testNode(comp, parent->getFirstChild(), value, msg);
}

static TR::Node *
testBinary(TR::Compilation *comp, TR::Node *parent, TR::ILOpCodes value, TR::ILOpCodes leftChildValue,
...
static TR::Node *
testBinaryIConst(TR::Compilation *comp, TR::Node *parent, TR::ILOpCodes value, TR::ILOpCodes leftChildValue,
...
TR_Arrayset::TR_Arrayset(TR::Compilation *comp, TR_InductionVariable *indVar) : TR_ArrayLoop(comp, indVar) {}
TR_Arraycopy::TR_Arraycopy(TR::Compilation *comp, TR_InductionVariable *indVar) : TR_ArrayLoop(comp, indVar) {}
...
TR_LRAddressTree::checkAiadd(TR::Node *aiaddNode, int32_t elementSize)
{
	...
	if (loadSym != indSym) {
		if (_matIndVarSymRef) {
			if (loadSym != _matIndVarSymRef->getSymbol()->getRegisterMappedSymbol()) {
				dumpOptDetails(comp(),
				        "checkAiadd: load in the aiadd tree does not match materialized induction "
				        "variable\n");
				return false;
			}
		} else {
	...
	if (_multiplyNode.isNull() && ((elementSize != _increment) && (elementSize != -_increment))) {
		dumpOptDetails(comp(),
		        "checkAiadd: sub-tree does not have induction variable change consistent with increment of "
		        "multiplier (%d %d)\n",
...
TR_Arrayset::checkArrayStore(TR::Node *storeNode)
{
	...
	if (!storeSecondChild->getOpCode().isLoadDirectOrReg()) {
		dumpOptDetails(
		        comp(), "arraystore tree does not have a constant load, or constant load is an address\n");
		return false;
	}
...
TR_Arraycopy::checkArrayStore(TR::Node *storeNode)
{
	...
	if (!loadNode->getOpCode().isLoadIndirect()) {
		dumpOptDetails(
		        comp(), "arraycopy arraystore tree does not have an indirect load as the second child\n");
		return false;
	}
...
TR_ByteToCharArraycopy::checkArrayStore(TR::Node *storeNode)
{
	if (storeNode->getOpCodeValue() != TR::cstorei) {
		dumpOptDetails(
		        comp(), "byte to char arraycopy arraystore tree does not have an indirect store as root\n");
		return false;
	}
	TR::Node *storeFirstChild = storeNode->getFirstChild();
	TR::ILOpCodes opCodeStoreFirstChild = storeFirstChild->getOpCodeValue();
...
TR_ByteToCharArraycopy::checkByteLoads(TR::Node *loadNodes)
{
	...
	if (hbs || lbs) {
		...
		if (!hbs || !lbs || hbs != lbs) {
			dumpOptDetails(comp(),
			        "checkByteLoads: at least one tree has a base sym, but both trees do not have the same "
			        "sym (%p %p)\n",
			        lbs, hbs);
			return false;
		}
...
TR_CharToByteArraycopy::checkArrayStores(TR::Node *origHighStoreNode, TR::Node *origLowStoreNode)
{
	...
	if (highStoreNode->getOpCodeValue() != TR::bstorei) {
		dumpOptDetails(comp(),
		        "checkArrayStores: char to byte arraycopy high arraystore tree does not have an indirect store "
		        "as root\n");
		return false;
	}
	if (lowStoreNode->getOpCodeValue() != TR::bstorei) {
		dumpOptDetails(comp(),
		        "checkArrayStores: char to byte arraycopy low arraystore tree does not have an indirect store "
		        "as root\n");
		return false;
	}
	...
	if (!c2i)
		return false;
	TR::Node *icload = testUnary(
	        comp(), c2i->getFirstChild(), TR::cloadi, "checkArrayStores: high store child is not icload\n");
	if (!icload)
		return false;
	...
	if (!c2i)
		return false;
	TR::Node *icloadDup = testUnary(
	        comp(), c2i->getFirstChild(), TR::cloadi, "checkArrayStores: low store child is not icload\n");
	if (!icloadDup)
		return false;
...
int32_t
TR_ArrayLoop::checkForPostIncrement(
...
void
TR_ArrayLoop::findIndVarLoads(TR::Node *node, TR::Node *indVarStoreNode, bool &storeFound, List<TR::Node> *ivLoads,
...
TR::Node *
TR_ArrayLoop::updateIndVarStore(
...
TR_LoopReducer::generateArraycopy(TR_InductionVariable *indVar, TR::Block *loopHeader)
{
	...
	case gc_modron_wrtbar_cardmark_incremental: needWriteBarrier = true; break;
	default: break;
	}
	...
	if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier && !comp()->cg()->getSupportsReferenceArrayCopy()) {
		dumpOptDetails(comp(),
		        "arraycopy arraystore tree has write barrier as root and write barriers are enabled but no "
		        "support for this platform- no arraycopy reduction\n");
		return false;
	}
	...
	if (storeNode->getType().isBCD() && !(storeSize == 1 || storeSize == 2 || storeSize == 4 || storeSize == 8)) {
		dumpOptDetails(comp(),
		        "arraycopy storeNode %p is a BCD type (%s) and the storeSize (%d) is not 1,2,4 or 8 so do not "
		        "reduce arraycopy\n",
	...
	// Create a new tree with a primitive arraycopy call node followed by an istore node to store the induction
	// variable The parameters will be:
	...
	if (!(arraycopyLoop.hasWriteBarrier() && needWriteBarrier)) {
		...
		if (arraycopyElementType.isBCD()) {
			...
			default:
				// a pre-transformation check just above guarantees only sizes 1,2,4 or 8 are
				// transformed
...
TR_LoopReducer::generateArrayset(TR_InductionVariable *indVar, TR::Block *loopHeader)
{
	...
	if (!performTransformation(comp(),
	            "%sReducing arrayset %d from storeNode [" POINTER_PRINTF_FORMAT "] and copyValueNode "
	...
	//   The number of elements to set to the value (induction variable initial value - comparison value) /
	...
	TR::Node *imul = arraysetLoop.updateIndVarStore(
	        arraysetLoop.getIndVarNode(), indVarStoreNode, arraysetLoop.getStoreAddress());
...
TR_Arraytranslate::checkLoad(TR::Node *loadNode)
{
	...
	if (aiaddNode->getOpCodeValue() != TR::aiadd && aiaddNode->getOpCodeValue() != TR::aladd
		...
		dumpOptDetails(
		        comp(), "...load tree does not have aiadd/aladd/iadd/ladd - no arraytranslate reduction\n");
		return false;
	}
	...
	if (aiaddNode->getOpCodeValue() == TR::aiadd || aiaddNode->getOpCodeValue() == TR::aladd) {
		if (aiaddFirstChild->getOpCodeValue() != TR::aload && aiaddFirstChild->getOpCodeValue() != TR::aloadi) {
			dumpOptDetails(
			        comp(), "...aiadd load tree does not have aload - no arraytranslate reduction\n");
			return false;
		}
		...
		if (aiaddSecondChild->getOpCodeValue() != TR::lloadi
			...
			dumpOptDetails(
			        comp(), "...iadd load tree does not have ilload - no arraytranslate reduction\n");
			return false;
		}
	...
	if (inLoadNode->getOpCodeValue() != TR::cloadi && inLoadNode->getOpCodeValue() != TR::bloadi) {
		dumpOptDetails(comp(),
		        "...load tree does not have 2nd icload/ibload - check if compiler-generated table lookup "
		        "match\n");
...
TR_Arraytranslate::checkStore(TR::Node *storeNode)
{
	...
	if (hasBranch()) {
		...
		if (shrinkNode->getOpCodeValue() != TR::i2s && shrinkNode->getOpCodeValue() != TR::i2b
			...
			dumpOptDetails(comp(),
			        "...store tree does not have i2c/i2b/cconst/bconst - no arraytranslate reduction\n");
			return false;
		}
		...
		if (shrinkNode->getOpCodeValue() == TR::i2b || shrinkNode->getOpCodeValue() == TR::i2s) {
			...
			if (iloadNode->getOpCodeValue() != TR::iload) {
				dumpOptDetails(
				        comp(), "...store tree does not have iload - no arraytranslate reduction\n");
				return false;
			}

			if (iloadNode->getSymbolReference() != _resultNode->getSymbolReference()) {
				dumpOptDetails(comp(),
				        "...store tree reference does not match load tree reference - no "
...
TR_Arraytranslate::checkBreak(TR::Block *breakBlock, TR::Node *breakNode, TR::Block *nextBlock)
{
	...
	if (valNode->getOpCodeValue() != TR::iconst
		...
		dumpOptDetails(comp(),
		        "...break tree does not have bconst/cconst/iconst, or not in range - no arraytranslate "
		        "reduction\n");
		return false;
	}
...
TR_Arraytranslate::checkMatIndVarStore(TR::Node *matIndVarStoreNode, TR::Node *indVarStoreNode)
{
	...
	if (storeFirstChild->getOpCodeValue() != TR::iadd && storeFirstChild->getOpCodeValue() != TR::isub) {
		dumpOptDetails(comp(),
		        "first child %p of materialized induction variable store is not TR::iadd/TR::isub\n",
		        storeFirstChild);
		return false;
	}
	...
	if (iaddFirstChild->getOpCodeValue() != TR::iadd && iaddFirstChild->getOpCodeValue() != TR::isub) {
		dumpOptDetails(
		        comp(), "materialized variable is not expressed in terms of primary iv %p\n", iaddFirstChild);
		return false;
	}
	...
	if (indVarLoad->getOpCodeValue() != TR::iload
		...
		dumpOptDetails(comp(),
		        "primary iv in the materialized tree is not TR::iload %p or second child is not TR::iconst "
		        "%p\n",
...
{
	switch (ifCmp) {
	case TR::ifbcmpeq: return TR::bcmpeq;
	case TR::ifscmpeq: return TR::scmpeq;
	case TR::ificmpeq: return TR::icmpeq;
	case TR::iflcmpeq: return TR::lcmpeq;
	case TR::iffcmpeq: return TR::fcmpeq;
	case TR::ifdcmpeq: return TR::dcmpeq;
	case TR::ifacmpeq: return TR::acmpeq;
	default: TR_ASSERT(0, "Unexpected TR_ifxcmp operator"); return TR::BadILOp;
	}
}
...
bool
TR_LoopReducer::generateArraycmp(
        TR_RegionStructure *whileLoop, TR_InductionVariable *indVar, TR::Block *branchBlock, TR::Block *incrementBlock)
{
	...
	//      aiadd
	//        aload <array #1 base>
	//        isub (this could be variable based on internal pointers, striding, type being loaded) [common base
	//        offset]
	...
	while (!queue.isEmpty()) {
		...
		if (block != _cfg->getEnd() && block != branchBlock && block != incrementBlock) {
			...
			if (blockState == transformerReadFirst) {
				dumpOptDetails(comp(),
				        "induction variable is read before write in block_%d after compare loop - no "
				        "arraycmp reduction\n",
				        block->getNumber());
				return false;
			}
			...
			if (blockState == transformerWrittenFirst) {
				dumpOptDetails(comp(),
				        "induction variable is written before read in block_%d after compare loop - "
				        "pruning successor\n",
				        block->getNumber());
				continue;
			}
	...
	if ((arraycmpLoop.getFirstAddress()->getIncrement() * arraycmpLoop.getFirstAddress()->getMultiplier() < 0)
	        || (arraycmpLoop.getSecondAddress()->getIncrement() * arraycmpLoop.getSecondAddress()->getMultiplier()
	                < 0)) {
	...
	//   The second address to compare to (the aiadd tree - this may reference the first iload in which case it
	//   can't be updated) The number of elements to compare (induction variable initial value - comparison value) /
	//   increment / multiplier
	//
	// Tree-Top
	// ArrayCmp call
	...
	TR::SymbolReference *compareTemp =
...
TR_Arraytranslate::getTableNode()
{
	if (compilerGeneratedTable() && (_tableNode == NULL)) {
		...
		if (inputBits == 8) {
			if (startB != endB) {
				TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits, (uint16_t)startA,
				        (uint16_t)endA, (uint16_t)startB, (uint16_t)endB, defaultValue);
...
bool
TR_LoopReducer::replaceInductionVariable(TR::Node *parent, TR::Node *node, int32_t childNum, int32_t indVarSymRefNumber,
        TR::Node *replacingNode, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return false;
        ...
        TR::Block *firstBlock, TR::Block *secondBlock, TR::Block *thirdBlock, TR::Block *fourthBlock)
{
	...
	if (((firstNum == 3 || firstNum == 4 || firstNum == 5) && secondNum == 0)
		...
		dumpOptDetails(comp(),
		        "Loop has wrong number of tree tops (%d,%d,%d,%d) - no arraytranslate reduction\n", firstNum,
	...
	if (arraytranslateLoop.compilerGeneratedTable() && !hasBranch && hasBreak) {
		dumpOptDetails(comp(),
		        "Attempt to reduce loop marked with no branch but also comp gen'ed table - no arraytranslate "
		        "reduction\n");
		return false;
	}
	...
	if (!hasBranch && !arraytranslateLoop.getByteInput() && !cg()->getSupportsTestCharComparisonControl()) {
		dumpOptDetails(comp(),
		        "Cannot reduce char->byte loops since hardware does not support termination character "
		        "control\n");
		return false;
	}
	...
	if (mayHaveMaterializedIV && !arraytranslateLoop.checkMatIndVarStore(matIndVarStoreNode, indVarStoreNode)) {
		dumpOptDetails(
		        comp(), "Loop does not match materialized indvar requirements - no arraytranslate reduction\n");
		return false;
	}
	...
	if (hasBranch) {
		...
		if (arraytranslateLoop.getTableNode() == NULL) {
			dumpOptDetails(
			        comp(), "Loop does not have a translation table - no arraytranslate reduction\n");
			return false;
		}
	}
	...
	if (!arraytranslateLoop.compilerGeneratedTable()
		...
		dumpOptDetails(
		        comp(), "Output array and translation table datatype mismatch - no arraytranslate reduction\n");
		return false;
	}
	...
	TR::Block *translateBlock =
	        TR::Block::createEmptyBlock(loadNode, comp(), firstBlock->getFrequency(), firstBlock);
	...
	*/
	...
	if (mask && !arraytranslateLoop.compilerGeneratedTable()) // compiler-generated tables aligned at allocation
	{
		...
		if (arraytranslateLoop.tableBackedByRawStorage()) {
			...
			TR::Node *hdrSizeNode = TR::Node::create(
			        loadNode, TR::iconst, 0, TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
	...
	TR::SymbolReference *translateTemp =
	        comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int32);
	...
	if (hasBranch) {
		...
		if (!hasBreak) {
			...
			TR::Node *icmpNode = TR::Node::create(
			        TR::icmpeq, 2, iNode, arraytranslateLoop.getFinalNode()->duplicateTree());
		...
		if (mayHaveMaterializedIV) {
			...
			TR::Node *decNode = TR::Node::create(
			        TR::isub, 2, firstChild, TR::Node::create(updatedMatIndVar, TR::iconst, 0, 1));
	...
	if (hasBreak) {
		...
		TR::Node *matchResultNode = TR::Node::createLoad(
		        branchAroundNode, arraytranslateLoop.getResultNode()->getSymbolReference());
		...
		/// lengthTestNode = TR::Node::create(TR::iadd, 2, lengthTestNode, TR::Node::create(loadNode,
		/// TR::iconst, 0, 1));
		TR::Node *dupFinalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
		...
		if (ivLoadNode->getOpCode().isArrayRef()) {
			...
			replaceInductionVariable(
			        NULL, ivLoadNode, -1, indVarSymRef->getReferenceNumber(), decNode, visitCount);
		}
...
TR_ArraytranslateAndTest::checkLoad(TR::Block *loadBlock, TR::Node *loadNode)
{
	if (loadNode->getOpCodeValue() != TR::ifbcmpeq && loadNode->getOpCodeValue() != TR::ificmpeq) {
		dumpOptDetails(
		        comp(), "...load tree does not have ifbcmpeq/ificmpeq - no arraytranslateAndTest reduction\n");
		return false;
	}
	...
	if (loadNode->getOpCodeValue() == TR::ificmpeq) {
		if (loadNode->getFirstChild()->getOpCodeValue() != TR::b2i) {
			dumpOptDetails(comp(),
			        "...load tree has ificmpeq but no widening from byte - no arraytranslateAndTest "
			        "reduction\n");
			return false;
		} else {
	...
	if (cmpVal->getOpCodeValue() != TR::bconst && cmpVal->getOpCodeValue() != TR::iconst) {
		dumpOptDetails(
		        comp(), "...load tree does not have bconst/iconst - no arraytranslateAndTest reduction\n");
		return false;
	}
...
// BBEnd (block 16)
bool
TR_LoopReducer::generateArraytranslateAndTest(
        TR_RegionStructure *whileLoop, TR_InductionVariable *indVar, TR::Block *firstBlock, TR::Block *secondBlock)
{
	...
	if (firstNum != 1 || (secondNum != 2 && secondNum != 3)) {
		dumpOptDetails(comp(),
		        "Loop has wrong number of tree tops (%d,%d) - no arraytranslateAndTest reduction\n", firstNum,
		        secondNum);
		return false;
	}
	...
	if (secondNum == 2) {
		...
		if (!asyncFound) {
			dumpOptDetails(comp(),
			        "Loop has wrong number of tree tops for no async-check (%d,%d) - no "
	...
	if (!arraytranslateAndTestLoop.checkFrequency(cg(), loadBlock, loadNode)) {
		dumpOptDetails(comp(),
		        "Loop frequency on fall-through not high enough - no arraytranslateAndTest reduction\n");
		return false;
	}
	if (!performTransformation(
	            comp(), "%sReducing arraytranslateAndTest %d\n", OPT_DETAILS, loadBlock->getNumber())) {
		return false;
	}
...
bool
TR_LoopReducer::generateByteToCharArraycopy(
        TR_InductionVariable *byteIndVar, TR_InductionVariable *charIndVar, TR::Block *loopHeader)
{
	...
	{
		dumpOptDetails(comp(),
		        "Loop does not have an increment of +/-1 and +/-2, but instead %d and %d - no byte to char "
		        "arraycopy reduction\n",
		        incrA, incrB);
		return false;
	}
	...
	if (loopHeader->getNumberOfRealTreeTops() != 4) // array store, induction variable store, induction variable
	                                                // store, loop comparison
	{
	...
	if (!arraycopyLoop.checkByteLoads(storeNode->getSecondChild())) {
		dumpOptDetails(comp(),
		        "second child of store does not look like OR of 2 byte loads - no byte to char arraycopy "
		        "performed\n");
		return false;
	}
	...
	if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
		...
		if (swap && arraycopyLoop.getStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
			...
			dumpOptDetails(comp(),
			        "Did not encounter byte array induction variable increment - no byte to char arraycopy "
			        "performed\n");
			return false;
		}
	} else if (!arraycopyLoop.getHighLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
		dumpOptDetails(comp(),
		        "Did not encounter char array induction variable increment - no byte to char arraycopy "
		        "performed\n");
		return false;
	}
	...
	if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
		...
		//   { ... }
		dumpOptDetails(comp(),
		        "Loop comparison does not match byte or char induction variable - no byte to char arraycopy "
		        "performed\n");
		return false;
	}
	...
	TR::SymbolReference *charIndVarTemp =
	        comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int32);
...
bool
TR_LoopReducer::generateCharToByteArraycopy(
        TR_InductionVariable *byteIndVar, TR_InductionVariable *charIndVar, TR::Block *loopHeader)
{
	if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
		dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
	...
	if (loopHeader->getNumberOfRealTreeTops() != 5) // array store low byte, array store high byte, induction
	                                                // variable store, induction variable store, loop comparison
	{
	...
	if (!arraycopyLoop.checkArrayStores(highStoreNode, lowStoreNode)) {
		dumpOptDetails(
		        comp(), "... no match - switch around high and low array store trees and try again...\n");
	...
	if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
		...
		if (swap && arraycopyLoop.getLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
			...
			dumpOptDetails(comp(),
			        "Did not encounter byte array induction variable increment - no char to byte arraycopy "
			        "performed\n");
			return false;
		}
	} else if (!arraycopyLoop.getHighStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
		dumpOptDetails(comp(),
		        "Did not encounter char array induction variable increment - no char to byte arraycopy "
		        "performed\n");
		return false;
	}
	...
	if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
		...
		//   { ... }
		dumpOptDetails(comp(),
		        "Loop comparison does not match byte or char induction variable - no char to byte arraycopy "
		        "performed\n");
		return false;
	}
	...
	TR::TreeTop::removeDeadTrees(comp(), treeList); // make these trees into nops, but leave them around to keep ref
	                                                // counts right
	...
	// following line is a bit messy - need to skip over the multiply-by-2, since want an unstrided value for ind
	// var
...
TR_LoopReducer::constrainedIndVar(TR_InductionVariable *indVar)
{
	...
	default: dumpOptDetails(comp(), "Loop has constant induction variable other than +/-1/2/4/8\n"); return false;
	}
	return true;
}
...
int
TR_LoopReducer::addRegionBlocks(
        TR_RegionStructure *region, TR::Block **blockList, int numBlocks, const int maxNumBlocks)
{
	...
	for (node = it.getFirst(); node != NULL; node = it.getNext()) {
		...
		if (sbs) {
			...
			// numBlocks = addRegionBlocks(node->getStructure()->asRegion(), blockList, numBlocks,
			// maxNumBlocks);
...
TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure *whileLoop)
{
	...
	if (nextIndVar) /* these reductions require 2 induction variables to be considered */
	{
		...
		if ((numBlocks == 2 /* || (numBlocks == 3 && block3->endsInGoto()) */)
			...
			dumpOptDetails(comp(),
			        "Loop matches possible arraytranslate - global deadstore elimination to be "
			        "performed\n");
		...
		if (((numBlocks == 1) && generateArrayset(indVar, loopHeader))
		        ...
		        || ((numBlocks == 3)
		                && generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3, NULL))
		        || ((numBlocks == 4)
		                && generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3, block2))
...
TR_LoopReducer::TR_LoopReducer(TR::OptimizationManager *manager) : TR_LoopTransformer(manager) {}
