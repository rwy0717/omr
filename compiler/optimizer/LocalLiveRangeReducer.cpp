...
TR_LocalLiveRangeReduction::TR_LocalLiveRangeReduction(TR::OptimizationManager *manager)
        : TR::Optimization(manager), _movedTreesList(trMemory()), _depPairList(trMemory())
{}
...
bool
TR_LocalLiveRangeReduction::transformExtendedBlock(TR::TreeTop *entryTree, TR::TreeTop *exitTree)
{
	...
	// Note that since the list adds elements to the head, we can iterate the list, and by this to cover chain of
	// moves (the later one will be in front) e.g. if a blocked by b and then b moved after c. The list will be
	// b-c;a-b
...
void
TR_LocalLiveRangeReduction::collectInfo(TR::TreeTop *entryTree, TR::TreeTop *exitTree)
{
...
void
TR_LocalLiveRangeReduction::collectRefInfo(
...
bool
TR_LocalLiveRangeReduction::investigateAndMove(TR_TreeRefInfo *treeRefInfo, int32_t passNumber)
{
...
void
TR_LocalLiveRangeReduction::populatePotentialDeps(TR_TreeRefInfo *treeRefInfo, TR::Node *node)
{
	TR::ILOpCode &opCode = node->getOpCode();
...
bool
TR_LocalLiveRangeReduction::isAnyDataConstraint(TR_TreeRefInfo *currentTreeRefInfo, TR_TreeRefInfo *movingTreeRefInfo)
{
        ...
        TR_TreeRefInfo *currentTreeRefInfo, TR::Node *currentNode, TR_TreeRefInfo *movingTreeRefInfo)
{
	...
	if (TR::Compiler->target.cpu.isPower() && opCode.getOpCodeValue() == TR::allocationFence) {
		...
		// Exclude all indirect stores, they may be for stack allocs, in which case the flush is needed at least
		// as a scheduling barrier Direct stores to autos and parms are the only safe candidates
		if (movingNode->getOpCode().isStoreIndirect()
			...
			if (trace())
				traceMsg(comp(), "cannot move %p beyond flush %p - (flush for possible stack alloc)",
				        movingNode, currentNode);
			return true;
		}
	}
	...
	for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
		...
		// Any node that has side effects (like call and newarrya) cannot be evaluated in the middle of the
		// tree.
		if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
			...
			if (child->exceptionsRaised()
				...
				if (trace())
					traceMsg(comp(),
					        "cannot move %p beyond %p - cannot change evaluation point of %p\n ",
				...
				if (trace())
					traceMsg(comp(), "cannot move %p beyond %p - node %p under ResolveOrNullCheck",
				...
				if (trace())
					traceMsg(comp(),
					        "cannot move %p beyond %p - changing the eval point of %p will casue "
					        "extra cg instruction ",
					        movingNode, currentTreeRefInfo->getTreeTop()->getNode(), currentNode);
				return true;
			}
		}
...
bool
TR_LocalLiveRangeReduction::moveTreeBefore(TR_TreeRefInfo *treeToMove, TR_TreeRefInfo *anchor, int32_t passNumber)
{
...
void
TR_LocalLiveRangeReduction::addDepPair(TR_TreeRefInfo *dep, TR_TreeRefInfo *anchor)
{
...
bool
TR_LocalLiveRangeReduction::containsCallOrCheck(TR_TreeRefInfo *treeRefInfo, TR::Node *node)
{
...
bool
TR_LocalLiveRangeReduction::matchFirstOrMidToLastRef(TR_TreeRefInfo *tree, TR_TreeRefInfo *treeToMove)
{
...
void
TR_LocalLiveRangeReduction::updateRefInfo(
...
bool
TR_LocalLiveRangeReduction::verifyRefInfo(List<TR::Node> *verifier, List<TR::Node> *refList)
{
	...
	if (!verifier->isEmpty()) {
		if (trace())
			traceMsg(
			        comp(), "LocalLiveRangeReduction: there are nodes that should have been in the List\n");
		return false;
	}
	return true;
}
...
void
TR_LocalLiveRangeReduction::printOnVerifyError(TR_TreeRefInfo *optRefInfo, TR_TreeRefInfo *verifier)
{
	if (trace()) {
