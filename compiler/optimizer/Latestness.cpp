        ...
        : TR_BackwardIntersectionBitVectorAnalysis(comp, comp->getFlowGraph(), optimizer, trace)
{
	...
	for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
		...
		*(_inSetInfo[blockStructure->getNumber()]) |=
		        *(_delayedness->_earliestness->_globalAnticipatability->_localAnticipatability
...
//
void
TR_Latestness::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *, ContainerType *)
{}

void
TR_Latestness::analyzeTreeTopsInBlockStructure(TR_BlockStructure *blockStructure)
{
