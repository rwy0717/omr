...
void TR_Rematerialization::rematerializeSSAddress(TR::Node* parent, int32_t addrChildIndex)
{
    ...
    if (addressNode->getReferenceCount() > 1 && ((addressNode->getOpCodeValue() == TR::loadaddr && addressNode->getSymbolReference()->getSymbol()->isAutoOrParm()) || (addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst() && cg()->getSupportsConstantOffsetInAddressing(addressNode->getSecondChild()->get64bitIntegralValue()))))
