...
TR_PeepHoleBasicBlocks::TR_PeepHoleBasicBlocks(TR::OptimizationManager *manager) : TR_BlockManipulator(manager) {}
...
TR_ExtendBasicBlocks::TR_ExtendBasicBlocks(TR::OptimizationManager *manager) : TR_BlockManipulator(manager) {}
	...
	for (; block; prevBlock = block, block = block->getNextBlock()) {
		...
		if (prevNode->getOpCodeValue() == TR::Goto) {
			if (!performTransformation(comp(), "%sMerge blocks %d and %d\n", optDetailString(),
			            prevBlock->getNumber(), block->getNumber()))
				continue;
				...
				if (prevNode->getBranchDestination() == nextBlock->getEntry() && !nextBlock->isCold()) {
					...
					for (uintptr_t c = 0; c < prevNode->getNumChildren(); ++c) {
						TR_ASSERT(prevNode->getChild(c)->getOpCodeValue() != TR::GlRegDeps,
						        "the conditional branch node has a GlRegDep child and we're "
					...
					// if the F and pT are the same (we have a redundant compare) - do not do any
					// transformation
					if (F == pT)
					...
					cfg->removeEdge(F, block); // ideally 'block' will become unreachable, and be
					                           // removed
					...
					if (!performTransformation(comp(),
					            "%sRemove backedge goto block_%d by reversing the branch in "
					            "block_%d\n",
...
TR_ExtendBasicBlocks::orderBlocksWithFrequencyInfo()
{
	...
	int32_t extendedBlockPreference = extendedBlockPointPreference * (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)
	        / 10000;
...
bool
TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block *block1, TR::Block *block2)
{
...
//
static bool
insideLoop(TR::Compilation *comp, TR::Block *block1, TR::Block *block2, TR::Block *block3)
{
	if (!comp->getFlowGraph()->getStructure())
		return false;
...
//
int32_t
TR_BlockManipulator::performChecksAndTreesMovement(TR::Block *newBlock, TR::Block *prevBlock, TR::Block *block,
        TR::TreeTop *endTreeTop, vcount_t visitCount, TR::Optimizer *optimizer)
{
	...
	if (newBlock->getVisitCount() < visitCount) {
		...
		for (cursor = newBlock, cursorNext = cursor->getNextBlock(); cursorNext
		...
		/////// This just surgically reattaches the treetops. This does NOT set successor/predecessor info or
		/// changes CFG
		...
		// If it was non-goto (cond. branch), just reverse condition and make it point to block (old
		// fallthrough) newBlock is the new fallthrough.
...
TR_HoistBlocks::TR_HoistBlocks(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
//
static TR::Node *
duplicateExact(TR::Node *node, List<TR::Node> *seenNodes, List<TR::Node> *duplicateNodes, TR::Compilation *comp)
{
		...
		if ((node1->getOpCode().isReturn() ||
			...
			for (auto nextEdge = block->getPredecessors().begin();
				...
				if (!tt->getNode()->getOpCode().isJumpWithMultipleTargets() && canHoist) {
					...
					else if ((block->getHotness(cfg) >= warm) && (prevBlock->getHotness(cfg) >=
					hot))
					   {
					...
					if (synergyExists
						...
						TR::TreeTop *newPrevTree = tt;
						{
							...
							while (!(currentTree == exitTreeTop)) {
								{
									...
									TR::Node *newCurrentNode =
									        TR::Node::copy(currentNode);
									currentNode->setVisitCount(visitCount);
									duplicateNodes.add(newCurrentNode);
									seenNodes.add(currentNode);

									for (int32_t childNum = 0; childNum
									        < currentNode->getNumChildren();
									        childNum++) {
										TR::Node *duplicateChildNode =
										        duplicateExact(
										                ...
										                &duplicateNodes,
										                comp());
										newCurrentNode->setChild(
										        childNum, duplicateChildNode);
									}

									newCurrentTree = TR::TreeTop::create(
									        comp(), newCurrentNode, NULL, NULL);
							...
							if (fallThroughSuccBlock) {
								comp()->getFlowGraph()->setStructure(NULL);
								extraGotoBlock = TR::Block::createEmptyBlock(node1,
								        comp(),
								        fallThroughTree->getNode()
								                ->getBlock()
								                ->getFrequency(),
								        fallThroughTree->getNode()->getBlock());
								...
								TR::TreeTop *gotoTreeTop = TR::TreeTop::create(comp(),
								        TR::Node::create(
								                node1, TR::Goto, 0, fallThroughTree));
						...
						for (auto nextSuccEdge = block->getSuccessors().begin();
							...
							if (extraGotoBlock && (succBlock == fallThroughSuccBlock)) {
								...
								// dumpOptDetails(comp(), "Added edge from %d to
								// %d\n\n", extraGotoBlock->getNumber(),
								// succBlock->getNumber());
								cfg->addEdge(extraGotoBlock, succBlock);
								int32_t extraGotoFrequency =
								        (*nextSuccEdge)->getFrequency();
								if (extraGotoFrequency < 0)
									extraGotoFrequency = (*nextSuccEdge)
									                             ->getTo()
									                             ->getFrequency();
								extraGotoBlock->setFrequency(extraGotoFrequency);
								if (extraGotoFrequency >= 0) {
									extraGotoBlock->getPredecessors()
									        .front()
									        ->setFrequency(extraGotoFrequency);
									extraGotoBlock->getSuccessors()
									        .front()
									        ->setFrequency(extraGotoFrequency);
								}
							} else {
								...
								// %d\n\n", prevBlock->getNumber(),
								// succBlock->getNumber());
								// We don't want to try to add another edge between
								// prevBlock and block as there already exists an edge;
								// this will result in the edge being added to the CFG
								// but NOT to the structure (which checks for duplicate
								// edges before adding them). Then removing the edge
								// from prevBlock->block will fail after structure
								// repair as there would be one edge in the CFG left,
								// but NONE in the structure subgraph.
								//
								if (block != succBlock)
									cfg->addEdge(TR::CFGEdge::createEdge(
									        prevBlock, succBlock, trMemory()));
								else
...
static void
markNodesUsedInIndirectAccesses(TR::Node *node, bool examineChildren, TR::Compilation *comp)
{
...
//
TR_CompactNullChecks::TR_CompactNullChecks(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
void
TR_CompactNullChecks::compactNullChecks(TR::Block *block, TR_BitVector *writtenSymbols)
{
		...
		if ((prevNode->getOpCodeValue() == TR::NULLCHK)
			...
			if (referenceChild == prevNode->getFirstChild()->getFirstChild()) {
				...
				while (!replacedPassThroughNode) {
					...
					replacedPassThroughNode = replacePassThroughIfPossible(cursorNode,
					        referenceChild, prevNode, NULL, &isTreeTopNode, writtenSymbols,
					        visitCount, initialVisitCount, cursorTreeTop);
			...
			while (cursorTreeTop != exitTree && cursorTreeTop->getNode()->getOpCodeValue() != TR::BBEnd) {
				TR::Node *cursorNode = cursorTreeTop->getNode();
				replacedNullCheck = replaceNullCheckIfPossible(cursorNode, objectRef, prevNode, NULL,
...
bool
TR_CompactNullChecks::replacePassThroughIfPossible(TR::Node *currentNode, TR::Node *referenceChild, TR::Node *prevNode,
        TR::Node *currentParent, bool *isTreeTopNode, TR_BitVector *writtenSymbols, vcount_t visitCount,
        vcount_t initialVisitCount, TR::TreeTop *currentTree)
{
	...
	for (i = 0; i < currentNode->getNumChildren(); ++i) {
		...
		if (replacePassThroughIfPossible(child, referenceChild, prevNode, currentNode, isTreeTopNode,
		...
		if (!(opCode.isArrayLength() && comp()->cg()->getDisableNullCheckOfArrayLength()) && childEquivalent
			...
			if (canCompact
				...
				if (child->getOpCodeValue() != TR::loadaddr) // For loadaddr, IsNonNull is always true
					child->setIsNonNull(false); // it is no longer known that the reference is non
					                            // null
...
bool
TR_CompactNullChecks::replaceNullCheckIfPossible(TR::Node *cursorNode, TR::Node *objectRef, TR::Node *prevNode,
        ...
        bool &compactionDone)
{
	...
	if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
			...
			if (canBeRemoved
				...
				if (!compactionDone) {
					...
					TR_Pair<TR_ByteCodeInfo, TR::Node> *bcInfo = new (trHeapMemory())
					        TR_Pair<TR_ByteCodeInfo, TR::Node>(
		...
		for (i = 0; i < cursorNode->getNumChildren(); i++) {
			...
			if (!replaceNullCheckIfPossible(child, objectRef, prevNode, cursorNode, writtenSymbols,
...
TR_ArraysetStoreElimination::TR_ArraysetStoreElimination(TR::OptimizationManager *manager) : TR::Optimization(manager)
{}
...
int32_t
TR_ArraysetStoreElimination::process(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
...
void
TR_ArraysetStoreElimination::reduceArraysetStores(TR::Block *block, TR_BitVector *readAndWrittenSymbols,
		...
		if ((prevNode->getNumChildren() > 0) && (prevNode->getFirstChild()->getOpCodeValue() == TR::newarray)) {
			...
			if (arraysetNode && (arraysetNode->getOpCodeValue() == TR::arrayset)
				...
				while (!optimizedArraysetNode) {
					...
					optimizedArraysetNode = optimizeArraysetIfPossible(cursorNode, addressChild,
					        arraysetTree, NULL, readAndWrittenSymbols, symbolsWrittenByArrayset,
					        scratch, initialVisitCount, currentTree);
			...
			if ((prevNode->getNumChildren() > 0)
				...
				readAndWrittenSymbols->empty();
				TR::Node *cursorNode = currentTree->getNode();
				optimizeArraysetIfPossible(cursorNode, prevNode->getFirstChild()->getFirstChild(),
				        prevTree, NULL, readAndWrittenSymbols, symbolsWrittenByArrayset, scratch,
...
bool
TR_ArraysetStoreElimination::optimizeArraysetIfPossible(TR::Node *currentNode, TR::Node *addressChild,
        TR::TreeTop *arraysetTree, TR::Node *currentParent, TR_BitVector *readAndWrittenSymbols,
        TR_BitVector *symbolsWrittenByArrayset, TR_BitVector *scratch, vcount_t initialVisitCount,
	...
	if (currentNode->getOpCodeValue() == TR::arraycopy) {
		...
		if ((arraycopyDestAddressNode == addressChild)
			...
			*scratch &= *readAndWrittenSymbols;
			...
			if (scratch->isEmpty()
			        && performTransformation(comp(),
			                "%sEliminating some stores done by arrayset node %p because stores done by a "
			                "following arraycopy node %p will overwrite them\n",
...
// condition.
//
TR_SimplifyAnds::TR_SimplifyAnds(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
static bool
isAndOfTwoFlags(TR::Compilation *comp, TR::Node *andNode, TR::Node *lastRealNode, TR::ILOpCodes ifOpCode,
			...
			while (seenAndNode) {
				...
				if ((andNode->getFirstChild() == lastRealNode->getFirstChild())
					...
					if (andOpCodeValue == lastRealOpCodeValue) {
				        ...
				        && lastRealNode->getOpCode().isBooleanCompare()) // this code was allowing
				                                                         // unsigned types in -- I made
				                                                         // this test stronger // NOTE
				                                                         // that isCompare is only true
				                                                         // for signed compares -- this
				                                                         // is very ugly though
				{
			...
			if (!seenAndBefore) {
							...
							if (firstNode->getOpCode().isStoreDirect()
								...
								if (firstNode->getFirstChild()
								        ...
								        && (firstNode->getFirstChild()
								                        ...
								                        ->getReferenceCount()
								                == 1)
								        && // in actual fact the ref count can be > 1 as
								           // long as this is the first reference to
								           // this load (we could check that in future
								           // if reqd)
								        (firstNode->getFirstChild()
								                        ->getFirstChild()
								                        ->getSymbolReference()
								...
								else if (treeBeforeLastRealNode->getOpCode()
								                 .isStoreDirect()
								        && (firstNode->getFirstChild()->getFirstChild()
								                == treeBeforeLastRealNode
								                           ->getFirstChild())
								...
								if (areTreesInRequiredForm
								        && performTransformation(comp(),
								                "%sSimplifying branch %p and store %p "
								                "into conditional\n",
									...
									TR::Node *lastFirstChild =
									        lastRealNode->getFirstChild();

									// Shift the lastFirstChild right by (size-1) to
									// generate 0x0 or 0xFF..FF
									...
									if (firstNode->getType().isInt32()
									        && !lastFirstChild->getType().isInt32())
										newIShrNode = TR::Node::create(
										        TR::l2i, 1, newIShrNode);
									else if (!firstNode->getType().isInt32()
									        && lastFirstChild->getType().isInt32())
										newIShrNode = TR::Node::create(
										        TR::i2l, 1, newIShrNode);

									// And with original value to generate value to
									// add/sub.
									TR::Node *newIAndNode = TR::Node::create(
									        firstNode->getType().isInt32()
									                ? TR::iand
									                : TR::land,
									        2, secondGrandChild, newIShrNode);

									secondGrandChild->decReferenceCount();
									firstNode->getFirstChild()->setAndIncChild(
									        1, newIAndNode);
									TR::TreeTop *prevTree =
									        lastRealTree->getPrevRealTreeTop();
									prevTree->join(firstTree);
									firstTree->join(lastRealTree);
									if (asyncTree) {
										asyncTree->join(
										        prevTree->getNextTreeTop());
										prevTree->join(asyncTree);
									}

									nextBlock->getEntry()->join(
									        nextBlock->getExit());

									optimizer()->prepareForTreeRemoval(
									        lastRealTree);
									TR::TransformUtil::removeTree(
									        comp(), lastRealTree);

									TR::CFGEdge *currentSucc =
									        block->getSuccessors().front();
									TR::CFGNode *succBlock = currentSucc->getTo();
									if (succBlock
									        != nextToNextBlockEntry->getNode()
									                   ->getBlock()) {
										currentSucc = *(++block->getSuccessors()
										                          .begin());
				...
				if (!removingConditional) {
					...
					noSideEffectsInBetween = true;
			                   ...
			                   "%sRemoving conditional node %p (condition has been checked before in this "
			                   "extended BB)\n",
			                   optDetailString(), lastRealNode)) {
			...
			if (prevNode && prevNode->getOpCodeValue() == lastRealNode->getOpCodeValue()) {
				...
				if (lastRealNode->getOpCode().isSpineCheck()) {
					...
					if (0 && (prevNode->getChild(2) == lastRealNode->getChild(2))
					        ...
					        && performTransformation(comp(),
					                "%sRemoving BNDCHK from BNDCHKwithSpineCHK lastRealNode %p, "
					                "prevNode=%p\n",
					...
					if ((prevNode->getFirstChild() == lastRealNode->getFirstChild())
					        && (compareValues(
					                    prevNode->getSecondChild(), lastRealNode->getSecondChild())
					                > 0)) {
...
//
TR_EliminateRedundantGotos::TR_EliminateRedundantGotos(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
void
TR_EliminateRedundantGotos::renumberExitEdges(TR_RegionStructure *region, int32_t oldNum, int32_t newNum)
{
...
int32_t
TR_EliminateRedundantGotos::process(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
	// The CFG must exist for this optimization.
	//
	TR::CFG *cfg = comp()->getFlowGraph();
...
void
TR_EliminateRedundantGotos::redirectPredecessors(
	...
	for (auto edge = preds.begin(); edge != preds.end(); ++edge) {
		...
		if (predBlock->getNextBlock() == destBlock) {
			...
			if (last->getOpCodeValue() == TR::Goto) {
				...
				TR_ASSERT_FATAL(exit->getNumChildren() == 0,
...
void
TR_EliminateRedundantGotos::fixPredecessorRegDeps(TR::Node *regdepsParent, TR::Block *destBlock)
{
	...
	TR_ASSERT_FATAL(newReceivingRegdeps->getOpCodeValue() == TR::GlRegDeps,
	        ...
	        newReceivingRegdeps->getGlobalIndex(), destEntry->getGlobalIndex(), destBlock->getNumber());
...
// goto.
//
TR_CleanseTrees::TR_CleanseTrees(TR::OptimizationManager *manager) : TR_BlockManipulator(manager) {}
...
TR_CleanseTrees::process(TR::TreeTop *startTree, TR::TreeTop *endTreeTop)
{
	...
	for (treeTop = startTree; (treeTop != endTreeTop); treeTop = exitTreeTop->getNextTreeTop()) {
		...
		// if (!node->getOpCode().isBranch()) // commented out since OrderBlocks is now the pass which will
		// decide fall throughs in cases where the branch ends in an 'if'
		if (node->getOpCodeValue() != TR::Goto)
			continue;
		...
		if (tt2 && node->getBranchDestination() == tt2) {
			...
			// if (node->getOpCodeValue() == TR::Goto)
			{
				result = performChecksAndTreesMovement(tt2->getNode()->getBlock(), block,
				        block->getNextBlock(), endTree, comp()->getVisitCount(), optimizer());
			}
			// else
			//{
			// result = performChecksAndTreesMovement(tt2->getNode()->getBlock(), block,
...
TR_ByteCodeInfo
TR_ProfiledNodeVersioning::temporarilySetProfilingBcInfoOnNewArrayLengthChild(TR::Node *newArray, TR::Compilation *comp)
{
		...
		if (node->getOpCodeValue() == TR::BBEnd) {
			...
			case TR::anewarray: {
				...
				TR_ValueInfo *numElementsInfo =
				        static_cast<TR_ValueInfo *>(TR_ValueProfileInfoManager::getProfiledValueInfo(
				...
				if (numElementsInfo) {
					...
					if (totalFrequency > 0) {
						static char *versionNewarrayForMultipleSizes = feGetEnv("TR_"
						                                                        "versionNewarra"
						                                                        "yForMultipleSi"
						                                                        "zes");
						if (trace())
						...
						for (TR_ExtraValueInfo *profiledInfo = i.getFirst();
							...
							float probability = (float)profiledInfo->_frequency
							        / totalFrequency;
							if (trace())
							...
							//
							if (!doProfiling
							        || (profiledInfo->_value < (uint32_t)(
							                    profiledSize * (1.0 + probability)))) {
								...
								numDifferentSizes++;
								profiledSize =
								        std::max(profiledSize, profiledInfo->_value);
								minProfiledSize =
					...
					if (doProfiling
						...
						block->createConditionalBlocksBeforeTree(tt, compareTree, slowTree,
						        fastTree, comp()->getFlowGraph(), false);
						...
						if (numDifferentSizes >= 2) {
							...
							TR::TreeTop::create(comp(), fastTree,
							        TR::Node::createWithSymRef(TR::istorei, 2, 2,
							                fixedSizeNew, fixedSizeNewOriginalSize,
							                comp()->getSymRefTab()
						...
						// Store new object to a temp, and replace uses of object with loads of
						// the temp
						//
						TR::SymbolReference *newObjectPointer = getSymRefTab()->createTemporary(
						...
						if (originalFrequency >= 0) {
							int16_t fastFrequency = std::max<int16_t>(
							        originalFrequency * combinedProbability,
							...
							if (trace()) {
								traceMsg(comp(),
								        " -> Setting fast block_%d frequency = %d; "
								        "slow block_%d frequency = %d\n",
...
void
TR_Rematerialization::rematerializeAddresses(TR::Node *indirectNode, TR::TreeTop *treeTop, vcount_t visitCount)
{
				...
				if ((node->getReferenceCount() > 1 && node->isInternalPointer()
					...
					if (performTransformation(comp(), "%sRematerializing node %p(%s)\n",
						...
						if (node->isInternalPointer() && node->getPinningArrayPointer())
							newChild->setPinningArrayPointer(
						...
						if (secondChild->getReferenceCount() > 1 && allowUncommoning) {
							if (performTransformation(comp(),
								...
								if (parent != node)
									childNum = 0;
								// printf("Found a chance in %s\n",
								// comp()->signature()); dumpOptDetails(comp(),
								// "Uncommoning mul %p\n", secondChild);
				...
				if (child->getReferenceCount() > 1) {
					if (performTransformation(comp(), "%sRematerializing node %p(%s)\n",
						...
						if (indirectNode->getOpCodeValue() == TR::PassThrough) {
							TR::Node *anchorNode =
							        TR::Node::create(TR::treetop, 1, newChild);
							TR::TreeTop *anchorTree =
							        TR::TreeTop::create(comp(), anchorNode);
...
bool
TR_Rematerialization::isRematerializable(TR::Node *parent, TR::Node *node, bool onlyConsiderOpCode)
{
...
void
TR_Rematerialization::removeNodeFromList(TR::Node *node, List<TR::Node> *nodes, List<List<TR::Node> > *parents,
	...
	if (checkSymRefs && loadsAlreadyVisited) {
		...
		while (visitedLoadsElement) {
			if ((visitedLoadsElement->getData()->getSymbolReference() == node->getSymbolReference())
			        || aliases.ValueAt(
			                visitedLoadsElement->getData()->getSymbolReference()->getReferenceNumber())) {
...
void
TR_Rematerialization::addParentToList(
...
bool
TR_Rematerialization::examineNode(TR::TreeTop *treeTop, TR::Node *parent, TR::Node *node, vcount_t visitCount,
        TR_RematState *state, TR_RematAdjustments &adjustments)
{
		...
		if (node->getOpCode().isFloatingPoint()) {
			...
			if (isRematerializable(parent, node, true) && node->isRematerializeable()) {
				if (isRematerializableLoad(node, parent))
					...
					removeNodeFromList(node, &state->_currentlyCommonedVector128Candidates,
			...
			if ((isRematerializable(parent, node, true) && node->isRematerializeable())
				...
				if (isRematerializableLoad(node, parent) || _prefetchNodes.find(node))
					...
					removeNodeFromList(
					        node, &state->_currentlyCommonedCandidates, &state->_parents, false);
			}
		}
	}
	...
	if (((((node->getFutureUseCount() & 0x7fff) > 0) && (node->getReferenceCount() > 1)))
		...
		if (node->getOpCode().isFloatingPoint()) {
			if (isRematerializableLoad(node, parent))
				...
				addParentToList(
				        node, &state->_currentlyCommonedFPCandidates, parent, &state->_fpParents);
		} else if (node->getOpCode().isVector()) {
			if (isRematerializableLoad(node, parent))
				addParentToList(node, &state->_currentlyCommonedVector128Loads, parent,
				        &state->_parentsOfCommonedVector128Loads);
			else
				addParentToList(node, &state->_currentlyCommonedVector128Candidates, parent,
				        &state->_vector128Parents);
		} else {
			if (isRematerializableLoad(node, parent))
				addParentToList(
				        node, &state->_currentlyCommonedLoads, parent, &state->_parentsOfCommonedLoads);
			else
	...
	*/
	...
	if (removeNodes) {
		...
		if (node->getOpCode().isFloatingPoint())
			removeNodeFromList(node, &state->_currentlyCommonedFPLoads, &state->_parentsOfCommonedFPLoads,
			        true, &state->_fpLoadsAlreadyVisited,
			        &state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized, aliases);
		else if (node->getOpCode().isVector()) {
			removeNodeFromList(node, &state->_currentlyCommonedVector128Loads,
			        &state->_parentsOfCommonedVector128Loads, true, &state->_vector128LoadsAlreadyVisited,
			...
			removeNodeFromList(node, &state->_currentlyCommonedLoads, &state->_parentsOfCommonedLoads, true,
			        &state->_loadsAlreadyVisited, &state->_loadsAlreadyVisitedThatCannotBeRematerialized,
			        aliases);
	}
	...
	if (trace()) {
		traceMsg(comp(),
		        "At node %p parent %p GPR pressure is %d (child adjust %d parent adjust %d) limit is %d\n",
		        node, parent, numRegisters + childAdjustment + adjustments.adjustmentFromParent,
	...
	if ((!considerRegPressure
		...
		rematerializeNode(treeTop, parent, node, visitCount, &state->_currentlyCommonedVector128Nodes,
		        ...
		        &state->_vector128LoadsAlreadyVisitedThatCannotBeRematerialized, rematSpecialNode);
	}
				...
				if (isRematerializableLoad(node, parent)) {
					if (node->getOpCode().isFloatingPoint()) {
						if (!state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized.find(
							...
							TR_ScratchList<TR::Node> *parentList = new (trStackMemory())
...
static List<TR::Node> *
getParentList(TR::Node *node, List<List<TR::Node> > *parents, List<TR::Node> *nodes)
{
...
void
TR_Rematerialization::rematerializeNode(TR::TreeTop *treeTop, TR::Node *parent, TR::Node *node, vcount_t visitCount,
        List<TR::Node> *currentlyCommonedNodes, List<TR::Node> *currentlyCommonedCandidates,
        ...
        List<TR::Node> *loadsAlreadyVisitedThatCannotBeRematerialized, bool rematSpecialNode)
{
	...
	if (!isAdjunctNode
	        && performTransformation(comp(), "%sRematerializing node %p(%s)\n", optDetailString(),
	                nodeToBeRematerialized, nodeToBeRematerialized->getOpCode().getName())) {
		...
		if (nodeToBeRematerialized->getNumChildren() > 0) {
			...
			if (isRematerializable(nodeToBeRematerialized, firstChild) && firstChild->isRematerializeable()
				...
				for (int32_t j = 0; j < newFirstChild->getNumChildren(); j++) {
					...
					parentList = getParentList(newFirstChild->getChild(j), parentsOfCommonedLoads,
		...
		for (int32_t j = 0; j < newNode->getNumChildren(); j++) {
			...
			List<TR::Node> *parentList =
			        getParentList(newNode->getChild(j), parents, currentlyCommonedCandidates);
			if (parentList) {
			...
			parentList =
			        getParentList(newNode->getChild(j), parentsOfCommonedLoads, currentlyCommonedLoads);
			if (parentList) {
				parentList->add(newNode);
				parentList->remove(nodeToBeRematerialized);
			}
				...
				for (int32_t j = 0; j < parentNode->getNumChildren(); j++) {
					if (parentNode->getChild(j) == nodeToBeRematerialized) {
						...
						parentList2 = getParentList(nodeToBeRematerialized,
						        parentsOfCommonedLoads, currentlyCommonedLoads);
						if (parentList2)
...
TR_BlockSplitter::TR_BlockSplitter(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
TR_BlockSplitter::perform()
{
	...
	if (disableSynergy() && comp()->isOptServer()) {
		if (trace())
			traceMsg(comp(),
			        "Large app detected, making BlockSplitter very conservative to avoid cloning code "
			        "(increases i-cache pressure)\n");
	}
	...
	case scorching:
		...
		break;
	default: maxDepth = 2; totalFrequency = (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT);
	}
	...
	static int32_t splitPredPercentage = (p = feGetEnv("TR_SplitPredPercentage")) ? atoi(p)
	...
	while (children->size() > 0) {
		...
		if (disableSynergy() && comp()->isOptServer() && !mergeNode->isEmptyBlock()) {
			...
			for (auto edge = mergeNode->getPredecessors().begin();
				...
				if (predBlock->getEntry() && (predBlock->getFrequency() >= splitFrequency)
					...
					if (trace())
						traceMsg(comp(),
						        "    rejecting because predecessor block %d has more than 4 "
						        "treetops\n",
						        predBlock->getNumber());
					break;
				}
		...
		if (!mergeNode->isEmptyBlock()) {
			...
			TR_IndexedBinaryHeapElement *temp = new (trStackMemory())
			        TR_IndexedBinaryHeapElement(mergeNode, index++);
			heap->add(temp);
		}
		children->pop();
	}
	...
	for (int32_t i = 0; i <= array->lastIndex(); ++i) {
		...
		for (auto edge = mergeNode->getPredecessors().begin(); edge != mergeNode->getPredecessors().end();
		        ++edge) {
			node = (*edge)->getFrom();
			if (trace())
			...
			if (toBlock(node)->getEntry()) {
				...
				// of a virtual guard that we are not allowed to reverse, if this is the case we must
				// chose someone else
				...
				// a GCR counting guard which could be patched, we are not allowed to reverse these
				// either and must choose someone else
				if (lastRealNode->getOpCode().isIf()
			...
			if (!splitPred || splitPred->getFrequency() < node->getFrequency()) {
				...
				if (trace())
					traceMsg(comp(),
					        "mergeNode(%d) splitPred(%d) blockfreq = %d edgefreq = %d candidate\n",
		...
		if (!splitPred || (splitPred->getFrequency() < splitPredFrequency) || !splitPred_to_mergeNode_edge ||
			...
			if (trace())
				traceMsg(
				        comp(), "Merge block_%d has no suitable predecessor\n", mergeNode->getNumber());
			continue;
		}
		...
		while (depth > 1) {
			if (trace())
				traceMsg(
				        comp(), "  Processing block_%d, depth budget %d\n", target->getNumber(), depth);

			TR::Block *child;
			int32_t startDepth = depth;
			TR::Node *lastRealNode = target->getExit()->getPrevRealTreeTop()->getNode();
			for (auto edge = target->getSuccessors().begin(); edge != target->getSuccessors().end();
			        ++edge) {
				...
				if (!(child->getPredecessors().size() == 1)) {
					if (trace())
						traceMsg(comp(),
						        "    Reject successor block_%d: merge block -- we'll process "
						        "this separately\n",
						        child->getNumber());
					continue;
				}

				// if we have a cloned node, structure questions need to be asked about the original
				// node so perform the mapping here
				...
				if (containCycle(child, &bMap)) {
					if (trace())
						traceMsg(comp(),
						        "    Reject successor block_%d: would cause cycle in block "
						        "mapper\n",
						        child->getNumber());
					continue;
				}

				// finally check to make sure we aren't about to chose a block that is reached as the
				// target of a virtual guard that we are not allowed to reverse, if this is the case we
				// must stop here
				if (lastRealNode->getOpCode().isIf()
				        && lastRealNode->isTheVirtualGuardForAGuardedInlinedCall()
				        && lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber()
		...
		if (pruneAndPopulateBlockMapper(&bMap, synergisticDepthCalculator(&bMap, splitPred)) > 0) {
			...
			int16_t coldEdgeFreq = (predEdgeFrequency - splitPred_to_mergeNode_edge->getFrequency())
			        / predEdgeFrequency;
			if (coldFreq < 6)
				coldFreq = 6;
...
void
TR_BlockSplitter::heapElementQuickSort(TR_Array<TR_IndexedBinaryHeapElement *> *array, int32_t left, int32_t right)
{
	if (right - left > 1) {
...
void
TR_BlockSplitter::quickSortSwap(TR_Array<TR_IndexedBinaryHeapElement *> *array, int32_t left, int32_t right)
{
...
int32_t
TR_BlockSplitter::pruneAndPopulateBlockMapper(TR_LinkHeadAndTail<BlockMapper> *bMap, int32_t depth)
{
	int32_t cloned = 0;
	if (depth) {
		if (trace()) {
		...
		for (itr = bMap->getFirst(); itr; itr = itr->getNext()) {
			...
			itr->_to = new (trHeapMemory()) TR::Block(
			        *itr->_from, TR::TreeTop::create(comp(), NULL), OMR::TreeTop::create(comp(), NULL));
...
int32_t
TR_BlockSplitter::synergisticDepthCalculator(TR_LinkHeadAndTail<BlockMapper> *bMap, TR::Block *startPoint)
{
	...
	if (trace()) {
		for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
			if (itr == bMap->getFirst())
				traceMsg(comp(), "    Splitting block_%d for %s", itr->_from->getNumber(),
				        comp()->signature());
...
bool
TR_BlockSplitter::containCycle(TR::Block *blk, TR_LinkHeadAndTail<BlockMapper> *bMap)
{
...
TR::Block *
TR_BlockSplitter::splitBlock(TR::Block *pred, TR_LinkHeadAndTail<BlockMapper> *bMap)
{
			...
			if (lastRealNode->getOpCode().isIf()) {
				if (lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber()
					...
					if (trace())
						traceMsg(comp(), "  Reversing branch, node %d now jumps to block_%d\n",
						        ...
						        lastRealNode->getBranchDestination()
						                ...
						                ->getBlock()
						                ->getNumber());
					// this check handles splitting through a loop header in a region where we have
					// not detected a valid loop header
					if (bMap->getLast()->_from->getNumber() == pred->getNumber()) {
						...
						if (trace())
							traceMsg(comp(),
							        "    Also reversing cloned branch, node %d now jumps "
							        "to block_%d\n",
	...
	for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
		...
		if (lastRealNode->getOpCode().isBranch() && lastRealNode->getOpCode().isIf()) {
			// if we are at the end of the clone we have to get back to the origional execution stream so
			...
			newBlock->append(
			        TR::TreeTop::create(comp(), TR::Node::create(lastRealNode, TR::Goto, 0, nextTree)));
			...
			if (trace())
		        ...
		        && !(lastRealNode->getNumChildren() >= 1
		                && lastRealNode->getFirstChild()->getOpCodeValue() == TR::athrow)) {
			itr->_to->append(
			        TR::TreeTop::create(comp(), TR::Node::create(lastRealNode, TR::Goto, 0, nextTree)));
			if (trace())
...
TR_InvariantArgumentPreexistence::perform()
{
...

	{
		...
		// so TR_InvariantArgumentPreexistence can validate preexistence args and propagate info onto
		// parmSymbols
		{
			...
			if (argInfo) {
				if (trace())
					traceMsg(comp(),
					        "PREX:    Populating parmInfo of inlined method %s from argInfo %p\n",
				...
				for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL;
					...
					if (classIsFixed || classIsPreexistent) {
						...
						TR_ASSERT(!clazz || !clazzFromMethod || clazz == clazzFromMethod
						                || fe()->isInstanceOf(
						                           clazz, clazzFromMethod, true, true, true)
						                        == TR_yes,
						...
						if (classIsFixed) {
							...
							if (trace()) {
								char *clazzSig = TR::Compiler->cls.classSignature(
								        comp(), clazz, trMemory());
								traceMsg(comp(),
								        "PREX:        Parm %d class %p is currently "
								        "final %s\n",
							...
							if (clazz != clazzFromMethod) {
								...
								if (trace())
									traceMsg(comp(),
									        "PREX:          Parm %d class is "
									        "refined -- declared as %.*s\n",
									        index, len, sig);
							}
						} else {
				...
				if (trace())
					traceMsg(comp(),
					        "PREX:    No argInfo -- can't populate parmInfo for inlined method "
					        "%s\n",
					        feMethod->signature(trMemory()));
			}
		}
		...
		for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL; p = parms.getNext(), index++) {
			...
			if (info.getSymbol()) {
				...
				// make sure this path isn't executed iff getCurrentPeekingArgInfo is provided
				        ...
				        && !(comp()->isPeekingMethod() && comp()->getCurrentPeekingArgInfo())) {
					...
					if (arg) {
						if (arg->classIsFixed()) {
							...
							if (trace())
								traceMsg(comp(),
								        "PREX:      Parm %d symbol [%p] has fixed type "
								        "%p\n",
...
void
TR_InvariantArgumentPreexistence::processNode(TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
	else
...
void
TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount)
{
#ifdef J9_PROJECT_SPECIFIC
	...
	TR_ResolvedMethod *resolvedMethod = methodSymbol->getResolvedMethodSymbol()
	...
	if (methodSymbol->isComputed()) {
		...
		TR_ResolvedMethod *refinedMethod = symRef->getOwningMethod(comp())->getResolvedVirtualMethod(
		...
		if (refinedMethod) {
			...
			if (!needsAssumptions || addAssumptions) {
				if (!refinedMethod->isSameMethod(resolvedMethod)) {
					if (performTransformation(comp(),
					            "%sspecialize and devirtualize invoke [%p] on currently fixed or "
					            "final parameter %d [%p]\n",
						...
						isModified = true;
				                   ...
				                   "%sdevirtualize invoke [%p] on currently fixed or final parameter  "
				                   "%d [%p]\n",
				                   optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
					node->devirtualizeCall(treeTop);
					isModified = true;
			...
			if (isModified && !(receiverInfo.classIsRefined() && receiverInfo.classIsFixed())
			        && receiverSymbol && receiverSymbol->getParmSymbol()->getFixedType() == 0) {
				...
				bool inc =
				        comp()->getCHTable()->recompileOnClassExtend(comp(), receiverInfo.getClass());
				if (classInfo && inc)
		...
		TR_ASSERT((fixedOrFinalInfoExists || (receiverSymbol->getParmSymbol()->getFixedType() == 0)),
		        "Symbol %p is fixed ref type - but is not marked to be currently final "
		        "(fixedOrFinalInfoExists=%d, fixedType=%p)",
		...
		if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
			...
			if (methSymbol->isInterface() || methodSymbol) {
				...
				if (recompInfo && recompInfo->getMethodInfo()->getNumberOfInvalidations() >= 1
					...
					if (comp()->getPersistentInfo()
					                ->getRuntimeAssumptionTable()
					                ->getAssumptionCount(RuntimeAssumptionOnClassExtend)
					        < 100000)
					...
					if (devirtualize) {
						if (resolvedMethod->isAbstract())
							method = chTable->findSingleAbstractImplementer(
							        ...
							        comp());
						else if (!chTable->isOverriddenInThisHierarchy(resolvedMethod,
						                 receiverInfo.getClass(), symRef->getOffset(),
						                 comp())) {
							method = symRef->getOwningMethod(comp())
							                 ->getResolvedVirtualMethod(comp(),
							                         receiverInfo.getClass(),
							                         symRef->getOffset());
							newMethod = false;
						}
				...
				if (method && !method->virtualMethodIsOverridden()) {
					...
					if (addAssumptions
						...
						if (classInfo) {
							classInfo->setShouldNotBeNewlyExtended(
							...
							for (TR_PersistentClassInfo *subClassInfo =
								...
								TR_OpaqueClassBlock *subClass =
								        (TR_OpaqueClassBlock *)
								                subClassInfo->getClassId();
								subClassInfo->setShouldNotBeNewlyExtended(
								        comp()->getCompThreadID());
								if (comp()->getCHTable()->recompileOnNewClassExtend(
								            comp(), subClass))
	...
	if (comp()->isPeekingMethod() && receiverInfo.getClass() && !isInterface) {
		...
		if (canRefine)
			resolvedMethod = symRef->getOwningMethod(comp())->getResolvedVirtualMethod(
			        comp(), receiverInfo.getClass(), offset);
...
void
TR_InvariantArgumentPreexistence::processIndirectLoad(TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount)
{
	...
	if (addressChild->getSymbolReference()->hasKnownObjectIndex()) {
		...
		somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChainAt(comp(), node, addressChild,
		        (uintptrj_t *)addressChild->getSymbol()->castToStaticSymbol()->getStaticAddress(),
		        &removedNode);
	...
	if (removedNode) {
		if (removedNode->getOpCode().isTreeTop())
			...
			TR::TreeTop::create(
			        comp(), treeTop->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, removedNode));
...
TR_CheckcastAndProfiledGuardCoalescer::perform()
{
	...
	for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
		...
		switch (node->getOpCodeValue()) {
		case TR::BBStart: checkcastTree = NULL; break;
		case TR::checkcast:
		...
		case TR::NULLCHK:
			if (!checkcastTree
			        || node->getNullCheckReference() != checkcastTree->getNode()->getFirstChild()) {
		...
		case TR::ifacmpne:
			if (node->isProfiledGuard() && node->getFirstChild() && checkcastTree) {
				...
				if (virtualGuard && transformationIsValid && virtualGuard->getTestType() == TR_VftTest
				        ...
				        && performTransformation(comp(),
				                "%s Merging checkcast [%p] and profiled guard [%p] (basic case)\n",
					...
					if (checkcastTree->getNode()->getOpCodeValue() == TR::checkcastAndNULLCHK) {
						...
						TR::Node *nullCheckNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1,
						        passThroughNode,
			...
			break;
		case TR::ifacmpeq:
			// It's safe to move a checkcast across a null test on the same object if we fall through when
			// non-null
			if (checkcastTree) {
				if (node->getFirstChild() != checkcastTree->getNode()->getFirstChild()
					...
					// Jump to the next BBStart, next iteration of the loop will start at the tree
					// following the BBStart
					tt = tt->getNextTreeTop()->getNextTreeTop();
					traceMsg(comp(),
					        "Found suitable ifacmpeq [%p] after checkcast [%p], safe to continue "
					        "to next block in search of a profiled guard\n",
...
TR::Node *
TR_CheckcastAndProfiledGuardCoalescer::storeObjectInATemporary(TR::TreeTop *checkcastTree)
{
...
void
TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(TR::TreeTop *checkcastTree, TR::TreeTop *profiledGuardTree)
{
	...
	TR::Node *newCheckcastNode = TR::Node::createWithSymRef(TR::checkcast, 2,
	        TR::Node::createLoad(storeNode, tempCheckcastSymRef), classConst, 0,
...
TR_ColdBlockMarker::TR_ColdBlockMarker(TR::OptimizationManager *manager)
        : TR_BlockManipulator(manager), _exceptionsAreRare(true), _notYetRunMeansCold(false), _enableFreqCBO(true)
{}
	...
	if (hasBeenRun != TR_maybe) {
		...
		if (node->getSymbolReference()->getSymbol()->isClassObject()
			...
			if (comp()->compileRelocatableCode() && !comp()->getOption(TR_UseSymbolValidationManager)
				...
				// currentely node->hasUnresolvedSymbolReference() returns true more often for AOT than
				// non-AOT beacause of literals() vs romLiterals() used in relocatableresolvedj9method
...
TR_ColdBlockOutlining::TR_ColdBlockOutlining(TR::OptimizationManager *manager) : TR_ColdBlockMarker(manager) {}
...
TR_ColdBlockOutlining::reorderColdBlocks()
{
	...
	for (currentTree = comp()->getStartTree(); currentTree != NULL; currentTree = exitTree->getNextTreeTop()) {
		...
		if (!coldBlock(
		            currentBlock, comp()) /*currentBlock->isCold()*/ /* && !currentBlock->isRare(comp()) */) {
		...
		// FIXME: the following should be an assume, but i am hesitant.. If anybody runs the cold propagtion
		// code ever at fixed opt levels (count=0) then we could results with the entry block as cold
		//
		if (prevOfStart == NULL)
			return;
...
TR_BlockManipulator::breakFallThrough(TR::Block *faller, TR::Block *fallee, bool isOutlineSuperColdBlock)
{
	...
	if (lastNode->getOpCode().isBranch() || lastNode->getOpCode().isJumpWithMultipleTargets()) {
		...
		TR::Block *gotoBlock = TR::Block::createEmptyBlock(lastNode, comp(),
		        (fallee->getFrequency() < faller->getFrequency()) ? fallee->getFrequency()
		                                                          : faller->getFrequency(),
		        fallee);
		...
		if (firstChild->getReferenceCount() == 1) {
			if (!firstChild->getOpCode().hasSymbolReference()
			        && performTransformation(comp, "%sUnlink trivial %s (%p) of %s (%p) with refCount==1\n",
			                optDetails, treeTop->getNode()->getOpCode().getName(), treeTop->getNode(),
			                firstChild->getOpCode().getName(), firstChild)) {
				if (trace)
					traceMsg(comp,
					        "\tfound trivially anchored ttNode %p with firstChild %s (%p -- "
					        "refCount == 1)\n",
				...
				for (int32_t i = 0; i < firstChild->getNumChildren(); i++) {
					...
					if (!grandChild->getOpCode().isLoadConst()
						...
						// use insertAfter so the newly anchored trees get visited in the next
						// interation(s)
				...
				if (trace)
					traceMsg(comp,
					        "\t\tremove trivially anchored ttNode %p with firstChild %s (%p) "
					        "treetop\n",
					        ttNode, firstChild->getOpCode().getName(), firstChild);
				treeTop->unlink(true);
			}
			...
			if (trace)
				traceMsg(comp,
				        "\tadd ttNode %p with firstChild %s (%p, refCount %d) to commonedTreeTopList\n",
				        ttNode, firstChild->getOpCode().getName(), firstChild,
...
void
TR_TrivialDeadTreeRemoval::processCommonedChild(TR::Node *child, TR::TreeTop *treeTop,
        List<TR::TreeTop> &commonedTreeTopList, char *optDetails, TR::Compilation *comp)
{
	...
	if (child->getReferenceCount() > 1) {
		if (!commonedTreeTopList.isEmpty()) {
			...
			if (trace)
				traceMsg(comp,
				        "commonedTreeTopList is not empty and found a commoned child %s (%p, refCount "
				        "%d)\n",
			...
			while (listTT) {
				...
				// if listTT == treeTop then the commoned referenced is under the same treetop as the
				// tracked treetop tt
				//   a
				//   =>a
				//
				if (trace)
					traceMsg(comp,
					        "\tcomparing listTT %p with firstChild %s (%p) to commoned child %s "
					        "(%p, refCount %d) (listTT == _currentTreeTop -- %s)\n",
					        listTT->getNode(),
				...
				if (listTT->getNode()->getFirstChild() == child) {
					if (listTT != treeTop) {
						if (performTransformation(comp,
						            "%sFound commoned reference to child %s (%p) so unlink %s "
						            "(0x%p)\n",
						...
						if (treeTop->getNode()->getNumChildren() == 1
							...
							if (performTransformation(comp,
							            "%sFound commoned reference to single child %s "
			...
			if (treeTop->getNode()->getNumChildren() == 1
			        ...
			        && treeTop->getNode()->getFirstChild() == child) {
				if (performTransformation(comp,
				            "%sFound commoned reference to single child %s (%p) case 2 so unlink %s "
				            "(0x%p)\n",
				            optDetails, child->getOpCode().getName(), child,
...
TR_TrivialBlockExtension::performOnBlock(TR::Block *block)
{
	...
	if (block->isExtensionOfPreviousBlock()) {
		...
		if (pred != block->getPrevBlock()) {
			...
			if (trace())
				traceMsg(comp(),
				        "BlockExtension: block_%d ends in a switch and so we will not mark block_%d as "
				        "an extension\n",
				        pred->getNumber(), block->getNumber());
		} else {
			if (performTransformation(comp(),
