    ...
    if (currentNode->getOpCodeValue() == TR::arraycopy) {
        ...
        if ((arraycopyDestAddressNode == addressChild)
            ...
            *scratch &= *readAndWrittenSymbols;
            ...
            if (scratch->isEmpty()
                && performTransformation(comp(),
                       "%sEliminating some stores done by arrayset node %p because stores done by a following "
            ...
            while (++throttle < 10) {
                if ((node->getReferenceCount() > 1 && node->isInternalPointer() && node->getNumChildren() == 2
                    ...
                    || (node->getReferenceCount() == 1 && node->isInternalPointer() && node->getNumChildren() == 2
                           ...
                           && node->getFirstChild()->isInternalPointer() && node->getFirstChild()->getNumChildren() == 2
                           && (node->getSecondChild()->getOpCode().isLoadConst()
        ...
        if ((commonedElement->getData() == node)
            || (checkSymRefs
                   && ((commonedElement->getData()->getSymbolReference() == node->getSymbolReference())
                          || aliases.ValueAt(
                                 commonedElement->getData()->getSymbolReference()->getReferenceNumber())))) {
            if (prevNodeElem) {
...
int32_t TR_InvariantArgumentPreexistence::perform()
{
...

    {
        ...
        if (_isOutermostMethod) {
            if (trace())
                traceMsg(
                    comp(), "PREX:    Populating parmInfo of outermost method %s\n", feMethod->signature(trMemory()));
                ...
                for (TR::ParameterSymbol* p = parms.getFirst(); p != NULL; p = parms.getNext(), index++) {
                    ...
                    if (classIsFixed || classIsPreexistent) {
                        ...
                        TR_ASSERT(!clazz || !clazzFromMethod || clazz == clazzFromMethod
                            ...
                            "Type from argInfo should be more specific clazz %p clazzFromMethod %p", clazz,
                            clazzFromMethod);
                        ...
                        if (info.hasKnownObjectIndex()) {
                            ...
                            if (trace())
                                traceMsg(comp(), "PREX:      Parm %d symbol [%p] is known object obj%d\n", index,
                                    symbol, symbol->getKnownObjectIndex());
                        }
                    }
                }
...
void TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node* node, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    if (methodSymbol->isComputed()) {
        ...
        TR_ASSERT((fixedOrFinalInfoExists || (receiverSymbol->getParmSymbol()->getFixedType() == 0)),
            "Symbol %p is fixed ref type - but is not marked to be currently final (fixedOrFinalInfoExists=%d, "
            "fixedType=%p)",
        ...
        if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
            ...
            if (methSymbol->isInterface() || methodSymbol) {
                ...
                if (recompInfo && recompInfo->getMethodInfo()->getNumberOfInvalidations() >= 1
                    ...
                    if (devirtualize) {
                        if (resolvedMethod->isAbstract())
                            method = chTable->findSingleAbstractImplementer(receiverInfo.getClass(),
                                symRef->getOffset(), node->getSymbolReference()->getOwningMethod(comp()), comp());
                ...
                if (method && !method->virtualMethodIsOverridden()) {
                    ...
                    if (addAssumptions
                        && performTransformation(comp(),
                               "%sspecialize and devirtualize invoke [%p] based on only a single implementation for "
                               "call on parameter %d [%p]\n",
                               optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
                ...
                if (virtualGuard && transformationIsValid && virtualGuard->getTestType() == TR_VftTest
                    ...
                    if (checkcastTree->getNode()->getOpCodeValue() == TR::checkcastAndNULLCHK) {
                        TR::Node* passThroughNode
                            = TR::Node::create(TR::PassThrough, 1, checkcastTree->getNode()->getFirstChild());
                        TR::Node* nullCheckNode = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passThroughNode,
        ...
        case TR::ifacmpeq:
            ...
            if (checkcastTree) {
                if (node->getFirstChild() != checkcastTree->getNode()->getFirstChild()
                    ...
                    traceMsg(comp(),
                        "Found suitable ifacmpeq [%p] after checkcast [%p], safe to continue to next block in search "
                        "of a profiled guard\n",
...
void TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(TR::TreeTop* checkcastTree, TR::TreeTop* profiledGuardTree)
{
    ...
    TR::Node* newCheckcastNode
        = TR::Node::createWithSymRef(TR::checkcast, 2, TR::Node::createLoad(storeNode, tempCheckcastSymRef), classConst,
            0, comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(comp()->getMethodSymbol()));
