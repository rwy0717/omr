        ...
        , _dominators(c->getFlowGraph()->getNextNodeNumber() + 1, static_cast<TR::Block *>(NULL), _region)
{
	...
	if (_topDfNum != _numNodes - 1) {
		if (_postDominators) {
			...
			if (trace())
				traceMsg(comp(),
				        "Some blocks are not reachable from exit. Post-dominator info is invalid.\n");
			return;
		} else
...
TR_Dominators::initialize(TR::Block *start, BBInfo *nullParent)
{
	...
	while (!stack.empty()) {
		...
		//
		StackInfo::list_type &exceptionSuccessors = _postDominators ? block->getExceptionPredecessors()
...
TR_PostDominators::findControlDependents()
{
	...
	for (i = 0; i < nextNodeNumber; i++)
		_directControlDependents[i] = new (_region)
		        TR_BitVector(nextNodeNumber, _region); //.AddEntry(comp()->allocator("PostDominators"));
