...
TR_LocalReordering::TR_LocalReordering(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
void
TR_LocalReordering::moveStoresEarlierIfRhsAnchored(
...
bool
TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop *storeTree, TR::TreeTop *treeTop, bool strictCheck)
{
	...
	while (currentTree != treeTop) {
		...
		if (!stopHere) {
			if (currentNode->getOpCode().isCheckCast()) // Moving a store earlier than a checkcast usually
		...
		if (stopHere) {
			if (performTransformation(comp(),
			            ...
			            currentTree->getNextTreeTop()->getNode(), storeTree->getPrevTreeTop()->getNode(),
	...
	if (!foundInsertionSpot
		...
		// dumpOptDetails(comp(), "\n%sInserting Definition @ 2 : [%p] between %p and %p (earlier between %p and
...
//
void
TR_LocalReordering::setUseTreeForSymbolReferencesIn(TR::TreeTop *treeTop, TR::Node *node, vcount_t visitCount)
{
...
//
void
TR_LocalReordering::insertDefinitionBetween(TR::TreeTop *treeTop, TR::TreeTop *exitTree)
{
	if (treeTop == exitTree)
		return;
	...
	while (!(currentTree == exitTree)) {
		...
		if (isAnySymInDefinedOrUsedBy(currentNode, visitCount1)) {
			if (performTransformation(comp(),
			            ...
			            OPT_DETAILS, originalNode, currentTree->getPrevTreeTop()->getNode(),
			            currentTree->getNode(), originalTree->getPrevTreeTop()->getNode(),
...
void
TR_LocalReordering::collectSymbolsUsedAndDefinedInNode(TR::Node *node, vcount_t visitCount)
{
	if (node->getVisitCount() == visitCount)
		return;
...
//
bool
TR_LocalReordering::isAnySymInDefinedOrUsedBy(TR::Node *node, vcount_t visitCount)
{
	if (visitCount == node->getVisitCount())
		return false;
