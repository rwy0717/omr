...
{
	if (!infoIsValid()) {
		// TR_ASSERT(0, "osr def analysis failed. need to undo whatever we did for OSR and disable OSR but we
		// don't. Implementation is not completed.\n");
			...
			while (cursor.hasMoreElements()) {
				...
				if (methodSymbol->sharesStackSlot(defSymRef)) {
					...
					comp()->getOSRCompilationData()->addSlotSharingInfo(point->getByteCodeInfo(),
					        slot, symRefNum, symRefOrder, defSymRef->getSymbol()->getSize(),
					        takesTwoSlots);
					if (trace()) {
						TR_ByteCodeInfo &bcInfo = point->getByteCodeInfo();
						traceMsg(comp(),
						        "added (callerIndex=%d, bcIndex=%d)->(slot=%d, ref#=%d) at OSR "
						        "point %d side %d def %d\n",
...
TR_OSRDefInfo::addSharingInfo(AuxiliaryData &aux)
{
				...
				*aux._defsForSymbol[symIndex] = unionDef;
				...
				if (!prevTwoSlotUnionDef->isEmpty()) {
					...
					for (TR::SymbolReference *prevSymRef = prevppsIt.getFirst(); prevSymRef;
						...
						bool doesPrevTakesTwoSlots = prevdt == TR::Int64
						        || prevdt == TR::Double;
						if (doesPrevTakesTwoSlots) {
							uint16_t prevSymIndex =
							        prevSymRef->getSymbol()->getLocalIndex();
	...
	for (int i = 0; autosListArray && i < autosListArray->size(); ++i) {
		...
		if ((autosList.getSize() > 1) || isTwoSlotSymRefAtThisSlot || isTwoSlotSymRefAtPrevSlot) {
			if (!autosIt.getFirst()
			        || (autosIt.getFirst()->getCPIndex() >= _methodSymbol->getFirstJitTempIndex()))
				continue;
			unionDef.empty();
			twoSlotUnionDef->empty();
	...
	*/
	...
	if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
		...
		buildOSRDefs(comp()->getStartTree()->getNode(),
		        blockInfo[comp()->getStartTree()->getNode()->getBlock()->getNumber()], NULL, nextOsrPoint, NULL,
		        aux);
		nextOsrPoint = NULL;
	}
...
void
TR_OSRDefInfo::buildOSRDefs(TR::Node *node, void *vanalysisInfo, TR_OSRPoint *osrPoint, TR_OSRPoint *osrPoint2,
...
TR_OSRDefAnalysis::TR_OSRDefAnalysis(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
...
TR_OSRDefAnalysis::perform()
{
	if (comp()->getOption(TR_EnableOSR)) {
		...
		if (trace())
			traceMsg(comp(),
			        "Options is not enabled -- returning from OSR reaching definitions analysis.\n");
		return 0;
	}
	...
	if (!requiresAnalysis()) {
		if (trace()) {
			traceMsg(comp(),
			        "%s OSR reaching definitions analysis is not required because there is no sharing\n",
			        optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
		...
		if (comp()->getOption(TR_TraceOSR)) {
			traceMsg(comp(),
			        "%s OSR reaching definitions analysis is not required because OSR is not supported\n",
			        optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
			traceMsg(comp(), "Returning...\n");
		}
		return 0;
		...
		if (osrMethodData->hasSlotSharingOrDeadSlotsInfo()) {
			if (trace()) {
				traceMsg(comp(),
				        "%s OSR reaching definition analysis is not required as it has already been "
				        "calculated\n",
				        optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
				traceMsg(comp(), "Returning...\n");
			}
...
TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
{
	if (!comp()->getOption(TR_EnableOSR)) {
		...
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "Should not perform OSRLiveRangeAnalysis -- Not required because we are peeking\n");
		return false;
	} else if (!comp()->supportsInduceOSR()) {
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "Should not perform OSRLiveRangeAnlysis -- OSR is not supported under the current "
			        "configuration\n");
		return false;
	} else if (comp()->getOSRMode() == TR::involuntaryOSR) {
		static const char *disableOSRPointDeadslotsBookKeeping = feGetEnv("TR_"
		                                                                  "DisableOSRPointDeadslotsBookKeepin"
		                                                                  "g");
		if (comp()->getOption(TR_MimicInterpreterFrameShape)) {
			if (comp()->getOption(TR_TraceOSR))
				traceMsg(comp(),
				        "No need to perform OSRLiveRangeAnlysis under mimic interpreter frame shape\n");
			return false;
		} else if (disableOSRPointDeadslotsBookKeeping) // save some compile time
		{
			if (comp()->getOption(TR_TraceOSR))
				traceMsg(comp(),
				        "Dead slots bookkeeping is disabled and therefore OSRLiveRangeAnlysis is not "
				        "needed\n");
			return false;
		}
	}
	...
	if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) {
		if (comp()->getOption(TR_TraceOSR)) {
			if (!_sharedSymRefs->isEmpty())
				traceMsg(comp(),
				        "OSRLiveRangeAnalysis is disabled but it is required for correctness. OSR "
...
TR_OSRLiveRangeAnalysis::partialAnalysis()
{
	...
	if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
		...
		pendingPushLiveRangeInfo(
		        comp()->getStartTree()->getNode(), NULL, _pendingPushSymRefs, osrPoint, osrMethodData);
 ...
 */
void
TR_OSRLiveRangeAnalysis::pendingPushLiveRangeInfo(TR::Node *node, TR_BitVector *liveSymRefs,
	...
	if (liveSymRefs && !liveSymRefs->isEmpty()) {
		...
		*_workBitVector &= *slotSharingSymRefs;
		...
		while (bvi.hasMoreElements()) {
			...
			comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(), slot,
			        symRefNum, symRefOrder, symRef->getSymbol()->getSize(), takesTwoSlots);
		}
	}
        ...
        TR::Node *node, TR_BitVector *livePendingPushSymRefs, TR_OSRPoint *osrPoint)
{
	...
	if (deadPPSSlots && !deadPPSSlots->isEmpty()) {
		...
		while (bvi.hasMoreElements()) {
			...
			comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(), slot,
			        -1 /* symRefNum */, -1 /* symRefOrder */,
			        TR::Compiler->om.sizeofReferenceAddress() /*symSize*/, false /*takesTwoSlots*/);
		}
	}
			...
			if (slotInfo.symRefOrder == -1) {
				...
				if (slotInfo.slot < 0 && containsPendingPushes) {
					TR_ASSERT(deadPPSSlots,
					        "with slotInfo.slot < 0  and containsPendingPushes, there must be at "
					        "least on pending push symbol in this method");
				...
				if (slotInfo.slot >= 0) {
					TR_ASSERT_FATAL(deadAutoSlots,
					        "with slotInfos.slot >= 0, there must be at least on auto symbol in "
					        "this method");
 ...
 */
void
TR_OSRLiveRangeAnalysis::buildDeadSlotsInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
...
int32_t
TR_OSRLiveRangeAnalysis::fullAnalysis(bool includeParms, bool containsPendingPushes)
{
	TR_Structure *rootStructure = TR_RegionAnalysis::getRegions(comp(), optimizer()->getMethodSymbol());
	optimizer()->getMethodSymbol()->getFlowGraph()->setStructure(rootStructure);
	TR_ASSERT(rootStructure, "Structure is NULL");
...

	{
		...
		TR_Liveness liveLocals(comp(), optimizer(), comp()->getFlowGraph()->getStructure(), ignoreOSRuses, NULL,
		        false, includeParms);
		...
		while (block) {
			...
			for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
				...
				if (comp()->isOSRTransitionTarget(TR::postExecutionOSR)
					...
					// If it is the start of this block or has a different bytecode index, it cannot
					// be the OSR point
				...
				if (isPotentialOSRPoint && comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
					...
					bcInfo.setByteCodeIndex(
					        bcInfo.getByteCodeIndex() + comp()->getOSRInductionOffset(node));
					TR_OSRPoint *offsetOSRPoint = comp()->getMethodSymbol()->findOSRPoint(bcInfo);
					TR_ASSERT(offsetOSRPoint != NULL, "Cannot find a post OSR point for node %p",
					        node);

					buildOSRLiveRangeInfo(node, _liveVars, offsetOSRPoint,
					        liveLocalIndexToSymRefNumberMap, numBits, osrMethodData,
				...
				while (offsetOSRTreeTop && offsetOSRTreeTop != tt) {
					maintainLiveness(offsetOSRTreeTop->getNode(), NULL, -1, visitCount, &liveLocals,
					        _liveVars, block);
					offsetOSRTreeTop = offsetOSRTreeTop->getPrevTreeTop();
				}
				...
				if ((isPotentialOSRPoint ||
				            ...
				            // In involuntaryOSR mode, there are cases where the first evaluation point
				            // of a node is under a treetop that is not an OSRPoint but another treetop
				            // with the same bci is an OSRPoint. Need to call buildOSRLiveRangeInfo in
				            // this case for the first evaluation point to make sure the liveness info
				            // is bookkept correctly for this bci
					...
					if (comp()->getOption(TR_FullSpeedDebug))
						buildDeadSlotsInfo(node, _liveVars, osrPoint,
						        liveLocalIndexToSymRefNumberMap, containsPendingPushes);
				}
...
//
TR::TreeTop *
TR_OSRLiveRangeAnalysis::collectPendingPush(TR_ByteCodeInfo bci, TR::TreeTop *tt, TR_BitVector *liveVars)
{
		...
		if (node->getOpCode().isStoreDirect()) {
			...
			if (node->getFirstChild()->getReferenceCount() == 1) {
				TR::AutomaticSymbol *local =
				        node->getFirstChild()->getSymbolReference()->getSymbol()->getAutoSymbol();
				int32_t localIndex = local->getLiveLocalIndex();
				_liveVars->set(localIndex);
				if (comp()->getOption(TR_TraceOSR))
			...
			TR_ASSERT(0,
			        "Unexpected OSR related node %p found, should be either pending push store or anchored "
			        "load",
			        node);
		}
...
void
TR_OSRLiveRangeAnalysis::maintainLiveness(TR::Node *node, TR::Node *parent, int32_t childNum, vcount_t visitCount,
        TR_Liveness *liveLocals, TR_BitVector *liveVars, TR::Block *block)
{
	...
	if (node->getOpCode().isStoreDirect()) {
		...
		if (local && !local->isLiveLocalIndexUninitialized()) {
			...
			if ((!disallowOSRPPS3
			            || !_pendingPushSymRefs->get(node->getSymbolReference()->getReferenceNumber()))
 ...
 */
void
TR_OSRLiveRangeAnalysis::buildOSRSlotSharingInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
...
void
TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
 ...
 */
bool
TR_OSRExceptionEdgeRemoval::addDeadStores(TR::Block *osrBlock, TR_BitVector &dead, bool inited)
{
 ...
 */
void
TR_OSRExceptionEdgeRemoval::removeDeadStores(TR::Block *osrBlock, TR_BitVector &dead)
{
	TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
...
TR_OSRExceptionEdgeRemoval::perform()
{
	if (comp()->getOption(TR_EnableOSR)) {
		...
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "OSR Option not enabled -- returning from OSR exception edge removal analysis.\n");
		return 0;
	}
	...
	if (comp()->getOSRMode() == TR::involuntaryOSR) {
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "Involuntary OSR enabled -- returning from OSR exception edge removal analysis since "
			        "we implement involuntary OSR with exception edges.\n");
		return 0;
	}
	...
	if (comp()->osrInfrastructureRemoved()) {
		if (comp()->getOption(TR_TraceOSR))
			traceMsg(comp(),
			        "OSR infrastructure removed -- returning from OSR exception edge removal analysis "
			        "since we have already removed OSR infrastructure.\n");
		return 0;
	}
		...
		for (auto edge = block->getExceptionSuccessors().begin();
			...
			if (catchBlock->isOSRCatchBlock()
			        && performTransformation(comp(),
			                "%s: Remove redundant exception edge from block_%d at [%p] to OSR catch "
			                "block_%d at [%p]\n",
