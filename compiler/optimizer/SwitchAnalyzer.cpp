...
TR::SwitchAnalyzer::SwitchAnalyzer(TR::OptimizationManager *manager) : TR::Optimization(manager)
{
...
TR::SwitchAnalyzer::perform()
{
	...
	for (tt = comp()->getStartTree(); tt; tt = exitTree->getNextRealTreeTop()) {
		...
		if (lastTree->getNode()->getOpCode().isSwitch()) {
			...
			case TR::table:
			case TR::lookup: analyze(lastTree->getNode(), block); break;
			default: break;
			}
		}
	}
...
TR::SwitchAnalyzer::analyze(TR::Node *node, TR::Block *block)
{
	...
	// For the unsigned case, we better use unsigned if the numbers straddle the TR::getMaxSigned<TR::Int32>()
	// boundary. If that's the case, the highVal will be less than (in the signed sense) the lowVal.
	//
	if (highVal < lowVal)
		_signed = false;
	else
	...
	// Caveats are that zero straddling set must be subdivided and TR::getMaxSigned<TR::Int32>() straddling set
...
//
void
TR::SwitchAnalyzer::chainInsert(TR_LinkHead<SwitchInfo> *chain, SwitchInfo *info)
{
...
void
TR::SwitchAnalyzer::printInfo(TR_FrontEnd *fe, TR::FILE *pOutFile, TR_LinkHead<SwitchInfo> *chain)
{
	if (!pOutFile)
		return;
...
void
TR::SwitchAnalyzer::SwitchInfo::print(TR_FrontEnd *fe, TR::FILE *pOutFile, int32_t indent)
{
	if (!pOutFile)
		return;
	...
	switch (_kind) {
	case Unique: trfprintf(pOutFile, " -> %3d Unique\n", _target->getNode()->getBlock()->getNumber()); break;
	case Range: trfprintf(pOutFile, " -> %3d Range\n", _target->getNode()->getBlock()->getNumber()); break;
	case Dense:
...
TR::SwitchAnalyzer::peelOffTheHottestValue(TR_LinkHead<SwitchInfo> *chain)
{
	...
	if (topNode && (topNode->_kind == Unique) && (maxFreq > cutOffFrequency)) {
		...
		if (trace()) {
			traceMsg(comp(),
			        "Found a dominant entry in a dense node for target 0x%p with frequency of %f.\n",
...
TR::SwitchAnalyzer::checkIfDefaultIsDominant(SwitchInfo *start)
{
	...
	if (trace()) {
		traceMsg(comp(),
		        "Looking to see if the default case is dominant. Number of cases is %d, cut off frequency set "
		        "to %f\n",
	...
	for (SwitchInfo *temp = start; temp; temp = temp->getNext()) {
		if (temp->_freq >= cutOffFrequency) {
			...
			if (trace()) {
				traceMsg(comp(),
				        "Found child with frequency of %f. The default case isn't that dominant.\n",
				        temp->_freq);
			}
			break;
		}
        ...
        TR_LinkHead<SwitchInfo> *chain, TR_LinkHead<SwitchInfo> *bound, TR_LinkHead<SwitchInfo> *earlyUniques)
{
	...
	if (_switch->getOpCodeValue() == TR::lookup
		...
		// if the number of cases is so small that it's always better to convert the switch to ifs, skip checks
		// for backing out
	...
	// This flag indicates that the possible values of the switch operand are withing the min max range of all the
	// case statements
	...
	if (majorsInBound > 0) {
		if (majorsInBound <= MIN_SIZE_FOR_BIN_SEARCH) {
			...
			newBlock = binSearch(
			        bound->getFirst(), getLastInChain(bound), majorsInBound, rangeLeft, rangeRight);
			if (comp()->isOptServer()) {
	...
	if (majorsInChain > 0) {
		if (majorsInChain <= MIN_SIZE_FOR_BIN_SEARCH) {
			...
			newBlock = binSearch(
			        chain->getFirst(), getLastInChain(chain), majorsInChain, rangeLeft, rangeRight);
			if (comp()->isOptServer()) {
...
TR::Block *
TR::SwitchAnalyzer::binSearch(SwitchInfo *startNode, SwitchInfo *endNode, int32_t numMajors, CASECONST_TYPE rangeLeft,
...
TR::SwitchAnalyzer::linearSearch(SwitchInfo *start)
{
	...
	SwitchInfo *cursor = (comp()->isOptServer() && _switch->getOpCodeValue() == TR::lookup)
...
TR::Block *
TR::SwitchAnalyzer::addIfBlock(TR::ILOpCodes opCode, CASECONST_TYPE val, TR::TreeTop *dest)
{
...
TR::SwitchAnalyzer::addTableBlock(SwitchInfo *dense)
{
	...
	if (_switch && _switch->chkCannotOverflow())
		node->setCannotOverflow(true); // Pass on info to code gen that table will have all cases covered and
		                               // not use default case
	...
	if (branchTable) {
		node->setAndIncChild(
		        2 + upperBound + 1, TR::Node::createWithSymRef(_switch, TR::loadaddr, 0, branchTableSymRef));
	}
...
TR::SwitchAnalyzer::setupFrequencies(TR::Node *node)
{
	...
	int32_t *frequencies =
	        (int32_t *)trMemory()->allocateStackMemory(node->getCaseIndexUpperBound() * sizeof(int32_t));
