...
static OMR::TreeInfo *
findOrCreateTreeInfo(TR::TreeTop *treeTop, List<OMR::TreeInfo> *targetTrees, TR::Compilation *comp)
{
...
static bool
fixUpTree(TR::Node *node, TR::TreeTop *treeTop, TR::NodeChecklist &visited, bool &highGlobalIndex,
...
bool
collectSymbolReferencesInNode(TR::Node *node, TR::SparseBitVector &symbolReferencesInNode, int32_t *numDeadSubNodes,
        vcount_t visitCount, TR::Compilation *comp, bool *seenInternalPointer, bool *seenArraylet,
...
static bool
containsNode(TR::Node *containerNode, TR::Node *node, vcount_t visitCount, TR::Compilation *comp, int32_t *height,
        int32_t *maxHeight, bool &canMoveIfVolatile)
{
	...
	for (int32_t i = 0; i < containerNode->getNumChildren(); ++i) {
		if (containsNode(
		            containerNode->getChild(i), node, visitCount, comp, height, maxHeight, canMoveIfVolatile))
			return true;
	}
	(*height)--;
...
static bool
isSafeToReplaceNode(TR::Node *currentNode, TR::TreeTop *curTreeTop, bool *seenConditionalBranch, vcount_t visitCount,
        TR::Compilation *comp, List<OMR::TreeInfo> *targetTrees, bool &cannotBeEliminated, LongestPathMap &longestPaths)
{
...

	{
		...
		if (cantMoveUnderBranch
		        && (node->getOpCode().isBranch() || node->getOpCode().isJumpWithMultipleTargets()))
			return false;
...
static void
removeGlRegDep(TR::Node *parent, TR_GlobalRegisterNumber registerNum, TR::Block *containingBlock, TR::Optimization *opt)
{
...
TR::DeadTreesElimination::process(TR::TreeTop *startTree, TR::TreeTop *endTree)
{
	...
	for (TR::TreeTopIterator iter(startTree, comp()); iter != endTree; ++iter) {
		...
		if (!treeCanPossiblyBeRemoved(node)
			 ...
			 * Given the following trees, the anchoring node can still be removed  if the first treetop is
			 * removed
			 *
			 * treetop
			 *    xloadi #x
		...
		if (visitCount == child->getVisitCount()) {
			...
			if (callWithNoSideEffects) {
				treeTopCanBeEliminated = true;
			                   ...
			                   || opCodeValue == TR::multianewarray || opCodeValue == TR::MergeNew
			                   || opCodeValue == TR::checkcast || opCodeValue == TR::Prefetch
			                   || opCodeValue == TR::iu2l
				...
				if (child->getReferenceCount() == 1) {
					...
					safeToReplaceNode = isSafeToReplaceNode(child, iter.currentTree(),
					        &seenConditionalBranch, visitCount, comp(), &_targetTrees,
					        _cannotBeEliminated, longestPaths);
				}
			...
			if (treeTopCanBeEliminated) {
				...
				for (int32_t i = 0; i < child->getNumChildren(); ++i) {
					...
					if (fixUpTree(child->getChild(i), iter.currentTree(), visited, highGlobalIndex,
					            self(), visitCount))
						containsFloatingPoint = true;
					if (highGlobalIndex) {
		...
		if (treeTopCanBeEliminated) {
			...
			if (!node->getOpCode().isStoreReg() || (node->getFirstChild()->getReferenceCount() == 1)) {
				...
				//
				if (performTransformation(comp(),
				            "%sRemove tree : [" POINTER_PRINTF_FORMAT "] ([" POINTER_PRINTF_FORMAT "] "
				            "= "
				            "%s)"
				            "\n",
					...
					if (nextTree->getNode()->getOpCodeValue() == TR::Goto
						...
						requestOpt(
						        OMR::redundantGotoElimination, prevTree->getNode()->getBlock());
					}
				}
			} else {
				if (performTransformation(comp(),
				            "%sMove tree : [" POINTER_PRINTF_FORMAT "]([" POINTER_PRINTF_FORMAT "] = "
				            "%s) "
				            "to "
				            "end "
				            "of "
					...
					if (lastNode->getOpCode().isIf() && !lastNode->getOpCode().isCompBranchOnly()
					        ...
					        && ((prevLastNode->getFirstChild() == lastNode->getFirstChild())
					                || (prevLastNode->getFirstChild()
					                        == lastNode->getSecondChild()))) {
	...
	for (auto it = anchors.begin(); it != anchors.end(); ++it) {
		...
		if (!heapBase->getOpCode().isLoadConst()) {
			it->tree->insertAfter(
			        TR::TreeTop::create(comp(), TR::Node::create(heapBase, TR::treetop, 1, heapBase)));
		}
...
TR::TreeTop *
TR::DeadTreesElimination::findLastTreetop(TR::Block *block, TR::TreeTop *prevTree)
{
