...
TR::LocalDeadStoreElimination::LocalDeadStoreElimination(TR::OptimizationManager *manager) : TR::Optimization(manager)
{}
...
bool
TR::LocalDeadStoreElimination::isNonRemovableStore(TR::Node *storeNode, bool &seenIdentityStore)
{
	...
	// istore "b"  // not legal to remove this store as the auto/parm 'escapes' to the parent (owning) routine as
	// they share a stack
	//    z
	// return
...
void
TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop *entryTree, TR::TreeTop *exitTree)
{
	_curTree = entryTree;
	...
	while (!(_curTree == entryTree)) {
		...
		if (comp()->getVisitCount() == MAX_VCOUNT - 2) // Inc visit count will assert at MAX_VCOUNT - 1, so this
		                                               // gives us the one extra incVisitCount that might occur
		                                               // if we remove a store tree.
		{
			if (trace())
		...
		if (storeNode) {
			...
			// the logic below is:
			// when nonRemovableStore=false then removal of the store can happen if one of the following
			// conditions is true: 1) seenIdentityStore=true 2) haven't seen a use and not volatile and one
			// of the following is also true
			//    a) the block contains a return and not nested and sym is an auto or parm (i.e. val doesn't
			//    escape) b) an identical store has been seen
	...
	if (_treesAnchored) {
		...
		while (!(_curTree == exitTree)) {
			currentNode = _curTree->getNode();
			TR::Node *firstChild =
			        currentNode->getOpCodeValue() == TR::treetop ? currentNode->getFirstChild() : 0;
...
void
TR::LocalDeadStoreElimination::examineNode(
...
void
TR::LocalDeadStoreElimination::setExternalReferenceCountToTree(TR::Node *node, rcount_t *externalReferenceCount)
{
...
bool
TR::LocalDeadStoreElimination::areLhsOfStoresSyntacticallyEquivalent(TR::Node *node1, TR::Node *node2)
{
	int32_t childAdjust;
...
void
TR::LocalDeadStoreElimination::adjustStoresInfo(TR::Node *node, TR_BitVector &deadSymbolReferences)
{
...
//
TR::Node *
TR::LocalDeadStoreElimination::getAnchorNode(
        TR::Node *parentNode, int32_t nodeIndex, TR::Node *node, TR::TreeTop *treeTop, TR::NodeChecklist &visited)
{
	...
	if (node->getReferenceCount() > 1) {
		...
		// TR_FirstNodeReferenceTracker (when used) will mark the earliest reference of a tree commoned node (>
		// 1 ref in same tree) as the first overall reference therefore visit children nodes from first (0) to
		// last (node->getNumChildren()-1) so the first reference flag is propagated correctly with
		// setIsFirstReferenceToNode in the code just above. When not using TR_FirstNodeReferenceTracker the
		// future use count is decremented below to handle this case. NOTE: TR_FirstNodeReferenceTracker is now
		...
		for (int i = 0; i < node->getNumChildren(); i++) // visit from 0->numChildren-1 see above comment
		{
			...
			if (!visited.contains(child)) {
				...
				if (child->getLocalIndex() > 1) {
					// A tree commoned node (> 1 ref in same tree) must have its future use count
					// decremented here so the anchored node previously created in this function
					// will be correctly recognized as a first reference in examineNode()
					child->decLocalIndex();
				}
			}
		}
...
bool
TR::LocalDeadStoreElimination::examineNewUsesForKill(TR::Node *node, TR::Node *storeNode, List<TR::Node> *currentNews,
        List<TR::Node> *removedNews, TR::Node *parent, int32_t childNum, vcount_t visitCount)
{
