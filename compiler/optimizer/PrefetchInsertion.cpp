...
TR_PrefetchInsertion::perform()
{
	if (comp()->requiresSpineChecks()) {
		...
		if (trace())
			traceMsg(comp(),
			        "Spine checks required for array element accesses -- returning from prefetch "
			        "insertion.\n");
		return 0;
	}
...
TR_PrefetchInsertion::insertPrefetchInstructions()
{
	...
	for (ArrayAccessInfo *aai = it.getFirst(); aai; aai = it.getNext()) {
		...
		if (!skip
			...
			// Enable tree simplification on this block to normalize any address computation so we can
			// produce an optimal instruction sequence during instruction selection
			...
			aai->_treeTop->getEnclosingBlock()->getStructureOf()->getContainingLoop()->getBlocks(
			        &blocksInLoop);
			...
			prefetchNode = TR::Node::createWithSymRef(TR::Prefetch, 4, 4, addressNode, offsetNode, sizeNode,
			        typeNode, comp()->getSymRefTab()->findOrCreatePrefetchSymbol());
...
TR::Node *
TR_PrefetchInsertion::createDeltaNode(TR::Node *node, TR::Node *pivNode, int32_t deltaOnBackEdge)
{
	if (node == pivNode) {
	...
	if (node->getOpCode().hasSymbolReference())
		newNode = TR::Node::createWithSymRef(
		        node, node->getOpCodeValue(), node->getNumChildren(), node->getSymbolReference());
	else
...
void
TR_PrefetchInsertion::examineNode(
	...
	if (symbol && symbol->isArrayShadowSymbol() && node->getOpCodeValue() == TR::aloadi
		...
		if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub())
			...
			if (firstChild->getOpCode().isLoadDirect()
				...
				int64_t stepInBytes = mulConstBytes
				...
				if (isTreetopInLoop
					...
					//
					ArrayAccessInfo *aai = (ArrayAccessInfo *)trMemory()->allocateStackMemory(
					...
					if (trace())
						traceMsg(comp(),
						        "Found array access: node %p, access address node %p, biv node "
						        "%p\n",
...
bool
TR_PrefetchInsertion::isBIV(TR::SymbolReference *symRef, TR::Block *block, TR_BasicInductionVariable *&biv)
{
