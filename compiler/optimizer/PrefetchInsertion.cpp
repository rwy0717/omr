...
int32_t TR_PrefetchInsertion::perform()
{
    if (comp()->requiresSpineChecks()) {
        ...
        if (trace())
            traceMsg(
                comp(), "Spine checks required for array element accesses -- returning from prefetch insertion.\n");
        return 0;
    }
...
void TR_PrefetchInsertion::insertPrefetchInstructions()
{
    ...
    for (ArrayAccessInfo* aai = it.getFirst(); aai; aai = it.getNext()) {
        ...
        if (!skip
            ...
            // Enable tree simplification on this block to normalize any address computation so we can produce an
            // optimal instruction sequence during instruction selection
        ...
        if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub())
            ...
            if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
                ...
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
                    mulConstBytes
                        = (int64_t)((firstChild->getOpCode().isMul()) ? firstChild->getSecondChild()->getInt()
            ...
            if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
                ...
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
                    addConst
                        = (int64_t)((firstChild->getOpCode().isAdd()) ? (int64_t)firstChild->getSecondChild()->getInt()
            ...
            if (firstChild->getOpCode().isLoadDirect()
                ...
                if (isTreetopInLoop
                    && ((stepInBytes > 0 && stepInBytes <= TR::Compiler->vm.heapTailPaddingSizeInBytes())
                           || (stepInBytes < 0
                                  && -stepInBytes <= TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))) {
...
const char* TR_PrefetchInsertion::optDetailString() const throw() { return "O^O PREFETCH INSERTION: "; }
