...
TR_VirtualGuardTailSplitter::TR_VirtualGuardTailSplitter(TR::OptimizationManager *manager) : TR::Optimization(manager)
{
...
//
TR::Block *
TR_VirtualGuardTailSplitter::lookAheadAndSplit(VGInfo *guard, List<TR::Block> *stack)
{
	VGInfo *lastInfo = 0;
	List<VGInfo> deferred(trMemory());
	...
	while (next != last) {
		...
		if (_cfg->getStructure()) {
			next->getStructureOf()->getParent()->asRegion()->addSubNode(
			        new (_cfg->structureRegion()) TR_StructureSubGraphNode(new (_cfg->structureRegion())
...
TR_VirtualGuardTailSplitter::VGInfo *
TR_VirtualGuardTailSplitter::recognizeVirtualGuard(TR::Block *block, VGInfo *parent)
{
	if (!block->getExit())
		return 0;
	...
	if (call && guardNode->isProfiledGuard()) {
		...
		static char *profiledGuardSplitProbabilityThresholdStr = feGetEnv("TR_"
		                                                                  "ProfiledGuardSplitProbabilityThresho"
		                                                                  "ld");
   ...
   fclose(file);
	...
	if (trace())
		traceMsg(comp(), "Split Guard Block %d->(%d,%d), %d->(%d,%d)\n", block->getNumber(),
		        blockA->getNumber(), blockB->getNumber(), cloneG->getNumber(), cloneA->getNumber(),
		        cloneB->getNumber());
}
...
TR_InnerPreexistence::TR_InnerPreexistence(TR::OptimizationManager *manager) : TR::Optimization(manager) {}
	...
	for (i = 0; i < _numInlinedSites; ++i) {
		...
		if (trace()) {
			...
			traceMsg(comp(), "Site %d (block_%d, parent-block_%d): thisVN: %d, argsVNs: {", i,
			        info->getBlock()->getNumber(), parent ? parent->getBlock()->getNumber() : -1,
			        info->getThisVN());
	...
	for (i = 0; i < _numInlinedSites; ++i) {
		...
		while (bvi.hasMoreElements()) {
			...
			if (!performTransformation(comp(),
			            "%sDevirtualizing call guarded by block_%d preexisting on guard %d\n", OPT_DETAILS,
			            descendant->getBlock()->getNumber(), info->getBlock()->getNumber()))
				continue;
