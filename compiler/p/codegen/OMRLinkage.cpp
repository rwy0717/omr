...
void
OMR::Power::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{}
...
void
OMR::Power::Linkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method)
{}
...
TR::MemoryReference *
OMR::Power::Linkage::getOutgoingArgumentMemRef(int32_t argSize, TR::Register *argReg, TR::InstOpCode::Mnemonic opCode,
...
// saveOnly=false
TR::Instruction *
OMR::Power::Linkage::saveArguments(TR::Instruction *cursor, bool fsd, bool saveOnly)
{
	return self()->saveArguments(
	        cursor, fsd, saveOnly, self()->comp()->getJittedMethodSymbol()->getParameterList());
}

TR::Instruction *
OMR::Power::Linkage::saveArguments(
	...
	for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
		...
		if (lri >= 0) {
			...
			if (((ai < 0 || self()->hasToBeOnStack(paramCursor)) && !fsd) || (fsd && saveOnly)) {
				...
				if (twoRegs) {
					cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::stw,
					        firstNode,
					        new (self()->trHeapMemory())
				...
				if (TR::Compiler->target.is32Bit() && type.isInt64()) {
					...
					if (!twoRegs) // Low part needs to come from memory
					{
						...
						if (freeScratchable.isSet(aiLow)) {
							cursor = generateTrg1MemInstruction(self()->cg(),
							        TR::InstOpCode::lwz, firstNode,
		...
		else if (ai >= 0 && (!fsd || !saveOnly)) // lri<0: arg needs to come from memory
		{
			...
			case TR::Int32:
				if (freeScratchable.isSet(ai)) {
					cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lwz,
					        firstNode, REAL_REGISTER(REGNUM(ai)),
					        new (self()->trHeapMemory())
			...
			case TR::Address:
				if (freeScratchable.isSet(ai)) {
					cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::Op_load,
					        firstNode, REAL_REGISTER(REGNUM(ai)),
					        new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset,
			...
			case TR::Int64:
				if (TR::Compiler->target.is64Bit()) {
					if (freeScratchable.isSet(ai)) {
						cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::ld,
						        firstNode, REAL_REGISTER(REGNUM(ai)),
						        new (self()->trHeapMemory())
			...
			case TR::Float:
				if (freeScratchable.isSet(ai)) {
					cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lfs,
					        firstNode, REAL_REGISTER(REGNUM(ai)),
					        new (self()->trHeapMemory())
					                TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
					        cursor);
			...
			case TR::Double:
				if (freeScratchable.isSet(ai)) {
					cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lfd,
					        firstNode, REAL_REGISTER(REGNUM(ai)),
					        new (self()->trHeapMemory())
					                TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
					        cursor);
				...
				if (!(target < 0) && freeScratchable.isSet(target)) {
					...
					if (busyMoves[2][i1] != 0) {
						cursor = generateTrg1MemInstruction(self()->cg(), op, firstNode,
						        ...
						        new (self()->trHeapMemory()) TR::MemoryReference(
						                stackPtr, source, length, self()->cg()),
						        cursor);
					}
	...
	while ((paramCursor != NULL)
		...
		case TR::Double:
			if (hasToLoadFromStack && numFloatArgs < properties.getNumFloatArgRegs()) {
				argRegister =
				        machine->getRealRegister(properties.getFloatArgumentRegister(numFloatArgs));
		...
		case TR::Int64:
			if (hasToStoreToStack && numIntArgs < properties.getNumIntArgRegs()) {
				argRegister =
				        machine->getRealRegister(properties.getIntegerArgumentRegister(numIntArgs));
				if (TR::Compiler->target.is64Bit())
					cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::Op_st,
					        firstNode,
					        new (self()->trHeapMemory())
					                TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
					        argRegister, cursor);
...
OMR::Power::Linkage::pushAddressArg(TR::Node *child)
{
	...
	if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
		...
		if (isClass && self()->cg()->wantToPatchClassPointer((TR_OpaqueClassBlock *)child->getAddress(), child))
			loadAddressConstantInSnippet(self()->cg(), child, child->getAddress(), pushRegister, NULL,
			        TR::InstOpCode::Op_load, NULL, NULL);
		else {
...
{
	switch (returnType) {
	case TR::NoType: return TR_VoidReturn;
	case TR::Int8:
	case TR::Int16:
	case TR::Int32: return TR_IntReturn;
	case TR::Int64: return TR_LongReturn;
	case TR::Address: return TR::Compiler->target.is64Bit() ? TR_ObjectReturn : TR_IntReturn;
	case TR::Float: return TR_FloatReturn;
	case TR::Double: return TR_DoubleReturn;
	}
