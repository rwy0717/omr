                ...
                if (!(target < 0) && freeScratchable.isSet(target)) {
                    ...
                    case 1:
                        cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lwz, firstNode,
                            ...
                            new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, source, 4, self()->cg()),
                            cursor);
                        break;
                    case 2:
                        cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::ld, firstNode,
                            REAL_REGISTER(REGNUM(target)),
                            new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, source, 8, self()->cg()),
                            cursor);
                        break;
                    case 3:
                        cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lfs, firstNode,
    ...
    while ((paramCursor != NULL)
        ...
        case TR::Double:
            if (hasToStoreToStack && numFloatArgs < properties.getNumFloatArgRegs()) {
                argRegister = machine->getRealRegister(properties.getFloatArgumentRegister(numFloatArgs));
                cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::stfd, firstNode,
                    new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset, 8, self()->cg()), argRegister,
                    cursor);
            }
            numFloatArgs++;
            break;
...
TR_StackMemory OMR::Power::Linkage::trStackMemory() { return self()->trMemory(); }
