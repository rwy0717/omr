...
int32_t
OMR::ConstantDataSnippet::addConstantRequest(void *v, TR::DataType type, TR::Instruction *nibble0,
        ...
        bool isUnloadablePicSite)
{
	...
	case TR::Address: {
		...
		while (acursor != NULL) {
			...
			//   so be conservative and do not combine them (e.g. HCR versus profiled inlined site
			//   enablement)
	...
	default: TR_ASSERT(0, "Only float and address constants are supported. Data type is %s.\n", type.toString());
	}
...
bool
OMR::ConstantDataSnippet::getRequestorsFromNibble(TR::Instruction *nibble, TR::Instruction **q, bool remove)
{
	ListIterator<PPCConstant<double> > diterator(&_doubleConstants);
	PPCConstant<double> *dcursor = diterator.getFirst();
	...
	if (cg()->profiledPointersRequireRelocation()) {
		...
		if (node != NULL && node->getOpCodeValue() == TR::aconst) {
			if (comp->getOption(TR_UseSymbolValidationManager)) {
				...
				TR::Relocation *relo = new (cg()->trHeapMemory()) TR::ExternalRelocation(codeCursor,
				        (uint8_t *)node->getAddress(), (uint8_t *)type, TR_SymbolFromManager, cg());
...
OMR::ConstantDataSnippet::emitSnippetBody()
{
	...
	// Emit order is double, address, float. In this order we can ensure the 8 bytes alignment of double and
	// address.
