    ...
    for (i = 0; i < size; i += count) {
        ...
        if (count == 4) {
            ...
            *(int32_t*)iloc2 |= LO_VALUE(addr) & 0x0000ffff;
            ...
            cg()->addExternalRelocation(new (_cg->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(iloc1, iloc2,
                                            (uint8_t*)recordInfo, TR_AbsoluteMethodAddressOrderedPair, cg()),
                __FILE__, __LINE__, requestors[i]->getNode());
        }
    }
}
