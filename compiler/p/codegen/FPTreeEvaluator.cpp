...
TR::Register *
OMR::Power::TreeEvaluator::ibits2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		generateMvFprGprInstructions(
		        cg, node, gprSp2fpr, TR::Compiler->target.is64Bit(), target, cg->evaluate(child));
...
TR::Register *
OMR::Power::TreeEvaluator::fbits2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		TR::MemoryReference *tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
		generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, floatReg,
		...
		floatReg = TR::Compiler->target.cpu.id() >= TR_PPCp8 ? cg->gprClobberEvaluate(child)
		                                                     : cg->evaluate(child);
...
TR::Register *
OMR::Power::TreeEvaluator::lbits2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
		...
		if (TR::Compiler->target.is64Bit())
			...
			if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
				...
				generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, target,
				        longReg->getHighOrder(), longReg->getLowOrder(), tmp1);
				cg->stopUsingRegister(tmp1);
			} else
				generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, target,
				        longReg->getHighOrder(), longReg->getLowOrder());
		}
		cg->decReferenceCount(child);
	}
...
TR::Register *
OMR::Power::TreeEvaluator::dbits2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	...
	if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
		...
		if (TR::Compiler->target.is64Bit()) {
			...
			TR::MemoryReference *tempMR2 = new (cg->trHeapMemory())
			        TR::MemoryReference(node, *tempMR, 0, 8, cg);
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		if (offset != PTOC_FULL_INDEX) {
			if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
				...
				generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, srcRegister,
				        cg->getTOCBaseRegister(), HI_VALUE(offset));
				generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister,
				        new (cg->trHeapMemory())
				                TR::MemoryReference(srcRegister, LO_VALUE(offset), 4, cg));
...
TR::Register *
OMR::Power::TreeEvaluator::fconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return fconstHandler(node, cg, node->getFloat());
}

TR::Register *
OMR::Power::TreeEvaluator::dconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dloadHelper(
	...
	if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
		...
		if (node->getSymbolReference()->isUnresolved()) {
			if (tempMR->getBaseRegister() != NULL) {
				TR::addDependency(
				        dependencies, tempMR->getBaseRegister(), TR::RealRegister::NoReg, TR_GPR, cg);
			...
			if (tempMR->getIndexRegister() != NULL)
				TR::addDependency(
				        dependencies, tempMR->getIndexRegister(), TR::RealRegister::NoReg, TR_GPR, cg);
		}
		generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
		        (uintptrj_t)vrlRef->getSymbol()->castToMethodSymbol()->getMethodAddress(), dependencies,
		        vrlRef);
...
TR::Register *
OMR::Power::TreeEvaluator::vsplatsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	...
	if (node->getDataType() == TR::VectorInt8) {
		...
		if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg), tempReg);
		...
		if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8
			...
			if (TR::Compiler->target.is64Bit()) {
				...
				generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, trgReg,
				        srcReg->getHighOrder(), srcReg->getLowOrder(), tempVectorReg);
				generateTrg1Src2ImmInstruction(
				        cg, TR::InstOpCode::xxpermdi, node, trgReg, trgReg, trgReg, 0x0);
...
TR::Register *
OMR::Power::TreeEvaluator::vdgetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::vdsetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
...
TR::Register *
OMR::Power::TreeEvaluator::fstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	bool indirect = node->getOpCode().isIndirect();
	int childIndex = indirect ? 1 : 0;
	TR::Node *child = node->getChild(childIndex);
...
TR::Register *
OMR::Power::TreeEvaluator::freturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
 ...
 */
static TR::Register *
generateFusedMultiplyAdd(TR::Node *addNode, TR::InstOpCode::Mnemonic OpCode, TR::CodeGenerator *cg)
{
	...
	generateTrg1Src3Instruction(
	        cg, OpCode, addNode, trgRegister, source1Register, source2Register, source3Register);
...
OMR::Power::TreeEvaluator::fnegEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (noFMA) {
		...
		trgRegister = node->getOpCode().isFloat() ? cg->allocateSinglePrecisionRegister()
...
TR::Register *
OMR::Power::TreeEvaluator::int2dbl(TR::Node *node, TR::Register *srcReg, bool canClobber, TR::CodeGenerator *cg)
{
	...
	if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit())) {
		...
		if ((TR::Compiler->target.cpu.id() >= TR_PPCp7)
			...
			// Generate the code to produce the float result here, setting the register flag is done
			// afterwards
...
OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (((TR::Compiler->target.cpu.id() >= TR_PPCp7
		...
		if (node->getOpCodeValue() == TR::i2f) {
			...
			generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwzx, node, tempReg, tempMR); // TR::iu2f, must
			                                                                               // be P7
	...
	if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit())) {
		if (TR::Compiler->target.is64Bit())
			...
			if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
				TR::Register *tmp1 = cg->allocateRegister(TR_FPR);
				generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, trgReg,
				        srcReg->getHighOrder(), srcReg->getLowOrder(), tmp1);
				cg->stopUsingRegister(tmp1);
			} else
				generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, trgReg,
...
OMR::Power::TreeEvaluator::long2float(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.id() >= TR_PPCp7
	        && (cg->is64BitProcessor()
	                || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit()))) {
		if (TR::Compiler->target.is64Bit())
			generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg, srcReg);
		else {
			if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
// also handles TR::iffcmpeq
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
// also handles TR::iffcmpeq
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
// also handles TR::iffcmpeq
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpequEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpltuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpgeuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpgtuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::ifdcmpleuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	int64_t imm = 0;
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	int64_t imm = 0;
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpequEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpltuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpgeuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpgtuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dcmpleuEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// TR::InstOpCode::ble is really not greater than which is equivalent to less than equal to or unordered
...
static TR::Register *
singlePrecisionEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic OpCode, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
...
static TR::Register *
doublePrecisionEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic OpCode, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *source1Register = cg->evaluate(firstChild);
...
static TR::Register *
compareFloatAndSetOrderedBoolean(TR::InstOpCode::Mnemonic branchOp1, TR::InstOpCode::Mnemonic branchOp2, int64_t imm,
...
TR::Register *
OMR::Power::TreeEvaluator::fRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
...
TR::Register *
OMR::Power::TreeEvaluator::vRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
...
TR::Register *
OMR::Power::TreeEvaluator::fRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
...
TR::Register *
OMR::Power::TreeEvaluator::getstackEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::deallocaEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Register *srcReg = cg->evaluate(firstChild);
	const TR::PPCLinkageProperties &properties = cg->getProperties();
...
TR::Register *
OMR::Power::TreeEvaluator::xfRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
...
TR::Register *
OMR::Power::TreeEvaluator::xdRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *globalReg = node->getRegister();
