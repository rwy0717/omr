...
extern TR::Register* addConstantToInteger(
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t value, TR::CodeGenerator* cg);
extern TR::Register* addConstantToLong(
    TR::Node* node, TR::Register* srcReg, int64_t value, TR::Register* trgReg, TR::CodeGenerator* cg);
extern TR::Register* addConstantToLong(TR::Node* node, TR::Register* srcHigh, TR::Register* srcLow, int32_t valHigh,
...
extern void generateSignExtendInstruction(
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, TR::CodeGenerator* cg);
...
extern TR::Register* generateZeroExtendedTempRegister(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::InstOpCode::Mnemonic loadOp
        = (operandSize == 4) ? TR::InstOpCode::lwz : (operandSize == 2) ? TR::InstOpCode::lhz : TR::InstOpCode::lbz;

    TR::Register* srcReg = NULL;
    if (node->getReferenceCount() == 1 && node->getOpCode().isMemoryReference() && node->getRegister() == NULL) {
...
static TR::Register* generateSignExtendedTempRegister(TR::Node* node, TR::CodeGenerator* cg)
{
    int32_t operandSize = node->getOpCode().getSize();
    TR_ASSERT(operandSize == 4 || operandSize == 2 || operandSize == 1,
        "operand size %d should not need zero extension", operandSize);
    TR::InstOpCode::Mnemonic loadOp
        = (operandSize == 4) ? TR::InstOpCode::lwa : (operandSize == 2) ? TR::InstOpCode::lha : TR::InstOpCode::lbz;
...
static void computeCC_xcmpStrengthReducedCC(TR::Node* node, TR::Register* trgReg, TR::Register* src1Reg,
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (isSignedCompare && value <= UPPER_IMMED) {
            // when ccMask == 0xc then the value is encoded in the addi below in 16 bits as -value.  When value = 0x8000
            // = LOWER_IMMED then -value also equals (in 16 bits) 0x8000 so an immediate form cannot be used.
            if (ccMask == 0xc && (value > LOWER_IMMED))
                useImmedForm = true;
            else if (ccMask != 0xc && (value >= LOWER_IMMED))
                useImmedForm = true;
        } else if (!isSignedCompare
            && (uint64_t)value <= 0x7fff) // in unsigned case cannot let addi2 and subfic sign extend
            useImmedForm = true;
    }
    ...
    if (ccMask == 0xc) // 0,1 possible
    {
        ...
        if (useImmedForm)
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi2, node, trgReg, src1Reg, -secondChild->get64bitIntegralValue());
        else
    ...
    } else if (ccMask == 0xa || ccMask == 0x6) // 0,2 or 1,2 possible
    {
        ...
        if (ccMask == 0xa)
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, trgReg, trgReg, 1); // sign_bit+1 : 0->1, 1->2
    }
...
TR::Register* computeCC_compareUnsigned(TR::Node* node, TR::Register* trgReg, TR::Register* src1Reg,
...
TR::Register* OMR::Power::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getOpCode().isBooleanCompare() && firstChild->getRegister() == NULL
        ...
        TR::PPCControlFlowInstruction* i
            = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::iternary, node);
        ...
        if (valueToCheck->getOpCode().isCompareForOrder()) {
            if (valueToCheck->getChild(0)->getOpCode().is8Byte() || valueToCheck->getChild(1)->getOpCode().is8Byte()) {
                ...
                TR::TreeEvaluator::compareIntsForOrder(
                    cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(), cg), slowPathLabel, valueToCheck,
                    cg, valueToCheck->getOpCode().isUnsignedCompare(), true, PPCOpProp_BranchUnlikely);
            }
        } else {
            ...
            if (valueToCheck->getChild(0)->getOpCode().is8Byte() || valueToCheck->getChild(1)->getOpCode().is8Byte()) {
                ...
                TR::TreeEvaluator::compareLongsForEquality(
                    cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(), cg), slowPathLabel, valueToCheck,
                    cg, true, PPCOpProp_BranchUnlikely);
            } else {
