...
generateZeroExtendedTempRegister(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::InstOpCode::Mnemonic loadOp = (operandSize == 4) ? TR::InstOpCode::lwz : (operandSize == 2) ? TR::InstOpCode::lhz : TR::InstOpCode::lbz;
...
generateSignExtendedTempRegister(TR::Node* node, TR::CodeGenerator* cg)
{
    int32_t operandSize = node->getOpCode().getSize();
    TR_ASSERT(operandSize == 4 || operandSize == 2 || operandSize == 1, "operand size %d should not need zero extension", operandSize);
    TR::InstOpCode::Mnemonic loadOp = (operandSize == 4) ? TR::InstOpCode::lwa : (operandSize == 2) ? TR::InstOpCode::lha : TR::InstOpCode::lbz;
...
TR::Register* OMR::Power::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* resultReg = type.getDataType() == TR::Float ? cg->allocateSinglePrecisionRegister() : (type.getDataType() == TR::Double ? cg->allocateRegister(TR_FPR) : cg->allocateRegister(TR_GPR));
...
TR::Register* OMR::Power::TreeEvaluator::compareIntsForEquality(TR::InstOpCode::Mnemonic branchOp, TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (firstChild->getOpCodeValue() == TR::b2i && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() &&
       ...
       */
        ...
        TR::InstOpCode::Mnemonic opCode = (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq || node->getOpCodeValue() == TR::ifscmpeq || node->getOpCodeValue() == TR::ifsucmpeq || node->getOpCodeValue() == TR::ifbcmpeq || node->getOpCodeValue() == TR::ifbucmpeq)
            ? TR::InstOpCode::beq
            : TR::InstOpCode::bne;
