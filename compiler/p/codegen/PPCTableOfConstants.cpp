...
void *
TR_PPCTableOfConstants::initTOC(TR_FrontEnd *fe, TR::PersistentInfo *pinfo, uintptrj_t systemTOC)
{
...
#if defined(JITTEST)
	// Store at the index of TR_PPCnumRuntimeHelpers, the systemTOC. Note: Slot tocBase[TR_PPCnumRuntimeHelpers-1]
	// is free. See TR::TreeEvaluator::restoreTOCRegister.
	tocBase[TR_PPCnumRuntimeHelpers] = systemTOC;
	// Set Down Cursor to be TR_PPCnumRuntimeHelpers+1, so nothing between [0..TR_PPCnumRuntimeHelpers] should be
	// used for hashtable.
...
TR_PPCTableOfConstants::reinitializeMemory()
{
	...
	memset(tocManagement->getTOCBase() + tocManagement->getDownCursorAfterPermanentEntries(), 0,
	        (tocManagement->getTOCSize() >> 1)
	                - tocManagement->getDownCursorAfterPermanentEntries() * sizeof(uintptrj_t));
	memset(tocManagement->getTOCPtr(), 0,
	        (tocManagement->getTOCSize() >> 1)
	                - (tocManagement->getUpCursorAfterPermanentEntries() + 1) * sizeof(uintptrj_t));
        ...
        int32_t val, struct TR_tocHashEntry *tmplate, int32_t *offsetInSlot, TR::CodeGenerator *cg)
{
	...
	*offsetInSlot = 0;
	...
	case TR_FLAG_tocAddrKey: hash[idx]._addrKey = tmplate->_addrKey; break;
	case TR_FLAG_tocDoubleKey:
...
int32_t
TR_PPCTableOfConstants::lookUp(int8_t *name, int32_t len, bool isAddr, intptrj_t keyTag, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
...
int32_t
TR_PPCTableOfConstants::lookUp(TR::SymbolReference *symRef, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	if (!symRef->isUnresolved() || symRef->getCPIndex() < 0 || sym->isAddressOfClassObject()
	        || sym->isConstObjectRef() || sym->isConst()) {
		...
		*(intptrj_t *)local_buffer = addr;
		...
		TR_OpaqueClassBlock *myClass = comp->getOwningMethodSymbol(symRef->getOwningMethodIndex())
		                                       ->getResolvedMethod()
		                                       ->containingClass();
...
int32_t
TR_PPCTableOfConstants::allocateChunk(uint32_t numEntries, TR::CodeGenerator *cg, bool grabMonitor)
{
...
void
TR_PPCTableOfConstants::permanentEntriesAddtionComplete() // only zEmulator uses this
{
	TR_PPCTableOfConstants *tocManagement =
	        toPPCTableOfConstants(TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
	if (!tocManagement)
...
bool
TR_PPCTableOfConstants::isPermanentEntriesAddtionComplete() // only zEmulator uses this
{
	bool retValue;
	TR_PPCTableOfConstants *tocManagement =
	        toPPCTableOfConstants(TR_PersistentMemory::getNonThreadSafePersistentInfo()->getPersistentTOC());
