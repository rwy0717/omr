...
// check within "toPPCImmInstruction"
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
TR::PPCImm2Instruction* TR::PPCImm2Instruction::getPPCImm2Instruction() { return this; }
#endif
...
TR::PPCDepImmInstruction* TR::PPCDepImmInstruction::getPPCDepImmInstruction() { return this; }
...
void TR::PPCControlFlowInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    case TR::InstOpCode::setblong:
        ...
        cg()->traceRAInstruction(cursor
            = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 1, cursor));
        cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
    ...
    case TR::InstOpCode::setbool:
        if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && getCmpOpValue() == TR::InstOpCode::fcmpu
            ...
            if (getOpCode2Value() != TR::InstOpCode::bgt)
                cg()->traceRAInstruction(cursor
                    = generateTrg1Src2ImmInstruction(cg(), TR::InstOpCode::cror, currentNode, getTargetRegister(0),
                        getTargetRegister(0), getTargetRegister(0), getSourceImmediate(2), cursor));

            cg()->traceRAInstruction(
                cursor = generateTrg1Src2ImmInstruction(cg(), TR::InstOpCode::crxor, currentNode, getTargetRegister(0),
                    getTargetRegister(0), getTargetRegister(0), imm, cursor));
    ...
    case TR::InstOpCode::cfnan:
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::beq,
                                         PPCOpProp_BranchLikely, currentNode, label2, getSourceRegister(1), cursor));
        else
            cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                         cg(), TR::InstOpCode::beq, currentNode, label2, getSourceRegister(1), cursor));
        cg()->traceRAInstruction(cursor
            = generateTrg1ImmInstruction(cg(), TR::InstOpCode::lis, currentNode, getTargetRegister(0), 0x7fc0, cursor));
        cg()->traceRAInstruction(
            cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
        break;
    case TR::InstOpCode::cdnan:
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::beq,
                                         PPCOpProp_BranchLikely, currentNode, label2, getSourceRegister(0), cursor));
        else
            cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                         cg(), TR::InstOpCode::beq, currentNode, label2, getSourceRegister(0), cursor));
        if (TR::Compiler->target.is64Bit()) {
            cg()->traceRAInstruction(cursor = loadConstant(cg(), currentNode, (int64_t)CONSTANT64(0x7ff8000000000000),
                                         getTargetRegister(0), cursor));
        } else {
            cg()->traceRAInstruction(cursor = generateTrg1ImmInstruction(
                                         cg(), TR::InstOpCode::lis, currentNode, getTargetRegister(1), 0x7ff8, cursor));
            cg()->traceRAInstruction(cursor
                = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(0), 0, cursor));
        }
        cg()->traceRAInstruction(
            cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
        break;
