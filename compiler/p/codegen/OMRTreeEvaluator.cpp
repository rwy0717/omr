...
TR::Instruction *
loadAddressConstantInSnippet(TR::CodeGenerator *cg, TR::Node *node, intptrj_t address, TR::Register *trgReg,
        TR::Register *tempReg, TR::InstOpCode::Mnemonic opCode, bool isUnloadablePicSite, TR::Instruction *cursor)
{
	TR::Instruction *q[4];
	...
	TR::Instruction *c =
	        fixedSeqMemAccess(cg, node, 0, q, trgReg, trgReg, opCode, sizeof(intptrj_t), cursor, tempReg);
...
TR::Instruction *
loadAddressConstant(TR::CodeGenerator *cg, bool isRelocatable, TR::Node *node, intptrj_t value, TR::Register *trgReg,
...
// loadAddressConstant could be merged with loadConstant 64-bit
TR::Instruction *
loadAddressConstant(TR::CodeGenerator *cg, TR::Node *node, intptrj_t value, TR::Register *trgReg,
        TR::Instruction *cursor, bool isPicSite, int16_t typeAddress)
{
...
TR::Instruction *
loadActualConstant(TR::CodeGenerator *cg, TR::Node *node, intptrj_t value, TR::Register *trgReg,
...
TR::Instruction *
loadConstant(TR::CodeGenerator *cg, TR::Node *node, int32_t value, TR::Register *trgReg, TR::Instruction *cursor,
        bool isPicSite)
{
...
TR::Instruction *
loadConstant(TR::CodeGenerator *cg, TR::Node *node, int64_t value, TR::Register *trgReg, TR::Instruction *cursor,
        bool isPicSite, bool useTOC)
{
	...
	if (offset != PTOC_FULL_INDEX) {
		...
		if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
			cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, trgReg,
			        cg->getTOCBaseRegister(), cg->hiValue(offset), cursor);
		...
		if (isPicSite) {
			// If the node is a call then value must be a class pointer (interpreter profiling
			// devirtualization)
			if (node->getOpCode().isCall() || node->isClassPointerConstant())
				comp->getStaticPICSites()->push_front(cursor);
			else if (node->isMethodPointerConstant())
...
TR::Instruction *
fixedSeqMemAccess(TR::CodeGenerator *cg, TR::Node *node, intptrj_t addr, TR::Instruction **nibbles,
        ...
        TR::Instruction *cursor, TR::Register *tempReg)
{
	...
	if (TR::Compiler->target.is32Bit()) {
		nibbles[0] = cursor =
		        generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, baseReg, hiAddr, cursor);
		idx = 1;
	} else {
		if (tempReg == NULL) {
	...
	if (opCode == TR::InstOpCode::lxvdsx) {
		...
		if (TR::Compiler->target.is64Bit() && tempReg) {
			...
			nibbles[idx] = cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, baseReg,
			        baseReg, ((int32_t)loAddr) << 16 >> 16, cursor);
...
OMR::Power::TreeEvaluator::iloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (reverseLoad) {
		...
		// For compr refs caseL if monitored loads is supported and we are loading an address, use monitored
		// loads instruction
	...
	for (uint16_t i = 0; i < checkNode->getNumChildren() && !result; i++) {
		...
		// If the class ptr load is feeding anything other than a nullchk it's probably needed
			...
			result = true;
		        ...
		        && (child->getOpCodeValue() == TR::aloadi || child->getOpCodeValue() == TR::aload))
			result = true;
		else
...
OMR::Power::TreeEvaluator::aloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		if (TR::Compiler->om.generateCompressedObjectHeaders()
			...
			// if monitored loads is supported and we are loading an address, use monitored loads
			// instruction
...
OMR::Power::TreeEvaluator::lloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	} else // 32 bit target
	{
		...
		if (needSync) {
			...
			if (!node->getSymbolReference()->isUnresolved() && cg->is64BitProcessor()) // resolved as
			                                                                           // volatile
			{
				TR::Register *doubleReg = cg->allocateRegister(TR_FPR);
				...
				if (tempMR->getIndexRegister() != NULL)
					generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, highReg,
					        tempMR->getBaseRegister(), tempMR->getIndexRegister());
				else
			...
			TR::MemoryReference *lowMR = new (cg->trHeapMemory())
			        TR::MemoryReference(node, *highMR, 4, 4, cg);
...
TR::Register *
OMR::Power::TreeEvaluator::commonByteLoadEvaluator(TR::Node *node, bool signExtend, TR::CodeGenerator *cg)
{
	TR::Register *trgReg = cg->allocateRegister();
	TR::MemoryReference *tempMR;
...
TR::Register *
OMR::Power::TreeEvaluator::buloadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::istoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
#ifdef J9_PROJECT_SPECIFIC
...
TR::Register *
OMR::Power::TreeEvaluator::astoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	TR::Node *nodeChild;
...
TR::Register *
OMR::Power::TreeEvaluator::lstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
#ifdef J9_PROJECT_SPECIFIC
			                                                                           ...
			                                                                           // volatile
			{
				...
				if (TR::Compiler->target.isSMP()) {
					...
					TR::MemoryReference *tempMRStore2 = new (cg->trHeapMemory())
					        TR::MemoryReference(node, *tempMRStore1, 4, 4, cg);
					generateMemSrc1Instruction(
					        cg, TR::InstOpCode::stw, node, tempMRStore1, valueReg->getHighOrder());
					generateMemSrc1Instruction(
					        cg, TR::InstOpCode::stw, node, tempMRStore2, valueReg->getLowOrder());
					generateInstruction(cg, TR::InstOpCode::lwsync, node);
					...
					if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
						TR::Register *tmp1 = cg->allocateRegister(TR_FPR);
						generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, doubleReg,
						        valueReg->getHighOrder(), valueReg->getLowOrder(), tmp1);
						cg->stopUsingRegister(tmp1);
					} else
						generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, doubleReg,
						        valueReg->getHighOrder(), valueReg->getLowOrder());
				...
				TR::addDependency(
				        dependencies, valueReg->getHighOrder(), TR::RealRegister::gr4, TR_GPR, cg);
				TR::addDependency(
				        dependencies, valueReg->getLowOrder(), TR::RealRegister::gr5, TR_GPR, cg);
...
TR::Register *
OMR::Power::TreeEvaluator::bstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::MemoryReference *tempMR;
	TR::Node *valueChild;
	if (node->getOpCode().isIndirect()) {
...
TR::Register *
OMR::Power::TreeEvaluator::sstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::Register *
OMR::Power::TreeEvaluator::cstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::MemoryReference *tempMR;
	TR::Node *valueChild;
	if (node->getOpCode().isIndirect()) {
...
TR::Register *
OMR::Power::TreeEvaluator::vstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::InstOpCode::Mnemonic opcode;
	...
	case TR::VectorInt16:
	case TR::VectorInt32:
	case TR::VectorFloat: opcode = TR::InstOpCode::stxvw4x; break;
	case TR::VectorInt64:
	case TR::VectorDouble: opcode = TR::InstOpCode::stxvd2x; break;
	default:
...
TR::Register *
OMR::Power::TreeEvaluator::awrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::awrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// The wrtbar IL op represents a store with side effects.
	// Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
...
TR::Register *
OMR::Power::TreeEvaluator::dwrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// The wrtbar IL op represents a store with side effects.
	// Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
	TR::Node *sideEffectNode = node->getSecondChild();
...
TR::Register *
OMR::Power::TreeEvaluator::dwrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// The wrtbar IL op represents a store with side effects.
	// Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
	TR::Node *sideEffectNode = node->getThirdChild();
...
TR::Register *
OMR::Power::TreeEvaluator::fwrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// The wrtbar IL op represents a store with side effects.
	// Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
	TR::Node *sideEffectNode = node->getSecondChild();
...
TR::Register *
OMR::Power::TreeEvaluator::fwrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	// The wrtbar IL op represents a store with side effects.
	// Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
	TR::Node *sideEffectNode = node->getThirdChild();
...
TR::Register *
OMR::Power::TreeEvaluator::inlineVectorUnaryOp(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op)
{
...
TR::Register *
OMR::Power::TreeEvaluator::inlineVectorBinaryOp(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op)
{
	TR::Node *firstChild = node->getFirstChild();
...
TR::Register *
OMR::Power::TreeEvaluator::inlineVectorTernaryOp(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
...
OMR::Power::TreeEvaluator::vandEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::VectorInt8:
	case TR::VectorInt16:
	case TR::VectorInt32: opCode = TR::InstOpCode::vand; break;
	default: opCode = TR::InstOpCode::xxland; break;
	}
...
OMR::Power::TreeEvaluator::vorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::VectorInt8:
	case TR::VectorInt16:
	case TR::VectorInt32: opCode = TR::InstOpCode::vor; break;
	default: opCode = TR::InstOpCode::xxlor; break;
	}
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, opCode);
}
...
OMR::Power::TreeEvaluator::vxorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	case TR::VectorInt8:
	case TR::VectorInt16:
	case TR::VectorInt32: opCode = TR::InstOpCode::vxor; break;
	default: opCode = TR::InstOpCode::xxlxor; break;
	}
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, opCode);
}
...
OMR::Power::TreeEvaluator::viremEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::addi2, node, srcV1IdxReg,
	        new (cg->trHeapMemory()) TR::MemoryReference(node, srcV1, 16, cg));
	generateTrg1MemInstruction(cg, TR::InstOpCode::addi2, node, srcV2IdxReg,
	        new (cg->trHeapMemory()) TR::MemoryReference(node, srcV2, 16, cg));

	// store the src regs to memory
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcV1IdxReg, 16, cg), lhsReg);
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcV2IdxReg, 16, cg), rhsReg);
	...
	for (i = 0; i < 4; i++) {
		...
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcV1IdxReg, i * 4, 4, cg), trgAReg);
...
TR::Register *
OMR::Power::TreeEvaluator::vigetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::addi2, node, resReg,
	        new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 16, cg));
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, resReg, 16, cg), addrReg);
...
TR::Register *
OMR::Power::TreeEvaluator::getvelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8
		...
		return TR::TreeEvaluator::getvelemMemoryMoveHelper(node, cg); // transfers data through memory instead
		                                                              // of via direct move instructions.
	}
}

TR::Register *
OMR::Power::TreeEvaluator::getvelemDirectMoveHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (elementCount == 4) {
			 ...
			 * If the splat is skipped, the input data will be in srcVectorReg instead of
			 * intermediateResReg.
			 */
		...
		} else // elementCount == 2
		{
			...
			if (readElemOne) {
				generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxsldwi, node, intermediateResReg,
				        srcVectorReg, srcVectorReg, 0x2);
			}
		...
		if (firstChild->getDataType() == TR::VectorInt32) {
			/*
			 * Conditional statements are used to determine if the indexReg has the value 0, 1, 2 or 3.
			 * Other values are invalid. The element indicated by indexReg needs to be move to element 1 in
			 * intermediateResReg. In most cases, this is done by splatting the indicated value into every
			 * element of intermediateResReg. If the indicated value is already in element 1 of
			 * srcVectorReg, the vector is just copied to intermediateResReg. Finally, a direct move
			 * instruction is used to move the data we want from element 1 of the vector into a GPR.
			 */
			...
			generateTrg1Src1ImmInstruction(
			        cg, TR::InstOpCode::xxspltw, node, intermediateResReg, srcVectorReg, 0x0);
			generateLabelInstruction(cg, TR::InstOpCode::b, node, jumpLabelDone);
			...
			generateTrg1Src1ImmInstruction(
			        cg, TR::InstOpCode::xxspltw, node, intermediateResReg, srcVectorReg, 0x2);
			generateLabelInstruction(cg, TR::InstOpCode::b, node, jumpLabelDone);

			generateLabelInstruction(cg, TR::InstOpCode::label, node, jumpLabel3);
			generateTrg1Src1ImmInstruction(
			        cg, TR::InstOpCode::xxspltw, node, intermediateResReg, srcVectorReg, 0x3);
			...
			/*
			 * Conditional statements are used to determine if the indexReg has the value 0, 1, 2 or 3.
			 * Other values are invalid. The element indicated by indexReg needs to be move to element 0 in
			 * intermediateResReg. In most cases, this is done by splatting the indicated value into every
			 * element of intermediateResReg. If the indicated value is already in element 0 of
			 * srcVectorReg, the vector is just copied to intermediateResReg. Finally, a direct move
			 * instruction is used to move the data we want from element 0 of the vector into an FPR.
			 */
			generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, indexReg, 1);
			generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, jumpLabel23, condReg);
			...
			generateTrg1Src1ImmInstruction(
			        cg, TR::InstOpCode::xxspltw, node, intermediateResReg, srcVectorReg, 0x1);
			generateLabelInstruction(cg, TR::InstOpCode::b, node, jumpLabelDone);
			...
			/*
			 * Conditional statements are used to determine if the indexReg has the value 0 or 1. Other
			 * values are invalid. The element indicated by indexReg needs to be move to element 0 in
			 * intermediateResReg. If indexReg is 0, the vector is just copied to intermediateResReg. If
			 * indexReg is 1, the vector is rotated so element 1 is moved to slot 0. Finally, a direct move
			 * instruction is used to move the data we want from element 0 to the resReg. 64 bit int on a 32
			 * bit system is a special case where the result is returned in a pair register.
			 */
			generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, indexReg, 1);
			generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, jumpLabel1, condReg);
			generateTrg1Src2Instruction(
			...
			if (TR::Compiler->target.is32Bit() && firstChild->getDataType() == TR::VectorInt64) {
				generateMvFprGprInstructions(cg, node, fpr2gprHost32, false, highResReg, lowResReg,
				        intermediateResReg, tempVectorReg);
			} else if (firstChild->getDataType() == TR::VectorInt64) {
				generateMvFprGprInstructions(
...
TR::Register *
OMR::Power::TreeEvaluator::getvelemMemoryMoveHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateMemSrc1Instruction(cg, vecStoreOpCode, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, resReg, 16, cg), vectorReg);
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		if (firstChild->getDataType() == TR::VectorFloat || firstChild->getDataType() == TR::VectorDouble) {
			generateTrg1MemInstruction(cg, loadOpCode, node, fResReg,
			        new (cg->trHeapMemory())
			                TR::MemoryReference(resReg, elem * (16 / elementCount), 16 / elementCount, cg));
			...
			if (!TR::Compiler->target.cpu.isLittleEndian()) {
				...
				generateTrg1MemInstruction(cg, loadOpCode, node, highResReg,
				        new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8, 4, cg));
				generateTrg1MemInstruction(cg, loadOpCode, node, lowResReg,
				        new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8 + 4, 4, cg));
			} else {
				highResReg = resReg;
				lowResReg = cg->allocateRegister();
				generateTrg1MemInstruction(cg, loadOpCode, node, lowResReg,
...
TR::Register *
OMR::Power::TreeEvaluator::visetelemEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Node *thirdChild = node->getThirdChild();
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::addi2, node, addrReg,
	        new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 16, cg));
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, addrReg, 16, cg), vectorReg);
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, elem * 4, 4, cg), valueReg);
		...
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::mulli, node, offsetReg, idxReg, 4);
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, offsetReg, 4, cg), valueReg);
		cg->stopUsingRegister(offsetReg);
	}

	generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, resReg,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, addrReg, 16, cg));
...
TR::Register *
OMR::Power::TreeEvaluator::vimergeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
...
TR::Register *
OMR::Power::TreeEvaluator::vdmaddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorTernaryOp(node, cg, TR::InstOpCode::xvmaddadp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdnmsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorTernaryOp(node, cg, TR::InstOpCode::xvnmsubadp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdmsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvcmpgtdp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvcmpgedp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvcmpeqdp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvcmpgedp);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvcmpgtdp);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpanyHelper(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpallHelper(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpalleqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpallHelper(node, cg, TR::InstOpCode::xvcmpeqdp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpallgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpallHelper(node, cg, TR::InstOpCode::xvcmpgedp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpallgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpallHelper(node, cg, TR::InstOpCode::xvcmpgtdp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpallleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vdcmpallHelper(node, cg, TR::InstOpCode::xvcmpgedp_r);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpallltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vdcmpallHelper(node, cg, TR::InstOpCode::xvcmpgtdp_r);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpanyeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpanyHelper(node, cg, TR::InstOpCode::xvcmpeqdp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpanygeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpanyHelper(node, cg, TR::InstOpCode::xvcmpgedp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpanygtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vdcmpanyHelper(node, cg, TR::InstOpCode::xvcmpgtdp_r);
}

TR::Register *
OMR::Power::TreeEvaluator::vdcmpanyleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vdcmpanyHelper(node, cg, TR::InstOpCode::xvcmpgedp_r);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vdcmpanyltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vdcmpanyHelper(node, cg, TR::InstOpCode::xvcmpgtdp_r);
}
...
{
	switch (node->getDataType()) {
	case TR::VectorInt32: return TR::TreeEvaluator::vnegInt32Helper(node, cg);
	case TR::VectorFloat: return TR::TreeEvaluator::vnegFloatHelper(node, cg);
	case TR::VectorDouble: return TR::TreeEvaluator::vnegDoubleHelper(node, cg);
	default: TR_ASSERT(false, "unrecognized vector type %s\n", node->getDataType().toString()); return NULL;
	}
}

TR::Register *
OMR::Power::TreeEvaluator::vnegInt32Helper(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild;
	TR::Register *srcReg;
	TR::Register *resReg;
...
TR::Register *
OMR::Power::TreeEvaluator::vnegFloatHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorUnaryOp(node, cg, TR::InstOpCode::xvnegsp);
}

TR::Register *
OMR::Power::TreeEvaluator::vnegDoubleHelper(TR::Node *node, TR::CodeGenerator *cg)
{
...
{
	switch (node->getDataType()) {
	case TR::VectorInt32: return TR::TreeEvaluator::vmulInt32Helper(node, cg);
	case TR::VectorFloat: return TR::TreeEvaluator::vmulFloatHelper(node, cg);
	case TR::VectorDouble: return TR::TreeEvaluator::vmulDoubleHelper(node, cg);
	default: TR_ASSERT(false, "unrecognized vector type %s\n", node->getDataType().toString()); return NULL;
	}
}

TR::Register *
OMR::Power::TreeEvaluator::vmulInt32Helper(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild;
	TR::Node *secondChild;
...
TR::Register *
OMR::Power::TreeEvaluator::vmulFloatHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvmulsp);
}

TR::Register *
OMR::Power::TreeEvaluator::vmulDoubleHelper(TR::Node *node, TR::CodeGenerator *cg)
{
...
{
	switch (node->getDataType()) {
	case TR::VectorInt32: return TR::TreeEvaluator::vdivInt32Helper(node, cg);
	case TR::VectorFloat: return TR::TreeEvaluator::vdivFloatHelper(node, cg);
	case TR::VectorDouble: return TR::TreeEvaluator::vdivDoubleHelper(node, cg);
	default: TR_ASSERT(false, "unrecognized vector type %s\n", node->getDataType().toString()); return NULL;
	}
}

TR::Register *
OMR::Power::TreeEvaluator::vdivFloatHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvdivsp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdivDoubleHelper(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::xvdivdp);
}

TR::Register *
OMR::Power::TreeEvaluator::vdivInt32Helper(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::vcmpequw);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::vcmpgtsw);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::inlineVectorBinaryOp(node, cg, TR::InstOpCode::vcmpgtsw);
}
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
...
TR::Register *
OMR::Power::TreeEvaluator::vdmergeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = cg->evaluate(firstChild);
...
TR::Register *
OMR::Power::TreeEvaluator::vdsqrtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
bool
OMR::Power::TreeEvaluator::inlineVectorCompareBranch(
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpallHelper(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op, int nBit)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpanyHelper(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op, int nBit)
{
	TR::Node *firstChild = node->getFirstChild();
	TR::Node *secondChild = node->getSecondChild();
	TR::Register *lhsReg = NULL, *rhsReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::vicmpalleqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpallHelper(node, cg, TR::InstOpCode::vcmpeuwr, 25);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpallneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::unImpOpEvaluator(node, cg);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpallgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpallHelper(node, cg, TR::InstOpCode::vcmpgswr, 25);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpallgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vicmpallHelper(node, cg, TR::InstOpCode::vcmpgswr, 27);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpallltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vicmpallHelper(node, cg, TR::InstOpCode::vcmpgswr, 25);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpallleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpallHelper(node, cg, TR::InstOpCode::vcmpgswr, 27);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpanyeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpanyHelper(node, cg, TR::InstOpCode::vcmpeuwr, 27);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpanyneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::vicmpanygtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpanyHelper(node, cg, TR::InstOpCode::vcmpgswr, 27);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpanygeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vicmpanyHelper(node, cg, TR::InstOpCode::vcmpgswr, 25);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpanyltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	node->swapChildren();
	return TR::TreeEvaluator::vicmpanyHelper(node, cg, TR::InstOpCode::vcmpgswr, 27);
}
TR::Register *
OMR::Power::TreeEvaluator::vicmpanyleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return TR::TreeEvaluator::vicmpanyHelper(node, cg, TR::InstOpCode::vcmpgswr, 25);
}

static void
inlineArrayCopy(TR::Node *node, int64_t byteLen, TR::Register *src, TR::Register *dst, TR::CodeGenerator *cg)
{
	if (byteLen == 0)
		return;
	...
	bool supportsLEArrayCopyInline = (processor >= TR_PPCp8) && !disableLEArrayCopyInline
	        && TR::Compiler->target.cpu.isLittleEndian() && TR::Compiler->target.cpu.hasFPU()
	...
	if (groups != 0) {
		...
		if (supportsLEArrayCopyInline) {
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, memRefSize, memRefSize, cg),
			        fpRegs[2]);
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, 2 * memRefSize, memRefSize, cg),
			        fpRegs[1]);
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, 3 * memRefSize, memRefSize, cg),
			        fpRegs[0]);
		} else {
			generateMemSrc1Instruction(cg, store, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, 0, memRefSize, cg), regs[3]);
			generateMemSrc1Instruction(cg, store, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, memRefSize, memRefSize, cg), regs[2]);
			generateMemSrc1Instruction(cg, store, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, 2 * memRefSize, memRefSize, cg),
			        regs[1]);
			generateMemSrc1Instruction(cg, store, node,
	...
	for (; residual >= memRefSize; residual -= memRefSize, ix += memRefSize) {
		if (supportsLEArrayCopyInline) {
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, memRefSize, cg), oneReg);
		} else {
	...
	if (residual != 0) {
		if (residual & 4) {
			if (supportsLEArrayCopyInline) {
				...
				generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, oneReg,
				        new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 4, cg));
				generateMemSrc1Instruction(cg, TR::InstOpCode::stfs, node,
				        new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 4, cg), oneReg);
			} else {
				TR::Register *oneReg = regs[regIx++];
				if (regIx > 3 || regs[regIx] == NULL)
					regIx = 0;
		...
		if (residual & 2) {
			...
			generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, oneReg,
			        new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 2, cg));
		...
		if (residual & 1) {
			generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, regs[regIx],
			        new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 1, cg));
			generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 1, cg), regs[regIx]);
		}
	}
...
static void
inlineVSXArrayCopy(TR::Node *node, TR::Register *srcAddrReg, TR::Register *dstAddrReg, TR::Register *lengthReg,
        TR::Register *cndRegister, TR::Register *tmp1Reg, TR::Register *tmp2Reg, TR::Register *tmp3Reg,
        TR::Register *tmp4Reg, TR::Register *fp1Reg, TR::Register *fp2Reg, TR::Register *vec0Reg, TR::Register *vec1Reg,
        bool supportsLEArrayCopy, TR::RegisterDependencyConditions *conditions, TR::CodeGenerator *cg)
{
	...
	if (!node->isForwardArrayCopy()) {
		...
		generateTrg1Src2Instruction(cg,
		        TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpl8 : TR::InstOpCode::cmpl4, node,
		        cndRegister, tmp4Reg, lengthReg);
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tmp2Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 1, cg));
	generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 1, cg), tmp2Reg);
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, tmp2Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 2, cg));
	generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 2, cg), tmp2Reg);
	...
	if (supportsLEArrayCopy) {
		generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, fp1Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 4, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::stfs, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), fp1Reg);
	} else {
		generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp2Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 4, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), tmp2Reg);
	}
	...
	if (supportsLEArrayCopy) {
		generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, fp2Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 8, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), fp2Reg);
	} else if (TR::Compiler->target.is64Bit()) {
		generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tmp2Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 8, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), tmp2Reg);
	} else {
		...
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), tmp1Reg);
		generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 4, 4, cg), tmp2Reg);
	}
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec0Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcAddrReg, 16, cg));
	...
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, dstAddrReg, 16, cg), vec0Reg);
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec1Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcAddrReg, 16, cg));
	generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(NULL, dstAddrReg, 16, cg), vec1Reg);
	...
	if (!node->isForwardArrayCopy()) {
		...
		generateTrg1MemInstruction(cg, TR::InstOpCode::lbz, node, tmp2Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -1, 1, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -1, 1, cg), tmp2Reg);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -1);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -1);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -1);
		...
		generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, tmp2Reg,
		        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -2, 2, cg));
		generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -2, 2, cg), tmp2Reg);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -2);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -2);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -2);
		...
		if (supportsLEArrayCopy) {
			generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, fp1Reg,
			        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfs, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -4, 4, cg), fp1Reg);
		} else {
			generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp2Reg,
			        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
			generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
		...
		if (supportsLEArrayCopy) {
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 8, cg), fp2Reg);
		} else if (TR::Compiler->target.is64Bit()) {
			generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tmp2Reg,
			        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -8, 8, cg));
			generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 8, cg), tmp2Reg);
		} else {
			generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp1Reg,
			        new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
			generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp2Reg,
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -4, 4, cg), tmp1Reg);
			generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 4, cg), tmp2Reg);
		}
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -8);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -8);
		generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -8);
...
TR::Register *
OMR::Power::TreeEvaluator::reverseStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::Register *
OMR::Power::TreeEvaluator::reverseLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::Register *
OMR::Power::TreeEvaluator::arraytranslateEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (verbosePPCTRTOOT)
		fprintf(stderr, "%s @ %s [isSourceByte: %d] [isOT: %d] [%d && %d && %d]: arraytranslate\n",
		        comp->signature(), comp->getHotnessName(comp->getMethodHotness()), sourceByte, arraytranslateOT,
		        sourceByte, (node->getChild(3)->getOpCodeValue() == TR::iconst),
...
static bool
loadValue2(TR::Node *node, int valueSize, int length, bool constFill, TR::Register *&tmp1Reg, TR::CodeGenerator *cg)
{
	bool retVal;
...
static bool
loadValue4(TR::Node *node, int valueSize, int length, bool constFill, TR::Register *&tmp1Reg, TR::CodeGenerator *cg)
{
	bool retVal;
...
static bool
loadValue8(TR::Node *node, int valueSize, int length, bool constFill, TR::Register *&tmp1Reg, TR::CodeGenerator *cg)
{
	bool retVal;
	...
	if (valueSize == 1) {
		...
		if (constFill) {
			...
			generateTrg1Src1Imm2Instruction(
			        cg, TR::InstOpCode::rlwimi, node, tmp1Reg, tmp1Reg, 16, 0xffff0000);
			generateTrg1Src1Imm2Instruction(
			        cg, TR::InstOpCode::rldimi, node, tmp1Reg, tmp1Reg, 32, 0xffffffff00000000);
		}
	} else if (valueSize == 2) {
		tmp1Reg = cg->allocateRegister();
		retVal = true;
...
static inline void
loadArrayCmpSources(TR::Node *node, TR::InstOpCode::Mnemonic Op_load, uint32_t loadSize, uint32_t byteLen,
        TR::Register *src1, TR::Register *src2, TR::Register *src1Addr, TR::Register *src2Addr, TR::CodeGenerator *cg)
{
...
inlineArrayCmp(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateTrg1Src1ImmInstruction(cg, (byteLen == 8) ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node,
	...
	generateTrg1MemInstruction(cg, TR::InstOpCode::lbzu, node, src1Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 1, 1, cg));
	generateTrg1MemInstruction(cg, TR::InstOpCode::lbzu, node, src2Reg,
	        new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 1, 1, cg));
...
TR::Register *
OMR::Power::TreeEvaluator::arraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
...
bool
OMR::Power::TreeEvaluator::stopUsingCopyReg(TR::Node *node, TR::Register *&reg, TR::CodeGenerator *cg)
{
	if (node != NULL) {
...
TR::Instruction *
OMR::Power::TreeEvaluator::generateHelperBranchAndLinkInstruction(TR_RuntimeHelper helperIndex, TR::Node *node,
...
TR::Register *
OMR::Power::TreeEvaluator::setmemoryEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Compilation *comp = cg->comp();
	...
	generateLabelInstruction(cg, TR::InstOpCode::label, node, loopStartLabel);
	generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), valueReg);
	generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 8, 8, cg), valueReg);
	generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 16, 8, cg), valueReg);
	generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 24, 8, cg), valueReg);
	generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
	...
	generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), valueReg);
	generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 4);
	...
	generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 2, cg), valueReg);
	generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 2);
	...
	generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 1, cg), valueReg);
...
TR::Register *
OMR::Power::TreeEvaluator::arraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	static bool disableVSXArrayCopyInlining = (feGetEnv("TR_enableVSXArrayCopyInlining") == NULL); // Disabling due
	                                                                                               // to a
	                                                                                               // performance
	                                                                                               // regression

	bool supportsLEArrayCopy = !disableLEArrayCopyHelper && TR::Compiler->target.cpu.isLittleEndian()
	...
	if (!disableVSXArrayCopyInlining && supportsVSX) {
		inlineVSXArrayCopy(node, srcAddrReg, dstAddrReg, lengthReg, cndRegister, tmp1Reg, tmp2Reg, tmp3Reg,
		        tmp4Reg, fp1Reg, fp2Reg, vec0Reg, vec1Reg, supportsLEArrayCopy, conditions, cg);
	} else {
		TR_RuntimeHelper helper;
...
static TR::Register *
inlineFixedTrg1Src1(TR::Node *node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator *cg)
{
...
static TR::Register *
inlineNumberOfTrailingZeros(TR::Node *node, TR::InstOpCode::Mnemonic op, int32_t subfconst, TR::CodeGenerator *cg)
{
...
inlineLongNumberOfTrailingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	generateTrg1Src2Instruction(
	        cg, TR::InstOpCode::andc, node, tempRegister, tempRegister, srcRegister->getLowOrder());
	generateTrg1Src2Instruction(
...
inlineLongHighestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::srd, node, targetRegister, targetRegister, tempRegister);
		...
		generateTrg1Src1Instruction(
		        cg, TR::InstOpCode::cntlzw, node, tempRegister, srcRegister->getHighOrder());
...
TR::Register *
OMR::Power::TreeEvaluator::PrefetchEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
static TR::Register *
inlineSimpleAtomicUpdate(TR::Node *node, bool isAddOp, bool isLong, bool isGetThenUpdate, TR::CodeGenerator *cg)
{
	...
	if (deltaChild->getOpCode().isLoadConst() && !deltaChild->getRegister()
		...
		const int64_t deltaLong = (deltaChild->getDataType() == TR::Int64) ? deltaChild->getLongInt()
	...
	if (isAddOp) {
		...
		if (isDeltaImmediateShifted)
			generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, newValueReg, oldValueReg,
			        ((delta & 0xFFFF0000) >> 16));
	...
	if (TR::Compiler->target.cpu.id() >= TR_PPCgp) {
		generateConditionalBranchInstruction(
		        cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel, cndReg);
	} else {
	...
	conditions = new (cg->trHeapMemory())
	        TR::RegisterDependencyConditions((uint16_t)numDeps, (uint16_t)numDeps, cg->trMemory());
...
bool
OMR::Power::CodeGenerator::inlineDirectCall(TR::Node *node, TR::Register *&resultReg)
{
	...
	if (symRef && symRef->getSymbol()->castToMethodSymbol()->isInlinedByCG()) {
		...
		if (comp->getSymRefTab()->isNonHelper(symRef, TR::SymbolReferenceTable::atomicAddSymbol)) {
			...
			doInline = true;
		                   ...
		                   symRef, TR::SymbolReferenceTable::atomicFetchAndAdd32BitSymbol)) {
			isAddOp = true;
			isLong = false;
			isGetThenUpdate = true;
			doInline = true;
		} else if (comp->getSymRefTab()->isNonHelper(
		                   symRef, TR::SymbolReferenceTable::atomicFetchAndAdd64BitSymbol)) {
			isAddOp = true;
			isLong = true;
			isGetThenUpdate = true;
			doInline = true;
...
TR::Register *
OMR::Power::TreeEvaluator::performCall(TR::Node *node, bool isIndirect, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::indirectCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::SymbolReference *symRef = node->getSymbolReference();
	TR::MethodSymbol *callee = symRef->getSymbol()->castToMethodSymbol();
	TR::Linkage *linkage = cg->getLinkage(callee->getLinkageConvention());
...
TR::Register *
OMR::Power::TreeEvaluator::treetopEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::exceptionRangeFenceEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::loadaddrEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Register *resultReg;
			...
			if (mref->hasDelayedOffset() || offset != 0 || comp->getOption(TR_AOT)) {
				...
				if (mref->hasDelayedOffset()) {
					...
					if (offset >= LOWER_IMMED && offset <= UPPER_IMMED) {
						...
						src2ForStatic = generateTrg1Src1ImmInstruction(cg,
						        TR::InstOpCode::addi2, node, resultReg, mref->getBaseRegister(),
						        offset);
						if (mref->getStaticRelocation() != NULL)
							mref->getStaticRelocation()->setSource2Instruction(
							        src2ForStatic);
					} else {
						...
						generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node,
						        resultReg, resultReg, offset & 0x0000ffff);
					}
				}
			} else {
				resultReg = mref->getBaseRegister();
...
TR::Register *
OMR::Power::TreeEvaluator::gprRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::gprRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	TR::Register *globalReg = cg->evaluate(child);

	if (node->getOpCodeValue() != TR::lRegLoad && node->getOpCodeValue() != TR::luRegLoad
	        && node->needsSignExtension())
...
TR::Register *
OMR::Power::TreeEvaluator::GlRegDepsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	int i;
...
OMR::Power::TreeEvaluator::passThroughEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if ((child->getReferenceCount() > 1 && node->getOpCodeValue() != TR::PassThrough && !skipCopy
		...
		TR::InstOpCode::Mnemonic move_opcode = (srcReg->getKind() == TR_GPR) ? TR::InstOpCode::mr
		                                                                     : TR::InstOpCode::fmr;
...
TR::Register *
OMR::Power::TreeEvaluator::BBStartEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (node->getLabel() != NULL) {
		if (deps == NULL) {
			node->getLabel()->setInstruction(labelInst = generateLabelInstruction(
			                                         cg, TR::InstOpCode::label, node, node->getLabel()));
		} else {
			node->getLabel()->setInstruction(labelInst = generateDepLabelInstruction(cg,
			                                         TR::InstOpCode::label, node, node->getLabel(), deps));
		}
	} else {
		...
		if (srcReg && offset) {
			...
			if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
				TR::MemoryReference *tempMR = new (cg->trHeapMemory())
				        TR::MemoryReference(srcReg, offset, 8, cg);
				generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tempReg, tempMR);
			} else {
				TR::MemoryReference *tempMR = new (cg->trHeapMemory())
				        TR::MemoryReference(srcReg, offset, 4, cg);
				generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tempReg, tempMR);
			}
...
TR::Register *
OMR::Power::TreeEvaluator::unImpOpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
#if defined(DEBUG)
...
void
OMR::Power::TreeEvaluator::postSyncConditions(TR::Node *node, TR::CodeGenerator *cg, TR::Register *dataReg,
...
TR::Register *
OMR::Power::TreeEvaluator::cmpsetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
		generateConditionalBranchInstruction(
		        cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, endLabel, condReg);
	else
...
bool
TR_PPCComputeCC::setCarryBorrow(TR::Node *flagNode, bool invertValue, TR::Register **flagReg, TR::CodeGenerator *cg)
{
	*flagReg = NULL;
...
TR::Register *
OMR::Power::TreeEvaluator::integerHighestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	return inlineIntegerHighestOneBit(node, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::integerLowestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::Power::TreeEvaluator::integerNumberOfLeadingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	return inlineFixedTrg1Src1(node, TR::InstOpCode::cntlzw, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::integerNumberOfTrailingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	return inlineNumberOfTrailingZeros(node, TR::InstOpCode::cntlzw, 32, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::integerBitCount(TR::Node *node, TR::CodeGenerator *cg)
{
	return inlineFixedTrg1Src1(node, TR::InstOpCode::popcntw, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::longHighestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	return inlineLongHighestOneBit(node, cg);
}

TR::Register *
OMR::Power::TreeEvaluator::longLowestOneBit(TR::Node *node, TR::CodeGenerator *cg)
{
	TR_UNIMPLEMENTED();
	return NULL;
}
...
TR::Register *
OMR::Power::TreeEvaluator::longNumberOfLeadingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	if (TR::Compiler->target.is64Bit())
...
TR::Register *
OMR::Power::TreeEvaluator::longNumberOfTrailingZeros(TR::Node *node, TR::CodeGenerator *cg)
{
	if (TR::Compiler->target.is64Bit())
        ...
        TR::Node *node, TR::CodeGenerator *cg, TR::RegisterDependencyConditions *dependencies)
{
	...
	// We need to preserve the JIT TOC whenever we call out. We're saving this on the caller TOC slot as defined by
	// the ABI.
	...
	int32_t callerSaveTOCOffset = (TR::Compiler->target.cpu.isBigEndian() ? 5 : 3)
	        * TR::Compiler->om.sizeofReferenceAddress();

	cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
	        new (cg->trHeapMemory()) TR::MemoryReference(
	                grSysStackReg, callerSaveTOCOffset, TR::Compiler->om.sizeofReferenceAddress(), cg),
	        grTOCReg, cursor);
...
OMR::Power::TreeEvaluator::restoreTOCRegister(
        TR::Node *node, TR::CodeGenerator *cg, TR::RegisterDependencyConditions *dependencies)
{
	// Here we restore the JIT TOC after returning from a call out. We're restoring from the caller TOC slot as
	// defined by the ABI.
	TR::Register *grTOCReg = cg->machine()->getRealRegister(TR::RealRegister::gr2);
	TR::Register *grSysStackReg = cg->machine()->getRealRegister(TR::RealRegister::gr1);
	TR::Compilation *comp = cg->comp();
...
TR::Register *
OMR::Power::TreeEvaluator::ibyteswapEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!firstChild->getRegister() && firstChild->getOpCode().isMemoryReference()
		...
		generateTrg1Src1Imm2Instruction(
		        cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcRegister, 8, 0x00000000ff);
		generateTrg1Src1Imm2Instruction(
		        cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 8, 0x0000ff0000);
		generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
		generateTrg1Src1Imm2Instruction(
		        cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 24, 0x000000ff00);
		generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
		generateTrg1Src1Imm2Instruction(
		        cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 24, 0x00ff000000);
		generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
