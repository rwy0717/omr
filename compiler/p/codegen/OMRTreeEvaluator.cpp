...
extern TR::Register* computeCC_compareUnsigned(TR::Node* node, TR::Register* trgReg, TR::Register* src1Reg,
    TR::Register* src2Reg, bool is64BitCompare, bool needsZeroExtension, TR::CodeGenerator* cg);
extern void generateZeroExtendInstruction(
...
TR::Instruction* loadActualConstant(TR::CodeGenerator* cg, TR::Node* node, intptrj_t value, TR::Register* trgReg,
    ...
    TR::Instruction* cursor, TR::Register* tempReg)
{
    ...
    if (TR::Compiler->target.is32Bit()) {
        ...
        if (tempReg == NULL) {
            nibbles[0] = cursor
                = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, baseReg, hiAddr >> 32, cursor);
            ...
            nibbles[0] = cursor
                = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, tempReg, hiAddr >> 32, cursor);
            nibbles[2] = cursor
        ...
        if (reverseLoad) // 32-bit reverse load. Handles sync
        {
            ...
            if (needSync) {
                ...
                generateInstruction(cg,
                    TR::Compiler->target.cpu.id() >= TR_PPCp7 ? TR::InstOpCode::lwsync : TR::InstOpCode::isync, node);
    ...
    TR::Node* node, int64_t byteLen, TR::Register* src, TR::Register* dst, TR::CodeGenerator* cg)
{
    ...
    if (groups != 0) {
        ...
        if (supportsLEArrayCopyInline) {
            ...
            generateMemSrc1Instruction(cg, store, node,
                new (cg->trHeapMemory()) TR::MemoryReference(dst, 2 * memRefSize, memRefSize, cg), regs[1]);
            generateMemSrc1Instruction(cg, store, node,
                new (cg->trHeapMemory()) TR::MemoryReference(dst, 3 * memRefSize, memRefSize, cg), regs[0]);
        }
        if (groups != 1) {
...
static void inlineVSXArrayCopy(TR::Node* node, TR::Register* srcAddrReg, TR::Register* dstAddrReg,
    ...
    TR::Register* vec1Reg, bool supportsLEArrayCopy, TR::RegisterDependencyConditions* conditions,
    TR::CodeGenerator* cg)
{
...
static TR::Register* inlineArrayCmp(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!byteLenRemainingRegister)
        ccReg = computeCC_compareUnsigned(
            node, ccReg, src1Reg, src2Reg, (byteLen > 4) ? true : false, false /* needsZeroExtenstion */, cg);
    else {
        generateTrg1Src2Instruction(
        ...
        generateTrg1Src1ImmInstruction(cg, (byteLen == 8) ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node,
            condRegToBeUsed, byteLenRemainingRegister, 0);
        generateTrg1Src2Instruction(
        ...
        generateTrg1Src2Instruction(cg, (byteLen == 8) ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node,
...
TR::Register* OMR::Power::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!disableVSXArrayCopyInlining && supportsVSX) {
        inlineVSXArrayCopy(node, srcAddrReg, dstAddrReg, lengthReg, cndRegister, tmp1Reg, tmp2Reg, tmp3Reg, tmp4Reg,
            fp1Reg, fp2Reg, vec0Reg, vec1Reg, supportsLEArrayCopy, conditions, cg);
    } else {
        TR_RuntimeHelper helper;
...
TR::Register* inlineIntegerRotateLeft(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, targetRegister, srcRegister, value, 0xffffffff);
    } else {
...
TR::Register* inlineLongRotateLeft(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicl, node, targetRegister, srcRegister, value, CONSTANT64(0xffffffffffffffff));
    } else {
        TR::Register* shiftAmountReg = cg->evaluate(secondChild);
            ...
            if (mref->hasDelayedOffset() || offset != 0 || comp->getOption(TR_AOT)) {
                ...
                if (mref->hasDelayedOffset()) {
                    ...
                    if (offset >= LOWER_IMMED && offset <= UPPER_IMMED) {
                        ...
                        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, resultReg,
                            mref->getBaseRegister(), ((offset >> 16) + ((offset & (1 << 15)) ? 1 : 0)) & 0x0000ffff);
                        generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi2, node, resultReg, resultReg, offset & 0x0000ffff);
                    }
                }
            } else {
