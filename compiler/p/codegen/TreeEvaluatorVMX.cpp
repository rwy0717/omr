...
TR::Register *
OMR::Power::TreeEvaluator::arraysetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	const uint32_t fillNodeSize = fillNode->getOpCode().isRef() ? TR::Compiler->om.sizeofReferenceField()
	                                                            : fillNode->getSize();
	...
	if (constFill) {
		...
		default: TR_ASSERT(false, "unrecognised address precision\n");
		}
		...
		if (fillNode->getDataType() != TR::Double && dofastPath) {
			...
			if (TR::Compiler->target.is32Bit()) {
				fixedSeqMemAccess(
				        cg, node, 0, q, fp1Reg, tempReg, TR::InstOpCode::lfd, 8, NULL, tempReg);
	...
	if (dofastPath) {
		...
		generateTrg1Src1ImmInstruction(cg,
		        lenNode->getOpCode().isInt() ? TR::InstOpCode::cmpli4 : TR::InstOpCode::cmpli8, node, condReg,
		        lenReg, 0);
		...
		if (1 == fillNodeSize) {
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 0, 1, cg), fillReg);
		...
		if (1 == fillNodeSize || 2 == fillNodeSize) {
			generateTrg1Src1ImmInstruction(cg,
			        lenNode->getOpCode().isInt() ? TR::InstOpCode::cmpli4 : TR::InstOpCode::cmpli8, node,
			        condReg, lenReg, 2);
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::sth, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 0, 2, cg), fillReg);
		...
		if (1 == fillNodeSize || 2 == fillNodeSize || 4 == fillNodeSize) {
			...
			generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
			        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, cg), fillReg);
		...
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 0, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 8, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 16, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 24, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 32, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 40, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 48, 8, cg), dblFillReg);
		generateMemSrc1Instruction(cg, dblStoreOp, node,
		        new (cg->trHeapMemory()) TR::MemoryReference(addrReg, 56, 8, cg), dblFillReg);
...
TR::Register *
OMR::Power::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	return NULL;
}
