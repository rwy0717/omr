...
TR::PPCAlignmentNopInstruction::generateBinaryEncoding()
{
	...
	if (currentMisalign) {
		...
		if (trace)
			traceMsg(cg()->comp(), "Expanding alignment nop %p into %u instructions: [ %p ", self(),
			        nopsToAdd, lastNop);
		...
		if (trace)
			traceMsg(cg()->comp(),
			        "Eliminating alignment nop %p, since the next instruction is already aligned\n",
			        self());
	}
...
static bool
reversedConditionalBranchOpCode(TR::InstOpCode::Mnemonic op, TR::InstOpCode::Mnemonic *rop)
{
	switch (op) {
	case TR::InstOpCode::bdnz: *rop = TR::InstOpCode::bdz; return (false);
	case TR::InstOpCode::bdz: *rop = TR::InstOpCode::bdnz; return (false);
	case TR::InstOpCode::beq: *rop = TR::InstOpCode::bne; return (false);
	case TR::InstOpCode::beql: *rop = TR::InstOpCode::bne; return (true);
	case TR::InstOpCode::bge: *rop = TR::InstOpCode::blt; return (false);
	case TR::InstOpCode::bgel: *rop = TR::InstOpCode::blt; return (true);
	case TR::InstOpCode::bgt: *rop = TR::InstOpCode::ble; return (false);
	case TR::InstOpCode::bgtl: *rop = TR::InstOpCode::ble; return (true);
	case TR::InstOpCode::ble: *rop = TR::InstOpCode::bgt; return (false);
	case TR::InstOpCode::blel: *rop = TR::InstOpCode::bgt; return (true);
	case TR::InstOpCode::blt: *rop = TR::InstOpCode::bge; return (false);
	case TR::InstOpCode::bltl: *rop = TR::InstOpCode::bge; return (true);
	case TR::InstOpCode::bne: *rop = TR::InstOpCode::beq; return (false);
	case TR::InstOpCode::bnel: *rop = TR::InstOpCode::beq; return (true);
	case TR::InstOpCode::bnun: *rop = TR::InstOpCode::bun; return (false);
	case TR::InstOpCode::bun: *rop = TR::InstOpCode::bnun; return (false);
	default:
...
TR::PPCConditionalBranchInstruction::generateBinaryEncoding()
{
		...
		if (label->getCodeLocation() != NULL) {
			...
			} else // too far - need fix up
			{
				...
				*(int32_t *)cursor |= 0x0008;
				...
				cursor = extraOpCode.copyBinaryToBuffer(cursor);
				cg()->addRelocation(
				        new (cg()->trHeapMemory()) TR::LabelRelative24BitRelocation(cursor, label));
			}
		}
	} else
		insertConditionRegister((uint32_t *)cursor);
	...
	if (haveHint()) {
		...
		if (getOpCode().setsCTR())
			...
			*(int32_t *)instructionStart |=
			        (getLikeliness() ? PPCOpProp_BranchLikelyMask : PPCOpProp_BranchUnlikelyMask);
	}
...
int32_t
TR::PPCConditionalBranchInstruction::estimateBinaryLength(int32_t currentEstimate)
{
	setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH);
	...
	if (needsAOTRelocation()) {
		...
		case TR_BodyInfoAddress:
			cg()->addExternalRelocation(new (cg()->trHeapMemory())
			        ...
			        __FILE__, __LINE__, getNode());
			break;
		default: TR_ASSERT(false, "Unsupported AOT relocation type specified.");
		}
	}
...
loadReturnAddress(TR::Node *node, uintptrj_t value, TR::Register *trgReg, TR::Instruction *cursor)
{
...
TR::PPCTrg1Src1ImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
	...
	if (std::find(comp->getStaticMethodPICSites()->begin(), comp->getStaticMethodPICSites()->end(), this)
		...
		cg()->jitAddPicToPatchOnClassUnload(
		        (void *)(cg()->fe()
		                         ->createResolvedMethod(cg()->trMemory(),
		                                 (TR_OpaqueMethodBlock *)(getSourceImmPtr()), comp->getCurrentMethod())
		                         ->classOfMethod()),
		        (void *)cursor);
	}
}
...
static void
insertMaskField(uint32_t *instruction, TR::InstOpCode::Mnemonic op, int64_t lmask)
{
	int32_t encoding;
...
TR::PPCControlFlowInstruction::estimateBinaryLength(int32_t currentEstimate)
{
	...
	case TR::InstOpCode::iternary:
		...
		break;
	case TR::InstOpCode::setbflt: setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH * 5); break;
	...
	case TR::InstOpCode::d2i: setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH * 6); break;
	case TR::InstOpCode::d2l:
		if (TR::Compiler->target.is64Bit())
			...
			setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH * 8);
		break;
	case TR::InstOpCode::lcmp: setEstimatedBinaryLength(PPC_INSTRUCTION_LENGTH * 11); break;
	default: TR_ASSERT(false, "unknown control flow instruction (estimateBinaryLength)");
	}
	return currentEstimate + getEstimatedBinaryLength();
}
...
TR::PPCVirtualGuardNOPInstruction::generateBinaryEncoding()
{
	...
	if (cg()->sizeOfInstructionToBePatched(this) == 0 ||
	        ...
	        // 1) Those instructions might have AOT data relocations (and therefore will be incorrectly patched
	        // again) 2) We might want to re-enable the code path and unpatch, in which case we would have to know
	        // what the old instruction was
