...
spillSizeForRegister(TR::Register *virtReg)
{
	...
	case TR_GPR: return TR::Compiler->om.sizeofReferenceAddress();
	case TR_FPR: return virtReg->isSinglePrecision() ? 4 : 8;
	case TR_VSX_SCALAR: return 8;
	case TR_CCR: return 4;
	case TR_VSX_VECTOR:
	case TR_VRF: return 16;
	}
...
static bool
boundNext(TR::Instruction *currentInstruction, int32_t realNum, TR::Register *virtReg, bool isOOL)
{
...
TR::RealRegister *
OMR::Power::Machine::findBestFreeRegister(TR::Instruction *currentInstruction, TR_RegisterKinds rk, bool excludeGPR0,
        bool considerUnlatched, TR::Register *virtualReg)
{
	...
	case TR_FPR:
	case TR_VSX_SCALAR:
	case TR_VSX_VECTOR: maskI = TR::RealRegister::FirstFPR; break;
	case TR_CCR:
		maskI = first = TR::RealRegister::FirstCCR;
		last = TR::RealRegister::LastCCR;
		break;
	case TR_VRF: maskI = TR::RealRegister::FirstVRF; break;
	}
	...
	if (rk == TR_FPR || rk == TR_VSX_SCALAR || rk == TR_VSX_VECTOR || rk == TR_VRF) {
		...
		for (int i = first; i <= last; i++) {
			...
			// Inject interference for last four assignments to prevent write-after-write dependancy in same
			// p6 dispatch group.
...
TR::RealRegister *
OMR::Power::Machine::freeBestRegister(
...

{
	...
	case TR_GPR:
		if (false /*!cg()->comp()->getOption(TR_DisableOOL) && (cg()->isOutOfLineColdPath() || cg()->isOutOfLineHotPath()) &&
			...
			if (candidates[0]->getBackingStorage()) {
				...
				// OOL region while the free spill list was locked and we didn't clean this up after
				// unlocking the list (see TODO in PPCInstruction.cpp). Therefore we need to set the
	...
	if (rk == TR_CCR) {
		...
		TR::Instruction *ccrInstr = generateTrg1ImmInstruction(self()->cg(), TR::InstOpCode::mtcrf, currentNode,
		        crtemp, 1 << (7 - toPPCCRBackingStore(location)->getCcrFieldIndex()), currentInstruction);
...
TR::Register *
OMR::Power::Machine::setVirtualAssociatedWithReal(TR::RealRegister::RegNum regNum, TR::Register *virtReg)
{
	if (virtReg) {
...
TR::Register *
OMR::Power::Machine::getVirtualAssociatedWithReal(TR::RealRegister::RegNum regNum)
{
	return (_registerAssociations[regNum]);
}

TR::RealRegister *
OMR::Power::Machine::reverseSpillState(TR::Instruction *currentInstruction, TR::Register *spilledRegister,
        TR::RealRegister *targetRegister, bool excludeGPR0)
{
	...
	if (targetRegister == NULL) {
		...
		if (targetRegister == NULL) {
			targetRegister =
			        self()->freeBestRegister(currentInstruction, spilledRegister, NULL, excludeGPR0);
		}
	...
	if (comp->getOption(TR_DisableOOL)) {
		...
		if (self()->cg()->isOutOfLineColdPath()) {
			...
			// Also free the spill if maxSpillDepth==0, which will be the case if the reverse spill also
			// occured on the hot path. If the reverse spill occured on both paths then this is the last
			// chance we have to free the spill slot.
			if (location->getMaxSpillDepth() == 3 || location->getMaxSpillDepth() == 0
			        || isOOLentryReverseSpill) {
				if (location->getMaxSpillDepth() != 0)
					location->setMaxSpillDepth(0);
				else if (debugObj)
					self()->cg()->traceRegisterAssignment("\nOOL: reverse spill %s in less "
					                                      "dominant path (%d / 3), reverse spill "
					                                      "on both paths indicated, free spill "
					                                      "slot (%p)\n",
				...
				if (debugObj)
					self()->cg()->traceRegisterAssignment("\nOOL: reverse spill %s in less "
					                                      "dominant path (%d / 3), protect spill "
					                                      "slot (%p)\n",
					        debugObj->getName(spilledRegister), location->getMaxSpillDepth(),
					        location);
			}
			...
			if (location->getMaxSpillDepth() == 2) {
				...
				if (debugObj)
					self()->cg()->traceRegisterAssignment("\nOOL: reverse spilling %s in less "
					                                      "dominant path (%d / 2), protect spill "
					                                      "slot (%p)\n",
					        debugObj->getName(spilledRegister), location->getMaxSpillDepth(),
					        location);
				// Reset maxSpillDepth here so that in the cold path we know to free the spill
				// and so that the spill is not included in future GC points in the hot path while it is
				// protected
				location->setMaxSpillDepth(0);
			}
		} else // main line
	...
	if (rk == TR_CCR) {
		...
		if (sindex != tindex) {
			...
			self()->cg()->traceRAInstruction(
			        generateTrg1Src1Imm2Instruction(self()->cg(), TR::InstOpCode::rlwinm, currentNode,
			                crtemp, crtemp, sindex, 0xFFFFFFFF, currentInstruction));
		}
		self()->cg()->traceRAInstruction(generateTrg1ImmInstruction(
		        self()->cg(), TR::InstOpCode::mfcr, currentNode, crtemp, 0xFF, currentInstruction));
	}
...
TR::RealRegister *
OMR::Power::Machine::assignOneRegister(TR::Instruction *currentInstruction, TR::Register *virtReg, bool excludeGPR0)
{
	...
	if (targetRegister->getState() == TR::RealRegister::Free
		...
		if (targetRegister->getState() == TR::RealRegister::Blocked) {
			...
			if (currentAssignedRegister != NULL && currentAssignedUseful) {
				...
				registerExchange(currentInstruction, ctv_rk, targetRegister, currentAssignedRegister,
				        spareReg, self()->cg());
				...
				if (currentAssignedRegister == NULL) {
					...
					registerCopy(currentInstruction, vr_rk, currentAssignedRegister, targetRegister,
					        self()->cg());
					currentAssignedRegister->setState(TR::RealRegister::Free);
					currentAssignedRegister->setAssignedRegister(NULL);
				}
...
TR::RealRegister **
OMR::Power::Machine::cloneRegisterFileByType(TR::RealRegister **registerFileClone, TR::RealRegister **registerFile,
...
OMR::Power::Machine::cloneRegisterFile(TR::RealRegister **registerFile, TR_AllocationKind allocKind)
{
	...
	registerFileClone = self()->cloneRegisterFileByType(registerFileClone, registerFile, TR::RealRegister::FirstGPR,
	        TR::RealRegister::LastGPR, TR_GPR, allocKind);
	registerFileClone = self()->cloneRegisterFileByType(registerFileClone, registerFile, TR::RealRegister::FirstFPR,
	        TR::RealRegister::LastFPR, TR_FPR, allocKind);
	registerFileClone = self()->cloneRegisterFileByType(registerFileClone, registerFile, TR::RealRegister::FirstCCR,
	        TR::RealRegister::LastCCR, TR_CCR, allocKind);
	registerFileClone = self()->cloneRegisterFileByType(registerFileClone, registerFile, TR::RealRegister::FirstVRF,
...
static void
registerCopy(TR::Instruction *precedingInstruction, TR_RegisterKinds rk, TR::RealRegister *targetReg,
        TR::RealRegister *sourceReg, TR::CodeGenerator *cg)
{
	...
	case TR_FPR:
		if (useVSXLogical)
			instr = generateTrg1Src2Instruction(cg, TR::InstOpCode::xxlor, currentNode, targetReg,
			        sourceReg, sourceReg, precedingInstruction);
		else
			instr = generateTrg1Src1Instruction(
...
static void
registerExchange(TR::Instruction *precedingInstruction, TR_RegisterKinds rk, TR::RealRegister *targetReg,
        TR::RealRegister *sourceReg, TR::RealRegister *middleReg, TR::CodeGenerator *cg)
{
	...
	if (middleReg != NULL) {
		...
		TR::InstOpCode::Mnemonic opCode;
		switch (rk) {
		case TR_GPR: opCode = TR::InstOpCode::XOR; break;
		case TR_FPR: opCode = TR::InstOpCode::xxlxor; break;
		case TR_VSX_SCALAR:
		case TR_VSX_VECTOR: opCode = TR::InstOpCode::xxlxor; break;
		case TR_VRF: opCode = TR::InstOpCode::vxor; break;
		case TR_CCR: TR_ASSERT(0, "Cannot exchange CCR without a third reg");
		}
...
OMR::Power::Machine::createCondForLiveAndSpilledGPRs(bool cleanRegState, TR::list<TR::Register *> *spilledRegisterList)
{
	...
	//
	int32_t c = 0;
	int32_t endReg = TR::RealRegister::LastCCR; // As the real registers order between VR and CR are exchanged,
	                                            // LastCCR will cover all VRFs
...
OMR::Power::Machine::restoreRegisterStateFromSnapShot()
{
	...
	for (i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters - 1; i++) // Skipping SpilledReg
	{
		...
		// Don't restore registers that died after the snapshot was taken since they are guaranteed to not be
	...
	for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters - 1; i++) // Skipping SpilledReg
	{
		...
		if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
			...
			if (location != NULL) {
				...
				if (trace)
					traceMsg(comp,
					        "\nDisassociating backing storage " POINTER_PRINTF_FORMAT " of size %u "
					        "from "
					        "assigned "
					        "virtual "
					        "%s\n",
