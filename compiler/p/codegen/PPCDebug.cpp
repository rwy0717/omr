...
const char* TR_Debug::getOpCodeName(TR::InstOpCode* opCode) { return ppcOpCodeToNameMap[opCode->getOpCodeValue()][1]; }
...
bool TR_Debug::isBranchToTrampoline(TR::SymbolReference* symRef, uint8_t* cursor, int32_t& distance)
{
    ...
    if (distance < BRANCH_BACKWARD_LIMIT || distance > BRANCH_FORWARD_LIMIT) {
        distance
            = _comp->fe()->indexedTrampolineLookup(symRef->getReferenceNumber(), (void*)cursor) - (intptrj_t)cursor;
        return true;
    }
    return false;
}
    ...
    if (cursor) {
        ...
        if (label) {
            ...
            if (refNum < TR_PPCnumRuntimeHelpers) {
                ...
                distance = _comp->fe()->methodTrampolineLookup(_comp, instr->getSymbolReference(), (void*)cursor)
                    - (intptrj_t)cursor;
            }
        }
    }
