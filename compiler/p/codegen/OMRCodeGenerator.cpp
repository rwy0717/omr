...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getOpCodeValue() == TR::table) {
            ...
            if (total <= 9) {
                for (ii = 3; ii < total && (node->getChild(ii)->getCaseConstant() - node->getChild(ii - 1)->getCaseConstant()) <= UPPER_IMMED; ii++)
                    ;
                if (ii == total)
                    return _numGPR - 2;
            }
...
j2Prof_startInterval(int32_t* preambLen, int32_t* ipAssistLen, int32_t* prologueLen, TR::Compilation* comp)
{
    ...
    *ipAssistLen = ((*sizePtr) >> 16) & 0x0000ffff;
    ...
    while (currentIntervalStart != NULL && (currentIntervalStart->getOpCodeValue() != TR::InstOpCode::fence || currentIntervalStart->getNode() == NULL || currentIntervalStart->getNode()->getOpCodeValue() != TR::BBStart))
        currentIntervalStart = currentIntervalStart->getNext();

    nextIntervalInstructionPtr = currentIntervalStart;
    nextIntervalBufferPtr = (currentIntervalStart == NULL) ? cg->getBinaryBufferCursor() : currentIntervalStart->getBinaryEncoding();
...
j2Prof_nextInterval(TR::Compilation* comp)
{
    ...
    while (nextBBend->getNext() != NULL && intervalLen == 0) {
        ...
        while (nextBBend->getNext() != NULL && (nextBBend->getOpCodeValue() != TR::InstOpCode::fence || nextBBend->getNode() == NULL || nextBBend->getNode()->getOpCodeValue() != TR::BBEnd))
            nextBBend = nextBBend->getNext();
...
bool OMR::Power::CodeGenerator::isRotateAndMask(TR::Node* node)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL &&
        ...
        ((secondOp == TR::iconst || secondOp == TR::iuconst) && contiguousBits(secondChild->getInt()) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (((firstChild->getOpCodeValue() == TR::imul || firstChild->getOpCodeValue() == TR::iumul) && (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst || firstChild->getSecondChild()->getOpCodeValue() == TR::iuconst) && firstChild->getSecondChild()->getInt() > 0 && isNonNegativePowerOf2(firstChild->getSecondChild()->getInt())) || ((firstChild->getOpCodeValue() == TR::ishr || firstChild->getOpCodeValue() == TR::iushr) && (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst || firstChild->getSecondChild()->getOpCodeValue() == TR::iuconst) && firstChild->getSecondChild()->getInt() > 0 && (firstChild->getOpCodeValue() == TR::iushr || leadingZeroes(secondChild->getInt()) >= firstChild->getSecondChild()->getInt())))))
        return true;
    else
        return false;
}
