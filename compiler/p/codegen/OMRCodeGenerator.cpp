        ...
        , _outOfLineCodeSectionList(getTypedAllocator<TR_PPCOutOfLineCodeSection *>(self()->comp()->allocator()))
{
	...
	if (TR::Compiler->target.is64Bit()) {
		self()->setTrackStatics(
		        new (self()->trHeapMemory()) TR_Array<TR::SymbolReference *>(self()->trMemory()));
	...
	/*
	 * TODO: TM is currently not compatible with read barriers. If read barriers are required, TM is disabled until
	 * the issue is fixed.
	 */
	...
	if (self()->comp()->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRARegisterStates)) {
		self()->setGPRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(
		        self()->machine(), TR::RealRegister::FirstGPR, TR::RealRegister::LastGPR));
...
TR::Instruction *
OMR::Power::CodeGenerator::generateSwitchToInterpreterPrePrologue(TR::Instruction *cursor, TR::Node *node)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		cursor = generateImmInstruction(
		        self(), TR::InstOpCode::dd, node, (int32_t)ramMethod, TR_RamMethod, cursor);
	}
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
		        TR::Compiler->target.cpu.isBigEndian() ? highBits : lowBits, TR_AbsoluteHelperAddress,
		        helperSymRef, cursor);
		cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
		        TR::Compiler->target.cpu.isBigEndian() ? lowBits : highBits, cursor);
	} else {
		cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node, (int32_t)helperAddr,
...
TR::Instruction *
OMR::Power::CodeGenerator::generateNop(TR::Node *n, TR::Instruction *preced, TR_NOPKind nopKind)
{
	TR::InstOpCode::Mnemonic nop;

	switch (nopKind) {
	case TR_NOPEndGroup: nop = TR::InstOpCode::genop; break;
	case TR_ProbeNOP: nop = TR::InstOpCode::probenop; break;
	default: nop = TR::InstOpCode::nop;
	}
...
TR::Instruction *
OMR::Power::CodeGenerator::generateGroupEndingNop(TR::Node *node, TR::Instruction *preced)
{
...
TR::Instruction *
OMR::Power::CodeGenerator::generateProbeNop(TR::Node *node, TR::Instruction *preced)
{
...
mrPeepholes(TR::CodeGenerator *cg, TR::Instruction *mrInstruction)
{
	...
	while (isWAWOrmrPeepholeCandidateInstr(current) && !isEBBTerminatingBranch(current)
	        && windowSize < maxWindowSize) {
		// Keep track of any GC points we cross in order to fix them if we remove a register move
		// Note: Don't care about GC points in snippets, since we won't remove a reg move if inEBB, i.e. if
		// we've encountered a branch (to a snippet or elsewhere)
		if (current->getGCMap()
		...
		if (memRef != NULL) {
			...
			case OMR::Instruction::IsTrg1Src1Imm2: {
				...
				if (inst->getSource1Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm2 source register %p.\n",
					            current)) {
						inst->setSource1Register(mrSourceReg);
						extendWindow = true;
					} else
			...
			case OMR::Instruction::IsTrg1Src2: {
				...
				if (inst->getSource1Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src2 1st source register %p.\n",
					            current)) {
						inst->setSource1Register(mrSourceReg);
						extendWindow = true;
					} else
				...
				if (inst->getSource2Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src2 2nd source register %p.\n",
					            current)) {
						inst->setSource2Register(mrSourceReg);
						extendWindow = true;
					} else
			...
			case OMR::Instruction::IsTrg1Src3: {
				...
				if (inst->getSource1Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src3 1st source register %p.\n",
					            current)) {
						inst->setSource1Register(mrSourceReg);
						extendWindow = true;
					} else
				...
				if (inst->getSource2Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src3 2nd source register %p.\n",
					            current)) {
						inst->setSource2Register(mrSourceReg);
						extendWindow = true;
					} else
				...
				if (inst->getSource3Register() == mrTargetReg) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Rewrite Trg1Src3 3nd source register %p.\n",
					            current)) {
		...
		if (current->defsRegister(toRealRegister(mrTargetReg))) {
			...
			if (!inEBB && all_mr_source_uses_rewritten
				...
				if (!stackMaps.empty()) {
					for (auto stackMapIter = stackMaps.begin(); stackMapIter != stackMaps.end();
					        ++stackMapIter) {
						if (cg->getDebug())
							if (comp->getOption(TR_TraceCG))
								traceMsg(comp,
								        "Adjusting register map %p; removing %s, "
								        "adding %s due to removal of mr %p\n",
								        *stackMapIter,
	...
	while (current != NULL && !current->isLabel()
		...
		// if target reg is same as source of compare
		{
			...
			if (current->getOpCode().hasRecordForm()) {
				...
				if (TR::Compiler->target.cpu.id() == TR_PPCgp
					...
					if (opCode == TR::InstOpCode::rlwinm) {
						...
						if (inst->getSourceImmediate() == 0
							...
							if (performTransformation(comp,
							            "O^O PPC PEEPHOLE: Change %p to andi_r, remove "
							            "compare immediate %p.\n",
							            current, cmpiInstruction)) {
								generateTrg1Src1ImmInstruction(cg,
								        TR::InstOpCode::andi_r, inst->getNode(),
							...
							if (performTransformation(comp,
							            "O^O PPC PEEPHOLE: Change %p to andis_r, remove "
							            "compare immediate %p.\n",
							            current, cmpiInstruction)) {
								generateTrg1Src1ImmInstruction(cg,
								        TR::InstOpCode::andis_r, inst->getNode(),
								        inst->getPrimaryTargetRegister(),
								        inst->getSourceRegister(0), cmpiTargetReg,
				...
				if (performTransformation(comp,
				            "O^O PPC PEEPHOLE: Change %p to record form, remove compare immediate "
				            "%p.\n",
				            current, cmpiInstruction)) {
...
lhsPeephole(TR::CodeGenerator *cg, TR::Instruction *storeInstruction)
{
	...
	// TODO: Relax the first constraint a bit and handle cases were unrelated instructions appear between the store
	// and the load
	...
	if (srcReg == trgReg) {
		...
		if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
			if (performTransformation(comp,
				...
				generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(),
	...
	if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
		if (performTransformation(comp,
		            ...
		            loadInstruction, storeInstruction)) {
			generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg,
		...
		generateTrg1Src1Instruction(
		        cg, TR::InstOpCode::mr, loadInstruction->getNode(), trgReg, srcReg, storeInstruction);
		loadInstruction->remove();
	}
}

static void
syncPeephole(TR::CodeGenerator *cg, TR::Instruction *instructionCursor, int window)
{
	TR::Compilation *comp = cg->comp();
	...
	while (visited < window) {
		...
		if (removeFirst) {
			...
			if (performTransformation(comp,
				...
				cg->generateNop(
				        instructionCursor->getNext()->getNode(), instructionCursor, TR_NOPStandard);
...
trapPeephole(TR::CodeGenerator *cg, TR::Instruction *instructionCursor)
{
	...
	for (int i = 0; i < WINDOW && next != NULL; i++) {
		...
		if (next->isLoad() && next->getSourceRegister(0) != NULL
			...
			// Page 0 which is readable on AIX is 4096 bytes long, and the largest possible load is 16
			// bytes.
	...
	if (currTargetReg && currTargetReg->getKind() != TR_CCR && isWAWOrmrPeepholeCandidateInstr(instr)
		...
		while (isWAWOrmrPeepholeCandidateInstr(current) && !current->isBranchOp() && !current->isCall()
			...
			if (current->getTargetRegister(0) == currTargetReg) {
				...
				if (isConstData) {
					if (performTransformation(comp,
					            "O^O PPC PEEPHOLE: Remove dead instrcution group from WAW %p -> "
					            "%p.\n",
					            instr, current)) {
...
bool
OMR::Power::CodeGenerator::checkAndFetchRequestor(TR::Instruction *instr, TR::Instruction **q)
{
	if (_constantData) {
...
OMR::Power::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
	...
	switch (lc) {
	case TR_System: linkage = new (self()->trHeapMemory()) TR::PPCSystemLinkage(self()); break;

	default: linkage = new (self()->trHeapMemory()) TR::PPCSystemLinkage(self());
	}
...
void
OMR::Power::CodeGenerator::generateBinaryEncodingPrologue(TR_PPCBinaryEncodingData *data)
{
...
OMR::Power::CodeGenerator::doBinaryEncoding()
{
	...
	if (!self()->comp()->getOption(TR_DisableOOL)) {
		...
		while (oiIterator != self()->getPPCOutOfLineCodeSectionList().end()) {
			...
			bool needsETE =
			        (*oiIterator)->getFirstInstruction()->getNode()->getOpCode().hasSymbolReference()
	...
	if (!self()->canClobberNodesRegister(node)) {
		if (TR::Compiler->target.is32Bit() && node->getType().isInt64()) {
			...
			generateTrg1Src1Instruction(self(),
			        resultReg->getKind() == TR_GPR ? TR::InstOpCode::mr : TR::InstOpCode::fmr, node,
			        targetReg, resultReg);
			return targetReg;
		}
	} else {
	...
	while (cursorInstruction) {
		...
		if (branch != NULL) {
			if (abs(branch->getEstimatedBinaryLocation()
			            - branch->getLabelSymbol()->getEstimatedCodeLocation())
			        > 16384) {
...
int32_t
OMR::Power::CodeGenerator::findOrCreateFloatConstant(
...
int32_t
OMR::Power::CodeGenerator::findOrCreateAddressConstant(void *v, TR::DataType t, TR::Instruction *n0,
...
bool
OMR::Power::CodeGenerator::isSnippetMatched(TR::Snippet *snippet, int32_t snippetKind, TR::SymbolReference *symRef)
{
	...
	case TR::Snippet::IsNonZeroAllocPrefetch: return true;
	default: return false;
	}
}
...
int32_t
OMR::Power::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart)
{
...
callInTree(TR::TreeTop *treeTop)
{
	...
	if (l1OpCode == TR::monent || l1OpCode == TR::monexit || l1OpCode == TR::checkcast || l1OpCode == TR::
	                instanceof || l1OpCode == TR::ArrayStoreCHK || l1OpCode == TR::MethodEnterHook
	                || l1OpCode == TR::MethodExitHook || l1OpCode == TR::New || l1OpCode == TR::newarray
	                || l1OpCode == TR::anewarray || l1OpCode == TR::multianewarray || l1OpCode == TR::MergeNew)
		return (true);
...
OMR::Power::CodeGenerator::pickRegister(TR_RegisterCandidate *regCan, TR::Block **barr, TR_BitVector &availRegs,
        TR_GlobalRegisterNumber &highRegisterNumber, TR_LinkHead<TR_RegisterCandidate> *candidates)
{
	...
	if (gprCandidate) {
		...
		if (!_assignedGlobalRegisters)
			_assignedGlobalRegisters = new (self()->trStackMemory()) TR_BitVector(
			        self()->comp()->getSymRefCount(), self()->trMemory(), stackAlloc, growable);
		...
		while (livec.hasMoreElements()) {
			...
			maxRegisterPressure = self()->estimateRegisterPressure(block, visitCount, maxStaticFrequency,
			        maxFrequency, vmThreadUsed, numAssignedGlobalRegs, _assignedGlobalRegisters,
	...
	if (hasCallInPath) {
		...
		if (!isParm || sym->getParmSymbol()->getLinkageRegisterIndex() < 0) {
			...
			while (i1 <= lastVolIndex
			        && (!availRegs.isSet(i1) || (isAddressType && i1 == _lastVolatileGPR)))
				i1++;
...
bool
OMR::Power::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate *rc, TR::Node *branchNode)
{
...
int32_t
OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node *node)
{
	if (node->getOpCode().isSwitch()) {
		...
		return (TR::Compiler->target.is64Bit() ? _numGPR : _numGPR - 4); // 4 GPRs are needed for long-compare
...
int32_t
OMR::Power::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(TR::Node *node)
{
	return 32; // pickRegister will ensure that the number of free registers isn't exceeded
}

TR_GlobalRegisterNumber
OMR::Power::CodeGenerator::getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex, TR::DataType type)
{
...
void
OMR::Power::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
...
void
OMR::Power::CodeGenerator::apply24BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label)
{
	TR_ASSERT(label->getCodeLocation(), "Attempt to relocate to a NULL label address!");
...
void
OMR::Power::CodeGenerator::apply16BitLoadLabelRelativeRelocation(TR::Instruction *liInstruction,
        TR::LabelSymbol *startLabel, TR::LabelSymbol *endLabel, int32_t deltaToStartLabel)
{
	...
	// This type of relocation is used in some projects so an exception handler can get the return address off of
	// the stack and then use this return address and the offset loaded here to find an exit label in the generated
	// code. If the register deps are not done properly then the register assigner may insert instructions after the
	// call but before the startLabel. If this bad register motion occurs then the loaded offset would be wrong
	// (when used in this way) and if an exception occurs we may end up with a wild branch. This would be a very
	// tough bug to find and fix so it is worth some ugliness in the function interface.
...
{
	// Patch up the mainline code that materializes a base register and does a load with update from this base
	// register This is the instruction sequence generated by fixedSeqMemAccess hiAddr = HI_VALUE(addrValue)
	...
	TR_ASSERT((lastPPCInstruction->getOpCodeValue() == TR::InstOpCode::ldu
	                ...
	                && lastPPCInstruction->getPrev()->getPrev()->getPrev()->getPrev()->getOpCodeValue()
	                        == TR::InstOpCode::lis,
...
getAssociationIndex(TR::Register *reg, TR::RealRegister::RegNum realNum)
{
	...
	switch (reg->getKind()) {
	case TR_GPR: associationIndex = realNum - TR::RealRegister::NoReg; break;
	case TR_FPR: associationIndex = realNum - TR::RealRegister::LastGPR; break;
	case TR_CCR:
		associationIndex = realNum - TR::RealRegister::LastVSR;
		maxIndex = 8;
		break;
	default: TR_ASSERT(0, "Unrecognized register kind.\n");
	}
...
void
OMR::Power::CodeGenerator::setRealRegisterAssociation(TR::Register *reg, TR::RealRegister::RegNum realNum)
{
...
void
OMR::Power::CodeGenerator::addRealRegisterInterference(TR::Register *reg, TR::RealRegister::RegNum realNum)
{
	if (!reg->isLive() || realNum == TR::RealRegister::NoReg)
		return;
	TR::RealRegister *realReg = self()->machine()->getRealRegister(realNum);
...
OMR::Power::CodeGenerator::findOutLinedInstructionsFromLabel(TR::LabelSymbol *label)
{
	auto oiIterator = self()->getPPCOutOfLineCodeSectionList().begin();
	while (oiIterator != self()->getPPCOutOfLineCodeSectionList().end()) {
...
TR::Snippet *
OMR::Power::CodeGenerator::findSnippetInstructionsFromLabel(TR::LabelSymbol *label)
{
...
TR::Instruction *
OMR::Power::CodeGenerator::generateDebugCounterBump(TR::Instruction *cursor, TR::DebugCounterBase *counter,
        TR::Register *deltaReg, TR::RegisterDependencyConditions *cond)
{
	...
	cursor = generateTrg1Src2Instruction(
	        self(), TR::InstOpCode::add, node, counterReg, counterReg, deltaReg, cursor);
	cursor = generateMemSrc1Instruction(self(), TR::InstOpCode::stw, node,
	        new (self()->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, self()), counterReg, cursor);
	if (cond) {
		uint32_t preCondCursor = cond->getAddCursorForPre();
...
OMR::Power::CodeGenerator::canEmitDataForExternallyRelocatableInstructions()
{
	...
	// these data fields). Hence, for the relocation to be applied correctly, we must ensure that the data fields
	// inside the instruction initially are zero.
	return !self()->comp()->compileRelocatableCode();
}

bool
OMR::Power::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i, TR::DataType dt)
{
...
bool
OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode, TR::DataType dt)
{
        ...
        TR::Node *node, TR::Instruction *firstInstruction, TR::Register *tempReg, int16_t typeAddress, intptrj_t value)
{
	...
	case TR_DebugCounter: {
		...
		if (counter == NULL)
			comp->failCompilation<TR::CompilationException>("Could not generate relocation for debug "
			                                                "counter in "
...
TR::Instruction *
OMR::Power::CodeGenerator::loadAddressConstantFixed(TR::Node *node, intptrj_t value, TR::Register *trgReg,
        TR::Instruction *cursor, TR::Register *tempReg, int16_t typeAddress, bool doAOTRelocation)
{
	TR::Compilation *comp = self()->comp();
...
void
OMR::Power::CodeGenerator::addMetaDataForLoadIntConstantFixed(TR::Node *node, TR::Instruction *firstInstruction,
        ...
        TR::Node *node, int32_t value, TR::Register *trgReg, TR::Instruction *cursor, int16_t typeAddress)
{
	...
	cursor = secondInstruction = generateTrg1Src1ImmInstruction(
	        self(), TR::InstOpCode::ori, node, trgReg, trgReg, value & 0x0000ffff, cursor);
        ...
        TR::Node *node, TR::LabelSymbol *label, TR::Instruction *firstInstruction, TR::Instruction *secondInstruction)
{
	...
	self()->getAheadOfTimeCompile()->getRelocationList().push_front(
	        new (self()->trHeapMemory()) TR::PPCPairedRelocation(firstInstruction, secondInstruction,
	                (uint8_t *)recordInfo, TR_AbsoluteMethodAddressOrderedPair, node));
...
TR::Instruction *
OMR::Power::CodeGenerator::fixedLoadLabelAddressIntoReg(TR::Node *node, TR::Register *trgReg, TR::LabelSymbol *label,
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		if (offset != PTOC_FULL_INDEX) {
			...
			if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
				...
				generateTrg1MemInstruction(self(), TR::InstOpCode::Op_load, node, trgReg,
				        new (self()->trHeapMemory())
				                TR::MemoryReference(self()->getTOCBaseRegister(), offset, 8, self()));
			}
		} else {
			TR::Instruction *q[4];
...
{
	/*
	 * If the target is 64 bit, the CPU must also be 64 bit (even if we don't specifically know which Power CPU it
	 * is) so this can just return true. If the target is not 64 bit, we need to check the CPU properties to try and
...
void
mulConstant(TR::Node *node, TR::Register *trgReg, TR::Register *sourceReg, int32_t value, TR::CodeGenerator *cg)
{
	if (value == 0) {
...
// Multiply a register by a constant
void
mulConstant(TR::Node *node, TR::Register *trgReg, TR::Register *sourceReg, int64_t value, TR::CodeGenerator *cg)
{
	if (value == 0) {
		loadConstant(cg, node, 0, trgReg);
	} else if (value == 1) {
...
TR::Register *
addConstantToLong(TR::Node *node, TR::Register *srcReg, int64_t value, TR::Register *trgReg, TR::CodeGenerator *cg)
{
	if (!trgReg)
...
TR::Register *
addConstantToLong(TR::Node *node, TR::Register *srcHighReg, TR::Register *srclowReg, int32_t highValue,
...
TR::Register *
addConstantToInteger(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t value, TR::CodeGenerator *cg)
{
...
TR::Register *
addConstantToInteger(TR::Node *node, TR::Register *srcReg, int32_t value, TR::CodeGenerator *cg)
{
