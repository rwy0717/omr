...
extern TR::Register* computeCC_bitwise(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* targetReg, bool needsZeroExtension = true);
    ...
    TR::Instruction* cursor, TR::Node* node)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
            TR::Compiler->target.cpu.isBigEndian() ? highBits : lowBits, TR_AbsoluteHelperAddress, helperSymRef,
            cursor);
        cursor = generateImmInstruction(
            self(), TR::InstOpCode::dd, node, TR::Compiler->target.cpu.isBigEndian() ? lowBits : highBits, cursor);
    } else {
    ...
    while (isWAWOrmrPeepholeCandidateInstr(current) && !isEBBTerminatingBranch(current) && windowSize < maxWindowSize) {
        ...
        if (memRef != NULL) {
            ...
            case OMR::Instruction::IsTrg1Src1: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src1 source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
            ...
            case OMR::Instruction::IsTrg1Src1Imm2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm2 source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
            ...
            case OMR::Instruction::IsSrc2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Src2 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Src2 2nd source register %p.\n", current)) {
            ...
            case OMR::Instruction::IsTrg1Src2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src2 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src2 2nd source register %p.\n", current)) {
                        inst->setSource2Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
            ...
            case OMR::Instruction::IsTrg1Src2Imm: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 2nd source register %p.\n", current)) {
                        inst->setSource2Register(mrSourceReg);
                        extendWindow = true;
                    } else
                        return;
            ...
            if (!inEBB && all_mr_source_uses_rewritten
                ...
                if (!stackMaps.empty()) {
                    for (auto stackMapIter = stackMaps.begin(); stackMapIter != stackMaps.end(); ++stackMapIter) {
                        ...
                        (*stackMapIter)
                            ->resetRegistersBits(cg->registerBitMask(toRealRegister(mrTargetReg)->getRegisterNumber()));
                        (*stackMapIter)
                            ->setRegisterBits(cg->registerBitMask(toRealRegister(mrSourceReg)->getRegisterNumber()));
                    }
                }
    ...
    while (current != NULL && !current->isLabel()
        ...
        // if target reg is same as source of compare
        {
            if (current->getOpCode().isRecordForm()) {
                if (performTransformation(
                        comp, "O^O PPC PEEPHOLE: Remove redundant compare immediate %p.\n", cmpiInstruction))
...
static void lhsPeephole(TR::CodeGenerator* cg, TR::Instruction* storeInstruction)
{
    ...
    if (srcReg == trgReg) {
        ...
        if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
            if (performTransformation(comp,
                ...
                generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg, srcReg,
                    0, 0xffffffff, storeInstruction);
    ...
    if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
        if (performTransformation(comp,
                ...
                " with rlwinm.\n",
                loadInstruction, storeInstruction)) {
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg, srcReg, 0,
                0xffffffff, storeInstruction);
            loadInstruction->remove();
        }
    } else if (performTransformation(comp,
...
static void syncPeephole(TR::CodeGenerator* cg, TR::Instruction* instructionCursor, int window)
{
    ...
    while (visited < window) {
        ...
        if (removeFirst) {
            if (performTransformation(
                    comp, "O^O PPC PEEPHOLE: Remove redundant syncronization instruction %p.\n", first)) {
    ...
    for (int i = 0; i < WINDOW && next != NULL; i++) {
        ...
        if (next->isLoad() && next->getSourceRegister(0) != NULL
            ...
            if (trap->getPrev() != NULL && load->getNext() != NULL) {
                ...
                performTransformation(
                    cg->comp(), "O^O PPC PEEPHOLE: Swap trap %p and load %p instructions.\n", trap, load);
            ...
            if ((data.cursorInstruction)->isNopCandidate()) {
                ...
                if (uselessFetched >= 8 - data.cursorInstruction->MAX_LOOP_ALIGN_NOPS()) {
                    if (TR::Compiler->target.cpu.id() >= TR_PPCp6) {
                        nop = self()->generateNop(data.cursorInstruction->getNode(), data.cursorInstruction->getPrev(),
                    ...
                    for (; uselessFetched < 8; uselessFetched++) {
                        nop = self()->generateNop(
                            data.cursorInstruction->getNode(), data.cursorInstruction->getPrev(), TR_NOPStandard);
                        nop->setNext(data.cursorInstruction);
                        data.cursorInstruction = nop;
                    }
                    skipLabel = true;
...
bool OMR::Power::CodeGenerator::canTransformUnsafeSetMemory() { return (TR::Compiler->target.is64Bit()); }
    ...
    TR::Instruction* liInstruction, TR::LabelSymbol* startLabel, TR::LabelSymbol* endLabel, int32_t deltaToStartLabel)
{
    ...
    // before the startLabel. If this bad register motion occurs then the loaded offset would be wrong (when used in
    // this way) and if an exception occurs we may end up with a wild branch. This would be a very tough bug to find and
    // fix so it is worth some ugliness in the function interface.
...
uintptrj_t j2Prof_startInterval(int32_t* preambLen, int32_t* ipAssistLen, int32_t* prologueLen, TR::Compilation* comp)
{
    ...
    *ipAssistLen = ((*sizePtr) >> 16) & 0x0000ffff;
    ...
    while (currentIntervalStart != NULL
        && (currentIntervalStart->getOpCodeValue() != TR::InstOpCode::fence || currentIntervalStart->getNode() == NULL
               || currentIntervalStart->getNode()->getOpCodeValue() != TR::BBStart))
        currentIntervalStart = currentIntervalStart->getNext();

    nextIntervalInstructionPtr = currentIntervalStart;
    nextIntervalBufferPtr
        = (currentIntervalStart == NULL) ? cg->getBinaryBufferCursor() : currentIntervalStart->getBinaryEncoding();
...
uint32_t j2Prof_nextInterval(TR::Compilation* comp)
{
    ...
    while (nextBBend->getNext() != NULL && intervalLen == 0) {
        ...
        while (nextBBend->getNext() != NULL
            && (nextBBend->getOpCodeValue() != TR::InstOpCode::fence || nextBBend->getNode() == NULL
...
void j2Prof_methodReport(TR_Method* method, TR::Compilation* comp)
{
    ...
    fprintf(j2Profile, "%s%s\n", comp->getMethodHotness(), comp->isProfilingCompilation() ? "/profiled" : "");
...
bool OMR::Power::CodeGenerator::isRotateAndMask(TR::Node* node)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL &&
        ...
        ((secondOp == TR::iconst || secondOp == TR::iuconst) && contiguousBits(secondChild->getInt())
            ...
            && (((firstChild->getOpCodeValue() == TR::imul || firstChild->getOpCodeValue() == TR::iumul)
                   ...
                   || ((firstChild->getOpCodeValue() == TR::ishr || firstChild->getOpCodeValue() == TR::iushr)
                          && (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
                                 || firstChild->getSecondChild()->getOpCodeValue() == TR::iuconst)
                          && firstChild->getSecondChild()->getInt() > 0
    ...
    if (tempReg == NULL) {
                ...
                if (doAOTRelocation)
                    self()->addExternalRelocation(
                        new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                            firstInstruction, (uint8_t*)value, (uint8_t*)seqKind, TR_FixedSequenceAddress2, self()),
                    ...
                    if (doAOTRelocation)
                        self()->addExternalRelocation(
                            new (self()->trHeapMemory())
                                TR::BeforeBinaryEncodingExternalRelocation(firstInstruction, (uint8_t*)value,
        ...
        if (value != 0x0) {
            ...
            if (typeAddress == -1) {
                ...
                if (typeAddress == TR_RamMethodSequence) // NOTE: 32 bit changed to use TR_RamMethodSequence for ordered
                                                         // pair, hence, we should check this instead
                {
                    if (doAOTRelocation)
                        self()->addExternalRelocation(
                            new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                                firstInstruction, (uint8_t*)value, (uint8_t*)seqKind, TR_RamMethodSequenceReg, self()),
                            __FILE__, __LINE__, node);
...
bool OMR::Power::CodeGenerator::supportsTransientPrefetch() { return TR::Compiler->target.cpu.id() >= TR_PPCp7; }
