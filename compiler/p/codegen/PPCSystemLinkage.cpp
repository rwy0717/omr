...
uint32_t
mappedOffsetToFirstLocal(TR::CodeGenerator *cg, const TR::PPCLinkageProperties &linkage)
{
...
TR::PPCSystemLinkage::PPCSystemLinkage(TR::CodeGenerator *cg) : TR::Linkage(cg)
{
	int i = 0;
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		if (TR::Compiler->target.isAIX()) {
			// Volatile GPR (0,2-12) + FPR (0-13) + CCR (0-1,5-7) + VR (0-19) + FPR (14-31) if used as
			// vector
			_properties._numberOfDependencyGPRegisters = 12 + 14 + 5 + 20 + 18;
			_properties._offsetToFirstParm = 24;
			_properties._offsetToFirstLocal = 24;
		} else {
			// Volatile GPR (0,3-12) + FPR (0-13) + CCR (0-1,5-7) + VR (0-19) + FPR (14-31) if used as
			// vector
...
void
TR::PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol *method, List<TR::ParameterSymbol> &parmList)
{
	...
	// XXX: This hack fixes ppc64le by saving params in the current stack frame, rather than the caller's parameter
	// save area, which may not exist
	//      This code needs to be refactored to accomodate ABIs that don't guarantee a parameter save area
	// XXX: Also, for some reason all these system linkages are marked passing parms right-to-left when they should
	// be left-to-right
	...
	if (linkage.getRightToLeft()) {
		...
		while (parmCursor != NULL) {
			parmCursor->setParameterOffset(sizeOfParameterArea - offset_from_top - parmCursor->getSize()
			        + stackIndex + offsetToFirstParm);
			offset_from_top += (parmCursor->getSize() + slot_size - 1) & (~(slot_size - 1));
			parmCursor = parameterIterator.getNext();
		}
	}
...
void
TR::PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t &stackIndex)
{
	size_t align = 1;
...
void
TR::PPCSystemLinkage::createPrologue(TR::Instruction *cursor, List<TR::ParameterSymbol> &parmList)
{
	TR::Machine *machine = cg()->machine();
	...
	if (machine->getLinkRegisterKilled())
		cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
		        new (trHeapMemory()) TR::MemoryReference(sp, 2 * TR::Compiler->om.sizeofReferenceAddress(),
	...
	if (savedFirst <= TR::RealRegister::LastGPR) {
		if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
			...
			cursor = generateMemSrc1Instruction(cg(),
			        (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::stw : TR::InstOpCode::stmw,
			        firstNode,
			        new (trHeapMemory()) TR::MemoryReference(
	...
	if (bodySymbol->isEHAware() || machine->getRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod()
		...
		cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
		        new (trHeapMemory()) TR::MemoryReference(sp, TR::Compiler->om.sizeofReferenceAddress(),
		                TR::Compiler->om.sizeofReferenceAddress(), cg()),
		        gr0, cursor);
	}
	...
	if (size > properties.getOffsetToFirstParm() - argSize) {
		if (size > (-LOWER_IMMED)) {
			...
			cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stux, firstNode,
			        new (trHeapMemory())
			                TR::MemoryReference(sp, gr11, TR::Compiler->om.sizeofReferenceAddress(), cg()),
			        sp, cursor);
		} else {
			cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, firstNode,
			        new (trHeapMemory())
			                TR::MemoryReference(sp, -size, TR::Compiler->om.sizeofReferenceAddress(), cg()),
			        sp, cursor);
		}
	} else // leaf routine
	{
...
TR::PPCSystemLinkage::createEpilogue(TR::Instruction *cursor)
{
	...
	for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
	        regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex - 1)) {
		saveSize = saveSize - 8;
		cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lfd, currentNode,
	...
	if (savedFirst <= TR::RealRegister::LastGPR) {
		if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
			...
			cursor = generateTrg1MemInstruction(cg(),
			        (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::lwz : TR::InstOpCode::lmw,
...
int32_t
TR::PPCSystemLinkage::buildArgs(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies)

{
				...
				if (!cg()->canClobberNodesRegister(child, 0)) {
					if (TR::Compiler->target.is64Bit()) {
						...
						generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode,
						        tempRegister, argRegister->getRegisterPair()->getHighOrder());
					...
					if (numIntegerArgs < properties.getNumIntArgRegs() - 1) {
						...
						if (numIntegerArgs == 0 && resType.isInt64()) {
							...
							dependencies->addPostCondition(
							        resultReg, TR::RealRegister::gr4);
						} else
							TR::addDependency(dependencies,
							        argRegister->getRegisterPair()->getLowOrder(),
							        properties.getIntegerArgumentRegister(
							                numIntegerArgs + 1),
							        TR_GPR, cg());
					...
					} else // numIntegerArgs >= properties.getNumIntArgRegs()
					{
						if (TR::Compiler->target.is64Bit() && TR::Compiler->target.isLinux()) {
							mref = getOutgoingArgumentMemRef(argSize + 4, argReg,
							        TR::InstOpCode::stfs, pushToMemory[argIndex++], 4);
						} else {
					...
					if (numIntegerArgs < properties.getNumIntArgRegs()) {
						...
						if (TR::Compiler->target.is32Bit()) {
							if ((numIntegerArgs + 1) < properties.getNumIntArgRegs())
								...
								mref = getOutgoingArgumentMemRef(argSize, argReg,
								        TR::InstOpCode::stfd, pushToMemory[argIndex++],
								        8);
							}
						}
					} else // numIntegerArgs >= properties.getNumIntArgRegs()
		...
		case TR::VectorDouble:
			...
			if (numVectorArgs < properties.getNumVectorArgRegs()) {
				...
				if (numVectorArgs == 0) {
					...
					TR::addDependency(dependencies, argReg,
					        properties.getVectorArgumentRegister(numVectorArgs), TR_VRF, cg());
			} else {
	...
	while (numIntegerArgs < properties.getNumIntArgRegs()) {
		if (numIntegerArgs == 0 && resType.isAddress()) {
			dependencies->addPreCondition(
			        cg()->allocateRegister(), properties.getIntegerArgumentRegister(0));
			dependencies->addPostCondition(
	...
	int32_t floatRegsUsed = (numFloatArgs > properties.getNumFloatArgRegs()) ? properties.getNumFloatArgRegs()
	                                                                         : numFloatArgs;
	...
	if (liveVMX || liveVSXScalar || liveVSXVector) {
		for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR + 1);
...
void
TR::PPCSystemLinkage::buildDirectCall(TR::Node *callNode, TR::SymbolReference *callSymRef,
        TR::RegisterDependencyConditions *dependencies, const TR::PPCLinkageProperties &pp, int32_t argSize)
{
	...
	if (aix_style_linkage) {
		...
		if (TR::Compiler->target.cpu.isBigEndian()) {
			...
			// For Little Endian, load target's Global Entry Point into r12, TOC will be restored at branch
			// target.
...
TR::PPCSystemLinkage::buildDirectDispatch(TR::Node *callNode)
{
	...
	case TR::fcall:
	case TR::dcall: returnRegister = dependencies->searchPostConditionRegister(pp.getFloatReturnRegister()); break;
	case TR::vcall: returnRegister = dependencies->searchPostConditionRegister(pp.getVectorReturnRegister()); break;
	case TR::call: returnRegister = NULL; break;
	default: returnRegister = NULL; TR_ASSERT(0, "Unknown direct call Opcode.");
	}
        ...
        TR::Node *callNode, TR::RegisterDependencyConditions *dependencies, uint32_t sizeOfArguments)
{
	...
	TR_ASSERT(
	        (gr3 != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on return register gr3.");
#endif
	...
	TR_ASSERT(
	        (gr0 != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on scratch register gr0.");
	...
	if (TR::Compiler->target.cpu.isBigEndian()) {
		...
		generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
		        new (trHeapMemory()) TR::MemoryReference(callNode->getChild(0)->getRegister(), 0,
		                TR::Compiler->om.sizeofReferenceAddress(), cg()));
		...
		generateTrg1Src1Instruction(
		        cg(), TR::InstOpCode::mr, callNode, gr12, callNode->getChild(0)->getRegister());
		targetRegister = gr12;
	}
...
TR::PPCSystemLinkage::buildIndirectDispatch(TR::Node *callNode)
{
	...
	case TR::dcalli:
		...
		break;
	case TR::calli: returnRegister = NULL; break;
	default: returnRegister = NULL; TR_ASSERT(0, "Unknown indirect call Opcode.");
	}
...
void
TR::PPCSystemLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol *method)
{
