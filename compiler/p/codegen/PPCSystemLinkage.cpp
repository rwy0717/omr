...
uintptr_t TR::PPCSystemLinkage::calculateActualParameterOffset(uintptr_t o, TR::ParameterSymbol& p)
{
        ...
        while (parmCursor != NULL) {
            if (saveParmsInLocalArea)
                parmCursor->setParameterOffset(
                    calculateActualParameterOffset(offset_from_top + stackIndex, *parmCursor));
            else
                parmCursor->setParameterOffset(
                    calculateActualParameterOffset(offset_from_top + offsetToFirstParm + stackIndex, *parmCursor));
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    new (trHeapMemory())
                        TR::MemoryReference(sp, argSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            ...
            cursor = generateMemSrc1Instruction(cg(),
                ...
                new (trHeapMemory())
                    TR::MemoryReference(sp, argSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
    ...
    if (size > properties.getOffsetToFirstParm() - argSize) {
        if (size > (-LOWER_IMMED)) {
            ...
            cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, firstNode,
                new (trHeapMemory()) TR::MemoryReference(sp, -size, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                sp, cursor);
        }
    } else // leaf routine
    {
        ...
        if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                    ...
                    new (trHeapMemory())
                        TR::MemoryReference(sp, saveSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                    cursor);
            }
        else {
            ...
            cursor = generateTrg1MemInstruction(cg(),
                ...
                new (trHeapMemory())
                    TR::MemoryReference(sp, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
                cursor);
        }
    }
        ...
        case TR::Int64:
            ...
            if (numIntegerArgs < properties.getNumIntArgRegs()) {
                ...
                if (numIntegerArgs == 0 && (resType.isAddress() || resType.isInt32() || resType.isInt64())) {
                    ...
                    if (TR::Compiler->target.is64Bit())
                        addDependency(dependencies, argRegister, properties.getIntegerArgumentRegister(numIntegerArgs),
                            TR_GPR, cg());
                    else
                        addDependency(dependencies, argRegister->getRegisterPair()->getHighOrder(),
                ...
                if (TR::Compiler->target.is32Bit()) {
                    if (numIntegerArgs < properties.getNumIntArgRegs() - 1) {
                        ...
                        if (numIntegerArgs == 0 && resType.isInt64()) {
                            ...
                            addDependency(dependencies, argRegister->getRegisterPair()->getLowOrder(),
                ...
                if (aix_style_linkage) {
                    if (numIntegerArgs < properties.getNumIntArgRegs()) {
                        if (numIntegerArgs == 0 && resType.isAddress()) {
                            ...
                            addDependency(dependencies, NULL, properties.getIntegerArgumentRegister(numIntegerArgs),
                                TR_GPR, cg());
                    } else // numIntegerArgs >= properties.getNumIntArgRegs()
                    {
        ...
        if (TR::Compiler->target.cpu.isBigEndian()) {
            ...
            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, geReg,
                new (cg()->trHeapMemory()) TR::MemoryReference(cg()->getTOCBaseRegister(),
                    (refNum - 1) * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(),
                    cg()));
        }
    }
    ...
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, uint32_t sizeOfArguments)
{
    ...
    if (TR::Compiler->target.cpu.isBigEndian()) {
        ...
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
            new (trHeapMemory()) TR::MemoryReference(
                callNode->getChild(0)->getRegister(), 0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
