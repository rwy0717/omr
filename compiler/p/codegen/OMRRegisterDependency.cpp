...
TR_PPCScratchRegisterDependencyConditions::createDependencyConditions(TR::CodeGenerator *cg,
        ...
        TR::CodeGenerator *cg, TR::Node *node, uint32_t extranum, TR::Instruction **cursorPtr)
{
	...
	for (i = 0; i < node->getNumChildren(); i++) {
		...
		if (regList.find(reg) || (highReg && regList.find(highReg))) {
			...
			switch (kind) {
			case TR_GPR: opCode = TR::InstOpCode::mr; break;
			case TR_FPR: opCode = TR::InstOpCode::fmr; break;
			case TR_VRF: opCode = TR::InstOpCode::vor; break;
			case TR_VSX_VECTOR: opCode = TR::InstOpCode::xxlor; break;
			case TR_CCR: opCode = TR::InstOpCode::mcrf; break;
...
void
OMR::Power::RegisterDependencyConditions::unionNoRegPostCondition(TR::Register *reg, TR::CodeGenerator *cg)
{
...
void
OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(TR::Instruction *instr, TR::CodeGenerator *cg)
{
	TR::Register *virtReg;
	TR::RealRegister::RegNum regNum;
	...
	// exists for instruction selection so we don't want these associations to clobber the associations that were
	...
	assoc = !isOOL ? new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, _addCursorForPre, cg->trMemory())
	               : 0;
...
TR::RegisterDependencyConditions *
OMR::Power::RegisterDependencyConditions::clone(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added)
{
	...
	for (idx = 0; idx < postNum; idx++) {
		...
		if (!((TR::RealRegister::NoReg == singlePair->getRealRegister())
			...
			result->addPostCondition(
			        singlePair->getRegister(), singlePair->getRealRegister(), singlePair->getFlags());
#if defined(DEBUG)
...
TR::RegisterDependencyConditions *
OMR::Power::RegisterDependencyConditions::cloneAndFix(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added)
{
	TR::RegisterDependencyConditions *result;
	TR::RegisterDependency *singlePair;
...
findDependencyChainHead(TR::RegisterDependency *dep, OMR::RegisterDependencyMap &map)
{
...
static void
assignFreeRegisters(TR::Instruction *currentInstruction, TR::RegisterDependency *dep, OMR::RegisterDependencyMap &map,
        TR::CodeGenerator *cg)
{
	TR::Machine *machine = cg->machine();
...
static void
assignContendedRegisters(TR::Instruction *currentInstruction, TR::RegisterDependency *dep,
        OMR::RegisterDependencyMap &map, bool depsBlocked, TR::CodeGenerator *cg)
{
	...
	if (!spareReg) {
		// If the regs in this dep group are not blocked we need to make sure we don't spill a reg that's in the
		// middle of the chain
		if (!depsBlocked) {
	...
	while (dep) {
		...
		assignedReg = virtReg->getAssignedRealRegister() ? toRealRegister(virtReg->getAssignedRealRegister())
		                                                 : NULL;
...
void
TR_PPCRegisterDependencyGroup::assignRegisters(TR::Instruction *currentInstruction, TR_RegisterKinds kindToBeAssigned,
        uint32_t numberOfRegisters, TR::CodeGenerator *cg)
{
		...
		for (i = 0; i < numberOfRegisters; i++) {
			...
			if (dependentRegNum == TR::RealRegister::SpilledReg) {
				...
				if (virtReg->getAssignedRealRegister()) {
					...
					TR::InstOpCode::Mnemonic opCode;
					switch (rk) {
					case TR_GPR: opCode = TR::InstOpCode::Op_load; break;
					case TR_FPR:
						opCode = virtReg->isSinglePrecision() ? TR::InstOpCode::lfs
						                                      : TR::InstOpCode::lfd;
						break;
					default: TR_ASSERT(0, "Register kind not supported in OOL spill"); break;
					}
				...
				if (!(std::find(cg->getSpilledRegisterList()->begin(),
				              cg->getSpilledRegisterList()->end(), virtReg)
	...
	for (i = 0; i < numberOfRegisters; i++) {
		...
		if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::SpilledReg
			...
			case TR_GPR: depsBlocked = haveSpareGPRs; break;
			case TR_FPR: depsBlocked = haveSpareFPRs; break;
			case TR_VRF: depsBlocked = haveSpareVRFs; break;
			}
	...
	for (i = 0; i < numberOfRegisters; i++) {
		...
		// dependentRegister->getAssignedRegister() is NULL if the reg has already been spilled due to a
		// spilledReg dep
...
TR::Register *
OMR::Power::RegisterDependencyConditions::getTargetRegister(uint32_t index, TR::CodeGenerator *cg)
{
...
void
OMR::Power::RegisterDependencyConditions::stopUsingDepRegs(TR::CodeGenerator *cg, int numRetReg, TR::Register **retReg)
{
