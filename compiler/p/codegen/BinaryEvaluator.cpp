    ...
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t bitsInTarget, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src1Imm2Instruction(cg,
        TR::Compiler->target.is64Bit() ? TR::InstOpCode::rldicl : TR::InstOpCode::rlwinm, node, trgReg, srcReg, 0,
        mask);
}
...
TR::Register* computeCC_setNotZero(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ccReg, TR::Register* testReg, bool needsZeroExtension)
{
    if (ccReg == NULL)
...
TR::Register* OMR::Power::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((secondOp == TR::iconst || secondOp == TR::lconst || secondOp == TR::iuconst || secondOp == TR::luconst)
        && secondChild->getRegister() == NULL) {
        int32_t shiftAmount;
        if (secondOp == TR::iconst || secondOp == TR::iuconst)
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            if (firstChild->getOpCodeValue() == TR::i2l && firstChild->getReferenceCount() == 1
                ...
                if (firstChild->getFirstChild()->isNonNegative()) {
                    ...
                    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldic, node, trgReg, srcReg, shiftAmount,
        ...
        } else // 32 bit target
        {
            ...
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::slw, node, trgReg->getLowOrder(), srcLowReg, shiftAmountReg);
...
void ClearForNegativeShiftAmount(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* trgReg, TR::Register* shiftAmountReg);
TR::Register* ShiftRightLong32Bit(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* srcReg, TR::Register* shiftAmountReg)
{
    TR::Register* trgReg;
    TR::Node* secondChild = node->getSecondChild();
    TR::Node* firstChild = node->getFirstChild();
...
TR::Register* ShiftRightLongImm32Bit(TR::Node* node, TR::CodeGenerator* cg, TR::Register* srcReg, int32_t shiftAmount)
{
    ...
    if (shiftAmount > 32) {
        ...
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::srawi, node, trgReg->getLowOrder(), srcHighReg, shiftAmount - 32);
    } else if (shiftAmount == 32) {
    ...
    } else // (shiftAmount < 32)
    {
        ...
        TR::Register* temp2Reg = cg->allocateRegister();
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::srawi, node, trgReg->getHighOrder(), srcHighReg, shiftAmount);
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* trgReg, TR::Register* shiftAmountReg)
{
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::andc, node, trgReg->getHighOrder(), trgReg->getHighOrder(), tmp2Reg);

    cg->stopUsingRegister(tmp1Reg);
    cg->stopUsingRegister(tmp2Reg);
}

TR::Register* ShiftLeftLong32Bit(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* srcReg, TR::Register* shiftAmountReg)
{
    TR::Register *srcLowReg, *srcHighReg, *trgReg;
...
TR::Register* OMR::Power::TreeEvaluator::lshflEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // Second Op Not Constant
        ...
        // 32 bit target
            ...
            trgReg = cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::OR, node, trgReg->getLowOrder(), temp1Reg->getLowOrder(), temp3Reg->getLowOrder());
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, trgReg->getHighOrder(), temp1Reg->getHighOrder(),
                temp3Reg->getHighOrder());
...
TR::Register* OMR::Power::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (firstChild->getOpCode().isAnd() && firstChild->getSecondChild()->getOpCode().isLoadConst()
            && contiguousBits(firstChild->getSecondChild()->getInt())
            && firstChild->getSecondChild()->getInt() & 0x80000000 == 0 && firstChild->getReferenceCount() == 1
            && firstChild->getRegister() == NULL) {
...
TR::Register* OMR::Power::TreeEvaluator::lshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else // 32 bit target
    {
        ...
        if (constShift && (srcHighValue == 0 || (srcHighValue == -1 && srcLowValue < 0))) {
            ...
            if (srcHighValue == 0) {
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::srw, node, trgReg->getLowOrder(), tempReg, shiftAmountReg);
            } else {
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::sraw, node, trgReg->getLowOrder(), tempReg, shiftAmountReg);
            }
            ...
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::sraw, node, trgReg->getHighOrder(), srcHighReg, shiftAmountReg);
            if (!(constShift && srcLowValue == 0)) {
                temp1Reg = cg->allocateRegister();
                generateTrg1Src2Instruction(cg, TR::InstOpCode::srw, node, temp1Reg, srcLowReg, shiftAmountReg);
            }
...
TR::Register* OMR::Power::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (isByte && (shiftAmount > 7)) {
            ...
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
        } else if (firstChild->getOpCode().isAnd() && firstChild->getSecondChild()->getOpCode().isLoadConst()
            && contiguousBits(firstChild->getSecondChild()->getInt()) && firstChild->getReferenceCount() == 1
            && firstChild->getRegister() == NULL) {
        ...
        if (isByte)
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, src1Reg, src1Reg, 0, (uint32_t)0x000000FF);
    ...
    } else // no constant shift amount
    {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            if (constShift && srcHighValue == 0) {
                ...
                TR::Register* temp5Reg = cg->allocateRegister();
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::srw, node, trgReg->getHighOrder(), srcHighReg, shiftAmountReg);
                if (!(constShift && srcLowValue == 0)) {
                    temp1Reg = cg->allocateRegister();
                    generateTrg1Src2Instruction(cg, TR::InstOpCode::srw, node, temp1Reg, srcLowReg, shiftAmountReg);
                }
                ...
                if (constShift && srcLowValue == 0) {
                    generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::OR, node, trgReg->getLowOrder(), temp3Reg, temp5Reg);
                } else {
                    TR::Register* temp6Reg = cg->allocateRegister();
                    generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, temp6Reg, temp3Reg, temp5Reg);
                    generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::OR, node, trgReg->getLowOrder(), temp1Reg, temp6Reg);
                    cg->stopUsingRegister(temp1Reg);
                    cg->stopUsingRegister(temp6Reg);
                }
...
TR::Register* OMR::Power::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        if (cg->isRotateAndMask(node)) {
            if (firstChild->getOpCodeValue() == TR::imul || firstChild->getOpCodeValue() == TR::iumul) {
...
// Multiply a register by a constant
static void mulConstant(
    TR::Node* node, TR::Register* trgReg, TR::Register* sourceReg, int64_t value, TR::CodeGenerator* cg)
{
    if (value == 0) {
        loadConstant(cg, node, 0, trgReg);
    } else if (value == 1) {
