...
TR::Register* OMR::Power::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && (firstChild->getOpCodeValue() == TR::imul || firstChild->getOpCodeValue() == TR::iumul) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
        trgReg = cg->allocateRegister();
        TR::Register* src2Reg = cg->evaluate(secondChild);
...
TR::Register* OMR::Power::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((secondOp == TR::iconst || secondOp == TR::lconst || secondOp == TR::iuconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
        int32_t shiftAmount;
        if (secondOp == TR::iconst || secondOp == TR::iuconst)
...
TR::Register* OMR::Power::TreeEvaluator::ishflEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondOp == TR::iconst || secondOp == TR::iuconst || ((secondOp == TR::b2i) && ((secondChild->getFirstChild()->getOpCodeValue()) == TR::bconst)) || ((secondOp == TR::s2i) && ((secondChild->getFirstChild()->getOpCodeValue()) == TR::sconst))) {
...
TR::Register* OMR::Power::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (firstChild->getOpCode().isAnd() && firstChild->getSecondChild()->getOpCode().isLoadConst() && contiguousBits(firstChild->getSecondChild()->getInt()) && firstChild->getSecondChild()->getInt() & 0x80000000 == 0 && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
...
TR::Register* OMR::Power::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (isByte && (shiftAmount > 7)) {
            ...
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
        } else if (firstChild->getOpCode().isAnd() && firstChild->getSecondChild()->getOpCode().isLoadConst() && contiguousBits(firstChild->getSecondChild()->getInt()) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
