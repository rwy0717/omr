...
iorTypeEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic immedOp, TR::InstOpCode::Mnemonic immedShiftedOp,
        TR::InstOpCode::Mnemonic regOp, TR::InstOpCode::Mnemonic regOp_r, TR::CodeGenerator *cg)
{
...
static bool
genNullTestForCompressedPointers(TR::Node *node, TR::Register *&trgReg, TR::Register *&condReg, TR::Register *src1Reg,
        TR::Register *&src2Reg, TR::LabelSymbol *&doneSkipAdd, TR::CodeGenerator *cg)
{
...
static TR::Register *
carrylessLongEvaluatorWithAnalyser(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic lowRegRegOpCode,
        TR::InstOpCode::Mnemonic highRegRegOpCode, TR::InstOpCode::Mnemonic copyRegRegOpCode)
{
	...
	} else // 64 bit target
	{
		...
		if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && !setsOrReadsCC
			...
			generateTrg1Src3Instruction(
			        cg, TR::InstOpCode::maddld, node, trgReg, lmulSrc1Reg, lmulSrc2Reg, src2Reg);
			...
			// to be an lconst
			else if (!setsOrReadsCC && (secondOp == TR::iconst || secondOp == TR::iuconst) && // may be true
			                                                                                  // if aladd?
			        secondChild->getRegister() == NULL) {
				...
				if (setsOrReadsCC) {
					...
					if ((node->getOpCodeValue() == TR::luaddc)
						...
						// Currently, only the path that calculates the CC handles addc.  This
						// is fine since the simplifier will lower addc to add in all other
						// cases.
						//
						// use adde rather than addc
						regToRegOpCode = TR::InstOpCode::adde;
			...
			} else // no constants or setsOrReadsCC
			{
				...
				if (setsOrReadsCC) {
					...
					if ((node->getOpCodeValue() == TR::lusubb)
						...
						// Currently, only the path that calculates the CC handles subb.  This
						// is fine since the simplifier will lower subb to sub in all other
						// cases.
						//
...
OMR::Power::TreeEvaluator::lsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (!setsOrReadsCC
	        && (secondChild->getOpCodeValue() == TR::lconst || secondChild->getOpCodeValue() == TR::luconst)
	        && secondChild->getRegister() == NULL) {
		TR::Register *src1Reg = cg->evaluate(firstChild);
		...
		trgReg = addConstantToLong(
		        node, src1Reg->getHighOrder(), src1Reg->getLowOrder(), highValue, lowValue, cg);

	} else {
		TR::Register *lowReg = cg->allocateRegister();
		TR::Register *highReg = cg->allocateRegister();
		if (!setsOrReadsCC
		        && (firstChild->getOpCodeValue() == TR::lconst || firstChild->getOpCodeValue() == TR::luconst)
		        && firstChild->getRegister() == NULL) {
			TR::Register *src2Reg = cg->evaluate(secondChild);
			...
			if (lowValue >= LOWER_IMMED && lowValue <= UPPER_IMMED) {
				...
				generateTrg1Src2Instruction(
				        cg, TR::InstOpCode::subfc, node, lowReg, src2Reg->getLowOrder(), tempReg);
				cg->stopUsingRegister(tempReg);
			}
			if (highValue == 0) {
				...
				generateTrg1Src2Instruction(
				        cg, TR::InstOpCode::subfe, node, highReg, src2Reg->getHighOrder(), tempReg);
				cg->stopUsingRegister(tempReg);
			}
		} else {
			TR::Register *src1Reg = cg->evaluate(firstChild);
...
OMR::Power::TreeEvaluator::imulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	} else // no constants
	{
		trgReg = cg->allocateRegister();
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::mullw, node, trgReg, src1Reg, cg->evaluate(secondChild));
	}
	cg->decReferenceCount(firstChild);
	cg->decReferenceCount(secondChild);
	node->setRegister(trgReg);
...
//
TR::Register *
OMR::Power::TreeEvaluator::dualMulHelper64(
...
//
TR::Register *
OMR::Power::TreeEvaluator::dualMulHelper32(
        TR::Node *node, TR::Node *lmulNode, TR::Node *lumulhNode, TR::CodeGenerator *cg)
{
	bool needsUnsignedHighMulOnly = (lmulNode == NULL);
	// requires:
...
OMR::Power::TreeEvaluator::lmulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		} else // no constants
		{
			...
			TR::Register *src1Reg = cg->evaluate(firstChild);
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::mulld, node, trgReg, src1Reg, cg->evaluate(secondChild));
		}
...
OMR::Power::TreeEvaluator::imulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	} else // no constants
	{
		...
		// put the large magic number into the RA position
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::mulhw, node, trgReg, cg->evaluate(secondChild), src1Reg);
	}
	cg->decReferenceCount(firstChild);
	cg->decReferenceCount(secondChild);
	node->setRegister(trgReg);
...
OMR::Power::TreeEvaluator::lmulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		if (secondChild->getOpCodeValue() == TR::lconst || secondChild->getOpCodeValue() == TR::luconst) {
			...
			// put the large magic number into the RA position
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::mulhd, node, trgReg, cg->evaluate(secondChild), src1Reg);
		}
		cg->decReferenceCount(firstChild);
		cg->decReferenceCount(secondChild);
		node->setRegister(trgReg);
...
static TR::Register *
signedIntegerDivisionOrRemainderAnalyser(TR::Node *node, TR::CodeGenerator *cg, TR::Register *dividendReg,
        int32_t divisorValue, bool isRemainder, TR::Register *trgReg = NULL, TR::Register *divisorReg = NULL,
        TR::Register *tmp1Reg = NULL, TR::Register *tmp2Reg = NULL)
{
...
OMR::Power::TreeEvaluator::idivEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst())
		...
		if (dividend != 0x80000000) {
			trgReg = cg->allocateRegister();
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::divw, node, trgReg, dividendReg, cg->evaluate(secondChild));
			cg->decReferenceCount(firstChild);
			cg->decReferenceCount(secondChild);
			node->setRegister(trgReg);
			return trgReg;
...
ldiv64Evaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst())
		...
		if (dividend != CONSTANT64(0x8000000000000000)) {
			trgReg = cg->allocateRegister();
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::divd, node, trgReg, dividendReg, cg->evaluate(secondChild));
			node->setRegister(trgReg);
			cg->decReferenceCount(firstChild);
			cg->decReferenceCount(secondChild);
			return trgReg;
	...
	*dr_lowReg = dr_l;
	...
	if (!isDividendImpossible32Bit && !isDivisorImpossible32Bit) {
		...
		if (secondChild->getOpCode().isLoadConst() && drConst <= (int64_t)TR::getMaxSigned<TR::Int32>()
			...
			signedIntegerDivisionOrRemainderAnalyser(node, cg, dd_l, drConst, isRemainder,
			        isRemainder ? dr_l : dd_l, dr_l, tmp1Reg, tmp2Reg);
			...
			// Didn't use record-form for scheduling issue, but post-pass will turn it into record-form
			// anyway
...
getExtensionMask(TR::ILOpCode op)
{
	...
	case TR::b2i:
	case TR::bu2l:
	case TR::b2l: return 0xff;
	case TR::su2i:
	case TR::s2i:
	case TR::su2l:
	case TR::s2l: return 0xffff;
	case TR::iu2l:
	case TR::i2l: return 0xffffffff;
	default: TR_ASSERT_FATAL(false, "Bad conversion opcode %s", op.getName());
	}
}
...
static TR::Register *
integerShiftRight(TR::Node *node, uint32_t operandSize, bool isUnsigned, TR::CodeGenerator *cg)
{
	uint32_t operandBits = operandSize * 8;
	uint64_t operandMask = operandBits == 64 ? 0xffffffffffffffffULL : ((1ULL << operandBits) - 1);
	...
	if (node->getSecondChild()->getOpCode().isLoadConst()) {
		...
		if (isMaskThenShiftRightCandidate(cg, node, operandBits, rhs, isUnsigned)) {
			...
			uint64_t mask =
			        (operandMask
			                & node->getFirstChild()->getSecondChild()->get64bitIntegralValueAsUnsigned())
			        >> rhs;
...
OMR::Power::TreeEvaluator::lrolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (secondChild->getOpCode().isLoadConst()) {
		...
		generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, targetRegister, srcRegister,
		        shiftAmount, CONSTANT64(0xffffffffffffffff));
	} else {
		TR::Register *shiftAmountReg = cg->evaluate(secondChild);
		generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::rldcl, node, targetRegister, srcRegister,
		        shiftAmountReg, CONSTANT64(0xffffffffffffffff));
	}
...
void
simplifyANDRegImm(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int64_t value, TR::CodeGenerator *cg,
...
void
simplifyANDRegImm(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t value, TR::CodeGenerator *cg,
        TR::Node *constNode)
{
	...
	if (localVal.getValue() == 0) {
		...
		generateTrg1Src1ImmInstruction(
		        cg, TR::InstOpCode::andi_r, node, trgReg, srcReg, tmpReg, localVal.getLowBits());
		cg->stopUsingRegister(tmpReg);
	} else if (localVal.getLowBits() == 0) {
		TR::Register *tmpReg = cg->allocateRegister(TR_CCR);
		generateTrg1Src1ImmInstruction(
        ...
        TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, TR::Node *constNode, TR::CodeGenerator *cg)
{
	...
	if (lowSpecial == true || highSpecial == true) {
		...
		simplifyANDRegImm(
		        node, trgReg->getHighOrder(), srcReg->getHighOrder(), constNode->getLongIntHigh(), cg);
	} else {
		...
		generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg->getHighOrder(),
		        srcReg->getHighOrder(), constReg->getHighOrder());
	}
}
...
static inline TR::Register *
lorTypeEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic immedOp, TR::InstOpCode::Mnemonic immedShiftedOp,
        TR::InstOpCode::Mnemonic regOp, TR::InstOpCode::Mnemonic regOp_r, TR::CodeGenerator *cg)
{
	TR::Register *trgReg = NULL;
	TR::Register *src1Reg = NULL;
		...
		if ((secondOp == TR::lconst || secondOp == TR::luconst) && secondChild->getRegister() == NULL) {
			...
			if (localVal.getValue() == 0) {
				...
				if (localVal.getHighBits() == 0) {
					generateTrg1Src1ImmInstruction(cg, immedOp, node, trgReg->getLowOrder(),
					        src1Reg->getLowOrder(), localVal.getLowBits());
				} else if (localVal.getLowBits() == 0) {
			...
			if (localVal.getValue() == 0) {
				...
				if (localVal.getHighBits() == 0) {
					generateTrg1Src1ImmInstruction(cg, immedOp, node, trgReg->getHighOrder(),
	...
	if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
		if (cg->isRotateAndMask(node)) {
			...
			simplifyANDRegImm(node, trgReg, cg->evaluate(firstChild),
			        (int32_t)secondChild->get64bitIntegralValue(), cg, secondChild);
	} else {
		generateTrg1Src2Instruction(
...
OMR::Power::TreeEvaluator::lxfrsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is32Bit()) {
		...
		// trg = absolute value of the first child
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, trgReg->getLowOrder(), src1Reg->getLowOrder(), tmp1Reg);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, trgReg->getHighOrder(), src1Reg->getHighOrder(), tmp1Reg);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::subfc, node, trgReg->getLowOrder(), tmp1Reg, trgReg->getLowOrder());
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::subfe, node, trgReg->getHighOrder(), tmp1Reg, trgReg->getHighOrder());
		// apply sign of the second child to trg
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, trgReg->getLowOrder(), trgReg->getLowOrder(), tmp2Reg);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, trgReg->getHighOrder(), trgReg->getHighOrder(), tmp2Reg);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::subfc, node, trgReg->getLowOrder(), tmp2Reg, trgReg->getLowOrder());
		generateTrg1Src2Instruction(
