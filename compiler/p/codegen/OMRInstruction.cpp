...
TR::PPCDepImmInstruction* OMR::Power::Instruction::getPPCDepImmInstruction() { return NULL; }
...
{
    return (self()->getNode() != NULL && self()->getNode()->getOpCodeValue() == TR::BBStart
        && self()->getNode()->getBlock() != NULL && self()->getNode()->getBlock()->firstBlockInLoop()
    ...
    if (getOpCodeValue() == TR::InstOpCode::bl || getOpCodeValue() == TR::InstOpCode::b) {
        ...
        if (resolvedMethod != NULL && resolvedMethod->isSameMethod(cg()->comp()->getCurrentMethod())) {
            ...
            *(int32_t*)cursor |= (jitTojitStart - cursor) & 0x03fffffc;
            ...
            TR_ASSERT(
                distance <= BRANCH_FORWARD_LIMIT && distance >= BRANCH_BACKWARD_LIMIT, "CodeCache is more than 32MB");
