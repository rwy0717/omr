		...
		if (!ref->isUnresolved() && isLocalObject) {
			...
			if (ref->isUnresolved()) {
				...
				self()->setUnresolvedSnippet(new (cg->trHeapMemory())
				                TR::UnresolvedDataSnippet(cg, rootLoadOrStore, ref, isStore, false));
...
void
OMR::Power::MemoryReference::addToOffset(TR::Node *node, intptrj_t amount, TR::CodeGenerator *cg)
{
		...
		if (_baseRegister != NULL) {
			if (node->getOpCode().isLoadConst() && node->getRegister()) {
				generateTrg1Src2Instruction(
				        cg, TR::InstOpCode::add, node, newBase, _baseRegister, node->getRegister());
			} else {
...
void
OMR::Power::MemoryReference::forceIndexedForm(TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *cursor)
{
...
void
OMR::Power::MemoryReference::bookKeepingRegisterUses(TR::Instruction *instr, TR::CodeGenerator *cg)
{
	if (_baseRegister != NULL) {
...
void
OMR::Power::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
	...
	// Skip sign extension if the offset is known to be >= 0; this is only safe to do if the child is the result of
	// a load, otherwise the upper 32 bits may be undefined
	...
	if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
		...
		if (subTree->getOpCode().isArrayRef()
			...
			if (TR::Compiler->target.is64Bit()) { // 64-bit
				...
				//    iconst(lconst)
				{
					self()->addToOffset(subTree,
					        subTree->getFirstChild()->getFirstChild()->getInt() << amount, cg);
					cg->decReferenceCount(subTree->getFirstChild()->getFirstChild());
				}
			} else
				self()->addToOffset(subTree,
				        subTree->getFirstChild()->getInt() << subTree->getSecondChild()->getInt(), cg);
			...
			intptrj_t amount = (subTree->getOpCodeValue() == TR::iconst) ? subTree->getInt()
			                                                             : subTree->getLongInt();
			self()->addToOffset(subTree, amount, cg);
		} else {
			if (_baseRegister != NULL) {
				self()->consolidateRegisters(
				        cg->evaluate(subTree), subTree, cg->canClobberNodesRegister(subTree), cg);
			} else {
				_baseRegister = cg->evaluate(subTree);
				_baseNode = subTree;
...
void
OMR::Power::MemoryReference::consolidateRegisters(
        TR::Register *srcReg, TR::Node *srcTree, bool srcModifiable, TR::CodeGenerator *cg)
{
	...
	if (self()->getUnresolvedSnippet() != NULL) {
		...
		if (_indexRegister != NULL) {
			...
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::add, tempNode, tempTargetRegister, _indexRegister, srcReg);
		...
		if (_indexRegister != NULL) {
			...
			generateTrg1Src2Instruction(
			        cg, TR::InstOpCode::add, tempNode, tempTargetRegister, _baseRegister, srcReg);
			if (_baseRegister != tempTargetRegister) {
				self()->decNodeReferenceCounts(cg);
				_baseNode = NULL;
			}
...
void
OMR::Power::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
	...
	if (_baseRegister != NULL) {
		...
		if ((assignedBaseRegister != NULL)
			...
			if ((alternativeRegister = machine->findBestFreeRegister(
			        ...
			        == NULL) {
				alternativeRegister =
				        machine->freeBestRegister(currentInstruction, _baseRegister, NULL, true);
			}
...
OMR::Power::MemoryReference::mapOpCode(TR::Instruction *currentInstruction)
{
	...
	switch (op) {
	case TR::InstOpCode::lbz: currentInstruction->setOpCodeValue(TR::InstOpCode::lbzx); break;
	case TR::InstOpCode::ld: currentInstruction->setOpCodeValue(TR::InstOpCode::ldx); break;
	case TR::InstOpCode::lfd: currentInstruction->setOpCodeValue(TR::InstOpCode::lfdx); break;
	case TR::InstOpCode::lfdu: currentInstruction->setOpCodeValue(TR::InstOpCode::lfdux); break;
	case TR::InstOpCode::lfs: currentInstruction->setOpCodeValue(TR::InstOpCode::lfsx); break;
	case TR::InstOpCode::lfsu: currentInstruction->setOpCodeValue(TR::InstOpCode::lfsux); break;
	case TR::InstOpCode::lha: currentInstruction->setOpCodeValue(TR::InstOpCode::lhax); break;
	case TR::InstOpCode::lhau: currentInstruction->setOpCodeValue(TR::InstOpCode::lhaux); break;
	case TR::InstOpCode::lhz: currentInstruction->setOpCodeValue(TR::InstOpCode::lhzx); break;
	case TR::InstOpCode::lhzu: currentInstruction->setOpCodeValue(TR::InstOpCode::lhzux); break;
	case TR::InstOpCode::lwa: currentInstruction->setOpCodeValue(TR::InstOpCode::lwax); break;
	case TR::InstOpCode::lwz: currentInstruction->setOpCodeValue(TR::InstOpCode::lwzx); break;
	case TR::InstOpCode::lwzu: currentInstruction->setOpCodeValue(TR::InstOpCode::lwzux); break;
	case TR::InstOpCode::stb: currentInstruction->setOpCodeValue(TR::InstOpCode::stbx); break;
	case TR::InstOpCode::stbu: currentInstruction->setOpCodeValue(TR::InstOpCode::stbux); break;
	case TR::InstOpCode::std: currentInstruction->setOpCodeValue(TR::InstOpCode::stdx); break;
	case TR::InstOpCode::stdu: currentInstruction->setOpCodeValue(TR::InstOpCode::stdux); break;
	case TR::InstOpCode::stfd: currentInstruction->setOpCodeValue(TR::InstOpCode::stfdx); break;
	case TR::InstOpCode::stfdu: currentInstruction->setOpCodeValue(TR::InstOpCode::stfdux); break;
	case TR::InstOpCode::stfs: currentInstruction->setOpCodeValue(TR::InstOpCode::stfsx); break;
	case TR::InstOpCode::stfsu: currentInstruction->setOpCodeValue(TR::InstOpCode::stfsux); break;
	case TR::InstOpCode::sth: currentInstruction->setOpCodeValue(TR::InstOpCode::sthx); break;
	case TR::InstOpCode::sthu: currentInstruction->setOpCodeValue(TR::InstOpCode::sthux); break;
	case TR::InstOpCode::stw: currentInstruction->setOpCodeValue(TR::InstOpCode::stwx); break;
	case TR::InstOpCode::stwu: currentInstruction->setOpCodeValue(TR::InstOpCode::stwux); break;
	default: break;
	}
}
			...
			if (displacement != PTOC_FULL_INDEX) {
				if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
					*wcursor = preserve | lower;
					toRealRegister(getModBase())->setRegisterFieldRA(wcursor); // mb is the modified
					                                                           // base
					cursor += PPC_INSTRUCTION_LENGTH;
				}
			} else {
				...
				if (comp->compileRelocatableCode() && symbol->isStatic() && symbol->isClassObject()) {
					...
					if (comp->getOption(TR_UseSymbolValidationManager)) {
						...
						cg->addExternalRelocation(
						        new (cg->trHeapMemory())
						                TR::ExternalRelocation(cursor, (uint8_t *)recordInfo,
...
choose_rX(TR::Instruction *currentInstruction, TR::RealRegister *base)
{
	...
	switch (exclude) {
	case 0:
	case 2: return (TR::RealRegister::gr10);
	case 1: return (TR::RealRegister::gr9);
	case 3: return (TR::RealRegister::gr8);
	}
...
{}
void
OMR::Power::MemoryReference::accessStaticItem(
        TR::Node *node, TR::SymbolReference *ref, bool isStore, TR::CodeGenerator *cg)
{
	...
	if (isPicSite && !cg->comp()->compileRelocatableCode()
		...
		loadAddressConstantInSnippet(cg, node ? node : cg->getCurrentEvaluationTreeTop()->getNode(), address,
		        reg, NULL, isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load, false, NULL);
		return;
	}
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		if (cg->comp()->compileRelocatableCode()) {
			...
			if (symbol->isCountForRecompile()) {
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(cg, true, nodeForSymbol, TR_CountForRecompile, reg, NULL, false,
				        TR_GlobalValue);
				return;
				...
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(
				        cg, true, nodeForSymbol, (intptrj_t)ref, reg, NULL, false, TR_ClassAddress);
				return;
			} else {
		...
		if (ref->isUnresolved() || comp->compileRelocatableCode()) {
			...
			if (symbol->isDebugCounter()) {
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol, 1, reg, NULL,
				        false, TR_DebugCounter);
				return;
			} else if (symbol->isCountForRecompile()) {
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol,
				        TR_CountForRecompile, reg, NULL, false, TR_GlobalValue);
				return;
			} else if (symbol->isRecompilationCounter()) {
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol, 0, reg, NULL,
				        false, TR_BodyInfoAddressLoad);
				return;
			} else if (symbol->isCompiledMethod()) {
				TR::Register *reg = _baseRegister = cg->allocateRegister();
				loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol, 0, reg, NULL,
...
void
OMR::Power::MemoryReference::setSymbol(TR::Symbol *symbol, TR::CodeGenerator *cg)
{
