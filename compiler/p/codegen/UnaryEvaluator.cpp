...
TR::Register *
OMR::Power::TreeEvaluator::iconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::aconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::lconstEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
OMR::Power::TreeEvaluator::lnegEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		generateTrg1Src1ImmInstruction(
		        cg, TR::InstOpCode::subfic, node, lowReg, sourceRegister->getLowOrder(), 0);
...
OMR::Power::TreeEvaluator::labsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		...
		generateTrg1Src1ImmInstruction(
		        cg, TR::InstOpCode::srawi, node, tempReg, sourceRegister->getHighOrder(), 31);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, highReg, sourceRegister->getHighOrder(), tempReg);
		generateTrg1Src2Instruction(
		        cg, TR::InstOpCode::XOR, node, lowReg, sourceRegister->getLowOrder(), tempReg);
...
OMR::Power::TreeEvaluator::s2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	} else // 32 bit target
	{
		...
		generateTrg1Src1ImmInstruction(
		        cg, TR::InstOpCode::srawi, node, trgReg->getHighOrder(), trgReg->getLowOrder(), 31);
		node->setRegister(trgReg);
		cg->decReferenceCount(child);
		return trgReg;
	}
...
OMR::Power::TreeEvaluator::iu2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *child = node->getFirstChild();
	if (TR::Compiler->target.is64Bit()) {
		if (node->getOpCodeValue() == TR::iu2l && child && child->getReferenceCount() == 1
		        && !child->getRegister()
		...
		if (!cg->useClobberEvaluate() && // Sharing registers between nodes only works when not using simple
		                                 // clobber evaluation
...
OMR::Power::TreeEvaluator::i2sEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.cpu.id() != TR_PPCp6 && // avoid algebraic loads on P6
	        child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference()
...
OMR::Power::TreeEvaluator::l2bEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference()
	        && (child->getRegister() == NULL)) {
...
OMR::Power::TreeEvaluator::l2cEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && (temp == NULL)) {
		...
		if (TR::Compiler->target.is64Bit())
			generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, trgReg, cg->evaluate(child),
			        0, CONSTANT64(0x000000000000ffff));
		else // 32 bit target
			generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, trgReg,
			        cg->evaluate(child)->getLowOrder(), 0, 0xffff);
		cg->decReferenceCount(child);
	}
	return node->setRegister(trgReg);
}
	...
	if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && child->getRegister() == NULL) {
		...
		if (TR::Compiler->target.is64Bit())
			...
			generateTrg1Src1Instruction(
			        cg, TR::InstOpCode::extsh, node, trgReg, cg->evaluate(child)->getLowOrder());
		cg->decReferenceCount(child);
	}
	return node->setRegister(trgReg);
}
...
OMR::Power::TreeEvaluator::bu2lEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	...
	if (TR::Compiler->target.is64Bit()) {
		if (child->getOpCode().isMemoryReference()) {
			...
			trgReg = cg->allocateRegister();
			generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, trgReg, cg->evaluate(child),
			        0, CONSTANT64(0x00000000000000ff));
		}
	} else // 32 bit target
	{
		TR::Register *lowReg;
...
TR::Register *
OMR::Power::TreeEvaluator::libmFuncEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::strcmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::strcFuncEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *
OMR::Power::TreeEvaluator::dfloorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
	TR::Node *firstChild = node->getFirstChild();
	...
	generateTrg1Src2Instruction(cg, TR::InstOpCode::fadd, node, tmp2Reg, trgReg, tmp1Reg); // tmp2  = trunc(x) -1
	                                                                                       // for floor
	                                                                                       //       = trunc(x) +1
	                                                                                       //       for ceil
	if ((node->getOpCodeValue() == TR::dfloor) || (node->getOpCodeValue() == TR::ffloor))
		generateTrg1Src2Instruction(cg, TR::InstOpCode::fsub, node, tmp3Reg, srcReg, trgReg); // tmp3  =
		                                                                                      // (x-trunc(x))
		                                                                                      // for floor
	else
		generateTrg1Src2Instruction(cg, TR::InstOpCode::fsub, node, tmp3Reg, trgReg, srcReg); // tmp3  =
		                                                                                      // (trunc(x)-x)
		                                                                                      // for ceil
	...
	generateTrg1Src2Instruction(cg, TR::InstOpCode::fsub, node, tmp5Reg, tmp5Reg, tmp4Reg); // tmp5  = 2**52 -
	                                                                                        // fabs(x)
