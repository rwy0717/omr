...
static uint32_t infoForGPR(
    struct OMRPortLibrary* portLibrary, struct J9Win32SignalInfo* info, int32_t index, const char** name, void** value);
...
static J9WinAMD64AsyncHandlerRecord* createAsyncHandlerRecord(
    ...
    void* handler_arg, uint32_t portlibSignalFlag, void** oldOSHandler)
{
    ...
    if (!foundHandler && (0 != portlibSignalFlag)) {
        J9WinAMD64AsyncHandlerRecord* record
...
uint32_t omrsig_get_options(struct OMRPortLibrary* portLibrary) { return signalOptions; }
    ...
    OMRPortLibrary* portLibrary, uint32_t flags, uint32_t allowedSubsetOfFlags, void** oldOSHandler)
{
    ...
    if (0 != flagsSignalsOnly) {
         ...
         */
        omrthread_monitor_enter(registerHandlerMonitor);
        for (portSignalType = OMRPORT_SIG_SMALLEST_SIGNAL_FLAG; portSignalType < allowedSubsetOfFlags;
...
static J9WinAMD64AsyncHandlerRecord* createAsyncHandlerRecord(
    struct OMRPortLibrary* portLibrary, omrsig_handler_fn handler, void* handler_arg, uint32_t flags)
{
    J9WinAMD64AsyncHandlerRecord* record = (J9WinAMD64AsyncHandlerRecord*)portLibrary->mem_allocate_memory(
