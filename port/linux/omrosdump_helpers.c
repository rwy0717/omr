...
static intptr_t deriveCoreFileName(struct OMRPortLibrary *portLibrary, char *corePatternFormat, BOOLEAN coreUsesPID,
...
renameDump(struct OMRPortLibrary *portLibrary, char *filename, pid_t pid, int signalNumber)
{
	...
	if (0 == procRC) {
		...
		if ('|' == corePatternFormat[0]) {
			...
			portLibrary->nls_printf(portLibrary, J9NLS_WARNING | J9NLS_STDERR,
			        J9NLS_PORT_LINUXDUMP_PIPE_CORE, corePatternFileName, corePatternFormat, pid);
			strncpy(corePatternFormat, "core", PATH_MAX);
		}
	} else if (1 == procRC) {
		/* corePatternFileName does not exist (it should be there from kernel 2.5 onwards), use the default core
		 * file name */
		strncpy(corePatternFormat, "core", PATH_MAX);
	} else {
	...
	if (NULL != lastSep) {
		/* filename was an absolute path: strip it down to the directory name, make sure to preserve the
		 * trailing front slash */
	...
	createCoreFileNameRC = deriveCoreFileName(portLibrary, corePatternFormat, coreUsesPID, tempPath,
 ...
 * filename
 * @param[in] baseDir           the null-terminated string indicating the absolute path (with trailing dir separator) of
 * the base directory to use if corePatternFormat does not specify an absolute path.
 ...
 */
static intptr_t
deriveCoreFileName(struct OMRPortLibrary *portLibrary, char *corePatternFormat, BOOLEAN coreUsesPID, char *baseDir,
        char *derivedCoreFileName, uintptr_t derivedCoreFileNameLen, pid_t pid, int signalNumber)
{
	...
	while ('\0' != *inCursor) {
		...
		case '%':
			...
			case 'E': {
				...
				charsPrinted =
				        portLibrary->str_printf(portLibrary, outCursor, bytesLeft, "%s", scratchSpace);
			} break;
			case 'I':
				charsPrinted = portLibrary->str_printf(portLibrary, outCursor, bytesLeft, "%s", "*");
				++numWildcards;
			...
			case 'p':
				...
				break;
			case 'u':
				charsPrinted = portLibrary->str_printf(portLibrary, outCursor, bytesLeft, "%u",
			...
			case 's':
				charsPrinted =
				        portLibrary->str_printf(portLibrary, outCursor, bytesLeft, "%i", signalNumber);
				break;
			case 'h':
...
#define PR_GET_NAME 16
				...
				if (prctl(PR_GET_NAME, scratchSpace) >= 0) {
					 ...
					 * scratchSpace contains the thread name, which is null terminated if shorter
					 * than 16 bytes.
					 */
					scratchSpace[16] = '\0'; /* ensure the name is terminated by a null if it is
			...
			break;
		default: *outCursor = *inCursor; charsPrinted = 1;
		}
	...
	*outCursor = '\0';
	...
	if (('\0' != *baseDir) && (NULL == strchr(derivedCoreFileName, '/'))) {
		charsPrinted = portLibrary->str_printf(
		        portLibrary, scratchSpace, PATH_MAX, "%s%s", baseDir, derivedCoreFileName);
		...
		while (0 != rc) {
			...
			while (NULL != (entry = readdir(coreDirectory))) {
				...
				if (strstr(filename, pathSeparator + 1) && strstr(filename, wildcardPos + 1)) {
					/* Good match, fix up the supplied core dump file name using the actual file
					 * name */
	...
	for (pass = 0; modified && (pass < log2N); pass++) {
		...
		if (fd != NULL) {
			...
			while (fread(buf, 1, sizeof(buf) - 1, fd) == sizeof(buf) - 1) {
				...
				/* use strtoull for correctness on 64-bit platforms and gratuitous extra precision on
				 * 32-bit platforms */
 ...
 */
static uintptr_t
getSharedAndPrivateDataSegments(
        struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize, uint8_t **ppStart, uint8_t **ppEnd)
{
	char ch;
	...
	if (count == EI_NIDENT) {
		...
		if (memcmp(pElfHeader->e_ident, magic, sizeof(magic)) == 0) {
			...
			if (1 == pElfHeader->e_ident[EI_CLASS]) {
				...
				count = portLibrary->file_read(
				        portLibrary, fd, &p32elfHeader->e_type, sizeof(Elf32_Ehdr) - EI_NIDENT);
				return 32;
			} else if (2 == pElfHeader->e_ident[EI_CLASS]) {
				/* it's for a 64 bit machine. */
				/* now read the rest of the header. */
				count = portLibrary->file_read(
				        portLibrary, fd, &p64elfHeader->e_type, sizeof(Elf64_Ehdr) - EI_NIDENT);
				return 64;
			}
		}
	}
 ...
 */
static uintptr_t
findProgramHeader(struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize, Elf64_Off phoff,
        Elf64_Half phnum, uint8_t *start, uint8_t *end)
{
	uintptr_t count;
	uintptr_t offset = 0;
...
copyDataIntoELFCore(struct OMRPortLibrary *portLibrary, intptr_t fd_core, uintptr_t addrSize, Elf64_Off phoff)
{
	...
	if (32 == addrSize) {
		...
		if (0 == p32elfPgmHdr->p_filesz) {
			/* the memory is not included in the file, so copy it to the end and update the 32bit program
			 * header. */

			/* copy the data to the end of the core file. */
			dataOffset = portLibrary->file_seek(portLibrary, fd_core, 0, EsSeekEnd);
			if (dataOffset != -1) {
				dataSize = portLibrary->file_write(portLibrary, fd_core,
				        (const void *)(uintptr_t)p32elfPgmHdr->p_vaddr, p32elfPgmHdr->p_memsz);
				if (dataSize > 0) {
		...
		if (0 == p64elfPgmHdr->p_filesz) {
			/* the memory is not included in the file, so copy it to the end and update the 64bit program
			 * header. */
			...
			if (dataOffset != -1) {
				dataSize = portLibrary->file_write(portLibrary, fd_core,
				        (const void *)(uintptr_t)p64elfPgmHdr->p_vaddr, p64elfPgmHdr->p_memsz);
				if (dataSize > 0) {
	...
	if (fd_maps != -1) {
		...
		if (fd_core != -1) {
			...
			while (getSharedAndPrivateDataSegments(portLibrary, fd_maps, addrSize, &start, &end)) {
				...
				phoff_update =
				        findProgramHeader(portLibrary, fd_core, addrSize, phoff, phnum, start, end);
				if (0 == phoff_update) {
					...
					/* we have a program header to work with. if the ph shows the data is not in the
					 * core */
