    ...
    uintptr_t inBufferSize, uint8_t* outBuffer, uintptr_t outBufferSize)
{
    ...
    while (mutf8Remaining > 0) { /* translated section by section as dictated by the size of wideBuffer */
        ...
        platformPartCount = convertWideToPlatform(
            portLibrary, encodingState, &wideBufferCursor, &wideBufferCount, outCursor, outLimit);
        /*
 ...
 * updates its inputs The output buffer may contain additional data such as a byte order mark (0xff, 0xfe or vice versa)
 * If the buffer is large enough, the byte after the last output character is set to 0.
 * @param [in] portLibrary port library
