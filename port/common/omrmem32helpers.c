	...
	if (NULL != portLibrary->portGlobals) {
		...
		while (NULL != heapWrapperCursor) {
			...
			if (!currentHeapWrapper->heap) {
				/* jmem double-accounting prevention: Increment the memory counter so it can be
				 * decremented again inside vmem_free_memory */
        ...
        struct OMRPortLibrary *portLibrary, uintptr_t regionSize, const char *callSite, uintptr_t vmemAllocOptions)
{
	...
	allocPtr = allocateVmemRegion32(portLibrary, roundedRegionSize, &heapWrapper, callSite,
	        OMRMEM_CATEGORY_PORT_LIBRARY, VMEM_MODE_COMMIT, vmemAllocOptions);
	...
	if (NULL != allocPtr) {
		...
		/* The memory category for the user of this memory will be incremented in omrmemtag.c. To prevent
		 * double-accounting, we must reset the port library counter.
		 */
...
iterateHeapsAndSubAllocate(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount)
{
	...
	while (NULL != heapWrapperCursor) {
		...
		if (NULL != heap) {
			...
			if (NULL != subAllocPtr) {
				...
				/* omrmem category double-accounting prevention: subtract the amount allocated from the
				 * unused category */
	...
	/**
	 * If we are here, then there is not enough space in the existing heapWrappers to satisfy the requested
	 * byteAmount. Then check the subCommitHeapWrapper whether it can grow or not. If it can grow, meaning not all
	 * of the initially reserved memory is committed, then commit more memory from the reserved memory, grow the
	 * heap and try to allocate byteAmount. If the memory is allocated successfully after growing the heap, then
	 * return the memory address of allocated memory, otherwise, return NULL.
	 *
	 */
	if ((NULL != PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper)
		 ...
		 * not the end address of the heap since it is probably different cause of rounding down in heap
		 * creation.
		 */
		commitPtr = (void *)(((uintptr_t)PPG_mem_mem32_subAllocHeapMem32.subCommitHeapWrapper->vmemID->address)
		        + PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize);
		commitPtr = omrvmem_commit_memory(portLibrary, commitPtr, commitSize,
 ...
 */
static void *
allocateRegion(struct OMRPortLibrary *portLibrary, uintptr_t regionSize, uintptr_t byteAmount, const char *callSite,
        uintptr_t vmemAllocOptions)
{
	...
	if (NULL != subAllocPtr) {
		...
		omrmem_categories_decrement_bytes(
		        omrmem_get_category(portLibrary, OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS),
		        byteAmount);

		Trc_PRT_mem_allocate_memory32_alloc_normal_region_suballoc(alloc32Ptr, omrheap, returnPtr, byteAmount);
	} else {
		/* if the byteAmount requested is so close to roundedRegionSize that suballocate byteAmount in the
		 * omrheap cannot be satisfied due to omrheap management overhead and roudning effect, we simply return
		 * the vmem allocated pointer and not treat it as a J9Heap.
		 */
		...
		/* Don't increment the category for port library - this category will be incremented for the user in
		 * omrmemtag.c */
 ...
 */
static void *
allocateVmemRegion32(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount, J9HeapWrapper **heapWrapper,
        const char *callSite, uint32_t memoryCategory, uintptr_t vmemMode, uintptr_t vmemAllocOptions)
{
	void *pointer = NULL;
		...
		if ((foundHeapWrapper = findMatchingHeap(portLibrary, memoryPointer, &heapWrapperUpdate)) != NULL) {
			...
			if (NULL == omrheap) {
				...
				*heapWrapperUpdate = foundHeapWrapper->nextHeapWrapper;
				...
				/* omrmem_category double-accounting prevention: add the size of the block to the
				 * "unused" category */
