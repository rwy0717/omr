...
static void* iterateHeapsAndSubAllocate(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount)
{
     ...
     * If the memory is allocated successfully after growing the heap, then return the memory address of allocated
    ...
    const char* callSite, uintptr_t vmemAllocOptions)
{
    ...
    if (NULL != subAllocPtr) {
         ...
         * cannot be satisfied due to omrheap management overhead and roudning effect, we simply return the vmem
...
uintptr_t ensure_capacity32(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount)
{
    ...
    if (0 == returnValue) {
        if (0 == PPG_mem_mem32_subAllocHeapMem32.subCommitCommittedMemorySize) {
            ...
            returnValue
                = (uintptr_t)reserveAndCommitRegion(portLibrary, byteAmount, OMR_GET_CALLSITE(), vmemAllocOptions);
        } else {
