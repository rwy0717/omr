 ...
 * @deprecated NLS API is deprecated.
 */
void j9nls_set_catalog(struct OMRPortLibrary* portLibrary, const char** paths, const int nPaths, const char* baseName,
                    ...
                    if (keyLength >= 0) {
                        ...
                        if (strncmp(searchKey, buf, sizeof(J9NLS_EXEMPLAR) - 1) == 0) {
                            ...
                            if (flags & J9NLS_DO_NOT_PRINT_MESSAGE_TAG) {
                                ...
                                entry = nls_allocateHashEntry(portLibrary, module_name, message_num, prefix,
...
static uint32_t nlsh_hash(uint32_t module_name, uint32_t message_num) { return (module_name ^ message_num); }
