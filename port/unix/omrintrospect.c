...
static int sem_destroy_r(sem_t_r* sem)
{
    ...
    if (old_value != sem->initial_value) {
        ...
        do {
            ...
            compareAndSwapUDATA((uintptr_t*)&sem->sem_value, old_value, old_value + sem->initial_value) != old_value);
...
static void upcall_handler(int signal, siginfo_t* siginfo, void* context_arg)
{
    ...
    if (ret != 0) {
        ...
        if (data->thread == NULL) {
            ...
            data->thread->dsa = __dsa_prev(
                getdsa(), __EDCWCCWI_LOGICAL, __EDCWCCWI_DOWN, NULL, data->thread->caa, &format, NULL, NULL);
...
static int setup_native_thread(J9ThreadWalkState* state, thread_context* sigContext, int heapAllocate)
{
    ...
    if (state->current_thread->callstack == NULL) {
        ...
        if (state->current_thread->callstack == NULL && data->uninitializedThreads) {
            ...
            /* we have to copy the message into heap allocated memory because we can't differentiate constant strings
             * and alloc'd strings when freeing thread data.
             */
