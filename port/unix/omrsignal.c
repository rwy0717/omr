    ...
    void* handler_arg, uint32_t portlibSignalFlag, void** oldOSHandler)
{
    ...
    if (OMR_ARE_ANY_BITS_SET(signalOptionsGlobal, OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS)) {
        /* -Xrs was set, we can't protect against any signals, do not install any handlers except SIGXFSZ*/
        if (OMR_ARE_ANY_BITS_SET(portlibSignalFlag, OMRPORT_SIG_FLAG_SIGXFSZ)
            && OMR_ARE_ANY_BITS_SET(signalOptionsGlobal, OMRPORT_SIG_OPTIONS_SIGXFSZ)) {
    ...
    OMRPortLibrary* portLibrary, uint32_t portLibrarySignalNo, unix_sigaction handler, void** oldOSHandler)
{
    ...
    /* Setting to SA_SIGINFO will result in "void (*sa_sigaction) (int, siginfo_t *, void *)" to be used, and not
     * "__sighandler_t sa_handler". Both are members of struct sigaction. Using the former allows us to access more than
...
uint32_t omrsig_get_options(struct OMRPortLibrary* portLibrary) { return signalOptionsGlobal; }
