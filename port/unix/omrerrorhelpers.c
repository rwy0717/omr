...
#if defined(AIXPPC)
    /* On AIX, strerror() returns an error string based on current locale encoding. When printing out the error message
     * in file_write_using_iconv(), The NLS message (UTF-8) + the error string is converted from UTF-8 to locale
     * encoding. So non-UTF-8 error string needs to be converted to UTF-8 here.
     */
