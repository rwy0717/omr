    ...
    struct OMRPortLibrary* portLibrary, J9PlatformThread* threadInfo, J9Heap* heap, void* signalInfo)
{
    ...
    if (threadInfo->dsa) {
         ...
         */
        dsaptr = __dsa_prev(
            dsaptr, __EDCWCCWI_LOGICAL, (int)dsa_format, NULL, NULL, (int*)&callers_dsa_format, NULL, NULL);
    ...
    for (frameNumber = 0; callers_dsaptr != NULL; frameNumber++) {
        ...
        errno = 0;
        callers_dsaptr
            = __dsa_prev(dsaptr, __EDCWCCWI_PHYSICAL, dsa_format, NULL, NULL, &callers_dsa_format, NULL, NULL);
    ...
    for (i = 0, frame = threadInfo->callstack; frame != NULL; frame = frame->parent_frame, i++) {
        ...
        if (symbol_name[0] != '\0') {
            ...
            cursor += omrstr_printf(
                portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "+0x%x ", symbol_offset);
        }
        *(cursor++) = '(';
        if (statement_id[0] != '\0') {
            cursor += omrstr_printf(portLibrary, cursor, sizeof(output_buf) - (cursor - output_buf), "%.*s, ",
                statement_id_length, statement_id);
        }
        cursor += omrstr_printf(
