...
static uint32_t mapZOS390ExceptionToPortlibType(uint32_t exceptionCode) { return 0; }
 ...
 *	@param[in]	token 	An J9ZOSLEConditionHandlerRecord as specified when this handler was registered by @ref
 ...
 *
 */
void j9vm_le_condition_handler(_FEEDBACK* fc, _INT4* token, _INT4* leResult, _FEEDBACK* newfc)
{
...
#pragma convlit(resume)
     ...
     *
     * If at this point portlibSignalNo == OMRPORT_SIG_FLAG_DOES_NOT_MAP_TO_POSIX, we have received a condition that
     * cannot be mapped to a harware-raised POSIX signal.
...
void ceehdlr_shutdown(struct OMRPortLibrary* portLibrary) { omrthread_tls_free(tlsKeyCurrentSignal); }
