...
#if defined(OMR_ENV_DATA64)
					result = omrdiscard_data(
					        (void *)address, byteAmount >> ZOS_REAL_FRAME_SIZE_SHIFT);
#else
					result = Pgser_Release((void *)address, byteAmount);
#endif /* defined(OMR_ENV_DATA64) */
					break;
        ...
        struct J9PortVmemIdentifier *identifier)
{
	...
	/* Default page Size */
	case OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY: {
		/* calculating the base address is done in omrvmem_reserve_memory, so the free address here must reflect
		 * the allocation address there */
...
static void *
default_pageSize_reserve_low_memory(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount,
        struct J9PortVmemIdentifier *identifier, uintptr_t mode, OMRMemCategory *category)
{
	...
	if (OMR_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
		/* No need to make byteAmount pagesize (=4K) aligned, as the caller omrvmem_reserve_memory_ex() already
		 * ensures that */
		...
		/* malloc31 is not guaranteed to be 4k aligned.  So align to 4k.  Add an extra uintptr_t to store the
		 * original address, which can be used by free
		 */
 ...
 */
static void *
reserve1MPageableBelowBar(struct OMRPortLibrary *portLibrary, J9PortVmemIdentifier *identifier, uintptr_t mode,
        uintptr_t byteAmount, OMRMemCategory *category)
{
	void *ptr = NULL;
	...
	if (NULL != ptr) {
		...
		update_vmemIdentifier(identifier, ptr, ptr, byteAmount, mode, ONE_M, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE,
		        allocator, category);
	} else {
 ...
 */
static void *
reservePagesAboveBar(struct OMRPortLibrary *portLibrary, J9PortVmemIdentifier *identifier, uintptr_t mode,
        uintptr_t byteAmount, uintptr_t pageSize, uintptr_t pageFlags, uintptr_t options, OMRMemCategory *category)
{
	/* The maximum amount of memory that can be allocated with userExtendedPrivateAreaMemoryType detected for 2To32G
	 * and 2To64 */
...
#define MAX_2TO64G_MEMORY_IN_MB ((UDATA)62 * 1024) /* unit is MB i.e. 62*1024 MB = 62 GB */
	...
	if ((ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)
		...
		if (TWO_G == pageSize) {
			LP_DEBUG_PRINTF2("\t *** ERROR reservePagesAboveBar max 1GB units is 0x%zx, requesting "
			                 "(0x%zx)\n",
			        (userExtendedPrivateAreaMemoryMax / 1024), numUnits);
		} else {
			LP_DEBUG_PRINTF2("\t *** ERROR reservePagesAboveBar max 1MB segments is 0x%zx, requesting "
			                 "(0x%zx)\n",
	...
	if ((FOUR_K == pageSize) && (ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)) {
		...
		if (0 != PPG_vmem_pageSize[index]) {
			...
			if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index])
				...
				LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling omrallocate_1M_fixed_pages(0x%zx, "
				                 "0x%x)\n",
			...
			if ((NULL == ptr) && (index > 0) && (ONE_M == PPG_vmem_pageSize[index])
				...
				LP_DEBUG_PRINTF3("\t omrallocate_1M_pageable_pages_above_bar(0x%zx, 0x%x) returned "
				                 "0x%zx\n",
				        numSegments, userExtendedPrivateAreaMemoryType, ptr);
				if (NULL == ptr) {
					if (TRUE == useStrictPageSize) {
			...
			LP_DEBUG_PRINTF4("\t reservePagesAboveBar invalid params: byteAmount = 0x%x, pageSize = 0x%x, "
			                 "pageFlags = 0x%x, userExtendedPrivateAreaMemoryType = 0x%x\n",
...
static void *
default_pageSize_reserve_memory(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount,
        struct J9PortVmemIdentifier *identifier, uintptr_t mode, OMRMemCategory *category)
{
	void *ptr = NULL;
	uintptr_t allocSize = 0;
	...
	if (OMR_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
...
#else /* if 0 */
		{
			...
			if (0 != rc) {
				/* As per z/OS C/C++ Run-Time Library Reference, __moservices() returns EINVAL or
				 * EMVSERR in case of failure. EINVAL indicates invalid argument. More detailed
				 * information is returned by __errno2(). EMVSERR indicates underlying IARV64 call
				 * failed.
				 */
				ptr = NULL;
				if (EINVAL == errno) {
		...
		/* malloc is not guaranteed to be 4k aligned.  So align to 4k.  Add an extra uintptr_t to store the
		 * original address, which can be used by free
		 */
		allocSize = GET_4K_ALIGNED_ALLOCATION_SIZE(byteAmount);
 ...
 */
static void *
reserve_memory_with_moservices(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        struct J9PortVmemParams *params, OMRMemCategory *category)
{
	void *ptr = NULL;
	/* Need to make byteAmount 1M aligned as __moservices()/IARV64 macro allocates memory in 1M chunks */
	...
	case ONE_M:
		if (OMR_ARE_ANY_BITS_SET(params->pageFlags, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE)) {
			...
			/* If pageable 1MB page frames are not available at first reference, pageable 4K page frames
			 * will be used. */
		} else {
	...
	case TWO_G:
		...
		break;
	default: Trc_PRT_vmem_reserve_memory_using_moservices_invalid_page_size(params->pageSize); rc = -1;
	}
        ...
        struct J9PortVmemParams *callerParams)
{
	 ...
	 *
	 *  Routines using IARV64 allocate memory above 2GB bar. Hence, they are applicaable for 64-bit z/OS systems
	 * only. Routines using STORAGE allocate memory below 2GB bar. Such routines can be used on both 31-bit and
	 * 64-bit z/OS system.s
	 */

	/* create a local copy of the parameters since we may change them and we don't want to change the caller's
	 * arguments */
	/* The "includeAlignmentInAllocation" helper creates a copy of the struct but also, until native alignment is
	 * implemented, adjusts the parameters to ensure that the requested alignment can be honoured within the
	 * allocated block.  Note that we still want to create a copy of the structure (because this function modifies
	 * the struct) if we implement native alignment and remove this helper.
	 */
	...
	LP_DEBUG_PRINTF4("\n omrvmem_reserve_memory_ex: address=0x%zx, byteAmount=0x%zx, pageSize=0x%zx, "
	                 "pageFlags=0x%zx\n",
	...
	if (use2To32GArea && (ZOS64_VMEM_ABOVE_BAR_GENERAL == PPG_userExtendedPrivateAreaMemoryType)) {
		 ...
		 *
		 * Having done the check here, we only need to check for the request for OMRPORT_VMEM_ZOS_USE2TO32G_AREA
		 * later on knowing that if they had asked for it and made it that far that the support is there
		 */

		LP_DEBUG_PRINTF("\n omrvmem_reserve_memory_ex: OMRPORT_VMEM_ZOS_USE2TO32G_AREA requested but not "
		                "supported\n");
 ...
 */
static void *
reservePages(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        struct J9PortVmemParams *params, OMRMemCategory *category)
{
	void *baseAddress = NULL;
	uintptr_t numPages = 0;
	BOOLEAN useStrictAddress = (OMRPORT_VMEM_STRICT_ADDRESS == (OMRPORT_VMEM_STRICT_ADDRESS & params->options));
	BOOLEAN use2To32GArea =
	        (OMRPORT_VMEM_ZOS_USE2TO32G_AREA == (OMRPORT_VMEM_ZOS_USE2TO32G_AREA & params->options));
	BOOLEAN useStrictPageSize =
	        (OMRPORT_VMEM_STRICT_PAGE_SIZE == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options));
	...
	if (FALSE == isLargePageSizeSupported(portLibrary, params->pageSize, params->pageFlags)) {
		...
		if (TRUE == use2To32GArea) {
			...
			if ((NULL != baseAddress) && (TRUE == isStrictAndOutOfRange(params, baseAddress))) {
				/* if strict flag is set and returned pointer is not within range then free the
				 * allocated memory and try again with 4K page size.
				 */
			...
			if ((NULL == baseAddress) && (FALSE == useStrictAddress)) {
				/* Allocating memory using large pages above bar is guaranteed not to return memory in
				 * low memory range. Therefore, this can be done only if strict address flag is not set.
				 */
				baseAddress = reservePagesAboveBar(portLibrary, identifier, params->mode,
				        params->byteAmount, params->pageSize, params->pageFlags, params->options,
				        category);
			}
		}
	} else {
 ...
 */
static void *
reserve4KPages(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        struct J9PortVmemParams *params, OMRMemCategory *category)
{
	void *baseAddress = NULL;
	BOOLEAN useStrictAddress = (OMRPORT_VMEM_STRICT_ADDRESS == (OMRPORT_VMEM_STRICT_ADDRESS & params->options));
	...
	if (!use2To32GArea) {
		...
		if (((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT) || (TRUE == useExecutablePages)) {
			/* Allocate executable memory below the 2G bit bar as ZOS does not properly support code loaded
			 * above this mark */
			/* Or if the desired address is in low memory, use the low 2G in the hopes that that's close to
			 * what's wanted */
...
getPageInfo(struct OMRPortLibrary *portLibrary)
{
	...
	LP_DEBUG_PRINTF1(
	        "\getUserExtendedPrivateAreaMemoryType returned : %i\n", PPG_userExtendedPrivateAreaMemoryType);
...
{
	uint32_t index = 1; /* Skip the first entry as it points to 4KB page size which is not considered large page
	                       size */
