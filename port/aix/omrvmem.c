...
omrvmem_startup(struct OMRPortLibrary *portLibrary)
{
	...
	if (num_psizes == -1) {
		...
		/* assert(num_psizes < OMRPORT_VMEM_PAGESIZE_COUNT); We don't want to do anything dangerous if this
		 * happens as it may, some day. We will use the earliest values in the set since they are the ones we
		 * should understand (if the OS adds new sizes at the beginning of the list there isn't much that we can
		 * do
		 */
        ...
        struct J9PortVmemIdentifier *identifier)
{
	...
	if (rangeIsValid(identifier, address, byteAmount)) {
		if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
			/* Assert that address and byteAmount are page aligned. These restrictions are not required for
			 * executable pages because when using default page size they are allocated using malloc() which
			 * may not return page aligned address and any attempt to align it to page boundary would waste
			 * one page of memory.
			 */
			...
			if (byteAmount > 0) {
				if ((OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY == identifier->allocator)
					...
					if ((FOUR_K == identifier->pageSize)
						...
						result = (intptr_t)disclaim64(
						        (void *)address, (size_t)byteAmount, flags);
					} else {
						/* noop: both 16M and 16G pages are pinned in memory so don't disclaim
						 * them */
						result = 0;
					}
...
int32_t
omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *userAddress, uintptr_t byteAmount,
        struct J9PortVmemIdentifier *identifier)
{
	int32_t result = 0;
...
{
	/* Under AIX 5.2, a 32-bit process can request up to 3.25 GB with each shmget call (essentially all of the
	 * available memory), and a 64-bit process can request at least 1 TB.  We will consider 1 TB to be an acceptable
	 * restriction, and not resort to the additional complexity of multiple shmget calls.
	 */
	...
	printf("\n\tomrvmem_reserve_memory_ex byteAmount: %p, startAddress: %p, endAddress: %p, pageSize: 0x%zX, %s, "
	       "%s, %s\n ",
	...
	if (0 == params->pageSize) {
		...
		if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
			memoryPointer = getMemoryInRangeForDefaultPages(portLibrary, identifier, category,
			        params->byteAmount, params->startAddress, params->endAddress, alignment,
		...
		/* If we were not called with the default page size, we must ensure that we were called with one of our
		 *understood sizes. AIX offers a total of 4 page sizes: 4 kB (default) 64 kB (5.3E or later on POWER5+)
		 ...
		 * check that PREFERRED_DEFAULT_PAGE_SIZE is in list as well - it will be used if original requested
		 * allocation is not succeed
		 */
		...
		if (requestedPageSizeIsSupported) {
			...
			memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category,
			        params->byteAmount, params->startAddress, params->endAddress, params->pageSize,
			        params->alignmentInBytes, params->options | OMRPORT_VMEM_STRICT_PAGE_SIZE,
			        params->mode);
			...
			if ((NULL == memoryPointer) && (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options))) {
				if (preferedDefaultPageSizeIsSupported && !preferedDefaultPageSizeIsRequested) {
					/*
					 * If requested page is not supported or memory can not be allocated with such
					 * page size repeat an allocation with PREFERRED_DEFAULT_PAGE_SIZE
					 */
					memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category,
				...
				if (NULL == memoryPointer) {
					...
					memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category,
					        params->byteAmount, params->startAddress, params->endAddress,
					        PPG_vmem_pageSize[0], params->alignmentInBytes, params->options,
					        params->mode);
				}
				...
				if (NULL == memoryPointer) {
					...
					fflush(stdout);
#endif
					memoryPointer = attemptToReserveInDefaultPages(portLibrary, identifier,
					        category, params->byteAmount, params->startAddress, params->endAddress,
					        PPG_vmem_pageSize[0], params->alignmentInBytes, params->options,
					        params->mode);
				}
			}
		...
		if (NULL == memoryPointer) {
			/* we failed to find the page size before hitting the null terminator so use the unsupported
			 * page size trace point and fail */
			update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
			if (!requestedPageSizeIsSupported) {
...
static void *
attemptToReserveInLargePages(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        OMRMemCategory *category, uintptr_t byteAmount, void *startAddress, void *endAddress, uintptr_t pageSize,
        uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
	void *memoryPointer = NULL;
	...
	if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
		memoryPointer = reserveLargePages(portLibrary, identifier, category, byteAmount, startAddress,
		        endAddress, pageSize, alignment, vmemOptions, mode);
	}
...
static void *
attemptToReserveInDefaultPages(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        OMRMemCategory *category, uintptr_t byteAmount, void *startAddress, void *endAddress, uintptr_t pageSize,
        uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
	void *memoryPointer = NULL;
	 ...
	 * are powers of two and, thus, their max is their lowest common multiple) */
	if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
		memoryPointer = getMemoryInRangeForDefaultPages(portLibrary, identifier, category, byteAmount,
		        startAddress, endAddress, alignment, vmemOptions, mode);
	}
...
reserveLargePages(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier, OMRMemCategory *category,
        uintptr_t byteAmount, void *startAddress, void *endAddress, uintptr_t pageSize, uintptr_t alignmentInBytes,
        uintptr_t vmemOptions, uintptr_t mode)
{
	void *memoryPointer = NULL;
	key_t addressKey;
...
#if defined(OMRVMEM_DEBUG)
	printf("\t\t omrvmem_reserve_memory_ex shmget(byteAmount 0x%zX, shmgetFlags = 0x%x) returning addressKey: "
	       "0x%zx\n",
	...
	if (-1 == addressKey) {
		...
		if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
			if (NULL != memoryPointer) {
				...
				if (FALSE
				        == detectAndRecordPageSize(portLibrary, identifier, addressKey, memoryPointer,
 ...
 */
static BOOLEAN
detectAndRecordPageSize(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier, key_t addressKey,
        void *memoryPointer, OMRMemCategory *category, uintptr_t pageSize, uintptr_t byteAmount, uintptr_t vmemOptions,
        uintptr_t mode)
{
	...
	update_vmemIdentifier(identifier, memoryPointer, (void *)(intptr_t)addressKey, byteAmount, mode,
	        (uintptr_t)page_info.pagesize, OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_SHM,
	        category);
	...
	if (0 != vmgetinfoResult) {
		...
		if (!vmgetinfoFailedFirstTime) {
			...
			portLibrary->tty_printf(
			        portLibrary, "Warning: not enough large pages to satisfy entire allocation\n");
		}
#endif
        ...
        struct J9PortVmemIdentifier *identifier, uintptr_t mode, uintptr_t pageSize, OMRMemCategory *category)
{
	...
	if (0 != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
		...
		update_vmemIdentifier(identifier, result, result, byteAmount, mode, pageSize,
		        OMRPORT_VMEM_PAGE_FLAG_NOT_USED, OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY, category);
		Trc_PRT_vmem_default_reserve_exit(result, address, byteAmount);
		return result;
	}
        ...
        struct OMRPortLibrary *portLibrary, uintptr_t mode, uintptr_t *pageSize, uintptr_t *pageFlags)
{
	if (NULL != pageSize) {
		if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE != (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
			/* AIX 5.3E (or later) on POWER5+ supports 4 kiB, 64 kiB, 16 MiB, 16 GiB page sizes (in that
			 * order). To be compatible with how the -Xlp option has historically worked, 16 MiB is the
			 * default page size. If, for some reason, that page size can't be found, we search for 64KB
			 * page size. If that is also not found, we will return the page size at [1] since failing is
			 * probably not a reasonable answer as it would break our support for that legacy option (and
			...
			while (0 != PPG_vmem_pageSize[index]) {
				...
				if (SIXTY_FOUR_K == PPG_vmem_pageSize[index]) {
					...
					/* We don't break here because preferred page size is 16M. So keep looking for
					 * 16M. */
				}
				index++;
			}
			*pageSize = preferredPageSize;
			 ...
			 * This is the same page size as used by JIT code cache when allocated using
        ...
        uintptr_t *pageFlags, BOOLEAN *isSizeSupported)
{
	...
	if (0 != validPageSize) {
		 ...
		 */
		if (OMRPORT_VMEM_MEMORY_MODE_EXECUTE == (OMRPORT_VMEM_MEMORY_MODE_EXECUTE & mode)) {
			if (portLibrary->sysinfo_get_env(portLibrary, "TR_ppcCodeCacheConsolidationEnabled", NULL, 0)
			        != -1) {
        ...
        uintptr_t alignmentInBytes, uintptr_t vmemOptions)
{
	...
	while ((startAddress <= currentAddress) && (endAddress >= currentAddress)) {
		...
		memoryPointer = allocateMemoryForLargePages(
		        portLibrary, identifier, currentAddress, addressKey, category, byteAmount);
	...
	allocAnywhere:
		memoryPointer =
		        allocateMemoryForLargePages(portLibrary, identifier, NULL, addressKey, category, byteAmount);
	}
        ...
        uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
#if !defined(OMR_ENV_DATA64)
	/**
	 * Using shmat to allocate vmem on AIX32 reduces the number of segments which can be used for thread stacks.
	 * Thus, explicitly use the mmap allocator for default-sized pages (large pages still must use shmat). The only
	 * reasons to use shmat, other than unification of the code paths, are to support large pages or NUMA.  NUMA is
	 * only supported on the Balanced policy which does not yet run on 32-bit and large page allocations come
	 * through a different path.
	 *
	 * See CMVC 178983 for more detail regarding this limitation.
	 */
	return getMemoryInRangeForDefaultPagesUsingMmap(portLibrary, identifier, category, byteAmount, startAddress,
	        endAddress, alignmentInBytes, vmemOptions, mode);
#else /* !defined(OMR_ENV_DATA64) */
	/*
	 * Balanced GC requires that omrvmem_numa_set_affinity() can be used on memory returned by
	 * omrvmem_reserve_memory(). This is not supported for memory returned by default_pageSize_reserve_memory(), as
	 * it uses the mmap() API for historical reasons.
	 *
	 * To support omrvmem_numa_set_affinity() under balanced, we redefine getMemoryInRangeForDefaultPages() to
	 * simply call reserveLargePages(), which uses the shmat()/shmget() APIs which do not have the same problem.
	 *
	 * Note that there is a limitation in older versions of AIX (pre-6.1) where shmem may be unable to allocate as
	 * much memory as mmap (up to one segment is lost - 256M).  However, the NUMA API is not supported prior to
	 * AIX 6.1 so these requirements never appear together.  Thus, if NUMA is supported on the system, use shmem.
	 * Otherwise, use mmap (as we will not be trying to use the NUMA API). See CMVC 178946 for more detail regarding
	 * this limitation
	 *
	 * Also note, do not use shmem for OMRPORT_VMEM_MEMORY_MODE_EXECUTE memory (JIT codecache) as it causes the
	 * following problems:
	 ...
	 * 			- Calls from one codeCache to another will need trampolines to jump more than 32MB of
	 * distance
	 * 		- puts pressure on hardware facility of virtual memory translations,
	 * 			- i.e. SLB(segment lookaside buffer), TLB(table lookaside buffer), and ERAT(effective
	 * address to real address table). In particular, each 256MB requires an SLB entry.
	 *
	 */
	if (__ENHANCED_AFFINITY()
		...
		return getMemoryInRangeForDefaultPagesUsingMmap(portLibrary, identifier, category, byteAmount,
		        startAddress, endAddress, alignmentInBytes, vmemOptions, mode);
	}
#endif /* !defined(OMR_ENV_DATA64) */
}

static void *
getMemoryInRangeForDefaultPagesUsingMmap(struct OMRPortLibrary *portLibrary, struct J9PortVmemIdentifier *identifier,
        OMRMemCategory *category, uintptr_t byteAmount, void *startAddress, void *endAddress,
        uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
	intptr_t direction = 1;
        ...
        struct J9PortVmemIdentifier *identifier)
{
	...
	if (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
		...
		/* don't forget to subtract one from the numaNode to shift it into the 0-indexed numbering scheme used
		 * by the system */
		...
		/* attach the specified subrange to the SRAD (allow the memory to be asynchronously migrated to the
		 * specified SRAD) */
		result = PTR_ra_attach(
		        R_SUBRANGE, thisSubrangeResourceID, R_SRADID, targetSRADResourceID, R_MIGRATE_ASYNC);
	}
#endif /* defined(OMR_ENV_DATA64) */
...
void *
omrvmem_get_contiguous_region_memory(struct OMRPortLibrary *portLibrary, void *addresses[], uintptr_t addressesCount,
