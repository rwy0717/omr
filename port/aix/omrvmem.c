...
void omrvmem_shutdown(struct OMRPortLibrary* portLibrary) { PPG_numa_platform_supports_numa = 0; }
...
int32_t omrvmem_startup(struct OMRPortLibrary* portLibrary)
{
    ...
    if (num_psizes == -1) {
        ...
        /* assert(num_psizes < OMRPORT_VMEM_PAGESIZE_COUNT); We don't want to do anything dangerous if this happens as
         * it may, some day. We will use the earliest values in the set since they are the ones we should understand (if
         * the OS adds new sizes at the beginning of the list there isn't much that we can do
         */
    ...
    struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
    ...
    if (rangeIsValid(identifier, address, byteAmount)) {
        if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
            /* Assert that address and byteAmount are page aligned. These restrictions are not required for executable
             * pages because when using default page size they are allocated using malloc() which may not return page
             * aligned address and any attempt to align it to page boundary would waste one page of memory.
             */
...
int32_t omrvmem_free_memory(struct OMRPortLibrary* portLibrary, void* userAddress, uintptr_t byteAmount,
    ...
    struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
    ...
    printf("\n\tomrvmem_reserve_memory_ex byteAmount: %p, startAddress: %p, endAddress: %p, pageSize: 0x%zX, %s, %s, "
           "%s\n ",
    ...
    if (0 == params->pageSize) {
         ...
         *understood sizes. AIX offers a total of 4 page sizes: 4 kB (default) 64 kB (5.3E or later on POWER5+) 16 MB
         *(legacy large page) 16 GB (very large page in 5.3E or later on POWER5+)
         */
        /* make sure that this is valid page size for this configuration
         * check that PREFERRED_DEFAULT_PAGE_SIZE is in list as well - it will be used if original requested allocation
         * is not succeed
         */
        ...
        if (requestedPageSizeIsSupported) {
            ...
            if ((NULL == memoryPointer) && (0 == (OMRPORT_VMEM_STRICT_PAGE_SIZE & params->options))) {
                ...
                if (NULL == memoryPointer) {
                    ...
                    memoryPointer = attemptToReserveInLargePages(portLibrary, identifier, category, params->byteAmount,
                        params->startAddress, params->endAddress, PPG_vmem_pageSize[0], params->alignmentInBytes,
                ...
                if (NULL == memoryPointer) {
                    ...
                    fflush(stdout);
#endif
                    memoryPointer = attemptToReserveInDefaultPages(portLibrary, identifier, category,
                        params->byteAmount, params->startAddress, params->endAddress, PPG_vmem_pageSize[0],
        ...
        if (NULL == memoryPointer) {
            /* we failed to find the page size before hitting the null terminator so use the unsupported page size trace
             * point and fail */
            update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
            if (!requestedPageSizeIsSupported) {
    ...
    uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
    ...
    if ((0 == minimumGranule) || (0 == (alignment % minimumGranule))) {
        memoryPointer = reserveLargePages(portLibrary, identifier, category, byteAmount, startAddress, endAddress,
    ...
    if (-1 == addressKey) {
        ...
        if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
            if (NULL != memoryPointer) {
                ...
                if (FALSE
                    == detectAndRecordPageSize(portLibrary, identifier, addressKey, memoryPointer, category, pageSize,
                           byteAmount, vmemOptions, mode)) {
    ...
    uintptr_t vmemOptions, uintptr_t mode)
{
    ...
    update_vmemIdentifier(identifier, memoryPointer, (void*)addressKey, byteAmount, mode, (uintptr_t)page_info.pagesize,
...
uintptr_t* omrvmem_supported_page_sizes(struct OMRPortLibrary* portLibrary) { return PPG_vmem_pageSize; }

uintptr_t* omrvmem_supported_page_flags(struct OMRPortLibrary* portLibrary) { return PPG_vmem_pageFlags; }
    ...
    void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
     ...
     * use shmat, other than unification of the code paths, are to support large pages or NUMA.  NUMA is only supported
     * on the Balanced policy which does not yet run on 32-bit and large page allocations come through a different path.
     *
     ...
     * historical reasons.
     *
     * To support omrvmem_numa_set_affinity() under balanced, we redefine getMemoryInRangeForDefaultPages() to simply
     * call reserveLargePages(), which uses the shmat()/shmget() APIs which do not have the same problem.
     *
    ...
    uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
    ...
    if (OMR_ARE_NO_BITS_SET(identifier->mode, OMRPORT_VMEM_NO_AFFINITY)) {
        ...
        /* attach the specified subrange to the SRAD (allow the memory to be asynchronously migrated to the specified
         * SRAD) */
