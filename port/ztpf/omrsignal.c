...
omrsig_protect(struct OMRPortLibrary *portLibrary, omrsig_protected_fn fn, void *fn_arg, omrsig_handler_fn handler,
        void *handler_arg, uint32_t flags, UDATA *result)
{
        ...
        struct OMRPortLibrary *portLibrary, omrsig_handler_fn handler, void *handler_arg, uint32_t flags)
{
	...
	while (NULL != cursor) {
		if ((cursor->portLib == portLibrary) && (cursor->handler == handler)
			...
			if (0 == flags) { /* Remove the listener, but not masterHandlers, which get removed at omrsignal
				             shutdown */
...
masterSynchSignalHandler(int signal, siginfo_t *sigInfo, void *contextInfo, UDATA breakingEventAddr)
{
	...
	if (NULL != thisThread) {
		...
		/* record this signal in tls so that jsig_handler can be called if any of the handlers decide we should
		 * be shutting down */
...
masterASynchSignalHandler(int signal, siginfo_t *sigInfo, void *contextInfo, UDATA nullArg)
{
	...
	case OMRPORT_SIG_FLAG_SIGQUIT: sem_post(&sigQuitPendingSem); break;
	case OMRPORT_SIG_FLAG_SIGABRT: sem_post(&sigAbrtPendingSem); break;
	case OMRPORT_SIG_FLAG_SIGTERM: sem_post(&sigTermPendingSem); break;
	case OMRPORT_SIG_FLAG_SIGRECONFIG: sem_post(&sigReconfigPendingSem); break;
	case OMRPORT_SIG_FLAG_SIGXFSZ: sem_post(&sigXfszPendingSem); break;
	}
#if 0
...
registerSignalHandlerWithOS(OMRPortLibrary *portLibrary, uint32_t portLibrarySignalNo, unix_sigaction handler)
{
	 ...
	 *		has been introduced in masterSynchSignalHandler() to obtain BEA on zLinux but not for other
	 *platforms. As a result, the total number of parameters in masterSynchSignalHandler() on zLinux & z/TPF is 4
	 *while the number is 3 on other platforms, because there is no need to change the signature of
	 *		masterSynchSignalHandler in there. Since the code is shared on all platforms, the change here is
	 *used to split them up to avoid any compiling error.
	 */
