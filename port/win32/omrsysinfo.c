...
intptr_t omrsysinfo_get_env(struct OMRPortLibrary* portLibrary, const char* envVar, char* infoString, uintptr_t bufSize)
{
    ...
    if (0 == rc) {
        ...
        if (0 == rc) { /* error, probably bogus Unicode */
            ...
            rc = WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, envVarWideCharValue, -1, infoString,
    ...
    */
    ...
    if (NULL == PPG_si_osType) {
        ...
        if (defaultTypeName == PPG_si_osType) {
            ...
            if (ERROR_SUCCESS
                == RegOpenKeyExA(
                       HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_READ, &hKey)) {
                DWORD nameSize = 0;
                ...
                if (ERROR_SUCCESS == RegQueryValueExA(hKey, "ProductName", NULL, NULL, NULL, &nameSize)) {
                    char* productNameBuffer = portLibrary->mem_allocate_memory(
                        portLibrary, nameSize, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
                    if (NULL != productNameBuffer) {
                        /* query the second time to get the value */
                        if (ERROR_SUCCESS
                            == RegQueryValueExA(
                                   hKey, "ProductName", NULL, NULL, (LPBYTE)productNameBuffer, &nameSize)) {
 ...
 * @return the PID.
 */
uintptr_t omrsysinfo_get_pid(struct OMRPortLibrary* portLibrary) { return GetCurrentProcessId(); }
...
int32_t omrsysinfo_get_processor_info(struct OMRPortLibrary* portLibrary, struct J9ProcessorInfos* procInfo)
{
    ...
    do {
        ...
        rc = AddCounter(
            portLibrary, statsHandle, procInfo, PROCESSOR_PRIVILEGED_TIME_COUNTER_NAME, &privilegedTimeCounter);
        if (0 != rc) {
            rc = OMRPORT_ERROR_SYSINFO_ERROR_READING_PROCESSOR_INFO;
    ...
    if (mutf8Size >= 0) {
        ...
        if (bufLen < (uintptr_t)mutf8Size) {
            ...
            mutf8Size = portLibrary->str_convert(portLibrary, J9STR_CODE_WIDE, J9STR_CODE_MUTF8,
                (const char*)unicodeBuffer, (uintptr_t)pathLengthInWChars * sizeof(wchar_t), buf, bufLen);
            if (mutf8Size < 0) {
