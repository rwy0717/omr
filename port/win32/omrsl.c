...
omrsl_open_shared_library(struct OMRPortLibrary *portLibrary, char *name, uintptr_t *descriptor, uintptr_t flags)
{
		...
		if (NULL == dllHandle) {
			...
			if (useNlsMessage || platformErrorFailed) {
				errorMessage = portLibrary->nls_lookup_message(portLibrary,
				        J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_EXECUTABLE_OPEN_ERROR,
				        NULL);
	 ...
	 * to process the remaining paths and exits immediately. To fix up the issue, ERROR_BAD_PATHNAME should be
	 * covered in the error code list below so as to proceed with the searching.
	 */
	...
	if (notFound) {
		...
		dllHandle =
		        LoadLibraryExW(unicodeName, NULL, DONT_RESOLVE_DLL_REFERENCES | LOAD_WITH_ALTERED_SEARCH_PATH);
	...
	if (0 != sizeof(errBuf)) {
		...
		if (NULL == unicodeName) {
			errorMessage = portLibrary->nls_lookup_message(portLibrary,
			        J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_SL_INTERNAL_ERROR, NULL);
			portLibrary->str_printf(portLibrary, errBuf, sizeof(errBuf), errorMessage, error);
			errBuf[sizeof(errBuf) - 1] = '\0';
			SetErrorMode(prevMode);
        ...
        struct OMRPortLibrary *portLibrary, uintptr_t descriptor, char *name, uintptr_t *func, const char *argSignature)
{
	...
	if (0 != result) {
		...
		/* + 7 comes from: 256 parameters * 8 bytes each = 2048 bytes -> 4 digits + @ symbol + leading '_' +
		 * NULL */
...
findError(int32_t errorCode)
{
	...
	case ERROR_MOD_NOT_FOUND: return OMRPORT_SL_INVALID;
	default: return OMRPORT_SL_UNKNOWN;
	}
}
