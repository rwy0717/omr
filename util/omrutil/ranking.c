...
static uintptr_t hashFn(void* key, void* userData) { return (uintptr_t)((hashTableEntry*)key)->data; }
...
OMRRanking* rankingNew(OMRPortLibrary* portLibrary, uint32_t size)
{
    ...
    newRanking->hashTable
        = hashTableNew(portLibrary, OMR_GET_CALLSITE(), size * HASHTABLE_RATIO, sizeof(hashTableEntry), 0,
            J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION, OMRMEM_CATEGORY_VM, hashFn, hashEqualFn, NULL, NULL);
