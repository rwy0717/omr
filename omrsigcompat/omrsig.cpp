...
int omrsig_signalOS_internal(int signum, const struct sigaction* act, struct sigaction* oldact)
{
    ...
    if (NULL == signalOS) {
        ...
        sighandler_t handler = SIG_DFL;
        /* signal() on WIN only takes SIG_IGN/SIG_DFL/function pointer as the second parameter (NULL is mapped to
         * SIG_DFL). If only querying the existing sighandler_t, use SIG_DFL to get the existing sighandler_t and then
         * set it back.
         */
...
__sighandler_t __sysv_signal(int sig, __sighandler_t handler) __THROW { return omrsig_signal_internal(sig, handler); }

sighandler_t ssignal(int sig, sighandler_t handler) __THROW { return omrsig_signal_internal(sig, handler); }
            ...
            for (unsigned int i = 0; i < NSIG; i += 1) {
                if (validSignalNum(i, false)) {
                    ...
                    while (((sigData[i].secondaryAction.sa_handler != oldsets[oldctIndex].__sa_handler)
                               || !checkMaskEquality(
                                      i, &sigData[i].secondaryAction.sa_mask, &oldsets[oldctIndex].__sa_mask)
...
sighandler_t sysv_signal(int signum, sighandler_t handler) __THROW { return omrsig_signal_internal(signum, handler); }
