    ...
    OMRSysInfoProcessCpuTime const* endRecord, OMRSysInfoProcessCpuTime const* startRecord, double* cpuLoad)
{
    ...
    if (timestampDelta < 0) {
        ...
        if (processTimeDelta < systemCpuTimeDelta) {
            /* operation processTimeDelta = systemCpuTimeDelta + userCpuTimeDelta overflow, set cpuLoad to 1 in this
             * case */
            *cpuLoad = 1.0;
        } else {
    ...
    if (NULL == vmThread) {
        ...
        if ((SUPPORTED == curSysInfo->systemCpuLoadCallStatus) || (NO_HISTORY == curSysInfo->systemCpuLoadCallStatus)) {
            ...
            if (OMR_ERROR_NONE != ret) {
                ...
                if (OK == rc) {
                    /* discard the oldestSystemCpuTime, replace it with interimSystemCpuTime and save
                     * newestSystemCpuTime as the new interimSystemCpuTime. */
                    memcpy(
                        &curSysInfo->oldestSystemCpuTime, &curSysInfo->interimSystemCpuTime, sizeof(J9SysinfoCPUTime));
                    memcpy(&curSysInfo->interimSystemCpuTime, &newestSystemCpuTime, sizeof(J9SysinfoCPUTime));
                } else {
                    /* interimSystemCpuTime or newestSystemCpuTime is not valid, or the elapsed time between them is too
                     * small. */
                    ...
                    if (OK == rc) {
                        ...
                        /* return OMR_ERROR_RETRY if interval between endRcord and oldestSystemCpuTime is still too
                         * small */
                        curSysInfo->systemCpuTimeNegativeElapsedTimeCount = 0;
                        ret = OMR_ERROR_RETRY;
                    } else if (ELAPSED_TIME_NEGATIVE == rc) {
                        if (MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT == curSysInfo->systemCpuTimeNegativeElapsedTimeCount) {
                            /* if this case found consecutively for MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT times, discard
                             * the oldestSystemCpuTime and replace it with newestSystemCpuTime. */
                            memcpy(&curSysInfo->oldestSystemCpuTime, &newestSystemCpuTime, sizeof(J9SysinfoCPUTime));
                            curSysInfo->systemCpuTimeNegativeElapsedTimeCount = 0;
                        } else {
    ...
    if (NULL == vmThread) {
        ...
        if ((SUPPORTED == curSysInfo->processCpuLoadCallStatus)
            ...
            if (OMR_ERROR_NONE != ret) {
                ...
                if (OK == rc) {
                    /* discard the oldestProcessCpuTime and replace it with interimProcessCpuTime, save
                     * newestProcessCpuTime as the new interimProcessCpuTime. */
                    memcpy(&curSysInfo->oldestProcessCpuTime, &curSysInfo->interimProcessCpuTime,
                        sizeof(OMRSysInfoProcessCpuTime));
                    memcpy(&curSysInfo->interimProcessCpuTime, &newestProcessCpuTime, sizeof(OMRSysInfoProcessCpuTime));
                } else {
                    /* interimProcessCpuTime or newestProcessCpuTime is not valid, or the elapsed time between them is
                     * too small. */
                    if (NEGATIVE_INTERVAL == rc) {
                    ...
                    rc = calculateProcessCpuLoad(
                        &newestProcessCpuTime, &curSysInfo->oldestProcessCpuTime, processCpuLoad);
                    if (OK == rc) {
                        ...
                        memcpy(
                            &curSysInfo->oldestProcessCpuTime, &newestProcessCpuTime, sizeof(OMRSysInfoProcessCpuTime));
                        curSysInfo->processCpuTimeNegativeElapsedTimeCount = 0;
                        ret = OMR_ERROR_INTERNAL;
                    } else if (ELAPSED_TIME_TOO_SMALL == rc) {
                        /* return OMR_ERROR_RETRY if interval between newestProcessCpuTime and oldestProcessCpuTime is
                         * still too small */
                        curSysInfo->processCpuTimeNegativeElapsedTimeCount = 0;
                        ret = OMR_ERROR_RETRY;
                    } else if (ELAPSED_TIME_NEGATIVE == rc) {
                        if (MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT == curSysInfo->processCpuTimeNegativeElapsedTimeCount) {
                            /* if this case found consecutively for MAXIMUM_NEGATIVE_ELAPSED_TIME_COUNT times, discard
                             * the oldestProcessCpuTime and replace it with newestProcessCpuTime. */
    ...
    if (NULL == vmThread) {
        ...
        if (categories_buffer) {
            ...
            userData.categories_mapping_size
                = ((uintptr_t)walkState.userData1) + 1 + ((uintptr_t)walkState.userData2) + 1;
