 ...
 */
void HookGen::writeComment(FILE* file, char* text) { fprintf(file, "/* %s */\n", text); }
 ...
 */
void HookGen::writeEventToPrivateHeader(const char* name, const char* condition, const char* once, int sampling,
    const char* structName, pugi::xml_node event)
{
    ...
    if (0 == sampling) {
        fprintf(_privateFile,
            "\t\t(*J9_HOOK_INTERFACE(hookInterface))->J9HookDispatch(J9_HOOK_INTERFACE(hookInterface), %s%s, "
            "&eventData); \\\n",
