...
J9TDFFile* TDFParser::parse()
{
    ...
    while (_fileReader->hasNext()) {
        ...
        if (0 == strlen(line) || StringUtils::startsWithUpperLower(line, "//")
            ...
            if (0
                != processTracePointDetail(line, tdf->lasttp, tdf->header.executable, id++, _fileReader->_fileName,
        ...
        size_t len = 5 /* \\\"\\\"\0 when no parameters specified */
            + (percentSignCount
                  * (strlen(TRACE_DATA_TYPE_STRING)
                        + strlen(TRACE_DATA_TYPE_PRECISION))) /* TRACE_DATA_TYPE_STRING is the longest parameter string
                                                                 + precision specifier*/
            ;
...
bool TDFParser::isNoEnv(const char* line) { return NULL != StringUtils::containsUpperLower((char*)line, " NoEnv "); }
...
bool TDFParser::isTest(const char* line) { return NULL != StringUtils::containsUpperLower((char*)line, " Test "); }
        ...
        while ((++pos < (formatTemplate + templateLength)) && (0 != (stateBits[state] & allowFromStates[0]))
                    ...
                    if (formatTemplate + templateLength > pos + 1) {
                        ...
                        case 'u':
                            if (_treatWarningAsError) {
                                eprintf("ERROR : %%l is not valid modifier. (parameter %u in mutated format string "
                                        "%s). %s:%u",
                                    parmCount, formatTemplate, fileName, lineNumber);
                                return RC_FAILED;
                            } else {
                                eprintf("WARNING : %%l is not valid modifier. (parameter %u in mutated format string "
                                        "%s). %s:%u",
                                    parmCount, formatTemplate, fileName, lineNumber);
                            }
                        }
            ...
            case 's':
                ...
                case 'u':
                    ...
                    default:
                        flagTdfSyntaxError(
                            "Invalid length modifier and type", parmCount, formatTemplate, fileName, lineNumber);
                        rc = RC_FAILED;
                    }
                    break;
