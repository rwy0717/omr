...
static RCType help(int argc, char* argv[])
{
    for (int i = 0; i < argc; i++) {
        if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") || 0 == strcmp(argv[i], "-h")
            ...
            printf("%s [-threshold num] [-w2cd] [-generateCfiles] [-treatWarningAsError] [-root rootDir] [-file "
                   "file.tdf] [-force]\n",
                argv[0]);
            printf("\t-threshold Ignore trace level below this threshold (default 1)\n");
            printf("\t-w2cd Write generated .C and .H files to current directory (default: generate in the same "
                   "directory as the TDF file)\n");
...
RCType startTraceGen(int argc, char* argv[])
{
    ...
    while (NULL != dir) {
        ...
        if (RC_OK
            != FileUtils::visitDirectory(&options, dir->path, TDF_FILE_EXT, &tracegen, TraceGen::generateCallBack)) {
