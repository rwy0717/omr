...
const char *TP_ASSERT_TEMPLATE = "#if UT_TRACE_OVERHEAD >= %u\n"
                                 ...
                                 "				%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, "
                                 "(UT_SPECIAL_ASSERTION | (%uu << 8) | %s_UtActive[%u]), \"\\377\\4\\377\", __FILE__, "
                                 "__LINE__, UT_STR((%s))); \\\n"
                                 "				Trace_Unreachable(); \\\n"
                                 "			} else { \\\n"
                                 "				fprintf(stderr, \"** ASSERTION FAILED ** %s.%u at "
...
const char *TP_AUX_TEMPLATE = "#if UT_TRACE_OVERHEAD >= %u\n"
                              ...
                              "		%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, ((%uu << 8)), %s%s);	} "
                              "while(0)\n"
                              "#else\n"
                              "%s" /* Place holder for option test macro (specified by "Test" option in tp spec) */
	...
	while (NULL != tp) {
		if (!tp->obsolete) {
			if (UT_ASSERT_TYPE == tp->type) {
				tpAssert(fd, tp->overhead, tp->test, tp->name, tdf->header.executable, id, tp->hasEnv,
				        tp->format, tp->parmCount);
			} else {
				tpTemplate(fd, tp->overhead, tp->test, tp->name, tdf->header.executable, id, tp->hasEnv,
				        tp->parameters, tp->parmCount, tdf->header.auxiliary);
			}
		}
		id++;
		tp = tp->nexttp;
 ...
 */
RCType
TraceHeaderWriter::tpTemplate(FILE *fd, unsigned int overhead, unsigned int test, const char *name, const char *module,
        unsigned int id, unsigned int envParam, const char *parameters, unsigned int parmCount, unsigned int auxiliary)
{
	RCType rc = RC_FAILED;
	...
	if (test) {
		...
		testMacro =
		        (char *)Port::omrmem_calloc(1, (strlen(testMacroTemplate) + strlen(name) + strlen(module) + 8));
		if (NULL == testMacro) {
			eprintf("Failed to allocate memory");
			goto failed;
		}
	...
	if (auxiliary) {
		...
		if (0 <= fprintf(fd, TP_TEMPLATE, overhead, testMacro, name, envParam ? "thr" : "",
		            ...
		            envParam ? UT_ENV_PARAM : UT_NOENV_PARAM, module, id, module, id, parameters, parmString,
		            testNop, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma,
		            module, id)) {
			rc = RC_OK;
		} else {
			rc = RC_FAILED;
...
RCType
TraceHeaderWriter::tpAssert(FILE *fd, unsigned int overhead, unsigned int test, const char *name, const char *module,
        unsigned int id, unsigned int envParam, const char *conditionStr, unsigned int parmCount)
{
	/* 5 characters allows "P999, " or nearly 1000 parameters. */
	RCType rc = RC_FAILED;
	char *parmString = NULL;
	...
	if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name, envParam ? "thr" : parmStringNoLeadingComma,
	            module, id, module, id, conditionStr, module, module, envParam ? "thr" : "(void *)NULL", module, id,
	            module, id, conditionStr, module, id, name, conditionStr, testnop, name,
...
TraceHeaderWriter::headerTemplate(J9TDFOptions *options, FILE *fd, const char *moduleName)
{
	...
	if (0 <= fprintf(fd, UT_H_FILE_HEADER_TEMPLATE, ucModule, ucModule, options->threshold, moduleName, moduleName,
	            moduleName, moduleName, ucModule, moduleName, ucModule, moduleName, moduleName, moduleName,
	            moduleName, moduleName, ucModule, moduleName, ucModule, moduleName)) {
		rc = RC_OK;
	} else {
		rc = RC_FAILED;
		goto failed;
