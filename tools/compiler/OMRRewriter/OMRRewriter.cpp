...
class OMRThisRewritingVisitor : public RecursiveASTVisitor<OMRThisRewritingVisitor> {
	 ...
	 */
	bool isSelfCall(CXXMemberCallExpr *call) { return ThisChecker->isSelfCall(call); }

	/**
	 * Delegated isExtensibleMemberCall to OMRThisCheckingVisitor
	 */
	bool isExtensibleMemberCall(CXXMemberCallExpr *call) { return ThisChecker->isExtensibleMemberCall(call); }

	/**
	 * Delegated isStaticExtensibleMemberCall to OMRThisCheckingVisitor
	 */
	bool isStaticExtensibleMemberCall(CallExpr *call) { return ThisChecker->isStaticExtensibleMemberCall(call); }
...
public:
	explicit OMRThisRewritingVisitor(
	        ASTContext *C, OMRThisCheckingVisitor *TC, OMRClassCheckingVisitor *CC, Rewriter &R)
	        : ThisChecker(TC), TheRewriter(R), ClassChecker(CC), SrcMgr(R.getSourceMgr()), Context(C)
	{}
	...
	bool VisitCallExpr(CallExpr *call)
	{
		...
		if ((referencedFunc = dyn_cast<DeclRefExpr>(call->getCallee()->IgnoreImplicit()))
			...
			auto calleeDecl = call->getCalleeDecl(); // needed to avoid potential segfault with dyn_cast and
			                                         // g++
			if (!calleeDecl || !(methodDecl = dyn_cast<CXXMethodDecl>(calleeDecl))) {
				return false;
			}
			/*DiagnosticsEngine &diagEngine = Context->getDiagnostics();
			unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "Static member function
			must be called with scope qualifier. Suggested fix:"); DiagnosticBuilder builder =
			diagEngine.Report(call->getExprLoc(), diagID); std::string qualifierHint = "TR::" +
	...
	void RewriteFiles()
	{
		for (std::map<FileID, int>::iterator files = replacements.begin(); files != replacements.end();
		        files++) {
			std::string filename = std::string(SrcMgr.getFileEntryForID(files->first)->getName())
			        + ".OMRRewritten";
