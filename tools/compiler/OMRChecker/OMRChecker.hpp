...
private:
    ...
    bool isExtensibleClassHierarchyCorrect(const CXXRecordDecl* decl)
    {
        ...
        if (decl->hasDefinition()) {
            for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(), BE = decl->bases_end(); BI != BE;
                 ++BI) {
        ...
        if (passNumber[decl] != 0) {
            if (Extensible.isExtensible(decl)) {
                ...
                if (pass != passNumber[decl]) {
                    std::string diagnostic("Extensible classes heirarchy was incorrect, multiple extensible classes "
                                           "deriving from non-extensible class.");
    ...
    void VerifyTypeStructure()
    {
        ...
        for (std::map<CXXRecordDecl*, bool>::iterator I = Types.begin(), E = Types.end(); I != E; ++I) {
            ...
            if (extensible) { // Extnsible type .
                ...
                for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(), BE = Type->bases_end(); BI != BE;
                     ++BI) {
                    CXXRecordDecl* base_class = BI->getType()->getAsCXXRecordDecl();
                    if (base_class && !isExtensible(base_class) // Ensure extensible parent.
                        ...
                        // Base is not extensible, but an extensible type reaches it, with no concrete class in the
                        // middle. Issue diagnostic.
            ...
            if (isSelfCall(call)) {
                ...
                if (callerAndCalleeHaveSameMDT(call)) {
                    ...
                    unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
                        "self call is resolving outside the current extensible class. You probably didn't define "
                        "self() in all extensible OMR layers.");
            ...
            if (thisExpr) {
                ...
                if (thisConcrete) {
                    ...
                    if (targetClass == NULL || targetClass != thisConcrete) {
                        DiagnosticsEngine& diagEngine = Context->getDiagnostics();
                        unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
                            "Static casts of this in extensible classes need to be the most derived type. The "
                            "preferred way of handling this is actually a call to self()");
