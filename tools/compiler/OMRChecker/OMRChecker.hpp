...
private:
	/**
	 * Iterates through the class hierarchy and calls isExtensibleClassHierarchyCorrect(const CXXRecordDecl*, bool,
	 * int) for all extensible classes found.
	 */
	bool isExtensibleClassHierarchyCorrect(const CXXRecordDecl *decl)
	{
		...
		if (decl->hasDefinition()) {
			for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(), BE = decl->bases_end();
			        BI != BE; ++BI) {
		...
		if (passNumber[decl] != 0) {
			if (Extensible.isExtensible(decl)) {
				...
				if (pass != passNumber[decl]) {
					std::string diagnostic("Extensible classes heirarchy was incorrect, multiple "
					                       "extensible classes deriving from non-extensible "
					                       "class.");
		...
		for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(), BE = decl->bases_end();
			...
			if (extensiblePhase) {
				...
				if (!Extensible.isExtensible(decl)) {
					...
					std::string diagnostic("Extensible classes heirarchy was incorrect, multiple "
					                       "extensible class chains in heirarchy.");
					DiagnosticsEngine &diagEngine = Context->getDiagnostics();
					unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "%0");
					diagEngine.Report(decl->getLocation(), diagID) << diagnostic;
					return false;
...
public:
			...
			if (extensible) { // Extnsible type .
				...
				for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(),
					...
					if (base_class && !isExtensible(base_class) // Ensure extensible parent.
					        && !isOMRRootType(Type)) { // OMR Root type can have non-extensible
						                           // parents.
						// Base is not extensible, but an extensible type reaches it, with no
						// concrete class in the middle. Issue diagnostic.
	 ...
	 */
	bool isOMRRootType(const CXXRecordDecl *decl) { return checkDeclForNamespace(decl, std::string("OMR")); }
	...
	const CXXRecordDecl *getAssociatedConcreteType(const CXXRecordDecl *decl)
	{
		...
		if (isExtensible(decl)) {
			...
			trace("Didn't find a concrete class for " << decl->getQualifiedNameAsString()
			                                          << " ... issue diagnostic?")
		}
		return NULL;
	}

	bool isExtensible(const CXXRecordDecl *decl) { return Extensible.isExtensible(decl); }
...
private:
		...
		for (auto BI = derivedType->bases_begin(), BE = derivedType->bases_end(); BI != BE; ++BI) {
			...
			if (base_class) {
				if (isOMRConcreteType(base_class)) { // Concrete parent terminates search.
					trace(prefix << "not searching " << base_class->getQualifiedNameAsString()
					             << ", is concrete");
				} else {
	...
	void BuildMostDerivedTypeMap(CXXRecordDecl *decl)
	{
		...
		for (CXXRecordDecl::base_class_iterator BI = decl->bases_begin(), BE = decl->bases_end(); BI != BE;
		        ++BI) {
		...
		while (!toProcess.empty()) {
			...
			if (toUpdate != toUpdate->getCanonicalDecl())
				trace("Missed cannonicalization")
				        ...
				        if (MostDerivedType.find(toUpdate) != MostDerivedType.end() // Found a most
				                                                                    // derived type.
...
public:
	...
	bool VisitCallExpr(CallExpr *call)
	{
		...
		if ((referencedFunc = dyn_cast<DeclRefExpr>(call->getCallee()->IgnoreImplicit()))
			...
			// get the static function's parent class. This cast should be guarenteed to work but let's play
			// it safe.
			...
			unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
			        "Static member function must be called with scope qualifier. Suggested fix:");
		...
		if (receiver->isImplicitCXXThis()) {
			...
			CXXRecordDecl *targetClass = cast->getType()->getAs<PointerType>()
			        ? cast->getType()
			                  ...
			                  ->getCanonicalDecl()
			        : NULL;
			trace("targetClass of static cast"
			...
			if (thisExpr) {
				DiagnosticsEngine &diagEngine = Context->getDiagnostics();
				unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
				        "Use of this in an extensible class can lead to unexpected behaviour");
				DiagnosticBuilder builder = diagEngine.Report(receiver->getExprLoc(), diagID);
				std::string staticCastHint = "self()->";
				builder.AddFixItHint(
	...
	const CXXMethodDecl *getCallerDecl(const clang::Stmt *expr) { return lastSeenMethodDecl; }
	...
	bool computeCallInformation(const CXXMemberCallExpr *call, const CXXRecordDecl *&calleeMostDerived,
...
private:
	bool isExtensible(const CXXRecordDecl *decl) { return ClassChecker->isExtensible(decl); }
